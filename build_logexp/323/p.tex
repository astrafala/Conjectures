...
\title{A proof of the conjectured recurrence for OEIS A081920}
...
\begin{abstract}
OEIS A081920 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A081920 is ``Expansion of exp(2x)/sqrt(1-x\^{}2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 14, 49, 202, 1069, 6470,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{e^{2 x}}{\sqrt{1 - x^{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) -2*a(n-1) -(n-1)\^{}2*a(n-2) +2*(n-1)*(n-2)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2018-02-19, revision 10) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=-2,\qquad p_{2}(n)=- \left(n - 1\right)^{2},\qquad p_{3}(n)=2 \left(n - 2\right) \left(n - 1\right)
\]
...
\section{The computation for A081920}
...
\[
1\,a(n) - 2\,a(n-1) -  \left(n - 1\right)^{2}\,a(n-2) + 2 \left(n - 2\right) \left(n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A081920, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A081920.
