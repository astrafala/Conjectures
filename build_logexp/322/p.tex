...
\title{A proof of the conjectured recurrence for OEIS A066052}
...
\begin{abstract}
OEIS A066052 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A066052 is ``Number of permutations in the symmetric group S\_n with order >= 3''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;0, 0, 2, 14, 94, 644, 4808, 39556,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{\left(1 - x\right) e^{\frac{x \left(x + 2\right)}{2}} - 1}{x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) +(-n-3)*a(n-1) +(2*n-1)*a(n-2) +n*(n-2)*a(n-3) -2*(n-2)*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Jan 04 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2020-07-07, revision 25) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- n - 3,\qquad p_{2}(n)=2 n - 1,\qquad p_{3}(n)=n \left(n - 2\right),\qquad p_{4}(n)=- 2 \left(n - 3\right) \left(n - 2\right)
\]
...
\section{The computation for A066052}
...
\[
1\,a(n) + (- n - 3)\,a(n-1) + (2 n - 1)\,a(n-2) + n \left(n - 2\right)\,a(n-3) -  2 \left(n - 3\right) \left(n - 2\right)\,a(n-4) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A066052, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A066052.
