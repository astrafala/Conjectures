...
\title{A proof of the conjectured recurrence for OEIS A108704}
...
\begin{abstract}
OEIS A108704 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A108704 is ``Number of partitions of 112233...nn into n pairs''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 18, 126, 1110, 12120, 156660,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{e^{\frac{x^{2}}{2}}}{\sqrt{1 - 2 x}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n)-(2*n-1)*a(n-1)-(n-1)*a(n-2)+2*(n-1)*(n-2)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 08 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2022-05-29, revision 31) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=1 - 2 n,\qquad p_{2}(n)=1 - n,\qquad p_{3}(n)=2 \left(n - 2\right) \left(n - 1\right)
\]
...
\section{The computation for A108704}
...
\[
1\,a(n) + (1 - 2 n)\,a(n-1) + (1 - n)\,a(n-2) + 2 \left(n - 2\right) \left(n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A108704, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A108704.
