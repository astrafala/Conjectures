...
\title{A proof of the conjectured recurrence for OEIS A097204}
...
\begin{abstract}
OEIS A097204 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A097204 is ``Binomial transform of A033312''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 8, 49, 294, 1893, 13572,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{\left(\left(1 - x\right) e^{x} - 1\right) e^{x}}{x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) +(-n-5)*a(n-1) +(5*n+3)*a(n-2) +4*(-2*n+3)*a(n-3) +4*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 16 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2016-11-23, revision 31) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- n - 5,\qquad p_{2}(n)=5 n + 3,\qquad p_{3}(n)=- 4 \left(2 n - 3\right),\qquad p_{4}(n)=4 \left(n - 3\right)
\]
...
\section{The computation for A097204}
...
\[
1\,a(n) + (- n - 5)\,a(n-1) + (5 n + 3)\,a(n-2) -  4 \left(2 n - 3\right)\,a(n-3) + 4 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A097204, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A097204.
