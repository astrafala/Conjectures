...
OEIS A085387 is ``E.g.f exp(x)*cosh(x+x\^{}2/2)''. It has offset $0$ and begins
\[
...
\begin{quote}\small
\textbf{Conjecture:} a(n) -3*a(n-1) +2*a(n-2) +(-2*n+3)*a(n-3) +-(n-3)\^{}2*a(n-4) +(n-3)*(n-4)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
