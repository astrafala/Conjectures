...
import localentry as LE, transfer10 as TA, ratrec, openness

MARK = re.compile(r'onjectur|Empirical', re.I)
CAP = int(sys.argv[1]) if len(sys.argv) > 1 else 20000
HITS, DONE = 'transfer10_hits.json', 'transfer10_done.json'
...
for a in sorted(names):
    ...
    S = sum(TA.npatterns(2 * p['fixed'], i) for i in range(1, p['K'] + 1))
    if S > CAP:
    ...
    built = TA.build(p, cap=CAP)
    if built is None:
        res['state space > cap'] += 1; continue
    adj, start, plist = built
    f = factorial(p['K']) * p['frac']
    v = TA.avals(adj, start, p, off + len(d) + 1)
    ...
    coeffs, dd = recs[0]
    order = max(coeffs)
    thr = TA.threshold(adj, start, coeffs, order, p)
    if thr is None:
        ...
        hits.append({'anum': a, 'name': nm, 'walk': p['walk'], 'fixed': p['fixed'],
                     ...
                     'nthr': thr, 'tex': p['tex'], 'rest': p['rest'],
                     'offs': p['offs']})
    done.add(a)
