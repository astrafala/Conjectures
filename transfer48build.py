#!/usr/bin/env python3
"""Papers for the pattern-avoidance family."""
import os, json, re, collections
import localentry as LE, phibuild, transferbuild, transfer48

PRE = phibuild.PRE
esc = phibuild.esc
rec_tex = transferbuild.rec_tex

DIRNAME = {(0, 1): 'along a row, left to right',
           (1, 0): 'down a column',
...
def pat_tex(kind, t):
    if kind == 'abs':
        return r'\mathtt{%s}' % ''.join(str(x) for x in t)
    def one(o):
        return 'z' if o == 0 else ('z%+d' % o)
    return '(' + ',\\,'.join(one(o) for o in t) + ')'
...
def build(h):
    ...
    off = h['offset']
    e = LE.get(a)
    p = transfer48.parse_name(e['name'])
    W, A = p['W'], p['alpha'] + 1
    ...
    for k, t, o in rules:
        by.setdefault(o, []).append(pat_tex(k, t))
    lines = '\n'.join('\\text{%s} & %s \\\\' % (DIRNAME[o], ',\\quad '.join(v))
                      for o, v in by.items())
    rel = any(k == 'rel' for k, _, _ in rules)
    reltxt = ('' if not rel else
              " A pattern written with the free letter $z$ forbids every window standing in "
              "those differences, whatever the value of $z$: the window $(v_1,\\dots,v_r)$ is "
              "an occurrence exactly when the numbers $v_s-o_s$ are all equal, $o$ being the "
              "offsets shown.")
    ...
    trans = ('' if not p['trans'] else
             ...
             "transposing exchanges a row with a column and fixes each diagonal direction. A "
             "pattern is read along its direction, and transposing does not reverse any of "
             "them, so no pattern is reversed either.")
    return rf"""{PRE}
...
\begin{{abstract}}
OEIS {a} counts arrays of width ${W}$ over ${aset}$ containing no occurrence of certain short
...
rather than empirical: no pattern is longer than ${L}$ letters and no direction moves the row
index by more than one, so every occurrence lies inside ${L}$ consecutive array rows and the
...
The array has ${W}$ cells across, with entries in ${aset}$. An OCCURRENCE of a pattern in a
...
\section{{Every occurrence lies in ${L}$ consecutive rows}}

\begin{{lemma}}\label{{lem:win}}
Each of the directions above changes the row index by $0$ or $1$ from one cell of a run to the
next, and no pattern is longer than ${L}$ letters. Hence every occurrence lies inside ${L}$
...
So the state is the window of the last ${L - 1}$ array rows, a row still to come being written
...
if __name__ == '__main__':
    hits = [h for h in json.load(open('uniall_hits.json'))
            if h.get('engine') == 'transfer48' and not h.get('FAILS')]
    for h in hits:
        dd = f"build/t48{h['anum']}"
