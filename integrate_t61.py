...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t61{a}/p.pdf"
    ...
    eng[nxt]={'engine':'pair-sum-order','order':h['order'],'degree':h['S'],
...
d['pair-sum-order|PROOF']=("two derived arrays of pair sums, ordered monotonically, "
                           "lexicographically, or by a nonvanishing determinant")
