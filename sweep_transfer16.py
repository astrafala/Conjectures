...
import localentry as LE, transfer16 as TG, ratrec, openness

MARK = re.compile(r'onjectur|Empirical', re.I)
CAP = int(sys.argv[1]) if len(sys.argv) > 1 else 20000
HITS, DONE = 'transfer16_hits.json', 'transfer16_done.json'
...
for a in sorted(names):
    ...
    if sum(i ** p['fixed'] for i in range(1, p['K'] + 1)) * (p['E'] + 1) > CAP:
    ...
    built = TG.build(p, cap=CAP)
    if built is None:
        res['state space > cap'] += 1; continue
    adj, start, end, plist = built
    f = factorial(p['K']) * p['frac']
    v = TG.avals(adj, start, end, p, off + len(d) + 1)
    ...
    coeffs, dd = recs[0]
    order = max(coeffs)
    thr = TG.threshold(adj, start, end, coeffs, order, p)
    if thr is None:
        ...
        hits.append({'anum': a, 'name': nm, 'walk': p['walk'], 'fixed': p['fixed'],
                     ...
                     'K': p['K'], 'E': p['E'], 'nthr': thr, 'tex': p['tex'],
                     'nbtex': p['nbtex'], 'offs': p['offs'], 'rest': p['rest']})
    done.add(a)
