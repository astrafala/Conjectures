...
import localentry as LE, transfer9 as T9, ratrec, openness

MARK = re.compile(r'onjectur|Empirical', re.I)
CAP = int(sys.argv[1]) if len(sys.argv) > 1 else 20000
HITS, DONE = 'transfer9_hits.json', 'transfer9_done.json'
...
for a in sorted(names):
    ...
    coeffs, dd = recs[0]
    order = max(coeffs)
    thr = T9.threshold(adj, start, end, coeffs, order, p)
    if thr is None:
        ...
        hits.append({'anum': a, 'name': nm, 'walk': p['walk'], 'fixed': p['fixed'],
                     ...
                     'nthr': thr, 'mode': p['mode'], 'tex': p['tex'], 'rest': p['rest'],
                     'offs': p['offs'], 'ul0': p['ul0']})
    done.add(a)
