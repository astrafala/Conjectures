...
hits=[h for h in json.load(open('uniall_hits.json'))
      if h.get('engine')=='transfer33' and not h.get('FAILS')]
...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t33{a}/p.pdf"
    ...
    eng[nxt]={'engine':'subblock-difference','order':h['order'],'degree':h['S'],
...
d['subblock-difference|PROOF']=("a statistic of every 2 X 2 subblock differing from its neighbours "
                                "by exactly a fixed amount")
