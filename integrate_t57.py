...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t57{a}/p.pdf"
    ...
    eng[nxt]={'engine':'subblock-line-sum','order':h['order'],'degree':h['S'],
...
d['subblock-line-sum|PROOF']=("the sums along the rows, columns and diagonals of every "
                              "K X K window of a grid, constrained or compared")
