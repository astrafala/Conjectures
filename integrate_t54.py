...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t54{a}/p.pdf"
    ...
    eng[nxt]={'engine':'offset-inequality','order':h['order'],'degree':h['S'],
...
d['offset-inequality|PROOF']=("no cell equal to the cells at a fixed list of offsets, "
                              "counted up to renaming")
