        ...
        for fn in sorted(os.listdir(dd)):
            ...
            for l in open(os.path.join(dd, fn), errors="ignore"):
                ...
                if l[1] == "N" and name is None:
                    ...
                    fl.append(b)
            # the first sweep required the name to BEGIN with "a(n) =", which misses
            # "Expansion of ...: a(n) = ..." and "Number of ...; a(n) = ...". Any name
            # that contains the definition will do; attack() splits it and checks each
            # part against the entry's own terms, so a wrong part cannot be used.
            if not name or not re.search(r"a\(n\)\s*=", name, re.I):
                continue
...
def attack(item):
    ...
    e = None
    parts = [name] + fsplit.split(name)
    m = re.search(r"a\(n\)\s*=.*$", name, re.I)
    if m:
        parts.append(m.group(0))
        parts += fsplit.split(m.group(0))
    for part in parts:
...
if __name__ == "__main__":
    ...
    json.dump(hits, open("name_hits2.json", "w"), indent=1)
    print(f"\n{len(hits)} proved")
