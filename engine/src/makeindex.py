...
with open(repopaths.PAPERS + '/index.csv', 'w', newline='') as fh:
    w = csv.writer(fh)
    w.writerow(['rank', 'anum', 'verdict', 'engine', 'file'])
    for m in rm:
        w.writerow([m['rank'], m['anum'], m['verdict'], m['engine'],
                    'papers/' + paperpath.band(m['rank']) + '/' + paperpath.name(m['rank'], m['verdict'])])
...
open(repopaths.PAPERS + '/README.md', 'w').write('\n'.join(lines) + '\n')
...
for b in sorted(bands):
    ...
    open(f'{repopaths.PAPERS}/{b}/README.md', 'w').write('\n'.join(out) + '\n')
