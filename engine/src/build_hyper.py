#!/usr/bin/env python3
"""Write and compile the hypergeometric-ratio papers.

    PAPER_DATE='15 September 2026' python3 src/build_hyper.py
    python3 src/install_vein.py hy hyper_hits.json hypergeometric-ratio
"""
import json
import os
import subprocess
...
HITS = os.environ.get('HITS', 'hyper_hits.json')
...
for h in sorted(json.load(open(HITS)), key=lambda r: r.get('anum', '')):
    a = h.get('anum')
    if not a or h.get('FAILS') or a in roster or withdrawnset.blocked(a, 'hypergeometric-ratio'):
        continue
    dd = f'build/hy{a}'
    os.makedirs(dd, exist_ok=True)
    try:
        open(f'{dd}/p.tex', 'w').write(hyperbuild.build(h))
