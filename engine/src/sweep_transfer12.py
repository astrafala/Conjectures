...
import localentry as LE, transfer12 as TC, ratrec, openness

MARK = re.compile(r'onjectur|Empirical', re.I)
CAP = int(sys.argv[1]) if len(sys.argv) > 1 else 20000
HITS, DONE = 'transfer12_hits.json', 'transfer12_done.json'
...
for a in sorted(names):
    ...
    S = sum(i ** (2 * p['fixed']) * (p['E'] + 1) for i in range(1, p['K'] + 1))
    if S > CAP:
    ...
    built = TC.build(p, cap=CAP)
    if built is None:
        res['state space > cap'] += 1; continue
    adj, start, end, plist = built
    f = factorial(p['K']) * p['frac']
    v = TC.avals(adj, start, end, p, off + len(d) + 1)
    ...
    coeffs, dd = recs[0]
    order = max(coeffs)
    thr = TC.threshold(adj, start, end, coeffs, order, p)
    if thr is None:
        ...
        hits.append({'anum': a, 'name': nm, 'walk': p['walk'], 'fixed': p['fixed'],
                     ...
                     'K': p['K'], 'E': p['E'], 'frac': p['frac'], 'S': len(adj),
                     'order': order, 'offset': off, 'nterms': len(d), 'claimed': dd,
                     'coeffs': {int(k): str(vv) for k, vv in coeffs.items()},
                     'nthr': thr, 'kind': p['kind'], 'lim': p['lim'], 'uneq': p['uneq'],
                     'tex': p['tex'], 'nbtex': p['nbtex'], 'rest': p['rest'],
                     'offs': p['offs']})
    done.add(a)
