#!/usr/bin/env python3
"""Build one paper per conjecture settled from the entry's NAME.

The whole argument is redone here, independently of the search run: the name is re-read
from the LIVE entry rather than from the local clone, re-parsed, re-checked against the
live DATA, and the class residuals recomputed from scratch. A paper is written only if
...
def build(spec):
    ...
    for v in spec:
        ...
        # the known side is the live name, not a stored string: if the entry has been
        # re-worded since the sweep, the paper must be built from what it says now
        import fsplit
        nm = e["name"].strip()
        cf = None
        for part in [nm] + fsplit.split(nm):
            cf = cfparse.parse(part)
            if cf is not None:
                break
        v = dict(v, formula=nm)
        if cf is None:
