...
import localentry as LE, transfer14 as TE, ratrec, openness

MARK = re.compile(r'onjectur|Empirical', re.I)
CAP = int(sys.argv[1]) if len(sys.argv) > 1 else 20000
HITS, DONE = 'transfer14_hits.json', 'transfer14_done.json'
...
for a in sorted(names):
    ...
    built = TE.build(p, cap=CAP)
    if built is None:
        res['state space > cap'] += 1; continue
    adj, start, end, plist = built
    f = p['frac']
    v = TE.avals(adj, start, end, p, off + len(d) + 1)
    ...
    coeffs, dd = recs[0]
    order = max(coeffs)
    thr = TE.threshold(adj, start, end, coeffs, order, p)
    if thr is None:
