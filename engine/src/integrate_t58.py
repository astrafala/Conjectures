...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t58{a}/p.pdf"
    ...
    eng[nxt]={'engine':'adjacency-precedence','order':h['order'],'degree':h['S'],
...
d['adjacency-precedence|PROOF']=("cells constrained by how many neighbours carry a value, or "
                                 "by what the cells to the left and above them carry")
