#!/usr/bin/env python3
"""Write and compile the P-recursive papers; they install through install_vein.

    PAPER_DATE='14 September 2026' python3 src/build_prec.py
    python3 src/install_vein.py pr prec_hits.json holonomic
"""
import json
import os
import subprocess
...
HITS = os.environ.get('HITS', 'prec_hits.json')
...
for h in sorted(json.load(open(HITS)), key=lambda x: x.get('anum', '')):
    a = h.get('anum')
    if not a or h.get('FAILS') or a in roster or withdrawnset.blocked(a, 'holonomic'):
        continue
    dd = f'build/pr{a}'
    os.makedirs(dd, exist_ok=True)
    try:
        open(f'{dd}/p.tex', 'w').write(precbuild.build(h))
