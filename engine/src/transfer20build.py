...
${E}$ has no edge, and a walk is accepted only when the count reaches exactly ${E}$.""")
    ...
    olist = [tuple(o) for o in
             (h.get("offs") or __import__("transfer20").parse_name(h["name"])["offs"])]
    nnb = len(olist)
    offs = ", ".join("(%d,%d)" % o for o in olist)
    o2 = h.get("offs2") or __import__("transfer20").parse_name(h["name"]).get("offs2")
    if o2:
        offs += r"\}$ and the second $\{" + ", ".join("(%d,%d)" % tuple(o) for o in o2)
...
if __name__ == "__main__":
    hits = [h for h in json.load(open("transfer20_hits.json")) if not h.get("FAILS")]
    for h in hits:
        dd = f"build/t20{h['anum']}"
