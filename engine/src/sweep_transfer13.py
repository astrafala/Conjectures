...
import localentry as LE, transfer13 as TD, ratrec, openness

MARK = re.compile(r'onjectur|Empirical', re.I)
CAP = int(sys.argv[1]) if len(sys.argv) > 1 else 20000
HITS, DONE = 'transfer13_hits.json', 'transfer13_done.json'
...
for a in sorted(names):
    ...
    built = TD.build(p, cap=CAP)
    if built is None:
        res['state space > cap'] += 1; continue
    adj, start, end, plist = built
    f = p['frac']
    v = TD.avals(adj, start, end, p, off + len(d) + 1)
    ...
    coeffs, dd = recs[0]
    order = max(coeffs)
    thr = TD.threshold(adj, start, end, coeffs, order, p)
    if thr is None:
        ...
        hits.append({'anum': a, 'name': nm, 'walk': p['walk'], 'fixed': p['fixed'],
                     ...
                     'nthr': thr, 'tex': p['tex'], 'nbtex': p['nbtex'], 'rest': p['rest'],
                     'offs': p['offs'], 'c': p['c'], 'self': p['self'],
                     'p': p['p'], 'q': p['q']})
    done.add(a)
