...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t55{a}/p.pdf"
    ...
    eng[nxt]={'engine':'repeated-value','order':h['order'],'degree':h['S'],
...
d['repeated-value|PROOF']=("the repeated values of each row and column, in the order they "
                           "occur, required to stand in a prescribed relation")
