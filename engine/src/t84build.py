#!/usr/bin/env python3
"""One paper per entry settled by transfer84.
...
def model_section(p, S, expo):
    L, k = p['L'], p['k']
    rel = "greater than" if p['strict'] else "greater than or equal to"
    alpha = (r'\{' + ','.join(str(i) for i in range(k)) + r'\}' if k <= 5
             else r'\{0,1,\dots,%d\}' % (k - 1))
    rows = (rf"permutations of ${alpha}$" if p['perm']
            else rf"arbitrary over the alphabet ${alpha}$")
    return "\n".join([
        r"\section{The count is a walk count}", "",
        rf"The entry counts arrays of ${L}$ columns whose rows are {rows}, and asks that no "
        rf"column $j$ be {rel} column $j-1$ \emph{{in all rows}}.",
        "",
        r"That is not a condition on any bounded window: ``in all rows'' quantifies over the "
        r"whole array, and a row arriving at the very end can still decide it. What makes it "
        r"a walk anyway is that it is a conjunction of independent facts about single rows. "
        rf"For each of the ${L-1}$ adjacent column pairs keep one bit, recording whether "
        rf"every row so far has had column $j$ {rel} column $j-1$. A new row can only clear "
        r"bits, never set one, and the array is admissible exactly when every bit has been "
        r"cleared by the time the array ends.",
        "",
        rf"So the state is that bit mask and nothing else --- at most $2^{{{L-1}}}$ of them, "
        rf"however large the alphabet is, which is why entries whose terms run to twenty "
        rf"digits have models this small. Rows with the same bit pattern act identically, so "
        rf"each row contributes one edge and the multiplicity does the counting. Merging "
        rf"vertices with identical futures leaves $S={S}$, and with $M$ the adjacency matrix "
        rf"and $\iota,\tau$ the vectors recording which masks may begin and end an array,",
    ])
...
def build(h):
    ...
    p = transfer84.parse_name(e['name'])
    kk = off - sh
    ...
    return rf"""{PRE}
\title{{No column above the one before it in every row: OEIS {a}}}
...
if __name__ == "__main__":
    hits = [h for h in json.load(open("uniall_hits.json"))
            if h.get("engine") == "transfer84" and not h.get("FAILS")]
    for h in hits:
