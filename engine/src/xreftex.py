#!/usr/bin/env python3
"""Paper template for a closed form the entry gives only through another sequence.

Identical to hypertex.py but for one paragraph. Some entries state their closed form only
as "a(n) = 2*A378933(n)", which becomes a fact about this sequence once the referenced
entry's own stated formula is substituted. The VIA slot names that step, so the paper says
where each ingredient came from rather than presenting a formula the entry does not carry.
"""
