...
import localentry as LE, transfer11 as TB, ratrec, openness

MARK = re.compile(r'onjectur|Empirical', re.I)
CAP = int(sys.argv[1]) if len(sys.argv) > 1 else 20000
HITS, DONE = 'transfer11_hits.json', 'transfer11_done.json'
...
for a in sorted(names):
    ...
    built = TB.build(p, cap=CAP)
    if built is None:
        res['state space > cap'] += 1; continue
    adj, start, end, plist = built
    f = p['frac']
    v = TB.avals(adj, start, end, p, off + len(d) + 1)
    ...
    coeffs, dd = recs[0]
    order = max(coeffs)
    thr = TB.threshold(adj, start, end, coeffs, order, p)
    if thr is None:
        ...
        hits.append({'anum': a, 'name': nm, 'walk': p['walk'], 'fixed': p['fixed'],
                     ...
                     'nthr': thr, 'kind': p['kind'], 'dat': p['dat'], 'exact': p['exact'],
