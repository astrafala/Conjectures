...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t47{a}/p.pdf"
    ...
    eng[nxt]={'engine':'clashing-pair','order':h['order'],'degree':h['S'],
...
d['clashing-pair|PROOF']=("neighbouring cells forbidden to carry values summing to m, with "
                          "the renaming clauses that pick one array per class carried in state")
