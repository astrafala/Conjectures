#!/usr/bin/env python3
"""Papers for the distance-inequality family."""
import conjquote
import os
...
import os, json, re
import texbits
import localentry as LE, phibuild, transferbuild, transfer62
...
def build(h):
    ...
    off = h['offset']
    e = LE.get(a)
    p = transfer62.parse_name(e['name'])
    W, m, K = p['W'], p['alpha'], p['K']
    import bf62
    raw = bf62.raw(e['name'])[1]
...
\]
whenever the second cell lies inside the array.{trans} The offsets are not written out in the entry: they are named by a
DISTANCE rule --- so many positions along a row or column, or a city block distance --- and the
list above is that rule read out. The closing clause ``new values $0..{m}$ introduced in row
