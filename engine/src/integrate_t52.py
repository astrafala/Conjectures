...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t52{a}/p.pdf"
    ...
    eng[nxt]={'engine':'ray-sum','order':h['order'],'degree':h['S'],
...
d['ray-sum|PROOF']=("no entry equal, modulo m, to a constant plus the sum of the entries "
                    "along a ray out of it")
