...
import localentry as LE, transfer22 as T, transfer19 as T19, ratrec, openness
...
HITS, DONE = 'transfer22_hits.json', 'transfer22_done.json'
...
for a in sorted(names):
    ...
    try:
        signal.alarm(BUDGET)
        t = T.terms(adj, start, end, len(d) + 4)
        signal.alarm(0)
    except Timeout:
        signal.alarm(0)
        res['term evaluation timed out'] += 1; done.add(a); continue
    sh = next((s for s in range(0, 5) if t[s:s + len(d)] == d), None)
    if sh is None:
    ...
    try:
        signal.alarm(BUDGET)
        thr = T19.threshold(adj, start, end, coeffs, order, S)
        signal.alarm(0)
    except Timeout:
        signal.alarm(0)
    ...
    if thr is None:
        ...
        if bad:
            ...
            hits.append({'anum': a, 'name': nm, 'walk': p['walk'], 'fixed': p['fixed'],
                         'base': p['base'], 'allowed': p['allowed'], 'R': p['R'],
