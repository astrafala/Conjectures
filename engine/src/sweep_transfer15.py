...
import localentry as LE, transfer15 as TF, ratrec, openness

MARK = re.compile(r'onjectur|Empirical', re.I)
CAP = int(sys.argv[1]) if len(sys.argv) > 1 else 20000
HITS, DONE = 'transfer15_hits.json', 'transfer15_done.json'
...
for a in sorted(names):
    ...
    if (4 * p['alpha'] + 1) * (p['alpha'] + 1) ** p['fixed'] > CAP:
    ...
    built = TF.build(p, cap=CAP)
    if built is None:
        res['state space > cap'] += 1; continue
    adj, start, end, plist = built
    f = p['frac']
    v = TF.avals(adj, start, end, p, off + len(d) + 1)
    ...
    coeffs, dd = recs[0]
    order = max(coeffs)
    thr = TF.threshold(adj, start, end, coeffs, order, p)
    if thr is None:
