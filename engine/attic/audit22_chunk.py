"""Audit the index-change papers in chunks: rebuild each model, check it still reproduces
every published term, that the recurrence holds well past the proved threshold, and that the
threshold is tight."""
import json, os, sys, collections
import localentry as LE, transfer22 as T

DONE = 'audit22_done.json'
hits = [x for x in json.load(open('transfer22_hits.json')) if not x.get('FAILS')]
...
print('checked this chunk', n, 'cumulative', len(done), 'of', len(hits),
      'problems', len(prob))
for q in prob:
    print(q)
