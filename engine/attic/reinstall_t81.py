#!/usr/bin/env python3
"""Install every transfer81 paper, replacing the thirty built before the parser was fixed."""
import json
import os
import shutil
...
where = {v['anum']: k for k, v in eng.items() if v['engine'] == 'canonical-subblock'}
nxt = max(eng) + 1
...
for h in sorted(hits, key=lambda x: x['anum']):
    ...
    eng[n] = {'engine': 'canonical-subblock', 'order': h['order'], 'degree': h['S'],
