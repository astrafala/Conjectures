...
hits=[h for h in json.load(open('uniall_hits.json'))
      if h.get('engine')=='transfer62' and not h.get('FAILS')]
...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t62{a}/p.pdf"
    ...
    eng[nxt]={'engine':'distance-inequality','order':h['order'],'degree':h['S'],
...
d['distance-inequality|PROOF']=("no two cells within a stated distance may agree, counted up "
                                "to renaming")
