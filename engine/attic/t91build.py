#!/usr/bin/env python3
"""One paper per entry settled by transfer91.
...
def dirs_text(ds):
    ...
    if len(ds) == 8 and all(max(abs(a), abs(b)) == 1 for a, b in ds):
        return "its eight king-move neighbours"
    if len(ds) == 8 and all(abs(a) + abs(b) == 3 for a, b in ds):
        return "the eight cells a knight's move away"
    parts = [k for k, v in NAMED.items() if v <= ds]
    if parts and set().union(*[NAMED[k] for k in parts]) == ds:
        return "its " + " and ".join(parts) + " neighbours"
    return "the cells at the offsets " + ", ".join(
        "$(%d,%d)$" % d for d in sorted(ds))


def cond_text(p):
    kind = p['kind']
    if kind == 'noany':
        return rf"no element may equal any of {dirs_text(p['dirs'])}"
    if kind == 'cmp':
        rel = {'more than': 'more than', 'at least': 'at least',
               'at most': 'at most', 'fewer than': 'fewer than'}[p['rel']]
        neg = 'may not' if p['rel'] in ('more than', 'at most') else 'must'
        if p['rel'] == 'more than':
            return (rf"no element may equal {rel} ${p['val']}$ of {dirs_text(p['dirs'])}")
        if p['rel'] == 'at least':
            return (rf"every element must equal {rel} ${p['val']}$ of "
                    rf"{dirs_text(p['dirs'])}")
        return rf"every element {neg} equal {rel} ${p['val']}$ of {dirs_text(p['dirs'])}"
    if kind == 'maxmin':
        return (rf"every element must equal either the largest or the smallest of "
                rf"{dirs_text(p['dirs'])}")
    return (rf"every element $x$ must have, among {dirs_text(p['dirs'])}, one equal to "
            rf"$x+{p['up']}$ and one equal to $x-{p['dn']}$, both modulo ${p['k']}$")
...
def model_section(p, S, expo):
    ...
    L1 = [r"\section{The count is a walk count}", "",
          ...
          "",
          rf"The furthest that condition reaches is ${R}$ {walk}"
          + ("s" if R != 1 else "") +
          rf", so a window of ${2*R+1}$ consecutive {walk}s decides it for the {walk} in the "
          rf"middle. Carry ${2*R}$ {walk}s as the state; a {walk} is judged when the {walk} "
          rf"${R}$ beyond it arrives, and the last ${R}$ {walk}"
          + ("s" if R != 1 else "") +
          rf" --- the ones with nothing far enough beyond them --- are judged by the end "
          rf"vector. A neighbour off the edge of the array is not a neighbour, which is what "
          rf"the entry means and what keeps the first few terms small."]
    if p['canon']:
        L1 += ["", rf"The entry also asks that new values be introduced in row major order. "
                   rf"That is not a condition on any window --- whether a value may appear "
                   rf"here depends on the whole prefix --- but it enters through a single "
                   rf"integer, how many values have been introduced so far, and the effect "
...
def build(h):
    ...
    p = transfer91.parse_name(e['name'])
    kk = off - sh
    ...
    return rf"""{PRE}
\title{{An element and the cells around it: OEIS {a}}}
...
if __name__ == "__main__":
    hits = [h for h in json.load(open("uniall_hits.json"))
            if h.get("engine") == "transfer91" and not h.get("FAILS")]
    for h in hits:
