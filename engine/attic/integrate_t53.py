...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t53{a}/p.pdf"
    ...
    eng[nxt]={'engine':'modular-neighbour','order':h['order'],'degree':h['S'],
...
d['modular-neighbour|PROOF']=("every cell required to see neighbours carrying the next and "
                              "the previous value modulo m")
