...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t25{a}/p.pdf"
    ...
    eng[nxt]={'engine':'circular-coloring','order':h['order'],'degree':h['S'],
...
d['circular-coloring|PROOF']=("proper colourings of a grid circular in one direction, counted up "
                              "to renaming the colours")
