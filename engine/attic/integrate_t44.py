...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t44{a}/p.pdf"
    ...
    eng[nxt]={'engine':'occupancy-turn','order':h['order'],'degree':h['S'],
...
d['occupancy-turn|PROOF']=("occupancy arrays of a grid of tokens each moving one step, "
                           "with the turn a token makes on arrival constrained")
