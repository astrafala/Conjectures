#!/usr/bin/env python3
"""Papers for the running-sum family, second wording."""
import conjquote
import os, json, re
import localentry as LE, phibuild, transferbuild, transfer63
...
def build(h):
    ...
    off = h['offset']
    e = LE.get(a)
    p = transfer63.parse_name(e['name'])
...
zero, and a ray the entry names without a constant carries the constant $0$.{trans}
...
if __name__ == '__main__':
    hits = [h for h in json.load(open('uniall_hits.json'))
            if h.get('engine') == 'transfer63' and not h.get('FAILS')]
    for h in hits:
        dd = f"build/t63{h['anum']}"
