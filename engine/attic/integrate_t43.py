...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t43{a}/p.pdf"
    ...
    eng[nxt]={'engine':'occupancy-image','order':h['order'],'degree':h['S'],
...
d['occupancy-image|PROOF']=("occupancy patterns reachable when every unit moves to a "
                            "neighbouring cell or stays put: counted by determinising the "
                            "map from movement to occupancy")
