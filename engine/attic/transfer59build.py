#!/usr/bin/env python3
"""Papers for the `every V next to a W' family."""
import conjquote
import os, json, re
import texbits
import localentry as LE, phibuild, transferbuild, transfer59
...
def build(h):
    ...
    off = h['offset']
    e = LE.get(a)
    p = transfer59.parse_name(e['name'])
    W, A = p['W'], p['alpha'] + 1
    ...
    items = '\n'.join(r'\item every cell carrying $%d$ has at least one neighbour carrying '
                      r'$%d$;' % (v, w) for v, w in p['pairs'])
    noeq = ('' if not p['noeq'] else
            r'\item no two neighbouring cells carry equal values.')
...
\begin{{abstract}}
OEIS {a} counts arrays of width ${W}$ over ${aset}$ in which every cell carrying one stated
...
{noeq}
\end{{itemize}}
A cell whose value is not named on the left of any clause is unconstrained.{trans}
