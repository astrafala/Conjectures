...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t48{a}/p.pdf"
    ...
    eng[nxt]={'engine':'pattern-avoidance','order':h['order'],'degree':h['S'],
...
d['pattern-avoidance|PROOF']=("short absolute or relative patterns forbidden along the rows, "
                              "columns and diagonals of a grid")
