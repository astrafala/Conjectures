...
import sys
_all = json.load(open("egf-open.json"))
_sel = sorted(_all)
...
order %(ORDER)d, contributed by its contributor. We prove it. The generating function of the
...
RES = json.load(open("egf-results.json"))
...
def build():
    os.makedirs("build_egf", exist_ok=True)
