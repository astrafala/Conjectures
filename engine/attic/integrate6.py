...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']
    assert a not in have, a
    src=f"build/t6{a}/p.pdf"
    assert os.path.exists(src) and os.path.getsize(src)>50000, a
    shutil.copy(src, f"papers-old-numbering/{nxt}-PROOF.pdf")
    eng[nxt]={'engine':'transfer-matrix','order':h['order'],'degree':h['S'],
