...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t56{a}/p.pdf"
    ...
    eng[nxt]={'engine':'forbidden-run','order':h['order'],'degree':h['S'],
...
d['forbidden-run|PROOF']=("runs of equal or of strictly increasing entries forbidden along "
                          "the directions of a grid")
