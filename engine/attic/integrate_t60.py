...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t60{a}/p.pdf"
    ...
    eng[nxt]={'engine':'capped-pair-count','order':h['order'],'degree':h['S'],
...
d['capped-pair-count|PROOF']=("a global count of adjacent pairs summing to a value, carried "
                              "in the state by capping it")
