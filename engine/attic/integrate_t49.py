...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t49{a}/p.pdf"
    ...
    eng[nxt]={'engine':'all-values-subblock','order':h['order'],'degree':h['S'],
...
d['all-values-subblock|PROOF']=("subblocks with a prescribed number of distinct values, "
                                "counted up to renaming and using every letter")
