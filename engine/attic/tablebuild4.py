...
holds from index ${c['nthr']}+1$ on.""")
    details = "\n\n".join(detail)
    first = cols[0]
    recs = sorted(h.get('rec', []), key=lambda c: (c.get('mode', 'col'), c['k']))
    recsec = ""
    if recs:
        import ordlinebuild
        blocks = []
        for c in recs:
            ...
            blocks.append(rf"""\subsection*{{{what[0].upper() + what[1:]}, recovered}}
The entry gives only the order for this line: ``{esc(c['line'])}''. Its model is a walk on
$S={c['S']}$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S={2 * c['S']}$ exact terms returns the minimal one, of order
${c['order']}$ --- the stated order --- with integer coefficients
{ordlinebuild.coeff_display(c['coeffs'], c['order'])}
Repeating with the first ${c['order']}+4$ terms removed gives order ${c['order']}$ again.""")
        recsec = (r"""
...
if __name__ == "__main__":
    hits = json.load(open("tabcomb_hits.json"))
    for h in hits:
        dd = f"build/tb4{h['anum']}"
