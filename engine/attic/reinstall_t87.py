#!/usr/bin/env python3
"""Install every transfer87 paper into the roster."""
import json
import os
import shutil
...
where = {v['anum']: k for k, v in eng.items() if v['engine'] == 'parity-difference'}
nxt = max(eng) + 1
...
for h in sorted(hits, key=lambda x: x['anum']):
    ...
    eng[n] = {'engine': 'parity-difference', 'order': h['order'], 'degree': h['S'],
