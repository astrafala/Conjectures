...
def build(h):
    ...
    npairs = (2 * m + 1) ** 2
    return rf"""{PRE}
\title{{The empirical recurrence for OEIS {a}, proved by a one-dimensional reduction}}
...
\begin{{abstract}}
OEIS {a} counts symmetric $(n+{p['off']})\times(n+{p['off']})$ matrices over
$\{{-{m},\dots,{m}\}}$ in which every $2\times2$ subblock sums to zero and has {dw} distinct
...
on $S={S}$ states, and the empirical recurrence of order ${order}$ contributed by R.~H.~Hardin
follows.
\end{{abstract}}
...
Let $x$ be an $N\times N$ symmetric matrix with entries in $\{{-{m},\dots,{m}\}}$ such that
\[
...
\qquad u_0,\dots,u_{{N-1}}\in\{{-{m},\dots,{m}\}}\ \text{{all of the same parity}},
\]
...
the same parity; and $|x(i,j)|=|u_i+u_j|/2\le{m}$ for all $i,j$ is equivalent to $|u_i|\le{m}$
...
$\{{-{m},\dots,{m}\}}$ with all entries of one parity. What remains is to say, in terms of $u$,
that every $2\times2$ subblock has {dw} distinct entries.
...
Call two consecutive pairs COMPATIBLE when that number lies in ${dset}$. The condition on the
...
the compatibility graph on the $\le {npairs}$ pairs.
\end{{corollary}}
...
$S={S}$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{{\!\top}}M^{{\,j}}\tau ,\qquad \tau=\mathbf{{1}},\qquad S={S}.
\]
...
symmetric matrices were written out entry by entry over $\{{-{m},\dots,{m}\}}$ and each
$2\times2$ subblock tested for summing to zero and for having {dw} distinct entries. The
