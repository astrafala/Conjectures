...
def build(h):
    a = h['anum']
    e = LE.get(a)
    cols = sorted(h['cols'], key=lambda c: c['k'])
    ks = ', '.join(str(c['k']) for c in cols)
    mod, rev = e['modified'], e['revision']
    blk = block(a)
    d = [int(v) for v in e['data'].split(',') if v.strip()]
    rows = []
    for c in cols:
        rows.append(rf"${c['k']}$ & ${c['order']}$ & ${c['S']}$ & $n>{c['nthr']}$ & "
                    rf"${c['nterms']}$ \\")
    tab = "\n".join(rows)
    blktex = "\n".join(esc(x) + r'\\' for x in blk)
    detail = []
    for c in cols:
        detail.append(rf"""\subsection*{{Column $k={c['k']}$}}
