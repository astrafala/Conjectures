...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t51{a}/p.pdf"
    ...
    eng[nxt]={'engine':'digit-divisibility','order':h['order'],'degree':h['S'],
...
d['digit-divisibility|PROOF']=("rows and columns read as base-b numbers and required to be "
                               "divisible, or not, by given moduli")
