...
for h in sorted(hits,key=lambda x:x['anum']):
    a=h['anum']; src=f"build/t38{a}/p.pdf"
    ...
    eng[nxt]={'engine':'edge-count-pattern','order':h['order'],'degree':h['S'],
...
d['edge-count-pattern|PROOF']=("equal-edge conditions on the subblocks, counted up to renaming the "
                               "letters")
