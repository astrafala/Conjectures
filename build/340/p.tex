...
\title{A proof of the conjectured recurrence for OEIS A081672}
...
\begin{abstract}
OEIS A081672 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 8 x^{3} + 4 x^{2} + 2 x$, a polynomial of degree $3$, which proves
...
OEIS A081672 is ``Expansion of exp(2x) - exp(0) + BesselI\_0(2x)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 6, 8, 22, 32, 84, 128,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x \sqrt{1 - 4 x^{2}} + 2 x - 1}{\sqrt{1 - 4 x^{2}} \left(2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +2*(1-n)*a(n-1) +4*(1-n)*a(n-2) +8*(n-2)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 12 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2016-06-14, revision 28) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(n - 1\right),\qquad p_{2}(n)=- 4 \left(n - 1\right),\qquad p_{3}(n)=8 \left(n - 2\right)
\]
...
\section{The computation for A081672}
...
\[
n\,a(n) -  2 \left(n - 1\right)\,a(n-1) -  4 \left(n - 1\right)\,a(n-2) + 8 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A081672, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 8 x^{3} + 4 x^{2} + 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A081672.
