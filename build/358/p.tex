...
\title{A proof of the conjectured recurrence for OEIS A126674}
...
\begin{abstract}
OEIS A126674 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A126674 is ``a(n) = n!*Sum\_\{j=0..n-1\} 2\^{}j/(j+1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 4, 20, 128, 1024, 9984, 115968,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{\log{\left(1 - 2 x \right)}}{2 \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) + (-3*n+2)*a(n-1) + 2*(n-1)\^{}2*a(n-2) = 0. - \emph{R. J. Mathar}, May 23 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2022-09-08, revision 41) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=2 - 3 n,\qquad p_{2}(n)=2 \left(n - 1\right)^{2}
\]
...
\section{The computation for A126674}
...
\[
1\,a(n) + (2 - 3 n)\,a(n-1) + 2 \left(n - 1\right)^{2}\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A126674, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A126674.
