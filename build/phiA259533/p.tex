...
\title{The reduction of OEIS A259533 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A259533 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
OEIS A259533 is ``Number of restricted barred preferential arrangements of an n-set having 3 bars in which 3 fixed sections are restricted sections and 1 section is a free section.''. It has offset $0$ and begins
\[
1, 4, 18, 94, 582, 4294, 37398, 378214,\ \dots
\]
...
\begin{quote}\small
\textbf{Conjecture}: for fixed k = 1,2,..., the sequence obtained by reducing a(n) modulo k is eventually periodic with the exact period dividing phi(k), where phi(k) is the Euler totient function A000010. For example, modulo 24 the sequence becomes [1, 4, 18, 22, 6, 22, 6, 22, 6, ...] with an apparent period of 2 beginning at a(3). --- \emph{Peter Bala}, Jul 08 2022
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:29, revision
61) the statement is still recorded as a conjecture, and nothing on the entry
...
\begin{proof}
$G$ is a rational function with integer coefficients whose denominator has constant term $1$.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j = 1, 4, 7, 8, 8, 8, \dots$, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 4, 7, 8, 8, 8, 8, 8, 8.
\]
...
Let $a(n)$ be OEIS A259533. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A259533.
