...
\title{The empirical recurrence for OEIS A205069, proved by transfer matrix}
...
\begin{abstract}
OEIS A205069 counts arrays over $\{0,\dots,1\}$ under a condition on the number of rightwards and downwards edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=584$ states and is
...
OEIS A205069 is ``Number of (n+1)X6 0..1 arrays with the number of rightwards and downwards edge increases in each 2X2 subblock differing from the number in all its horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
584, 244, 347, 528, 856, 1232, 1852, 2640,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 11:45:12, revision 9) this is still
...
\]
on the $S=584$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $5$
subblocks; the entry's first term is $584$, which the model
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-2) - 2\,a(n-6) - 3\,a(n-8) - 3\,a(n-10) - a(n-12) - a(n-14) + 5\,a(n-16) + 9\,a(n-18) + 11\,a(n-20) + 11\,a(n-22) + 10\,a(n-24) + 6\,a(n-26) + 3\,a(n-28) + a(n-30)
\]
for every $n>34$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205069.
