...
\title{The empirical column recurrences for the table OEIS A264142}
...
\begin{abstract}
OEIS A264142 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264142 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having index change +-(.,.) 0,0 1,-2 or 1,1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 8, 4, 18, 27, 8, 45, 144,\ \dots
\]
...
k=6: [order 95]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 11:16:35, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having index change +-(.,.) 0,0 1,-2 or 1,1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having index change +-(.,.) 0,0 1,-2 or 1,1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(4) arrays of permutations of 0..(n+1)*(4)-1 with each element having index change +-(.,.) 0,0 1,-2 or 1,1.''. Its rows are the vertices of a digraph on
...
model reproduces $21$ published values, $64$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264142.
