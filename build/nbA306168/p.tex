...
\title{The empirical recurrence for OEIS A306168, proved by transfer matrix}
...
\begin{abstract}
OEIS A306168 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,3,5,6,7\}$, and it carries an empirical
...
OEIS A306168 is ``Number of nX4 0..1 arrays with every element unequal to 0, 1, 3, 5, 6 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
5, 21, 30, 93, 249, 544, 1372, 3411,\ \dots
\]
...
has score in $\{0,1,3,5,6,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,3,5,6,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,3,5,6,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,3,5,6,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,3,5,6,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A306168.
