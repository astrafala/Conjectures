...
\title{A proof of the conjectured recurrence for OEIS A002538}
...
\begin{abstract}
OEIS A002538 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A002538 is ``Second-order Eulerian numbers <<n+1,n-1>>''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 8, 58, 444, 3708, 33984, 341136, 3733920,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{x + 2 \log{\left(1 - x \right)}}{\left(x - 1\right)^{3}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) + 2*(-n-2)*a(n-1) + (n\^{}2+4*n+1)*a(n-2) - n*(n-1)*a(n-3) = 0. - \emph{R. J. Mathar}, Oct 27 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2026-08-18, revision 138) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- 2 \left(n + 2\right),\qquad p_{2}(n)=n^{2} + 4 n + 1,\qquad p_{3}(n)=- n \left(n - 1\right)
\]
...
\section{The computation for A002538}
...
\[
1\,a(n) -  2 \left(n + 2\right)\,a(n-1) + (n^{2} + 4 n + 1)\,a(n-2) -  n \left(n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A002538, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A002538.
