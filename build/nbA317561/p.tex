...
\title{The empirical recurrence for OEIS A317561, proved by transfer matrix}
...
\begin{abstract}
OEIS A317561 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,3,4,5,7,8\}$, and it carries an empirical
recurrence of order $17$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A317561 is ``Number of nX4 0..1 arrays with every element unequal to 0, 1, 2, 3, 4, 5, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
8, 128, 1619, 20312, 255788, 3218063, 40491721, 509501103,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 31 2018 12:35:50, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;11\,a(n-1) + 39\,a(n-2) - 150\,a(n-3) - 1100\,a(n-4) - 681\,a(n-5) + 2764\,a(n-6) + 4703\,a(n-7) + 743\,a(n-8) - 4088\,a(n-9) - 3791\,a(n-10) - 2043\,a(n-11) - 927\,a(n-12) + 544\,a(n-13) + 1971\,a(n-14) + 594\,a(n-15) + 192\,a(n-16) + 50\,a(n-17)
\end{equation}
for all $n>17$.
...
has score in $\{0,1,2,3,4,5,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,4,5,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,4,5,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,4,5,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,4,5,7,8\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-17}$ whenever $n-17\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-17$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-17}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $17$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=17+1$ onwards, and the run was continued
...
compared against the entry's DATA at every one of the $16$ published indices; they
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A317561.
