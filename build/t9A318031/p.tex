...
\title{The empirical recurrence for OEIS A318031, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A318031 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A318031 is ``Number of nX2 0..1 arrays with every element unequal to 0, 1, 3 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
2, 3, 6, 11, 22, 46, 91, 182,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +4*a(n-3) +a(n-5) -a(n-6)
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 13 2018 14:21:15, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 4\,a(n-3) + a(n-5) - a(n-6)
\end{equation}
for all $n>7$.
...
\#\{\text{neighbours with a value }different from\ x_{t,u}\}\in\{0,1,3,4\}
\end{equation}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A318031.
