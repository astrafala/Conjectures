...
\title{A proof of the conjectured recurrence for OEIS A156849}
...
\begin{abstract}
OEIS A156849 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $156 x^{3} + 217 x^{2} + 156 x$, a polynomial of degree $3$, which proves
...
OEIS A156849 is ``Numbers k such that k\^{}2 == 2 (mod 23\^{}2)''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;156, 373, 685, 902, 1214, 1431, 1743, 1960,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{x \left(156 x^{2} + 217 x + 156\right)}{x^{3} - x^{2} - x + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = a(n-1) + a(n-2) - a(n-3) = 529*n/2 - 529/4 - 95*(-1)\^{}n/4. - \emph{R. J. Mathar}, Oct 18 2010
\end{quote}
As of the ``Last modified'' line on the live entry (2026-06-30, revision 43) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=-1,\qquad p_{2}(n)=-1,\qquad p_{3}(n)=1
\]
...
\section{The computation for A156849}
...
\[
1\,a(n) - 1\,a(n-1) - 1\,a(n-2) + 1\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A156849, agreeing on all 13 terms compared.
...
vanish, for all 36 values of $n$ from $n=5$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=156 x^{3} + 217 x^{2} + 156 x$ is an
identity of functions and the theorem holds for all $n>3$ simultaneously.

\begin{remark}
The comment quoted in Section~1 chains two claims: the recurrence proved above, and the
closed form $a(n)=\tfrac{529n}{2}-\tfrac{529}{4}-\tfrac{95(-1)^{n}}{4}$. The second holds
as well, by the same generating function. Writing $\theta=x\frac{d}{dx}$, the sequence
$n\mapsto n$ has generating function $\theta\bigl[(1-x)^{-1}\bigr]=x/(1-x)^{2}$ and
$n\mapsto(-1)^{n}$ has $1/(1+x)$, so the closed form has generating function
\[
\frac{529}{2}\cdot\frac{x}{(1-x)^{2}}-\frac{529}{4}\cdot\frac{1}{1-x}
-\frac{95}{4}\cdot\frac{1}{1+x},
\]
and subtracting \eqref{eq:gf} this difference cancels identically to $0$. It was also
checked directly against all 40 published terms. Both halves of the comment therefore
hold.
\end{remark}

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A156849.
