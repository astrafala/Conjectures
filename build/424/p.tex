...
\title{A proof of the conjectured recurrence for OEIS A125306}
...
\begin{abstract}
OEIS A125306 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x^{5} - x^{3} - 12 x^{2} + 4$, a polynomial of degree $5$, which proves
...
OEIS A125306 is ``Number of 123-segmented permutations of length n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 6, 18, 56, 182, 607,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x \left(2 x^{2} + 2 x + \sqrt{1 - 4 x} + 1\right) - \left(x^{2} + 1\right) \left(2 x + \sqrt{1 - 4 x} - 1\right)}{x \left(2 x^{2} + 2 x + \sqrt{1 - 4 x} + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n+2)*a(n) - 6*n*a(n-1) + 4*(-n-2)*a(n-2) + (-13*n+34)*a(n-3) + 2*(-5*n+17)*a(n-4) + (-7*n+40)*a(n-5) + 2*(-2*n+11)*a(n-6) = 0. - \emph{R. J. Mathar}, May 30 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2020-08-07, revision 29) the
...
\[
p_{0}(n)=2 \left(n + 2\right),\qquad p_{1}(n)=- 6 n,\qquad p_{2}(n)=- 4 \left(n + 2\right),\qquad p_{3}(n)=34 - 13 n,\qquad p_{4}(n)=- 2 \left(5 n - 17\right),\qquad p_{5}(n)=40 - 7 n,\qquad p_{6}(n)=- 2 \left(2 n - 11\right)
\]
...
\section{The computation for A125306}
...
\[
2 \left(n + 2\right)\,a(n) -  6 n\,a(n-1) -  4 \left(n + 2\right)\,a(n-2) + (34 - 13 n)\,a(n-3) -  2 \left(5 n - 17\right)\,a(n-4) + (40 - 7 n)\,a(n-5) -  2 \left(2 n - 11\right)\,a(n-6) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A125306, agreeing on all 13 terms compared.
...
vanish, for all 20 values of $n$ from $n=6$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=3 x^{5} - x^{3} - 12 x^{2} + 4$ is an
...
\url{https://oeis.org}, 2026. Sequence A125306.
