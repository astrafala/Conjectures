...
\title{The empirical recurrence for OEIS A254971, proved by transfer matrix}
...
\begin{abstract}
OEIS A254971 counts $(n+2)\times3$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
every $3\times3$ subblock, and it carries an empirical recurrence of order $10$
...
OEIS A254971 is ``Number of (n+2) X (1+2) 0..1 arrays with no 3 x 3 subblock diagonal sum 0 or 1 and no antidiagonal sum 0 or 1 and no row sum 0 and no column sum 0.''. It has offset $1$ and begins
\[
121, 310, 1008, 3241, 9961, 30529, 94351, 292130,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + 2*a(n-2) + 3*a(n-3) + 5*a(n-4) - a(n-5) - 6*a(n-6) - 7*a(n-7) + 2*a(n-9) - a(n-10) for n>11.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 18 2018 09:25:44, revision 8) this is still
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 2\,a(n-2) + 3\,a(n-3) + 5\,a(n-4) - a(n-5) - 6\,a(n-6) - 7\,a(n-7) + 2\,a(n-9) - a(n-10)
\]
for every $n>11$.
\end{theorem}

\begin{proof}
The vector $w=q(M)\mathbf{1}$ was formed by $10$ matrix--vector products in exact
integer arithmetic and $\mathbf{1}^{\!\top}M^{j}w$ evaluated for $j=0,1,\dots$, again
exactly. Those integers vanish from the index corresponding to $n=11+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>11$. The bound is exact: at $n=11$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A254971.
