...
\title{The empirical recurrence for OEIS A316118, proved by transfer matrix}
...
\begin{abstract}
OEIS A316118 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,2,3,5,6,7\}$, and it carries an empirical
...
OEIS A316118 is ``Number of nX3 0..1 arrays with every element unequal to 0, 2, 3, 5, 6 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 7, 13, 23, 47, 144, 391, 1118,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 24 2018 16:32:40, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) - 7\,a(n-3) - 10\,a(n-4) + 7\,a(n-5) + 24\,a(n-6) + 20\,a(n-7) - 38\,a(n-8) - 69\,a(n-9) + 22\,a(n-10) + 115\,a(n-11) + 43\,a(n-12) - 92\,a(n-13) - 56\,a(n-14) + 18\,a(n-15) + 16\,a(n-16) + 4\,a(n-17)
\end{equation}
for all $n>18$.
...
has score in $\{0,2,3,5,6,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,2,3,5,6,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,2,3,5,6,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,2,3,5,6,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,2,3,5,6,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A316118.
