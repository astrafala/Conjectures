...
\title{A disproof of the conjectured recurrence for OEIS A119967,\\ and the recurrence
that holds instead}
...
\begin{abstract}
OEIS A119967 carries a conjectured linear recurrence with polynomial coefficients, of
...
the smallest failure is at $n=4$, where the left-hand side equals $54$
...
OEIS A119967 is ``A transform of the central binomial coefficients C(n,floor(n/2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 4, 9, 21, 49, 113,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0}a(n)x^{n}\;=\;\frac{2 \left(x - 1\right)}{x^{2} + 2 x - \sqrt{- 3 x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1} - 1} .
\end{equation}
This is the only input taken from the entry, and it was checked against every term of the
entry's DATA section before use --- all $31$ of them --- not against a sample.

The entry separately carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*(n-3)*a(n) +(2+2*n-n\^{}2)*a(n-1) +(3*n-5)*(n-2)*a(n-2) -(n-2)*(n-3)*a(n-3) -(n-2)*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 14 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-19, revision 16) the
...
\left(n - 3\right) \left(n + 2\right)\,a(n) + (- n^{2} + 2 n + 2)\,a(n-1) + \left(n - 2\right) \left(3 n - 5\right)\,a(n-2) -  \left(n - 3\right) \left(n - 2\right)\,a(n-3) -  \left(n - 3\right) \left(n - 2\right)\,a(n-4)\;=\;0 .
\end{equation}
...
Carrying out \eqref{eq:B} for A119967 and reducing in $\mathbb{Q}(x)[y]/\bigl(P(x,y)\bigr)$, the residual $B$ is
...
\begin{theorem}
The recurrence \eqref{eq:conj} conjectured on OEIS A119967 is false. Its smallest failure
is at $n=4$, where the left-hand side equals $54$, and it fails for
infinitely many $n$.
\end{theorem}
...
the sequence A119967 itself; the entry at $n=4$ is nonzero. That $B$ is not a
...
For all $n\ge6$,
\[
\left(- n^{2} - 2 n\right)\,a(n) + \left(6 n^{2} + n - 4\right)\,a(n-1) + \left(- 15 n^{2} + 29 n - 4\right)\,a(n-2) + \left(20 n^{2} - 82 n + 72\right)\,a(n-3) + \left(- 11 n^{2} + 84 n - 136\right)\,a(n-4) + \left(- 2 n^{2} - 3 n + 44\right)\,a(n-5) + \left(3 n^{2} - 27 n + 60\right)\,a(n-6)\;=\;0 .
\]
\end{theorem}
...
DATA at every one of the $31$ published indices; the values agree exactly. A
...
terms, and vanishes at all $25$ indices where it applies.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A119967.
