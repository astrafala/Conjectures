...
\title{A proof of the conjectured identity for OEIS A309878}
...
\begin{abstract}
OEIS A309878 carries a conjectured identity expressing its terms through the terms of
A009545. We prove it. Each entry involved posts a generating function, so the claim is
a single identity between generating functions: writing $F_{a}$ for the generating
function of A309878 and $F_{j}$ for those of the entries on the right, the conjecture
...
F_{a}(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{x \left(2 x - 1\right)}{\left(x - 1\right)^{2} \left(2 x^{2} - 2 x + 1\right)} ,
\end{equation}
and the entry carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = 2*A009545(n)-n. - \emph{R. J. Mathar}, Mar 06 2022
\end{quote}
As of the ``Last modified'' line on the live entry (2024-10-14, revision 37) the
...
\[
F_{A009545}(x) \;=\; \frac{x}{2 x^{2} - 2 x + 1}
\]
...
\section{The computation for A309878}

Assembling the right-hand side by Lemmas~\ref{lem:shift} and~\ref{lem:theta} gives
\[
R(x)\;=\;- \frac{x \left(2 x - 1\right)}{\left(x^{2} - 2 x + 1\right) \left(2 x^{2} - 2 x + 1\right)} ,
\]
...
both sides were formed from the entries' own values and found to agree, for all 45
values of $n$ from $n=0$ upward. This is what Corollary~\ref{cor:criterion}
...
\url{https://oeis.org}, 2026. Sequence A309878.
