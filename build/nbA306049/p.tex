...
\title{The empirical recurrence for OEIS A306049, proved by transfer matrix}
...
\begin{abstract}
OEIS A306049 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,3,6,7\}$, and it carries an empirical
...
OEIS A306049 is ``Number of nX4 0..1 arrays with every element unequal to 0, 1, 2, 3, 6 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
8, 49, 142, 458, 1962, 6604, 22446, 86464,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 18 2018 07:36:51, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;7\,a(n-1) - 17\,a(n-2) + 39\,a(n-3) - 133\,a(n-4) + 234\,a(n-5) - 178\,a(n-6) + 296\,a(n-7) - 581\,a(n-8) + 168\,a(n-9) + 290\,a(n-10) + 137\,a(n-11) - 67\,a(n-12) - 620\,a(n-13) + 433\,a(n-14) - 15\,a(n-15) + 166\,a(n-16) - 206\,a(n-17) + 44\,a(n-18) - 6\,a(n-19) + 14\,a(n-20) - 4\,a(n-21)
\end{equation}
for all $n>24$.
...
has score in $\{0,1,2,3,6,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,6,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,6,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,6,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,6,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A306049.
