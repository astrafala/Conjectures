...
\title{A proof of the conjectured generating function for OEIS A192382}
...
\begin{abstract}
OEIS A192382 carries a conjectured generating function. We prove it. The entry states, as fact,
...
of order $2$ whose leading coefficient does not vanish, together with $2$
...
OEIS A192382 is ``Coefficient of x in the reduction by x\textasciicircum{}2 -> x+2 of the polynomial p(n,x) defined below in Comments''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;0, 2, 4, 24, 80, 352, 1344, 5504,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = 4\textasciicircum{}n*(1 - (-1/2)\textasciicircum{}n)/3. - \emph{Peter Luschny}, Oct 02 2019
\end{quote}
and separately carries the comment
\begin{quote}\small
G.f.: 2*x\textasciicircum{}2 / ((1+2*x)*(1-4*x)). .
\end{quote}
As of the ``Last modified'' line on the live entry (2023-03-03, revision 21) the second
...
-8\,a(n) - 2\,a(n+1) + 1\,a(n+2) \;=\; 0
\end{equation}

The leading coefficient of \eqref{eq:rec} is $1$. It has no integer zero at all, so the recurrence steps at every index.
...
For all $n\ge 1$,
\[
\sum_{n\ge 1} a(n)x^{n} \;=\; - \frac{2 x^{2}}{\left(2 x + 1\right) \left(4 x - 1\right)}
\]
\end{theorem}
...
previous section. They agree at the $2$ consecutive indices beginning
...
\url{https://oeis.org}, 2026. Sequence A192382.
