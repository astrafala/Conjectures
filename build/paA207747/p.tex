...
\title{The empirical recurrence for OEIS A207747, proved by transfer matrix}
...
\begin{abstract}
OEIS A207747 counts arrays over $\{0,\dots,1\}$ of width $3$ that avoid the patterns
$(0,0,1), (0,1,1)$ along one axis and $(0,0,1), (1,0,0)$ along the other, and carries an empirical recurrence of
order $11$ contributed by R.~H.~Hardin. It is true. The patterns have length $3$, so
...
$\mathbf{1}^{\!\top}P^{\,n-2-11}q(P)\mathbf{1}$, which vanishes identically
...
OEIS A207747 is ``Number of n X 3 0..1 arrays avoiding 0 0 1 and 0 1 1 horizontally and 0 0 1 and 1 0 0 vertically.''. It has offset
$1$ and begins
\[
6, 36, 102, 270, 798, 2354, 7210,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 7*a(n-1) - 9*a(n-2) - 32*a(n-3) + 72*a(n-4) + 36*a(n-5) - 147*a(n-6) + 9*a(n-7) + 109*a(n-8) - 28*a(n-9) - 24*a(n-10) + 8*a(n-11) for n>12. Corrected by \_Colin Barker\_, Mar 06 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 06 2018 08:54:53, revision
8) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;7\,a(n-1) - 9\,a(n-2) - 32\,a(n-3) + 72\,a(n-4) + 36\,a(n-5) - 147\,a(n-6) + 9\,a(n-7) + 109\,a(n-8) - 28\,a(n-9) - 24\,a(n-10) + 8\,a(n-11)
\end{equation}
for all $n>12$. 
...
$(0,0,1), (0,1,1)$ as $3$ consecutive entries. There are $6$ admissible rows; that condition
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-11}$ whenever $n-11\ge2$. Hence
\eqref{eq:conj} holds for every $n>t$ exactly when $u_j=0$ for every
$j>t-2-11$. Moreover $(u_j)$ satisfies the monic linear recurrence whose
...
$P^{\,n-2-11}$ gives the identity. The equivalence follows by letting the exponent
...
\begin{proof}
The vector $q(P)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $11$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated in turn. They vanish from the
index corresponding to $n=12+1$ onwards, and the run was continued past $S$ consecutive
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207747.
