...
\title{The empirical recurrence for OEIS A207026, proved by transfer matrix}
...
\begin{abstract}
OEIS A207026 counts arrays over $\{0,\dots,1\}$ of width $3$ that avoid the patterns
$(0,0,1), (1,0,1)$ along one axis and $(0,0,0), (0,1,1)$ along the other, and carries an empirical recurrence of
order $19$ contributed by R.~H.~Hardin. It is true. The patterns have length $3$, so
...
$\mathbf{1}^{\!\top}P^{\,n-2-19}q(P)\mathbf{1}$, which vanishes identically
...
OEIS A207026 is ``Number of 3Xn 0..1 arrays avoiding 0 0 0 and 0 1 1 horizontally and 0 0 1 and 1 0 1 vertically.''. It has offset
$1$ and begins
\[
6, 36, 90, 252, 624, 1350, 3025,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 13:24:17, revision
9) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + a(n-2) - 5\,a(n-3) - 10\,a(n-4) + 12\,a(n-5) + 11\,a(n-6) + a(n-7) - 15\,a(n-8) + a(n-9) - 7\,a(n-10) + 3\,a(n-11) + 3\,a(n-12) + 9\,a(n-13) - 4\,a(n-14) - 2\,a(n-16) - a(n-18) + a(n-19)
\end{equation}
for all $n>18$. The entry's array has a fixed number of rows and a growing number of
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-19}$ whenever $n-19\ge2$. Hence
\eqref{eq:conj} holds for every $n>t$ exactly when $u_j=0$ for every
$j>t-2-19$. Moreover $(u_j)$ satisfies the monic linear recurrence whose
...
$P^{\,n-2-19}$ gives the identity. The equivalence follows by letting the exponent
...
\begin{proof}
The vector $q(P)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $19$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated in turn. They vanish from the
index corresponding to $n=18+1$ onwards, and the run was continued past $S$ consecutive
...
compared against the entry's DATA at every one of the $29$ published indices; the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207026.
