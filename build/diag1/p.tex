...
\title{A proof of the conjectured recurrence for OEIS A156894}
...
\begin{abstract}
OEIS A156894 carries a conjectured linear recurrence with polynomial coefficients, of
...
$- 24 x^{3} - 24 x^{2} - 96 x$, of degree $3$, which proves the recurrence for every $n>3$.
\end{abstract}
...
OEIS A156894 is ``a(n) = Sum\_\{k=0..n\} binomial(n,k)*binomial(2*n+k-1,k)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 19, 138, 1059, 8378, 67582, 552576,\ \dots
\]
...
a(n)\;=\;[x^{n}]\,f(x)\,g(x)^{n},
\qquad f(x)=1,\qquad g(x)=\frac{x + 1}{x^{2} - 2 x + 1} .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} 64*n*(2*n-1)*a(n) -16*(89*n\^{}2 -134*n +63)*a(n-1) +4*(661*n\^{}2 -2619*n +2576)*a(n-2) -3*(119*n\^{}2 -713*n +1092)*a(n-3) +6*(2*n-7)*(n-4)*a(n-4) = 0. - \emph{R. J. Mathar}, Feb 05 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 46) the
...
For A156894 this yields
\[
4 t^{2} y^{3} - 4 t^{2} y^{2} + t^{2} y - 71 t y^{3} + 71 t y^{2} - 28 t y + 4 t + 8 y^{3} - 8 y^{2} \;=\;0 ,
\]
...
\[
64 n \left(2 n - 1\right)\,a(n) -  16 \left(89 n^{2} - 134 n + 63\right)\,a(n-1) + 4 \left(661 n^{2} - 2619 n + 2576\right)\,a(n-2) -  3 \left(119 n^{2} - 713 n + 1092\right)\,a(n-3) + 6 \left(n - 4\right) \left(2 n - 7\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
The result was then compared term by term with the DATA section of A156894, agreeing on
...
all 19 values of $n$ from $n=4$ upward.
...
the identity $B=- 24 x^{3} - 24 x^{2} - 96 x$ is an identity of functions and the theorem holds for all
$n>3$ simultaneously.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A156894.
