...
\title{The empirical recurrence for OEIS A205067, proved by transfer matrix}
...
\begin{abstract}
OEIS A205067 counts arrays over $\{0,\dots,1\}$ under a condition on the number of rightwards and downwards edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=98$ states and is
...
OEIS A205067 is ``Number of (n+1)X4 0..1 arrays with the number of rightwards and downwards edge increases in each 2X2 subblock differing from the number in all its horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
98, 90, 144, 220, 347, 524, 850, 1268,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-2) -2*a(n-6) -2*a(n-8) -4*a(n-10) +2*a(n-14) +3*a(n-16) +a(n-18) for n>20
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 11:44:58, revision 9) this is still
...
\]
on the $S=98$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $3$
subblocks; the entry's first term is $98$, which the model
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-2) - 2\,a(n-6) - 2\,a(n-8) - 4\,a(n-10) + 2\,a(n-14) + 3\,a(n-16) + a(n-18)
\]
for every $n>20$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205067.
