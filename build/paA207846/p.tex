...
\title{The empirical recurrence for OEIS A207846, proved by transfer matrix}
...
\begin{abstract}
OEIS A207846 counts arrays over $\{0,\dots,1\}$ of width $3$ that avoid the patterns
$(0,0,0), (1,1,1)$ along one axis and $(0,0,0), (0,0,1)$ along the other, and carries an empirical recurrence of
order $2$ contributed by R.~H.~Hardin. It is true. The patterns have length $3$, so
...
$\mathbf{1}^{\!\top}P^{\,n-2-2}q(P)\mathbf{1}$, which vanishes identically
...
OEIS A207846 is ``Number of 3 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 0 and 1 1 1 vertically.''. It has offset
$1$ and begins
\[
6, 36, 72, 180, 432, 1044, 2520,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) + a(n-2) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 25 2018 15:55:10, revision
...
a(n)\;=\;2\,a(n-1) + a(n-2)
\end{equation}
for all $n>1$. The entry's array has a fixed number of rows and a growing number of
...
$(0,0,0), (1,1,1)$ as $3$ consecutive entries. There are $6$ admissible columns; that condition
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-2}$ whenever $n-2\ge2$. Hence
\eqref{eq:conj} holds for every $n>t$ exactly when $u_j=0$ for every
$j>t-2-2$. Moreover $(u_j)$ satisfies the monic linear recurrence whose
...
$P^{\,n-2-2}$ gives the identity. The equivalence follows by letting the exponent
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>1$. This is the statement of
Section~1.
\end{theorem}

\begin{proof}
The vector $q(P)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $2$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated in turn. They vanish from the
index corresponding to $n=1+1$ onwards, and the run was continued past $S$ consecutive
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207846.
