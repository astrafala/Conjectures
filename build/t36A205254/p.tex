...
\title{The empirical recurrence for OEIS A205254, proved by transfer matrix}
...
\begin{abstract}
OEIS A205254 counts arrays over $\{0,\dots,1\}$ under a condition on the number of clockwise edge increases
in every $2\times2$ subblock, and carries an empirical recurrence of order $20$ contributed by
...
OEIS A205254 is ``Number of (n+1)X8 0..1 arrays with the number of clockwise edge increases in every 2X2 subblock the same.''. It has offset $1$ and begins
\[
8752, 323856, 12301024, 471988176, 18189909484, 702420750928, 27149753088796, 1049837466171440,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 11:55:15, revision 9) this is still
...
a state and rows $r,s$ may follow one another when all $7$ of their subblocks give $v$.
Taking the disjoint union over $v$,
\[
a(n)\;=\;\iota^{\!\top}M^{\,n}\tau,\qquad\iota=\tau=(1,\dots,1)^{\!\top},
\]
on the $S=260$ states that survive. Rows carrying no edge at all are dropped: such a row could
...
subblocks; the entry's first term is $8752$, which the model
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205254.
