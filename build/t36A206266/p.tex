...
\title{The empirical recurrence for OEIS A206266, proved by transfer matrix}
...
\begin{abstract}
OEIS A206266 counts arrays over $\{0,\dots,1\}$ under a condition on the number of rightwards and downwards edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=154$ states and is
...
OEIS A206266 is ``Number of (n+1) X 8 0..1 arrays with the number of rightwards and downwards edge increases in each 2 X 2 subblock equal to the number in all its horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
154, 174, 503, 1296, 3011, 6444, 12878, 24319,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 8*a(n-1) - 27*a(n-2) + 48*a(n-3) - 42*a(n-4) + 42*a(n-6) - 48*a(n-7) + 27*a(n-8) - 8*a(n-9) + a(n-10) for n>11.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 15 2018 08:27:03, revision 8) this is still
...
\]
on the $S=154$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $7$
subblocks; the entry's first term is $154$, which the model
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206266.
