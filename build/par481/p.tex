...
\begin{quote}\small
\textbf{Conjecture:} 4*n*(n+1)*(6*n\^{}2 - 15*n + 8)*a(n) + 6*n*(9*n-7)*a(n-1) - 3*(3*n-4)*(3*n-2)*(6*n\^{}2-3*n-1)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 07 2014
\end{quote}
...
4 n \left(n + 1\right) \left(6 n^{2} - 15 n + 8\right)\,a(n) + 6 n \left(9 n - 7\right)\,a(n-1) -  3 \left(3 n - 4\right) \left(3 n - 2\right) \left(6 n^{2} - 3 n - 1\right)\,a(n-2)
\end{equation}
...
exact integer arithmetic at every index where it applies: $28$ instances beginning at
