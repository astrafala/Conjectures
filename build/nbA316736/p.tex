...
\title{The empirical recurrence for OEIS A316736, proved by transfer matrix}
...
\begin{abstract}
OEIS A316736 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{1,2,3,6,7,8\}$, and it carries an empirical
...
OEIS A316736 is ``Number of nX4 0..1 arrays with every element unequal to 1, 2, 3, 6, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
2, 33, 36, 95, 410, 392, 1124, 3964,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 11 2018 20:31:29, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 6\,a(n-3) - 31\,a(n-4) + 6\,a(n-5) + 10\,a(n-6) + 73\,a(n-7) - 15\,a(n-8) - 70\,a(n-9) - 42\,a(n-10) - 6\,a(n-11) + 63\,a(n-12) + 32\,a(n-13) + 12\,a(n-14) - 25\,a(n-15) - 20\,a(n-16) - 3\,a(n-17) + 4\,a(n-18) + 4\,a(n-19)
\end{equation}
for all $n>23$.
...
has score in $\{1,2,3,6,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,2,3,6,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,2,3,6,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,2,3,6,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,2,3,6,7,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A316736.
