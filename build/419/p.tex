...
\title{A proof of the conjectured recurrence for OEIS A032096}
...
\begin{abstract}
OEIS A032096 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $15 x^{4} - 4 x^{3} - 5 x^{2} + 2 x$, a polynomial of degree $4$, which proves
...
OEIS A032096 is ``"BHK" (reversible, identity, unlabeled) transform of 2,2,2,2,''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;2, 3, 8, 23, 74, 227, 704, 2135,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{x \left(- 15 x^{3} + 4 x^{2} + 5 x - 2\right)}{9 x^{4} - 12 x^{3} + 4 x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = 4*a(n-1) - 12*a(n-3) + 9*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (2024-05-25, revision 49) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=-4,\qquad p_{2}(n)=0,\qquad p_{3}(n)=12,\qquad p_{4}(n)=-9
\]
...
\section{The computation for A032096}
...
\[
1\,a(n) - 4\,a(n-1) + 12\,a(n-3) - 9\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A032096, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=15 x^{4} - 4 x^{3} - 5 x^{2} + 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A032096.
