...
\title{The empirical recurrence for OEIS A305583, proved by transfer matrix}
...
\begin{abstract}
OEIS A305583 counts $5$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,7,8\}$, and it carries an empirical
recurrence of order $6$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A305583 is ``Number of nX5 0..1 arrays with every element unequal to 0, 1, 2, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
16, 8, 19, 38, 76, 327, 967, 2641,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +3*a(n-2) +12*a(n-3) -9*a(n-5) -27*a(n-6) for n>10
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 05 2018 18:28:52, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 3\,a(n-2) + 12\,a(n-3) - 9\,a(n-5) - 27\,a(n-6)
\end{equation}
for all $n>9$.
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-6}$ whenever $n-6\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-6$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-6}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $6$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=9+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A305583.
