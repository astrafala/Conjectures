...
\title{The empirical recurrence for OEIS A305906, proved by transfer matrix}
...
\begin{abstract}
OEIS A305906 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,4,5,7\}$, and it carries an empirical
...
OEIS A305906 is ``Number of nX3 0..1 arrays with every element unequal to 0, 1, 2, 4, 5 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
4, 14, 35, 98, 358, 1124, 3534, 11667,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 14 2018 06:56:27, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 4\,a(n-2) + 11\,a(n-3) - 19\,a(n-4) - 49\,a(n-5) - 54\,a(n-6) + 80\,a(n-7) + 161\,a(n-8) + 107\,a(n-9) - 128\,a(n-10) - 154\,a(n-11) - 22\,a(n-12) + 37\,a(n-13) - 26\,a(n-14) - 33\,a(n-15)
\end{equation}
for all $n>15$.
...
has score in $\{0,1,2,4,5,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,4,5,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,4,5,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,4,5,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,4,5,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A305906.
