...
\title{The empirical recurrence for OEIS A186457, proved by transfer matrix}
...
\begin{abstract}
OEIS A186457 counts the $(n+1)\times5$ arrays over $\{0,\dots,1\}$ in which
...
a state, the count is a walk count on $S=61$ of them, and the conjectured recurrence is then
...
OEIS A186457 is ``Number of (n+1) X 5 binary arrays with every 2 X 2 subblock commuting with each of its horizontal and vertical 2 X 2 subblock neighbors.''. It has offset $1$ and begins
\[
61, 88, 162, 309, 449, 673, 945, 1493,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - 2*a(n-3) + 2*a(n-4) - a(n-5) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 17 2018 14:10:02, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The contiguous $2\times2$ subblocks of an $(n+1)\times5$ array form a grid of $n$
...
At $n=1$ the array has $2$ rows and the grid is a single row of $4$
subblocks, with only the horizontal pairs to test; the entry's first term is
$61$, which the model reproduces along with every later term.
...
\]
on the $S=61$ admissible $2$-tuples. In particular $a$ is $C$-finite.

This state does not compress. What a step needs of the past is the previous subblock row, and
for $5\ge2$ that row already determines the $2$ array rows it was built from, so nothing
is lost by carrying the rows and nothing would be gained by carrying the matrices instead. The
size of the construction is therefore $2^{2\cdot5}$ before the condition prunes
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A186457.
