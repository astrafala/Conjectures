...
\title{The empirical recurrence for OEIS A300134, proved by transfer matrix}
...
\begin{abstract}
OEIS A300134 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{0,3,4,5,6,7,8\}$, and it carries an empirical
...
OEIS A300134 is ``Number of nX4 0..1 arrays with every element equal to 0, 3, 4, 5, 6, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 2, 17, 27, 204, 667, 3273, 13765,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 26 2018 08:00:26, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) + 14\,a(n-2) - 31\,a(n-3) - 116\,a(n-4) + 19\,a(n-5) + 283\,a(n-6) + 152\,a(n-7) + 65\,a(n-8) + 179\,a(n-9) - 731\,a(n-10) - 1210\,a(n-11) + 271\,a(n-12) + 1279\,a(n-13) + 488\,a(n-14) - 481\,a(n-15) - 349\,a(n-16) + 435\,a(n-17) + 266\,a(n-18) - 478\,a(n-19) - 284\,a(n-20) + 202\,a(n-21) + 80\,a(n-22) - 93\,a(n-23) - 2\,a(n-24) + 46\,a(n-25) + 6\,a(n-26) - 8\,a(n-27) - 2\,a(n-28)
\end{equation}
for all $n>28$.
...
has score in $\{0,3,4,5,6,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,3,4,5,6,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,3,4,5,6,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,3,4,5,6,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,3,4,5,6,7,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A300134.
