...
\title{The empirical recurrence for OEIS A299556, proved by transfer matrix}
...
\begin{abstract}
OEIS A299556 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{0,2,3,5,7,8\}$, and it carries an empirical
...
OEIS A299556 is ``Number of nX3 0..1 arrays with every element equal to 0, 2, 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 7, 5, 15, 21, 57, 121, 289,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 12 2018 20:30:05, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 5\,a(n-2) - 5\,a(n-3) - 2\,a(n-4) + 20\,a(n-5) + 9\,a(n-6) - 39\,a(n-7) + 18\,a(n-8) - 10\,a(n-9) - 41\,a(n-10) + 83\,a(n-11) + 26\,a(n-12) - 44\,a(n-13) + 7\,a(n-14) - 9\,a(n-15) - 17\,a(n-16) - 3\,a(n-17) + 6\,a(n-18) + 2\,a(n-19)
\end{equation}
for all $n>19$.
...
has score in $\{0,2,3,5,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,2,3,5,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,2,3,5,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,2,3,5,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,2,3,5,7,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A299556.
