...
\title{The reduction of OEIS A316144 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A316144 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
\[
1, 4, 28, 268, 3148, 43564, 692428, 12390508,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
Expansion of e.g.f. Product\_{k>=1} ((1 + (exp(x)-1)\^{}k) / (1 - (exp(x)-1)\^{}k))\^{}2.
\end{quote}
that is, writing $A(x)=\sum_{n\ge 0}a(n)x^{n}/n!$,
\begin{equation}\label{eq:egf}
A(x)\;=\;\prod_{k\ge 1}\left(\frac{1+(e^{x}-1)^{k}}{1-(e^{x}-1)^{k}}\right)^{2}.
\end{equation}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 28 2024 10:47:57, revision
21) the statement is still recorded as a conjecture, and nothing on the entry
...
G(t)\;=\;\prod_{k\ge 1}\left(\frac{1+t^{k}}{1-t^{k}}\right)^{2}.
\end{equation}
...
\begin{proof}
$G$ is a product of integer series, only finitely many of which affect any one coefficient.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j = 1, 4, 12, 32, 76, 168, \dots$, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 4, 12, 32, 76, 168, 352, 704, 1356.
\]
...
Let $a(n)$ be OEIS A316144. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A316144.
