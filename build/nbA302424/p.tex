...
\title{The empirical recurrence for OEIS A302424, proved by transfer matrix}
...
\begin{abstract}
OEIS A302424 counts $5$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the six horizontal, diagonal and antidiagonal neighbours, a number $equal to$ itself lying in $\{0,1,3,5\}$, and it carries an empirical
...
OEIS A302424 is ``Number of nX5 0..1 arrays with every element equal to 0, 1, 3 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
8, 25, 38, 83, 372, 992, 2503, 7611,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 07 2018 17:19:43, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + a(n-2) + 21\,a(n-3) - 11\,a(n-4) - 9\,a(n-5) - 149\,a(n-6) + 82\,a(n-7) + 36\,a(n-8) + 514\,a(n-9) - 390\,a(n-10) - 78\,a(n-11) - 888\,a(n-12) + 942\,a(n-13) + 22\,a(n-14) + 646\,a(n-15) - 1008\,a(n-16) + 68\,a(n-17) + 48\,a(n-18) + 368\,a(n-19) - 288\,a(n-21)
\end{equation}
for all $n>23$.
...
has score in $\{0,1,3,5\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,3,5\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,3,5\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,3,5\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,3,5\}$. Read the sequence as the
...
They vanish from the index corresponding to $n=23+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302424.
