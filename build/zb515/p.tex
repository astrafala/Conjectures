...
\title{A proof of the conjectured recurrence for OEIS A000180}
...
\begin{abstract}
OEIS A000180 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A000180 is ``Expansion of E.g.f. exp(-x)/(1-3x)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 13, 116, 1393, 20894, 376093, 7897952,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = Sum\emph{\{k=0..n\} (-1)\textasciicircum{}(n+k)*binomial(n,k)*k!*3\textasciicircum{}k. - }Benoit Cloitre\_, Nov 02 2003
\end{quote}
so that, writing $F(n,k)$ for the summand,
\begin{equation}\label{eq:sum}
a(n)\;=\;\sum_{k=0}^{n}F(n,k),
\qquad F(n,k)=\left(-1\right)^{k + n} 3^{k} {\binom{n}{k}} k! .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) -(n+1)*(3*n-1)*a(n-1) -3*(n-1)*(n+1)*a(n-2) = 0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-07-11, revision 69) the
...
$(\sigma,R)$ can be \emph{checked}, exactly, whatever produced it. For A000180 such a pair
exists with $r=1$:
\[
\sigma_{0}(n)=3,\qquad \sigma_{1}(n)=- \frac{1}{n + 1},
\qquad
R(n,k)\;=\;\frac{1}{k - n - 1} .
\]
...
For A000180 this gives
\begin{equation}\label{eq:h}
h(n)\;=\;\frac{\left(-1\right)^{n}}{n + 1} .
\end{equation}
...
\]
One extra order buys the homogeneity. Explicitly, $L=\left(\frac{3 n + 3}{n + 2}\right) + \left(\frac{3 n + 5}{n + 2}\right)\,N + \left(- \frac{1}{n + 2}\right)\,N^{2}$.
...
\[
(n + 1)\,a(n) -  \left(n + 1\right) \left(3 n - 1\right)\,a(n-1) -  3 \left(n - 1\right) \left(n + 1\right)\,a(n-2) \;=\;0
\]
...
all 16 values of $n$ from $n=2$ upward. The formula \eqref{eq:sum} was
...
\url{https://oeis.org}, 2026. Sequence A000180.
