...
\title{A proof of the conjectured recurrence for OEIS A124431}
...
\begin{abstract}
OEIS A124431 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A124431 is ``a(n) = Sum\_\{k=0..n\} 2\^{}k*C([(n+k)/2],k)*C([(n+k+1)/2],k) where [x]=floor(x)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 9, 29, 97, 331, 1145, 4001,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{\frac{x^{2} + 1}{x^{2} - 4 x + 1}} - 1}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) -2*(2*n+1)*a(n-1) +2*(n-1)*a(n-2) +2*(5-2*n)*a(n-3) +(n-3)*a(n-4) =0. - \emph{R. J. Mathar}, Aug 09 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 41) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(2 n + 1\right),\qquad p_{2}(n)=2 \left(n - 1\right),\qquad p_{3}(n)=- 2 \left(2 n - 5\right),\qquad p_{4}(n)=n - 3
\]
...
\[
(n + 1)\,a(n) -  2 \left(2 n + 1\right)\,a(n-1) + 2 \left(n - 1\right)\,a(n-2) -  2 \left(2 n - 5\right)\,a(n-3) + (n - 3)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A124431, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A124431.
