...
\title{The empirical recurrence for OEIS A251273, proved by transfer matrix}
...
\begin{abstract}
OEIS A251273 counts $7\times(n+1)$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
every $2\times2$ subblock, and carries an empirical recurrence of order $14$ contributed
...
on $S=128$ vertices whose edges are the admissible consecutive pairs, and the count is a
...
OEIS A251273 is ``Number of (6+1)X(n+1) 0..1 arrays with no 2X2 subblock having x11-x00 less than x10-x01.''. It has offset $1$ and begins
\[
3268, 37772, 273607, 1457326, 6247708, 22806904, 73607411, 215673388,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:20:24, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;15\,a(n-1) - 104\,a(n-2) + 442\,a(n-3) - 1287\,a(n-4) + 2717\,a(n-5) - 4290\,a(n-6) + 5148\,a(n-7) - 4719\,a(n-8) + 3289\,a(n-9) - 1716\,a(n-10) + 650\,a(n-11) - 169\,a(n-12) + 27\,a(n-13) - 2\,a(n-14)
\end{equation}
for all $n>13$.
...
$V=\{0,1,\dots,1\}^{7}$, so $|V|=S=128$. A $2\times2$ subblock of the array
...
For every $n$ with $n+1-1\ge14$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n+1-1-14}\,q(M)\,\mathbf{1} .
\]
...
=\mathbf{1}^{\!\top}M^{\,m-14}\,q(M)\,\mathbf{1}
\]
...
$m-14=j$ run over $\ge0$. For the last clause put $w=q(M)\mathbf{1}$. By the
...
$S^2$ pairs. The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was then formed by $14$
...
index corresponding to $n=13+1$ onwards, and the run continued until $w$ itself was the
zero vector, which settles every larger $j$ at once. By Lemma~\ref{lem:crit} the recurrence
holds for every $n>13$.
\end{proof}
...
every one of the $20$ published indices; the values agree exactly. That is what ties
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251273.
