...
\title{The empirical recurrence for OEIS A251270, proved by transfer matrix}
...
\begin{abstract}
OEIS A251270 counts $4\times(n+1)$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
every $2\times2$ subblock, and carries an empirical recurrence of order $8$ contributed
...
OEIS A251270 is ``Number of (3+1) X (n+1) 0..1 arrays with no 2 X 2 subblock having x11-x00 less than x10-x01.''. It has offset $1$ and begins
\[
108, 480, 1667, 4930, 13052, 31936, 73805, 163604,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 9*a(n-1) - 35*a(n-2) + 77*a(n-3) - 105*a(n-4) + 91*a(n-5) - 49*a(n-6) + 15*a(n-7) - 2*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 27 2018 11:58:40, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;9\,a(n-1) - 35\,a(n-2) + 77\,a(n-3) - 105\,a(n-4) + 91\,a(n-5) - 49\,a(n-6) + 15\,a(n-7) - 2\,a(n-8)
\end{equation}
for all $n>7$.
...
$V=\{0,1,\dots,1\}^{4}$, so $|V|=S=16$. A $2\times2$ subblock of the array
...
For every $n$ with $n+1-1\ge8$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n+1-1-8}\,q(M)\,\mathbf{1} .
\]
...
=\mathbf{1}^{\!\top}M^{\,m-8}\,q(M)\,\mathbf{1}
\]
...
$m-8=j$ run over $\ge0$. For the last clause put $w=q(M)\mathbf{1}$. By the
...
$S^2$ pairs. The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was then formed by $8$
...
index corresponding to $n=7+1$ onwards, and the run continued until $w$ itself was the
...
every one of the $24$ published indices; the values agree exactly. That is what ties
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251270.
