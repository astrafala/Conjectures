...
\title{A proof of the conjectured recurrence for OEIS A242566}
...
\begin{abstract}
OEIS A242566 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $- 18 x^{2} + 6 x$, a polynomial of degree $2$, which proves
...
OEIS A242566 is ``Expansion of (1-sqrt(1-(2*(1-sqrt(1-4*x\^{}2)))/x))/2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 1, 3, 7, 22, 67, 225,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1}{2} - \frac{\sqrt{\frac{x + 2 \sqrt{1 - 4 x^{2}} - 2}{x}}}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: 2*n*(2*n+1)*a(n) +(-49*n\^{}2+97*n-36)*a(n-1) +12*(10*n\^{}2-42*n+41)*a(n-2) +4*(49*n-97)*(n-3)*a(n-3) -544*(n-3)*(n-4)*a(n-4)=0. - \emph{R. J. Mathar}, Jan 25 2020
\end{quote}
As of the ``Last modified'' line on the live entry (2020-01-30, revision 51) the
...
\[
p_{0}(n)=2 n \left(2 n + 1\right),\qquad p_{1}(n)=- 49 n^{2} + 97 n - 36,\qquad p_{2}(n)=12 \left(10 n^{2} - 42 n + 41\right),\qquad p_{3}(n)=4 \left(n - 3\right) \left(49 n - 97\right),\qquad p_{4}(n)=- 544 \left(n - 4\right) \left(n - 3\right)
\]
...
\[
2 n \left(2 n + 1\right)\,a(n) + (- 49 n^{2} + 97 n - 36)\,a(n-1) + 12 \left(10 n^{2} - 42 n + 41\right)\,a(n-2) + 4 \left(n - 3\right) \left(49 n - 97\right)\,a(n-3) -  544 \left(n - 4\right) \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A242566, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 18 x^{2} + 6 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A242566.
