...
\title{The empirical column recurrences for the table OEIS A234555}
...
\begin{abstract}
OEIS A234555 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234555 is ``T(n,k) is the number of (n+1) X (k+1) 0..6 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
360, 2716, 2716, 20416, 22458, 20416, 153752, 184816,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 118*a(n-2) -3660*a(n-4) +10800*a(n-6).\\
k=2: [order 34].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 21:05:31, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..6 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;118\,a(n-2) - 3660\,a(n-4) + 10800\,a(n-6)
\]
holds for every $n>6$.
...
values, $70$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234555.
