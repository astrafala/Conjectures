...
\title{The reduction of OEIS A167137 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A167137 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
OEIS A167137 is ``E.g.f.: P(exp(x)-1) where P(x) is the g.f. of the partition numbers (A000041).''. It has offset $0$ and begins
\[
1, 1, 5, 31, 257, 2551, 30065, 407191,\ \dots
\]
...
A(x)\;=\;P\bigl(e^{x}-1\bigr),\quad P(t)=\prod_{k\ge 1}\frac{1}{1-t^{k}}.
\end{equation}
This is the only input taken from the entry, and it was checked against every one of the
$22$ terms of the entry's DATA section before use, not against a sample.

The entry separately carries the following comment:
\begin{quote}\small
\textbf{Conjecture}: Let k be a positive integer. The sequence obtained by reducing a(n) modulo k is eventually periodic with the period dividing phi(k) = A000010(k). For example, modulo 16 we obtain the sequence [1, 1, 5, 15, 1, 7, 1, 7, 1, 7, ...], with an apparent period of 2 beginning at a(4). --- \emph{Peter Bala}, Jul 07 2022
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 27 2024 13:55:48, revision
31) the statement is still recorded as a conjecture, and nothing on the entry
...
\begin{proof}
$G$ is the generating function of the partition numbers; each factor $1/(1-t^{k})$ is an integer series and only finitely many factors affect any one coefficient.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j=p(j)$, the partition numbers A000041, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 1, 2, 3, 5, 7, 11, 15, 22.
\]
...
Let $a(n)$ be OEIS A167137. For every integer $k\ge1$,
\[
...
against the entry's DATA at every one of the $22$ published indices; the values
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A167137.
