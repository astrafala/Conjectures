...
\title{The empirical recurrence for OEIS A207694, proved by transfer matrix}
...
\begin{abstract}
OEIS A207694 counts arrays over $\{0,\dots,1\}$ of width $3$ that avoid the patterns
$(0,0,1), (1,1,1)$ along one axis and $(0,0,0), (0,1,0)$ along the other, and carries an empirical recurrence of
order $4$ contributed by R.~H.~Hardin. It is true. The patterns have length $3$, so
...
$\mathbf{1}^{\!\top}P^{\,n-2-4}q(P)\mathbf{1}$, which vanishes identically
...
OEIS A207694 is ``Number of 3 X n 0..1 arrays avoiding 0 0 0 and 0 1 0 horizontally and 0 0 1 and 1 1 1 vertically.''. It has offset
$1$ and begins
\[
6, 36, 60, 100, 240, 576, 1296,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) + 3*a(n-3) + 9*a(n-4) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 05 2018 11:15:09, revision
8) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 3\,a(n-3) + 9\,a(n-4)
\end{equation}
for all $n>5$. The entry's array has a fixed number of rows and a growing number of
...
$(0,0,1), (1,1,1)$ as $3$ consecutive entries. There are $6$ admissible columns; that condition
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-4}$ whenever $n-4\ge2$. Hence
\eqref{eq:conj} holds for every $n>t$ exactly when $u_j=0$ for every
$j>t-2-4$. Moreover $(u_j)$ satisfies the monic linear recurrence whose
...
$P^{\,n-2-4}$ gives the identity. The equivalence follows by letting the exponent
...
\begin{proof}
The vector $q(P)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $4$ matrix--vector
...
compared against the entry's DATA at every one of the $28$ published indices; the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207694.
