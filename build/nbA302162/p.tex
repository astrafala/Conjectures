...
\title{The empirical recurrence for OEIS A302162, proved by transfer matrix}
...
\begin{abstract}
OEIS A302162 counts $7$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the six horizontal, diagonal and antidiagonal neighbours, a number $equal to$ itself lying in $\{0,1,3\}$, and it carries an empirical
recurrence of order $27$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A302162 is ``Number of nX7 0..1 arrays with every element equal to 0, 1 or 3 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
21, 185, 56, 206, 603, 1247, 2853, 6870,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 02 2018 14:48:40, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + a(n-2) + 11\,a(n-3) - 7\,a(n-4) - 7\,a(n-5) - 41\,a(n-6) + 26\,a(n-7) + 17\,a(n-8) + 74\,a(n-9) - 59\,a(n-10) - 19\,a(n-11) - 73\,a(n-12) + 82\,a(n-13) + 11\,a(n-14) + 44\,a(n-15) - 69\,a(n-16) - 5\,a(n-17) - 23\,a(n-18) + 34\,a(n-19) + 3\,a(n-20) + 14\,a(n-21) - 9\,a(n-22) - a(n-23) - 6\,a(n-24) + a(n-25) + a(n-27)
\end{equation}
for all $n>38$.
...
has score in $\{0,1,3\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,3\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,3\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,3\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,3\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-27}$ whenever $n-27\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-27$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-27}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $27$ matrix--vector
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302162.
