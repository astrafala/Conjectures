...
\title{The empirical recurrence for OEIS A206014, proved by transfer matrix}
...
\begin{abstract}
OEIS A206014 counts arrays over $\{0,\dots,3\}$ under a condition on the number of clockwise edge increases
in every $2\times2$ subblock, and carries an empirical recurrence of order $2$ contributed by
R.~H.~Hardin. It is true, and it is decidable rather than empirical. The condition is local to
a bounded window of consecutive rows, so the count is a walk count on $S=16$ states and is
...
OEIS A206014 is ``Number of (n+1) X 2 0..3 arrays with the number of clockwise edge increases in every 2 X 2 subblock unequal to the number of counterclockwise edge increases.''. It has offset $1$ and begins
\[
104, 680, 4440, 29000, 189400, 1237000, 8079000, 52765000,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-1) + 10*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 13 2018 08:28:56, revision 15) this is still
...
another exactly when all $1$ subblocks they form pass. An $(n+1)$-row array is a walk of $n$
steps and
\[
...
subblock; the entry's first term is $104$, which the model
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206014.
