...
\title{The empirical column recurrences for the table OEIS A264257}
...
\begin{abstract}
OEIS A264257 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264257 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 0,1 0,-1 0,2 1,0 or -1,0.''. It has offset $1$, is
read by antidiagonals, and begins
\[
4, 10, 9, 29, 33, 25, 89, 157,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 2*a(n-1) +2*a(n-2) -a(n-3)\\
k=2: a(n) = a(n-1) +16*a(n-2) +9*a(n-3) -29*a(n-4) -9*a(n-5) +16*a(n-6) -a(n-7) -a(n-8)\\
k=3: [order 33]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 12:00:47, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 0,1 0,-1 0,2 1,0 or -1,0.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having directed index change 0,1 0,-1 0,2 1,0 or -1,0.''. Its rows are the vertices of a digraph on
$S=64$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + 16\,a(n-2) + 9\,a(n-3) - 29\,a(n-4) - 9\,a(n-5) + 16\,a(n-6) - a(n-7) - a(n-8)
\]
...
model reproduces $18$ published values, $64$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264257.
