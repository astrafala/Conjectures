...
\title{The reduction of OEIS A316143 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A316143 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
\[
1, 2, 12, 92, 912, 10772, 148512, 2328692,\ \dots
\]
...
A(x)\;=\;\prod_{k\ge 1}\frac{1}{\bigl(1-(e^{x}-1)^{k}\bigr)^{2}}.
\end{equation}
This is the only input taken from the entry, and it was checked against every one of the
$20$ terms of the entry's DATA section before use, not against a sample.
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 28 2024 10:47:53, revision
20) the statement is still recorded as a conjecture, and nothing on the entry
...
\begin{proof}
$G$ is a product of integer series $1/(1-t^{k})^{2}$, only finitely many of which affect any one coefficient.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j = 1, 2, 5, 10, 20, 36, 65, \dots$, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 2, 5, 10, 20, 36, 65, 110, 185.
\]
...
Let $a(n)$ be OEIS A316143. For every integer $k\ge1$,
\[
...
against the entry's DATA at every one of the $20$ published indices; the values
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A316143.
