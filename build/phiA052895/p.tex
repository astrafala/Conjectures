...
\title{The reduction of OEIS A052895 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A052895 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
\[
1, 1, 5, 43, 545, 9211, 195305, 4990483,\ \dots
\]
...
A(x)\;=\;\frac{1}{2\,(e^{x}-1)}\Bigl(1-\sqrt{5-4e^{x}}\Bigr).
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture}: for fixed k = 1,2,..., the sequence a(n) (mod k) is eventually periodic with the exact period dividing phi(k), where phi(k) is the Euler totient function A000010. For example, modulo 10 the sequence becomes (1, 1, 5, 3, 5, 1, 5, 3, 5, ...), with an apparent period 1, 5, 3, 5 of length 4 = phi(10) beginning at a(1). (End) --- \emph{Peter Bala}, Jan 15 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 03 2026 04:50:46, revision
53) the statement is still recorded as a conjecture, and nothing on the entry
...
\[
5-4e^{x}=5-4(1+t)=1-4t,\qquad e^{x}-1=t,
\]
so that
...
\begin{proof}
$G$ is the ordinary generating function of the Catalan numbers, whose coefficients are integers.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j=\binom{2j}{j}/(j+1)$, the Catalan numbers A000108, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 1, 2, 5, 14, 42, 132, 429, 1430.
\]
...
Let $a(n)$ be OEIS A052895. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A052895.
