...
\title{The empirical recurrence for OEIS A304772, proved by transfer matrix}
...
\begin{abstract}
OEIS A304772 counts $5$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,3,7\}$, and it carries an empirical
recurrence of order $25$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A304772 is ``Number of nX5 0..1 arrays with every element unequal to 0, 1, 2, 3 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
16, 120, 139, 443, 1581, 5060, 15349, 50030,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 18 2018 08:12:58, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) - 4\,a(n-2) + 11\,a(n-3) - 29\,a(n-4) + 47\,a(n-5) - 79\,a(n-6) + 95\,a(n-7) - 99\,a(n-8) + 108\,a(n-9) - 43\,a(n-10) - 25\,a(n-11) + 69\,a(n-12) - 154\,a(n-13) + 158\,a(n-14) - 144\,a(n-15) + 133\,a(n-16) - 42\,a(n-17) + 5\,a(n-19) - 31\,a(n-20) + 45\,a(n-21) - 36\,a(n-22) + 6\,a(n-23) + 10\,a(n-24) - 4\,a(n-25)
\end{equation}
for all $n>27$.
...
has score in $\{0,1,2,3,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,7\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-25}$ whenever $n-25\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-25$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-25}$ out of
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>27$. This is the statement of
Section~1.
\end{theorem}

\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $25$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=27+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A304772.
