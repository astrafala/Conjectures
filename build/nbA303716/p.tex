...
\title{The empirical recurrence for OEIS A303716, proved by transfer matrix}
...
\begin{abstract}
OEIS A303716 counts $5$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,5\}$, and it carries an empirical
...
OEIS A303716 is ``Number of nX5 0..1 arrays with every element unequal to 0, 1 or 5 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
8, 7, 14, 36, 81, 169, 361, 784,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +a(n-2) +3*a(n-3) +a(n-4) -a(n-5) -a(n-6) for n>9
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 29 2018 10:52:45, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + a(n-2) + 3\,a(n-3) + a(n-4) - a(n-5) - a(n-6)
\end{equation}
for all $n>8$.
...
has score in $\{0,1,5\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,5\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,5\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,5\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,5\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303716.
