...
\title{The empirical recurrence for OEIS A233647, proved by transfer matrix}
...
\begin{abstract}
OEIS A233647 counts $(n+1)\times3$ arrays over $\{0,\dots,5\}$ subject to a condition imposed on
every $2\times2$ subblock, and carries an empirical recurrence of order $38$ contributed
...
OEIS A233647 is ``Number of (n+1)X(2+1) 0..5 arrays with every 2X2 subblock having the sum of the squares of the edge differences equal to 14.''. It has offset $1$ and begins
\[
772, 5232, 36784, 272564, 2012472, 15071680, 112180004, 840367588,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 08:55:08, revision 6) the statement is
...
For every $n$ with $n+1-1\ge38$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n+1-1-38}\,q(M)\,\mathbf{1} .
\]
...
=\mathbf{1}^{\!\top}M^{\,m-38}\,q(M)\,\mathbf{1}
\]
...
$m-38=j$ run over $\ge0$. For the last clause put $w=q(M)\mathbf{1}$. By the
...
$S^2$ pairs. The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was then formed by $38$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233647.
