...
\title{The empirical recurrence for OEIS A302220, proved by transfer matrix}
...
\begin{abstract}
OEIS A302220 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the six horizontal, diagonal and antidiagonal neighbours, a number $equal to$ itself lying in $\{1,2,3\}$, and it carries an empirical
recurrence of order $30$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A302220 is ``Number of nX4 0..1 arrays with every element equal to 1, 2 or 3 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
2, 45, 164, 934, 4237, 21777, 105585, 523414,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 03 2018 12:13:21, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) + 11\,a(n-2) - 42\,a(n-3) - 95\,a(n-4) + 21\,a(n-5) + 327\,a(n-6) + 1255\,a(n-7) + 317\,a(n-8) - 5188\,a(n-9) - 6921\,a(n-10) + 6005\,a(n-11) + 22780\,a(n-12) + 10157\,a(n-13) - 27962\,a(n-14) - 42374\,a(n-15) - 2355\,a(n-16) + 48767\,a(n-17) + 45664\,a(n-18) - 6331\,a(n-19) - 46138\,a(n-20) - 32422\,a(n-21) + 6577\,a(n-22) + 23809\,a(n-23) + 15150\,a(n-24) - 2372\,a(n-25) - 6308\,a(n-26) - 2794\,a(n-27) - 68\,a(n-28) + 832\,a(n-29) + 20\,a(n-30)
\end{equation}
for all $n>33$.
...
has score in $\{1,2,3\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,2,3\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,2,3\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,2,3\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,2,3\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-30}$ whenever $n-30\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-30$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-30}$ out of
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>33$. This is the statement of
Section~1.
\end{theorem}

\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $30$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=33+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302220.
