...
\title{The empirical recurrence for OEIS A251286, proved by transfer matrix}
...
\begin{abstract}
OEIS A251286 counts $(n+1)\times3$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
...
OEIS A251286 is ``Number of (n+1) X (2+1) 0..1 arrays with every 2 X 2 subblock having a single 1 or two 1s on the same edge or main diagonally.''. It has offset $1$ and begins
\[
22, 71, 219, 701, 2193, 6980, 21966, 69681,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) + 7*a(n-2) - 9*a(n-3) - 7*a(n-4) + 7*a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 28 2018 08:03:59, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 7\,a(n-2) - 9\,a(n-3) - 7\,a(n-4) + 7\,a(n-5)
\end{equation}
for all $n>6$.
...
\#\{x:x=1\}=1\ \text{ or the two }1\text{s are adjacent}\ \text{ or }\ (\alpha,\delta)=(1,1),
\end{equation}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251286.
