...
\title{The empirical recurrence for OEIS A305084, proved by transfer matrix}
...
\begin{abstract}
OEIS A305084 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{1,2,3,6,7\}$, and it carries an empirical
...
OEIS A305084 is ``Number of nX3 0..1 arrays with every element unequal to 1, 2, 3, 6 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 14, 22, 34, 182, 438, 920, 3431,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 25 2018 08:16:51, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 3\,a(n-2) + 12\,a(n-3) - 2\,a(n-4) - 36\,a(n-5) - 26\,a(n-6) + 9\,a(n-7) + 47\,a(n-8) + 11\,a(n-9) + 6\,a(n-10) - 2\,a(n-11) - 4\,a(n-12) - 16\,a(n-13) - 8\,a(n-14)
\end{equation}
for all $n>15$.
...
has score in $\{1,2,3,6,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,2,3,6,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,2,3,6,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,2,3,6,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,2,3,6,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A305084.
