...
\title{The empirical recurrence for OEIS A255084, proved by transfer matrix}
...
\begin{abstract}
OEIS A255084 counts $(n+2)\times3$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
no $3\times3$ subblock, and it carries an empirical recurrence of order $20$
...
$a(n)$ is a walk count on $S=64$ vertices. Such a sequence is $C$-finite, so whether a given
...
OEIS A255084 is ``Number of (n+2)X(1+2) 0..1 arrays with no 3x3 subblock diagonal sum 1 and no antidiagonal sum 2 and no row sum 0 and no column sum 0.''. It has offset $1$ and begins
\[
108, 265, 623, 1483, 3658, 8830, 21442, 51939,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:36:45, revision 6) this is still
...
$\{0,\dots,1\}^{3}$. A $3\times3$ subblock occupies three consecutive rows and
...
$S=(1+1)^{2\cdot3}=64$, and put an edge $(r,s)\to(s,t)$ exactly when the condition
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-2) + 4\,a(n-3) + 5\,a(n-4) + 5\,a(n-5) + a(n-6) - 14\,a(n-7) - 47\,a(n-8) - 17\,a(n-9) + 7\,a(n-10) + 29\,a(n-11) + 17\,a(n-12) + 26\,a(n-13) + 21\,a(n-14) - 10\,a(n-15) + 3\,a(n-16) - 2\,a(n-17) - 3\,a(n-18) + a(n-19) - a(n-20)
\]
for every $n>23$.
\end{theorem}

\begin{proof}
The vector $w=q(M)\mathbf{1}$ was formed by $20$ matrix--vector products in exact
integer arithmetic and $\mathbf{1}^{\!\top}M^{j}w$ evaluated for $j=0,1,\dots$, again
exactly. Those integers vanish from the index corresponding to $n=23+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>23$. The bound is exact: at $n=23$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A255084.
