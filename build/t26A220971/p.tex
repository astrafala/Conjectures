...
\title{The empirical recurrence for OEIS A220971, proved by transfer matrix}
...
\begin{abstract}
OEIS A220971 counts the distinct indicator arrays obtained from the $n\times2$ arrays over
...
indicator row by row turns it into one, on $S=15$ states, after which the conjectured
...
OEIS A220971 is ``Equals one maps: number of n X 2 binary arrays indicating the locations of corresponding elements equal to exactly one of their king-move neighbors in a random 0..1 n X 2 array.''. It has offset $1$ and begins
\[
2, 2, 9, 23, 58, 149, 379, 969,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 4*a(n-2) + 3*a(n-3) - 5*a(n-4) - 3*a(n-5) + 6*a(n-6) - 8*a(n-7) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 03 2018 08:12:13, revision 8) this is still
...
\]
There are $S=15$ reachable sets, generated from $D_0$ outward, and $a$ is $C$-finite. Note
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) - 4\,a(n-2) + 3\,a(n-3) - 5\,a(n-4) - 3\,a(n-5) + 6\,a(n-6) - 8\,a(n-7)
\]
for every $n>10$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220971.
