...
\title{The empirical column recurrences for the table OEIS A231806}
...
\begin{abstract}
OEIS A231806 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231806 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with no element having a strict majority of its horizontal, vertical, diagonal and antidiagonal neighbors equal to one.''. It has offset $1$, is
read by antidiagonals, and begins
\[
5, 13, 13, 32, 61, 32, 79, 252,\ \dots
\]
...
k=3: [order 45]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:42:34, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..1 arrays with no element having a strict majority of its horizontal, vertical, diagonal and antidiagonal neighbors equal to one.''. Its rows are the vertices of a digraph on
...
model reproduces $9$ published values, $25$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231806.
