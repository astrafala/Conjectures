...
\title{The empirical recurrence for OEIS A317379, proved by transfer matrix}
...
\begin{abstract}
OEIS A317379 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,5,6,7,8\}$, and it carries an empirical
...
OEIS A317379 is ``Number of nX4 0..1 arrays with every element unequal to 0, 1, 2, 5, 6, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
8, 9, 36, 123, 414, 1580, 5704, 20162,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2018 16:59:26, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 6\,a(n-2) + 4\,a(n-3) - 6\,a(n-4) - 32\,a(n-5) - 43\,a(n-6) + 7\,a(n-7) + 82\,a(n-8) + 59\,a(n-9) + 16\,a(n-10) + 10\,a(n-11) - 62\,a(n-12) - 69\,a(n-13) - 24\,a(n-14) - 7\,a(n-15) + 24\,a(n-16) + 34\,a(n-17) + 8\,a(n-18) - 8\,a(n-19)
\end{equation}
for all $n>22$.
...
has score in $\{0,1,2,5,6,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,5,6,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,5,6,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,5,6,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,5,6,7,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A317379.
