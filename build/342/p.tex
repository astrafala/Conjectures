...
\title{A proof of the conjectured recurrence for OEIS A093387}
...
\begin{abstract}
OEIS A093387 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 x^{3}$, a polynomial of degree $3$, which proves
...
OEIS A093387 is ``a(n) = 2\^{}(n-1) - binomial(n, floor(n/2))''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;0, 0, 1, 2, 6, 12, 29, 58,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;- \frac{\left(\sqrt{1 - 4 x^{2}} - 1\right)^{2}}{4 x \left(2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 2*(-n-1)*a(n-1) + 4*(-n+2)*a(n-2) + 8*(n-2)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 30 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-07-23, revision 67) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(n + 1\right),\qquad p_{2}(n)=- 4 \left(n - 2\right),\qquad p_{3}(n)=8 \left(n - 2\right)
\]
...
\section{The computation for A093387}
...
\[
(n + 1)\,a(n) -  2 \left(n + 1\right)\,a(n-1) -  4 \left(n - 2\right)\,a(n-2) + 8 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A093387, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=4 x^{3}$ is an
...
\url{https://oeis.org}, 2026. Sequence A093387.
