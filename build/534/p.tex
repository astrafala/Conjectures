...
\title{A proof of the conjectured recurrence for OEIS A010845}
...
\begin{abstract}
OEIS A010845 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A010845 is ``a(n) = 3*n*a(n-1) + 1, a(0) = 1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 25, 226, 2713, 40696, 732529, 15383110,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;- \frac{e^{x}}{3 x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) +(-3*n-1)*a(n-1) +3*(n-1)*a(n-2)=0. - \emph{R. J. Mathar}, Feb 16 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-07-02, revision 52) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- 3 n - 1,\qquad p_{2}(n)=3 \left(n - 1\right)
\]
...
\section{The computation for A010845}
...
\[
1\,a(n) + (- 3 n - 1)\,a(n-1) + 3 \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A010845, agreeing on all 13 terms compared.
...
vanish, for all 14 values of $n$ from $n=3$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A010845.
