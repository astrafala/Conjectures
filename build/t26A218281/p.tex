...
\title{The empirical recurrence for OEIS A218281, proved by transfer matrix}
...
\begin{abstract}
OEIS A218281 counts the distinct indicator arrays obtained from the $n\times1$ arrays over
$\{0,\dots,4\}$ by marking the cells whose value is at least as large as every one of their neighbours, and carries an
...
indicator row by row turns it into one, on $S=14$ states, after which the conjectured
...
OEIS A218281 is ``Hilltop maps: number of n X 1 binary arrays indicating the locations of corresponding elements not exceeded by any horizontal or vertical neighbor in a random 0..4 n X 1 array.''. It has offset $1$ and begins
\[
1, 3, 7, 15, 31, 61, 121, 241,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + a(n-2) + a(n-3) + a(n-4) + a(n-5) + a(n-6) + a(n-7) + a(n-8) + a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 24 2018 09:39:05, revision 7) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

Let $R=\{0,\dots,4\}^{1}$ be the set of rows and let the neighbours of a cell be the
...
\]
There are $S=14$ reachable sets, generated from $D_0$ outward, and $a$ is $C$-finite. Note
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A218281.
