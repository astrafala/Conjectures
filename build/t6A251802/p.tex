...
\title{The empirical recurrence for OEIS A251802, proved by transfer matrix}
...
\begin{abstract}
OEIS A251802 counts $3\times(n+1)$ arrays over $\{0,\dots,3\}$ subject to a condition imposed on
...
on $S=64$ vertices whose edges are the admissible consecutive pairs, and the count is a
...
OEIS A251802 is ``Number of (2+1)X(n+1) 0..3 arrays with no 2X2 subblock having x11-x00 less than x10-x01.''. It has offset $1$ and begins
\[
1740, 28932, 348679, 3352272, 27291608, 195753488, 1271711747, 7634258216,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:32:26, revision 6) the statement is
...
$V=\{0,1,\dots,3\}^{3}$, so $|V|=S=64$. A $2\times2$ subblock of the array
...
\begin{proof}
An array with $L$ columns and $3$ rows is precisely a sequence
...
index corresponding to $n=21+1$ onwards, and the run continued until $w$ itself was the
zero vector, which settles every larger $j$ at once. By Lemma~\ref{lem:crit} the recurrence
holds for every $n>21$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251802.
