...
\title{The empirical recurrence for OEIS A302514, proved by transfer matrix}
...
\begin{abstract}
OEIS A302514 counts $7$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the six horizontal, diagonal and antidiagonal neighbours, a number $equal to$ itself lying in $\{0,1,4,5\}$, and it carries an empirical
...
OEIS A302514 is ``Number of nX7 0..1 arrays with every element equal to 0, 1, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
21, 23, 118, 235, 689, 1784, 4558, 12324,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 09 2018 09:08:05, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 12\,a(n-3) + 11\,a(n-5) - 38\,a(n-6) - 5\,a(n-7) - 17\,a(n-8) + 27\,a(n-9) - 32\,a(n-10) + 12\,a(n-11) + 16\,a(n-12) + 24\,a(n-13) + 16\,a(n-15)
\end{equation}
for all $n>20$.
...
has score in $\{0,1,4,5\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,4,5\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,4,5\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,4,5\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,4,5\}$. Read the sequence as the
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>20$. This is the statement of
Section~1.
\end{theorem}
...
They vanish from the index corresponding to $n=20+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302514.
