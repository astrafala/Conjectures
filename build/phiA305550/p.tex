...
\title{The reduction of OEIS A305550 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A305550 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
\[
1, 1, 3, 19, 135, 1171, 12543, 156619,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
Expansion of e.g.f. Product\_{k>=1} (1 + (exp(x) - 1)\^{}k).
\end{quote}
that is, writing $A(x)=\sum_{n\ge 0}a(n)x^{n}/n!$,
\begin{equation}\label{eq:egf}
A(x)\;=\;\prod_{k\ge 1}\Bigl(1+(e^{x}-1)^{k}\Bigr).
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture}: Let k be a positive integer. The sequence obtained by reducing a(n) modulo k is eventually periodic with the period dividing phi(k) = A000010(k). For example, modulo 16 we obtain the sequence [1, 1, 3, 3, 7, 3, 15, 11, 7, 3, 15, 11, 7, 3, 15, 11, ...], with an apparent period of 4 beginning at a(4). Cf. A167137. --- \emph{Peter Bala}, Jul 08 2022
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:54, revision
74) the statement is still recorded as a conjecture, and nothing on the entry
...
G(t)\;=\;\prod_{k\ge 1}\bigl(1+t^{k}\bigr).
\end{equation}
...
\begin{proof}
$G$ is the generating function for partitions into distinct parts; each factor is a polynomial with integer coefficients and only finitely many affect any one coefficient.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j=q(j)$, partitions into distinct parts A000009, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 1, 1, 2, 2, 3, 4, 5, 6.
\]
...
Let $a(n)$ be OEIS A305550. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A305550.
