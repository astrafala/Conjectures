...
\title{The empirical recurrence for OEIS A298631, proved by transfer matrix}
...
\begin{abstract}
OEIS A298631 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{0,2,3,6,7\}$, and it carries an empirical
recurrence of order $23$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A298631 is ``Number of nX4 0..1 arrays with every element equal to 0, 2, 3, 6 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 17, 5, 15, 25, 45, 81, 186,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 23 2018 12:40:51, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) - 5\,a(n-3) + 5\,a(n-4) - 5\,a(n-5) - 6\,a(n-6) + 6\,a(n-7) + 20\,a(n-9) - 4\,a(n-10) - 2\,a(n-11) - 16\,a(n-12) - 9\,a(n-13) + 13\,a(n-14) + 8\,a(n-15) + 3\,a(n-16) - 18\,a(n-17) - 4\,a(n-18) + 9\,a(n-19) - 2\,a(n-20) + 8\,a(n-21) - a(n-22) - 2\,a(n-23)
\end{equation}
for all $n>28$.
...
has score in $\{0,2,3,6,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,2,3,6,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,2,3,6,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,2,3,6,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,2,3,6,7\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-23}$ whenever $n-23\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-23$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-23}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $23$ matrix--vector
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298631.
