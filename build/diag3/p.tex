...
$- 24 x$, of degree $1$, which proves the recurrence for every $n>1$.
\end{abstract}
...
\begin{quote}\small
\textbf{Conjecture:} 4*n*(2*n-1)*(17*n\^{}2 -52*n +39)*a(n) -(1207*n\^{}4 -4899*n\^{}3 +6692*n\^{}2 -3504*n +576)*a(n-1) +2*(n-2)*(2*n-3)*(17*n\^{}2 -18*n +4)*a(n-2) = 0. - \emph{R. J. Mathar}, Feb 05 2015 [the Maple command sumrecursion (binomial(n,k) * binomial(2*n+k-1,k), k, a(n)) verifies this recurrence. - \emph{Peter Bala}, Oct 05 2015 ]
\end{quote}
...
\[
4 n \left(2 n - 1\right) \left(17 n^{2} - 52 n + 39\right)\,a(n) + (- 1207 n^{4} + 4899 n^{3} - 6692 n^{2} + 3504 n - 576)\,a(n-1) + 2 \left(n - 2\right) \left(2 n - 3\right) \left(17 n^{2} - 18 n + 4\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
the identity $B=- 24 x$ is an identity of functions and the theorem holds for all
$n>1$ simultaneously.
