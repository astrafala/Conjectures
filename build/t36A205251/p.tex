...
\title{The empirical recurrence for OEIS A205251, proved by transfer matrix}
...
\begin{abstract}
OEIS A205251 counts arrays over $\{0,\dots,1\}$ under a condition on the number of clockwise edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=36$ states and is
...
OEIS A205251 is ``Number of (n+1) X 5 0..1 arrays with the number of clockwise edge increases in every 2 X 2 subblock the same.''. It has offset $1$ and begins
\[
328, 3416, 36304, 387328, 4136296, 44183032, 471988176, 5042151648,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 17*a(n-1) - 81*a(n-2) + 157*a(n-3) - 140*a(n-4) + 56*a(n-5) - 8*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 08:18:23, revision 8) this is still
...
\]
on the $S=36$ states that survive. Rows carrying no edge at all are dropped: such a row could
...
subblocks; the entry's first term is $328$, which the model
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205251.
