...
\title{Eventual periodicity modulo $m$ with period dividing $\varphi(m)$:\\a proof of Bala's conjecture on OEIS A354242}
...
OEIS A354242 is ``Expansion of e.g.f. 1/sqrt(5 - 4 * exp(x))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 14, 158, 2486, 50222, 1239254, 36126638,\ \dots
\]
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} Let k be a positive integer. The sequence obtained by reducing a(n) modulo k is eventually periodic with the period dividing phi(k) = A000010(k). For example, modulo 16 we obtain the sequence [1, 2, 14, 14, 6, 14, 6, 14, 6, ...], with an apparent period of 2 beginning at a(3). Cf. A354253.
\end{quote}
As of the ``Last modified'' line on the live entry (2023-11-16, revision 37) the
...
\section{Application to A354242}\label{sec:apply}

\begin{corollary}\label{cor:this}
The conjecture quoted in Section~1 is true: for every $m\ge1$ the sequence
A354242$(n) \bmod m$ is eventually periodic with period dividing $\varphi(m)$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:main} it suffices to exhibit the entry's exponential generating
function in the form $G(e^{x}-1)$ with $G$ integral. The entry gives both ``E.g.f.: $1/\sqrt{5-4\exp(x)}$'' and ``E.g.f.: $\sum_{k\ge0}\binom{2k}{k}(\exp(x)-1)^{k}$''. The second is already of the required shape; the first agrees with it because $5-4(1+y)=1-4y$. Therefore
\[
\sum_{n\ge0}a(n)\frac{x^{n}}{n!}=G(y),\qquad G(t)=\sum_{k\ge0}\binom{2k}{k}y^{k}=\frac{1}{\sqrt{1-4y}} ,
\]
whose coefficients are the central binomial coefficients, all integers. So $G\in\Z[[t]]$ and
Theorem~\ref{thm:main} applies.
\end{proof}

Concretely, the first coefficients of $G$ are
\[
c_{0},c_{1},c_{2},\dots \;=\; 1, 2, 6, 20, 70, 252, 924, \dots ,
\]
...
$18$ terms published on A354242 returns integers at every $k$, and the values
...
exact period  & $4$ & $6$ & $1$ & $6$ & $10$ & $1$ & $12$ & $2$ & $20$ & $18$\\
\bottomrule
\end{tabular}
\end{center}
...
\url{https://oeis.org}, 2026. Sequence A354242; conjecture of P.\ Bala.
