...
\title{A proof of the conjectured recurrence for OEIS A155587}
...
\begin{abstract}
OEIS A155587 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A155587 is ``Expansion of (1 + x*c(x))/(1 - x), where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 3, 5, 10, 24, 66, 198,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{1 - 4 x} - 3}{2 \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + (6-5*n)*a(n-1) + 2*(2*n-3)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2022-06-24, revision 39) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=6 - 5 n,\qquad p_{2}(n)=2 \left(2 n - 3\right)
\]
...
\section{The computation for A155587}
...
\[
n\,a(n) + (6 - 5 n)\,a(n-1) + 2 \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A155587, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A155587.
