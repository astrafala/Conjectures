...
\title{The empirical recurrence for OEIS A218804, proved by transfer matrix}
...
\begin{abstract}
OEIS A218804 counts the distinct indicator arrays obtained from the $n\times2$ arrays over
...
indicator row by row turns it into one, on $S=20$ states, after which the conjectured
...
OEIS A218804 is ``Hilltop maps: number of n X 2 binary arrays indicating the locations of corresponding elements not exceeded by any horizontal or antidiagonal neighbor in a random 0..3 n X 2 array.''. It has offset $1$ and begins
\[
3, 15, 57, 225, 895, 3551, 14089, 55905,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 3*a(n-2) + 3*a(n-3) + a(n-4) + a(n-5) + a(n-6) + a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 24 2018 10:03:31, revision 7) this is still
...
\]
There are $S=20$ reachable sets, generated from $D_0$ outward, and $a$ is $C$-finite. Note
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 3\,a(n-2) + 3\,a(n-3) + a(n-4) + a(n-5) + a(n-6) + a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A218804.
