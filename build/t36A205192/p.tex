...
\title{The empirical recurrence for OEIS A205192, proved by transfer matrix}
...
\begin{abstract}
OEIS A205192 counts arrays over $\{0,\dots,1\}$ under a condition on the number of clockwise edge increases
in every $2\times2$ subblock, and carries an empirical recurrence of order $8$ contributed by
R.~H.~Hardin. It is true, and it is decidable rather than empirical. The condition is local to
a bounded window of consecutive rows, so the count is a walk count on $S=432$ states and is
...
OEIS A205192 is ``Number of (n+1) X 8 0..1 arrays with the number of clockwise edge increases in every 2 X 2 subblock differing from each horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
432, 440, 708, 864, 1568, 2048, 3904, 4608,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-2) +a(n-8) for n>13.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 04:18:33, revision 8) this is still
...
\]
on the $S=432$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $7$
subblocks; the entry's first term is $432$, which the model
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205192.
