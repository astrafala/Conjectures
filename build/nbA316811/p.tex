...
\title{The empirical recurrence for OEIS A316811, proved by transfer matrix}
...
\begin{abstract}
OEIS A316811 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,3,4,5,7\}$, and it carries an empirical
...
OEIS A316811 is ``Number of nX4 0..1 arrays with every element unequal to 0, 1, 2, 3, 4, 5 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
8, 128, 1603, 19816, 246196, 3056047, 37935501, 470942175,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 14 2018 15:06:24, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;11\,a(n-1) + 33\,a(n-2) - 111\,a(n-3) - 963\,a(n-4) - 704\,a(n-5) + 2343\,a(n-6) + 3981\,a(n-7) + 490\,a(n-8) - 3271\,a(n-9) - 2446\,a(n-10) - 1376\,a(n-11) - 902\,a(n-12) + 600\,a(n-13) + 1658\,a(n-14) - 484\,a(n-15) - 668\,a(n-16) - 26\,a(n-17)
\end{equation}
for all $n>17$.
...
has score in $\{0,1,2,3,4,5,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,4,5,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,4,5,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,4,5,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,4,5,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A316811.
