...
\title{Eventual periodicity modulo $m$ with period dividing $\varphi(m)$:\\a proof of Bala's conjecture on OEIS A052895}
...
OEIS A052895 is ``Expansion of e.g.f. (1/2)/(exp(x) - 1) * (1 - (5 - 4*exp(x))\^{}(1/2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 5, 43, 545, 9211, 195305, 4990483,\ \dots
\]
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} for fixed k = 1,2,..., the sequence a(n) (mod k) is eventually periodic with the exact period dividing phi(k), where phi(k) is the Euler totient function A000010. For example, modulo 10 the sequence becomes (1, 1, 5, 3, 5, 1, 5, 3, 5, ...), with an apparent period 1, 5, 3, 5 of length 4 = phi(10) beginning at a(1). (End)
\end{quote}
As of the ``Last modified'' line on the live entry (2026-03-03, revision 53) the
...
\section{Application to A052895}\label{sec:apply}

\begin{corollary}\label{cor:this}
The conjecture quoted in Section~1 is true: for every $m\ge1$ the sequence
A052895$(n) \bmod m$ is eventually periodic with period dividing $\varphi(m)$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:main} it suffices to exhibit the entry's exponential generating
function in the form $G(e^{x}-1)$ with $G$ integral. The entry states ``E.g.f.: $C(\exp(x)-1)$, where $C(x)=(1-\sqrt{1-4x})/(2x)$ is the o.g.f.\ for A000108'', which is already of the required shape. Therefore
\[
\sum_{n\ge0}a(n)\frac{x^{n}}{n!}=G(y),\qquad G(t)=C(y),\quad C(t)=\frac{1-\sqrt{1-4t}}{2t} ,
\]
whose coefficients are the Catalan numbers, all integers. So $G\in\Z[[t]]$ and
Theorem~\ref{thm:main} applies.
\end{proof}

Concretely, the first coefficients of $G$ are
\[
c_{0},c_{1},c_{2},\dots \;=\; 1, 1, 2, 5, 14, 42, 132, \dots ,
\]
...
$18$ terms published on A052895 returns integers at every $k$, and the values
...
\url{https://oeis.org}, 2026. Sequence A052895; conjecture of P.\ Bala.
