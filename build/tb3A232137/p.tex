...
\title{The empirical column recurrences for the table OEIS A232137}
...
\begin{abstract}
OEIS A232137 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A232137 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with no element equal to a strict majority of its horizontal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
6, 36, 44, 200, 728, 328, 1140, 10956,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:54:12, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with no element equal to a strict majority of its horizontal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=84$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays with no element equal to a strict majority of its horizontal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=612$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;22\,a(n-1) - 36\,a(n-2) + 16\,a(n-3)
\]
holds from index $2+1$ on.

\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) 0..2 arrays with no element equal to a strict majority of its horizontal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=88$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $21$ published values, $92$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232137.
