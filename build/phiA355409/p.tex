...
\title{The reduction of OEIS A355409 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A355409 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
\[
1, 1, 7, 55, 571, 7471, 117307, 2148175,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 19 2024 04:35:29, revision
19) the statement is still recorded as a conjecture, and nothing on the entry
...
Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j = 1, 1, 3, 6, 13, 28, 60, \dots$, and the
first few are
\[
...
Let $a(n)$ be OEIS A355409. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A355409.
