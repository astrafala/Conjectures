...
\title{The empirical column recurrences for the table OEIS A251292}
...
\begin{abstract}
OEIS A251292 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A251292 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with every 2X2 subblock having a single 1 or two 1s on the same edge or main diagonally.''. It has offset $1$, is
read by antidiagonals, and begins
\[
9, 22, 22, 52, 71, 52, 125, 219,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 2*a(n-1) +2*a(n-2) -3*a(n-3) +a(n-4)\\
k=2: a(n) = 2*a(n-1) +7*a(n-2) -9*a(n-3) -7*a(n-4) +7*a(n-5) for n>6\\
k=3: [order 11]\\
k=4: [order 19]\\
k=5: [order 33] for n>34\\
k=6: [order 58] for n>59\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 13:14:56, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..1 arrays with every 2X2 subblock having a single 1 or two 1s on the same edge or main diagonally.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..1 arrays with every 2X2 subblock having a single 1 or two 1s on the same edge or main diagonally.''. Its rows are the vertices of a digraph on
...
values, $54$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251292.
