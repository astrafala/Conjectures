...
\title{The empirical column recurrences for the table OEIS A251373}
...
\begin{abstract}
OEIS A251373 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A251373 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every 2X2 subblock summing to 1 2 3 4 5 6 or 7.''. It has offset $1$, is
read by antidiagonals, and begins
\[
79, 695, 695, 6113, 17969, 6113, 53769, 464393,\ \dots
\]
...
k=5: [order 20]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:24:01, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with every 2X2 subblock summing to 1 2 3 4 5 6 or 7.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays with every 2X2 subblock summing to 1 2 3 4 5 6 or 7.''. Its rows are the vertices of a digraph on
$S=27$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;24\,a(n-1) + 49\,a(n-2) - 34\,a(n-3)
\]
holds for every $n>2$.
...
values, $76$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251373.
