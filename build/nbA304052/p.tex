...
\title{The empirical recurrence for OEIS A304052, proved by transfer matrix}
...
\begin{abstract}
OEIS A304052 counts $2$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{1,2,4,5\}$, and it carries an empirical
recurrence of order $5$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A304052 is ``Number of nX2 0..1 arrays with every element unequal to 1, 2, 4 or 5 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 3, 11, 10, 51, 165, 306, 993,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +3*a(n-2) +8*a(n-3) -4*a(n-4) -16*a(n-5) for n>6
\end{quote}
As of the ``Last modified'' line on the live entry (May 05 2018 15:17:21, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 3\,a(n-2) + 8\,a(n-3) - 4\,a(n-4) - 16\,a(n-5)
\end{equation}
for all $n>5$.
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-5}$ whenever $n-5\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-5$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-5}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $5$ matrix--vector
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A304052.
