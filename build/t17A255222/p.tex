...
\title{The empirical recurrence for OEIS A255222, proved by transfer matrix}
...
\begin{abstract}
OEIS A255222 counts $(n+2)\times4$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
...
OEIS A255222 is ``Number of (n+2) X (2+2) 0..1 arrays with no 3 x 3 subblock diagonal sum 0 and no antidiagonal sum 0 and no row sum 2 and no column sum 2.''. It has offset $1$ and begins
\[
22, 28, 37, 49, 67, 94, 133, 190,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - a(n-2) + a(n-3) - a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 19 2018 09:04:10, revision 11) this is still
...
exactly. Those integers vanish from the index corresponding to $n=4+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>4$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A255222.
