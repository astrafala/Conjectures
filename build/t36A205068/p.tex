...
\title{The empirical recurrence for OEIS A205068, proved by transfer matrix}
...
\begin{abstract}
OEIS A205068 counts arrays over $\{0,\dots,1\}$ under a condition on the number of rightwards and downwards edge increases
in every $2\times2$ subblock, and carries an empirical recurrence of order $24$ contributed by
R.~H.~Hardin. It is true, and it is decidable rather than empirical. The condition is local to
a bounded window of consecutive rows, so the count is a walk count on $S=238$ states and is
...
OEIS A205068 is ``Number of (n+1)X5 0..1 arrays with the number of rightwards and downwards edge increases in each 2X2 subblock differing from the number in all its horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
238, 146, 220, 376, 528, 784, 1192, 1792,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 11:45:05, revision 9) this is still
...
\]
on the $S=238$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $4$
subblocks; the entry's first term is $238$, which the model
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-2) - 2\,a(n-6) - 3\,a(n-8) - 2\,a(n-10) - 3\,a(n-12) + 2\,a(n-14) + 5\,a(n-16) + 6\,a(n-18) + 6\,a(n-20) + 3\,a(n-22) + a(n-24)
\]
for every $n>27$.
\end{theorem}
...
until the working vector was identically zero, which settles every larger $j$ at once. The bound is exact: at $n=27$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205068.
