...
\title{A proof of the conjectured recurrence for OEIS A242429}
...
\begin{abstract}
OEIS A242429 carries a conjectured linear recurrence with polynomial coefficients, of
order 4. The entry posts no generating function, so the usual route through a
...
therefore reduces to 3 rational-function identities, one for each similarity
...
OEIS A242429 is ``Length of longest chain of nonempty proper subsemigroups of the monoid of partial injective order-preserving functions of a chain with n elements''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 5, 17, 53, 167, 550, 1899, 6809,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = binomial(2*n,n)/2 + 3*2\^{}(n-1) - n - 2. - \emph{Gheorghe Coserea}, May 16 2016
\end{quote}
that is,
\begin{equation}\label{eq:cf}
a(n)\;=\;3 \cdot 2^{n - 1} - n + \frac{{\binom{2 n}{n}}}{2} - 2 .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} n*(131*n-376)*a(n) +2*(-563*n\^{}2+1993*n-1185)*a(n-1) +3*(1099*n\^{}2-4678*n+4684)*a(n-2) +2*(-1987*n\^{}2+9803*n-12021)*a(n-3) +4*(209*n-387)*(2*n-7)*a(n-4)=0. - \emph{R. J. Mathar}, Oct 20 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 26) the
...
n \left(131 n - 376\right)\,a(n) -  2 \left(563 n^{2} - 1993 n + 1185\right)\,a(n-1) + 3 \left(1099 n^{2} - 4678 n + 4684\right)\,a(n-2) -  2 \left(1987 n^{2} - 9803 n + 12021\right)\,a(n-3) + 4 \left(2 n - 7\right) \left(209 n - 387\right)\,a(n-4)
\end{equation}
...
\section{The computation for A242429}

The closed form \eqref{eq:cf} splits into 3 similarity classes, with
representativees
\[
-2,\qquad \frac{{\binom{2 n}{n}}}{2},\qquad \frac{3 \cdot 2^{n}}{2}
\]
Carrying out Proposition~\ref{prop:main} with the coefficients of \eqref{eq:conj} gives
\[
S_{1}(n)\;=\;0,\qquad S_{2}(n)\;=\;0,\qquad S_{3}(n)\;=\;0
\]
...
entry \href{https://oeis.org/A242429}{A242429}, 2025-11-05.
