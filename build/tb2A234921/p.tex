...
\title{The empirical column recurrences for the table OEIS A234921}
...
\begin{abstract}
OEIS A234921 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234921 is ``T(n,k) is the number of (n+1) X (k+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
40, 120, 120, 376, 360, 376, 1200, 1128,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 4*a(n-1) -2*a(n-2) -2*a(n-3) +a(n-4).\\
k=2: [order 13].\\
k=3: [order 19].\\
k=4: [order 54].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 20:44:54, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;4\,a(n-1) - 2\,a(n-2) - 2\,a(n-3) + a(n-4)
\]
holds for every $n>4$.
...
values, $38$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234921.
