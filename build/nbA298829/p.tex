...
\title{The empirical recurrence for OEIS A298829, proved by transfer matrix}
...
\begin{abstract}
OEIS A298829 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{1,3,4,5,8\}$, and it carries an empirical
...
OEIS A298829 is ``Number of nX3 0..1 arrays with every element equal to 1, 3, 4, 5 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 4, 1, 6, 10, 15, 60, 128,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 27 2018 07:31:54, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 3\,a(n-2) + 5\,a(n-3) - 22\,a(n-4) - 15\,a(n-5) + 17\,a(n-6) + 44\,a(n-7) - 13\,a(n-8) - 49\,a(n-9) + 74\,a(n-10) + 38\,a(n-11) - 91\,a(n-12) - 61\,a(n-13) + 42\,a(n-14) + 24\,a(n-15) - 4\,a(n-16) + a(n-17) - a(n-18) - a(n-19)
\end{equation}
for all $n>19$.
...
has score in $\{1,3,4,5,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,3,4,5,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,3,4,5,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,3,4,5,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,3,4,5,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298829.
