...
\title{A proof of the conjectured recurrence for OEIS A162477}
...
\begin{abstract}
OEIS A162477 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 3 x^{2} + 2 x + 1$, a polynomial of degree $2$, which proves
...
OEIS A162477 is ``Expansion of (1/(1-x)\^{}2)*c(x/(1-x)\^{}4) where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 11, 50, 255, 1391, 7939, 46821,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x - 1\right)^{2} - \sqrt{x^{4} - 4 x^{3} + 6 x^{2} - 8 x + 1}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +4*(1-2*n)*a(n-1) +6*(n-2)*a(n-2) +2*(7-2*n)*a(n-3) +(n-5)*a(n-4) = 0. - \emph{R. J. Mathar}, Nov 17 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2023-07-30, revision 25) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 4 \left(2 n - 1\right),\qquad p_{2}(n)=6 \left(n - 2\right),\qquad p_{3}(n)=- 2 \left(2 n - 7\right),\qquad p_{4}(n)=n - 5
\]
...
\[
(n + 1)\,a(n) -  4 \left(2 n - 1\right)\,a(n-1) + 6 \left(n - 2\right)\,a(n-2) -  2 \left(2 n - 7\right)\,a(n-3) + (n - 5)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A162477, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 3 x^{2} + 2 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A162477.
