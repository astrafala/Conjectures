...
\title{The empirical recurrence for OEIS A264507, proved by transfer matrix}
...
\begin{abstract}
OEIS A264507 counts the arrangements of $0,1,\dots,LW-1$ in a grid of $W=2$ columns in which
...
partly used. Carrying those as bitmasks makes the count a walk on $S=16$ vertices, so it is
...
OEIS A264507 is ``Number of (1+1) X (n+1) arrays of permutations of 0..n*2+1 with each element having directed index change -1,1 0,-1 0,1 or 1,0.''. It has offset $1$ and begins
\[
1, 1, 3, 6, 12, 24, 46, 91,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + a(n-2) - 3*a(n-3) + a(n-4) + 2*a(n-5) - a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 08 2019 08:19:43, revision 8) this is still
...
Hence $a$ is a walk count on $S=16$ vertices,
\[
...
Second, the transfer model reproduces all $34$ terms this entry publishes, exactly, in
integer arithmetic.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264507.
