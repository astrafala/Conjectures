...
\title{The empirical column recurrences for the table OEIS A256810}
...
\begin{abstract}
OEIS A256810 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A256810 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 0 and no antidiagonal sum 0 and no row sum 0 or 1 and no column sum 0 or 1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
32, 86, 86, 237, 312, 237, 641, 1167,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1) +a(n-2) +7*a(n-3) +7*a(n-4) +3*a(n-5) -a(n-6) -6*a(n-7) -2*a(n-8)\\
k=2: [order 14]\\
k=3: [order 28]\\
k=4: [order 52]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:46:13, revision 6) these are still
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with no 3x3 subblock diagonal sum 0 and no antidiagonal sum 0 and no row sum 0 or 1 and no column sum 0 or 1.''. Its rows are the vertices of a digraph on
...
model reproduces $10$ published values, $39$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A256810.
