...
\title{The empirical recurrence for OEIS A304269, proved by transfer matrix}
...
\begin{abstract}
OEIS A304269 counts $7$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,5,8\}$, and it carries an empirical
recurrence of order $24$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A304269 is ``Number of n X 7 0..1 arrays with every element unequal to 0, 1, 5 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
21, 23, 67, 259, 1074, 3788, 13767, 53839,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 30 2023 22:59:33, revision
6) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 2\,a(n-2) + 33\,a(n-3) + 20\,a(n-4) - 18\,a(n-5) - 257\,a(n-6) - 260\,a(n-7) - 44\,a(n-8) + 912\,a(n-9) + 798\,a(n-10) + 241\,a(n-11) - 1094\,a(n-12) - 868\,a(n-13) - 450\,a(n-14) + 667\,a(n-15) + 251\,a(n-16) + 158\,a(n-17) - 247\,a(n-18) - 19\,a(n-19) + 25\,a(n-20) + 81\,a(n-21) + 7\,a(n-22) - 8\,a(n-24)
\end{equation}
for all $n>24$.
...
has score in $\{0,1,5,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,5,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,5,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,5,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,5,8\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-24}$ whenever $n-24\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-24$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-24}$ out of
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>24$. This is the statement of
Section~1.
\end{theorem}

\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $24$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=24+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A304269.
