...
\title{A proof of the conjectured recurrence for OEIS A359643}
...
\begin{abstract}
OEIS A359643 carries a conjectured linear recurrence with polynomial coefficients, of
order 5. The entry posts no generating function; it defines the sequence by an
...
$0$, of degree $-1$, which proves the recurrence for every $n>-1$.
\end{abstract}
...
OEIS A359643 is ``a(n) = Sum\_\{k=0..n\} binomial(n,k) * binomial(4*k,k)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 5, 37, 317, 2885, 27105, 259765, 2523813,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = [x\^{}n] (1 + 5*x + 6*x\^{}2 + 4*x\^{}3 + x\^{}4)\^{}n. - \emph{Ilya Gutkovskiy}, Apr 17 2025
\end{quote}
that is,
\begin{equation}\label{eq:def}
a(n)\;=\;[x^{n}]\,f(x)\,g(x)^{n},
\qquad f(x)=1,\qquad g(x)=x^{4} + 4 x^{3} + 6 x^{2} + 5 x + 1 .
\end{equation}
...
\begin{quote}\small
Conjecture D-finite with recurrence +81*n*(3*n-1)*(3*n-2)*a(n) +3*(243*n\^{}3-8433*n\^{}2+14984*n-7064)*a(n-1) +2*(-58607*n\^{}3+297306*n\^{}2-491401*n+269124)*a(n-2) +6*(n-2)*(56663*n\^{}2-237722*n+252221)*a(n-3) -3*(n-2)*(n-3)*(111625*n-286402)*a(n-4) +110653*(n-2)*(n-3)*(n-4)*a(n-5)=0. - \emph{R. J. Mathar}, Jan 09 2023
\end{quote}
As of the ``Last modified'' line on the live entry (2025-04-17, revision 25) the
...
\[
283 t^{4} y^{4} - 876 t^{3} y^{4} + 930 t^{2} y^{4} - 18 t^{2} y^{2} - 364 t y^{4} + 36 t y^{2} + 8 t y + 27 y^{4} - 18 y^{2} - 8 y - 1 \;=\;0 ,
\]
...
\[
81 n \left(3 n - 2\right) \left(3 n - 1\right)\,a(n) + 3 \left(243 n^{3} - 8433 n^{2} + 14984 n - 7064\right)\,a(n-1) -  2 \left(58607 n^{3} - 297306 n^{2} + 491401 n - 269124\right)\,a(n-2) + 6 \left(n - 2\right) \left(56663 n^{2} - 237722 n + 252221\right)\,a(n-3) -  3 \left(n - 3\right) \left(n - 2\right) \left(111625 n - 286402\right)\,a(n-4) + 110653 \left(n - 4\right) \left(n - 3\right) \left(n - 2\right)\,a(n-5) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
The result was then compared term by term with the DATA section of A359643, agreeing on
...
all 17 values of $n$ from $n=5$ upward.
...
the identity $B=0$ is an identity of functions and the theorem holds for all
$n>-1$ simultaneously.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A359643.
