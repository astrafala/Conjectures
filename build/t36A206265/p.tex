...
\title{The empirical recurrence for OEIS A206265, proved by transfer matrix}
...
\begin{abstract}
OEIS A206265 counts arrays over $\{0,\dots,1\}$ under a condition on the number of rightwards and downwards edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=105$ states and is
...
OEIS A206265 is ``Number of (n+1) X 7 0..1 arrays with the number of rightwards and downwards edge increases in each 2 X 2 subblock equal to the number in all its horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
105, 128, 339, 800, 1725, 3440, 6444, 11448,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 7*a(n-1) - 20*a(n-2) + 28*a(n-3) - 14*a(n-4) - 14*a(n-5) + 28*a(n-6) - 20*a(n-7) + 7*a(n-8) - a(n-9) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 15 2018 08:20:25, revision 9) this is still
...
\]
on the $S=105$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $6$
subblocks; the entry's first term is $105$, which the model
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206265.
