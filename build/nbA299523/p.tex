...
\title{The empirical recurrence for OEIS A299523, proved by transfer matrix}
...
\begin{abstract}
OEIS A299523 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{1,3,4,5,6,8\}$, and it carries an empirical
...
OEIS A299523 is ``Number of n X 3 0..1 arrays with every element equal to 1, 3, 4, 5, 6 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 4, 1, 12, 35, 60, 269, 873,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 05 2022 19:41:35, revision
6) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 3\,a(n-2) + 18\,a(n-3) - 36\,a(n-4) - 45\,a(n-5) - 51\,a(n-6) + 111\,a(n-7) + 88\,a(n-8) + 94\,a(n-9) + 61\,a(n-10) - 89\,a(n-11) - 143\,a(n-12) - 109\,a(n-13) + 52\,a(n-14) + 11\,a(n-15) - 7\,a(n-16) + 3\,a(n-17)
\end{equation}
for all $n>17$.
...
has score in $\{1,3,4,5,6,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,3,4,5,6,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,3,4,5,6,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,3,4,5,6,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,3,4,5,6,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A299523.
