...
\title{The reduction of OEIS A378576 modulo $5$}
...
\begin{abstract}
OEIS A378576 is defined by the functional equation $A(x)=x+x\,A^{6}(x)$, where
$A^{6}$ is the $6$-th compositional iterate, and carries a conjecture of
P.~D.~Hanna that $a(n)\equiv1\pmod{5}$ for every $n\ge1$. It is true. The equation determines the coefficients one at a
time, with $a(N)$ entering with coefficient $1$, so it determines them modulo $5$ as
well. Modulo $5$ the series $x/(1-x)$ satisfies the equation exactly, because its
iterates are $x/(1-jx)$ and $6\equiv1$ modulo $5$. Uniqueness then identifies the
reduction. The argument is an identity between rational functions over
$\mathbb{Z}/5\mathbb{Z}$; nothing is truncated or estimated.
\end{abstract}
...
OEIS A378576 is ``G.f. A(x) satisfies A(x) = x + x*A(A(A(A(A(A(x)))))), so that this sequence shifts left under the 6th self-COMPOSE.''. It has offset $1$ and begins
\[
1, 1, 6, 66, 981, 17576, 359101,\ \dots
\]
...
\begin{quote}\small
Conjecture: a(n) == 1 (mod 5) for n >= 1. --- \emph{Paul D. Hanna}
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 13 2026 09:39:21, revision 20) the statement is
...
\begin{proof}
Comparing coefficients of $x$ in \eqref{eq:fe} gives $a(1)=1$, since $x\,A^{6}(x)$ has
no linear term. Let $N\ge2$. The left side contributes $a(N)$. On the right,
$\bigl[x^{N}\bigr]\bigl(x\,A^{6}\bigr)=\bigl[x^{N-1}\bigr]A^{6}$, which by
...
\section{The reduction modulo $5$}

Write $f(x)=\dfrac{x}{1-x}$ over $\mathbb{Z}/5\mathbb{Z}$.
...
Over $\mathbb{Z}/5\mathbb{Z}$, $f$ satisfies $f(x)=x+x\,f^{6}(x)$.
\end{lemma}

\begin{proof}
By Lemma~\ref{lem:iter}, $f^{6}(x)=\dfrac{x}{1-6x}$. Since $6\equiv1$ modulo
$5$ --- this is the whole content of the choice of modulus, as $5=6-1$ --- we have
$1-6x=1-x$ in $(\mathbb{Z}/5\mathbb{Z})[x]$, so $f^{6}(x)=\dfrac{x}{1-x}$.
Therefore
\[
x+x\,f^{6}(x)=x+\frac{x^{2}}{1-x}
=\frac{x(1-x)+x^{2}}{1-x}=\frac{x}{1-x}=f(x).
\]
\end{proof}

\begin{theorem}\label{thm:main}
Over $\mathbb{Z}/5\mathbb{Z}$ the reduction of $A$ is $x/(1-x)$; equivalently
$a(n)\equiv1\pmod{5}$ for every $n\ge1$. This is the conjecture of Section~1.
\end{theorem}
...
\begin{remark}
The proof used only $6\equiv1$ modulo $5$. The same argument shows that for every
...
to $n=24$ and every term reduced modulo $5$; all residues are $1$.

Third, both sides of \eqref{eq:fe} were expanded independently as formal power series over
$\mathbb{Z}/5\mathbb{Z}$ to order $30$, starting from the coefficients of $x/(1-x)$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A378576.
