...
\title{A proof of the conjectured recurrence for OEIS A000774}
...
\begin{abstract}
OEIS A000774 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A000774 is ``a(n) = n!*(1 + Sum\_\{i=1..n\} 1/i)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 17, 74, 394, 2484, 18108,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{\log{\left(1 - x \right)} - 1}{x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) +(-2*n+1)*a(n-1) +(n-1)\^{}2*a(n-2)=0. - \emph{R. J. Mathar}, Nov 26 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 102) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=1 - 2 n,\qquad p_{2}(n)=\left(n - 1\right)^{2}
\]
...
\section{The computation for A000774}
...
\[
1\,a(n) + (1 - 2 n)\,a(n-1) + (n^{2} - 2 n + 1)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A000774, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A000774.
