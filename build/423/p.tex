...
\title{A proof of the conjectured recurrence for OEIS A026327}
...
\begin{abstract}
OEIS A026327 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A026327 is ``a(n) = number of (s(0), s(1), ..., s(n)) such that s(i) is a nonnegative integer and |s(i) - s(i-1)| <= 1 for i = 1,2,...,n, s(0) = 2, s(n) = 4. Also a(n) = T(n,n-2), where T is the array in A026323''. It has offset $2$ and begins
\[
a(2),\dots,a(9)\;=\;1, 3, 10, 30, 90, 266, 783, 2295,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 2} a(n)\,x^{n}\;=\;\frac{\left(x + \sqrt{- 3 x^{2} - 2 x + 1} - 1\right)^{2} \left(x^{2} - x \left(x - 1\right) + x - 1\right)}{2 x^{2} \left(- 2 x^{2} \left(x - 1\right) + 2 x^{2} - 2 x \left(x - 1\right) - 2 x \left(x^{2} + x \left(1 - x\right) + x - 1\right) + 2 x + \left(x + \sqrt{- 3 x^{2} - 2 x + 1} - 1\right) \left(x^{2} + x \left(1 - x\right) + x - 1\right) - 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+8)*(n-2)*a(n) +3*(2*n\^{}2+7*n-28)*a(n-1) +3*(-3*n\^{}2-n+36)*a(n-2) -4*(n+4)*(n-1)*a(n-3) +12*(n-1)*(n-2)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 23 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2017-02-08, revision 17) the
...
\[
p_{0}(n)=- \left(n - 2\right) \left(n + 8\right),\qquad p_{1}(n)=3 \left(2 n^{2} + 7 n - 28\right),\qquad p_{2}(n)=- 3 \left(3 n^{2} + n - 36\right),\qquad p_{3}(n)=- 4 \left(n - 1\right) \left(n + 4\right),\qquad p_{4}(n)=12 \left(n - 2\right) \left(n - 1\right)
\]
...
\section{The computation for A026327}
...
\[
- \left(n - 2\right) \left(n + 8\right)\,a(n) + 3 \left(2 n^{2} + 7 n - 28\right)\,a(n-1) -  3 \left(3 n^{2} + n - 36\right)\,a(n-2) -  4 \left(n - 1\right) \left(n + 4\right)\,a(n-3) + 12 \left(n - 2\right) \left(n - 1\right)\,a(n-4) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A026327, agreeing on all 13 terms compared.
...
vanish, for all 23 values of $n$ from $n=6$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A026327.
