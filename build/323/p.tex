...
\begin{quote}\small
\textbf{Conjecture:} (-n+3)*a(n) +(n\^{}2-2*n-1)*a(n-1) +(-n+1)*a(n-2) -(n-1)*(n-2)\^{}2*a(n-3)=0. - \emph{R. J. Mathar}, Jan 04 2017
\end{quote}
...
\[
p_{0}(n)=3 - n,\qquad p_{1}(n)=n^{2} - 2 n - 1,\qquad p_{2}(n)=1 - n,\qquad p_{3}(n)=- \left(n - 2\right)^{2} \left(n - 1\right)
\]
...
\[
(3 - n)\,a(n) + (n^{2} - 2 n - 1)\,a(n-1) + (1 - n)\,a(n-2) -  \left(n - 2\right)^{2} \left(n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
