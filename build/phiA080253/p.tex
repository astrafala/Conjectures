...
\title{The reduction of OEIS A080253 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A080253 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
OEIS A080253 is ``a(n) is the number of elements in the Coxeter complex of type B\_n (or C\_n).''. It has offset $0$ and begins
\[
1, 3, 17, 147, 1697, 24483, 423857, 8560947,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
E.g.f.: exp(x)/(2-exp(2*x)).
\end{quote}
...
\[
e^{2x}=(1+t)^{2},\qquad 2-(1+t)^{2}=1-2t-t^{2},
\]
so that
\begin{equation}\label{eq:G}
A(x)\;=\;G\bigl(e^{x}-1\bigr),\qquad
G(t)\;=\;\frac{1+t}{1-2t-t^{2}}.
\end{equation}
...
Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j = 1, 3, 7, 17, 41, 99, \dots$, the half-companion Pell numbers A001333, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 3, 7, 17, 41, 99, 239, 577, 1393.
\]
...
Let $a(n)$ be OEIS A080253. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A080253.
