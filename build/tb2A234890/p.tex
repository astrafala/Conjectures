...
\title{The empirical column recurrences for the table OEIS A234890}
...
\begin{abstract}
OEIS A234890 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234890 is ``T(n,k) is the number of (n+1) X (k+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
80, 272, 272, 856, 980, 856, 2896, 3164,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 14*a(n-2) -37*a(n-4) +21*a(n-6).\\
k=2: [order 27].\\
k=3: [order 46].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 21:11:55, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;14\,a(n-2) - 37\,a(n-4) + 21\,a(n-6)
\]
holds for every $n>6$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234890.
