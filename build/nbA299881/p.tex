...
\title{The empirical recurrence for OEIS A299881, proved by transfer matrix}
...
\begin{abstract}
OEIS A299881 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{0,1,3,4,5,7,8\}$, and it carries an empirical
...
OEIS A299881 is ``Number of nX3 0..1 arrays with every element equal to 0, 1, 3, 4, 5, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
3, 4, 10, 28, 44, 222, 573, 1551,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 21 2018 11:57:35, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 3\,a(n-2) + 13\,a(n-3) - 27\,a(n-4) - 25\,a(n-5) - 51\,a(n-6) + 69\,a(n-7) + 57\,a(n-8) + 44\,a(n-9) + 75\,a(n-10) + 9\,a(n-11) - 109\,a(n-12) - 88\,a(n-13) - 49\,a(n-14) - 51\,a(n-15) + 19\,a(n-16) + 20\,a(n-17) + 2\,a(n-18)
\end{equation}
for all $n>18$.
...
has score in $\{0,1,3,4,5,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,3,4,5,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,3,4,5,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,3,4,5,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,3,4,5,7,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A299881.
