...
\begin{quote}\small
\textbf{Conjecture:} n*(n+2)*a(n) + (-5*n\^{}2-n+3)*a(n-1) + (10*n\^{}2-16*n+3)*a(n-2) + (-10*n\^{}2+34*n-27)*a(n-3) - (11*n-5)*(n-3)*a(n-4) + 15*(n-3)*(n-4)*a(n-5) = 0. - \emph{R. J. Mathar}, Feb 20 2015 [conjecture is true for n = 0..800; checked by \emph{Gennady Eremin}, Jan 25 2021]
\end{quote}
...
\[
p_{0}(n)=n \left(n + 2\right),\qquad p_{1}(n)=- 5 n^{2} - n + 3,\qquad p_{2}(n)=10 n^{2} - 16 n + 3,\qquad p_{3}(n)=- 10 n^{2} + 34 n - 27,\qquad p_{4}(n)=- \left(n - 3\right) \left(11 n - 5\right),\qquad p_{5}(n)=15 \left(n - 4\right) \left(n - 3\right)
\]
...
\[
n \left(n + 2\right)\,a(n) + (- 5 n^{2} - n + 3)\,a(n-1) + (10 n^{2} - 16 n + 3)\,a(n-2) + (- 10 n^{2} + 34 n - 27)\,a(n-3) -  \left(n - 3\right) \left(11 n - 5\right)\,a(n-4) + 15 \left(n - 4\right) \left(n - 3\right)\,a(n-5) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
