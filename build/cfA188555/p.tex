...
\title{The empirical closed form for OEIS A188555, proved}
...
\begin{abstract}
OEIS A188555 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
OEIS A188555 is ``Number of 4 X n binary arrays without the pattern 0 1 diagonally, vertically, antidiagonally or horizontally.''. It has offset $0$ and begins
\[
1, 5, 9, 16, 28, 48, 80, 129,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = (1/24)*n\^{}4 - (1/12)*n\^{}3 + (23/24)*n\^{}2 + (13/12)*n + 3.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 10 2024 20:45:01, revision 29) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;\frac{n^{4}}{24} - \frac{n^{3}}{12} + \frac{23 n^{2}}{24} + \frac{13 n}{12} + 3.
\]
...
vanish from the point corresponding to $n=7$ onwards, and the run was continued until the
working vector was identically zero, which settles every larger $j$ at once. So $a$ satisfies
\eqref{eq:rec} for every $n>6$. By Lemma~\ref{lem:ann} $f$ satisfies \eqref{eq:rec}
identically. Both were then evaluated in exact arithmetic at the $5$ consecutive indices
$n=7,\dots,11$ and agree at each. A monic recurrence of order $5$
determines $a(m)$ from $a(m-1),\dots,a(m-5)$, and for every $m\ge12$ both
$m>6$ and $m-5\ge7$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge7$.
Below $n=7$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=7$ on. The remaining indices $n=1,\dots,6$ were
therefore checked one at a time, directly against the walk count in exact integer arithmetic,
and agree.
\end{proof}

The range is tight in the sense that was checked: at $n=0$ the closed form and the sequence differ, so no larger range is available.

\section{Verification}

Four checks, each independent of the algebra above.

First, the walk model reproduces every one of the $43$ terms the entry publishes,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188555.
