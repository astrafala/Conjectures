...
\title{A proof of the conjectured recurrence for OEIS A081205}
...
\begin{abstract}
OEIS A081205 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A081205 is ``Staircase on Pascal's triangle''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 10, 20, 70, 126, 462, 792,\ \dots
\]
...
a(n)\;=\;{\binom{n + \left\lceil{\frac{n}{2} + \frac{1}{2}}\right\rceil + 1}{n}} .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} 4*n*(n-1)*(n+4)*(6*n\^{}2-9*n-1)*a(n) + 6*(n-1)*(27*n\^{}2+29*n+4)*a(n-1) - 3*n*(3*n-1)*(3*n+1)*(6*n\^{}2+3*n-4)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 19 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-12-07, revision 16) the
...
4 n \left(n - 1\right) \left(n + 4\right) \left(6 n^{2} - 9 n - 1\right)\,a(n) + 6 \left(n - 1\right) \left(27 n^{2} + 29 n + 4\right)\,a(n-1) -  3 n \left(3 n - 1\right) \left(3 n + 1\right) \left(6 n^{2} + 3 n - 4\right)\,a(n-2)
\end{equation}
...
\section{The computation for A081205}
...
entry \href{https://oeis.org/A081205}{A081205}, 2025-12-07.
