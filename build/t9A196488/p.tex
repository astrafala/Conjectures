...
\title{The empirical recurrence for OEIS A196488, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A196488 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A196488 is ``Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 2,3,0,1,4 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
1, 3, 9, 23, 69, 199, 553, 1563,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +a(n-2) +6*a(n-3) +10*a(n-4) +15*a(n-5) +9*a(n-6) +4*a(n-7) +a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (May 09 2018 08:16:53, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + a(n-2) + 6\,a(n-3) + 10\,a(n-4) + 15\,a(n-5) + 9\,a(n-6) + 4\,a(n-7) + a(n-8)
\end{equation}
for all $n>9$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(2,3,0,1,4)
\end{equation}
for every cell $(t,u)$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196488.
