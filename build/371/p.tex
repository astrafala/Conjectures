...
\title{A proof of the conjectured recurrence for OEIS A162972}
...
\begin{abstract}
OEIS A162972 carries a conjectured linear recurrence with polynomial coefficients, of
order 7. We prove it. The entry's exponential generating function is
...
OEIS A162972 is ``Number of cycles in all non-derangement permutations of \{1,2,...,n\}''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 9, 38, 210, 1339, 9870, 82368,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{\left(- x - \left(1 - e^{x}\right) \log{\left(1 - x \right)}\right) e^{- x}}{x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence a(n) +(-3*n+8)*a(n-1) +(n-3)*(3*n-14)*a(n-2) +(-n\^{}3+19*n\^{}2-98*n+156)*a(n-3) +(-4*n\^{}3+56*n\^{}2-258*n+395)*a(n-4) +(-6*n\^{}3+84*n\^{}2-395*n+624)*a(n-5) -(n-5)*(4*n\^{}2-41*n+106)*a(n-6) -(n-5)*(n-6)\^{}2*a(n-7)=0. - \emph{R. J. Mathar}, Jul 26 2022
\end{quote}
As of the ``Last modified'' line on the live entry (2022-07-26, revision 15) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=8 - 3 n,\qquad p_{2}(n)=\left(n - 3\right) \left(3 n - 14\right),\qquad p_{3}(n)=- n^{3} + 19 n^{2} - 98 n + 156,\qquad p_{4}(n)=- 4 n^{3} + 56 n^{2} - 258 n + 395,\qquad p_{5}(n)=- 6 n^{3} + 84 n^{2} - 395 n + 624,\qquad p_{6}(n)=- \left(n - 5\right) \left(4 n^{2} - 41 n + 106\right),\qquad p_{7}(n)=- \left(n - 6\right)^{2} \left(n - 5\right)
\]
...
\section{The computation for A162972}
...
\[
1\,a(n) + (8 - 3 n)\,a(n-1) + \left(n - 3\right) \left(3 n - 14\right)\,a(n-2) + (- n^{3} + 19 n^{2} - 98 n + 156)\,a(n-3) + (- 4 n^{3} + 56 n^{2} - 258 n + 395)\,a(n-4) + (- 6 n^{3} + 84 n^{2} - 395 n + 624)\,a(n-5) -  \left(n - 5\right) \left(4 n^{2} - 41 n + 106\right)\,a(n-6) -  \left(n - 6\right)^{2} \left(n - 5\right)\,a(n-7) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A162972, agreeing on all 13 terms compared.
...
vanish, for all 15 values of $n$ from $n=8$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A162972.
