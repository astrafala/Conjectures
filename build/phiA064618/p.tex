...
\title{The reduction of OEIS A064618 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A064618 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 14 2022 11:48:24, revision
...
\begin{proof}
$G$ is the hypergeometric series ${}_{2}F_{0}(1,1;\,;t)=\sum_j (1)_j (1)_j t^{j}/j! =\sum_j j!\,t^{j}$, whose coefficients are factorials and so integers.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j=j!$, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 1, 2, 6, 24, 120, 720, 5040, 40320.
\]
...
Let $a(n)$ be OEIS A064618. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A064618.
