...
\title{A proof of the conjectured recurrence for OEIS A066534}
...
\begin{abstract}
OEIS A066534 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A066534 is ``Total number of walks with length > 0 in the Hasse diagram of a Boolean algebra of order n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 6, 30, 152, 840, 5232, 37072,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;- \frac{x e^{2 x}}{x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) + (-n-4)*a(n-1) + 4*(n)*a(n-2) + 4*(-n+2)*a(n-3) = 0. - \emph{R. J. Mathar}, Dec 04 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-08-29, revision 41) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- n - 4,\qquad p_{2}(n)=4 n,\qquad p_{3}(n)=- 4 \left(n - 2\right)
\]
...
\section{The computation for A066534}
...
\[
1\,a(n) + (- n - 4)\,a(n-1) + 4 n\,a(n-2) -  4 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A066534, agreeing on all 13 terms compared.
...
vanish, for all 16 values of $n$ from $n=4$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A066534.
