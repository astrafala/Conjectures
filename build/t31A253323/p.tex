...
\title{The empirical recurrence for OEIS A253323, proved by transfer matrix}
...
\begin{abstract}
OEIS A253323 counts the arrays over $\{0,\dots,1\}$ whose $2\times2$ subblocks carry a
...
so a pair of consecutive array rows is a state, the count is a walk count on $S=676$ of them,
...
OEIS A253323 is ``Number of (n+1)X(5+1) 0..1 arrays with every 2X2 subblock sum nondecreasing horizontally, vertically and antidiagonally ne-to-sw.''. It has offset $1$ and begins
\[
676, 2797, 11506, 23153, 58505, 98741, 210417, 349857,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 13:57:28, revision 6) this is still
...
\]
on the $S=676$ admissible pairs. In particular $a$ is $C$-finite. The alignment of the walk
...
until the working vector was identically zero, which settles every larger $j$ at once. The bound is exact: at $n=22$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253323.
