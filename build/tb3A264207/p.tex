...
\title{The empirical column recurrences for the table OEIS A264207}
...
\begin{abstract}
OEIS A264207 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264207 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having index change +-(.,.) 0,0 0,2 or 1,0.''. It has offset $1$, is
read by antidiagonals, and begins
\[
4, 18, 9, 81, 96, 25, 288, 1024,\ \dots
\]
...
k=5: [order 51]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 11:37:44, revision 6) these are still
...
column $k$$=1$ & $3$ & $16$ & $>1$ & $8$ \\
column $k$$=2$ & $6$ & $64$ & $>4$ & $10$ \\
column $k$$=3$ & $5$ & $256$ & $>3$ & $9$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having index change +-(.,.) 0,0 0,2 or 1,0.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having index change +-(.,.) 0,0 0,2 or 1,0.''. Its rows are the vertices of a digraph on
$S=64$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1) + 18\,a(n-2) + 5\,a(n-3) - 18\,a(n-4) + 3\,a(n-5) + a(n-6)
\]
holds from index $4+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+1)X(4) arrays of permutations of 0..(n+1)*(4)-1 with each element having index change +-(.,.) 0,0 0,2 or 1,0.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;11\,a(n-1) + 44\,a(n-2) - 44\,a(n-3) - 11\,a(n-4) + a(n-5)
\]
...
model reproduces $27$ published values, $135$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264207.
