...
\title{A proof of the conjectured recurrence for OEIS A005561}
...
\begin{abstract}
OEIS A005561 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A005561 is ``Number of walks on square lattice. Column y=3 of A052174''. It has offset $3$ and begins
\[
a(3),\dots,a(10)\;=\;1, 4, 24, 84, 392, 1344, 5760, 19800,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = C(n+4, ceiling(n/2))*C(n+3, floor(n/2)) - C(n+4, ceiling((n-1)/2))*C(n+3, floor((n-1)/2)). - \emph{Paul D. Hanna}, Apr 16 2004
\end{quote}
that is,
\begin{equation}\label{eq:cf}
a(n)\;=\;- {\binom{n}{\left\lfloor{\frac{n}{2}}\right\rfloor - 2}} {\binom{n + 1}{\left\lceil{\frac{n}{2}}\right\rceil - 2}} + {\binom{n}{\left\lfloor{\frac{n}{2} - \frac{3}{2}}\right\rfloor}} {\binom{n + 1}{\left\lceil{\frac{n}{2} - \frac{3}{2}}\right\rceil}} .
\end{equation}
...
assumed: the formula as posted gives $a(n+3)$, and is used here shifted accordingly.

The entry separately carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n-2)*(n-3)*(2*n+1)*(n+6)*(n+5)*a(n) - 4*n*(n+1)*(2*n\^{}2+4*n+33)*a(n-1) - 16*n\^{}2*(n-1)*(2*n+3)*(n+1)*a(n-2) = 0. - \emph{R. J. Mathar}, Apr 02 2017
\end{quote}
...
\left(n - 3\right) \left(n - 2\right) \left(n + 5\right) \left(n + 6\right) \left(2 n + 1\right)\,a(n) -  4 n \left(n + 1\right) \left(2 n^{2} + 4 n + 33\right)\,a(n-1) -  16 n^{2} \left(n - 1\right) \left(n + 1\right) \left(2 n + 3\right)\,a(n-2)
\end{equation}
...
a(2m)\;=\;e_{0}(m)\;=\;\frac{4 {\binom{2 m + 1}{m + 2}} \left(2 m\right)!}{\left(m - 2\right)! \left(m + 3\right)!},
\qquad
a(2m+1)\;=\;e_{1}(m)\;=\;\frac{8 \left(2 m + 1\right)!^{2}}{\left(m + 2\right) \left(m + 3\right) m! \left(m - 1\right)!^{2} \left(m + 3\right)!},
\end{equation}
...
\section{The computation for A005561}
...
entry \href{https://oeis.org/A005561}{A005561}, 2025-02-03.
