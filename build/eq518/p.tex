...
\title{A proof of the conjectured generating function for OEIS A082975}
...
\begin{abstract}
OEIS A082975 carries a conjectured generating function. We prove it. The entry states, as fact,
...
of order $10$ whose leading coefficient does not vanish, together with $10$
...
OEIS A082975 is ``Denominators of continued fraction convergents to (sqrt(37)-4)/3''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 10, 13, 36, 121, 157,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
(1 + x + 3*x\textasciicircum{}2 - 2*x\textasciicircum{}3 + x\textasciicircum{}4) / (1 - 12*x\textasciicircum{}3 - x\textasciicircum{}6)
\end{quote}
and separately carries the comment
\begin{quote}\small
g.f.: 1 + x*(1+3*x+10*x\textasciicircum{}2+x\textasciicircum{}3+x\textasciicircum{}5)/(1-12*x\textasciicircum{}3-x\textasciicircum{}6). - \emph{Colin Barker}, Jan 04 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2017-12-07, revision 32) the second
...
\left(n + 2\right)\,a(n) + \left(- 2 n - 8\right)\,a(n+1) + \left(3 n + 18\right)\,a(n+2) + \left(13 n + 32\right)\,a(n+3) + \left(- 23 n - 86\right)\,a(n+4) + \left(36 n + 216\right)\,a(n+5) + \left(11 n + 94\right)\,a(n+6) + \left(14 n + 128\right)\,a(n+7) + \left(- 3 n - 18\right)\,a(n+8) + \left(- n - 8\right)\,a(n+9) + \left(- n - 10\right)\,a(n+10) \;=\; 0
\end{equation}

The leading coefficient of \eqref{eq:rec} is $- n - 10$. Its only integer zeros are $n\in\{-10\}$, all below the range claimed here.
...
For all $n\ge 0$,
\[
\sum_{n\ge 0} a(n)x^{n} \;=\; \frac{- x^{4} + 2 x^{3} - 3 x^{2} - x - 1}{x^{6} + 12 x^{3} - 1}
\]
\end{theorem}

\begin{proof}
Both sides satisfy \eqref{eq:rec}: the sequence because the recurrence was derived from
the entry's own a generating function, and the conjectured generating function by the computation of the
previous section. They agree at the $10$ consecutive indices beginning
$n=0$, which was checked against the entry's DATA in exact integer arithmetic.
The leading coefficient has no zero at any integer $n\ge0$, so
...
\url{https://oeis.org}, 2026. Sequence A082975.
