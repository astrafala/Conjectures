...
\title{The empirical recurrence for OEIS A303804, proved by transfer matrix}
...
\begin{abstract}
OEIS A303804 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,3,4\}$, and it carries an empirical
...
OEIS A303804 is ``Number of nX4 0..1 arrays with every element unequal to 0, 1, 3 or 4 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
5, 17, 15, 38, 55, 107, 202, 438,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 30 2018 17:19:58, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) - 4\,a(n-2) - a(n-3) + 9\,a(n-4) - 25\,a(n-5) + 34\,a(n-6) - 21\,a(n-7) - 17\,a(n-8) + 20\,a(n-9) - 9\,a(n-10) + 80\,a(n-11) - 82\,a(n-12) + 5\,a(n-13) + 115\,a(n-14) - 65\,a(n-15) - 48\,a(n-16) + 9\,a(n-17) - 5\,a(n-18) + 2\,a(n-20)
\end{equation}
for all $n>23$.
...
has score in $\{0,1,3,4\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,3,4\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,3,4\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,3,4\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,3,4\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303804.
