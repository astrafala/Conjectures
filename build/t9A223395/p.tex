...
\title{The empirical recurrence for OEIS A223395, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A223395 counts arrays over $\{0,\dots,15\}$ in which every cell is constrained by the
...
OEIS A223395 is ``4 X 4 square grid graph coloring a rectangular array: number of n X 1 0..15 arrays where 0..15 label nodes of the square grid graph and every array movement to a horizontal or vertical neighbor moves along an edge of this graph.''. It has offset $1$ and begins
\[
16, 48, 152, 488, 1576, 5096, 16488, 53352,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) + 2*a(n-2) - 4*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 20 2018 08:50:40, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 2\,a(n-2) - 4\,a(n-3)
\end{equation}
for all $n>4$.
...
\{x_{t,u},x_{t+d_1,u+d_2}\}\in E\ \text{ for every offset in }\mathcal N,\quad E=\text{the 4\times4 grid graph}
\end{equation}
for every cell $(t,u)$.
...
Let $V=\{0,\dots,15\}^{1}$ be the possible rows and take
...
of the $(15+1)^{1}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223395.
