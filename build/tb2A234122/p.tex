...
\title{The empirical column recurrences for the table OEIS A234122}
...
\begin{abstract}
OEIS A234122 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234122 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every 2X2 subblock having the absolute values of all six edge and diagonal differences no larger than 1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
31, 145, 145, 673, 1361, 673, 3127, 12593,\ \dots
\]
...
k=7: [order 62]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 09:12:23, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with every 2X2 subblock having the absolute values of all six edge and diagonal differences no larger than 1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays with every 2X2 subblock having the absolute values of all six edge and diagonal differences no larger than 1.''. Its rows are the vertices of a digraph on
$S=27$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;10\,a(n-1) - 4\,a(n-2) - 26\,a(n-3) + 5\,a(n-4)
\]
holds for every $n>4$.

\subsection*{Column $k=3$}
The sequence is ``Number of (n+1)X(4) 0..2 arrays with every 2X2 subblock having the absolute values of all six edge and diagonal differences no larger than 1.''. Its rows are the vertices of a digraph on
$S=81$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;20\,a(n-1) - 10\,a(n-2) - 324\,a(n-3) - 277\,a(n-4) + 144\,a(n-5)
\]
holds for every $n>5$.
...
different counts. Across the columns settled here the model reproduces $25$ published
values, $166$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234122.
