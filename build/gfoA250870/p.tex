...
\title{The conjectured generating function for OEIS A250870, proved}
...
\begin{abstract}
OEIS A250870 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=16$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $24$ coefficients in exact arithmetic
...
OEIS A250870 is ``Number of (n+1) X (1+1) 0..3 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing x(i,j)+x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
90, 440, 2002, 8736, 37130, 155080,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: 2*x*(45 - 140*x + 96*x\^{}2) / ((1 - x)*(1 - 3*x)*(1 - 4*x)). - \_Colin Barker\_, Nov 22 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 22 2018 08:23:32, revision 7) this is still
...
\]
and there are $S=16$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $19$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{19}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $24$
of them --- more than the $19+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{3}$ read off $D_2$, the first few being $8,\ -19,\ 12$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250870.
