...
\title{The empirical recurrence for OEIS A184047, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184047 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A184047 is ``1/9 the number of (n+1) X 9 0..2 arrays with all 2 X 2 subblocks having the same four values.''. It has offset $1$ and begins
\[
561, 567, 577, 597, 633, 705, 841, 1113,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 14:16:50, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2232 x^{4} + 1124 x^{3} + 1116 x^{2} - 561 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2232 x^{4} - 1124 x^{3} - 1116 x^{2} + 561 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184047.
