...
\title{The empirical recurrence for OEIS A218313, proved by transfer matrix}
...
\begin{abstract}
OEIS A218313 counts the distinct indicator arrays obtained from the $n\times2$ arrays over
...
indicator row by row turns it into one, on $S=12$ states, after which the conjectured
...
OEIS A218313 is ``Hilltop maps: number of n X 2 binary arrays indicating the locations of corresponding elements not exceeded by any king-move neighbor in a random 0..2 n X 2 array.''. It has offset $1$ and begins
\[
3, 15, 63, 249, 993, 3969, 15867, 63423,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 3*a(n-2) + 3*a(n-3) + 3*a(n-4) + 3*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 10 2018 04:52:58, revision 8) this is still
...
\]
There are $S=12$ reachable sets, generated from $D_0$ outward, and $a$ is $C$-finite. Note
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 3\,a(n-2) + 3\,a(n-3) + 3\,a(n-4) + 3\,a(n-5)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A218313.
