...
\title{The empirical recurrence for OEIS A304699, proved by transfer matrix}
...
\begin{abstract}
OEIS A304699 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{1,2,4,5,6\}$, and it carries an empirical
...
OEIS A304699 is ``Number of nX3 0..1 arrays with every element unequal to 1, 2, 4, 5 or 6 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 11, 8, 21, 46, 103, 201, 512,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 17 2018 10:00:12, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 2\,a(n-2) + 10\,a(n-3) - a(n-4) - 20\,a(n-5) - 48\,a(n-6) - 31\,a(n-7) + 56\,a(n-8) + 107\,a(n-9) + 109\,a(n-10) - 30\,a(n-11) - 92\,a(n-12) - 70\,a(n-13) - 9\,a(n-14) - 35\,a(n-15) - 32\,a(n-16) - 4\,a(n-17)
\end{equation}
for all $n>19$.
...
has score in $\{1,2,4,5,6\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,2,4,5,6\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,2,4,5,6\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,2,4,5,6\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,2,4,5,6\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A304699.
