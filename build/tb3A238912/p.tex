...
\title{The empirical column recurrences for the table OEIS A238912}
...
\begin{abstract}
OEIS A238912 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A238912 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with no element equal to all horizontal neighbors or equal to all vertical neighbors, and new values 0..2 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
3, 9, 9, 54, 41, 54, 261, 486,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:55:52, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with no element equal to all horizontal neighbors or equal to all vertical neighbors, and new values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A238912.
