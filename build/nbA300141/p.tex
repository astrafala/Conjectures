...
\title{The empirical recurrence for OEIS A300141, proved by transfer matrix}
...
\begin{abstract}
OEIS A300141 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{1,3,4,5,6,7,8\}$, and it carries an empirical
...
OEIS A300141 is ``Number of nX3 0..1 arrays with every element equal to 1, 3, 4, 5, 6, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 4, 1, 18, 41, 130, 510, 1740,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 26 2018 08:08:02, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 3\,a(n-2) + 18\,a(n-3) - 21\,a(n-4) - 54\,a(n-5) - 54\,a(n-6) + 79\,a(n-7) + 119\,a(n-8) + 4\,a(n-9) - 3\,a(n-10) - a(n-11) + 26\,a(n-12) - 53\,a(n-13) - 17\,a(n-14) + 15\,a(n-15)
\end{equation}
for all $n>16$.
...
has score in $\{1,3,4,5,6,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,3,4,5,6,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,3,4,5,6,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,3,4,5,6,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,3,4,5,6,7,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A300141.
