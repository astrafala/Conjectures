...
\title{The empirical recurrence for OEIS A233163, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233163 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 09 2018 14:55:44, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{288 x^{3} - 64 x^{2} + 3 x}{128 x^{2} - 24 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;128 x^{2} - 24 x + 1
\]
