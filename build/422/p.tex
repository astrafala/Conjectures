...
\title{A proof of the conjectured recurrence for OEIS A026017}
...
\begin{abstract}
OEIS A026017 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 6 x^{3} + 6 x^{2}$, a polynomial of degree $3$, which proves
...
OEIS A026017 is ``a(n) = number of (s(0), s(1), ..., s(2*n-1)) such that s(i) is a nonnegative integer and |s(i) - s(i-1)| = 1 for i = 1,2,...,n, s(0) = 2, s(2*n-1) = 5. Also a(n) = T(2*n-1,n-2), where T is the array defined in A026009''. It has offset $2$ and begins
\[
a(2),\dots,a(9)\;=\;1, 5, 21, 83, 319, 1209, 4550, 17068,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 2} a(n)\,x^{n}\;=\;\frac{\left(8 x^{2} - \left(\sqrt{1 - 4 x} - 1\right)^{3}\right) \left(\sqrt{1 - 4 x} - 1\right)^{4}}{128 x^{4}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+4)*a(n) + (-8*n-17)*a(n-1) + (19*n+1)*a(n-2) + 6*(-2*n+5)*a(n-3) = 0. - \emph{R. J. Mathar}, Jun 20 2013
\end{quote}
...
\[
p_{0}(n)=n + 4,\qquad p_{1}(n)=- 8 n - 17,\qquad p_{2}(n)=19 n + 1,\qquad p_{3}(n)=- 6 \left(2 n - 5\right)
\]
...
\section{The computation for A026017}
...
\[
(n + 4)\,a(n) + (- 8 n - 17)\,a(n-1) + (19 n + 1)\,a(n-2) -  6 \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A026017, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 6 x^{3} + 6 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A026017.
