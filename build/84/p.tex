...
\title{A proof of the conjectured recurrence for OEIS A121724}
...
\begin{abstract}
OEIS A121724 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $1 - 8 x$, a polynomial of degree $1$, which proves
...
OEIS A121724 is ``Generalized central binomial coefficients for k=2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 5, 9, 45, 97, 485, 1145,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{1 - 16 x^{2}}}{x \left(8 x + \sqrt{1 - 16 x^{2}} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) = 5*(n+1)*a(n-1) + 16*(n-2)*a(n-2) - 80*(n-2)*a(n-3). - \emph{R. J. Mathar}, Nov 26 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2022-11-08, revision 23) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 5 \left(n + 1\right),\qquad p_{2}(n)=- 16 \left(n - 2\right),\qquad p_{3}(n)=80 \left(n - 2\right)
\]
...
\section{The computation for A121724}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \left(4 x - 1\right) \left(4 x + 1\right) .
\]
...
\[
(n + 1)\,a(n) -  5 \left(n + 1\right)\,a(n-1) -  16 \left(n - 2\right)\,a(n-2) + 80 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A121724, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=1 - 8 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A121724.
