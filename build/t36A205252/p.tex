...
\title{The empirical recurrence for OEIS A205252, proved by transfer matrix}
...
\begin{abstract}
OEIS A205252 counts arrays over $\{0,\dots,1\}$ under a condition on the number of clockwise edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=68$ states and is
...
OEIS A205252 is ``Number of (n+1)X6 0..1 arrays with the number of clockwise edge increases in every 2X2 subblock the same.''. It has offset $1$ and begins
\[
976, 15568, 253072, 4136296, 67731268, 1109832184, 18189909484, 298154846440,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 31*a(n-1) -321*a(n-2) +1569*a(n-3) -4179*a(n-4) +6420*a(n-5) -5671*a(n-6) +2668*a(n-7) -516*a(n-8)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 11:55:00, revision 9) this is still
...
a state and rows $r,s$ may follow one another when all $5$ of their subblocks give $v$.
Taking the disjoint union over $v$,
\[
a(n)\;=\;\iota^{\!\top}M^{\,n}\tau,\qquad\iota=\tau=(1,\dots,1)^{\!\top},
\]
on the $S=68$ states that survive. Rows carrying no edge at all are dropped: such a row could
...
subblocks; the entry's first term is $976$, which the model
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205252.
