...
\title{The empirical recurrence for OEIS A205190, proved by transfer matrix}
...
\begin{abstract}
OEIS A205190 counts arrays over $\{0,\dots,1\}$ under a condition on the number of clockwise edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=144$ states and is
...
OEIS A205190 is ``Number of (n+1) X 6 0..1 arrays with the number of clockwise edge increases in every 2 X 2 subblock differing from each horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
144, 168, 292, 384, 736, 896, 1568, 1920,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-2) +a(n-6) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 04:18:14, revision 8) this is still
...
\]
on the $S=144$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $5$
subblocks; the entry's first term is $144$, which the model
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205190.
