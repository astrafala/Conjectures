...
\title{A proof of the conjectured recurrence for OEIS A107587}
...
\begin{abstract}
OEIS A107587 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A107587 is ``Number of Motzkin n-paths with an even number of up steps''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 1, 1, 3, 11, 31, 71,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- \sqrt{- 3 x^{2} - 2 x + 1} + \sqrt{5 x^{2} - 2 x + 1}}{4 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-2)*(n+2)*a(n) - (2*n-1)*(2*n\^{}2-2*n-3)*a(n-1) + 3*(n-1)*(2*n\^{}2-4*n+1)*a(n-2) - 2*(n-1)*(n-2)*(2*n-3)*a(n-3) - 15*(n-1)*(n-2)*(n-3)*a(n-4) = 0. - \emph{R. J. Mathar}, Feb 20 2015 [conjecture is true for n = 0..800; checked by \emph{Gennady Eremin}, Jan 25 2021]
\end{quote}
As of the ``Last modified'' line on the live entry (2021-09-19, revision 81) the
...
\[
p_{0}(n)=n \left(n - 2\right) \left(n + 2\right),\qquad p_{1}(n)=- \left(2 n - 1\right) \left(2 n^{2} - 2 n - 3\right),\qquad p_{2}(n)=3 \left(n - 1\right) \left(2 n^{2} - 4 n + 1\right),\qquad p_{3}(n)=- 2 \left(n - 2\right) \left(n - 1\right) \left(2 n - 3\right),\qquad p_{4}(n)=- 15 \left(n - 3\right) \left(n - 2\right) \left(n - 1\right)
\]
...
\[
n \left(n - 2\right) \left(n + 2\right)\,a(n) -  \left(2 n - 1\right) \left(2 n^{2} - 2 n - 3\right)\,a(n-1) + 3 \left(n - 1\right) \left(2 n^{2} - 4 n + 1\right)\,a(n-2) -  2 \left(n - 2\right) \left(n - 1\right) \left(2 n - 3\right)\,a(n-3) -  15 \left(n - 3\right) \left(n - 2\right) \left(n - 1\right)\,a(n-4) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A107587, agreeing on all 13 terms compared.
...
vanish, for all 28 values of $n$ from $n=4$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A107587.
