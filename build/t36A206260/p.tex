...
\title{The empirical recurrence for OEIS A206260, proved by transfer matrix}
...
\begin{abstract}
OEIS A206260 counts arrays over $\{0,\dots,1\}$ under a condition on the number of rightwards and downwards edge increases
...
OEIS A206260 is ``Number of (n+1) X 2 0..1 arrays with the number of rightwards and downwards edge increases in each 2 X 2 subblock equal to the number in all its horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
16, 24, 34, 50, 72, 105, 154, 229,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 5*a(n-2) + a(n-3) + 2*a(n-4) - a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 14 2018 16:22:25, revision 8) this is still
...
$(n+1)\times2$ array over $\{0,\dots,1\}$ form a grid of $n$ rows and $1$
...
\]
on the $S=16$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $1$
subblock; the entry's first term is $16$, which the model
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206260.
