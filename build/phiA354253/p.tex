...
\title{The reduction of OEIS A354253 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A354253 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
\[
1, 4, 52, 1108, 32980, 1261204, 58928212, 3253363348,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 17 2023 11:20:13, revision
30) the statement is still recorded as a conjecture, and nothing on the entry
...
\begin{proof}
$G$ is $\sum_j \binom{2j}{j}2^{j}t^{j}$, whose coefficients are integers.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j=\binom{2j}{j}2^{j}$, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 4, 24, 160, 1120, 8064, 59136, 439296, 3294720.
\]
...
Let $a(n)$ be OEIS A354253. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A354253.
