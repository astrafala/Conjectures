...
\title{A proof of the conjectured recurrence for OEIS A213203}
...
\begin{abstract}
OEIS A213203 carries a conjectured linear recurrence with polynomial coefficients, of
...
a(n)\;=\;\frac{n \left(\left(n - 2\right) \left(n - 1\right)! - 1\right) \left(n - 1\right)!}{2} .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} a(n) + (-n\^{}2-n-11)*a(n-1) + (n\^{}3+7*n\^{}2-13*n+39)*a(n-2) - 2*(n-2)*(4*n\^{}2-2*n-15)*a(n-3) + 20*(n-2)*(n-3)*(n-4)*a(n-4) = 0. - \emph{R. J. Mathar}, Mar 21 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2013-03-21, revision 23) the
...
1\,a(n) + (- n^{2} - n - 11)\,a(n-1) + (n^{3} + 7 n^{2} - 13 n + 39)\,a(n-2) -  2 \left(n - 2\right) \left(4 n^{2} - 2 n - 15\right)\,a(n-3) + 20 \left(n - 4\right) \left(n - 3\right) \left(n - 2\right)\,a(n-4)
\end{equation}
...
\section{The computation for A213203}

The closed form \eqref{eq:cf} splits into two similarity classes, with
representativees
\[
\frac{n^{2} \left(n - 1\right)!^{2}}{2},\qquad - \frac{n \left(n - 1\right)!}{2}
\]
...
exact integer arithmetic at every index where it applies: $11$ instances beginning at
...
entry \href{https://oeis.org/A213203}{A213203}, 2013-03-21.
