...
\title{The reduction of OEIS A306082 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A306082 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
OEIS A306082 is ``Expansion of e.g.f. Product\_{k>=1} 1/(1 - (exp(x) - 1)\^{}(k\^{}2)).''. It has offset $0$ and begins
\[
1, 1, 3, 13, 99, 901, 8763, 92653,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
Expansion of e.g.f. Product\_{k>=1} 1/(1 - (exp(x) - 1)\^{}(k\^{}2)).
\end{quote}
that is, writing $A(x)=\sum_{n\ge 0}a(n)x^{n}/n!$,
\begin{equation}\label{eq:egf}
A(x)\;=\;\prod_{k\ge 1}\frac{1}{1-(e^{x}-1)^{k^{2}}}.
\end{equation}
...
G(t)\;=\;\prod_{k\ge 1}\frac{1}{1-t^{k^{2}}}.
\end{equation}
...
\begin{proof}
$G$ is the generating function for partitions into square parts; each factor is an integer series and only finitely many affect any one coefficient.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j$ counts partitions of $j$ into square parts, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 1, 1, 1, 2, 2, 2, 2, 3.
\]
...
Let $a(n)$ be OEIS A306082. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A306082.
