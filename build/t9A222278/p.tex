...
\title{The empirical recurrence for OEIS A222278, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A222278 counts arrays over $\{0,\dots,5\}$ in which every cell is constrained by the
...
with $S=7776$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A222278 is ``Number of nX5 0..5 arrays with no entry increasing mod 6 by 5 rightwards or downwards, starting with upper left zero.''. It has offset $1$ and begins
\[
625, 972405, 1513010465, 2354171487645, 3662978221194885, 5699418948690910545, 8868023343395876315745, 13798220262404977892742445,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 20:15:23, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;1784\,a(n-1) - 372607\,a(n-2) + 28170251\,a(n-3) - 798596554\,a(n-4) + 1889917225\,a(n-5) + 177172468750\,a(n-6) - 651776500000\,a(n-7)
\end{equation}
for all $n>7$.
...
Let $V=\{0,\dots,5\}^{5}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=7776$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=7776$ pairs and each
of the $(5+1)^{5}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222278.
