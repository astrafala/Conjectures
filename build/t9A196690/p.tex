...
\title{The empirical recurrence for OEIS A196690, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A196690 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A196690 is ``Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 2,1,3,0,4 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
2, 5, 11, 34, 101, 299, 890, 2643,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) + 3*a(n-2) - a(n-4) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 22 2018 11:12:13, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 3\,a(n-2) - a(n-4)
\end{equation}
for all $n>5$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(2,1,3,0,4)
\end{equation}
for every cell $(t,u)$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196690.
