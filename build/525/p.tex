...
\title{A proof of the conjectured recurrence for OEIS A105695}
...
\begin{abstract}
OEIS A105695 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{3} - x^{2} - x + 1$, a polynomial of degree $3$, which proves
...
OEIS A105695 is ``Expansion of (1-x)*c(x/(1+x)), where c(x) is the g.f. of the Catalan numbers (A000108)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 0, 1, 2, 5, 12, 30,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x - 1\right) \left(x + 1\right) \left(\sqrt{\frac{1 - 3 x}{x + 1}} - 1\right)}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n)+(2-3n)*a(n-1) +(1-n)*a(n-2)+3*(n-4)*a(n-3)=0. - \emph{R. J. Mathar}, Dec 13 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-06-27, revision 27) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=2 - 3 n,\qquad p_{2}(n)=1 - n,\qquad p_{3}(n)=3 \left(n - 4\right)
\]
...
\section{The computation for A105695}
...
\[
(n + 1)\,a(n) + (2 - 3 n)\,a(n-1) + (1 - n)\,a(n-2) + 3 \left(n - 4\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A105695, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=x^{3} - x^{2} - x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A105695.
