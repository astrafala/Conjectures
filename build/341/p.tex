...
\title{A proof of the conjectured recurrence for OEIS A081923}
...
\begin{abstract}
OEIS A081923 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A081923 is ``Expansion of e.g.f.: exp(2x)/(1-x)\^{}2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 18, 92, 536, 3552, 26608, 223456,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{e^{2 x}}{\left(x - 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) +(-n-3)*a(n-1) +2*(n-1)*a(n-2)=0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-03-31, revision 56) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- n - 3,\qquad p_{2}(n)=2 \left(n - 1\right)
\]
...
\section{The computation for A081923}
...
\[
1\,a(n) + (- n - 3)\,a(n-1) + 2 \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A081923, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A081923.
