...
\title{The empirical recurrence for OEIS A205188, proved by transfer matrix}
...
\begin{abstract}
OEIS A205188 counts arrays over $\{0,\dots,1\}$ under a condition on the number of clockwise edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=48$ states and is
...
OEIS A205188 is ``Number of (n+1) X 4 0..1 arrays with the number of clockwise edge increases in every 2 X 2 subblock differing from each horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
48, 64, 124, 160, 292, 384, 708, 928,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-2) +a(n-4) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 04:17:58, revision 8) this is still
...
\]
on the $S=48$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $3$
subblocks; the entry's first term is $48$, which the model
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205188.
