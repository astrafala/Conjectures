...
\title{The empirical column recurrences for the table OEIS A264336}
...
\begin{abstract}
OEIS A264336 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264336 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change -1,1 1,0 1,-1 or 0,-1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 1, 1, 1, 2, 1, 1, 4,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 11 2015 13:18:28, revision 4) these are still
...
column $k$$=1$ & $3$ & $16$ & $>1$ & $11$ \\
column $k$$=2$ & $1$ & $64$ & $>1$ & $10$ \\
column $k$$=4$ & $6$ & $1024$ & $>7$ & $8$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change -1,1 1,0 1,-1 or 0,-1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having directed index change -1,1 1,0 1,-1 or 0,-1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(5) arrays of permutations of 0..(n+1)*(5)-1 with each element having directed index change -1,1 1,0 1,-1 or 0,-1.''. Its rows are the vertices of a digraph on
...
model reproduces $29$ published values, $58$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264336.
