...
\title{The empirical recurrence for OEIS A304136, proved by transfer matrix}
...
\begin{abstract}
OEIS A304136 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,2,3,6\}$, and it carries an empirical
...
OEIS A304136 is ``Number of n X 3 0..1 arrays with every element unequal to 0, 2, 3 or 6 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 7, 5, 9, 15, 16, 33, 57,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +a(n-2) +5*a(n-3) -5*a(n-4) -5*a(n-5) -4*a(n-6) +4*a(n-7) +4*a(n-8) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 17 2023 01:19:28, revision
6) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + a(n-2) + 5\,a(n-3) - 5\,a(n-4) - 5\,a(n-5) - 4\,a(n-6) + 4\,a(n-7) + 4\,a(n-8)
\end{equation}
for all $n>9$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A304136.
