...
\title{The empirical recurrence for OEIS A195972, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A195972 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A195972 is ``Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 2,0,1,3,4 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
3, 9, 14, 23, 43, 78, 139, 249,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) +a(n-3) -a(n-4) -2*a(n-5) -2*a(n-6) -a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (May 08 2018 06:20:01, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + a(n-3) - a(n-4) - 2\,a(n-5) - 2\,a(n-6) - a(n-7)
\end{equation}
for all $n>8$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(2,0,1,3,4)
\end{equation}
for every cell $(t,u)$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195972.
