...
\title{The empirical column recurrences for the table OEIS A234227}
...
\begin{abstract}
OEIS A234227 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234227 is ``T(n,k) is the number of (n+1) X (k+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
104, 436, 436, 1824, 1560, 1824, 7696, 5612,\ \dots
\]
...
k=1: a(n) = 4*a(n-1) +20*a(n-2) -80*a(n-3).\\
k=2: a(n) = 7*a(n-1) +20*a(n-2) -224*a(n-3) +144*a(n-4) +1680*a(n-5) -2880*a(n-6).\\
k=3: [order 11].\\
k=4: [order 16].\\
k=5: [same order 16].\\
k=6: [same order 16].\\
k=7: [same order 16].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 20:58:39, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;4\,a(n-1) + 20\,a(n-2) - 80\,a(n-3)
\]
holds for every $n>2$.

\subsection*{Column $k=2$}
The sequence is ``Number of (n+1) X (3) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
$S=125$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;7\,a(n-1) + 20\,a(n-2) - 224\,a(n-3) + 144\,a(n-4) + 1680\,a(n-5) - 2880\,a(n-6)
\]
holds for every $n>5$.
...
different counts. Across the columns settled here the model reproduces $18$ published
values, $95$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234227.
