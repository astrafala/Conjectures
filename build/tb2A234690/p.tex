...
\title{The empirical column recurrences for the table OEIS A234690}
...
\begin{abstract}
OEIS A234690 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234690 is ``T(n,k) is the number of (n+1) X (k+1) 0..6 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 5 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
232, 1148, 1148, 5472, 4416, 5472, 27344, 16580,\ \dots
\]
...
k=1: a(n) = 72*a(n-2) -1712*a(n-4) +13440*a(n-6).\\
k=2: [order 24].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 20:54:11, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..6 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 5 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;72\,a(n-2) - 1712\,a(n-4) + 13440\,a(n-6)
\]
holds for every $n>5$.
...
different counts. Across the columns settled here the model reproduces $8$ published
values, $43$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234690.
