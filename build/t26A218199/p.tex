...
\title{The empirical recurrence for OEIS A218199, proved by transfer matrix}
...
\begin{abstract}
OEIS A218199 counts the distinct indicator arrays obtained from the $n\times1$ arrays over
...
indicator row by row turns it into one, on $S=8$ states, after which the conjectured
...
OEIS A218199 is ``Hilltop maps: number of n X 1 binary arrays indicating the locations of corresponding elements not exceeded by any horizontal or vertical neighbor in a random 0..2 n X 1 array.''. It has offset $1$ and begins
\[
1, 3, 7, 13, 25, 49, 97, 191,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) +a(n-2) +a(n-3) +a(n-4) +a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 28 2025 09:58:28, revision 15) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

Let $R=\{0,\dots,2\}^{1}$ be the set of rows and let the neighbours of a cell be the
cells at the offsets
\[
$(-1,0)$, $(0,-1)$, $(0,1)$, $(1,0)$,
\]
those falling outside the array being simply absent. For an array
$A=(r_1,\dots,r_n)\in R^n$ let $\Phi(A)\in\{0,1\}^{n\times1}$ mark the cells whose value
...
on rows $i-1,i,i+1$. Write $\beta(p,c,x)\in\{0,1\}^{1}$ for the marking of the row $c$
...
For a set $D\subseteq(R\cup\{\bot\})\times R$ of pairs and a row $b\in\{0,1\}^{1}$ put
\[
...
\]
the sum over all sequences of rows in $\{0,1\}^{1}$, empty $D$'s contributing nothing.
\end{lemma}
...
\]
There are $S=8$ reachable sets, generated from $D_0$ outward, and $a$ is $C$-finite. Note
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A218199.
