...
to the polynomial $- 3 x^{2}$, of degree $2$, which proves the recurrence for every
$n>2$.
\end{abstract}
...
\begin{quote}\small
\textbf{Conjecture:} (-n+2)*a(n) +(n-1)*(2*n-5)*a(n-1) -(n-1)*(n-2)*(n-3)*a(n-2)=0. - \emph{R. J. Mathar}, Jul 18 2015
\end{quote}
...
\[
p_{0}(n)=2 - n,\qquad p_{1}(n)=\left(n - 1\right) \left(2 n - 5\right),\qquad p_{2}(n)=- \left(n - 3\right) \left(n - 2\right) \left(n - 1\right)
\]
...
\[
(2 - n)\,a(n) + \left(n - 1\right) \left(2 n - 5\right)\,a(n-1) -  \left(n - 3\right) \left(n - 2\right) \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
