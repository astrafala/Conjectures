...
\title{The empirical recurrence for OEIS A234416, proved by transfer matrix}
...
\begin{abstract}
OEIS A234416 counts $(n+1)\times4$ arrays over $\{0,\dots,7\}$ subject to a condition imposed on
every $2\times2$ subblock, and carries an empirical recurrence of order $33$ contributed
...
OEIS A234416 is ``Number of (n+1)X(3+1) 0..7 arrays with every 2X2 subblock having the absolute values of all six edge and diagonal differences no larger than 1.''. It has offset $1$ and begins
\[
2848, 56768, 1131708, 22692600, 455740110, 9162597900, 184360780592, 3711992019408,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 09:18:26, revision 6) the statement is
...
For every $n$ with $n+1-1\ge33$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n+1-1-33}\,q(M)\,\mathbf{1} .
\]
...
=\mathbf{1}^{\!\top}M^{\,m-33}\,q(M)\,\mathbf{1}
\]
...
$m-33=j$ run over $\ge0$. For the last clause put $w=q(M)\mathbf{1}$. By the
...
$S^2$ pairs. The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was then formed by $33$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234416.
