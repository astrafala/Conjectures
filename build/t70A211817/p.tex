...
\title{The empirical recurrence for OEIS A211817, proved by a one-dimensional reduction}
...
\begin{abstract}
OEIS A211817 counts symmetric $(n+1)\times(n+1)$ matrices over
$\{-10,\dots,10\}$ in which every $2\times2$ subblock sums to zero and has one, two or three distinct
...
on $S=1793$ states, and the empirical recurrence of order $37$ contributed by R.~H.~Hardin
follows.
\end{abstract}
...
OEIS A211817 is ``Number of (n+1)X(n+1) -10..10 symmetric matrices with every 2X2 subblock having sum zero and one, two or three distinct values.''. It has offset $1$ and begins
\[
221, 623, 1477, 3361, 7389, 16191, 34993, 76417,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 16:23:06, revision 8) this is still
...
$S=1793$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad \tau=\mathbf{1},\qquad S=1793.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211817.
