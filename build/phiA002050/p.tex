...
\title{The reduction of OEIS A002050 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A002050 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
\[
0, 1, 5, 25, 149, 1081, 9365, 94585,\ \dots
\]
...
A(x)\;=\;\frac{e^{2x}-e^{x}}{2-e^{x}}.
\end{equation}
This is the only input taken from the entry, and it was checked against every one of the
$19$ terms of the entry's DATA section before use, not against a sample.
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 05 2026 11:19:36, revision
97) the statement is still recorded as a conjecture, and nothing on the entry
...
\[
e^{2x}-e^{x}=e^{x}\bigl(e^{x}-1\bigr)=(1+t)\,t,\qquad 2-e^{x}=1-t,
\]
so that
...
\begin{proof}
$G$ is a quotient of polynomials with integer coefficients whose denominator has constant term $1$.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_0=0$, $c_1=1$ and $c_j=2$ for $j\ge 2$, and the
first few are
\[
c_0,\dots,c_8 \;=\; 0, 1, 2, 2, 2, 2, 2, 2, 2.
\]
...
Let $a(n)$ be OEIS A002050. For every integer $k\ge1$,
\[
...
against the entry's DATA at every one of the $19$ published indices; the values
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A002050.
