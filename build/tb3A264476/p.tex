...
\title{The empirical column recurrences for the table OEIS A264476}
...
\begin{abstract}
OEIS A264476 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1$ and the rows $n=1, 2$.
\end{abstract}
...
OEIS A264476 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 0,1 1,0 2,1 or -1,-1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 1, 1, 0, 2, 0, 0, 4,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 15 2015 07:34:00, revision 9) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 0,1 1,0 2,1 or -1,-1.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having directed index change 0,1 1,0 2,1 or -1,-1.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of (3)X(n+1) arrays of permutations of 0..(3)*(n+1)-1 with each element having directed index change 0,1 1,0 2,1 or -1,-1.''. Its rows are the vertices of a digraph on
...
model reproduces $31$ published values, $40$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264476.
