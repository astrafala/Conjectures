...
\title{The empirical recurrence for OEIS A255221, proved by transfer matrix}
...
\begin{abstract}
OEIS A255221 counts $(n+2)\times3$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
no $3\times3$ subblock, and it carries an empirical recurrence of order $4$
...
OEIS A255221 is ``Number of (n+2) X (1+2) 0..1 arrays with no 3 x 3 subblock diagonal sum 0 and no antidiagonal sum 0 and no row sum 2 and no column sum 2.''. It has offset $1$ and begins
\[
23, 22, 31, 43, 61, 88, 127, 184,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - a(n-2) + a(n-3) - a(n-4) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 19 2018 09:04:04, revision 7) this is still
...
\begin{proof}
The vector $w=q(M)\mathbf{1}$ was formed by $4$ matrix--vector products in exact
integer arithmetic and $\mathbf{1}^{\!\top}M^{j}w$ evaluated for $j=0,1,\dots$, again
exactly. Those integers vanish from the index corresponding to $n=5+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>5$. The bound is exact: at $n=5$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A255221.
