...
\title{The reduction of OEIS A000670 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A000670 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
OEIS A000670 is ``Fubini numbers: number of preferential arrangements of n labeled elements; or number of weak orders on n labeled elements; or number of ordered partitions of [n].''. It has offset $0$ and begins
\[
1, 1, 3, 13, 75, 541, 4683, 47293,\ \dots
\]
...
\begin{quote}\small
\textbf{Conjecture}: Let k be a positive integer. The sequence obtained by reducing a(n) modulo k is eventually periodic with the period dividing phi(k) = A000010(k). For example, modulo 16 we obtain the sequence [1, 1, 3, 13, 11, 13, 11, 13, 11, 13, ...], with an apparent period of 2 beginning at a(4). Cf. A354242. --- \emph{Peter Bala}, Jul 08 2022
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 14 2026 16:54:57, revision
909) the statement is still recorded as a conjecture, and nothing on the entry
...
\begin{proof}
$G$ is a geometric series, all of whose coefficients equal $1$.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j=1$ for every $j$, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 1, 1, 1, 1, 1, 1, 1, 1.
\]
...
Let $a(n)$ be OEIS A000670. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A000670.
