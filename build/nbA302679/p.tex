...
\title{The empirical recurrence for OEIS A302679, proved by transfer matrix}
...
\begin{abstract}
OEIS A302679 counts $7$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the six horizontal, diagonal and antidiagonal neighbours, a number $equal to$ itself lying in $\{0,1,4,6\}$, and it carries an empirical
...
OEIS A302679 is ``Number of nX7 0..1 arrays with every element equal to 0, 1, 4 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
21, 23, 94, 217, 661, 2013, 6311, 19497,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +a(n-2) +12*a(n-3) +14*a(n-4) +10*a(n-5) -9*a(n-6) -26*a(n-7) +2*a(n-8) +18*a(n-9) +6*a(n-10) -4*a(n-12) for n>19
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 11 2018 12:27:42, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + a(n-2) + 12\,a(n-3) + 14\,a(n-4) + 10\,a(n-5) - 9\,a(n-6) - 26\,a(n-7) + 2\,a(n-8) + 18\,a(n-9) + 6\,a(n-10) - 4\,a(n-12)
\end{equation}
for all $n>18$.
...
has score in $\{0,1,4,6\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,4,6\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,4,6\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,4,6\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,4,6\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302679.
