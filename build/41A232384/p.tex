...
\title{The empirical recurrence for OEIS A232384, proved by transfer matrix}
...
\begin{abstract}
OEIS A232384 counts arrays of width $2$ over $\{0,\dots,3\}$ under a condition on each cell
and its neighbours, and carries an empirical recurrence of order $15$ contributed by
...
OEIS A232384 is ``Number of (n+1)X(1+1) 0..3 arrays with every element next to itself plus and minus one within the range 0..3 horizontally, vertically or antidiagonally, with no adjacent elements equal.''. It has offset $1$ and begins
\[
12, 18, 78, 190, 670, 1954, 6224, 19178,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 07:23:18, revision 6) this is still
...
\]
those falling outside the array being absent. The entry requires that every cell $v$ has, among its neighbours, one equal to $v+1$ and one equal to $v-1$ --- each required only when that value lies in $\{0,\dots,3\}$, which is what ``within the range $0..3$'' says, and no cell equals a neighbour.
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 5\,a(n-2) - 9\,a(n-3) - 14\,a(n-4) - 12\,a(n-5) - 3\,a(n-6) + 23\,a(n-7) + 43\,a(n-8) + 51\,a(n-9) + 27\,a(n-10) + a(n-11) - 12\,a(n-12) - 6\,a(n-13) - a(n-14) + a(n-15)
\]
for every $n>15$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232384.
