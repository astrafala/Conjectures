...
\title{The empirical recurrence for OEIS A251311, proved by transfer matrix}
...
\begin{abstract}
OEIS A251311 counts $(n+1)\times3$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
every $2\times2$ subblock, and carries an empirical recurrence of order $6$ contributed
...
OEIS A251311 is ``Number of (n+1) X (2+1) 0..1 arrays with no 2 X 2 subblock having the maximum of its diagonal elements greater than the absolute difference of its antidiagonal elements.''. It has offset $1$ and begins
\[
25, 79, 238, 720, 2199, 6717, 20484, 62464,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) - 9*a(n-2) + 13*a(n-3) - 13*a(n-4) + 6*a(n-5) - 2*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 28 2018 10:57:48, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 9\,a(n-2) + 13\,a(n-3) - 13\,a(n-4) + 6\,a(n-5) - 2\,a(n-6)
\end{equation}
for all $n>6$.
...
For every $n$ with $n+1-1\ge6$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n+1-1-6}\,q(M)\,\mathbf{1} .
\]
...
=\mathbf{1}^{\!\top}M^{\,m-6}\,q(M)\,\mathbf{1}
\]
...
$m-6=j$ run over $\ge0$. For the last clause put $w=q(M)\mathbf{1}$. By the
...
$S^2$ pairs. The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was then formed by $6$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251311.
