...
\title{The empirical recurrence for OEIS A304922, proved by transfer matrix}
...
\begin{abstract}
OEIS A304922 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,5,7\}$, and it carries an empirical
...
OEIS A304922 is ``Number of nX4 0..1 arrays with every element unequal to 0, 1, 2, 5 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
8, 9, 31, 87, 266, 851, 2510, 7348,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 21 2018 09:13:19, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 2\,a(n-3) - 4\,a(n-4) - 6\,a(n-5) - 6\,a(n-6) - a(n-7) + 23\,a(n-8) - 12\,a(n-9) + 4\,a(n-10) + 8\,a(n-11) - 2\,a(n-12) - 8\,a(n-13)
\end{equation}
for all $n>17$.
...
has score in $\{0,1,2,5,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,5,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,5,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,5,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,5,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A304922.
