...
\title{A proof of the conjectured recurrence for OEIS A386834}
...
\begin{abstract}
OEIS A386834 carries a conjectured linear recurrence with polynomial coefficients, of
order 3. The entry separately states a recurrence of order 2 that is not
...
OEIS A386834 is ``a(n) = Sum\_\{k=0..n\} binomial(4*n+1,k) * binomial(4*n-k-1,n-k)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 8, 111, 1738, 28701, 488412, 8473387, 148994510,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
Conjecture D-finite with recurrence +3*n*(16543753*n -26995933)*(3*n-1)*(3*n-2)*a(n) +(-1669899251*n\^{}4 -26931977989*n\^{}3 +131963667975*n\^{}2 -188283072995*n +85757456660)*a(n-1) +2*(-61301926003*n\^{}4 +515926265010*n\^{}3 -1655392333929*n\^{}2 +2311146075302*n -1165379619540)*a(n-2) -96*(39221117*n -50949760)*(4*n-9)*(2*n-5)*(4*n-7)*a(n-3)=0. - \emph{R. J. Mathar}, Aug 19 2025
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-09, revision 18) it is still
...
\begin{quote}\small
Recurrence (of order 2): 3*n*(3*n - 2)*(3*n - 1)*(77*n\^{}3 - 347*n\^{}2 + 516*n - 252)*a(n) = (37345*n\^{}6 - 227431*n\^{}5 + 539727*n\^{}4 - 630029*n\^{}3 + 373808*n\^{}2 - 103260*n + 9360)*a(n-1) + 16*(2*n - 3)*(4*n - 5)*(4*n - 3)*(77*n\^{}3 - 116*n\^{}2 + 53*n - 6)*a(n-2). - \emph{Vaclav Kotesovec}, Nov 09 2025
\end{quote}
...
\section{The computation for A386834}

Reading both relations as operators and dividing the conjectured one by the stated one
leaves remainder $0$, with quotient
\[
Q \;=\; \left(\frac{235326702 n + 400281546}{77 n^{3} + 346 n^{2} + 513 n + 252}\right) + \left(\frac{16543753 n + 22635326}{77 n^{3} + 346 n^{2} + 513 n + 252}\right)\,N .
\]
...
\[
3 n \left(3 n - 2\right) \left(3 n - 1\right) \left(16543753 n - 26995933\right)\,a(n) + (- 1669899251 n^{4} - 26931977989 n^{3} + 131963667975 n^{2} - 188283072995 n + 85757456660)\,a(n-1) -  2 \left(61301926003 n^{4} - 515926265010 n^{3} + 1655392333929 n^{2} - 2311146075302 n + 1165379619540\right)\,a(n-2) -  96 \left(2 n - 5\right) \left(4 n - 9\right) \left(4 n - 7\right) \left(39221117 n - 50949760\right)\,a(n-3) \;=\;0
\]
...
\url{https://oeis.org}, 2026. Sequence A386834.
