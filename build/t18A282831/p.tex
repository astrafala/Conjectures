...
\title{The empirical recurrence for OEIS A282831, proved by transfer matrix}
...
\begin{abstract}
OEIS A282831 counts $n\times1$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
...
walk, and $a(n)$ is a walk count on $S=18$ vertices, which is $C$-finite. Whether the
...
OEIS A282831 is ``Number of nX1 0..1 arrays with no 1 equal to more than one of its king-move neighbors, with the exception of exactly two elements.''. It has offset $1$ and begins
\[
0, 0, 0, 1, 2, 5, 13, 29,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) -2*a(n-3) -6*a(n-4) +4*a(n-6) +6*a(n-7) +3*a(n-8) +a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 23 2017 04:29:50, revision 6) this is still
...
$\{0,\dots,1\}^{1}$. The neighbourhood of a cell in row $u_i$ meets $u_{i-1}$,
$u_i$ and $u_{i+1}$ and nothing else. Take as vertices the pairs
$(r,s)\in(\{0,\dots,1\}^{1}\cup\{\varnothing\})\times\{0,\dots,1\}^{1}$ \times \{0,\dots,2\},
and put an edge $(r,s)\to(s,t)$ when row $s$ satisfies the condition at every one of its
$1$ cells, given $r$ above and $t$ below. An array with $L$ rows is then a walk of length
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 2\,a(n-3) - 6\,a(n-4) + 4\,a(n-6) + 6\,a(n-7) + 3\,a(n-8) + a(n-9)
\]
for every $n>9$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A282831.
