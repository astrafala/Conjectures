...
\title{The empirical recurrence for OEIS A235549, proved by transfer matrix}
...
\begin{abstract}
OEIS A235549 counts arrays of width $2$ over $\{0,1\}$ by reducing every $2\times2$ subblock to a
...
about it is one bit per adjacent pair of columns, so the count is a walk count on $S=15$
states.
\end{abstract}
...
OEIS A235549 is ``Number of (n+1) X (1+1) 0..1 arrays with the sum of each 2 X 2 subblock two extreme terms minus its two median terms lexicographically nondecreasing rowwise and columnwise.''. It has offset $1$ and begins
\[
16, 46, 120, 288, 660, 1456, 3136, 6624,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + 4*a(n-2) - 8*a(n-3) - 4*a(n-4) + 8*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 19 2018 10:35:57, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The arrays have $2$ columns and a growing number of rows, with entries in $\{0,1\}$. A
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 4\,a(n-2) - 8\,a(n-3) - 4\,a(n-4) + 8\,a(n-5)
\]
for every $n>5$.
\end{theorem}
...
array of width $2$ over $\{0,1\}$ with two and with three rows was written out cell by cell,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235549.
