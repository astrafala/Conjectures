...
\title{The empirical recurrence for OEIS A305343, proved by transfer matrix}
...
\begin{abstract}
OEIS A305343 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,3,5,8\}$, and it carries an empirical
...
OEIS A305343 is ``Number of nX4 0..1 arrays with every element unequal to 0, 1, 3, 5 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
5, 21, 28, 77, 199, 454, 1074, 2619,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 30 2018 17:04:51, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 4\,a(n-3) - 3\,a(n-4) - 4\,a(n-5) + 7\,a(n-6) - 8\,a(n-7) + a(n-8) - 21\,a(n-9) + 5\,a(n-10) + 10\,a(n-11) - 7\,a(n-12) + 8\,a(n-13) - 3\,a(n-14) + 34\,a(n-15) + 7\,a(n-16) + 2\,a(n-17) - a(n-18) + 5\,a(n-20) - 16\,a(n-21) - 12\,a(n-22) - 17\,a(n-23) - 3\,a(n-25) - 4\,a(n-26) - 4\,a(n-27) + a(n-28) + 10\,a(n-29) + 3\,a(n-30) + a(n-31) + a(n-32) + 3\,a(n-33) + 2\,a(n-34) - a(n-35) - a(n-36)
\end{equation}
for all $n>36$.
...
has score in $\{0,1,3,5,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,3,5,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,3,5,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,3,5,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,3,5,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A305343.
