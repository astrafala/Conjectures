...
\title{The reduction of OEIS A320352 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A320352 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
\[
0, 1, 3, 19, 159, 1651, 20583, 299419,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 20 2025 00:30:17, revision
16) the statement is still recorded as a conjecture, and nothing on the entry
...
\[
e^{x}-e^{2x}+1=(1+t)-(1+t)^{2}+1=1-t-t^{2},
\]
so that
...
\begin{proof}
$G$ is the generating function of the Fibonacci numbers, which are integers.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j=F_j$, the Fibonacci numbers A000045, and the
first few are
\[
c_0,\dots,c_8 \;=\; 0, 1, 1, 2, 3, 5, 8, 13, 21.
\]
...
Let $a(n)$ be OEIS A320352. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A320352.
