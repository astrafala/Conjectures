...
\title{The reduction of OEIS A354242 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A354242 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
\[
1, 2, 14, 158, 2486, 50222, 1239254, 36126638,\ \dots
\]
...
\begin{quote}\small
\textbf{Conjecture}: Let k be a positive integer. The sequence obtained by reducing a(n) modulo k is eventually periodic with the period dividing phi(k) = A000010(k). For example, modulo 16 we obtain the sequence [1, 2, 14, 14, 6, 14, 6, 14, 6, ...], with an apparent period of 2 beginning at a(3). Cf. A354253. --- \emph{Peter Bala}, Jul 07 2022
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 16 2023 11:50:46, revision
37) the statement is still recorded as a conjecture, and nothing on the entry
...
\begin{proof}
$G$ is $\sum_j \binom{2j}{j}t^{j}$, whose coefficients are central binomial coefficients.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j=\binom{2j}{j}$, the central binomial coefficients A000984, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 2, 6, 20, 70, 252, 924, 3432, 12870.
\]
...
Let $a(n)$ be OEIS A354242. For every integer $k\ge1$,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A354242.
