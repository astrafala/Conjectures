...
\title{A proof of the conjectured recurrence for OEIS A054341}
...
\begin{abstract}
OEIS A054341 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 - 2 x$, a polynomial of degree $1$, which proves
...
OEIS A054341 is ``Row sums of triangle A054336 (central binomial convolutions)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 12, 30, 74, 185, 460,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{- 4 x + \sqrt{1 - 4 x^{2}} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n+1)*a(n) +5*(-n-1)*a(n-1) +8*(-n+2)*a(n-2) +20*(n-2)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 30 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-02, revision 59) the
...
\[
p_{0}(n)=2 \left(n + 1\right),\qquad p_{1}(n)=- 5 \left(n + 1\right),\qquad p_{2}(n)=- 8 \left(n - 2\right),\qquad p_{3}(n)=20 \left(n - 2\right)
\]
...
\section{The computation for A054341}
...
\[
2 \left(n + 1\right)\,a(n) -  5 \left(n + 1\right)\,a(n-1) -  8 \left(n - 2\right)\,a(n-2) + 20 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A054341, agreeing on all 13 terms compared.
...
vanish, for all 28 values of $n$ from $n=3$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=2 - 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A054341.
