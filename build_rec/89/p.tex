...
\title{A proof of the conjectured recurrence for OEIS A157003}
...
\begin{abstract}
OEIS A157003 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 15 x^{3} - 3 x^{2} + 5 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A157003 is ``Transform of Catalan numbers whose Hankel transform gives the Somos-4 sequence''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 4, 10, 27, 78, 234,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{- 4 x \left(1 - x^{2}\right) + 1} - 1}{2 x \left(x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(n+2)*a(n-1) +(-21*n+29)*a(n-2) +(3*n-16)*a(n-3) +40*(n-3)*a(n-4) +2*(-2*n+7)*a(n-5) +10*(-2*n+9)*a(n-6)=0. - \emph{R. J. Mathar}, Nov 19 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 37) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=n + 2,\qquad p_{2}(n)=29 - 21 n,\qquad p_{3}(n)=3 n - 16,\qquad p_{4}(n)=40 \left(n - 3\right),\qquad p_{5}(n)=- 2 \left(2 n - 7\right),\qquad p_{6}(n)=- 10 \left(2 n - 9\right)
\]
...
\section{The computation for A157003}
...
\[
(n + 1)\,a(n) + (n + 2)\,a(n-1) + (29 - 21 n)\,a(n-2) + (3 n - 16)\,a(n-3) + 40 \left(n - 3\right)\,a(n-4) -  2 \left(2 n - 7\right)\,a(n-5) -  10 \left(2 n - 9\right)\,a(n-6) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A157003, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 15 x^{3} - 3 x^{2} + 5 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A157003.
