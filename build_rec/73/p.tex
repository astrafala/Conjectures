...
\title{A proof of the conjectured recurrence for OEIS A100097}
...
\begin{abstract}
OEIS A100097 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A100097 is ``An inverse Chebyshev transform of the Pell numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 2, 8, 20, 64, 172, 512,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{x \left(2 x + \sqrt{1 - 4 x^{2}}\right)}{\sqrt{1 - 4 x^{2}} \left(8 x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+2)*a(n) +(-n+3)*a(n-1) +4*(3*n-7)*a(n-2) +4*(3*n-10)*a(n-3) +32*(-n+3)*a(n-4) +32*(-n+4)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-17, revision 26) the
...
\[
p_{0}(n)=2 - n,\qquad p_{1}(n)=3 - n,\qquad p_{2}(n)=4 \left(3 n - 7\right),\qquad p_{3}(n)=4 \left(3 n - 10\right),\qquad p_{4}(n)=- 32 \left(n - 3\right),\qquad p_{5}(n)=- 32 \left(n - 4\right)
\]
...
\section{The computation for A100097}
...
\[
(2 - n)\,a(n) + (3 - n)\,a(n-1) + 4 \left(3 n - 7\right)\,a(n-2) + 4 \left(3 n - 10\right)\,a(n-3) -  32 \left(n - 3\right)\,a(n-4) -  32 \left(n - 4\right)\,a(n-5) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A100097, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A100097.
