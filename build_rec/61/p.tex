...
\title{A proof of the conjectured recurrence for OEIS A064091}
...
\begin{abstract}
OEIS A064091 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $120 x$, a polynomial of degree $1$, which proves
...
OEIS A064091 is ``Generalized Catalan numbers C(8; n)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 9, 145, 2905, 65121, 1563561, 39322929,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{16}{\sqrt{1 - 32 x} + 15} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 7*n*a(n) +(-223*n+336)*a(n-1) +16*(-2*n+3)*a(n-2)=0. - \emph{R. J. Mathar}, Jun 07 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 26) the
...
\[
p_{0}(n)=7 n,\qquad p_{1}(n)=336 - 223 n,\qquad p_{2}(n)=- 16 \left(2 n - 3\right)
\]
...
\section{The computation for A064091}
...
\[
7 n\,a(n) + (336 - 223 n)\,a(n-1) -  16 \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A064091, agreeing on all 13 terms compared.
...
vanish, for all 15 values of $n$ from $n=2$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=120 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A064091.
