...
\title{A proof of the conjectured recurrence for OEIS A100096}
...
\begin{abstract}
OEIS A100096 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A100096 is ``An inverse Chebyshev transform of the Jacobsthal numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 1, 6, 9, 36, 66, 218,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x \left(- 6 x + \sqrt{1 - 4 x^{2}} + 3\right)}{2 \left(20 x^{3} - 8 x^{2} - 5 x + 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(-n+1)*a(n) +(n+3)*a(n-1) +18*(n-2)*a(n-2) +4*(-n-2)*a(n-3) +40*(-n+3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-16, revision 18) the
...
\[
p_{0}(n)=- 2 \left(n - 1\right),\qquad p_{1}(n)=n + 3,\qquad p_{2}(n)=18 \left(n - 2\right),\qquad p_{3}(n)=- 4 \left(n + 2\right),\qquad p_{4}(n)=- 40 \left(n - 3\right)
\]
...
\section{The computation for A100096}
...
\[
- 2 \left(n - 1\right)\,a(n) + (n + 3)\,a(n-1) + 18 \left(n - 2\right)\,a(n-2) -  4 \left(n + 2\right)\,a(n-3) -  40 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A100096, agreeing on all 13 terms compared.
...
vanish, for all 26 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=3 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A100096.
