...
\title{A proof of the conjectured recurrence for OEIS A064092}
...
\begin{abstract}
OEIS A064092 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $153 x$, a polynomial of degree $1$, which proves
...
OEIS A064092 is ``Generalized Catalan numbers C(9; n)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 10, 181, 4078, 102826, 2777212, 78571837,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{18}{\sqrt{1 - 36 x} + 17} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 8*n*a(n) +(-287*n+432)*a(n-1) +18*(-2*n+3)*a(n-2)=0. - \emph{R. J. Mathar}, Jun 07 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 22) the
...
\[
p_{0}(n)=8 n,\qquad p_{1}(n)=432 - 287 n,\qquad p_{2}(n)=- 18 \left(2 n - 3\right)
\]
...
\section{The computation for A064092}
...
\[
8 n\,a(n) + (432 - 287 n)\,a(n-1) -  18 \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A064092, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=153 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A064092.
