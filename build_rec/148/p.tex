...
\title{A proof of the conjectured recurrence for OEIS A128743}
...
\begin{abstract}
OEIS A128743 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A128743 is ``Number of UU's (i.e., doublerises) in all skew Dyck paths of semilength n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 2, 13, 69, 346, 1700, 8286,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} - 4 x + \left(x - 1\right) \sqrt{5 x^{2} - 6 x + 1} + 1}{2 x \sqrt{5 x^{2} - 6 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*(n-2)\^{}2*a(n) -(n-1)*(6*n\^{}2-15*n+4)*a(n-1) +5*(n-2)*(n-1)\^{}2*a(n-2)=0. - \emph{R. J. Mathar}, Jun 17 2016
\end{quote}
...
\[
p_{0}(n)=\left(n - 2\right)^{2} \left(n + 1\right),\qquad p_{1}(n)=- \left(n - 1\right) \left(6 n^{2} - 15 n + 4\right),\qquad p_{2}(n)=5 \left(n - 2\right) \left(n - 1\right)^{2}
\]
...
\section{The computation for A128743}
...
\[
\left(n - 2\right)^{2} \left(n + 1\right)\,a(n) -  \left(n - 1\right) \left(6 n^{2} - 15 n + 4\right)\,a(n-1) + 5 \left(n - 2\right) \left(n - 1\right)^{2}\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A128743, agreeing on all 13 terms compared.
...
vanish, for all 21 values of $n$ from $n=2$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A128743.
