...
\title{A proof of the conjectured recurrence for OEIS A166696}
...
\begin{abstract}
OEIS A166696 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A166696 is ``A transform of A103210''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 21, 162, 1365, 12219, 114156, 1100649,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} - 3 x - \sqrt{x^{4} - 14 x^{3} + 27 x^{2} - 14 x + 1} + 1}{4 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +7*(-2*n+1)*a(n-1) +27*(n-2)*a(n-2) +7*(-2*n+7)*a(n-3) +(n-5)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 10 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2016-05-24, revision 12) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 7 \left(2 n - 1\right),\qquad p_{2}(n)=27 \left(n - 2\right),\qquad p_{3}(n)=- 7 \left(2 n - 7\right),\qquad p_{4}(n)=n - 5
\]
...
\[
(n + 1)\,a(n) -  7 \left(2 n - 1\right)\,a(n-1) + 27 \left(n - 2\right)\,a(n-2) -  7 \left(2 n - 7\right)\,a(n-3) + (n - 5)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A166696, agreeing on all 13 terms compared.
...
vanish, for all 14 values of $n$ from $n=5$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A166696.
