...
\title{A proof of the conjectured recurrence for OEIS A098662}
...
\begin{abstract}
OEIS A098662 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A098662 is ``Expansion of e.g.f. BesselI(0,2*sqrt(3)*x) + BesselI(1,2*sqrt(3)*x)/sqrt(3)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 6, 9, 54, 90, 540, 945,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{6 x - \sqrt{1 - 12 x^{2}} + 1}{6 x \sqrt{1 - 12 x^{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 6(n-1)*a(n-1) - 12n*a(n-2) + 72*(2-n)*a(n-3) = 0. - \emph{R. J. Mathar}, Dec 08 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-16, revision 24) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=6 \left(n - 1\right),\qquad p_{2}(n)=- 12 n,\qquad p_{3}(n)=- 72 \left(n - 2\right)
\]
...
\section{The computation for A098662}
...
\[
(n + 1)\,a(n) + 6 \left(n - 1\right)\,a(n-1) -  12 n\,a(n-2) -  72 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A098662, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A098662.
