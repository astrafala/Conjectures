...
\title{A proof of the conjectured recurrence for OEIS A171416}
...
\begin{abstract}
OEIS A171416 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{4} + 3 x^{3} + 5 x^{2} + 5 x + 2$, a polynomial of degree $4$, which proves
...
OEIS A171416 is ``A sequence with Somos-4 Hankel transform''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 3, 7, 13, 31, 65,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{2} - \sqrt{x^{4} - 4 x^{3} - 6 x^{2} + 1} + 1}{2 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +(n+1)*a(n-1) +6*(1-n)*a(n-2) +2*(11-5*n)*a(n-3) +(10-3*n)*a(n-4) +(n-5)*a(n-5)=0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 35) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=n + 1,\qquad p_{2}(n)=- 6 \left(n - 1\right),\qquad p_{3}(n)=- 2 \left(5 n - 11\right),\qquad p_{4}(n)=10 - 3 n,\qquad p_{5}(n)=n - 5
\]
...
\section{The computation for A171416}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x + 1\right) \left(x^{3} - 5 x^{2} - x + 1\right) .
\]
...
\[
(n + 2)\,a(n) + (n + 1)\,a(n-1) -  6 \left(n - 1\right)\,a(n-2) -  2 \left(5 n - 11\right)\,a(n-3) + (10 - 3 n)\,a(n-4) + (n - 5)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A171416, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=x^{4} + 3 x^{3} + 5 x^{2} + 5 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A171416.
