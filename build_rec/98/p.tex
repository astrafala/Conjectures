...
\title{A proof of the conjectured recurrence for OEIS A186341}
...
\begin{abstract}
OEIS A186341 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $12 x^{7} - 12 x^{6} - 31 x^{5} + 5 x^{4} + 20 x^{3} - x^{2} - 7 x + 2$, a polynomial of degree $7$, which proves
...
OEIS A186341 is ``a(n)=sum\{k=0..floor(n/2), binomial(n-k,k)*A186338(k)\}''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 5, 15, 33, 95, 237,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{3 x^{2} + x + \sqrt{\frac{16 x^{6} + 25 x^{5} - 15 x^{4} - 19 x^{3} + 7 x^{2} + 3 x - 1}{x - 1}} - 1}{2 x^{2} \left(2 x^{2} + x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +5*(-n-1)*a(n-1) +2*(-n+3)*a(n-2) +(38*n-59)*a(n-3) +(-22*n+41)*a(n-4) +4*(-22*n+81)*a(n-5) +3*(19*n-79)*a(n-6) +3*(29*n-164)*a(n-7) +2*(-17*n+98)*a(n-8) +16*(-2*n+15)*a(n-9)=0. - \emph{R. J. Mathar}, Oct 08 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-10-08, revision 13) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 5 \left(n + 1\right),\qquad p_{2}(n)=- 2 \left(n - 3\right),\qquad p_{3}(n)=38 n - 59,\qquad p_{4}(n)=41 - 22 n,\qquad p_{5}(n)=- 4 \left(22 n - 81\right),\qquad p_{6}(n)=3 \left(19 n - 79\right),\qquad p_{7}(n)=3 \left(29 n - 164\right),\qquad p_{8}(n)=- 2 \left(17 n - 98\right),\qquad p_{9}(n)=- 16 \left(2 n - 15\right)
\]
...
\section{The computation for A186341}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \frac{\left(x^{2} + x - 1\right) \left(16 x^{4} + 9 x^{3} - 8 x^{2} - 2 x + 1\right)}{x - 1} .
\]
...
\[
B(x)\;=\;12 x^{7} - 12 x^{6} - 31 x^{5} + 5 x^{4} + 20 x^{3} - x^{2} - 7 x + 2 ,
\]
...
\[
(n + 2)\,a(n) -  5 \left(n + 1\right)\,a(n-1) -  2 \left(n - 3\right)\,a(n-2) + (38 n - 59)\,a(n-3) + (41 - 22 n)\,a(n-4) -  4 \left(22 n - 81\right)\,a(n-5) + 3 \left(19 n - 79\right)\,a(n-6) + 3 \left(29 n - 164\right)\,a(n-7) -  2 \left(17 n - 98\right)\,a(n-8) -  16 \left(2 n - 15\right)\,a(n-9) \;=\;0
\]
holds for every $n>7$.
\end{theorem}
...
section of A186341, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=12 x^{7} - 12 x^{6} - 31 x^{5} + 5 x^{4} + 20 x^{3} - x^{2} - 7 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A186341.
