...
\title{A proof of the conjectured recurrence for OEIS A105864}
...
\begin{abstract}
OEIS A105864 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A105864 is ``Expansion of (1/(1-x\^{}2))*c(x/(1-x\^{}2)), where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 7, 21, 65, 215, 735,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{x^{2} + 4 x - 1}{x^{2} - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 2*(1-2*n)*a(n-1) + 2*(1-n)*a(n-2) + 2*(2*n-3)*a(n-3) + (n-3)*a(n-4) = 0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2024-09-01, revision 43) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(2 n - 1\right),\qquad p_{2}(n)=- 2 \left(n - 1\right),\qquad p_{3}(n)=2 \left(2 n - 3\right),\qquad p_{4}(n)=n - 3
\]
...
\section{The computation for A105864}
...
\[
(n + 1)\,a(n) -  2 \left(2 n - 1\right)\,a(n-1) -  2 \left(n - 1\right)\,a(n-2) + 2 \left(2 n - 3\right)\,a(n-3) + (n - 3)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A105864, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A105864.
