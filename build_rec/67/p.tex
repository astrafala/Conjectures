...
\title{A proof of the conjectured recurrence for OEIS A098520}
...
\begin{abstract}
OEIS A098520 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A098520 is ``E.g.f. exp(x)*BesselI(1,4*x)/2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 2, 15, 52, 285, 1206, 6027,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{2 x}{15 x^{2} + 2 x + \left(x - 1\right) \sqrt{- 15 x^{2} - 2 x + 1} - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n-1)*(n+1)*a(n) - n*(2n-1)*a(n-1) - 15*n*(n-1)*a(n-2) = 0. - \emph{R. J. Mathar}, Dec 11 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2021-05-23, revision 19) the
...
\[
p_{0}(n)=\left(n - 1\right) \left(n + 1\right),\qquad p_{1}(n)=- n \left(2 n - 1\right),\qquad p_{2}(n)=- 15 n \left(n - 1\right)
\]
...
\section{The computation for A098520}
...
\[
\left(n - 1\right) \left(n + 1\right)\,a(n) -  n \left(2 n - 1\right)\,a(n-1) -  15 n \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A098520, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A098520.
