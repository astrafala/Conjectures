...
\title{A proof of the conjectured recurrence for OEIS A102319}
...
\begin{abstract}
OEIS A102319 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $0$, a polynomial of degree $-1$, which proves
...
OEIS A102319 is ``A mean binomial transform of the central binomial numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 7, 26, 107, 462, 2065, 9438,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1}{2 \sqrt{5 x^{2} - 6 x + 1}} + \frac{1}{2 \sqrt{- 3 x^{2} - 2 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:}+n*(n-1)*(12*n\^{}2-48*n+41)*a(n) -8*(n-1)*(12*n\^{}3-54*n\^{}2+65*n-17)*a(n-1) +2*(84*n\^{}4-504*n\^{}3+1025*n\^{}2-775*n+131)*a(n-2) +8*(n-2)*(12*n\^{}3-54*n\^{}2+65*n-20)*a(n-3) -15*(n-2)*(n-3)*(12*n\^{}2-24*n+5)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 20 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2024-08-31, revision 20) the
...
\[
p_{0}(n)=n \left(n - 1\right) \left(12 n^{2} - 48 n + 41\right),\qquad p_{1}(n)=- 8 \left(n - 1\right) \left(12 n^{3} - 54 n^{2} + 65 n - 17\right),\qquad p_{2}(n)=2 \left(84 n^{4} - 504 n^{3} + 1025 n^{2} - 775 n + 131\right),\qquad p_{3}(n)=8 \left(n - 2\right) \left(12 n^{3} - 54 n^{2} + 65 n - 20\right),\qquad p_{4}(n)=- 15 \left(n - 3\right) \left(n - 2\right) \left(12 n^{2} - 24 n + 5\right)
\]
...
\section{The computation for A102319}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)\bigl[\sqrt{D_{1}},\dots,\sqrt{D_{2}}\bigr]$ with
\[
D \;=\; \left(x - 1\right) \left(5 x - 1\right),\quad - \left(x + 1\right) \left(3 x - 1\right) .
\]
...
\[
n \left(n - 1\right) \left(12 n^{2} - 48 n + 41\right)\,a(n) -  8 \left(n - 1\right) \left(12 n^{3} - 54 n^{2} + 65 n - 17\right)\,a(n-1) + 2 \left(84 n^{4} - 504 n^{3} + 1025 n^{2} - 775 n + 131\right)\,a(n-2) + 8 \left(n - 2\right) \left(12 n^{3} - 54 n^{2} + 65 n - 20\right)\,a(n-3) -  15 \left(n - 3\right) \left(n - 2\right) \left(12 n^{2} - 24 n + 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A102319, agreeing on all 13 terms compared.
...
vanish, for all 17 values of $n$ from $n=6$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A102319.
