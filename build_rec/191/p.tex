...
\title{A proof of the conjectured recurrence for OEIS A185020}
...
\begin{abstract}
OEIS A185020 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A185020 is ``a(n) = A000108(n)*A002605(n+1), where A000108 are the Catalan numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 12, 80, 616, 5040, 43296, 384384,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{- 24 x - 6 \sqrt{- 32 x^{2} - 8 x + 1} + 6}}{12 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +n*(n+1)*a(n) -4*n*(2*n-1)*a(n-1) -8*(2*n-1)*(2*n-3)*a(n-2)=0. - \emph{R. J. Mathar}, Oct 08 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2024-12-14, revision 24) the
...
\[
p_{0}(n)=n \left(n + 1\right),\qquad p_{1}(n)=- 4 n \left(2 n - 1\right),\qquad p_{2}(n)=- 8 \left(2 n - 3\right) \left(2 n - 1\right)
\]
...
\section{The computation for A185020}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)\bigl[\sqrt{D_{1}},\dots,\sqrt{D_{3}}\bigr]$ with
\[
D \;=\; - 4 x - \sqrt{- 32 x^{2} - 8 x + 1} + 1,\quad 6,\quad - 32 x^{2} - 8 x + 1 .
\]
...
\[
n \left(n + 1\right)\,a(n) -  4 n \left(2 n - 1\right)\,a(n-1) -  8 \left(2 n - 3\right) \left(2 n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A185020, agreeing on all 13 terms compared.
...
vanish, for all 20 values of $n$ from $n=2$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A185020.
