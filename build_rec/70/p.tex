...
\title{A proof of the conjectured recurrence for OEIS A099363}
...
\begin{abstract}
OEIS A099363 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 - 6 x$, a polynomial of degree $1$, which proves
...
OEIS A099363 is ``An inverse Chebyshev transform of 1-x''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -1, 1, -2, 2, -5, 5, -14,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} - \frac{\left(x - 1\right) \left(\sqrt{1 - 4 x^{2}} - 1\right)}{2}}{x^{3}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+3)*a(n) +(-n-1)*a(n-1) -4*n*a(n-2) +4*(n-2)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 26 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 24) the
...
\[
p_{0}(n)=n + 3,\qquad p_{1}(n)=- n - 1,\qquad p_{2}(n)=- 4 n,\qquad p_{3}(n)=4 \left(n - 2\right)
\]
...
\section{The computation for A099363}
...
\[
(n + 3)\,a(n) + (- n - 1)\,a(n-1) -  4 n\,a(n-2) + 4 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A099363, agreeing on all 13 terms compared.
...
vanish, for all 36 values of $n$ from $n=3$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=3 - 6 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A099363.
