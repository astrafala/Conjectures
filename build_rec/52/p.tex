...
\title{A proof of the conjectured recurrence for OEIS A025268}
...
\begin{abstract}
OEIS A025268 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 40 x^{5} - 23 x^{4} - 3 x^{3} + 5 x^{2} + x$, a polynomial of degree $5$, which proves
...
OEIS A025268 is ``a(n) = a(1)*a(n-1) + a(2)*a(n-2) + ...+ a(n-1)*a(1) for n >= 5, with initial values 1,1,1,1''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 1, 1, 4, 11, 32, 95,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{1}{2} - \frac{\sqrt{8 x^{4} + 4 x^{3} - 4 x + 1}}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(n+1)*a(n-1) +10*(-2*n+5)*a(n-2) +2*(2*n-9)*a(n-3) +2*(14*n-79)*a(n-4) +40*(n-7)*a(n-5)=0. - \emph{R. J. Mathar}, Jan 25 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-02-04, revision 27) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=n + 1,\qquad p_{2}(n)=- 10 \left(2 n - 5\right),\qquad p_{3}(n)=2 \left(2 n - 9\right),\qquad p_{4}(n)=2 \left(14 n - 79\right),\qquad p_{5}(n)=40 \left(n - 7\right)
\]
...
\section{The computation for A025268}
...
\[
B(x)\;=\;- 40 x^{5} - 23 x^{4} - 3 x^{3} + 5 x^{2} + x ,
\]
...
\[
n\,a(n) + (n + 1)\,a(n-1) -  10 \left(2 n - 5\right)\,a(n-2) + 2 \left(2 n - 9\right)\,a(n-3) + 2 \left(14 n - 79\right)\,a(n-4) + 40 \left(n - 7\right)\,a(n-5) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A025268, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 40 x^{5} - 23 x^{4} - 3 x^{3} + 5 x^{2} + x$ is an
...
\url{https://oeis.org}, 2026. Sequence A025268.
