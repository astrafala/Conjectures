...
\title{A proof of the conjectured recurrence for OEIS A104722}
...
\begin{abstract}
OEIS A104722 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 x^{3} + 12 x^{2} + 12 x + 4$, a polynomial of degree $3$, which proves
...
OEIS A104722 is ``Self-convolution of repeated Catalan numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 3, 4, 7, 10, 19, 28,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x + 1\right)^{2} \left(\sqrt{1 - 4 x^{2}} - 1\right)^{2}}{4 x^{4}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+4)*a(n) + (n+1)*a(n-1) - 4*(n+1)*a(n-2) + 4*(2-n)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 09 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-07-02, revision 36) the
...
\[
p_{0}(n)=n + 4,\qquad p_{1}(n)=n + 1,\qquad p_{2}(n)=- 4 \left(n + 1\right),\qquad p_{3}(n)=- 4 \left(n - 2\right)
\]
...
\section{The computation for A104722}
...
\[
(n + 4)\,a(n) + (n + 1)\,a(n-1) -  4 \left(n + 1\right)\,a(n-2) -  4 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A104722, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=4 x^{3} + 12 x^{2} + 12 x + 4$ is an
...
\url{https://oeis.org}, 2026. Sequence A104722.
