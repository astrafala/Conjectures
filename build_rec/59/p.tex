...
\title{A proof of the conjectured recurrence for OEIS A064089}
...
\begin{abstract}
OEIS A064089 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $66 x$, a polynomial of degree $1$, which proves
...
OEIS A064089 is ``Generalized Catalan numbers C(6; n)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 7, 85, 1279, 21517, 387607, 7312789,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{12}{\sqrt{1 - 24 x} + 11} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 5*n*a(n) +(-119*n+180)*a(n-1) +12*(-2*n+3)*a(n-2)=0. - \emph{R. J. Mathar}, Jun 07 2013
\end{quote}
...
\[
p_{0}(n)=5 n,\qquad p_{1}(n)=180 - 119 n,\qquad p_{2}(n)=- 12 \left(2 n - 3\right)
\]
...
\section{The computation for A064089}
...
\[
5 n\,a(n) + (180 - 119 n)\,a(n-1) -  12 \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A064089, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=66 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A064089.
