...
\title{A proof of the conjectured recurrence for OEIS A185089}
...
\begin{abstract}
OEIS A185089 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{3} - x^{2} - 3 x + 2$, a polynomial of degree $3$, which proves
...
OEIS A185089 is ``A transform of the little Schroeder numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 3, 7, 14, 35, 81,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{2} + x + \sqrt{x^{4} + 6 x^{3} - 5 x^{2} - 2 x + 1} - 1}{4 x^{2} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
conjecture: (n+2)*a(n) -3*(n+1)*a(n-1) +3*(2-n)*a(n-2) +(11*n-20)*a(n-3) +(11-5*n)*a(n-4) +(4-n)*a(n-5) =0. - \emph{R. J. Mathar}, Nov 16 2011 (Formula verified and used for computations. - \emph{Fung Lam}, Feb 24 2014)
\end{quote}
...
\section{The computation for A185089}
...
section of A185089, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=x^{3} - x^{2} - 3 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A185089.
