...
\title{A proof of the conjectured recurrence for OEIS A114121}
...
\begin{abstract}
OEIS A114121 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A114121 is ``Expansion of (sqrt(1 - 4*x) + (1 - 2*x))/(2*(1 - 4*x))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 7, 26, 99, 382, 1486, 5812,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 4 x + \sqrt{1 - 4 x} \left(1 - 2 x\right) + 1}{2 \left(1 - 4 x\right)^{\frac{3}{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + 2*(3 - 4*n)*a(n-1) + 8*(2*n-3)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 07 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-04-16, revision 64) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(4 n - 3\right),\qquad p_{2}(n)=8 \left(2 n - 3\right)
\]
...
\section{The computation for A114121}
...
\[
n\,a(n) -  2 \left(4 n - 3\right)\,a(n-1) + 8 \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A114121, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A114121.
