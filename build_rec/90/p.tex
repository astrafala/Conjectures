...
\title{A proof of the conjectured recurrence for OEIS A157021}
...
\begin{abstract}
OEIS A157021 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 32 x^{3} + 160 x^{2} - 240 x + 128$, a polynomial of degree $3$, which proves
...
OEIS A157021 is ``a(n)=sum\{k=0..floor(n/2), C(n,2k)*A000108(floor(k/2))\}. Inverse binomial transform is aeration of doubled Catalan numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 4, 8, 16, 32, 64,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x - 1\right) \left(\sqrt{\frac{- 4 x^{4} + \left(1 - x\right)^{4}}{\left(1 - x\right)^{4}}} - 1\right) \left(2 x^{2} - 2 x + 1\right)}{2 x^{4}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:}+(n+4)*(n\^{}2-11*n+32)*a(n) +(-5*n\^{}3+41*n\^{}2-26*n-360)*a(n-1) +2*(5*n\^{}3-47*n\^{}2+112*n+64)*a(n-2) +2*(-5*n\^{}3+53*n\^{}2-182*n+168)*a(n-3) +(n-4)*(n\^{}2-3*n-8)*a(n-4) +3*(n-4)*(n\^{}2-9*n+22)*a(n-5)=0. - \emph{R. J. Mathar}, Feb 05 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2017-10-30, revision 7) the
...
\[
p_{0}(n)=\left(n + 4\right) \left(n^{2} - 11 n + 32\right),\qquad p_{1}(n)=- 5 n^{3} + 41 n^{2} - 26 n - 360,\qquad p_{2}(n)=2 \left(5 n^{3} - 47 n^{2} + 112 n + 64\right),\qquad p_{3}(n)=- 2 \left(5 n^{3} - 53 n^{2} + 182 n - 168\right),\qquad p_{4}(n)=\left(n - 4\right) \left(n^{2} - 3 n - 8\right),\qquad p_{5}(n)=3 \left(n - 4\right) \left(n^{2} - 9 n + 22\right)
\]
...
\section{The computation for A157021}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \frac{\left(x^{2} + 2 x - 1\right) \left(3 x^{2} - 2 x + 1\right)}{\left(x - 1\right)^{4}} .
\]
...
\[
\left(n + 4\right) \left(n^{2} - 11 n + 32\right)\,a(n) + (- 5 n^{3} + 41 n^{2} - 26 n - 360)\,a(n-1) + 2 \left(5 n^{3} - 47 n^{2} + 112 n + 64\right)\,a(n-2) -  2 \left(5 n^{3} - 53 n^{2} + 182 n - 168\right)\,a(n-3) + \left(n - 4\right) \left(n^{2} - 3 n - 8\right)\,a(n-4) + 3 \left(n - 4\right) \left(n^{2} - 9 n + 22\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A157021, agreeing on all 13 terms compared.
...
vanish, for all 25 values of $n$ from $n=7$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 32 x^{3} + 160 x^{2} - 240 x + 128$ is an
...
\url{https://oeis.org}, 2026. Sequence A157021.
