...
\title{A proof of the conjectured recurrence for OEIS A154623}
...
\begin{abstract}
OEIS A154623 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A154623 is ``Expansion of 1+(x/(1-5*x))*c(x/(1-5*x)) where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 6, 37, 235, 1539, 10392, 72267,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{3}{2} - \frac{\sqrt{\frac{9 x - 1}{5 x - 1}}}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + 2*(8-7*n)*a(n-1) + 45*(n-2)*a(n-2) = 0. - \emph{R. J. Mathar}, Dec 14 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-08, revision 20) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(7 n - 8\right),\qquad p_{2}(n)=45 \left(n - 2\right)
\]
...
\section{The computation for A154623}
...
\[
n\,a(n) -  2 \left(7 n - 8\right)\,a(n-1) + 45 \left(n - 2\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A154623, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A154623.
