...
\title{A proof of the conjectured recurrence for OEIS A179648}
...
\begin{abstract}
OEIS A179648 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{2} + 1$, a polynomial of degree $2$, which proves
...
OEIS A179648 is ``Expansion of (1/(1+4x-2x\^{}2))*c(x/(1+4x-2x\^{}2)), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -3, 12, -47, 190, -778, 3224, -13475,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{\sqrt{2 x^{2} - 1}}{2 x \sqrt{2 x^{2} - 4 x - 1}} + \frac{1}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(2n+1)*a(n-1) +4*(1-n)*a(n-2) +4*(5-2n)*a(n-3) +4*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 17 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2022-09-08, revision 18) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=2 \left(2 n + 1\right),\qquad p_{2}(n)=- 4 \left(n - 1\right),\qquad p_{3}(n)=- 4 \left(2 n - 5\right),\qquad p_{4}(n)=4 \left(n - 3\right)
\]
...
\[
(n + 1)\,a(n) + 2 \left(2 n + 1\right)\,a(n-1) -  4 \left(n - 1\right)\,a(n-2) -  4 \left(2 n - 5\right)\,a(n-3) + 4 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A179648, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{2} + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A179648.
