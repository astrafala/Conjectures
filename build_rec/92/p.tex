...
\title{A proof of the conjectured recurrence for OEIS A162548}
...
\begin{abstract}
OEIS A162548 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{4} + x^{3} - x + 1$, a polynomial of degree $4$, which proves
...
OEIS A162548 is ``A Chebyshev transform of the little Schroeder numbers A001003''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 9, 37, 156, 695, 3203,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} + x - \sqrt{x^{4} - 6 x^{3} + 3 x^{2} - 6 x + 1} + 1}{4 x \left(x^{2} + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:}  (n+1)*a(n) +3*(-2*n+1)*a(n-1) +(4*n-5)*a(n-2) +12*(-n+2)*a(n-3) +(4*n-11)*a(n-4) +3*(-2*n+7)*a(n-5) +(n-5)*a(n-6)=0. - \emph{R. J. Mathar}, Nov 15 2012. (Formula verified and used for computations. - \emph{Fung Lam}, Feb 19 2014)
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 3 \left(2 n - 1\right),\qquad p_{2}(n)=4 n - 5,\qquad p_{3}(n)=- 12 \left(n - 2\right),\qquad p_{4}(n)=4 n - 11,\qquad p_{5}(n)=- 3 \left(2 n - 7\right),\qquad p_{6}(n)=n - 5
\]
...
\[
(n + 1)\,a(n) -  3 \left(2 n - 1\right)\,a(n-1) + (4 n - 5)\,a(n-2) -  12 \left(n - 2\right)\,a(n-3) + (4 n - 11)\,a(n-4) -  3 \left(2 n - 7\right)\,a(n-5) + (n - 5)\,a(n-6) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A162548, agreeing on all 13 terms compared.
...
vanish, for all 18 values of $n$ from $n=6$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{4} + x^{3} - x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A162548.
