...
\title{A proof of the conjectured recurrence for OEIS A106272}
...
\begin{abstract}
OEIS A106272 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A106272 is ``Antidiagonal sums of number triangle A106270''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -1, -1, -6, -15, -48, -147, -477,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 4 x - \sqrt{1 - 4 x} + 1}{2 x \left(x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 2*(1-2*n)*a(n-1) - (n+1)*a(n-2) + 2*(2*n-1)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 09 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2024-09-02, revision 33) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(2 n - 1\right),\qquad p_{2}(n)=- n - 1,\qquad p_{3}(n)=2 \left(2 n - 1\right)
\]
...
\section{The computation for A106272}
...
\[
(n + 1)\,a(n) -  2 \left(2 n - 1\right)\,a(n-1) + (- n - 1)\,a(n-2) + 2 \left(2 n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A106272, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A106272.
