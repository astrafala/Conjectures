...
\title{A proof of the conjectured recurrence for OEIS A157100}
...
\begin{abstract}
OEIS A157100 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A157100 is ``Transform of Catalan numbers whose Hankel transform satisfies the Somos-4 recurrence''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 3, 6, 14, 37, 105, 312,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{4 x^{3} - 4 x + 1} - 1}{2 x \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(-5*n+1)*a(n-1) +2*(2*n-1)*a(n-2) +2*(2*n-7)*a(n-3) +2*(-2*n+7)*a(n-4) = 0. - \emph{R. J. Mathar}, Feb 05 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 14) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=1 - 5 n,\qquad p_{2}(n)=2 \left(2 n - 1\right),\qquad p_{3}(n)=2 \left(2 n - 7\right),\qquad p_{4}(n)=- 2 \left(2 n - 7\right)
\]
...
\section{The computation for A157100}
...
\[
(n + 1)\,a(n) + (1 - 5 n)\,a(n-1) + 2 \left(2 n - 1\right)\,a(n-2) + 2 \left(2 n - 7\right)\,a(n-3) -  2 \left(2 n - 7\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A157100, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A157100.
