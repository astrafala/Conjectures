...
\title{A proof of the conjectured recurrence for OEIS A102879}
...
\begin{abstract}
OEIS A102879 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A102879 is ``A Chebyshev transform of the first kind of the central binomial numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 4, 14, 48, 162, 556, 1934,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - x^{2}}{\sqrt{x^{4} - 4 x^{3} + 2 x^{2} - 4 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-3)*a(n) - 2*(2*n-1)*(n-3)*a(n-1) + 2*(2-4*n+n\^{}2)*a(n-2) - 2*(n-1)*(2*n-7)*a(n-3) + (n-1)*(n-4)*a(n-4) = 0. - \emph{R. J. Mathar}, Nov 09 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 21) the
...
\[
p_{0}(n)=n \left(n - 3\right),\qquad p_{1}(n)=- 2 \left(n - 3\right) \left(2 n - 1\right),\qquad p_{2}(n)=2 \left(n^{2} - 4 n + 2\right),\qquad p_{3}(n)=- 2 \left(n - 1\right) \left(2 n - 7\right),\qquad p_{4}(n)=\left(n - 4\right) \left(n - 1\right)
\]
...
\section{The computation for A102879}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x^{2} + 1\right) \left(x^{2} - 4 x + 1\right) .
\]
...
\[
n \left(n - 3\right)\,a(n) -  2 \left(n - 3\right) \left(2 n - 1\right)\,a(n-1) + 2 \left(n^{2} - 4 n + 2\right)\,a(n-2) -  2 \left(n - 1\right) \left(2 n - 7\right)\,a(n-3) + \left(n - 4\right) \left(n - 1\right)\,a(n-4) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A102879, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A102879.
