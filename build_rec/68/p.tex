...
\title{A proof of the conjectured recurrence for OEIS A098521}
...
\begin{abstract}
OEIS A098521 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A098521 is ``E.g.f. exp(x)*BesselI(2,2*sqrt(2)*x)/2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 3, 14, 50, 195, 721,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 3 x^{2} - 2 x + \left(x - 1\right) \sqrt{- 7 x^{2} - 2 x + 1} + 1}{8 x^{2} \sqrt{- 7 x^{2} - 2 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) -(4n+3)*a(n-1) -3*(2n+1)*a(n-2) +(20n-29)*a(n-3) +21*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Dec 08 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2022-09-08, revision 23) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 4 n - 3,\qquad p_{2}(n)=- 3 \left(2 n + 1\right),\qquad p_{3}(n)=20 n - 29,\qquad p_{4}(n)=21 \left(n - 3\right)
\]
...
\section{The computation for A098521}
...
\[
(n + 2)\,a(n) + (- 4 n - 3)\,a(n-1) -  3 \left(2 n + 1\right)\,a(n-2) + (20 n - 29)\,a(n-3) + 21 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A098521, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=4 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A098521.
