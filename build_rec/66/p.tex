...
\title{A proof of the conjectured recurrence for OEIS A097331}
...
\begin{abstract}
OEIS A097331 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{2} + 3 x + 1$, a polynomial of degree $2$, which proves
...
OEIS A097331 is ``Expansion of 1 + 2x/(1 + sqrt(1 - 4x\^{}2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 0, 1, 0, 2, 0, 5,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x - \sqrt{1 - 4 x^{2}} + 1}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + n*a(n-1) + 4*(-n+2)*a(n-2) + 4*(-n+3)*a(n-3)=0. - \emph{R. J. Mathar}, Dec 02 2012
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=n,\qquad p_{2}(n)=- 4 \left(n - 2\right),\qquad p_{3}(n)=- 4 \left(n - 3\right)
\]
...
\section{The computation for A097331}
...
\[
(n + 1)\,a(n) + n\,a(n-1) -  4 \left(n - 2\right)\,a(n-2) -  4 \left(n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A097331, agreeing on all 13 terms compared.
...
vanish, for all 46 values of $n$ from $n=3$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{2} + 3 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A097331.
