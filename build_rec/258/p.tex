...
closed form. Here it equals $12 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A097180 is ``Row sums of triangle A097179, in which the n-th row polynomial R\_n(y) is formed from the initial (n+1) terms of g.f. A077860(y)\^{}(n+1), where R\_n(1/2) = 4\^{}n for all n>=0''. It has offset $0$ and begins
\[
...
\begin{quote}\small
\textbf{Conjecture:} n*(n-1)*(n+1)*a(n) -12*n*(2*n-1)*(n-1)*a(n-1) +12*(n-1) * (16*n\^{}2-32*n+17)*a(n-2) -16*(4*n-5)*(4*n-7)*(2*n-3)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 16 2012
\end{quote}
