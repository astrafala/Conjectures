...
\title{A proof of the conjectured recurrence for OEIS A184018}
...
\begin{abstract}
OEIS A184018 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{2} + 1$, a polynomial of degree $2$, which proves
...
OEIS A184018 is ``Expansion of c(x/(1-x-x\^{}2)) / (1-x-x\^{}2), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 6, 19, 67, 254, 1017, 4236,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} + x + \sqrt{\left(x^{2} + x - 1\right) \left(x^{2} + 5 x - 1\right)} - 1}{2 x \left(x^{2} + x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 2*(1-3*n)*a(n-1) + 3*(n-1)*a(n-2) + 2*(3*n-5)*a(n-3) + (n-3)*a(n-4) = 0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-05-23, revision 31) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=3 \left(n - 1\right),\qquad p_{3}(n)=2 \left(3 n - 5\right),\qquad p_{4}(n)=n - 3
\]
...
\section{The computation for A184018}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x^{2} + x - 1\right) \left(x^{2} + 5 x - 1\right) .
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + 3 \left(n - 1\right)\,a(n-2) + 2 \left(3 n - 5\right)\,a(n-3) + (n - 3)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A184018, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=x^{2} + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A184018.
