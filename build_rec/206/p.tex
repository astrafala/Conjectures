...
\title{A proof of the conjectured recurrence for OEIS A136304}
...
\begin{abstract}
OEIS A136304 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{3} + 3 x^{2} - 3 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A136304 is ``Expansion of g.f. (1-z)\^{}2*(1-sqrt(1-4*z))/(2*z*(1 - 3*z + 2*z\^{}2 - z\^{}3))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 14, 40, 116, 344, 1047,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x - 1\right)^{2} \left(\sqrt{1 - 4 x} - 1\right)}{2 x \left(x^{3} - 2 x^{2} + 3 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + (-8*n+1)*a(n-1) + 3*(7*n-8)*a(n-2) + (-23*n+49)*a(n-3) + (13*n-32)*a(n-4) + 2*(-2*n+7)*a(n-5) = 0. - \emph{R. J. Mathar}, Feb 29 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-02-24, revision 33) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=1 - 8 n,\qquad p_{2}(n)=3 \left(7 n - 8\right),\qquad p_{3}(n)=49 - 23 n,\qquad p_{4}(n)=13 n - 32,\qquad p_{5}(n)=- 2 \left(2 n - 7\right)
\]
...
\section{The computation for A136304}
...
\[
(n + 1)\,a(n) + (1 - 8 n)\,a(n-1) + 3 \left(7 n - 8\right)\,a(n-2) + (49 - 23 n)\,a(n-3) + (13 n - 32)\,a(n-4) -  2 \left(2 n - 7\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A136304, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{3} + 3 x^{2} - 3 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A136304.
