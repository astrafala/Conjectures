...
\title{A proof of the conjectured recurrence for OEIS A051292}
...
\begin{abstract}
OEIS A051292 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{3} - x^{2} + 2 x$, a polynomial of degree $3$, which proves
...
OEIS A051292 is ``Whitney number of level n of the lattice of the ideals of the crown of size 2 n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;2, 1, 1, 4, 9, 21, 52, 127,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{2} + \sqrt{x^{4} - 2 x^{3} - x^{2} - 2 x + 1} + 1}{\sqrt{x^{4} - 2 x^{3} - x^{2} - 2 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-3)*a(n) - (2*n-1)*(n-3)*a(n-1) + (-n\^{}2+4*n-5)*a(n-2) - (n-1)*(2*n-7)*a(n-3) + (n-1)*(n-4)*a(n-4) = 0. - \emph{R. J. Mathar}, Nov 30 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 49) the
...
\[
p_{0}(n)=n \left(n - 3\right),\qquad p_{1}(n)=- \left(n - 3\right) \left(2 n - 1\right),\qquad p_{2}(n)=- n^{2} + 4 n - 5,\qquad p_{3}(n)=- \left(n - 1\right) \left(2 n - 7\right),\qquad p_{4}(n)=\left(n - 4\right) \left(n - 1\right)
\]
...
\section{The computation for A051292}
...
\[
n \left(n - 3\right)\,a(n) -  \left(n - 3\right) \left(2 n - 1\right)\,a(n-1) + (- n^{2} + 4 n - 5)\,a(n-2) -  \left(n - 1\right) \left(2 n - 7\right)\,a(n-3) + \left(n - 4\right) \left(n - 1\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A051292, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{3} - x^{2} + 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A051292.
