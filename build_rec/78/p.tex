...
\title{A proof of the conjectured recurrence for OEIS A102880}
...
\begin{abstract}
OEIS A102880 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x^{4} - 3$, a polynomial of degree $4$, which proves
...
OEIS A102880 is ``A Chebyshev transform of the first kind of the Catalan numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 0, 2, 8, 22, 64, 198,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x^{2} - 1\right) \left(\sqrt{\frac{x^{2} - 4 x + 1}{x^{2} + 1}} - 1\right)}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*(n-3)*a(n) -2*(2*n-1)*(n-3)*a(n-1) +2*(1-4*n+n\^{}2)*a(n-2) -2*(n-1)*(2*n-7)*a(n-3) +(n-1)*(n-5)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 09 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2012-11-09, revision 6) the
...
\[
p_{0}(n)=\left(n - 3\right) \left(n + 1\right),\qquad p_{1}(n)=- 2 \left(n - 3\right) \left(2 n - 1\right),\qquad p_{2}(n)=2 \left(n^{2} - 4 n + 1\right),\qquad p_{3}(n)=- 2 \left(n - 1\right) \left(2 n - 7\right),\qquad p_{4}(n)=\left(n - 5\right) \left(n - 1\right)
\]
...
\section{The computation for A102880}
...
\[
\left(n - 3\right) \left(n + 1\right)\,a(n) -  2 \left(n - 3\right) \left(2 n - 1\right)\,a(n-1) + 2 \left(n^{2} - 4 n + 1\right)\,a(n-2) -  2 \left(n - 1\right) \left(2 n - 7\right)\,a(n-3) + \left(n - 5\right) \left(n - 1\right)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A102880, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=3 x^{4} - 3$ is an
...
\url{https://oeis.org}, 2026. Sequence A102880.
