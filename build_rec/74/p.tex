...
\title{A proof of the conjectured recurrence for OEIS A100098}
...
\begin{abstract}
OEIS A100098 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A100098 is ``An inverse Chebyshev transform of (1-x)/(1-2x)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 7, 22, 46, 130, 295,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{6 x - \sqrt{1 - 4 x^{2}} - 3}{2 \sqrt{1 - 4 x^{2}} \left(5 x - 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*n*a(n) + (-5*n+4)*a(n-1) + 2*(-4*n+3)*a(n-2) + 20*(n-2)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 22 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-16, revision 21) the
...
\[
p_{0}(n)=2 n,\qquad p_{1}(n)=4 - 5 n,\qquad p_{2}(n)=- 2 \left(4 n - 3\right),\qquad p_{3}(n)=20 \left(n - 2\right)
\]
...
\section{The computation for A100098}
...
\[
2 n\,a(n) + (4 - 5 n)\,a(n-1) -  2 \left(4 n - 3\right)\,a(n-2) + 20 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A100098, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A100098.
