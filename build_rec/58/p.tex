...
\title{A proof of the conjectured recurrence for OEIS A064088}
...
\begin{abstract}
OEIS A064088 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $45 x$, a polynomial of degree $1$, which proves
...
OEIS A064088 is ``Generalized Catalan numbers C(5; n)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 6, 61, 766, 10746, 161376, 2537781,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{10}{\sqrt{1 - 20 x} + 9} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 4*n*a(n) +(-79*n+120)*a(n-1) +10*(-2*n+3)*a(n-2)=0. - \emph{R. J. Mathar}, Jun 07 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 33) the
...
\[
p_{0}(n)=4 n,\qquad p_{1}(n)=120 - 79 n,\qquad p_{2}(n)=- 10 \left(2 n - 3\right)
\]
...
\section{The computation for A064088}
...
\[
4 n\,a(n) + (120 - 79 n)\,a(n-1) -  10 \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A064088, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=45 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A064088.
