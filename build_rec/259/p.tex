...
closed form. Here it equals $6 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A097189 is ``Row sums of triangle A097186, in which the n-th row polynomial R\_n(y) is formed from the initial (n+1) terms of g.f. A057083(y)\^{}(n+1), where R\_n(1/3) = 3\^{}n for all n >= 0''. It has offset $0$ and begins
\[
...
\begin{quote}\small
\textbf{Conjecture:} n*(n-1)*a(n) - (19*n-18)*(n-1)*a(n-1) + 9*(11*n\^{}2-31*n+22)*a(n-2) - 9*(3*n-4)*(3*n-5)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 16 2012
\end{quote}
