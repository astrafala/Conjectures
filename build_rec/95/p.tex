...
\title{A proof of the conjectured recurrence for OEIS A174808}
...
\begin{abstract}
OEIS A174808 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{3} + 3 x^{2} + 3 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A174808 is ``A transform of the large Schroeder numbers A006318''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 8, 34, 162, 820, 4338, 23694,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{2} - x - \sqrt{x^{4} + 2 x^{3} - 5 x^{2} - 6 x + 1} + 1}{2 x \left(x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(-5*n+4)*a(n-1) +(-11*n+13)*a(n-2) +3*(-n+1)*a(n-3) +3*(n-4)*a(n-4) +(n-5)*a(n-5)=0. - \emph{R. J. Mathar}, Feb 10 2015
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=4 - 5 n,\qquad p_{2}(n)=13 - 11 n,\qquad p_{3}(n)=- 3 \left(n - 1\right),\qquad p_{4}(n)=3 \left(n - 4\right),\qquad p_{5}(n)=n - 5
\]
...
\section{The computation for A174808}
...
\[
(n + 1)\,a(n) + (4 - 5 n)\,a(n-1) + (13 - 11 n)\,a(n-2) -  3 \left(n - 1\right)\,a(n-3) + 3 \left(n - 4\right)\,a(n-4) + (n - 5)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A174808, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{3} + 3 x^{2} + 3 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A174808.
