...
\title{A proof of the conjectured recurrence for OEIS A100095}
...
\begin{abstract}
OEIS A100095 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A100095 is ``An inverse Chebyshev transform of the Fibonacci numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 1, 5, 7, 25, 41, 125,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x \left(- 4 x^{2} + x \sqrt{1 - 4 x^{2}} + 1\right)}{20 x^{4} - 9 x^{2} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+2)*a(n) +(-n+3)*a(n-1) +(9*n-22)*a(n-2) +(9*n-31)*a(n-3) +20*(-n+3)*a(n-4) +20*(-n+4)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-16, revision 28) the
...
\[
p_{0}(n)=2 - n,\qquad p_{1}(n)=3 - n,\qquad p_{2}(n)=9 n - 22,\qquad p_{3}(n)=9 n - 31,\qquad p_{4}(n)=- 20 \left(n - 3\right),\qquad p_{5}(n)=- 20 \left(n - 4\right)
\]
...
\section{The computation for A100095}
...
\[
(2 - n)\,a(n) + (3 - n)\,a(n-1) + (9 n - 22)\,a(n-2) + (9 n - 31)\,a(n-3) -  20 \left(n - 3\right)\,a(n-4) -  20 \left(n - 4\right)\,a(n-5) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A100095, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A100095.
