...
\title{A proof of the conjectured recurrence for OEIS A110199}
...
\begin{abstract}
OEIS A110199 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A110199 is ``a(n) = Sum\_\{k=0..floor(n/2)\} Catalan(k)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 2, 4, 4, 9, 9,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{1 - 4 x^{2}} - 1}{2 x^{2} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+2)*a(n) + (n+2)*a(n-1) + 4*(n-1)*a(n-2) + 4*(1-n)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 09 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2020-03-11, revision 22) the
...
\[
p_{0}(n)=- n - 2,\qquad p_{1}(n)=n + 2,\qquad p_{2}(n)=4 \left(n - 1\right),\qquad p_{3}(n)=- 4 \left(n - 1\right)
\]
...
\section{The computation for A110199}
...
\[
(- n - 2)\,a(n) + (n + 2)\,a(n-1) + 4 \left(n - 1\right)\,a(n-2) -  4 \left(n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A110199, agreeing on all 13 terms compared.
...
vanish, for all 34 values of $n$ from $n=3$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A110199.
