...
\title{A proof of the conjectured recurrence for OEIS A188314}
...
\begin{abstract}
OEIS A188314 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $10 x^{4} + 17 x^{3} + 13 x^{2} + 7 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A188314 is ``Expansion of (1/(1-x))*c(x/((1-x)*(1-x\^{}2))) where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 16, 57, 219, 883, 3687,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{2} - \sqrt{x^{4} - 6 x^{2} - 4 x + 1} + 1}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(n+2)*a(n-1) +(42-26*n)*a(n-2) +30*(3-n)*a(n-3) +(n-5)*a(n-4) +5*(n-6)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=n + 2,\qquad p_{2}(n)=- 2 \left(13 n - 21\right),\qquad p_{3}(n)=- 30 \left(n - 3\right),\qquad p_{4}(n)=n - 5,\qquad p_{5}(n)=5 \left(n - 6\right)
\]
...
\section{The computation for A188314}
...
\[
B(x)\;=\;10 x^{4} + 17 x^{3} + 13 x^{2} + 7 x + 1 ,
\]
...
\[
(n + 1)\,a(n) + (n + 2)\,a(n-1) -  2 \left(13 n - 21\right)\,a(n-2) -  30 \left(n - 3\right)\,a(n-3) + (n - 5)\,a(n-4) + 5 \left(n - 6\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A188314, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=10 x^{4} + 17 x^{3} + 13 x^{2} + 7 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A188314.
