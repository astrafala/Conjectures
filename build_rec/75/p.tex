...
\title{A proof of the conjectured recurrence for OEIS A100099}
...
\begin{abstract}
OEIS A100099 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{2} + 2 x$, a polynomial of degree $2$, which proves
...
OEIS A100099 is ``An inverse Chebyshev transform of x/(1-2x)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 2, 7, 16, 46, 110, 295,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 4 x - \sqrt{1 - 4 x^{2}} + 1}{2 \sqrt{1 - 4 x^{2}} \left(5 x - 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2n*a(n) +(-13n+16)*a(n-1) +4(3n-8)*a(n-2) +4(13n-29)*a(n-3) +80(3-n)*a(n-4)=0. - \emph{R. J. Mathar}, Dec 14 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-16, revision 19) the
...
\[
p_{0}(n)=2 n,\qquad p_{1}(n)=16 - 13 n,\qquad p_{2}(n)=4 \left(3 n - 8\right),\qquad p_{3}(n)=4 \left(13 n - 29\right),\qquad p_{4}(n)=- 80 \left(n - 3\right)
\]
...
\section{The computation for A100099}
...
\[
2 n\,a(n) + (16 - 13 n)\,a(n-1) + 4 \left(3 n - 8\right)\,a(n-2) + 4 \left(13 n - 29\right)\,a(n-3) -  80 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A100099, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{2} + 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A100099.
