...
\title{A proof of the conjectured recurrence for OEIS A100193}
...
\begin{abstract}
OEIS A100193 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 24 x^{2} + 3 x$, a polynomial of degree $2$, which proves
...
OEIS A100193 is ``a(n) = Sum\_\{k=0..n\} binomial(2*n,n+k)*3\^{}k''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 5, 27, 146, 787, 4230, 22686, 121476,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- \sqrt{1 - 4 x} - 1}{2 \left(8 x + \sqrt{1 - 4 x} - 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 9*n*a(n) + 6*(11-18*n)*a(n-1) + 16*(26*n-37)*a(n-2) + 256*(5-2*n)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 09 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-16, revision 38) the
...
\[
p_{0}(n)=9 n,\qquad p_{1}(n)=- 6 \left(18 n - 11\right),\qquad p_{2}(n)=16 \left(26 n - 37\right),\qquad p_{3}(n)=- 256 \left(2 n - 5\right)
\]
...
\section{The computation for A100193}
...
\[
9 n\,a(n) -  6 \left(18 n - 11\right)\,a(n-1) + 16 \left(26 n - 37\right)\,a(n-2) -  256 \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A100193, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 24 x^{2} + 3 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A100193.
