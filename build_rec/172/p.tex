...
\title{A proof of the conjectured recurrence for OEIS A166587}
...
\begin{abstract}
OEIS A166587 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 x$, a polynomial of degree $1$, which proves
...
OEIS A166587 is ``A signed variant of the Motzkin numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, -1, 2, -4, 9, -21, 51,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{3 x - \sqrt{- 3 x^{2} + 2 x + 1} + 1}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n+1)*a(n) + n*(n+1)*a(n-1) - (5*n-3)*(n-2)*a(n-2) + 3*(n-2)*(n-3)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 15 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-23, revision 33) the
...
\[
p_{0}(n)=n \left(n + 1\right),\qquad p_{1}(n)=n \left(n + 1\right),\qquad p_{2}(n)=- \left(n - 2\right) \left(5 n - 3\right),\qquad p_{3}(n)=3 \left(n - 3\right) \left(n - 2\right)
\]
...
\section{The computation for A166587}
...
\[
n \left(n + 1\right)\,a(n) + n \left(n + 1\right)\,a(n-1) -  \left(n - 2\right) \left(5 n - 3\right)\,a(n-2) + 3 \left(n - 3\right) \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A166587, agreeing on all 13 terms compared.
...
vanish, for all 29 values of $n$ from $n=3$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A166587.
