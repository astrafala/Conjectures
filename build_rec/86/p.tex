...
\title{A proof of the conjectured recurrence for OEIS A114464}
...
\begin{abstract}
OEIS A114464 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 3 x^{4} + 4 x^{3} - 6 x^{2} + 1$, a polynomial of degree $4$, which proves
...
OEIS A114464 is ``Number of Dyck paths of semilength n having no ascents of length 2 that start at an even level''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 1, 2, 6, 18, 54, 166,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x + \frac{\left(x - 1\right)^{3} \left(\sqrt{\frac{- 4 x^{2} + \left(1 - x\right)^{4}}{\left(1 - x\right)^{4}}} - 1\right)}{2}}{x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(-5*n+3)*a(n-1) +2*(3*n-7)*a(n-2) +2*(-3*n+11)*a(n-3) +(5*n-27)*a(n-4) +(-n+7)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 26 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2014-02-15, revision 12) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=3 - 5 n,\qquad p_{2}(n)=2 \left(3 n - 7\right),\qquad p_{3}(n)=- 2 \left(3 n - 11\right),\qquad p_{4}(n)=5 n - 27,\qquad p_{5}(n)=7 - n
\]
...
\section{The computation for A114464}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \frac{\left(x^{2} + 1\right) \left(x^{2} - 4 x + 1\right)}{\left(x - 1\right)^{4}} .
\]
...
\[
(n + 1)\,a(n) + (3 - 5 n)\,a(n-1) + 2 \left(3 n - 7\right)\,a(n-2) -  2 \left(3 n - 11\right)\,a(n-3) + (5 n - 27)\,a(n-4) + (7 - n)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A114464, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 3 x^{4} + 4 x^{3} - 6 x^{2} + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A114464.
