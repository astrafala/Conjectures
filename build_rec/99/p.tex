...
\title{A proof of the conjectured recurrence for OEIS A188312}
...
\begin{abstract}
OEIS A188312 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{2} + x + 1$, a polynomial of degree $2$, which proves
...
OEIS A188312 is ``Expansion of (1/(1-x\^{}2))*c(x/((1-x)*(1-x\^{}2))) where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 12, 45, 174, 709, 2978,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{2} - \sqrt{x^{4} - 6 x^{2} - 4 x + 1} + 1}{2 x \left(x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(3-4*n)*a(n-1) + (7-6*n)*a(n-2) -a(n-3) +(n-4)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2024-10-19, revision 29) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=3 - 4 n,\qquad p_{2}(n)=7 - 6 n,\qquad p_{3}(n)=-1,\qquad p_{4}(n)=n - 4
\]
...
\section{The computation for A188312}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x + 1\right) \left(x^{3} - x^{2} - 5 x + 1\right) .
\]
...
\[
(n + 1)\,a(n) + (3 - 4 n)\,a(n-1) + (7 - 6 n)\,a(n-2) - 1\,a(n-3) + (n - 4)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A188312, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{2} + x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A188312.
