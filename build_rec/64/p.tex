...
\title{A proof of the conjectured recurrence for OEIS A090413}
...
\begin{abstract}
OEIS A090413 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x + 3$, a polynomial of degree $1$, which proves
...
OEIS A090413 is ``A Chebyshev transform of 3\^{}n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 8, 21, 56, 150, 400, 1065,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{4 x^{2} + 1} - 1}{x \left(2 x - 3 \sqrt{4 x^{2} + 1} + 3\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 3*(n+1)*a(n) -8*(n+1)*a(n-1) +12*(n-2)*a(n-2) +32*(2-n)*a(n-3)=0. - \emph{R. J. Mathar}, Sep 27 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-02-25, revision 11) the
...
\[
p_{0}(n)=3 \left(n + 1\right),\qquad p_{1}(n)=- 8 \left(n + 1\right),\qquad p_{2}(n)=12 \left(n - 2\right),\qquad p_{3}(n)=- 32 \left(n - 2\right)
\]
...
\section{The computation for A090413}
...
\[
3 \left(n + 1\right)\,a(n) -  8 \left(n + 1\right)\,a(n-1) + 12 \left(n - 2\right)\,a(n-2) -  32 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A090413, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x + 3$ is an
...
\url{https://oeis.org}, 2026. Sequence A090413.
