...
\title{A proof of the conjectured recurrence for OEIS A220902}
...
\begin{abstract}
OEIS A220902 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{4} - 8 x^{3} + 6 x^{2}$, a polynomial of degree $4$, which proves
...
OEIS A220902 is ``a(n) = Catalan(n) - A000245(n-2)''. It has offset $2$ and begins
\[
a(2),\dots,a(9)\;=\;2, 4, 11, 33, 104, 339, 1133, 3861,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 2} a(n)\,x^{n}\;=\;\frac{- x^{2} - \frac{\left(\sqrt{1 - 4 x} - 1\right) \left(8 x^{2} + 8 x - \left(\sqrt{1 - 4 x} - 1\right)^{3}\right)}{16}}{x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + (-5*n+3)*a(n-1) + (5*n-13)*a(n-2) + 2*(-2*n+9)*a(n-3) = 0. - \emph{R. J. Mathar}, May 30 2014
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=3 - 5 n,\qquad p_{2}(n)=5 n - 13,\qquad p_{3}(n)=- 2 \left(2 n - 9\right)
\]
...
\section{The computation for A220902}
...
\[
(n + 1)\,a(n) + (3 - 5 n)\,a(n-1) + (5 n - 13)\,a(n-2) -  2 \left(2 n - 9\right)\,a(n-3) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A220902, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=x^{4} - 8 x^{3} + 6 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A220902.
