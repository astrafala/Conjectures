...
\title{A proof of the conjectured recurrence for OEIS A106271}
...
\begin{abstract}
OEIS A106271 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $1 - 4 x$, a polynomial of degree $1$, which proves
...
OEIS A106271 is ``Row sums of number triangle A106270''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, -2, -7, -21, -63, -195, -624,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 4 x - \sqrt{1 - 4 x} + 1}{2 x \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + (1-5*n)*a(n-1) + 2*(2*n-1)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 09 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2019-07-22, revision 33) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=1 - 5 n,\qquad p_{2}(n)=2 \left(2 n - 1\right)
\]
...
\section{The computation for A106271}
...
\[
(n + 1)\,a(n) + (1 - 5 n)\,a(n-1) + 2 \left(2 n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A106271, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=1 - 4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A106271.
