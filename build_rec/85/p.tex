...
\title{A proof of the conjectured recurrence for OEIS A114190}
...
\begin{abstract}
OEIS A114190 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{3} + 20 x^{2} - 4 x$, a polynomial of degree $3$, which proves
...
OEIS A114190 is ``Expansion of 1/(1+x*(1-x)*c(-2*x)), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -1, 4, -17, 87, -490, 2945, -18517,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4}{x - \left(x - 1\right) \sqrt{8 x + 1} + 3} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + (7*n-10)*a(n-1) + 2*(14-3*n)*a(n-2) + (13*n-20)*a(n-3) + (66-23*n)*a(n-4) + 4*(2*n-7)*a(n-5) = 0. - \emph{R. J. Mathar}, Dec 10 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-06, revision 18) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=7 n - 10,\qquad p_{2}(n)=- 2 \left(3 n - 14\right),\qquad p_{3}(n)=13 n - 20,\qquad p_{4}(n)=66 - 23 n,\qquad p_{5}(n)=4 \left(2 n - 7\right)
\]
...
\section{The computation for A114190}
...
\[
n\,a(n) + (7 n - 10)\,a(n-1) -  2 \left(3 n - 14\right)\,a(n-2) + (13 n - 20)\,a(n-3) + (66 - 23 n)\,a(n-4) + 4 \left(2 n - 7\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A114190, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{3} + 20 x^{2} - 4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A114190.
