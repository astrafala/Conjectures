...
\title{A proof of the conjectured recurrence for OEIS A141353}
...
\begin{abstract}
OEIS A141353 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $8 x^{3} - 12 x^{2} + 1$, a polynomial of degree $3$, which proves
...
OEIS A141353 is ``a(n) = Catalan(n) + 2\^{}n - 0\^{}n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 6, 13, 30, 74, 196, 557,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 4 x^{2} + \left(1 - \sqrt{1 - 4 x}\right) \left(2 x - 1\right)}{2 x \left(2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 2*(-4*n+1)*a(n-1) + 4*(5*n-7)*a(n-2) + 8*(-2*n+5)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 15 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-24, revision 19) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(4 n - 1\right),\qquad p_{2}(n)=4 \left(5 n - 7\right),\qquad p_{3}(n)=- 8 \left(2 n - 5\right)
\]
...
\section{The computation for A141353}
...
\[
(n + 1)\,a(n) -  2 \left(4 n - 1\right)\,a(n-1) + 4 \left(5 n - 7\right)\,a(n-2) -  8 \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A141353, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=8 x^{3} - 12 x^{2} + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A141353.
