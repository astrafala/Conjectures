...
\title{The empirical recurrence for OEIS A231703, proved by transfer matrix}
...
\begin{abstract}
OEIS A231703 counts $(n+1)\times2$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
...
walk, and $a(n)$ is a walk count on $S=20$ vertices, which is $C$-finite. Whether the
...
OEIS A231703 is ``Number of (n+1) X (1+1) 0..1 arrays with no element having a strict majority of its horizontal, vertical and antidiagonal neighbors equal to one.''. It has offset $1$ and begins
\[
5, 17, 42, 121, 351, 978, 2768, 7851,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) - a(n-2) + 4*a(n-3) - 8*a(n-4) + 3*a(n-5) - 3*a(n-6) + 3*a(n-7) - 2*a(n-8) + a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 30 2018 20:23:10, revision 8) this is still
...
$(r,s)\in(\{0,\dots,1\}^{2}\cup\{\varnothing\})\times\{0,\dots,1\}^{2}$,
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - a(n-2) + 4\,a(n-3) - 8\,a(n-4) + 3\,a(n-5) - 3\,a(n-6) + 3\,a(n-7) - 2\,a(n-8) + a(n-9)
\]
for every $n>8$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231703.
