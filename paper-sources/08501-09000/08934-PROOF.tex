...
\title{The empirical recurrence for OEIS A202443, proved by transfer matrix}
...
\begin{abstract}
OEIS A202443 carries an empirical recurrence of order $7$ contributed by R.~H.~Hardin. It is
true, and it is decidable rather than empirical. The entry's condition is local to a bounded window of consecutive lines, so
the admissible configurations of such a window are the vertices of a finite digraph and the
arrays counted are exactly the walks in it; hence $a(n)$ is a walk count on $S=157$ vertices
...
OEIS A202443 is ``Number of (n+2) X 6 binary arrays avoiding patterns 001 and 110 in rows, columns and nw-to-se diagonals.''. It has offset $1$ and begins
\[
338, 574, 1102, 1890, 3122, 4822, 7238, 10394,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) -a(n-2) -5*a(n-3) +5*a(n-4) +a(n-5) -3*a(n-6) +a(n-7) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (May 29 2018 08:29:06, revision 8) this is still
...
admissible array is then exactly a walk, so with $M$ the adjacency matrix on $S=157$ vertices
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202443.
