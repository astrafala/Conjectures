...
\title{The empirical recurrence for OEIS A235896, proved by transfer matrix}
...
\begin{abstract}
OEIS A235896 counts arrays of width $3$ over $\{0,1,2\}$ in which the smallest and largest of the
...
wider, so the count is a walk count on $S=27$ states.
\end{abstract}
...
OEIS A235896 is ``Number of (n+1) X (2+1) 0..2 arrays with the minimum plus the maximum equal to the lower median plus the upper median in every 2 X 2 subblock.''. It has offset $1$ and begins
\[
125, 641, 3265, 17001, 89857, 482809, 2633969, 14575785,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 13*a(n-1) - 44*a(n-2) - 24*a(n-3) + 192*a(n-4) + 144*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 20 2018 03:58:13, revision 12) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The arrays have $3$ columns and a growing number of rows, with entries in $\{0,1,2\}$. Take any
...
$\mathbf{1}$, and the states are all $3^{3}=27$ rows. Thus
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235896.
