...
\title{The empirical recurrence for OEIS A197368, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A197368 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A197368 is ``Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 2,4,1,0,1 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
1, 3, 32, 45, 189, 646, 1873, 6209,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) +2*a(n-2) +7*a(n-3) -3*a(n-4) -7*a(n-6) -5*a(n-7) +3*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (May 12 2018 08:45:58, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 2\,a(n-2) + 7\,a(n-3) - 3\,a(n-4) - 7\,a(n-6) - 5\,a(n-7) + 3\,a(n-8)
\end{equation}
for all $n>9$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(2,4,1,0,1)
\end{equation}
for every cell $(t,u)$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A197368.
