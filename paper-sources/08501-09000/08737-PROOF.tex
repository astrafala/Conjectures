...
\title{The empirical recurrence for OEIS A302625, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A302625 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $8$ contributed
...
OEIS A302625 is ``Number of 4Xn 0..1 arrays with every element equal to 0, 1, 2 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
8, 81, 71, 130, 251, 497, 1051, 2274,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +2*a(n-2) +2*a(n-3) +a(n-4) -4*a(n-5) -a(n-6) -2*a(n-7) +2*a(n-8) for n>11
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 19:14:28, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 2\,a(n-2) + 2\,a(n-3) + a(n-4) - 4\,a(n-5) - a(n-6) - 2\,a(n-7) + 2\,a(n-8)
\end{equation}
for all $n>11$.
...
publishes, $n=28$, the arrays have $4\cdot28$ cells over
...
The threshold below is exact rather than merely sufficient: at $n=11$ the conjectured
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-8}$ out of
...
the middle column. The vector $w$ was formed by $8$ applications of $G$ in exact integer
arithmetic and $\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly.
Every one of those integers vanishes from the index corresponding to $n=11+1$ onwards,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302625.
