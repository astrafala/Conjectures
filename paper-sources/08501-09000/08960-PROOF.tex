...
\title{The empirical recurrence for OEIS A223410, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A223410 counts arrays over $\{0,\dots,9\}$ in which every cell is constrained by the
...
with $S=1000$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A223410 is ``4X4X4 triangular graph without horizontal edges coloring a rectangular array: number of nX3 0..9 arrays where 0..9 label nodes of a graph with edges 0,1 0,2 1,3 1,4 2,4 2,5 3,6 3,7 4,7 4,8 5,8 5,9 and every array movement to a horizontal or vertical neighbor moves along an edge of this graph.''. It has offset $1$ and begins
\[
66, 780, 10496, 139552, 1905248, 25615364, 350374768, 4718334196,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 21:38:11, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;246\,a(n-2) - 12495\,a(n-4) + 213864\,a(n-6) - 1242839\,a(n-8) + 3210236\,a(n-10) - 3946254\,a(n-12) + 2244580\,a(n-14) - 563760\,a(n-16) + 50544\,a(n-18)
\end{equation}
for all $n>19$.
...
Let $V=\{0,\dots,9\}^{3}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=1000$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
The size of $\mathcal V$ is worth seeing concretely. There are $9+1=10$ values
and $3$ cells in a row, so $|V|=10^{3}=1000$, and the vertex set is
$V\times V$, of size $1000^2=1000$. That is the whole of the state: two consecutive
...
$10$ values, so there are about $10^{48}$ of them to sift. The
walk count replaces that by $1000$ states and one matrix--vector product per row.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=1000$ pairs and each
of the $(9+1)^{3}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223410.
