...
\title{The empirical recurrence for OEIS A300347, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A300347 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A300347 is ``Number of nX5 0..1 arrays with every element equal to 1 or 2 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
3, 37, 57, 152, 306, 729, 1496, 3223,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) -8*a(n-2) +4*a(n-3) +a(n-6) -3*a(n-7) +4*a(n-8) -2*a(n-9) +a(n-10) -a(n-11) -a(n-14) +a(n-15) for n>19
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 03 2018 08:30:36, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 8\,a(n-2) + 4\,a(n-3) + a(n-6) - 3\,a(n-7) + 4\,a(n-8) - 2\,a(n-9) + a(n-10) - a(n-11) - a(n-14) + a(n-15)
\end{equation}
for all $n>19$.
...
publishes, $n=28$, the arrays have $5\cdot28$ cells over
$2$ values, so there are about $10^{42}$ of them to sift. The
walk count replaces that by $1024$ states and one matrix--vector product per row.

The threshold below is exact rather than merely sufficient: at $n=19$ the conjectured
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A300347.
