...
\title{The empirical recurrence for OEIS A223213, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A223213 counts arrays over $\{0,\dots,5\}$ in which every cell is constrained by the
...
with $S=216$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A223213 is ``3X3X3 triangular graph coloring a rectangular array: number of nX3 0..5 arrays where 0..5 label nodes of a graph with edges 0,1 0,2 1,2 1,3 1,4 2,4 3,4 2,5 4,5 and every array movement to a horizontal or vertical neighbor moves along an edge of this graph.''. It has offset $1$ and begins
\[
60, 918, 15498, 254694, 4232586, 70014654, 1160465118, 19217863458,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 10*a(n-1) +118*a(n-2) -120*a(n-3) -577*a(n-4) +380*a(n-5) +504*a(n-6) -16*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 21:21:45, revision 10) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;10\,a(n-1) + 118\,a(n-2) - 120\,a(n-3) - 577\,a(n-4) + 380\,a(n-5) + 504\,a(n-6) - 16\,a(n-8)
\end{equation}
for all $n>8$.
...
$\mathcal V=V\times V$, of size $S=216$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
$6$ values, so there are about $10^{35}$ of them to sift. The
walk count replaces that by $216$ states and one matrix--vector product per row.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=216$ pairs and each
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223213.
