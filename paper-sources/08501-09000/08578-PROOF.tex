...
\title{A condition on each element and its neighbours: OEIS A296390}
...
\begin{abstract}
OEIS A296390 carries an empirical recurrence of order $9$ contributed by R.~H.~Hardin. It is
...
OEIS A296390 is ``Number of n X 2 0..1 arrays with each 1 horizontally, vertically or antidiagonally adjacent to 1 or 3 neighboring 1's.''. It has offset $1$ and begins
\[
2, 6, 11, 28, 63, 139, 328, 735,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + 2*a(n-2) + 2*a(n-3) - a(n-4) + 4*a(n-5) - a(n-6) - 4*a(n-7) + a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (May 09 2022 00:18:47, revision 10) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $2$ columns over the alphabet $\{0,1\}$, growing one row at a time. Its condition is that every entry equal to $1$ must have exactly $1$ or $3$ of its horizontal and vertical and antidiagonal neighbours equal to $1$.
...
Vertices with identical futures are merged, which leaves $S=9$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 2\,a(n-2) + 2\,a(n-3) - a(n-4) + 4\,a(n-5) - a(n-6) - 4\,a(n-7) + a(n-9)
\]
for every $n>9$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A296390.
