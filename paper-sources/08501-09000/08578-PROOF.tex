...
\title{The empirical recurrence for OEIS A296582, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A296582 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A296582 is ``Number of n X 2 0..1 arrays with each 1 horizontally, vertically or antidiagonally adjacent to 0, 2 or 3 neighboring 1s.''. It has offset $1$ and begins
\[
3, 9, 22, 62, 172, 480, 1358, 3849,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) + a(n-2) - a(n-3) - 8*a(n-4) - 3*a(n-6) - 2*a(n-7) - 5*a(n-8) + a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 23 2019 07:24:16, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + a(n-2) - a(n-3) - 8\,a(n-4) - 3\,a(n-6) - 2\,a(n-7) - 5\,a(n-8) + a(n-9)
\end{equation}
for all $n>10$.
...
Let $V=\{0,\dots,1\}^{2}$ be the possible rows and take
...
walk count replaces that by $16$ states and one matrix--vector product per row.
...
of the $(1+1)^{2}$ possible next rows, the condition was tested on every cell of
the middle row. The vector $w$ was formed by $9$ applications of $G$ in exact integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A296582.
