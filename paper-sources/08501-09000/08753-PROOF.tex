...
\title{The empirical recurrence for OEIS A304298, proved by transfer matrix}
...
\begin{abstract}
OEIS A304298 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{1,2,5,8\}$, and it carries an empirical
...
OEIS A304298 is ``Number of nX4 0..1 arrays with every element unequal to 1, 2, 5 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
2, 1, 5, 4, 13, 30, 43, 129,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 10 2018 09:00:13, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + a(n-2) + 5\,a(n-3) - 2\,a(n-4) + 2\,a(n-5) - 8\,a(n-6) - 2\,a(n-7) - a(n-8) - 4\,a(n-9) - a(n-10) - 2\,a(n-12) - a(n-13) - a(n-14) - a(n-15)
\end{equation}
for all $n>19$.
...
has score in $\{1,2,5,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,2,5,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,2,5,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,2,5,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,2,5,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A304298.
