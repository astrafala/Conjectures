...
\title{The empirical recurrence for OEIS A234544, proved by transfer matrix}
...
\begin{abstract}
OEIS A234544 counts $3$-column arrays over $\{0,\dots,5\}$ in which every
$2\times2$ subblock has its diagonal sum differing from its antidiagonal sum by
exactly $3$, and it
carries an empirical recurrence of order $15$ contributed by R.~H.~Hardin. It is true,
...
the residual is $\mathbf{1}^{\!\top}M^{\,n-15}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234544 is ``Number of (n+1) X (2+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
1212, 6896, 39188, 222988, 1269812, 7237508, 41285540,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 19:14:35, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;17\,a(n-1) - 30\,a(n-2) - 944\,a(n-3) + 5146\,a(n-4) + 8720\,a(n-5) - 112704\,a(n-6) + 148224\,a(n-7) + 392632\,a(n-8) - 632528\,a(n-9) - 548544\,a(n-10) + 867968\,a(n-11) + 341120\,a(n-12) - 478976\,a(n-13) - 76800\,a(n-14) + 92160\,a(n-15)
\end{equation}
...
For $n\ge15$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-15}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-15$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-15}\bigl(M^{15}-\sum_i c_i M^{15-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-15$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $15$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=14-15+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>14$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234544.
