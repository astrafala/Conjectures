...
\title{The empirical recurrence for OEIS A255776, proved by transfer matrix}
...
\begin{abstract}
OEIS A255776 counts $(n+2)\times3$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
every $3\times3$ subblock, and it carries an empirical recurrence of order $18$
...
OEIS A255776 is ``Number of (n+2)X(1+2) 0..1 arrays with no 3x3 subblock diagonal sum 1 and no antidiagonal sum 1 and no row sum 0 and no column sum 0.''. It has offset $1$ and begins
\[
140, 436, 1512, 5126, 17325, 58707, 198635, 672183,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:41:23, revision 6) this is still
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 4\,a(n-2) + 3\,a(n-3) + a(n-4) - 7\,a(n-5) - 6\,a(n-6) - 20\,a(n-7) - 16\,a(n-8) - 25\,a(n-9) - 25\,a(n-10) - 20\,a(n-11) - 21\,a(n-12) - 7\,a(n-13) - 6\,a(n-14) + 4\,a(n-15) + 4\,a(n-16) + 4\,a(n-17) + 2\,a(n-18)
\]
for every $n>18$.
\end{theorem}

\begin{proof}
The vector $w=q(M)\mathbf{1}$ was formed by $18$ matrix--vector products in exact
integer arithmetic and $\mathbf{1}^{\!\top}M^{j}w$ evaluated for $j=0,1,\dots$, again
exactly. Those integers vanish from the index corresponding to $n=18+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>18$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A255776.
