...
\title{The empirical recurrence for OEIS A195964, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A195964 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A195964 is ``Number of n X 3 0..4 arrays with each element equal to the number its horizontal and vertical neighbors equal to itself.''. It has offset $1$ and begins
\[
2, 2, 5, 7, 14, 25, 35, 56,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-3) +2*a(n-4) +2*a(n-5) +4*a(n-6) -a(n-9) for n>11.
\end{quote}
As of the ``Last modified'' line on the live entry (May 08 2018 06:20:08, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-3) + 2\,a(n-4) + 2\,a(n-5) + 4\,a(n-6) - a(n-9)
\end{equation}
for all $n>11$.
...
x_{t,u}=\#\{\text{neighbours with a value }equal to\ x_{t,u}\}
\end{equation}
for every cell $(t,u)$.
...
publishes, $n=37$, the arrays have $3\cdot37$ cells over
$5$ values, so there are about $10^{77}$ of them to sift. The
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195964.
