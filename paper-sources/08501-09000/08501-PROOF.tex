...
\title{The empirical recurrence for OEIS A196631, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A196631 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A196631 is ``Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 1,3,2,0,4 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
1, 5, 21, 62, 232, 780, 2716, 9427,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) + 4*a(n-3) + 5*a(n-4) + 5*a(n-5) - 12*a(n-6) - a(n-8) - 4*a(n-9) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 01 2018 09:42:51, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 4\,a(n-3) + 5\,a(n-4) + 5\,a(n-5) - 12\,a(n-6) - a(n-8) - 4\,a(n-9)
\end{equation}
for all $n>10$.
...
$5$ values, so there are about $10^{52}$ of them to sift. The
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196631.
