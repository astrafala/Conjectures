...
\title{The empirical recurrence for OEIS A196206, proved by transfer matrix}
...
\begin{abstract}
OEIS A196206 counts $n\times4$ arrays over $\{0,\dots,4\}$ subject to a condition imposed on
...
walk, and $a(n)$ is a walk count on $S=1288$ vertices, which is $C$-finite. Whether the
...
OEIS A196206 is ``Number of nX4 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 2,0,3,4,1 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
5, 11, 14, 35, 70, 136, 305, 611,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 06:25:09, revision 9) this is still
...
settled by the terminal weight, which evaluates it with $\varnothing$ below. Most of those pairs are indistinguishable, and exactly so. Every offset named above has $|dj|\le 1$, so whether the cell at column $j$ of the middle row is satisfied depends on the three rows only through their windows at $j-1,j,j+1$. Collect for each $j$ the indicator over the window of the row below, and call that tuple $P(r,s)$: the number of cells any $t$ violates is read off $P$, so is the number violated at the bottom edge, and the successor $(s,t)$ carries $P(s,t)$, which does not mention $r$. Hence two pairs $(r,s)$ and $(r',s)$ with the same $P$ have the same outgoing rows and the same successors, and the vertex is $(s,P)$. That leaves $S=1288$ vertices in place of $390625$. Hence,
with $M$ the adjacency matrix on $S=1288$ vertices, $\iota$ the starting vector and $\tau$ the
terminal weight,
\[
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) - 6\,a(n-2) + 7\,a(n-3) - 5\,a(n-4) - a(n-5) - a(n-6) - 6\,a(n-7) + 12\,a(n-8) - 2\,a(n-9) + 18\,a(n-10) - 29\,a(n-11) + 12\,a(n-12) - 2\,a(n-13) - 4\,a(n-14) + 16\,a(n-15) - 9\,a(n-16) + 7\,a(n-17) - 4\,a(n-18) - 3\,a(n-19)
\]
for every $n>19$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196206.
