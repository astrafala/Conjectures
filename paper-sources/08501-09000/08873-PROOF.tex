...
\title{The empirical recurrence for OEIS A206264, proved by transfer matrix}
...
\begin{abstract}
OEIS A206264 counts arrays over $\{0,\dots,1\}$ under a condition on the number of rightwards and downwards edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=72$ states and is
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 15 2018 07:59:34, revision 8) this is still
...
\]
on the $S=72$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $5$
subblocks; the entry's first term is $72$, which the model
