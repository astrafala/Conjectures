...
\title{The empirical recurrence for OEIS A197444, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A197444 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A197444 is ``Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 2,1,1,0,0 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
2, 5, 12, 36, 107, 321, 957, 2868,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) +5*a(n-2) -4*a(n-3) -6*a(n-4) -a(n-5) +4*a(n-7) +a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (May 12 2018 14:36:38, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 5\,a(n-2) - 4\,a(n-3) - 6\,a(n-4) - a(n-5) + 4\,a(n-7) + a(n-8)
\end{equation}
for all $n>9$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(2,1,1,0,0)
\end{equation}
for every cell $(t,u)$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A197444.
