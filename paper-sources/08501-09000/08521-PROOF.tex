...
\title{The empirical recurrence for OEIS A197174, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A197174 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
with $S=625$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A197174 is ``Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 2,2,1,0,0 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
1, 5, 10, 23, 58, 144, 363, 917,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) +a(n-2) +2*a(n-3) -a(n-4) -4*a(n-5) -2*a(n-6) +a(n-7) +a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (May 11 2018 10:43:25, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + a(n-2) + 2\,a(n-3) - a(n-4) - 4\,a(n-5) - 2\,a(n-6) + a(n-7) + a(n-8)
\end{equation}
for all $n>9$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(2,2,1,0,0)
\end{equation}
for every cell $(t,u)$.
...
Let $V=\{0,\dots,4\}^{2}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=625$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
$5$ values, so there are about $10^{39}$ of them to sift. The
walk count replaces that by $625$ states and one matrix--vector product per row.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=625$ pairs and each
of the $(4+1)^{2}$ possible next rows, the condition was tested on every cell of
the middle row. The vector $w$ was formed by $8$ applications of $G$ in exact integer
...
every one of the $28$ published indices, with no shift fitted: the number of rows is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A197174.
