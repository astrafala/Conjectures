...
\title{The empirical product formula for OEIS A140924, proved}
...
\begin{abstract}
OEIS A140924 counts $9\times9$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $9\times9\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $81$. The entry's own expression, cleared of its denominator,
...
OEIS A140924 is ``Number of 9 X 9 matrices with elements in 0..n with each row and each column in nondecreasing order. 9,9,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 48620, 449141836, 1371597504992, 1832516612010448, 1274867710694037824,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2025 11:11:21, revision 8) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $9\times9$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$9\times9$ rectangle --- a plane partition in a $9\times9\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{9}\prod_{j=1}^{9}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $81$ with rational coefficients.
\end{theorem}
...
$9\cdot9=81$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,9,9)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $117$ and $36$ respectively.
\end{lemma}

\begin{proof}
With $s=n+9+9-1$ one has $s-(n+i)=9+9-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{9+9-1-i}
=\frac{(s-9-9+2+i)(s-9-9+3+i)\cdots s}{(9+9-1-i)!},
\]
a polynomial in $n$ of degree $9+9-1-i$. Multiplying over $i=0,\dots,9-1$ gives
$\deg N=\sum_{i=0}^{9-1}(9+9-1-i)=117$. The denominator is
...
of degree at most $81+36=117$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,117$, that is at $118$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $117$ has at most $117$ roots, so it is identically zero. Since $D(n)\neq0$
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,117$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140924.
