...
\title{The conjectured generating function for OEIS A290072, proved}
...
\begin{abstract}
OEIS A290072 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $17$ coefficients in exact arithmetic
...
OEIS A290072 is ``Binary representation of the diagonal from the origin to the corner of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 641", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 1, 101, 11, 10111, 1111,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 - 90*x\^{}3 + 100*x\^{}4) / ((1 - x)*(1 - 10*x)*(1 + 10*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:40, revision 15) this is still
...
\[
C(n+2)\;=\;C(n)\qquad\text{as configurations of the whole plane, for every } n\ge 2.
\]
This was checked directly, and one instance of it is all that is needed: the automaton is a
deterministic map on configurations, so $C(n_0+2)=C(n_0)$ forces $C(n_0+k\cdot2)=C(n_0)$ for
...
neighbourhood to a live cell flips the ENTIRE plane, and rule 641 is read
...
w(n+2)\;=\;L_r + w(n) + R_r
\]
with $L_r$ and $R_r$ fixed words depending only on the residue $r$ of $n$ modulo $2$ --- an
...
In the shift $T=S^{\,2}$ that is a first-order recurrence with a geometric forcing term and a
constant, killed by $(T-B^{|R_r|})(T-B^{|L_r|+|R_r|})(T-1)$. Taking the DISTINCT roots that
occur across the residue classes, pulling each back to $S^{\,2}-\lambda$, and allowing the
pre-period to raise the numerator's degree gives a monic annihilator of order $S=8$.
...
$A(x)=\sum_{n\ge0}a(n)x^n=N_1(x)/D_1(x)$ where $D_1(x)$ has degree at most $S=8$ and
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=4$ and $\deg D_2=3$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $12$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{12}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $17$
of them --- more than the $12+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{3}$ read off $D_2$, the first few being $1,\ 100,\ -100$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A290072.
