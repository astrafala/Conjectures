...
\title{The empirical recurrence for OEIS A235182, proved by transfer matrix}
...
\begin{abstract}
OEIS A235182 counts $5$-column arrays over $\{0,\dots,5\}$ in which every
...
carries an empirical recurrence of order $27$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=3750$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-27}q(M)\mathbf{1}$, so the recurrence
...
OEIS A235182 is ``Number of (n+1) X (4+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 5, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
1696, 3796, 8224, 19584, 45128, 112432, 270056,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 02:18:41, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) + 41\,a(n-2) - 177\,a(n-3) - 709\,a(n-4) + 3405\,a(n-5) + 6658\,a(n-6) - 37423\,a(n-7) - 35857\,a(n-8) + 259792\,a(n-9) + 100713\,a(n-10) - 1189883\,a(n-11) - 44519\,a(n-12) + 3652705\,a(n-13) - 650588\,a(n-14) - 7506845\,a(n-15) + 2237158\,a(n-16) + 10206566\,a(n-17) - 3478912\,a(n-18) - 9024208\,a(n-19) + 2879248\,a(n-20) + 5075680\,a(n-21) - 1279200\,a(n-22) - 1728912\,a(n-23) + 283680\,a(n-24) + 319968\,a(n-25) - 24192\,a(n-26) - 24192\,a(n-27)
\end{equation}
for all $n>27$, which is every $n$ at which a recurrence of order $27$ can be
stated.
...
Let $V$ consist of those $r\in\{0,\dots,5\}^{5}$ with $r_j\ne r_{j+1}$ for every
$j$ --- the rows in which no two horizontally adjacent entries are equal, and put $S=|V|=3750$. For $r,s\in V$ declare $r\to s$ an edge
when $\bigl|r_j+s_{j+1}-r_{j+1}-s_j\bigr|=5$ for every $j$ with
$1\le j\le5-1$, and $r_j\ne s_j$ for every $j$. Let $M\in\{0,1\}^{S\times S}$ be the adjacency matrix and $\mathbf{1}$
the all-ones vector.
...
For $n\ge27$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-27}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-27$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-27}\bigl(M^{27}-\sum_i c_i M^{27-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-27$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $27$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=27-27+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>27$.
\end{proof}
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=3750$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A235182.
