...
\title{The empirical product formula for OEIS A140918, proved}
...
\begin{abstract}
OEIS A140918 counts $3\times9$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $3\times9\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $27$. The entry's own expression, cleared of its denominator,
...
OEIS A140918 is ``Number of 3 X 9 matrices with elements in 0..n with each row and each column in nondecreasing order. 3,9,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 220, 15730, 572572, 13026013, 208416208,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2025 11:09:09, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $3\times9$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$3\times9$ rectangle --- a plane partition in a $3\times9\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{3}\prod_{j=1}^{9}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $27$ with rational coefficients.
\end{theorem}
...
$3\cdot9=27$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,9,3)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $30$ and $3$ respectively.
\end{lemma}

\begin{proof}
With $s=n+9+3-1$ one has $s-(n+i)=9+3-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{9+3-1-i}
=\frac{(s-9-3+2+i)(s-9-3+3+i)\cdots s}{(9+3-1-i)!},
\]
a polynomial in $n$ of degree $9+3-1-i$. Multiplying over $i=0,\dots,3-1$ gives
$\deg N=\sum_{i=0}^{3-1}(9+3-1-i)=30$. The denominator is
...
of degree at most $27+3=30$. It was evaluated in exact rational arithmetic at
...
First, Theorem~\ref{thm:poly} reproduces all $18$ terms the entry publishes,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A140918.
