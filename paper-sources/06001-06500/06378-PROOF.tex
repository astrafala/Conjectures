...
\title{The empirical recurrence for OEIS A300171, proved by transfer matrix}
...
\begin{abstract}
OEIS A300171 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{2,3,4,5,6,7,8\}$, and it carries an empirical
...
OEIS A300171 is ``Number of nX4 0..1 arrays with every element equal to 2, 3, 4, 5, 6, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 6, 37, 268, 2620, 23522, 223445, 2117743,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 27 2018 12:16:34, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;9\,a(n-1) + 27\,a(n-2) - 166\,a(n-3) - 548\,a(n-4) + 941\,a(n-5) + 4154\,a(n-6) - 129\,a(n-7) - 9373\,a(n-8) - 6012\,a(n-9) - 2163\,a(n-10) - 462\,a(n-11) + 4609\,a(n-12) + 11124\,a(n-13) + 1499\,a(n-14) + 2175\,a(n-15) - 5904\,a(n-16) + 1481\,a(n-17) - 561\,a(n-18) - 5302\,a(n-19) + 2380\,a(n-20) + 1091\,a(n-21) + 890\,a(n-22) - 2400\,a(n-23) + 1432\,a(n-24) - 280\,a(n-25)
\end{equation}
for all $n>26$.
...
has score in $\{2,3,4,5,6,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{2,3,4,5,6,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{2,3,4,5,6,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{2,3,4,5,6,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{2,3,4,5,6,7,8\}$. Read the sequence as the
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>26$. This is the statement of
Section~1.
\end{theorem}
...
They vanish from the index corresponding to $n=26+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A300171.
