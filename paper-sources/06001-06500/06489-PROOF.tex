...
\title{The empirical recurrence for OEIS A234723, proved by transfer matrix}
...
\begin{abstract}
OEIS A234723 counts $4$-column arrays over $\{0,\dots,7\}$ in which every
$2\times2$ subblock has its diagonal sum differing from its antidiagonal sum by
exactly $7$, and it
carries an empirical recurrence of order $30$ contributed by R.~H.~Hardin. It is true,
...
the residual is $\mathbf{1}^{\!\top}M^{\,n-30}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234723 is ``Number of (n+1) X (3+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 7 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
4192, 12464, 33560, 116168, 360760, 1349336, 4577624,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 18:59:00, revision 6) the statement is
...
\end{equation}
for all $n>30$, which is every $n$ at which a recurrence of order $30$ can be
stated.
...
For $n\ge30$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-30}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-30$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-30}\bigl(M^{30}-\sum_i c_i M^{30-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-30$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $30$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=30-30+1$ onwards, and the run was continued past
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234723.
