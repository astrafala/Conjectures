...
\title{The empirical product formula for OEIS A140928, proved}
...
\begin{abstract}
OEIS A140928 counts $5\times10$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $5\times10\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $50$. The entry's own expression, cleared of its denominator,
...
OEIS A140928 is ``Number of 5 X 10 matrices with elements in 0..n with each row and each column in nondecreasing order. 5,10,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 3003, 2186184, 644195552, 98561919456, 9095857138368,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 20 2025 10:52:02, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $5\times10$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$5\times10$ rectangle --- a plane partition in a $5\times10\times n$ box. The
correspondence is a bijection, being an involution on arrays, so
\[
a(n)\;=\;\mathrm{PP}(5,10,n).
\]
...
\[
a(n)\;=\;\prod_{i=1}^{5}\prod_{j=1}^{10}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $50$ with rational coefficients.
\end{theorem}
...
$5\cdot10=50$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,10,5)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $60$ and $10$ respectively.
\end{lemma}

\begin{proof}
With $s=n+10+5-1$ one has $s-(n+i)=10+5-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{10+5-1-i}
=\frac{(s-10-5+2+i)(s-10-5+3+i)\cdots s}{(10+5-1-i)!},
\]
a polynomial in $n$ of degree $10+5-1-i$. Multiplying over $i=0,\dots,5-1$ gives
$\deg N=\sum_{i=0}^{5-1}(10+5-1-i)=60$. The denominator is
...
of degree at most $50+10=60$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,60$, that is at $61$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $60$ has at most $60$ roots, so it is identically zero. Since $D(n)\neq0$
...
First, Theorem~\ref{thm:poly} reproduces all $14$ terms the entry publishes,
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,60$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140928.
