...
\title{The empirical product formula for OEIS A140926, proved}
...
\begin{abstract}
OEIS A140926 counts $3\times10$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $3\times10\times n$ box, so
...
OEIS A140926 is ``Number of 3 X 10 matrices with elements in 0..n with each row and each column in nondecreasing order. 3,10,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 286, 26026, 1184183, 33157124, 644195552,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 20 2025 10:51:52, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $3\times10$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$3\times10$ rectangle --- a plane partition in a $3\times10\times n$ box. The
correspondence is a bijection, being an involution on arrays, so
\[
a(n)\;=\;\mathrm{PP}(3,10,n).
\]
...
\[
a(n)\;=\;\prod_{i=1}^{3}\prod_{j=1}^{10}\frac{n+i+j-1}{i+j-1},
\]
...
$3\cdot10=30$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,10,3)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $33$ and $3$ respectively.
\end{lemma}

\begin{proof}
With $s=n+10+3-1$ one has $s-(n+i)=10+3-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{10+3-1-i}
=\frac{(s-10-3+2+i)(s-10-3+3+i)\cdots s}{(10+3-1-i)!},
\]
a polynomial in $n$ of degree $10+3-1-i$. Multiplying over $i=0,\dots,3-1$ gives
$\deg N=\sum_{i=0}^{3-1}(10+3-1-i)=33$. The denominator is
...
of degree at most $30+3=33$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,33$, that is at $34$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $33$ has at most $33$ roots, so it is identically zero. Since $D(n)\neq0$
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,33$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140926.
