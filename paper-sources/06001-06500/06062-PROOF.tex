...
\title{The empirical product formula for OEIS A140943, proved}
...
\begin{abstract}
OEIS A140943 counts $11\times11$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $11\times11\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $121$. The entry's own expression, cleared of its denominator,
...
OEIS A140943 is ``Number of 11 X 11 matrices with elements in 0..n with each row and each column in nondecreasing order. 11,11,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 705432, 79483257308, 2543610972177184, 31148053701600494400,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 28 2025 10:47:20, revision 8) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $11\times11$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$11\times11$ rectangle --- a plane partition in a $11\times11\times n$ box. The
correspondence is a bijection, being an involution on arrays, so
\[
a(n)\;=\;\mathrm{PP}(11,11,n).
\]
...
\[
a(n)\;=\;\prod_{i=1}^{11}\prod_{j=1}^{11}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $121$ with rational coefficients.
\end{theorem}
...
$11\cdot11=121$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,11,11)$ and claims
\[
a(n)\;=\;\prod_{i=0}^{11-1}\binom{s}{n+i}\,\frac{i!}{(s-i)^{11-1-i}}\;=\;
\frac{N(n)}{D(n)} .
\]

\begin{lemma}\label{lem:deg}
$N$ and $D$ are polynomials in $n$ of degrees $176$ and $55$ respectively.
\end{lemma}

\begin{proof}
With $s=n+11+11-1$ one has $s-(n+i)=11+11-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{11+11-1-i}
=\frac{(s-11-11+2+i)(s-11-11+3+i)\cdots s}{(11+11-1-i)!},
\]
a polynomial in $n$ of degree $11+11-1-i$. Multiplying over $i=0,\dots,11-1$ gives
$\deg N=\sum_{i=0}^{11-1}(11+11-1-i)=176$. The denominator is
$\prod_{i=0}^{11-1}(s-i)^{11-1-i}$, of degree
$\sum_{i=0}^{11-1}(11-1-i)=55$, and it does not vanish for $n\ge0$.
\end{proof}
...
of degree at most $121+55=176$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,176$, that is at $177$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $176$ has at most $176$ roots, so it is identically zero. Since $D(n)\neq0$
...
First, Theorem~\ref{thm:poly} reproduces all $5$ terms the entry publishes,
exactly, in integer arithmetic.

Second, the matrices themselves were counted for the smallest alphabets, directly from the
definition: the nondecreasing rows were listed and the chains of $11$ of them increasing
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,176$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140943.
