...
\title{The empirical recurrence for OEIS A237231, proved by transfer matrix}
...
\begin{abstract}
OEIS A237231 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=9628$
...
OEIS A237231 is ``Number of (n+1) X (5+1) 0..2 arrays with the maximum plus the minimum of every 2 X 2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$ and begins
\[
9628, 36180, 97862, 233536, 721854, 1737372, 5299502, 12992080,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 05 2026 14:50:04, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The $2\times2$ subblocks of an $(n+1)\times6$ array form an
$n\times5$ grid, and each carries the value $\sigma=a+d$, where $a\le b\le c\le d$ are its four entries sorted --- its minimum, lower median, upper median and maximum. The entry asks that the values of
...
differ by $1$. The entry's first term $9628$ counts those, and the model was made to
...
\]
over the $S=9628$ pairs whose own subblock row already satisfies the condition along itself.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237231.
