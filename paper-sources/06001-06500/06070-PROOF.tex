...
\title{The empirical product formula for OEIS A140931, proved}
...
\begin{abstract}
OEIS A140931 counts $8\times10$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $8\times10\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $80$. The entry's own expression, cleared of its denominator,
...
OEIS A140931 is ``Number of 8 X 10 matrices with elements in 0..n with each row and each column in nondecreasing order. 8,10,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 43758, 367479684, 1028698128744, 1268665346776464, 819557814017595744,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 25 2025 12:15:36, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $8\times10$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$8\times10$ rectangle --- a plane partition in a $8\times10\times n$ box. The
correspondence is a bijection, being an involution on arrays, so
\[
a(n)\;=\;\mathrm{PP}(8,10,n).
\]
...
\[
a(n)\;=\;\prod_{i=1}^{8}\prod_{j=1}^{10}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $80$ with rational coefficients.
\end{theorem}
...
$8\cdot10=80$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,10,8)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $108$ and $28$ respectively.
\end{lemma}

\begin{proof}
With $s=n+10+8-1$ one has $s-(n+i)=10+8-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{10+8-1-i}
=\frac{(s-10-8+2+i)(s-10-8+3+i)\cdots s}{(10+8-1-i)!},
\]
a polynomial in $n$ of degree $10+8-1-i$. Multiplying over $i=0,\dots,8-1$ gives
$\deg N=\sum_{i=0}^{8-1}(10+8-1-i)=108$. The denominator is
...
of degree at most $80+28=108$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,108$, that is at $109$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $108$ has at most $108$ roots, so it is identically zero. Since $D(n)\neq0$
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,108$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140931.
