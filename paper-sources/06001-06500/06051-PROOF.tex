...
\title{The empirical column recurrences for the table OEIS A262759}
...
\begin{abstract}
OEIS A262759 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A262759 is ``T(n,k) = Number of (n+2) X (k+2) 0..1 arrays with each row divisible by 5 and each column divisible by 7, read as a binary number with top and left being the most significant bits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 4, 3, 7, 9, 5, 13, 17,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 19 2022 15:03:07, revision 6) these are still
...
The sequence is ``Number of (n+2) X3 0..1 arrays with each row divisible by 5 and each column divisible by 7, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+2) X4 0..1 arrays with each row divisible by 5 and each column divisible by 7, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
$S=49$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-1) + 9\,a(n-3) - 36\,a(n-4) - 8\,a(n-6) + 32\,a(n-7)
\]
...
model reproduces $19$ published values, $51$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262759.
