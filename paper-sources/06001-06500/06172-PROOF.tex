...
\title{The empirical recurrence for OEIS A252562, proved by transfer matrix}
...
\begin{abstract}
OEIS A252562 counts $6\times(n+2)$ arrays over $\{0,\dots,3\}$ subject to a condition imposed on
every $3\times3$ subblock, and it carries an empirical recurrence of order $24$
...
$a(n)$ is a walk count on $S=309936$ vertices. Such a sequence is $C$-finite, so whether a given
...
OEIS A252562 is ``Number of (4+2)X(n+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 1 2 4 6 or 7 and every 3X3 column and antidiagonal sum not equal to 1 2 4 6 or 7.''. It has offset $1$ and begins
\[
1265, 3084, 10585, 24336, 104515, 386004, 902276, 3886119,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:22:28, revision 6) this is still
...
\begin{theorem}
\[
a(n)\;=\;78\,a(n-3) - 1701\,a(n-6) + 7656\,a(n-9) - 30460\,a(n-12) + 70284\,a(n-15) - 70416\,a(n-18) + 25968\,a(n-21) - 1408\,a(n-24)
\]
for every $n>30$.
\end{theorem}

\begin{proof}
The vector $w=q(M)\mathbf{1}$ was formed by $24$ matrix--vector products in exact
integer arithmetic and $\mathbf{1}^{\!\top}M^{j}w$ evaluated for $j=0,1,\dots$, again
exactly. Those integers vanish from the index corresponding to $n=30+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>30$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252562.
