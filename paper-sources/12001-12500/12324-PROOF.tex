...
\title{A proof of the conjectured recurrence for OEIS A116385}
...
\begin{abstract}
OEIS A116385 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A116385 is ``Expansion of e.g.f. Bessel\_I(2,2x) + 2*Bessel\_I(3,2x) + Bessel\_I(4,2x)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 2, 5, 10, 21, 42,\ \dots
\]
...
a(n)\;=\;\left(1 - \left(-1\right)^{n}\right) {\binom{n}{\left\lfloor{\frac{n}{2} - \frac{3}{2}}\right\rfloor}} + \frac{\left(\left(-1\right)^{n} + 1\right) {\binom{n + 1}{\left\lfloor{\frac{n}{2}}\right\rfloor - 1}}}{2} .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} (n+4)*a(n) -2*a(n-1) +(-7*n-8)*a(n-2) +6*a(n-3) +12*(n-2)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 13 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-23, revision 35) the
...
(n + 4)\,a(n) - 2\,a(n-1) + (- 7 n - 8)\,a(n-2) + 6\,a(n-3) + 12 \left(n - 2\right)\,a(n-4)
\end{equation}
...
a(2m)\;=\;e_{0}(m)\;=\;{\binom{2 m + 1}{m + 2}},
\qquad
a(2m+1)\;=\;e_{1}(m)\;=\;2 {\binom{2 m + 1}{m + 2}},
\end{equation}
...
\section{The computation for A116385}
...
exact integer arithmetic at every index where it applies: $30$ instances beginning at
...
entry \href{https://oeis.org/A116385}{A116385}, 2025-01-23.
