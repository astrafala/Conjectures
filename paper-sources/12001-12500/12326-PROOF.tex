...
\title{A proof of the conjectured identity for OEIS A346370}
...
\begin{abstract}
OEIS A346370 carries a conjectured identity expressing its terms through the terms of
A008733, A093907. We prove it. Each entry involved posts a generating function, so the claim is
a single identity between generating functions: writing $F_{a}$ for the generating
function of A346370 and $F_{j}$ for those of the entries on the right, the conjecture
...
OEIS A346370 is ``Upper bound for the number of solutions of the TRINTUM cube puzzle n X 1 X 1 (cuboid formed by 4n + 2 parts) different by the set of parts, which are distinguished by the amount of surface area they contribute to the assembled cuboid''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;3, 8, 10, 19, 22, 34, 38, 54,\ \dots
\]
Its generating function, as posted on the entry, is
\begin{equation}\label{eq:gf}
F_{a}(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;- \frac{x \left(2 x^{6} - 2 x^{5} - 2 x^{4} + 4 x^{3} - x^{2} + 5 x + 3\right)}{\left(x - 1\right)^{3} \left(x + 1\right)^{2} \left(x^{2} + 1\right)} ,
\end{equation}
and the entry carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = A008733(n-1) + A093907(n) for n > 0 (noticed by Sequence Machine). - \emph{Mikhail Kurkov}, Oct 14 2021 [verification needed]
\end{quote}
As of the ``Last modified'' line on the live entry (2024-04-21, revision 48) the
...
\[
F_{A008733}(x) \;=\; \frac{- x^{2} + x - 1}{\left(x - 1\right)^{3} \left(x + 1\right)^{2} \left(x^{2} + 1\right)} \qquad F_{A093907}(x) \;=\; - \frac{2 x \left(x^{4} - x^{3} - 2 x^{2} + 3 x + 1\right)}{\left(x - 1\right)^{3} \left(x + 1\right)^{2}}
\]
...
\section{The computation for A346370}

Assembling the right-hand side by Lemmas~\ref{lem:shift} and~\ref{lem:theta} gives
\[
R(x)\;=\;- \frac{x \left(x^{2} - x + 2 \left(x^{2} + 1\right) \left(x^{4} - x^{3} - 2 x^{2} + 3 x + 1\right) + 1\right)}{\left(x - 1\right)^{3} \left(x + 1\right)^{2} \left(x^{2} + 1\right)} ,
\]
...
\url{https://oeis.org}, 2026. Sequence A346370.
