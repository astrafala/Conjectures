...
\title{A proof of the conjectured recurrence for OEIS A102882}
...
\begin{abstract}
OEIS A102882 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A102882 is ``Expansion of (1+2x)/sqrt((1-3x\^{}2)(1+4x+5x\^{}2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 1, 2, -3, 16, -27, 66,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x + 1}{\sqrt{- 15 x^{4} - 12 x^{3} + 2 x^{2} + 4 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n+1)*a(n) +2*(2*n+3)*(n-1)*a(n-1) +2*(n\^{}2-7)*a(n-2) +6*(5+n-2*n\^{}2)*a(n-3) -15*(n+2)*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 09 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2014-02-03, revision 8) the
...
\[
p_{0}(n)=n \left(n + 1\right),\qquad p_{1}(n)=2 \left(n - 1\right) \left(2 n + 3\right),\qquad p_{2}(n)=2 \left(n^{2} - 7\right),\qquad p_{3}(n)=- 6 \left(2 n^{2} - n - 5\right),\qquad p_{4}(n)=- 15 \left(n - 3\right) \left(n + 2\right)
\]
...
\section{The computation for A102882}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \left(3 x^{2} - 1\right) \left(5 x^{2} + 4 x + 1\right) .
\]
...
\[
n \left(n + 1\right)\,a(n) + 2 \left(n - 1\right) \left(2 n + 3\right)\,a(n-1) + 2 \left(n^{2} - 7\right)\,a(n-2) -  6 \left(2 n^{2} - n - 5\right)\,a(n-3) -  15 \left(n - 3\right) \left(n + 2\right)\,a(n-4) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A102882, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A102882.
