...
\title{A proof of the conjectured recurrence for OEIS A026029}
...
\begin{abstract}
OEIS A026029 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 - 8 x$, a polynomial of degree $1$, which proves
...
OEIS A026029 is ``Number of (s(0), s(1), ..., s(2n)) such that s(i) is a nonnegative integer and |s(i) - s(i-1)| = 1 for i = 1,2,...,n, s(0) = 3, s(2n) = 3. Also T(2n,n), where T is defined in A026022''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 6, 20, 69, 242, 858, 3068,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(16 x^{2} + \left(\sqrt{1 - 4 x} - 1\right)^{4}\right) \left(\sqrt{1 - 4 x} - 1\right)^{2}}{64 x^{4}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+4)*a(n) + 6*(-n-2)*a(n-1) + 4*(2*n-1)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-01-21, revision 40) the
...
\[
p_{0}(n)=n + 4,\qquad p_{1}(n)=- 6 \left(n + 2\right),\qquad p_{2}(n)=4 \left(2 n - 1\right)
\]
...
\section{The computation for A026029}
...
\[
(n + 4)\,a(n) -  6 \left(n + 2\right)\,a(n-1) + 4 \left(2 n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A026029, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=4 - 8 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A026029.
