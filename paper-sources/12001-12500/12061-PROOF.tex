...
\title{A proof of the conjectured recurrence for OEIS A176751}
...
\begin{abstract}
OEIS A176751 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $10 x^{3} - 18 x^{2} + 6 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A176751 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=5, k=0 and l=-1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 5, 9, 42, 173, 846, 4177, 21691,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 20 x^{3} + 12 x^{2} + 5 x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2(1-3n)*a(n-1) +(19-7n)*a(n-2) +4*(8n-25)*a(n-3) +20(4-n)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 27 2011
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=19 - 7 n,\qquad p_{3}(n)=4 \left(8 n - 25\right),\qquad p_{4}(n)=- 20 \left(n - 4\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + (19 - 7 n)\,a(n-2) + 4 \left(8 n - 25\right)\,a(n-3) -  20 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A176751, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=10 x^{3} - 18 x^{2} + 6 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176751.
