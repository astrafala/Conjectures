...
\title{A proof of the conjectured recurrence for OEIS A176605}
...
\begin{abstract}
OEIS A176605 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A176605 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=1, k=0 and l=1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 8, 23, 72, 239, 825,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{4 x \left(- x \left(1 - x\right) + 1\right) + x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-3*n+1)*a(n-1) +(9*n-13)*a(n-2) +8*(-n+3)*a(n-3) +4*(n-4)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 29 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-09, revision 19) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=9 n - 13,\qquad p_{3}(n)=- 8 \left(n - 3\right),\qquad p_{4}(n)=4 \left(n - 4\right)
\]
...
\section{The computation for A176605}
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + (9 n - 13)\,a(n-2) -  8 \left(n - 3\right)\,a(n-3) + 4 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A176605, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A176605.
