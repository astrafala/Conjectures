...
\title{A proof of the conjectured recurrence for OEIS A003435}
...
\begin{abstract}
OEIS A003435 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A003435 is ``Number of directed Hamiltonian circuits on n-octahedron with a marked starting node''. It has offset $2$ and begins
\[
a(2),\dots,a(9)\;=\;8, 192, 11904, 1125120, 153262080, 28507207680, 6951513784320, 2153151603671040,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} a(n) -(4*n\^{}2 - 2*n + 5)*a(n-1) + 2*(n-1)*(4*n-17)*a(n-2) + 12*(n-1)*(n-2)*a(n-3) = 0. - \emph{R. J. Mathar}, Oct 02 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 57) it is still
...
\begin{quote}\small
Recurrence: (2*n-3)*a(n) = 2*n*(4*n\^{}2 - 8*n + 5)*a(n-1) + 4*(n-1)*n*(2*n-1)*a(n-2). - \emph{Vaclav Kotesovec}, Feb 12 2014
\end{quote}
...
\[
1\,a(n) + (- 4 n^{2} + 2 n - 5)\,a(n-1) + 2 \left(n - 1\right) \left(4 n - 17\right)\,a(n-2) + 12 \left(n - 2\right) \left(n - 1\right)\,a(n-3) \;=\;0
\]
...
entry's own values and found to vanish, for all 10 values of $n$ from
$n=5$ upward.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A003435.
