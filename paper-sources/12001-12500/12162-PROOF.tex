...
\title{A proof of the conjectured recurrence for OEIS A026027}
...
\begin{abstract}
OEIS A026027 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $1008 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A026027 is ``a(n) = number of (s(0), s(1), ..., s(2*n-1)) such that s(i) is a nonnegative integer and |s(i) - s(i-1)| = 1 for i = 1,2,...,n, s(0) = 3, s(2*n-1) = 6. Also a(n) = T(2*n-1,n-2), where T is defined in A026022''. It has offset $2$ and begins
\[
a(2),\dots,a(9)\;=\;1, 5, 21, 84, 329, 1274, 4900, 18768,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 2} a(n)\,x^{n}\;=\;\frac{\left(- 16 x^{2} - \left(\sqrt{1 - 4 x} - 1\right)^{4}\right) \left(\sqrt{1 - 4 x} - 1\right)^{5}}{512 x^{5}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (13*n+118)*(n+5)*a(n) +(13*n\^{}2-969*n-3490)*a(n-1) +2*(-203*n\^{}2+1080*n+758)*a(n-2) +4*(73*n-182)*(2*n-5)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 22 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2026-01-21, revision 16) the
...
\[
p_{0}(n)=\left(n + 5\right) \left(13 n + 118\right),\qquad p_{1}(n)=13 n^{2} - 969 n - 3490,\qquad p_{2}(n)=- 2 \left(203 n^{2} - 1080 n - 758\right),\qquad p_{3}(n)=4 \left(2 n - 5\right) \left(73 n - 182\right)
\]
...
\section{The computation for A026027}
...
\[
\left(n + 5\right) \left(13 n + 118\right)\,a(n) + (13 n^{2} - 969 n - 3490)\,a(n-1) -  2 \left(203 n^{2} - 1080 n - 758\right)\,a(n-2) + 4 \left(2 n - 5\right) \left(73 n - 182\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A026027, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=1008 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A026027.
