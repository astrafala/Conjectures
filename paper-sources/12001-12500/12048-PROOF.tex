...
\title{A proof of the conjectured recurrence for OEIS A055217}
...
\begin{abstract}
OEIS A055217 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $1$, a polynomial of degree $0$, which proves
...
OEIS A055217 is ``a(n) = sum of the first n coefficients of (1+x+x\^{}2)\^{}n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 10, 31, 96, 294, 897, 2727,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x + \sqrt{- 3 x^{2} - 2 x + 1} - 1}{2 x \left(3 x^{2} + 2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) -(5*n+1)*a(n-1) +3*(n-1)*a(n-2) +9*(n-1)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 14 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-05, revision 54) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 5 n - 1,\qquad p_{2}(n)=3 \left(n - 1\right),\qquad p_{3}(n)=9 \left(n - 1\right)
\]
...
\section{The computation for A055217}
...
\[
(n + 1)\,a(n) + (- 5 n - 1)\,a(n-1) + 3 \left(n - 1\right)\,a(n-2) + 9 \left(n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A055217, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A055217.
