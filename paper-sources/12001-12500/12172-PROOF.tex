...
\title{The empirical recurrence for OEIS A301842, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A301842 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A301842 is ``Number of 2Xn 0..1 arrays with every element equal to 0, 1 or 2 horizontally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
2, 8, 25, 81, 263, 855, 2778, 9027,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) +a(n-2) -2*a(n-4) for n>6
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 27 2018 14:32:10, revision 4) the statement is
...
publishes, $n=25$, the arrays have $2\cdot25$ cells over
$2$ values, so there are about $10^{15}$ of them to sift. The
walk count replaces that by $16$ states and one matrix--vector product per column.

The threshold below is exact rather than merely sufficient: at $n=6$ the conjectured
...
Every one of those integers vanishes from the index corresponding to $n=6+1$ onwards,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A301842.
