...
\title{A proof of the conjectured recurrence for OEIS A165203}
...
\begin{abstract}
OEIS A165203 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $305 x^{3} + 137 x^{2} - 119 x - 31$, a polynomial of degree $3$, which proves
...
OEIS A165203 is ``Expansion of (1+x)*c(x)\^{}3/(1-x*c(x)\^{}3), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 5, 20, 81, 332, 1372, 5702, 23793,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x + 1\right) \left(- 5 x + \sqrt{1 - 4 x} \left(x - 1\right) + 1\right)}{2 x \left(x^{2} + 4 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*(5*n-31)*a(n) +(5*n\^{}2+74*n+62)*a(n-1) +(-285*n\^{}2+ 1072*n-757)*a(n-2) +(695*n\^{}2-3674*n+4206)*a(n-3) +2*(45*n-74)*(2*n-7)*a(n-4)=0. - \emph{R. J. Mathar}, Dec 11 2011
\end{quote}
...
\[
p_{0}(n)=\left(n + 1\right) \left(5 n - 31\right),\qquad p_{1}(n)=5 n^{2} + 74 n + 62,\qquad p_{2}(n)=- 285 n^{2} + 1072 n - 757,\qquad p_{3}(n)=695 n^{2} - 3674 n + 4206,\qquad p_{4}(n)=2 \left(2 n - 7\right) \left(45 n - 74\right)
\]
...
\section{The computation for A165203}
...
\[
\left(n + 1\right) \left(5 n - 31\right)\,a(n) + (5 n^{2} + 74 n + 62)\,a(n-1) + (- 285 n^{2} + 1072 n - 757)\,a(n-2) + (695 n^{2} - 3674 n + 4206)\,a(n-3) + 2 \left(2 n - 7\right) \left(45 n - 74\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A165203, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=305 x^{3} + 137 x^{2} - 119 x - 31$ is an
...
\url{https://oeis.org}, 2026. Sequence A165203.
