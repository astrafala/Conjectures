...
\title{A proof of the conjectured recurrence for OEIS A108623}
...
\begin{abstract}
OEIS A108623 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A108623 is ``G.f. satisfies x = A(x)*(1-A(x))/(1-A(x)-(A(x))\^{}2)''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 0, -1, -1, 1, 4, 3, -8,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{- x + \sqrt{5 x^{2} - 2 x + 1} - 1}{2 \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +3*(1-n)*a(n-1) +(7*n-18)*a(n-2) +5*(3-n)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-20, revision 26) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 3 \left(n - 1\right),\qquad p_{2}(n)=7 n - 18,\qquad p_{3}(n)=- 5 \left(n - 3\right)
\]
...
\section{The computation for A108623}
...
\[
n\,a(n) -  3 \left(n - 1\right)\,a(n-1) + (7 n - 18)\,a(n-2) -  5 \left(n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A108623, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A108623.
