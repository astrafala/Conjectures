...
\title{A proof of the conjectured recurrence for OEIS A051560}
...
\begin{abstract}
OEIS A051560 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A051560 is ``Second unsigned column of triangle A051379''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 17, 242, 3382, 48504, 725592, 11393808,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;- \frac{\log{\left(1 - x \right)}}{\left(x - 1\right)^{8}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) +(-2*n-13)*a(n-1) +(n+6)\textasciicircum{}2*a(n-2)=0.
\end{quote}
As of the ``Last modified'' line on the live entry (2023-01-25, revision 38) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- 2 n - 13,\qquad p_{2}(n)=\left(n + 6\right)^{2}
\]
...
\section{The computation for A051560}
...
\[
1\,a(n) + (- 2 n - 13)\,a(n-1) + (n^{2} + 12 n + 36)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A051560, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A051560.
