...
\title{A proof of the conjectured recurrence for OEIS A107231}
...
\begin{abstract}
OEIS A107231 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A107231 is ``a(n) = C(n+2,2)*C(n,floor(n/2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 12, 30, 90, 210, 560, 1260,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(- x - 1\right) \left(\sqrt{1 - 4 x^{2}} - 1\right)^{3} \left(- 4 x^{2} + \sqrt{1 - 4 x^{2}} + 1\right)^{2}}{8 x^{4} \left(1 - 4 x^{2}\right)^{\frac{5}{2}} \left(2 x + \sqrt{1 - 4 x^{2}} - 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(n-4)*a(n-1) +2*(-2*n-5)*a(n-2) -4*n*a(n-3)=0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2024-09-04, revision 25) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=n - 4,\qquad p_{2}(n)=- 2 \left(2 n + 5\right),\qquad p_{3}(n)=- 4 n
\]
...
\section{The computation for A107231}
...
\[
n\,a(n) + (n - 4)\,a(n-1) -  2 \left(2 n + 5\right)\,a(n-2) -  4 n\,a(n-3) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A107231, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A107231.
