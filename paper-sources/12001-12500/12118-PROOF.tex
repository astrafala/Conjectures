...
\title{A proof of the conjectured recurrence for OEIS A176959}
...
\begin{abstract}
OEIS A176959 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{4} - 6 x^{3} + 6 x^{2} - 5 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A176959 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=0, k=-1 and l=1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, -1, -4, -11, -25, -47, -62,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 4 x \left(- x^{2} - x \left(1 - x\right)^{2} + \left(1 - x\right)^{2}\right) + \left(1 - x\right)^{2}}{\left(x - 1\right)^{2}}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(n+1)*a(n) +(-7*n+2)*a(n-1) +(19*n-29)*a(n-2) +3*(-7*n+18)*a(n-3) +12*(n-4)*a(n-4) +4*(-n+5)*a(n-5)=0. - \emph{R. J. Mathar}, Mar 02 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-03-02, revision 4) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=2 - 7 n,\qquad p_{2}(n)=19 n - 29,\qquad p_{3}(n)=- 3 \left(7 n - 18\right),\qquad p_{4}(n)=12 \left(n - 4\right),\qquad p_{5}(n)=- 4 \left(n - 5\right)
\]
...
\section{The computation for A176959}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \frac{4 x^{4} - 8 x^{3} + 13 x^{2} - 6 x + 1}{\left(x - 1\right)^{2}} .
\]
...
\[
(n + 1)\,a(n) + (2 - 7 n)\,a(n-1) + (19 n - 29)\,a(n-2) -  3 \left(7 n - 18\right)\,a(n-3) + 12 \left(n - 4\right)\,a(n-4) -  4 \left(n - 5\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A176959, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{4} - 6 x^{3} + 6 x^{2} - 5 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176959.
