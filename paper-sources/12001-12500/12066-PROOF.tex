...
\title{A proof of the conjectured recurrence for OEIS A025577}
...
\begin{abstract}
OEIS A025577 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A025577 is ``Expansion of (x/(1-x))*sqrt((1+x)/(1-3*x))''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 3, 7, 17, 43, 113, 305, 839,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;- \frac{x \sqrt{- \frac{x + 1}{3 x - 1}}}{x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+1)*a(n) +3*(n-1)*a(n-1) +(n-7)*a(n-2) +3*(-n+3)*a(n-3)=0. - \emph{R. J. Mathar}, Jul 02 2018
\end{quote}
As of the ``Last modified'' line on the live entry (2018-07-02, revision 20) the
...
\[
p_{0}(n)=1 - n,\qquad p_{1}(n)=3 \left(n - 1\right),\qquad p_{2}(n)=n - 7,\qquad p_{3}(n)=- 3 \left(n - 3\right)
\]
...
\section{The computation for A025577}
...
\[
(1 - n)\,a(n) + 3 \left(n - 1\right)\,a(n-1) + (n - 7)\,a(n-2) -  3 \left(n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A025577, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A025577.
