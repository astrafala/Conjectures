...
\title{A proof of the conjectured recurrence for OEIS A176756}
...
\begin{abstract}
OEIS A176756 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $10 x^{3} - 17 x^{2} + 4 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A176756 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=4, k=0 and l=-2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 6, 26, 98, 438, 1970, 9294,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 20 x^{3} + 8 x^{2} + 5 x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-3*n+1)*a(n-1) +(-3*n+11)*a(n-2) +2*(14*n-45)*a(n-3) +20*(-n+4)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 18 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-02-18, revision 5) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=11 - 3 n,\qquad p_{3}(n)=2 \left(14 n - 45\right),\qquad p_{4}(n)=- 20 \left(n - 4\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + (11 - 3 n)\,a(n-2) + 2 \left(14 n - 45\right)\,a(n-3) -  20 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A176756, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=10 x^{3} - 17 x^{2} + 4 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176756.
