...
\title{A proof of the conjectured recurrence for OEIS A128734}
...
\begin{abstract}
OEIS A128734 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A128734 is ``Number of LD's in all skew Dyck paths of semilength n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 0, 1, 8, 48, 261, 1358,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{7 x^{2} + 3 x \sqrt{5 x^{2} - 6 x + 1} - 6 x - \sqrt{5 x^{2} - 6 x + 1} + 1}{x \left(5 x + 3 \sqrt{5 x^{2} - 6 x + 1} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +2*(3*n-20)*(n-3)*(n+1)*a(n) +(-39*n\^{}3+362*n\^{}2-739*n+300)*a(n-1) +8*(n-2) *(6*n\^{}2-49*n+75) *a(n-2) -5*(n-2)*(n-3)*(3*n-17)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 17 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 16) the
...
\[
p_{0}(n)=2 \left(n - 3\right) \left(n + 1\right) \left(3 n - 20\right),\qquad p_{1}(n)=- 39 n^{3} + 362 n^{2} - 739 n + 300,\qquad p_{2}(n)=8 \left(n - 2\right) \left(6 n^{2} - 49 n + 75\right),\qquad p_{3}(n)=- 5 \left(n - 3\right) \left(n - 2\right) \left(3 n - 17\right)
\]
...
\section{The computation for A128734}
...
\[
2 \left(n - 3\right) \left(n + 1\right) \left(3 n - 20\right)\,a(n) + (- 39 n^{3} + 362 n^{2} - 739 n + 300)\,a(n-1) + 8 \left(n - 2\right) \left(6 n^{2} - 49 n + 75\right)\,a(n-2) -  5 \left(n - 3\right) \left(n - 2\right) \left(3 n - 17\right)\,a(n-3) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A128734, agreeing on all 13 terms compared.
...
vanish, for all 21 values of $n$ from $n=3$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A128734.
