...
\title{A proof of the conjectured recurrence for OEIS A176759}
...
\begin{abstract}
OEIS A176759 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{4} - 8 x^{3} + 12 x^{2} - 5 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A176759 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=0, k=1 and l=-1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 1, 4, 11, 27, 67, 178,\ \dots
\]
...
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(-7*n+2)*a(n-1) +(19*n-29)*a(n-2) +(-29*n+82)*a(n-3) +4*(5*n-19)*a(n-4) +4*(-n+5)*a(n-5)=0. - \emph{R. J. Mathar}, Feb 18 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-02-18, revision 6) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=2 - 7 n,\qquad p_{2}(n)=19 n - 29,\qquad p_{3}(n)=82 - 29 n,\qquad p_{4}(n)=4 \left(5 n - 19\right),\qquad p_{5}(n)=- 4 \left(n - 5\right)
\]
...
\section{The computation for A176759}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \frac{4 x^{4} - 16 x^{3} + 13 x^{2} - 6 x + 1}{\left(x - 1\right)^{2}} .
\]
...
\[
(n + 1)\,a(n) + (2 - 7 n)\,a(n-1) + (19 n - 29)\,a(n-2) + (82 - 29 n)\,a(n-3) + 4 \left(5 n - 19\right)\,a(n-4) -  4 \left(n - 5\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A176759, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{4} - 8 x^{3} + 12 x^{2} - 5 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176759.
