...
\title{A proof of the conjectured recurrence for OEIS A025175}
...
\begin{abstract}
OEIS A025175 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A025175 is ``Jacobi polynomial P((1, 1), n, (1/2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 3, -40, -190, -168, 2023, 10096,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x + \sqrt{16 x^{2} - 4 x + 1} - 1}{6 x^{2} \sqrt{16 x^{2} - 4 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) + 6*(-n-1)*a(n-1) + 12*(2*n+1)*a(n-2) + 32*(-n+1)*a(n-3) = 0. - \emph{R. J. Mathar}, Mar 03 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-18, revision 37) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 6 \left(n + 1\right),\qquad p_{2}(n)=12 \left(2 n + 1\right),\qquad p_{3}(n)=- 32 \left(n - 1\right)
\]
...
\section{The computation for A025175}
...
\[
(n + 2)\,a(n) -  6 \left(n + 1\right)\,a(n-1) + 12 \left(2 n + 1\right)\,a(n-2) -  32 \left(n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A025175, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A025175.
