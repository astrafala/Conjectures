...
\title{A proof of the conjectured recurrence for OEIS A177126}
...
\begin{abstract}
OEIS A177126 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 56 x^{4} + 126 x^{3} - 84 x^{2} + 15 x$, a polynomial of degree $4$, which proves
...
OEIS A177126 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=10, k=1 and l=1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 10, 23, 150, 765, 5065, 32337, 223672,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 28 x^{4} + 56 x^{3} - 27 x^{2} - 6 x + 1}{x^{2} - 2 x + 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n+1)*a(n) -n*(7*n-2)*a(n-1) -3*n*(7*n-17)*a(n-2) +n*(83*n-250)*a(n-3) -84*n*(n-4)*a(n-4) +28*n*(n-5)*a(n-5) =0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2014-01-20, revision 5) the
...
\[
p_{0}(n)=n \left(n + 1\right),\qquad p_{1}(n)=- n \left(7 n - 2\right),\qquad p_{2}(n)=- 3 n \left(7 n - 17\right),\qquad p_{3}(n)=n \left(83 n - 250\right),\qquad p_{4}(n)=- 84 n \left(n - 4\right),\qquad p_{5}(n)=28 n \left(n - 5\right)
\]
...
\section{The computation for A177126}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \frac{28 x^{4} - 56 x^{3} + 27 x^{2} + 6 x - 1}{\left(x - 1\right)^{2}} .
\]
...
\[
B(x)\;=\;- 56 x^{4} + 126 x^{3} - 84 x^{2} + 15 x ,
\]
...
\[
n \left(n + 1\right)\,a(n) -  n \left(7 n - 2\right)\,a(n-1) -  3 n \left(7 n - 17\right)\,a(n-2) + n \left(83 n - 250\right)\,a(n-3) -  84 n \left(n - 4\right)\,a(n-4) + 28 n \left(n - 5\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A177126, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 56 x^{4} + 126 x^{3} - 84 x^{2} + 15 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A177126.
