...
\title{A proof of the conjectured recurrence for OEIS A005558}
...
\begin{abstract}
OEIS A005558 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A005558 is ``a(n) is the number of n-step walks on square lattice such that 0 <= y <= x at each step''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 6, 20, 50, 175, 490,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = C(n+1, ceiling(n/2))*C(n, floor(n/2)) - C(n+1, ceiling((n-1)/2))*C(n, floor((n-1)/2)). - \emph{Paul D. Hanna}, Apr 16 2004
\end{quote}
that is,
\begin{equation}\label{eq:cf}
a(n)\;=\;{\binom{n}{\left\lfloor{\frac{n}{2}}\right\rfloor}} {\binom{n + 1}{\left\lceil{\frac{n}{2}}\right\rceil}} - {\binom{n}{\left\lfloor{\frac{n}{2} - \frac{1}{2}}\right\rfloor}} {\binom{n + 1}{\left\lceil{\frac{n}{2} - \frac{1}{2}}\right\rceil}} .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} (n+3)*(n+2)*a(n) - 4*(n\^{}2+3*n+1)*a(n-1) + 16*(-n\^{}2+n+1)*a(n-2) + 64*(n-1)*(n-2)*a(n-3) = 0. - \emph{R. J. Mathar}, Apr 02 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-10, revision 172) the
...
\left(n + 2\right) \left(n + 3\right)\,a(n) -  4 \left(n^{2} + 3 n + 1\right)\,a(n-1) -  16 \left(n^{2} - n - 1\right)\,a(n-2) + 64 \left(n - 2\right) \left(n - 1\right)\,a(n-3)
\end{equation}
...
a(2m)\;=\;e_{0}(m)\;=\;\frac{{\binom{2 m}{m}} \left(2 m + 1\right)!}{\left(m + 1\right)!^{2}},
\qquad
a(2m+1)\;=\;e_{1}(m)\;=\;\frac{2 {\binom{2 m + 1}{m}} \left(2 m + 1\right)!}{m! \left(m + 2\right)!},
\end{equation}
...
\section{The computation for A005558}
...
entry \href{https://oeis.org/A005558}{A005558}, 2025-11-10.
