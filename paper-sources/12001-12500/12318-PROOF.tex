...
\title{A proof of the conjectured recurrence for OEIS A191796}
...
\begin{abstract}
OEIS A191796 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $211380 x^{4}$, a polynomial of degree $4$, which proves
...
OEIS A191796 is ``Number of DUU's in all length n left factors of Dyck paths; here U=(1,1) and D=(1,-1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 0, 0, 1, 3, 9, 21,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4 x^{4} - x^{3} - 5 x^{2} + \sqrt{1 - 4 x^{2}} \left(x^{3} + 3 x^{2} - 1\right) + 1}{2 x \sqrt{1 - 4 x^{2}} \left(2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+1)*(2527*n\^{}2+15963*n-146560)*a(n) +(-2527*n\^{}3+68000*n\^{}2-231053*n-293120)*a(n-1) +2*(12635*n\^{}3+906*n\^{}2-429395*n+746484)*a(n-2) +4*(2527*n\^{}3-70527*n\^{}2+316742*n-316524)*a(n-3) -24*(n-5)*(2527*n\^{}2-232*n-28664)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-28, revision 13) the
...
\[
p_{0}(n)=- \left(n + 1\right) \left(2527 n^{2} + 15963 n - 146560\right),\qquad p_{1}(n)=- 2527 n^{3} + 68000 n^{2} - 231053 n - 293120,\qquad p_{2}(n)=2 \left(12635 n^{3} + 906 n^{2} - 429395 n + 746484\right),\qquad p_{3}(n)=4 \left(2527 n^{3} - 70527 n^{2} + 316742 n - 316524\right),\qquad p_{4}(n)=- 24 \left(n - 5\right) \left(2527 n^{2} - 232 n - 28664\right)
\]
...
\section{The computation for A191796}
...
\[
- \left(n + 1\right) \left(2527 n^{2} + 15963 n - 146560\right)\,a(n) + (- 2527 n^{3} + 68000 n^{2} - 231053 n - 293120)\,a(n-1) + 2 \left(12635 n^{3} + 906 n^{2} - 429395 n + 746484\right)\,a(n-2) + 4 \left(2527 n^{3} - 70527 n^{2} + 316742 n - 316524\right)\,a(n-3) -  24 \left(n - 5\right) \left(2527 n^{2} - 232 n - 28664\right)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A191796, agreeing on all 13 terms compared.
...
vanish, for all 31 values of $n$ from $n=5$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=211380 x^{4}$ is an
...
\url{https://oeis.org}, 2026. Sequence A191796.
