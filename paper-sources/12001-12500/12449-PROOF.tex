...
\title{A proof of the conjectured recurrence for OEIS A113956}
...
\begin{abstract}
OEIS A113956 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{4} - 2 x^{3} - 4 x^{2} + x$, a polynomial of degree $4$, which proves
...
OEIS A113956 is ``Expansion of (1/((1-4x)c(x)))/(1-x\^{}2c(x)/sqrt(1-4x)), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 12, 48, 194, 787, 3199, 13017,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- \sqrt{1 - 4 x} - 1}{4 x^{2} + x \sqrt{1 - 4 x} + 7 x - 2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: n*a(n) +2*(-5*n+4)*a(n-1) +(27*n-46)*a(n-2) +(5*n-2)*a(n-3) +(-57*n+188)*a(n-4) +2*(-21*n+83)*a(n-5) +4*(-2*n+9)*a(n-6)=0. - \emph{R. J. Mathar}, Jan 24 2020
\end{quote}
As of the ``Last modified'' line on the live entry (2026-02-15, revision 12) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(5 n - 4\right),\qquad p_{2}(n)=27 n - 46,\qquad p_{3}(n)=5 n - 2,\qquad p_{4}(n)=188 - 57 n,\qquad p_{5}(n)=- 2 \left(21 n - 83\right),\qquad p_{6}(n)=- 4 \left(2 n - 9\right)
\]
...
\section{The computation for A113956}
...
\[
n\,a(n) -  2 \left(5 n - 4\right)\,a(n-1) + (27 n - 46)\,a(n-2) + (5 n - 2)\,a(n-3) + (188 - 57 n)\,a(n-4) -  2 \left(21 n - 83\right)\,a(n-5) -  4 \left(2 n - 9\right)\,a(n-6) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A113956, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{4} - 2 x^{3} - 4 x^{2} + x$ is an
...
\url{https://oeis.org}, 2026. Sequence A113956.
