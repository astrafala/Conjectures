...
\title{The empirical recurrence for OEIS A263690, derived from the entry's generating function}
...
\begin{abstract}
OEIS A263690 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A263690 is ``Number of length n arrays of permutations of 0..n-1 with each element moved by -n to n places and every three consecutive elements having its maximum within 3 of its minimum.''. It has offset $0$ and begins
\[
1, 1, 2, 6, 24, 36, 36, 48,\ \dots
\]
...
\begin{quote}
G.f.: (8*x\^{}12+8*x\^{}11-4*x\^{}10-8*x\^{}8+6*x\^{}7+20*x\^{}6-7*x\^{}5-16*x\^{}4-4*x\^{}3-1) / (x\^{}4-x\^{}3+x\^{}2+x-1). - \_Alois P. Heinz\_, Apr 08 2020
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2020 16:18:46, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{8 x^{12} + 8 x^{11} - 4 x^{10} - 8 x^{8} + 6 x^{7} + 20 x^{6} - 7 x^{5} - 16 x^{4} - 4 x^{3} - 1}{x^{4} - x^{3} + x^{2} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{4} + x^{3} - x^{2} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 8 x^{12} - 8 x^{11} + 4 x^{10} + 8 x^{8} - 6 x^{7} - 20 x^{6} + 7 x^{5} + 16 x^{4} + 4 x^{3} + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263690.
