...
\title{A proof of the conjectured recurrence for OEIS A085781}
...
\begin{abstract}
OEIS A085781 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A085781 is ``a(n) = 2*binomial(2*n+1,n+1) - 2\textasciicircum{}n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 16, 62, 236, 892, 3368, 12742,\ \dots
\]
The name is itself the definition of the sequence:
\begin{quote}\small
a(n) = 2*binomial(2*n+1,n+1) - 2\textasciicircum{}n.
\end{quote}
that is,
\begin{equation}\label{eq:cf}
a(n)\;=\;- 2^{n} + 2 {\binom{2 n + 1}{n + 1}} .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) -2*(4*n+1)*a(n-1) +4*(5*n-3)*a(n-2) +8*(-2*n+3)*a(n-3)=0. - \emph{R. J. Mathar}, Jan 25 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2021-08-30, revision 17) the
...
(n + 1)\,a(n) -  2 \left(4 n + 1\right)\,a(n-1) + 4 \left(5 n - 3\right)\,a(n-2) -  8 \left(2 n - 3\right)\,a(n-3)
\end{equation}
...
exact integer arithmetic at every index where it applies: $21$ instances beginning at
...
entry \href{https://oeis.org/A085781}{A085781}, 2021-08-30.
