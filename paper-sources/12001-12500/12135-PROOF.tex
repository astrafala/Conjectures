...
\title{A proof of the conjectured recurrence for OEIS A176750}
...
\begin{abstract}
OEIS A176750 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $8 x^{3} - 14 x^{2} + 4 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A176750 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=4, k=0 and l=-1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 7, 29, 113, 506, 2321, 11112,\ \dots
\]
The entry posts no generating function of its own. Its NAME states the
relation \texttt{the entry states the generating function as G.f f: f(z)=(1-sqrt(1-4*z*(a(0)-z*a(0)\textasciicircum{}2+z*a(1)+(k+l)*z\textasciicircum{}2/(1-z)+k*z\textasciicircum{}2/(1-z)\textasciicircum{}2)))/(2*z) (k=0, l=-1); a(0) and a(1) are read off the entry's own terms and the parameters are the values given on that line} to another entry, and that entry's exponential generating
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 16 x^{3} + 8 x^{2} + 5 x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-3*n+1)*a(n-1) +(-3*n+11)*a(n-2) +4*(6*n-19)*a(n-3) +16*(-n+4)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 18 2016
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=11 - 3 n,\qquad p_{3}(n)=4 \left(6 n - 19\right),\qquad p_{4}(n)=- 16 \left(n - 4\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + (11 - 3 n)\,a(n-2) + 4 \left(6 n - 19\right)\,a(n-3) -  16 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A176750, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=8 x^{3} - 14 x^{2} + 4 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176750.
