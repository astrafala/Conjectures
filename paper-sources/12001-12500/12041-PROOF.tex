...
\title{A proof of the conjectured recurrence for OEIS A006231}
...
\begin{abstract}
OEIS A006231 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A006231 is ``a(n) = Sum\_\{k=2..n\} n(n-1)...(n-k+1)/k''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;0, 1, 5, 20, 84, 409, 2365, 16064,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,\frac{x^{n}}{n!}\;=\;\left(- x + \log{\left(- \frac{1}{x - 1} \right)}\right) e^{x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) + (-n-2)*a(n-1) + (3*n-2)*a(n-2) + 3*(-n+2)*a(n-3) + (n-3)*a(n-4) = 0. - \emph{R. J. Mathar}, Aug 06 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 93) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- n - 2,\qquad p_{2}(n)=3 n - 2,\qquad p_{3}(n)=- 3 \left(n - 2\right),\qquad p_{4}(n)=n - 3
\]
...
\section{The computation for A006231}
...
\[
1\,a(n) + (- n - 2)\,a(n-1) + (3 n - 2)\,a(n-2) -  3 \left(n - 2\right)\,a(n-3) + (n - 3)\,a(n-4) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A006231, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A006231.
