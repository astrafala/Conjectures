...
\title{A proof of the conjectured recurrence for OEIS A210474}
...
\begin{abstract}
OEIS A210474 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x^{2} - 4 x + 1$, a polynomial of degree $2$, which proves
...
OEIS A210474 is ``The number of different lattice paths from (0,0) to (2n,0) using steps of S=\{(i,i) or (i,-i): i=1,2,...,n\} with j flaws(j=1,2,...,n-1), where the j flaws is the sum of lengths of down steps below the x-axis. (For down steps that are partly above and partly below the x-axis we just count the part below the x-axis.) This number is independent of the number of flaws''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 4, 24, 156, 1072, 7668, 56520,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{\left(x - 1\right) \left(3 x - \sqrt{9 x^{2} - 10 x + 1} + 1\right)}{8 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:}  (n+1)*a(n) +2*(-5*n+3)*a(n-1) +9*(n-3)*a(n-2)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2023-04-12, revision 26) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(5 n - 3\right),\qquad p_{2}(n)=9 \left(n - 3\right)
\]
...
\section{The computation for A210474}
...
\[
(n + 1)\,a(n) -  2 \left(5 n - 3\right)\,a(n-1) + 9 \left(n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A210474, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=3 x^{2} - 4 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A210474.
