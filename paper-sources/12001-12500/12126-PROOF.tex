...
\title{A proof of the conjectured recurrence for OEIS A051524}
...
\begin{abstract}
OEIS A051524 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A051524 is ``Second unsigned column of triangle A051338''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 13, 146, 1650, 19524, 245004, 3272688,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;- \frac{\log{\left(1 - x \right)}}{\left(x - 1\right)^{6}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) +(-2*n-9)*a(n-1) +(n+4)\^{}2*a(n-2)=0. - \emph{R. J. Mathar}, Aug 04 2013
\end{quote}
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- 2 n - 9,\qquad p_{2}(n)=\left(n + 4\right)^{2}
\]
...
\section{The computation for A051524}
...
\[
1\,a(n) + (- 2 n - 9)\,a(n-1) + (n^{2} + 8 n + 16)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A051524, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A051524.
