...
\title{A proof of the conjectured recurrence for OEIS A138164}
...
\begin{abstract}
OEIS A138164 carries a conjectured linear recurrence with polynomial coefficients, of
order 6. The entry separately states a recurrence of order 4 that is not
...
OEIS A138164 is ``Row sums of Riordan array (c(-x\^{}2),xc(-x\^{}2)\^{}2)\^{}(-1) where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 4, 9, 20, 47, 109,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} -8*n*(n-1)*a(n) + 12*(n+3)*(n-1)*a(n-1) + 2*(89*n\^{}2-512*n+651)*a(n-2) + 3*(-127*n\^{}2+715*n-966)*a(n-3) + 3*(-247*n\^{}2+2431*n-5698)*a(n-4) + 9*(75*n-341)*(3*n-16)*a(n-5) - 72*(3*n-14)*(3*n-16)*a(n-6) = 0. - \emph{R. J. Mathar}, Feb 24 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2021-11-15, revision 35) it is still
...
\section{The computation for A138164}

Reading both relations as operators and dividing the conjectured one by the stated one
leaves remainder $0$, with quotient
\[
Q \;=\; \left(\frac{24}{11 n^{2} + 60 n + 73}\right) + \left(\frac{- 33 n^{2} - 213 n - 426}{121 n^{4} + 1562 n^{3} + 7307 n^{2} + 14626 n + 10512}\right)\,N + \left(- \frac{2}{11 n^{2} + 82 n + 144}\right)\,N^{2} .
\]
...
\[
- 8 n \left(n - 1\right)\,a(n) + 12 \left(n - 1\right) \left(n + 3\right)\,a(n-1) + 2 \left(89 n^{2} - 512 n + 651\right)\,a(n-2) -  3 \left(127 n^{2} - 715 n + 966\right)\,a(n-3) -  3 \left(247 n^{2} - 2431 n + 5698\right)\,a(n-4) + 9 \left(3 n - 16\right) \left(75 n - 341\right)\,a(n-5) -  72 \left(3 n - 16\right) \left(3 n - 14\right)\,a(n-6) \;=\;0
\]
...
entry's own values and found to vanish, for all 24 values of $n$ from
$n=6$ upward.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A138164.
