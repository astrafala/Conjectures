...
\title{A proof of the conjectured recurrence for OEIS A073591}
...
\begin{abstract}
OEIS A073591 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A073591 is ``a(n) = A000522(n) + 1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;2, 3, 6, 17, 66, 327, 1958, 13701,\ \dots
\]
The entry posts no generating function of its own. Its NAME states the
relation \texttt{a(n) = A000522(n) + 1} to another entry, and that entry's exponential generating
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{\left(x - 2\right) e^{x}}{x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) +(-n-2)*a(n-1) +(2*n-1)*a(n-2) +(-n+2)*a(n-3)=0.
\end{quote}
As of the ``Last modified'' line on the live entry (2026-08-27, revision 38) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- n - 2,\qquad p_{2}(n)=2 n - 1,\qquad p_{3}(n)=2 - n
\]
...
\section{The computation for A073591}
...
\[
1\,a(n) + (- n - 2)\,a(n-1) + (2 n - 1)\,a(n-2) + (2 - n)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A073591, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A073591.
