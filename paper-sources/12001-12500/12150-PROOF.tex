...
\title{The empirical recurrence for OEIS A230269, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A230269 counts arrays over $\{0,\dots,2\}$ in which every cell is constrained by the
...
OEIS A230269 is ``Number of n X 2 0..2 arrays x(i,j) with each element horizontally or vertically next to at least one element with value (x(i,j)+1) mod 3, and upper left element zero.''. It has offset $1$ and begins
\[
0, 0, 2, 8, 30, 108, 386, 1376,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) - a(n-2) - 2*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 24 2018 07:50:38, revision 8) the statement is
...
$3$ values, so there are about $10^{24}$ of them to sift. The
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230269.
