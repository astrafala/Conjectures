...
\title{A proof of the conjectured recurrence for OEIS A118093}
...
\begin{abstract}
OEIS A118093 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 6 x^{3}$, a polynomial of degree $3$, which proves
...
OEIS A118093 is ``Numbers of rooted hypermaps on the torus with n darts (darts are semi-edges in the particular case of ordinary maps)''. It has offset $3$ and begins
\[
a(3),\dots,a(10)\;=\;1, 15, 165, 1611, 14805, 131307, 1138261, 9713835,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 3} a(n)\,x^{n}\;=\;\frac{- 4 x^{2} + 7 x + \sqrt{1 - 8 x} \left(1 - 3 x\right) - 1}{8 \left(x + 1\right) \left(8 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +n*(5*n-17)*a(n) -15*(n-1)*(5*n-16)*a(n-1) +12*(20*n\^{}2-103*n+140)*a(n-2) +32*(5*n-12)*(2*n-5)*a(n-3)=0. - \emph{R. J. Mathar}, Apr 05 2018
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 53) the
...
\[
p_{0}(n)=n \left(5 n - 17\right),\qquad p_{1}(n)=- 15 \left(n - 1\right) \left(5 n - 16\right),\qquad p_{2}(n)=12 \left(20 n^{2} - 103 n + 140\right),\qquad p_{3}(n)=32 \left(2 n - 5\right) \left(5 n - 12\right)
\]
...
\section{The computation for A118093}
...
\[
n \left(5 n - 17\right)\,a(n) -  15 \left(n - 1\right) \left(5 n - 16\right)\,a(n-1) + 12 \left(20 n^{2} - 103 n + 140\right)\,a(n-2) + 32 \left(2 n - 5\right) \left(5 n - 12\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A118093, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 6 x^{3}$ is an
...
\url{https://oeis.org}, 2026. Sequence A118093.
