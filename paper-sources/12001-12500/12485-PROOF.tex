...
\title{A proof of the conjectured recurrence for OEIS A166694}
...
\begin{abstract}
OEIS A166694 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A166694 is ``A transform of the large Schroeder numbers, A006318''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 10, 52, 290, 1706, 10440, 65822,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} - 3 x - \sqrt{x^{4} - 10 x^{3} + 19 x^{2} - 10 x + 1} + 1}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +5*(1-2n)*a(n-1) +19*(n-2)*a(n-2) +5*(7-2*n)*a(n-3) +(n-5)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 16 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2016-05-24, revision 10) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 5 \left(2 n - 1\right),\qquad p_{2}(n)=19 \left(n - 2\right),\qquad p_{3}(n)=- 5 \left(2 n - 7\right),\qquad p_{4}(n)=n - 5
\]
...
\[
(n + 1)\,a(n) -  5 \left(2 n - 1\right)\,a(n-1) + 19 \left(n - 2\right)\,a(n-2) -  5 \left(2 n - 7\right)\,a(n-3) + (n - 5)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A166694, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A166694.
