...
\title{A proof of the conjectured closed form for OEIS A126501}
...
\begin{abstract}
OEIS A126501 carries a conjectured closed form for its terms. We prove it. The entry
...
explicit rational function; here it collapses to the polynomial $0$, of degree
...
OEIS A126501 is ``Number of n-tuples of numbers [0..5] (leading zeros allowed) in which adjacent digits differ by 4 or less''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 6, 34, 194, 1106, 6306, 35954, 204994,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x - 1}{4 x^{2} + 5 x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
(2\^{}(-1-n)*((5-sqrt(41))\^{}n*(-7+sqrt(41)) + (5+sqrt(41))\^{}n*(7+sqrt(41)))) / sqrt(41). - \emph{Colin Barker}, Jan 20 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2022-10-21, revision 47) the
...
\[
f(n)\;=\;\frac{2^{- n - 1} \sqrt{41} \left(\left(-7 + \sqrt{41}\right) \left(5 - \sqrt{41}\right)^{n} + \left(5 + \sqrt{41}\right)^{n} \left(\sqrt{41} + 7\right)\right)}{41} .
\]
...
For every $n>0$,
\[
a(n)\;=\;\frac{2^{- n - 1} \sqrt{41} \left(\left(-7 + \sqrt{41}\right) \left(5 - \sqrt{41}\right)^{n} + \left(5 + \sqrt{41}\right)^{n} \left(\sqrt{41} + 7\right)\right)}{41} .
\]
\end{theorem}
...
section of A126501, agreeing on all 13 terms compared.
...
with the entry's own value for every $n>0$ in range, agreeing in all 21 cases
from $n=0$ upward. This is what Corollary~\ref{cor:criterion} predicts from the
...
\url{https://oeis.org}, 2026. Sequence A126501.
