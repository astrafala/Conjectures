...
\title{A proof of the conjectured recurrence for OEIS A168506}
...
\begin{abstract}
OEIS A168506 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $24 x^{4} + 100 x^{3} - 16 x^{2}$, a polynomial of degree $4$, which proves
...
OEIS A168506 is ``Number of rooted plane trees of total weight n whose nodes are themselves planted plane trees whose roots are distinguished as either red or blue, the weight of each such node being equal to the size of the corresponding planted tree''. It has offset $2$ and begins
\[
a(2),\dots,a(9)\;=\;2, 2, 8, 18, 64, 188, 656, 2154,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 2} a(n)\,x^{n}\;=\;\frac{1}{2} - \frac{\sqrt{4 x \sqrt{1 - 4 x} - 4 x + 1}}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-1)*(n-6)*a(n) -2*(6*n-35)*(n-1)*(n-2)*a(n-1) +4*(8*n\^{}3-88*n\^{}2+305*n-330)*a(n-2) +8*(8*n\^{}3-92*n\^{}2+314*n-305)*a(n-3) -16*(n-5)*(4*n-19)*(4*n-17)*a(n-4) =0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2021-05-20, revision 13) the
...
\[
p_{0}(n)=n \left(n - 6\right) \left(n - 1\right),\qquad p_{1}(n)=- 2 \left(n - 2\right) \left(n - 1\right) \left(6 n - 35\right),\qquad p_{2}(n)=4 \left(8 n^{3} - 88 n^{2} + 305 n - 330\right),\qquad p_{3}(n)=8 \left(8 n^{3} - 92 n^{2} + 314 n - 305\right),\qquad p_{4}(n)=- 16 \left(n - 5\right) \left(4 n - 19\right) \left(4 n - 17\right)
\]
...
\[
n \left(n - 6\right) \left(n - 1\right)\,a(n) -  2 \left(n - 2\right) \left(n - 1\right) \left(6 n - 35\right)\,a(n-1) + 4 \left(8 n^{3} - 88 n^{2} + 305 n - 330\right)\,a(n-2) + 8 \left(8 n^{3} - 92 n^{2} + 314 n - 305\right)\,a(n-3) -  16 \left(n - 5\right) \left(4 n - 19\right) \left(4 n - 17\right)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A168506, agreeing on all 13 terms compared.
...
vanish, for all 20 values of $n$ from $n=7$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=24 x^{4} + 100 x^{3} - 16 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A168506.
