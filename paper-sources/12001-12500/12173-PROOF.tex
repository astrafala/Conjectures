...
\title{The empirical recurrence for OEIS A303721, proved by transfer matrix}
...
\begin{abstract}
OEIS A303721 counts $2$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,3\}$, and it carries an empirical
...
OEIS A303721 is ``Number of nX2 0..1 arrays with every element unequal to 0, 1, 2 or 3 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
2, 8, 21, 49, 120, 293, 719, 1774,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) +a(n-2) -4*a(n-3) -4*a(n-4) for n>5
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 29 2018 11:08:01, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + a(n-2) - 4\,a(n-3) - 4\,a(n-4)
\end{equation}
for all $n>5$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303721.
