...
\title{A proof of the conjectured recurrence for OEIS A191786}
...
\begin{abstract}
OEIS A191786 carries a conjectured linear recurrence with polynomial coefficients, of
order 8, contributed by R.~J.~Mathar. We prove it. The generating function of the
...
closed form. Here it equals $396 x^{7} + 342 x^{6} + 428 x^{5} + 306 x^{4} - 264 x^{3} - 374 x^{2} + 132 x + 210$, a polynomial of degree $7$, which proves
the recurrence for every $n>7$.
\end{abstract}
...
OEIS A191786 is ``Number of length n left factors of Dyck paths having no triple-rises (triple-rise = three consecutive (1,1)-steps)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 2, 4, 5, 9, 12,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 \left(- x^{2} - x - 1\right)}{2 x^{3} + x^{2} - \sqrt{- 3 x^{4} - 2 x^{2} + 1} - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+3)*(13*n-70)*a(n) +(-13*n\^{}2+19*n-102)*a(n-1) +(65*n\^{}2-221*n-516) *a(n-2) +(65*n\^{}2-197*n+288)*a(n-3) -(n+6)*(13*n-97) *a(n-4) +3*(-13*n\^{}2+35*n-70) *a(n-5) +(-169*n\^{}2+1201*n-2208) *a(n-6) -9*(13*n-40)*(n-5) *a(n-7) -6*(13*n-25)*(n-6) *a(n-8)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-06-14, revision 9) the
statement is still recorded as a conjecture, and no proof appears among the entry's links,
formulas or programs.

Writing the conjectured relation in the form $\sum_{i=0}^{8}p_{i}(n)\,a(n-i)=0$,
the coefficient polynomials are
\[
p_{0}(n)=- \left(n + 3\right) \left(13 n - 70\right),\qquad p_{1}(n)=- 13 n^{2} + 19 n - 102,\qquad p_{2}(n)=65 n^{2} - 221 n - 516,\qquad p_{3}(n)=65 n^{2} - 197 n + 288,\qquad p_{4}(n)=- \left(n + 6\right) \left(13 n - 97\right),\qquad p_{5}(n)=- 3 \left(13 n^{2} - 35 n + 70\right),\qquad p_{6}(n)=- 169 n^{2} + 1201 n - 2208,\qquad p_{7}(n)=- 9 \left(n - 5\right) \left(13 n - 40\right),\qquad p_{8}(n)=- 6 \left(n - 6\right) \left(13 n - 25\right)
\]
...
\section{The computation for A191786}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \left(x^{2} + 1\right) \left(3 x^{2} - 1\right) .
\]
...
\[
B(x)\;=\;396 x^{7} + 342 x^{6} + 428 x^{5} + 306 x^{4} - 264 x^{3} - 374 x^{2} + 132 x + 210 ,
\]
...
\[
- \left(n + 3\right) \left(13 n - 70\right)\,a(n) + (- 13 n^{2} + 19 n - 102)\,a(n-1) + (65 n^{2} - 221 n - 516)\,a(n-2) + (65 n^{2} - 197 n + 288)\,a(n-3) -  \left(n + 6\right) \left(13 n - 97\right)\,a(n-4) -  3 \left(13 n^{2} - 35 n + 70\right)\,a(n-5) + (- 169 n^{2} + 1201 n - 2208)\,a(n-6) -  9 \left(n - 5\right) \left(13 n - 40\right)\,a(n-7) -  6 \left(n - 6\right) \left(13 n - 25\right)\,a(n-8) \;=\;0
\]
holds for every $n>7$.
\end{theorem}
...
$n\le7$ some of the shifted terms $a(n-i)$ fall outside the range of the sequence,
...
section of A191786, agreeing on all 13 terms compared.
...
vanish, for all 33 values of $n$ from $n=8$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=396 x^{7} + 342 x^{6} + 428 x^{5} + 306 x^{4} - 264 x^{3} - 374 x^{2} + 132 x + 210$ is an
identity of functions and the theorem holds for all $n>7$ simultaneously.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A191786.
