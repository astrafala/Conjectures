...
\title{The empirical recurrence for OEIS A316515, proved by transfer matrix}
...
\begin{abstract}
OEIS A316515 counts $5$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,3,7,8\}$, and it carries an empirical
...
OEIS A316515 is ``Number of nX5 0..1 arrays with every element unequal to 0, 1, 2, 3, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
16, 120, 174, 619, 2995, 11977, 48677, 201771,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 05 2018 07:15:00, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 4\,a(n-2) + 8\,a(n-3) - 34\,a(n-4) + 39\,a(n-5) - 54\,a(n-6) + 102\,a(n-7) - 71\,a(n-8) + 15\,a(n-9) - 21\,a(n-10) + 2\,a(n-11) + 32\,a(n-12) - 63\,a(n-13) + 115\,a(n-14) - 97\,a(n-15) + 30\,a(n-16) - 13\,a(n-17) + 22\,a(n-18) - 26\,a(n-19) + 5\,a(n-20) + 22\,a(n-21) - 18\,a(n-22) + 5\,a(n-23) + 2\,a(n-24) - 2\,a(n-25)
\end{equation}
for all $n>28$.
...
has score in $\{0,1,2,3,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,7,8\}$. Read the sequence as the
...
They vanish from the index corresponding to $n=28+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A316515.
