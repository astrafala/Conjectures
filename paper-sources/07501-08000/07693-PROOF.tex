...
\title{The empirical recurrence for OEIS A235304, proved by transfer matrix}
...
\begin{abstract}
OEIS A235304 counts $3$-column arrays over $\{0,\dots,5\}$ in which every
$2\times2$ subblock has its diagonal sum differing from its antidiagonal sum by
exactly $6$, and it
carries an empirical recurrence of order $14$ contributed by R.~H.~Hardin. It is true,
...
the residual is $\mathbf{1}^{\!\top}M^{\,n-14}q(M)\mathbf{1}$, so the recurrence
...
OEIS A235304 is ``Number of (n+1) X (2+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 6 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
210, 518, 1106, 2918, 6786, 18662, 46226,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 18 2022 23:19:54, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) + 25\,a(n-2) - 112\,a(n-3) - 231\,a(n-4) + 1260\,a(n-5) + 875\,a(n-6) - 7280\,a(n-7) - 224\,a(n-8) + 22736\,a(n-9) - 7980\,a(n-10) - 36288\,a(n-11) + 21456\,a(n-12) + 23040\,a(n-13) - 17280\,a(n-14)
\end{equation}
for all $n>14$, which is every $n$ at which a recurrence of order $14$ can be
stated.
...
For $n\ge14$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-14}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-14$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-14}\bigl(M^{14}-\sum_i c_i M^{14-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-14$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $14$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=14-14+1$ onwards, and the run was continued past
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235304.
