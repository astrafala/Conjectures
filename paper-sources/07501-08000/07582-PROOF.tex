...
\title{The empirical recurrence for OEIS A234985, proved by transfer matrix}
...
\begin{abstract}
OEIS A234985 counts $3$-column arrays over $\{0,\dots,5\}$ in which every
...
carries an empirical recurrence of order $28$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=150$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-28}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234985 is ``Number of (n+1) X (2+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
370, 1490, 6908, 35598, 189200, 1026784, 5628414,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 01:45:04, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;10\,a(n-1) + 9\,a(n-2) - 317\,a(n-3) + 216\,a(n-4) + 4392\,a(n-5) - 4481\,a(n-6) - 34461\,a(n-7) + 37067\,a(n-8) + 168161\,a(n-9) - 177688\,a(n-10) - 531245\,a(n-11) + 547375\,a(n-12) + 1099678\,a(n-13) - 1121930\,a(n-14) - 1478131\,a(n-15) + 1530027\,a(n-16) + 1252813\,a(n-17) - 1355278\,a(n-18) - 636426\,a(n-19) + 748324\,a(n-20) + 180286\,a(n-21) - 245436\,a(n-22) - 25096\,a(n-23) + 44796\,a(n-24) + 1272\,a(n-25) - 4080\,a(n-26) + 144\,a(n-28)
\end{equation}
for all $n>28$, which is every $n$ at which a recurrence of order $28$ can be
stated.
...
Let $V$ consist of those $r\in\{0,\dots,5\}^{3}$ with $r_j\ne r_{j+1}$ for every
$j$ --- the rows in which no two horizontally adjacent entries are equal, and put $S=|V|=150$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge28$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-28}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-28$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-28}\bigl(M^{28}-\sum_i c_i M^{28-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-28$ gives the equivalence. For the
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>28$. This is the statement of
Section~1.
\end{theorem}

\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $28$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=28-28+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>28$.
\end{proof}
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=150$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A234985.
