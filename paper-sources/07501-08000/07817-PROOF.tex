...
\title{The empirical recurrence for OEIS A234916, proved by transfer matrix}
...
\begin{abstract}
OEIS A234916 counts $4$-column arrays over $\{0,\dots,4\}$ in which every
...
carries an empirical recurrence of order $19$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=320$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-19}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234916 is ``Number of (n+1) X (3+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
376, 1128, 3684, 13336, 49056, 186500, 710304,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 14 2025 07:46:10, revision 10) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;9\,a(n-1) - 2\,a(n-2) - 188\,a(n-3) + 413\,a(n-4) + 1077\,a(n-5) - 3828\,a(n-6) - 1678\,a(n-7) + 14385\,a(n-8) - 3643\,a(n-9) - 26998\,a(n-10) + 16004\,a(n-11) + 26132\,a(n-12) - 21362\,a(n-13) - 12072\,a(n-14) + 12608\,a(n-15) + 2016\,a(n-16) - 3096\,a(n-17) + 224\,a(n-19)
\end{equation}
for all $n>19$, which is every $n$ at which a recurrence of order $19$ can be
stated.
...
Let $V$ consist of those $r\in\{0,\dots,4\}^{4}$ with $r_j\ne r_{j+1}$ for every
$j$ --- the rows in which no two horizontally adjacent entries are equal, and put $S=|V|=320$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge19$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-19}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-19$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-19}\bigl(M^{19}-\sum_i c_i M^{19-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-19$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $19$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=19-19+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>19$.
\end{proof}
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=320$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A234916.
