...
\title{The empirical recurrence for OEIS A303418, proved by transfer matrix}
...
\begin{abstract}
OEIS A303418 counts $5$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the six horizontal, diagonal and antidiagonal neighbours, a number $equal to$ itself lying in $\{0,1,2,3,4,5\}$, and it carries an empirical
recurrence of order $16$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A303418 is ``Number of nX5 0..1 arrays with every element equal to 0, 1, 2, 3, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
16, 512, 15647, 480953, 14783632, 454381369, 13965759339, 429248347970,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 23 2018 10:52:44, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;25\,a(n-1) + 145\,a(n-2) + 920\,a(n-3) + 1265\,a(n-4) + 1083\,a(n-5) - 13664\,a(n-6) - 14525\,a(n-7) - 1491\,a(n-8) + 33615\,a(n-9) + 17971\,a(n-10) - 5436\,a(n-11) - 22517\,a(n-12) - 1311\,a(n-13) + 444\,a(n-14) + 1917\,a(n-15) - 144\,a(n-16)
\end{equation}
for all $n>16$.
...
has score in $\{0,1,2,3,4,5\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,4,5\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,4,5\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,4,5\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,4,5\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-16}$ whenever $n-16\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-16$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-16}$ out of
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>16$. This is the statement of
Section~1.
\end{theorem}

\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $16$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=16+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303418.
