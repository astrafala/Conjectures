...
\title{The empirical recurrence for OEIS A251519, proved by transfer matrix}
...
\begin{abstract}
OEIS A251519 counts $4$-column arrays over $\{0,\dots,3\}$ in which every
$2\times2$ subblock sums to one of $\{2, 3, 4, 5, 6, 7, 8, 9, 10\}$, and carries an empirical
recurrence of order $15$ contributed by R.~H.~Hardin. It is true, and it is not an
...
OEIS A251519 is ``Number of (n+1)X(3+1) 0..3 arrays with every 2X2 subblock summing to 2 3 4 5 6 7 8 9 or 10.''. It has offset $1$
and begins
\[
58618, 13580282, 3143505700, 727704325306, 168458473890432, 38996983803988656, 9027534432491502252, 2089812347474112645290,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:27:14, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;225\,a(n-1) + 2020\,a(n-2) - 118805\,a(n-3) - 274529\,a(n-4) + 18217165\,a(n-5) - 41626758\,a(n-6) - 389697238\,a(n-7) + 754598477\,a(n-8) + 1991291650\,a(n-9) - 4108606440\,a(n-10) - 1995135464\,a(n-11) + 6354576576\,a(n-12) - 1281849696\,a(n-13) - 2223288576\,a(n-14) + 901255680\,a(n-15)
\end{equation}
for all $n>14$, which is every $n$ at which a recurrence of order $15$ can be
stated.
...
\begin{itemize}
\item $r_j+r_{j+1}+s_j+s_{j+1}\in\{2, 3, 4, 5, 6, 7, 8, 9, 10\}$;
...
Let $q(t)=t^{15}-\sum_{i}c_i\,t^{15-i}$ be the characteristic polynomial of
\eqref{eq:conj}, with the $c_i$ read off that equation.

\begin{lemma}\label{lem:crit}
For every $n\ge15$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n-15}\,q(M)\,\mathbf{1} .
\]
Hence \eqref{eq:conj} holds for every $n\ge15$ if and only if
...
=\mathbf{1}^{\!\top}M^{\,n-15}\Bigl(M^{15}-\sum_i c_i M^{15-i}\Bigr)\mathbf{1},
\]
which is the stated identity. The equivalence follows by letting $n-15=j$ run over
...
\begin{proof}
The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $15$ matrix--vector
...
$j=14-15+1$ onwards, and the run was continued past $S$ consecutive zeros, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251519.
