...
\title{The empirical recurrence for OEIS A235014, proved by transfer matrix}
...
\begin{abstract}
OEIS A235014 counts $6$-column arrays over $\{0,\dots,4\}$ in which every
...
carries an empirical recurrence of order $10$ contributed by R.~H.~Hardin. It is true,
...
the residual is $\mathbf{1}^{\!\top}M^{\,n-10}q(M)\mathbf{1}$, so the recurrence
...
OEIS A235014 is ``Number of (n+1) X (5+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
1736, 2568, 4008, 6856, 12168, 23016, 44680,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) + a(n-2) - 38*a(n-3) + 33*a(n-4) + 97*a(n-5) - 127*a(n-6) - 88*a(n-7) + 154*a(n-8) + 12*a(n-9) - 48*a(n-10).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 01:26:56, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) + a(n-2) - 38\,a(n-3) + 33\,a(n-4) + 97\,a(n-5) - 127\,a(n-6) - 88\,a(n-7) + 154\,a(n-8) + 12\,a(n-9) - 48\,a(n-10)
\end{equation}
for all $n>10$, which is every $n$ at which a recurrence of order $10$ can be
stated.
...
For $n\ge10$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-10}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-10$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-10}\bigl(M^{10}-\sum_i c_i M^{10-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-10$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $10$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=10-10+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>10$.
\end{proof}
...
the $24$ published indices; the values agree exactly. That is what ties the digraph to
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235014.
