...
\title{The empirical recurrence for OEIS A207509, proved by transfer matrix}
...
\begin{abstract}
OEIS A207509 counts arrays over $\{0,\dots,1\}$ of width $3$ that avoid the patterns
$(0,0,0), (0,1,1)$ along one axis and $(0,0,1), (1,1,1)$ along the other, and carries an empirical recurrence of
order $10$ contributed by R.~H.~Hardin. It is true. The patterns have length $3$, so
...
$\mathbf{1}^{\!\top}P^{\,n-2-10}q(P)\mathbf{1}$, which vanishes identically
...
OEIS A207509 is ``Number of n X 3 0..1 arrays avoiding 0 0 0 and 0 1 1 horizontally and 0 0 1 and 1 1 1 vertically.''. It has offset
$1$ and begins
\[
6, 36, 72, 166, 360, 660, 1292,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) + 2*a(n-3) - 5*a(n-4) + a(n-5) - 4*a(n-6) + 5*a(n-7) - a(n-8) + 2*a(n-9) - a(n-10) for n>11.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 24 2025 08:22:35, revision
12) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 2\,a(n-3) - 5\,a(n-4) + a(n-5) - 4\,a(n-6) + 5\,a(n-7) - a(n-8) + 2\,a(n-9) - a(n-10)
\end{equation}
for all $n>11$. 
...
$(0,0,0), (0,1,1)$ as $3$ consecutive entries. There are $6$ admissible rows; that condition
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-10}$ whenever $n-10\ge2$. Hence
\eqref{eq:conj} holds for every $n>t$ exactly when $u_j=0$ for every
$j>t-2-10$. Moreover $(u_j)$ satisfies the monic linear recurrence whose
...
$P^{\,n-2-10}$ gives the identity. The equivalence follows by letting the exponent
...
\begin{proof}
The vector $q(P)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $10$ matrix--vector
...
compared against the entry's DATA at every one of the $33$ published indices; the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207509.
