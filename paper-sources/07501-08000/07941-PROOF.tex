...
\title{The empirical recurrence for OEIS A303313, proved by transfer matrix}
...
\begin{abstract}
OEIS A303313 counts $7$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the six horizontal, diagonal and antidiagonal neighbours, a number $equal to$ itself lying in $\{0,1,4,5,6\}$, and it carries an empirical
recurrence of order $20$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A303313 is ``Number of nX7 0..1 arrays with every element equal to 0, 1, 4, 5 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
21, 23, 158, 439, 1799, 7096, 26184, 100128,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 21 2018 12:47:38, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + a(n-2) + 36\,a(n-3) + 23\,a(n-4) + 15\,a(n-5) - 252\,a(n-6) - 288\,a(n-7) - 81\,a(n-8) + 830\,a(n-9) + 496\,a(n-10) + 172\,a(n-11) - 720\,a(n-12) - 184\,a(n-13) - 396\,a(n-14) + 352\,a(n-15) - 248\,a(n-16) + 112\,a(n-17) - 176\,a(n-18) - 64\,a(n-20)
\end{equation}
for all $n>23$.
...
has score in $\{0,1,4,5,6\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,4,5,6\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,4,5,6\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,4,5,6\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,4,5,6\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-20}$ whenever $n-20\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-20$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-20}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $20$ matrix--vector
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303313.
