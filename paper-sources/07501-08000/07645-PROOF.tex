...
\title{The empirical recurrence for OEIS A250695, proved by transfer matrix}
...
\begin{abstract}
OEIS A250695 counts arrays over $\{0,1,2,3\}$ in which a statistic taken over short runs
...
OEIS A250695 is ``Number of (4+1)X(n+1) 0..3 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
11736, 76320, 383440, 1670016, 6671458, 25235016, 92169818, 329292956,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 12:42:24, revision 6) this is still
...
the difference of a cell and the cell above it. The entry writes the array with $n$ as the number of COLUMNS. The walk is run down the transpose, which exchanges the roles of the two directions once and for all; the arguments of each statistic keep their order under that exchange, the later cell staying the first argument.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250695.
