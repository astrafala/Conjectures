...
\title{The empirical recurrence for OEIS A196632, proved by transfer matrix}
...
\begin{abstract}
OEIS A196632 counts $n\times4$ arrays over $\{0,\dots,4\}$ subject to a condition imposed on
...
walk, and $a(n)$ is a walk count on $S=1010$ vertices, which is $C$-finite. Whether the
...
OEIS A196632 is ``Number of nX4 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 1,3,2,0,4 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
1, 10, 62, 327, 1927, 10708, 61040, 344949,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 06:38:05, revision 9) this is still
...
settled by the terminal weight, which evaluates it with $\varnothing$ below. Most of those pairs are indistinguishable, and exactly so. Every offset named above has $|dj|\le 1$, so whether the cell at column $j$ of the middle row is satisfied depends on the three rows only through their windows at $j-1,j,j+1$. Collect for each $j$ the indicator over the window of the row below, and call that tuple $P(r,s)$: the number of cells any $t$ violates is read off $P$, so is the number violated at the bottom edge, and the successor $(s,t)$ carries $P(s,t)$, which does not mention $r$. Hence two pairs $(r,s)$ and $(r',s)$ with the same $P$ have the same outgoing rows and the same successors, and the vertex is $(s,P)$. That leaves $S=1010$ vertices in place of $390625$. Hence,
with $M$ the adjacency matrix on $S=1010$ vertices, $\iota$ the starting vector and $\tau$ the
terminal weight,
\[
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-1) + 3\,a(n-2) + 11\,a(n-3) - 7\,a(n-4) - 152\,a(n-5) - 246\,a(n-6) + 567\,a(n-7) - 29\,a(n-8) + 56\,a(n-9) - 254\,a(n-10) + 605\,a(n-11) - 634\,a(n-12) - 182\,a(n-13) - 31\,a(n-14) - 76\,a(n-15) - 17\,a(n-16) + 49\,a(n-17) + 203\,a(n-18) - 49\,a(n-19) + 21\,a(n-20) - 17\,a(n-21) - 7\,a(n-22) - 2\,a(n-24)
\]
for every $n>24$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196632.
