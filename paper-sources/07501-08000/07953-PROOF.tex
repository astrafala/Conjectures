...
\title{The empirical recurrence for OEIS A251493, proved by transfer matrix}
...
\begin{abstract}
OEIS A251493 counts $3$-column arrays over $\{0,\dots,3\}$ in which every
$2\times2$ subblock sums to one of $\{4, 5, 6, 7, 8\}$, and carries an empirical
recurrence of order $12$ contributed by R.~H.~Hardin. It is true, and it is not an
...
OEIS A251493 is ``Number of (n+1)X(2+1) 0..3 arrays with every 2X2 subblock summing to 4 5 6 7 or 8.''. It has offset $1$
and begins
\[
2270, 86946, 3361296, 130169514, 5042543734, 195351371872, 7568125133206, 293198072124546,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:25:41, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;46\,a(n-1) - 243\,a(n-2) - 1796\,a(n-3) + 12368\,a(n-4) - 4170\,a(n-5) - 57120\,a(n-6) + 67165\,a(n-7) + 32054\,a(n-8) - 72328\,a(n-9) + 15648\,a(n-10) + 13056\,a(n-11) - 4608\,a(n-12)
\end{equation}
for all $n>11$, which is every $n$ at which a recurrence of order $12$ can be
stated.
...
\begin{itemize}
\item $r_j+r_{j+1}+s_j+s_{j+1}\in\{4, 5, 6, 7, 8\}$;
...
Let $q(t)=t^{12}-\sum_{i}c_i\,t^{12-i}$ be the characteristic polynomial of
\eqref{eq:conj}, with the $c_i$ read off that equation.

\begin{lemma}\label{lem:crit}
For every $n\ge12$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n-12}\,q(M)\,\mathbf{1} .
\]
Hence \eqref{eq:conj} holds for every $n\ge12$ if and only if
...
=\mathbf{1}^{\!\top}M^{\,n-12}\Bigl(M^{12}-\sum_i c_i M^{12-i}\Bigr)\mathbf{1},
\]
which is the stated identity. The equivalence follows by letting $n-12=j$ run over
...
\begin{proof}
The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $12$ matrix--vector
...
$j=11-12+1$ onwards, and the run was continued past $S$ consecutive zeros, which by
...
the $13$ published indices; the values agree exactly. That is what ties the digraph to
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251493.
