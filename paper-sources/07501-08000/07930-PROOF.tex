...
\title{The empirical recurrence for OEIS A229928, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A229928 counts arrays over $\{0,\dots,2\}$ in which every cell is constrained by the
...
with $S=81$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A229928 is ``Number of nX2 0..2 arrays x(i,j) with each element horizontally, vertically or antidiagonally next to at least one element with value 2-x(i,j).''. It has offset $1$ and begins
\[
3, 19, 115, 739, 4679, 29569, 187211, 1185283,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 08:39:39, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) - 3\,a(n-2) + 32\,a(n-3) - a(n-4) + 7\,a(n-5) + 43\,a(n-6) + 47\,a(n-7) + 7\,a(n-8) + 26\,a(n-9) - 13\,a(n-10) + 6\,a(n-11) - 5\,a(n-12) + a(n-13)
\end{equation}
for all $n>14$.
...
Let $V=\{0,\dots,2\}^{2}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=81$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
$3$ values, so there are about $10^{19}$ of them to sift. The
walk count replaces that by $81$ states and one matrix--vector product per row.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=81$ pairs and each
of the $(2+1)^{2}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229928.
