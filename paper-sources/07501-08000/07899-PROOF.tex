...
\title{The empirical product formula for OEIS A140902, proved}
...
\begin{abstract}
OEIS A140902 counts $4\times5$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $4\times5\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $20$. The entry's own expression, cleared of its denominator,
...
OEIS A140902 is ``Number of 4 X 5 matrices with elements in 0..n with each row and each column in nondecreasing order. 4,5,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 126, 5292, 116424, 1646568, 16818516,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 17 2025 09:28:00, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $4\times5$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$4\times5$ rectangle --- a plane partition in a $4\times5\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{4}\prod_{j=1}^{5}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $20$ with rational coefficients.
\end{theorem}
...
$4\cdot5=20$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,5,4)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $26$ and $6$ respectively.
\end{lemma}

\begin{proof}
With $s=n+5+4-1$ one has $s-(n+i)=5+4-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{5+4-1-i}
=\frac{(s-5-4+2+i)(s-5-4+3+i)\cdots s}{(5+4-1-i)!},
\]
a polynomial in $n$ of degree $5+4-1-i$. Multiplying over $i=0,\dots,4-1$ gives
$\deg N=\sum_{i=0}^{4-1}(5+4-1-i)=26$. The denominator is
...
of degree at most $20+6=26$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,26$, that is at $27$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $26$ has at most $26$ roots, so it is identically zero. Since $D(n)\neq0$
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,26$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140902.
