...
\title{The empirical recurrence for OEIS A316927, proved by transfer matrix}
...
\begin{abstract}
OEIS A316927 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,2,3,4,5,6,7\}$, and it carries an empirical
...
OEIS A316927 is ``Number of n X 3 0..1 arrays with every element unequal to 0, 2, 3, 4, 5, 6 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 8, 25, 143, 851, 5114, 31197, 191330,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 24 2021 14:47:51, revision
6) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;7\,a(n-1) - 3\,a(n-2) - 3\,a(n-3) - 64\,a(n-4) - 24\,a(n-5) + 77\,a(n-6) + 105\,a(n-7) + 88\,a(n-8) - 14\,a(n-9) - 40\,a(n-10) + 10\,a(n-11) + 18\,a(n-12) + 4\,a(n-13)
\end{equation}
for all $n>15$.
...
has score in $\{0,2,3,4,5,6,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,2,3,4,5,6,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,2,3,4,5,6,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,2,3,4,5,6,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,2,3,4,5,6,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A316927.
