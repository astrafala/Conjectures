...
\title{The empirical product formula for OEIS A140912, proved}
...
\begin{abstract}
OEIS A140912 counts $3\times8$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $3\times8\times n$ box, so
...
OEIS A140912 is ``Number of 3 X 8 matrices with elements in 0..n with each row and each column in nondecreasing order. 3,8,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 165, 9075, 259545, 4723719, 61408347,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2025 11:09:33, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $3\times8$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$3\times8$ rectangle --- a plane partition in a $3\times8\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{3}\prod_{j=1}^{8}\frac{n+i+j-1}{i+j-1},
\]
...
$3\cdot8=24$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,8,3)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $27$ and $3$ respectively.
\end{lemma}

\begin{proof}
With $s=n+8+3-1$ one has $s-(n+i)=8+3-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{8+3-1-i}
=\frac{(s-8-3+2+i)(s-8-3+3+i)\cdots s}{(8+3-1-i)!},
\]
a polynomial in $n$ of degree $8+3-1-i$. Multiplying over $i=0,\dots,3-1$ gives
$\deg N=\sum_{i=0}^{3-1}(8+3-1-i)=27$. The denominator is
...
of degree at most $24+3=27$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,27$, that is at $28$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $27$ has at most $27$ roots, so it is identically zero. Since $D(n)\neq0$
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,27$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140912.
