...
\title{The empirical recurrence for OEIS A252309, proved by transfer matrix}
...
\begin{abstract}
OEIS A252309 counts $6\times(n+2)$ arrays over $\{0,\dots,3\}$ subject to a condition imposed on
every $3\times3$ subblock, and it carries an empirical recurrence of order $9$
...
$a(n)$ is a walk count on $S=457929$ vertices. Such a sequence is $C$-finite, so whether a given
...
OEIS A252309 is ``Number of (4+2)X(n+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 1 3 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 1 3 6 or 7.''. It has offset $1$ and begins
\[
1763, 1516, 2151, 3032, 5230, 8130, 11639, 20350,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 7*a(n-3) -14*a(n-6) +8*a(n-9) for n>15
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:03:33, revision 6) this is still
...
\[
\text{every row, diagonal sum} \in \{0,1,3,6,7\}\ \text{ and }\ \text{every column, antidiagonal sum} \notin \{0,1,3,6,7\}
\]
...
\begin{theorem}
\[
a(n)\;=\;7\,a(n-3) - 14\,a(n-6) + 8\,a(n-9)
\]
for every $n>15$.
\end{theorem}

\begin{proof}
The vector $w=q(M)\mathbf{1}$ was formed by $9$ matrix--vector products in exact
...
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>15$. The bound is exact: at $n=15$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252309.
