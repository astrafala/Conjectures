...
\title{The empirical recurrence for OEIS A234445, proved by transfer matrix}
...
\begin{abstract}
OEIS A234445 counts $3$-column arrays over $\{0,\dots,4\}$ in which every
$2\times2$ subblock has its diagonal sum differing from its antidiagonal sum by
exactly $1$, and it
carries an empirical recurrence of order $17$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=125$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-17}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234445 is ``Number of (n+1) X (2+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
1120, 11574, 125080, 1377422, 15269704, 170001402, 1894235272,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 23:49:29, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;16\,a(n-1) + 122\,a(n-2) - 2854\,a(n-3) + 3035\,a(n-4) + 114162\,a(n-5) - 324660\,a(n-6) - 1581792\,a(n-7) + 6105836\,a(n-8) + 6998392\,a(n-9) - 41646592\,a(n-10) + 4188416\,a(n-11) + 100180352\,a(n-12) - 55355328\,a(n-13) - 54337536\,a(n-14) + 19902720\,a(n-15) + 7110912\,a(n-16) - 1815552\,a(n-17)
\end{equation}
for all $n>17$, which is every $n$ at which a recurrence of order $17$ can be
stated.
...
Let $V$ consist of all of $\{0,\dots,4\}^{3}$, and put $S=|V|=125$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge17$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-17}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-17$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-17}\bigl(M^{17}-\sum_i c_i M^{17-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-17$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $17$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=17-17+1$ onwards, and the run was continued past
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=125$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A234445.
