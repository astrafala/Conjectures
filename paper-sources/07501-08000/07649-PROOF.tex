...
\title{The empirical recurrence for OEIS A234915, proved by transfer matrix}
...
\begin{abstract}
OEIS A234915 counts $3$-column arrays over $\{0,\dots,4\}$ in which every
...
carries an empirical recurrence of order $13$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=80$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-13}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234915 is ``Number of (n+1) X (2+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
120, 360, 1128, 3912, 13368, 47980, 169128,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 15:37:17, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 20\,a(n-2) - 60\,a(n-3) - 110\,a(n-4) + 376\,a(n-5) + 70\,a(n-6) - 758\,a(n-7) + 309\,a(n-8) + 455\,a(n-9) - 336\,a(n-10) - 16\,a(n-11) + 60\,a(n-12) - 12\,a(n-13)
\end{equation}
for all $n>13$, which is every $n$ at which a recurrence of order $13$ can be
stated.
...
Let $V$ consist of those $r\in\{0,\dots,4\}^{3}$ with $r_j\ne r_{j+1}$ for every
$j$ --- the rows in which no two horizontally adjacent entries are equal, and put $S=|V|=80$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge13$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-13}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-13$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-13}\bigl(M^{13}-\sum_i c_i M^{13-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-13$ gives the equivalence. For the
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>13$. This is the statement of
Section~1.
\end{theorem}

\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $13$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=13-13+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>13$.
\end{proof}
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=80$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A234915.
