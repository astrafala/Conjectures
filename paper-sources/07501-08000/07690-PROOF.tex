...
\title{The empirical recurrence for OEIS A302166, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A302166 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $22$ contributed
...
OEIS A302166 is ``Number of 4Xn 0..1 arrays with every element equal to 0, 1 or 3 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
8, 10, 17, 29, 51, 109, 206, 407,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 02 2018 14:51:31, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + a(n-2) + 8\,a(n-4) - 4\,a(n-5) - 4\,a(n-6) - a(n-7) - 22\,a(n-8) + a(n-9) + 5\,a(n-10) + 2\,a(n-11) + 28\,a(n-12) + 6\,a(n-13) - 3\,a(n-14) - 3\,a(n-15) - 17\,a(n-16) - 4\,a(n-17) + 2\,a(n-18) + 2\,a(n-19) + 4\,a(n-20) - a(n-22)
\end{equation}
for all $n>23$.
...
publishes, $n=32$, the arrays have $4\cdot32$ cells over
$2$ values, so there are about $10^{38}$ of them to sift. The
walk count replaces that by $256$ states and one matrix--vector product per column.

The threshold below is exact rather than merely sufficient: at $n=23$ the conjectured
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-22}$ out of
...
the middle column. The vector $w$ was formed by $22$ applications of $G$ in exact integer
arithmetic and $\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly.
Every one of those integers vanishes from the index corresponding to $n=23+1$ onwards,
...
every one of the $32$ published indices, with no shift fitted: the number of columns is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302166.
