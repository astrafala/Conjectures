...
\title{The empirical recurrence for OEIS A303317, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A303317 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $12$ contributed
...
OEIS A303317 is ``Number of 5 X n 0..1 arrays with every element equal to 0, 1, 4, 5 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
16, 9, 19, 74, 139, 410, 1799, 5907,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 06 2026 20:19:01, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) - a(n-2) + 11\,a(n-3) + 21\,a(n-4) - 13\,a(n-5) + 5\,a(n-6) - 37\,a(n-7) - 34\,a(n-8) + 4\,a(n-9) - 20\,a(n-10) + 18\,a(n-11) - 7\,a(n-12)
\end{equation}
for all $n>16$.
...
The threshold below is exact rather than merely sufficient: at $n=16$ the conjectured
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-12}$ out of
...
the middle column. The vector $w$ was formed by $12$ applications of $G$ in exact integer
arithmetic and $\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly.
Every one of those integers vanishes from the index corresponding to $n=16+1$ onwards,
...
every one of the $26$ published indices, with no shift fitted: the number of columns is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303317.
