...
\title{The empirical column recurrences for the table OEIS A264299}
...
\begin{abstract}
OEIS A264299 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264299 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 0,1 0,-2 1,0 or -1,0.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 0, 5, 9, 1, 10, 8,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-2)\\
k=2: a(n) = a(n-1) +4*a(n-2) +5*a(n-3) -5*a(n-4) -5*a(n-5) +4*a(n-6) -a(n-7) -a(n-8)\\
k=3: [order 21]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 11 2015 07:40:02, revision 4) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 0,1 0,-2 1,0 or -1,0.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having directed index change 0,1 0,-2 1,0 or -1,0.''. Its rows are the vertices of a digraph on
...
$c_{1}$ & $5$ & $c_{7}$ & $-220$ & $c_{13}$ & $-610$ & $c_{19}$ & $-2$ \\
$c_{2}$ & $2$ & $c_{8}$ & $610$ & $c_{14}$ & $220$ & $c_{20}$ & $-5$ \\
$c_{3}$ & $-34$ & $c_{9}$ & $-142$ & $c_{15}$ & $-60$ & $c_{21}$ & $1$ \\
$c_{4}$ & $79$ & $c_{10}$ & $-1276$ & $c_{16}$ & $103$ &  &  \\
$c_{5}$ & $-103$ & $c_{11}$ & $1276$ & $c_{17}$ & $-79$ &  &  \\
$c_{6}$ & $60$ & $c_{12}$ & $142$ & $c_{18}$ & $34$ &  &  \\
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264299.
