...
\title{The empirical column recurrences for the table OEIS A236997}
...
\begin{abstract}
OEIS A236997 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A236997 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with the maximum plus the lower median minus the upper median minus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 360, 360, 1600, 2376, 1600, 7200, 15552,\ \dots
\]
...
k=7: [order 15]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 09:44:25, revision 6) these are still
...
column $k$$=1$ & $4$ & $81$ & $>4$ & $8$ \\
column $k$$=2$ & $4$ & $360$ & $>4$ & $7$ \\
column $k$$=3$ & $5$ & $1600$ & $>5$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with the maximum plus the lower median minus the upper median minus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays with the maximum plus the lower median minus the upper median minus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
$S=360$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;1944\,a(n-4)
...
The sequence is ``Number of (n+1)X(4) 0..2 arrays with the maximum plus the lower median minus the upper median minus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
$S=1600$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $25$ published values, $161$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A236997.
