...
\title{The empirical recurrence for OEIS A231025, proved by transfer matrix}
...
\begin{abstract}
OEIS A231025 counts arrays of width $2$ over $\{0,1,2\}$ in which every cell must see, among its
...
the count is a walk count on $S=82$ states.
\end{abstract}
...
OEIS A231025 is ``Number of n X 2 0..2 arrays x(i,j) with each element horizontally, vertically or antidiagonally next to at least one element with value (x(i,j)+1) mod 3 and at least one element with value (x(i,j)-1) mod 3, and upper left element zero.''. It has offset $1$ and begins
\[
0, 2, 12, 62, 344, 1864, 10162, 55324,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) + 8*a(n-2) - 4*a(n-4) + a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 25 2018 14:51:15, revision 8) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231025.
