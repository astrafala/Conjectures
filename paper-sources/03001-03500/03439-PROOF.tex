...
\title{The empirical column recurrences for the table OEIS A264341}
...
\begin{abstract}
OEIS A264341 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264341 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having index change +-(.,.) 0,0 0,1 or 1,2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
4, 13, 8, 49, 55, 16, 181, 490,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 11 2015 19:43:33, revision 4) these are still
...
column $k$$=1$ & $1$ & $16$ & $>-1$ & $8$ \\
column $k$$=2$ & $2$ & $64$ & $>0$ & $7$ \\
column $k$$=3$ & $1$ & $256$ & $>1$ & $6$ \\
column $k$$=4$ & $2$ & $1024$ & $>1$ & $5$ \\
row $n$$=1$ & $4$ & $256$ & $>2$ & $9$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having index change +-(.,.) 0,0 0,1 or 1,2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having index change +-(.,.) 0,0 0,1 or 1,2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(4) arrays of permutations of 0..(n+1)*(4)-1 with each element having index change +-(.,.) 0,0 0,1 or 1,2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(5) arrays of permutations of 0..(n+1)*(5)-1 with each element having index change +-(.,.) 0,0 0,1 or 1,2.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having index change +-(.,.) 0,0 0,1 or 1,2.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $35$ published values, $125$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264341.
