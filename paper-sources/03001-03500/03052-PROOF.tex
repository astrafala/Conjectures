...
\title{The empirical column recurrences for the table OEIS A232908}
...
\begin{abstract}
OEIS A232908 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A232908 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 10, 4, 52, 34, 8, 82, 320,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 2*a(n-1) -a(n-2) +4*a(n-3)\\
k=2: a(n) = 4*a(n-1) -a(n-2) +26*a(n-3) +16*a(n-4)\\
k=3: [order 8]\\
k=4: [order 18]\\
k=5: [order 34] for n>35\\
k=6: [order 75] for n>77\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 07:42:44, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally.''. Its rows are the vertices of a digraph on
$S=756$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-1) - a(n-2) + 26\,a(n-3) + 16\,a(n-4)
\]
...
model reproduces $15$ published values, $41$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232908.
