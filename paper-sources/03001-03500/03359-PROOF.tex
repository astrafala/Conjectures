...
\title{The empirical recurrence for OEIS A198717, proved by counting patterns}
...
\begin{abstract}
OEIS A198717 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $140$ to $S=13$. The entry's empirical recurrence of order
$5$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A198717 is ``Number of n X 2 0..6 arrays with values 0..6 introduced in row major order and no element equal to any horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
1, 4, 34, 499, 10507, 272410, 7817980, 234638905,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 55*a(n-1) - 918*a(n-2) + 5818*a(n-3) - 13417*a(n-4) + 8463*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 22 2018 16:29:15, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;55\,a(n-1) - 918\,a(n-2) + 5818\,a(n-3) - 13417\,a(n-4) + 8463\,a(n-5)
\end{equation}
for all $n>5$.

\section{What the canonical-form clause counts}

Fix the shape: $2$ columns and $L=n+0$ rows, so $L\cdot2$ cells. Call a
...
Fix $i$ and let $V_i=\{0,\dots,i-1\}^{2}$ be the possible rows. For $r,s\in V_i$
...
Assembling the $7$ lumped chains into one block-diagonal matrix $N$ of size $S=13$ and
...
\end{equation}
The unlumped alphabet had $140$ rows in all; the lumped chain has $S=13$ states.
...
For every $n$ with $m=n+0-1\ge5$,
\[
7!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-5}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-5}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $5$ matrix--vector products
...
corresponding to $n=5+1$ onwards, and the run continued until $w$ was the zero vector,
...
\eqref{eq:walk}, divided by $7!$, was evaluated at every one of the $17$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198717.
