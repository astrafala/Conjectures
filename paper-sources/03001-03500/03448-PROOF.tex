...
\title{The empirical column recurrences for the table OEIS A269152}
...
\begin{abstract}
OEIS A269152 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A269152 is ``T(n,k) = Number of n X k 0..3 arrays with some element plus some horizontally, antidiagonally or vertically adjacent neighbor totalling three exactly once.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 4, 4, 24, 80, 24, 108, 768,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 20 2024 17:47:42, revision 6) these are still
...
The sequence is ``Number of n X 1 0..3 arrays with some element plus some horizontally, antidiagonally or vertically adjacent neighbor totalling three exactly once.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 2 0..3 arrays with some element plus some horizontally, antidiagonally or vertically adjacent neighbor totalling three exactly once.''. Its rows are the vertices of a digraph on
...
model reproduces $15$ published values, $51$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269152.
