...
\title{The empirical recurrence for OEIS A262918, proved by transfer matrix}
...
\begin{abstract}
OEIS A262918 counts arrays of width $5$ over $\{0,1\}$ whose rows, read as base-$2$
numbers, are divisible by $7$ and whose columns, read the same way, are divisible by
$3$. The entry carries an empirical recurrence of order $8$ contributed by
...
OEIS A262918 is ``Number of (4+1) X (n+1) 0..1 arrays with each row divisible by 3 and each column divisible by 7, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
5, 9, 53, 137, 853, 2953, 17333, 71721,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 7*a(n-1) + 11*a(n-2) - 147*a(n-3) + 99*a(n-4) + 777*a(n-5) - 1019*a(n-6) - 637*a(n-7) + 910*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 01 2019 08:43:55, revision 8) this is still
...
$7$ and every $\gamma_j$ be divisible by $3$. The entry writes the array with $n$ as the number of COLUMNS. The walk is run down the transpose, which exchanges the row condition with the column condition; both readings put the FIRST digit in the most significant place, so nothing else changes under that exchange.
...
\begin{theorem}
\[
a(n)\;=\;7\,a(n-1) + 11\,a(n-2) - 147\,a(n-3) + 99\,a(n-4) + 777\,a(n-5) - 1019\,a(n-6) - 637\,a(n-7) + 910\,a(n-8)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262918.
