...
\title{The empirical recurrence for OEIS A208256, proved by counting patterns}
...
\begin{abstract}
OEIS A208256 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
$17$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A208256 is ``Number of nX6 0..2 arrays with new values 0..2 introduced in row major order and no element equal to more than one of its immediate leftward or upward neighbors.''. It has offset $1$ and begins
\[
122, 49152, 19566912, 7782199824, 3094987890864, 1230875651548608, 489518751475118880, 194681408759751734640,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 14:43:29, revision 9) the statement is
...
For every $n$ with $m=n+0-1\ge17$,
\[
3!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-17}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-17}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $17$ matrix--vector products
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208256.
