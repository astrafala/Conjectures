...
\title{The empirical recurrence for OEIS A198648, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A198648 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
lumpable, cutting $140$ pairs down to $S=13$ states. The entry's empirical recurrence of
order $11$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A198648 is ``Number of n X 1 0..6 arrays with values 0..6 introduced in row major order and each element equal to at least one horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 15 2018 09:00:57, revision 11) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) + a(n-2) - 60\,a(n-3) + 50\,a(n-4) + 226\,a(n-5) - 231\,a(n-6) - 416\,a(n-7) + 361\,a(n-8) + 388\,a(n-9) - 180\,a(n-10) - 144\,a(n-11)
\end{equation}
for all $n>12$.
...
Assembling the $7$ lumped chains block-diagonally into $N$ of size $S=13$, with the
...
index,
\[
7!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-11}\,
\Bigl(h_{11}-\sum_i c_i h_{11-i}\Bigr)
\]
...
$w=h_{11}-\sum_i c_ih_{11-i}$; and it is enough to check $m=0,1,\dots,S-1$.
\end{lemma}
...
index corresponding to $n=12+1$ onwards, and the run was continued until $S+1$
...
\eqref{eq:walk2}, divided by $7!$, was evaluated at every one of the $32$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198648.
