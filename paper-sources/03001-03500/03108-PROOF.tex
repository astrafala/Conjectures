...
\title{The empirical column recurrences for the table OEIS A264583}
...
\begin{abstract}
OEIS A264583 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 2, 4$.
\end{abstract}
...
OEIS A264583 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 1,0 0,-1 1,2 or -1,1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 1, 1, 0, 2, 0, 1, 4,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 18 2015 20:34:40, revision 4) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 1,0 0,-1 1,2 or -1,1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having directed index change 1,0 0,-1 1,2 or -1,1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(5) arrays of permutations of 0..(n+1)*(5)-1 with each element having directed index change 1,0 0,-1 1,2 or -1,1.''. Its rows are the vertices of a digraph on
$S=1024$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-1) + 36\,a(n-3) + 9\,a(n-4) - 243\,a(n-6)
\]
...
$c_{1}$ & $0$ & $c_{5}$ & $-4$ & $c_{9}$ & $25$ & $c_{13}$ & $0$ \\
$c_{2}$ & $2$ & $c_{6}$ & $-33$ & $c_{10}$ & $-8$ & $c_{14}$ & $-2$ \\
$c_{3}$ & $10$ & $c_{7}$ & $4$ & $c_{11}$ & $4$ & $c_{15}$ & $1$ \\
$c_{4}$ & $4$ & $c_{8}$ & $0$ & $c_{12}$ & $-10$ &  &  \\
...
model reproduces $29$ published values, $62$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264583.
