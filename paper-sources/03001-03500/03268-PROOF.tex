...
\title{The empirical recurrence for OEIS A223500, proved by transfer matrix}
...
\begin{abstract}
OEIS A223500 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$4$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is graph named in the entry. The entry carries an empirical recurrence of order $7$
...
single slice is a state and the count is a walk count on a finite digraph with $S=163$ vertices.
\end{abstract}
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=163.
\]
...
\begin{theorem}
\[
a(n)\;=\;31\,a(n-1) - 127\,a(n-2) - 20\,a(n-3) + 705\,a(n-4) - 1027\,a(n-5) + 499\,a(n-6) - 60\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223500.
