...
\title{The empirical column recurrences for the table OEIS A263799}
...
\begin{abstract}
OEIS A263799 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A263799 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits, and rows and columns lexicographically nonincreasing.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 2, 2, 3, 2, 3, 3, 3,\ \dots
\]
...
k=6: [order 83]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 15:39:01, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits, and rows and columns lexicographically nonincreasing.''. Its rows are the vertices of a digraph on
$S=6$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+1)X3 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits, and rows and columns lexicographically nonincreasing.''. Its rows are the vertices of a digraph on
$S=7$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + a(n-2) - a(n-3)
...
The sequence is ``Number of (n+1)X4 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits, and rows and columns lexicographically nonincreasing.''. Its rows are the vertices of a digraph on
$S=26$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $36$ published values, $43$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263799.
