...
\title{The empirical recurrence for OEIS A199649, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A199649 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
order $4$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A199649 is ``Number of n X 2 0..2 arrays with values 0..2 introduced in row major order and each element equal to no more than two horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
2, 14, 113, 953, 8037, 67774, 571530, 4819638,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 8*a(n-1) + 4*a(n-2) - 2*a(n-3) - 8*a(n-4) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 02 2018 11:27:16, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;8\,a(n-1) + 4\,a(n-2) - 2\,a(n-3) - 8\,a(n-4)
\end{equation}
for all $n>6$.
...
index,
\[
3!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-4}\,
\Bigl(h_{4}-\sum_i c_i h_{4-i}\Bigr)
\]
...
$w=h_{4}-\sum_i c_ih_{4-i}$; and it is enough to check $m=0,1,\dots,S-1$.
\end{lemma}
...
index corresponding to $n=6+1$ onwards, and the run was continued until $S+1$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A199649.
