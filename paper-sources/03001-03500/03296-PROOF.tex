...
\title{The conjectured generating function for OEIS A207449, proved}
...
\begin{abstract}
OEIS A207449 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A207449 is ``Number of n X 4 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 1 0 1 vertically.''. It has offset $1$ and begins
\[
10, 100, 330, 760, 1450, 2460,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: 10*x*(1 + 6*x - x\^{}2) / (1 - x)\^{}4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 22 2018 15:10:15, revision 10) this is still
...
\]
a polynomial of degree at most $114$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{114}$ then $R$ has a zero of order greater than its own
...
of them --- more than the $114+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207449.
