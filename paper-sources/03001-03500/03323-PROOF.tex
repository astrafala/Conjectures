...
\title{The empirical recurrence for OEIS A262473, proved by transfer matrix}
...
\begin{abstract}
OEIS A262473 counts arrays of width $4$ over $\{0,1\}$ whose rows, read as base-$2$
numbers, are divisible by $5$ and whose columns, read the same way, are divisible by
$3$. The entry carries an empirical recurrence of order $5$ contributed by
...
is then a walk count on $S=9$ states.
\end{abstract}
...
OEIS A262473 is ``Number of (3+1) X (n+1) 0..1 arrays with each row divisible by 3 and each column divisible by 5, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
4, 9, 36, 121, 484, 1849, 7396, 29241,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) + 5*a(n-2) - 20*a(n-3) - 4*a(n-4) + 16*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 31 2018 20:23:43, revision 7) this is still
...
So the vector $\bigl(\gamma_1\bmod 3,\dots,\gamma_{4}\bmod
3\bigr)$ is a state: at most $3^{4}$ of them, and each array row
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262473.
