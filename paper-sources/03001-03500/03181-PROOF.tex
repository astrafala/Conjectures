...
\title{The empirical column recurrences for the table OEIS A224550}
...
\begin{abstract}
OEIS A224550 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A224550 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 matrices with each 2X2 subblock idempotent.''. It has offset $1$, is
read by antidiagonals, and begins
\[
8, 16, 16, 28, 32, 28, 48, 52,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:27:31, revision 8) these are still
...
The sequence is ``Number of (n+1)X2 0..1 matrices with each 2X2 subblock idempotent.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..1 matrices with each 2X2 subblock idempotent.''. Its rows are the vertices of a digraph on
...
model reproduces $21$ published values, $55$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224550.
