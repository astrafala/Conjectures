...
\title{The empirical recurrence for OEIS A268909, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A268909 counts arrays over $\{0,\dots,2\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=1458$ vertices,
and the empirical recurrence of order $18$ contributed by R.~H.~Hardin then follows from
...
OEIS A268909 is ``Number of 6Xn 0..2 arrays with some element plus some horizontally or antidiagonally adjacent neighbor totalling two exactly once.''. It has offset $1$ and begins
\[
0, 33792, 715392, 14644152, 282550680, 5344944120, 99308573208, 1821165633864,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 15 2016 15:03:39, revision 4) the statement is
...
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,2\}^{6},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=1458,
\]
...
the digraph dense, with $(2+1)^{2\cdot6}$ edges instead of the sparse set used here.
\end{remark}
...
\]
for $j\ge18$. Hence \eqref{eq:conj} holds from that point on if and only if
...
formed by $18$ applications of $G$ in exact integer arithmetic and
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268909.
