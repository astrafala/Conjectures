...
\title{The empirical column recurrences for the table OEIS A235679}
...
\begin{abstract}
OEIS A235679 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A235679 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with the difference between each 2X2 subblock maximum and minimum lexicographically nondecreasing columnwise and nonincreasing rowwise.''. It has offset $1$, is
read by antidiagonals, and begins
\[
16, 58, 58, 208, 380, 208, 742, 2456,\ \dots
\]
...
k=6: [order 87]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:44:24, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with the difference between each 2X2 subblock maximum and minimum lexicographically nondecreasing columnwise and nonincreasing rowwise.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..1 arrays with the difference between each 2X2 subblock maximum and minimum lexicographically nondecreasing columnwise and nonincreasing rowwise.''. Its rows are the vertices of a digraph on
$S=25$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;8\,a(n-1) - 9\,a(n-2) - 9\,a(n-3) + 10\,a(n-4) + 3\,a(n-5) - 2\,a(n-6)
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235679.
