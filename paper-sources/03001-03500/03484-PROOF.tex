...
\title{The empirical column recurrences for the table OEIS A234266}
...
\begin{abstract}
OEIS A234266 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234266 is ``T(n,k) is the number of (n+1) X (k+1) 0..2 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
20, 46, 46, 104, 88, 104, 244, 170,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 20:52:38, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..2 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234266.
