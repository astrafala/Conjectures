...
\title{The empirical recurrence for OEIS A224570, proved by transfer matrix}
...
\begin{abstract}
OEIS A224570 counts the $(n+5)\times6$ matrices over $\{0,\dots,1\}$ all of whose
$6\times6$ contiguous subblocks are idempotent, and carries an empirical recurrence of order
$19$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical.
...
makes $a(n)$ a walk count on $S=104865$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224570 is ``Number of (n+5)X6 0..1 matrices with each 6X6 subblock idempotent.''. It has offset $1$ and begins
\[
96608, 18044, 16696, 18868, 22096, 25769, 28708, 33705,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:28:16, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $6\times6$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $6\times6$ matrix a \emph{band} when every one of its $1$ contiguous
$6\times6$ subblocks is a window. What the entry counts is the matrices of $6$ columns
whose every $6$ consecutive rows form a band.

Take as states the $5\times6$ strips that occur as the top $5$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=104865$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{30}$ strips of $5$ rows and $6$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $6\times5$ entries, so a band is exactly a walk of $1$ windows in
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) - 2\,a(n-2) + 2\,a(n-3) - a(n-4) + 2\,a(n-6) - 3\,a(n-7) + 4\,a(n-8) - 4\,a(n-9) + 3\,a(n-10) - 2\,a(n-11) - 2\,a(n-14) + 2\,a(n-15) - 2\,a(n-16) + 2\,a(n-17) - a(n-18) + a(n-19)
\]
for every $n>30$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224570.
