...
\title{The empirical closed form for OEIS A240003, proved}
...
\begin{abstract}
OEIS A240003 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=804$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A240003 is ``Number of 4Xn 0..3 arrays with no element equal to one plus the sum of elements to its left or two plus the sum of the elements above it or one plus the sum of the elements diagonally to its northwest, modulo 4.''. It has offset $1$ and begins
\[
28, 256, 1372, 6527, 27415, 104291, 363859, 1173141,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 09:37:31, revision 6) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\begin{multline*}
f(n)\;=\;\frac{n^{19}}{30411275102208000} + \frac{n^{18}}{914624815104000} \\
+ \frac{397 n^{17}}{2134124568576000} + \frac{37 n^{16}}{62768369664000} \\
+ \frac{2567 n^{15}}{6276836966400} - \frac{77899 n^{14}}{8966909952000} \\
+ \frac{121042813 n^{13}}{188305108992000} - \frac{5389171 n^{12}}{258660864000} \\
+ \frac{469998043 n^{11}}{603542016000} - \frac{20022197893 n^{10}}{877879296000} \\
+ \frac{5869313250161 n^{9}}{9656672256000} - \frac{9505177279259 n^{8}}{689762304000} \\
+ \frac{12579369755410273 n^{7}}{47076277248000} - \frac{3647143231803217 n^{6}}{840647808000} \\
+ \frac{50430900400493621 n^{5}}{871782912000} - \frac{804068701944948239 n^{4}}{1307674368000} \\
+ \frac{64298607619642973 n^{3}}{12864852000} - \frac{25675604169133123 n^{2}}{882161280} \\
+ \frac{25119199779142691 n}{232792560} - 191027452.
\end{multline*}
...
walk, so with $M$ the adjacency matrix of that digraph on $S=804$ vertices, and $u,w$ the
...
$n=41,\dots,60$ and agree at each. A monic recurrence of order $20$
determines $a(m)$ from $a(m-1),\dots,a(m-20)$, and for every $m\ge61$ both
$m>40$ and $m-20\ge41$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge41$.
Below $n=41$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=41$ on. The remaining indices $n=21,\dots,40$ were
therefore checked one at a time, directly against the walk count in exact integer arithmetic,
and agree.
\end{proof}

The range is tight in the sense that was checked: at $n=20$ the closed form and the sequence differ, so no larger range is available.
...
at every published term from $n=21$ on, and agrees with the entry's own DATA at each.
...
$n=21$, far past where the proof already settles the matter.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A240003.
