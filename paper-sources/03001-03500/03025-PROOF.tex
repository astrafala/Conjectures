...
\title{The empirical column recurrences for the table OEIS A206206}
...
\begin{abstract}
OEIS A206206 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 5$.
\end{abstract}
...
OEIS A206206 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with every 2X3 or 3X2 subblock having exactly one clockwise edge increases.''. It has offset $1$, is
read by antidiagonals, and begins
\[
256, 480, 480, 1768, 1060, 1768, 7388, 3008,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:47:20, revision 9) these are still
...
column $k$$=1$ & $5$ & $256$ & $>6$ & $8$ \\
column $k$$=5$ & $3$ & $29724$ & $>15$ & $8$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X2 0..3 arrays with every 2X3 or 3X2 subblock having exactly one clockwise edge increases.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;3\,a(n-1) + 17\,a(n-3) - 5\,a(n-4) + 2\,a(n-5)
\]
holds from index $6+1$ on.

\subsection*{Column $k$$=5$}
The sequence is ``Number of (n+1)X6 0..3 arrays with every 2X3 or 3X2 subblock having exactly one clockwise edge increases.''. Its rows are the vertices of a digraph on
$S=29724$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;10\,a(n-3)
...
model reproduces $16$ published values, $83$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206206.
