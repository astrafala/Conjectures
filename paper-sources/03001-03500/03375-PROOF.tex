...
\title{The empirical column recurrences for the table OEIS A206267}
...
\begin{abstract}
OEIS A206267 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A206267 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with the number of rightwards and downwards edge increases in each 2X2 subblock equal to the number in all its horizontal and vertical neighbors.''. It has offset $1$, is
read by antidiagonals, and begins
\[
16, 24, 24, 34, 28, 34, 50, 44,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 4*a(n-1) -5*a(n-2) +a(n-3) +2*a(n-4) -a(n-5) for n>6\\
k=2: a(n) = 3*a(n-1) -2*a(n-2) -2*a(n-3) +3*a(n-4) -a(n-5) for n>6\\
k=3: a(n) = 4*a(n-1) -5*a(n-2) +5*a(n-4) -4*a(n-5) +a(n-6) for n>7\\
k=4: a(n) = 5*a(n-1) -9*a(n-2) +5*a(n-3) +5*a(n-4) -9*a(n-5) +5*a(n-6) -a(n-7) for n>8\\
k=5: a(n) = 6*a(n-1) -14*a(n-2) +14*a(n-3) -14*a(n-5) +14*a(n-6) -6*a(n-7) +a(n-8) for n>9\\
k=6: a(n) = 7*a(n-1) -20*a(n-2) +28*a(n-3) -14*a(n-4) -14*a(n-5) +28*a(n-6) -20*a(n-7) +7*a(n-8) -a(n-9) for n>10\\
k=7: a(n) = 8*a(n-1) -27*a(n-2) +48*a(n-3) -42*a(n-4) +42*a(n-6) -48*a(n-7) +27*a(n-8) -8*a(n-9) +a(n-10) for n>11\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:50:12, revision 9) these are still
...
column $k$$=1$ & $5$ & $16$ & $>6$ & $11$ \\
column $k$$=2$ & $5$ & $24$ & $>6$ & $10$ \\
column $k$$=3$ & $6$ & $34$ & $>7$ & $8$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X2 0..1 arrays with the number of rightwards and downwards edge increases in each 2X2 subblock equal to the number in all its horizontal and vertical neighbors.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..1 arrays with the number of rightwards and downwards edge increases in each 2X2 subblock equal to the number in all its horizontal and vertical neighbors.''. Its rows are the vertices of a digraph on
$S=24$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of (n+1)X4 0..1 arrays with the number of rightwards and downwards edge increases in each 2X2 subblock equal to the number in all its horizontal and vertical neighbors.''. Its rows are the vertices of a digraph on
$S=34$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $29$ published values, $74$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206267.
