...
\title{The empirical column recurrences for the table OEIS A239155}
...
\begin{abstract}
OEIS A239155 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A239155 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with no element equal to all horizontal neighbors or unequal to all vertical neighbors, and new values 0..3 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 1, 7, 2, 7, 24, 7,\ \dots
\]
...
k=3: [order 9]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:57:46, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..3 arrays with no element equal to all horizontal neighbors or unequal to all vertical neighbors, and new values 0..3 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=384$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-1) + 7\,a(n-2) - 26\,a(n-3) + 5\,a(n-4) + 14\,a(n-5)
\]
...
model reproduces $8$ published values, $18$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A239155.
