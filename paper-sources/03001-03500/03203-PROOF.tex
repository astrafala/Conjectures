...
\title{The empirical recurrence for OEIS A224575, proved by transfer matrix}
...
\begin{abstract}
OEIS A224575 counts the $(n+5)\times11$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=28223$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224575 is ``Number of (n+5) X 11 0..1 matrices with each 6 X 6 subblock idempotent.''. It has offset $1$ and begins
\[
25769, 7386, 7884, 8714, 9480, 10086, 10561, 12373,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) -3*a(n-2) +a(n-3) +2*a(n-6) -5*a(n-7) +4*a(n-8) -a(n-9) -a(n-12) +2*a(n-13) -a(n-14), for n>18.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 21 2014 01:15:32, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $6\times6$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $6\times11$ matrix a \emph{band} when every one of its $6$ contiguous
$6\times6$ subblocks is a window. What the entry counts is the matrices of $11$ columns
whose every $6$ consecutive rows form a band.

Take as states the $5\times11$ strips that occur as the top $5$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=28223$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{55}$ strips of $5$ rows and $11$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $6\times5$ entries, so a band is exactly a walk of $6$ windows in
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 3\,a(n-2) + a(n-3) + 2\,a(n-6) - 5\,a(n-7) + 4\,a(n-8) - a(n-9) - a(n-12) + 2\,a(n-13) - a(n-14)
\]
for every $n>18$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224575.
