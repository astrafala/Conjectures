...
\title{The empirical column recurrences for the table OEIS A231080}
...
\begin{abstract}
OEIS A231080 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the rows $n=2$.
\end{abstract}
...
OEIS A231080 is ``T(n,k) = number of n X k 0..3 arrays x(i,j) with each element horizontally, diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4 and at least one element with value (x(i,j)-1) mod 4, and upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 0, 0, 0, 2, 0, 0, 14,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=2: a(n) = 2*a(n-1) +6*a(n-2) -5*a(n-3)\\
k=3: [order 11]\\
k=4: [order 36]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 21 2025 21:57:06, revision 11) these are still
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of 2 X n 0..3 arrays x(i,j) with each element horizontally, diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4 and at least one element with value (x(i,j)-1) mod 4, and upper left element zero.''. Its rows are the vertices of a digraph on
$S=165$ vertices, edges being the admissible consecutive pairs, and the count is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231080.
