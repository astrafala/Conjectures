...
\title{The empirical column recurrences for the table OEIS A230935}
...
\begin{abstract}
OEIS A230935 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A230935 is ``T(n,k)=Number of black-square subarrays of (n+2)X(k+2) 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 2, 2, 8, 6, 8, 8, 16,\ \dots
\]
...
k=6: [order 71]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:07:24, revision 6) these are still
...
The sequence is ``Number of black-square subarrays of (n+2)X3 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. Its rows are the vertices of a digraph on
$S=58$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of black-square subarrays of (n+2)X4 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230935.
