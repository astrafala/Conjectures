...
\title{The empirical recurrence for OEIS A198710, proved by counting patterns}
...
\begin{abstract}
OEIS A198710 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $100$ to $S=15$. The entry's empirical recurrence of order
...
OEIS A198710 is ``Number of n X 3 0..3 arrays with values 0..3 introduced in row major order and no element equal to any horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
2, 25, 401, 6548, 107042, 1749965, 28609241, 467717288,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 19*a(n-1) - 45*a(n-2) + 27*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 02 2018 08:03:14, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;19\,a(n-1) - 45\,a(n-2) + 27\,a(n-3)
\end{equation}
for all $n>3$.
...
Assembling the $4$ lumped chains into one block-diagonal matrix $N$ of size $S=15$ and
...
\end{equation}
The unlumped alphabet had $100$ rows in all; the lumped chain has $S=15$ states.
...
\[
4!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-3}q(N)\mathbf{1} .
\]
...
\eqref{eq:walk}, divided by $4!$, was evaluated at every one of the $17$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198710.
