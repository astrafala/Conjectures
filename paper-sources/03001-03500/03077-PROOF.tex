...
\title{The empirical column recurrences for the table OEIS A252532}
...
\begin{abstract}
OEIS A252532 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=2, 3$ and the rows $n=2, 3$.
\end{abstract}
...
OEIS A252532 is ``T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 2 3 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 2 3 6 or 7.''. It has offset $1$, is
read by antidiagonals, and begins
\[
750, 730, 595, 621, 337, 460, 719, 341,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:20:06, revision 6) these are still
...
column $k$$=2$ & $6$ & $4515$ & $>10$ & $8$ \\
column $k$$=3$ & $6$ & $41161$ & $>8$ & $10$ \\
row $n$$=2$ & $6$ & $4474$ & $>10$ & $9$ \\
row $n$$=3$ & $6$ & $40954$ & $>8$ & $11$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X(4) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 2 3 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 2 3 6 or 7.''. Its rows are the vertices of a digraph on
$S=4515$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+2)X(5) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 2 3 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 2 3 6 or 7.''. Its rows are the vertices of a digraph on
$S=41161$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;6\,a(n-3) - 8\,a(n-6)
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of (4)X(n+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 2 3 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 2 3 6 or 7.''. Its rows are the vertices of a digraph on
$S=4474$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;6\,a(n-3) - 8\,a(n-6)
...
\subsection*{Row $n$$=3$}
The sequence is ``Number of (5)X(n+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 2 3 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 2 3 6 or 7.''. Its rows are the vertices of a digraph on
$S=40954$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;6\,a(n-3) - 8\,a(n-6)
...
model reproduces $38$ published values, $134$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252532.
