...
\title{The empirical column recurrences for the table OEIS A264244}
...
\begin{abstract}
OEIS A264244 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264244 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 0,0 1,1 0,-1 -1,1 or 0,-2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 8, 3, 21, 21, 5, 57, 95,\ \dots
\]
...
k=5: [order 48]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 11:50:01, revision 8) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 0,0 1,1 0,-1 -1,1 or 0,-2.''. Its rows are the vertices of a digraph on
...
a(n)\;=\;a(n-1) + a(n-2)
\]
holds from index $0+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having directed index change 0,0 1,1 0,-1 -1,1 or 0,-2.''. Its rows are the vertices of a digraph on
$S=64$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-1) + 6\,a(n-2) - a(n-3)
\]
holds from index $1+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+1)X(4) arrays of permutations of 0..(n+1)*(4)-1 with each element having directed index change 0,0 1,1 0,-1 -1,1 or 0,-2.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having directed index change 0,0 1,1 0,-1 -1,1 or 0,-2.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-1) + a(n-2) + 4\,a(n-3) - a(n-4) - a(n-6)
\]
holds from index $4+1$ on.
...
The entry gives only the order for this line: ``k=4: [order 18]''. Its model is a walk on
...
$c_{1}$ & $19$ & $c_{6}$ & $-29636$ & $c_{11}$ & $66048$ & $c_{16}$ & $246016$ \\
$c_{2}$ & $-42$ & $c_{7}$ & $37104$ & $c_{12}$ & $320$ & $c_{17}$ & $15360$ \\
$c_{3}$ & $-600$ & $c_{8}$ & $54432$ & $c_{13}$ & $115712$ & $c_{18}$ & $-81920$ \\
$c_{4}$ & $2768$ & $c_{9}$ & $-121808$ & $c_{14}$ & $-190720$ &  &  \\
$c_{5}$ & $2064$ & $c_{10}$ & $3440$ & $c_{15}$ & $-115456$ &  &  \\
...
model reproduces $33$ published values, $103$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264244.
