...
\title{The empirical column recurrences for the table OEIS A231441}
...
\begin{abstract}
OEIS A231441 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231441 is ``T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with no element unequal to a strict majority of its horizontal, vertical, diagonal and antidiagonal neighbors, with values 0..3 introduced in row major order.''. It has offset $1$, is
...
k=3: [order 24]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:30:10, revision 6) these are still
...
The sequence is ``Number of (n+2)X(3) 0..3 arrays with no element unequal to a strict majority of its horizontal, vertical, diagonal and antidiagonal neighbors, with values 0..3 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=40$ vertices, edges being the admissible consecutive pairs, and the count is
...
model reproduces $11$ published values, $23$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231441.
