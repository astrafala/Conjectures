...
\title{The empirical recurrence for OEIS A231050, proved by transfer matrix}
...
\begin{abstract}
OEIS A231050 counts arrays of width $2$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
the count is a walk count on $S=57$ states.
\end{abstract}
...
OEIS A231050 is ``Number of 2 X n 0..3 arrays x(i,j) with each element horizontally, diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4 and at least one element with value (x(i,j)-1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
0, 2, 14, 32, 100, 428, 1616, 5784,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 8*a(n-3) + 6*a(n-4) - 4*a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 26 2018 06:20:51, revision 7) this is still
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 8\,a(n-3) + 6\,a(n-4) - 4\,a(n-5)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231050.
