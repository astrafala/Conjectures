...
\title{The empirical column recurrences for the table OEIS A230708}
...
\begin{abstract}
OEIS A230708 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A230708 is ``T(n,k)=Number of (n+3)X(k+3) 0..2 white square subarrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 3, and upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 8, 8, 30, 28, 30, 102, 244,\ \dots
\]
...
k=4: [order 50]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 05:58:11, revision 6) these are still
...
The sequence is ``Number of (n+3)X4 0..2 white square subarrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 3, and upper left element zero.''. Its rows are the vertices of a digraph on
...
model reproduces $9$ published values, $28$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230708.
