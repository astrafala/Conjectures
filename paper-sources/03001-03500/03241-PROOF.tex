...
\title{The empirical column recurrences for the table OEIS A231463}
...
\begin{abstract}
OEIS A231463 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231463 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with no element unequal to a strict majority of its horizontal, diagonal and antidiagonal neighbors, with values 0..3 introduced in row major order.''. It has offset $1$, is
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:32:04, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..3 arrays with no element unequal to a strict majority of its horizontal, diagonal and antidiagonal neighbors, with values 0..3 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=80$ vertices, edges being the admissible consecutive pairs, and the count is
...
\subsection*{Row $n=1$, recovered}
The entry gives only the order for this line: ``n=1: [order 10]''. Its model is a walk on
$S=180$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=360$ exact terms returns the minimal one, of order
$10$ --- the stated order --- with integer coefficients
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $5$ & $c_{4}$ & $11$ & $c_{7}$ & $-176$ & $c_{10}$ & $-72$ \\
$c_{2}$ & $3$ & $c_{5}$ & $137$ & $c_{8}$ & $108$ &  &  \\
$c_{3}$ & $-41$ & $c_{6}$ & $-82$ & $c_{9}$ & $108$ &  &  \\
...
model reproduces $9$ published values, $18$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231463.
