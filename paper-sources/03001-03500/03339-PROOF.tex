...
\title{Arrays counted up to renaming: the empirical recurrence for OEIS A205356}
...
\begin{abstract}
OEIS A205356 carries an empirical recurrence of order $5$ contributed by R.~H.~Hardin. It is
...
both in the state the arrays are exactly the walks in a digraph on $S=11$ vertices, so $a(n)$
...
OEIS A205356 is ``Number of (n+1) X 4 0..2 arrays with every 2 X 2 subblock having the same number of equal diagonal or antidiagonal elements, and new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
194, 1562, 12866, 107330, 904514, 7690946, 65916482, 568956866,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 14:49:47, revision 9) this is still
...
What is compared \emph{across} windows is carried too: the state holds the score fixed by the first band, since every later subblock is compared with it. With $3$ subblocks to a band that is a bounded amount of bookkeeping and no more.
...
\begin{theorem}
\[
a(n)\;=\;23\,a(n-1) - 187\,a(n-2) + 645\,a(n-3) - 912\,a(n-4) + 432\,a(n-5)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205356.
