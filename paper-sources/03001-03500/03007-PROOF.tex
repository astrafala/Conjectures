...
\title{The empirical recurrence for OEIS A231241, proved by transfer matrix}
...
\begin{abstract}
OEIS A231241 counts arrays of width $3$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
the count is a walk count on $S=453$ states.
\end{abstract}
...
OEIS A231241 is ``Number of n X 3 0..3 arrays x(i,j) with each element horizontally, diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
0, 14, 78, 552, 3820, 26658, 185074, 1287036,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) +16*a(n-2) +26*a(n-3) +37*a(n-4) +6*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 09 2018 11:08:53, revision 7) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=453.
\]
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) + 16\,a(n-2) + 26\,a(n-3) + 37\,a(n-4) + 6\,a(n-5)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231241.
