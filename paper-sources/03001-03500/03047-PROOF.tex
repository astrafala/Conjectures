...
\title{The empirical column recurrences for the table OEIS A263464}
...
\begin{abstract}
OEIS A263464 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A263464 is ``T(n,k)=Number of (n+1)X(k+1) 0..4 arrays with each row and column divisible by 11, read as a base-5 number with top and left being the most significant digits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 3, 3, 8, 17, 8, 23, 173,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 3*a(n-1) +a(n-5) -3*a(n-6)\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 19 2015 09:46:05, revision 4) these are still
...
The sequence is ``Number of (n+1)X2 0..4 arrays with each row and column divisible by 11, read as a base-5 number with top and left being the most significant digits.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263464.
