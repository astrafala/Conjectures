...
\title{The empirical column recurrences for the table OEIS A234728}
...
\begin{abstract}
OEIS A234728 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234728 is ``T(n,k) is the number of (n+1) X (k+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 7 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
240, 1096, 1096, 4192, 3976, 4192, 19120, 12464,\ \dots
\]
...
k=1: a(n) = 68*a(n-2) -1724*a(n-4) +19312*a(n-6) -80640*a(n-8).\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 20:29:36, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 7 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234728.
