...
\title{The empirical recurrence for OEIS A239532, proved by transfer matrix}
...
\begin{abstract}
OEIS A239532 counts arrays of width $4$ over $\{0,\dots,2\}$ under a condition on each cell
and its neighbours, and carries an empirical recurrence of order $6$ contributed by
...
a walk count on $S=6916$ of them, and the conjectured recurrence is then settled by a finite
exact computation.
\end{abstract}
...
OEIS A239532 is ``Number of (n+1) X (3+1) 0..2 arrays with no element equal to all horizontal neighbors or unequal to all vertical neighbors, and new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
6, 6, 216, 1014, 13254, 98304, 984150, 8368566,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 8*a(n-1) + 26*a(n-2) - 166*a(n-3) + 96*a(n-4) + 198*a(n-5) - 81*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 26 2018 09:15:06, revision 7) this is still
...
(0,-1), (0,1), (-1,0), (1,0),
\]
those falling outside the array being absent. The entry requires that no cell equals all of its horizontal neighbours, and none differs from all of its vertical ones.
...
terms. There are $S=6916$ states, and $a$ is $C$-finite.

At $n=1$ the array has one row, and
the condition is tested there with nothing above or below it; the entry's first term is
$6$, which the model reproduces along with every later term. That is the cheapest check
...
first term. There are $3^{4}=81$ possible rows over the entry's own alphabet.
...
\begin{theorem}
\[
a(n)\;=\;8\,a(n-1) + 26\,a(n-2) - 166\,a(n-3) + 96\,a(n-4) + 198\,a(n-5) - 81\,a(n-6)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A239532.
