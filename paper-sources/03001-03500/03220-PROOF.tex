...
\title{The empirical recurrence for OEIS A186842, proved by transfer matrix}
...
\begin{abstract}
OEIS A186842 counts arrays over $\{0,\dots,1\}$ under a condition comparing a statistic of
...
empirical. The count is a walk count on $S=16$ states and is $C$-finite; whether the
...
OEIS A186842 is ``Number of (n+1) X 2 binary arrays with no 2 X 2 subblock diagonal sum less antidiagonal sum equal to any horizontal or vertical neighbor 2 X 2 subblock diagonal sum less antidiagonal sum.''. It has offset $1$ and begins
\[
16, 50, 162, 512, 1626, 5182, 16476, 52378,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + 3*a(n-2) + 10*a(n-3) + 8*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 19 2018 06:21:38, revision 8) this is still
...
2}=16$ tuples of $2$ rows, $S=16$ survive the condition inside a single subblock row;
...
$16$, which the model reproduces along with every later term, and that is the cheapest
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 3\,a(n-2) + 10\,a(n-3) + 8\,a(n-4)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A186842.
