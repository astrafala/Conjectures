...
\title{The empirical recurrence for OEIS A262753, proved by transfer matrix}
...
\begin{abstract}
OEIS A262753 counts arrays of width $4$ over $\{0,1\}$ whose rows, read as base-$2$
numbers, are divisible by $5$ and whose columns, read the same way, are divisible by
$7$. The entry carries an empirical recurrence of order $7$ contributed by
...
OEIS A262753 is ``Number of (n+2) X (2+2) 0..1 arrays with each row divisible by 5 and each column divisible by 7, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
4, 9, 25, 100, 361, 1369, 5476, 21609,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) + 9*a(n-3) - 36*a(n-4) - 8*a(n-6) + 32*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 31 2018 20:23:50, revision 7) this is still
...
$5$ and every $\gamma_j$ be divisible by $7$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262753.
