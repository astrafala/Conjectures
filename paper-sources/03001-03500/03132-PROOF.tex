...
\title{The empirical column recurrences for the table OEIS A299015}
...
\begin{abstract}
OEIS A299015 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A299015 is ``T(n,k) = Number of n X k 0..1 arrays with every element equal to 0, 1, 2, 4, 5 or 6 king-move adjacent elements, with upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 4, 7, 4, 8, 13,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 20 2024 14:10:38, revision 6) these are still
...
The sequence is ``Number of n X 1 0..1 arrays with every element equal to 0, 1, 2, 4, 5 or 6 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 2 0..1 arrays with every element equal to 0, 1, 2, 4, 5 or 6 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
model reproduces $20$ published values, $44$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A299015.
