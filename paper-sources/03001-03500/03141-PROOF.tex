...
\title{The empirical recurrence for OEIS A208312, proved by counting patterns}
...
\begin{abstract}
OEIS A208312 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $65$ to $S=33$. The entry's empirical recurrence of order
$16$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A208312 is ``Number of n X 6 0..1 arrays with new values 0..1 introduced in row major order and no element equal to more than one of its immediate leftward or upward neighbors.''. It has offset $1$ and begins
\[
32, 486, 6222, 75744, 906120, 10771434, 127750122, 1513877040,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 14:45:20, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;33\,a(n-1) - 465\,a(n-2) + 3933\,a(n-3) - 23001\,a(n-4) + 99921\,a(n-5) - 334593\,a(n-6) + 877689\,a(n-7) - 1807461\,a(n-8) + 2898261\,a(n-9) - 3560841\,a(n-10) + 3279285\,a(n-11) - 2205225\,a(n-12) + 1050489\,a(n-13) - 338985\,a(n-14) + 67797\,a(n-15) - 6561\,a(n-16)
\end{equation}
for all $n>16$.

\section{What the canonical-form clause counts}

Fix the shape: $6$ columns and $L=n+0$ rows, so $L\cdot6$ cells. Call a
...
Assembling the $2$ lumped chains into one block-diagonal matrix $N$ of size $S=33$ and
...
\end{equation}
The unlumped alphabet had $65$ rows in all; the lumped chain has $S=33$ states.
...
For every $n$ with $m=n+0-1\ge16$,
\[
2!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-16}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-16}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $16$ matrix--vector products
...
corresponding to $n=16+1$ onwards, and the run continued until $w$ was the zero vector,
...
\eqref{eq:walk}, divided by $2!$, was evaluated at every one of the $16$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208312.
