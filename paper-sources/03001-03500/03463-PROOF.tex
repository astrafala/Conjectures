...
\title{The empirical column recurrences for the table OEIS A253350}
...
\begin{abstract}
OEIS A253350 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 2$ and the rows $n=1, 2$.
\end{abstract}
...
OEIS A253350 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with every 2X2 subblock diagonal maximum minus antidiagonal maximum nondecreasing horizontally, vertically and ne-to-sw antidiagonally.''. It has offset $1$, is
read by antidiagonals, and begins
\[
16, 47, 47, 125, 173, 125, 335, 724,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 13:59:05, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..1 arrays with every 2X2 subblock diagonal maximum minus antidiagonal maximum nondecreasing horizontally, vertically and ne-to-sw antidiagonally.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..1 arrays with every 2X2 subblock diagonal maximum minus antidiagonal maximum nondecreasing horizontally, vertically and ne-to-sw antidiagonally.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) 0..1 arrays with every 2X2 subblock diagonal maximum minus antidiagonal maximum nondecreasing horizontally, vertically and ne-to-sw antidiagonally.''. Its rows are the vertices of a digraph on
$S=16$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of (3)X(n+1) 0..1 arrays with every 2X2 subblock diagonal maximum minus antidiagonal maximum nondecreasing horizontally, vertically and ne-to-sw antidiagonally.''. Its rows are the vertices of a digraph on
$S=47$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $32$ published values, $120$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253350.
