...
\title{The empirical column recurrences for the table OEIS A259297}
...
\begin{abstract}
OEIS A259297 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A259297 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with each 2X2 subblock having clockwise pattern 0001 0101 or 0111.''. It has offset $1$, is
read by antidiagonals, and begins
\[
10, 26, 26, 66, 94, 66, 170, 322,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1) +4*a(n-2)\\
k=2: a(n) = 3*a(n-1) +4*a(n-2) -8*a(n-3)\\
k=3: a(n) = 2*a(n-1) +18*a(n-2) -13*a(n-3) -70*a(n-4) +24*a(n-5) +64*a(n-6)\\
k=4: [order 9]\\
k=5: [order 20]\\
k=6: [order 36]\\
k=7: [order 72]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:50:42, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with each 2X2 subblock having clockwise pattern 0001 0101 or 0111.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..1 arrays with each 2X2 subblock having clockwise pattern 0001 0101 or 0111.''. Its rows are the vertices of a digraph on
$S=9$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1) + 4\,a(n-2) - 8\,a(n-3)
\]
holds from index $2+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+1)X4 0..1 arrays with each 2X2 subblock having clockwise pattern 0001 0101 or 0111.''. Its rows are the vertices of a digraph on
$S=17$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-1) + 18\,a(n-2) - 13\,a(n-3) - 70\,a(n-4) + 24\,a(n-5) + 64\,a(n-6)
\]
...
model reproduces $27$ published values, $113$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A259297.
