...
\title{The empirical column recurrences for the table OEIS A255159}
...
\begin{abstract}
OEIS A255159 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A255159 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 1 and no antidiagonal sum 1 and no row sum 0 or 1 and no column sum 0 or 1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
28, 62, 62, 141, 158, 141, 331, 459,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 2*a(n-1) -a(n-2) +3*a(n-3) +a(n-4) +2*a(n-5) +2*a(n-6)\\
k=2: [order 11]\\
k=3: [order 18] for n>20\\
k=4: [order 29] for n>31\\
k=5: [order 49] for n>54\\
k=6: [order 79] for n>86\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:40:12, revision 6) these are still
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with no 3x3 subblock diagonal sum 1 and no antidiagonal sum 1 and no row sum 0 or 1 and no column sum 0 or 1.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;2\,a(n-1) - a(n-2) + 3\,a(n-3) + a(n-4) + 2\,a(n-5) + 2\,a(n-6)
\]
...
$c_{1}$ & $1$ & $c_{4}$ & $4$ & $c_{7}$ & $0$ & $c_{10}$ & $-3$ \\
$c_{2}$ & $2$ & $c_{5}$ & $0$ & $c_{8}$ & $-6$ & $c_{11}$ & $-1$ \\
$c_{3}$ & $7$ & $c_{6}$ & $1$ & $c_{9}$ & $-6$ &  &  \\
...
model reproduces $9$ published values, $30$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A255159.
