...
\title{The empirical recurrence for OEIS A269054, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A269054 counts arrays over $\{0,\dots,2\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=54$ vertices,
and the empirical recurrence of order $6$ contributed by R.~H.~Hardin then follows from
...
OEIS A269054 is ``Number of 3 X n 0..2 arrays with some element plus some horizontally, diagonally or antidiagonally adjacent neighbor totalling two not more than once.''. It has offset $1$ and begins
\[
27, 174, 594, 3078, 13140, 58752, 253416, 1090344,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) - a(n-2) - 28*a(n-3) - 4*a(n-4) + 16*a(n-5) - 4*a(n-6) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 18 2019 15:06:53, revision 7) the statement is
...
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,2\}^{3},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=54,
\]
...
the digraph dense, with $(2+1)^{2\cdot3}$ edges instead of the sparse set used here.
\end{remark}
...
$o=1n_0+0-1$,
\[
a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\;=\;\,\mathbf{v}^{\!\top}G^{\,j-6}w,
\qquad w=h_{6}-\sum_i c_i h_{6-i},
\]
for $j\ge6$. Hence \eqref{eq:conj} holds from that point on if and only if
...
formed by $6$ applications of $G$ in exact integer arithmetic and
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269054.
