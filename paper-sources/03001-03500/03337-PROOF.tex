...
\title{The empirical recurrence for OEIS A214232, proved by transfer matrix}
...
\begin{abstract}
OEIS A214232 counts the proper colourings of a grid $n\times3$, wrapped in the direction of width $3$, with at most $8$ colours and
...
down to $S=6$ and makes the exact residual test affordable.
\end{abstract}
...
OEIS A214232 is ``Number of 0..7 colorings of an n X 3 array circular in the 3 direction with new values 0..7 introduced in row major order.''. It has offset $1$ and begins
\[
1, 18, 1067, 133265, 24424881, 5178830173, 1151035900897, 259584805701645,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 343*a(n-1) - 29965*a(n-2) + 856493*a(n-3) - 7269038*a(n-4) + 11346368*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 22 2018 08:43:55, revision 9) this is still
...
A row of the array is a proper colouring of the cycle $C_{3}$, since the wrapping makes
its first and last cells adjacent, and two consecutive rows must differ in every one of the
$3$ positions. So $L_i(n)$ is the number of walks of $n-1$ steps in the digraph whose
vertices are the proper colourings of $C_{3}$ over $i$ colours and whose edges join
colourings differing everywhere:
\[
L_i(n)\;=\;\iota^{\!\top}M_i^{\,n-1}\iota ,\qquad \iota=(1,\dots,1)^{\!\top} .
\]
There are $(i-1)^{3}+(-1)^{3}(i-1)$ such colourings, which at $i=8$ is already more
...
state count is $S=6$ rather than the number of colourings.
...
\begin{theorem}
\[
a(n)\;=\;343\,a(n-1) - 29965\,a(n-2) + 856493\,a(n-3) - 7269038\,a(n-4) + 11346368\,a(n-5)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A214232.
