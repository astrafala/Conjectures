...
\title{The empirical column recurrences for the table OEIS A264569}
...
\begin{abstract}
OEIS A264569 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 2, 4$ and the rows $n=1, 2$.
\end{abstract}
...
OEIS A264569 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 1,0 1,1 0,-1 or -1,1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 1, 1, 1, 2, 1, 2, 4,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 17 2015 19:12:29, revision 4) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 1,0 1,1 0,-1 or -1,1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having directed index change 1,0 1,1 0,-1 or -1,1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(5) arrays of permutations of 0..(n+1)*(5)-1 with each element having directed index change 1,0 1,1 0,-1 or -1,1.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having directed index change 1,0 1,1 0,-1 or -1,1.''. Its rows are the vertices of a digraph on
$S=16$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of (3)X(n+1) arrays of permutations of 0..(3)*(n+1)-1 with each element having directed index change 1,0 1,1 0,-1 or -1,1.''. Its rows are the vertices of a digraph on
$S=64$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
$c_{1}$ & $2$ & $c_{5}$ & $6$ & $c_{9}$ & $33$ & $c_{13}$ & $1$ \\
$c_{2}$ & $1$ & $c_{6}$ & $-27$ & $c_{10}$ & $-4$ & $c_{14}$ & $-1$ \\
$c_{3}$ & $6$ & $c_{7}$ & $5$ & $c_{11}$ & $7$ & $c_{15}$ & $1$ \\
$c_{4}$ & $-6$ & $c_{8}$ & $-9$ & $c_{12}$ & $-12$ &  &  \\
\end{tabular}\end{center}
Repeating with the first $15+4$ terms removed gives order $15$ again.

\subsection*{Row $n=3$, recovered}
The entry gives only the order for this line: ``n=3: [order 70]''. Its model is a walk on
$S=256$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=512$ exact terms returns the minimal one, of order
$70$ --- the stated order --- with integer coefficients
\[
(c_1,\dots,c_{70})=(3,\ 1,\ 4,\ 41,\ -104,\ 14,\ 167,\ -715,\ 1169,\ -1018,\ -3680,\ 8691,\ -10335,\ 2525,\ 35277,\ -50582,\ 64373,\ 11060,\ -207508,\ 200293,\ -204582,\ -124748,\ 704108,\ -499827,\ 365700,\ 473743,\ -1541579,\ 729138,\ -331526,\ -929717,\ 2313138,\ -591289,\ 10149,\ 1058138,\ -2448606,\ 225766,\ 341725,\ -740229,\ 1867654,\ -9921,\ -441602,\ 343946,\ -1047499,\ -9489,\ 322880,\ -125643,\ 431100,\ -8968,\ -160742,\ 44437,\ -124400,\ 6980,\ 54449,\ -13758,\ 23185,\ -1859,\ -11695,\ 2887,\ -2532,\ 298,\ 1457,\ -383,\ 155,\ -26,\ -100,\ 29,\ -4,\ 1,\ 3,\ -1).
\]
Repeating with the first $70+4$ terms removed gives order $70$ again.

\subsection*{Row $n=4$, recovered}
The entry gives only the order for this line: ``n=4: [order 56]''. Its model is a walk on
...
$56$ --- the stated order --- with integer coefficients
\[
(c_1,\dots,c_{56})=(0,\ 16,\ 82,\ 511,\ 534,\ -4064,\ -14458,\ -70728,\ -144898,\ 151878,\ 433218,\ 2890398,\ 10670008,\ -4868295,\ -20924360,\ -31737025,\ -310112176,\ -14252445,\ 1245030998,\ 763844484,\ 3489894872,\ 3560972611,\ -34707540848,\ -31672936556,\ 3843945156,\ -86344072802,\ 620453233878,\ 502105476999,\ -872784719202,\ 1071474978240,\ -8716412967258,\ -2718817163241,\ 18039607291386,\ -8425254174294,\ 99379421903268,\ -12543597486831,\ -218700026142678,\ 61107180304023,\ -847877206023450,\ 232550242508493,\ 1750974893762346,\ -425263024937781,\ 5052007175623272,\ -779003213283180,\ -9156884027421744,\ 1819843951836978,\ -19372704305240304,\ -3165916553282376,\ 28325088341449956,\ -2407523512142871,\ 41173142687273142,\ 24188395222378764,\ -42276312456767928,\ -9265100944259205,\ -44472484532444184,\ -50031545098999707).
\]
Repeating with the first $56+4$ terms removed gives order $56$ again.
...
model reproduces $50$ published values, $97$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264569.
