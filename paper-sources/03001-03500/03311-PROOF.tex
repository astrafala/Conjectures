...
\title{The empirical column recurrences for the table OEIS A233497}
...
\begin{abstract}
OEIS A233497 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A233497 is ``T(n,k)=Number of (n+2)X(k+2) 0..2 arrays with no increasing sequence of length 3 horizontally or antidiagonally downwards.''. It has offset $1$, is
read by antidiagonals, and begins
\[
16928, 388908, 424009, 8946936, 26851581, 10619963, 205862149, 1709348346,\ \dots
\]
...
k=3: [order 7]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:04:25, revision 6) these are still
...
The sequence is ``Number of (n+2)X3 0..2 arrays with no increasing sequence of length 3 horizontally or antidiagonally downwards.''. Its rows are the vertices of a digraph on
$S=703$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;25\,a(n-1) + 2\,a(n-2) - 21\,a(n-3) + 3\,a(n-4)
\]
...
model reproduces $6$ published values, $50$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233497.
