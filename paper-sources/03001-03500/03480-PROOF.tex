...
\title{The empirical column recurrences for the table OEIS A264506}
...
\begin{abstract}
OEIS A264506 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 2, 4$ and the rows $n=1$.
\end{abstract}
...
OEIS A264506 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change -1,1 0,-1 0,1 or 1,0.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 1, 2, 3, 2, 3, 6, 14,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 15 2015 18:38:06, revision 4) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change -1,1 0,-1 0,1 or 1,0.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having directed index change -1,1 0,-1 0,1 or 1,0.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(5) arrays of permutations of 0..(n+1)*(5)-1 with each element having directed index change -1,1 0,-1 0,1 or 1,0.''. Its rows are the vertices of a digraph on
$S=1024$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;6\,a(n-1) - 5\,a(n-2) + 36\,a(n-3) - 72\,a(n-4) - 243\,a(n-6)
\]
holds from index $7+1$ on.

\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having directed index change -1,1 0,-1 0,1 or 1,0.''. Its rows are the vertices of a digraph on
$S=16$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-1) + a(n-2) - 3\,a(n-3) + a(n-4) + 2\,a(n-5) - a(n-6)
\]
...
$c_{1}$ & $0$ & $c_{5}$ & $5$ & $c_{9}$ & $11$ & $c_{13}$ & $-2$ \\
$c_{2}$ & $5$ & $c_{6}$ & $-29$ & $c_{10}$ & $20$ & $c_{14}$ & $-1$ \\
$c_{3}$ & $17$ & $c_{7}$ & $-44$ & $c_{11}$ & $11$ &  &  \\
$c_{4}$ & $22$ & $c_{8}$ & $-25$ & $c_{12}$ & $-1$ &  &  \\
\end{tabular}\end{center}
Repeating with the first $14+4$ terms removed gives order $14$ again.

\subsection*{Row $n=2$, recovered}
The entry gives only the order for this line: ``n=2: [order 14]''. Its model is a walk on
...
$14$ --- the stated order --- with integer coefficients
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $2$ & $c_{5}$ & $10$ & $c_{9}$ & $-10$ & $c_{13}$ & $-2$ \\
$c_{2}$ & $7$ & $c_{6}$ & $-1$ & $c_{10}$ & $9$ & $c_{14}$ & $1$ \\
$c_{3}$ & $-8$ & $c_{7}$ & $0$ & $c_{11}$ & $8$ &  &  \\
$c_{4}$ & $-9$ & $c_{8}$ & $1$ & $c_{12}$ & $-7$ &  &  \\
\end{tabular}\end{center}
Repeating with the first $14+4$ terms removed gives order $14$ again.

\subsection*{Row $n=3$, recovered}
The entry gives only the order for this line: ``n=3: [order 69]''. Its model is a walk on
$S=256$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=512$ exact terms returns the minimal one, of order
$69$ --- the stated order --- with integer coefficients
\[
(c_1,\dots,c_{69})=(5,\ 36,\ -176,\ -572,\ 2881,\ 5073,\ -29109,\ -26399,\ 203151,\ 67333,\ -1031624,\ 81220,\ 3923298,\ -1487873,\ -11390628,\ 6830109,\ 25686170,\ -19178520,\ -46090847,\ 36730185,\ 69059380,\ -47592894,\ -94616664,\ 35474627,\ 130906196,\ 2629842,\ -183002698,\ -48616074,\ 234007356,\ 83938401,\ -250443639,\ -114923191,\ 217898108,\ 161146609,\ -161146609,\ -217898108,\ 114923191,\ 250443639,\ -83938401,\ -234007356,\ 48616074,\ 183002698,\ -2629842,\ -130906196,\ -35474627,\ 94616664,\ 47592894,\ -69059380,\ -36730185,\ 46090847,\ 19178520,\ -25686170,\ -6830109,\ 11390628,\ 1487873,\ -3923298,\ -81220,\ 1031624,\ -67333,\ -203151,\ 26399,\ 29109,\ -5073,\ -2881,\ 572,\ 176,\ -36,\ -5,\ 1).
\]
Repeating with the first $69+4$ terms removed gives order $69$ again.
...
model reproduces $37$ published values, $90$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264506.
