...
\title{The empirical column recurrences for the table OEIS A236009}
...
\begin{abstract}
OEIS A236009 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A236009 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with the minimum plus the maximum equal to the lower median plus the upper median in every 2X2 subblock.''. It has offset $1$, is
read by antidiagonals, and begins
\[
88, 508, 508, 2960, 4568, 2960, 17464, 41608,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 16*a(n-1) -86*a(n-2) +156*a(n-3) +48*a(n-4) -256*a(n-5) -64*a(n-6)\\
k=2: [order 17]\\
k=3: [order 70]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 09:00:07, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..3 arrays with the minimum plus the maximum equal to the lower median plus the upper median in every 2X2 subblock.''. Its rows are the vertices of a digraph on
...
model reproduces $8$ published values, $41$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A236009.
