...
\title{The empirical recurrence for OEIS A267716, proved by transfer matrix}
...
\begin{abstract}
OEIS A267716 counts arrays in which the REPEATED VALUES of each row, and of each column, stand in a
...
value so far in each column of the scan, so the count is a walk count on $S=1632$ states.
\end{abstract}
...
OEIS A267716 is ``Number of nX5 0..1 arrays with every repeated value in every row and column greater than the previous repeated value.''. It has offset $1$ and begins
\[
12, 144, 546, 1915, 4444, 10489, 20226, 39485,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2016 20:15:06, revision 4) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=1632,\quad\tau=\mathbf{1} .
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267716.
