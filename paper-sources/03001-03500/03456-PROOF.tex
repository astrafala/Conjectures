...
\title{The conjectured generating function for OEIS A206840, proved}
...
\begin{abstract}
OEIS A206840 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=273$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $291$ coefficients in exact arithmetic
...
OEIS A206840 is ``Number of 2 X n 0..3 arrays avoiding the pattern z-2 z-1 z in any row, column, nw-to-se diagonal or ne-to-sw antidiagonal.''. It has offset $0$ and begins
\[
1, 16, 256, 3844, 58081, 876096,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 + 7*x\^{}2 - 20*x\^{}3 + 7*x\^{}4 + x\^{}6) / ((1 - 16*x + 14*x\^{}2 - 4*x\^{}3 + x\^{}4)*(1 - 7*x\^{}2 + 12*x\^{}3 + 7*x\^{}4 - x\^{}6)). - \_Colin Barker\_, Jul 08 2019
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 03 2023 15:13:11, revision 20) this is still
...
\]
and there are $S=273$ states. The digraph is built by reading the entry's own English, and the
model reproduces all $19$ terms the entry publishes, exactly, in integer arithmetic --- a
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=6$ and $\deg D_2=10$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $282$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{282}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $291$
of them --- more than the $282+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{10}$ read off $D_2$, the first few being $16,\ -7,\ -120,\ 282,\ -84,\ -42$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
First, the model reproduces every one of the $19$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206840.
