...
\title{The conjectured generating function for OEIS A250754, proved}
...
\begin{abstract}
OEIS A250754 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250754 is ``Number of (n+1) X (7+1) 0..2 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing x(i,j)+x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
527, 1707, 5471, 17211, 53327, 163467,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: x*(527 - 1455*x + 1026*x\^{}2) / ((1 - x)*(1 - 2*x)*(1 - 3*x)). - \_Colin Barker\_, Nov 17 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 17 2018 20:55:00, revision 8) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250754.
