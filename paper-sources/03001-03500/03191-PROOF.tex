...
\title{The empirical recurrence for OEIS A208313, proved by counting patterns}
...
\begin{abstract}
OEIS A208313 counts arrays subject to a local condition \emph{and} to the clause ``new values
$0..1$ introduced in row major order'', which says the array is written in canonical
...
$2!\,a(n)=\sum_i\binom{2}{i}D_{2-i}L_i(n)$ with $D_m$ the derangement numbers,
...
state space down from $129$ to $S=65$. The entry's empirical recurrence of order
$32$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A208313 is ``Number of nX7 0..1 arrays with new values 0..1 introduced in row major order and no element equal to more than one of its immediate leftward or upward neighbors.''. It has offset $1$ and begins
\[
64, 1458, 26772, 459810, 7702668, 127750122, 2110085532, 34794255294,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 14:45:27, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;65\,a(n-1) - 1953\,a(n-2) + 36966\,a(n-3) - 502122\,a(n-4) + 5256909\,a(n-5) - 44318169\,a(n-6) + 309490947\,a(n-7) - 1823975982\,a(n-8) + 9183492558\,a(n-9) - 39813195762\,a(n-10) + 149339766420\,a(n-11) - 486017305326\,a(n-12) + 1374228095598\,a(n-13) - 3377800131066\,a(n-14) + 7218074976993\,a(n-15) - 13409006027409\,a(n-16) + 21654224930979\,a(n-17) - 30400201179594\,a(n-18) + 37104158581146\,a(n-19) - 39367401731406\,a(n-20) + 36289563240060\,a(n-21) - 29023819710498\,a(n-22) + 20084298224346\,a(n-23) - 11967106417902\,a(n-24) + 6091710309801\,a(n-25) - 2616943561281\,a(n-26) + 931245658623\,a(n-27) - 266848217802\,a(n-28) + 58935744018\,a(n-29) - 9341138457\,a(n-30) + 932678955\,a(n-31) - 43046721\,a(n-32)
\end{equation}
for all $n>32$.
...
The clause ``new values $0..1$ introduced in row major order'' selects, from each class
...
$v+1$. Every pattern with at most $K=2$ classes has exactly one such representative, and
...
\qquad
2!\,\sum_{j=0}^{2}N_j(n)=\sum_{i=0}^{2}\binom{2}{i}D_{2-i}\,L_i(n),
\]
...
$j\le 2$ and exchanging the order of summation,
\[
\sum_{j=0}^{2}N_j=\sum_{i=0}^{2}L_i\sum_{j=i}^{2}\frac{(-1)^{j-i}}{i!\,(j-i)!}
=\sum_{i=0}^{2}\frac{L_i}{i!}\sum_{t=0}^{2-i}\frac{(-1)^t}{t!} ,
\]
and multiplying by $2!$ turns the coefficient of $L_i$ into
$\frac{2!}{i!}\sum_{t\le 2-i}(-1)^t/t!=\binom{2}{i}(2-i)!\sum_{t\le 2-i}(-1)^t/t!
...
Assembling the $2$ lumped chains into one block-diagonal matrix $N$ of size $S=65$ and
putting the weights of Lemma~\ref{lem:inv} into the start vector,
$\mathbf{v}=\bigl(\binom{2}{i}D_{2-i}\varepsilon_P\bigr)_{i,P}$, gives
\begin{equation}\label{eq:walk}
2!\;a(n)\;=\;\mathbf{v}^{\!\top}N^{\,n+0-1}\mathbf{1} .
\end{equation}
The unlumped alphabet had $129$ rows in all; the lumped chain has $S=65$ states.
...
For every $n$ with $m=n+0-1\ge32$,
\[
2!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-32}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-32}$ out of
$N^{m}-\sum_i c_iN^{m-i}$. The scalar $2!$ is nonzero, so it does not affect whether
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $32$ matrix--vector products
...
corresponding to $n=32+1$ onwards, and the run continued until $w$ was the zero vector,
...
\eqref{eq:walk}, divided by $2!$, was evaluated at every one of the $15$
...
show up as a mismatch, and the quotient by $2!$ came out an integer at every index.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208313.
