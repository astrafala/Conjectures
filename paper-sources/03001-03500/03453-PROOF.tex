...
\title{No column above the one before it in every row: OEIS A212941}
...
\begin{abstract}
OEIS A212941 carries an empirical recurrence of order $11$ contributed by R.~H.~Hardin. It is
...
OEIS A212941 is ``Number of n X 6 0..5 arrays with no column j greater than or equal to than column j-1 in all rows.''. It has offset $1$ and begins
\[
1, 152747323, 28744943858947, 2438624218076957695, 153322267564381742818531, 8389667096561737605103329223, 427593510370812774462812710051387, 20973117493971065058428815227499445215,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 17 2022 21:53:33, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays of $6$ columns whose rows are arbitrary over the alphabet $\{0,1,\dots,5\}$, and asks that no column $j$ be greater than or equal to column $j-1$ \emph{in all rows}.

That is not a condition on any bounded window: ``in all rows'' quantifies over the whole array, and a row arriving at the very end can still decide it. What makes it a walk anyway is that it is a conjunction of independent facts about single rows. For each of the $5$ adjacent column pairs keep one bit, recording whether every row so far has had column $j$ greater than or equal to column $j-1$. A new row can only clear bits, never set one, and the array is admissible exactly when every bit has been cleared by the time the array ends.
...
\begin{theorem}
\[
a(n)\;=\;130453\,a(n-1) - 6763333836\,a(n-2) + 185208872760324\,a(n-3) - 2991440828618168976\,a(n-4) + 29966228467174160130816\,a(n-5) - 189618846625740940674481152\,a(n-6) + 754213725094693350993932353536\,a(n-7) - 1831929539263722316502211827073024\,a(n-8) + 2545588680910314328406052486644760576\,a(n-9) - 1758206800538810904218951296840095498240\,a(n-10) + 418951577236630476027552063815969641857024\,a(n-11)
\]
for every $n>11$.
\end{theorem}
...
First, the model reproduces all $8$ terms the entry publishes, exactly, in integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A212941.
