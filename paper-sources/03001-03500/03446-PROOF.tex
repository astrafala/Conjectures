...
\title{The empirical column recurrences for the table OEIS A252544}
...
\begin{abstract}
OEIS A252544 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A252544 is ``T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 1 3 4 6 or 7 and every 3X3 column and antidiagonal sum not equal to 1 3 4 6 or 7.''. It has offset $1$, is
read by antidiagonals, and begins
\[
478, 1048, 850, 2532, 3880, 2142, 5608, 15480,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:20:27, revision 7) these are still
...
column $k$$=1$ & $6$ & $209$ & $>8$ & $10$ \\
column $k$$=4$ & $6$ & $88249$ & $>7$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X(3) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 1 3 4 6 or 7 and every 3X3 column and antidiagonal sum not equal to 1 3 4 6 or 7.''. Its rows are the vertices of a digraph on
$S=209$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+2)X(6) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 1 3 4 6 or 7 and every 3X3 column and antidiagonal sum not equal to 1 3 4 6 or 7.''. Its rows are the vertices of a digraph on
$S=88249$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $20$ published values, $127$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252544.
