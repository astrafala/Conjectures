...
\title{The empirical recurrence for OEIS A224407, proved by transfer matrix}
...
\begin{abstract}
OEIS A224407 counts arrays over $\{0,\dots,1\}$ whose rows, columns, diagonals or
antidiagonals are required to be monotone or unimodal, and carries an empirical recurrence of
order $6$ contributed by R.~H.~Hardin. A monotone condition is local, but a unimodal one
...
is a walk count on $S=22$ vertices and the recurrence follows from a finite exact
computation.
\end{abstract}
...
OEIS A224407 is ``Number of n X 6 0..1 arrays with rows unimodal and antidiagonals nondecreasing.''. It has offset $1$ and begins
\[
22, 169, 879, 3902, 16420, 68282, 284254, 1187830,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 7*a(n-1) -14*a(n-2) +9*a(n-3) +4*a(n-5) +16*a(n-6) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 30 2018 06:10:31, revision 7) the statement is
...
bit vector), $|\mathcal V|=S=22$; let $(r,f)\to(s,f')$ be an edge when placing $s$ after $r$
...
$h_j=G^{j}N^{o}\mathbf{1}$, we have for $j\ge6$
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224407.
