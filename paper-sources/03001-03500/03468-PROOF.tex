...
\title{The empirical recurrence for OEIS A198662, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A198662 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
lumpable, cutting $55$ pairs down to $S=9$ states. The entry's empirical recurrence of
order $7$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A198662 is ``Number of n X 1 0..4 arrays with values 0..4 introduced in row major order and each element equal to one or two horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 02 2018 08:03:23, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) + a(n-2) - 17\,a(n-3) + 6\,a(n-4) + 21\,a(n-5) - 6\,a(n-6) - 8\,a(n-7)
\end{equation}
for all $n>8$.
...
Assembling the $5$ lumped chains block-diagonally into $N$ of size $S=9$, with the
...
index,
\[
5!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-7}\,
\Bigl(h_{7}-\sum_i c_i h_{7-i}\Bigr)
\]
...
$w=h_{7}-\sum_i c_ih_{7-i}$; and it is enough to check $m=0,1,\dots,S-1$.
\end{lemma}
...
index corresponding to $n=8+1$ onwards, and the run was continued until $S+1$
...
\eqref{eq:walk2}, divided by $5!$, was evaluated at every one of the $32$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198662.
