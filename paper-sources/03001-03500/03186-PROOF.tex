...
\title{The conjectured generating function for OEIS A218348, proved}
...
\begin{abstract}
OEIS A218348 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A218348 is ``Number of n X 2 binary arrays with every 0 a horizontal or vertical neighbor to some 1.''. It has offset $1$ and begins
\[
3, 11, 41, 149, 547, 2007,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(3 + 2*x + 2*x\^{}2 - 2*x\^{}3 - x\^{}4)/(1 - 3*x - 2*x\^{}2 - 2*x\^{}3 + x\^{}4 + x\^{}5). - \_Andrew Howroyd\_, May 10 2017
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 19 2026 12:23:55, revision 30) this is still
...
The family this entry belongs to is read by one model, described by its author as: ```Number of n X W 0..m arrays with every element both >= and <= some horizontal or antidiagonal neighbor', and the rest of the family in which a cell demands that SOME neighbour of it look a certain way.''
...
\]
a polynomial of degree at most $26$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{26}$ then $R$ has a zero of order greater than its own
...
of them --- more than the $26+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{5}$ read off $D_2$, the first few being $3,\ 2,\ 2,\ -1,\ -1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A218348.
