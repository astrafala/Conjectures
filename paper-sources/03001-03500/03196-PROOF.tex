...
\title{The empirical column recurrences for the table OEIS A253367}
...
\begin{abstract}
OEIS A253367 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A253367 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with every 2X2 and 3X3 subblock diagonal maximum minus antidiagonal minimum unequal to its neighbors horizontally and vertically.''. It has offset $1$, is
read by antidiagonals, and begins
\[
152, 272, 272, 560, 424, 560, 976, 960,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 4*a(n-2) -3*a(n-4) +4*a(n-6)\\
k=2: a(n) = 5*a(n-2) -8*a(n-4) +8*a(n-6) +4*a(n-8)\\
k=3: a(n) = 7*a(n-2) -15*a(n-4) +17*a(n-6) +16*a(n-8) for n>11\\
k=4: [order 14] for n>16\\
k=5: [order 16] for n>19\\
k=6: [order 22] for n>26\\
k=7: [order 30] for n>33\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 14:00:20, revision 6) these are still
...
column $k$$=1$ & $6$ & $201$ & $>6$ & $9$ \\
column $k$$=2$ & $8$ & $386$ & $>8$ & $10$ \\
column $k$$=3$ & $8$ & $825$ & $>11$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X3 0..1 arrays with every 2X2 and 3X3 subblock diagonal maximum minus antidiagonal minimum unequal to its neighbors horizontally and vertically.''. Its rows are the vertices of a digraph on
$S=201$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+2)X4 0..1 arrays with every 2X2 and 3X3 subblock diagonal maximum minus antidiagonal minimum unequal to its neighbors horizontally and vertically.''. Its rows are the vertices of a digraph on
$S=386$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;5\,a(n-2) - 8\,a(n-4) + 8\,a(n-6) + 4\,a(n-8)
\]
holds from index $8+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+2)X5 0..1 arrays with every 2X2 and 3X3 subblock diagonal maximum minus antidiagonal minimum unequal to its neighbors horizontally and vertically.''. Its rows are the vertices of a digraph on
$S=825$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;7\,a(n-2) - 15\,a(n-4) + 17\,a(n-6) + 16\,a(n-8)
\]
...
model reproduces $29$ published values, $122$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253367.
