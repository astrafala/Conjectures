...
\title{The empirical recurrence for OEIS A189429, proved by transfer matrix}
...
\begin{abstract}
OEIS A189429 counts the permutations of the cells of an $n\times3$ array in which every cell
...
carried is which images inside a window of $8$ consecutive positions are already taken.
That makes $a$ a walk count on $S=16$ states, hence $C$-finite, and whether the published
...
OEIS A189429 is ``Number of n X 3 array permutations with each element not moving, or moving one space N, SW or SE.''. It has offset $1$ and begins
\[
1, 3, 9, 31, 109, 367, 1245, 4247,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) +4*a(n-2) +10*a(n-3) +12*a(n-4) +8*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (May 02 2018 09:23:08, revision 8) this is still
...
\section{A window of 8 positions}

Number the cells in row-major order, $k=i\cdot3+j$. An offset $(\delta_1,\delta_2)$ moves a
cell by $\delta_13+\delta_2$ places in that order, so every image lies between $lo=-3$ and
...
taken yet. What has to be remembered is therefore exactly which of the $8$ positions
$k+-3,\dots,k+4$ are taken --- a subset of a set of size $8$, and nothing else.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A189429.
