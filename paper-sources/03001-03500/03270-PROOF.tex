...
\title{The empirical recurrence for OEIS A267784, proved by transfer matrix}
...
\begin{abstract}
OEIS A267784 counts arrays in which the REPEATED VALUES of each row, and of each column, stand in a
...
value so far in each column of the scan, so the count is a walk count on $S=1191$ states.
\end{abstract}
...
OEIS A267784 is ``Number of nX4 0..1 arrays with every repeated value in every row greater than or equal to, and in every column greater than, the previous repeated value.''. It has offset $1$ and begins
\[
15, 225, 1056, 5081, 14956, 44742, 102954, 238813,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 20 2016 12:28:19, revision 4) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=1191,\quad\tau=\mathbf{1} .
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267784.
