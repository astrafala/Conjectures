...
\title{The empirical recurrence for OEIS A281950, proved by transfer matrix}
...
\begin{abstract}
OEIS A281950 counts $n\times3$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
every CELL over a neighbour set the entry names, and counted UP TO RELABELLING of the
alphabet; it carries an empirical recurrence of order $10$ contributed by
...
OEIS A281950 is ``Number of nX3 0..1 arrays with no element unequal to more than four of its king-move neighbors and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
4, 30, 133, 587, 2559, 11251, 49293, 216274,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) +7*a(n-2) -3*a(n-3) -5*a(n-4) +2*a(n-5) +2*a(n-6) -12*a(n-7) +2*a(n-8) -3*a(n-10) for n>12
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 03 2017 08:21:17, revision 4) this is still
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 7\,a(n-2) - 3\,a(n-3) - 5\,a(n-4) + 2\,a(n-5) + 2\,a(n-6) - 12\,a(n-7) + 2\,a(n-8) - 3\,a(n-10)
\]
for every $n>12$.
\end{theorem}
...
By Lemma~\ref{lem:crit} the recurrence holds for every $n>12$. The bound is exact: at $n=12$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A281950.
