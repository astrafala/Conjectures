...
\title{The empirical column recurrences for the table OEIS A260138}
...
\begin{abstract}
OEIS A260138 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A260138 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 01010101 or 01010111.''. It has offset $1$, is
read by antidiagonals, and begins
\[
36, 58, 58, 138, 128, 138, 266, 342,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 12:01:45, revision 6) these are still
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 01010101 or 01010111.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+2)X(4) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 01010101 or 01010111.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + a(n-2) + 5\,a(n-3) + a(n-4) - 3\,a(n-5) - 3\,a(n-6)
\]
...
model reproduces $17$ published values, $56$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A260138.
