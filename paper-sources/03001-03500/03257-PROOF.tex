...
\title{The empirical recurrence for OEIS A262334, proved by transfer matrix}
...
\begin{abstract}
OEIS A262334 counts arrays of width $5$ over $\{0,1\}$ whose rows, read as base-$2$
numbers, are divisible by $9$ and whose columns, read the same way, are divisible by
$9$. The entry carries an empirical recurrence of order $11$ contributed by
...
OEIS A262334 is ``Number of (n+3)X(2+3) 0..1 arrays with each row and column divisible by 9, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
4, 16, 64, 225, 841, 3249, 12996, 51984,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 18 2015 07:56:41, revision 4) this is still
...
$9$ and every $\gamma_j$ be divisible by $9$.
...
\gamma_j^{(i)}\bmod 9\;=\;
\bigl(2\,(\gamma_j^{(i-1)}\bmod 9)+x(i,j)\bigr)\bmod 9 .
\]
\end{lemma}
...
$9$ commutes with both operations.
\end{proof}

So the vector $\bigl(\gamma_1\bmod 9,\dots,\gamma_{5}\bmod
9\bigr)$ is a state: at most $9^{5}$ of them, and each array row
...
which every entry is divisible by $9$ --- so the end vector is not the all-ones
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) - 8\,a(n-2) - 8\,a(n-3) + 48\,a(n-4) - 64\,a(n-5) + a(n-6) - 6\,a(n-7) + 8\,a(n-8) + 8\,a(n-9) - 48\,a(n-10) + 64\,a(n-11)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262334.
