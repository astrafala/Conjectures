...
\title{The empirical column recurrences for the table OEIS A229578}
...
\begin{abstract}
OEIS A229578 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 2, 3, 4, 5$.
\end{abstract}
...
OEIS A229578 is ``T(n,k) = number of defective 4-colorings of an n X k 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..3 order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 1, 1, 2, 6, 2, 6, 26,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 27 2021 21:15:43, revision 8) these are still
...
column $k$$=1$ & $4$ & $20$ & $>6$ & $10$ \\
column $k$$=2$ & $2$ & $60$ & $>3$ & $9$ \\
column $k$$=3$ & $4$ & $200$ & $>5$ & $8$ \\
column $k$$=4$ & $4$ & $708$ & $>7$ & $7$ \\
column $k$$=5$ & $4$ & $2600$ & $>7$ & $6$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of defective 4-colorings of an n X 1 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..3 order.''. Its rows are the vertices of a digraph on
$S=20$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of defective 4-colorings of an n X 2 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..3 order.''. Its rows are the vertices of a digraph on
$S=60$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-1) - 4\,a(n-2)
...
The sequence is ``Number of defective 4-colorings of an n X 3 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..3 order.''. Its rows are the vertices of a digraph on
$S=200$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of defective 4-colorings of an n X 4 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..3 order.''. Its rows are the vertices of a digraph on
$S=708$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;6\,a(n-1) - 13\,a(n-2) + 12\,a(n-3) - 4\,a(n-4)
...
The sequence is ``Number of defective 4-colorings of an n X 5 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..3 order.''. Its rows are the vertices of a digraph on
$S=2600$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;6\,a(n-1) - 13\,a(n-2) + 12\,a(n-3) - 4\,a(n-4)
...
model reproduces $40$ published values, $106$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229578.
