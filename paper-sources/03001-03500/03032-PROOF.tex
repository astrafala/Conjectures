...
\title{The empirical column recurrences for the table OEIS A230977}
...
\begin{abstract}
OEIS A230977 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A230977 is ``T(n,k)=Number of (n+2)X(k+2) 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 16, 16, 64, 144, 64, 256, 1024,\ \dots
\]
...
k=3: [order 25]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:09:27, revision 6) these are still
...
column $k$$=1$ & $4$ & $737$ & $>2$ & $8$ \\
column $k$$=2$ & $5$ & $5841$ & $>3$ & $7$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X3 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. Its rows are the vertices of a digraph on
$S=737$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-1) + 20\,a(n-3) + 25\,a(n-4)
\]
holds from index $2+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+2)X4 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. Its rows are the vertices of a digraph on
$S=5841$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;9\,a(n-1) - 8\,a(n-2) + 72\,a(n-3) + 9\,a(n-4) - 81\,a(n-5)
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230977.
