...
\title{The empirical column recurrences for the table OEIS A204607}
...
\begin{abstract}
OEIS A204607 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=4, 5$.
\end{abstract}
...
OEIS A204607 is ``T(n,k)=Number of (n+2)X(k+2) 0..2 arrays with every 3X3 subblock having three strictly increasing elements in a row horizontally, vertically or nw-to-se diagonally exactly one way.''. It has offset $1$, is
read by antidiagonals, and begins
\[
3562, 29678, 29678, 328845, 144168, 328845, 2275645, 741296,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: (order 24 recurrence)\\
k=2: (order 30 recurrence for n>33)\\
k=3: a(n) = 442*a(n-3) +63*a(n-5) +43*a(n-6) -63*a(n-8) +81*a(n-9) -81*a(n-12) for n>20\\
k=4: a(n) = 442*a(n-3) +43*a(n-6) for n>15\\
k=5: a(n) = 442*a(n-3) +43*a(n-6) for n>16\\
k=6: a(n) = 442*a(n-3) +43*a(n-6) for n>17\\
k=7: a(n) = 442*a(n-3) +43*a(n-6) for n>18\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 11:21:14, revision 9) these are still
...
column $k$$=4$ & $6$ & $18773$ & $>15$ & $8$ \\
column $k$$=5$ & $6$ & $106496$ & $>16$ & $8$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X(6) 0..2 arrays with every 3X3 subblock having three strictly increasing elements in a row horizontally, vertically or nw-to-se diagonally exactly one way.''. Its rows are the vertices of a digraph on
$S=18773$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;442\,a(n-3) + 43\,a(n-6)
\]
holds from index $15+1$ on.

\subsection*{Column $k$$=5$}
The sequence is ``Number of (n+2)X(7) 0..2 arrays with every 3X3 subblock having three strictly increasing elements in a row horizontally, vertically or nw-to-se diagonally exactly one way.''. Its rows are the vertices of a digraph on
$S=106496$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;442\,a(n-3) + 43\,a(n-6)
...
model reproduces $16$ published values, $139$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204607.
