...
\title{The empirical recurrence for OEIS A197435, proved by transfer matrix}
...
\begin{abstract}
OEIS A197435 counts arrays of width $1$ over $\{0,1,2\}$ in which no cell is the average of its neighbours. The entry carries an
...
$3$ consecutive rows and the count is a walk count on $S=13$ states.
\end{abstract}
...
OEIS A197435 is ``Number of n X 1 0..2 arrays with no element equal to the average of its horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
3, 6, 10, 28, 66, 164, 400, 982,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) +2*a(n-2) +3*a(n-3) +2*a(n-4) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (May 12 2018 17:25:02, revision 12) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The arrays have $1$ columns and a growing number of rows, with entries in $\{0,1,2\}$. Fix the
...
array of width $1$ over $\{0,1,2\}$ with a few rows was written out cell by cell and each cell
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A197435.
