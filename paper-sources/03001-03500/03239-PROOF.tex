...
\title{The empirical column recurrences for the table OEIS A235902}
...
\begin{abstract}
OEIS A235902 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A235902 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with the minimum plus the maximum equal to the lower median plus the upper median in every 2X2 subblock.''. It has offset $1$, is
read by antidiagonals, and begins
\[
33, 125, 125, 473, 641, 473, 1797, 3265,\ \dots
\]
...
k=7: [order 15]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:55:14, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..2 arrays with the minimum plus the maximum equal to the lower median plus the upper median in every 2X2 subblock.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;7\,a(n-1) - 10\,a(n-2) - 8\,a(n-3)
\]
holds from index $2+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+1)X3 0..2 arrays with the minimum plus the maximum equal to the lower median plus the upper median in every 2X2 subblock.''. Its rows are the vertices of a digraph on
$S=27$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;13\,a(n-1) - 44\,a(n-2) - 24\,a(n-3) + 192\,a(n-4) + 144\,a(n-5)
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235902.
