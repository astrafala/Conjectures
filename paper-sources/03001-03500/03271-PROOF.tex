...
\title{The empirical column recurrences for the table OEIS A234490}
...
\begin{abstract}
OEIS A234490 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234490 is ``T(n,k) is the number of (n+1) X (k+1) 0..3 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
62, 242, 242, 944, 926, 944, 3686, 3540,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 7*a(n-1) -9*a(n-2) -12*a(n-3).\\
k=2: a(n) = 10*a(n-1) -28*a(n-2) +a(n-3) +54*a(n-4) +24*a(n-5).\\
k=3: [order 11].\\
k=4: [order 18].\\
k=5: [order 31].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 20:33:36, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..3 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;7\,a(n-1) - 9\,a(n-2) - 12\,a(n-3)
\]
holds from index $2+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+1) X (3) 0..3 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
$S=64$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;10\,a(n-1) - 28\,a(n-2) + a(n-3) + 54\,a(n-4) + 24\,a(n-5)
\]
...
$c_{1}$ & $13$ & $c_{4}$ & $865$ & $c_{7}$ & $2822$ & $c_{10}$ & $-1512$ \\
$c_{2}$ & $-43$ & $c_{5}$ & $-1033$ & $c_{8}$ & $2712$ & $c_{11}$ & $-288$ \\
$c_{3}$ & $-107$ & $c_{6}$ & $-1916$ & $c_{9}$ & $-1332$ &  &  \\
...
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $16$ & $c_{6}$ & $-15789$ & $c_{11}$ & $304674$ & $c_{16}$ & $6480$ \\
$c_{2}$ & $-69$ & $c_{7}$ & $56088$ & $c_{12}$ & $-145486$ & $c_{17}$ & $-16992$ \\
$c_{3}$ & $-186$ & $c_{8}$ & $1729$ & $c_{13}$ & $-269396$ & $c_{18}$ & $-3456$ \\
$c_{4}$ & $2233$ & $c_{9}$ & $-189898$ & $c_{14}$ & $58980$ &  &  \\
$c_{5}$ & $-3610$ & $c_{10}$ & $95019$ & $c_{15}$ & $118584$ &  &  \\
\end{tabular}\end{center}
...
model reproduces $15$ published values, $66$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234490.
