...
\title{The empirical column recurrences for the table OEIS A238323}
...
\begin{abstract}
OEIS A238323 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A238323 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with no element greater than all horizontal neighbors or less than all vertical neighbors.''. It has offset $1$, is
read by antidiagonals, and begins
\[
3, 9, 9, 22, 67, 22, 51, 376,\ \dots
\]
...
k=3: [order 43]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:52:11, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with no element greater than all horizontal neighbors or less than all vertical neighbors.''. Its rows are the vertices of a digraph on
...
$c_{1}$ & $6$ & $c_{5}$ & $62$ & $c_{9}$ & $-41$ & $c_{13}$ & $1$ \\
$c_{2}$ & $-12$ & $c_{6}$ & $49$ & $c_{10}$ & $-12$ &  &  \\
$c_{3}$ & $39$ & $c_{7}$ & $33$ & $c_{11}$ & $6$ &  &  \\
$c_{4}$ & $6$ & $c_{8}$ & $-32$ & $c_{12}$ & $5$ &  &  \\
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A238323.
