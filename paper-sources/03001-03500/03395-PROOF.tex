...
\title{The empirical column recurrences for the table OEIS A252343}
...
\begin{abstract}
OEIS A252343 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A252343 is ``T(n,k)=Number of (n+2)X(k+2) 0..2 arrays with every 3X3 subblock row, column, diagonal and antidiagonal sum not equal to 2 3 or 4.''. It has offset $1$, is
read by antidiagonals, and begins
\[
52, 108, 108, 234, 268, 234, 538, 810,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1) +3*a(n-3) +4*a(n-4) +4*a(n-5) +2*a(n-6) for n>7\\
k=2: [order 8] for n>9\\
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:07:16, revision 6) these are still
...
The sequence is ``Number of (n+2)X(3) 0..2 arrays with every 3X3 subblock row, column, diagonal and antidiagonal sum not equal to 2 3 or 4.''. Its rows are the vertices of a digraph on
...
model reproduces $9$ published values, $33$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252343.
