...
\title{A proof of the conjectured recurrence for OEIS A118974}
...
\begin{abstract}
OEIS A118974 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 144 x^{3} - 24 x^{2}$, a polynomial of degree $3$, which proves
...
OEIS A118974 is ``Sum of the lengths of the first descents in all hill-free Dyck paths of semilength n (a hill in a Dyck path is a peak at level 1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 2, 4, 11, 31, 94, 298,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(\sqrt{1 - 4 x} - 1\right)^{2} \left(x \sqrt{1 - 4 x} + x - \sqrt{1 - 4 x} + 1\right)}{4 x \left(x - 1\right) \left(\sqrt{1 - 4 x} - 3\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n+1)*(17*n\^{}2-65*n+60)*a(n) -3*(3*n-4)*(17*n\^{}2-48*n+15)*a(n-1) +3*(17*n\^{}3-82*n\^{}2+121*n-60)*a(n-2) +2*(2*n-5) *(17*n\^{}2-31*n+12) *a(n-3)=0. - \emph{R. J. Mathar}, Jun 22 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-24, revision 22) the
...
\[
p_{0}(n)=2 \left(n + 1\right) \left(17 n^{2} - 65 n + 60\right),\qquad p_{1}(n)=- 3 \left(3 n - 4\right) \left(17 n^{2} - 48 n + 15\right),\qquad p_{2}(n)=3 \left(17 n^{3} - 82 n^{2} + 121 n - 60\right),\qquad p_{3}(n)=2 \left(2 n - 5\right) \left(17 n^{2} - 31 n + 12\right)
\]
...
\section{The computation for A118974}
...
\[
2 \left(n + 1\right) \left(17 n^{2} - 65 n + 60\right)\,a(n) -  3 \left(3 n - 4\right) \left(17 n^{2} - 48 n + 15\right)\,a(n-1) + 3 \left(17 n^{3} - 82 n^{2} + 121 n - 60\right)\,a(n-2) + 2 \left(2 n - 5\right) \left(17 n^{2} - 31 n + 12\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A118974, agreeing on all 13 terms compared.
...
vanish, for all 25 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 144 x^{3} - 24 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A118974.
