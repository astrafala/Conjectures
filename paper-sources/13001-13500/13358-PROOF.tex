...
\title{A proof of the conjectured recurrence for OEIS A164586}
...
\begin{abstract}
OEIS A164586 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{4} + 2 x^{3} + 2 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A164586 is ``Row sums of generalized rhombic triangle A164585''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 8, 19, 51, 134, 354,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{3} - 2 x^{2} - 2 x - \sqrt{x^{6} - 4 x^{4} - 10 x^{3} - 4 x^{2} + 1} + 1}{2 x \left(x^{4} + 3 x^{3} + 4 x^{2} + x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(n-1)*a(n-1) +2*(-2*n-1)*a(n-2) +2*(-7*n+10)*a(n-3) +14*(-n+2)*a(n-4) +2*(-2*n+3)*a(n-5) +(n-7)*a(n-6) +(n-5)*a(n-7)=0. - \emph{R. J. Mathar}, Feb 10 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2017-08-12, revision 10) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=n - 1,\qquad p_{2}(n)=- 2 \left(2 n + 1\right),\qquad p_{3}(n)=- 2 \left(7 n - 10\right),\qquad p_{4}(n)=- 14 \left(n - 2\right),\qquad p_{5}(n)=- 2 \left(2 n - 3\right),\qquad p_{6}(n)=n - 7,\qquad p_{7}(n)=n - 5
\]
...
\section{The computation for A164586}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x^{3} - 2 x^{2} - 2 x - 1\right) \left(x^{3} + 2 x^{2} + 2 x - 1\right) .
\]
...
\[
(n + 1)\,a(n) + (n - 1)\,a(n-1) -  2 \left(2 n + 1\right)\,a(n-2) -  2 \left(7 n - 10\right)\,a(n-3) -  14 \left(n - 2\right)\,a(n-4) -  2 \left(2 n - 3\right)\,a(n-5) + (n - 7)\,a(n-6) + (n - 5)\,a(n-7) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A164586, agreeing on all 13 terms compared.
...
vanish, for all 23 values of $n$ from $n=7$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=x^{4} + 2 x^{3} + 2 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A164586.
