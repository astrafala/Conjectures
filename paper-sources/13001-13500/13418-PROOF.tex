...
\title{A proof of the conjectured recurrence for OEIS A135052}
...
\begin{abstract}
OEIS A135052 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 4 x^{3} + 8 x^{2} - 6 x + 2$, a polynomial of degree $3$, which proves
...
OEIS A135052 is ``Expansion of g.f.: (1-2*x-sqrt(1-4*x+8*x\textasciicircum{}3-4*x\textasciicircum{}4))/(2*x\textasciicircum{}2*(1-x))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 7, 19, 51, 143, 407,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x + \sqrt{- 4 x^{4} + 8 x^{3} - 4 x + 1} - 1}{2 x^{2} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +(-5*n-4)*a(n-1) +2*(2*n+1)*a(n-2) +4*(2*n-5)*a(n-3) +12*(-n+3)*a(n-4) +4*(n-4)*a(n-5)=0. - \emph{R. J. Mathar}, Apr 19 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 41) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 5 n - 4,\qquad p_{2}(n)=2 \left(2 n + 1\right),\qquad p_{3}(n)=4 \left(2 n - 5\right),\qquad p_{4}(n)=- 12 \left(n - 3\right),\qquad p_{5}(n)=4 \left(n - 4\right)
\]
...
\section{The computation for A135052}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \left(2 x^{2} - 1\right) \left(2 x^{2} - 4 x + 1\right) .
\]
...
\[
(n + 2)\,a(n) + (- 5 n - 4)\,a(n-1) + 2 \left(2 n + 1\right)\,a(n-2) + 4 \left(2 n - 5\right)\,a(n-3) -  12 \left(n - 3\right)\,a(n-4) + 4 \left(n - 4\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A135052, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 4 x^{3} + 8 x^{2} - 6 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A135052.
