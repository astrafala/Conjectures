...
\title{A proof of the conjectured recurrence for OEIS A191531}
...
\begin{abstract}
OEIS A191531 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A191531 is ``Sum of lengths of initial and final horizontal segments over all dispersed Dyck paths of semilength n (i.e., over all Motzkin paths of length n with no (1,0)-steps at positive heights)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 2, 5, 10, 21, 40, 79,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{x \left(2 x + \sqrt{1 - 4 x^{2}} - 3\right)}{\left(x - 1\right)^{2} \left(- 2 x + \sqrt{1 - 4 x^{2}} + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +n*(n-3)*a(n) -2*(n\^{}2-2*n-2)*a(n-1) -(3*n\^{}2-21*n+32)*a(n-2) +2*(n-2)*(4*n-13)*a(n-3) -4*(n-2)*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-29, revision 17) the
...
\[
p_{0}(n)=n \left(n - 3\right),\qquad p_{1}(n)=- 2 \left(n^{2} - 2 n - 2\right),\qquad p_{2}(n)=- 3 n^{2} + 21 n - 32,\qquad p_{3}(n)=2 \left(n - 2\right) \left(4 n - 13\right),\qquad p_{4}(n)=- 4 \left(n - 3\right) \left(n - 2\right)
\]
...
\section{The computation for A191531}
...
\[
n \left(n - 3\right)\,a(n) -  2 \left(n^{2} - 2 n - 2\right)\,a(n-1) + (- 3 n^{2} + 21 n - 32)\,a(n-2) + 2 \left(n - 2\right) \left(4 n - 13\right)\,a(n-3) -  4 \left(n - 3\right) \left(n - 2\right)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A191531, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A191531.
