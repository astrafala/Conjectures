...
\title{The empirical recurrence for OEIS A240297, recovered from its tail}
...
\begin{abstract}
OEIS A240297 records ``Empirical recurrence of order $80$'' and keeps the recurrence itself in
a linked file rather than in the entry, so the conjecture cannot be read off the entry at all.
It can still be settled. The entry counts arrays under a condition local to a bounded window of
lines, which makes the count a walk count on a finite digraph and forces a monic integer
recurrence of order at most the number $S'=125$ of states with distinct futures. Berlekamp--Massey on
...
Its last coefficient is $c_{80}=8508416\neq0$, so no further tail satisfies anything
...
OEIS A240297 is ``Number of 3Xn 0..3 arrays with no element equal to zero plus the sum of elements to its left or two plus the sum of the elements above it or two plus the sum of the elements diagonally to its northwest, modulo 4.''. It has offset $1$ and begins
\[
4, 24, 206, 1322, 9970, 63892, 459420, 2983714,\ \dots
\]
The entry states:
\begin{quote}
Empirical recurrence of order 80 (see link above)
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 09:47:53, revision 7)
...
merging $S'=125$ states remain, and it is that bound every computation below uses.

\begin{lemma}\label{lem:cf}
$a$ satisfies a monic linear recurrence with integer coefficients of order at most $S'=125$.
\end{lemma}
...
Applied to $2S'=250$ exact terms from $n=2$ on, it returns order $80$ --- the
...
\begin{theorem}
The recurrence above holds for every $n\ge82$, and it is the recurrence OEIS A240297 records.
\end{theorem}
...
$-c_{80}=-8508416$, which is not zero, so $q_{\min}(0)\neq0$ and $q_{\min}$ has no
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A240297.
