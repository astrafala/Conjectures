...
closed form. Here it equals $9 - 28 x$, a polynomial of degree $1$, which proves
...
\begin{quote}\small
\textbf{Conjecture:} 3*(2*n+3)*(n+1)*a(n) +2*(-28*n\^{}2-52*n+21)*a(n-1) +32*(2*n+5)*(2*n-3)*a(n-2)=0. - \emph{R. J. Mathar}, Feb 05 2015
\end{quote}
...
\[
p_{0}(n)=3 \left(n + 1\right) \left(2 n + 3\right),\qquad p_{1}(n)=- 2 \left(28 n^{2} + 52 n - 21\right),\qquad p_{2}(n)=32 \left(2 n - 3\right) \left(2 n + 5\right)
\]
...
\[
3 \left(n + 1\right) \left(2 n + 3\right)\,a(n) -  2 \left(28 n^{2} + 52 n - 21\right)\,a(n-1) + 32 \left(2 n - 3\right) \left(2 n + 5\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
cancellation in $K$, not by series truncation, so the identity $B=9 - 28 x$ is an
