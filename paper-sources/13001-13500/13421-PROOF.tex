...
\title{A proof of the conjectured recurrence for OEIS A126322}
...
\begin{abstract}
OEIS A126322 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 132678 x^{3} + 103518 x^{2} - 24156 x + 4548$, a polynomial of degree $3$, which proves
...
OEIS A126322 is ``Number of hex trees with n edges and no branches of length 1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 9, 27, 90, 297, 1053, 3888,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{\left(3 x + \sqrt{- 36 x^{4} + 9 x^{2} - 6 x + 1} - 1\right) \left(9 x^{2} - 3 x + 1\right)}{18 x^{4}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+4)*(25*n\^{}2+230*n+1137)*a(n) +3*(-50*n\^{}3-585*n\^{}2-3169*n-4248) *a(n-1) +9*(25*n\^{}3+255*n\^{}2+932*n-1764) *a(n-2) +29484*a(n-3) -36*(n-4) *(25*n\^{}2+280*n+1392) *a(n-4)=0. - \emph{R. J. Mathar}, Jun 17 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2022-09-08, revision 13) the
...
\[
p_{0}(n)=\left(n + 4\right) \left(25 n^{2} + 230 n + 1137\right),\qquad p_{1}(n)=- 3 \left(50 n^{3} + 585 n^{2} + 3169 n + 4248\right),\qquad p_{2}(n)=9 \left(25 n^{3} + 255 n^{2} + 932 n - 1764\right),\qquad p_{3}(n)=29484,\qquad p_{4}(n)=- 36 \left(n - 4\right) \left(25 n^{2} + 280 n + 1392\right)
\]
...
\section{The computation for A126322}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \left(6 x^{2} - 3 x + 1\right) \left(6 x^{2} + 3 x - 1\right) .
\]
...
\[
B(x)\;=\;- 132678 x^{3} + 103518 x^{2} - 24156 x + 4548 ,
\]
...
\[
\left(n + 4\right) \left(25 n^{2} + 230 n + 1137\right)\,a(n) -  3 \left(50 n^{3} + 585 n^{2} + 3169 n + 4248\right)\,a(n-1) + 9 \left(25 n^{3} + 255 n^{2} + 932 n - 1764\right)\,a(n-2) + 29484\,a(n-3) -  36 \left(n - 4\right) \left(25 n^{2} + 280 n + 1392\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A126322, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 132678 x^{3} + 103518 x^{2} - 24156 x + 4548$ is an
...
\url{https://oeis.org}, 2026. Sequence A126322.
