...
\title{A proof of the conjectured recurrence for OEIS A071264}
...
\begin{abstract}
OEIS A071264 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $370002$, a polynomial of degree $0$, which proves
...
OEIS A071264 is ``Expansion of (1+x*C\textasciicircum{}4)*C\textasciicircum{}3, where C = (1-(1-4*x)\textasciicircum{}(1/2))/(2*x) is g.f. for Catalan numbers, A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 16, 63, 244, 934, 3549, 13428,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(- 16 x^{3} - \left(\sqrt{1 - 4 x} - 1\right)^{4}\right) \left(\sqrt{1 - 4 x} - 1\right)^{3}}{128 x^{6}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} D-finite with recurrence (n+6) *(8587*n\textasciicircum{}2 +62958*n +61667) *a(n) +(8587*n\textasciicircum{}3 -377960*n\textasciicircum{}2 -2268967*n -1091596) *a(n-1) -2 *(2*n-1) *(42935*n\textasciicircum{}2 -14497*n -123334) *a(n-2)=0. - \emph{R. J. Mathar}, Dec 10 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2024-09-13, revision 13) the
...
\[
p_{0}(n)=\left(n + 6\right) \left(8587 n^{2} + 62958 n + 61667\right),\qquad p_{1}(n)=8587 n^{3} - 377960 n^{2} - 2268967 n - 1091596,\qquad p_{2}(n)=- 2 \left(2 n - 1\right) \left(42935 n^{2} - 14497 n - 123334\right)
\]
...
\section{The computation for A071264}
...
\[
\left(n + 6\right) \left(8587 n^{2} + 62958 n + 61667\right)\,a(n) + (8587 n^{3} - 377960 n^{2} - 2268967 n - 1091596)\,a(n-1) -  2 \left(2 n - 1\right) \left(42935 n^{2} - 14497 n - 123334\right)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A071264, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=370002$ is an
...
\url{https://oeis.org}, 2026. Sequence A071264.
