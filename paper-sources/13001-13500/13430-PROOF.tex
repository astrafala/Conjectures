...
\title{A proof of the conjectured recurrence for OEIS A234269}
...
\begin{abstract}
OEIS A234269 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{3} + 3 x^{2}$, a polynomial of degree $3$, which proves
...
OEIS A234269 is ``Expansion of (1-2*x\^{}2-sqrt(1-4*x\^{}2-4*x\^{}3))/(2*x*sqrt(1-4*x\^{}2-4*x\^{}3))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 1, 4, 7, 18, 39,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x^{2} - \sqrt{- 4 x^{3} - 4 x^{2} + 1} + 1}{2 x \sqrt{- 4 x^{3} - 4 x^{2} + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: -(n+1)*(2*n-5)*a(n) -2*n*(n-4)*a(n-1) +4*(n-1)*(2*n-3)*a(n-2) +2*(8*n\^{}2-32*n+29)*a(n-3) +4*(n-2)*(2*n-7)*a(n-4)=0. - \emph{R. J. Mathar}, Jan 24 2020
\end{quote}
As of the ``Last modified'' line on the live entry (2022-09-08, revision 20) the
...
\[
p_{0}(n)=- \left(n + 1\right) \left(2 n - 5\right),\qquad p_{1}(n)=- 2 n \left(n - 4\right),\qquad p_{2}(n)=4 \left(n - 1\right) \left(2 n - 3\right),\qquad p_{3}(n)=2 \left(8 n^{2} - 32 n + 29\right),\qquad p_{4}(n)=4 \left(n - 2\right) \left(2 n - 7\right)
\]
...
\section{The computation for A234269}
...
\[
- \left(n + 1\right) \left(2 n - 5\right)\,a(n) -  2 n \left(n - 4\right)\,a(n-1) + 4 \left(n - 1\right) \left(2 n - 3\right)\,a(n-2) + 2 \left(8 n^{2} - 32 n + 29\right)\,a(n-3) + 4 \left(n - 2\right) \left(2 n - 7\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A234269, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{3} + 3 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A234269.
