...
\title{A proof of the conjectured recurrence for OEIS A240558}
...
\begin{abstract}
OEIS A240558 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $10$, a polynomial of degree $0$, which proves
...
OEIS A240558 is ``a(n) = 2\^{}n*n!/((floor(n/2)+1)*floor(n/2)!\^{}2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 4, 24, 32, 320, 320, 4480,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x \sqrt{1 - 16 x^{2}} + 2 x \left(8 x + 1\right) + \sqrt{1 - 16 x^{2}} - 1}{8 x^{2} \sqrt{1 - 16 x^{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+2)*(n\^{}2-5)*a(n) +8*(-2*n-1)*a(n-1) +16*(n-1)*(n\^{}2+2*n-4)*a(n-2)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
...
\[
p_{0}(n)=- \left(n + 2\right) \left(n^{2} - 5\right),\qquad p_{1}(n)=- 8 \left(2 n + 1\right),\qquad p_{2}(n)=16 \left(n - 1\right) \left(n^{2} + 2 n - 4\right)
\]
...
\section{The computation for A240558}
...
\[
- \left(n + 2\right) \left(n^{2} - 5\right)\,a(n) -  8 \left(2 n + 1\right)\,a(n-1) + 16 \left(n - 1\right) \left(n^{2} + 2 n - 4\right)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A240558, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=10$ is an
...
\url{https://oeis.org}, 2026. Sequence A240558.
