...
\title{A proof of the conjectured recurrence for OEIS A135334}
...
\begin{abstract}
OEIS A135334 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 10 x^{3} + 9 x^{2} - 4 x + 2$, a polynomial of degree $3$, which proves
...
OEIS A135334 is ``Number of Dyck paths of semilength n having no UDDU's starting at level 1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 4, 10, 29, 90, 290,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x \left(\left(\sqrt{1 - 4 x} - 1\right)^{2} + 4\right) + \left(\sqrt{1 - 4 x} - 1\right)^{2}}{x \left(\left(\sqrt{1 - 4 x} - 1\right)^{2} + 4\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n+1)*a(n) + 2*(1-5n)*a(n-1) + 3*(3n-1)*a(n-2) + 2*(1-2n)*a(n-3) = 0. - \emph{R. J. Mathar}, Dec 18 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 37) the
...
\[
p_{0}(n)=2 \left(n + 1\right),\qquad p_{1}(n)=- 2 \left(5 n - 1\right),\qquad p_{2}(n)=3 \left(3 n - 1\right),\qquad p_{3}(n)=- 2 \left(2 n - 1\right)
\]
...
\section{The computation for A135334}
...
\[
2 \left(n + 1\right)\,a(n) -  2 \left(5 n - 1\right)\,a(n-1) + 3 \left(3 n - 1\right)\,a(n-2) -  2 \left(2 n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A135334, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 10 x^{3} + 9 x^{2} - 4 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A135334.
