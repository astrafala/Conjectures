...
\title{A proof of the conjectured recurrence for OEIS A158196}
...
\begin{abstract}
OEIS A158196 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $32 x^{2} - 20 x + 3$, a polynomial of degree $2$, which proves
...
OEIS A158196 is ``Expansion of (1-x\^{}2*c(x)\^{}4)/(1-3*x*c(x)\^{}2), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 14, 71, 370, 1950, 10332, 54895,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{16 x^{2} - \left(\sqrt{1 - 4 x} - 1\right)^{4}}{4 x \left(4 x - 3 \left(\sqrt{1 - 4 x} - 1\right)^{2}\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +3*(n+1)*a(n) +2*(-26*n+7)*a(n-1) +16*(18*n-25)*a(n-2) +256*(-2*n+5)*a(n-3)=0. - \emph{R. J. Mathar}, Feb 05 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2015-02-05, revision 4) the
...
\[
p_{0}(n)=3 \left(n + 1\right),\qquad p_{1}(n)=- 2 \left(26 n - 7\right),\qquad p_{2}(n)=16 \left(18 n - 25\right),\qquad p_{3}(n)=- 256 \left(2 n - 5\right)
\]
...
\section{The computation for A158196}
...
\[
3 \left(n + 1\right)\,a(n) -  2 \left(26 n - 7\right)\,a(n-1) + 16 \left(18 n - 25\right)\,a(n-2) -  256 \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A158196, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=32 x^{2} - 20 x + 3$ is an
...
\url{https://oeis.org}, 2026. Sequence A158196.
