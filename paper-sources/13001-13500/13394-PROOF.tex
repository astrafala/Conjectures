...
\title{A proof of the conjectured recurrence for OEIS A182879}
...
\begin{abstract}
OEIS A182879 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A182879 is ``The sum of the lengths of all weighted lattice paths in L\_n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 3, 11, 33, 96, 278, 787,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x \left(- x^{3} + 2 x^{2} + 1\right)}{\left(\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)\right)^{\frac{3}{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n-1)*(310*n-781)*a(n) + (-882*n\^{}2+3845*n-3679)*a(n-1) + (214*n\^{}2-2903*n+3751)*a(n-2) + (-358*n\^{}2-459*n+1955)*a(n-3) + (834*n\^{}2-3631*n+4065)*a(n-4) - (262*n-663)*(n-3)*a(n-5) = 0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 36) the
...
\[
p_{0}(n)=\left(n - 1\right) \left(310 n - 781\right),\qquad p_{1}(n)=- 882 n^{2} + 3845 n - 3679,\qquad p_{2}(n)=214 n^{2} - 2903 n + 3751,\qquad p_{3}(n)=- 358 n^{2} - 459 n + 1955,\qquad p_{4}(n)=834 n^{2} - 3631 n + 4065,\qquad p_{5}(n)=- \left(n - 3\right) \left(262 n - 663\right)
\]
...
\section{The computation for A182879}
...
\[
\left(n - 1\right) \left(310 n - 781\right)\,a(n) + (- 882 n^{2} + 3845 n - 3679)\,a(n-1) + (214 n^{2} - 2903 n + 3751)\,a(n-2) + (- 358 n^{2} - 459 n + 1955)\,a(n-3) + (834 n^{2} - 3631 n + 4065)\,a(n-4) -  \left(n - 3\right) \left(262 n - 663\right)\,a(n-5) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A182879, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A182879.
