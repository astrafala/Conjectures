...
\title{A proof of the conjectured recurrence for OEIS A091699}
...
\begin{abstract}
OEIS A091699 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A091699 is ``Row sums of triangle A091698''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, -1, 3, -9, 30, -109, 420,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{3 x - \sqrt{5 x^{2} + 6 x + 1} + 3} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*n*a(n) + (13*n-16)*a(n-1) + 4*(4*n-7)*a(n-2) + 5*(n-2)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2023-03-17, revision 22) the
...
\[
p_{0}(n)=2 n,\qquad p_{1}(n)=13 n - 16,\qquad p_{2}(n)=4 \left(4 n - 7\right),\qquad p_{3}(n)=5 \left(n - 2\right)
\]
...
\section{The computation for A091699}
...
\[
2 n\,a(n) + (13 n - 16)\,a(n-1) + 4 \left(4 n - 7\right)\,a(n-2) + 5 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A091699, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A091699.
