...
\title{A proof of the conjectured recurrence for OEIS A177175}
...
\begin{abstract}
OEIS A177175 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 10 x^{4} + 28 x^{3} - 24 x^{2} + 7 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A177175 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=6, k=1 and l=-1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 6, 13, 64, 287, 1515, 8143, 46030,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 20 x^{4} + 32 x^{3} - 11 x^{2} - 6 x + 1}{x^{2} - 2 x + 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(2-7*n)*a(n-1) +(19-5*n)*a(n-2) +(43*n-134)*a(n-3) +4*(53-13*n)*a(n-4) +20*(n-5)*a(n-5)=0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=2 - 7 n,\qquad p_{2}(n)=19 - 5 n,\qquad p_{3}(n)=43 n - 134,\qquad p_{4}(n)=- 4 \left(13 n - 53\right),\qquad p_{5}(n)=20 \left(n - 5\right)
\]
...
\section{The computation for A177175}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \frac{20 x^{4} - 32 x^{3} + 11 x^{2} + 6 x - 1}{\left(x - 1\right)^{2}} .
\]
...
\[
B(x)\;=\;- 10 x^{4} + 28 x^{3} - 24 x^{2} + 7 x + 1 ,
\]
...
\[
(n + 1)\,a(n) + (2 - 7 n)\,a(n-1) + (19 - 5 n)\,a(n-2) + (43 n - 134)\,a(n-3) -  4 \left(13 n - 53\right)\,a(n-4) + 20 \left(n - 5\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A177175, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 10 x^{4} + 28 x^{3} - 24 x^{2} + 7 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A177175.
