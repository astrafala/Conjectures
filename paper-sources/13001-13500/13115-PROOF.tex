...
\title{A proof of the conjectured recurrence for OEIS A126180}
...
\begin{abstract}
OEIS A126180 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 38484 x^{2} + 18234 x$, a polynomial of degree $2$, which proves
...
OEIS A126180 is ``Number of branches in all hex trees with n edges (n>=1)''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;3, 11, 48, 224, 1071, 5169, 25053, 121711,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;- \frac{\left(3 x - 1\right) \left(5 x^{2} - 9 x + \left(3 x - 2\right) \sqrt{5 x^{2} - 6 x + 1} + 2\right)}{2 x^{2} \sqrt{5 x^{2} - 6 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -2*(n+2)*(239*n-1252)*a(n) +21*(220*n\^{}2-997*n-578)*a(n-1) +(-12902*n\^{}2+76676*n-77157)*a(n-2) +15*(n-3)*(584*n-2237)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 17 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 15) the
...
\[
p_{0}(n)=- 2 \left(n + 2\right) \left(239 n - 1252\right),\qquad p_{1}(n)=21 \left(220 n^{2} - 997 n - 578\right),\qquad p_{2}(n)=- 12902 n^{2} + 76676 n - 77157,\qquad p_{3}(n)=15 \left(n - 3\right) \left(584 n - 2237\right)
\]
...
\section{The computation for A126180}
...
\[
- 2 \left(n + 2\right) \left(239 n - 1252\right)\,a(n) + 21 \left(220 n^{2} - 997 n - 578\right)\,a(n-1) + (- 12902 n^{2} + 76676 n - 77157)\,a(n-2) + 15 \left(n - 3\right) \left(584 n - 2237\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A126180, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 38484 x^{2} + 18234 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A126180.
