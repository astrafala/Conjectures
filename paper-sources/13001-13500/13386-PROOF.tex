...
\title{A proof of the conjectured recurrence for OEIS A191307}
...
\begin{abstract}
OEIS A191307 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A191307 is ``Sum of the heights of the first peaks in all dispersed Dyck paths of length n (i.e., in Motzkin paths of length n with no (1,0)-steps at positive heights)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 2, 6, 11, 26, 47,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{1 - 4 x^{2}} \left(- x^{2} - x + 1\right) + \left(2 x - 1\right) \left(- x^{2} + x + 1\right)}{2 x^{3} \left(x - 1\right) \left(2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+3)*(n-2)*a(n) +(n\^{}2+3*n-6)*a(n-1) +2*n*(2*n-3)*a(n-2) - 4*n*(n-1)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-27, revision 17) the
...
\[
p_{0}(n)=- \left(n - 2\right) \left(n + 3\right),\qquad p_{1}(n)=n^{2} + 3 n - 6,\qquad p_{2}(n)=2 n \left(2 n - 3\right),\qquad p_{3}(n)=- 4 n \left(n - 1\right)
\]
...
\section{The computation for A191307}
...
\[
- \left(n - 2\right) \left(n + 3\right)\,a(n) + (n^{2} + 3 n - 6)\,a(n-1) + 2 n \left(2 n - 3\right)\,a(n-2) -  4 n \left(n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A191307, agreeing on all 13 terms compared.
...
vanish, for all 33 values of $n$ from $n=3$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A191307.
