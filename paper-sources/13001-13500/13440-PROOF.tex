...
\title{A proof of the conjectured recurrence for OEIS A116409}
...
\begin{abstract}
OEIS A116409 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A116409 is ``Expansion of (1-x-2x\^{}2+sqrt(1-2x-3x\^{}2))/(2(1-x)(1-2x-3x\^{}2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 13, 36, 102, 294, 855,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x^{2} - x + \sqrt{- 3 x^{2} - 2 x + 1} + 1}{2 \left(x - 1\right) \left(3 x^{2} + 2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +2*(-3*n+2)*a(n-1) +2*(4*n-5)*a(n-2) +6*(n-2)*a(n-3) +9*(-n+2)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 11 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2016-05-30, revision 7) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(3 n - 2\right),\qquad p_{2}(n)=2 \left(4 n - 5\right),\qquad p_{3}(n)=6 \left(n - 2\right),\qquad p_{4}(n)=- 9 \left(n - 2\right)
\]
...
\section{The computation for A116409}
...
\[
n\,a(n) -  2 \left(3 n - 2\right)\,a(n-1) + 2 \left(4 n - 5\right)\,a(n-2) + 6 \left(n - 2\right)\,a(n-3) -  9 \left(n - 2\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A116409, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A116409.
