...
\title{A proof of the conjectured recurrence for OEIS A106181}
...
\begin{abstract}
OEIS A106181 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{2} + 3 x$, a polynomial of degree $2$, which proves
...
OEIS A106181 is ``Expansion of c(-x\^{}2)(1+2x-sqrt(1+4x\^{}2))/2, c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, -1, -1, 2, 2, -5, -5,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(\sqrt{4 x^{2} + 1} - 1\right) \left(2 x - \sqrt{4 x^{2} + 1} + 1\right)}{4 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) + n*a(n-1) + 4*(n-1)*a(n-2) + 4*(n-3)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2021-07-29, revision 12) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=n,\qquad p_{2}(n)=4 \left(n - 1\right),\qquad p_{3}(n)=4 \left(n - 3\right)
\]
...
\section{The computation for A106181}
...
\[
(n + 2)\,a(n) + n\,a(n-1) + 4 \left(n - 1\right)\,a(n-2) + 4 \left(n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A106181, agreeing on all 13 terms compared.
...
vanish, for all 37 values of $n$ from $n=3$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{2} + 3 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A106181.
