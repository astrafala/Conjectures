...
\title{A proof of the conjectured recurrence for OEIS A158197}
...
\begin{abstract}
OEIS A158197 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 - 12 x$, a polynomial of degree $1$, which proves
...
OEIS A158197 is ``Expansion of (1-x\^{}2*c(x)\^{}4)/(1-4*x*c(x)\^{}2), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 23, 140, 866, 5388, 33603, 209796,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} - \frac{\left(\sqrt{1 - 4 x} - 1\right)^{4}}{16}}{x \left(x - \left(\sqrt{1 - 4 x} - 1\right)^{2}\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +4*(n+1)\^{}2*a(n) +(-41*n\^{}2-58*n+23)*a(n-1) +50*(n+2)*(2*n-3)*a(n-2)=0. - \emph{R. J. Mathar}, Feb 05 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2026-02-14, revision 6) the
...
\[
p_{0}(n)=4 \left(n + 1\right)^{2},\qquad p_{1}(n)=- 41 n^{2} - 58 n + 23,\qquad p_{2}(n)=50 \left(n + 2\right) \left(2 n - 3\right)
\]
...
\section{The computation for A158197}
...
\[
4 \left(n + 1\right)^{2}\,a(n) + (- 41 n^{2} - 58 n + 23)\,a(n-1) + 50 \left(n + 2\right) \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A158197, agreeing on all 13 terms compared.
...
vanish, for all 19 values of $n$ from $n=2$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=4 - 12 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A158197.
