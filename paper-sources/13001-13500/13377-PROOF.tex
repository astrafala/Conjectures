...
\title{A proof of the conjectured recurrence for OEIS A189053}
...
\begin{abstract}
OEIS A189053 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{2} + x$, a polynomial of degree $2$, which proves
...
OEIS A189053 is ``Expansion of d/dx log(1/(1-x/sqrt(1-4*x\^{}2)))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 7, 9, 41, 61, 225, 369,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{1 - 4 x^{2}}}{16 x^{4} - 8 x^{2} - x \left(1 - 4 x^{2}\right)^{\frac{3}{2}} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(n-1)*a(n-1) +(-13*n+12)*a(n-2) +(-13*n+25)*a(n-3) +4*(14*n-27) *a(n-4) +4*(14*n-41)*a(n-5) +80*(-n+3)*a(n-6) +80*(-n+4)*a(n-7)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-23, revision 19) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=n - 1,\qquad p_{2}(n)=12 - 13 n,\qquad p_{3}(n)=25 - 13 n,\qquad p_{4}(n)=4 \left(14 n - 27\right),\qquad p_{5}(n)=4 \left(14 n - 41\right),\qquad p_{6}(n)=- 80 \left(n - 3\right),\qquad p_{7}(n)=- 80 \left(n - 4\right)
\]
...
\section{The computation for A189053}
...
\[
n\,a(n) + (n - 1)\,a(n-1) + (12 - 13 n)\,a(n-2) + (25 - 13 n)\,a(n-3) + 4 \left(14 n - 27\right)\,a(n-4) + 4 \left(14 n - 41\right)\,a(n-5) -  80 \left(n - 3\right)\,a(n-6) -  80 \left(n - 4\right)\,a(n-7) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A189053, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=x^{2} + x$ is an
...
\url{https://oeis.org}, 2026. Sequence A189053.
