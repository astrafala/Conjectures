...
\title{A proof of the conjectured recurrence for OEIS A081052}
...
\begin{abstract}
OEIS A081052 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A081052 is ``Difference of Stirling numbers of the first kind''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, -4, 17, -85, 499, -3388, 26200,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{\left(2 - \log{\left(x + 1 \right)}\right) \log{\left(x + 1 \right)}}{2 \left(x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) +3*(n-1)*a(n-1) +(3*n\^{}2-9*n+7)*a(n-2) +(n-2)\^{}3*a(n-3)=0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=3 \left(n - 1\right),\qquad p_{2}(n)=3 n^{2} - 9 n + 7,\qquad p_{3}(n)=\left(n - 2\right)^{3}
\]
...
\section{The computation for A081052}
...
\[
1\,a(n) + 3 \left(n - 1\right)\,a(n-1) + (3 n^{2} - 9 n + 7)\,a(n-2) + (n^{3} - 6 n^{2} + 12 n - 8)\,a(n-3) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A081052, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A081052.
