...
\title{A proof of the conjectured recurrence for OEIS A181933}
...
\begin{abstract}
OEIS A181933 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{3} + 4 x^{2} + 2 x$, a polynomial of degree $3$, which proves
...
OEIS A181933 is ``a(n) = Sum\_\{k=0..n\} binomial(n+k,k)*sin(Pi*(n+k)/2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, -3, 9, -30, 106, -385, 1421,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{3 x - \left(x + 1\right) \sqrt{4 x + 1} + 1}{2 \left(4 x^{2} + 5 x - \sqrt{4 x + 1} \left(x^{2} + 3 x + 1\right) + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +2*n*a(n) +8*n*a(n-1) +(-n+20)*a(n-2) +5*(-n+4)*a(n-3) +2*(-2*n+5)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2024-11-11, revision 31) the
...
\[
p_{0}(n)=2 n,\qquad p_{1}(n)=8 n,\qquad p_{2}(n)=20 - n,\qquad p_{3}(n)=- 5 \left(n - 4\right),\qquad p_{4}(n)=- 2 \left(2 n - 5\right)
\]
...
\section{The computation for A181933}
...
\[
2 n\,a(n) + 8 n\,a(n-1) + (20 - n)\,a(n-2) -  5 \left(n - 4\right)\,a(n-3) -  2 \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A181933, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{3} + 4 x^{2} + 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A181933.
