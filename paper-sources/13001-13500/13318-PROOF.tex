...
\title{A proof of the conjectured recurrence for OEIS A190788}
...
\begin{abstract}
OEIS A190788 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $16 x^{7} - 32 x^{6} + 12 x^{5} + 4 x^{4} + 4 x^{3} - 3 x^{2} - 2 x + 1$, a polynomial of degree $7$, which proves
...
OEIS A190788 is ``Expansion of ((x-1)*sqrt(1-4*x\^{}2))/((x-1)*sqrt(1-4*x\^{}2)+x)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 6, 14, 38, 94, 248,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{1 - 4 x^{2}} \left(x - 1\right)}{x + \sqrt{1 - 4 x^{2}} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: (-n+1)*a(n) +(3*n-5)*a(n-1) +2*(3*n-7)*a(n-2) +4*(-6*n+19)*a(n-3) +4*(n-3)*a(n-4) +4*(11*n-53)*a(n-5) +16*(-3*n+16)*a(n-6) +16*(n-6)*a(n-7)=0. - \emph{R. J. Mathar}, Nov 28 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2020-01-30, revision 16) the
...
\[
p_{0}(n)=1 - n,\qquad p_{1}(n)=3 n - 5,\qquad p_{2}(n)=2 \left(3 n - 7\right),\qquad p_{3}(n)=- 4 \left(6 n - 19\right),\qquad p_{4}(n)=4 \left(n - 3\right),\qquad p_{5}(n)=4 \left(11 n - 53\right),\qquad p_{6}(n)=- 16 \left(3 n - 16\right),\qquad p_{7}(n)=16 \left(n - 6\right)
\]
...
\section{The computation for A190788}
...
\[
B(x)\;=\;16 x^{7} - 32 x^{6} + 12 x^{5} + 4 x^{4} + 4 x^{3} - 3 x^{2} - 2 x + 1 ,
\]
...
\[
(1 - n)\,a(n) + (3 n - 5)\,a(n-1) + 2 \left(3 n - 7\right)\,a(n-2) -  4 \left(6 n - 19\right)\,a(n-3) + 4 \left(n - 3\right)\,a(n-4) + 4 \left(11 n - 53\right)\,a(n-5) -  16 \left(3 n - 16\right)\,a(n-6) + 16 \left(n - 6\right)\,a(n-7) \;=\;0
\]
holds for every $n>7$.
\end{theorem}
...
section of A190788, agreeing on all 13 terms compared.
...
vanish, for all 20 values of $n$ from $n=8$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=16 x^{7} - 32 x^{6} + 12 x^{5} + 4 x^{4} + 4 x^{3} - 3 x^{2} - 2 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A190788.
