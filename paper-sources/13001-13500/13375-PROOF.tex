...
\title{A proof of the conjectured recurrence for OEIS A143013}
...
\begin{abstract}
OEIS A143013 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{2} + 3 x + 2$, a polynomial of degree $2$, which proves
...
OEIS A143013 is ``Number of Motzkin n-paths with two kinds of level steps one of which is a final step''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 3, 7, 17, 43, 114, 310,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x - \sqrt{- 4 x^{3} - 3 x^{2} - 2 x + 1} + 1}{2 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) -(2*n+1)*a(n-1) +3*(1-n)*a(n-2) +2*(5-2*n)*a(n-3)=0. - \emph{R. J. Mathar}, Oct 25 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-07-11, revision 38) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 2 n - 1,\qquad p_{2}(n)=- 3 \left(n - 1\right),\qquad p_{3}(n)=- 2 \left(2 n - 5\right)
\]
...
\section{The computation for A143013}
...
\[
(n + 2)\,a(n) + (- 2 n - 1)\,a(n-1) -  3 \left(n - 1\right)\,a(n-2) -  2 \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A143013, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{2} + 3 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A143013.
