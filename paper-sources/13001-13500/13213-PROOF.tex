...
\title{A proof of the conjectured recurrence for OEIS A346074}
...
\begin{abstract}
OEIS A346074 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $5 - 4 x$, a polynomial of degree $1$, which proves
...
OEIS A346074 is ``a(n) = 1 + Sum\_\{k=0..n-5\} a(k) * a(n-k-5)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 1, 1, 1, 2, 3, 4,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x + \sqrt{\left(x - 1\right) \left(4 x^{5} + x - 1\right)} - 1}{2 x^{5} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence (n+5)*a(n) +2*(-n-4)*a(n-1) +(n+3)*a(n-2) +2*(-2*n+5)*a(n-5) +4*(n-3)*a(n-6)=0. - \emph{R. J. Mathar}, Feb 17 2022
\end{quote}
As of the ``Last modified'' line on the live entry (2023-01-22, revision 13) the
...
\[
p_{0}(n)=n + 5,\qquad p_{1}(n)=- 2 \left(n + 4\right),\qquad p_{2}(n)=n + 3,\qquad p_{3}(n)=0,\qquad p_{4}(n)=0,\qquad p_{5}(n)=- 2 \left(2 n - 5\right),\qquad p_{6}(n)=4 \left(n - 3\right)
\]
...
\section{The computation for A346074}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x - 1\right) \left(4 x^{5} + x - 1\right) .
\]
...
\[
(n + 5)\,a(n) -  2 \left(n + 4\right)\,a(n-1) + (n + 3)\,a(n-2) -  2 \left(2 n - 5\right)\,a(n-5) + 4 \left(n - 3\right)\,a(n-6) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A346074, agreeing on all 13 terms compared.
...
vanish, for all 39 values of $n$ from $n=6$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=5 - 4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A346074.
