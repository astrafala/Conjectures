...
\title{A proof of the conjectured recurrence for OEIS A242172}
...
\begin{abstract}
OEIS A242172 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 x + 1$, a polynomial of degree $1$, which proves
...
OEIS A242172 is ``a(n) = 2\^{}n*binomial((n + 1 + (n mod 2))/2, 1/2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 6, 15, 30, 70, 140, 315,\ \dots
\]
The entry posts no generating function of its own. Its NAME states the
relation \texttt{a(n) = A100071(n+2)/2. - \_Michel Marcus\_, Sep 14 2015} to another entry, and that entry's exponential generating
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{1}{2 x} - \frac{1}{x \left(2 x + \sqrt{1 - 4 x^{2}} - 1\right)^{2}} + \frac{1}{x \sqrt{1 - 4 x^{2}} \left(2 x + \sqrt{1 - 4 x^{2}} - 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) -2*a(n-1) +4*(-n-1)*a(n-2)=0. - \emph{R. J. Mathar}, May 11 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2023-11-23, revision 19) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=-2,\qquad p_{2}(n)=- 4 \left(n + 1\right)
\]
...
\section{The computation for A242172}
...
\[
(n + 1)\,a(n) - 2\,a(n-1) -  4 \left(n + 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A242172, agreeing on all 13 terms compared.
...
vanish, for all 31 values of $n$ from $n=2$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=4 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A242172.
