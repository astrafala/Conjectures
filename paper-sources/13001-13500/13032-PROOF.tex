...
\title{A proof of the conjectured recurrence for OEIS A152120}
...
\begin{abstract}
OEIS A152120 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $9 x^{4} - 6 x^{3} - 10 x^{2} + 2 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A152120 is ``a(n) = 2\^{}n * Product\_\{k=1..(n-1)/2\} (2 + 3*cos(k*Pi/n)\^{}2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 4, 22, 56, 290, 748, 3862,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{9 x^{4} - 6 x^{3} - 10 x^{2} + 2 x + 1}{9 x^{4} - 14 x^{2} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = 14*a(n-2) - 9*a(n-4) for n > 4.
\end{quote}
As of the ``Last modified'' line on the live entry (2021-01-07, revision 12) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=0,\qquad p_{2}(n)=-14,\qquad p_{3}(n)=0,\qquad p_{4}(n)=9
\]
...
\section{The computation for A152120}
...
section of A152120, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=9 x^{4} - 6 x^{3} - 10 x^{2} + 2 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A152120.
