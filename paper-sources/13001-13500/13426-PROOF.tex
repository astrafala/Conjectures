...
\title{A proof of the conjectured recurrence for OEIS A254314}
...
\begin{abstract}
OEIS A254314 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 12 x^{4} + 21 x^{3} - 14 x^{2} + 3 x$, a polynomial of degree $4$, which proves
...
OEIS A254314 is ``Hankel transform of a(n) is A006720(n). Hankel transform of a(n+1) is A006720(n+2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 5, 14, 43, 143, 507,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{2} + 6 x + \sqrt{- 4 x^{3} + \left(x^{2} - 4 x + 1\right)^{2}} - 3}{2 \left(2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +2*(-5*n+6)*a(n-1) +2*(17*n-39)*a(n-2) +6*(-8*n+27)*a(n-3) +(25*n-114)*a(n-4) +2*(-n+6)*a(n-5)=0. - \emph{R. J. Mathar}, Jun 07 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2024-03-14, revision 32) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(5 n - 6\right),\qquad p_{2}(n)=2 \left(17 n - 39\right),\qquad p_{3}(n)=- 6 \left(8 n - 27\right),\qquad p_{4}(n)=25 n - 114,\qquad p_{5}(n)=- 2 \left(n - 6\right)
\]
...
\section{The computation for A254314}
...
\[
n\,a(n) -  2 \left(5 n - 6\right)\,a(n-1) + 2 \left(17 n - 39\right)\,a(n-2) -  6 \left(8 n - 27\right)\,a(n-3) + (25 n - 114)\,a(n-4) -  2 \left(n - 6\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A254314, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 12 x^{4} + 21 x^{3} - 14 x^{2} + 3 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A254314.
