...
\title{A proof of the conjectured recurrence for OEIS A071722}
...
\begin{abstract}
OEIS A071722 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $5 x^{2} - 4 x + 6$, a polynomial of degree $2$, which proves
...
OEIS A071722 is ``Expansion of (1+x\^{}2*C\^{}2)*C\^{}3, where C = (1-(1-4*x)\^{}(1/2))/(2*x) is g.f. for Catalan numbers, A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 10, 33, 110, 372, 1276, 4433,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(\sqrt{1 - 4 x} - 1\right)^{3} \left(- \left(\sqrt{1 - 4 x} - 1\right)^{2} - 4\right)}{32 x^{3}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n+3)*a(n) + 4*(-3*n-4)*a(n-1) + (17*n-9)*a(n-2) + 2*(-2*n+5)*a(n-3) = 0. - \emph{R. J. Mathar}, Aug 25 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-04, revision 20) the
...
\[
p_{0}(n)=2 \left(n + 3\right),\qquad p_{1}(n)=- 4 \left(3 n + 4\right),\qquad p_{2}(n)=17 n - 9,\qquad p_{3}(n)=- 2 \left(2 n - 5\right)
\]
...
\section{The computation for A071722}
...
\[
2 \left(n + 3\right)\,a(n) -  4 \left(3 n + 4\right)\,a(n-1) + (17 n - 9)\,a(n-2) -  2 \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A071722, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=5 x^{2} - 4 x + 6$ is an
...
\url{https://oeis.org}, 2026. Sequence A071722.
