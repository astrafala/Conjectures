...
\title{A proof of the conjectured recurrence for OEIS A155051}
...
\begin{abstract}
OEIS A155051 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{2} + 4 x + 2$, a polynomial of degree $2$, which proves
...
OEIS A155051 is ``Expansion of c(x\^{}2)*(1+x)/(1-x), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 3, 4, 6, 8, 13, 18,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x + 1\right) \left(\sqrt{1 - 4 x^{2}} - 1\right)}{2 x^{2} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) -2*a(n-1) +(-5*n+4)*a(n-2) +8*a(n-3) +4*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 05 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2017-09-30, revision 9) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=-2,\qquad p_{2}(n)=4 - 5 n,\qquad p_{3}(n)=8,\qquad p_{4}(n)=4 \left(n - 3\right)
\]
...
\section{The computation for A155051}
...
\[
(n + 2)\,a(n) - 2\,a(n-1) + (4 - 5 n)\,a(n-2) + 8\,a(n-3) + 4 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A155051, agreeing on all 13 terms compared.
...
vanish, for all 31 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{2} + 4 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A155051.
