...
\title{A proof of the conjectured recurrence for OEIS A135582}
...
\begin{abstract}
OEIS A135582 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A135582 is ``Expansion of 1/((1-x\^{}2*c(x))(1-x-2x\^{}2)) where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 7, 18, 39, 95, 232,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{\left(- x \left(\sqrt{1 - 4 x} - 1\right) - 2\right) \left(2 x^{2} + x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+1)*a(n) +6*(n-2)*a(n-1) +(-7*n+19)*a(n-2) +(-7*n+13)*a(n-3) +(13*n-31)*a(n-4) +2*(-n+4)*a(n-5) +4*(-2*n+5)*a(n-6)=0. - \emph{R. J. Mathar}, Feb 23 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2016-10-20, revision 11) the
...
\[
p_{0}(n)=1 - n,\qquad p_{1}(n)=6 \left(n - 2\right),\qquad p_{2}(n)=19 - 7 n,\qquad p_{3}(n)=13 - 7 n,\qquad p_{4}(n)=13 n - 31,\qquad p_{5}(n)=- 2 \left(n - 4\right),\qquad p_{6}(n)=- 4 \left(2 n - 5\right)
\]
...
\section{The computation for A135582}
...
\[
(1 - n)\,a(n) + 6 \left(n - 2\right)\,a(n-1) + (19 - 7 n)\,a(n-2) + (13 - 7 n)\,a(n-3) + (13 n - 31)\,a(n-4) -  2 \left(n - 4\right)\,a(n-5) -  4 \left(2 n - 5\right)\,a(n-6) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A135582, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A135582.
