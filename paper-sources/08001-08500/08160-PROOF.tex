...
\title{The empirical recurrence for OEIS A317226, proved by transfer matrix}
...
\begin{abstract}
OEIS A317226 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,3,6,7,8\}$, and it carries an empirical
...
OEIS A317226 is ``Number of nX4 0..1 arrays with every element unequal to 0, 1, 2, 3, 6, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
8, 49, 150, 490, 2306, 7988, 29512, 118116,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 24 2018 10:29:28, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;7\,a(n-1) - 15\,a(n-2) + 27\,a(n-3) - 109\,a(n-4) + 212\,a(n-5) - 152\,a(n-6) + 262\,a(n-7) - 585\,a(n-8) + 224\,a(n-9) + 232\,a(n-10) + 159\,a(n-11) - 87\,a(n-12) - 588\,a(n-13) + 449\,a(n-14) - 47\,a(n-15) + 166\,a(n-16) - 212\,a(n-17) + 58\,a(n-18) - 10\,a(n-19) + 14\,a(n-20) - 4\,a(n-21)
\end{equation}
for all $n>25$.
...
has score in $\{0,1,2,3,6,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,6,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,6,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,6,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,6,7,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A317226.
