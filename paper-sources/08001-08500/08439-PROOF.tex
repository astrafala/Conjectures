...
\title{The empirical recurrence for OEIS A301660, proved by transfer matrix}
...
\begin{abstract}
OEIS A301660 counts $6$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the four horizontal and vertical neighbours, a number $equal to$ itself lying in $\{0,1,4\}$, and it carries an empirical
...
OEIS A301660 is ``Number of nX6 0..1 arrays with every element equal to 0, 1 or 4 horizontally or vertically adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
13, 14, 26, 45, 99, 254, 696, 2013,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 25 2018 11:27:24, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) - 2\,a(n-2) + 2\,a(n-3) - 14\,a(n-4) - 7\,a(n-5) + 3\,a(n-6) + 44\,a(n-7) + 22\,a(n-8) - 28\,a(n-9) - 45\,a(n-10) - 15\,a(n-11) + 25\,a(n-12) + 22\,a(n-13) + 2\,a(n-14) - 11\,a(n-15) - 4\,a(n-16) + a(n-17) + 2\,a(n-18)
\end{equation}
for all $n>18$.
...
has score in $\{0,1,4\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,4\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,4\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,4\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,4\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A301660.
