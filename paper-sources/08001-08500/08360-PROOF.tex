...
\title{The empirical recurrence for OEIS A299460, proved by transfer matrix}
...
\begin{abstract}
OEIS A299460 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{1,2,3,5,6,8\}$, and it carries an empirical
...
OEIS A299460 is ``Number of nX3 0..1 arrays with every element equal to 1, 2, 3, 5, 6 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 18, 57, 226, 861, 3432, 13268, 51790,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 10 2018 10:43:11, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 4\,a(n-2) + 5\,a(n-3) - 24\,a(n-4) - 10\,a(n-5) + 39\,a(n-6) + 23\,a(n-7) + 35\,a(n-8) - 161\,a(n-9) - 15\,a(n-10) + 115\,a(n-11) + 360\,a(n-12) - 23\,a(n-13) - 236\,a(n-14) - 149\,a(n-15) + 54\,a(n-16) + 33\,a(n-17) - 36\,a(n-18) - 6\,a(n-19) + 4\,a(n-20)
\end{equation}
for all $n>21$.
...
has score in $\{1,2,3,5,6,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,2,3,5,6,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,2,3,5,6,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,2,3,5,6,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,2,3,5,6,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A299460.
