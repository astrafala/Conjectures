...
\title{The empirical recurrence for OEIS A197369, proved by transfer matrix}
...
\begin{abstract}
OEIS A197369 counts $n\times4$ arrays over $\{0,\dots,4\}$ subject to a condition imposed on
every CELL over a neighbour set the entry names, and it carries an empirical recurrence of
order $11$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than
...
walk, and $a(n)$ is a walk count on $S=1169$ vertices, which is $C$-finite. Whether the
...
OEIS A197369 is ``Number of n X 4 0..4 arrays with each element x equal to the number of its horizontal and vertical neighbors equal to 2,4,1,0,1 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
1, 7, 45, 161, 899, 4023, 19387, 92169,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 07:10:01, revision 12) this is still
...
settled by the terminal weight, which evaluates it with $\varnothing$ below. Most of those pairs are indistinguishable, and exactly so. Every offset named above has $|dj|\le 1$, so whether the cell at column $j$ of the middle row is satisfied depends on the three rows only through their windows at $j-1,j,j+1$. Collect for each $j$ the indicator over the window of the row below, and call that tuple $P(r,s)$: the number of cells any $t$ violates is read off $P$, so is the number violated at the bottom edge, and the successor $(s,t)$ carries $P(s,t)$, which does not mention $r$. Hence two pairs $(r,s)$ and $(r',s)$ with the same $P$ have the same outgoing rows and the same successors, and the vertex is $(s,P)$. That leaves $S=1169$ vertices in place of $390625$. Hence,
with $M$ the adjacency matrix on $S=1169$ vertices, $\iota$ the starting vector and $\tau$ the
terminal weight,
\[
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 10\,a(n-2) + 18\,a(n-3) - 7\,a(n-4) - 33\,a(n-5) - 58\,a(n-6) + 13\,a(n-7) + 22\,a(n-8) - 31\,a(n-9) + 18\,a(n-10) - 3\,a(n-11)
\]
for every $n>11$.
\end{theorem}
...
integers vanish from the index corresponding to $n=11+1$ onwards, and the run was
continued until the working vector was identically zero, which settles all larger $j$ at once.
By Lemma~\ref{lem:crit} the recurrence holds for every $n>11$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A197369.
