...
\title{The empirical recurrence for OEIS A230529, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A230529 counts arrays over $\{0,\dots,6\}$ in which every cell is constrained by the
...
OEIS A230529 is ``Number of n X 2 0..6 arrays x(i,j) with each element horizontally or vertically next to at least one element with value (x(i,j)+1) mod 7, and upper left element zero.''. It has offset $1$ and begins
\[
0, 0, 0, 0, 0, 0, 2, 8,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) - 4*a(n-3) - 8*a(n-4) - 12*a(n-5) - 17*a(n-6) - 10*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 19 2025 16:35:11, revision 19) the statement is
...
\exists\ \text{a neighbour with value }(x_{t,u}+1)\bmod 7
\end{equation}
...
$7$ values, so there are about $10^{54}$ of them to sift. The
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230529.
