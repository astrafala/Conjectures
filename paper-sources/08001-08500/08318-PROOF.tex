...
\title{The empirical recurrence for OEIS A301961, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A301961 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A301961 is ``Number of nX5 0..1 arrays with every element equal to 0, 1 or 4 horizontally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
8, 14, 41, 111, 311, 874, 2454, 6906,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) +3*a(n-2) -a(n-3) -2*a(n-4) -3*a(n-5) +2*a(n-6) for n>8
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 29 2018 13:00:02, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 3\,a(n-2) - a(n-3) - 2\,a(n-4) - 3\,a(n-5) + 2\,a(n-6)
\end{equation}
for all $n>8$.
...
The threshold below is exact rather than merely sufficient: at $n=8$ the conjectured
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A301961.
