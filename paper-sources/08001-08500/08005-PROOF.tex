...
\title{The empirical recurrence for OEIS A252257, proved by transfer matrix}
...
\begin{abstract}
OEIS A252257 counts $(n+2)\times3$ arrays over $\{0,\dots,3\}$ subject to a condition imposed on
...
OEIS A252257 is ``Number of (n+2)X(1+2) 0..3 arrays with every 3X3 subblock row and column sum not equal to 0 2 3 6 or 7 and every 3X3 diagonal and antidiagonal sum equal to 0 2 3 6 or 7.''. It has offset $1$ and begins
\[
1072, 1331, 965, 972, 1086, 1187, 1468, 1936,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-3) +a(n-5) -8*a(n-6) -4*a(n-8) +4*a(n-9) +5*a(n-11) -2*a(n-14) for n>27
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:59:06, revision 6) this is still
...
\[
\text{every row, column sum} \notin \{0,2,3,6,7\}\ \text{ and }\ \text{every diagonal, antidiagonal sum} \in \{0,2,3,6,7\}
\]
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-3) + a(n-5) - 8\,a(n-6) - 4\,a(n-8) + 4\,a(n-9) + 5\,a(n-11) - 2\,a(n-14)
\]
for every $n>27$.
\end{theorem}
...
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>27$. The bound is exact: at $n=27$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252257.
