...
\title{The empirical recurrence for OEIS A185900, proved by transfer matrix}
...
\begin{abstract}
OEIS A185900 counts $n\times3$ arrays over $\{0,\dots,7\}$ subject to a condition imposed on
...
walk, and $a(n)$ is a walk count on $S=4088$ vertices, which is $C$-finite. Whether the
...
OEIS A185900 is ``1/8 the number of nX3 0..7 arrays with every element equal to exactly one or two of its horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
1, 196, 7854, 546280, 28967421, 1748982326, 98953708266, 5781843976125,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 03:17:54, revision 11) this is still
...
$\{0,\dots,7\}^{3}$. The neighbourhood of a cell in row $u_i$ meets $u_{i-1}$,
$u_i$ and $u_{i+1}$ and nothing else. Take as vertices the pairs
$(r,s)\in(\{0,\dots,7\}^{3}\cup\{\varnothing\})\times\{0,\dots,7\}^{3}$,
...
settled by the terminal weight, which evaluates it with $\varnothing$ below. Most of those pairs are indistinguishable, and exactly so. Every offset named above has $|dj|\le 1$, so whether the cell at column $j$ of the middle row is satisfied depends on the three rows only through their windows at $j-1,j,j+1$. Collect for each $j$ the indicator over the window of the row below, and call that tuple $P(r,s)$: the number of cells any $t$ violates is read off $P$, so is the number violated at the bottom edge, and the successor $(s,t)$ carries $P(s,t)$, which does not mention $r$. Hence two pairs $(r,s)$ and $(r',s)$ with the same $P$ have the same outgoing rows and the same successors, and the vertex is $(s,P)$. That leaves $S=4088$ vertices in place of $262144$. Hence,
with $M$ the adjacency matrix on $S=4088$ vertices, $\iota$ the starting vector and $\tau$ the
terminal weight,
\[
a(n)\;=\;\tfrac1{8}\,\iota^{\!\top}M^{\,n-1}\tau ,
\]
...
$\tfrac1{8}\iota^{\!\top}M^{\,j}q(M)\tau$ for the corresponding walk index $j$, so the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A185900.
