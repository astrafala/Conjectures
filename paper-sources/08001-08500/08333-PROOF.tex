...
\title{The empirical recurrence for OEIS A235072, proved by transfer matrix}
...
\begin{abstract}
OEIS A235072 counts $2$-column arrays over $\{0,\dots,7\}$ in which every
...
carries an empirical recurrence of order $12$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=56$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-12}q(M)\mathbf{1}$, so the recurrence
...
OEIS A235072 is ``Number of (n+1) X (1+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
288, 1828, 11976, 79848, 534008, 3577048, 23966856,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 61*a(n-2) -813*a(n-4) +4271*a(n-6) -9694*a(n-8) +8264*a(n-10) -1536*a(n-12).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 23:38:51, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;61\,a(n-2) - 813\,a(n-4) + 4271\,a(n-6) - 9694\,a(n-8) + 8264\,a(n-10) - 1536\,a(n-12)
\end{equation}
for all $n>12$, which is every $n$ at which a recurrence of order $12$ can be
stated.
...
Let $V$ consist of those $r\in\{0,\dots,7\}^{2}$ with $r_j\ne r_{j+1}$ for every
$j$ --- the rows in which no two horizontally adjacent entries are equal, and put $S=|V|=56$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge12$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-12}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-12$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-12}\bigl(M^{12}-\sum_i c_i M^{12-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-12$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $12$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=12-12+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>12$.
\end{proof}
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=56$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A235072.
