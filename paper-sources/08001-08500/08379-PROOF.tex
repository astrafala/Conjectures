...
\title{A condition on every subblock: the empirical recurrence for OEIS A181248}
...
\begin{abstract}
OEIS A181248 carries an empirical recurrence of order $7$ contributed by R.~H.~Hardin. It is
...
OEIS A181248 is ``Number of nX5 binary matrices with no 2X2 block having four 1's.''. It has offset $1$ and begins
\[
32, 819, 22077, 587920, 15701273, 419045269, 11185495872, 298561305103,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n)=20*a(n-1)+188*a(n-2)-192*a(n-3)-1660*a(n-4)+2804*a(n-5)-507*a(n-6)-624*a(n-7)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 01:28:01, revision 7) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $5$ columns over the alphabet $\{0,1\}$, growing one row at a time, and the condition it imposes is that no $2\times2$ subblock may contain $4$ ones.

That condition is decided by a bounded window. A $2\times2$ subblock occupies $2$ consecutive rows, so a window of $2$ consecutive rows decides everything that the arrival of a new row settles; nothing outside the window is consulted. Take those windows as the vertices of a digraph, with an edge from $u$ to $v$ when $v$'s new row may follow $u$, and merge vertices with identical futures; that leaves $S=8$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which windows may begin and end an array,
\[
...
\begin{theorem}
\[
a(n)\;=\;20\,a(n-1) + 188\,a(n-2) - 192\,a(n-3) - 1660\,a(n-4) + 2804\,a(n-5) - 507\,a(n-6) - 624\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A181248.
