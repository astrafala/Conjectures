...
\title{The empirical recurrence for OEIS A280603, proved by transfer matrix}
...
\begin{abstract}
OEIS A280603 counts $n\times7$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
every CELL over a neighbour set the entry names, and counted UP TO RELABELLING of the
alphabet; it carries an empirical recurrence of order $17$ contributed by
...
OEIS A280603 is ``Number of nX7 0..1 arrays with no element equal to more than two of its horizontal, vertical and antidiagonal neighbors and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
64, 1063, 1844, 3996, 8031, 15792, 31152, 62349,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 06 2017 11:31:07, revision 4) this is still
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-1) - 8\,a(n-2) + 4\,a(n-3) + 2\,a(n-8) - 2\,a(n-9) - 3\,a(n-10) + 5\,a(n-11) - a(n-12) - a(n-13) - a(n-16) + a(n-17)
\]
for every $n>22$.
\end{theorem}
...
By Lemma~\ref{lem:crit} the recurrence holds for every $n>22$. The bound is exact: at $n=22$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A280603.
