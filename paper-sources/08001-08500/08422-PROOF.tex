...
\title{The empirical recurrence for OEIS A208496, proved by transfer matrix}
...
\begin{abstract}
OEIS A208496 counts arrays over $\{0,\dots,1\}$ of width $3$ that avoid the patterns
$(0,0,0), (0,0,1)$ along one axis and $(0,1,0), (1,1,1)$ along the other, and carries an empirical recurrence of
order $9$ contributed by R.~H.~Hardin. It is true. The patterns have length $3$, so
...
$\mathbf{1}^{\!\top}P^{\,n-2-9}q(P)\mathbf{1}$, which vanishes identically
...
OEIS A208496 is ``Number of n X 3 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 1 0 and 1 1 1 vertically.''. It has offset
$1$ and begins
\[
6, 36, 60, 126, 234, 456, 896,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-2) + 4*a(n-3) + 2*a(n-4) + 2*a(n-5) - 2*a(n-6) + a(n-7) + a(n-9) for n>11.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 06 2018 14:58:49, revision
10) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-2) + 4\,a(n-3) + 2\,a(n-4) + 2\,a(n-5) - 2\,a(n-6) + a(n-7) + a(n-9)
\end{equation}
for all $n>11$. 
...
$(0,0,0), (0,0,1)$ as $3$ consecutive entries. There are $6$ admissible rows; that condition
...
rows, so $S=36$, and declare $w\to w'$ an edge when $w'$ is $w$ with its first entry
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-9}$ whenever $n-9\ge2$. Hence
\eqref{eq:conj} holds for every $n>t$ exactly when $u_j=0$ for every
$j>t-2-9$. Moreover $(u_j)$ satisfies the monic linear recurrence whose
...
$P^{\,n-2-9}$ gives the identity. The equivalence follows by letting the exponent
...
\begin{proof}
The vector $q(P)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $9$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated in turn. They vanish from the
index corresponding to $n=11+1$ onwards, and the run was continued past $S$ consecutive
...
compared against the entry's DATA at every one of the $30$ published indices; the
values agree exactly, including the initial count of $6$ admissible rows. That is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208496.
