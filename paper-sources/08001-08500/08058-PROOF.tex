...
\title{The empirical recurrence for OEIS A317522, proved by transfer matrix}
...
\begin{abstract}
OEIS A317522 counts $5$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,3,4,5,6,8\}$, and it carries an empirical
recurrence of order $22$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A317522 is ``Number of nX5 0..1 arrays with every element unequal to 0, 1, 2, 3, 4, 5, 6 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
16, 512, 14892, 431529, 12547746, 364882328, 10610584063, 308552557709,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 11:38:10, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;30\,a(n-1) - 18\,a(n-2) - 204\,a(n-3) - 1492\,a(n-4) + 132\,a(n-5) + 8866\,a(n-6) + 4220\,a(n-7) - 41133\,a(n-8) - 73867\,a(n-9) + 58901\,a(n-10) + 176892\,a(n-11) + 32562\,a(n-12) + 274\,a(n-13) - 27282\,a(n-14) - 152740\,a(n-15) - 43104\,a(n-16) + 22364\,a(n-17) - 13084\,a(n-18) + 26664\,a(n-19) + 23952\,a(n-20) - 5376\,a(n-21) - 4096\,a(n-22)
\end{equation}
for all $n>22$.
...
has score in $\{0,1,2,3,4,5,6,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,4,5,6,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,4,5,6,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,4,5,6,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,4,5,6,8\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-22}$ whenever $n-22\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-22$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-22}$ out of
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>22$. This is the statement of
Section~1.
\end{theorem}

\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $22$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=22+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A317522.
