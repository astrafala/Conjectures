...
\title{The empirical recurrence for OEIS A299509, proved by transfer matrix}
...
\begin{abstract}
OEIS A299509 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{1,2,4,5,6,8\}$, and it carries an empirical
...
OEIS A299509 is ``Number of nX3 0..1 arrays with every element equal to 1, 2, 4, 5, 6 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 7, 15, 29, 63, 167, 477, 1233,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 11 2018 06:05:58, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 7\,a(n-2) + 4\,a(n-3) - 12\,a(n-4) + 25\,a(n-5) - 22\,a(n-6) + 8\,a(n-7) + 9\,a(n-8) - 7\,a(n-9) + 13\,a(n-10) - 50\,a(n-11) + 64\,a(n-12) - 39\,a(n-13) + 10\,a(n-15)
\end{equation}
for all $n>17$.
...
has score in $\{1,2,4,5,6,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,2,4,5,6,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,2,4,5,6,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,2,4,5,6,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,2,4,5,6,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A299509.
