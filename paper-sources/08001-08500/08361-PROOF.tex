...
\title{The empirical recurrence for OEIS A196598, proved by transfer matrix}
...
\begin{abstract}
OEIS A196598 counts $n\times5$ arrays over $\{0,\dots,4\}$ subject to a condition imposed on
...
walk, and $a(n)$ is a walk count on $S=3841$ vertices, which is $C$-finite. Whether the
...
OEIS A196598 is ``Number of nX5 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 1,2,4,0,3 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
1, 17, 116, 667, 4529, 26558, 170468, 1067723,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 06:37:30, revision 9) this is still
...
settled by the terminal weight, which evaluates it with $\varnothing$ below. Most of those pairs are indistinguishable, and exactly so. Every offset named above has $|dj|\le 1$, so whether the cell at column $j$ of the middle row is satisfied depends on the three rows only through their windows at $j-1,j,j+1$. Collect for each $j$ the indicator over the window of the row below, and call that tuple $P(r,s)$: the number of cells any $t$ violates is read off $P$, so is the number violated at the bottom edge, and the successor $(s,t)$ carries $P(s,t)$, which does not mention $r$. Hence two pairs $(r,s)$ and $(r',s)$ with the same $P$ have the same outgoing rows and the same successors, and the vertex is $(s,P)$. That leaves $S=3841$ vertices in place of $9765625$. Hence,
with $M$ the adjacency matrix on $S=3841$ vertices, $\iota$ the starting vector and $\tau$ the
terminal weight,
\[
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 16\,a(n-2) + 43\,a(n-3) + 120\,a(n-4) + 242\,a(n-5) - 17\,a(n-6) - 727\,a(n-7) - 529\,a(n-8) + 362\,a(n-9) + 378\,a(n-10) + 5\,a(n-11) - 55\,a(n-12) + 5\,a(n-13) - 51\,a(n-14) + 65\,a(n-15) + 15\,a(n-16) - 5\,a(n-17) - 2\,a(n-18) - a(n-19)
\]
for every $n>19$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196598.
