...
\title{The empirical recurrence for OEIS A302737, proved by transfer matrix}
...
\begin{abstract}
OEIS A302737 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the six horizontal, diagonal and antidiagonal neighbours, a number $equal to$ itself lying in $\{0,1,2,3,4\}$, and it carries an empirical
...
OEIS A302737 is ``Number of nX4 0..1 arrays with every element equal to 0, 1, 2, 3 or 4 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
8, 128, 1637, 21625, 286631, 3798398, 50347423, 667361051,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 12 2018 11:18:50, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;12\,a(n-1) + 20\,a(n-2) - 18\,a(n-3) - 326\,a(n-4) - 375\,a(n-5) + 241\,a(n-6) + 1721\,a(n-7) + 1034\,a(n-8) - 798\,a(n-9) - 966\,a(n-10) + 86\,a(n-11) + 278\,a(n-12) - 242\,a(n-13) - 348\,a(n-14) - 112\,a(n-15)
\end{equation}
for all $n>15$.
...
has score in $\{0,1,2,3,4\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,4\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,4\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,4\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,4\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302737.
