...
\title{The empirical recurrence for OEIS A184131, proved by transfer matrix}
...
\begin{abstract}
OEIS A184131 counts arrays over $\{0,\dots,2\}$ whose rows, columns, diagonals or
...
is a walk count on $S=108$ vertices and the recurrence follows from a finite exact
computation.
\end{abstract}
...
OEIS A184131 is ``Number of nX3 0..2 arrays with rows and columns in nondecreasing order.''. It has offset $1$ and begins
\[
10, 112, 1169, 10020, 70243, 414848, 2126064, 9681099,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 02:20:58, revision 9) the statement is
...
bit vector), $|\mathcal V|=S=108$; let $(r,f)\to(s,f')$ be an edge when placing $s$ after $r$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184131.
