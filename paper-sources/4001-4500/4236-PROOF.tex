...
\title{The empirical recurrence for OEIS A235193, proved by transfer matrix}
...
\begin{abstract}
OEIS A235193 counts $4$-column arrays over $\{0,\dots,6\}$ in which every
...
OEIS A235193 is ``Number of (n+1) X (3+1) 0..6 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 5, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
1880, 4724, 12176, 35868, 105208, 333340, 1043256,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 03:34:07, revision 10) the statement is
...
\end{equation}
for all $n>39$, which is every $n$ at which a recurrence of order $39$ can be
stated.
...
$j=0,1,\dots$. They vanish from $j=39-39+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>39$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235193.
