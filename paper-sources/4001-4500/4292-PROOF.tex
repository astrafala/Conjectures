...
\title{The empirical recurrence for OEIS A236973, proved by transfer matrix}
...
\begin{abstract}
OEIS A236973 counts arrays over $\{0,\dots,3\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=74$
...
OEIS A236973 is ``Number of (n+1)X(1+1) 0..3 arrays with the maximum plus the upper median minus the minimum of every 2X2 subblock equal.''. It has offset $1$ and begins
\[
256, 1238, 6024, 31008, 160764, 849024, 4508480, 24165576,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 09:41:59, revision 7) this is still
...
array, which is not among the objects counted. There are $S=74$ states left.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A236973.
