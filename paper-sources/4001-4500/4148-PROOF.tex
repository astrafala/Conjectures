...
and $5$ cells in a row, so $|V|=3^{5}=243$, and the vertex set is
$V\times V$, of size $243^2=59049$. That is the whole of the state: two consecutive
...
publishes, $n=13$, the arrays have $5\cdot13$ cells over
$3$ values, so there are about $10^{31}$ of them to sift. The
walk count replaces that by $59049$ states and one matrix--vector product per row.
