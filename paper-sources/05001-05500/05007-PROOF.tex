...
\title{The empirical column recurrences for the table OEIS A207149}
...
\begin{abstract}
OEIS A207149 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A207149 is ``T(n,k) = Number of (n+1) X (k+1) 0..2 arrays with every 2 X 2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. It has offset $1$, is
read by antidiagonals, and begins
\[
50, 30, 30, 80, 48, 80, 62, 56,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 13:31:42, revision 9) these are still
...
column $k$$=1$ & $3$ & $50$ & $>8$ & $11$ \\
column $k$$=2$ & $3$ & $30$ & $>9$ & $9$ \\
column $k$$=3$ & $3$ & $80$ & $>10$ & $8$ \\
column $k$$=4$ & $3$ & $62$ & $>11$ & $7$ \\
column $k$$=5$ & $3$ & $159$ & $>12$ & $6$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1) X2 0..2 arrays with every 2 X 2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
$S=50$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+1) X3 0..2 arrays with every 2 X 2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X4 0..2 arrays with every 2 X 2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X5 0..2 arrays with every 2 X 2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
$S=62$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + 2\,a(n-2) - 2\,a(n-3)
...
The sequence is ``Number of (n+1) X6 0..2 arrays with every 2 X 2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
$S=159$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + 2\,a(n-2) - 2\,a(n-3)
...
model reproduces $41$ published values, $112$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207149.
