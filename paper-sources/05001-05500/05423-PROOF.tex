...
\title{The empirical recurrence for OEIS A268905, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A268905 counts arrays over $\{0,\dots,2\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=18$ vertices,
...
OEIS A268905 is ``Number of 2 X n 0..2 arrays with some element plus some horizontally or antidiagonally adjacent neighbor totalling two exactly once.''. It has offset $1$ and begins
\[
0, 36, 168, 696, 2664, 9720, 34344, 118584,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) - 9*a(n-2) for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 16 2019 15:03:36, revision 8) the statement is
...
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,2\}^{2},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=18,
\]
...
the digraph dense, with $(2+1)^{2\cdot2}$ edges instead of the sparse set used here.
\end{remark}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268905.
