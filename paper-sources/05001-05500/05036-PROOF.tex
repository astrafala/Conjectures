...
\title{The conjectured generating function for OEIS A250757, proved}
...
\begin{abstract}
OEIS A250757 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250757 is ``Number of (2+1) X (n+1) 0..2 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing x(i,j)+x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
105, 237, 423, 663, 957, 1305,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: 3*x*(35 - 26*x + 9*x\^{}2) / (1 - x)\^{}3.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 18 2018 07:38:46, revision 8) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250757.
