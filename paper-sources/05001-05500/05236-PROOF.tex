...
\title{The empirical closed form for OEIS A203820, proved}
...
\begin{abstract}
OEIS A203820 carries an empirical closed form for $a(n)$ --- not a recurrence relating
consecutive terms but an explicit formula. It is true, and it is decidable rather than
empirical. The entry counts arrays subject to a condition on every $2\times2$ subblock, hence on two consecutive lines, so the lines of an array are the
vertices of a finite digraph, an array is a walk in it, and $a(n)$ is a walk count on
$S=64$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
monic linear recurrence whose characteristic polynomial is
$\prod_j(x-\lambda_j)^{\deg p_j+1}$; here that polynomial is $q(x)=(x-21)$, of degree
$1$. Two things are then checked exactly: that the walk count satisfies the same
recurrence from some index on, which the Cayley--Hamilton theorem reduces to a finite
computation, and that the two sequences agree at $1$ consecutive indices past that
...
OEIS A203820 is ``Number of (n+1)X3 0..3 arrays with no 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. It has offset $1$ and begins
\[
1296, 27216, 571536, 12002256, 252047376, 5292994896, 111152892816, 2334210749136,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 1296*21\^{}(n-1)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:44:31, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;1296 \cdot 21^{n - 1}.
\]

\section{The count is a walk count}

The entry's defining condition is local: it is a condition on every $2\times2$ subblock, hence on two consecutive lines. Take as vertices the
...
walk, so with $M$ the adjacency matrix of that digraph on $S=64$ vertices, and $u,w$ the
...
Here $q(x)=(x-21)$, of degree $1$, and the recurrence it encodes is
...
\begin{proof}
The vector $w'=q(M)w$ was formed by $1$ matrix--vector products in exact integer
arithmetic, and $u^{\!\top}M^{j}w'$ evaluated for $j=0,1,\dots$, again exactly. Those integers
vanish from the point corresponding to $n=2$ onwards, and the run was continued until the
working vector was identically zero, which settles every larger $j$ at once. So $a$ satisfies
\eqref{eq:rec} for every $n>1$. By Lemma~\ref{lem:ann} $f$ satisfies \eqref{eq:rec}
identically. Both were then evaluated in exact arithmetic at the $1$ consecutive indices
$n=2,\dots,2$ and agree at each. A monic recurrence of order $1$
determines $a(m)$ from $a(m-1),\dots,a(m-1)$, and for every $m\ge3$ both
$m>1$ and $m-1\ge2$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge2$.
Below $n=2$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=2$ on. The remaining indices $n=1,\dots,1$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203820.
