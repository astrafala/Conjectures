...
\title{The empirical column recurrences for the table OEIS A263873}
...
\begin{abstract}
OEIS A263873 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A263873 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits, and rows and columns lexicographically nondecreasing.''. It has offset $1$, is
...
k=6: [order 79]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 28 2015 11:50:30, revision 4) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits, and rows and columns lexicographically nondecreasing.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits, and rows and columns lexicographically nondecreasing.''. Its rows are the vertices of a digraph on
$S=4$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of (n+1)X4 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits, and rows and columns lexicographically nondecreasing.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263873.
