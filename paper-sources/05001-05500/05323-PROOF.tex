...
\title{The empirical closed form for OEIS A187513, proved}
...
\begin{abstract}
OEIS A187513 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=11$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A187513 is ``Number of 8-step S, E, and NW-moving king's tours on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 12, 390, 1910, 4880, 9250, 14969,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 669*n\^{}2 - 4316*n + 6681 for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 27 2017 12:49:18, revision 12) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;669 n^{2} - 4316 n + 6681.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=11$ vertices, and $u,w$ the
...
$n=10,\dots,12$ and agree at each. A monic recurrence of order $3$
determines $a(m)$ from $a(m-1),\dots,a(m-3)$, and for every $m\ge13$ both
$m>9$ and $m-3\ge10$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge10$.
Below $n=10$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=10$ on. The remaining indices $n=7,\dots,9$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187513.
