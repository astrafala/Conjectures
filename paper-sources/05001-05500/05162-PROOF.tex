...
\title{The empirical recurrence for OEIS A198279, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A198279 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
OEIS A198279 is ``Number of 2n X 4 0..2 arrays with values 0..2 introduced in row major order and each element equal to exactly two horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
1, 4, 14, 52, 190, 700, 2574, 9476,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) +a(n-2) -8*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (May 14 2018 08:09:13, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) + a(n-2) - 8\,a(n-3)
\end{equation}
for all $n>3$.
...
\eqref{eq:walk2}, divided by $3!$, was evaluated at every one of the $24$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198279.
