...
\title{The empirical recurrence for OEIS A199142, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A199142 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
lumpable, cutting $30$ pairs down to $S=7$ states. The entry's empirical recurrence of
...
OEIS A199142 is ``Number of n X 1 0..3 arrays with values 0..3 introduced in row major order and each element equal to one or two horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 22 2018 12:57:30, revision 8) the statement is
...
Assembling the $4$ lumped chains block-diagonally into $N$ of size $S=7$, with the
...
index,
\[
4!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-5}\,
...
\eqref{eq:walk2}, divided by $4!$, was evaluated at every one of the $32$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A199142.
