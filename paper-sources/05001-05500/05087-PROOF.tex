...
\title{The empirical column recurrences for the table OEIS A262472}
...
\begin{abstract}
OEIS A262472 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A262472 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with each row divisible by 3 and each column divisible by 5, read as a binary number with top and left being the most significant bits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 1, 2, 1, 3, 4, 1, 6,\ \dots
\]
...
k=4: [order 73]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 23 2015 18:52:24, revision 4) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with each row divisible by 3 and each column divisible by 5, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;3\,a(n-1) - 3\,a(n-2) + 3\,a(n-3) - 2\,a(n-4)
\]
...
model reproduces $10$ published values, $19$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262472.
