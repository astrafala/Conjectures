...
\title{The empirical column recurrences for the table OEIS A231227}
...
\begin{abstract}
OEIS A231227 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231227 is ``T(n,k)=Number of (n+2)X(k+2) 0..2 arrays with no element unequal to a strict majority of its horizontal, vertical, diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 3, 4, 3, 6, 8,\ \dots
\]
...
k=4: [order 96]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:19:38, revision 6) these are still
...
The sequence is ``Number of (n+2)X(3) 0..2 arrays with no element unequal to a strict majority of its horizontal, vertical, diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
...
model reproduces $11$ published values, $21$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231227.
