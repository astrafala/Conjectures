...
\title{The empirical recurrence for OEIS A204644, proved by transfer matrix}
...
\begin{abstract}
OEIS A204644 counts arrays of width $2$ over $\{0,1\}$ through two derived arrays of PAIR SUMS ---
each cell added to its left neighbour, and each cell added to the one above it --- on which the
entry imposes an order condition. The entry carries an empirical recurrence of order $3$
contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical: the count is
a walk count on $S=5$ states.
\end{abstract}
...
OEIS A204644 is ``Number of (n+1) X 2 0..1 arrays with column and row pair sums b(i,j)=a(i,j)+a(i,j-1) and c(i,j)=a(i,j)+a(i-1,j) nondecreasing in column and row directions, respectively.''. It has offset $1$ and begins
\[
8, 16, 28, 48, 80, 132, 216, 352,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 05 2019 12:23:51, revision 28) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array $a$ has $2$ cells across, with entries in $\{0,1\}$. Two derived arrays are formed,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204644.
