...
\title{The empirical column recurrences for the table OEIS A222462}
...
\begin{abstract}
OEIS A222462 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A222462 is ``T(n,k) = number of n X k 0..7 arrays with no entry increasing mod 8 by 7 rightwards or downwards, starting with upper left zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 7, 7, 49, 301, 49, 343, 12943,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 28 2017 02:11:00, revision 13) these are still
...
The sequence is ``Number of n X 1 0..7 arrays with no entry increasing mod 8 by 7 rightwards or downwards, starting with upper left zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 2 0..7 arrays with no entry increasing mod 8 by 7 rightwards or downwards, starting with upper left zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 3 0..7 arrays with no entry increasing mod 8 by 7 rightwards or downwards, starting with upper left zero.''. Its rows are the vertices of a digraph on
...
model reproduces $19$ published values, $96$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222462.
