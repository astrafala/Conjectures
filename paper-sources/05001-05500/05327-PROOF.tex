...
\title{The empirical recurrence for OEIS A239249, proved by transfer matrix}
...
\begin{abstract}
OEIS A239249 counts arrays over $\{0,1,\dots,4\}$ in which no entry may equal, modulo $5$, a given
...
count is then a walk count on $S=5$ states.
\end{abstract}
...
OEIS A239249 is ``Number of n X 1 0..4 arrays with no element equal to one plus the sum of elements to its left or one plus the sum of elements above it, modulo 5.''. It has offset $1$ and begins
\[
4, 13, 41, 131, 419, 1339, 4279, 13675,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 2*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 25 2018 12:24:42, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array has $1$ cells across one way, entries $x(i,j)\in\{0,1,\dots,4\}$, and the entry names these
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A239249.
