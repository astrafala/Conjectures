...
\title{The empirical closed form for OEIS A239360, proved}
...
\begin{abstract}
OEIS A239360 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=724$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A239360 is ``Number of nX7 0..2 arrays with no element equal to one plus the sum of elements to its left or one plus the sum of elements above it or one plus the sum of the elements diagonally to its northwest, modulo 3.''. It has offset $1$ and begins
\[
8, 22, 46, 96, 218, 409, 729, 1154,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 7*n\^{}3 - 54*n\^{}2 - 37*n + 1252 for n>16
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 09:26:59, revision 6) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;7 n^{3} - 54 n^{2} - 37 n + 1252.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=724$ vertices, and $u,w$ the
...
$n=21,\dots,24$ and agree at each. A monic recurrence of order $4$
determines $a(m)$ from $a(m-1),\dots,a(m-4)$, and for every $m\ge25$ both
$m>20$ and $m-4\ge21$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge21$.
Below $n=21$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=21$ on. The remaining indices $n=17,\dots,20$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A239360.
