...
\title{The empirical column recurrences for the table OEIS A263558}
...
\begin{abstract}
OEIS A263558 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A263558 is ``T(n,k)=Number of (n+1)X(k+1) 0..4 arrays with each row and column divisible by 7, read as a base-5 number with top and left being the most significant digits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
4, 10, 10, 40, 123, 40, 148, 2210,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 4*a(n-1) +2*a(n-2) -8*a(n-3)\\
k=2: [order 47]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 21 2015 07:02:03, revision 4) these are still
...
The sequence is ``Number of (n+1)X2 0..4 arrays with each row and column divisible by 7, read as a base-5 number with top and left being the most significant digits.''. Its rows are the vertices of a digraph on
...
model reproduces $8$ published values, $24$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263558.
