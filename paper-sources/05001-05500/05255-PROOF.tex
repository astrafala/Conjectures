...
\title{The empirical column recurrences for the table OEIS A299142}
...
\begin{abstract}
OEIS A299142 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A299142 is ``T(n,k) = Number of n X k 0..1 arrays with every element equal to 1, 2, 3, 4, 5 or 7 king-move adjacent elements, with upper left element zero.''. It has offset $1$, is
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 24 2021 15:02:10, revision 6) these are still
...
The sequence is ``Number of n X 1 0..1 arrays with every element equal to 1, 2, 3, 4, 5 or 7 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 2 0..1 arrays with every element equal to 1, 2, 3, 4, 5 or 7 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
model reproduces $19$ published values, $39$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A299142.
