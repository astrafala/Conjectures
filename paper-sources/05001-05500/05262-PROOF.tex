...
\title{The empirical column recurrences for the table OEIS A262795}
...
\begin{abstract}
OEIS A262795 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A262795 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each row divisible by 5 and column not divisible by 5, read as a binary number with top and left being the most significant bits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 36, 0, 76, 144, 0, 344, 528,\ \dots
\]
...
k=3: [order 53]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 02 2015 07:48:31, revision 4) these are still
...
The sequence is ``Number of (n+2)X3 0..1 arrays with each row divisible by 5 and column not divisible by 5, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262795.
