...
\title{The empirical column recurrences for the table OEIS A233155}
...
\begin{abstract}
OEIS A233155 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the rows $n=1, 2, 3, 4$.
\end{abstract}
...
OEIS A233155 is ``T(n,k) = Number of n X k 0..2 arrays with no element x(i,j) adjacent to value 2-x(i,j) horizontally or antidiagonally.''. It has offset $1$, is
read by antidiagonals, and begins
\[
3, 6, 9, 12, 24, 27, 24, 72,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 03 2022 20:38:27, revision 6) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of 1 X n 0..2 arrays with no element x(i,j) adjacent to value 2-x(i,j) horizontally or antidiagonally.''. Its rows are the vertices of a digraph on
$S=1$ vertices, edges being the admissible consecutive pairs, and the count is
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of 2 X n 0..2 arrays with no element x(i,j) adjacent to value 2-x(i,j) horizontally or antidiagonally.''. Its rows are the vertices of a digraph on
$S=2$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1)
...
\subsection*{Row $n$$=3$}
The sequence is ``Number of 3 X n 0..2 arrays with no element x(i,j) adjacent to value 2-x(i,j) horizontally or antidiagonally.''. Its rows are the vertices of a digraph on
$S=5$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;5\,a(n-1) - 2\,a(n-2)
...
\subsection*{Row $n$$=4$}
The sequence is ``Number of 4 X n 0..2 arrays with no element x(i,j) adjacent to value 2-x(i,j) horizontally or antidiagonally.''. Its rows are the vertices of a digraph on
$S=14$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;9\,a(n-1) - 15\,a(n-2) + 6\,a(n-3)
\]
...
model reproduces $30$ published values, $95$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233155.
