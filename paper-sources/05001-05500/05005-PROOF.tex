...
\title{The conjectured generating function for OEIS A250806, proved}
...
\begin{abstract}
OEIS A250806 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250806 is ``Number of (n+1) X (2+1) 0..2 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
100, 379, 1315, 4321, 13735, 42769,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: x*(100 - 221*x + 141*x\^{}2) / ((1 - x)*(1 - 2*x)*(1 - 3*x)). - \_Colin Barker\_, Nov 20 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 20 2018 16:32:01, revision 9) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250806.
