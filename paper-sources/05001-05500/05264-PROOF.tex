...
\title{The conjectured generating function for OEIS A239359, proved}
...
\begin{abstract}
OEIS A239359 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=267$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $290$ coefficients in exact arithmetic
...
OEIS A239359 is ``Number of n X 6 0..2 arrays with no element equal to one plus the sum of elements to its left or one plus the sum of elements above it or one plus the sum of the elements diagonally to its northwest, modulo 3.''. It has offset $1$ and begins
\[
7, 19, 39, 77, 156, 266,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 26 2018 08:07:00, revision 8) this is still
...
\]
and there are $S=267$ states. The digraph is built by reading the entry's own English, and the
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=17$ and $\deg D_2=4$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $284$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{284}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $290$
of them --- more than the $284+1$ the argument requires --- agree.
\end{proof}
...
\[
a(n)\;=\;\sum_{i\ge1}c_i\,a(n-i)\qquad\text{for every }n>17,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A239359.
