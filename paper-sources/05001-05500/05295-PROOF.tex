...
\title{The empirical closed form for OEIS A187176, proved}
...
\begin{abstract}
OEIS A187176 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=14$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A187176 is ``Number of 6-step left-handed knight's tours (moves only out two, left one) on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 0, 0, 0, 88, 496, 1440,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 284*n\^{}2 - 3100*n + 7944 for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 22 2018 17:47:33, revision 13) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;284 n^{2} - 3100 n + 7944.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=14$ vertices, and $u,w$ the
...
$n=13,\dots,15$ and agree at each. A monic recurrence of order $3$
determines $a(m)$ from $a(m-1),\dots,a(m-3)$, and for every $m\ge16$ both
$m>12$ and $m-3\ge13$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge13$.
Below $n=13$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=13$ on. The remaining indices $n=10,\dots,12$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187176.
