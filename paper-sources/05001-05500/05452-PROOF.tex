...
\title{The empirical column recurrences for the table OEIS A298070}
...
\begin{abstract}
OEIS A298070 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A298070 is ``T(n,k) is the number of n X k 0..1 arrays with every element equal to 2, 3, 4 or 6 king-move adjacent elements, with upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 0, 0, 0, 1, 0, 0, 2,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 31 2026 07:53:04, revision 6) these are still
...
The sequence is ``Number of n X 1 0..1 arrays with every element equal to 2, 3, 4 or 6 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 2 0..1 arrays with every element equal to 2, 3, 4 or 6 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
$S=16$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-2) + 4\,a(n-3) + 2\,a(n-4)
\]
...
model reproduces $21$ published values, $30$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298070.
