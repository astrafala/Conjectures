...
\title{The empirical recurrence for OEIS A198474, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A198474 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
OEIS A198474 is ``Number of 2n X 2 0..2 arrays with values 0..2 introduced in row major order and each element equal to an odd number of horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
2, 12, 76, 492, 3212, 21036, 137932, 904812,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 9*a(n-1) - 16*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 02 2018 06:27:16, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;9\,a(n-1) - 16\,a(n-2)
\end{equation}
for all $n>2$.
...
index,
\[
3!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-2}\,
...
index corresponding to $n=2+1$ onwards, and the run was continued until $S+1$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198474.
