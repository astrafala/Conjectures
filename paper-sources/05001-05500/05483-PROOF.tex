...
\title{The empirical recurrence for OEIS A263334, proved by transfer matrix}
...
\begin{abstract}
OEIS A263334 counts arrays of width $3$ over $\{0,1,2\}$ whose rows, read as base-$3$
numbers, are divisible by $13$ and whose columns, read the same way, are divisible by
$13$. The entry carries an empirical recurrence of order $4$ contributed by
...
OEIS A263334 is ``Number of (n+2) X (1+2) 0..2 arrays with each row and column divisible by 13, read as a base-3 number with top and left being the most significant digits.''. It has offset $1$ and begins
\[
3, 7, 19, 57, 169, 505, 1515, 4543,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + a(n-3) - 3*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 01 2019 10:54:43, revision 9) this is still
...
\gamma_j^{(i)}\bmod 13\;=\;
\bigl(3\,(\gamma_j^{(i-1)}\bmod 13)+x(i,j)\bigr)\bmod 13 .
\]
\end{lemma}
...
So the vector $\bigl(\gamma_1\bmod 13,\dots,\gamma_{3}\bmod
13\bigr)$ is a state: at most $13^{3}$ of them, and each array row
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263334.
