...
\title{The empirical recurrence for OEIS A231081, proved by transfer matrix}
...
\begin{abstract}
OEIS A231081 counts arrays of width $2$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
the count is a walk count on $S=165$ states.
\end{abstract}
...
OEIS A231081 is ``Number of 2 X n 0..3 arrays x(i,j) with each element horizontally, diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4 and at least one element with value (x(i,j)-1) mod 4, and upper left element zero.''. It has offset $1$ and begins
\[
0, 2, 14, 40, 356, 1840, 10128, 60808,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 12*a(n-2) + 30*a(n-3) - 64*a(n-4) + 16*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 22 2025 02:36:37, revision 11) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=165.
\]
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 12\,a(n-2) + 30\,a(n-3) - 64\,a(n-4) + 16\,a(n-5)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231081.
