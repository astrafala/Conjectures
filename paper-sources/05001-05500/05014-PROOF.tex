...
\title{The empirical column recurrences for the table OEIS A204713}
...
\begin{abstract}
OEIS A204713 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A204713 is ``T(n,k) is the number of (n+1) X (k+1) 0..1 arrays with the permanents of all 2 X 2 subblocks equal and nonzero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
7, 13, 13, 25, 33, 25, 49, 81,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 19 2021 10:55:41, revision 22) these are still
...
The sequence is ``Number of (n+1) X2 0..1 arrays with the permanents of all 2 X 2 subblocks equal and nonzero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X3 0..1 arrays with the permanents of all 2 X 2 subblocks equal and nonzero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X4 0..1 arrays with the permanents of all 2 X 2 subblocks equal and nonzero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X5 0..1 arrays with the permanents of all 2 X 2 subblocks equal and nonzero.''. Its rows are the vertices of a digraph on
$S=14$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + 13\,a(n-2) + 3\,a(n-3) - 16\,a(n-4)
\]
...
model reproduces $33$ published values, $105$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204713.
