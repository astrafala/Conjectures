...
\title{The empirical recurrence for OEIS A208309, proved by counting patterns}
...
\begin{abstract}
OEIS A208309 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
$2$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A208309 is ``Number of n X 3 0..1 arrays with new values 0..1 introduced in row major order and no element equal to more than one of its immediate leftward or upward neighbors.''. It has offset $1$ and begins
\[
4, 18, 78, 336, 1446, 6222, 26772, 115194,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) - 3*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 30 2018 14:54:01, revision 8) the statement is
...
For every $n$ with $m=n+0-1\ge2$,
\[
2!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-2}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-2}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $2$ matrix--vector products
...
corresponding to $n=2+1$ onwards, and the run continued until $w$ was the zero vector,
...
\eqref{eq:walk}, divided by $2!$, was evaluated at every one of the $22$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208309.
