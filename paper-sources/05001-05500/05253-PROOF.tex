...
\title{The empirical closed form for OEIS A194936, proved}
...
\begin{abstract}
OEIS A194936 counts the lower triangles of a $7\times7$ array over $\{0,\dots,n\}$ in which
adjacent entries differ by at most $1$, and carries closed form recorded as empirical. The shape here is FIXED
...
OEIS A194936 is ``Number of lower triangles of a 7X7 0..n array with no element differing from any of its horizontal or vertical neighbors by more than one.''. It has offset $1$ and begins
\[
268435456, 11761200209, 46128809462, 91397735301, 139323386328, 187700534197,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 06:11:22, revision 9) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

The lower triangle of a $7\times7$ array is the set of cells $(i,j)$ with $0\le j\le
i<7$, so there are $T=28$ of them. Two cells are adjacent when they differ by one of the
horizontal, vertical offsets, which here gives $42$ adjacent pairs. Each cell carries a value in
...
Every $y\in\mathcal P$ has $\mathrm{sp}(y)\le 1\cdot12=12$, where $12$ is the
diameter of the neighbour graph.
\end{lemma}

\begin{proof}
The neighbour graph is connected, so any two cells are joined by a path of at most $12$
edges, and each edge changes the value by at most $1$. Hence the largest and the smallest
entry differ by at most $1\cdot12$.
\end{proof}
...
\[
a(n)\;=\;48439766655\,n-102944165347\qquad\text{for every }n\ge11,
\]
and $\mathcal P$ has exactly $48439766655$ elements.
\end{theorem}

\begin{proof}
For $n\ge12$ every term of Lemma~\ref{lem:trans} is positive by
Lemma~\ref{lem:bound}, so
\[
a(n)=\sum_{y\in\mathcal P}\bigl(n+1-\mathrm{sp}(y)\bigr)
=|\mathcal P|\,(n+1)-\sum_{y}\mathrm{sp}(y),
\]
which is a linear function of $n$. Two values determine it, and $a(12)$ and
$a(12+1)$ were computed exactly, giving slope $48439766655$ and the stated intercept; the slope
is $|\mathcal P|$. That the identity already holds from $n=11$ was then read off by
comparing it with the exact values of $a(n)$ below $12$, each computed the same way.
\end{proof}

The counts $a(n)$ themselves are obtained by walking over the cells in row major order and
carrying only the entries a later cell can still see --- for this shape at most $7+1$ of
...
First, the count reproduces all $16$ terms the entry publishes, exactly, in integer
arithmetic.

Second, the arrays themselves were enumerated for the smallest alphabets --- every assignment
of $0..n$ to the $28$ cells written out and every adjacent pair tested --- with no
...
most'' condition is no condition and the count is $2^{28}$, while ``differing by exactly
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A194936.
