...
\title{The empirical column recurrences for the table OEIS A262420}
...
\begin{abstract}
OEIS A262420 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A262420 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with each row divisible by 3 and column not divisible by 3, read as a binary number with top and left being the most significant bits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 0, 5, 6, 4, 10, 0, 45,\ \dots
\]
...
k=7: [order 19]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 22 2015 09:33:46, revision 7) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with each row divisible by 3 and column not divisible by 3, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..1 arrays with each row divisible by 3 and column not divisible by 3, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X4 0..1 arrays with each row divisible by 3 and column not divisible by 3, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
model reproduces $24$ published values, $65$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262420.
