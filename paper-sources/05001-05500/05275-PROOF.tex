...
\title{The empirical closed form for OEIS A184540, proved}
...
\begin{abstract}
OEIS A184540 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=17$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
\begin{quote}
Empirical: a(n) = (84 + 149*n + 36*n\^{}2 + n\^{}3) / 6. Corrected by \_Colin Barker\_, Apr 12 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 25 2026 16:04:18, revision 11) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;\frac{n^{3}}{6} + 6 n^{2} + \frac{149 n}{6} + 14.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=17$ vertices, and $u,w$ the
