...
\title{The conjectured generating function for OEIS A232042, proved}
...
\begin{abstract}
OEIS A232042 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=50$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $61$ coefficients in exact arithmetic
...
OEIS A232042 is ``Number of n X 3 0..1 arrays with no element less than a strict majority of its horizontal and antidiagonal neighbors.''. It has offset $1$ and begins
\[
4, 15, 80, 446, 2477, 13752,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: x*(4 - x - 16*x\^{}2 - 5*x\^{}3 + 12*x\^{}4) / (1 - 4*x - 9*x\^{}2 + x\^{}3 + 6*x\^{}4). - \_Colin Barker\_, Oct 02 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 02 2018 06:31:54, revision 7) this is still
...
\]
and there are $S=50$ states. The digraph is built by reading the entry's own English, and the
...
The family this entry belongs to is read by one model, described by its author as: ``Transfer matrix for cell-centred conditions over a NAMED neighbour set.''
...
\]
a polynomial of degree at most $55$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{55}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $61$
of them --- more than the $55+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{4}$ read off $D_2$, the first few being $4,\ 9,\ -1,\ -6$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232042.
