...
\title{The empirical recurrence for OEIS A275498, proved by counting patterns}
...
\begin{abstract}
OEIS A275498 counts arrays in which no entry may repeat a value found at any of three stated
...
cutting $98$ pairs to $S=23$ states. The entry's empirical recurrence of order $4$,
...
OEIS A275498 is ``Number of n X 2 0..2 arrays with no element equal to any value at offset (-2,-1) (-2,0) or (-1,-1) and new values introduced in order 0..2.''. It has offset $1$ and begins
\[
2, 9, 16, 28, 56, 104, 200, 380,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) + 2*a(n-2) - a(n-4) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 04 2019 09:08:49, revision 8) the statement is
...
$o=1n_0+0-2$, we have for $j\ge4$
\[
3!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)
=\mathbf{v}^{\!\top}G^{\,j-4}w,\qquad
w=h_{4}-\sum_i c_i h_{4-i} .
\]
...
the survivors by the pattern of the new pair. The vector $w$ was formed by $4$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A275498.
