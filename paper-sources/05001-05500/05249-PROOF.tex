...
\title{The empirical column recurrences for the table OEIS A258921}
...
\begin{abstract}
OEIS A258921 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A258921 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum equal to the antidiagonal sum or central row sum equal to the central column sum.''. It has offset $1$, is
read by antidiagonals, and begins
\[
200, 576, 576, 1680, 1156, 1680, 5184, 1936,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for diagonal and column k:\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 15:32:52, revision 6) these are still
...
The sequence is ``Number of (n+2)X3 0..1 arrays with no 3x3 subblock diagonal sum equal to the antidiagonal sum or central row sum equal to the central column sum.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+2)X6 0..1 arrays with no 3x3 subblock diagonal sum equal to the antidiagonal sum or central row sum equal to the central column sum.''. Its rows are the vertices of a digraph on
...
model reproduces $19$ published values, $76$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A258921.
