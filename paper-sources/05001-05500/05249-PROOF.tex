...
\title{The empirical closed form for OEIS A187305, proved}
...
\begin{abstract}
OEIS A187305 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
OEIS A187305 is ``Number of 10-step one space leftwards or up, two space rightwards or down asymmetric rook's tours on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 0, 3096, 44824, 187924, 505878, 1044900,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 124248*n\^{}2 - 1238146*n + 2807812 for n>17
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 03:54:00, revision 11) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;124248 n^{2} - 1238146 n + 2807812.
\]
...
$n=21,\dots,23$ and agree at each. A monic recurrence of order $3$
determines $a(m)$ from $a(m-1),\dots,a(m-3)$, and for every $m\ge24$ both
$m>20$ and $m-3\ge21$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge21$.
Below $n=21$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=21$ on. The remaining indices $n=18,\dots,20$ were
therefore checked one at a time, directly against the walk count in exact integer arithmetic,
and agree.
\end{proof}

The range is tight in the sense that was checked: at $n=17$ the closed form and the sequence differ, so no larger range is available.
...
at every published term from $n=18$ on, and agrees with the entry's own DATA at each.
...
$n=18$, far past where the proof already settles the matter.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187305.
