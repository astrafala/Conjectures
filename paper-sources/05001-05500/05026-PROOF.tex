...
\title{The conjectured generating function for OEIS A250813, proved}
...
\begin{abstract}
OEIS A250813 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250813 is ``Number of (1+1) X (n+1) 0..2 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
36, 100, 225, 441, 784, 1296,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(36 - 80*x + 85*x\^{}2 - 44*x\^{}3 + 9*x\^{}4) / (1 - x)\^{}5.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 21 2018 06:13:47, revision 8) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250813.
