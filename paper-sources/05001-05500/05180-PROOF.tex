...
\title{The empirical column recurrences for the table OEIS A229685}
...
\begin{abstract}
OEIS A229685 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A229685 is ``T(n,k) = number of defective 3-colorings of an n X k 0..2 array connected diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..2 order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 0, 0, 0, 2, 0, 0, 36,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 28 2021 01:25:38, revision 9) these are still
...
column $k$$=1$ & $1$ & $18$ & $>2$ & $10$ \\
column $k$$=2$ & $3$ & $42$ & $>5$ & $8$ \\
column $k$$=3$ & $3$ & $108$ & $>4$ & $7$ \\
column $k$$=4$ & $3$ & $294$ & $>6$ & $6$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of defective 3-colorings of an n X 1 0..2 array connected diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of defective 3-colorings of an n X 2 0..2 array connected diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of defective 3-colorings of an n X 3 0..2 array connected diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of defective 3-colorings of an n X 4 0..2 array connected diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
model reproduces $31$ published values, $98$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229685.
