...
\title{The empirical closed form for OEIS A187028, proved}
...
\begin{abstract}
OEIS A187028 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
OEIS A187028 is ``Number of 3-step one or two collinear space at a time queen's tours on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 24, 296, 1008, 2232, 3936, 6120, 8784,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 240*n\^{}2 - 936*n + 912 for n>3
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 03:51:12, revision 10) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;240 n^{2} - 936 n + 912.
\]
...
$n=7,\dots,9$ and agree at each. A monic recurrence of order $3$
determines $a(m)$ from $a(m-1),\dots,a(m-3)$, and for every $m\ge10$ both
$m>6$ and $m-3\ge7$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge7$.
Below $n=7$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=7$ on. The remaining indices $n=4,\dots,6$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187028.
