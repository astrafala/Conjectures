...
\title{The conjectured generating function for OEIS A207106, proved}
...
\begin{abstract}
OEIS A207106 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=43$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $52$ coefficients in exact arithmetic
...
OEIS A207106 is ``Number of n X 3 0..1 arrays avoiding 0 0 0 and 0 1 1 horizontally and 0 0 1 and 1 1 0 vertically.''. It has offset $1$ and begins
\[
6, 36, 98, 200, 350, 556,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: 2*x*(3 + 6*x - 5*x\^{}2) / (1 - x)\^{}4.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 22 2018 05:29:13, revision 11) this is still
...
\]
and there are $S=43$ states. The digraph is built by reading the entry's own English, and the
model reproduces all $40$ terms the entry publishes, exactly, in integer arithmetic --- a
...
\]
a polynomial of degree at most $46$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{46}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $52$
of them --- more than the $46+1$ the argument requires --- agree.
\end{proof}
...
First, the model reproduces every one of the $40$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207106.
