...
\title{The conjectured generating function for OEIS A250655, proved}
...
\begin{abstract}
OEIS A250655 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=256$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $262$ coefficients in exact arithmetic
...
OEIS A250655 is ``Number of (n+1) X (7+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
81, 169, 345, 697, 1401, 2809,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: x*(81 - 74*x) / ((1 - x)*(1 - 2*x)). - \_Colin Barker\_, Nov 15 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 15 2018 14:03:47, revision 7) this is still
...
\]
and there are $S=256$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $258$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{258}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $262$
of them --- more than the $258+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{2}$ read off $D_2$, the first few being $3,\ -2$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250655.
