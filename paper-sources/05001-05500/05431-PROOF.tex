...
\title{The conjectured generating function for OEIS A250878, proved}
...
\begin{abstract}
OEIS A250878 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250878 is ``Number of (1+1) X (n+1) 0..3 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing x(i,j)+x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
90, 288, 678, 1328, 2306, 3680,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: 2*x*(45 - 36*x + 33*x\^{}2 - 8*x\^{}3) / (1 - x)\^{}4.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 22 2018 11:34:08, revision 8) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250878.
