...
\title{The empirical recurrence for OEIS A269822, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A269822 counts arrays over $\{0,\dots,4\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=10$ vertices,
...
OEIS A269822 is ``Number of n X 1 0..4 arrays with some element plus some horizontally, diagonally, antidiagonally or vertically adjacent neighbor totalling four exactly once.''. It has offset $1$ and begins
\[
0, 5, 40, 240, 1280, 6400, 30720, 143360,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 8*a(n-1) - 16*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 22 2018 06:52:07, revision 7) the statement is
...
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,4\}^{1},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=10,
\]
...
the digraph dense, with $(4+1)^{2\cdot1}$ edges instead of the sparse set used here.
\end{remark}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269822.
