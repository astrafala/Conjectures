...
\title{The empirical recurrence for OEIS A196339, proved by transfer matrix}
...
\begin{abstract}
OEIS A196339 counts $n\times4$ arrays over $\{0,\dots,4\}$ subject to a condition imposed on
...
walk, and $a(n)$ is a walk count on $S=1496$ vertices, which is $C$-finite. Whether the
...
OEIS A196339 is ``Number of nX4 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 1,4,0,2,3 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
3, 23, 88, 495, 2223, 10854, 53647, 262013,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 06:31:03, revision 9) this is still
...
settled by the terminal weight, which evaluates it with $\varnothing$ below. Most of those pairs are indistinguishable, and exactly so. Every offset named above has $|dj|\le 1$, so whether the cell at column $j$ of the middle row is satisfied depends on the three rows only through their windows at $j-1,j,j+1$. Collect for each $j$ the indicator over the window of the row below, and call that tuple $P(r,s)$: the number of cells any $t$ violates is read off $P$, so is the number violated at the bottom edge, and the successor $(s,t)$ carries $P(s,t)$, which does not mention $r$. Hence two pairs $(r,s)$ and $(r',s)$ with the same $P$ have the same outgoing rows and the same successors, and the vertex is $(s,P)$. That leaves $S=1496$ vertices in place of $390625$. Hence,
with $M$ the adjacency matrix on $S=1496$ vertices, $\iota$ the starting vector and $\tau$ the
terminal weight,
\[
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 7\,a(n-2) + 21\,a(n-3) + 56\,a(n-4) + 84\,a(n-5) - 35\,a(n-6) - 304\,a(n-7) - 675\,a(n-8) - 1177\,a(n-9) - 936\,a(n-10) + 247\,a(n-11) + 1040\,a(n-12) + 616\,a(n-13) - 301\,a(n-14) - 617\,a(n-15) - 204\,a(n-16) + 999\,a(n-17) + 2210\,a(n-18) + 1973\,a(n-19) + 68\,a(n-20) - 1440\,a(n-21) - 1074\,a(n-22) - 47\,a(n-23) + 267\,a(n-24) + 42\,a(n-25) - 22\,a(n-26) + 35\,a(n-27) + 17\,a(n-28) - 14\,a(n-29) - 3\,a(n-30) + 2\,a(n-31)
\]
for every $n>31$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196339.
