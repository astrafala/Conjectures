...
\title{The empirical recurrence for OEIS A160850, derived from the entry's generating function}
...
\begin{abstract}
OEIS A160850 carries an empirical recurrence of order $20$. The entry separately records a
...
OEIS A160850 is ``Number of lines through at least 2 points of a 10 X n grid of points.''. It has offset $0$ and begins
\[
0, 1, 102, 203, 370, 563, 836, 1111,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 25 2017 11:22:26, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 10 x^{21} - 10 x^{20} - 21 x^{19} - 112 x^{18} - 214 x^{17} - 462 x^{16} - 746 x^{15} - 1165 x^{14} - 1531 x^{13} - 1959 x^{12} - 2168 x^{11} - 2334 x^{10} - 2188 x^{9} - 1999 x^{8} - 1571 x^{7} - 1205 x^{6} - 766 x^{5} - 472 x^{4} - 204 x^{3} - 102 x^{2} - x}{x^{20} + x^{18} - x^{15} - x^{14} - 2 x^{13} - x^{12} - x^{11} + x^{9} + x^{8} + 2 x^{7} + x^{6} + x^{5} - x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{20} - x^{18} + x^{15} + x^{14} + 2 x^{13} + x^{12} + x^{11} - x^{9} - x^{8} - 2 x^{7} - x^{6} - x^{5} + x^{2} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;-a(n-2) + a(n-5) + a(n-6) + 2\,a(n-7) + a(n-8) + a(n-9) - a(n-11) - a(n-12) - 2\,a(n-13) - a(n-14) - a(n-15) + a(n-18) + a(n-20)
\]
for every $n>21$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;10 x^{21} + 10 x^{20} + 21 x^{19} + 112 x^{18} + 214 x^{17} + 462 x^{16} + 746 x^{15} + 1165 x^{14} + 1531 x^{13} + 1959 x^{12} + 2168 x^{11} + 2334 x^{10} + 2188 x^{9} + 1999 x^{8} + 1571 x^{7} + 1205 x^{6} + 766 x^{5} + 472 x^{4} + 204 x^{3} + 102 x^{2} + x,
\]
a POLYNOMIAL of degree $21$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $21$, so by
Lemma~\ref{lem:key} the residual $a(n)-\sum_ic_ia(n-i)$ vanishes for every
$n>21$. The bound is exact: at $n=21$ the recurrence fails on the entry's own published terms.
\end{proof}
...
\[
x^{20} + x^{18} - x^{15} - x^{14} - 2 x^{13} - x^{12} - x^{11} + x^{9} + x^{8} + 2 x^{7} + x^{6} + x^{5} - x^{2} - 1,
\]
of degree $20$, and that degree is the order of the SHORTEST linear recurrence with
...
they stop at index $21$ because that polynomial has degree $21$. A reader who trusts
...
indices beyond $n=21$ and holds at every one, and at $n=21$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A160850.
