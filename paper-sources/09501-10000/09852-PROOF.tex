...
\title{The empirical recurrence for OEIS A250390, derived from the entry's generating function}
...
\begin{abstract}
OEIS A250390 carries an empirical recurrence of order $8$. The entry separately records a
...
OEIS A250390 is ``Number of length 4+3 0..n arrays with no four consecutive terms having the maximum of any two terms equal to the minimum of the remaining two terms.''. It has offset $1$ and begins
\[
6, 274, 3384, 21950, 96866, 331128, 944272, 2352468,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 21 2017 05:56:01, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{140 x^{6} + 1094 x^{5} + 2214 x^{4} + 1360 x^{3} + 226 x^{2} + 6 x}{x^{8} - 8 x^{7} + 28 x^{6} - 56 x^{5} + 70 x^{4} - 56 x^{3} + 28 x^{2} - 8 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;140 x^{6} + 1094 x^{5} + 2214 x^{4} + 1360 x^{3} + 226 x^{2} + 6 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250390.
