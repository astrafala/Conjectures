...
\title{The empirical recurrence for OEIS A223270, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223270 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A223270 is ``Rolling cube footprints: number of 2 X n 0..5 arrays starting with 0 where 0..5 label faces of a cube and every array movement to a horizontal, diagonal or antidiagonal neighbor moves across a corresponding cube edge.''. It has offset $1$ and begins
\[
6, 48, 576, 6144, 67584, 737280, 8060928, 88080384,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 18 2018 08:42:58, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;- \frac{6 x}{32 x^{2} + 8 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 32 x^{2} - 8 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223270.
