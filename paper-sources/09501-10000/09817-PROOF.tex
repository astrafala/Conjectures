...
\title{The empirical recurrence for OEIS A213244, derived from the entry's generating function}
...
\begin{abstract}
OEIS A213244 carries an empirical recurrence of order $8$. The entry separately records a
...
OEIS A213244 is ``Number of nonzero elements in GF(2\^{}n) that are 5th powers.''. It has offset $1$ and begins
\[
1, 3, 7, 3, 31, 63, 127, 51,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 11 2025 10:55:02, revision 32) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{8 x^{7} + 12 x^{6} + 14 x^{5} + 3 x^{4} + 7 x^{3} + 3 x^{2} + x}{16 x^{8} - 17 x^{4} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;16 x^{8} - 17 x^{4} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;8 x^{7} + 12 x^{6} + 14 x^{5} + 3 x^{4} + 7 x^{3} + 3 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A213244.
