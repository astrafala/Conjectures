...
\title{The empirical recurrence for OEIS A026975, derived from the entry's generating function}
...
\begin{abstract}
OEIS A026975 carries an empirical recurrence of order $8$. The entry separately records a
...
OEIS A026975 is ``a(n) = Sum\_{k=0..n} (k+1) * A026648(n,k).''. It has offset $0$ and begins
\[
1, 3, 10, 25, 66, 154, 392, 882,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 20 2019 19:48:54, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{7} - 10 x^{5} - 2 x^{4} + x^{3} + 2 x^{2} + 3 x + 1}{4 x^{8} + 16 x^{6} + 12 x^{4} - 8 x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{8} + 16 x^{6} + 12 x^{4} - 8 x^{2} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;8\,a(n-2) - 12\,a(n-4) - 16\,a(n-6) - 4\,a(n-8)
\]
for every $n>7$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 2 x^{7} - 10 x^{5} - 2 x^{4} + x^{3} + 2 x^{2} + 3 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A026975.
