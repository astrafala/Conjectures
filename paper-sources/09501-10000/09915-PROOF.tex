...
\title{The empirical recurrence for OEIS A211439, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211439 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A211439 is ``Number of ordered triples (w,x,y) with all terms in {-n,...,0,...,n} and w+3x+3y=0.''. It has offset $0$ and begins
\[
1, 3, 5, 19, 25, 31, 59, 69,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 15 2017 10:58:20, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} - 2 x^{5} - 2 x^{4} - 12 x^{3} - 2 x^{2} - 2 x - 1}{x^{7} - x^{6} - 2 x^{4} + 2 x^{3} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} + 2 x^{5} + 2 x^{4} + 12 x^{3} + 2 x^{2} + 2 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211439.
