...
\title{The empirical recurrence for OEIS A208826, derived from the entry's generating function}
...
\begin{abstract}
OEIS A208826 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A208826 is ``Number of 5-bead necklaces labeled with numbers -n..n allowing reversal, with sum zero.''. It has offset $1$ and begins
\[
7, 45, 155, 415, 905, 1755, 3085, 5077,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 06 2018 17:11:59, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{7} + 2 x^{6} - 5 x^{5} - 30 x^{4} - 27 x^{3} - 24 x^{2} - 7 x}{x^{7} - 3 x^{6} + x^{5} + 5 x^{4} - 5 x^{3} - x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{7} - 2 x^{6} + 5 x^{5} + 30 x^{4} + 27 x^{3} + 24 x^{2} + 7 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208826.
