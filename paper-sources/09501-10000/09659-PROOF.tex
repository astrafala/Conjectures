...
\title{The empirical recurrence for OEIS A004485, derived from the entry's generating function}
...
\begin{abstract}
OEIS A004485 carries an empirical recurrence of order $13$. The entry separately records a
...
OEIS A004485 is ``Sprague-Grundy values for game of Wyt Queens.''. It has offset $0$ and begins
\[
4, 5, 3, 2, 7, 6, 9, 0,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 30 2026 16:39:47, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{21} + 6 x^{20} + 6 x^{19} - 4 x^{18} + 4 x^{16} - 4 x^{15} + x^{14} + 4 x^{13} - 5 x^{12} - x^{11} + 5 x^{10} + 7 x^{9} + x^{8} - 9 x^{7} + 3 x^{6} - x^{5} + 5 x^{4} - x^{3} - 2 x^{2} + x + 4}{x^{13} - x^{12} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{13} - x^{12} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 8 x^{21} + 6 x^{20} + 6 x^{19} - 4 x^{18} + 4 x^{16} - 4 x^{15} + x^{14} + 4 x^{13} - 5 x^{12} - x^{11} + 5 x^{10} + 7 x^{9} + x^{8} - 9 x^{7} + 3 x^{6} - x^{5} + 5 x^{4} - x^{3} - 2 x^{2} + x + 4,
\]
...
compared against all $62$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A004485.
