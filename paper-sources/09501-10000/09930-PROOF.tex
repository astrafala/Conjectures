...
\title{The empirical recurrence for OEIS A250943, proved by transfer matrix}
...
\begin{abstract}
OEIS A250943 counts arrays over $\{0,1,2\}$ in which a statistic taken over short runs
...
window of consecutive rows, so the count is a walk count on $S=9$ states, and the
...
OEIS A250943 is ``Number of (1+1) X (n+1) 0..2 arrays with nondecreasing x(i,j)+x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
37, 155, 599, 2247, 8189, 29281, 103093, 358683,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) - 6*a(n-2) - 15*a(n-3) + 14*a(n-4) + 12*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 23 2018 21:18:17, revision 11) this is still
...
So a single row is a state. Let $M$ be the matrix on the $3^{2}=9$ rows with
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) - 6\,a(n-2) - 15\,a(n-3) + 14\,a(n-4) + 12\,a(n-5)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250943.
