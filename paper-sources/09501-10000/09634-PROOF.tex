...
\title{The empirical recurrence for OEIS A066337, derived from the entry's generating function}
...
\begin{abstract}
OEIS A066337 carries an empirical recurrence of order $29$. The entry separately records a
...
OEIS A066337 is ``Decimal equivalent of A066327.''. It has offset $0$ and begins
\[
0, 2, 5, 4, 6, 9, 8, 10,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 19 2024 01:59:59, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{28} + 2 x^{27} - x^{24} + 3 x^{23} + 2 x^{22} - x^{21} + 2 x^{20} + 5 x^{19} + x^{18} + 2 x^{17} + 4 x^{16} + 4 x^{15} + 4 x^{14} + 4 x^{13} + 5 x^{12} - 14 x^{11} + 4 x^{10} + 4 x^{9} + 5 x^{8} + x^{7} + 2 x^{6} + 5 x^{5} + 2 x^{4} - x^{3} + 3 x^{2} + 2 x}{x^{29} - x^{28} + x^{25} - x^{24} + x^{21} - x^{20} + x^{17} - x^{16} - x^{13} + x^{12} - x^{9} + x^{8} - x^{5} + x^{4} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{29} - x^{28} + x^{25} - x^{24} + x^{21} - x^{20} + x^{17} - x^{16} - x^{13} + x^{12} - x^{9} + x^{8} - x^{5} + x^{4} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) - a(n-4) + a(n-5) - a(n-8) + a(n-9) - a(n-12) + a(n-13) + a(n-16) - a(n-17) + a(n-20) - a(n-21) + a(n-24) - a(n-25) + a(n-28) - a(n-29)
\]
for every $n>28$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{28} + 2 x^{27} - x^{24} + 3 x^{23} + 2 x^{22} - x^{21} + 2 x^{20} + 5 x^{19} + x^{18} + 2 x^{17} + 4 x^{16} + 4 x^{15} + 4 x^{14} + 4 x^{13} + 5 x^{12} - 14 x^{11} + 4 x^{10} + 4 x^{9} + 5 x^{8} + x^{7} + 2 x^{6} + 5 x^{5} + 2 x^{4} - x^{3} + 3 x^{2} + 2 x,
\]
a POLYNOMIAL of degree $28$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $28$, so by
...
\[
x^{29} - x^{28} + x^{25} - x^{24} + x^{21} - x^{20} + x^{17} - x^{16} - x^{13} + x^{12} - x^{9} + x^{8} - x^{5} + x^{4} - x + 1,
\]
of degree $29$, and that degree is the order of the SHORTEST linear recurrence with
...
they stop at index $28$ because that polynomial has degree $28$. A reader who trusts
...
compared against all $87$ terms the entry publishes: it reproduces every one. This is
...
indices beyond $n=28$ and holds at every one, and at $n=28$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A066337.
