...
\title{The empirical recurrence for OEIS A186388, derived from the entry's generating function}
...
\begin{abstract}
OEIS A186388 carries an empirical recurrence of order $11$. The entry separately records a
...
OEIS A186388 is ``Adjusted joint rank sequence of (f(i)) and (g(j)) with f(i) before g(j) when f(i)=g(j), where f(i)=6i and g(j)=j(j+1)/2 (triangular number).  Complement of A186387.''. It has offset $1$ and begins
\[
1, 2, 4, 5, 7, 9, 11, 14,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 13 2026 01:22:03, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{10} - 2 x^{7} + x^{6} - x^{5} + 2 x^{4} - x^{3} - x}{x^{11} - 2 x^{10} + x^{9} - x^{8} + 2 x^{7} - x^{6} + x^{5} - 2 x^{4} + x^{3} - x^{2} + 2 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{11} + 2 x^{10} - x^{9} + x^{8} - 2 x^{7} + x^{6} - x^{5} + 2 x^{4} - x^{3} + x^{2} - 2 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{10} + 2 x^{7} - x^{6} + x^{5} - 2 x^{4} + x^{3} + x,
\]
...
\[
x^{11} - 2 x^{10} + x^{9} - x^{8} + 2 x^{7} - x^{6} + x^{5} - 2 x^{4} + x^{3} - x^{2} + 2 x - 1,
\]
...
compared against all $93$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A186388.
