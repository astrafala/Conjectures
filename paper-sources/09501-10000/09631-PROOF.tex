...
\title{The empirical recurrence for OEIS A164733, derived from the entry's generating function}
...
\begin{abstract}
OEIS A164733 carries an empirical recurrence of order $31$. The entry separately records a
...
OEIS A164733 is ``Number of n-digit fixed points under the Kaprekar map A151949.''. It has offset $1$ and begins
\[
1, 0, 1, 1, 0, 2, 0, 2,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 01:52:19, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{33} + x^{32} - x^{30} + x^{29} - x^{28} + x^{27} - x^{25} + 2 x^{24} - x^{23} + x^{22} + x^{21} + 2 x^{19} - x^{18} + x^{17} + 2 x^{16} - 3 x^{15} + 2 x^{14} - x^{13} + x^{12} - x^{10} + 2 x^{9} - x^{7} + x^{6} - x^{5} + x^{4} + x}{x^{31} - x^{29} - x^{25} + x^{23} - x^{22} + x^{20} - x^{17} + x^{16} + x^{15} - x^{14} + x^{11} - x^{9} + x^{8} - x^{6} - x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{31} - x^{29} - x^{25} + x^{23} - x^{22} + x^{20} - x^{17} + x^{16} + x^{15} - x^{14} + x^{11} - x^{9} + x^{8} - x^{6} - x^{2} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-2) + a(n-6) - a(n-8) + a(n-9) - a(n-11) + a(n-14) - a(n-15) - a(n-16) + a(n-17) - a(n-20) + a(n-22) - a(n-23) + a(n-25) + a(n-29) - a(n-31)
\]
for every $n>33$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{33} + x^{32} - x^{30} + x^{29} - x^{28} + x^{27} - x^{25} + 2 x^{24} - x^{23} + x^{22} + x^{21} + 2 x^{19} - x^{18} + x^{17} + 2 x^{16} - 3 x^{15} + 2 x^{14} - x^{13} + x^{12} - x^{10} + 2 x^{9} - x^{7} + x^{6} - x^{5} + x^{4} + x,
\]
a POLYNOMIAL of degree $33$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $33$, so by
Lemma~\ref{lem:key} the residual $a(n)-\sum_ic_ia(n-i)$ vanishes for every
$n>33$. The bound is exact: at $n=33$ the recurrence fails on the entry's own published terms.
\end{proof}
...
\[
x^{31} - x^{29} - x^{25} + x^{23} - x^{22} + x^{20} - x^{17} + x^{16} + x^{15} - x^{14} + x^{11} - x^{9} + x^{8} - x^{6} - x^{2} + 1,
\]
of degree $31$, and that degree is the order of the SHORTEST linear recurrence with
...
they stop at index $33$ because that polynomial has degree $33$. A reader who trusts
...
indices beyond $n=33$ and holds at every one, and at $n=33$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A164733.
