...
\title{The empirical recurrence for OEIS A267159, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267159 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A267159 is ``Total number of ON (black) cells after n iterations of the "Rule 107" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 2, 5, 10, 16, 24, 31, 44,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{12} - x^{11} - x^{10} + x^{9} + 4 x^{8} - x^{7} + x^{6} - 2 x^{5} - 2 x^{4} - 4 x^{3} - 2 x^{2} - x - 1}{x^{7} - x^{6} - x^{5} + x^{4} - x^{3} + x^{2} + x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267159.
