...
\title{The empirical recurrence for OEIS A028359, derived from the entry's generating function}
...
\begin{abstract}
OEIS A028359 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A028359 is ``Two-bell analog of A028355.''. It has offset $1$ and begins
\[
1, 2, 12, 121, 212, 1212, 12121, 21212,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 19 2024 01:59:53, revision 25) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{10 x^{5} + 20 x^{4} + 12 x^{3} + 2 x^{2} + x}{100 x^{6} - 101 x^{3} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;100 x^{6} - 101 x^{3} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;10 x^{5} + 20 x^{4} + 12 x^{3} + 2 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A028359.
