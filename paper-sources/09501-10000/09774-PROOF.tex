...
\title{The empirical recurrence for OEIS A230588, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A230588 counts arrays over $\{0,\dots,5\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $4$ contributed
...
OEIS A230588 is ``Number of n X 2 0..5 arrays x(i,j) with each element horizontally or vertically next to at least one element with value (x(i,j)+1) mod 6, and upper left element zero.''. It has offset $1$ and begins
\[
0, 0, 2, 8, 30, 120, 506, 2144,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) -5*a(n-2) +5*a(n-3) +8*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (May 29 2024 21:02:16, revision 11) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 5\,a(n-2) + 5\,a(n-3) + 8\,a(n-4)
\end{equation}
for all $n>5$.
...
\exists\ \text{a neighbour with value }(x_{t,u}+1)\bmod 6
\end{equation}
...
$6$ values, so there are about $10^{42}$ of them to sift. The
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-4}$ out of
...
the middle row. The vector $w$ was formed by $4$ applications of $G$ in exact integer
...
every one of the $27$ published indices, with no shift fitted: the number of rows is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230588.
