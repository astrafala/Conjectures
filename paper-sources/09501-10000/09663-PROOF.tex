...
\title{The empirical recurrence for OEIS A226334, derived from the entry's generating function}
...
\begin{abstract}
OEIS A226334 carries an empirical recurrence of order $13$. The entry separately records a
...
OEIS A226334 is ``Number of nondecreasing -2..2 vectors of length n whose dot product with some other -2..2 vector equals n.''. It has offset $1$ and begins
\[
2, 14, 23, 67, 100, 202, 281, 479,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 16 2018 10:01:20, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{13} + x^{12} + 2 x^{11} - x^{10} + 2 x^{8} - 12 x^{7} - 2 x^{6} - 13 x^{5} - 20 x^{4} - 5 x^{3} - 12 x^{2} - 2 x}{x^{13} - x^{12} - 2 x^{11} + 2 x^{10} - x^{9} + x^{8} + 4 x^{7} - 4 x^{6} - x^{5} + x^{4} - 2 x^{3} + 2 x^{2} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{13} + x^{12} + 2 x^{11} - 2 x^{10} + x^{9} - x^{8} - 4 x^{7} + 4 x^{6} + x^{5} - x^{4} + 2 x^{3} - 2 x^{2} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 2\,a(n-2) - 2\,a(n-3) + a(n-4) - a(n-5) - 4\,a(n-6) + 4\,a(n-7) + a(n-8) - a(n-9) + 2\,a(n-10) - 2\,a(n-11) - a(n-12) + a(n-13)
\]
for every $n>13$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{13} - x^{12} - 2 x^{11} + x^{10} - 2 x^{8} + 12 x^{7} + 2 x^{6} + 13 x^{5} + 20 x^{4} + 5 x^{3} + 12 x^{2} + 2 x,
\]
...
\[
x^{13} - x^{12} - 2 x^{11} + 2 x^{10} - x^{9} + x^{8} + 4 x^{7} - 4 x^{6} - x^{5} + x^{4} - 2 x^{3} + 2 x^{2} + x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A226334.
