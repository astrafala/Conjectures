...
\title{The empirical recurrence for OEIS A267230, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267230 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A267230 is ``Number of length-n 0..6 arrays with no following elements greater than or equal to the first repeated value.''. It has offset $1$ and begins
\[
7, 49, 315, 1981, 12327, 76237, 469623, 2884909,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 06 2018 04:16:47, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 720 x^{8} - 1512 x^{7} + 7476 x^{6} - 8428 x^{5} + 4270 x^{4} - 1099 x^{3} + 140 x^{2} - 7 x}{4320 x^{7} - 11304 x^{6} + 11508 x^{5} - 6034 x^{4} + 1785 x^{3} - 301 x^{2} + 27 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 4320 x^{7} + 11304 x^{6} - 11508 x^{5} + 6034 x^{4} - 1785 x^{3} + 301 x^{2} - 27 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;27\,a(n-1) - 301\,a(n-2) + 1785\,a(n-3) - 6034\,a(n-4) + 11508\,a(n-5) - 11304\,a(n-6) + 4320\,a(n-7)
\]
for every $n>8$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;720 x^{8} + 1512 x^{7} - 7476 x^{6} + 8428 x^{5} - 4270 x^{4} + 1099 x^{3} - 140 x^{2} + 7 x,
\]
...
$n>8$. The bound is exact: at $n=8$ the recurrence fails on the entry's own published terms.
\end{proof}
...
\[
4320 x^{7} - 11304 x^{6} + 11508 x^{5} - 6034 x^{4} + 1785 x^{3} - 301 x^{2} + 27 x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267230.
