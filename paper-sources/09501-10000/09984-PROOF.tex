...
\title{The empirical recurrence for OEIS A202865, derived from the entry's generating function}
...
\begin{abstract}
OEIS A202865 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A202865 is ``Number of 3 X 3 0..n arrays with row and column sums one greater than the previous row and column.''. It has offset $1$ and begins
\[
2, 42, 228, 776, 2046, 4578, 9128, 16704,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2018 10:34:58, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{4} + 6 x^{3} + 30 x^{2} + 2 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 2 x^{4} + 6 x^{3} + 30 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202865.
