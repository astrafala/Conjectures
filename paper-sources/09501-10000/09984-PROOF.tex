...
\title{The empirical recurrence for OEIS A266617, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266617 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A266617 is ``Total number of OFF (white) cells after n iterations of the "Rule 41" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 3, 6, 10, 18, 21, 32, 36,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{5} - 5 x^{4} - x^{3} - 3 x^{2} - 3 x}{x^{7} - x^{6} - x^{5} + x^{4} - x^{3} + x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 4 x^{5} + 5 x^{4} + x^{3} + 3 x^{2} + 3 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266617.
