...
\title{The empirical recurrence for OEIS A027154, derived from the entry's generating function}
...
\begin{abstract}
OEIS A027154 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A027154 is ``a(n) = Sum\_{k=0..floor(n/2)} A027144(n-k, k).''. It has offset $0$ and begins
\[
1, 1, 3, 5, 9, 15, 31, 49,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2025 00:59:19, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{5} - 3 x^{3} - x^{2} - x - 1}{4 x^{6} + 3 x^{4} + 2 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 4 x^{6} - 3 x^{4} - 2 x^{2} + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A027154.
