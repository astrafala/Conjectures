...
\title{The empirical recurrence for OEIS A261958, derived from the entry's generating function}
...
\begin{abstract}
OEIS A261958 carries an empirical recurrence of order $10$. The entry separately records a
...
OEIS A261958 is ``Start with a single square for n=0; for the odd n-th generation add a square at each expandable vertex of the squares of the (n-1)-th generation (this is the "vertex to vertex" version); for the even n-th generation use the "side to vertex" version; a(n) is the number of squares added in the n-th generation.''. It has offset $0$ and begins
\[
1, 4, 12, 16, 24, 32, 28, 36,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 15 2026 09:36:15, revision 37) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{10} + 4 x^{9} + 3 x^{8} + 4 x^{7} + 4 x^{6} + 16 x^{5} + 12 x^{4} + 12 x^{3} + 11 x^{2} + 4 x + 1}{x^{10} - x^{8} - x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{10} + 4 x^{9} + 3 x^{8} + 4 x^{7} + 4 x^{6} + 16 x^{5} + 12 x^{4} + 12 x^{3} + 11 x^{2} + 4 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A261958.
