...
\title{The empirical recurrence for OEIS A103621, derived from the entry's generating function}
...
\begin{abstract}
OEIS A103621 carries an empirical recurrence of order $9$. The entry separately records a
...
OEIS A103621 is ``Trajectory of 7 under repeated application of the map n -> A102370(n).''. It has offset $1$ and begins
\[
7, 9, 11, 13, 23, 25, 27, 61,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 14 2024 02:32:06, revision 34) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{9} + 34 x^{8} + 2 x^{7} + 2 x^{6} + 10 x^{5} + 2 x^{4} + 2 x^{3} + 2 x^{2} + 7 x}{x^{9} - x^{8} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{9} + 34 x^{8} + 2 x^{7} + 2 x^{6} + 10 x^{5} + 2 x^{4} + 2 x^{3} + 2 x^{2} + 7 x,
\]
...
compared against all $56$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A103621.
