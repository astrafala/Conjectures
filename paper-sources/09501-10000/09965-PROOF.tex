...
\title{The empirical recurrence for OEIS A182614, derived from the entry's generating function}
...
\begin{abstract}
OEIS A182614 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A182614 is ``Least k such that floor(k/r\^{}n)=n, where r = golden ratio = (1+sqrt(5))/2.''. It has offset $1$ and begins
\[
2, 6, 13, 28, 56, 108, 204, 376,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 12 2023 20:11:09, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{5} + 2 x^{4} + 3 x^{3} - 2 x^{2} - 2 x}{x^{6} + 2 x^{5} - 2 x^{4} - 4 x^{3} + 2 x^{2} + 2 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{5} - 2 x^{4} - 3 x^{3} + 2 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A182614.
