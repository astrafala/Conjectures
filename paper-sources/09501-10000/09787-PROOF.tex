...
\title{The empirical recurrence for OEIS A299592, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A299592 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $6$ contributed
...
OEIS A299592 is ``Number of n X 5 0..1 arrays with every element unequal to 0, 1 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
8, 8, 19, 53, 113, 256, 541, 1148,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +a(n-2) +3*a(n-3) +a(n-4) -a(n-5) -a(n-6) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 15 2018 12:17:15, revision 11) the statement is
...
\#\{\text{neighbours with a value }different from\ x_{t,u}\}\in\{0,1,4\}
\end{equation}
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-6}$ out of
...
the middle row. The vector $w$ was formed by $6$ applications of $G$ in exact integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A299592.
