...
\title{The empirical recurrence for OEIS A074354, derived from the entry's generating function}
...
\begin{abstract}
OEIS A074354 carries an empirical recurrence of order $8$. The entry separately records a
...
OEIS A074354 is ``Coefficient of q\^{}3 in nu(n), where nu(0)=1, nu(1)=b and, for n>=2, nu(n)=b*nu(n-1)+lambda*(1+q+q\^{}2+...+q\^{}(n-2))*nu(n-2) with (b,lambda)=(1,2).''. It has offset $0$ and begins
\[
0, 0, 0, 0, 0, 14, 64, 218,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 20 2022 07:50:41, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{48 x^{10} + 72 x^{9} - 28 x^{8} - 66 x^{7} + 8 x^{6} + 14 x^{5}}{16 x^{8} + 32 x^{7} - 8 x^{6} - 40 x^{5} + x^{4} + 20 x^{3} - 2 x^{2} - 4 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;16 x^{8} + 32 x^{7} - 8 x^{6} - 40 x^{5} + x^{4} + 20 x^{3} - 2 x^{2} - 4 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) + 2\,a(n-2) - 20\,a(n-3) - a(n-4) + 40\,a(n-5) + 8\,a(n-6) - 32\,a(n-7) - 16\,a(n-8)
\]
for every $n>10$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;48 x^{10} + 72 x^{9} - 28 x^{8} - 66 x^{7} + 8 x^{6} + 14 x^{5},
\]
...
\[
16 x^{8} + 32 x^{7} - 8 x^{6} - 40 x^{5} + x^{4} + 20 x^{3} - 2 x^{2} - 4 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A074354.
