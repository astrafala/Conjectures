...
\title{The empirical recurrence for OEIS A121782, derived from the entry's generating function}
...
\begin{abstract}
OEIS A121782 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A121782 is ``Series expansion for mean-squared radius of gyration of rectangles on square lattice.''. It has offset $1$ and begins
\[
0, 8, 66, 288, 900, 2280, 4998, 9856,\ \dots
\]
...
\begin{quote}
G.f.: 2*x\^{}2*(1 + x)\^{}2*(4 + x) / (1 - x)\^{}6.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 18 2017 08:37:41, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{5} + 12 x^{4} + 18 x^{3} + 8 x^{2}}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{5} + 12 x^{4} + 18 x^{3} + 8 x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A121782.
