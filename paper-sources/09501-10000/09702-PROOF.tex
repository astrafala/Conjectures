...
\title{The empirical recurrence for OEIS A071284, derived from the entry's generating function}
...
\begin{abstract}
OEIS A071284 carries an empirical recurrence of order $10$. The entry separately records a
...
OEIS A071284 is ``Denominators of Peirce sequence of order 4.''. It has offset $0$ and begins
\[
2, 4, 3, 1, 4, 3, 4, 2,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 30 2022 18:19:59, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{9} - 3 x^{8} - 2 x^{7} - 4 x^{6} - 3 x^{5} - 4 x^{4} - x^{3} - 3 x^{2} - 4 x - 2}{x^{10} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - x^{10}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;4 x^{9} + 3 x^{8} + 2 x^{7} + 4 x^{6} + 3 x^{5} + 4 x^{4} + x^{3} + 3 x^{2} + 4 x + 2,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A071284.
