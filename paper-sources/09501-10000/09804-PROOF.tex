...
\title{The empirical recurrence for OEIS A302225, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A302225 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $5$ contributed
...
OEIS A302225 is ``Number of 2Xn 0..1 arrays with every element equal to 1, 2 or 3 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
0, 3, 14, 45, 146, 537, 1934, 6861,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) +3*a(n-2) +6*a(n-3) +10*a(n-4) +4*a(n-5) for n>6
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 03 2018 12:17:15, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 3\,a(n-2) + 6\,a(n-3) + 10\,a(n-4) + 4\,a(n-5)
\end{equation}
for all $n>6$.
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-5}$ out of
...
the middle column. The vector $w$ was formed by $5$ applications of $G$ in exact integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302225.
