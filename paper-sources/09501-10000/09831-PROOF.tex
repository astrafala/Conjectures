...
\title{The empirical recurrence for OEIS A249847, derived from the entry's generating function}
...
\begin{abstract}
OEIS A249847 carries an empirical recurrence of order $8$. The entry separately records a
...
OEIS A249847 is ``Number of length 3+4 0..n arrays with no five consecutive terms having the maximum of any two terms equal to the minimum of the remaining three terms.''. It has offset $1$ and begins
\[
10, 359, 4064, 25206, 108250, 363349, 1022672, 2522796,\ \dots
\]
...
\begin{quote}
G.f.: x*(10 + 279*x + 1472*x\^{}2 + 2186*x\^{}3 + 990*x\^{}4 + 103*x\^{}5) / (1 - x)\^{}8.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 18 2017 18:10:49, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{103 x^{6} + 990 x^{5} + 2186 x^{4} + 1472 x^{3} + 279 x^{2} + 10 x}{x^{8} - 8 x^{7} + 28 x^{6} - 56 x^{5} + 70 x^{4} - 56 x^{3} + 28 x^{2} - 8 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;103 x^{6} + 990 x^{5} + 2186 x^{4} + 1472 x^{3} + 279 x^{2} + 10 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A249847.
