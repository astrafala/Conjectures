...
\title{The empirical recurrence for OEIS A195549, derived from the entry's generating function}
...
\begin{abstract}
OEIS A195549 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A195549 is ``Hypotenuses of primitive Pythagorean triples in A195547 and A195548.''. It has offset $1$ and begins
\[
1, 5, 13, 17, 89, 233, 305, 1597,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 07 2026 14:45:59, revision 32) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} - x^{5} - x^{4} + 13 x^{3} + 5 x^{2} + x}{x^{6} - 18 x^{3} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{6} - 18 x^{3} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} - x^{5} - x^{4} + 13 x^{3} + 5 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195549.
