...
\title{The empirical recurrence for OEIS A211853, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211853 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A211853 is ``Number of nonnegative integer arrays of length 2n+8 with new values 0 upwards introduced in order, no three adjacent elements all unequal, and containing the value n+1.''. It has offset $1$ and begins
\[
1731, 6936, 21897, 57913, 134164, 280751, 542235, 981675,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 20 2018 08:22:12, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 323 x^{7} + 2210 x^{6} - 6435 x^{5} + 10295 x^{4} - 9696 x^{3} + 5181 x^{2} - 1731 x}{x^{7} - 7 x^{6} + 21 x^{5} - 35 x^{4} + 35 x^{3} - 21 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;323 x^{7} - 2210 x^{6} + 6435 x^{5} - 10295 x^{4} + 9696 x^{3} - 5181 x^{2} + 1731 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211853.
