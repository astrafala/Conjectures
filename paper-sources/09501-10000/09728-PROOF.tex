...
\title{The empirical recurrence for OEIS A196324, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A196324 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A196324 is ``Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 1,3,0,4,2 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
2, 10, 27, 68, 206, 573, 1596, 4526,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +3*a(n-2) +5*a(n-3) +3*a(n-4) -a(n-5) +a(n-6) -2*a(n-8) -a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (May 08 2018 09:58:53, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 3\,a(n-2) + 5\,a(n-3) + 3\,a(n-4) - a(n-5) + a(n-6) - 2\,a(n-8) - a(n-9)
\end{equation}
for all $n>10$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(1,3,0,4,2)
\end{equation}
for every cell $(t,u)$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196324.
