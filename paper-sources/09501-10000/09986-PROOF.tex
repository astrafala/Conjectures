...
\title{The empirical recurrence for OEIS A267161, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267161 carries an empirical recurrence of order $7$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{12} + x^{11} + x^{10} - x^{9} - 4 x^{8} + x^{7} - x^{6} + x^{5} - x^{4} - 2 x^{2} - 2 x}{x^{7} - x^{6} - x^{5} + x^{4} - x^{3} + x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{12} - x^{11} - x^{10} + x^{9} + 4 x^{8} - x^{7} + x^{6} - x^{5} + x^{4} + 2 x^{2} + 2 x,
\]
...
compared against all $54$ terms the entry publishes: it reproduces every one. This is
