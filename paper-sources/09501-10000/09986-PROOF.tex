...
\title{The empirical recurrence for OEIS A028841, derived from the entry's generating function}
...
\begin{abstract}
OEIS A028841 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A028841 is ``Iterated sum of digits of n is a Fibonacci number.''. It has offset $1$ and begins
\[
1, 2, 3, 5, 8, 10, 11, 12,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 28 2020 09:52:30, revision 28) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} + 3 x^{5} + 2 x^{4} + x^{3} + x^{2} + x}{x^{6} - x^{5} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{6} - x^{5} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} + 3 x^{5} + 2 x^{4} + x^{3} + x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A028841.
