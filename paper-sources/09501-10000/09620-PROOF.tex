...
\title{The empirical recurrence for OEIS A300374, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A300374 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A300374 is ``Number of nX2 0..1 arrays with every element equal to 0, 2 or 3 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
1, 3, 2, 10, 13, 42, 74, 188,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-2) +2*a(n-3) +a(n-5) -3*a(n-6) +a(n-7)
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 04 2018 09:15:37, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-2) + 2\,a(n-3) + a(n-5) - 3\,a(n-6) + a(n-7)
\end{equation}
for all $n>8$.
...
publishes, $n=31$, the arrays have $2\cdot31$ cells over
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A300374.
