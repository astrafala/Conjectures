...
\title{The empirical recurrence for OEIS A220932, derived from the entry's generating function}
...
\begin{abstract}
OEIS A220932 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A220932 is ``Equals two maps: number of n X 3 binary arrays indicating the locations of corresponding elements equal to exactly two of their horizontal and antidiagonal neighbors in a random 0..2 n X 3 array.''. It has offset $1$ and begins
\[
2, 16, 92, 556, 3332, 19996, 119972, 719836,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 13 2018 16:33:46, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{2} - 2 x}{6 x^{2} + 5 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 6 x^{2} - 5 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220932.
