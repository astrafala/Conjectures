...
\title{The empirical recurrence for OEIS A027117, derived from the entry's generating function}
...
\begin{abstract}
OEIS A027117 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A027117 is ``a(n) = A027113(n, n+4).''. It has offset $4$ and begins
\[
16, 59, 155, 352, 738, 1475, 2861, 5444,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 23 2019 20:33:10, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{8} - 5 x^{7} - x^{6} - 5 x^{5} + 16 x^{4}}{x^{6} - 2 x^{5} + x^{4} - 2 x^{3} + 5 x^{2} - 4 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{6} - 2 x^{5} + x^{4} - 2 x^{3} + 5 x^{2} - 4 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) - 5\,a(n-2) + 2\,a(n-3) - a(n-4) + 2\,a(n-5) - a(n-6)
\]
for every $n>8$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{8} - 5 x^{7} - x^{6} - 5 x^{5} + 16 x^{4},
\]
...
\[
x^{6} - 2 x^{5} + x^{4} - 2 x^{3} + 5 x^{2} - 4 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A027117.
