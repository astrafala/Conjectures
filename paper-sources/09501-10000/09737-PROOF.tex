...
\title{The empirical recurrence for OEIS A131662, derived from the entry's generating function}
...
\begin{abstract}
OEIS A131662 carries an empirical recurrence of order $9$. The entry separately records a
...
OEIS A131662 is ``Numbers n where either n or n+1 is divisible by the numbers from 1 to 12.''. It has offset $1$ and begins
\[
2519, 11879, 13320, 14399, 15840, 25200, 27719, 27720,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 14 2024 12:28:58, revision 15) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{8} + 2519 x^{7} + 9360 x^{6} + 1441 x^{5} + 1079 x^{4} + 1441 x^{3} + 9360 x^{2} + 2519 x}{x^{9} - x^{8} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{8} + 2519 x^{7} + 9360 x^{6} + 1441 x^{5} + 1079 x^{4} + 1441 x^{3} + 9360 x^{2} + 2519 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A131662.
