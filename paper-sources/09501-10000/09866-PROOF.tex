...
\title{The empirical recurrence for OEIS A267534, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267534 carries an empirical recurrence of order $8$. The entry separately records a
...
OEIS A267534 is ``Indices of Lucas numbers that are not of the form x\^{}2 + y\^{}2 + z\^{}2 where x, y and z are integers.''. It has offset $1$ and begins
\[
4, 8, 11, 16, 20, 21, 23, 28,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 30 2016 14:21:02, revision 24) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{8} + 2 x^{7} + x^{6} + 4 x^{5} + 5 x^{4} + 3 x^{3} + 4 x^{2} + 4 x}{x^{8} - x^{7} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{8} - x^{7} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{8} + 2 x^{7} + x^{6} + 4 x^{5} + 5 x^{4} + 3 x^{3} + 4 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267534.
