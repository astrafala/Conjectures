...
\title{The empirical recurrence for OEIS A195523, derived from the entry's generating function}
...
\begin{abstract}
OEIS A195523 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A195523 is ``Number of lower triangles of a 5 X 5 -n..n array with all row and column sums zero.''. It has offset $1$ and begins
\[
199, 3753, 27267, 121367, 401565, 1089411, 2563933, 5423365,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 08 2018 06:20:24, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{7} + 6 x^{6} - 213 x^{5} - 2346 x^{4} - 5175 x^{3} - 2360 x^{2} - 199 x}{x^{7} - 7 x^{6} + 21 x^{5} - 35 x^{4} + 35 x^{3} - 21 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{7} - 6 x^{6} + 213 x^{5} + 2346 x^{4} + 5175 x^{3} + 2360 x^{2} + 199 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195523.
