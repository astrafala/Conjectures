...
\title{The empirical recurrence for OEIS A211435, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211435 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A211435 is ``Number of ordered triples (w,x,y) with all terms in {-n,...,0,...,n} and w+4x+5y=0.''. It has offset $0$ and begins
\[
1, 3, 5, 11, 17, 25, 35, 45,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 23 2022 01:21:37, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} - x^{5} - 4 x^{3} - x - 1}{x^{7} - 2 x^{6} + x^{5} - x^{2} + 2 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{7} + 2 x^{6} - x^{5} + x^{2} - 2 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211435.
