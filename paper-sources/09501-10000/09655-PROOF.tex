...
\title{The empirical recurrence for OEIS A244241, derived from the entry's generating function}
...
\begin{abstract}
OEIS A244241 carries an empirical recurrence of order $15$. The entry separately records a
...
OEIS A244241 is ``Number of partitions of n into 5 parts such that every i-th smallest part (counted with multiplicity) is different from i.''. It has offset $20$ and begins
\[
1, 5, 11, 19, 29, 39, 53, 67,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 28 2025 09:00:06, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{40} + 2 x^{36} + 2 x^{35} + 4 x^{34} + x^{33} - 3 x^{32} - 7 x^{31} - 9 x^{30} - 5 x^{29} + 2 x^{28} + 8 x^{27} + 9 x^{26} + 8 x^{25} + x^{24} - 3 x^{23} - 5 x^{22} - 4 x^{21} - x^{20}}{x^{15} - x^{14} - x^{13} + x^{10} + x^{9} + x^{8} - x^{7} - x^{6} - x^{5} + x^{2} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{15} + x^{14} + x^{13} - x^{10} - x^{9} - x^{8} + x^{7} + x^{6} + x^{5} - x^{2} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-2) - a(n-5) - a(n-6) - a(n-7) + a(n-8) + a(n-9) + a(n-10) - a(n-13) - a(n-14) + a(n-15)
\]
for every $n>40$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{40} - 2 x^{36} - 2 x^{35} - 4 x^{34} - x^{33} + 3 x^{32} + 7 x^{31} + 9 x^{30} + 5 x^{29} - 2 x^{28} - 8 x^{27} - 9 x^{26} - 8 x^{25} - x^{24} + 3 x^{23} + 5 x^{22} + 4 x^{21} + x^{20},
\]
a POLYNOMIAL of degree $40$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $40$, so by
Lemma~\ref{lem:key} the residual $a(n)-\sum_ic_ia(n-i)$ vanishes for every
$n>40$. The bound is exact: at $n=40$ the recurrence fails on the entry's own published terms.
\end{proof}
...
\]
of degree $15$, and that degree is the order of the SHORTEST linear recurrence with
...
they stop at index $40$ because that polynomial has degree $40$. A reader who trusts
...
compared against all $49$ terms the entry publishes: it reproduces every one. This is
...
indices beyond $n=40$ and holds at every one, and at $n=40$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A244241.
