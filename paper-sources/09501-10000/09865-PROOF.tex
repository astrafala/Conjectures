...
\title{The empirical recurrence for OEIS A267514, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267514 carries an empirical recurrence of order $8$. The entry separately records a
...
OEIS A267514 is ``Binary representation of the middle column of the "Rule 137" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 10, 101, 1010, 10101, 101010, 1010100, 10101001,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} + x^{2} + 1}{10 x^{8} - x^{7} - 10 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;10 x^{8} - x^{7} - 10 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267514.
