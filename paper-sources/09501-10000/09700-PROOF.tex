...
\title{The empirical recurrence for OEIS A025093, derived from the entry's generating function}
...
\begin{abstract}
OEIS A025093 carries an empirical recurrence of order $10$. The entry separately records a
...
OEIS A025093 is ``a(n) = s(1)t(n) + s(2)t(n-1) + ... + s(k)t(n-k+1), where k = [ n/2 ], s = (Lucas numbers), t = (odd natural numbers).''. It has offset $1$ and begins
\[
3, 5, 22, 30, 66, 82, 161, 191,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 07 2019 06:52:57, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{8} - 2 x^{7} - x^{6} - 5 x^{5} - 7 x^{4} + 9 x^{3} - x^{2} + 3 x}{x^{10} - 2 x^{9} + 3 x^{8} - 4 x^{7} + x^{6} + 2 x^{5} - 3 x^{4} + 4 x^{3} - x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{10} - 2 x^{9} + 3 x^{8} - 4 x^{7} + x^{6} + 2 x^{5} - 3 x^{4} + 4 x^{3} - x^{2} - 2 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + a(n-2) - 4\,a(n-3) + 3\,a(n-4) - 2\,a(n-5) - a(n-6) + 4\,a(n-7) - 3\,a(n-8) + 2\,a(n-9) - a(n-10)
\]
for every $n>8$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 2 x^{8} - 2 x^{7} - x^{6} - 5 x^{5} - 7 x^{4} + 9 x^{3} - x^{2} + 3 x,
\]
...
\[
x^{10} - 2 x^{9} + 3 x^{8} - 4 x^{7} + x^{6} + 2 x^{5} - 3 x^{4} + 4 x^{3} - x^{2} - 2 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A025093.
