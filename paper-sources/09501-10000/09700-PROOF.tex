...
\title{The empirical recurrence for OEIS A115792, derived from the entry's generating function}
...
\begin{abstract}
OEIS A115792 carries an empirical recurrence of order $10$. The entry separately records a
...
OEIS A115792 is ``a(n) = ceiling(g(A000073(n))) with g(k) = (k-1)\^{}2/(4k).''. It has offset $2$ and begins
\[
0, 0, 1, 1, 2, 3, 6, 11,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 17 2025 09:42:32, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} - x^{5} - x^{4}}{x^{10} + 2 x^{9} + 3 x^{8} + 2 x^{7} + 2 x^{6} + 2 x^{5} + 2 x^{4} + 2 x^{3} + x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{10} - 2 x^{9} - 3 x^{8} - 2 x^{7} - 2 x^{6} - 2 x^{5} - 2 x^{4} - 2 x^{3} - x^{2} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-2) + 2\,a(n-3) + 2\,a(n-4) + 2\,a(n-5) + 2\,a(n-6) + 2\,a(n-7) + 3\,a(n-8) + 2\,a(n-9) + a(n-10)
\]
for every $n>6$.
\end{theorem}
...
\[
x^{10} + 2 x^{9} + 3 x^{8} + 2 x^{7} + 2 x^{6} + 2 x^{5} + 2 x^{4} + 2 x^{3} + x^{2} - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A115792.
