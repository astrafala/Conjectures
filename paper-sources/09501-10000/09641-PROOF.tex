...
\title{The empirical recurrence for OEIS A111002, derived from the entry's generating function}
...
\begin{abstract}
OEIS A111002 carries an empirical recurrence of order $21$. The entry separately records a
...
OEIS A111002 is ``a(n) = gcd(f(n), f(n+1)) where f(n) = n\^{}4 + n\^{}2 + 1.''. It has offset $0$ and begins
\[
1, 3, 7, 91, 21, 31, 43, 57,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 08 2022 08:45:20, revision 26) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{20} - 3 x^{19} - 7 x^{18} - 91 x^{17} - 21 x^{16} - 31 x^{15} - 43 x^{14} - 54 x^{13} - 64 x^{12} - 70 x^{11} - 504 x^{10} - 70 x^{9} - 64 x^{8} - 54 x^{7} - 43 x^{6} - 31 x^{5} - 21 x^{4} - 91 x^{3} - 7 x^{2} - 3 x - 1}{x^{21} - 3 x^{14} + 3 x^{7} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{21} + 3 x^{14} - 3 x^{7} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{20} + 3 x^{19} + 7 x^{18} + 91 x^{17} + 21 x^{16} + 31 x^{15} + 43 x^{14} + 54 x^{13} + 64 x^{12} + 70 x^{11} + 504 x^{10} + 70 x^{9} + 64 x^{8} + 54 x^{7} + 43 x^{6} + 31 x^{5} + 21 x^{4} + 91 x^{3} + 7 x^{2} + 3 x + 1,
\]
a POLYNOMIAL of degree $20$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $20$, so by
...
they stop at index $20$ because that polynomial has degree $20$. A reader who trusts
...
indices beyond $n=20$ and holds at every one, and at $n=20$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A111002.
