...
\title{The empirical recurrence for OEIS A202807, derived from the entry's generating function}
...
\begin{abstract}
OEIS A202807 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A202807 is ``Number of n X 3 nonnegative integer arrays with each row and column increasing from zero by 0, 1 or 2.''. It has offset $1$ and begins
\[
1, 6, 32, 121, 356, 881, 1925, 3830,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2018 10:34:45, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} - 6 x^{5} + 12 x^{4} - 11 x^{3} + x^{2} - x}{x^{7} - 7 x^{6} + 21 x^{5} - 35 x^{4} + 35 x^{3} - 21 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + 6 x^{5} - 12 x^{4} + 11 x^{3} - x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202807.
