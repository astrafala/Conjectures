...
\title{The empirical recurrence for OEIS A026923, derived from the entry's generating function}
...
\begin{abstract}
OEIS A026923 carries an empirical recurrence of order $11$. The entry separately records a
...
OEIS A026923 is ``Number of partitions of n into an odd number of parts, the greatest being 3; also, a(n+5) = number of partitions of n+2 into an even number of parts, each <= 3.''. It has offset $1$ and begins
\[
0, 0, 1, 0, 1, 1, 3, 2,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 27 2026 15:18:34, revision 27) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{7} - x^{5} + x^{4} - x^{3}}{x^{11} - x^{10} - x^{7} + x^{6} - x^{5} + x^{4} + x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A026923.
