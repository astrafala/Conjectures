...
\title{The empirical recurrence for OEIS A026219, derived from the entry's generating function}
...
\begin{abstract}
OEIS A026219 carries an empirical recurrence of order $8$. The entry separately records a
...
OEIS A026219 is ``Position of n in A026218.''. It has offset $1$ and begins
\[
1, 2, 5, 3, 9, 4, 13, 6,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 24 2025 13:13:26, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{7} + x^{6} + 4 x^{5} + x^{4} + 4 x^{3} + 2 x^{2} + x}{x^{8} - x^{6} - x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{8} - x^{6} - x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{7} + x^{6} + 4 x^{5} + x^{4} + 4 x^{3} + 2 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A026219.
