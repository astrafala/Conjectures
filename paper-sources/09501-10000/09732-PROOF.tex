...
\title{The empirical recurrence for OEIS A261956, derived from the entry's generating function}
...
\begin{abstract}
OEIS A261956 carries an empirical recurrence of order $10$. The entry separately records a
...
OEIS A261956 is ``Start with a single equilateral triangle for n=0; for the odd n-th generation add a triangle at each expandable vertex of the triangles of the (n-1)-th generation (this is the "side to vertex" version); for the even n-th generation use the "side to side" version; a(n) is the number of triangles added in the n-th generation.''. It has offset $0$ and begins
\[
1, 3, 6, 9, 12, 18, 15, 21,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 12 2015 16:20:30, revision 29) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 9 x^{13} - 9 x^{12} + 12 x^{11} + 13 x^{10} + 12 x^{9} + 5 x^{8} + 3 x^{7} + 3 x^{6} + 9 x^{5} + 6 x^{4} + 6 x^{3} + 5 x^{2} + 3 x + 1}{x^{10} - x^{8} - x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 9 x^{13} - 9 x^{12} + 12 x^{11} + 13 x^{10} + 12 x^{9} + 5 x^{8} + 3 x^{7} + 3 x^{6} + 9 x^{5} + 6 x^{4} + 6 x^{3} + 5 x^{2} + 3 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A261956.
