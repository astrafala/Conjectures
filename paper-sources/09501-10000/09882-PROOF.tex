...
\title{The empirical recurrence for OEIS A165025, derived from the entry's generating function}
...
\begin{abstract}
OEIS A165025 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A165025 is ``Number of cycles of n-digit numbers (including fixed points) under the base-4 Kaprekar map A165012.''. It has offset $1$ and begins
\[
1, 0, 1, 2, 1, 3, 1, 4,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2017 00:44:27, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{11} - 3 x^{9} + 2 x^{7} - x^{4} + x^{3} - x}{x^{7} - 2 x^{5} - x^{4} + x^{3} + 2 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{7} + 2 x^{5} + x^{4} - x^{3} - 2 x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{11} + 3 x^{9} - 2 x^{7} + x^{4} - x^{3} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A165025.
