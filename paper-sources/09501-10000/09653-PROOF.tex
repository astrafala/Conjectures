...
\title{The empirical recurrence for OEIS A251146, proved by transfer matrix}
...
\begin{abstract}
OEIS A251146 counts $6$-column arrays over $\{0,\dots,2\}$ in which every
$2\times2$ subblock sums to one of $\{4\}$, and in which no $2\times2$ subblock has exactly two nonzero entries, and carries an empirical
recurrence of order $5$ contributed by R.~H.~Hardin. It is true, and it is not an
...
OEIS A251146 is ``Number of (n+1) X (5+1) 0..2 arrays with every 2 X 2 subblock summing to 4 and no 2 X 2 subblock having exactly two nonzero entries.''. It has offset $1$
and begins
\[
213, 299, 433, 667, 1061, 1743, 2925, 5003,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) - 5*a(n-3) + a(n-4) + 2*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 26 2018 16:27:47, revision 17) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) - 5\,a(n-3) + a(n-4) + 2\,a(n-5)
\end{equation}
for all $n>5$, which is every $n$ at which a recurrence of order $5$ can be
stated.
...
Let $q(t)=t^{5}-\sum_{i}c_i\,t^{5-i}$ be the characteristic polynomial of
\eqref{eq:conj}, with the $c_i$ read off that equation.

\begin{lemma}\label{lem:crit}
For every $n\ge5$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n-5}\,q(M)\,\mathbf{1} .
\]
Hence \eqref{eq:conj} holds for every $n\ge5$ if and only if
...
=\mathbf{1}^{\!\top}M^{\,n-5}\Bigl(M^{5}-\sum_i c_i M^{5-i}\Bigr)\mathbf{1},
\]
which is the stated identity. The equivalence follows by letting $n-5=j$ run over
...
\begin{proof}
The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $5$ matrix--vector
...
$j=5-5+1$ onwards, and the run was continued past $S$ consecutive zeros, which by
...
the $29$ published indices; the values agree exactly. That is what ties the digraph to
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251146.
