...
\title{The empirical recurrence for OEIS A133552, derived from the entry's generating function}
...
\begin{abstract}
OEIS A133552 carries an empirical recurrence of order $15$. The entry separately records a
...
OEIS A133552 is ``Number of length n binary sequences with at most 4 of every adjacent 6 bits set.''. It has offset $0$ and begins
\[
1, 2, 4, 8, 16, 31, 57, 109,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 30 2026 16:09:41, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{14} - x^{13} + x^{11} + x^{10} + 2 x^{8} + 3 x^{7} + 2 x^{6} - 2 x^{5} - 2 x^{4} - x^{3} - x^{2} - x - 1}{x^{15} - x^{12} - 2 x^{9} - 2 x^{8} + 2 x^{6} + x^{5} + x^{3} + x^{2} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{15} + x^{12} + 2 x^{9} + 2 x^{8} - 2 x^{6} - x^{5} - x^{3} - x^{2} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-2) + a(n-3) + a(n-5) + 2\,a(n-6) - 2\,a(n-8) - 2\,a(n-9) - a(n-12) + a(n-15)
\]
for every $n>14$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{14} + x^{13} - x^{11} - x^{10} - 2 x^{8} - 3 x^{7} - 2 x^{6} + 2 x^{5} + 2 x^{4} + x^{3} + x^{2} + x + 1,
\]
...
\[
x^{15} - x^{12} - 2 x^{9} - 2 x^{8} + 2 x^{6} + x^{5} + x^{3} + x^{2} + x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A133552.
