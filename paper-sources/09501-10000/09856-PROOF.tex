...
\title{The empirical recurrence for OEIS A267214, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267214 carries an empirical recurrence of order $8$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{7} + 2 x^{6} - 2 x^{5} - 5 x^{3} - 2 x}{x^{8} - x^{7} - x^{5} + x^{3} + x - 1},
\]
