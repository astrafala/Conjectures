...
\title{The empirical recurrence for OEIS A264797, derived from the entry's generating function}
...
\begin{abstract}
OEIS A264797 carries an empirical recurrence of order $8$. The entry separately records a
...
OEIS A264797 is ``Number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 342", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 5, 8, 24, 17, 53, 32, 96,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:30, revision 23) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 5 x^{5} - x^{4} - 14 x^{3} - 6 x^{2} - 5 x - 1}{x^{8} - 2 x^{6} + 2 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{8} + 2 x^{6} - 2 x^{2} + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264797.
