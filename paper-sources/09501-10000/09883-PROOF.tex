...
\title{The empirical recurrence for OEIS A165027, derived from the entry's generating function}
...
\begin{abstract}
OEIS A165027 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A165027 is ``Number of n-digit fixed points under the base-4 Kaprekar map A165012.''. It has offset $1$ and begins
\[
1, 0, 1, 1, 0, 3, 1, 3,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 01:56:26, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{8} - x^{7} - 2 x^{6} + x^{5} + x^{3} - x}{x^{7} - 2 x^{5} - x^{4} + x^{3} + 2 x^{2} - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{8} + x^{7} + 2 x^{6} - x^{5} - x^{3} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A165027.
