...
\title{The empirical recurrence for OEIS A215191, derived from the entry's generating function}
...
\begin{abstract}
OEIS A215191 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A215191 is ``Number of arrays of 4 0..n integers with no sum of consecutive elements equal to a disjoint adjacent sum of an equal number of elements.''. It has offset $1$ and begins
\[
0, 18, 88, 276, 664, 1366, 2512, 4264,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 22 2018 12:32:12, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 14 x^{4} - 16 x^{3} - 18 x^{2}}{x^{6} - 4 x^{5} + 5 x^{4} - 5 x^{2} + 4 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A215191.
