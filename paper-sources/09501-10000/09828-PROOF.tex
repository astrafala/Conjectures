...
\title{The empirical recurrence for OEIS A269103, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269103 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A269103 is ``Number of n X 2 0..3 arrays with some element plus some horizontally or vertically adjacent neighbor totalling three no more than once.''. It has offset $1$ and begins
\[
16, 180, 1740, 15540, 132300, 1090740, 8787660, 69580980,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 21 2018 12:17:26, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{3} - 44 x^{2} + 16 x}{49 x^{2} - 14 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269103.
