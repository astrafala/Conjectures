...
\title{The empirical recurrence for OEIS A202752, derived from the entry's generating function}
...
\begin{abstract}
OEIS A202752 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A202752 is ``Number of n X 4 nonnegative integer arrays with each row and column increasing from zero by 0 or 1.''. It has offset $1$ and begins
\[
1, 4, 17, 62, 184, 462, 1022, 2052,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 01 2018 11:57:38, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{5} + 8 x^{4} - 10 x^{3} + 3 x^{2} - x}{x^{7} - 7 x^{6} + 21 x^{5} - 35 x^{4} + 35 x^{3} - 21 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{5} - 8 x^{4} + 10 x^{3} - 3 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202752.
