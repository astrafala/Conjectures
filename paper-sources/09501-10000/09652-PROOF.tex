...
\title{The empirical recurrence for OEIS A079279, derived from the entry's generating function}
...
\begin{abstract}
OEIS A079279 carries an empirical recurrence of order $15$. The entry separately records a
...
OEIS A079279 is ``a(n)=n for n<=3; for n>3, a(n) is next integer sharing common factors with 1 or 2 of previous 3 terms.''. It has offset $1$ and begins
\[
1, 2, 3, 4, 8, 9, 10, 14,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 16 2019 11:51:53, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{16} + x^{14} + 4 x^{13} + x^{12} + 5 x^{11} + x^{10} + x^{9} + 4 x^{8} + x^{7} + x^{6} + 4 x^{5} + x^{4} + x^{3} + x^{2} + x}{x^{15} - x^{14} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{15} - x^{14} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{16} + x^{14} + 4 x^{13} + x^{12} + 5 x^{11} + x^{10} + x^{9} + 4 x^{8} + x^{7} + x^{6} + 4 x^{5} + x^{4} + x^{3} + x^{2} + x,
\]
...
compared against all $65$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A079279.
