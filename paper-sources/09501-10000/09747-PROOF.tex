...
\title{The empirical recurrence for OEIS A154145, derived from the entry's generating function}
...
\begin{abstract}
OEIS A154145 carries an empirical recurrence of order $9$. The entry separately records a
...
OEIS A154145 is ``Indices k such that 15 plus the k-th triangular number is a perfect square.''. It has offset $1$ and begins
\[
1, 4, 6, 11, 20, 33, 43, 70,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 23 2024 14:53:42, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{9} + 3 x^{8} + 2 x^{7} + 5 x^{6} - 3 x^{5} - 5 x^{4} - 2 x^{3} - 3 x^{2} - x}{x^{9} - x^{8} - 6 x^{5} + 6 x^{4} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{9} + x^{8} + 6 x^{5} - 6 x^{4} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 2 x^{9} - 3 x^{8} - 2 x^{7} - 5 x^{6} + 3 x^{5} + 5 x^{4} + 2 x^{3} + 3 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A154145.
