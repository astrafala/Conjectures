...
\title{The empirical recurrence for OEIS A198179, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A198179 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A198179 is ``Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 0,2,1,0,0 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
0, 4, 3, 8, 16, 21, 56, 70,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-2) +2*a(n-3) -2*a(n-4) -2*a(n-5) +4*a(n-7) -a(n-8) -3*a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (May 14 2018 08:46:54, revision 10) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-2) + 2\,a(n-3) - 2\,a(n-4) - 2\,a(n-5) + 4\,a(n-7) - a(n-8) - 3\,a(n-9)
\end{equation}
for all $n>10$.
...
publishes, $n=34$, the arrays have $2\cdot34$ cells over
$5$ values, so there are about $10^{47}$ of them to sift. The
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198179.
