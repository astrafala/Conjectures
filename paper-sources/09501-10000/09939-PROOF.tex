...
\title{The empirical recurrence for OEIS A252310, proved by transfer matrix}
...
\begin{abstract}
OEIS A252310 counts $7\times(n+2)$ arrays over $\{0,\dots,3\}$ subject to a condition imposed on
every $3\times3$ subblock, and it carries an empirical recurrence of order $12$
...
$a(n)$ is a walk count on $S=17524$ vertices. Such a sequence is $C$-finite, so whether a given
...
OEIS A252310 is ``Number of (5+2)X(n+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 1 3 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 1 3 6 or 7.''. It has offset $1$ and begins
\[
2587, 2350, 3669, 5117, 9368, 15444, 22521, 43870,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 15*a(n-3) -70*a(n-6) +120*a(n-9) -64*a(n-12) for n>21
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:03:40, revision 6) this is still
...
\begin{theorem}
\[
a(n)\;=\;15\,a(n-3) - 70\,a(n-6) + 120\,a(n-9) - 64\,a(n-12)
\]
for every $n>21$.
\end{theorem}

\begin{proof}
The vector $w=q(M)\mathbf{1}$ was formed by $12$ matrix--vector products in exact
integer arithmetic and $\mathbf{1}^{\!\top}M^{j}w$ evaluated for $j=0,1,\dots$, again
exactly. Those integers vanish from the index corresponding to $n=21+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>21$. The bound is exact: at $n=21$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252310.
