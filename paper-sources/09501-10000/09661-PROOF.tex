...
\title{The empirical recurrence for OEIS A165066, derived from the entry's generating function}
...
\begin{abstract}
OEIS A165066 carries an empirical recurrence of order $13$. The entry separately records a
...
OEIS A165066 is ``Number of n-digit fixed points under the base-6 Kaprekar map A165051.''. It has offset $1$ and begins
\[
1, 0, 1, 0, 1, 3, 0, 2,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 01:59:19, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{15} + x^{9} - x^{6} + x^{5} - x^{3} - x^{2} - x}{x^{13} + x^{12} - x^{10} - 2 x^{9} - 2 x^{8} - x^{7} + x^{6} + 2 x^{5} + 2 x^{4} + x^{3} - x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{13} - x^{12} + x^{10} + 2 x^{9} + 2 x^{8} + x^{7} - x^{6} - 2 x^{5} - 2 x^{4} - x^{3} + x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;-a(n-1) + a(n-3) + 2\,a(n-4) + 2\,a(n-5) + a(n-6) - a(n-7) - 2\,a(n-8) - 2\,a(n-9) - a(n-10) + a(n-12) + a(n-13)
\]
for every $n>15$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{15} - x^{9} + x^{6} - x^{5} + x^{3} + x^{2} + x,
\]
...
\[
x^{13} + x^{12} - x^{10} - 2 x^{9} - 2 x^{8} - x^{7} + x^{6} + 2 x^{5} + 2 x^{4} + x^{3} - x - 1,
\]
...
compared against all $78$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A165066.
