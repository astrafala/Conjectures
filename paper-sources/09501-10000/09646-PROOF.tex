...
\title{The empirical recurrence for OEIS A317767, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A317767 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A317767 is ``Number of nX2 0..1 arrays with every element unequal to 0, 1 or 3 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
2, 3, 4, 7, 11, 18, 30, 49,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +2*a(n-3) -a(n-6)
\end{quote}
As of the ``Last modified'' line on the live entry (May 09 2023 06:27:05, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 2\,a(n-3) - a(n-6)
\end{equation}
for all $n>7$.
...
\#\{\text{neighbours with a value }different from\ x_{t,u}\}\in\{0,1,3\}
\end{equation}
...
every one of the $37$ published indices, with no shift fitted: the number of rows is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A317767.
