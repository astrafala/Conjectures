...
\title{The empirical recurrence for OEIS A168222, derived from the entry's generating function}
...
\begin{abstract}
OEIS A168222 carries an empirical recurrence of order $18$. The entry separately records a
...
OEIS A168222 is ``a(n) = A006369(A006369(n)).''. It has offset $0$ and begins
\[
0, 1, 2, 3, 7, 9, 5, 6,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 27 2024 18:51:22, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{17} + 2 x^{16} + 3 x^{15} + 7 x^{14} + 9 x^{13} + 5 x^{12} + 6 x^{11} + 15 x^{10} + 4 x^{9} + 15 x^{8} + 6 x^{7} + 5 x^{6} + 9 x^{5} + 7 x^{4} + 3 x^{3} + 2 x^{2} + x}{x^{18} - 2 x^{9} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{18} - 2 x^{9} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{17} + 2 x^{16} + 3 x^{15} + 7 x^{14} + 9 x^{13} + 5 x^{12} + 6 x^{11} + 15 x^{10} + 4 x^{9} + 15 x^{8} + 6 x^{7} + 5 x^{6} + 9 x^{5} + 7 x^{4} + 3 x^{3} + 2 x^{2} + x,
\]
...
x^{18} - 2 x^{9} + 1,
\]
of degree $18$, and that degree is the order of the SHORTEST linear recurrence with
...
compared against all $71$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A168222.
