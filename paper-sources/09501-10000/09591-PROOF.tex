...
\title{The empirical recurrence for OEIS A223409, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A223409 counts arrays over $\{0,\dots,9\}$ in which every cell is constrained by the
...
OEIS A223409 is ``4 X 4 X 4 triangular graph without horizontal edges coloring a rectangular array: number of n X 2 0..9 arrays where 0..9 label nodes of a graph with edges 0,1 0,2 1,3 1,4 2,4 2,5 3,6 3,7 4,7 4,8 5,8 5,9 and every array movement to a horizontal or vertical neighbor moves along an edge of this graph.''. It has offset $1$ and begins
\[
24, 132, 780, 4664, 28060, 169124, 1020056, 6153860,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 9*a(n-1) - 17*a(n-2) - 10*a(n-3) + 26*a(n-4) + 10*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 20 2018 08:49:44, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;9\,a(n-1) - 17\,a(n-2) - 10\,a(n-3) + 26\,a(n-4) + 10\,a(n-5)
\end{equation}
for all $n>6$.
...
and $2$ cells in a row, so $|V|=10^{2}=100$, and the vertex set is
$V\times V$, of size $100^2=10000$. That is the whole of the state: two consecutive
...
$10$ values, so there are about $10^{38}$ of them to sift. The
walk count replaces that by $10000$ states and one matrix--vector product per row.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223409.
