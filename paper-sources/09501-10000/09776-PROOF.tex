...
\title{The empirical recurrence for OEIS A246481, derived from the entry's generating function}
...
\begin{abstract}
OEIS A246481 carries an empirical recurrence of order $9$. The entry separately records a
...
OEIS A246481 is ``Number of length 2+3 0..n arrays with no pair in any consecutive four terms totalling exactly n.''. It has offset $1$ and begins
\[
2, 14, 132, 484, 1734, 4386, 10376, 20840,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 06 2018 06:44:59, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{230 x^{7} + 144 x^{6} + 382 x^{5} + 104 x^{4} + 90 x^{3} + 8 x^{2} + 2 x}{x^{9} - 3 x^{8} + 8 x^{6} - 6 x^{5} - 6 x^{4} + 8 x^{3} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{9} - 3 x^{8} + 8 x^{6} - 6 x^{5} - 6 x^{4} + 8 x^{3} - 3 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;230 x^{7} + 144 x^{6} + 382 x^{5} + 104 x^{4} + 90 x^{3} + 8 x^{2} + 2 x,
\]
...
\[
x^{9} - 3 x^{8} + 8 x^{6} - 6 x^{5} - 6 x^{4} + 8 x^{3} - 3 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A246481.
