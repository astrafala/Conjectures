...
\title{The empirical recurrence for OEIS A205344, derived from the entry's generating function}
...
\begin{abstract}
OEIS A205344 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A205344 is ``Number of length 7 nonnegative integer arrays starting and ending with 0 with adjacent elements unequal but differing by no more than n.''. It has offset $1$ and begins
\[
5, 93, 689, 2912, 8927, 22297, 48335, 94456,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 15:52:46, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{5 x^{5} + 73 x^{4} + 206 x^{3} + 63 x^{2} + 5 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;5 x^{5} + 73 x^{4} + 206 x^{3} + 63 x^{2} + 5 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205344.
