...
\title{The empirical recurrence for OEIS A211440, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211440 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A211440 is ``Number of ordered triples (w,x,y) with all terms in {-n,...,0,...,n} and 2w+3x+3y=0.''. It has offset $0$ and begins
\[
1, 3, 5, 17, 23, 29, 53, 63,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 29 2021 16:30:51, revision 15) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} - 2 x^{5} - 2 x^{4} - 10 x^{3} - 2 x^{2} - 2 x - 1}{x^{7} - x^{6} - 2 x^{4} + 2 x^{3} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{7} + x^{6} + 2 x^{4} - 2 x^{3} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} + 2 x^{5} + 2 x^{4} + 10 x^{3} + 2 x^{2} + 2 x + 1,
\]
...
compared against all $48$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211440.
