...
\title{The empirical recurrence for OEIS A202331, derived from the entry's generating function}
...
\begin{abstract}
OEIS A202331 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A202331 is ``Number of (n+1) X 5 binary arrays with consecutive windows of two bits considered as a binary number nondecreasing in every row and column.''. It has offset $1$ and begins
\[
49, 129, 289, 576, 1052, 1796, 2906, 4501,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 27 2018 14:42:42, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 15 x^{6} + 86 x^{5} - 203 x^{4} + 250 x^{3} - 165 x^{2} + 49 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 15 x^{6} + 86 x^{5} - 203 x^{4} + 250 x^{3} - 165 x^{2} + 49 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202331.
