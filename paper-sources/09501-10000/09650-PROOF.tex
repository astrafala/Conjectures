...
\title{The empirical recurrence for OEIS A111093, derived from the entry's generating function}
...
\begin{abstract}
OEIS A111093 carries an empirical recurrence of order $16$. The entry separately records a
...
OEIS A111093 is ``Like sequence A111072 but moving right by the squares of the sequence of positive integers.''. It has offset $0$ and begins
\[
0, 1, 6, 10, 10, 15, 16, 16,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 19 2024 11:14:21, revision 24) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{9 x^{13} + 5 x^{12} + 6 x^{11} + 5 x^{9} - 5 x^{7} + 5 x^{5} + 4 x^{3} + 5 x^{2} + x}{x^{16} - x^{15} - x^{11} + x^{10} + x^{6} - x^{5} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{16} - x^{15} - x^{11} + x^{10} + x^{6} - x^{5} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-5) - a(n-6) - a(n-10) + a(n-11) + a(n-15) - a(n-16)
\]
for every $n>13$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;9 x^{13} + 5 x^{12} + 6 x^{11} + 5 x^{9} - 5 x^{7} + 5 x^{5} + 4 x^{3} + 5 x^{2} + x,
\]
...
\[
x^{16} - x^{15} - x^{11} + x^{10} + x^{6} - x^{5} - x + 1,
\]
...
compared against all $60$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A111093.
