...
\title{A proof of the conjectured recurrence for OEIS A005559}
...
\begin{abstract}
OEIS A005559 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A005559 is ``Number of walks on square lattice. Column y=1 of A052174''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 8, 20, 75, 210, 784, 2352,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = C(n+1,ceiling((n-1)/2)) *C(n,floor((n-1)/2)) -C(n+1,ceiling((n-2)/2)) *C(n,floor((n-2)/2)). - \emph{Paul D. Hanna}, Apr 16 2004
\end{quote}
that is,
\begin{equation}\label{eq:cf}
a(n)\;=\;- {\binom{n}{\left\lfloor{\frac{n}{2}}\right\rfloor - 1}} {\binom{n + 1}{\left\lceil{\frac{n}{2}}\right\rceil - 1}} + {\binom{n}{\left\lfloor{\frac{n}{2} - \frac{1}{2}}\right\rfloor}} {\binom{n + 1}{\left\lceil{\frac{n}{2} - \frac{1}{2}}\right\rceil}} .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} -(n-1)*(2*n+1)*(n+4)*(n+3)*a(n) +4*(n+1)*(2*n\^{}2+4*n+9)*a(n-1) +16*n*(n-1)*(2*n+3)*(n+1)*a(n-2)=0. - \emph{R. J. Mathar}, Apr 02 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2025-02-03, revision 60) the
...
- \left(n - 1\right) \left(n + 3\right) \left(n + 4\right) \left(2 n + 1\right)\,a(n) + 4 \left(n + 1\right) \left(2 n^{2} + 4 n + 9\right)\,a(n-1) + 16 n \left(n - 1\right) \left(n + 1\right) \left(2 n + 3\right)\,a(n-2)
\end{equation}
...
a(2m)\;=\;e_{0}(m)\;=\;\frac{2 {\binom{2 m + 1}{m}} \left(2 m\right)!}{\left(m - 1\right)! \left(m + 2\right)!},
\qquad
a(2m+1)\;=\;e_{1}(m)\;=\;\frac{4 \left(2 m + 1\right)!^{2}}{\left(m + 2\right) m!^{3} \left(m + 2\right)!},
\end{equation}
...
\section{The computation for A005559}
...
entry \href{https://oeis.org/A005559}{A005559}, 2025-02-03.
