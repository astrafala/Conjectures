...
\title{The empirical recurrence for OEIS A007390, derived from the entry's generating function}
...
\begin{abstract}
OEIS A007390 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A007390 is ``Number of strict (-1)st-order maximal independent sets in cycle graph.''. It has offset $1$ and begins
\[
0, 0, 0, 4, 5, 15, 21, 44,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 14 2019 21:48:55, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{7} + 2 x^{6} - x^{5} - 4 x^{4}}{x^{6} + x^{5} - 3 x^{4} - 2 x^{3} + 3 x^{2} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{6} - x^{5} + 3 x^{4} + 2 x^{3} - 3 x^{2} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 3\,a(n-2) - 2\,a(n-3) - 3\,a(n-4) + a(n-5) + a(n-6)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A007390.
