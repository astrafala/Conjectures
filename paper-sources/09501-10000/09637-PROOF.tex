...
\title{The empirical recurrence for OEIS A233766, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233766 carries an empirical recurrence of order $24$. The entry separately records a
...
OEIS A233766 is ``Digital root of Lucas numbers (A030133) indexed by natural numbers not divisible by 2, 3 or 5 (A007775).''. It has offset $1$ and begins
\[
2, 9, 6, 7, 2, 9, 3, 7,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 22 2019 20:47:55, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{24} - 6 x^{23} - 9 x^{22} - 7 x^{21} - 2 x^{20} - 3 x^{19} - 9 x^{18} - 7 x^{17} - 6 x^{16} - 7 x^{15} - 2 x^{14} - 3 x^{13} - 9 x^{12} - 2 x^{11} - 7 x^{10} - 9 x^{9} - 7 x^{8} - 3 x^{7} - 9 x^{6} - 2 x^{5} - 7 x^{4} - 6 x^{3} - 9 x^{2} - 2 x}{x^{24} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - x^{24}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{24} + 6 x^{23} + 9 x^{22} + 7 x^{21} + 2 x^{20} + 3 x^{19} + 9 x^{18} + 7 x^{17} + 6 x^{16} + 7 x^{15} + 2 x^{14} + 3 x^{13} + 9 x^{12} + 2 x^{11} + 7 x^{10} + 9 x^{9} + 7 x^{8} + 3 x^{7} + 9 x^{6} + 2 x^{5} + 7 x^{4} + 6 x^{3} + 9 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233766.
