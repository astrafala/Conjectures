...
\title{The empirical recurrence for OEIS A071288, derived from the entry's generating function}
...
\begin{abstract}
OEIS A071288 carries an empirical recurrence of order $21$. The entry separately records a
...
OEIS A071288 is ``Denominators of Peirce sequence of order 6.''. It has offset $0$ and begins
\[
2, 4, 6, 5, 3, 1, 6, 5,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 30 2022 18:19:59, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{20} - 5 x^{19} - 4 x^{18} - 3 x^{17} - 6 x^{16} - 5 x^{15} - 2 x^{14} - 6 x^{13} - 4 x^{12} - 5 x^{11} - 3 x^{10} - 6 x^{9} - 4 x^{8} - 5 x^{7} - 6 x^{6} - x^{5} - 3 x^{4} - 5 x^{3} - 6 x^{2} - 4 x - 2}{x^{21} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - x^{21}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;6 x^{20} + 5 x^{19} + 4 x^{18} + 3 x^{17} + 6 x^{16} + 5 x^{15} + 2 x^{14} + 6 x^{13} + 4 x^{12} + 5 x^{11} + 3 x^{10} + 6 x^{9} + 4 x^{8} + 5 x^{7} + 6 x^{6} + x^{5} + 3 x^{4} + 5 x^{3} + 6 x^{2} + 4 x + 2,
\]
...
compared against all $80$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A071288.
