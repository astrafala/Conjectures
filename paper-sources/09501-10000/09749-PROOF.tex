...
\title{The empirical recurrence for OEIS A173777, derived from the entry's generating function}
...
\begin{abstract}
OEIS A173777 carries an empirical recurrence of order $9$. The entry separately records a
...
OEIS A173777 is ``Infinite sequence gradually builds a triangle plus another more widely spaced triangle on top of it, or overlapping, if you will.''. It has offset $1$ and begins
\[
2, 2, 2, 2, 3, 3, 4, 5,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 02 2019 09:24:44, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{10} - 2 x^{9} - x^{7} + 2 x^{6} - x^{5} + 2 x^{4} - 2 x}{x^{9} - x^{8} - x^{6} + x^{5} - x^{4} + x^{3} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{9} + x^{8} + x^{6} - x^{5} + x^{4} - x^{3} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{10} + 2 x^{9} + x^{7} - 2 x^{6} + x^{5} - 2 x^{4} + 2 x,
\]
...
compared against all $68$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A173777.
