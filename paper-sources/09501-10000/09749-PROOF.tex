...
\title{The empirical recurrence for OEIS A232921, derived from the entry's generating function}
...
\begin{abstract}
OEIS A232921 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A232921 is ``Number of 2 X n 0..2 arrays with no element x(i,j) adjacent to value 2-x(i,j) horizontally, diagonally or antidiagonally.''. It has offset $1$ and begins
\[
9, 18, 54, 144, 396, 1080, 2952, 8064,\ \dots
\]
...
G.f.: 9*x / (1 - 2*x - 2*x\^{}2).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 06 2018 05:30:24, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;- \frac{9 x}{2 x^{2} + 2 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 2 x^{2} - 2 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232921.
