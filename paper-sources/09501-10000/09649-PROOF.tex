...
\title{The empirical recurrence for OEIS A101348, derived from the entry's generating function}
...
\begin{abstract}
OEIS A101348 carries an empirical recurrence of order $16$. The entry separately records a
...
OEIS A101348 is ``Arises from a particular cyclic transformation of the floretion - .5'i - .5i' - .5'ij' - .5'ik' + .5'ji' + .5'ki'.''. It has offset $0$ and begins
\[
0, 1, 1, 2, 2, 0, 3, 0,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = a(n-1) + a(n-3) - a(n-4) - 2*a(n-6) + 2*a(n-7) + 2*a(n-9) - 2*a(n-10) - a(n-12) + a(n-13) + a(n-15) - a(n-16) for n>15.
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: x*(1 + x\^{}2 - x\^{}3 - 2*x\^{}4 + 2*x\^{}5 - x\^{}6 + 2*x\^{}7 - x\^{}8 + x\^{}9) / ((1 - x)\^{}2*(1 + x\^{}2)\^{}2*(1 + x + x\^{}2)*(1 - x\^{}2 + x\^{}4)\^{}2).
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 30 2019 15:14:41, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{10} - x^{9} + 2 x^{8} - x^{7} + 2 x^{6} - 2 x^{5} - x^{4} + x^{3} + x}{x^{16} - x^{15} - x^{13} + x^{12} + 2 x^{10} - 2 x^{9} - 2 x^{7} + 2 x^{6} + x^{4} - x^{3} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{16} - x^{15} - x^{13} + x^{12} + 2 x^{10} - 2 x^{9} - 2 x^{7} + 2 x^{6} + x^{4} - x^{3} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-3) - a(n-4) - 2\,a(n-6) + 2\,a(n-7) + 2\,a(n-9) - 2\,a(n-10) - a(n-12) + a(n-13) + a(n-15) - a(n-16)
\]
for every $n>10$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{10} - x^{9} + 2 x^{8} - x^{7} + 2 x^{6} - 2 x^{5} - x^{4} + x^{3} + x,
\]
a POLYNOMIAL of degree $10$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $10$, so by
...
\[
x^{16} - x^{15} - x^{13} + x^{12} + 2 x^{10} - 2 x^{9} - 2 x^{7} + 2 x^{6} + x^{4} - x^{3} - x + 1,
\]
of degree $16$, and that degree is the order of the SHORTEST linear recurrence with
...
they stop at index $10$ because that polynomial has degree $10$. A reader who trusts
...
compared against all $90$ terms the entry publishes: it reproduces every one. This is
...
indices beyond $n=10$ and holds at every one, and at $n=10$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A101348.
