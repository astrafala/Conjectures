...
\title{The empirical recurrence for OEIS A034108, derived from the entry's generating function}
...
\begin{abstract}
OEIS A034108 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A034108 is ``Fractional part of square root of a(n) starts with 2: first term of runs.''. It has offset $1$ and begins
\[
5, 18, 28, 39, 52, 68, 85, 105,\ \dots
\]
...
\begin{quote}
G.f.: x*(-6*x\^{}7 + 7*x\^{}6 + 2*x\^{}5 - 2*x\^{}4 - x\^{}3 + 3*x\^{}2 - 8*x - 5)/((x - 1)\^{}3*(x\^{}4 + x\^{}3 + x\^{}2 + x + 1)). (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 15 2024 04:57:43, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{8} + 7 x^{7} + 2 x^{6} - 2 x^{5} - x^{4} + 3 x^{3} - 8 x^{2} - 5 x}{x^{7} - 2 x^{6} + x^{5} - x^{2} + 2 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;6 x^{8} - 7 x^{7} - 2 x^{6} + 2 x^{5} + x^{4} - 3 x^{3} + 8 x^{2} + 5 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A034108.
