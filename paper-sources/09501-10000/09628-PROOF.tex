...
\title{The empirical recurrence for OEIS A267912, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267912 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 25 2018 08:49:35, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{5} + 3 x^{4} - 4 x^{3} - x^{2} + x}{8 x^{3} - 2 x^{2} - 3 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;4 x^{5} + 3 x^{4} - 4 x^{3} - x^{2} + x,
\]
