...
\title{The empirical recurrence for OEIS A203094, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A203094 counts arrays over $\{0,\dots,3\}$ in which every cell is constrained by the
...
OEIS A203094 is ``Number of nX1 0..3 arrays with every nonzero element less than or equal to some horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
1, 4, 16, 50, 144, 422, 1268, 3823,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) -6*a(n-2) +10*a(n-3) -5*a(n-4) +6*a(n-5) -a(n-6) +a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 18 2023 12:26:26, revision 15) the statement is
...
Let $V=\{0,\dots,3\}^{1}$ be the possible rows and take
...
publishes, $n=26$, the arrays have $1\cdot26$ cells over
$4$ values, so there are about $10^{15}$ of them to sift. The
...
of the $(3+1)^{1}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203094.
