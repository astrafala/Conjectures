...
\title{The empirical recurrence for OEIS A071286, derived from the entry's generating function}
...
\begin{abstract}
OEIS A071286 carries an empirical recurrence of order $15$. The entry separately records a
...
OEIS A071286 is ``Denominators of Peirce sequence of order 5.''. It has offset $0$ and begins
\[
2, 4, 5, 3, 1, 5, 4, 3,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 5 x^{14} - 4 x^{13} - 3 x^{12} - 5 x^{11} - 2 x^{10} - 4 x^{9} - 5 x^{8} - 3 x^{7} - 4 x^{6} - 5 x^{5} - x^{4} - 3 x^{3} - 5 x^{2} - 4 x - 2}{x^{15} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - x^{15}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;5 x^{14} + 4 x^{13} + 3 x^{12} + 5 x^{11} + 2 x^{10} + 4 x^{9} + 5 x^{8} + 3 x^{7} + 4 x^{6} + 5 x^{5} + x^{4} + 3 x^{3} + 5 x^{2} + 4 x + 2,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A071286.
