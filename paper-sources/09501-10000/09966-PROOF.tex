...
\title{The empirical recurrence for OEIS A182615, derived from the entry's generating function}
...
\begin{abstract}
OEIS A182615 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A182615 is ``Greatest k such that floor(k/r\^{}n)=n, where r = golden mean = (1+sqrt(5))/2.''. It has offset $1$ and begins
\[
3, 7, 16, 34, 66, 125, 232, 422,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 12 2023 20:10:49, revision 24) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{8} - x^{7} + 3 x^{6} + 4 x^{3} - x^{2} - 3 x}{x^{6} + 2 x^{5} - 2 x^{4} - 4 x^{3} + 2 x^{2} + 2 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{6} - 2 x^{5} + 2 x^{4} + 4 x^{3} - 2 x^{2} - 2 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 2\,a(n-2) - 4\,a(n-3) - 2\,a(n-4) + 2\,a(n-5) + a(n-6)
\]
for every $n>8$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{8} + x^{7} - 3 x^{6} - 4 x^{3} + x^{2} + 3 x,
\]
...
\[
x^{6} + 2 x^{5} - 2 x^{4} - 4 x^{3} + 2 x^{2} + 2 x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A182615.
