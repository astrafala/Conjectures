...
\title{The empirical recurrence for OEIS A186390, derived from the entry's generating function}
...
\begin{abstract}
OEIS A186390 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A186390 is ``Adjusted joint rank sequence of (f(i)) and (g(j)) with f(i) after g(j) when f(i)=g(j), where f(i)=6i and g(j)=j(j+1)/2 (triangular number).  Complement of A186389.''. It has offset $1$ and begins
\[
1, 2, 3, 5, 7, 9, 11, 13,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 13 2026 01:21:58, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{7} - x^{3} - x}{x^{7} - 2 x^{6} + 2 x^{5} - 3 x^{4} + 3 x^{3} - 2 x^{2} + 2 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{7} + 2 x^{6} - 2 x^{5} + 3 x^{4} - 3 x^{3} + 2 x^{2} - 2 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) - 2\,a(n-2) + 3\,a(n-3) - 3\,a(n-4) + 2\,a(n-5) - 2\,a(n-6) + a(n-7)
\]
for every $n>7$.
\end{theorem}
...
\[
x^{7} - 2 x^{6} + 2 x^{5} - 3 x^{4} + 3 x^{3} - 2 x^{2} + 2 x - 1,
\]
...
compared against all $86$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A186390.
