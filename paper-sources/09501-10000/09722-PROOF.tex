...
\title{The empirical recurrence for OEIS A253463, proved by transfer matrix}
...
\begin{abstract}
OEIS A253463 counts arrays of width $3$ over $\{0,1,2\}$ in which the largest entry on the main diagonal minus the smallest on the antidiagonal of every $2\times2$ subblock differs from the same number at the neighbouring subblock. The entry carries an
...
$3$ consecutive rows and the count is a walk count on $S=568$ states.
\end{abstract}
...
OEIS A253463 is ``Number of (n+1) X (2+1) 0..2 arrays with every 2 X 2 subblock diagonal maximum minus antidiagonal minimum unequal to its neighbors horizontally, vertically, diagonally and antidiagonally.''. It has offset $1$ and begins
\[
540, 800, 1400, 3072, 8536, 15888, 44464, 94416,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-2) + 8*a(n-4) - 12*a(n-6) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 14 2018 06:07:41, revision 10) this is still
...
Thus
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=568,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253463.
