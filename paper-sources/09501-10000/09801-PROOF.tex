...
\title{A condition on each element and its neighbours: OEIS A240888}
...
\begin{abstract}
OEIS A240888 carries an empirical recurrence of order $4$ contributed by R.~H.~Hardin. It is
...
OEIS A240888 is ``Number of n X 3 0..2 arrays with no element equal to a different number of vertical neighbors than horizontal neighbors, with new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
2, 11, 57, 289, 1485, 7609, 38981, 199761,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 8*a(n-2) + 14*a(n-3) + 4*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 29 2018 06:43:16, revision 7) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $3$ columns over the alphabet $\{0,1,2\}$, growing one row at a time. Its condition is that every element must equal the same number of its vertical neighbours as it does of its horizontal neighbours.
...
Vertices with identical futures are merged, which leaves $S=6$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 8\,a(n-2) + 14\,a(n-3) + 4\,a(n-4)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A240888.
