...
\title{The empirical recurrence for OEIS A221466, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221466 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A221466 is ``Number of 0..n arrays of length 7 with each element unequal to at least one neighbor, starting with 0.''. It has offset $1$ and begins
\[
8, 240, 1944, 8960, 30000, 81648, 192080, 405504,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 05 2018 15:06:02, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{16 x^{5} - 112 x^{4} - 432 x^{3} - 184 x^{2} - 8 x}{x^{7} - 7 x^{6} + 21 x^{5} - 35 x^{4} + 35 x^{3} - 21 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 16 x^{5} + 112 x^{4} + 432 x^{3} + 184 x^{2} + 8 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221466.
