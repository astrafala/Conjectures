...
\title{The empirical recurrence for OEIS A004484, derived from the entry's generating function}
...
\begin{abstract}
OEIS A004484 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A004484 is ``Sprague-Grundy values for game of Wyt Queens.''. It has offset $0$ and begins
\[
3, 4, 5, 6, 2, 0, 1, 9,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 30 2026 16:39:47, revision 24) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{14} - 12 x^{13} + 7 x^{12} + x^{11} + x^{9} + 7 x^{7} - 2 x^{6} - 2 x^{5} - 4 x^{4} + x^{3} + x^{2} + x + 3}{x^{7} - x^{6} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{7} - x^{6} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;4 x^{14} - 12 x^{13} + 7 x^{12} + x^{11} + x^{9} + 7 x^{7} - 2 x^{6} - 2 x^{5} - 4 x^{4} + x^{3} + x^{2} + x + 3,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A004484.
