...
\title{The empirical recurrence for OEIS A199534, derived from the entry's generating function}
...
\begin{abstract}
OEIS A199534 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A199534 is ``Number of -n..n arrays x(0..6) of 7 elements with zero sum and no two consecutive zero elements.''. It has offset $1$ and begins
\[
200, 6346, 53302, 252154, 860854, 2378412, 5662636, 12071420,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 16 2018 05:38:02, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{6} - 8 x^{5} - 5306 x^{4} - 13080 x^{3} - 4946 x^{2} - 200 x}{x^{7} - 7 x^{6} + 21 x^{5} - 35 x^{4} + 35 x^{3} - 21 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;8 x^{6} + 8 x^{5} + 5306 x^{4} + 13080 x^{3} + 4946 x^{2} + 200 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A199534.
