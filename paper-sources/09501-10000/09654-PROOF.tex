...
\title{The empirical recurrence for OEIS A133722, derived from the entry's generating function}
...
\begin{abstract}
OEIS A133722 carries an empirical recurrence of order $15$. The entry separately records a
...
OEIS A133722 is ``Column 3 of triangle in A133721.''. It has offset $1$ and begins
\[
0, 0, 1, 1, 1, 1, 7, 3,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 09 2020 14:37:19, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{15} + 4 x^{12} - x^{11} - 6 x^{9} + 2 x^{8} - 2 x^{7} + 4 x^{6} - x^{5} - x^{4} - x^{3}}{x^{15} - 5 x^{12} + 10 x^{9} - 10 x^{6} + 5 x^{3} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{15} + 5 x^{12} - 10 x^{9} + 10 x^{6} - 5 x^{3} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-3) - 10\,a(n-6) + 10\,a(n-9) - 5\,a(n-12) + a(n-15)
\]
for every $n>15$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{15} - 4 x^{12} + x^{11} + 6 x^{9} - 2 x^{8} + 2 x^{7} - 4 x^{6} + x^{5} + x^{4} + x^{3},
\]
...
\[
x^{15} - 5 x^{12} + 10 x^{9} - 10 x^{6} + 5 x^{3} - 1,
\]
...
compared against all $55$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A133722.
