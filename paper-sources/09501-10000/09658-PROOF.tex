...
\title{The empirical recurrence for OEIS A166212, derived from the entry's generating function}
...
\begin{abstract}
OEIS A166212 carries an empirical recurrence of order $14$. The entry separately records a
...
OEIS A166212 is ``Number of 4 X 4 X 4 triangular nonnegative integer arrays, symmetric under 120 degree rotation, with all sums of an element and its neighbors <= n.''. It has offset $0$ and begins
\[
1, 3, 6, 14, 25, 39, 66, 99,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 26 2018 19:24:08, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{6} - x^{5} - x^{4} - 3 x^{3} - x^{2} - x - 1}{x^{14} - 2 x^{13} + x^{12} - 2 x^{11} + 4 x^{10} - 2 x^{9} + 2 x^{5} - 4 x^{4} + 2 x^{3} - x^{2} + 2 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{14} + 2 x^{13} - x^{12} + 2 x^{11} - 4 x^{10} + 2 x^{9} - 2 x^{5} + 4 x^{4} - 2 x^{3} + x^{2} - 2 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) - a(n-2) + 2\,a(n-3) - 4\,a(n-4) + 2\,a(n-5) - 2\,a(n-9) + 4\,a(n-10) - 2\,a(n-11) + a(n-12) - 2\,a(n-13) + a(n-14)
\]
for every $n>6$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{6} + x^{5} + x^{4} + 3 x^{3} + x^{2} + x + 1,
\]
...
\[
x^{14} - 2 x^{13} + x^{12} - 2 x^{11} + 4 x^{10} - 2 x^{9} + 2 x^{5} - 4 x^{4} + 2 x^{3} - x^{2} + 2 x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A166212.
