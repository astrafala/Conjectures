...
\title{The empirical recurrence for OEIS A246919, derived from the entry's generating function}
...
\begin{abstract}
OEIS A246919 carries an empirical recurrence of order $12$. The entry separately records a
...
OEIS A246919 is ``The length of the longest nontrivial integral cevian of an equilateral triangle of side n that divides an edge into two integral parts, or 0 if no such cevian exists.''. It has offset $1$ and begins
\[
0, 0, 7, 0, 19, 14, 37, 13,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 28 2026 15:48:10, revision 27) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{16} - 2 x^{14} - x^{13} + 11 x^{12} - x^{11} + 4 x^{10} - 4 x^{9} - 13 x^{8} - 16 x^{7} - 14 x^{6} - 19 x^{5} - 7 x^{3}}{x^{12} - 3 x^{8} + 3 x^{4} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{12} + 3 x^{8} - 3 x^{4} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;4 x^{16} + 2 x^{14} + x^{13} - 11 x^{12} + x^{11} - 4 x^{10} + 4 x^{9} + 13 x^{8} + 16 x^{7} + 14 x^{6} + 19 x^{5} + 7 x^{3},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A246919.
