...
\title{The empirical recurrence for OEIS A026970, derived from the entry's generating function}
...
\begin{abstract}
OEIS A026970 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A026970 is ``a(n) = Sum\_{k=0..n} (k+1) * A026637(n,k).''. It has offset $0$ and begins
\[
1, 3, 8, 25, 60, 147, 336, 765,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 20 2019 16:23:00, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{6} - 4 x^{5} - 7 x^{4} + 7 x^{3} - 2 x^{2} - x + 1}{4 x^{6} - 4 x^{5} - 7 x^{4} + 8 x^{3} + 2 x^{2} - 4 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{6} - 4 x^{5} - 7 x^{4} + 8 x^{3} + 2 x^{2} - 4 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) - 2\,a(n-2) - 8\,a(n-3) + 7\,a(n-4) + 4\,a(n-5) - 4\,a(n-6)
\]
for every $n>6$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;4 x^{6} - 4 x^{5} - 7 x^{4} + 7 x^{3} - 2 x^{2} - x + 1,
\]
...
\[
4 x^{6} - 4 x^{5} - 7 x^{4} + 8 x^{3} + 2 x^{2} - 4 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A026970.
