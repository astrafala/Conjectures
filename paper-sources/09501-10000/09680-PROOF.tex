...
\title{The empirical recurrence for OEIS A267068, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267068 carries an empirical recurrence of order $12$. The entry separately records a
...
OEIS A267068 is ``a(n) = (n+1) / A189733(n).''. It has offset $0$ and begins
\[
1, 2, 3, 2, 5, 1, 7, 2,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2016 10:58:01, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{11} + x^{10} - 2 x^{9} - 3 x^{8} - 2 x^{7} + 5 x^{6} + x^{5} + 5 x^{4} + 2 x^{3} + 3 x^{2} + 2 x + 1}{x^{12} - 2 x^{6} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{12} - 2 x^{6} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{11} + x^{10} - 2 x^{9} - 3 x^{8} - 2 x^{7} + 5 x^{6} + x^{5} + 5 x^{4} + 2 x^{3} + 3 x^{2} + 2 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267068.
