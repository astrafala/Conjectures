...
\title{The empirical recurrence for OEIS A239530, derived from the entry's generating function}
...
\begin{abstract}
OEIS A239530 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 26 2018 08:58:58, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{2} + x}{3 x^{3} - 4 x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;3 x^{3} - 4 x^{2} - 2 x + 1
\]
