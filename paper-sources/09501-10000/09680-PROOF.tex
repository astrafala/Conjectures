...
\title{The empirical recurrence for OEIS A026927, derived from the entry's generating function}
...
\begin{abstract}
OEIS A026927 carries an empirical recurrence of order $11$. The entry separately records a
...
OEIS A026927 is ``Number of partitions of n into an even number of parts, the greatest being 3; also, a(n+5) = number of partitions of n+2 into an odd number of parts, each <= 3.''. It has offset $1$ and begins
\[
0, 0, 0, 1, 1, 2, 1, 3,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 27 2026 15:22:32, revision 23) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{8} + x^{7} - x^{6} - x^{4}}{x^{11} - x^{10} - x^{7} + x^{6} - x^{5} + x^{4} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{11} + x^{10} + x^{7} - x^{6} + x^{5} - x^{4} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-4) - a(n-5) + a(n-6) - a(n-7) - a(n-10) + a(n-11)
\]
for every $n>8$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A026927.
