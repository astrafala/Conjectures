...
\title{The empirical recurrence for OEIS A202457, derived from the entry's generating function}
...
\begin{abstract}
OEIS A202457 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A202457 is ``Number of (n+2) X 6 binary arrays with consecutive windows of three bits considered as a binary number nondecreasing in every row and column.''. It has offset $1$ and begins
\[
1331, 2835, 5735, 10918, 19614, 33468, 54618, 85779,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 31 2018 11:10:38, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 613 x^{7} + 4003 x^{6} - 10983 x^{5} + 16277 x^{4} - 13841 x^{3} + 6482 x^{2} - 1331 x}{x^{7} - 7 x^{6} + 21 x^{5} - 35 x^{4} + 35 x^{3} - 21 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;613 x^{7} - 4003 x^{6} + 10983 x^{5} - 16277 x^{4} + 13841 x^{3} - 6482 x^{2} + 1331 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202457.
