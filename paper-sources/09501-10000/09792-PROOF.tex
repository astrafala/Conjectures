...
\title{The empirical recurrence for OEIS A269511, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269511 carries an empirical recurrence of order $9$. The entry separately records a
...
OEIS A269511 is ``Partial sums of the number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 342", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 6, 14, 38, 55, 108, 140, 236,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:31, revision 18) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{5 x^{5} + x^{4} + 14 x^{3} + 6 x^{2} + 5 x + 1}{x^{9} - x^{8} - 2 x^{7} + 2 x^{6} + 2 x^{3} - 2 x^{2} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{9} - x^{8} - 2 x^{7} + 2 x^{6} + 2 x^{3} - 2 x^{2} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 2\,a(n-2) - 2\,a(n-3) - 2\,a(n-6) + 2\,a(n-7) + a(n-8) - a(n-9)
\]
for every $n>5$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;5 x^{5} + x^{4} + 14 x^{3} + 6 x^{2} + 5 x + 1,
\]
...
\[
x^{9} - x^{8} - 2 x^{7} + 2 x^{6} + 2 x^{3} - 2 x^{2} - x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269511.
