...
\title{The empirical recurrence for OEIS A230835, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A230835 counts arrays over $\{0,\dots,3\}$ in which every cell is constrained by the
...
OEIS A230835 is ``Number of n X 2 0..3 arrays x(i,j) with each element horizontally, vertically or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, and upper left element zero.''. It has offset $1$ and begins
\[
0, 2, 22, 162, 1202, 8974, 66990, 500018,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 8*a(n-1) - 5*a(n-2) + 8*a(n-3) - 4*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 24 2018 02:47:24, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;8\,a(n-1) - 5\,a(n-2) + 8\,a(n-3) - 4\,a(n-4)
\end{equation}
for all $n>5$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230835.
