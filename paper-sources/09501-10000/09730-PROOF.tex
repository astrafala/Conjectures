...
\title{The empirical recurrence for OEIS A261951, derived from the entry's generating function}
...
\begin{abstract}
OEIS A261951 carries an empirical recurrence of order $10$. The entry separately records a
...
OEIS A261951 is ``Start with a single equilateral triangle for n=0; for the odd n-th generation add a triangle at each expandable vertex of the triangles of the (n-1)-th generation (this is the "vertex to vertex" version); for the even n-th generation use the "vertex to side" version; a(n) is the number of triangles added in the n-th generation.''. It has offset $0$ and begins
\[
1, 3, 9, 12, 24, 24, 39, 27,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 13 2015 02:26:01, revision 24) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{7 x^{10} + 3 x^{9} + 14 x^{8} + 3 x^{7} + 15 x^{6} + 12 x^{5} + 15 x^{4} + 9 x^{3} + 8 x^{2} + 3 x + 1}{x^{10} - x^{8} - x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;7 x^{10} + 3 x^{9} + 14 x^{8} + 3 x^{7} + 15 x^{6} + 12 x^{5} + 15 x^{4} + 9 x^{3} + 8 x^{2} + 3 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A261951.
