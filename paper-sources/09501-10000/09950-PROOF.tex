...
\title{The empirical recurrence for OEIS A020879, derived from the entry's generating function}
...
\begin{abstract}
OEIS A020879 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A020879 is ``Number of elementary edge-subgraphs in Moebius ladder M\_n.''. It has offset $2$ and begins
\[
17, 58, 181, 602, 2006, 6797, 23205, 79771,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 30 2026 16:39:57, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 7 x^{7} - 10 x^{6} + 51 x^{5} + 85 x^{4} + 10 x^{3} - 17 x^{2}}{x^{6} + x^{5} - 8 x^{4} - 11 x^{3} + 2 x^{2} + 4 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{6} - x^{5} + 8 x^{4} + 11 x^{3} - 2 x^{2} - 4 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) + 2\,a(n-2) - 11\,a(n-3) - 8\,a(n-4) + a(n-5) + a(n-6)
\]
for every $n>7$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;7 x^{7} + 10 x^{6} - 51 x^{5} - 85 x^{4} - 10 x^{3} + 17 x^{2},
\]
...
\[
x^{6} + x^{5} - 8 x^{4} - 11 x^{3} + 2 x^{2} + 4 x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A020879.
