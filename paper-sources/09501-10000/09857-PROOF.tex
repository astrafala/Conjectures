...
\title{The empirical recurrence for OEIS A016729, derived from the entry's generating function}
...
\begin{abstract}
OEIS A016729 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A016729 is ``Highest minimal Hamming distance of any Type 4\^{}H+ Hermitian additive self-dual code over GF(4) of length n.''. It has offset $1$ and begins
\[
1, 2, 2, 2, 3, 4, 3, 4,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 05 2025 15:24:35, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{7} + x^{6} + x^{5} + x^{2} + x}{x^{7} - x^{6} - x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A016729.
