...
\title{The empirical recurrence for OEIS A186454, proved by transfer matrix}
...
\begin{abstract}
OEIS A186454 counts the $(n+1)\times2$ arrays over $\{0,\dots,1\}$ in which
...
a state, the count is a walk count on $S=16$ of them, and the conjectured recurrence is then
...
OEIS A186454 is ``Number of (n+1) X 2 binary arrays with every 2 X 2 subblock commuting with each of its horizontal and vertical 2 X 2 subblock neighbors.''. It has offset $1$ and begins
\[
16, 21, 42, 61, 106, 151, 245, 359,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - 2*a(n-3) + 2*a(n-4) - a(n-5) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 28 2018 08:34:17, revision 8) this is still
...
$16$, which the model reproduces along with every later term.
...
\]
on the $S=16$ admissible $2$-tuples. In particular $a$ is $C$-finite.
...
size of the construction is therefore $2^{2\cdot2}$ before the condition prunes
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A186454.
