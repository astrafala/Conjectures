...
\title{The empirical recurrence for OEIS A074356, derived from the entry's generating function}
...
\begin{abstract}
OEIS A074356 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A074356 is ``Coefficient of q\^{}2 in nu(n), where nu(0)=1, nu(1)=b and, for n>=2, nu(n)=b*nu(n-1)+lambda*(1+q+q\^{}2+...+q\^{}(n-2))*nu(n-2) with (b,lambda)=(1,3).''. It has offset $0$ and begins
\[
0, 0, 0, 0, 12, 42, 180, 561,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 23 2017 15:51:39, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{27 x^{7} + 18 x^{6} - 6 x^{5} - 12 x^{4}}{27 x^{6} + 27 x^{5} - 18 x^{4} - 17 x^{3} + 6 x^{2} + 3 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 27 x^{6} - 27 x^{5} + 18 x^{4} + 17 x^{3} - 6 x^{2} - 3 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 6\,a(n-2) - 17\,a(n-3) - 18\,a(n-4) + 27\,a(n-5) + 27\,a(n-6)
\]
for every $n>7$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 27 x^{7} - 18 x^{6} + 6 x^{5} + 12 x^{4},
\]
...
\[
27 x^{6} + 27 x^{5} - 18 x^{4} - 17 x^{3} + 6 x^{2} + 3 x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A074356.
