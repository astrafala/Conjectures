...
\title{The empirical recurrence for OEIS A229016, derived from the entry's generating function}
...
\begin{abstract}
OEIS A229016 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A229016 is ``Number of arrays of median of three adjacent elements of some length 8 0..n array, with no adjacent equal elements in the latter.''. It has offset $1$ and begins
\[
2, 105, 830, 3527, 10860, 27379, 60180, 119653,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 14 2018 11:57:20, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{7} - 9 x^{6} + 4 x^{5} + 148 x^{4} - 137 x^{3} - 91 x^{2} - 2 x}{x^{7} - 7 x^{6} + 21 x^{5} - 35 x^{4} + 35 x^{3} - 21 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{7} + 9 x^{6} - 4 x^{5} - 148 x^{4} + 137 x^{3} + 91 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229016.
