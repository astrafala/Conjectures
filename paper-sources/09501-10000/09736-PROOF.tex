...
\title{The empirical recurrence for OEIS A131607, derived from the entry's generating function}
...
\begin{abstract}
OEIS A131607 carries an empirical recurrence of order $9$. The entry separately records a
...
OEIS A131607 is ``Pell companion numbers A001333 without last digit.''. It has offset $4$ and begins
\[
1, 4, 9, 23, 57, 139, 336, 811,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 05 2020 21:43:05, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{8} + x^{7} - 2 x^{6} + x^{5} + x^{4}}{x^{9} + x^{8} - 3 x^{7} + x^{6} + x^{3} + x^{2} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{9} + x^{8} - 3 x^{7} + x^{6} + x^{3} + x^{2} - 3 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - a(n-2) - a(n-3) - a(n-6) + 3\,a(n-7) - a(n-8) - a(n-9)
\]
for every $n>8$.
\end{theorem}
...
\[
x^{9} + x^{8} - 3 x^{7} + x^{6} + x^{3} + x^{2} - 3 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A131607.
