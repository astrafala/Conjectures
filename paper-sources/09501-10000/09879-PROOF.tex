...
\title{The empirical recurrence for OEIS A243514, proved by transfer matrix}
...
\begin{abstract}
OEIS A243514 carries an empirical recurrence of order $4$ contributed by R.~H.~Hardin. It is
true, and it is decidable rather than empirical. The entry's condition is local to a bounded window of consecutive lines, so
the admissible configurations of such a window are the vertices of a finite digraph and the
arrays counted are exactly the walks in it; hence $a(n)$ is a walk count on $S=67$ vertices
...
OEIS A243514 is ``Number of length n+2 0..5 arrays with no three elements in a row with pattern aba or abb (with a!=b) and new values 0..5 introduced in 0..5 order.''. It has offset $1$ and begins
\[
3, 5, 10, 25, 76, 263, 978, 3773,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 8*a(n-1) - 21*a(n-2) + 22*a(n-3) - 8*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 02 2018 12:29:32, revision 8) this is still
...
admissible array is then exactly a walk, so with $M$ the adjacency matrix on $S=67$ vertices
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A243514.
