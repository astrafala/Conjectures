...
\title{The empirical recurrence for OEIS A064386, derived from the entry's generating function}
...
\begin{abstract}
OEIS A064386 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A064386 is ``Numbers of the form 2\^{}k+1 or 4\^{}k-2\^{}k+1.''. It has offset $1$ and begins
\[
1, 2, 3, 5, 9, 13, 17, 33,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2019 17:08:48, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{9} + 2 x^{6} + 2 x^{5} + 4 x^{4} - x^{3} - x^{2} - x}{8 x^{7} - 8 x^{6} - 6 x^{4} + 6 x^{3} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 8 x^{7} + 8 x^{6} + 6 x^{4} - 6 x^{3} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;8 x^{9} - 2 x^{6} - 2 x^{5} - 4 x^{4} + x^{3} + x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A064386.
