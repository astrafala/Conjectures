...
\title{The empirical recurrence for OEIS A057870, derived from the entry's generating function}
...
\begin{abstract}
OEIS A057870 carries an empirical recurrence of order $13$. The entry separately records a
...
OEIS A057870 is ``Number of singular points on n-th order Chmutov surface.''. It has offset $1$ and begins
\[
0, 1, 3, 14, 28, 57, 93, 154,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:32:43, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{12} + 11 x^{11} + 18 x^{10} + 25 x^{9} + 31 x^{8} + 28 x^{7} + 27 x^{6} + 21 x^{5} + 12 x^{4} + 3 x^{3} + x^{2}}{x^{13} - 2 x^{11} - x^{10} + x^{9} + 2 x^{8} - x^{7} - x^{6} + 2 x^{5} + x^{4} - x^{3} - 2 x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{13} - 2 x^{11} - x^{10} + x^{9} + 2 x^{8} - x^{7} - x^{6} + 2 x^{5} + x^{4} - x^{3} - 2 x^{2} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-2) + a(n-3) - a(n-4) - 2\,a(n-5) + a(n-6) + a(n-7) - 2\,a(n-8) - a(n-9) + a(n-10) + 2\,a(n-11) - a(n-13)
\]
for every $n>12$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{12} + 11 x^{11} + 18 x^{10} + 25 x^{9} + 31 x^{8} + 28 x^{7} + 27 x^{6} + 21 x^{5} + 12 x^{4} + 3 x^{3} + x^{2},
\]
...
\[
x^{13} - 2 x^{11} - x^{10} + x^{9} + 2 x^{8} - x^{7} - x^{6} + 2 x^{5} + x^{4} - x^{3} - 2 x^{2} + 1,
\]
...
compared against all $42$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A057870.
