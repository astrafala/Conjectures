...
\title{The empirical recurrence for OEIS A267454, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267454 carries an empirical recurrence of order $8$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} - 3 x^{5} - 3 x^{4} - 5 x^{3} - 3 x^{2} - 2 x}{x^{8} - x^{7} - x^{5} + x^{3} + x - 1},
\]
