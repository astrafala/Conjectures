...
\title{The empirical recurrence for OEIS A166213, derived from the entry's generating function}
...
\begin{abstract}
OEIS A166213 carries an empirical recurrence of order $12$. The entry separately records a
...
OEIS A166213 is ``Number of 5 X 5 X 5 triangular nonnegative integer arrays, symmetric under 120 degree rotation, with all sums of an element and its neighbors <= n.''. It has offset $0$ and begins
\[
1, 4, 12, 30, 64, 122, 218, 364,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 26 2018 19:24:14, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{4} + 4 x^{3} + 3 x^{2} + 2 x + 1}{x^{12} - 2 x^{11} - x^{10} + 2 x^{9} + 3 x^{8} - 6 x^{6} + 3 x^{4} + 2 x^{3} - x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{12} - 2 x^{11} - x^{10} + 2 x^{9} + 3 x^{8} - 6 x^{6} + 3 x^{4} + 2 x^{3} - x^{2} - 2 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + a(n-2) - 2\,a(n-3) - 3\,a(n-4) + 6\,a(n-6) - 3\,a(n-8) - 2\,a(n-9) + a(n-10) + 2\,a(n-11) - a(n-12)
\]
for every $n>4$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{4} + 4 x^{3} + 3 x^{2} + 2 x + 1,
\]
...
\[
x^{12} - 2 x^{11} - x^{10} + 2 x^{9} + 3 x^{8} - 6 x^{6} + 3 x^{4} + 2 x^{3} - x^{2} - 2 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A166213.
