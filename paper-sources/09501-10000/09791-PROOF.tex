...
\title{The empirical recurrence for OEIS A235895, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235895 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A235895 is ``Number of (n+1) X (1+1) 0..2 arrays with the minimum plus the maximum equal to the lower median plus the upper median in every 2 X 2 subblock.''. It has offset $1$ and begins
\[
33, 125, 473, 1797, 6849, 26189, 100457, 386517,\ \dots
\]
...
\begin{quote}
G.f.: x*(33 - 106*x - 72*x\^{}2) / ((1 - 4*x)*(1 - 3*x - 2*x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 20 2018 03:58:17, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 72 x^{3} - 106 x^{2} + 33 x}{8 x^{3} + 10 x^{2} - 7 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;8 x^{3} + 10 x^{2} - 7 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235895.
