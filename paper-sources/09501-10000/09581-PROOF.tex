...
\title{The empirical recurrence for OEIS A232509, proved by transfer matrix}
...
\begin{abstract}
OEIS A232509 counts arrays of width $3$ over $\{0,\dots,2\}$ under a condition on each cell
and its neighbours, and carries an empirical recurrence of order $7$ contributed by
...
OEIS A232509 is ``Number of (n+1)X(2+1) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally, diagonally or antidiagonally, with no adjacent elements equal.''. It has offset $1$ and begins
\[
16, 52, 200, 784, 3052, 11900, 46432, 181184,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) +a(n-2) -4*a(n-3) -5*a(n-4) -2*a(n-5) +a(n-6) +2*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 07:27:55, revision 8) this is still
...
\]
those falling outside the array being absent. The entry requires that every cell $v$ has, among its neighbours, one equal to $v+1$ and one equal to $v-1$ --- each required only when that value lies in $\{0,\dots,2\}$, which is what ``within the range $0..2$'' says, and no cell equals a neighbour.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232509.
