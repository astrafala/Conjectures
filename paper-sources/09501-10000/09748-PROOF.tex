...
\title{The empirical recurrence for OEIS A164736, derived from the entry's generating function}
...
\begin{abstract}
OEIS A164736 carries an empirical recurrence of order $9$. The entry separately records a
...
OEIS A164736 is ``Number of n-digit cycles of length 5 under the Kaprekar map A151949.''. It has offset $1$ and begins
\[
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 01:52:33, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{15} - x^{14} + 2 x^{13} - x^{12} + x^{11}}{x^{9} - x^{8} - x^{7} + 2 x^{6} - x^{5} - x^{4} + 2 x^{3} - x^{2} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{9} - x^{8} - x^{7} + 2 x^{6} - x^{5} - x^{4} + 2 x^{3} - x^{2} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-2) - 2\,a(n-3) + a(n-4) + a(n-5) - 2\,a(n-6) + a(n-7) + a(n-8) - a(n-9)
\]
for every $n>15$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{15} - x^{14} + 2 x^{13} - x^{12} + x^{11},
\]
a POLYNOMIAL of degree $15$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $15$, so by
Lemma~\ref{lem:key} the residual $a(n)-\sum_ic_ia(n-i)$ vanishes for every
$n>15$. The bound is exact: at $n=15$ the recurrence fails on the entry's own published terms.
\end{proof}
...
\[
x^{9} - x^{8} - x^{7} + 2 x^{6} - x^{5} - x^{4} + 2 x^{3} - x^{2} - x + 1,
\]
...
they stop at index $15$ because that polynomial has degree $15$. A reader who trusts
...
compared against all $81$ terms the entry publishes: it reproduces every one. This is
...
indices beyond $n=15$ and holds at every one, and at $n=15$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A164736.
