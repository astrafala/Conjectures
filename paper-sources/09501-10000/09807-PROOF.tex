...
\title{The empirical recurrence for OEIS A197642, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A197642 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A197642 is ``Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 4,2,2,0,2 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
1, 2, 11, 31, 66, 185, 595, 1760,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) +a(n-2) +a(n-3) +14*a(n-4) -9*a(n-5) -8*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (May 14 2018 05:50:18, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + a(n-2) + a(n-3) + 14\,a(n-4) - 9\,a(n-5) - 8\,a(n-6)
\end{equation}
for all $n>7$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(4,2,2,0,2)
\end{equation}
for every cell $(t,u)$.
...
$5$ values, so there are about $10^{37}$ of them to sift. The
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A197642.
