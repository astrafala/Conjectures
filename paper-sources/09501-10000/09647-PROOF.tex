...
\title{The empirical recurrence for OEIS A230113, derived from the entry's generating function}
...
\begin{abstract}
OEIS A230113 carries an empirical recurrence of order $17$. The entry separately records a
...
OEIS A230113 is ``Digital root of summed Fibonacci and Lucas digital roots indexed by numbers not divisible by 2, 3 or 5.''. It has offset $1$ and begins
\[
3, 4, 5, 6, 6, 5, 4, 3,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 22 2019 12:19:44, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{17} - 2 x^{16} + x^{14} + x^{13} + x^{12} - 2 x^{10} - x^{9} + x^{8} + x^{7} + x^{6} - x^{4} - x^{3} - x^{2} - 3 x}{x^{17} - x^{16} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{17} + x^{16} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;4 x^{17} + 2 x^{16} - x^{14} - x^{13} - x^{12} + 2 x^{10} + x^{9} - x^{8} - x^{7} - x^{6} + x^{4} + x^{3} + x^{2} + 3 x,
\]
a POLYNOMIAL of degree $17$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $17$, so by
...
x^{17} - x^{16} + x - 1,
\]
of degree $17$, and that degree is the order of the SHORTEST linear recurrence with
...
they stop at index $17$ because that polynomial has degree $17$. A reader who trusts
...
compared against all $96$ terms the entry publishes: it reproduces every one. This is
...
indices beyond $n=17$ and holds at every one, and at $n=17$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230113.
