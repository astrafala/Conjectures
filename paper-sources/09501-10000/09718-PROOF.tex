...
\title{The empirical recurrence for OEIS A244240, derived from the entry's generating function}
...
\begin{abstract}
OEIS A244240 carries an empirical recurrence of order $10$. The entry separately records a
...
OEIS A244240 is ``Number of partitions of n into 4 parts such that every i-th smallest part (counted with multiplicity) is different from i.''. It has offset $14$ and begins
\[
1, 4, 7, 11, 15, 19, 25, 30,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 28 2025 09:00:13, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{26} + 2 x^{23} + 3 x^{22} - x^{20} - 5 x^{19} - 3 x^{18} + 2 x^{16} + 3 x^{15} + x^{14}}{x^{10} - x^{9} - x^{8} + 2 x^{5} - x^{2} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{10} - x^{9} - x^{8} + 2 x^{5} - x^{2} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{26} + 2 x^{23} + 3 x^{22} - x^{20} - 5 x^{19} - 3 x^{18} + 2 x^{16} + 3 x^{15} + x^{14},
\]
a POLYNOMIAL of degree $26$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $26$, so by
Lemma~\ref{lem:key} the residual $a(n)-\sum_ic_ia(n-i)$ vanishes for every
$n>26$. The bound is exact: at $n=26$ the recurrence fails on the entry's own published terms.
\end{proof}
...
they stop at index $26$ because that polynomial has degree $26$. A reader who trusts
...
indices beyond $n=26$ and holds at every one, and at $n=26$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A244240.
