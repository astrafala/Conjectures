...
\title{The empirical recurrence for OEIS A267453, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267453 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A267453 is ``Number of OFF (white) cells in the n-th iteration of the "Rule 131" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 2, 3, 5, 5, 8, 7, 10,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 3 x^{5} + 3 x^{4} + 5 x^{3} + 3 x^{2} + 2 x}{x^{7} - x^{4} - x^{3} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + 3 x^{5} + 3 x^{4} + 5 x^{3} + 3 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267453.
