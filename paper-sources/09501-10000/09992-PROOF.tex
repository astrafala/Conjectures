...
\title{The empirical recurrence for OEIS A204710, proved by transfer matrix}
...
\begin{abstract}
OEIS A204710 counts arrays over $\{0,\dots,1\}$ under a condition on the permanent of every
...
a bounded window of consecutive rows, so the count is a walk count on $S=22$ states and is
...
OEIS A204710 is ``Number of (n+1) X 6 0..1 arrays with the permanents of all 2 X 2 subblocks equal and nonzero.''. It has offset $1$ and begins
\[
97, 529, 2689, 14145, 73345, 382849, 1992321, 10382977,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) +15*a(n-2) -38*a(n-3) -52*a(n-4) +72*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 09 2018 06:49:30, revision 9) this is still
...
row is a state and rows $r,s$ may follow one another when all $5$ of their subblocks give
$v$. Taking the disjoint union over $v$ and with $M$ its adjacency matrix,
\[
a(n)\;=\;\iota^{\!\top}M^{\,n}\tau,\qquad\iota=\tau=(1,\dots,1)^{\!\top},
\]
on the $S=22$ states that survive. Rows carrying no edge at all are dropped: such a row could
...
$2\times6$ arrays over $\{0,\dots,1\}$ all of whose subblocks pass. The entry's
first term is $97$, and the model reproduces it along with every later term; that is the
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) + 15\,a(n-2) - 38\,a(n-3) - 52\,a(n-4) + 72\,a(n-5)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204710.
