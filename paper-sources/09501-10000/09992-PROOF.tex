...
\title{The empirical recurrence for OEIS A206261, proved by transfer matrix}
...
\begin{abstract}
OEIS A206261 counts arrays over $\{0,\dots,1\}$ under a condition on the number of rightwards and downwards edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=24$ states and is
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 14 2018 14:45:07, revision 8) this is still
...
\]
on the $S=24$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $2$
subblocks; the entry's first term is $24$, which the model
