...
\title{The empirical recurrence for OEIS A068962, derived from the entry's generating function}
...
\begin{abstract}
OEIS A068962 carries an empirical recurrence of order $16$. The entry separately records a
...
OEIS A068962 is ``Number of successive terms of A028356 that add to n; or length of n-th term of A028355.''. It has offset $1$ and begins
\[
1, 1, 1, 1, 2, 3, 2, 4,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 02 2025 08:28:30, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{15} + x^{11} + x^{10} - x^{9} + 2 x^{8} - x^{7} + x^{6} + x^{5} + x}{x^{16} - x^{15} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{16} - x^{15} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{15} + x^{11} + x^{10} - x^{9} + 2 x^{8} - x^{7} + x^{6} + x^{5} + x,
\]
...
compared against all $77$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A068962.
