...
\title{The empirical recurrence for OEIS A183631, proved by transfer matrix}
...
\begin{abstract}
OEIS A183631 counts $9$-column arrays over $\{0,\dots,2\}$ in which every
...
\mathbf{1}$ for the adjacency matrix $M$ of that digraph, on $S=19683$ vertices. Writing
...
OEIS A183631 is ``Number of (n+1) X 9 0..2 arrays with every 2 X 2 subblock summing to 4.''. It has offset $1$
and begins
\[
20709, 22767, 26901, 35223, 52029, 86127, 155781, 299463,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) - 11*a(n-2) + 6*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 31 2018 14:25:59, revision 12) the statement is
...
Let $V=\{0,1,\dots,2\}^{9}$, the set of possible rows, so $|V|=S=19683$. For
$r,s\in V$ declare $r\to s$ an edge when, for every $j$ with $1\le j\le 9-1$,
\begin{itemize}
...
\begin{proof}
An array with $n+1$ rows and $9$ columns is precisely a sequence
...
bound $j<S=19683$ rather than to a sampled prefix, in exact integer arithmetic, and returned
zero throughout.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A183631.
