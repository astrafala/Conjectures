...
\title{The empirical recurrence for OEIS A267213, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267213 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A267213 is ``Number of OFF (white) cells in the n-th iteration of the "Rule 109" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 2, 0, 5, 2, 4, 3, 10,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{7} - 2 x^{6} + 2 x^{5} + 5 x^{3} + 2 x}{x^{7} - x^{4} - x^{3} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{7} - 2 x^{6} + 2 x^{5} + 5 x^{3} + 2 x,
\]
...
compared against all $67$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267213.
