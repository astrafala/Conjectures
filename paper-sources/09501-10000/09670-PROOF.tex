...
\title{The empirical recurrence for OEIS A194772, derived from the entry's generating function}
...
\begin{abstract}
OEIS A194772 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A194772 is ``Number of lower triangles of an (n+2) X (n+2) 0..2 array with new values introduced in row major order 0..2 and no element unequal to more than one horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
7, 17, 41, 95, 219, 493, 1101, 2427,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 05 2018 08:24:00, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 12 x^{5} - 4 x^{4} + 17 x^{3} + 4 x^{2} - 7 x}{4 x^{5} - 7 x^{3} + x^{2} + 3 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 4 x^{5} + 7 x^{3} - x^{2} - 3 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;12 x^{5} + 4 x^{4} - 17 x^{3} - 4 x^{2} + 7 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A194772.
