...
\title{The empirical recurrence for OEIS A267451, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267451 carries an empirical recurrence of order $7$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{6} + 2 x^{4} + x^{3} + 2 x^{2} + x + 1}{x^{7} - x^{4} - x^{3} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{6} + 2 x^{4} + x^{3} + 2 x^{2} + x + 1,
\]
