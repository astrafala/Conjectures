...
\title{The empirical recurrence for OEIS A283574, proved by transfer matrix}
...
\begin{abstract}
OEIS A283574 counts $3\times n$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
every CELL over a neighbour set the entry names, and it carries an empirical recurrence of
order $12$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than
...
walk, and $a(n)$ is a walk count on $S=90$ vertices, which is $C$-finite. Whether the
...
OEIS A283574 is ``Number of 3Xn 0..1 arrays with no 1 equal to more than one of its horizontal, diagonal and antidiagonal neighbors, with the exception of exactly one element.''. It has offset $1$ and begins
\[
0, 16, 169, 674, 4313, 21186, 104464, 513458,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 11 2017 08:19:25, revision 4) this is still
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) + 10\,a(n-2) - 6\,a(n-3) - 105\,a(n-4) - 108\,a(n-5) - 85\,a(n-6) - 14\,a(n-7) - 250\,a(n-8) + 176\,a(n-9) - 145\,a(n-10) + 44\,a(n-11) - 4\,a(n-12)
\]
for every $n>12$.
\end{theorem}
...
integers vanish from the index corresponding to $n=12+1$ onwards, and the run was
continued until the working vector was identically zero, which settles all larger $j$ at once.
By Lemma~\ref{lem:crit} the recurrence holds for every $n>12$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A283574.
