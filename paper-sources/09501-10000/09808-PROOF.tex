...
\title{The empirical recurrence for OEIS A215230, derived from the entry's generating function}
...
\begin{abstract}
OEIS A215230 carries an empirical recurrence of order $8$. The entry separately records a
...
OEIS A215230 is ``Number of length-7 0..k arrays connected end-around, with no sequence of L<n elements immediately followed by itself (periodic "squarefree").''. It has offset $1$ and begins
\[
0, 0, 840, 10080, 57960, 228480, 710640, 1874880,\ \dots
\]
...
\begin{quote}
G.f.: 840*x\^{}3*(1 + 4*x + x\^{}2) / (1 - x)\^{}8.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2018 06:17:05, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{840 x^{5} + 3360 x^{4} + 840 x^{3}}{x^{8} - 8 x^{7} + 28 x^{6} - 56 x^{5} + 70 x^{4} - 56 x^{3} + 28 x^{2} - 8 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;840 x^{5} + 3360 x^{4} + 840 x^{3},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A215230.
