...
\title{The empirical recurrence for OEIS A317736, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A317736 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A317736 is ``Number of nX3 0..1 arrays with every element unequal to 2 or 3 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
0, 1, 1, 3, 2, 4, 6, 14,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = -a(n-1) -a(n-2) +2*a(n-3) +7*a(n-4) +7*a(n-5) +4*a(n-6) -5*a(n-7) -10*a(n-8) -9*a(n-9) -3*a(n-10) for n>13
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 05 2018 15:56:27, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;-a(n-1) - a(n-2) + 2\,a(n-3) + 7\,a(n-4) + 7\,a(n-5) + 4\,a(n-6) - 5\,a(n-7) - 10\,a(n-8) - 9\,a(n-9) - 3\,a(n-10)
\end{equation}
for all $n>13$.
...
publishes, $n=41$, the arrays have $3\cdot41$ cells over
...
every one of the $41$ published indices, with no shift fitted: the number of rows is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A317736.
