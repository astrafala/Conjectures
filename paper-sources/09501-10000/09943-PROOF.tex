...
\title{The empirical recurrence for OEIS A214385, derived from the entry's generating function}
...
\begin{abstract}
OEIS A214385 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A214385 is ``Number of 3 X 3 X 3 triangular 0..n arrays with no element lying outside the (possibly reversed) range delimited by its sw and se neighbors, and no element equal to its horizontal neighbors.''. It has offset $1$ and begins
\[
8, 94, 456, 1506, 3976, 9044, 18480, 34812,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 22 2018 08:44:08, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{4} + 34 x^{3} - 38 x^{2} - 8 x}{x^{7} - 7 x^{6} + 21 x^{5} - 35 x^{4} + 35 x^{3} - 21 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;8 x^{4} - 34 x^{3} + 38 x^{2} + 8 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A214385.
