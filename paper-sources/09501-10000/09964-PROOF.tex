...
\title{The empirical recurrence for OEIS A246480, derived from the entry's generating function}
...
\begin{abstract}
OEIS A246480 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A246480 is ``Number of length 1+3 0..n arrays with no pair in any consecutive four terms totalling exactly n.''. It has offset $1$ and begins
\[
2, 10, 60, 172, 462, 966, 1880, 3256,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 06 2018 06:44:52, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 46 x^{5} - 12 x^{4} - 32 x^{3} - 4 x^{2} - 2 x}{x^{7} - 3 x^{6} + x^{5} + 5 x^{4} - 5 x^{3} - x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;46 x^{5} + 12 x^{4} + 32 x^{3} + 4 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A246480.
