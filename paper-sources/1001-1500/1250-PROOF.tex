...
\title{The empirical recurrence for OEIS A262846, proved by transfer matrix}
...
\begin{abstract}
OEIS A262846 counts arrays of width $6$ over $\{0,1\}$ whose rows, read as base-$2$
numbers, are divisible by $7$ and whose columns, read the same way, are not divisible by
$7$. The entry carries an empirical recurrence of order $29$ contributed by
...
is then a walk count on $S=2401$ states.
\end{abstract}
...
OEIS A262846 is ``Number of (n+2)X(4+2) 0..1 arrays with each row divisible by 7 and column not divisible by 7, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
318, 3181, 37635, 420654, 3896365, 39348387, 417279054, 3999504445,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2015 16:09:01, revision 4) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array has $6$ cells across and entries $x(i,j)\in\{0,1\}$, the digits of
base $2$. Row $i$ of an array with $R$ rows is the number
\[
\rho_i\;=\;\sum_{j=1}^{6}x(i,j)\,2^{6-j} ,
\]
...
So the vector $\bigl(\gamma_1\bmod 7,\dots,\gamma_{6}\bmod
7\bigr)$ is a state: at most $7^{6}$ of them, and each array row
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=2401.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262846.
