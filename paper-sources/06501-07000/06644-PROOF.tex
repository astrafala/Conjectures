...
\title{The empirical recurrence for OEIS A230750, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A230750 counts arrays over $\{0,\dots,2\}$ in which every cell is constrained by the
...
OEIS A230750 is ``Number of (n+3)X(1+3) 0..2 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 3, and upper left element zero.''. It has offset $1$ and begins
\[
12, 192, 2700, 31212, 363312, 4120752, 46949808, 532746828,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 05:59:41, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;12\,a(n-1) + 18\,a(n-2) - 330\,a(n-3) + 381\,a(n-4) + 1326\,a(n-5) - 1995\,a(n-6) - 1701\,a(n-7) + 2955\,a(n-8) + 462\,a(n-9) - 1434\,a(n-10) + 147\,a(n-11) + 198\,a(n-12) - 33\,a(n-13) - 6\,a(n-14) + a(n-15)
\end{equation}
for all $n>14$.

\section{The condition, and why one row is not enough}

The arrays have $4$ columns and $L=n+3$ rows; write $x_{t,u}$ for the entry in
...
\[
a(n)\;=\;\,\mathbf{v}^{\!\top}N^{\,L-2}\mathbf{u},\qquad L=n+3 .
\]
\end{lemma}
...
publishes, $n=17$, the arrays have $4\cdot20$ cells over
$3$ values, so there are about $10^{38}$ of them to sift. The
...
read off the entry's own name as $L=n+3$ and the entry's offset says which index the first
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230750.
