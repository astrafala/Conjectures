...
\title{The empirical recurrence for OEIS A316125, proved by transfer matrix}
...
\begin{abstract}
OEIS A316125 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{1,2,3,5,6,7\}$, and it carries an empirical
...
OEIS A316125 is ``Number of nX3 0..1 arrays with every element unequal to 1, 2, 3, 5, 6 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 16, 60, 197, 816, 3421, 13488, 54585,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 24 2018 16:40:57, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 4\,a(n-2) + 12\,a(n-3) - 25\,a(n-4) - 79\,a(n-5) - 35\,a(n-6) + 82\,a(n-7) + 219\,a(n-8) + 105\,a(n-9) - 20\,a(n-10) - 245\,a(n-11) - 206\,a(n-12) - 64\,a(n-13) + 38\,a(n-14) + 162\,a(n-15) + 97\,a(n-16) + 46\,a(n-17) + 18\,a(n-18) - 16\,a(n-19) - 8\,a(n-20)
\end{equation}
for all $n>21$.
...
has score in $\{1,2,3,5,6,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,2,3,5,6,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,2,3,5,6,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,2,3,5,6,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,2,3,5,6,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A316125.
