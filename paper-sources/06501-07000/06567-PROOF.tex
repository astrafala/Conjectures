...
\title{The empirical recurrence for OEIS A299340, proved by transfer matrix}
...
\begin{abstract}
OEIS A299340 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{0,1,2,3,5,8\}$, and it carries an empirical
...
OEIS A299340 is ``Number of nX3 0..1 arrays with every element equal to 0, 1, 2, 3, 5 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
4, 26, 93, 357, 1399, 5513, 21642, 85000,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 07 2018 09:49:08, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) - 9\,a(n-2) + 3\,a(n-3) + 7\,a(n-4) - 34\,a(n-5) + 62\,a(n-6) - 54\,a(n-7) + 39\,a(n-8) - 89\,a(n-9) + 52\,a(n-10) + 47\,a(n-11) + 21\,a(n-12) - 33\,a(n-13) - 32\,a(n-14) + 16\,a(n-15) + 12\,a(n-16)
\end{equation}
for all $n>18$.
...
has score in $\{0,1,2,3,5,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,5,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,5,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,5,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,5,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A299340.
