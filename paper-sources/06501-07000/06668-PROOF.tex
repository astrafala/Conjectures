...
\title{The empirical recurrence for OEIS A250946, proved by transfer matrix}
...
\begin{abstract}
OEIS A250946 counts arrays over $\{0,1,2\}$ in which a statistic taken over short runs
...
window of consecutive rows, so the count is a walk count on $S=243$ states, and the
...
OEIS A250946 is ``Number of (4+1) X (n+1) 0..2 arrays with nondecreasing x(i,j)+x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
1333, 10528, 71014, 444421, 2572787, 14019652, 72722706, 362340909,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 14 2025 12:22:57, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array has $5$ cells across and entries $x(i,j)\in\{0,1,2\}$.
...
So a single row is a state. Let $M$ be the matrix on the $3^{5}=243$ rows with
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250946.
