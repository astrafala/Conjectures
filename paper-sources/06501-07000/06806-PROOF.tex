...
\title{The empirical recurrence for OEIS A253701, proved by transfer matrix}
...
\begin{abstract}
OEIS A253701 counts arrays of width $5$ over $\{0,1\}$ in which two diagonal numbers of every $2\times2$ subblock each differ from the same number at a neighbouring subblock. The entry carries an
...
$3$ consecutive rows and the count is a walk count on $S=297$ states.
\end{abstract}
...
OEIS A253701 is ``Number of (4+1)X(n+1) 0..1 arrays with every 2X2 subblock ne-sw antidiagonal difference unequal to its neighbors horizontally and nw+se diagonal sum unequal to its neighbors vertically.''. It has offset $1$ and begins
\[
264, 304, 480, 700, 992, 1262, 1790, 2538,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 14:10:29, revision 6) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The arrays have $5$ columns and a growing number of rows, with entries in $\{0,1\}$.

The entry states two conditions at once. For a $k\times k$ block $B$ of
cells of the array put
\[
f(B)=B_{1,k}-B_{k,1}\qquad\text{compared vertically,}
\]
that is, the northeast entry minus the southwest entry;
\[
g(B)=B_{1,1}+B_{k,k}\qquad\text{compared horizontally,}
\]
...
$(\delta_1,\delta_2)$ the step for the direction concerned --- $(1,0)$ vertically, $(0,1)$ horizontally ---
\[
...
$2\times2$. The entry writes the array with $n$ as the number of COLUMNS. Transposing
...
Take the array one column at a time and let the state be the last 2 rows written so far, with a
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-2) + a(n-4) + a(n-5) - 3\,a(n-6) + 2\,a(n-7) + a(n-8) - 9\,a(n-9) + 2\,a(n-10) + 4\,a(n-11) - 3\,a(n-12) + 7\,a(n-13) - 5\,a(n-15) - a(n-17) + a(n-18) + a(n-19)
\]
for every $n>26$.
\end{theorem}
...
array of width $5$ over $\{0,1\}$ with a few rows was written out cell by cell, every block of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253701.
