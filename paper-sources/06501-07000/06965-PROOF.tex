...
\title{The empirical recurrence for OEIS A303685, proved by transfer matrix}
...
\begin{abstract}
OEIS A303685 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{2,3,4\}$, and it carries an empirical
...
OEIS A303685 is ``Number of nX3 0..1 arrays with every element unequal to 2, 3 or 4 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 5, 2, 20, 27, 108, 191, 599,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 28 2018 09:30:27, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 6\,a(n-2) - 3\,a(n-3) - 9\,a(n-4) + 7\,a(n-5) + 3\,a(n-6) - 20\,a(n-7) - 3\,a(n-8) + 12\,a(n-9) - 12\,a(n-11) - 3\,a(n-12) + 2\,a(n-13) - 5\,a(n-14) + a(n-15) + 2\,a(n-16)
\end{equation}
for all $n>18$.
...
has score in $\{2,3,4\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{2,3,4\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{2,3,4\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{2,3,4\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{2,3,4\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303685.
