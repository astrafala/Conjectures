...
\title{The empirical recurrence for OEIS A233215, proved by transfer matrix}
...
\begin{abstract}
OEIS A233215 counts arrays over $\{0,\dots,5\}$ in which no two entries that are neighbours in
...
order $7$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than
empirical: the clash condition is local to two consecutive rows and the renaming clauses are
carried by one bounded piece of extra state, so the count is a walk count on $S=30$ states.
\end{abstract}
...
OEIS A233215 is ``Number of n X 6 0..5 arrays with no element x(i,j) adjacent to value 5-x(i,j) horizontally or antidiagonally, top left element zero, and 1 appearing before 2 3 and 4, and 2 appearing before 3 in row major order.''. It has offset $1$ and begins
\[
452, 3054973, 23644611625, 183912498314536, 1430752433413485334, 11130643973331790638169, 86591682356567376541898371, 673646513468251754412792158962,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 27 2023 00:56:37, revision 6) this is still
...
(0,1), (1,-1),
\]
that is, in the horizontal and antidiagonal (ne--sw) directions, and the condition is that no two neighbouring cells carry
...
identified; after that identification there are $S=30$ of them.
...
\begin{theorem}
\[
a(n)\;=\;8117\,a(n-1) - 2645074\,a(n-2) + 156363286\,a(n-3) - 2788528505\,a(n-4) + 19055308405\,a(n-5) - 46305234612\,a(n-6) + 29884728384\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233215.
