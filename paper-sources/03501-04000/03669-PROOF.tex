...
\title{The empirical column recurrences for the table OEIS A229521}
...
\begin{abstract}
OEIS A229521 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A229521 is ``T(n,k) = number of n X k 0..2 arrays of the median of the corresponding element, the element to the east and the element to the south in a larger (n+1) X (k+1) 0..2 array without adjacent equal elements in the latter.''. It has offset $1$, is
read by antidiagonals, and begins
\[
3, 7, 7, 17, 33, 17, 39, 129,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 2*a(n-1) + a(n-2) - a(n-3) + a(n-4).\\
k=2: [order 20].\\
k=3: [order 44] for n > 45.\\
k=4: [order 75] for n > 77.\\
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 27 2021 20:57:50, revision 8) these are still
...
The sequence is ``Number of n X 1 0..2 arrays of the median of the corresponding element, the element to the east and the element to the south in a larger (n+1) X2 0..2 array without adjacent equal elements in the latter.''. Its rows are the vertices of a digraph on
...
model reproduces $9$ published values, $22$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229521.
