...
\title{The empirical recurrence for OEIS A209101, proved by transfer matrix}
...
\begin{abstract}
OEIS A209101 counts $2\times n$ arrays over $\{0,\dots,2\}$ subject to a condition imposed on
...
OEIS A209101 is ``Number of 2 X n 0..2 arrays with new values 0..2 introduced in row major order and no element equal to more than one of its immediate leftward or upward or right-upward antidiagonal neighbors.''. It has offset $1$ and begins
\[
2, 11, 76, 520, 3552, 24256, 165632, 1131008,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 8*a(n-1) - 8*a(n-2) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 08 2018 10:48:15, revision 9) this is still
...
By Lemma~\ref{lem:crit} the recurrence holds for every $n>3$. The bound is exact: at $n=3$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A209101.
