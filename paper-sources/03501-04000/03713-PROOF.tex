...
\title{The conjectured generating function for OEIS A183712, proved}
...
\begin{abstract}
OEIS A183712 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=126$ vertices, so its generating function is a rational function whose numerator
...
OEIS A183712 is ``1/20 of the number of (n+1) X 3 0..4 arrays with every 2 X 2 subblock strictly increasing clockwise or counterclockwise with one decrease.''. It has offset $1$ and begins
\[
5, 17, 54, 174, 559, 1797,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(5+2*x-2*x\^{}2) / (1-3*x-x\^{}2+x\^{}3). - \_Colin Barker\_, Mar 16 2016
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 02 2025 23:20:40, revision 41) this is still
...
\]
and there are $S=126$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $129$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{129}$ then $R$ has a zero of order greater than its own
...
of them --- more than the $129+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{3}$ read off $D_2$, the first few being $3,\ 1,\ -1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183712.
