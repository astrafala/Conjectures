...
\title{The empirical column recurrences for the table OEIS A235017}
...
\begin{abstract}
OEIS A235017 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A235017 is ``T(n,k) is the number of (n+1) X (k+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
40, 104, 104, 264, 224, 264, 680, 488,\ \dots
\]
...
k=3: [order 8].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 21:19:42, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X (3) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
$c_{1}$ & $4$ & $c_{3}$ & $-30$ & $c_{5}$ & $74$ & $c_{7}$ & $-60$ \\
$c_{2}$ & $4$ & $c_{4}$ & $7$ & $c_{6}$ & $-46$ & $c_{8}$ & $48$ \\
...
model reproduces $17$ published values, $65$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235017.
