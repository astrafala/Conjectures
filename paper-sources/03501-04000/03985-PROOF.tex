...
\title{The empirical closed form for OEIS A195252, proved}
...
\begin{abstract}
OEIS A195252 counts the lower triangles of a $6\times6$ array over $\{0,\dots,n\}$ in which
...
OEIS A195252 is ``Number of lower triangles of a 6X6 0..n array with each element differing from all of its diagonal, vertical, antidiagonal and horizontal neighbors by two or less.''. It has offset $1$ and begins
\[
2097152, 10460353203, 74236765958, 192172956591, 333437946202, 481573562101,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 06:14:39, revision 9) these are still
...
horizontal, antidiagonal, vertical, diagonal offsets, which here gives $55$ adjacent pairs. Each cell carries a value in
...
Every $y\in\mathcal P$ has $\mathrm{sp}(y)\le 2\cdot5=10$, where $5$ is the
diameter of the neighbour graph.
\end{lemma}

\begin{proof}
The neighbour graph is connected, so any two cells are joined by a path of at most $5$
edges, and each edge changes the value by at most $2$. Hence the largest and the smallest
entry differ by at most $2\cdot5$.
\end{proof}
...
\[
a(n)\;=\;149824887097\,n-417651128341\qquad\text{for every }n\ge9,
\]
and $\mathcal P$ has exactly $149824887097$ elements.
\end{theorem}
...
$a(10+1)$ were computed exactly, giving slope $149824887097$ and the stated intercept; the slope
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195252.
