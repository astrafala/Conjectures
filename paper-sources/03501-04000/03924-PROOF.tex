...
\title{The empirical recurrence for OEIS A224130, proved by transfer matrix}
...
\begin{abstract}
OEIS A224130 counts arrays over $\{0,\dots,1\}$ whose rows, columns, diagonals or
antidiagonals are required to be monotone or unimodal, and carries an empirical recurrence of
order $5$ contributed by R.~H.~Hardin. A monotone condition is local, but a unimodal one
...
OEIS A224130 is ``Number of n X 5 0..1 arrays with rows nondecreasing and antidiagonals unimodal.''. It has offset $1$ and begins
\[
6, 36, 175, 760, 3149, 12874, 52581, 215136,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) - 9*a(n-2) + 4*a(n-3) + 2*a(n-4) + 8*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 27 2018 06:04:24, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) - 9\,a(n-2) + 4\,a(n-3) + 2\,a(n-4) + 8\,a(n-5)
\end{equation}
for all $n>5$.
...
$h_j=G^{j}N^{o}\mathbf{1}$, we have for $j\ge5$
\[
...
every one of the $22$ published indices, with no shift fitted. The values agree exactly.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224130.
