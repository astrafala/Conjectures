...
\title{The empirical recurrence for OEIS A269896, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A269896 counts arrays over $\{0,\dots,6\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=98$ vertices,
...
OEIS A269896 is ``Number of n X 2 0..6 arrays with some element plus some horizontally or vertically adjacent neighbor totalling six exactly once.''. It has offset $1$ and begins
\[
7, 840, 45360, 2005080, 80724000, 3078006120, 113260615440, 4064194275960,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 62*a(n-1) - 961*a(n-2) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 29 2019 15:06:46, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;62\,a(n-1) - 961\,a(n-2)
\end{equation}
for all $n>3$.
...
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,6\}^{2},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=98,
\]
...
the digraph dense, with $(6+1)^{2\cdot2}$ edges instead of the sparse set used here.
\end{remark}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269896.
