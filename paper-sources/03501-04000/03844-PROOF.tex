...
\title{The empirical column recurrences for the table OEIS A303682}
...
\begin{abstract}
OEIS A303682 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 2, 3, 4, 5, 6$.
\end{abstract}
...
OEIS A303682 is ``T(n,k) = Number of n X k 0..1 arrays with every element unequal to 0, 1 or 3 king-move adjacent elements, with upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 3, 5, 3, 5, 7,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 10 2023 13:39:23, revision 6) these are still
...
column $k$$=1$ & $2$ & $4$ & $>3$ & $12$ \\
column $k$$=2$ & $4$ & $16$ & $>5$ & $11$ \\
column $k$$=3$ & $4$ & $64$ & $>5$ & $9$ \\
column $k$$=4$ & $5$ & $256$ & $>8$ & $8$ \\
column $k$$=5$ & $6$ & $1024$ & $>10$ & $9$ \\
column $k$$=6$ & $7$ & $4096$ & $>12$ & $9$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of n X 1 0..1 arrays with every element unequal to 0, 1 or 3 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 2 0..1 arrays with every element unequal to 0, 1 or 3 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 3 0..1 arrays with every element unequal to 0, 1 or 3 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 4 0..1 arrays with every element unequal to 0, 1 or 3 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 5 0..1 arrays with every element unequal to 0, 1 or 3 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 6 0..1 arrays with every element unequal to 0, 1 or 3 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
model reproduces $58$ published values, $122$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303682.
