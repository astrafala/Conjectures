...
\title{The empirical column recurrences for the table OEIS A237859}
...
\begin{abstract}
OEIS A237859 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A237859 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with no element greater than all horizontal neighbors or less than all vertical neighbors.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 4, 4, 7, 14, 7, 12, 41,\ \dots
\]
...
k=6: [order 98]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:31:25, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..1 arrays with no element greater than all horizontal neighbors or less than all vertical neighbors.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..1 arrays with no element greater than all horizontal neighbors or less than all vertical neighbors.''. Its rows are the vertices of a digraph on
...
$S=56$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=112$ exact terms returns the minimal one, of order
$11$ --- the stated order --- with integer coefficients
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $5$ & $c_{4}$ & $11$ & $c_{7}$ & $9$ & $c_{10}$ & $0$ \\
$c_{2}$ & $-6$ & $c_{5}$ & $-5$ & $c_{8}$ & $-10$ & $c_{11}$ & $1$ \\
$c_{3}$ & $13$ & $c_{6}$ & $4$ & $c_{9}$ & $-10$ &  &  \\
...
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $8$ & $c_{7}$ & $271$ & $c_{13}$ & $407$ & $c_{19}$ & $-17$ \\
$c_{2}$ & $-20$ & $c_{8}$ & $-483$ & $c_{14}$ & $-1089$ & $c_{20}$ & $-1$ \\
$c_{3}$ & $71$ & $c_{9}$ & $-760$ & $c_{15}$ & $-275$ & $c_{21}$ & $1$ \\
$c_{4}$ & $6$ & $c_{10}$ & $-1175$ & $c_{16}$ & $318$ &  &  \\
$c_{5}$ & $62$ & $c_{11}$ & $355$ & $c_{17}$ & $82$ &  &  \\
$c_{6}$ & $230$ & $c_{12}$ & $1437$ & $c_{18}$ & $-25$ &  &  \\
\end{tabular}\end{center}
...
model reproduces $19$ published values, $47$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237859.
