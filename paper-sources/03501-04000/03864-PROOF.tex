...
\title{The conjectured generating function for OEIS A282454, proved}
...
\begin{abstract}
OEIS A282454 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (-8*x\^{}3 + 2*x\^{}2 + 2*x + 1)/(4*x\^{}4 - 5*x\^{}2 + 1). (End)
\end{quote}
