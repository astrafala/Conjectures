...
\title{The empirical recurrence for OEIS A224572, proved by transfer matrix}
...
\begin{abstract}
OEIS A224572 counts the $(n+5)\times8$ matrices over $\{0,\dots,1\}$ all of whose
$6\times6$ contiguous subblocks are idempotent, and carries an empirical recurrence of order
$16$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical.
...
makes $a(n)$ a walk count on $S=20040$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224572 is ``Number of (n+5) X 8 0..1 matrices with each 6 X 6 subblock idempotent.''. It has offset $1$ and begins
\[
16696, 5696, 5896, 6659, 7295, 7884, 8294, 9831,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 21 2024 22:56:30, revision 6) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $6\times6$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $6\times8$ matrix a \emph{band} when every one of its $3$ contiguous
$6\times6$ subblocks is a window. What the entry counts is the matrices of $8$ columns
whose every $6$ consecutive rows form a band.

Take as states the $5\times8$ strips that occur as the top $5$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=20040$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{40}$ strips of $5$ rows and $8$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $6\times5$ entries, so a band is exactly a walk of $3$ windows in
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 4\,a(n-2) + 4\,a(n-3) - 3\,a(n-4) + a(n-5) + 2\,a(n-6) - 5\,a(n-7) + 6\,a(n-8) - 6\,a(n-9) + 4\,a(n-10) - a(n-11) - a(n-12) + 2\,a(n-13) - 2\,a(n-14) + 2\,a(n-15) - a(n-16)
\]
for every $n>21$.
\end{theorem}
...
until the working vector was identically zero, which settles every larger $j$ at once. The bound is exact: at $n=21$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224572.
