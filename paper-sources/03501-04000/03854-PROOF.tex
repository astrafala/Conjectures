...
\title{The empirical closed form for OEIS A184541, proved}
...
\begin{abstract}
OEIS A184541 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=65$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A184541 is ``Number of (n+2) X 4 binary arrays with each 3 X 3 subblock having rows and columns in lexicographically nondecreasing order.''. It has offset $1$ and begins
\[
89, 193, 340, 537, 792, 1114, 1513, 2000,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = (1/24)*n\^{}4 + (3/4)*n\^{}3 + (383/24)*n\^{}2 + (201/4)*n + 22.
\end{quote}
As of the ``Last modified'' line on the live entry (May 25 2021 05:09:59, revision 11) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;\frac{n^{4}}{24} + \frac{3 n^{3}}{4} + \frac{383 n^{2}}{24} + \frac{201 n}{4} + 22.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=65$ vertices, and $u,w$ the
...
vanish from the point corresponding to $n=6$ onwards, and the run was continued until the
working vector was identically zero, which settles every larger $j$ at once. So $a$ satisfies
\eqref{eq:rec} for every $n>5$. By Lemma~\ref{lem:ann} $f$ satisfies \eqref{eq:rec}
identically. Both were then evaluated in exact arithmetic at the $5$ consecutive indices
$n=6,\dots,10$ and agree at each. A monic recurrence of order $5$
determines $a(m)$ from $a(m-1),\dots,a(m-5)$, and for every $m\ge11$ both
$m>5$ and $m-5\ge6$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge6$.
Below $n=6$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=6$ on. The remaining indices $n=1,\dots,5$ were
...
First, the walk model reproduces every one of the $38$ terms the entry publishes,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184541.
