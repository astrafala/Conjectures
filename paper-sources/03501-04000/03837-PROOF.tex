...
\title{The empirical column recurrences for the table OEIS A235223}
...
\begin{abstract}
OEIS A235223 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A235223 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with 2X2 edge jumps all no more than +1 in one of the clockwise or counterclockwise directions or both.''. It has offset $1$, is
read by antidiagonals, and begins
\[
59, 427, 427, 3037, 8029, 3037, 21693, 145669,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 6*a(n-1) +9*a(n-2) -6*a(n-3) -2*a(n-4)\\
k=2: [order 8]\\
k=3: [order 21]\\
k=4: [order 59]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:30:01, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..2 arrays with 2X2 edge jumps all no more than +1 in one of the clockwise or counterclockwise directions or both.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;6\,a(n-1) + 9\,a(n-2) - 6\,a(n-3) - 2\,a(n-4)
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235223.
