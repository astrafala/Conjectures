...
\title{The empirical column recurrences for the table OEIS A252269}
...
\begin{abstract}
OEIS A252269 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A252269 is ``T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row, column, diagonal and antidiagonal sum not equal to 2 3 4 6 or 7.''. It has offset $1$, is
read by antidiagonals, and begins
\[
103, 130, 130, 258, 272, 258, 598, 822,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1) +3*a(n-3) +4*a(n-4) +4*a(n-5) +2*a(n-6) for n>8\\
k=2: [order 8] for n>9\\
k=3: [order 15] for n>17\\
k=4: [order 23] for n>25\\
k=5: [order 43] for n>45\\
k=6: [order 71] for n>73\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:59:55, revision 6) these are still
...
The sequence is ``Number of (n+2)X(3) 0..3 arrays with every 3X3 subblock row, column, diagonal and antidiagonal sum not equal to 2 3 4 6 or 7.''. Its rows are the vertices of a digraph on
...
model reproduces $9$ published values, $34$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252269.
