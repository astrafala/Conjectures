...
\title{The empirical column recurrences for the table OEIS A204417}
...
\begin{abstract}
OEIS A204417 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A204417 is ``T(n,k)=Number of (n+2)X(k+2) 0..2 arrays with every 3X3 subblock having three strictly increasing elements in a row horizontally or vertically, exactly one way.''. It has offset $1$, is
read by antidiagonals, and begins
\[
3570, 34274, 34274, 423416, 154938, 423416, 2827014, 781884,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 11:14:00, revision 9) these are still
...
column $k$$=4$ & $3$ & $11737$ & $>15$ & $8$ \\
column $k$$=5$ & $3$ & $59692$ & $>16$ & $8$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X(6) 0..2 arrays with every 3X3 subblock having three strictly increasing elements in a row horizontally or vertically, exactly one way.''. Its rows are the vertices of a digraph on
$S=11737$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+2)X(7) 0..2 arrays with every 3X3 subblock having three strictly increasing elements in a row horizontally or vertically, exactly one way.''. Its rows are the vertices of a digraph on
$S=59692$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;577\,a(n-3)
...
model reproduces $16$ published values, $140$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204417.
