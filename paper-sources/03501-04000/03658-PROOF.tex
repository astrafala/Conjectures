...
\title{The conjectured generating function for OEIS A183645, proved}
...
\begin{abstract}
OEIS A183645 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=125$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $137$ coefficients in exact arithmetic
...
OEIS A183645 is ``Number of (n+1) X 3 0..4 arrays with every 2 X 2 subblock summing to 8.''. It has offset $1$ and begins
\[
325, 1001, 3445, 12785, 50125, 205001,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(325 - 3874*x + 16055*x\^{}2 - 26930*x\^{}3 + 15000*x\^{}4) / ((1 - x)*(1 - 2*x)*(1 - 3*x)*(1 - 4*x)*(1 - 5*x)). - \_Colin Barker\_, Apr 01 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 18 2026 07:58:05, revision 25) this is still
...
\]
and there are $S=125$ states. The digraph is built by reading the entry's own English, and the
...
The family this entry belongs to is read by one model, described by its author as: ``Generic transfer matrix for Hardin arrays whose condition is local to each 2 X 2 block.''
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $137$
...
numerator. Its coefficients are the integers $c_1,\dots,c_{5}$ read off $D_2$, the first few being $15,\ -85,\ 225,\ -274,\ 120$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183645.
