...
\title{The conjectured generating function for OEIS A272567, proved}
...
\begin{abstract}
OEIS A272567 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A272567 is ``First differences of number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 501", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
7, -3, 39, -31, 99, -87,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:33, revision 13) this is still
...
model reproduces all $43$ terms the entry publishes, exactly, in integer arithmetic --- a
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=10$ and $\deg D_2=5$; its expansion as a
...
\[
a(n)\;=\;\sum_{i\ge1}c_i\,a(n-i)\qquad\text{for every }n>10,
\]
a constant-coefficient linear recurrence of order $5$, valid past the degree of the
numerator. Its coefficients are the integers $c_1,\dots,c_{5}$ read off $D_2$, the first few being $-1,\ 2,\ 2,\ -1,\ -1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
First, the model reproduces every one of the $43$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A272567.
