...
\title{The empirical closed form for OEIS A188862, proved}
...
\begin{abstract}
OEIS A188862 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=32$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A188862 is ``Number of n X 5 binary arrays without the pattern 0 1 diagonally, vertically or antidiagonally.''. It has offset $1$ and begins
\[
32, 99, 178, 259, 340, 421, 502, 583,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 81*n - 65 for n>2.
\end{quote}
As of the ``Last modified'' line on the live entry (May 01 2018 08:43:22, revision 8) this is still
...
walk, so with $M$ the adjacency matrix of that digraph on $S=32$ vertices, and $u,w$ the
...
$n=5,\dots,6$ and agree at each. A monic recurrence of order $2$
determines $a(m)$ from $a(m-1),\dots,a(m-2)$, and for every $m\ge7$ both
$m>4$ and $m-2\ge5$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge5$.
Below $n=5$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=5$ on. The remaining indices $n=3,\dots,4$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188862.
