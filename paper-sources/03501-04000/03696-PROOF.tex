...
\title{The empirical column recurrences for the table OEIS A264090}
...
\begin{abstract}
OEIS A264090 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264090 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having index change +-(.,.) 0,0 0,1 or 2,-2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
4, 9, 8, 25, 39, 16, 64, 245,\ \dots
\]
...
k=3: a(n) = 10*a(n-1) for n>3\\
k=4: a(n) = 19*a(n-1) +304*a(n-3) +256*a(n-4)\\
k=5: [order 14]\\
k=6: [order 25] for n>27\\
k=7: [order 25] for n>27\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 11:22:06, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having index change +-(.,.) 0,0 0,1 or 2,-2.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having index change +-(.,.) 0,0 0,1 or 2,-2.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $17$ published values, $41$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264090.
