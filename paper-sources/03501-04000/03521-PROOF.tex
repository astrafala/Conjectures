...
\title{The empirical column recurrences for the table OEIS A264217}
...
\begin{abstract}
OEIS A264217 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264217 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 0,0 0,1 1,0 -1,0 or 0,-2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
4, 17, 9, 56, 96, 25, 172, 623,\ \dots
\]
...
k=3: [order 23]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 11:41:35, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 0,0 0,1 1,0 -1,0 or 0,-2.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having directed index change 0,0 0,1 1,0 -1,0 or 0,-2.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-1) + 2\,a(n-2) + 6\,a(n-3) + 2\,a(n-4) - 2\,a(n-5) - a(n-6)
\]
...
$c_{1}$ & $6$ & $c_{3}$ & $-16$ & $c_{5}$ & $16$ & $c_{7}$ & $-6$ \\
$c_{2}$ & $3$ & $c_{4}$ & $3$ & $c_{6}$ & $3$ & $c_{8}$ & $-1$ \\
\end{tabular}\end{center}
Repeating with the first $8+4$ terms removed gives order $8$ again.

\subsection*{Column $k=3$, recovered}
The entry gives only the order for this line: ``k=3: [order 23]''. Its model is a walk on
...
$23$ --- the stated order --- with integer coefficients
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $17$ & $c_{7}$ & $12721$ & $c_{13}$ & $5279$ & $c_{19}$ & $-1433$ \\
$c_{2}$ & $-43$ & $c_{8}$ & $3719$ & $c_{14}$ & $22631$ & $c_{20}$ & $285$ \\
$c_{3}$ & $-285$ & $c_{9}$ & $-22631$ & $c_{15}$ & $-3719$ & $c_{21}$ & $43$ \\
$c_{4}$ & $1433$ & $c_{10}$ & $-5279$ & $c_{16}$ & $-12721$ & $c_{22}$ & $-17$ \\
$c_{5}$ & $-479$ & $c_{11}$ & $61275$ & $c_{17}$ & $7539$ & $c_{23}$ & $1$ \\
$c_{6}$ & $-7539$ & $c_{12}$ & $-61275$ & $c_{18}$ & $479$ &  &  \\
\end{tabular}\end{center}
Repeating with the first $23+4$ terms removed gives order $23$ again.
...
model reproduces $17$ published values, $49$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264217.
