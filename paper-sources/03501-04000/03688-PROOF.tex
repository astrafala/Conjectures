...
\title{The empirical column recurrences for the table OEIS A262332}
...
\begin{abstract}
OEIS A262332 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A262332 is ``T(n,k) = Number of (n+1) X (k+1) 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 3, 3, 6, 5, 6, 11, 15,\ \dots
\]
...
k=7: [order 19]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 20 2025 09:51:03, revision 22) these are still
...
The sequence is ``Number of (n+1) X2 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X3 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X4 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262332.
