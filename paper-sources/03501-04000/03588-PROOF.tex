...
\title{The empirical column recurrences for the table OEIS A253231}
...
\begin{abstract}
OEIS A253231 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A253231 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with every 2X2 subblock diagonal minimum minus antidiagonal minimum nondecreasing horizontally and diagonal maximum minus antidiagonal maximum nondecreasing vertically.''. It has offset $1$, is
read by antidiagonals, and begins
\[
16, 47, 47, 125, 132, 125, 335, 306,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 3*a(n-1) -2*a(n-3) for n>6\\
k=2: a(n) = 5*a(n-1) -8*a(n-2) +4*a(n-3) for n>7\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 13:55:27, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..1 arrays with every 2X2 subblock diagonal minimum minus antidiagonal minimum nondecreasing horizontally and diagonal maximum minus antidiagonal maximum nondecreasing vertically.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..1 arrays with every 2X2 subblock diagonal minimum minus antidiagonal minimum nondecreasing horizontally and diagonal maximum minus antidiagonal maximum nondecreasing vertically.''. Its rows are the vertices of a digraph on
$S=47$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $17$ published values, $59$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253231.
