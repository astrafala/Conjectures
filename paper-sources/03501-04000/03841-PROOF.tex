...
\title{The empirical recurrence for OEIS A198901, proved by counting patterns}
...
\begin{abstract}
OEIS A198901 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $225$ to $S=20$. The entry's empirical recurrence of order
...
OEIS A198901 is ``Number of n X 3 0..4 arrays with values 0..4 introduced in row major order and no element equal to any horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
2, 33, 1211, 50384, 2125425, 89793204, 3794115705, 160319061892,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 51*a(n-1) - 393*a(n-2) + 1013*a(n-3) - 902*a(n-4) + 232*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 02 2018 10:49:53, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;51\,a(n-1) - 393\,a(n-2) + 1013\,a(n-3) - 902\,a(n-4) + 232\,a(n-5)
\end{equation}
for all $n>5$.
...
Assembling the $5$ lumped chains into one block-diagonal matrix $N$ of size $S=20$ and
...
\end{equation}
The unlumped alphabet had $225$ rows in all; the lumped chain has $S=20$ states.
...
\[
5!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-5}q(N)\mathbf{1} .
\]
...
\eqref{eq:walk}, divided by $5!$, was evaluated at every one of the $15$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198901.
