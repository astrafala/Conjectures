...
\title{The empirical recurrence for OEIS A208707, proved by counting patterns}
...
\begin{abstract}
OEIS A208707 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
$6$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A208707 is ``Number of n X 6 0..1 arrays with new values 0..1 introduced in row major order and no element equal to more than two of its immediate leftward or upward or right-upward antidiagonal neighbors.''. It has offset $1$ and begins
\[
32, 1268, 52184, 2142580, 87985748, 3613193828, 148378294612, 6093257064980,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 45*a(n-1) - 168*a(n-2) + 272*a(n-3) - 324*a(n-4) + 240*a(n-5) - 64*a(n-6) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 06 2018 08:11:44, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;45\,a(n-1) - 168\,a(n-2) + 272\,a(n-3) - 324\,a(n-4) + 240\,a(n-5) - 64\,a(n-6)
\end{equation}
for all $n>7$.
...
For every $n$ with $m=n+0-1\ge6$,
\[
2!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-6}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-6}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $6$ matrix--vector products
...
corresponding to $n=7+1$ onwards, and the run continued until $w$ was the zero vector,
...
\eqref{eq:walk}, divided by $2!$, was evaluated at every one of the $14$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208707.
