...
\title{The empirical column recurrences for the table OEIS A234786}
...
\begin{abstract}
OEIS A234786 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234786 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with no adjacent elements equal and with each 2X2 subblock having the number of clockwise edge increases equal to the number of counterclockwise edge increases.''. It has offset $1$, is
read by antidiagonals, and begins
\[
76, 484, 484, 3084, 6660, 3084, 19652, 91916,\ \dots
\]
...
k=5: [order 70]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:27:04, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..3 arrays with no adjacent elements equal and with each 2X2 subblock having the number of clockwise edge increases equal to the number of counterclockwise edge increases.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..3 arrays with no adjacent elements equal and with each 2X2 subblock having the number of clockwise edge increases equal to the number of counterclockwise edge increases.''. Its rows are the vertices of a digraph on
$S=37$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;16\,a(n-1) - 31\,a(n-2) + 10\,a(n-3)
\]
...
model reproduces $13$ published values, $69$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234786.
