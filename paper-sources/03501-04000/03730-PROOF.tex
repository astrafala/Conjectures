...
\title{The empirical column recurrences for the table OEIS A224568}
...
\begin{abstract}
OEIS A224568 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 3, 5, 7$.
\end{abstract}
...
OEIS A224568 is ``T(n,k)=Number of (n+3)X(k+3) 0..1 matrices with each 4X4 subblock idempotent.''. It has offset $1$, is
read by antidiagonals, and begins
\[
452, 370, 370, 512, 340, 512, 739, 428,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:28:01, revision 8) these are still
...
column $k$$=1$ & $8$ & $416$ & $>10$ & $10$ \\
column $k$$=3$ & $8$ & $517$ & $>10$ & $10$ \\
column $k$$=5$ & $8$ & $990$ & $>10$ & $10$ \\
column $k$$=7$ & $8$ & $1905$ & $>10$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+3)X4 0..1 matrices with each 4X4 subblock idempotent.''. Its rows are the vertices of a digraph on
$S=416$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+3)X6 0..1 matrices with each 4X4 subblock idempotent.''. Its rows are the vertices of a digraph on
$S=517$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of (n+3)X8 0..1 matrices with each 4X4 subblock idempotent.''. Its rows are the vertices of a digraph on
$S=990$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1) - 2\,a(n-2) - 2\,a(n-3) + 4\,a(n-4) - 3\,a(n-5) + 2\,a(n-7) - a(n-8)
...
The sequence is ``Number of (n+3)X10 0..1 matrices with each 4X4 subblock idempotent.''. Its rows are the vertices of a digraph on
$S=1905$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1) - 2\,a(n-2) - 2\,a(n-3) + 4\,a(n-4) - 3\,a(n-5) + 2\,a(n-7) - a(n-8)
...
model reproduces $40$ published values, $144$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224568.
