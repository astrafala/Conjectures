...
\title{The empirical recurrence for OEIS A231909, proved by transfer matrix}
...
\begin{abstract}
OEIS A231909 counts arrays of width $2$ over $\{0,1,2\}$ in which no cell is beaten by a strict majority of its neighbours. The entry carries an
...
$3$ consecutive rows and the count is a walk count on $S=85$ states.
\end{abstract}
...
OEIS A231909 is ``Number of 2 X n 0..2 arrays with no element having a strict majority of its horizontal, diagonal and antidiagonal neighbors equal to itself plus one mod 3, with upper left element zero (rock paper and scissors drawn positions).''. It has offset $1$ and begins
\[
3, 17, 137, 948, 6975, 50323, 366170, 2657785,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 9*a(n-1) - 5*a(n-2) - 66*a(n-3) + 78*a(n-4) + 4*a(n-5) - 16*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 01 2018 09:14:13, revision 8) this is still
...
the border simply has fewer neighbours. The entry writes the array with $n$ as the number of COLUMNS; transposing it exchanges the two coordinates of every offset, and the offsets above are already written in the transposed frame.
...
$3$ consecutive rows, and nothing wider. Take the array one column at a time
...
\begin{theorem}
\[
a(n)\;=\;9\,a(n-1) - 5\,a(n-2) - 66\,a(n-3) + 78\,a(n-4) + 4\,a(n-5) - 16\,a(n-6)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231909.
