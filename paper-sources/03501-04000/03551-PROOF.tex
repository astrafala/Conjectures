...
\title{The empirical column recurrences for the table OEIS A229534}
...
\begin{abstract}
OEIS A229534 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the rows $n=1, 2, 3$.
\end{abstract}
...
OEIS A229534 is ``T(n,k) = number of defective 3-colorings of an n X k 0..2 array connected horizontally, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 1, 0, 2, 4, 0, 6, 8,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 27 2021 21:24:32, revision 9) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of defective 3-colorings of an 1 X n 0..2 array connected horizontally, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of defective 3-colorings of an 2 X n 0..2 array connected horizontally, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
$S=28$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
\subsection*{Row $n$$=3$}
The sequence is ``Number of defective 3-colorings of an 3 X n 0..2 array connected horizontally, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
model reproduces $27$ published values, $71$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229534.
