...
\title{The empirical recurrence for OEIS A263375, proved by transfer matrix}
...
\begin{abstract}
OEIS A263375 counts arrays of width $2$ over $\{0,1,\dots,4\}$ whose rows, read as base-$5$
numbers, are divisible by $9$ and whose columns, read the same way, are divisible by
$9$. The entry carries an empirical recurrence of order $9$ contributed by
...
OEIS A263375 is ``Number of (n+1) X (1+1) 0..4 arrays with each row and column divisible by 9, read as a base-5 number with top and left being the most significant digits.''. It has offset $1$ and begins
\[
2, 4, 11, 31, 89, 253, 748, 2214,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 3*a(n-2) - 9*a(n-3) + 3*a(n-6) - 9*a(n-7) - 9*a(n-8) + 27*a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 01 2019 20:13:30, revision 8) this is still
...
\gamma_j^{(i)}\bmod 9\;=\;
\bigl(5\,(\gamma_j^{(i-1)}\bmod 9)+x(i,j)\bigr)\bmod 9 .
\]
\end{lemma}
...
So the vector $\bigl(\gamma_1\bmod 9,\dots,\gamma_{2}\bmod
9\bigr)$ is a state: at most $9^{2}$ of them, and each array row
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 3\,a(n-2) - 9\,a(n-3) + 3\,a(n-6) - 9\,a(n-7) - 9\,a(n-8) + 27\,a(n-9)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263375.
