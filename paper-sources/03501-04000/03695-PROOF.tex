...
\title{The empirical recurrence for OEIS A263366, proved by transfer matrix}
...
\begin{abstract}
OEIS A263366 counts arrays of width $2$ over $\{0,1,2\}$ whose rows, read as base-$3$
numbers, are divisible by $7$ and whose columns, read the same way, are divisible by
$7$. The entry carries an empirical recurrence of order $6$ contributed by
...
OEIS A263366 is ``Number of (n+1) X (1+1) 0..2 arrays with each row and column divisible by 7, read as a base-3 number with top and left being the most significant digits.''. It has offset $1$ and begins
\[
1, 1, 2, 5, 10, 19, 37, 73,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) - 3*a(n-2) + 3*a(n-3) - 3*a(n-4) + 3*a(n-5) - 2*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 01 2019 20:13:23, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array has $2$ cells across and entries $x(i,j)\in\{0,1,2\}$, the digits of
base $3$. Row $i$ of an array with $R$ rows is the number
\[
\rho_i\;=\;\sum_{j=1}^{2}x(i,j)\,3^{2-j} ,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263366.
