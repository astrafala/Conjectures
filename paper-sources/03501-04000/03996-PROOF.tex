...
\title{Arrays counted up to renaming: the empirical recurrence for OEIS A205220}
...
\begin{abstract}
OEIS A205220 carries an empirical recurrence of order $2$ contributed by R.~H.~Hardin. It is
...
OEIS A205220 is ``Number of (n+1) X 3 0..1 arrays with the number of equal 2 X 2 subblock diagonal pairs and equal antidiagonal pairs differing from each horizontal or vertical neighbor, and new values 0..1 introduced in row major order.''. It has offset $1$ and begins
\[
20, 52, 132, 340, 868, 2228, 5700, 14612,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 05:10:17, revision 10) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays of 3 columns over the alphabet $\{0,1\}$, and asks two different kinds of thing of them at once.
...
What is compared \emph{across} windows is carried too: the state holds the scores of the whole last band, one number per subblock, since the band arriving next is compared with it subblock by subblock. With $2$ subblocks to a band that is a bounded amount of bookkeeping and no more.

\textbf{The relabelling condition is not local.} ``New values introduced in row major order'' says that, reading the array one row at a time and each row left to right, a value may appear for the first time only once every smaller value already has. Whether a given entry is allowed here therefore depends on the whole prefix of the array, not on any bounded window of it, and no amount of widening the window captures it. What saves it is that the prefix matters only through one number: let $m$ be how many values have been introduced so far. A row $r_1r_2\cdots r_3$ may follow precisely when each $r_i\le m$ at the moment it is read and each $r_i=m$ raises $m$ by one. So $m$, an integer in $\{0,1,\dots,2\}$, is an exact summary of everything the condition needs, and it joins the state.

The vertices of the digraph are therefore the triples (the last $1$ row, the count $m$, what the last band scored), an edge $u\to v$ meaning that $v$'s new row may follow $u$. Of the $8$ possible rows only those admissible from the current $m$ appear, and states with identical futures are merged, which leaves $S=4$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
a(n)\;=\;\iota^{\!\top}M^{\,n}\tau ,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205220.
