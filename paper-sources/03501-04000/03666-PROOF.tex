...
\title{The conjectured generating function for OEIS A250758, proved}
...
\begin{abstract}
OEIS A250758 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250758 is ``Number of (3+1) X (n+1) 0..2 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing x(i,j)+x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
332, 756, 1353, 2123, 3066, 4182,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(332 - 240*x + 81*x\^{}2) / (1 - x)\^{}3.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 18 2018 09:06:18, revision 8) this is still
...
model reproduces all $36$ terms the entry publishes, exactly, in integer arithmetic --- a
...
First, the model reproduces every one of the $36$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250758.
