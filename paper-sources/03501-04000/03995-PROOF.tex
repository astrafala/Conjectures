...
\title{The empirical column recurrences for the table OEIS A232257}
...
\begin{abstract}
OEIS A232257 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A232257 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every element both >= and <= some horizontal or antidiagonal neighbor.''. It has offset $1$, is
read by antidiagonals, and begins
\[
9, 37, 37, 215, 421, 163, 1287, 6189,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 4*a(n-1) +2*a(n-2) -a(n-4)\\
k=2: a(n) = 10*a(n-1) +14*a(n-2) -6*a(n-3) -a(n-4)\\
k=3: [order 9] for n>10\\
k=4: [order 21]\\
k=5: [order 51] for n>54\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:58:14, revision 8) these are still
...
The sequence is ``Number of (n+1)X2 0..2 arrays with every element both >= and <= some horizontal or antidiagonal neighbor.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..2 arrays with every element both >= and <= some horizontal or antidiagonal neighbor.''. Its rows are the vertices of a digraph on
$S=487$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;10\,a(n-1) + 14\,a(n-2) - 6\,a(n-3) - a(n-4)
\]
holds from index $3+1$ on.
...
model reproduces $13$ published values, $50$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232257.
