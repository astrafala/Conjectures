...
\title{The empirical recurrence for OEIS A224587, proved by transfer matrix}
...
\begin{abstract}
OEIS A224587 counts the $(n+6)\times13$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=164414$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224587 is ``Number of (n+6)X13 0..1 matrices with each 7X7 subblock idempotent.''. It has offset $1$ and begins
\[
146715, 28629, 29066, 31741, 34107, 35891, 37202, 38192,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) -6*a(n-2) +4*a(n-3) -a(n-4) +a(n-7) -3*a(n-8) +3*a(n-9) -a(n-10) for n>16
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:29:44, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $7\times7$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $7\times13$ matrix a \emph{band} when every one of its $7$ contiguous
$7\times7$ subblocks is a window. What the entry counts is the matrices of $13$ columns
whose every $7$ consecutive rows form a band.

Take as states the $6\times13$ strips that occur as the top $6$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=164414$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{78}$ strips of $6$ rows and $13$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $7\times6$ entries, so a band is exactly a walk of $7$ windows in
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) - 6\,a(n-2) + 4\,a(n-3) - a(n-4) + a(n-7) - 3\,a(n-8) + 3\,a(n-9) - a(n-10)
\]
for every $n>16$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224587.
