...
\title{The conjectured generating function for OEIS A228478, proved}
...
\begin{abstract}
OEIS A228478 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A228478 is ``Number of n X 4 binary arrays with top left value 1 and no two ones adjacent horizontally, vertically or antidiagonally.''. It has offset $1$ and begins
\[
3, 9, 41, 172, 728, 3084,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: x*(3 + 6*x + 2*x\^{}2 - 4*x\^{}3 - x\^{}4 + 3*x\^{}5 - x\^{}6) / (1 - x - 10*x\^{}2 - 15*x\^{}3 - 4*x\^{}4 + 6*x\^{}5 + x\^{}6 - 3*x\^{}7 + x\^{}8). - \_Colin Barker\_, Sep 11 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 11 2018 09:46:19, revision 12) this is still
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=7$ and $\deg D_2=8$; its expansion as a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A228478.
