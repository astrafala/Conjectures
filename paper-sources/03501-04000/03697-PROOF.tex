...
\title{The empirical recurrence for OEIS A262333, proved by transfer matrix}
...
\begin{abstract}
OEIS A262333 counts arrays of width $4$ over $\{0,1\}$ whose rows, read as base-$2$
numbers, are divisible by $9$ and whose columns, read the same way, are divisible by
$9$. The entry carries an empirical recurrence of order $5$ contributed by
...
OEIS A262333 is ``Number of (n+3) X (1+3) 0..1 arrays with each row and column divisible by 9, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
2, 4, 8, 15, 29, 57, 114, 228,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) - 2*a(n-2) - a(n-3) + 3*a(n-4) - 2*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 31 2018 06:16:01, revision 9) this is still
...
So the vector $\bigl(\gamma_1\bmod 9,\dots,\gamma_{4}\bmod
9\bigr)$ is a state: at most $9^{4}$ of them, and each array row
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262333.
