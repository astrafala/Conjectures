...
\title{The conjectured generating function for OEIS A280371, proved}
...
\begin{abstract}
OEIS A280371 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A280371 is ``Binary representation of the x-axis, from the left edge to the origin, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 260", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 10, 101, 1000, 10000, 101000,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 + 10*x + 101*x\^{}2 + 1000*x\^{}3 + 1000*x\^{}5 + 100*x\^{}6 + 10*x\^{}7 + x\^{}8 - 10000*x\^{}12) / ((1 - 10*x)*(1 + 10*x)*(1 + 100*x\^{}2)).
\end{quote}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{4}$ read off $D_2$, the first few being $0,\ 0,\ 0,\ 10000$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A280371.
