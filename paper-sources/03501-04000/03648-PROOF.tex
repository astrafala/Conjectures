...
\title{The empirical recurrence for OEIS A268997, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A268997 counts arrays over $\{0,\dots,1\}$ subject to a condition on the array as a
...
and the empirical recurrence of order $9$ contributed by R.~H.~Hardin then follows from
...
OEIS A268997 is ``Number of 3 X n binary arrays with some element plus some horizontally or antidiagonally adjacent neighbor totalling two not more than once.''. It has offset $1$ and begins
\[
8, 41, 174, 805, 3331, 14080, 57287, 232449,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) + 12*a(n-2) - 16*a(n-3) - 62*a(n-4) - 34*a(n-5) + 16*a(n-6) + 12*a(n-7) - a(n-8) - a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 18 2019 06:33:30, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 12\,a(n-2) - 16\,a(n-3) - 62\,a(n-4) - 34\,a(n-5) + 16\,a(n-6) + 12\,a(n-7) - a(n-8) - a(n-9)
\end{equation}
for all $n>9$.
...
\]
for $j\ge9$. Hence \eqref{eq:conj} holds from that point on if and only if
...
formed by $9$ applications of $G$ in exact integer arithmetic and
$\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly. Every one of
those integers vanishes from the index corresponding to $n=9+1$ onwards, and the run
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268997.
