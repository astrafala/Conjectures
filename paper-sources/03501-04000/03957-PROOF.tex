...
\title{The empirical recurrence for OEIS A198976, proved by counting patterns}
...
\begin{abstract}
OEIS A198976 counts arrays subject to a local condition \emph{and} to the clause ``new values
$0..5$ introduced in row major order'', which says the array is written in canonical
...
$6!\,a(n)=\sum_i\binom{6}{i}D_{6-i}L_i(n)$ with $D_m$ the derangement numbers,
...
state space down from $91$ to $S=11$. The entry's empirical recurrence of order
$4$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A198976 is ``Number of n X 2 0..5 arrays with values 0..5 introduced in row major order and no element equal to any horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
1, 4, 34, 481, 8731, 174454, 3603244, 75251971,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 32*a(n-1) - 262*a(n-2) + 672*a(n-3) - 441*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 22 2018 16:29:02, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;32\,a(n-1) - 262\,a(n-2) + 672\,a(n-3) - 441\,a(n-4)
\end{equation}
for all $n>4$.
...
The clause ``new values $0..5$ introduced in row major order'' selects, from each class
...
$v+1$. Every pattern with at most $K=6$ classes has exactly one such representative, and
...
\qquad
6!\,\sum_{j=0}^{6}N_j(n)=\sum_{i=0}^{6}\binom{6}{i}D_{6-i}\,L_i(n),
\]
...
$j\le 6$ and exchanging the order of summation,
\[
\sum_{j=0}^{6}N_j=\sum_{i=0}^{6}L_i\sum_{j=i}^{6}\frac{(-1)^{j-i}}{i!\,(j-i)!}
=\sum_{i=0}^{6}\frac{L_i}{i!}\sum_{t=0}^{6-i}\frac{(-1)^t}{t!} ,
\]
and multiplying by $6!$ turns the coefficient of $L_i$ into
$\frac{6!}{i!}\sum_{t\le 6-i}(-1)^t/t!=\binom{6}{i}(6-i)!\sum_{t\le 6-i}(-1)^t/t!
=\binom{6}{i}D_{6-i}$, an integer.
\end{proof}
...
Assembling the $6$ lumped chains into one block-diagonal matrix $N$ of size $S=11$ and
putting the weights of Lemma~\ref{lem:inv} into the start vector,
$\mathbf{v}=\bigl(\binom{6}{i}D_{6-i}\varepsilon_P\bigr)_{i,P}$, gives
\begin{equation}\label{eq:walk}
6!\;a(n)\;=\;\mathbf{v}^{\!\top}N^{\,n+0-1}\mathbf{1} .
\end{equation}
The unlumped alphabet had $91$ rows in all; the lumped chain has $S=11$ states.
...
For every $n$ with $m=n+0-1\ge4$,
\[
6!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-4}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-4}$ out of
$N^{m}-\sum_i c_iN^{m-i}$. The scalar $6!$ is nonzero, so it does not affect whether
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $4$ matrix--vector products
...
corresponding to $n=4+1$ onwards, and the run continued until $w$ was the zero vector,
...
\eqref{eq:walk}, divided by $6!$, was evaluated at every one of the $17$
...
show up as a mismatch, and the quotient by $6!$ came out an integer at every index.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198976.
