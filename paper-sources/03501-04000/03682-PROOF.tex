...
\title{The empirical column recurrences for the table OEIS A203835}
...
\begin{abstract}
OEIS A203835 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A203835 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. It has offset $1$, is
read by antidiagonals, and begins
\[
45, 225, 225, 1125, 1971, 1125, 5625, 17289,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:45:52, revision 9) these are still
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(4) 0..2 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. Its rows are the vertices of a digraph on
...
different counts. Across the columns settled here the model reproduces $12$ published
values, $72$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203835.
