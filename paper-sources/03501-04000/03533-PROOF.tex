...
\title{The empirical recurrence for OEIS A233197, proved by transfer matrix}
...
\begin{abstract}
OEIS A233197 counts arrays over $\{0,\dots,7\}$ in which no two entries that are neighbours in
...
carried by one bounded piece of extra state, so the count is a walk count on $S=4$ states.
\end{abstract}
...
identified; after that identification there are $S=4$ of them.
...
\begin{theorem}
\[
a(n)\;=\;64\,a(n-1) - 960\,a(n-2) + 4096\,a(n-3)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233197.
