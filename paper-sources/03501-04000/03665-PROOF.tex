...
\title{The empirical recurrence for OEIS A208265, proved by counting patterns}
...
\begin{abstract}
OEIS A208265 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $17$ to $S=9$. The entry's empirical recurrence of order
...
OEIS A208265 is ``Number of n X 4 0..1 arrays with new values 0..1 introduced in row major order and no element equal to more than two of its immediate leftward or upward or left-upward diagonal neighbors.''. It has offset $1$ and begins
\[
8, 89, 1033, 11929, 137845, 1592731, 18403423, 212644499,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 10*a(n-1) + 20*a(n-2) - 21*a(n-3) - 30*a(n-4) + 8*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 29 2018 08:35:25, revision 8) the statement is
...
Assembling the $2$ lumped chains into one block-diagonal matrix $N$ of size $S=9$ and
...
\end{equation}
The unlumped alphabet had $17$ rows in all; the lumped chain has $S=9$ states.
...
\[
2!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-5}q(N)\mathbf{1} .
\]
...
\eqref{eq:walk}, divided by $2!$, was evaluated at every one of the $17$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208265.
