...
\title{The conjectured generating function for OEIS A250848, proved}
...
\begin{abstract}
OEIS A250848 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250848 is ``Number of (n+1) X (3+1) 0..3 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
1225, 8037, 44797, 223933, 1043885, 4648157,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: x*(1225 - 4213*x + 7302*x\^{}2 - 3992*x\^{}3) / ((1 - x)*(1 - 2*x)*(1 - 3*x)*(1 - 4*x)). -\_Colin Barker\_, Nov 21 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 21 2018 09:17:47, revision 10) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250848.
