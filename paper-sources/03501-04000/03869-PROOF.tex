...
\title{The empirical closed form for OEIS A188861, proved}
...
\begin{abstract}
OEIS A188861 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
OEIS A188861 is ``Number of n X 4 binary arrays without the pattern 0 1 diagonally, vertically or antidiagonally.''. It has offset $1$ and begins
\[
16, 41, 68, 95, 122, 149, 176, 203,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 27*n - 13 for n>1.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 28 2018 09:55:07, revision 11) this is still
...
vanish from the point corresponding to $n=4$ onwards, and the run was continued until the
working vector was identically zero, which settles every larger $j$ at once. So $a$ satisfies
\eqref{eq:rec} for every $n>3$. By Lemma~\ref{lem:ann} $f$ satisfies \eqref{eq:rec}
identically. Both were then evaluated in exact arithmetic at the $2$ consecutive indices
$n=4,\dots,5$ and agree at each. A monic recurrence of order $2$
determines $a(m)$ from $a(m-1),\dots,a(m-2)$, and for every $m\ge6$ both
$m>3$ and $m-2\ge4$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge4$.
Below $n=4$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=4$ on. The remaining indices $n=2,\dots,3$ were
...
First, the walk model reproduces every one of the $51$ terms the entry publishes,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188861.
