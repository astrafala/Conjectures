...
\title{The empirical recurrence for OEIS A224584, proved by transfer matrix}
...
\begin{abstract}
OEIS A224584 counts the $(n+6)\times10$ matrices over $\{0,\dots,1\}$ all of whose
$7\times7$ contiguous subblocks are idempotent, and carries an empirical recurrence of order
$17$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical.
The condition ties together $7$ consecutive rows and $7$ consecutive columns at once, so a
single row is not a state; the strip of the last $6$ rows is, and the admissible strips
are read off the overlap graph of the admissible windows instead of being enumerated. That
makes $a(n)$ a walk count on $S=150022$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224584 is ``Number of (n+6)X10 0..1 matrices with each 7X7 subblock idempotent.''. It has offset $1$ and begins
\[
122508, 24643, 24293, 26528, 28783, 30456, 31741, 32736,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:29:25, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $7\times7$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $7\times10$ matrix a \emph{band} when every one of its $4$ contiguous
$7\times7$ subblocks is a window. What the entry counts is the matrices of $10$ columns
whose every $7$ consecutive rows form a band.

Take as states the $6\times10$ strips that occur as the top $6$ rows of a band,
together with those that occur as its bottom $6$ rows, and put an edge from $u$ to $v$
when the band whose first $6$ rows are $u$ and whose last $6$ rows are $v$ exists.
A band is determined by that pair, so the edge is unique when it exists. Reading a counted
matrix from the top, its rows $1..6$, then $2..7$, and so on, form a walk, and every
walk arises from exactly one matrix; a matrix with $n+6$ rows gives a walk of $n$ edges.
Since the first $6$ rows of a counted matrix with at least $7$ rows are the top of a
band, and since no further condition falls on where a walk starts or ends,
\[
a(n)\;=\;\iota^{\!\top}M^{\,n}\tau ,\qquad \iota=\tau=(1,1,\dots,1)^{\!\top},
\]
with $M$ the adjacency matrix on $S=150022$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{60}$ strips of $6$ rows and $10$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $7\times6$ entries, so a band is exactly a walk of $4$ windows in
the graph that joins a window to those whose first $6$ columns are its last $6$
columns. Enumerating those walks produces the bands, and with them the states and the edges,
directly.

\section{The admissible windows}

The vertices of the overlap graph are the $0/1$ matrices $B$ of size $7\times7$ with
$B^2=B$. There are $2^{49}$ matrices to sift, which is already awkward at $7=7$ and
...
Generating from Lemma~\ref{lem:idem} gives $2185738$ admissible $7\times7$ windows. For
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 4\,a(n-2) + 4\,a(n-3) - 3\,a(n-4) + a(n-5) + a(n-6) - 2\,a(n-7) + 2\,a(n-8) - 2\,a(n-9) + a(n-10) - a(n-13) + 2\,a(n-14) - 2\,a(n-15) + 2\,a(n-16) - a(n-17)
\]
for every $n>23$.
\end{theorem}
...
until the working vector was identically zero, which settles every larger $j$ at once. The bound is exact: at $n=23$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224584.
