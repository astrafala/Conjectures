...
\title{The empirical recurrence for OEIS A268093, proved by transfer matrix}
...
\begin{abstract}
OEIS A268093 counts arrays in which the REPEATED VALUES of each row, and of each column, stand in a
...
value so far in each column of the scan, so the count is a walk count on $S=13$ states.
\end{abstract}
...
OEIS A268093 is ``Number of 1 X n 0..2 arrays with every repeated value in every row not one larger and in every column one larger mod 3 than the previous repeated value, and upper left element zero.''. It has offset $1$ and begins
\[
1, 3, 9, 26, 74, 208, 580, 1608,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 2*a(n-2) - 4*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 11 2019 15:23:07, revision 8) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=13,\quad\tau=\mathbf{1} .
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268093.
