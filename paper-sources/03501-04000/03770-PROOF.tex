...
\title{The empirical column recurrences for the table OEIS A237637}
...
\begin{abstract}
OEIS A237637 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A237637 is ``T(n,k) = Number of (n+1) X (k+1) 0..2 arrays with the upper median of every 2 X 2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 260, 260, 702, 636, 702, 1786, 1220,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 36*a(n-4) +48*a(n-6) for n>9.\\
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 21 2014 03:55:50, revision 8) these are still
...
column $k$$=1$ & $6$ & $81$ & $>9$ & $9$ \\
column $k$$=2$ & $8$ & $260$ & $>12$ & $10$ \\
column $k$$=3$ & $4$ & $702$ & $>13$ & $7$ \\
column $k$$=4$ & $4$ & $1786$ & $>14$ & $6$ \\
column $k$$=5$ & $4$ & $4621$ & $>13$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1) X (2) 0..2 arrays with the upper median of every 2 X 2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;36\,a(n-4) + 48\,a(n-6)
\]
holds from index $9+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+1) X (3) 0..2 arrays with the upper median of every 2 X 2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
$S=260$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of (n+1) X (4) 0..2 arrays with the upper median of every 2 X 2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
$S=702$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-4)
...
The sequence is ``Number of (n+1) X (5) 0..2 arrays with the upper median of every 2 X 2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
$S=1786$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;5\,a(n-4)
...
The sequence is ``Number of (n+1) X (6) 0..2 arrays with the upper median of every 2 X 2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
$S=4621$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-4)
...
model reproduces $42$ published values, $174$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237637.
