...
\title{The conjectured generating function for OEIS A207020, proved}
...
\begin{abstract}
OEIS A207020 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=91$ vertices, so its generating function is a rational function whose numerator
...
OEIS A207020 is ``Number of n X 4 0..1 arrays avoiding 0 0 0 and 0 1 1 horizontally and 0 0 1 and 1 0 1 vertically.''. It has offset $1$ and begins
\[
9, 81, 252, 558, 1035, 1719,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: 9*x*(1 + 5*x - 2*x\^{}2) / (1 - x)\^{}4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 17 2018 14:39:47, revision 10) this is still
...
\]
and there are $S=91$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $94$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{94}$ then $R$ has a zero of order greater than its own
...
of them --- more than the $94+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207020.
