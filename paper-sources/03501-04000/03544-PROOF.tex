...
\title{Arrays counted up to renaming: the empirical recurrence for OEIS A206170}
...
\begin{abstract}
OEIS A206170 carries an empirical recurrence of order $4$ contributed by R.~H.~Hardin. It is
...
both in the state the arrays are exactly the walks in a digraph on $S=7$ vertices, so $a(n)$
...
OEIS A206170 is ``Number of 2 X (n+1) 0..3 arrays with every 2 X 2 subblock in a row having an equal number of equal diagonal or equal antidiagonal elements, adjacent rows differing in this number, and new values 0..3 introduced in row major order.''. It has offset $1$ and begins
\[
15, 85, 641, 5257, 44585, 385465, 3375401, 29817817,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 04 2018 06:41:02, revision 9) this is still
...
What is compared \emph{across} windows is carried too: the state holds the single number the last band scored, since a band is constant and only that one number is compared with the next. With $1$ subblock to a band that is a bounded amount of bookkeeping and no more.
...
\begin{theorem}
\[
a(n)\;=\;18\,a(n-1) - 101\,a(n-2) + 192\,a(n-3) - 108\,a(n-4)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206170.
