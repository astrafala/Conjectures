...
\title{The conjectured generating function for OEIS A282911, proved}
...
\begin{abstract}
OEIS A282911 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=13$ vertices, so its generating function is a rational function whose numerator
...
OEIS A282911 is ``Binary representation of the x-axis, from the left edge to the origin, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 526", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 11, 111, 1101, 11001, 111011,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 + 10*x + 100*x\^{}2 + 990*x\^{}3 - 100*x\^{}4 + 10*x\^{}5) / ((1 - x)*(1 - 10*x)*(1 + 10*x)*(1 + 100*x\^{}2)).
\end{quote}
...
\]
and there are $S=13$ states. The digraph is built by reading the entry's own English, and the
...
numerator. Its coefficients are the integers $c_1,\dots,c_{5}$ read off $D_2$, the first few being $1,\ 0,\ 0,\ 10000,\ -10000$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A282911.
