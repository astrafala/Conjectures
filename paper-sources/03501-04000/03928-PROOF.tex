...
\title{The empirical column recurrences for the table OEIS A302953}
...
\begin{abstract}
OEIS A302953 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A302953 is ``T(n,k) = Number of n X k 0..1 arrays with every element equal to 1, 2, 3, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 1, 0, 1, 3, 0, 2, 15,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 27 2021 14:53:22, revision 6) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of 1 X n 0..1 arrays with every element equal to 1, 2, 3, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of 2 X n 0..1 arrays with every element equal to 1, 2, 3, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302953.
