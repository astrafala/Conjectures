...
\title{The empirical recurrence for OEIS A198447, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A198447 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
lumpable, cutting $98$ pairs down to $S=23$ states. The entry's empirical recurrence of
...
OEIS A198447 is ``Number of 2n X 2 0..2 arrays with values 0..2 introduced in row major order and each element unequal to exactly two horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
3, 13, 71, 433, 2763, 17941, 117263, 768313,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 10*a(n-1) -25*a(n-2) +16*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (May 14 2018 10:30:32, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;10\,a(n-1) - 25\,a(n-2) + 16\,a(n-3)
\end{equation}
for all $n>3$.
...
Assembling the $3$ lumped chains block-diagonally into $N$ of size $S=23$, with the
...
index,
\[
3!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-3}\,
...
\eqref{eq:walk2}, divided by $3!$, was evaluated at every one of the $20$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198447.
