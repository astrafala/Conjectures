...
\title{The conjectured generating function for OEIS A273743, proved}
...
\begin{abstract}
OEIS A273743 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $33$ coefficients in exact arithmetic
...
OEIS A273743 is ``Number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 901", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 8, 25, 49, 81, 121,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1+5*x+4*x\^{}2-3*x\^{}3+x\^{}4) / (1-x)\^{}3.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:34, revision 34) this is still
...
\]
a polynomial of degree at most $28$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{28}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $33$
of them --- more than the $28+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A273743.
