...
\title{The empirical recurrence for OEIS A184000, proved by splitting on the common sum}
...
\begin{abstract}
OEIS A184000 counts arrays over $\{0,\dots,2\}$ in which all $2\times2$ subblock sums are
...
computation on $S=19683$ vertices.
\end{abstract}
...
OEIS A184000 is ``Number of (n+1) X 7 0..2 arrays with all 2 X 2 subblock sums the same.''. It has offset $1$ and begins
\[
5175, 6417, 8415, 12525, 19935, 35565, 66591, 134853,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) - 60*a(n-3) + 85*a(n-4) + 150*a(n-5) - 360*a(n-6) + 396*a(n-8) - 216*a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 09 2018 14:45:49, revision 8) the statement is
...
The arrays have $7$ columns and $L=n+1$ rows, entries in
$\{0,\dots,2\}$, and every $2\times2$ subblock has the same sum. Since $L\ge2$ and
$7\ge2$, every such array contains at least one $2\times2$ subblock, and that subblock's
...
$V=\{0,\dots,2\}^{7}$ be the possible rows and let $M_c$ be the adjacency
matrix of the digraph in which $r\to s$ when
$r_j+r_{j+1}+s_j+s_{j+1}=c$ for every $j$ with $1\le j\le 7-1$. Then
...
Writing $N=\bigoplus_{c=0}^{8}M_c$, of size $S=19683$, and $\mathbf{v}$ for the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184000.
