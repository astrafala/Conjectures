...
\title{The empirical column recurrences for the table OEIS A238929}
...
\begin{abstract}
OEIS A238929 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A238929 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with no element greater than all horizontal neighbors or equal to all vertical neighbors.''. It has offset $1$, is
read by antidiagonals, and begins
\[
12, 116, 36, 782, 864, 144, 5008, 13090,\ \dots
\]
...
k=2: [order 29]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:56:28, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..3 arrays with no element greater than all horizontal neighbors or equal to all vertical neighbors.''. Its rows are the vertices of a digraph on
...
model reproduces $7$ published values, $23$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A238929.
