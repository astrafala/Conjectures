...
\title{The conjectured generating function for OEIS A283583, proved}
...
\begin{abstract}
OEIS A283583 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $49$ coefficients in exact arithmetic
...
OEIS A283583 is ``Decimal representation of the x-axis, from the left edge to the origin, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 646", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 3, 5, 13, 21, 53,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 + 3*x + 4*x\^{}2 + 10*x\^{}3 + 16*x\^{}4 + 40*x\^{}5 + 60*x\^{}6 + 166*x\^{}7 - 16*x\^{}8 - 120*x\^{}9 + 4*x\^{}10 + 10*x\^{}11) / ((1 - x)*(1 + x)*(1 - 2*x)*(1 + 2*x)*(1 + 4*x\^{}2)*(1 + 16*x\^{}4)).
\end{quote}
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=11$ and $\deg D_2=10$; its expansion as a
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $49$
...
numerator. Its coefficients are the integers $c_1,\dots,c_{10}$ read off $D_2$, the first few being $0,\ 1,\ 0,\ 0,\ 0,\ 0$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A283583.
