...
\title{The empirical column recurrences for the table OEIS A231324}
...
\begin{abstract}
OEIS A231324 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231324 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
6, 24, 24, 216, 432, 216, 1536, 9600,\ \dots
\]
...
k=4: [order 81]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:24:47, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
...
$c_{1}$ & $22$ & $c_{4}$ & $944$ & $c_{7}$ & $0$ &  &  \\
$c_{2}$ & $0$ & $c_{5}$ & $7168$ & $c_{8}$ & $9216$ &  &  \\
$c_{3}$ & $-776$ & $c_{6}$ & $-12416$ & $c_{9}$ & $-4096$ &  &  \\
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231324.
