...
\title{The empirical column recurrences for the table OEIS A230949}
...
\begin{abstract}
OEIS A230949 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A230949 is ``T(n,k)=Number of (n+3)X(k+3) 0..3 white square subarrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, and upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
6, 16, 16, 76, 34, 76, 314, 724,\ \dots
\]
...
k=4: [order 70]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:08:33, revision 6) these are still
...
The sequence is ``Number of (n+3)X4 0..3 white square subarrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, and upper left element zero.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230949.
