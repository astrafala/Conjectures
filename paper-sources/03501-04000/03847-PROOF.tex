...
\title{The empirical column recurrences for the table OEIS A234769}
...
\begin{abstract}
OEIS A234769 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234769 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with 2X2 edge jumps all no more than +1 in one of the clockwise or counterclockwise directions but not both.''. It has offset $1$, is
read by antidiagonals, and begins
\[
56, 240, 240, 916, 1624, 916, 3800, 9404,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1) +14*a(n-2) -a(n-3) -25*a(n-4) -10*a(n-5)\\
k=2: [order 13]\\
k=3: [order 27]\\
k=4: [order 73]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:25:27, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..3 arrays with 2X2 edge jumps all no more than +1 in one of the clockwise or counterclockwise directions but not both.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;a(n-1) + 14\,a(n-2) - a(n-3) - 25\,a(n-4) - 10\,a(n-5)
\]
...
model reproduces $8$ published values, $34$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234769.
