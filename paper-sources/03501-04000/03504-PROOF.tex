...
\title{The empirical column recurrences for the table OEIS A229755}
...
\begin{abstract}
OEIS A229755 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 2, 3, 4$.
\end{abstract}
...
OEIS A229755 is ``T(n,k) = number of defective 4-colorings of an n X k 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..3 order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 0, 0, 1, 3, 1, 3, 60,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 08 2021 07:25:15, revision 10) these are still
...
column $k$$=1$ & $6$ & $30$ & $>9$ & $10$ \\
column $k$$=2$ & $3$ & $90$ & $>5$ & $9$ \\
column $k$$=3$ & $6$ & $300$ & $>8$ & $8$ \\
column $k$$=4$ & $6$ & $1062$ & $>11$ & $8$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of defective 4-colorings of an n X 1 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..3 order.''. Its rows are the vertices of a digraph on
$S=30$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;12\,a(n-1) - 57\,a(n-2) + 136\,a(n-3) - 171\,a(n-4) + 108\,a(n-5) - 27\,a(n-6)
\]
holds from index $9+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of defective 4-colorings of an n X 2 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..3 order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of defective 4-colorings of an n X 3 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..3 order.''. Its rows are the vertices of a digraph on
$S=300$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of defective 4-colorings of an n X 4 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..3 order.''. Its rows are the vertices of a digraph on
$S=1062$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;9\,a(n-1) - 33\,a(n-2) + 63\,a(n-3) - 66\,a(n-4) + 36\,a(n-5) - 8\,a(n-6)
...
model reproduces $35$ published values, $116$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229755.
