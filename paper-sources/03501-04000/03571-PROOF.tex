...
\title{The empirical recurrence for OEIS A214188, proved by transfer matrix}
...
\begin{abstract}
OEIS A214188 counts the proper colourings of a grid $1\times(n+1)$, wrapped in the growing direction, with at most $7$ colours and
counted up to renaming the colours, and carries an empirical recurrence of order $6$
...
down to $S=28$ and makes the exact residual test affordable.
\end{abstract}
...
OEIS A214188 is ``Number of 0..6 colorings of a 1 X (n+1) array circular in the n+1 direction with new values 0..6 introduced in row major order.''. It has offset $1$ and begins
\[
1, 1, 4, 11, 41, 162, 714, 3397,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 15*a(n-1) - 79*a(n-2) + 165*a(n-3) - 64*a(n-4) - 180*a(n-5) + 144*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 11 2024 23:11:04, revision 11) this is still
...
A column of the array is a proper colouring of the path $P_{1}$ and two consecutive
columns must differ in every one of the $1$ positions; the wrapping makes the last column
adjacent to the first. A colouring of the array is therefore a CLOSED walk of $m=n+1$ steps
in the digraph $T_i$ whose vertices are the proper colourings of $P_{1}$ over $i$ colours
...
$S_i$ acts on the colourings of $P_{1}$ preserving $T_i$; two colourings lie in one orbit
...
state count is $S=28$ rather than the number of colourings.
...
\begin{theorem}
\[
a(n)\;=\;15\,a(n-1) - 79\,a(n-2) + 165\,a(n-3) - 64\,a(n-4) - 180\,a(n-5) + 144\,a(n-6)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A214188.
