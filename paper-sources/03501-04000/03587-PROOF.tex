...
\title{The empirical column recurrences for the table OEIS A235087}
...
\begin{abstract}
OEIS A235087 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A235087 is ``T(n,k) is the number of (n+1) X (k+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
38, 90, 90, 202, 198, 202, 478, 418,\ \dots
\]
...
k=1: a(n) = 2*a(n-1) +6*a(n-2) -10*a(n-3) -5*a(n-4).\\
k=2: [order 10].\\
k=3: [order 15].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 21:07:11, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
$S=80$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=160$ exact terms returns the minimal one, of order
$10$ --- the stated order --- with integer coefficients
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $3$ & $c_{4}$ & $-28$ & $c_{7}$ & $-111$ & $c_{10}$ & $10$ \\
$c_{2}$ & $10$ & $c_{5}$ & $111$ & $c_{8}$ & $-27$ &  &  \\
$c_{3}$ & $-33$ & $c_{6}$ & $20$ & $c_{9}$ & $30$ &  &  \\
...
$S=320$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=640$ exact terms returns the minimal one, of order
$15$ --- the stated order --- with integer coefficients
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $5$ & $c_{5}$ & $462$ & $c_{9}$ & $1841$ & $c_{13}$ & $604$ \\
$c_{2}$ & $9$ & $c_{6}$ & $-368$ & $c_{10}$ & $-2237$ & $c_{14}$ & $-300$ \\
$c_{3}$ & $-78$ & $c_{7}$ & $-1304$ & $c_{11}$ & $-1346$ & $c_{15}$ & $-120$ \\
$c_{4}$ & $12$ & $c_{8}$ & $1433$ & $c_{12}$ & $1420$ &  &  \\
\end{tabular}\end{center}
...
model reproduces $9$ published values, $32$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235087.
