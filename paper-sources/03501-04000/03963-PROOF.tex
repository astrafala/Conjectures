...
\title{The empirical recurrence for OEIS A198902, proved by counting patterns}
...
\begin{abstract}
OEIS A198902 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $979$ to $S=53$. The entry's empirical recurrence of order
$7$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A198902 is ``Number of nX4 0..4 arrays with values 0..4 introduced in row major order and no element equal to any horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
5, 380, 50384, 6907736, 948656912, 130292546801, 17895005957823, 2457786852894234,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 163*a(n-1) -3751*a(n-2) +32097*a(n-3) -120287*a(n-4) +201471*a(n-5) -148296*a(n-6) +38604*a(n-7)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 08:01:24, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;163\,a(n-1) - 3751\,a(n-2) + 32097\,a(n-3) - 120287\,a(n-4) + 201471\,a(n-5) - 148296\,a(n-6) + 38604\,a(n-7)
\end{equation}
for all $n>7$.
...
Assembling the $5$ lumped chains into one block-diagonal matrix $N$ of size $S=53$ and
...
\end{equation}
The unlumped alphabet had $979$ rows in all; the lumped chain has $S=53$ states.
...
For every $n$ with $m=n+0-1\ge7$,
\[
5!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-7}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-7}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $7$ matrix--vector products
...
\eqref{eq:walk}, divided by $5!$, was evaluated at every one of the $12$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198902.
