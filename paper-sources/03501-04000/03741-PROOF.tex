...
\title{The conjectured generating function for OEIS A203175, proved}
...
\begin{abstract}
OEIS A203175 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $20$ coefficients in exact arithmetic
...
OEIS A203175 is ``Number of nX2 0..2 arrays with every 1 immediately preceded by 0 to the left or above, no 0 immediately preceded by a 0, and every 2 immediately preceded by 0 1 to the left or above.''. It has offset $1$ and begins
\[
1, 1, 2, 4, 6, 10,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical G.f.: -x*(1+x\^{}2) / ( -1+x+2*x\^{}3 ). - \_R. J. Mathar\_, Jul 02 2013
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:21, revision 29) this is still
...
\]
and there are $S=12$ states. The digraph is built by reading the entry's own English, and the
model reproduces all $38$ terms the entry publishes, exactly, in integer arithmetic --- a
...
The family this entry belongs to is read by one model, described by its author as: ```Number of n X W binary arrays with each 1 adjacent to exactly two other 1s' and `Number of n X W 0..m arrays with every 1 immediately preceded by 0 to the left or above, and every 2 immediately preceded by 0 1 to the left or above.'''
...
\]
a polynomial of degree at most $15$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{15}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $20$
of them --- more than the $15+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{3}$ read off $D_2$, the first few being $1,\ 0,\ 2$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
First, the model reproduces every one of the $38$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203175.
