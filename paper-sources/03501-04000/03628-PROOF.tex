...
\title{The empirical column recurrences for the table OEIS A205986}
...
\begin{abstract}
OEIS A205986 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A205986 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every 2X3 or 3X2 subblock having exactly two clockwise edge increases.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 423, 423, 2232, 1980, 2232, 11568, 8004,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:22:37, revision 9) these are still
...
The sequence is ``Number of (n+1)X4 0..2 arrays with every 2X3 or 3X2 subblock having exactly two clockwise edge increases.''. Its rows are the vertices of a digraph on
$S=2232$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;60\,a(n-3) + 8\,a(n-5)
\]
...
model reproduces $8$ published values, $47$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205986.
