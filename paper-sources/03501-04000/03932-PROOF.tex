...
\title{The conjectured generating function for OEIS A279879, proved}
...
\begin{abstract}
OEIS A279879 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (1 +2*x -5*x\^{}2 -2*x\^{}3 +7*x\^{}4 -14*x\^{}6 +8*x\^{}8) / ((1 -x)*(1 +x)*(1 -2*x)*(1 +2*x)*(1 -2*x\^{}2)).
\end{quote}
