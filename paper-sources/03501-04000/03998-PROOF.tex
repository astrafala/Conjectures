...
\title{The conjectured generating function for OEIS A242544, proved}
...
\begin{abstract}
OEIS A242544 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=100$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $118$ coefficients in exact arithmetic
...
OEIS A242544 is ``Number of length n+3 0..3 arrays with no four elements in a row with pattern aabb (possibly a=b) and new values 0..3 introduced in 0..3 order.''. It has offset $1$ and begins
\[
13, 42, 150, 554, 2072, 7808,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.: -x*(-13+10*x-21*x\^{}2+50*x\^{}3+88*x\^{}4+78*x\^{}5+99*x\^{}6+42*x\^{}7) / ( (x\^{}2+x-1)*(3*x\^{}2+3*x-1)*(3*x\^{}2+1)*(x\^{}2+1) ). - \_R. J. Mathar\_, May 18 2014
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 11:17:13, revision 8) this is still
...
\]
and there are $S=100$ states. The digraph is built by reading the entry's own English, and the
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=8$ and $\deg D_2=8$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $108$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{108}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $118$
of them --- more than the $108+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{8}$ read off $D_2$, the first few being $4,\ -3,\ 10,\ -2,\ -12,\ -9$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A242544.
