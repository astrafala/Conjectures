...
\title{The empirical recurrence for OEIS A203722, proved by transfer matrix}
...
\begin{abstract}
OEIS A203722 counts arrays of width $3$ over $\{0,1,2\}$ through two derived arrays of PAIR SUMS ---
each cell added to its left neighbour, and each cell added to the one above it --- on which the
entry imposes an order condition. The entry carries an empirical recurrence of order $14$
contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical: the count is
a walk count on $S=28$ states.
\end{abstract}
...
OEIS A203722 is ``Number of (n+1)X3 0..2 arrays with column and row pair sums b(i,j)=a(i,j)+a(i,j-1) and c(i,j)=a(i,j)+a(i-1,j) nondecreasing in column and row directions, respectively.''. It has offset $1$ and begins
\[
113, 480, 1632, 4737, 12470, 30335, 69612, 152260,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:40:25, revision 9) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203722.
