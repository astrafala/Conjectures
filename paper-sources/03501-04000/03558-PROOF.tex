...
\title{The empirical column recurrences for the table OEIS A261113}
...
\begin{abstract}
OEIS A261113 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A261113 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010001 00010101 or 01010101.''. It has offset $1$, is
read by antidiagonals, and begins
\[
28, 33, 33, 63, 52, 63, 97, 106,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1) +3*a(n-2) -3*a(n-3) for n>6\\
k=2: a(n) = a(n-1) +2*a(n-3) +2*a(n-4) +2*a(n-5) +a(n-6) for n>7\\
k=3: a(n) = 3*a(n-2) +10*a(n-4) +5*a(n-6) +2*a(n-8) -2*a(n-10) +a(n-12) for n>13\\
k=4: [order 13] for n>14\\
k=5: [order 24] for n>25\\
k=6: [order 27] for n>28\\
k=7: [order 48] for n>49\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 12:14:27, revision 6) these are still
...
column $k$$=1$ & $3$ & $64$ & $>6$ & $10$ \\
column $k$$=2$ & $6$ & $256$ & $>7$ & $9$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010001 00010101 or 01010101.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+2)X(4) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010001 00010101 or 01010101.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + 2\,a(n-3) + 2\,a(n-4) + 2\,a(n-5) + a(n-6)
\]
...
model reproduces $19$ published values, $56$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A261113.
