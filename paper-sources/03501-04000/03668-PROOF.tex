...
\title{The conjectured generating function for OEIS A270934, proved}
...
\begin{abstract}
OEIS A270934 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $40$ coefficients in exact arithmetic
...
OEIS A270934 is ``Number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 221", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 8, 4, 44, 9, 116,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:32, revision 20) this is still
...
model reproduces all $59$ terms the entry publishes, exactly, in integer arithmetic --- a
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=8$ and $\deg D_2=6$; its expansion as a
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $40$
...
numerator. Its coefficients are the integers $c_1,\dots,c_{6}$ read off $D_2$, the first few being $0,\ 3,\ 0,\ -3,\ 0,\ 1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
First, the model reproduces every one of the $59$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270934.
