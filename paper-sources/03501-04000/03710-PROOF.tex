...
\title{The empirical recurrence for OEIS A230800, proved by transfer matrix}
...
\begin{abstract}
OEIS A230800 counts arrays of width $2$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
the count is a walk count on $S=133$ states.
\end{abstract}
...
OEIS A230800 is ``Number of n X 2 0..3 arrays x(i,j) with each element horizontally or vertically next to at least one element with value (x(i,j)+1) mod 4 and at least one element with value (x(i,j)-1) mod 4, and upper left element zero.''. It has offset $1$ and begins
\[
0, 2, 2, 16, 34, 154, 432, 1618,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + 6*a(n-2) - 5*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 17 2018 05:46:20, revision 8) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=133.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230800.
