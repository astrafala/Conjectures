...
\title{The conjectured generating function for OEIS A207165, proved}
...
\begin{abstract}
OEIS A207165 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $101$ coefficients in exact arithmetic
...
OEIS A207165 is ``Number of n X 4 0..1 arrays avoiding 0 0 0 and 1 0 1 horizontally and 0 0 1 and 1 0 1 vertically.''. It has offset $1$ and begins
\[
9, 81, 261, 603, 1161, 1989,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: 9*x*(1 + 5*x - x\^{}2 + x\^{}3) / (1 - x)\^{}4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2018 12:28:19, revision 8) this is still
...
\]
a polynomial of degree at most $95$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{95}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $101$
of them --- more than the $95+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207165.
