...
\title{The empirical column recurrences for the table OEIS A207050}
...
\begin{abstract}
OEIS A207050 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A207050 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every 2X3 or 3X2 subblock having exactly one clockwise edge increases.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 150, 150, 441, 270, 441, 1416, 552,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 13:25:09, revision 9) these are still
...
column $k$$=1$ & $4$ & $81$ & $>5$ & $9$ \\
column $k$$=2$ & $5$ & $150$ & $>7$ & $8$ \\
column $k$$=3$ & $4$ & $441$ & $>11$ & $7$ \\
column $k$$=4$ & $3$ & $1416$ & $>12$ & $6$ \\
column $k$$=5$ & $3$ & $4371$ & $>13$ & $5$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X2 0..2 arrays with every 2X3 or 3X2 subblock having exactly one clockwise edge increases.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;2\,a(n-1) + a(n-2) + 8\,a(n-3) - 2\,a(n-4)
\]
holds from index $5+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+1)X3 0..2 arrays with every 2X3 or 3X2 subblock having exactly one clockwise edge increases.''. Its rows are the vertices of a digraph on
$S=150$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-2) + 5\,a(n-3) - a(n-4) - a(n-5)
\]
holds from index $7+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+1)X4 0..2 arrays with every 2X3 or 3X2 subblock having exactly one clockwise edge increases.''. Its rows are the vertices of a digraph on
$S=441$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-3) + a(n-4)
...
The sequence is ``Number of (n+1)X5 0..2 arrays with every 2X3 or 3X2 subblock having exactly one clockwise edge increases.''. Its rows are the vertices of a digraph on
$S=1416$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-3)
...
The sequence is ``Number of (n+1)X6 0..2 arrays with every 2X3 or 3X2 subblock having exactly one clockwise edge increases.''. Its rows are the vertices of a digraph on
$S=4371$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-3)
...
model reproduces $35$ published values, $137$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207050.
