...
\title{The empirical column recurrences for the table OEIS A236018}
...
\begin{abstract}
OEIS A236018 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 3$.
\end{abstract}
...
OEIS A236018 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays colored with the sum of the maximum and minimum values of each 2X2 subblock.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 306, 306, 1276, 1126, 1276, 4672, 3744,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 9*a(n-2) +14*a(n-3) +30*a(n-4) +16*a(n-5) +8*a(n-6) for n>7\\
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 09:00:57, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays colored with the sum of the maximum and minimum values of each 2X2 subblock.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;9\,a(n-2) + 14\,a(n-3) + 30\,a(n-4) + 16\,a(n-5) + 8\,a(n-6)
\]
holds from index $7+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+1)X(4) 0..2 arrays colored with the sum of the maximum and minimum values of each 2X2 subblock.''. Its rows are the vertices of a digraph on
$S=1276$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $18$ published values, $95$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A236018.
