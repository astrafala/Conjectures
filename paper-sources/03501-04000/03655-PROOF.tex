...
\title{The empirical recurrence for OEIS A233099, proved by transfer matrix}
...
\begin{abstract}
OEIS A233099 counts arrays over $\{0,\dots,3\}$ under a neighbour condition together with
...
was seen first. With that, $a(n)$ is a walk count on $S=144$ vertices and the empirical
...
OEIS A233099 is ``Number of 2 X n 0..3 arrays with no element x(i,j) adjacent to value 3-x(i,j) horizontally, diagonally or antidiagonally, top left element zero, and 1 appearing before 2 in row major order.''. It has offset $1$ and begins
\[
3, 11, 65, 386, 2315, 13886, 83315, 499886,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) + a(n-2) - 6*a(n-3) for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 07 2018 14:34:56, revision 7) the statement is
...
first occurrences came first. The state therefore carries one flag for each of the $2$
...
Let $\mathcal V$ be the set of pairs (a column, a flag vector), $|\mathcal V|=S=144$, let
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233099.
