...
\title{The empirical recurrence for OEIS A281710, proved by transfer matrix}
...
\begin{abstract}
OEIS A281710 counts $n\times3$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
every CELL over a neighbour set the entry names, and counted UP TO RELABELLING of the
alphabet; it carries an empirical recurrence of order $3$ contributed by
...
OEIS A281710 is ``Number of n X 3 0..1 arrays with no element unequal to a strict majority of its horizontal, diagonal and antidiagonal neighbors and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
1, 7, 8, 14, 17, 22, 30, 43,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - a(n-3) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 20 2019 05:35:07, revision 7) this is still
...
By Lemma~\ref{lem:crit} the recurrence holds for every $n>6$. The bound is exact: at $n=6$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
First, the transfer model reproduces all $37$ terms the entry publishes, exactly, in
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A281710.
