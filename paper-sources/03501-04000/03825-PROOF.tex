...
\title{The empirical recurrence for OEIS A208388, proved by counting patterns}
...
\begin{abstract}
OEIS A208388 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $98$ to $S=23$. The entry's empirical recurrence of order
...
OEIS A208388 is ``Number of n X 4 0..2 arrays with new values 0..2 introduced in row major order and no element equal to more than two of its immediate leftward or upward or right-upward antidiagonal neighbors.''. It has offset $1$ and begins
\[
14, 1017, 76806, 5800644, 438083928, 33085555344, 2498731184736, 188712490047552,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 76*a(n-1) - 36*a(n-2) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 02 2018 09:13:05, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;76\,a(n-1) - 36\,a(n-2)
\end{equation}
for all $n>3$.
...
Assembling the $3$ lumped chains into one block-diagonal matrix $N$ of size $S=23$ and
...
\end{equation}
The unlumped alphabet had $98$ rows in all; the lumped chain has $S=23$ states.
...
\[
3!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-2}q(N)\mathbf{1} .
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208388.
