...
\title{The empirical recurrence for OEIS A231038, proved by transfer matrix}
...
\begin{abstract}
OEIS A231038 counts arrays of width $2$ over $\{0,1,2\}$ in which every cell must see, among its
...
the count is a walk count on $S=85$ states.
\end{abstract}
...
OEIS A231038 is ``Number of 2 X n 0..2 arrays x(i,j) with each element horizontally, diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 3 and at least one element with value (x(i,j)-1) mod 3, and upper left element zero.''. It has offset $1$ and begins
\[
0, 0, 0, 4, 0, 32, 80, 560,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 6*a(n-2) + 32*a(n-4) - 20*a(n-5) + 24*a(n-6) - 16*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 25 2018 12:19:44, revision 8) this is still
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 6\,a(n-2) + 32\,a(n-4) - 20\,a(n-5) + 24\,a(n-6) - 16\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231038.
