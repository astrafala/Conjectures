...
\title{The empirical column recurrences for the table OEIS A235954}
...
\begin{abstract}
OEIS A235954 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 2, 3, 4, 5, 6, 7$.
\end{abstract}
...
OEIS A235954 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays colored with the upper median value of each 2X2 subblock.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 294, 294, 905, 722, 905, 2584, 1372,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for diagonal and column k:\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:56:52, revision 6) these are still
...
column $k$$=1$ & $6$ & $81$ & $>9$ & $9$ \\
column $k$$=2$ & $8$ & $294$ & $>12$ & $10$ \\
column $k$$=3$ & $4$ & $905$ & $>13$ & $7$ \\
column $k$$=4$ & $4$ & $2584$ & $>14$ & $6$ \\
column $k$$=5$ & $4$ & $7353$ & $>13$ & $10$ \\
column $k$$=6$ & $4$ & $20800$ & $>14$ & $10$ \\
column $k$$=7$ & $4$ & $58896$ & $>15$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays colored with the upper median value of each 2X2 subblock.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;8\,a(n-3) + 36\,a(n-4) + 48\,a(n-6)
\]
holds from index $9+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+1)X(3) 0..2 arrays colored with the upper median value of each 2X2 subblock.''. Its rows are the vertices of a digraph on
$S=294$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-3) + 10\,a(n-4) - 12\,a(n-7) - 6\,a(n-8)
\]
holds from index $12+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+1)X(4) 0..2 arrays colored with the upper median value of each 2X2 subblock.''. Its rows are the vertices of a digraph on
$S=905$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-4)
...
The sequence is ``Number of (n+1)X(5) 0..2 arrays colored with the upper median value of each 2X2 subblock.''. Its rows are the vertices of a digraph on
$S=2584$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;5\,a(n-4)
\]
holds from index $14+1$ on.

\subsection*{Column $k$$=5$}
The sequence is ``Number of (n+1)X(6) 0..2 arrays colored with the upper median value of each 2X2 subblock.''. Its rows are the vertices of a digraph on
$S=7353$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-4)
...
The sequence is ``Number of (n+1)X(7) 0..2 arrays colored with the upper median value of each 2X2 subblock.''. Its rows are the vertices of a digraph on
$S=20800$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;5\,a(n-4)
...
The sequence is ``Number of (n+1)X(8) 0..2 arrays colored with the upper median value of each 2X2 subblock.''. Its rows are the vertices of a digraph on
$S=58896$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-4)
\]
holds from index $15+1$ on.
...
model reproduces $62$ published values, $268$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235954.
