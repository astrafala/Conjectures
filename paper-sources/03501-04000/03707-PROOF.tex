...
\title{The empirical closed form for OEIS A238806, proved}
...
\begin{abstract}
OEIS A238806 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=9$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
monic linear recurrence whose characteristic polynomial is
$\prod_j(x-\lambda_j)^{\deg p_j+1}$; here that polynomial is $q(x)=(x-1)^{4}$, of degree
$4$. Two things are then checked exactly: that the walk count satisfies the same
recurrence from some index on, which the Cayley--Hamilton theorem reduces to a finite
computation, and that the two sequences agree at $4$ consecutive indices past that
...
OEIS A238806 is ``Number of n X 2 0..2 arrays with no element equal to one plus the sum of elements to its left or one plus the sum of the sum of elements above it, modulo 3.''. It has offset $1$ and begins
\[
3, 8, 15, 25, 39, 58, 83, 115,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = (1/6)*n\^{}3 + (23/6)*n - 1.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 24 2018 08:57:12, revision 7) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;\frac{n^{3}}{6} + \frac{23 n}{6} - 1.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=9$ vertices, and $u,w$ the
...
Here $q(x)=(x-1)^{4}$, of degree $4$; the closed form is a polynomial in $n$, so this is $(x-1)^{4}$, and the recurrence it encodes is
...
\begin{proof}
The vector $w'=q(M)w$ was formed by $4$ matrix--vector products in exact integer
arithmetic, and $u^{\!\top}M^{j}w'$ evaluated for $j=0,1,\dots$, again exactly. Those integers
vanish from the point corresponding to $n=5$ onwards, and the run was continued until the
working vector was identically zero, which settles every larger $j$ at once. So $a$ satisfies
\eqref{eq:rec} for every $n>4$. By Lemma~\ref{lem:ann} $f$ satisfies \eqref{eq:rec}
identically. Both were then evaluated in exact arithmetic at the $4$ consecutive indices
$n=5,\dots,8$ and agree at each. A monic recurrence of order $4$
determines $a(m)$ from $a(m-1),\dots,a(m-4)$, and for every $m\ge9$ both
$m>4$ and $m-4\ge5$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge5$.
Below $n=5$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=5$ on. The remaining indices $n=1,\dots,4$ were
...
First, the walk model reproduces every one of the $45$ terms the entry publishes,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A238806.
