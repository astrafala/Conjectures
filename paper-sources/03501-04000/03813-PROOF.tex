...
\title{The empirical column recurrences for the table OEIS A303254}
...
\begin{abstract}
OEIS A303254 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A303254 is ``T(n,k) = Number of n X k 0..1 arrays with every element equal to 1, 2, 3, 5 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 1, 0, 1, 3, 0, 2, 14,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 11 2023 14:20:00, revision 6) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of 1 X n 0..1 arrays with every element equal to 1, 2, 3, 5 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of 2 X n 0..1 arrays with every element equal to 1, 2, 3, 5 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
model reproduces $19$ published values, $38$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303254.
