...
\title{The empirical recurrence for OEIS A252690, proved by transfer matrix}
...
\begin{abstract}
OEIS A252690 counts arrays of width $5$ over an alphabet of $4$ letters in which every three
...
brings the state count down to $S=2592$.
\end{abstract}
...
OEIS A252690 is ``Number of (n+2) X (3+2) 0..3 arrays with every consecutive three elements in every row and column having exactly two distinct values, and in every diagonal and antidiagonal not having exactly two distinct values, and new values 0 upwards introduced in row major order.''. It has offset $1$ and begins
\[
148, 204, 134, 407, 291, 881, 727, 2067,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 2*a(n-2) - 7*a(n-3) + 2*a(n-4) + 6*a(n-5) + 14*a(n-6) - 20*a(n-7) - 4*a(n-8) + 8*a(n-9) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 05 2018 06:14:07, revision 7) this is still
...
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,
\]
with $S=2592$ states in all. The walk index $j$ and the entry's own $n$ differ by a constant,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252690.
