...
\title{The empirical recurrence for OEIS A252726, proved by transfer matrix}
...
\begin{abstract}
OEIS A252726 counts arrays of width $9$ over an alphabet of $4$ letters in which every three
...
brings the state count down to $S=98835$.
\end{abstract}
...
OEIS A252726 is ``Number of (7+2) X (n+2) 0..3 arrays with every consecutive three elements in every row and diagonal having exactly two distinct values, and in every column and antidiagonal not having exactly two distinct values, and new values 0 upwards introduced in row major order.''. It has offset $1$ and begins
\[
257, 220, 224, 257, 288, 324, 373, 452,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = -a(n-1) + 3*a(n-3) + 3*a(n-4) - 2*a(n-6) - 2*a(n-7) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 06 2018 10:09:59, revision 8) this is still
...
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,
\]
with $S=98835$ states in all. The walk index $j$ and the entry's own $n$ differ by a constant,
...
\begin{theorem}
\[
a(n)\;=\;-a(n-1) + 3\,a(n-3) + 3\,a(n-4) - 2\,a(n-6) - 2\,a(n-7)
\]
for every $n>9$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252726.
