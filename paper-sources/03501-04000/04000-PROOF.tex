...
\title{The empirical column recurrences for the table OEIS A224559}
...
\begin{abstract}
OEIS A224559 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A224559 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 matrices with each 3X3 subblock idempotent.''. It has offset $1$, is
read by antidiagonals, and begins
\[
50, 70, 70, 113, 94, 113, 172, 133,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 3*a(n-1) -3*a(n-2) +2*a(n-3) -a(n-4) -a(n-5) +a(n-6) for n>7\\
k=2: a(n) = 3*a(n-1) -2*a(n-2) -a(n-3) +a(n-4) -a(n-5) +2*a(n-6) -a(n-7) for n>9\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:27:46, revision 8) these are still
...
The sequence is ``Number of (n+2)X3 0..1 matrices with each 3X3 subblock idempotent.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;3\,a(n-1) - 3\,a(n-2) + 2\,a(n-3) - a(n-4) - a(n-5) + a(n-6)
\]
holds from index $7+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+2)X4 0..1 matrices with each 3X3 subblock idempotent.''. Its rows are the vertices of a digraph on
$S=59$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224559.
