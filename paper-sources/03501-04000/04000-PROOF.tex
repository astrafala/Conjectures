...
\title{The empirical column recurrences for the table OEIS A255756}
...
\begin{abstract}
OEIS A255756 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=3$.
\end{abstract}
...
OEIS A255756 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with every 3X3 subblock sum of the medians of the diagonal and antidiagonal minus the two sums of the central row and column nondecreasing horizontally, vertically and ne-to-sw antidiagonally.''. It has offset $1$, is
read by antidiagonals, and begins
\[
512, 2485, 2485, 8776, 7765, 8776, 30182, 24536,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: [linear recurrence of order 36] for n>41\\
k=2: [order 44] for n>53\\
k=3: a(n) = 2*a(n-1) -a(n-2) +4*a(n-4) -8*a(n-5) +4*a(n-6) for n>18\\
k=4: [order 14] for n>26\\
k=5: a(n) = 2*a(n-1) -a(n-2) +4*a(n-4) -8*a(n-5) +4*a(n-6) for n>18\\
k=6: [same order 14] for n>28\\
k=7: a(n) = 2*a(n-1) -a(n-2) +4*a(n-4) -8*a(n-5) +4*a(n-6) for n>20\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 15:11:57, revision 6) these are still
...
The sequence is ``Number of (n+2)X5 0..1 arrays with every 3X3 subblock sum of the medians of the diagonal and antidiagonal minus the two sums of the central row and column nondecreasing horizontally, vertically and ne-to-sw antidiagonally.''. Its rows are the vertices of a digraph on
$S=3564$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-1) - a(n-2) + 4\,a(n-4) - 8\,a(n-5) + 4\,a(n-6)
\]
...
model reproduces $10$ published values, $54$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A255756.
