...
\title{The conjectured generating function for OEIS A284542, proved}
...
\begin{abstract}
OEIS A284542 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (1 + 2*x + 4*x\^{}2 + 6*x\^{}3 - 8*x\^{}5 - 4*x\^{}6 + 10*x\^{}7 + 4*x\^{}8) / ((1 - x)*(1 - 2*x)*(1 + 2*x)*(1 + 4*x\^{}2)).
\end{quote}
