...
\title{The empirical recurrence for OEIS A224561, proved by transfer matrix}
...
\begin{abstract}
OEIS A224561 counts the $(n+3)\times4$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=416$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224561 is ``Number of (n+3) X 4 0..1 matrices with each 4 X 4 subblock idempotent.''. It has offset $1$ and begins
\[
452, 370, 512, 739, 990, 1345, 1852, 2659,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) + a(n-4) - 2*a(n-5) + 2*a(n-6) - 2*a(n-7) + a(n-8) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 01 2018 09:17:30, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $4\times4$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $4\times4$ matrix a \emph{band} when every one of its $1$ contiguous
$4\times4$ subblocks is a window. What the entry counts is the matrices of $4$ columns
whose every $4$ consecutive rows form a band.

Take as states the $3\times4$ strips that occur as the top $3$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=416$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{12}$ strips of $3$ rows and $4$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $4\times3$ entries, so a band is exactly a walk of $1$ windows in
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 3\,a(n-2) + a(n-3) + a(n-4) - 2\,a(n-5) + 2\,a(n-6) - 2\,a(n-7) + a(n-8)
\]
for every $n>10$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224561.
