...
\title{The conjectured generating function for OEIS A221394, proved}
...
\begin{abstract}
OEIS A221394 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=348$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $374$ coefficients in exact arithmetic
...
OEIS A221394 is ``Number of n X 3 arrays of occupancy after each element stays put or moves to some horizontal, vertical or antidiagonal neighbor, with every occupancy equal to zero or two.''. It has offset $1$ and begins
\[
0, 18, 0, 631, 0, 23188,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: x\^{}2*(18 - 233*x\^{}2 + 820*x\^{}4 - 1050*x\^{}6 + 412*x\^{}8 + 46*x\^{}10) / ((1 - x)*(1 + x)*(1 - 47*x\^{}2 + 393*x\^{}4 - 605*x\^{}6 + 268*x\^{}8 + 16*x\^{}10)). - \_Colin Barker\_, Aug 05 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 05 2018 08:21:21, revision 9) this is still
...
\]
and there are $S=348$ states. The digraph is built by reading the entry's own English, and the
...
The family this entry belongs to is read by one model, described by its author as: ```Number of n X W arrays of OCCUPANCY after each element moves to some <neighbour>.'''
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=12$ and $\deg D_2=12$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $360$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{360}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $374$
of them --- more than the $360+1$ the argument requires --- agree.
\end{proof}
...
\]
a constant-coefficient linear recurrence of order $12$, valid past the degree of the
numerator. Its coefficients are the integers $c_1,\dots,c_{12}$ read off $D_2$, the first few being $0,\ 48,\ 0,\ -440,\ 0,\ 998$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221394.
