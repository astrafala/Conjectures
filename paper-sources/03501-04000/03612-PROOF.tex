...
\title{The empirical column recurrences for the table OEIS A235098}
...
\begin{abstract}
OEIS A235098 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A235098 is ``T(n,k) is the number of (n+1) X (k+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
80, 256, 256, 808, 656, 808, 2580, 1728,\ \dots
\]
...
k=1: a(n) = 3*a(n-1) +13*a(n-2) -39*a(n-3) -26*a(n-4) +78*a(n-5).\\
k=2: [order 16].\\
k=3: [order 22].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 20:38:14, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
$c_{1}$ & $6$ & $c_{5}$ & $2028$ & $c_{9}$ & $32278$ & $c_{13}$ & $14456$ \\
$c_{2}$ & $20$ & $c_{6}$ & $-1568$ & $c_{10}$ & $-57324$ & $c_{14}$ & $-82056$ \\
$c_{3}$ & $-178$ & $c_{7}$ & $-11316$ & $c_{11}$ & $-42818$ & $c_{15}$ & $10920$ \\
$c_{4}$ & $-45$ & $c_{8}$ & $15017$ & $c_{12}$ & $104201$ & $c_{16}$ & $16380$ \\
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235098.
