...
\title{The empirical recurrence for OEIS A198448, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A198448 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
$3!\,a(n)=\sum_i\binom{3}{i}D_{3-i}L_i(n)$ with $D_m$ the derangement numbers and
...
lumpable, cutting $6818$ pairs down to $S=1223$ states. The entry's empirical recurrence of
order $13$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A198448 is ``Number of 2nX4 0..2 arrays with values 0..2 introduced in row major order and each element unequal to exactly two horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
13, 177, 1399, 14109, 146187, 1532249, 16411255, 176845253,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 07:52:27, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;20\,a(n-1) - 94\,a(n-2) - 208\,a(n-3) + 1351\,a(n-4) + 5600\,a(n-5) - 22336\,a(n-6) - 39628\,a(n-7) + 116496\,a(n-8) + 216496\,a(n-9) - 188416\,a(n-10) - 809920\,a(n-11) - 267264\,a(n-12) + 1638400\,a(n-13)
\end{equation}
for all $n>13$.
...
class, so with $K=3$ available values the entry counts admissible patterns with at most $K$
classes.
...
$\binom{3}{i}(3-i)!\sum_{t\le3-i}(-1)^t/t!=\binom{3}{i}D_{3-i}$.
\end{proof}
...
Assembling the $3$ lumped chains block-diagonally into $N$ of size $S=1223$, with the
...
3!\;a(n)\;=\;\mathbf{v}^{\!\top}N^{\,L-2}\mathbf{u},\qquad L=2n+0 .
\end{equation}
...
index,
\[
3!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-13}\,
\Bigl(h_{13}-\sum_i c_i h_{13-i}\Bigr)
\]
...
$w=h_{13}-\sum_i c_ih_{13-i}$; and it is enough to check $m=0,1,\dots,S-1$.
\end{lemma}

\begin{proof}
Substitute \eqref{eq:walk2} and factor. The scalar $3!$ is nonzero, so it does not
...
$13$ applications of $G$ in exact integer arithmetic and $\mathbf{v}^{\!\top}G^{m}w$
evaluated for $m=0,1,\dots$, again exactly. Every one of those integers vanishes from the
index corresponding to $n=13+1$ onwards, and the run was continued until $S+1$
...
\eqref{eq:walk2}, divided by $3!$, was evaluated at every one of the $17$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198448.
