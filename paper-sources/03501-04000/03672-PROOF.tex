...
\title{The conjectured generating function for OEIS A273405, proved}
...
\begin{abstract}
OEIS A273405 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A273405 is ``Number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 673", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 4, 21, 44, 77, 116,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1+2*x+13*x\^{}2+4*x\^{}3-4*x\^{}4) / ((1-x)\^{}3*(1+x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:34, revision 15) this is still
...
numerator. Its coefficients are the integers $c_1,\dots,c_{4}$ read off $D_2$, the first few being $2,\ 0,\ -2,\ 1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A273405.
