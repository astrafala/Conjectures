...
\title{The conjectured generating function for OEIS A282653, proved}
...
\begin{abstract}
OEIS A282653 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=18$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $37$ coefficients in exact arithmetic
...
OEIS A282653 is ``Binary representation of the x-axis, from the origin to the right edge, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 491", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 10, 111, 10, 11111, 10,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 + 10*x + 11*x\^{}2 - 990*x\^{}3 + 10*x\^{}4 - 1000*x\^{}5 + 100000*x\^{}7 + 10000000*x\^{}11) / ((1 - x)*(1 + x)*(1 - 10*x)*(1 + 10*x)*(1 + x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:37, revision 11) this is still
...
\]
and there are $S=18$ states. The digraph is built by reading the entry's own English, and the
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=11$ and $\deg D_2=6$; its expansion as a
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $37$
...
\[
a(n)\;=\;\sum_{i\ge1}c_i\,a(n-i)\qquad\text{for every }n>11,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A282653.
