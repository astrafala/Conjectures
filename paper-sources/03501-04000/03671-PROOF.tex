...
\title{The conjectured generating function for OEIS A273334, proved}
...
\begin{abstract}
OEIS A273334 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $34$ coefficients in exact arithmetic
...
OEIS A273334 is ``Number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 657", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 4, 17, 48, 80, 120,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1+x+8*x\^{}2+8*x\^{}3-17*x\^{}4+7*x\^{}5) / (1-x)\^{}3.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:34, revision 14) this is still
...
model reproduces all $47$ terms the entry publishes, exactly, in integer arithmetic --- a
...
The family this entry belongs to is read by one model, described by its author as: ``Active-cell counts of a two-dimensional cellular automaton.''
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=5$ and $\deg D_2=3$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $29$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{29}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $34$
of them --- more than the $29+1$ the argument requires --- agree.
\end{proof}
...
First, the model reproduces every one of the $47$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A273334.
