...
\title{The empirical closed form for OEIS A203872, proved}
...
\begin{abstract}
OEIS A203872 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=100$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
monic linear recurrence whose characteristic polynomial is
$\prod_j(x-\lambda_j)^{\deg p_j+1}$; here that polynomial is $q(x)=(x-21)(x-1)$, of degree
...
OEIS A203872 is ``Number of (n+1)X3 0..3 arrays with no 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..3 introduced in row major order.''. It has offset $1$ and begins
\[
55, 1135, 23815, 500095, 10501975, 220541455, 4631370535, 97258781215,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 54*21\^{}(n-1) +1
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:47:51, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;54 \cdot 21^{n - 1} + 1.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=100$ vertices, and $u,w$ the
...
Here $q(x)=(x-21)(x-1)$, of degree $2$, and the recurrence it encodes is
...
vanish from the point corresponding to $n=3$ onwards, and the run was continued until the
working vector was identically zero, which settles every larger $j$ at once. So $a$ satisfies
\eqref{eq:rec} for every $n>2$. By Lemma~\ref{lem:ann} $f$ satisfies \eqref{eq:rec}
identically. Both were then evaluated in exact arithmetic at the $2$ consecutive indices
$n=3,\dots,4$ and agree at each. A monic recurrence of order $2$
determines $a(m)$ from $a(m-1),\dots,a(m-2)$, and for every $m\ge5$ both
$m>2$ and $m-2\ge3$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge3$.
Below $n=3$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=3$ on. The remaining indices $n=1,\dots,2$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203872.
