...
\title{The empirical column recurrences for the table OEIS A252228}
...
\begin{abstract}
OEIS A252228 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A252228 is ``T(n,k)=Number of (n+2)X(k+2) 0..2 arrays with every 3X3 subblock row and column sum 2 3 or 4 and every diagonal and antidiagonal sum not 2 3 or 4.''. It has offset $1$, is
read by antidiagonals, and begins
\[
228, 368, 368, 812, 1028, 812, 1442, 3066,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:57:58, revision 6) these are still
...
column $k$$=1$ & $7$ & $757$ & $>10$ & $9$ \\
column $k$$=2$ & $6$ & $6643$ & $>9$ & $8$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X3 0..2 arrays with every 3X3 subblock row and column sum 2 3 or 4 and every diagonal and antidiagonal sum not 2 3 or 4.''. Its rows are the vertices of a digraph on
$S=757$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+2)X4 0..2 arrays with every 3X3 subblock row and column sum 2 3 or 4 and every diagonal and antidiagonal sum not 2 3 or 4.''. Its rows are the vertices of a digraph on
$S=6643$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $17$ published values, $72$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252228.
