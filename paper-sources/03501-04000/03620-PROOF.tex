...
\title{The empirical recurrence for OEIS A224586, proved by transfer matrix}
...
\begin{abstract}
OEIS A224586 counts the $(n+6)\times12$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=161304$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224586 is ``Number of (n+6)X12 0..1 matrices with each 7X7 subblock idempotent.''. It has offset $1$ and begins
\[
138581, 27611, 27945, 30456, 32814, 34588, 35891, 36872,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) -2*a(n-2) -2*a(n-3) +3*a(n-4) -a(n-5) +a(n-7) -2*a(n-8) +2*a(n-10) -a(n-11) for n>17
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:29:38, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $7\times7$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $7\times12$ matrix a \emph{band} when every one of its $6$ contiguous
$7\times7$ subblocks is a window. What the entry counts is the matrices of $12$ columns
whose every $7$ consecutive rows form a band.

Take as states the $6\times12$ strips that occur as the top $6$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=161304$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{72}$ strips of $6$ rows and $12$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $7\times6$ entries, so a band is exactly a walk of $6$ windows in
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 2\,a(n-2) - 2\,a(n-3) + 3\,a(n-4) - a(n-5) + a(n-7) - 2\,a(n-8) + 2\,a(n-10) - a(n-11)
\]
for every $n>17$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224586.
