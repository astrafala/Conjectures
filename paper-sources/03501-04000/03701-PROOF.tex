...
\title{The empirical recurrence for OEIS A199143, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A199143 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
order $9$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A199143 is ``Number of nX2 0..3 arrays with values 0..3 introduced in row major order and each element equal to one or two horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
1, 3, 13, 65, 393, 2539, 16866, 113432,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 7*a(n-1) +2*a(n-2) -16*a(n-3) -49*a(n-4) -35*a(n-5) +56*a(n-6) +54*a(n-7) +45*a(n-8) -27*a(n-9) for n>11
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 08:06:40, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;7\,a(n-1) + 2\,a(n-2) - 16\,a(n-3) - 49\,a(n-4) - 35\,a(n-5) + 56\,a(n-6) + 54\,a(n-7) + 45\,a(n-8) - 27\,a(n-9)
\end{equation}
for all $n>11$.
...
index,
\[
4!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-9}\,
\Bigl(h_{9}-\sum_i c_i h_{9-i}\Bigr)
\]
...
$w=h_{9}-\sum_i c_ih_{9-i}$; and it is enough to check $m=0,1,\dots,S-1$.
\end{lemma}
...
index corresponding to $n=11+1$ onwards, and the run was continued until $S+1$
...
\eqref{eq:walk2}, divided by $4!$, was evaluated at every one of the $21$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A199143.
