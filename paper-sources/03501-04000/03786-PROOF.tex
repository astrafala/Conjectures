...
\title{The empirical recurrence for OEIS A269760, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A269760 counts arrays over $\{0,\dots,5\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=12$ vertices,
...
OEIS A269760 is ``Number of n X 1 0..5 arrays with some element plus some horizontally or vertically adjacent neighbor totalling five exactly once.''. It has offset $1$ and begins
\[
0, 6, 60, 450, 3000, 18750, 112500, 656250,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 10*a(n-1) - 25*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 22 2018 06:51:27, revision 7) the statement is
...
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,5\}^{1},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=12,
\]
...
the digraph dense, with $(5+1)^{2\cdot1}$ edges instead of the sparse set used here.
\end{remark}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269760.
