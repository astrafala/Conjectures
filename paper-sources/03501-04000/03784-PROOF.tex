...
\title{The empirical recurrence for OEIS A231104, proved by transfer matrix}
...
\begin{abstract}
OEIS A231104 counts arrays of width $4$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
carries an empirical recurrence of order $2$ contributed by R.~H.~Hardin. It is true, and
it is decidable rather than empirical: a cell's neighbours lie in three consecutive rows, so
the count is a walk count on $S=4785$ states.
\end{abstract}
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 25 2021 05:18:42, revision 9) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=4785.
\]
