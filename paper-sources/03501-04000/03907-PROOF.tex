...
\title{The conjectured generating function for OEIS A225011, proved}
...
\begin{abstract}
OEIS A225011 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=80$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $100$ coefficients in exact arithmetic
...
OEIS A225011 is ``Number of 4 X n 0..1 arrays with rows unimodal and columns nondecreasing.''. It has offset $1$ and begins
\[
5, 25, 95, 295, 791, 1897,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.: -x*(x\^{}4 - 5*x\^{}3 + 10*x\^{}2 - 10*x + 5) *(x\^{}4 - 3*x\^{}3 + 4*x\^{}2 - 2*x + 1) / (x-1)\^{}9. - \_R. J. Mathar\_, May 17 2014
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 09 2023 15:37:30, revision 14) this is still
...
\]
and there are $S=80$ states. The digraph is built by reading the entry's own English, and the
...
The family this entry belongs to is read by one model, described by its author as: ``Order conditions along rows, columns, diagonals and antidiagonals.''
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=9$ and $\deg D_2=9$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $89$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{89}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $100$
of them --- more than the $89+1$ the argument requires --- agree.
\end{proof}
...
\]
a constant-coefficient linear recurrence of order $9$, valid past the degree of the
numerator. Its coefficients are the integers $c_1,\dots,c_{9}$ read off $D_2$, the first few being $9,\ -36,\ 84,\ -126,\ 126,\ -84$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A225011.
