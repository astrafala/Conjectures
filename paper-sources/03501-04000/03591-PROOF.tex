...
\title{The empirical column recurrences for the table OEIS A280398}
...
\begin{abstract}
OEIS A280398 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the rows $n=1$.
\end{abstract}
...
OEIS A280398 is ``T(n,k) = Number of n X k 0..2 arrays with no element unequal to a strict majority of its horizontal and antidiagonal neighbors, with the exception of exactly one element, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 0, 0, 2, 2, 0, 2, 6,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1);\\
k=2: a(n) = 10*a(n-1) -33*a(n-2) +40*a(n-3) -16*a(n-4);\\
k=3: [order 6] for n>7;\\
k=4: [order 14] for n>15;\\
k=5: [order 24] for n>25;\\
k=6: [order 64] for n>65.\\
\end{quote}
As of the ``Last modified'' line on the live entry (May 09 2022 00:11:53, revision 6) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of 1 X n 0..2 arrays with no element unequal to a strict majority of its horizontal and antidiagonal neighbors, with the exception of exactly one element, and with new values introduced in order 0 sequentially upwards.''. Its rows are the vertices of a digraph on
$S=31$ vertices, edges being the admissible consecutive pairs, and the count is
...
model reproduces $10$ published values, $17$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A280398.
