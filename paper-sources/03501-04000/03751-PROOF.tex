...
\title{The conjectured generating function for OEIS A250767, proved}
...
\begin{abstract}
OEIS A250767 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=128$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $136$ coefficients in exact arithmetic
...
OEIS A250767 is ``Number of (n+1) X (6+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
262, 410, 622, 962, 1558, 2666,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: 2*x*(131 - 319*x + 146*x\^{}2) / ((1 - x)\^{}2*(1 - 2*x)). - \_Colin Barker\_, Nov 18 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 18 2018 12:23:25, revision 8) this is still
...
\]
and there are $S=128$ states. The digraph is built by reading the entry's own English, and the
model reproduces all $27$ terms the entry publishes, exactly, in integer arithmetic --- a
...
\]
a polynomial of degree at most $131$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{131}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $136$
of them --- more than the $131+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{3}$ read off $D_2$, the first few being $4,\ -5,\ 2$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
First, the model reproduces every one of the $27$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250767.
