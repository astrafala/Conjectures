...
\title{The empirical column recurrences for the table OEIS A251451}
...
\begin{abstract}
OEIS A251451 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A251451 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with no 2X2 subblock having the minimum of its diagonal elements less than the absolute difference of its antidiagonal elements.''. It has offset $1$, is
read by antidiagonals, and begins
\[
45, 230, 230, 1223, 2429, 1223, 6505, 26633,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 5*a(n-1) +4*a(n-2) -11*a(n-3) -6*a(n-4) +5*a(n-5) +4*a(n-6)\\
k=2: [order 18]\\
k=3: [order 36] for n>37\\
k=4: [order 88] for n>90\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 13:22:29, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with no 2X2 subblock having the minimum of its diagonal elements less than the absolute difference of its antidiagonal elements.''. Its rows are the vertices of a digraph on
...
$c_{1}$ & $9$ & $c_{6}$ & $534$ & $c_{11}$ & $-110$ & $c_{16}$ & $1$ \\
$c_{2}$ & $34$ & $c_{7}$ & $-258$ & $c_{12}$ & $234$ & $c_{17}$ & $-4$ \\
$c_{3}$ & $-100$ & $c_{8}$ & $-265$ & $c_{13}$ & $2$ & $c_{18}$ & $1$ \\
$c_{4}$ & $-243$ & $c_{9}$ & $231$ & $c_{14}$ & $-65$ &  &  \\
$c_{5}$ & $272$ & $c_{10}$ & $-151$ & $c_{15}$ & $20$ &  &  \\
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251451.
