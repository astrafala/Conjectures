...
\title{A proof of the conjectured recurrence for OEIS A306948}
...
\begin{abstract}
OEIS A306948 carries a conjectured linear recurrence with polynomial coefficients, of
order 4. The entry gives no generating function, but it does give the sequence as
...
OEIS A306948 is ``Expansion of e.g.f. (1 + x)*log(1 + x)*exp(x)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 3, 5, 8, 9, 19, -15,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = Sum\_\{k=1..n\} (-1)\textasciicircum{}(k-1)*binomial(n,k)*(n - k + 1)*(k - 1)!.
\end{quote}
so that, writing $F(n,k)$ for the summand,
\begin{equation}\label{eq:sum}
a(n)\;=\;\sum_{k=1}^{n}F(n,k),
\qquad F(n,k)=\left(-1\right)^{k - 1} \left(- k + n + 1\right) {\binom{n}{k}} \left(k - 1\right)! .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} D-finite with recurrence a(n) +(n-5)*a(n-1) +(-3*n+10)*a(n-2) +3*(n-3)*a(n-3) +(-n+3)*a(n-4)=0. - \emph{R. J. Mathar}, Aug 20 2021
\end{quote}
As of the ``Last modified'' line on the live entry (2021-10-29, revision 13) the
...
$(\sigma,R)$ can be \emph{checked}, exactly, whatever produced it. For A306948 such a pair
exists with $r=2$:
\[
\sigma_{0}(n)=1,\qquad \sigma_{1}(n)=\frac{1 - n}{n + 1},\qquad \sigma_{2}(n)=- \frac{1}{n + 1},
\qquad
R(n,k)\;=\;\frac{k^{2} - k n - 3 k}{k^{3} - 3 k^{2} n - 4 k^{2} + 3 k n^{2} + 8 k n + 5 k - n^{3} - 4 n^{2} - 5 n - 2} .
\]
...
\[
\frac{h(n+1)}{h(n)}\;=\;\frac{n^{2} + 4 n + 3}{n^{2} + 4 n + 4},
\]
...
\]
One extra order buys the homogeneity. Explicitly, $L=\left(\frac{- n^{2} - 4 n - 3}{n^{2} + 4 n + 4}\right) + \left(\frac{2 n^{2} + 6 n + 1}{n^{2} + 4 n + 4}\right)\,N + \left(\frac{- n^{2} - n + 3}{n^{2} + 4 n + 4}\right)\,N^{2} + \left(- \frac{1}{n + 2}\right)\,N^{3}$.
...
\[
Q\;=\;\left(\frac{n^{2} + 4 n + 4}{n + 3}\right) + \left(- n - 3\right)\,N .
\]
The coefficients of $Q$ are rational functions of $n$; their only integer poles are $n\in\{-3\}$, below the range claimed here.

\begin{theorem}
The conjectured recurrence
\[
1\,a(n) + (n - 5)\,a(n-1) + (10 - 3 n)\,a(n-2) + 3 \left(n - 3\right)\,a(n-3) + (3 - n)\,a(n-4) \;=\;0
\]
...
to the conjectured operator. The integer poles of $Q$ all lie below $n=4$.
\end{proof}
...
all 21 values of $n$ from $n=4$ upward. The formula \eqref{eq:sum} was
...
\url{https://oeis.org}, 2026. Sequence A306948.
