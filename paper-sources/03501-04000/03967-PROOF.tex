...
\title{The conjectured generating function for OEIS A284246, proved}
...
\begin{abstract}
OEIS A284246 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=23$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $47$ coefficients in exact arithmetic
...
OEIS A284246 is ``Decimal representation of the x-axis, from the origin to the right edge, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 841", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 0, 6, 14, 30, 46,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (4096*x\^{}16 - 1024*x\^{}14 - 4096*x\^{}12 - 1024*x\^{}11 + 768*x\^{}10 + 192*x\^{}9 + 64*x\^{}8 - 8*x\^{}7 - 76*x\^{}6 - 10*x\^{}5 + 5*x\^{}4 + 14*x\^{}3 + 2*x\^{}2 + 1)/(4*x\^{}6 - x\^{}4 - 4*x\^{}2 + 1). (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:38, revision 12) this is still
...
\]
and there are $S=23$ states. The digraph is built by reading the entry's own English, and the
model reproduces all $34$ terms the entry publishes, exactly, in integer arithmetic --- a
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=16$ and $\deg D_2=6$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $39$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{39}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $47$
of them --- more than the $39+1$ the argument requires --- agree.
\end{proof}
...
\[
a(n)\;=\;\sum_{i\ge1}c_i\,a(n-i)\qquad\text{for every }n>16,
\]
a constant-coefficient linear recurrence of order $6$, valid past the degree of the
numerator. Its coefficients are the integers $c_1,\dots,c_{6}$ read off $D_2$, the first few being $0,\ 4,\ 0,\ 1,\ 0,\ -4$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
First, the model reproduces every one of the $34$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A284246.
