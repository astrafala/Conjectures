...
\title{The conjectured generating function for OEIS A239358, proved}
...
\begin{abstract}
OEIS A239358 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=105$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $123$ coefficients in exact arithmetic
...
OEIS A239358 is ``Number of n X 5 0..2 arrays with no element equal to one plus the sum of elements to its left or one plus the sum of elements above it or one plus the sum of the elements diagonally to its northwest, modulo 3.''. It has offset $1$ and begins
\[
6, 16, 32, 60, 106, 156,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 25 2018 12:24:22, revision 8) this is still
...
\]
and there are $S=105$ states. The digraph is built by reading the entry's own English, and the
model reproduces all $42$ terms the entry publishes, exactly, in integer arithmetic --- a
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=12$ and $\deg D_2=4$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $117$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{117}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $123$
of them --- more than the $117+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{4}$ read off $D_2$, the first few being $4,\ -6,\ 4,\ -1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
First, the model reproduces every one of the $42$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A239358.
