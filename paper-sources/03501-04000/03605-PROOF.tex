...
\title{The empirical recurrence for OEIS A230970, proved by transfer matrix}
...
\begin{abstract}
OEIS A230970 counts arrays of width $3$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
the count is a walk count on $S=737$ states.
\end{abstract}
...
OEIS A230970 is ``Number of (n+2) X (1+2) 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
0, 16, 64, 256, 1344, 7056, 34944, 173056,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) + 20*a(n-3) + 25*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 25 2018 08:53:01, revision 10) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=737.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230970.
