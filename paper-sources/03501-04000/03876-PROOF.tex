...
\title{The conjectured generating function for OEIS A284179, proved}
...
\begin{abstract}
OEIS A284179 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=22$ vertices, so its generating function is a rational function whose numerator
...
\[
1, 10, 101, 1010, 10111, 101111,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 + 9*x + 91*x\^{}2 + 909*x\^{}3 - 899*x\^{}4 + 1000*x\^{}5 - 1000*x\^{}6 + 900*x\^{}7 + 90*x\^{}8 + 9*x\^{}9 + x\^{}10 + 10000*x\^{}13 - 10000*x\^{}14) / ((1 - x)*(1 - 10*x)*(1 + 10*x)*(1 + 100*x\^{}2)).
\end{quote}
...
\]
and there are $S=22$ states. The digraph is built by reading the entry's own English, and the
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=14$ and $\deg D_2=5$; its expansion as a
