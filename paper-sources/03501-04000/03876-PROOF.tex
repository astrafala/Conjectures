...
\title{The empirical recurrence for OEIS A269895, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A269895 counts arrays over $\{0,\dots,6\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=14$ vertices,
...
OEIS A269895 is ``Number of n X 1 0..6 arrays with some element plus some horizontally or vertically adjacent neighbor totalling six exactly once.''. It has offset $1$ and begins
\[
0, 7, 84, 756, 6048, 45360, 326592, 2286144,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 12*a(n-1) - 36*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 22 2018 06:51:18, revision 7) the statement is
...
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,6\}^{1},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=14,
\]
...
the digraph dense, with $(6+1)^{2\cdot1}$ edges instead of the sparse set used here.
\end{remark}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269895.
