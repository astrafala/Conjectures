...
\title{The empirical column recurrences for the table OEIS A237375}
...
\begin{abstract}
OEIS A237375 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A237375 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with the maximum plus the upper median plus the lower median plus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 288, 288, 1024, 1164, 1024, 3840, 4760,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 196*a(n-4) for n>6\\
k=2: a(n) = 370*a(n-4) -3916*a(n-8) +6904*a(n-12) -1984*a(n-16) for n>18\\
k=3: [order 92] for n>94\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:07:37, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with the maximum plus the upper median plus the lower median plus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
...
model reproduces $8$ published values, $35$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237375.
