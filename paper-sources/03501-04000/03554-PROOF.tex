...
\title{The empirical column recurrences for the table OEIS A259250}
...
\begin{abstract}
OEIS A259250 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A259250 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with each 2X2 subblock having clockwise pattern 0000 0011 or 0111.''. It has offset $1$, is
read by antidiagonals, and begins
\[
9, 21, 21, 48, 65, 48, 111, 192,\ \dots
\]
...
k=7: [order 54]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:50:01, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with each 2X2 subblock having clockwise pattern 0000 0011 or 0111.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..1 arrays with each 2X2 subblock having clockwise pattern 0000 0011 or 0111.''. Its rows are the vertices of a digraph on
$S=9$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1) + 4\,a(n-2) - 11\,a(n-3) - 3\,a(n-4)
\]
holds from index $3+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+1)X4 0..1 arrays with each 2X2 subblock having clockwise pattern 0000 0011 or 0111.''. Its rows are the vertices of a digraph on
$S=17$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-1) + 12\,a(n-2) - 11\,a(n-3) - 30\,a(n-4)
\]
...
model reproduces $24$ published values, $82$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A259250.
