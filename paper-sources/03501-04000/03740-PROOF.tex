...
\title{The conjectured generating function for OEIS A250766, proved}
...
\begin{abstract}
OEIS A250766 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $72$ coefficients in exact arithmetic
...
OEIS A250766 is ``Number of (n+1) X (5+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
133, 214, 344, 572, 996, 1812,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: x*(133 - 318*x + 153*x\^{}2) / ((1 - x)\^{}2*(1 - 2*x)). - \_Colin Barker\_, Nov 18 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 18 2018 12:24:03, revision 8) this is still
...
\]
a polynomial of degree at most $67$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{67}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $72$
of them --- more than the $67+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250766.
