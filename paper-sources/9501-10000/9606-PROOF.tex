...
\title{The empirical recurrence for OEIS A233252, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233252 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 10 2018 15:53:50, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 416 x^{2} + 10 x}{2304 x^{2} - 160 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2304 x^{2} - 160 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233252.
