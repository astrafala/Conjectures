...
\title{A proof of the conjectured recurrence for OEIS A217358}
...
\begin{abstract}
OEIS A217358 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A217358 is ``Series reversion of x-x\^{}3-x\^{}4''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 0, 1, 1, 3, 7, 16, 45,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} 46*n*(n-1)*(n-2)*a(n) -(n-1)*(n-2)*(11*n-74)*a(n-1) -(n-2)*(336*n\^{}2-1359*n+1351)*a(n-2) +(-347*n\^{}3+2190*n\^{}2-3861*n+1330)*a(n-3) + 8*(2*n-7)*(4*n-15)*(4*n-17)*a(n-4) = 0.
\end{quote}
As of the ``Last modified'' line on the live entry (2013-09-11, revision 12) it is still
...
\begin{quote}\small
Recurrence (order 3): 23*(n-2)*(n-1)*n*(9*n-25)*a(n) = -(n-2)*(n-1)*(54*n\^{}2 - 231*n + 248)*a(n-1) + (n-2)*(1485*n\^{}3 - 10065*n\^{}2 + 22292*n - 16088)*a(n-2) + 8*(2*n-5)*(4*n-13)*(4*n-11)*(9*n-16)*a(n-3). - \emph{Vaclav Kotesovec}, Sep 10 2013
\end{quote}
...
\[
46 n \left(n - 2\right) \left(n - 1\right)\,a(n) -  \left(n - 2\right) \left(n - 1\right) \left(11 n - 74\right)\,a(n-1) -  \left(n - 2\right) \left(336 n^{2} - 1359 n + 1351\right)\,a(n-2) + (- 347 n^{3} + 2190 n^{2} - 3861 n + 1330)\,a(n-3) + 8 \left(2 n - 7\right) \left(4 n - 17\right) \left(4 n - 15\right)\,a(n-4) \;=\;0
\]
...
entry's own values and found to vanish, for all 27 values of $n$ from
$n=5$ upward.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A217358.
