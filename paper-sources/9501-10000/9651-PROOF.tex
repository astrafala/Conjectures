...
\title{The empirical recurrence for OEIS A233211, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233211 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 10 2018 10:00:55, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 165 x^{3} + 47 x^{2} - 2 x}{225 x^{3} - 259 x^{2} + 35 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 225 x^{3} + 259 x^{2} - 35 x + 1
\]
