...
\title{A proof of the conjectured recurrence for OEIS A208355}
...
\begin{abstract}
OEIS A208355 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $6$, a polynomial of degree $0$, which proves
...
OEIS A208355 is ``Right edge of the triangle in A208101''. It has offset $0$ and begins
\[
...
\begin{quote}\small
\textbf{Conjecture:} -(n+3)*(n-2)*a(n) - 4*a(n-1) + 4*(n-1)\^{}2*a(n-2) = 0. - \emph{R. J. Mathar}, Aug 04 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2026-03-11, revision 84) the
...
\[
p_{0}(n)=- \left(n - 2\right) \left(n + 3\right),\qquad p_{1}(n)=-4,\qquad p_{2}(n)=4 \left(n - 1\right)^{2}
\]
...
\section{The computation for A208355}
...
\[
- \left(n - 2\right) \left(n + 3\right)\,a(n) - 4\,a(n-1) + 4 \left(n - 1\right)^{2}\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A208355, agreeing on all 13 terms compared.
...
vanish, for all 36 values of $n$ from $n=2$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=6$ is an
...
\url{https://oeis.org}, 2026. Sequence A208355.
