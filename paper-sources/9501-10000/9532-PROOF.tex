...
\title{The empirical recurrence for OEIS A204646, derived from the entry's generating function}
...
\begin{abstract}
OEIS A204646 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A204646 is ``Number of (n+1) X 4 0..1 arrays with column and row pair sums b(i,j)=a(i,j)+a(i,j-1) and c(i,j)=a(i,j)+a(i-1,j) nondecreasing in column and row directions, respectively.''. It has offset $1$ and begins
\[
28, 56, 104, 178, 284, 434, 637, 908,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 08 2018 10:02:50, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{7} - 20 x^{6} + 48 x^{5} - 42 x^{4} - 20 x^{3} + 56 x^{2} - 28 x}{x^{6} - 4 x^{5} + 5 x^{4} - 5 x^{2} + 4 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{6} + 4 x^{5} - 5 x^{4} + 5 x^{2} - 4 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 3 x^{7} + 20 x^{6} - 48 x^{5} + 42 x^{4} + 20 x^{3} - 56 x^{2} + 28 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204646.
