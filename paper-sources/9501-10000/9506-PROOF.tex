...
\title{A proof of the conjectured recurrence for OEIS A268554}
...
\begin{abstract}
OEIS A268554 carries a conjectured linear recurrence with polynomial coefficients, of
order 1. The entry posts no generating function, so the usual route through a
...
OEIS A268554 is ``Diagonal of the rational function 1/((1 - w - u v) * (1 - x y - x z - y z))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 36, 6300, 1552320, 445945500, 139815211536, 46384755633216, 16009450307136000,\ \dots
\]
...
a(n)\;=\;\frac{\left(3 n\right)! \left(4 n\right)!}{n!^{3} \left(2 n\right)!^{2}} .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} n\^{}3*(2*n-1)*a(n) -6*(4*n-1)*(3*n-1)*(3*n-2)*(4*n-3)*a(n-1)=0. - \emph{R. J. Mathar}, Mar 10 2016
\end{quote}
...
n^{3} \left(2 n - 1\right)\,a(n) -  6 \left(3 n - 2\right) \left(3 n - 1\right) \left(4 n - 3\right) \left(4 n - 1\right)\,a(n-1)
\end{equation}
...
\section{The computation for A268554}

The closed form \eqref{eq:cf} splits into one similarity class, with
representative
\[
\frac{\left(3 n\right)! \left(4 n\right)!}{n!^{3} \left(2 n\right)!^{2}}
\]
...
exact integer arithmetic at every index where it applies: $12$ instances beginning at
$n=1$, all of them zero. This is not part of the proof, which is symbolic, but it
...
entry \href{https://oeis.org/A268554}{A268554}, 2025-11-05.
