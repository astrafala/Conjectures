...
\title{A proof of the conjectured recurrence for OEIS A002867}
...
\begin{abstract}
OEIS A002867 carries a conjectured linear recurrence with polynomial coefficients, of
order 2, contributed by its contributor. We prove it. The generating function of the
...
OEIS A002867 is ``a(n) = binomial(n,floor(n/2))*(n+1)!''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 12, 72, 720, 7200, 100800, 1411200,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;- \frac{1}{\sqrt{1 - 4 x^{2}} \left(2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) - 2*a(n-1) - 4*n*(n-1)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=-2,\qquad p_{2}(n)=- 4 n \left(n - 1\right)
\]
...
\section{The computation for A002867}
...
\[
1\,a(n) - 2\,a(n-1) -  4 n \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A002867, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A002867.
