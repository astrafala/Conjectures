...
\title{A proof of the conjectured recurrence for OEIS A119975}
...
\begin{abstract}
OEIS A119975 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A119975 is ``E.g.f. exp(x)*(Bessel\_I(0,2*sqrt(2)x) + Bessel\_I(1,2*sqrt(2)x)/sqrt(2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 7, 22, 77, 266, 947, 3382,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{3 x - \sqrt{- 7 x^{2} - 2 x + 1} + 1}{4 x \sqrt{- 7 x^{2} - 2 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(n-4)*a(n-1) +(9-13*n)*a(n-2) +21*(2-n)*a(n-3) =0. - \emph{R. J. Mathar}, Dec 10 2011
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=n - 4,\qquad p_{2}(n)=9 - 13 n,\qquad p_{3}(n)=- 21 \left(n - 2\right)
\]
...
\section{The computation for A119975}
...
\[
(n + 1)\,a(n) + (n - 4)\,a(n-1) + (9 - 13 n)\,a(n-2) -  21 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A119975, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A119975.
