...
\title{A proof of the conjectured recurrence for OEIS A257072}
...
\begin{abstract}
OEIS A257072 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $36 x^{2} + 4 x$, a polynomial of degree $2$, which proves
...
OEIS A257072 is ``Number of 3-colored Schroeder paths of semilength n in which there are no (2,0)-steps at level 1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 17, 77, 374, 1959, 11085, 67500,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{8}{- 27 x + \sqrt{9 x^{2} - 10 x + 1} + 7} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 3*n*a(n) +(-53*n+45)*a(n-1) +2*(151*n-213)*a(n-2) +9*(-73*n+144)*a(n-3) +405*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Sep 24 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-09-24, revision 29) the
...
\[
p_{0}(n)=3 n,\qquad p_{1}(n)=45 - 53 n,\qquad p_{2}(n)=2 \left(151 n - 213\right),\qquad p_{3}(n)=- 9 \left(73 n - 144\right),\qquad p_{4}(n)=405 \left(n - 3\right)
\]
...
\section{The computation for A257072}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x - 1\right) \left(9 x - 1\right) .
\]
...
\[
3 n\,a(n) + (45 - 53 n)\,a(n-1) + 2 \left(151 n - 213\right)\,a(n-2) -  9 \left(73 n - 144\right)\,a(n-3) + 405 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A257072, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=36 x^{2} + 4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A257072.
