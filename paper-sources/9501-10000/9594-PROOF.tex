...
\title{A proof of the conjectured recurrence for OEIS A182887}
...
\begin{abstract}
OEIS A182887 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A182887 is ``Number of (1,0)-steps in all weighted lattice paths in L\_n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 3, 7, 21, 60, 166, 463,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{x \left(x + 1\right) \left(x^{2} + x - 1\right)}{\left(\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)\right)^{\frac{3}{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n-1)*(182*n-279)*a(n) + (-230*n\^{}2+11*n+643)*a(n-1) + (-450*n\^{}2+1603*n-315)*a(n-2) + (-498*n\^{}2+971*n+57)*a(n-3) + (-86*n\^{}2+959*n-529)*a(n-4) + (134*n-59)*(n-3)*a(n-5) = 0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 20) the
...
\[
p_{0}(n)=\left(n - 1\right) \left(182 n - 279\right),\qquad p_{1}(n)=- 230 n^{2} + 11 n + 643,\qquad p_{2}(n)=- 450 n^{2} + 1603 n - 315,\qquad p_{3}(n)=- 498 n^{2} + 971 n + 57,\qquad p_{4}(n)=- 86 n^{2} + 959 n - 529,\qquad p_{5}(n)=\left(n - 3\right) \left(134 n - 59\right)
\]
...
\section{The computation for A182887}
...
\[
\left(n - 1\right) \left(182 n - 279\right)\,a(n) + (- 230 n^{2} + 11 n + 643)\,a(n-1) + (- 450 n^{2} + 1603 n - 315)\,a(n-2) + (- 498 n^{2} + 971 n + 57)\,a(n-3) + (- 86 n^{2} + 959 n - 529)\,a(n-4) + \left(n - 3\right) \left(134 n - 59\right)\,a(n-5) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A182887, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A182887.
