...
\begin{quote}\small
\textbf{Conjecture:} +n*(n\^{}2-11*n+22)*a(n) +2*(-4*n\^{}3+45*n\^{}2-113*n+60)*a(n-1) +(15*n\^{}3-173*n\^{}2+530*n-480)*a(n-2) +2*(2*n-5)*(n\^{}2-9*n+12)*a(n-3)=0. - \emph{R. J. Mathar}, Mar 12 2017
\end{quote}
...
\[
p_{0}(n)=n \left(n^{2} - 11 n + 22\right),\qquad p_{1}(n)=- 2 \left(4 n^{3} - 45 n^{2} + 113 n - 60\right),\qquad p_{2}(n)=15 n^{3} - 173 n^{2} + 530 n - 480,\qquad p_{3}(n)=2 \left(2 n - 5\right) \left(n^{2} - 9 n + 12\right)
\]
...
\[
n \left(n^{2} - 11 n + 22\right)\,a(n) -  2 \left(4 n^{3} - 45 n^{2} + 113 n - 60\right)\,a(n-1) + (15 n^{3} - 173 n^{2} + 530 n - 480)\,a(n-2) + 2 \left(2 n - 5\right) \left(n^{2} - 9 n + 12\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
