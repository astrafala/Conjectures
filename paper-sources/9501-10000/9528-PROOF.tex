...
\title{A proof of the conjectured recurrence for OEIS A190171}
...
\begin{abstract}
OEIS A190171 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{5} - 3 x^{4} - 2 x^{3} - 7 x^{2} + 3 x + 2$, a polynomial of degree $5$, which proves
...
OEIS A190171 is ``Number of peakless Motzkin paths of length n having no UHD's starting at level 0; here U=(1,1), H=(1,0), and D=(1,-1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 1, 1, 2, 5, 12, 27,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{x \left(2 x^{2} + x - 1\right) + \sqrt{\left(x \left(x - 3\right) + 1\right) \left(x^{2} + x + 1\right)} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence (n+2)*a(n) +(-n+1)*a(n-1) +2*(-2*n-1)*a(n-2) +9*a(n-3) +(-n+1)*a(n-4) -9*a(n-5) +2*(-2*n+5)*a(n-6) +(-n+1)*a(n-7) +(n-4)*a(n-8)=0. - \emph{R. J. Mathar}, Jul 22 2022
\end{quote}
As of the ``Last modified'' line on the live entry (2022-07-22, revision 10) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=1 - n,\qquad p_{2}(n)=- 2 \left(2 n + 1\right),\qquad p_{3}(n)=9,\qquad p_{4}(n)=1 - n,\qquad p_{5}(n)=-9,\qquad p_{6}(n)=- 2 \left(2 n - 5\right),\qquad p_{7}(n)=1 - n,\qquad p_{8}(n)=n - 4
\]
...
\section{The computation for A190171}
...
\[
B(x)\;=\;x^{5} - 3 x^{4} - 2 x^{3} - 7 x^{2} + 3 x + 2 ,
\]
...
\[
(n + 2)\,a(n) + (1 - n)\,a(n-1) -  2 \left(2 n + 1\right)\,a(n-2) + 9\,a(n-3) + (1 - n)\,a(n-4) - 9\,a(n-5) -  2 \left(2 n - 5\right)\,a(n-6) + (1 - n)\,a(n-7) + (n - 4)\,a(n-8) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A190171, agreeing on all 13 terms compared.
...
vanish, for all 26 values of $n$ from $n=8$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=x^{5} - 3 x^{4} - 2 x^{3} - 7 x^{2} + 3 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A190171.
