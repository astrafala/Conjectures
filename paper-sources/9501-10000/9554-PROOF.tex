...
\title{A proof of the conjectured recurrence for OEIS A104625}
...
\begin{abstract}
OEIS A104625 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 54 x^{4} + 12 x^{3} + 2 x^{2}$, a polynomial of degree $4$, which proves
...
OEIS A104625 is ``Expansion of 1/(sqrt(1-4*x) - x\^{}2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 7, 24, 87, 322, 1211, 4604,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{1}{x^{2} - \sqrt{1 - 4 x}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + (n-3)*a(n-1) + 2*(-28*n+51)*a(n-2) + 72*(2*n-5)*a(n-3) - n*a(n-4) + (-5*n+3)*a(n-5) + 18*(2*n-5)*a(n-6) = 0. - \emph{R. J. Mathar}, Feb 20 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-19, revision 28) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=n - 3,\qquad p_{2}(n)=- 2 \left(28 n - 51\right),\qquad p_{3}(n)=72 \left(2 n - 5\right),\qquad p_{4}(n)=- n,\qquad p_{5}(n)=3 - 5 n,\qquad p_{6}(n)=18 \left(2 n - 5\right)
\]
...
\section{The computation for A104625}
...
\[
n\,a(n) + (n - 3)\,a(n-1) -  2 \left(28 n - 51\right)\,a(n-2) + 72 \left(2 n - 5\right)\,a(n-3) -  n\,a(n-4) + (3 - 5 n)\,a(n-5) + 18 \left(2 n - 5\right)\,a(n-6) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A104625, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 54 x^{4} + 12 x^{3} + 2 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A104625.
