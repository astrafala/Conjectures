...
\title{A proof of the conjectured recurrence for OEIS A141342}
...
\begin{abstract}
OEIS A141342 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{2} - 12 x - 1$, a polynomial of degree $2$, which proves
...
OEIS A141342 is ``A transform of the Fibonacci numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, -1, 3, -13, 65, -353, 2025,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{1}{2 x^{2} - x \sqrt{4 x^{2} + 8 x + 1} + 2 x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n-1)*a(n) +4*(n-4)*a(n-1) + (65-29*n)*a(n-2) +12*(7-2*n)*a(n-3)+ 4*(4-n)*a(n-4) =0. - \emph{R. J. Mathar}, Nov 14 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-22, revision 18) the
...
\[
p_{0}(n)=n - 1,\qquad p_{1}(n)=4 \left(n - 4\right),\qquad p_{2}(n)=65 - 29 n,\qquad p_{3}(n)=- 12 \left(2 n - 7\right),\qquad p_{4}(n)=- 4 \left(n - 4\right)
\]
...
\section{The computation for A141342}
...
\[
(n - 1)\,a(n) + 4 \left(n - 4\right)\,a(n-1) + (65 - 29 n)\,a(n-2) -  12 \left(2 n - 7\right)\,a(n-3) -  4 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A141342, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{2} - 12 x - 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A141342.
