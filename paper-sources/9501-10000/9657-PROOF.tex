...
\title{A proof of the conjectured recurrence for OEIS A128746}
...
\begin{abstract}
OEIS A128746 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A128746 is ``Height of the last peak summed over all skew Dyck paths of semilength n''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 5, 22, 94, 401, 1723, 7475, 32749,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{2 x \left(x + \sqrt{5 x^{2} - 6 x + 1} + 1\right)}{\left(- 3 x + \sqrt{5 x^{2} - 6 x + 1} + 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+2)*(n-1)*a(n) +(6*n\^{}2-3*n+2)*a(n-1) -5*n*(n-2)*a(n-2)=0. - \emph{R. J. Mathar}, Aug 08 2015
\end{quote}
...
\[
p_{0}(n)=- \left(n - 1\right) \left(n + 2\right),\qquad p_{1}(n)=6 n^{2} - 3 n + 2,\qquad p_{2}(n)=- 5 n \left(n - 2\right)
\]
...
\section{The computation for A128746}
...
\[
- \left(n - 1\right) \left(n + 2\right)\,a(n) + (6 n^{2} - 3 n + 2)\,a(n-1) -  5 n \left(n - 2\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A128746, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A128746.
