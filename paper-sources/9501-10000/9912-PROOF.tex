...
\title{A proof of the conjectured closed form for OEIS A267847}
...
\begin{abstract}
OEIS A267847 carries a conjectured closed form for its terms. We prove it. The entry
...
explicit rational function; here it collapses to the polynomial $- 2 x - 5$, of degree
...
OEIS A267847 is ``Decimal representation of the n-th iteration of the "Rule 227" elementary cellular automaton starting with a single ON (black) cell''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 26, 122, 506, 2042, 8186, 32762,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{8 x^{3} + 10 x^{2} - x + 1}{\left(x - 1\right) \left(4 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = 2*(4\^{}n - 3) for n>1. - \emph{Colin Barker}, Nov 26 2016
\end{quote}
...
\section{The computation for A267847}

By Lemma~\ref{lem:gf} the conjectured formula has generating function
\[
F(x)\;=\;\frac{2 \left(11 x - 2\right)}{4 x^{2} - 5 x + 1} ,
\]
...
section of A267847, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x - 5$ is an
...
\url{https://oeis.org}, 2026. Sequence A267847.
