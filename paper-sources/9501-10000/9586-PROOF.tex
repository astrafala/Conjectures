...
\title{The empirical recurrence for OEIS A208086, derived from the entry's generating function}
...
\begin{abstract}
OEIS A208086 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A208086 is ``Number of 4 X (n+1) 0..1 arrays with every 2 X 2 subblock having the same number of equal edges as its horizontal neighbors and a different number from its vertical neighbors, and new values 0..1 introduced in row major order.''. It has offset $1$ and begins
\[
24, 56, 134, 344, 888, 2318, 6056, 15848,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 27 2018 14:17:42, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 14 x^{4} + 34 x^{3} + 16 x^{2} - 24 x}{x^{4} - 3 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;14 x^{4} - 34 x^{3} - 16 x^{2} + 24 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208086.
