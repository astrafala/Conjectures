...
\title{A proof of the conjectured recurrence for OEIS A114584}
...
\begin{abstract}
OEIS A114584 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 12 x^{4} - 4 x^{3} + 6 x + 2$, a polynomial of degree $4$, which proves
...
OEIS A114584 is ``Number of Motzkin paths of length n having no UHD's (U=(1,1), H=(1,0), D=(1,-1))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 3, 7, 15, 36, 85,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{3} - x - \sqrt{\left(x^{3} - 3 x + 1\right) \left(x^{3} + x + 1\right)} + 1}{2 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(n+2)*a(n) +(n+2)*a(n-1) +3*(-3*n+2)*a(n-2) +(-7*n+13)*a(n-3) +(4*n-13)*a(n-4) +6*(-n+5)*a(n-5) +(n-7)*a(n-6) +3*(n-8)*a(n-7)=0. - \emph{R. J. Mathar}, Feb 16 2018
\end{quote}
As of the ``Last modified'' line on the live entry (2024-04-27, revision 41) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=n + 2,\qquad p_{2}(n)=- 3 \left(3 n - 2\right),\qquad p_{3}(n)=13 - 7 n,\qquad p_{4}(n)=4 n - 13,\qquad p_{5}(n)=- 6 \left(n - 5\right),\qquad p_{6}(n)=n - 7,\qquad p_{7}(n)=3 \left(n - 8\right)
\]
...
\section{The computation for A114584}
...
\[
(n + 2)\,a(n) + (n + 2)\,a(n-1) -  3 \left(3 n - 2\right)\,a(n-2) + (13 - 7 n)\,a(n-3) + (4 n - 13)\,a(n-4) -  6 \left(n - 5\right)\,a(n-5) + (n - 7)\,a(n-6) + 3 \left(n - 8\right)\,a(n-7) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A114584, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 12 x^{4} - 4 x^{3} + 6 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A114584.
