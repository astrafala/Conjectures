...
\title{A proof of the conjectured recurrence for OEIS A105865}
...
\begin{abstract}
OEIS A105865 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A105865 is ``Expansion of (1/(1-2*x\^{}2))*c(x/(1-2*x\^{}2)), where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 9, 30, 94, 328, 1165,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{2 x^{2} + 4 x - 1}{2 x^{2} - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2(1-2n)*a(n-1) +4*(1-n)*a(n-2) +4*(2n-3)*a(n-3) +4*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Dec 13 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2023-01-09, revision 19) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(2 n - 1\right),\qquad p_{2}(n)=- 4 \left(n - 1\right),\qquad p_{3}(n)=4 \left(2 n - 3\right),\qquad p_{4}(n)=4 \left(n - 3\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(2 n - 1\right)\,a(n-1) -  4 \left(n - 1\right)\,a(n-2) + 4 \left(2 n - 3\right)\,a(n-3) + 4 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A105865, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A105865.
