...
\title{A proof of the conjectured closed form for OEIS A267802}
...
\begin{abstract}
OEIS A267802 carries a conjectured closed form for its terms. We prove it. The entry
...
explicit rational function; here it collapses to the polynomial $- 8 x - 12$, of degree
$1$, which proves the closed form for every $n>1$. The computation is exact
...
OEIS A267802 is ``Decimal representation of the n-th iteration of the "Rule 213" elementary cellular automaton starting with a single ON (black) cell''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 19, 115, 499, 2035, 8179, 32755,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(2 x + 1\right) \left(16 x^{2} - 4 x + 1\right)}{\left(x - 1\right) \left(4 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = 2\^{}(2*n+1) - 13 for n>1. - \emph{Colin Barker}, Nov 25 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-02-16, revision 21) the
...
section of A267802, agreeing on all 13 terms compared.
...
with the entry's own value for every $n>1$ in range, agreeing in all 23 cases
from $n=2$ upward. This is what Corollary~\ref{cor:criterion} predicts from the
...
cancellation in $K$, not by series truncation, so the identity $B=- 8 x - 12$ is an
...
\url{https://oeis.org}, 2026. Sequence A267802.
