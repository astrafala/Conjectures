...
\title{The empirical recurrence for OEIS A204708, derived from the entry's generating function}
...
\begin{abstract}
OEIS A204708 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A204708 is ``Number of (n+1) X 4 0..1 arrays with the permanents of all 2 X 2 subblocks equal and nonzero.''. It has offset $1$ and begins
\[
25, 81, 257, 833, 2689, 8705, 28161, 91137,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 09 2018 06:49:36, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 36 x^{3} + 6 x^{2} + 25 x}{4 x^{3} - 2 x^{2} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{3} - 2 x^{2} - 3 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204708.
