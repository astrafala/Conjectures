...
\title{A proof of the conjectured recurrence for OEIS A116387}
...
\begin{abstract}
OEIS A116387 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $30 x^{3} - 216 x^{2}$, a polynomial of degree $3$, which proves
...
OEIS A116387 is ``Expansion of 1/(sqrt(1-2*x-3*x\^{}2)*(2-M(x))), where M(x) is the g.f. of the Motzkin numbers A001006''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 7, 22, 72, 234, 763, 2486,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x^{2}}{\sqrt{- 3 x^{2} - 2 x + 1} \left(4 x^{2} + x + \sqrt{- 3 x^{2} - 2 x + 1} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(17*n-142)*a(n) + (17*n\^{}2 + 95*n + 138)*a(n-1) + (-391*n\^{}2 + 2488*n - 2908)*a(n-2) + (-17*n\^{}2 - 603*n + 1892)*a(n-3) + 2*(697*n-2021)*(n-4)*a(n-4) + 60*(17*n-47)*(n-4)*a(n-5) = 0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 27) the
...
\[
p_{0}(n)=n \left(17 n - 142\right),\qquad p_{1}(n)=17 n^{2} + 95 n + 138,\qquad p_{2}(n)=- 391 n^{2} + 2488 n - 2908,\qquad p_{3}(n)=- 17 n^{2} - 603 n + 1892,\qquad p_{4}(n)=2 \left(n - 4\right) \left(697 n - 2021\right),\qquad p_{5}(n)=60 \left(n - 4\right) \left(17 n - 47\right)
\]
...
\section{The computation for A116387}
...
\[
n \left(17 n - 142\right)\,a(n) + (17 n^{2} + 95 n + 138)\,a(n-1) + (- 391 n^{2} + 2488 n - 2908)\,a(n-2) + (- 17 n^{2} - 603 n + 1892)\,a(n-3) + 2 \left(n - 4\right) \left(697 n - 2021\right)\,a(n-4) + 60 \left(n - 4\right) \left(17 n - 47\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A116387, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=30 x^{3} - 216 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A116387.
