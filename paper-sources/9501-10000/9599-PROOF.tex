...
\title{The empirical recurrence for OEIS A223552, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223552 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 21 2018 05:54:54, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 18 x^{2} + 27 x}{16 x^{2} - 41 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;16 x^{2} - 41 x + 1
\]
...
compared against all $14$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223552.
