...
\title{A proof of the conjectured recurrence for OEIS A215973}
...
\begin{abstract}
OEIS A215973 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{3} + 3 x^{2} - 2 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A215973 is ``a(0) = 1, for n > 0: a(n) = Sum\_\{k=0..n-1\} a(k) * (1 + a(n-1-k))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 7, 28, 122, 565, 2735, 13682,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x + \sqrt{- 4 x^{3} + 12 x^{2} - 8 x + 1} - 1}{2 x \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +3*(-3*n+1)*a(n-1) +4*(5*n-7)*a(n-2) +2*(-8*n+19)*a(n-3) +2*(2*n-7)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 38) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 3 \left(3 n - 1\right),\qquad p_{2}(n)=4 \left(5 n - 7\right),\qquad p_{3}(n)=- 2 \left(8 n - 19\right),\qquad p_{4}(n)=2 \left(2 n - 7\right)
\]
...
\[
(n + 1)\,a(n) -  3 \left(3 n - 1\right)\,a(n-1) + 4 \left(5 n - 7\right)\,a(n-2) -  2 \left(8 n - 19\right)\,a(n-3) + 2 \left(2 n - 7\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A215973, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{3} + 3 x^{2} - 2 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A215973.
