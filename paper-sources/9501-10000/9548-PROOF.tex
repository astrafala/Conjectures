...
\title{A proof of the conjectured recurrence for OEIS A162482}
...
\begin{abstract}
OEIS A162482 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 x^{3} - 6 x^{2} + 2$, a polynomial of degree $3$, which proves
...
OEIS A162482 is ``Expansion of (1/(1-x)\textasciicircum{}3)*M(x/(1-x)\textasciicircum{}3), M(x) the g.f. of Motzkin numbers A001006''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 14, 53, 218, 945, 4235, 19441,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x + \sqrt{\frac{- 3 x^{2} - 2 x \left(1 - x\right)^{3} + \left(1 - x\right)^{6}}{\left(x - 1\right)^{6}}} \left(x - 1\right)^{3} - \left(x - 1\right)^{3}}{2 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +4*(-2*n-1)*a(n-1) +18*(n-1)*a(n-2) +13*(-2*n+5)*a(n-3) +17*(n-4)*a(n-4) +3*(-2*n+11)*a(n-5) +(n-7)*a(n-6)=0. - \emph{R. J. Mathar}, Feb 10 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2024-04-04, revision 12) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 4 \left(2 n + 1\right),\qquad p_{2}(n)=18 \left(n - 1\right),\qquad p_{3}(n)=- 13 \left(2 n - 5\right),\qquad p_{4}(n)=17 \left(n - 4\right),\qquad p_{5}(n)=- 3 \left(2 n - 11\right),\qquad p_{6}(n)=n - 7
\]
...
\section{The computation for A162482}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \frac{\left(x^{3} - 3 x^{2} + 2 x - 1\right) \left(x^{3} - 3 x^{2} + 6 x - 1\right)}{\left(x - 1\right)^{6}} .
\]
...
\[
(n + 2)\,a(n) -  4 \left(2 n + 1\right)\,a(n-1) + 18 \left(n - 1\right)\,a(n-2) -  13 \left(2 n - 5\right)\,a(n-3) + 17 \left(n - 4\right)\,a(n-4) -  3 \left(2 n - 11\right)\,a(n-5) + (n - 7)\,a(n-6) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A162482, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=4 x^{3} - 6 x^{2} + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A162482.
