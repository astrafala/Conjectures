...
\title{The empirical recurrence for OEIS A223197, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223197 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A223197 is ``Rolling cube footprints: number of n X 3 0..5 arrays starting with 0 where 0..5 label faces of a cube and every array movement to a horizontal or vertical neighbor moves across a corresponding cube edge.''. It has offset $1$ and begins
\[
16, 576, 20992, 765952, 27951104, 1020002304, 37222350848, 1358333739008,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 16 2018 06:34:53, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 64 x^{2} + 16 x}{128 x^{2} - 40 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;128 x^{2} - 40 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223197.
