...
\title{A proof of the conjectured recurrence for OEIS A225887}
...
\begin{abstract}
OEIS A225887 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $1 - 7 x$, a polynomial of degree $1$, which proves
...
OEIS A225887 is ``a(n) = A212205(2*n + 1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 18, 86, 426, 2162, 11166, 58438,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{- 5 x + \sqrt{x^{2} - 6 x + 1} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +3*(-4*n-1)*a(n-1) +(37*n-20)*a(n-2) +6*(-n+2)*a(n-3)=0. - \emph{R. J. Mathar}, May 23 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-02-09, revision 41) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 3 \left(4 n + 1\right),\qquad p_{2}(n)=37 n - 20,\qquad p_{3}(n)=- 6 \left(n - 2\right)
\]
...
\section{The computation for A225887}
...
\[
(n + 1)\,a(n) -  3 \left(4 n + 1\right)\,a(n-1) + (37 n - 20)\,a(n-2) -  6 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A225887, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=1 - 7 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A225887.
