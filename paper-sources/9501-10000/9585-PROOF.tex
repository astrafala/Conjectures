...
closed form. Here it equals $2 x + 6$, a polynomial of degree $1$, which proves
...
\begin{quote}\small
\textbf{Conjecture:} -(n+2)*(n-3)*a(n) +(n\^{}2-n-10)*a(n-1) +4*(n\^{}2-4*n+5)*a(n-2) -4*(n-2)\^{}2*a(n-3)=0. - \emph{R. J. Mathar}, Feb 05 2015
\end{quote}
...
\[
p_{0}(n)=- \left(n - 3\right) \left(n + 2\right),\qquad p_{1}(n)=n^{2} - n - 10,\qquad p_{2}(n)=4 \left(n^{2} - 4 n + 5\right),\qquad p_{3}(n)=- 4 \left(n - 2\right)^{2}
\]
...
\[
- \left(n - 3\right) \left(n + 2\right)\,a(n) + (n^{2} - n - 10)\,a(n-1) + 4 \left(n^{2} - 4 n + 5\right)\,a(n-2) -  4 \left(n - 2\right)^{2}\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
vanish, for all 32 values of $n$ from $n=3$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=2 x + 6$ is an
