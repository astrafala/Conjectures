...
\title{A proof of the conjectured recurrence for OEIS A119012}
...
\begin{abstract}
OEIS A119012 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A119012 is ``Number of valleys strictly above the x-axis in all hill-free Dyck paths of semilength n (a hill in a Dyck path is a peak at level 1)''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;0, 0, 1, 5, 23, 98, 405, 1644,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{2 \left(- 3 x + \sqrt{1 - 4 x} \left(x - 1\right) + 1\right)}{\sqrt{1 - 4 x} \left(2 x + \sqrt{1 - 4 x} + 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n-3)*(3*n-2)*(n+2)*a(n) +(-21*n\^{}3+50*n\^{}2-13*n-4)*a(n-1) -2*(n-1) *(2*n-1)*(3*n+1)*a(n-2)=0. - \emph{R. J. Mathar}, Jun 22 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 18) the
...
\[
p_{0}(n)=2 \left(n - 3\right) \left(n + 2\right) \left(3 n - 2\right),\qquad p_{1}(n)=- 21 n^{3} + 50 n^{2} - 13 n - 4,\qquad p_{2}(n)=- 2 \left(n - 1\right) \left(2 n - 1\right) \left(3 n + 1\right)
\]
...
\section{The computation for A119012}
...
\[
2 \left(n - 3\right) \left(n + 2\right) \left(3 n - 2\right)\,a(n) + (- 21 n^{3} + 50 n^{2} - 13 n - 4)\,a(n-1) -  2 \left(n - 1\right) \left(2 n - 1\right) \left(3 n + 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A119012, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A119012.
