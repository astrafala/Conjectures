...
\title{The empirical recurrence for OEIS A184044, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184044 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 01 2025 17:05:42, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 312 x^{4} + 164 x^{3} + 156 x^{2} - 81 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;312 x^{4} - 164 x^{3} - 156 x^{2} + 81 x,
\]
