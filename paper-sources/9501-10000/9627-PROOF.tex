...
\title{A proof of the conjectured recurrence for OEIS A278472}
...
\begin{abstract}
OEIS A278472 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2$, a polynomial of degree $0$, which proves
...
OEIS A278472 is ``a(n) = Sum\_\{i=0..n\} Fibonacci(i+1)*binomial(2*n-i+2, n+2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 5, 22, 92, 376, 1518, 6085, 24285,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x - \sqrt{1 - 4 x} + 1}{2 x^{2} \left(x \sqrt{1 - 4 x} - 4 x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(n+2)*(n\^{}2-4*n+1)*a(n) +2*(-4*n\^{}3+9*n\^{}2+16*n-6)*a(n-1) +(15*n\^{}3-38*n\^{}2-9*n+14)*a(n-2) +2*(2*n-1)*(n\^{}2-2*n-2)*a(n-3)=0. - \emph{R. J. Mathar}, Mar 06 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2025-02-23, revision 26) the
...
\[
p_{0}(n)=\left(n + 2\right) \left(n^{2} - 4 n + 1\right),\qquad p_{1}(n)=- 2 \left(4 n^{3} - 9 n^{2} - 16 n + 6\right),\qquad p_{2}(n)=15 n^{3} - 38 n^{2} - 9 n + 14,\qquad p_{3}(n)=2 \left(2 n - 1\right) \left(n^{2} - 2 n - 2\right)
\]
...
\section{The computation for A278472}
...
\[
\left(n + 2\right) \left(n^{2} - 4 n + 1\right)\,a(n) -  2 \left(4 n^{3} - 9 n^{2} - 16 n + 6\right)\,a(n-1) + (15 n^{3} - 38 n^{2} - 9 n + 14)\,a(n-2) + 2 \left(2 n - 1\right) \left(n^{2} - 2 n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A278472, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2$ is an
...
\url{https://oeis.org}, 2026. Sequence A278472.
