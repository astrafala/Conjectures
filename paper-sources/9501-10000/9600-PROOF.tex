...
\title{A proof of the conjectured recurrence for OEIS A191790}
...
\begin{abstract}
OEIS A191790 carries a conjectured linear recurrence with polynomial coefficients, of
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{\left(\sqrt{1 - 4 x^{2}} - 1\right)^{2}}{2 x \left(x^{2} - 1\right) \left(2 x + \sqrt{1 - 4 x^{2}} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(-n-1)*a(n-1) +5*(-n+1)*a(n-2) +(5*n-11)*a(n-3) +2*(2*n-3)*a(n-4) +4*(-n+3)*a(n-5)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-28, revision 18) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- n - 1,\qquad p_{2}(n)=- 5 \left(n - 1\right),\qquad p_{3}(n)=5 n - 11,\qquad p_{4}(n)=2 \left(2 n - 3\right),\qquad p_{5}(n)=- 4 \left(n - 3\right)
\]
...
\section{The computation for A191790}
...
\[
(n + 1)\,a(n) + (- n - 1)\,a(n-1) -  5 \left(n - 1\right)\,a(n-2) + (5 n - 11)\,a(n-3) + 2 \left(2 n - 3\right)\,a(n-4) -  4 \left(n - 3\right)\,a(n-5) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A191790, agreeing on all 13 terms compared.
...
vanish, for all 33 values of $n$ from $n=5$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A191790.
