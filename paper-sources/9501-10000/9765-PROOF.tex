...
\title{A proof of the conjectured closed form for OEIS A267879}
...
\begin{abstract}
OEIS A267879 carries a conjectured closed form for its terms. We prove it. The entry
...
explicit rational function; here it collapses to the polynomial $- \frac{x^{2}}{10} - \frac{x}{100} - \frac{101}{1000}$, of degree
$2$, which proves the closed form for every $n>2$. The computation is exact
...
OEIS A267879 is ``Binary representation of the middle column of the "Rule 233" elementary cellular automaton starting with a single ON (black) cell''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 10, 101, 1010, 10101, 101011, 1010111, 10101111,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{4} - x^{3} + x^{2} - x + 1}{\left(x - 1\right) \left(10 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = (9091*10\^{}(n-3) - 1)/9 for n > 2. - \emph{Stefano Spezia}, Dec 25 2021
\end{quote}
As of the ``Last modified'' line on the live entry (2025-02-16, revision 20) the
...
\[
f(n)\;=\;\frac{9091 \cdot 10^{n - 3}}{9} - \frac{1}{9} .
\]
...
For every $n>2$,
\[
a(n)\;=\;\frac{9091 \cdot 10^{n - 3}}{9} - \frac{1}{9} .
\]
\end{theorem}
...
section of A267879, agreeing on all 13 terms compared.
...
with the entry's own value for every $n>2$ in range, agreeing in all 17 cases
from $n=3$ upward. This is what Corollary~\ref{cor:criterion} predicts from the
...
cancellation in $K$, not by series truncation, so the identity $B=- \frac{x^{2}}{10} - \frac{x}{100} - \frac{101}{1000}$ is an
...
\url{https://oeis.org}, 2026. Sequence A267879.
