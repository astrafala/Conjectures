...
\title{The empirical recurrence for OEIS A218805, proved by transfer matrix}
...
\begin{abstract}
OEIS A218805 counts the distinct indicator arrays obtained from the $n\times3$ arrays over
...
indicator row by row turns it into one, on $S=51$ states, after which the conjectured
...
OEIS A218805 is ``Hilltop maps: number of n X 3 binary arrays indicating the locations of corresponding elements not exceeded by any horizontal or antidiagonal neighbor in a random 0..3 n X 3 array.''. It has offset $1$ and begins
\[
7, 61, 481, 3843, 30719, 245509, 1962145, 15681787,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 7*a(n-1) + 7*a(n-2) + 7*a(n-3) + 3*a(n-4) + 3*a(n-5) + 3*a(n-6) + 3*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 24 2018 14:38:58, revision 8) this is still
...
\]
There are $S=51$ reachable sets, generated from $D_0$ outward, and $a$ is $C$-finite. Note
...
\begin{theorem}
\[
a(n)\;=\;7\,a(n-1) + 7\,a(n-2) + 7\,a(n-3) + 3\,a(n-4) + 3\,a(n-5) + 3\,a(n-6) + 3\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A218805.
