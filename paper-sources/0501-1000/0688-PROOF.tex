...
\title{The empirical recurrence for OEIS A223361, proved by transfer matrix}
...
\begin{abstract}
OEIS A223361 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$6$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is octahedron graph. The entry carries an empirical recurrence of order $9$
contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical. A slice of
the array would be a state, but there are $46656$ legal slices here and that is too many to
...
then be run on the ORBITS of slices instead of on the slices, and $S=1248$ of those suffice.
\end{abstract}
...
OEIS A223361 is ``Rolling cube footprints: number of 6Xn 0..5 arrays starting with 0 where 0..5 label faces of a cube and every array movement to a horizontal or antidiagonal neighbor moves across a corresponding cube edge.''. It has offset $1$ and begins
\[
7776, 4194304, 4076863488, 3962711310336, 4060947412942848, 4204783428144463872, 4387933871654786039808, 4591848540270748206366720,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 21:33:54, revision 8) this is still
...
There are $46656$ legal $column$s, which is more than can be carried one at a time. They
need not be.
...
\]
and the count the entry publishes is that divided by $6$, by Lemma~\ref{lem:fibre}. Here $S=|\mathcal{O}|=1248$, down from $46656$.
...
$\widehat{M}^{j+9}-\sum_ic_i\widehat{M}^{j+9-i}$. For the bound,
...
\begin{theorem}
\[
a(n)\;=\;1920\,a(n-1) - 1159168\,a(n-2) + 272105472\,a(n-3) - 9613344768\,a(n-4) - 7199438929920\,a(n-5) + 1446957302153216\,a(n-6) - 121544413380870144\,a(n-7) + 4845873199050653696\,a(n-8) - 74363437047141629952\,a(n-9)
\]
for every $n>15$.
\end{theorem}

\begin{proof}
The vector $w=q(\widehat{M})\mathbf{1}$ was formed by $9$ matrix--vector products in
...
$n=15+1$ onwards and the run continued until the working vector was identically zero, which
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223361.
