...
\title{The empirical recurrence for OEIS A230929, proved by transfer matrix}
...
\begin{abstract}
OEIS A230929 counts the black-square subarrays of arrays of width $4$ over $\{0,1,2,3\}$ in which
every cell has a diagonal neighbour one greater than it modulo 4, and none equal to it. The entry carries an empirical recurrence of order $3$ contributed by
...
OEIS A230929 is ``Number of black-square subarrays of (n+2) X (2+2) 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
2, 6, 16, 48, 146, 438, 1312, 3936,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) - a(n-2) + 3*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 17 2018 07:10:17, revision 8) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=157.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230929.
