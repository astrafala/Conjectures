...
\title{The empirical recurrence for OEIS A221270, proved by transfer matrix}
...
\begin{abstract}
OEIS A221270 counts the distinct OCCUPANCY arrays obtained when every cell of an $n\times2$ grid
...
determinising the machine that emits the occupancy row by row turns it into one, on $S=39$
...
OEIS A221270 is ``Number of n X 2 arrays of occupancy after each element moves to some king-move neighbor, with every occupancy equal to zero or two.''. It has offset $1$ and begins
\[
0, 6, 16, 52, 174, 588, 1984, 6688,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) - 10*a(n-2) + 4*a(n-3) - 2*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 14 2018 11:20:22, revision 10) this is still
...
\]
There are $S=39$ reachable sets. In particular $a$ is $C$-finite.
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) - 10\,a(n-2) + 4\,a(n-3) - 2\,a(n-5)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221270.
