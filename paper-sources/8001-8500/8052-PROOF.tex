...
\title{The empirical recurrence for OEIS A197244, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A197244 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A197244 is ``Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 0,0,1,1,1 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
2, 4, 8, 16, 40, 80, 192, 400,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-2) +2*a(n-3) +a(n-4) -2*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (May 11 2018 15:25:30, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-2) + 2\,a(n-3) + a(n-4) - 2\,a(n-5)
\end{equation}
for all $n>6$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(0,0,1,1,1)
\end{equation}
for every cell $(t,u)$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A197244.
