...
\title{The empirical recurrence for OEIS A204707, derived from the entry's generating function}
...
\begin{abstract}
OEIS A204707 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A204707 is ``Number of (n+1) X 3 0..1 arrays with the permanents of all 2 X 2 subblocks equal and nonzero.''. It has offset $1$ and begins
\[
13, 33, 81, 209, 529, 1361, 3473, 8913,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 09 2018 06:49:39, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 24 x^{3} + 7 x^{2} + 13 x}{4 x^{3} - 3 x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{3} - 3 x^{2} - 2 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204707.
