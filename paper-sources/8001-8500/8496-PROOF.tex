...
\title{The empirical recurrence for OEIS A233107, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233107 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A233107 is ``Number of n X 2 0..5 arrays with no element x(i,j) adjacent to value 5-x(i,j) horizontally or vertically, top left element zero, and 1 appearing before 2 3 and 4, and 2 appearing before 3 in row major order.''. It has offset $1$ and begins
\[
2, 19, 313, 6046, 123352, 2565169, 53692063, 1126297996,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 19 2018 09:30:52, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 112 x^{3} + 39 x^{2} - 2 x}{147 x^{3} - 175 x^{2} + 29 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 147 x^{3} + 175 x^{2} - 29 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;29\,a(n-1) - 175\,a(n-2) + 147\,a(n-3)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233107.
