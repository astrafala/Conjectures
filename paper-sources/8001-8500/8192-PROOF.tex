...
\title{The empirical recurrence for OEIS A236992, proved by transfer matrix}
...
\begin{abstract}
OEIS A236992 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=1600$
...
OEIS A236992 is ``Number of (n+1) X (3+1) 0..2 arrays with the maximum plus the lower median minus the upper median minus the minimum of every 2 X 2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$ and begins
\[
1600, 15552, 149632, 1513728, 14963200, 145732608, 1406076928, 14212288512,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 10*a(n-1) + 8704*a(n-4) - 87040*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 21 2018 11:10:56, revision 7) this is still
...
differ by $1$. The entry's first term $1600$ counts those, and the model was made to
...
\]
over the $S=1600$ pairs whose own subblock row already satisfies the condition along itself.
...
\begin{theorem}
\[
a(n)\;=\;10\,a(n-1) + 8704\,a(n-4) - 87040\,a(n-5)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A236992.
