...
\title{A proof of the conjectured recurrence for OEIS A071684}
...
\begin{abstract}
OEIS A071684 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $6 x^{2} - 54 x$, a polynomial of degree $2$, which proves
...
OEIS A071684 is ``Number of plane trees with n edges and having an odd number of leaves''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 2, 7, 22, 66, 212, 715,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{- 2 x - \sqrt{1 - 4 x} + \sqrt{4 x^{2} + 1}}{4 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n+1)*(10*n-37)*a(n) + 2*n*(5*n\^{}2-42*n+91)*a(n-1) + 4*(-40*n\^{}3+270*n\^{}2-560*n+357)*a(n-2) + 8*(n-3)*(5*n\^{}2-42*n+91)*a(n-3) - 16*(n-4)*(25*n-51)*(2*n-7)*a(n-4) = 0. - \emph{R. J. Mathar}, Jul 05 2018
\end{quote}
As of the ``Last modified'' line on the live entry (2025-12-19, revision 45) the
...
\[
p_{0}(n)=n \left(n + 1\right) \left(10 n - 37\right),\qquad p_{1}(n)=2 n \left(5 n^{2} - 42 n + 91\right),\qquad p_{2}(n)=- 4 \left(40 n^{3} - 270 n^{2} + 560 n - 357\right),\qquad p_{3}(n)=8 \left(n - 3\right) \left(5 n^{2} - 42 n + 91\right),\qquad p_{4}(n)=- 16 \left(n - 4\right) \left(2 n - 7\right) \left(25 n - 51\right)
\]
...
\[
n \left(n + 1\right) \left(10 n - 37\right)\,a(n) + 2 n \left(5 n^{2} - 42 n + 91\right)\,a(n-1) -  4 \left(40 n^{3} - 270 n^{2} + 560 n - 357\right)\,a(n-2) + 8 \left(n - 3\right) \left(5 n^{2} - 42 n + 91\right)\,a(n-3) -  16 \left(n - 4\right) \left(2 n - 7\right) \left(25 n - 51\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A071684, agreeing on all 13 terms compared.
...
vanish, for all 24 values of $n$ from $n=5$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=6 x^{2} - 54 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A071684.
