...
\title{A condition on the multiset of every subblock: OEIS A235850}
...
\begin{abstract}
OEIS A235850 carries an empirical recurrence of order $5$ contributed by R.~H.~Hardin. It is
...
on $S=6$ vertices and is $C$-finite, and whether the conjectured recurrence annihilates it
...
OEIS A235850 is ``Number of (n+1) X (1+1) 0..3 arrays with the upper median equal to the lower median in every 2 X 2 subblock.''. It has offset $1$ and begins
\[
100, 688, 4524, 30624, 203804, 1370656, 9159084, 61448672,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + 37*a(n-2) - 10*a(n-3) - 200*a(n-4) + 128*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 19 2018 08:08:25, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $2$ columns over the alphabet $\{0,1,2,3\}$, growing one row at a time, and asks that in every $2\times2$ subblock the two middle order statistics are equal: writing the $4$ entries of the block in nondecreasing order $v_1\le\cdots\le v_{4}$, the lower median is $v_{2}$ and the upper median $v_{3}$, and these must be equal.
...
Merging vertices with identical futures leaves $S=6$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
a(n)\;=\;\iota^{\!\top}M^{\,n}\tau ,
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 37\,a(n-2) - 10\,a(n-3) - 200\,a(n-4) + 128\,a(n-5)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235850.
