...
\title{The empirical recurrence for OEIS A230509, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A230509 counts arrays over $\{0,\dots,7\}$ in which every cell is constrained by the
...
OEIS A230509 is ``Number of n X 2 0..7 arrays x(i,j) with each element horizontally or vertically next to at least one element with value (x(i,j)+1) mod 8, and upper left element zero.''. It has offset $1$ and begins
\[
0, 0, 0, 2, 8, 32, 118, 440,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) - 4*a(n-2) - 5*a(n-3) + 9*a(n-4) + 12*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 19 2025 16:09:46, revision 12) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 4\,a(n-2) - 5\,a(n-3) + 9\,a(n-4) + 12\,a(n-5)
\end{equation}
for all $n>6$.
...
\exists\ \text{a neighbour with value }(x_{t,u}+1)\bmod 8
\end{equation}
...
Let $V=\{0,\dots,7\}^{2}$ be the possible rows and take
...
and $2$ cells in a row, so $|V|=8^{2}=64$, and the vertex set is
...
$8$ values, so there are about $10^{52}$ of them to sift. The
...
of the $(7+1)^{2}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230509.
