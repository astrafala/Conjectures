...
\title{The empirical recurrence for OEIS A235282, proved by transfer matrix}
...
\begin{abstract}
OEIS A235282 counts $2$-column arrays over $\{0,\dots,3\}$ in which every
$2\times2$ subblock has its diagonal sum differing from its antidiagonal sum by
exactly $4$, and it
carries an empirical recurrence of order $3$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=16$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-3}q(M)\mathbf{1}$, so the recurrence
...
OEIS A235282 is ``Number of (n+1) X (1+1) 0..3 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
20, 40, 68, 136, 236, 472, 836,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) + 3*a(n-2) - 6*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 18 2022 23:37:48, revision 10) the statement is
...
\end{equation}
for all $n>3$, which is every $n$ at which a recurrence of order $3$ can be
stated.
...
Let $V$ consist of all of $\{0,\dots,3\}^{2}$, and put $S=|V|=16$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge3$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-3}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-3$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-3}\bigl(M^{3}-\sum_i c_i M^{3-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-3$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $3$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=3-3+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>3$.
\end{proof}
...
the $30$ published indices; the values agree exactly. That is what ties the digraph to
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235282.
