...
\title{The empirical recurrence for OEIS A184031, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184031 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A184031 is ``1/16 the number of (n+1) X 2 0..3 arrays with all 2 X 2 subblocks having the same four values.''. It has offset $1$ and begins
\[
16, 28, 49, 91, 169, 325, 625, 1225,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 05:26:59, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 40 x^{4} + 35 x^{3} + 20 x^{2} - 16 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;40 x^{4} - 35 x^{3} - 20 x^{2} + 16 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184031.
