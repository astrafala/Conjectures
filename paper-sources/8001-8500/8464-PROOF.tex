...
\title{A proof of the conjectured recurrence for OEIS A243760}
...
\begin{abstract}
OEIS A243760 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $- 12288 x^{5} + 24576 x^{4} - 8112 x^{3} + 630 x^{2}$, a polynomial of degree $5$, which proves
...
OEIS A243760 is ``Expansion of (sqrt(8*x+4*sqrt(1-4*x)-3)-1)/(2*sqrt(1-4*x)-2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 1, 4, 11, 40, 138, 512,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{1 - 2 \left(1 - \sqrt{1 - 4 x}\right)^{2}} - 1}{2 \left(\sqrt{1 - 4 x} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: 245*n*(n-1)*(n+1)*a(n) -140*n*(n-1)*(28*n-53)*a(n-1) +4*(n-1)*(5812*n\^{}2-29864*n+36585)*a(n-2) +16*(-3368*n\^{}3+35040*n\^{}2-111784*n+110667)*a(n-3) +128*(-152*n\^{}3-1224*n\^{}2+14756*n-29655)*a(n-4) +2048*(2*n-9)*(74*n\^{}2-567*n+988)*a(n-5) -98304*(n-6)*(2*n-9)*(2*n-11)*a(n-6)=0. - \emph{R. J. Mathar}, Jun 07 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2024-11-09, revision 23) the
...
\[
p_{0}(n)=245 n \left(n - 1\right) \left(n + 1\right),\qquad p_{1}(n)=- 140 n \left(n - 1\right) \left(28 n - 53\right),\qquad p_{2}(n)=4 \left(n - 1\right) \left(5812 n^{2} - 29864 n + 36585\right),\qquad p_{3}(n)=- 16 \left(3368 n^{3} - 35040 n^{2} + 111784 n - 110667\right),\qquad p_{4}(n)=- 128 \left(152 n^{3} + 1224 n^{2} - 14756 n + 29655\right),\qquad p_{5}(n)=2048 \left(2 n - 9\right) \left(74 n^{2} - 567 n + 988\right),\qquad p_{6}(n)=- 98304 \left(n - 6\right) \left(2 n - 11\right) \left(2 n - 9\right)
\]
...
\[
B(x)\;=\;- 12288 x^{5} + 24576 x^{4} - 8112 x^{3} + 630 x^{2} ,
\]
...
\[
245 n \left(n - 1\right) \left(n + 1\right)\,a(n) -  140 n \left(n - 1\right) \left(28 n - 53\right)\,a(n-1) + 4 \left(n - 1\right) \left(5812 n^{2} - 29864 n + 36585\right)\,a(n-2) -  16 \left(3368 n^{3} - 35040 n^{2} + 111784 n - 110667\right)\,a(n-3) -  128 \left(152 n^{3} + 1224 n^{2} - 14756 n + 29655\right)\,a(n-4) + 2048 \left(2 n - 9\right) \left(74 n^{2} - 567 n + 988\right)\,a(n-5) -  98304 \left(n - 6\right) \left(2 n - 11\right) \left(2 n - 9\right)\,a(n-6) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A243760, agreeing on all 13 terms compared.
...
vanish, for all 22 values of $n$ from $n=6$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 12288 x^{5} + 24576 x^{4} - 8112 x^{3} + 630 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A243760.
