...
\title{The empirical recurrence for OEIS A250779, derived from the entry's generating function}
...
\begin{abstract}
OEIS A250779 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A250779 is ``Number of (n+1) X (4+1) 0..1 arrays with nondecreasing x(i,j)+x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
99, 238, 534, 1152, 2426, 5028, 10306, 20960,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 19 2018 09:14:03, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 73 x^{5} + 304 x^{4} - 492 x^{3} + 356 x^{2} - 99 x}{2 x^{5} - 9 x^{4} + 16 x^{3} - 14 x^{2} + 6 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 2 x^{5} + 9 x^{4} - 16 x^{3} + 14 x^{2} - 6 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;73 x^{5} - 304 x^{4} + 492 x^{3} - 356 x^{2} + 99 x,
\]
...
\[
2 x^{5} - 9 x^{4} + 16 x^{3} - 14 x^{2} + 6 x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250779.
