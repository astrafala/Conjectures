...
\title{The empirical recurrence for OEIS A267960, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267960 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A267960 is ``Number of n X 1 0..2 arrays with every repeated value in every row and column greater than the previous repeated value.''. It has offset $1$ and begins
\[
3, 9, 24, 63, 159, 394, 957, 2292,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 25 2018 15:14:44, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 17 x^{6} - 3 x^{5} + 24 x^{4} - 3 x^{3} - 9 x^{2} + 3 x}{16 x^{6} - 24 x^{4} + 8 x^{3} + 9 x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;16 x^{6} - 24 x^{4} + 8 x^{3} + 9 x^{2} - 6 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) - 9\,a(n-2) - 8\,a(n-3) + 24\,a(n-4) - 16\,a(n-6)
\]
for every $n>6$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 17 x^{6} - 3 x^{5} + 24 x^{4} - 3 x^{3} - 9 x^{2} + 3 x,
\]
...
\[
16 x^{6} - 24 x^{4} + 8 x^{3} + 9 x^{2} - 6 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267960.
