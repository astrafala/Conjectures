...
\title{A condition on each element and its neighbours: OEIS A241133}
...
\begin{abstract}
OEIS A241133 carries an empirical recurrence of order $5$ contributed by R.~H.~Hardin. It is
...
OEIS A241133 is ``Number of n X 2 0..2 arrays with no element equal to the same number of vertical neighbors as horizontal neighbors, with new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
1, 2, 5, 17, 56, 195, 691, 2476,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) + 2*a(n-2) - 11*a(n-3) - 6*a(n-4) + 8*a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 29 2018 20:15:58, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $2$ columns over the alphabet $\{0,1,2\}$, growing one row at a time. Its condition is that every element must equal a different number of its vertical neighbours than it does of its horizontal neighbours.
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) + 2\,a(n-2) - 11\,a(n-3) - 6\,a(n-4) + 8\,a(n-5)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A241133.
