...
\title{The empirical recurrence for OEIS A278015, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A278015 counts arrays over $\{0,\dots,2\}$ in which every cell is constrained by the
...
OEIS A278015 is ``Number of 2 X n 0..2 arrays with every element plus 1 mod 3 equal to some element at offset (-1,-1) (-1,0) (-1,1) (0,-1) (0,1) or (1,0), with upper left element zero.''. It has offset $1$ and begins
\[
0, 6, 28, 168, 960, 5530, 31808, 183000,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) - 8*a(n-3) - a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 07 2019 08:41:31, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) - 8\,a(n-3) - a(n-4)
\end{equation}
for all $n>5$.
...
Let $V=\{0,\dots,2\}^{2}$ be the possible columns and take
...
$3$ values, so there are about $10^{20}$ of them to sift. The
walk count replaces that by $81$ states and one matrix--vector product per column.
...
of the $(2+1)^{2}$ possible next columns, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A278015.
