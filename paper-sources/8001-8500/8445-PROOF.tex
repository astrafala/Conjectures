...
\title{The empirical recurrence for OEIS A222336, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A222336 counts arrays over $\{0,\dots,6\}$ in which every cell is constrained by the
...
with $S=343$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A222336 is ``Number of n X 3 0..6 arrays with no entry increasing mod 7 by 6 rightwards or downwards, starting with upper left zero.''. It has offset $1$ and begins
\[
36, 5766, 923526, 147918906, 23691810366, 3794659477146, 607781352505806, 97346856728146986,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 165*a(n-1) - 774*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 15 2018 06:45:15, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;165\,a(n-1) - 774\,a(n-2)
\end{equation}
for all $n>2$.
...
Let $V=\{0,\dots,6\}^{3}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=343$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
publishes, $n=12$, the arrays have $3\cdot12$ cells over
$7$ values, so there are about $10^{30}$ of them to sift. The
walk count replaces that by $343$ states and one matrix--vector product per row.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=343$ pairs and each
of the $(6+1)^{3}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222336.
