...
\title{The empirical recurrence for OEIS A184368, proved by transfer matrix}
...
\begin{abstract}
OEIS A184368 counts $(n+1)\times2$ arrays over $\{0,\dots,2\}$ subject to a condition imposed on
...
on $S=9$ vertices whose edges are the admissible consecutive pairs, and the count is a
...
OEIS A184368 is ``1/3 the number of (n+1) X 2 0..2 arrays with every 2 X 2 subblock having at least two equal elements connected horizontally or vertically.''. It has offset $1$ and begins
\[
21, 153, 1107, 8019, 58077, 420633, 3046491, 22064643,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) + 9*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 27 2018 08:44:08, revision 8) the statement is
...
$V=\{0,1,\dots,2\}^{2}$, so $|V|=S=9$. A $2\times2$ subblock of the array
...
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\tfrac1{3}\mathbf{1}^{\!\top}M^{\,n+1-1-2}\,q(M)\,\mathbf{1} .
\]
...
=\tfrac1{3}\mathbf{1}^{\!\top}M^{\,m-2}\,q(M)\,\mathbf{1}
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184368.
