...
\title{The empirical recurrence for OEIS A223299, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A223299 counts arrays over $\{0,\dots,9\}$ in which every cell is constrained by the
...
with $S=10000$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A223299 is ``4 X 4 X 4 triangular graph coloring a rectangular array: number of n X 2 0..9 arrays where 0..9 label nodes of a graph with edges 0,1 0,2 1,2 1,3 1,4 2,4 3,4 2,5 4,5 3,6 3,7 4,7 6,7 4,8 5,8 7,8 5,9 8,9 and every array movement to a horizontal or vertical neighbor moves along an edge of this graph.''. It has offset $1$ and begins
\[
36, 324, 3132, 30564, 298620, 2918052, 28515132, 278649828,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 11*a(n-1) - 12*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 19 2018 07:36:31, revision 8) the statement is
...
row $t$ and column $u$, each in $\{0,\dots,9\}$. With the neighbour offsets
\[
\mathcal N=\{(0,1),\ (1,0)\},
\]
...
\{x_{t,u},x_{t+d_1,u+d_2}\}\in E\ \text{ for every offset in }\mathcal N,\quad E=\{\{0,1\},\{0,2\},\{1,2\},\{1,3\},\{1,4\},\{2,4\},\{2,5\},\{3,4\},\{3,6\},\{3,7\},\{4,5\},\{4,7\},\{4,8\},\{5,8\},\{5,9\},\{6,7\},\{7,8\},\{8,9\}\}
\end{equation}
for every cell $(t,u)$.
...
Let $V=\{0,\dots,9\}^{2}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=10000$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=10000$ pairs and each
of the $(9+1)^{2}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223299.
