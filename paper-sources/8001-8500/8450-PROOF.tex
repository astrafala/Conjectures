...
\title{The empirical recurrence for OEIS A204609, derived from the entry's generating function}
...
\begin{abstract}
OEIS A204609 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A204609 is ``Number of (n+1) X 2 0..1 arrays with the determinants of 2 X 2 subblocks nondecreasing rightwards and downwards.''. It has offset $1$ and begins
\[
16, 47, 125, 335, 907, 2470, 6740, 18406,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 20 2018 07:38:48, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n-1}\;=\;\frac{- x^{5} - 4 x^{4} - 8 x^{3} - 16 x^{2} - x + 16}{2 x^{3} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{3} - 3 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{5} - 4 x^{4} - 8 x^{3} - 16 x^{2} - x + 16,
\]
...
they stop at index $6$ because that polynomial has degree $5$. A reader who trusts
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204609.
