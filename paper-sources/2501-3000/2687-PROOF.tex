...
\title{The conjectured generating function for OEIS A250819, proved}
...
\begin{abstract}
OEIS A250819 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $6573$ coefficients in exact arithmetic
...
OEIS A250819 is ``Number of (7+1) X (n+1) 0..2 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
41112, 131455, 315561, 641571, 1167796, 1962717,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 21 2018 08:10:18, revision 7) this is still
...
\]
a polynomial of degree at most $6566$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{6566}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $6573$
of them --- more than the $6566+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250819.
