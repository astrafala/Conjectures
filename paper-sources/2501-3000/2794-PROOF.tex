...
\title{The empirical recurrence for OEIS A189436, proved by transfer matrix}
...
\begin{abstract}
OEIS A189436 counts the permutations of the cells of an $n\times4$ array in which every cell
...
That makes $a$ a walk count on $S=14$ states, hence $C$-finite, and whether the published
...
OEIS A189436 is ``Number of 4 X n array permutations with each element not moving, or moving one space N, SW or SE.''. It has offset $1$ and begins
\[
1, 9, 31, 140, 571, 2413, 10069, 42205,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) +2*a(n-2) -6*a(n-3) +4*a(n-4) -a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (May 02 2018 08:06:08, revision 8) this is still
...
cell by $\delta_14+\delta_2$ places in that order, so every image lies between $lo=-3$ and
...
$k+-3,\dots,k+5$ are taken --- a subset of a set of size $9$, and nothing else.
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) + 2\,a(n-2) - 6\,a(n-3) + 4\,a(n-4) - a(n-5)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A189436.
