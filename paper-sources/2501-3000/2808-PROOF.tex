...
\title{The empirical column recurrences for the table OEIS A231451}
...
\begin{abstract}
OEIS A231451 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231451 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with no element unequal to a strict majority of its horizontal and antidiagonal neighbors, with values 0..3 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 3, 6, 4, 15, 23, 7, 32,\ \dots
\]
...
k=5: [order 65]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:30:55, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..3 arrays with no element unequal to a strict majority of its horizontal and antidiagonal neighbors, with values 0..3 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=130$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;11\,a(n-1) - 41\,a(n-2) + 65\,a(n-3) - 43\,a(n-4) + 9\,a(n-5)
\]
holds from index $4+1$ on.

\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) 0..3 arrays with no element unequal to a strict majority of its horizontal and antidiagonal neighbors, with values 0..3 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=100$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $17$ published values, $40$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231451.
