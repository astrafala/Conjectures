...
order 5. The entry gives no generating function, but it does give the sequence as
...
\begin{quote}\small
\textbf{Conjecture:} +(n+1)*a(n) -n*(2*n+5)*a(n-1) +(n-1)*(n\textasciicircum{}2+6*n+3)*a(n-2) -(n-2)*(3*n\textasciicircum{}2-2)*a(n-3) +(n-2)*(n-3)*(3*n-4)*a(n-4) -(n-4)*(n-3)\textasciicircum{}2*a(n-5) = 0. - \emph{R. J. Mathar}, Feb 28 2015
\end{quote}
...
\[
Q\;=\;\left(\frac{n^{5} + 10 n^{4} + 36 n^{3} + 52 n^{2} + 15 n - 18}{n^{2} + 4 n + 2}\right) + \left(\frac{- 2 n^{5} - 32 n^{4} - 197 n^{3} - 573 n^{2} - 760 n - 336}{n^{2} + 6 n + 7}\right)\,N + \left(n^{3} + 14 n^{2} + 65 n + 100\right)\,N^{2} .
\]
...
\[
(n + 1)\,a(n) -  n \left(2 n + 5\right)\,a(n-1) + \left(n - 1\right) \left(n^{2} + 6 n + 3\right)\,a(n-2) -  \left(n - 2\right) \left(3 n^{2} - 2\right)\,a(n-3) + \left(n - 3\right) \left(n - 2\right) \left(3 n - 4\right)\,a(n-4) -  \left(n - 4\right) \left(n - 3\right)^{2}\,a(n-5) \;=\;0
\]
...
all 18 values of $n$ from $n=5$ upward. The formula \eqref{eq:sum} was
