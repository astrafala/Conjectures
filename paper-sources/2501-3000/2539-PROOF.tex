...
\title{The empirical column recurrences for the table OEIS A264550}
...
\begin{abstract}
OEIS A264550 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264550 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change -1,1 -1,2 1,0 or 0,-1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 1, 1, 1, 2, 0, 1, 6,\ \dots
\]
...
k=5: [order 84]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 17 2015 15:37:08, revision 4) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change -1,1 -1,2 1,0 or 0,-1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having directed index change -1,1 -1,2 1,0 or 0,-1.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having directed index change -1,1 -1,2 1,0 or 0,-1.''. Its rows are the vertices of a digraph on
...
$c_{1}$ & $2$ & $c_{5}$ & $8$ & $c_{9}$ & $33$ & $c_{13}$ & $-2$ \\
$c_{2}$ & $0$ & $c_{6}$ & $-25$ & $c_{10}$ & $4$ & $c_{14}$ & $0$ \\
$c_{3}$ & $10$ & $c_{7}$ & $0$ & $c_{11}$ & $-4$ & $c_{15}$ & $1$ \\
$c_{4}$ & $-4$ & $c_{8}$ & $-4$ & $c_{12}$ & $-10$ &  &  \\
...
model reproduces $30$ published values, $39$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264550.
