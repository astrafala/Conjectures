...
\title{The empirical column recurrences for the table OEIS A231219}
...
\begin{abstract}
OEIS A231219 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231219 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with no element unequal to a strict majority of its horizontal and vertical neighbors, with values 0..2 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
3, 4, 4, 7, 9, 7, 12, 22,\ \dots
\]
...
k=4: [order 68]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:18:51, revision 6) these are still
...
column $k$$=1$ & $3$ & $36$ & $>3$ & $10$ \\
column $k$$=2$ & $4$ & $112$ & $>3$ & $9$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with no element unequal to a strict majority of its horizontal and vertical neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays with no element unequal to a strict majority of its horizontal and vertical neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=112$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231219.
