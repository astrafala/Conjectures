...
\title{The empirical column recurrences for the table OEIS A203732}
...
\begin{abstract}
OEIS A203732 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A203732 is ``T(n,k)=Number of (n+1)X(k+1) 0..7 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. It has offset $1$, is
read by antidiagonals, and begins
\[
960, 14400, 14400, 216000, 539456, 216000, 3240000, 19071424,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 64*15\^{}n\\
k=2: a(n) = 29*a(n-1) +238*a(n-2)\\
k=3: a(n) = 99*a(n-1) -609*a(n-2) -588*a(n-3)\\
k=4: a(n) = 141*a(n-1) +34391*a(n-2) -1914087*a(n-3) -5360404*a(n-4) +185626112*a(n-5) +226020536*a(n-6) -801895584*a(n-7)\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:41:26, revision 9) these are still
...
The sequence is ``Number of (n+1)X(3) 0..7 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. Its rows are the vertices of a digraph on
$S=512$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+1)X(4) 0..7 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. Its rows are the vertices of a digraph on
$S=4096$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;99\,a(n-1) - 609\,a(n-2) - 588\,a(n-3)
\]
holds for every $n>2$.
...
different counts. Across the columns settled here the model reproduces $11$ published
values, $105$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203732.
