...
\title{The conjectured generating function for OEIS A228278, proved}
...
\begin{abstract}
OEIS A228278 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A228278 is ``Number of n X 3 binary arrays with top left value 1 and no two ones adjacent horizontally, vertically or nw-se diagonally.''. It has offset $1$ and begins
\[
2, 3, 13, 35, 112, 337,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(2 + x - x\^{}3) / ((1 + x)*(1 - 2*x - 3*x\^{}2 - x\^{}3 + x\^{}4)). - \_Colin Barker\_, Mar 16 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 16 2018 12:44:02, revision 14) this is still
...
numerator. Its coefficients are the integers $c_1,\dots,c_{5}$ read off $D_2$, the first few being $1,\ 5,\ 4,\ 0,\ -1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A228278.
