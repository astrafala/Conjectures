...
\title{The empirical recurrence for OEIS A230920, proved by transfer matrix}
...
\begin{abstract}
OEIS A230920 counts arrays of width $2$ over $\{0,1,2\}$ in which every cell must see, among its
...
OEIS A230920 is ``Number of n X 2 0..2 arrays x(i,j) with each element horizontally, vertically, diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 3 and at least one element with value (x(i,j)-1) mod 3, and upper left element zero.''. It has offset $1$ and begins
\[
0, 12, 54, 432, 3108, 22428, 162504, 1175592,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-1) + 14*a(n-2) + 16*a(n-3) - 2*a(n-4) + 4*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 23 2018 09:07:00, revision 8) this is still
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-1) + 14\,a(n-2) + 16\,a(n-3) - 2\,a(n-4) + 4\,a(n-5)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230920.
