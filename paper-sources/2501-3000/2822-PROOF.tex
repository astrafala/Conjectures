...
\title{The conjectured generating function for OEIS A239844, proved}
...
\begin{abstract}
OEIS A239844 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A239844 is ``Number of n X 2 0..3 arrays with no element equal to one plus the sum of elements to its left or one plus the sum of elements above it or two plus the sum of the elements diagonally to its northwest, modulo 4.''. It has offset $1$ and begins
\[
3, 8, 17, 35, 64, 109,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(1 - x + x\^{}2)*(3 - 4*x + 4*x\^{}3 - 2*x\^{}4) / (1 - x)\^{}5.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:26, revision 10) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A239844.
