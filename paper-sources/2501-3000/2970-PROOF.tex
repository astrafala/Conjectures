...
\title{The conjectured generating function for OEIS A250661, proved}
...
\begin{abstract}
OEIS A250661 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $264$ coefficients in exact arithmetic
...
OEIS A250661 is ``Number of (7+1) X (n+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
639, 1150, 1789, 2556, 3451, 4474,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(639 - 767*x + 256*x\^{}2) / (1 - x)\^{}3.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 16 2018 05:57:01, revision 7) this is still
...
model reproduces all $37$ terms the entry publishes, exactly, in integer arithmetic --- a
...
\]
a polynomial of degree at most $259$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{259}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $264$
of them --- more than the $259+1$ the argument requires --- agree.
\end{proof}
...
First, the model reproduces every one of the $37$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250661.
