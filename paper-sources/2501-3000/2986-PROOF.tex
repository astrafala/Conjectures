...
\title{The empirical column recurrences for the table OEIS A235301}
...
\begin{abstract}
OEIS A235301 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A235301 is ``T(n,k) is the number of (n+1) X (k+1) 0..3 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
40, 108, 108, 272, 232, 272, 744, 476,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 18 2022 23:20:08, revision 7) these are still
...
The sequence is ``Number of (n+1) X (2) 0..3 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
values, $34$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235301.
