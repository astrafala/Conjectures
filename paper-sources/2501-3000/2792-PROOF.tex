...
\title{Arrays counted up to renaming: the empirical recurrence for OEIS A206377}
...
\begin{abstract}
OEIS A206377 carries an empirical recurrence of order $5$ contributed by R.~H.~Hardin. It is
...
both in the state the arrays are exactly the walks in a digraph on $S=12$ vertices, so $a(n)$
...
OEIS A206377 is ``Number of (n+1) X 3 0..3 arrays with every 2 X 2 subblock having the same number of equal diagonal or antidiagonal elements, and new values 0..3 introduced in row major order.''. It has offset $1$ and begins
\[
85, 1369, 25945, 520097, 10690969, 222329489, 4648291369, 97418918945,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 15 2018 12:45:11, revision 9) this is still
...
What is compared \emph{across} windows is carried too: the state holds the score fixed by the first band, since every later subblock is compared with it. With $2$ subblocks to a band that is a bounded amount of bookkeeping and no more.
...
\begin{theorem}
\[
a(n)\;=\;36\,a(n-1) - 377\,a(n-2) + 1398\,a(n-3) - 2064\,a(n-4) + 1008\,a(n-5)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206377.
