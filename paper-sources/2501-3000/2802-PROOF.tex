...
\title{The empirical recurrence for OEIS A223333, proved by transfer matrix}
...
\begin{abstract}
OEIS A223333 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$4$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is cube graph $Q_3$. The entry carries an empirical recurrence of order $4$
...
single slice is a state and the count is a walk count on a finite digraph with $S=961$ vertices.
\end{abstract}
...
OEIS A223333 is ``Rolling cube footprints: number of 4 X n 0..7 arrays starting with 0 where 0..7 label vertices of a cube and every array movement to a horizontal or antidiagonal neighbor moves along a corresponding cube edge.''. It has offset $1$ and begins
\[
512, 2187, 83349, 3176523, 121264857, 4630596579, 176834343105, 6753068175483,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 48*a(n-1) - 402*a(n-2) + 1064*a(n-3) - 789*a(n-4) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 19 2018 16:42:31, revision 10) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=961.
\]
...
of length $4$ over $8$ vertices is one of $8^{4}$ words, but only $S-1$ of them are
...
\begin{theorem}
\[
a(n)\;=\;48\,a(n-1) - 402\,a(n-2) + 1064\,a(n-3) - 789\,a(n-4)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223333.
