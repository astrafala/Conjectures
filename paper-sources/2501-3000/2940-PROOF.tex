...
\title{The conjectured generating function for OEIS A250810, proved}
...
\begin{abstract}
OEIS A250810 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250810 is ``Number of (n+1) X (6+1) 0..2 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
1296, 5139, 18537, 62469, 201741, 634509,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: 3*x*(432 - 879*x + 653*x\^{}2) / ((1 - x)*(1 - 2*x)*(1 - 3*x)). - \_Colin Barker\_, Nov 20 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 20 2018 16:32:26, revision 8) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250810.
