...
\title{The conjectured generating function for OEIS A207450, proved}
...
\begin{abstract}
OEIS A207450 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $282$ coefficients in exact arithmetic
...
OEIS A207450 is ``Number of n X 5 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 1 0 1 vertically.''. It has offset $1$ and begins
\[
16, 256, 1008, 2560, 5200, 9216,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: 16*x*(1 + 12*x + 5*x\^{}2) / (1 - x)\^{}4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 23 2018 07:21:16, revision 12) this is still
...
model reproduces all $32$ terms the entry publishes, exactly, in integer arithmetic --- a
...
\]
a polynomial of degree at most $276$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{276}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $282$
of them --- more than the $276+1$ the argument requires --- agree.
\end{proof}
...
First, the model reproduces every one of the $32$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207450.
