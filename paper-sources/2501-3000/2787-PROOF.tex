...
\title{The empirical recurrence for OEIS A198508, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A198508 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
lumpable, cutting $979$ pairs down to $S=53$ states. The entry's empirical recurrence of
...
OEIS A198508 is ``Number of 2n X 2 0..4 arrays with values 0..4 introduced in row major order and each element unequal to exactly two horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
4, 59, 1626, 52065, 1720080, 57202471, 1905005414, 63461666189,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 51*a(n-1) -691*a(n-2) +3601*a(n-3) -7056*a(n-4) +4096*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (May 14 2018 10:52:01, revision 10) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;51\,a(n-1) - 691\,a(n-2) + 3601\,a(n-3) - 7056\,a(n-4) + 4096\,a(n-5)
\end{equation}
for all $n>5$.
...
Assembling the $5$ lumped chains block-diagonally into $N$ of size $S=53$, with the
...
5!\;a(n)\;=\;\mathbf{v}^{\!\top}N^{\,L-2}\mathbf{u},\qquad L=2n+0 .
\end{equation}
...
index,
\[
5!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-5}\,
...
index corresponding to $n=5+1$ onwards, and the run was continued until $S+1$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198508.
