...
\title{The empirical recurrence for OEIS A199641, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A199641 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
order $5$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A199641 is ``Number of n X 2 0..2 arrays with values 0..2 introduced in row major order and each element equal to one or two horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 16 2018 06:43:23, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 7\,a(n-2) + 8\,a(n-3) - 8\,a(n-5)
\end{equation}
for all $n>7$.
...
index,
\[
3!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-5}\,
\Bigl(h_{5}-\sum_i c_i h_{5-i}\Bigr)
\]
...
$w=h_{5}-\sum_i c_ih_{5-i}$; and it is enough to check $m=0,1,\dots,S-1$.
\end{lemma}
...
index corresponding to $n=7+1$ onwards, and the run was continued until $S+1$
...
\eqref{eq:walk2}, divided by $3!$, was evaluated at every one of the $23$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A199641.
