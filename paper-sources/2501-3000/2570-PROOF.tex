...
\title{The empirical recurrence for OEIS A224549, proved by transfer matrix}
...
\begin{abstract}
OEIS A224549 counts the $(n+1)\times8$ matrices over $\{0,\dots,1\}$ all of whose
...
OEIS A224549 is ``Number of (n+1) X 8 0..1 matrices with each 2 X 2 subblock idempotent.''. It has offset $1$ and begins
\[
216, 345, 472, 682, 943, 1323, 1858, 2652,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 5*a(n-2) + a(n-3) + 2*a(n-4) - a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 31 2018 06:37:56, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $2\times2$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $2\times8$ matrix a \emph{band} when every one of its $7$ contiguous
$2\times2$ subblocks is a window. What the entry counts is the matrices of $8$ columns
whose every $2$ consecutive rows form a band.

Take as states the $1\times8$ strips that occur as the top $1$ rows of a band,
...
There are $(1 + 1)^{8}$ strips of $1$ rows and $8$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $2\times1$ entries, so a band is exactly a walk of $7$ windows in
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224549.
