...
\title{The conjectured generating function for OEIS A232584, proved}
...
\begin{abstract}
OEIS A232584 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=6642$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $6650$ coefficients in exact arithmetic
...
OEIS A232584 is ``Number of (n+1)X(3+1) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally, with no adjacent elements equal.''. It has offset $1$ and begins
\[
2, 12, 42, 122, 332, 882,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.: -2*x*(1+x)\^{}2 / ( (x-1)*(x\^{}2-3*x+1) ). - \_R. J. Mathar\_, Nov 23 2014
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 07:30:39, revision 14) this is still
...
\]
and there are $S=6642$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $6645$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{6645}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $6650$
of them --- more than the $6645+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{3}$ read off $D_2$, the first few being $4,\ -4,\ 1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232584.
