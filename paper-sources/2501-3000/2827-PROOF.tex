...
\title{No column above the one before it in every row: OEIS A212939}
...
\begin{abstract}
OEIS A212939 carries an empirical recurrence of order $5$ contributed by R.~H.~Hardin. It is
...
OEIS A212939 is ``Number of nX4 0..3 arrays with no column j greater than or equal to than column j-1 in all rows.''. It has offset $1$ and begins
\[
1, 10311, 6470341, 2509306671, 801439905901, 232665778445031, 64146409058631061, 17171607076234160991,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 631*a(n-1) -144700*a(n-2) +15035200*a(n-3) -702208000*a(n-4) +11468800000*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 16:41:09, revision 10) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays of $4$ columns whose rows are arbitrary over the alphabet $\{0,1,2,3\}$, and asks that no column $j$ be greater than or equal to column $j-1$ \emph{in all rows}.

That is not a condition on any bounded window: ``in all rows'' quantifies over the whole array, and a row arriving at the very end can still decide it. What makes it a walk anyway is that it is a conjunction of independent facts about single rows. For each of the $3$ adjacent column pairs keep one bit, recording whether every row so far has had column $j$ greater than or equal to column $j-1$. A new row can only clear bits, never set one, and the array is admissible exactly when every bit has been cleared by the time the array ends.

So the state is that bit mask and nothing else --- at most $2^{3}$ of them, however large the alphabet is, which is why entries whose terms run to twenty digits have models this small. Rows with the same bit pattern act identically, so each row contributes one edge and the multiplicity does the counting. Merging vertices with identical futures leaves $S=5$, and with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which masks may begin and end an array,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A212939.
