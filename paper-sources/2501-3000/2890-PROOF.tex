...
\title{The empirical recurrence for OEIS A223558, proved by transfer matrix}
...
\begin{abstract}
OEIS A223558 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$3$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is graph named in the entry. The entry carries an empirical recurrence of order $4$
...
single slice is a state and the count is a walk count on a finite digraph with $S=162$ vertices.
\end{abstract}
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=162.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223558.
