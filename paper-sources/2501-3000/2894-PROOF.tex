...
\title{The empirical recurrence for OEIS A206614, proved by transfer matrix}
...
\begin{abstract}
OEIS A206614 counts, up to renaming the letters, the arrays of width $2$ over at most $3$
letters in which every $2\times3$ and $3\times2$ subblock has at most $2$ equal edges, an edge of such a window being one of the six steps of its perimeter cycle and an equal edge one whose two cells agree. It carries an empirical recurrence of order $4$ contributed by
...
OEIS A206614 is ``Number of (n+1) X 2 0..2 arrays with every 2 X 3 or 3 X 2 subblock having no more than two equal edges, and new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
14, 86, 580, 4035, 27895, 192358, 1327931, 9168477,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 7*a(n-1) - 5*a(n-2) + 36*a(n-3) - 42*a(n-4) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 17 2018 08:10:20, revision 9) this is still
...
\begin{theorem}
\[
a(n)\;=\;7\,a(n-1) - 5\,a(n-2) + 36\,a(n-3) - 42\,a(n-4)
\]
for every $n>5$.
\end{theorem}
...
the integers. The bound is exact: at $n=5$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206614.
