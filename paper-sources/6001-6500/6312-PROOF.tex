...
\title{The empirical recurrence for OEIS A304762, proved by transfer matrix}
...
\begin{abstract}
OEIS A304762 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{2,3,4,5,6\}$, and it carries an empirical
...
OEIS A304762 is ``Number of nX3 0..1 arrays with every element unequal to 2, 3, 4, 5 or 6 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 5, 16, 103, 609, 3680, 22187, 133917,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 18 2018 08:04:02, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) + 8\,a(n-2) - a(n-3) - 51\,a(n-4) - 45\,a(n-5) + 30\,a(n-6) + 33\,a(n-7) + 7\,a(n-8) - 13\,a(n-9) - 2\,a(n-10) + 5\,a(n-11) + 8\,a(n-12) + 2\,a(n-13)
\end{equation}
for all $n>15$.
...
has score in $\{2,3,4,5,6\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{2,3,4,5,6\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{2,3,4,5,6\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{2,3,4,5,6\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{2,3,4,5,6\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A304762.
