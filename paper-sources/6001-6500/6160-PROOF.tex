...
\title{The empirical recurrence for OEIS A298096, proved by transfer matrix}
...
\begin{abstract}
OEIS A298096 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{3,4,5,6\}$, and it carries an empirical
...
OEIS A298096 is ``Number of nX4 0..1 arrays with every element equal to 3, 4, 5 or 6 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 2, 1, 3, 7, 20, 42, 121,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 12 2018 09:46:57, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-2) + 8\,a(n-3) + 2\,a(n-4) - 15\,a(n-5) - 23\,a(n-6) - 22\,a(n-7) - 9\,a(n-8) + 5\,a(n-9) + 20\,a(n-10) + 24\,a(n-11) + 23\,a(n-12) + 8\,a(n-13) - 3\,a(n-14) - 4\,a(n-15) - 2\,a(n-16)
\end{equation}
for all $n>16$.
...
has score in $\{3,4,5,6\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{3,4,5,6\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{3,4,5,6\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{3,4,5,6\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{3,4,5,6\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298096.
