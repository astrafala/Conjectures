...
\title{The empirical recurrence for OEIS A318340, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A318340 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $15$ contributed
...
OEIS A318340 is ``Number of nX5 0..1 arrays with every element unequal to 0, 1, 2 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
16, 42, 76, 148, 393, 1115, 3321, 10111,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 24 2018 11:02:57, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 5\,a(n-2) - 12\,a(n-3) - 17\,a(n-4) + 20\,a(n-5) + 31\,a(n-6) - 13\,a(n-7) - 33\,a(n-8) + a(n-9) + 20\,a(n-10) + 2\,a(n-11) - 4\,a(n-12) - 3\,a(n-13) + a(n-15)
\end{equation}
for all $n>18$.
...
\#\{\text{neighbours with a value }different from\ x_{t,u}\}\in\{0,1,2,6\}
\end{equation}
...
publishes, $n=25$, the arrays have $5\cdot25$ cells over
$2$ values, so there are about $10^{37}$ of them to sift. The
walk count replaces that by $1024$ states and one matrix--vector product per row.

The threshold below is exact rather than merely sufficient: at $n=18$ the conjectured
...
$o=1n_0+0-2$. Then for $j\ge15$,
\[
a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\;=\;\,\mathbf{v}^{\!\top}G^{\,j-15}w,
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-15}$ out of
...
the middle row. The vector $w$ was formed by $15$ applications of $G$ in exact integer
arithmetic and $\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly.
Every one of those integers vanishes from the index corresponding to $n=18+1$ onwards,
...
every one of the $25$ published indices, with no shift fitted: the number of rows is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A318340.
