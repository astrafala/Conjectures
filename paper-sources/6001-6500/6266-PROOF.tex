...
\title{The empirical recurrence for OEIS A261549, proved by transfer matrix}
...
\begin{abstract}
OEIS A261549 counts $(n+2)\times4$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
every $3\times3$ subblock, and it carries an empirical recurrence of order $16$
...
OEIS A261549 is ``Number of (n+2)X(2+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00000001 00000101 or 00001011.''. It has offset $1$ and begins
\[
80, 132, 269, 484, 871, 1702, 3230, 6084,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 24 2015 10:57:00, revision 4) this is still
...
\[
\text{the clockwise perimeter word of }g\text{ is a rotation of one of }\{00000001,\ 00000101,\ 00001011\}
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-2) + 3\,a(n-3) + a(n-4) - a(n-5) + a(n-6) - 2\,a(n-7) - 2\,a(n-8) - 3\,a(n-9) + 2\,a(n-10) - a(n-11) - 7\,a(n-12) + 3\,a(n-13) - 4\,a(n-14) - 2\,a(n-15) + 2\,a(n-16)
\]
for every $n>17$.
\end{theorem}

\begin{proof}
The vector $w=q(M)\mathbf{1}$ was formed by $16$ matrix--vector products in exact
integer arithmetic and $\mathbf{1}^{\!\top}M^{j}w$ evaluated for $j=0,1,\dots$, again
exactly. Those integers vanish from the index corresponding to $n=17+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>17$. The bound is exact: at $n=17$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A261549.
