...
\title{The empirical recurrence for OEIS A223397, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A223397 counts arrays over $\{0,\dots,15\}$ in which every cell is constrained by the
...
OEIS A223397 is ``4X4 square grid graph coloring a rectangular array: number of nX3 0..15 arrays where 0..15 label nodes of the square grid graph and every array movement to a horizontal or vertical neighbor moves along an edge of this graph.''. It has offset $1$ and begins
\[
152, 2360, 39480, 672784, 11561512, 199425400, 3445747472, 59582558136,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 21:36:50, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;18\,a(n-1) + 69\,a(n-2) - 1554\,a(n-3) + 2085\,a(n-4) + 10353\,a(n-5) - 18959\,a(n-6) - 11633\,a(n-7) + 31508\,a(n-8) - 5302\,a(n-9) - 10390\,a(n-10) + 4408\,a(n-11) - 440\,a(n-12)
\end{equation}
for all $n>13$.
...
row $t$ and column $u$, each in $\{0,\dots,15\}$. With the neighbour offsets
\[
\mathcal N=\{(0,1),\ (1,0)\},
\]
...
\{x_{t,u},x_{t+d_1,u+d_2}\}\in E\ \text{ for every offset in }\mathcal N,\quad E=\text{the 4}\times\text{4 grid graph}
\end{equation}
for every cell $(t,u)$.
...
Let $V=\{0,\dots,15\}^{3}$ be the possible rows and take
...
and $3$ cells in a row, so $|V|=16^{3}=4096$, and the vertex set is
$V\times V$, of size $4096^2=4096$. That is the whole of the state: two consecutive
...
publishes, $n=15$, the arrays have $3\cdot15$ cells over
$16$ values, so there are about $10^{54}$ of them to sift. The
walk count replaces that by $4096$ states and one matrix--vector product per row.

The threshold below is exact rather than merely sufficient: at $n=13$ the conjectured
...
of the $(15+1)^{3}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223397.
