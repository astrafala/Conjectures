...
\title{The empirical recurrence for OEIS A233164, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233164 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 09 2018 14:39:53, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3840 x^{3} - 464 x^{2} + 11 x}{512 x^{2} - 48 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;512 x^{2} - 48 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233164.
