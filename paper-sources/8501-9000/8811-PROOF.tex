...
\title{The empirical recurrence for OEIS A205329, derived from the entry's generating function}
...
\begin{abstract}
OEIS A205329 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A205329 is ``Number of (n+1) X 3 0..2 arrays with the number of equal 2 X 2 subblock diagonal pairs and equal antidiagonal pairs differing from each horizontal or vertical neighbor, and new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
72, 594, 4860, 39852, 326592, 2676888, 21939984, 179823888,\ \dots
\]
...
\begin{quote}
G.f.: 18*x*(4 + 9*x) / (1 - 6*x - 18*x\^{}2). - \_Colin Barker\_, Jun 11 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 11:53:25, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 162 x^{2} - 72 x}{18 x^{2} + 6 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 18 x^{2} - 6 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205329.
