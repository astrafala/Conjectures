...
\title{A proof of the conjectured recurrence for OEIS A025245}
...
\begin{abstract}
OEIS A025245 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{4} + 9 x^{3} + 3 x^{2} + 4 x$, a polynomial of degree $4$, which proves
...
OEIS A025245 is ``a(n) = (1/2)*s(n+3), where s = A025244''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 5, 11, 26, 65, 163, 416,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{- 4 x^{3} - x^{2} - x - \sqrt{x^{4} - 6 x^{3} - x^{2} - 2 x + 1} + 1}{4 x^{3}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+3)*a(n) +(-2*n-3)*a(n-1) -n*a(n-2) +3*(-2*n+3)*a(n-3) +(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 25 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2022-11-11, revision 15) the
...
\[
p_{0}(n)=n + 3,\qquad p_{1}(n)=- 2 n - 3,\qquad p_{2}(n)=- n,\qquad p_{3}(n)=- 3 \left(2 n - 3\right),\qquad p_{4}(n)=n - 3
\]
...
\section{The computation for A025245}
...
\[
(n + 3)\,a(n) + (- 2 n - 3)\,a(n-1) -  n\,a(n-2) -  3 \left(2 n - 3\right)\,a(n-3) + (n - 3)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A025245, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{4} + 9 x^{3} + 3 x^{2} + 4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A025245.
