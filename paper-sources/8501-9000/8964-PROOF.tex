...
\title{The empirical recurrence for OEIS A250784, derived from the entry's generating function}
...
\begin{abstract}
OEIS A250784 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 19 2018 12:58:41, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 16 x^{4} + 6 x^{3} + 26 x^{2} - 18 x}{2 x^{4} - x^{3} - 4 x^{2} + 4 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 2 x^{4} + x^{3} + 4 x^{2} - 4 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;16 x^{4} - 6 x^{3} - 26 x^{2} + 18 x,
\]
