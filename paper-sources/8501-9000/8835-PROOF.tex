...
\title{The empirical recurrence for OEIS A223258, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223258 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 18 2018 08:43:12, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 105 x^{2} + 25 x}{129 x^{2} - 38 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;129 x^{2} - 38 x + 1
\]
