...
\title{A proof of the conjectured recurrence for OEIS A257300}
...
\begin{abstract}
OEIS A257300 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $6 x^{5} + 2 x^{4} - 6 x^{3} + 9 x^{2} - x$, a polynomial of degree $5$, which proves
...
OEIS A257300 is ``Number of Motzkin paths of length n with no peaks at level 2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 4, 8, 17, 38, 88,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x^{2} + x - \sqrt{- 3 x^{2} - 2 x + 1} - 1}{x \left(2 x^{2} - x + \sqrt{- 3 x^{2} - 2 x + 1} + 1\right) + x - \sqrt{- 3 x^{2} - 2 x + 1} - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(-5*n+3)*a(n-1) +6*(n)*a(n-2) +2*(n-9)*a(n-3) +6*(-n+4)*a(n-4) +(n-6)*a(n-5) +3*(-n+3)*a(n-6)=0. - \emph{R. J. Mathar}, Sep 24 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-02-14, revision 31) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=3 - 5 n,\qquad p_{2}(n)=6 n,\qquad p_{3}(n)=2 \left(n - 9\right),\qquad p_{4}(n)=- 6 \left(n - 4\right),\qquad p_{5}(n)=n - 6,\qquad p_{6}(n)=- 3 \left(n - 3\right)
\]
...
\section{The computation for A257300}
...
\[
n\,a(n) + (3 - 5 n)\,a(n-1) + 6 n\,a(n-2) + 2 \left(n - 9\right)\,a(n-3) -  6 \left(n - 4\right)\,a(n-4) + (n - 6)\,a(n-5) -  3 \left(n - 3\right)\,a(n-6) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A257300, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=6 x^{5} + 2 x^{4} - 6 x^{3} + 9 x^{2} - x$ is an
...
\url{https://oeis.org}, 2026. Sequence A257300.
