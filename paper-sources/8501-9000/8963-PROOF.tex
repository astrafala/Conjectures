...
\title{A proof of the conjectured recurrence for OEIS A185087}
...
\begin{abstract}
OEIS A185087 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 - 2 x$, a polynomial of degree $1$, which proves
...
OEIS A185087 is ``a(n) = Sum\_\{k=0..floor(n/2)\} binomial(n-k,k)*A000108(k+1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 5, 12, 24, 55, 119,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x^{2} - x - \sqrt{4 x^{3} - 3 x^{2} - 2 x + 1} + 1}{2 x^{4}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+4)*a(n) -(2*n+5)*a(n-1) -3*(n+1)*a(n-2) +2*(2*n-1)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 16 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2017-06-23, revision 18) the
...
\[
p_{0}(n)=n + 4,\qquad p_{1}(n)=- 2 n - 5,\qquad p_{2}(n)=- 3 \left(n + 1\right),\qquad p_{3}(n)=2 \left(2 n - 1\right)
\]
...
\section{The computation for A185087}
...
\[
(n + 4)\,a(n) + (- 2 n - 5)\,a(n-1) -  3 \left(n + 1\right)\,a(n-2) + 2 \left(2 n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A185087, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=4 - 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A185087.
