...
\title{The empirical recurrence for OEIS A262267, derived from the entry's generating function}
...
\begin{abstract}
OEIS A262267 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 21 2018 19:19:39, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{4} + x^{3} - 2 x^{2} + 2 x}{2 x^{4} - 3 x^{3} + 3 x^{2} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{4} - 3 x^{3} + 3 x^{2} - 3 x + 1
\]
