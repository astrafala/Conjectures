...
\title{The empirical recurrence for OEIS A208087, derived from the entry's generating function}
...
\begin{abstract}
OEIS A208087 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 27 2018 14:17:54, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 42 x^{4} + 102 x^{3} + 48 x^{2} - 72 x}{x^{4} - 3 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;42 x^{4} - 102 x^{3} - 48 x^{2} + 72 x,
\]
