...
\title{The empirical recurrence for OEIS A233402, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233402 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A233402 is ``Number of (n+1) X (1+1) 0..2 arrays with row and column sums nondecreasing, and no adjacent elements equal.''. It has offset $1$ and begins
\[
9, 11, 22, 24, 41, 42, 66, 65,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 11 2018 05:39:27, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{6} - 2 x^{5} + 9 x^{4} + 5 x^{3} - 11 x^{2} - 9 x}{x^{6} - 3 x^{4} + 3 x^{2} - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{6} + 2 x^{5} - 9 x^{4} - 5 x^{3} + 11 x^{2} + 9 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233402.
