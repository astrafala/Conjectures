...
\title{A proof of the conjectured recurrence for OEIS A116383}
...
\begin{abstract}
OEIS A116383 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 10 x^{4} - 2 x^{3} + 2 x^{2}$, a polynomial of degree $4$, which proves
...
OEIS A116383 is ``Row sums of number triangle A116382''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 6, 19, 33, 93, 175,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4 x^{2} - x - \sqrt{1 - 4 x^{2}} \left(x + 1\right) - 1}{2 \sqrt{1 - 4 x^{2}} \left(5 x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + (n-2)*a(n-1) + (-13*n+20)*a(n-2) + (-9*n+22)*a(n-3) + 4*(14*n-45)*a(n-4) + 20*(n-3)*a(n-5) + 80*(-n+5)*a(n-6) = 0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 25) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=n - 2,\qquad p_{2}(n)=20 - 13 n,\qquad p_{3}(n)=22 - 9 n,\qquad p_{4}(n)=4 \left(14 n - 45\right),\qquad p_{5}(n)=20 \left(n - 3\right),\qquad p_{6}(n)=- 80 \left(n - 5\right)
\]
...
\section{The computation for A116383}
...
\[
n\,a(n) + (n - 2)\,a(n-1) + (20 - 13 n)\,a(n-2) + (22 - 9 n)\,a(n-3) + 4 \left(14 n - 45\right)\,a(n-4) + 20 \left(n - 3\right)\,a(n-5) -  80 \left(n - 5\right)\,a(n-6) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A116383, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 10 x^{4} - 2 x^{3} + 2 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A116383.
