...
\title{A proof of the conjectured recurrence for OEIS A071717}
...
\begin{abstract}
OEIS A071717 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x^{3} + 6 x^{2} + x + 2$, a polynomial of degree $3$, which proves
...
OEIS A071717 is ``Expansion of (1 + x\^{}2*C)*C\^{}2, where C = (1 - sqrt(1-4*x))/(2*x) is g.f. for Catalan numbers, A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 6, 17, 51, 160, 519, 1727,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 3 x^{2} - x + \sqrt{1 - 4 x} \left(x^{2} - x - 1\right) + 1}{2 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +(-3*n-2)*a(n-1) +(-5*n+8)*a(n-2) +2*(2*n-7)*a(n-3)=0. - \emph{R. J. Mathar}, Aug 25 2013
\end{quote}
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 3 n - 2,\qquad p_{2}(n)=8 - 5 n,\qquad p_{3}(n)=2 \left(2 n - 7\right)
\]
...
\section{The computation for A071717}
...
\[
(n + 2)\,a(n) + (- 3 n - 2)\,a(n-1) + (8 - 5 n)\,a(n-2) + 2 \left(2 n - 7\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A071717, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=3 x^{3} + 6 x^{2} + x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A071717.
