...
\title{A proof of the conjectured recurrence for OEIS A141771}
...
\begin{abstract}
OEIS A141771 carries a conjectured linear recurrence with polynomial coefficients, of
order 1, contributed by R.~J.~Mathar. We prove it. The generating function of the
...
closed form. Here it equals $2232 x^{2} + 1728 x + 3000$, a polynomial of degree $2$, which proves
...
OEIS A141771 is ``Expansion of (1-sqrt(1-4*x))/(2*x) + 8*x\^{}3/((sqrt(1-4*x))*(1+sqrt(1-4*x))\^{}3)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 6, 19, 63, 216, 759,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{8 x^{3}}{\sqrt{1 - 4 x} \left(\sqrt{1 - 4 x} + 1\right)^{3}} - \frac{\sqrt{1 - 4 x}}{2 x} + \frac{1}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1) *(n\^{}2+5*n-12) *(n\^{}3-5*n\^{}2+38*n-250)*a(n) -2 *(2*n-3) *(n\^{}2+7*n-6) *(n\^{}3-5*n\^{}2+38*n-250) *a(n-1)=0. - \emph{R. J. Mathar}, Apr 30 2016
\end{quote}
...
Writing the conjectured relation in the form $\sum_{i=0}^{1}p_{i}(n)\,a(n-i)=0$,
the coefficient polynomials are
\[
p_{0}(n)=\left(n + 1\right) \left(n^{2} + 5 n - 12\right) \left(n^{3} - 5 n^{2} + 38 n - 250\right),\qquad p_{1}(n)=- 2 \left(2 n - 3\right) \left(n^{2} + 7 n - 6\right) \left(n^{3} - 5 n^{2} + 38 n - 250\right)
\]
...
\section{The computation for A141771}
...
\[
\left(n + 1\right) \left(n^{2} + 5 n - 12\right) \left(n^{3} - 5 n^{2} + 38 n - 250\right)\,a(n) -  2 \left(2 n - 3\right) \left(n^{2} + 7 n - 6\right) \left(n^{3} - 5 n^{2} + 38 n - 250\right)\,a(n-1) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A141771, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2232 x^{2} + 1728 x + 3000$ is an
...
\url{https://oeis.org}, 2026. Sequence A141771.
