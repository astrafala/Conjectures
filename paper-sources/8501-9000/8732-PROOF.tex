...
\title{A proof of the conjectured recurrence for OEIS A166076}
...
\begin{abstract}
OEIS A166076 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $12 x^{2} + 20 x$, a polynomial of degree $2$, which proves
...
OEIS A166076 is ``Expansion of (7+5x-sqrt(1-10x-7x\^{}2))/(2(3+5x+2x\^{}2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 2, 10, 70, 530, 4278, 36050,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{5 x - \sqrt{- 7 x^{2} - 10 x + 1} + 7}{2 \left(2 x^{2} + 5 x + 3\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 3*(n)*a(n) +5*(-5*n+9)*a(n-1) +69*(-n+2)*a(n-2) +5*(-11*n+27)*a(n-3) +14*(-n+3)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 10 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2016-04-24, revision 11) the
...
\[
p_{0}(n)=3 n,\qquad p_{1}(n)=- 5 \left(5 n - 9\right),\qquad p_{2}(n)=- 69 \left(n - 2\right),\qquad p_{3}(n)=- 5 \left(11 n - 27\right),\qquad p_{4}(n)=- 14 \left(n - 3\right)
\]
...
\section{The computation for A166076}
...
\[
3 n\,a(n) -  5 \left(5 n - 9\right)\,a(n-1) -  69 \left(n - 2\right)\,a(n-2) -  5 \left(11 n - 27\right)\,a(n-3) -  14 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A166076, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=12 x^{2} + 20 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A166076.
