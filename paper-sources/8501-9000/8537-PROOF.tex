...
\title{The empirical recurrence for OEIS A222940, derived from the entry's generating function}
...
\begin{abstract}
OEIS A222940 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 16 2018 15:13:14, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{6 x^{4} - 19 x^{3} - 13 x^{2} + 2 x}{9 x^{2} - 10 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;9 x^{2} - 10 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;6 x^{4} - 19 x^{3} - 13 x^{2} + 2 x,
\]
