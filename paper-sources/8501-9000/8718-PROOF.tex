...
\title{A proof of the conjectured recurrence for OEIS A257104}
...
\begin{abstract}
OEIS A257104 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x^{6} + 3 x^{5} - 17 x^{4} + 21 x^{3} + 3 x^{2} - 9 x + 2$, a polynomial of degree $6$, which proves
...
OEIS A257104 is ``Number of Motzkin paths of length n with no peaks at level 3''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 4, 9, 21, 50, 121,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x^{3} - x^{2} + x \sqrt{- 3 x^{2} - 2 x + 1} + 2 x - \sqrt{- 3 x^{2} - 2 x + 1} - 1}{2 x^{3} - 2 x^{2} + 2 x \sqrt{- 3 x^{2} - 2 x + 1} + 3 x - \sqrt{- 3 x^{2} - 2 x + 1} - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} D-finite with recurrence (-n+2)*a(n) +(7*n-17)*a(n-1) +2*(-7*n+17)*a(n-2) +(n+22)*a(n-3) +(16*n-89)*a(n-4) +(-4*n+23)*a(n-5) +3*(n-5)*a(n-6)=0. - \emph{R. J. Mathar}, Sep 24 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2020-06-17, revision 43) the
...
\[
p_{0}(n)=2 - n,\qquad p_{1}(n)=7 n - 17,\qquad p_{2}(n)=- 2 \left(7 n - 17\right),\qquad p_{3}(n)=n + 22,\qquad p_{4}(n)=16 n - 89,\qquad p_{5}(n)=23 - 4 n,\qquad p_{6}(n)=3 \left(n - 5\right)
\]
...
\section{The computation for A257104}
...
\[
B(x)\;=\;3 x^{6} + 3 x^{5} - 17 x^{4} + 21 x^{3} + 3 x^{2} - 9 x + 2 ,
\]
...
\[
(2 - n)\,a(n) + (7 n - 17)\,a(n-1) -  2 \left(7 n - 17\right)\,a(n-2) + (n + 22)\,a(n-3) + (16 n - 89)\,a(n-4) + (23 - 4 n)\,a(n-5) + 3 \left(n - 5\right)\,a(n-6) \;=\;0
\]
holds for every $n>6$.
\end{theorem}
...
section of A257104, agreeing on all 13 terms compared.
...
vanish, for all 28 values of $n$ from $n=7$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=3 x^{6} + 3 x^{5} - 17 x^{4} + 21 x^{3} + 3 x^{2} - 9 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A257104.
