...
\title{The empirical recurrence for OEIS A183586, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183586 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A183586 is ``1/24 the number of (n+1) X 2 0..3 arrays with every 2 X 2 subblock containing exactly three distinct values.''. It has offset $1$ and begins
\[
6, 56, 520, 4832, 44896, 417152, 3875968, 36013568,\ \dots
\]
...
\begin{quote}
G.f.: 2*x*(3 + 4*x) / (1 - 8*x - 12*x\^{}2).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 29 2018 15:06:38, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{2} - 6 x}{12 x^{2} + 8 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 12 x^{2} - 8 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183586.
