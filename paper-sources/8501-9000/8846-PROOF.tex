...
\title{The empirical recurrence for OEIS A233020, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233020 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A233020 is ``Number of n X 2 0..3 arrays with no element x(i,j) adjacent to value 3-x(i,j) horizontally, vertically, diagonally or antidiagonally, and top left element zero.''. It has offset $1$ and begins
\[
3, 15, 81, 435, 2337, 12555, 67449, 362355,\ \dots
\]
...
G.f.: 3*x / (1 - 5*x - 2*x\^{}2).
\end{quote}
As of the ``Last modified'' line on the live entry (May 11 2022 10:28:40, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;- \frac{3 x}{2 x^{2} + 5 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 2 x^{2} - 5 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233020.
