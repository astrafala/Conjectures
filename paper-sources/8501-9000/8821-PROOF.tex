...
\title{The empirical recurrence for OEIS A189604, derived from the entry's generating function}
...
\begin{abstract}
OEIS A189604 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A189604 is ``Number of n X 3 array permutations with each element not moving, or moving one space E, S or NW.''. It has offset $1$ and begins
\[
1, 6, 20, 72, 256, 912, 3248, 11568,\ \dots
\]
...
\begin{quote}
G.f.: (x+3*x\^{}2)/(1-3*x-2*x\^{}2). - \_Vladimir Kruchinin\_, May 13 2011
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 12 2026 13:18:30, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{2} - x}{2 x^{2} + 3 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A189604.
