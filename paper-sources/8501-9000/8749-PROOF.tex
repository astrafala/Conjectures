...
\title{The empirical recurrence for OEIS A183430, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183430 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A183430 is ``One third the number of n X 2 0..3 arrays with no element equal to its row sum plus its column sum mod 4.''. It has offset $1$ and begins
\[
4, 27, 244, 2187, 19684, 177147, 1594324, 14348907,\ \dots
\]
...
\begin{quote}
G.f.: x*(4 - 9*x - 3*x\^{}2) / ((1 - x)*(1 + x)*(1 - 9*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 29 2018 05:30:44, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{3} - 9 x^{2} + 4 x}{9 x^{3} - x^{2} - 9 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;9 x^{3} - x^{2} - 9 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183430.
