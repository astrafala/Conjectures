...
\title{The empirical recurrence for OEIS A263794, derived from the entry's generating function}
...
\begin{abstract}
OEIS A263794 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A263794 is ``Number of (n+1) X (3+1) 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits, and rows and columns lexicographically nonincreasing.''. It has offset $1$ and begins
\[
3, 3, 7, 7, 14, 14, 25, 25,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 23 2016 22:55:03, revision 24) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{7} + 4 x^{5} - 5 x^{3} + 3 x}{x^{7} - x^{6} - 3 x^{5} + 3 x^{4} + 3 x^{3} - 3 x^{2} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{7} - x^{6} - 3 x^{5} + 3 x^{4} + 3 x^{3} - 3 x^{2} - x + 1
\]
...
compared against all $52$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263794.
