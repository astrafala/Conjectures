...
\title{A proof of the conjectured recurrence for OEIS A130655}
...
\begin{abstract}
OEIS A130655 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $8 x^{2} - 6 x$, a polynomial of degree $2$, which proves
...
OEIS A130655 is ``Catalan transform of Catalan numbers C(n+1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 7, 28, 119, 524, 2363, 10844,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- \sqrt{1 - 4 x} + \sqrt{2 \sqrt{1 - 4 x} - 1}}{2 x + \sqrt{1 - 4 x} - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 3*n*(n-2)*(n+2)*a(n) +4*(-10*n\^{}3+21*n\^{}2+7*n-15)*a(n-1) +16*(11*n\^{}3-47*n\^{}2+57*n-15)*a(n-2) -8*(2*n-5)*(4*n-9)*(4*n-7)*a(n-3)=0. - \emph{R. J. Mathar}, Mar 01 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-22, revision 19) the
...
\[
p_{0}(n)=3 n \left(n - 2\right) \left(n + 2\right),\qquad p_{1}(n)=- 4 \left(10 n^{3} - 21 n^{2} - 7 n + 15\right),\qquad p_{2}(n)=16 \left(11 n^{3} - 47 n^{2} + 57 n - 15\right),\qquad p_{3}(n)=- 8 \left(2 n - 5\right) \left(4 n - 9\right) \left(4 n - 7\right)
\]
...
\[
3 n \left(n - 2\right) \left(n + 2\right)\,a(n) -  4 \left(10 n^{3} - 21 n^{2} - 7 n + 15\right)\,a(n-1) + 16 \left(11 n^{3} - 47 n^{2} + 57 n - 15\right)\,a(n-2) -  8 \left(2 n - 5\right) \left(4 n - 9\right) \left(4 n - 7\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A130655, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=8 x^{2} - 6 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A130655.
