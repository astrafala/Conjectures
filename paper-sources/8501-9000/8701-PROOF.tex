...
\title{The empirical recurrence for OEIS A221764, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221764 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A221764 is ``Number of n X 3 arrays of occupancy after each element moves to some horizontal or vertical neighbor, without 2-loops or left turns.''. It has offset $1$ and begins
\[
0, 3, 16, 92, 536, 3125, 18216, 106174,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2018 17:34:10, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{6 x^{4} - 8 x^{3} + 3 x^{2}}{x^{4} - 8 x^{3} + 14 x^{2} - 8 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221764.
