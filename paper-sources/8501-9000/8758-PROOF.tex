...
\title{The empirical recurrence for OEIS A222335, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A222335 counts arrays over $\{0,\dots,6\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $1$ contributed
...
with $S=2401$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A222335 is ``Number of nX2 0..6 arrays with no entry increasing mod 7 by 6 rightwards or downwards, starting with upper left zero.''. It has offset $1$ and begins
\[
6, 186, 5766, 178746, 5541126, 171774906, 5325022086, 165075684666,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 31*a(n-1)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 20:17:45, revision 9) the statement is
...
x_{t,u+1}\ne x_{t,u}+6\ (\mathrm{mod}\ 7)\ \text{ and }\ x_{t+1,u}\ne x_{t,u}+6\ (\mathrm{mod}\ 7)
\end{equation}
...
Let $V=\{0,\dots,6\}^{2}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=2401$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
The size of $\mathcal V$ is worth seeing concretely. There are $6+1=7$ values
and $2$ cells in a row, so $|V|=7^{2}=49$, and the vertex set is
$V\times V$, of size $49^2=2401$. That is the whole of the state: two consecutive
...
publishes, $n=15$, the arrays have $2\cdot15$ cells over
$7$ values, so there are about $10^{25}$ of them to sift. The
walk count replaces that by $2401$ states and one matrix--vector product per row.
...
$o=1n_0+0-2$. Then for $j\ge1$,
\[
a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\;=\;\,\mathbf{v}^{\!\top}G^{\,j-1}w,
\qquad w=h_{1}-\sum_i c_i\,h_{1-i} .
\]
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-1}$ out of
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=2401$ pairs and each
of the $(6+1)^{2}$ possible next rows, the condition was tested on every cell of
the middle row. The vector $w$ was formed by $1$ applications of $G$ in exact integer
arithmetic and $\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly.
Every one of those integers vanishes from the index corresponding to $n=2+1$ onwards,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222335.
