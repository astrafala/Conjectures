...
\title{The empirical recurrence for OEIS A221082, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221082 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 03 2018 08:13:01, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x}{9 x^{2} - 12 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;9 x^{2} - 12 x + 1
\]
