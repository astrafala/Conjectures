...
closed form. Here it equals $4 x^{2} + 4 x$, a polynomial of degree $2$, which proves
...
\begin{quote}\small
\textbf{Conjecture:} (n+3)*a(n) +4*(-n-2)*a(n-1) +2*a(n-2) +8*(n-1)*a(n-3) +3*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Apr 03 2015
\end{quote}
...
\[
p_{0}(n)=n + 3,\qquad p_{1}(n)=- 4 \left(n + 2\right),\qquad p_{2}(n)=2,\qquad p_{3}(n)=8 \left(n - 1\right),\qquad p_{4}(n)=3 \left(n - 3\right)
\]
...
\[
(n + 3)\,a(n) -  4 \left(n + 2\right)\,a(n-1) + 2\,a(n-2) + 8 \left(n - 1\right)\,a(n-3) + 3 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
cancellation in $K$, not by series truncation, so the identity $B=4 x^{2} + 4 x$ is an
