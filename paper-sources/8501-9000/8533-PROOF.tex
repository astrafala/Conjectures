...
\title{The empirical recurrence for OEIS A184033, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184033 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 04:52:10, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 172 x^{4} + 101 x^{3} + 86 x^{2} - 49 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;172 x^{4} - 101 x^{3} - 86 x^{2} + 49 x,
\]
