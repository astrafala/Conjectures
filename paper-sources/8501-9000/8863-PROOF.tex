...
\title{The empirical recurrence for OEIS A235887, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235887 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
G.f.: 8*x*(14 - 69*x + 64*x\^{}2) / ((1 - x)*(1 - 2*x)*(1 - 4*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 19 2018 13:23:52, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 512 x^{3} + 552 x^{2} - 112 x}{8 x^{3} - 14 x^{2} + 7 x - 1},
\]
