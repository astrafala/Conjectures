...
\title{The empirical recurrence for OEIS A222098, derived from the entry's generating function}
...
\begin{abstract}
OEIS A222098 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A222098 is ``Number of n X 2 0..5 arrays with entries increasing mod 6 by 0, 1, 2 or 3 rightwards and downwards, starting with upper left zero.''. It has offset $1$ and begins
\[
4, 46, 530, 6108, 70394, 811286, 9350020, 107758398,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 15 2018 12:37:53, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{2} + 4 x}{17 x^{2} - 13 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;17 x^{2} - 13 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222098.
