...
\title{A proof of the conjectured recurrence for OEIS A213801}
...
\begin{abstract}
OEIS A213801 carries a conjectured linear recurrence with polynomial coefficients, of
order 8. The entry posts no generating function, and the closed form it does post
...
OEIS A213801 is ``Number of 3 X 3 0..n symmetric arrays with all rows summing to floor(n*3/2)''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;4, 13, 29, 57, 96, 153, 226, 323,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = (14*n\textasciicircum{}3+42*n\textasciicircum{}2+53*n+25+3*(n+1)*(-1)\textasciicircum{}n+2*((-1)\textasciicircum{}((2*n+1-(-1)\textasciicircum{}n)/4)-(-1)\textasciicircum{}((6*n+5-(-1)\textasciicircum{}n)/4)))/32. - \emph{Luce ETIENNE}, Aug 23 2014
\end{quote}
that is,
\begin{equation}\label{eq:cf}
a(n)\;=\;\frac{\left(-1\right)^{n} \left(3 n + 3\right)}{32} + \frac{\left(-1\right)^{- \frac{\left(-1\right)^{n}}{4} + \frac{n}{2} + \frac{1}{4}}}{16} - \frac{\left(-1\right)^{- \frac{\left(-1\right)^{n}}{4} + \frac{3 n}{2} + \frac{5}{4}}}{16} + \frac{7 n^{3}}{16} + \frac{21 n^{2}}{16} + \frac{53 n}{32} + \frac{25}{32} .
\end{equation}
...
\end{quote}
As of the ``Last modified'' line on the live entry (2024-10-27, revision 40) the
...
1\,a(n) - 2\,a(n-1) + 2\,a(n-3) - 2\,a(n-4) + 2\,a(n-5) - 2\,a(n-7) + 1\,a(n-8)
\end{equation}
...
a(2m)\;=\;e_{0}(m)\;=\;\frac{\left(-1\right)^{m}}{8} + \frac{7 m^{3}}{2} + \frac{21 m^{2}}{4} + \frac{7 m}{2} + \frac{7}{8},
\qquad
a(2m+1)\;=\;e_{1}(m)\;=\;\frac{7 m^{3}}{2} + \frac{21 m^{2}}{2} + 11 m + 4,
\end{equation}
...
\section{The computation for A213801}
...
does every similarity class of $T(2m+1)$. The even half splits into 0 classes
and the odd half into 0 classes.
...
exact integer arithmetic at every index where it applies: $35$ instances beginning at
$n=9$, all of them zero. This is not part of the proof, which is symbolic, but it
...
entry \href{https://oeis.org/A213801}{A213801}, 2024-10-27.
