...
\title{The empirical recurrence for OEIS A183690, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183690 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A183690 is ``Number of (n+1) X 2 0..2 arrays with every 2 X 2 subblock nonsingular.''. It has offset $1$ and begins
\[
50, 314, 1970, 12362, 77570, 486746, 3054290, 19165418,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 04 2018 10:48:15, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 64 x^{2} - 50 x}{8 x^{2} + 5 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 8 x^{2} - 5 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183690.
