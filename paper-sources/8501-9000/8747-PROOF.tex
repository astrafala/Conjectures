...
\title{The empirical recurrence for OEIS A266007, proved by transfer matrix}
...
\begin{abstract}
OEIS A266007 counts arrays of width $3$ over $\{0,\dots,4\}$ under a condition on each cell
...
a walk count on $S=15750$ of them, and the conjectured recurrence is then settled by a finite
exact computation.
\end{abstract}
...
OEIS A266007 is ``Number of n X 3 integer arrays with each element equal to the number of horizontal and antidiagonal neighbors equal to itself.''. It has offset $1$ and begins
\[
2, 2, 6, 14, 22, 46, 102, 190,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + 4*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 09 2019 09:12:40, revision 7) this is still
...
(-1,1), (0,-1), (0,1), (1,-1),
\]
those falling outside the array being absent. The entry requires that every cell equals the NUMBER of its neighbours that equal it --- so its value is bounded by the size of the neighbour set, which is what makes an ``integer array'' a finite alphabet here.
...
terms. There are $S=15750$ states, and $a$ is $C$-finite.
...
first term. There are $5^{3}=125$ possible rows over the entry's own alphabet. The state count $S=15750$ is exactly $125\cdot126$,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266007.
