...
\title{The empirical recurrence for OEIS A221619, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221619 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A221619 is ``Number of n X 4 arrays with each row a permutation of 1..4 having at least as many downsteps as the preceding row.''. It has offset $1$ and begins
\[
24, 410, 6120, 85035, 1130256, 14576404, 183919920, 2282493365,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 09 2018 22:13:04, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 121 x^{4} + 264 x^{3} - 166 x^{2} + 24 x}{121 x^{4} - 264 x^{3} + 166 x^{2} - 24 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;121 x^{4} - 264 x^{3} + 166 x^{2} - 24 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;24\,a(n-1) - 166\,a(n-2) + 264\,a(n-3) - 121\,a(n-4)
\]
for every $n>4$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 121 x^{4} + 264 x^{3} - 166 x^{2} + 24 x,
\]
...
\[
121 x^{4} - 264 x^{3} + 166 x^{2} - 24 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221619.
