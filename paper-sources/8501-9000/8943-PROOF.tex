...
\title{The empirical recurrence for OEIS A184041, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184041 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A184041 is ``1/9 the number of (n+1) X 3 0..2 arrays with all 2 X 2 subblocks having the same four values.''. It has offset $1$ and begins
\[
15, 21, 31, 51, 87, 159, 295, 567,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 08:32:19, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 48 x^{4} + 32 x^{3} + 24 x^{2} - 15 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;48 x^{4} - 32 x^{3} - 24 x^{2} + 15 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184041.
