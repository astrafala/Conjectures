...
\title{A proof of the conjectured recurrence for OEIS A176967}
...
\begin{abstract}
OEIS A176967 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 8 x^{4} + 24 x^{3} - 24 x^{2} + 5 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A176967 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=5, k=-1 and l=1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 5, 9, 41, 169, 825, 4073, 21113,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 16 x^{4} + 32 x^{3} - 7 x^{2} - 6 x + 1}{x^{2} - 2 x + 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(n+1)*a(n) +(-7*n+2)*a(n-1) +(-n+11)*a(n-2) +3*(13*n-42)*a(n-3) +48*(-n+4)*a(n-4) +16*(n-5)*a(n-5)=0. - \emph{R. J. Mathar}, Mar 02 2016
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=2 - 7 n,\qquad p_{2}(n)=11 - n,\qquad p_{3}(n)=3 \left(13 n - 42\right),\qquad p_{4}(n)=- 48 \left(n - 4\right),\qquad p_{5}(n)=16 \left(n - 5\right)
\]
...
\section{The computation for A176967}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \frac{\left(4 x^{2} - 7 x + 1\right) \left(4 x^{2} - x - 1\right)}{\left(x - 1\right)^{2}} .
\]
...
\[
(n + 1)\,a(n) + (2 - 7 n)\,a(n-1) + (11 - n)\,a(n-2) + 3 \left(13 n - 42\right)\,a(n-3) -  48 \left(n - 4\right)\,a(n-4) + 16 \left(n - 5\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A176967, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 8 x^{4} + 24 x^{3} - 24 x^{2} + 5 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176967.
