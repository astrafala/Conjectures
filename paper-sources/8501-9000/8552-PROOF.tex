...
\title{A local condition on an array: OEIS A183355}
...
\begin{abstract}
OEIS A183355 carries an empirical recurrence of order $1$ contributed by R.~H.~Hardin. It is
...
walks in it. Hence $a(n)$ is a walk count on $S=3$ vertices and is $C$-finite, and whether the
...
OEIS A183355 is ``One quarter the number of nX3 1..4 arrays with no two neighbors of any element equal to each other.''. It has offset $1$ and begins
\[
12, 144, 432, 1296, 3888, 11664, 34992, 104976,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n)=3*a(n-1) for n>2
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 01:36:18, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $3$ columns over the alphabet $\{1,2,3,4\}$, growing one row at a time, and asks that the neighbours of any one element --- those directly above, below, left and right of it --- must all be different from each other.
...
Merging vertices with identical futures leaves $S=3$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
...
until the working vector was identically zero, which settles every larger $j$ at once. The bound is exact: at $n=2$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183355.
