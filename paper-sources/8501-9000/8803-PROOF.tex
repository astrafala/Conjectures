...
\title{The empirical recurrence for OEIS A211719, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211719 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A211719 is ``Number of (n+1) X (n+1) -11..11 symmetric matrices with every 2 X 2 subblock having sum zero.''. It has offset $1$ and begins
\[
265, 3059, 35377, 409883, 4757545, 55318979, 644340577, 7517728043,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 19 2018 15:28:50, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3036 x^{2} + 265 x}{132 x^{2} - 23 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;132 x^{2} - 23 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211719.
