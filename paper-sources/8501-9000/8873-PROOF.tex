...
\title{A proof of the conjectured recurrence for OEIS A212205}
...
\begin{abstract}
OEIS A212205 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 6 x^{6} + 12 x^{5} + 15 x^{4} - 18 x^{3} - 10 x^{2} + 2 x + 1$, a polynomial of degree $6$, which proves
...
OEIS A212205 is ``G.f.: ((1+2*x)*sqrt(1-6*x\^{}2+x\^{}4)-1+5*x\^{}2-2*x\^{}3)/(2*x*(1-6*x\^{}2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 4, 8, 18, 36, 86,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x^{3} - 5 x^{2} - \left(2 x + 1\right) \sqrt{x^{4} - 6 x^{2} + 1} + 1}{2 x \left(6 x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: (n+1)*a(n) +2*(n-1)*a(n-1) +6*(-2*n+1)*a(n-2) +12*(-2*n+5)*a(n-3) +(37*n-77)*a(n-4) +2*(37*n-151)*a(n-5) +6*(-n+5)*a(n-6) +12*(-n+7)*a(n-7)=0. - \emph{R. J. Mathar}, Aug 20 2018
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=2 \left(n - 1\right),\qquad p_{2}(n)=- 6 \left(2 n - 1\right),\qquad p_{3}(n)=- 12 \left(2 n - 5\right),\qquad p_{4}(n)=37 n - 77,\qquad p_{5}(n)=2 \left(37 n - 151\right),\qquad p_{6}(n)=- 6 \left(n - 5\right),\qquad p_{7}(n)=- 12 \left(n - 7\right)
\]
...
\section{The computation for A212205}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x^{2} - 2 x - 1\right) \left(x^{2} + 2 x - 1\right) .
\]
...
\[
B(x)\;=\;- 6 x^{6} + 12 x^{5} + 15 x^{4} - 18 x^{3} - 10 x^{2} + 2 x + 1 ,
\]
...
\[
(n + 1)\,a(n) + 2 \left(n - 1\right)\,a(n-1) -  6 \left(2 n - 1\right)\,a(n-2) -  12 \left(2 n - 5\right)\,a(n-3) + (37 n - 77)\,a(n-4) + 2 \left(37 n - 151\right)\,a(n-5) -  6 \left(n - 5\right)\,a(n-6) -  12 \left(n - 7\right)\,a(n-7) \;=\;0
\]
holds for every $n>6$.
\end{theorem}
...
section of A212205, agreeing on all 13 terms compared.
...
vanish, for all 32 values of $n$ from $n=7$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 6 x^{6} + 12 x^{5} + 15 x^{4} - 18 x^{3} - 10 x^{2} + 2 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A212205.
