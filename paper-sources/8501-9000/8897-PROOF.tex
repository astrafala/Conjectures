...
\title{A proof of the conjectured recurrence for OEIS A139376}
...
\begin{abstract}
OEIS A139376 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{2} - 6 x + 1$, a polynomial of degree $2$, which proves
...
OEIS A139376 is ``Expansion of 1/((1-x\^{}2*c(x))(1-x-x\^{}2)) where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 5, 11, 23, 54, 136,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{\left(- x \left(\sqrt{1 - 4 x} - 1\right) - 2\right) \left(x^{2} + x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+1)*a(n) +6*(n-2)*a(n-1) +4*(-2*n+5)*a(n-2) +2*(-n+1)*a(n-3) +3*(3*n-7)*a(n-4) +3*(-n+3)*a(n-5) +2*(-2*n+5)*a(n-6)=0. - \emph{R. J. Mathar}, Feb 05 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2016-10-20, revision 7) the
...
\[
p_{0}(n)=1 - n,\qquad p_{1}(n)=6 \left(n - 2\right),\qquad p_{2}(n)=- 4 \left(2 n - 5\right),\qquad p_{3}(n)=- 2 \left(n - 1\right),\qquad p_{4}(n)=3 \left(3 n - 7\right),\qquad p_{5}(n)=- 3 \left(n - 3\right),\qquad p_{6}(n)=- 2 \left(2 n - 5\right)
\]
...
\section{The computation for A139376}
...
\[
(1 - n)\,a(n) + 6 \left(n - 2\right)\,a(n-1) -  4 \left(2 n - 5\right)\,a(n-2) -  2 \left(n - 1\right)\,a(n-3) + 3 \left(3 n - 7\right)\,a(n-4) -  3 \left(n - 3\right)\,a(n-5) -  2 \left(2 n - 5\right)\,a(n-6) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A139376, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=x^{2} - 6 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A139376.
