...
closed form. Here it equals $11 x^{2} - 6 x + 1$, a polynomial of degree $2$, which proves
...
\begin{quote}\small
\textbf{Conjecture:} -(n-1)*(n+1)*a(n) +2*(5*n\^{}2-9*n+1)*a(n-1) +2*(-15*n\^{}2+58*n-49)*a(n-2) +2*(10*n\^{}2-76*n+123)*a(n-3) +(31*n-55)*(n-3)*a(n-4) -30*(n-3)*(n-4)*a(n-5)=0. - \emph{R. J. Mathar}, Jun 08 2016
\end{quote}
...
\[
p_{0}(n)=- \left(n - 1\right) \left(n + 1\right),\qquad p_{1}(n)=2 \left(5 n^{2} - 9 n + 1\right),\qquad p_{2}(n)=- 2 \left(15 n^{2} - 58 n + 49\right),\qquad p_{3}(n)=2 \left(10 n^{2} - 76 n + 123\right),\qquad p_{4}(n)=\left(n - 3\right) \left(31 n - 55\right),\qquad p_{5}(n)=- 30 \left(n - 4\right) \left(n - 3\right)
\]
...
\[
D_{1} \;=\; \frac{5 x - 1}{x - 1},\quad D_{2} \;=\; - \frac{3 x - 1}{x + 1} .
\]
...
\[
- \left(n - 1\right) \left(n + 1\right)\,a(n) + 2 \left(5 n^{2} - 9 n + 1\right)\,a(n-1) -  2 \left(15 n^{2} - 58 n + 49\right)\,a(n-2) + 2 \left(10 n^{2} - 76 n + 123\right)\,a(n-3) + \left(n - 3\right) \left(31 n - 55\right)\,a(n-4) -  30 \left(n - 4\right) \left(n - 3\right)\,a(n-5) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
vanish, for all 19 values of $n$ from $n=5$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=11 x^{2} - 6 x + 1$ is an
