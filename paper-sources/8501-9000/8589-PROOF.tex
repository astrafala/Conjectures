...
\title{The empirical recurrence for OEIS A235888, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235888 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
G.f.: 8*x*(44 - 237*x + 256*x\^{}2) / ((1 - x)*(1 - 2*x)*(1 - 4*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 19 2018 13:24:10, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2048 x^{3} + 1896 x^{2} - 352 x}{8 x^{3} - 14 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2048 x^{3} - 1896 x^{2} + 352 x,
\]
