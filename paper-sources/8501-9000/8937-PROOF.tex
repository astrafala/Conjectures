...
\title{A proof of the conjectured recurrence for OEIS A110198}
...
\begin{abstract}
OEIS A110198 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A110198 is ``Antidiagonal sums of number triangle A110197''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 4, 9, 20, 46, 109, 262,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{1}{\sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(-3*n+1)*a(n-1) +n*a(n-2) +(-n+2)*a(n-3) +(3*n-5)*a(n-4) +(-n+2)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 15 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-03-21, revision 22) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=1 - 3 n,\qquad p_{2}(n)=n,\qquad p_{3}(n)=2 - n,\qquad p_{4}(n)=3 n - 5,\qquad p_{5}(n)=2 - n
\]
...
\section{The computation for A110198}
...
\[
n\,a(n) + (1 - 3 n)\,a(n-1) + n\,a(n-2) + (2 - n)\,a(n-3) + (3 n - 5)\,a(n-4) + (2 - n)\,a(n-5) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A110198, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A110198.
