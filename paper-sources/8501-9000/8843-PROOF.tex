...
\title{The empirical recurrence for OEIS A232956, derived from the entry's generating function}
...
\begin{abstract}
OEIS A232956 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A232956 is ``Number of 2 X n 0..3 arrays with no element x(i,j) adjacent to value 3-x(i,j) horizontally, diagonally or antidiagonally, and top left element zero.''. It has offset $1$ and begins
\[
4, 21, 129, 771, 4629, 27771, 166629, 999771,\ \dots
\]
...
G.f.: x*(4 + x) / ((1 + x)*(1 - 6*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 06 2018 11:37:57, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{2} - 4 x}{6 x^{2} + 5 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232956.
