...
\title{A proof of the conjectured recurrence for OEIS A000781}
...
\begin{abstract}
OEIS A000781 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $5 x^{4} - 10 x^{3} + 5 x^{2} + 6 x$, a polynomial of degree $4$, which proves
...
OEIS A000781 is ``a(n) = 3*Catalan(n) - Catalan(n-1) - 1''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 4, 12, 36, 111, 353, 1154, 3860,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{x^{2} + \frac{\left(x - 1\right) \left(x \left(\sqrt{1 - 4 x} - 7\right) - 3 \sqrt{1 - 4 x} + 3\right)}{2}}{x \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 3*(n+1)*a(n) + (-19*n+7)*a(n-1) + (33*n-53)*a(n-2) + (-21*n+61)*a(n-3) + 2*(2*n-9)*a(n-4) = 0. - \emph{R. J. Mathar}, Aug 20 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 27) the
...
\[
p_{0}(n)=3 \left(n + 1\right),\qquad p_{1}(n)=7 - 19 n,\qquad p_{2}(n)=33 n - 53,\qquad p_{3}(n)=61 - 21 n,\qquad p_{4}(n)=2 \left(2 n - 9\right)
\]
...
\section{The computation for A000781}
...
\[
3 \left(n + 1\right)\,a(n) + (7 - 19 n)\,a(n-1) + (33 n - 53)\,a(n-2) + (61 - 21 n)\,a(n-3) + 2 \left(2 n - 9\right)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A000781, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=5 x^{4} - 10 x^{3} + 5 x^{2} + 6 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A000781.
