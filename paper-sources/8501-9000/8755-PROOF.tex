...
\title{The empirical recurrence for OEIS A214160, derived from the entry's generating function}
...
\begin{abstract}
OEIS A214160 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A214160 is ``Number of 0..5 colorings of an n X 3 array circular in the 3 direction with new values 0..5 introduced in row major order.''. It has offset $1$ and begins
\[
1, 18, 902, 60320, 4242606, 300785428, 21350933122, 1515863103360,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 22 2018 06:42:16, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 335 x^{3} + 66 x^{2} - x}{1562 x^{3} - 945 x^{2} + 84 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 1562 x^{3} + 945 x^{2} - 84 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;84\,a(n-1) - 945\,a(n-2) + 1562\,a(n-3)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A214160.
