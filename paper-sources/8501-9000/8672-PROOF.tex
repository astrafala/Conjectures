...
\title{The empirical recurrence for OEIS A220749, derived from the entry's generating function}
...
\begin{abstract}
OEIS A220749 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A220749 is ``Equals one maps: number of n X 4 binary arrays indicating the locations of corresponding elements equal to exactly one of their horizontal and vertical neighbors in a random 0..3 n X 4 array.''. It has offset $1$ and begins
\[
8, 176, 3778, 65084, 1048014, 16776688, 268434892, 4294966696,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 03 2018 08:11:52, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 1120 x^{8} + 2374 x^{7} + 240 x^{6} + 1640 x^{5} - 2760 x^{4} - 874 x^{3} - 32 x^{2} - 8 x}{16 x^{3} - 33 x^{2} + 18 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 16 x^{3} + 33 x^{2} - 18 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;18\,a(n-1) - 33\,a(n-2) + 16\,a(n-3)
\]
for every $n>8$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;1120 x^{8} - 2374 x^{7} - 240 x^{6} - 1640 x^{5} + 2760 x^{4} + 874 x^{3} + 32 x^{2} + 8 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220749.
