...
\title{A proof of the conjectured recurrence for OEIS A026743}
...
\begin{abstract}
OEIS A026743 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $12 x^{7} + 6 x^{6} - 24 x^{5} - 12 x^{4} + 4 x^{3} + 2 x^{2}$, a polynomial of degree $7$, which proves
...
OEIS A026743 is ``a(n) = Sum\_\{j=0..n\} T(n,j), T given by A026736''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 4, 8, 17, 34, 73, 146,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{\frac{- 2 x - 1}{2 x - 1}} \left(3 x^{2} - 1\right) - \left(2 x + 1\right) \left(x^{2} + 1\right)}{2 \left(x^{4} + 4 x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence n*a(n) -2*a(n-1) +(-11*n+20)*a(n-2) +14*a(n-3) +(39*n-152)*a(n-4) -22*a(n-5) +(-41*n+268)*a(n-6) -6*a(n-7) +12*(-n+6)*a(n-8)=0. - \emph{R. J. Mathar}, Jan 13 2023
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 23) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=-2,\qquad p_{2}(n)=20 - 11 n,\qquad p_{3}(n)=14,\qquad p_{4}(n)=39 n - 152,\qquad p_{5}(n)=-22,\qquad p_{6}(n)=268 - 41 n,\qquad p_{7}(n)=-6,\qquad p_{8}(n)=- 12 \left(n - 6\right)
\]
...
\section{The computation for A026743}
...
\[
B(x)\;=\;12 x^{7} + 6 x^{6} - 24 x^{5} - 12 x^{4} + 4 x^{3} + 2 x^{2} ,
\]
...
\[
n\,a(n) - 2\,a(n-1) + (20 - 11 n)\,a(n-2) + 14\,a(n-3) + (39 n - 152)\,a(n-4) - 22\,a(n-5) + (268 - 41 n)\,a(n-6) - 6\,a(n-7) -  12 \left(n - 6\right)\,a(n-8) \;=\;0
\]
holds for every $n>7$.
\end{theorem}
...
section of A026743, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=12 x^{7} + 6 x^{6} - 24 x^{5} - 12 x^{4} + 4 x^{3} + 2 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A026743.
