...
\title{The empirical recurrence for OEIS A233167, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233167 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 15 2024 01:58:03, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{32636928 x^{3} - 446464 x^{2} + 1248 x}{32768 x^{2} - 384 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;32768 x^{2} - 384 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;32636928 x^{3} - 446464 x^{2} + 1248 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233167.
