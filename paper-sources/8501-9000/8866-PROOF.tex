...
\title{A proof of the conjectured identity for OEIS A298022}
...
\begin{abstract}
OEIS A298022 carries a conjectured identity expressing its terms through the terms of
A049347. We prove it. Each entry involved posts a generating function, so the claim is
a single identity between generating functions: writing $F_{a}$ for the generating
function of A298022 and $F_{j}$ for those of the entries on the right, the conjecture
...
closed form and equals $- 2 x^{4} - 2 x^{3} - 2 x^{2} - 2 x + 1$, of degree $4$, which proves the identity for
every $n>4$.
\end{abstract}
...
OEIS A298022 is ``Coordination sequence for Dual(3\^{}3.4\^{}2) tiling with respect to a trivalent node''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 7, 12, 17, 23, 28, 33,\ \dots
\]
Its generating function, as posted on the entry, is
\begin{equation}\label{eq:gf}
F_{a}(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x^{8} + 2 x^{5} + 3 x^{4} + 4 x^{3} + 4 x^{2} + 2 x + 1}{x^{4} - x^{3} - x + 1} ,
\end{equation}
and the entry carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = 2*(21*n + 3*A049347(n+2)/2)/9 for n > 4. - \emph{Stefano Spezia}, Nov 24 2024
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 48) the
...
\section{The computation for A298022}

Assembling the right-hand side by Lemmas~\ref{lem:shift} and~\ref{lem:theta} gives
\[
R(x)\;=\;\frac{x \left(5 x^{2} + 4 x + 5\right)}{x^{4} - x^{3} - x + 1} ,
\]
and subtracting \eqref{eq:gf} the rational parts cancel, leaving
\[
F_{a}(x)-R(x)\;=\;- 2 x^{4} - 2 x^{3} - 2 x^{2} - 2 x + 1 ,
\]
a polynomial of degree $4$.

\begin{theorem}
The conjectured identity holds for every $n>4$.
\end{theorem}
...
both sides were formed from the entries' own values and found to agree, for all 52
values of $n$ from $n=5$ upward. This is what Corollary~\ref{cor:criterion}
...
functions and the theorem holds for all $n>4$ simultaneously.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A298022.
