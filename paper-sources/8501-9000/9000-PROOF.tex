...
\title{A proof of the conjectured recurrence for OEIS A089324}
...
\begin{abstract}
OEIS A089324 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{4} + 4 x^{3} - 4 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A089324 is ``Number of lattice paths from (0,0) to the line x+y=n that use the step set \{(0,1),(1,0),(2,0),(3,0),...\} and never pass below y=x''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 3, 7, 12, 29, 53,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{\left(x - 1\right)^{2} + \sqrt{x^{4} - 6 x^{2} + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +3*(-n-1)*a(n-1) +(-5*n+13)*a(n-2) +18*(n-2)*a(n-3) +(-5*n+7)*a(n-4) +3*(-n+5)*a(n-5) +(n-5)*a(n-6)=0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2014-02-10, revision 15) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 3 \left(n + 1\right),\qquad p_{2}(n)=13 - 5 n,\qquad p_{3}(n)=18 \left(n - 2\right),\qquad p_{4}(n)=7 - 5 n,\qquad p_{5}(n)=- 3 \left(n - 5\right),\qquad p_{6}(n)=n - 5
\]
...
\section{The computation for A089324}
...
\[
(n + 1)\,a(n) -  3 \left(n + 1\right)\,a(n-1) + (13 - 5 n)\,a(n-2) + 18 \left(n - 2\right)\,a(n-3) + (7 - 5 n)\,a(n-4) -  3 \left(n - 5\right)\,a(n-5) + (n - 5)\,a(n-6) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A089324, agreeing on all 13 terms compared.
...
vanish, for all 26 values of $n$ from $n=6$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{4} + 4 x^{3} - 4 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A089324.
