...
\title{A proof of the conjectured recurrence for OEIS A143955}
...
\begin{abstract}
OEIS A143955 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $5 x^{3}$, a polynomial of degree $3$, which proves
...
OEIS A143955 is ``Sum of the altitudes of the leftmost valleys of all Dyck paths of semilength n (if path has no valley, then this altitude is taken to be 0)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 0, 1, 6, 26, 101, 376,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(\sqrt{1 - 4 x} - 1\right)^{5}}{32 x^{2} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) -4*(2*n+1)*a(n-1) +2*(10*n-9)*a(n-2) +17*(2-n)*a(n-3) +2*(2*n-7)*a(n-4)=0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-25, revision 26) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 4 \left(2 n + 1\right),\qquad p_{2}(n)=2 \left(10 n - 9\right),\qquad p_{3}(n)=- 17 \left(n - 2\right),\qquad p_{4}(n)=2 \left(2 n - 7\right)
\]
...
\section{The computation for A143955}
...
\[
(n + 2)\,a(n) -  4 \left(2 n + 1\right)\,a(n-1) + 2 \left(10 n - 9\right)\,a(n-2) -  17 \left(n - 2\right)\,a(n-3) + 2 \left(2 n - 7\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A143955, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=5 x^{3}$ is an
...
\url{https://oeis.org}, 2026. Sequence A143955.
