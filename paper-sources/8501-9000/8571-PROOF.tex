...
\title{The empirical recurrence for OEIS A223363, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223363 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 20 2018 05:31:03, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 36 x^{3} - 36 x^{2} + 21 x}{8 x^{3} + 4 x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;8 x^{3} + 4 x^{2} - 6 x + 1
\]
