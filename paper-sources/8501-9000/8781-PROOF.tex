...
\title{The empirical recurrence for OEIS A165372, derived from the entry's generating function}
...
\begin{abstract}
OEIS A165372 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A165372 is ``Number of slanted n X 3 (i=1..n) X (j=i..3+i-1) 1..4 arrays with all 1s connected, all 2s connected, all 3s connected, all 4s connected, 1 in the upper left corner, 2 in the upper right corner, 3 in the lower left corner, and 4 in the lower right corner.''. It has offset $2$ and begins
\[
9, 40, 104, 168, 232, 296, 360, 424,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 25 2018 11:41:49, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{33 x^{4} + 22 x^{3} + 9 x^{2}}{x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A165372.
