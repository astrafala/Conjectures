...
\title{The empirical recurrence for OEIS A222001, derived from the entry's generating function}
...
\begin{abstract}
OEIS A222001 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A222001 is ``Number of n X 3 arrays with each row a permutation of 1..3 having at least as many downsteps as the preceding row, with rows in lexicographically nonincreasing order.''. It has offset $1$ and begins
\[
6, 12, 22, 37, 58, 86, 122, 167,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 14 2018 08:58:06, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{4} + 10 x^{3} - 12 x^{2} + 6 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 3 x^{4} + 10 x^{3} - 12 x^{2} + 6 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222001.
