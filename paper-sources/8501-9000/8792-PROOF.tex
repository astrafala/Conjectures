...
\title{The empirical recurrence for OEIS A269285, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269285 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A269285 is ``Number of n X 4 0..3 arrays with some element plus some horizontally or antidiagonally adjacent neighbor totalling three no more than once.''. It has offset $1$ and begins
\[
216, 19188, 1363572, 87559668, 5306911092, 309846524148, 17623065834612, 983118947312628,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 21 2019 06:27:57, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{1764 x^{3} - 1980 x^{2} + 216 x}{2401 x^{2} - 98 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2401 x^{2} - 98 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;1764 x^{3} - 1980 x^{2} + 216 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269285.
