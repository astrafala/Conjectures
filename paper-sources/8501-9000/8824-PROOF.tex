...
\title{The empirical recurrence for OEIS A223277, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223277 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A223277 is ``Rolling icosahedron face footprints: number of n X 3 0..19 arrays starting with 0 where 0..19 label faces of an icosahedron and every array movement to a horizontal, diagonal or antidiagonal neighbor moves across an icosahedral edge.''. It has offset $1$ and begins
\[
9, 87, 849, 8295, 81057, 792087, 7740273, 75637959,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 18 2018 08:42:44, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 12 x^{2} + 9 x}{12 x^{2} - 11 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;12 x^{2} - 11 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223277.
