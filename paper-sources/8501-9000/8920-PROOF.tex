...
\title{A proof of the conjectured recurrence for OEIS A159771}
...
\begin{abstract}
OEIS A159771 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{4} - 3 x^{2} + 2 x$, a polynomial of degree $4$, which proves
...
OEIS A159771 is ``Number of n-leaf binary trees that do not contain (()((()())((()())()))) as a subtree''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 2, 5, 14, 41, 124, 385,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{3 x^{2} - 2 x - \sqrt{\left(x^{2} + 1\right) \left(x^{2} - 4 x + 1\right)} + 1}{4 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-2*n+1)*a(n-1) +2*(n-2)*a(n-2) +2*(-2*n+7)*a(n-3) +(n-5)*a(n-4)=0. - \emph{R. J. Mathar}, Jul 17 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-02, revision 55) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(2 n - 1\right),\qquad p_{2}(n)=2 \left(n - 2\right),\qquad p_{3}(n)=- 2 \left(2 n - 7\right),\qquad p_{4}(n)=n - 5
\]
...
\section{The computation for A159771}
...
\[
(n + 1)\,a(n) -  2 \left(2 n - 1\right)\,a(n-1) + 2 \left(n - 2\right)\,a(n-2) -  2 \left(2 n - 7\right)\,a(n-3) + (n - 5)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A159771, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{4} - 3 x^{2} + 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A159771.
