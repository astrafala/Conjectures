...
\title{The empirical recurrence for OEIS A190093, derived from the entry's generating function}
...
\begin{abstract}
OEIS A190093 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A190093 is ``Number of rhombuses on an (n+1) X 5 grid.''. It has offset $1$ and begins
\[
4, 15, 36, 66, 96, 130, 164, 204,\ \dots
\]
...
\begin{quote}
G.f.: x*(4 + 7*x + 6*x\^{}2 + 2*x\^{}3 - 10*x\^{}4 - 5*x\^{}5 + 2*x\^{}7 + 4*x\^{}8 - 2*x\^{}9 - 4*x\^{}10) / ((1 - x)\^{}3*(1 + x)).
\end{quote}
As of the ``Last modified'' line on the live entry (May 04 2018 12:04:35, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{11} + 2 x^{10} - 4 x^{9} - 2 x^{8} + 5 x^{6} + 10 x^{5} - 2 x^{4} - 6 x^{3} - 7 x^{2} - 4 x}{x^{4} - 2 x^{3} + 2 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 4 x^{11} - 2 x^{10} + 4 x^{9} + 2 x^{8} - 5 x^{6} - 10 x^{5} + 2 x^{4} + 6 x^{3} + 7 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A190093.
