...
\title{The empirical recurrence for OEIS A224669, derived from the entry's generating function}
...
\begin{abstract}
OEIS A224669 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A224669 is ``Number of (n+1) X 2 0..2 matrices with each 2 X 2 subblock idempotent.''. It has offset $1$ and begins
\[
12, 25, 41, 70, 115, 189, 308, 501,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 17 2018 05:32:16, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 9 x^{3} + x^{2} + 12 x}{x^{3} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224669.
