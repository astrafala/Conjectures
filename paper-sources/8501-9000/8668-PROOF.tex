...
\title{The empirical recurrence for OEIS A235886, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235886 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A235886 is ``Number of (n+1) X (1+1) 0..3 arrays with the minimum plus the upper median equal to the lower median plus the maximum in every 2 X 2 subblock.''. It has offset $1$ and begins
\[
40, 112, 352, 1216, 4480, 17152, 67072, 265216,\ \dots
\]
...
\begin{quote}
G.f.: 8*x*(5 - 16*x) / ((1 - 2*x)*(1 - 4*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 19 2018 12:01:09, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 128 x^{2} + 40 x}{8 x^{2} - 6 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235886.
