...
\title{The empirical recurrence for OEIS A250461, derived from the entry's generating function}
...
\begin{abstract}
OEIS A250461 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A250461 is ``Number of (n+1)X(1+1) 0..1 arrays with nondecreasing min(x(i,j),x(i,j-1)) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
11, 31, 85, 233, 637, 1741, 4757, 12997,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 14 2018 12:06:37, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{3} - 2 x^{2} + 11 x}{2 x^{3} - 3 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250461.
