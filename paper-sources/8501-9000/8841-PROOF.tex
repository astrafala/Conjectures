...
\title{A proof of the conjectured recurrence for OEIS A173993}
...
\begin{abstract}
OEIS A173993 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $10 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A173993 is ``Sequence whose Hankel transform is the Somos (4) sequence''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 6, 17, 50, 146, 430, 1267,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x^{2} + 4 x + \sqrt{- 8 x^{4} + 12 x^{3} - 4 x + 1} - 1}{2 x \left(3 x^{2} - 4 x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-4*n-1)*a(n-1) +(19*n-5)*a(n-2) -36*a(n-3) +8*(-7*n+26)*a(n-4) +2*(34*n-143)*a(n-5) +24*(-n+5)*a(n-6)=0. - \emph{R. J. Mathar}, Oct 10 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2024-08-18, revision 21) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(4 n + 1\right),\qquad p_{2}(n)=19 n - 5,\qquad p_{3}(n)=-36,\qquad p_{4}(n)=- 8 \left(7 n - 26\right),\qquad p_{5}(n)=2 \left(34 n - 143\right),\qquad p_{6}(n)=- 24 \left(n - 5\right)
\]
...
\section{The computation for A173993}
...
\[
B(x)\;=\;10 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1 ,
\]
...
\[
(n + 1)\,a(n) -  2 \left(4 n + 1\right)\,a(n-1) + (19 n - 5)\,a(n-2) - 36\,a(n-3) -  8 \left(7 n - 26\right)\,a(n-4) + 2 \left(34 n - 143\right)\,a(n-5) -  24 \left(n - 5\right)\,a(n-6) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A173993, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=10 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A173993.
