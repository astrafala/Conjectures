...
\title{A proof of the conjectured recurrence for OEIS A185010}
...
\begin{abstract}
OEIS A185010 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A185010 is ``a(n) = A000108(n)*A015518(n+1), where A000108 are the Catalan numbers and A015518(n) = 2*A015518(n-1) + 3*A015518(n-2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 14, 100, 854, 7644, 72204, 703560,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{- 8 x - 2 \sqrt{- 48 x^{2} - 8 x + 1} + 2}}{8 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n+1)*a(n) -4*n*(2*n-1)*a(n-1) -12*(2*n-1)*(2*n-3)*a(n-2)=0. - \emph{R. J. Mathar}, Oct 08 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-07-27, revision 34) the
...
\[
p_{0}(n)=n \left(n + 1\right),\qquad p_{1}(n)=- 4 n \left(2 n - 1\right),\qquad p_{2}(n)=- 12 \left(2 n - 3\right) \left(2 n - 1\right)
\]
...
\[
n \left(n + 1\right)\,a(n) -  4 n \left(2 n - 1\right)\,a(n-1) -  12 \left(2 n - 3\right) \left(2 n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A185010, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A185010.
