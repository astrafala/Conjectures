...
\title{A proof of the conjectured recurrence for OEIS A133306}
...
\begin{abstract}
OEIS A133306 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A133306 is ``a(n) = (1/n)*Sum\_\{i=0..n-1\} C(n,i)*C(n,i+1)*5\^{}i*6\^{}(n-i), a(0)=1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 6, 66, 906, 13926, 229326, 3956106, 70572066,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x - \sqrt{x^{2} - 22 x + 1} + 1}{10 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 11*(-2*n+1)*a(n-1) + (n-2)*a(n-2) = 0. - \emph{R. J. Mathar}, May 23 2014
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 11 \left(2 n - 1\right),\qquad p_{2}(n)=n - 2
\]
...
\section{The computation for A133306}
...
\[
(n + 1)\,a(n) -  11 \left(2 n - 1\right)\,a(n-1) + (n - 2)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A133306, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A133306.
