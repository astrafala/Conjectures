...
\title{The empirical recurrence for OEIS A235889, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235889 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A235889 is ``Number of (n+1) X (4+1) 0..3 arrays with the minimum plus the upper median equal to the lower median plus the maximum in every 2 X 2 subblock.''. It has offset $1$ and begins
\[
1216, 1624, 2536, 4744, 10696, 28744, 89416, 309064,\ \dots
\]
...
\begin{quote}
G.f.: 8*x*(152 - 861*x + 1024*x\^{}2) / ((1 - x)*(1 - 2*x)*(1 - 4*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 20 2018 03:58:29, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8192 x^{3} + 6888 x^{2} - 1216 x}{8 x^{3} - 14 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;8192 x^{3} - 6888 x^{2} + 1216 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235889.
