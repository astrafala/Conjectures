...
closed form. Here it equals $x^{5} - x^{4} + 3 x^{3}$, a polynomial of degree $5$, which proves
...
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(-4*n+3)*a(n-1) +3*(n-1)*a(n-2) +(n-9)*a(n-3) +(3*n-5)*a(n-4) +(-3*n+7)*a(n-5) +(-2*n+13)*a(n-6) +(n-6)*a(n-7)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=3 - 4 n,\qquad p_{2}(n)=3 \left(n - 1\right),\qquad p_{3}(n)=n - 9,\qquad p_{4}(n)=3 n - 5,\qquad p_{5}(n)=7 - 3 n,\qquad p_{6}(n)=13 - 2 n,\qquad p_{7}(n)=n - 6
\]
...
\[
n\,a(n) + (3 - 4 n)\,a(n-1) + 3 \left(n - 1\right)\,a(n-2) + (n - 9)\,a(n-3) + (3 n - 5)\,a(n-4) + (7 - 3 n)\,a(n-5) + (13 - 2 n)\,a(n-6) + (n - 6)\,a(n-7) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
cancellation in $K$, not by series truncation, so the identity $B=x^{5} - x^{4} + 3 x^{3}$ is an
