...
\title{A proof of the conjectured recurrence for OEIS A187252}
...
\begin{abstract}
OEIS A187252 carries a conjectured linear recurrence with polynomial coefficients, of
...
to the polynomial $2$, of degree $0$, which proves the recurrence for every
$n>0$.
\end{abstract}
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{2 x + e^{2 x} + 4 \log{\left(1 - x \right)} - 1}{4 \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence a(n) +(-2*n-3)*a(n-1) +(n+7)*(n-1)*a(n-2) +4*(-n\textasciicircum{}2+2*n+1)*a(n-3) +4*(n-3)\textasciicircum{}2*a(n-4)=0. - \emph{R. J. Mathar}, Jul 22 2022
\end{quote}
As of the ``Last modified'' line on the live entry (2022-07-22, revision 19) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- 2 n - 3,\qquad p_{2}(n)=\left(n - 1\right) \left(n + 7\right),\qquad p_{3}(n)=- 4 \left(n^{2} - 2 n - 1\right),\qquad p_{4}(n)=4 \left(n - 3\right)^{2}
\]
...
\section{The computation for A187252}
...
\[
1\,a(n) + (- 2 n - 3)\,a(n-1) + \left(n - 1\right) \left(n + 7\right)\,a(n-2) -  4 \left(n^{2} - 2 n - 1\right)\,a(n-3) + 4 \left(n - 3\right)^{2}\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A187252, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A187252.
