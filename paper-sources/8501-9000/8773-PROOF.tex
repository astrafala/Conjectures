...
\title{The empirical recurrence for OEIS A268164, derived from the entry's generating function}
...
\begin{abstract}
OEIS A268164 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A268164 is ``Number of n X 1 0..3 arrays with every repeated value in every row and column one larger mod 4 than the previous repeated value, and upper left element zero.''. It has offset $1$ and begins
\[
1, 4, 15, 55, 197, 695, 2421, 8351,\ \dots
\]
...
\begin{quote}
G.f.: x*(1 - x - 3*x\^{}2) / ((1 - 3*x)*(1 - 2*x - 4*x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 11 2019 10:44:41, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{3} - x^{2} + x}{12 x^{3} + 2 x^{2} - 5 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;12 x^{3} + 2 x^{2} - 5 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268164.
