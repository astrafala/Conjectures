...
\title{A proof of the conjectured recurrence for OEIS A090826}
...
\begin{abstract}
OEIS A090826 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A090826 is ``Convolution of Catalan and Fibonacci numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 2, 5, 12, 31, 85, 248,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{1 - 4 x} - 1}{2 \left(x^{2} + x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + (-5*n+6)*a(n-1) + 3*(n-2)*a(n-2) + 2*(2*n-3)*a(n-3)=0. - \emph{R. J. Mathar}, Jul 09 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-02, revision 39) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=6 - 5 n,\qquad p_{2}(n)=3 \left(n - 2\right),\qquad p_{3}(n)=2 \left(2 n - 3\right)
\]
...
\section{The computation for A090826}
...
\[
n\,a(n) + (6 - 5 n)\,a(n-1) + 3 \left(n - 2\right)\,a(n-2) + 2 \left(2 n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A090826, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A090826.
