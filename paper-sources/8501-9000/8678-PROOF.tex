...
\title{A proof of the conjectured recurrence for OEIS A025567}
...
\begin{abstract}
OEIS A025567 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A025567 is ``a(n) = T(n,n+1), where T is the array defined in A025564''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 4, 13, 40, 120, 356, 1050, 3088,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{x^{2} \sqrt{1 - 3 x} - \sqrt{1 - 3 x} + \sqrt{x + 1} \left(- x^{2} - 2 x + 1\right)}{2 x^{3} \sqrt{1 - 3 x}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n-1)*(n-2)*(n+3)*a(n) -2*n*(n-2)*(n+2)*a(n-1) -3*n*(n-1)\^{}2*a(n-2)=0. - \emph{R. J. Mathar}, Apr 03 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 29) the
...
\[
p_{0}(n)=\left(n - 2\right) \left(n - 1\right) \left(n + 3\right),\qquad p_{1}(n)=- 2 n \left(n - 2\right) \left(n + 2\right),\qquad p_{2}(n)=- 3 n \left(n - 1\right)^{2}
\]
...
\[
\left(n - 2\right) \left(n - 1\right) \left(n + 3\right)\,a(n) -  2 n \left(n - 2\right) \left(n + 2\right)\,a(n-1) -  3 n \left(n - 1\right)^{2}\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A025567, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A025567.
