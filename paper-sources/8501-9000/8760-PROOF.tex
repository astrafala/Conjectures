...
\title{The empirical recurrence for OEIS A265987, derived from the entry's generating function}
...
\begin{abstract}
OEIS A265987 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A265987 is ``Number of n X 3 integer arrays with each element equal to the number of horizontal and antidiagonal neighbors not equal to itself.''. It has offset $1$ and begins
\[
2, 2, 6, 10, 30, 66, 182, 442,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 09 2019 05:38:42, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{3} - 2 x^{2} + 2 x}{4 x^{3} - 3 x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A265987.
