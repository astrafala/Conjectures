...
\title{A proof of the conjectured recurrence for OEIS A166135}
...
\begin{abstract}
OEIS A166135 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $6 x^{2} + 6 x$, a polynomial of degree $2$, which proves
...
OEIS A166135 is ``Number of possible paths to each node that lies along the edge of a cut 4-nomial tree, that is rooted one unit from the cut''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 3, 7, 22, 65, 213, 693,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{\sqrt{\frac{- 3 x - 2 \sqrt{1 - 4 x} + 2}{x}}}{2} - \frac{1}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*n*(2*n+1)*a(n) + (17*n\^{}2-53*n+24)*a(n-1) + 6*(-13*n\^{}2+43*n-36)*a(n-2) - 108*(2*n-5)*(n-3)*a(n-3) = 0. - \emph{R. J. Mathar}, Oct 08 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 68) the
...
\[
p_{0}(n)=2 n \left(2 n + 1\right),\qquad p_{1}(n)=17 n^{2} - 53 n + 24,\qquad p_{2}(n)=- 6 \left(13 n^{2} - 43 n + 36\right),\qquad p_{3}(n)=- 108 \left(n - 3\right) \left(2 n - 5\right)
\]
...
\section{The computation for A166135}

The generating function \eqref{eq:gf} is algebraic over $\mathbb{Q}(x)$;
its minimal polynomial is
\[
P(x,y) \;=\; 16 \left(x y^{4} + 2 x y^{3} + 3 x y^{2} + 2 x y + x - y^{2} - y\right) ,
\]
...
\[
2 n \left(2 n + 1\right)\,a(n) + (17 n^{2} - 53 n + 24)\,a(n-1) -  6 \left(13 n^{2} - 43 n + 36\right)\,a(n-2) -  108 \left(n - 3\right) \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A166135, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=6 x^{2} + 6 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A166135.
