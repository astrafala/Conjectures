...
\title{The empirical recurrence for OEIS A223381, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223381 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 20 2018 05:50:28, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{5 x^{3} - 7 x^{2} + 2 x}{10 x^{2} - 8 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;10 x^{2} - 8 x + 1
\]
