...
\title{A proof of the conjectured recurrence for OEIS A101500}
...
\begin{abstract}
OEIS A101500 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A101500 is ``A Chebyshev transform of the central binomial numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 16, 53, 178, 609, 2112,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1}{\sqrt{x^{2} + 1} \sqrt{x^{2} - 4 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +2*(-2*n+1)*a(n-1) +2*(n-1)*a(n-2) +2*(-2*n+3)*a(n-3) +(n-2)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 16 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 66) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(2 n - 1\right),\qquad p_{2}(n)=2 \left(n - 1\right),\qquad p_{3}(n)=- 2 \left(2 n - 3\right),\qquad p_{4}(n)=n - 2
\]
...
\[
n\,a(n) -  2 \left(2 n - 1\right)\,a(n-1) + 2 \left(n - 1\right)\,a(n-2) -  2 \left(2 n - 3\right)\,a(n-3) + (n - 2)\,a(n-4) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A101500, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A101500.
