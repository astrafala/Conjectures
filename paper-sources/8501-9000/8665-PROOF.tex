...
\title{The empirical recurrence for OEIS A184032, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184032 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 04:52:31, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 88 x^{4} + 59 x^{3} + 44 x^{2} - 28 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;88 x^{4} - 59 x^{3} - 44 x^{2} + 28 x,
\]
