...
\title{The empirical recurrence for OEIS A235880, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235880 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A235880 is ``Number of (n+1) X (4+1) 0..2 arrays with the minimum plus the upper median equal to the lower median plus the maximum in every 2 X 2 subblock.''. It has offset $1$ and begins
\[
339, 537, 951, 1833, 3759, 8097, 18231, 42873,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 19 2018 09:12:27, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 1458 x^{3} + 1497 x^{2} - 339 x}{6 x^{3} - 11 x^{2} + 6 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;1458 x^{3} - 1497 x^{2} + 339 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235880.
