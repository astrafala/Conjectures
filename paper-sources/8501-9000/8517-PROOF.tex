...
\title{The empirical recurrence for OEIS A234436, proved by transfer matrix}
...
\begin{abstract}
OEIS A234436 counts $2$-column arrays over $\{0,\dots,2\}$ in which every
$2\times2$ subblock has its diagonal sum differing from its antidiagonal sum by
exactly $1$, and it
carries an empirical recurrence of order $2$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=9$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-2}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234436 is ``Number of (n+1) X (1+1) 0..2 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
32, 120, 448, 1680, 6272, 23520, 87808,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 14*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 18:20:40, revision 6) the statement is
...
a(n)\;=\;14\,a(n-2)
\end{equation}
for all $n>2$, which is every $n$ at which a recurrence of order $2$ can be
stated.
...
Let $V$ consist of all of $\{0,\dots,2\}^{2}$, and put $S=|V|=9$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge2$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-2}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-2$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-2}\bigl(M^{2}-\sum_i c_i M^{2-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-2$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $2$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=2-2+1$ onwards, and the run was continued past
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=9$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A234436.
