...
\title{The empirical recurrence for OEIS A233213, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233213 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A233213 is ``Number of n X 4 0..5 arrays with no element x(i,j) adjacent to value 5-x(i,j) horizontally or antidiagonally, top left element zero, and 1 appearing before 2 3 and 4, and 2 appearing before 3 in row major order.''. It has offset $1$ and begins
\[
23, 7222, 3054973, 1340889772, 591021089923, 260625046992322, 114934898294104873, 50686253523654314872,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 10 2018 10:01:06, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 17248 x^{3} + 4071 x^{2} - 23 x}{21609 x^{3} - 22099 x^{2} + 491 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 21609 x^{3} + 22099 x^{2} - 491 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;491\,a(n-1) - 22099\,a(n-2) + 21609\,a(n-3)
\]
for every $n>3$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;17248 x^{3} - 4071 x^{2} + 23 x,
\]
...
compared against all $11$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233213.
