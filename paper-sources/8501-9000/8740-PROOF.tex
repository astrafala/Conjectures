...
\title{A proof of the conjectured recurrence for OEIS A188482}
...
\begin{abstract}
OEIS A188482 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A188482 is ``Diagonal sums of the Riordan matrix (1/(1-4x),(1-sqrt(1-4x))/(2*sqrt(1-4x))) (A188481)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 17, 71, 295, 1221, 5040, 20761,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 4 x^{2} + x \sqrt{1 - 4 x} - 7 x + 2}{2 \left(4 x^{4} + 15 x^{3} + 8 x^{2} - 7 x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+1)*a(n) + (7*n-9)*a(n-1) + 2*(-4*n+7)*a(n-2) + (-15*n+23)*a(n-3) + 2*(-2*n+3)*a(n-4) = 0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2023-12-26, revision 14) the
...
\[
p_{0}(n)=1 - n,\qquad p_{1}(n)=7 n - 9,\qquad p_{2}(n)=- 2 \left(4 n - 7\right),\qquad p_{3}(n)=23 - 15 n,\qquad p_{4}(n)=- 2 \left(2 n - 3\right)
\]
...
\section{The computation for A188482}
...
\[
(1 - n)\,a(n) + (7 n - 9)\,a(n-1) -  2 \left(4 n - 7\right)\,a(n-2) + (23 - 15 n)\,a(n-3) -  2 \left(2 n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A188482, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A188482.
