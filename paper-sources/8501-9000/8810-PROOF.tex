...
\title{A proof of the conjectured recurrence for OEIS A270724}
...
\begin{abstract}
OEIS A270724 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{6} + 6 x^{5} - 10 x^{3} + 6 x + 2$, a polynomial of degree $6$, which proves
...
OEIS A270724 is ``a(n) = ((n+2)/2)*Sum\_\{k=0..n/2\} (Sum\_\{i=0..n-2*k\} (binomial(k+1,n-2*k-i)*binomial(k+i,k))/(k+1)*C(k)), where C(k) is Catalan numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 5, 10, 20, 42, 93, 213,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(\sqrt{\frac{4 x^{2} \left(x + 1\right) + x - 1}{x - 1}} - 1\right) \left(- x^{2} + x + 1\right)}{2 x^{2} \left(x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +(-n-2)*a(n-1) +(-7*n+6)*a(n-2) +10*a(n-3) +(13*n-32)*a(n-4) +(5*n-32)*a(n-5) +(-11*n+52)*a(n-6) +4*(-n+6)*a(n-7) +4*(n-7)*a(n-8)=0. - \emph{R. J. Mathar}, Oct 07 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-10-07, revision 18) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- n - 2,\qquad p_{2}(n)=6 - 7 n,\qquad p_{3}(n)=10,\qquad p_{4}(n)=13 n - 32,\qquad p_{5}(n)=5 n - 32,\qquad p_{6}(n)=52 - 11 n,\qquad p_{7}(n)=- 4 \left(n - 6\right),\qquad p_{8}(n)=4 \left(n - 7\right)
\]
...
\[
(n + 2)\,a(n) + (- n - 2)\,a(n-1) + (6 - 7 n)\,a(n-2) + 10\,a(n-3) + (13 n - 32)\,a(n-4) + (5 n - 32)\,a(n-5) + (52 - 11 n)\,a(n-6) -  4 \left(n - 6\right)\,a(n-7) + 4 \left(n - 7\right)\,a(n-8) \;=\;0
\]
holds for every $n>6$.
\end{theorem}
...
section of A270724, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{6} + 6 x^{5} - 10 x^{3} + 6 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A270724.
