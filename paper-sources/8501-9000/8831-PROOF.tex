...
\title{The empirical recurrence for OEIS A183356, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183356 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 22 2018 04:53:10, revision 15) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 108 x^{4} + 72 x^{3} + 504 x^{2} + 36 x}{x^{3} - 2 x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 108 x^{4} + 72 x^{3} + 504 x^{2} + 36 x,
\]
