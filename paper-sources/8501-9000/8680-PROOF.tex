...
\title{The empirical recurrence for OEIS A190091, derived from the entry's generating function}
...
\begin{abstract}
OEIS A190091 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A190091 is ``Number of rhombuses on a (n+1) X 3 grid.''. It has offset $1$ and begins
\[
2, 6, 10, 15, 20, 26, 32, 39,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 04 2018 11:43:19, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} + 2 x^{3} - 2 x^{2} - 2 x}{x^{4} - 2 x^{3} + 2 x - 1},
\]
...
compared against all $57$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A190091.
