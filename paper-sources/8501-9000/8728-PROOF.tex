...
\title{The empirical recurrence for OEIS A211490, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211490 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A211490 is ``Number of (n+1) X (n+1) -4..4 symmetric matrices with every 2 X 2 subblock having sum zero and one or two distinct values.''. It has offset $1$ and begins
\[
13, 17, 23, 33, 49, 75, 117, 185,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 18 2018 07:47:34, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 11 x^{3} - 9 x^{2} + 13 x}{x^{3} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{3} - 2 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211490.
