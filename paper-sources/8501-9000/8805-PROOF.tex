...
\title{The empirical recurrence for OEIS A262327, derived from the entry's generating function}
...
\begin{abstract}
OEIS A262327 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 31 2018 06:08:25, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 54 x^{3} - 21 x^{2} + 6 x}{54 x^{3} - 9 x^{2} - 6 x + 1},
\]
