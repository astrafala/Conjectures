...
\title{The empirical recurrence for OEIS A235891, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235891 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
G.f.: 8*x*(2144 - 12669*x + 16384*x\^{}2) / ((1 - x)*(1 - 2*x)*(1 - 4*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 20 2018 03:58:26, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 131072 x^{3} + 101352 x^{2} - 17152 x}{8 x^{3} - 14 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;131072 x^{3} - 101352 x^{2} + 17152 x,
\]
