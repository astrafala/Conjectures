...
\title{The empirical recurrence for OEIS A202730, derived from the entry's generating function}
...
\begin{abstract}
OEIS A202730 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A202730 is ``Number of n X 3 nonnegative integer arrays each row and column an ascent sequence (interior element no greater than one plus up-steps preceding it) and the lower right element equal to min(n,3)-1.''. It has offset $1$ and begins
\[
1, 2, 2, 14, 78, 406, 2062, 10374,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 01 2018 11:35:26, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{20 x^{4} - 2 x^{3} - 5 x^{2} + x}{10 x^{2} - 7 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;10 x^{2} - 7 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202730.
