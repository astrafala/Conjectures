...
\title{A proof of the conjectured recurrence for OEIS A129149}
...
\begin{abstract}
OEIS A129149 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A129149 is ``Permutations with exactly 7 fixed points''. It has offset $7$ and begins
\[
a(7),\dots,a(14)\;=\;1, 0, 36, 240, 2970, 34848, 454740, 6362928,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 7} a(n)\,\frac{x^{n}}{n!}\;=\;- \frac{x^{7} e^{- x}}{5040 \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+7)*a(n) +n*(n-8)*a(n-1) +n*(n-1)*a(n-2)=0. - \emph{R. J. Mathar}, Nov 02 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2026-01-12, revision 30) the
...
\[
p_{0}(n)=7 - n,\qquad p_{1}(n)=n \left(n - 8\right),\qquad p_{2}(n)=n \left(n - 1\right)
\]
...
\section{The computation for A129149}
...
\[
(7 - n)\,a(n) + n \left(n - 8\right)\,a(n-1) + n \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A129149, agreeing on all 13 terms compared.
...
vanish, for all 15 values of $n$ from $n=10$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A129149.
