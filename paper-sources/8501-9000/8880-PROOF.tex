...
\title{A proof of the conjectured recurrence for OEIS A114851}
...
\begin{abstract}
OEIS A114851 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{5} - 3 x^{3} + 4 x^{2}$, a polynomial of degree $5$, which proves
...
OEIS A114851 is ``Number of lambda calculus terms of size n, where size(lambda x.M) = 2 + size(M), size(M N) = 2 + size(M) + size(N), and size(V) = 1 + i for a variable V bound by the i-th enclosing lambda (corresponding to a binary encoding)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 1, 2, 2, 4, 5,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{3} + x^{2} + x + \sqrt{- 4 x^{4} + 8 x^{3} - 4 x + \left(- x^{3} - x^{2} + x + 1\right)^{2}} - 1}{2 x^{2} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) + 2*(-n-1)*a(n-1) + (-n+2)*a(n-2) + 4*(n-2)*a(n-3) + (-5*n+18)*a(n-4) + 2*(n-4)*a(n-5) + (n-6)*a(n-6) = 0. - \emph{R. J. Mathar}, Mar 04 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2026-02-07, revision 80) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 2 \left(n + 1\right),\qquad p_{2}(n)=2 - n,\qquad p_{3}(n)=4 \left(n - 2\right),\qquad p_{4}(n)=18 - 5 n,\qquad p_{5}(n)=2 \left(n - 4\right),\qquad p_{6}(n)=n - 6
\]
...
\section{The computation for A114851}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x - 1\right) \left(x^{5} + 3 x^{4} - 2 x^{3} + 2 x^{2} + x - 1\right) .
\]
...
\[
(n + 2)\,a(n) -  2 \left(n + 1\right)\,a(n-1) + (2 - n)\,a(n-2) + 4 \left(n - 2\right)\,a(n-3) + (18 - 5 n)\,a(n-4) + 2 \left(n - 4\right)\,a(n-5) + (n - 6)\,a(n-6) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A114851, agreeing on all 13 terms compared.
...
vanish, for all 34 values of $n$ from $n=6$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{5} - 3 x^{3} + 4 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A114851.
