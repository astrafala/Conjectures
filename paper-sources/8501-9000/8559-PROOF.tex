...
\title{The empirical recurrence for OEIS A185858, derived from the entry's generating function}
...
\begin{abstract}
OEIS A185858 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A185858 is ``1/128 the number of (n+2) X 3 binary arrays with no 3 X 3 subblock trace equal to any horizontal or vertical neighbor 3 X 3 subblock trace.''. It has offset $1$ and begins
\[
4, 22, 124, 694, 3892, 21814, 122284, 685462,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 17 2018 06:18:34, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{2} - 4 x}{9 x^{2} + 4 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 9 x^{2} - 4 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A185858.
