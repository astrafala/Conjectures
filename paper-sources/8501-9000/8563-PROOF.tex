...
\title{The empirical recurrence for OEIS A234779, derived from the entry's generating function}
...
\begin{abstract}
OEIS A234779 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A234779 is ``Number of (n+1) X (1+1) 0..3 arrays with no adjacent elements equal and with each 2 X 2 subblock having the number of clockwise edge increases equal to the number of counterclockwise edge increases.''. It has offset $1$ and begins
\[
76, 484, 3084, 19652, 125228, 797988, 5085004, 32403076,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 16 2018 05:42:03, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 48 x^{2} + 76 x}{4 x^{2} - 7 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{2} - 7 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234779.
