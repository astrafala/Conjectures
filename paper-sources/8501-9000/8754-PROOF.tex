...
\title{The empirical recurrence for OEIS A262326, derived from the entry's generating function}
...
\begin{abstract}
OEIS A262326 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A262326 is ``Number of (n+1) X (2+1) 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
3, 5, 15, 33, 99, 261, 783, 2241,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 20 2018 20:36:53, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 9 x^{3} - 4 x^{2} + 3 x}{9 x^{3} - 3 x^{2} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;9 x^{3} - 3 x^{2} - 3 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262326.
