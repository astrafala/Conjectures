...
\title{A proof of the conjectured recurrence for OEIS A026023}
...
\begin{abstract}
OEIS A026023 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $-4$, a polynomial of degree $0$, which proves
...
OEIS A026023 is ``a(n) = number of (s(0), s(1), ..., s(n)) such that s(i) is a nonnegative integer and |s(i) - s(i-1)| = 1 for i = 1,2,...,n and s(0) = 3. Also a(n) = Sum\{T(n,k), k = 0,1,...,[ (n+3)/2 ]\}, where T is defined in A026022''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 4, 8, 15, 30, 56, 112,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 4 x^{2} - \sqrt{- 16 x^{6} + 20 x^{4} - 8 x^{2} + 1} + 1}{2 x^{4} \left(2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+4)*(n-1)*a(n) +(n-1)*(n+1)*a(n-1) -2*(n+1)*(2*n+1)*a(n-2) -4*(n-1)*(n+1)*a(n-3)=0. - \emph{R. J. Mathar}, Sep 29 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-13, revision 47) the
...
\[
p_{0}(n)=\left(n - 1\right) \left(n + 4\right),\qquad p_{1}(n)=\left(n - 1\right) \left(n + 1\right),\qquad p_{2}(n)=- 2 \left(n + 1\right) \left(2 n + 1\right),\qquad p_{3}(n)=- 4 \left(n - 1\right) \left(n + 1\right)
\]
...
\section{The computation for A026023}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \left(2 x - 1\right) \left(2 x + 1\right) \left(2 x^{2} - 1\right)^{2} .
\]
...
\[
\left(n - 1\right) \left(n + 4\right)\,a(n) + \left(n - 1\right) \left(n + 1\right)\,a(n-1) -  2 \left(n + 1\right) \left(2 n + 1\right)\,a(n-2) -  4 \left(n - 1\right) \left(n + 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A026023, agreeing on all 13 terms compared.
...
vanish, for all 31 values of $n$ from $n=3$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=-4$ is an
...
\url{https://oeis.org}, 2026. Sequence A026023.
