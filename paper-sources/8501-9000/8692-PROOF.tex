...
\title{The empirical recurrence for OEIS A250792, derived from the entry's generating function}
...
\begin{abstract}
OEIS A250792 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A250792 is ``Number of (n+1) X (3+1) 0..1 arrays with nondecreasing min(x(i,j),x(i,j-1)) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
54, 162, 482, 1430, 4258, 12706, 37986, 113694,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 20 2018 09:08:42, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 96 x^{5} + 158 x^{4} + 50 x^{3} - 162 x^{2} + 54 x}{6 x^{5} - 11 x^{4} + 10 x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;6 x^{5} - 11 x^{4} + 10 x^{2} - 6 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) - 10\,a(n-2) + 11\,a(n-4) - 6\,a(n-5)
\]
for every $n>5$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 96 x^{5} + 158 x^{4} + 50 x^{3} - 162 x^{2} + 54 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250792.
