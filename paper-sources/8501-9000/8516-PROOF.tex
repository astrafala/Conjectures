...
\title{The empirical recurrence for OEIS A269290, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269290 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A269290 is ``Number of 2 X n 0..3 arrays with some element plus some horizontally or antidiagonally adjacent neighbor totalling three no more than once.''. It has offset $1$ and begins
\[
16, 216, 2124, 19188, 164556, 1363572, 11026764, 87559668,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 21 2019 09:24:48, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{36 x^{4} - 116 x^{3} - 8 x^{2} + 16 x}{49 x^{2} - 14 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;49 x^{2} - 14 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;36 x^{4} - 116 x^{3} - 8 x^{2} + 16 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269290.
