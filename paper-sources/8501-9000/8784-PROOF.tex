...
\title{The empirical recurrence for OEIS A233124, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233124 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 19 2018 09:41:50, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 192 x^{3} + 64 x^{2} - 3 x}{1024 x^{3} - 448 x^{2} + 48 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 1024 x^{3} + 448 x^{2} - 48 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233124.
