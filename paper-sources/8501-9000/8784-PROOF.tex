...
\title{The empirical recurrence for OEIS A221829, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221829 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A221829 is ``Number of 2 X n arrays of occupancy after each element moves to some horizontal, vertical or antidiagonal neighbor, without 2-loops or left turns.''. It has offset $1$ and begins
\[
0, 3, 16, 73, 333, 1519, 6929, 31607,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 11 2018 10:45:49, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} + x^{3} + 3 x^{2}}{2 x^{2} - 5 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221829.
