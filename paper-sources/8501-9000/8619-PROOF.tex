...
\title{The empirical recurrence for OEIS A221788, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221788 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2018 17:34:26, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{20 x^{6} - 16 x^{5} + 5 x^{4}}{x^{4} - 8 x^{3} + 14 x^{2} - 8 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{4} - 8 x^{3} + 14 x^{2} - 8 x + 1
\]
