...
\title{The empirical recurrence for OEIS A208088, derived from the entry's generating function}
...
\begin{abstract}
OEIS A208088 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A208088 is ``Number of 7 X (n+1) 0..1 arrays with every 2 X 2 subblock having the same number of equal edges as its horizontal neighbors and a different number from its vertical neighbors, and new values 0..1 introduced in row major order.''. It has offset $1$ and begins
\[
108, 180, 288, 468, 756, 1224, 1980, 3204,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 27 2018 15:30:13, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 72 x^{2} - 108 x}{x^{2} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{2} - x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208088.
