...
\title{The empirical recurrence for OEIS A264123, derived from the entry's generating function}
...
\begin{abstract}
OEIS A264123 carries an empirical recurrence of order $1$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 29 2023 11:23:22, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3969 x^{5} + 27 x^{4} - 126 x^{3} - 5 x^{2} - 4 x}{10 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 3969 x^{5} - 27 x^{4} + 126 x^{3} + 5 x^{2} + 4 x,
\]
