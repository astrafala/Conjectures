...
\title{The empirical recurrence for OEIS A235892, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235892 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
G.f.: 8*x*(8384 - 49917*x + 65536*x\^{}2) / ((1 - x)*(1 - 2*x)*(1 - 4*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 20 2018 03:58:21, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 524288 x^{3} + 399336 x^{2} - 67072 x}{8 x^{3} - 14 x^{2} + 7 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 8 x^{3} + 14 x^{2} - 7 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;524288 x^{3} - 399336 x^{2} + 67072 x,
\]
