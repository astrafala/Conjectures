...
\title{The empirical recurrence for OEIS A205220, derived from the entry's generating function}
...
\begin{abstract}
OEIS A205220 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 05:10:17, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 32 x^{2} - 20 x}{4 x^{2} + x - 1},
\]
