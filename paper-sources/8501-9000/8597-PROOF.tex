...
\title{The empirical recurrence for OEIS A276300, derived from the entry's generating function}
...
\begin{abstract}
OEIS A276300 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A276300 is ``Number of 3 X n 0..2 arrays with no element equal to any value at offset (-2,-1) (-1,1) or (0,-1) and new values introduced in order 0..2.''. It has offset $1$ and begins
\[
5, 11, 45, 173, 693, 2765, 11061, 44237,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 05 2019 18:42:35, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{4} - 4 x^{3} - 9 x^{2} + 5 x}{4 x^{3} - x^{2} - 4 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{3} - x^{2} - 4 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A276300.
