...
\title{The empirical recurrence for OEIS A239024, derived from the entry's generating function}
...
\begin{abstract}
OEIS A239024 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A239024 is ``Number of n X 2 0..2 arrays with no element equal to the sum of elements to its left or one plus the sum of elements above it, modulo 3.''. It has offset $1$ and begins
\[
1, 3, 4, 11, 16, 43, 64, 171,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 25 2018 05:48:07, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{4} - x^{3} + 3 x^{2} + x}{4 x^{4} - 5 x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{4} - 5 x^{2} + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A239024.
