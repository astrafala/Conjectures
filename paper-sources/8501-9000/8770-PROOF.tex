...
\title{The empirical recurrence for OEIS A231103, derived from the entry's generating function}
...
\begin{abstract}
OEIS A231103 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A231103 is ``Number of n X 3 0..3 arrays x(i,j) with each element horizontally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
0, 2, 8, 42, 208, 1042, 5208, 26042,\ \dots
\]
...
G.f.: 2*x\^{}2 / ((1 + x)*(1 - 5*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 26 2018 08:20:47, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;- \frac{2 x^{2}}{5 x^{2} + 4 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 5 x^{2} - 4 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231103.
