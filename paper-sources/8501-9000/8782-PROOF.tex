...
\title{The empirical recurrence for OEIS A267946, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267946 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
G.f.: x*(1 - 2*x\^{}2) / ((1 - 2*x)*(1 - x - 3*x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 21 2018 09:38:20, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{3} + x}{6 x^{3} - x^{2} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;6 x^{3} - x^{2} - 3 x + 1
\]
