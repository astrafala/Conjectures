...
\title{A proof of the conjectured recurrence for OEIS A190736}
...
\begin{abstract}
OEIS A190736 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $54 x^{3} - 102 x^{2} - 108 x$, a polynomial of degree $3$, which proves
...
OEIS A190736 is ``Diagonal sums of the Riordan matrix A121576''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 7, 29, 139, 731, 4096, 24005,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{2} - \frac{5 x}{2} - \frac{\left(x + 2\right) \sqrt{4 x^{2} - 8 x + 1}}{2} + 2}{x^{3} + 2 x^{2} - x + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(11*n-35)*a(n) + 3*(-33*n\^{}2+149*n-136)*a(n-1) +2*(77*n\^{}2-377*n+396)*a(n-2) +(-209*n\^{}2+1061*n-1200)*a(n-3) +12*(-11*n+30)*a(n-4) +4*(11*n-24)*(n-4)*a(n-5)=0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2022-09-08, revision 14) the
...
\[
p_{0}(n)=n \left(11 n - 35\right),\qquad p_{1}(n)=- 3 \left(33 n^{2} - 149 n + 136\right),\qquad p_{2}(n)=2 \left(77 n^{2} - 377 n + 396\right),\qquad p_{3}(n)=- 209 n^{2} + 1061 n - 1200,\qquad p_{4}(n)=- 12 \left(11 n - 30\right),\qquad p_{5}(n)=4 \left(n - 4\right) \left(11 n - 24\right)
\]
...
\section{The computation for A190736}
...
\[
n \left(11 n - 35\right)\,a(n) -  3 \left(33 n^{2} - 149 n + 136\right)\,a(n-1) + 2 \left(77 n^{2} - 377 n + 396\right)\,a(n-2) + (- 209 n^{2} + 1061 n - 1200)\,a(n-3) -  12 \left(11 n - 30\right)\,a(n-4) + 4 \left(n - 4\right) \left(11 n - 24\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A190736, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=54 x^{3} - 102 x^{2} - 108 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A190736.
