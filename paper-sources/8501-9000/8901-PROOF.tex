...
\title{The empirical recurrence for OEIS A190092, derived from the entry's generating function}
...
\begin{abstract}
OEIS A190092 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A190092 is ``Number of rhombuses on a (n+1) X 4 grid.''. It has offset $1$ and begins
\[
3, 10, 22, 36, 50, 66, 82, 100,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 04 2018 11:51:12, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{11} - 2 x^{9} + 5 x^{5} + 2 x^{4} - 2 x^{3} - 4 x^{2} - 3 x}{x^{4} - 2 x^{3} + 2 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 2 x^{11} + 2 x^{9} - 5 x^{5} - 2 x^{4} + 2 x^{3} + 4 x^{2} + 3 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A190092.
