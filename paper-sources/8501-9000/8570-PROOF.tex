...
\title{The empirical recurrence for OEIS A221374, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221374 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A221374 is ``Number of n X 2 arrays of occupancy after each element stays put or moves to some horizontal, vertical or antidiagonal neighbor, with no occupancy greater than 2.''. It has offset $1$ and begins
\[
3, 19, 139, 1035, 7723, 57643, 430251, 3211435,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 05 2018 08:21:28, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{3} + 8 x^{2} - 3 x}{4 x^{3} - 12 x^{2} + 9 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 4 x^{3} + 12 x^{2} - 9 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221374.
