...
\title{A proof of the conjectured recurrence for OEIS A392976}
...
\begin{abstract}
OEIS A392976 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $0$, a polynomial of degree $-1$, which proves
...
OEIS A392976 is ``Expansion of (1/x) * Series\_Reversion( x * ((1-x\textasciicircum{}2)\textasciicircum{}2 - x) )''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 15, 67, 315, 1556, 7938,\ \dots
\]
The entry posts no generating function of its own. Its NAME states the
relation \texttt{the entry states that the generating function satisfies G.f. A(x) satisfies A(x) = 1/((1 - (x*A(x))\textasciicircum{}2)\textasciicircum{}2 - x*A(x))} to another entry, and that entry's exponential generating
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\text{the branch of } P(x,y)=0 \text{ with } y(0)=1 ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence 283*n*(n-1)*(n-2)*(83022305213*n -178385058984)*(n+1) *a(n) -6*n*(n-1)*(n-2)*(12196786904129*n\textasciicircum{}2 -45110300936020*n +52641391340361)*a(n-1) -6*(n-1)*(n-2)*(67790996421377*n\textasciicircum{}3 -248299346995054*n\textasciicircum{}2 +202497447227007*n +3061418504418)*a(n-2) -2*(n-2)*(192167357944360*n\textasciicircum{}4 -559010679688491*n\textasciicircum{}3 -2120668219560541*n\textasciicircum{}2 +9123363136130610*n -8342981628089220)*a(n-3) +5*(15640122296875*n\textasciicircum{}5 -565964885220500*n\textasciicircum{}4 +4989857715427325*n\textasciicircum{}3 -18667678380008380*n\textasciicircum{}2 +32085126156301848*n -20925325629575136)*a(n-4) +10*(5*n-17)*(5*n-18) *(5*n-21)*(5*n-19)*(22509687685*n-60308240636)*a(n-5)=0. - \emph{R. J. Mathar}, Jan 29 2026
\end{quote}
As of the ``Last modified'' line on the live entry (2026-01-31, revision 19) the
...
\[
p_{0}(n)=283 n \left(n - 2\right) \left(n - 1\right) \left(n + 1\right) \left(83022305213 n - 178385058984\right),\qquad p_{1}(n)=- 6 n \left(n - 2\right) \left(n - 1\right) \left(12196786904129 n^{2} - 45110300936020 n + 52641391340361\right),\qquad p_{2}(n)=- 6 \left(n - 2\right) \left(n - 1\right) \left(67790996421377 n^{3} - 248299346995054 n^{2} + 202497447227007 n + 3061418504418\right),\qquad p_{3}(n)=- 2 \left(n - 2\right) \left(192167357944360 n^{4} - 559010679688491 n^{3} - 2120668219560541 n^{2} + 9123363136130610 n - 8342981628089220\right),\qquad p_{4}(n)=5 \left(15640122296875 n^{5} - 565964885220500 n^{4} + 4989857715427325 n^{3} - 18667678380008380 n^{2} + 32085126156301848 n - 20925325629575136\right),\qquad p_{5}(n)=10 \left(5 n - 21\right) \left(5 n - 19\right) \left(5 n - 18\right) \left(5 n - 17\right) \left(22509687685 n - 60308240636\right)
\]
...
\[
283 n \left(n - 2\right) \left(n - 1\right) \left(n + 1\right) \left(83022305213 n - 178385058984\right)\,a(n) -  6 n \left(n - 2\right) \left(n - 1\right) \left(12196786904129 n^{2} - 45110300936020 n + 52641391340361\right)\,a(n-1) -  6 \left(n - 2\right) \left(n - 1\right) \left(67790996421377 n^{3} - 248299346995054 n^{2} + 202497447227007 n + 3061418504418\right)\,a(n-2) -  2 \left(n - 2\right) \left(192167357944360 n^{4} - 559010679688491 n^{3} - 2120668219560541 n^{2} + 9123363136130610 n - 8342981628089220\right)\,a(n-3) + 5 \left(15640122296875 n^{5} - 565964885220500 n^{4} + 4989857715427325 n^{3} - 18667678380008380 n^{2} + 32085126156301848 n - 20925325629575136\right)\,a(n-4) + 10 \left(5 n - 21\right) \left(5 n - 19\right) \left(5 n - 18\right) \left(5 n - 17\right) \left(22509687685 n - 60308240636\right)\,a(n-5) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
$n\le-1$ some of the shifted terms $a(n-i)$ fall outside the range of the sequence,
...
section of A392976, agreeing on all 13 terms compared.
...
vanish, for all 20 values of $n$ from $n=5$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A392976.
