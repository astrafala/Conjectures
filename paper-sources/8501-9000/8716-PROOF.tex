...
\title{The empirical recurrence for OEIS A233175, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233175 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 10 2018 06:15:03, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{224 x^{4} - 16 x^{3} - 28 x^{2} + 3 x}{128 x^{3} - 12 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;128 x^{3} - 12 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;224 x^{4} - 16 x^{3} - 28 x^{2} + 3 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233175.
