...
\title{A proof of the conjectured recurrence for OEIS A166300}
...
\begin{abstract}
OEIS A166300 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $1 - x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A166300 is ``Number of Dyck paths of semilength n, having no ascents and no descents of length 1, and having no UUDD's starting at level 0''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 0, 1, 1, 2, 5, 10,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{x^{2} + x + \sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-n+1)*a(n-1) +(-n+1)*a(n-2) +2*(-n+1)*a(n-3) +(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-05-06, revision 53) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(n - 1\right),\qquad p_{2}(n)=1 - n,\qquad p_{3}(n)=- 2 \left(n - 1\right),\qquad p_{4}(n)=n - 3
\]
...
\section{The computation for A166300}
...
\[
(n + 1)\,a(n) -  2 \left(n - 1\right)\,a(n-1) + (1 - n)\,a(n-2) -  2 \left(n - 1\right)\,a(n-3) + (n - 3)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A166300, agreeing on all 13 terms compared.
...
vanish, for all 29 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=1 - x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A166300.
