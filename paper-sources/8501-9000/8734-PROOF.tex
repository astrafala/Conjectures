...
\title{The empirical recurrence for OEIS A263908, derived from the entry's generating function}
...
\begin{abstract}
OEIS A263908 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A263908 is ``Number of (2n+1) X (3+2) 0..1 arrays with each row and column modulo 3 equal to 1, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
33, 2399, 252097, 29452071, 3532758473, 426525918799, 51580839266577, 6240392439847991,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 03 2019 09:02:21, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 195052 x^{4} + 57437 x^{3} - 3178 x^{2} + 33 x}{195052 x^{4} - 71671 x^{3} + 6387 x^{2} - 169 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;195052 x^{4} - 71671 x^{3} + 6387 x^{2} - 169 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;169\,a(n-1) - 6387\,a(n-2) + 71671\,a(n-3) - 195052\,a(n-4)
\]
for every $n>4$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 195052 x^{4} + 57437 x^{3} - 3178 x^{2} + 33 x,
\]
...
\[
195052 x^{4} - 71671 x^{3} + 6387 x^{2} - 169 x + 1,
\]
...
compared against all $12$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263908.
