...
\title{The empirical recurrence for OEIS A250610, derived from the entry's generating function}
...
\begin{abstract}
OEIS A250610 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A250610 is ``Number of (n+1) X (7+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
1013, 2336, 4353, 7320, 11749, 18664, 30113, 50192,\ \dots
\]
...
\begin{quote}
G.f.: x*(1013 - 2729*x + 1790*x\^{}2 - 512*x\^{}3) / ((1 - x)\^{}3*(1 - 2*x)). - \_Colin Barker\_, Nov 15 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 15 2018 11:07:39, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 512 x^{4} + 1790 x^{3} - 2729 x^{2} + 1013 x}{2 x^{4} - 7 x^{3} + 9 x^{2} - 5 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 512 x^{4} + 1790 x^{3} - 2729 x^{2} + 1013 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250610.
