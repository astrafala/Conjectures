...
\title{The empirical recurrence for OEIS A205738, proved by transfer matrix}
...
\begin{abstract}
OEIS A205738 counts arrays over $\{0,\dots,2\}$ under a condition on the number of clockwise edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=360$ states and is
...
OEIS A205738 is ``Number of 3X(n+1) 0..2 arrays with every 2 X 2 subblock having the same number of clockwise edge increases as its horizontal neighbors and no 2 X 2 subblock having the same number of counterclockwise edge increases as its vertical neighbors.''. It has offset $1$ and begins
\[
360, 978, 3888, 17532, 80916, 376314, 1756446, 8212662,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) -2*a(n-2) -21*a(n-3) +a(n-4) +25*a(n-5) +14*a(n-6) -4*a(n-8) -2*a(n-9) for n>13.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 09 2015 22:34:13, revision 7) this is still
...
\]
on the $S=360$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $2$
subblocks; the entry's first term is $360$, which the model
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) - 2\,a(n-2) - 21\,a(n-3) + a(n-4) + 25\,a(n-5) + 14\,a(n-6) - 4\,a(n-8) - 2\,a(n-9)
\]
for every $n>13$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205738.
