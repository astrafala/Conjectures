...
\title{The empirical recurrence for OEIS A298185, proved by transfer matrix}
...
\begin{abstract}
OEIS A298185 counts $6$-column arrays over $\{0,\dots,1\}$ in which every entry has,
...
OEIS A298185 is ``Number of nX6 0..1 arrays with every element equal to 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 5, 5, 6, 7, 10, 14, 20,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) -2*a(n-2) -a(n-3) +2*a(n-4) -2*a(n-5) +a(n-7) for n>8
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 14 2018 09:39:06, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) - 2\,a(n-2) - a(n-3) + 2\,a(n-4) - 2\,a(n-5) + a(n-7)
\end{equation}
for all $n>8$.
...
compared against the entry's DATA at every one of the $39$ published indices; they
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298185.
