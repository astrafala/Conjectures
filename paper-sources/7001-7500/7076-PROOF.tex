...
\title{The empirical recurrence for OEIS A303726, proved by transfer matrix}
...
\begin{abstract}
OEIS A303726 counts $7$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,3\}$, and it carries an empirical
recurrence of order $10$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A303726 is ``Number of nX7 0..1 arrays with every element unequal to 0, 1, 2 or 3 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
64, 719, 294, 406, 564, 894, 1476, 2252,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) -a(n-4) -a(n-5) +a(n-7) -a(n-8) -a(n-9) +2*a(n-10) for n>18
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 29 2018 11:11:18, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) - a(n-4) - a(n-5) + a(n-7) - a(n-8) - a(n-9) + 2\,a(n-10)
\end{equation}
for all $n>18$.
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-10}$ whenever $n-10\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-10$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-10}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $10$ matrix--vector
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303726.
