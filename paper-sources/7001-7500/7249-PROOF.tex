...
\title{The empirical recurrence for OEIS A234264, proved by transfer matrix}
...
\begin{abstract}
OEIS A234264 counts $7$-column arrays over $\{0,\dots,2\}$ in which every
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=2187$ vertices, which
...
OEIS A234264 is ``Number of (n+1) X (6+1) 0..2 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
1336, 1690, 2324, 3616, 6076, 11140, 21164,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) + 6*a(n-2) - 24*a(n-3) + 4*a(n-4) + 36*a(n-5) - 24*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 21:33:19, revision 12) the statement is
...
Let $V$ consist of all of $\{0,\dots,2\}^{7}$, and put $S=|V|=2187$. For $r,s\in V$ declare $r\to s$ an edge
when $\bigl|r_j+s_{j+1}-r_{j+1}-s_j\bigr|=2$ for every $j$ with
$1\le j\le7-1$. Let $M\in\{0,1\}^{S\times S}$ be the adjacency matrix and $\mathbf{1}$
the all-ones vector.
...
\begin{proof}
An array with $n+1$ rows and $7$ columns is a sequence $r^{(0)},\dots,r^{(n)}$ of
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=2187$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A234264.
