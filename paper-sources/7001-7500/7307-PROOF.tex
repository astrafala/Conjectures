...
\title{The empirical recurrence for OEIS A223408, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A223408 counts arrays over $\{0,\dots,9\}$ in which every cell is constrained by the
...
with $S=100$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A223408 is ``4 X 4 X 4 triangular graph without horizontal edges coloring a rectangular array: number of n X 1 0..9 arrays where 0..9 label nodes of a graph with edges 0,1 0,2 1,3 1,4 2,4 2,5 3,6 3,7 4,7 4,8 5,8 5,9 and every array movement to a horizontal or vertical neighbor moves along an edge of this graph.''. It has offset $1$ and begins
\[
10, 24, 66, 176, 486, 1312, 3622, 9792,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 9*a(n-2) - 12*a(n-4) + 4*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 20 2018 08:37:37, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;9\,a(n-2) - 12\,a(n-4) + 4\,a(n-6)
\end{equation}
for all $n>7$.
...
\{x_{t,u},x_{t+d_1,u+d_2}\}\in E\ \text{ for every offset in }\mathcal N,\quad E=\{\{0,1\},\{0,2\},\{1,3\},\{1,4\},\{2,4\},\{2,5\},\{3,6\},\{3,7\},\{4,7\},\{4,8\},\{5,8\},\{5,9\}\}
\end{equation}
for every cell $(t,u)$.
...
Let $V=\{0,\dots,9\}^{1}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=100$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=100$ pairs and each
of the $(9+1)^{1}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223408.
