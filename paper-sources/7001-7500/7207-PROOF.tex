...
\title{The empirical recurrence for OEIS A196858, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A196858 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A196858 is ``Number of n X 3 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 3,4,2,0,1 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
1, 3, 7, 12, 26, 56, 113, 235,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) -a(n-2) +3*a(n-3) -2*a(n-4) +2*a(n-5) -a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (May 09 2018 11:43:38, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) - a(n-2) + 3\,a(n-3) - 2\,a(n-4) + 2\,a(n-5) - a(n-6)
\end{equation}
for all $n>7$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(3,4,2,0,1)
\end{equation}
for every cell $(t,u)$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196858.
