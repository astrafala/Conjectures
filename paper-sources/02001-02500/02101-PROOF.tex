...
\title{The empirical recurrence for OEIS A224683, proved by transfer matrix}
...
\begin{abstract}
OEIS A224683 counts the $(n+4)\times5$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=5875$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224683 is ``Number of (n+4)X5 0..1 matrices with each 5X5 subblock idempotent.''. It has offset $1$ and begins
\[
5682, 2351, 2662, 3465, 4405, 5385, 6713, 8897,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) -a(n-2) +a(n-3) -2*a(n-4) +2*a(n-5) -a(n-7) -a(n-9) +a(n-10) -a(n-11) +a(n-12) for n>18
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:35:34, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $5\times5$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $5\times5$ matrix a \emph{band} when every one of its $1$ contiguous
$5\times5$ subblocks is a window. What the entry counts is the matrices of $5$ columns
whose every $5$ consecutive rows form a band.

Take as states the $4\times5$ strips that occur as the top $4$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=5875$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{20}$ strips of $4$ rows and $5$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $5\times4$ entries, so a band is exactly a walk of $1$ windows in
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) - a(n-2) + a(n-3) - 2\,a(n-4) + 2\,a(n-5) - a(n-7) - a(n-9) + a(n-10) - a(n-11) + a(n-12)
\]
for every $n>18$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224683.
