...
\title{The empirical recurrence for OEIS A250015, proved from an exact model}
...
\begin{abstract}
OEIS A250015 carries an empirical recurrence of order $7$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}6 + 3*n\^{}5 + 5*n\^{}4 + 5*n\^{}3 + 4*n\^{}2 + 2*n. a(n) = 7*a(n-1) - 21*a(n-2) + 35*a(n-3) - 35*a(n-4) + 21*a(n-5) - 7*a(n-6) + a(n-7) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2018 12:43:59, revision 8)
