...
\title{The empirical closed form for OEIS A220154, proved}
...
\begin{abstract}
OEIS A220154 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=48$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
monic recurrence with characteristic polynomial $(x-1)^{3}$; that the walk count
...
OEIS A220154 is ``Number of 2 X n arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 2 X n array.''. It has offset $1$ and begins
\[
6, 11, 23, 50, 88, 137, 197, 268,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (11/2)*n\^{}2 - (23/2)*n + 8 for n>2.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 13 2018 11:13:10, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=48$
...
states number $S=48$. The last filtered row is emitted with no row below it, so it is not a
...
\[
q(x)=(x-1)^{3},\qquad\text{that is}\qquad a(n)=3\,a(n-1) - 3\,a(n-2) + a(n-3).
\]

\begin{theorem}
The closed form OEIS A220154 records is correct for all $n>2$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $5$.
Second, the polynomial and the walk count agree at every index from $3$ up to
$1 + 45 - 1$, which is more than the $3$ consecutive agreements a
common recurrence of order $3$ needs. Two sequences satisfying the same monic recurrence
of order $3$ and agreeing at $3$ consecutive indices past both their thresholds are
equal from there on.
\end{proof}
...
First, the model reproduces all $45$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220154.
