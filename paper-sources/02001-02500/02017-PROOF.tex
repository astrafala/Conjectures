...
\title{The empirical recurrence for OEIS A221517, proved from an exact model}
...
\begin{abstract}
OEIS A221517 carries an empirical recurrence of order $6$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = 1*n\^{}5 - 20*n\^{}3 + 78*n\^{}2 - 146*n + 125 for n>4. a(n) = 6*a(n-1) - 15*a(n-2) + 20*a(n-3) - 15*a(n-4) + 6*a(n-5) - a(n-6) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 06 2018 05:35:49, revision 7)
...
\]
annihilates $a$, and $S=\deg A=18$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=18$, and
