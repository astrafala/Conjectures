...
\title{The empirical recurrence for OEIS A198206, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A198206 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
lumpable, cutting $72354$ pairs down to $S=4018$ states. The entry's empirical recurrence of
order $15$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A198206 is ``Number of 2nX4 0..3 arrays with values 0..3 introduced in row major order and each element unequal to exactly two horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
43, 1765, 50729, 1660672, 56315642, 1930887802, 66941645630, 2331298473193,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 07:49:33, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;65\,a(n-1) - 1221\,a(n-2) + 4909\,a(n-3) + 38995\,a(n-4) + 97941\,a(n-5) - 5336901\,a(n-6) + 24513309\,a(n-7) - 24792291\,a(n-8) + 22615443\,a(n-9) + 29343465\,a(n-10) - 1299185649\,a(n-11) + 509455089\,a(n-12) + 6460607223\,a(n-13) - 508707135\,a(n-14) - 5208653241\,a(n-15)
\end{equation}
for all $n>15$.

\section{The condition, and what the canonical-form clause counts}

The arrays have $4$ columns and $L=2n+0$ rows. Write $x_{t,u}$ for the entry in
...
Fix $i$ and let $V_i=\{0,\dots,i-1\}^{4}$. Because \eqref{eq:loc} at a cell of row
...
partitions of the $2\cdot4$ cells of a pair into at most $i$ classes.
\end{lemma}
...
Assembling the $4$ lumped chains block-diagonally into $N$ of size $S=4018$, with the
...
4!\;a(n)\;=\;\mathbf{v}^{\!\top}N^{\,L-2}\mathbf{u},\qquad L=2n+0 .
\end{equation}

\section{The criterion, and the computation}

Let $q(t)=t^{15}-\sum_i c_i\,t^{15-i}$ be the characteristic polynomial of
\eqref{eq:conj} and put $G=N^{2}$, so that consecutive values of $n$ advance the
walk by $2$ rows.
...
index,
\[
4!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-15}\,
\Bigl(h_{15}-\sum_i c_i h_{15-i}\Bigr)
\]
...
$w=h_{15}-\sum_i c_ih_{15-i}$; and it is enough to check $m=0,1,\dots,S-1$.
\end{lemma}
...
index corresponding to $n=15+1$ onwards, and the run was continued until $S+1$
...
\eqref{eq:walk2}, divided by $4!$, was evaluated at every one of the $15$
...
alignment of the entry's index with $L=2n+0$ would each show up as a mismatch.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198206.
