...
\title{The empirical recurrence for OEIS A124999, proved from an exact model}
...
\begin{abstract}
OEIS A124999 carries an empirical recurrence of order $3$. It is true, and it is
...
OEIS A124999 is ``Number of base 5 circular n-digit numbers with adjacent digits differing by 3 or less.''. It has offset $0$ and begins
\[
1, 5, 23, 101, 467, 2165, 10055, 46709,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
[Empirical] a(base,n)=a(base-1,n)+A025012(n+1) for base>=3.int(n/2)+1 a(n) = 5*a(n-1) - a(n-2) - 3*a(n-3) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:01:43, revision 18)
this is still recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a trace}

Let $G$ be the graph on the digits $0,1,\dots,4$ with an edge between $u$ and $v$ exactly
...
\[
a(n)\;=\;\sum_{u=0}^{4}(M^{n})_{uu}\;=\;\operatorname{tr}M^{n} .
\]
...
string, where the trace is $5$; that is the entry's convention at one index and the two agree
at every $n\ge1$, on every published term.

\section{The bound}

$\operatorname{tr}M^{n}$ is the $n$-th power sum of the eigenvalues of $M$, so by
Cayley--Hamilton the characteristic polynomial of $M$ --- monic of degree exactly $S=5$,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A124999.
