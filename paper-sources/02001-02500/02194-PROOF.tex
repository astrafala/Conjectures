...
\title{The empirical closed form for OEIS A219499, proved}
...
\begin{abstract}
OEIS A219499 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=84$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219499 is ``Number of n X 5 arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 n X 5 array.''. It has offset $1$ and begins
\[
5, 9, 26, 58, 107, 179, 281, 421,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/24)*n\^{}4 + (1/4)*n\^{}3 + (35/24)*n\^{}2 + (21/4)*n - 13 for n>2.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2018 06:35:05, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=84$
...
states number $S=84$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219499 records is correct for all $n>2$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219499.
