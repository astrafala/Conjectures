...
\title{The conjectured generating function for OEIS A202048, proved}
...
\begin{abstract}
OEIS A202048 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=157$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $173$ coefficients in exact arithmetic
...
OEIS A202048 is ``Number of (n+2) X 6 binary arrays avoiding patterns 001 and 110 in rows and columns.''. It has offset $1$ and begins
\[
636, 1968, 4980, 11016, 22092, 41088,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 25 2018 12:21:27, revision 10) this is still
...
\]
and there are $S=157$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $164$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{164}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $173$
of them --- more than the $164+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202048.
