...
\title{A further conjecture on OEIS A208402,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A208402 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $2, 15, 187, 2795, 43947, 700075, 11188907, 178973355,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=21\,a(n-1) - 84\,a(n-2) + 64\,a(n-3) ,
\]
a linear recurrence of order $3$ with characteristic polynomial
\[
  q(x)=x^{3} - 21 x^{2} + 84 x - 64 .
\]
...
\begin{quote}\raggedright a(n) = (8 + 3*2\^{}(1+2*n) + 16\^{}n) / 24.\end{quote}
