...
\title{A further conjecture on OEIS A208640,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A208640 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $16, 89, 284, 722, 1652, 3577, 7504, 15448,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=6\,a(n-1) - 14\,a(n-2) + 16\,a(n-3) - 9\,a(n-4) + 2\,a(n-5) ,
\]
a linear recurrence of order $5$ with characteristic polynomial
\[
  q(x)=x^{5} - 6 x^{4} + 14 x^{3} - 16 x^{2} + 9 x - 2 .
\]
...
\begin{quote}\raggedright a(n) = -84 + 63*2\^{}n - (191*n)/6 - 4*n\^{}2 - n\^{}3/6 for n>2.\end{quote}
...
\[
  p(x)=x^{5} - 6 x^{4} + 14 x^{3} - 16 x^{2} + 9 x - 2 ,
\]
...
indices, and $a$ and $f$ were checked to agree at $n=8,\dots,12$,
