...
\title{The empirical recurrence for OEIS A267472, proved from an exact model}
...
\begin{abstract}
OEIS A267472 carries an empirical recurrence of order $5$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}4 + (17/6)*n\^{}3 + 4*n\^{}2 + (19/6)*n + 1. a(n) = 5*a(n-1) - 10*a(n-2) + 10*a(n-3) - 5*a(n-4) + a(n-5) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 05 2018 09:36:15, revision 7)
