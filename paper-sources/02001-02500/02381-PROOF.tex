...
\title{The empirical closed form for OEIS A199833, proved from an exact model}
...
\begin{abstract}
OEIS A199833 carries an empirical closed form: a polynomial in $n$ of degree $3$, recorded on the
entry and never marked settled. It is true. The entry's count is given exactly by an Ehrhart
quasi-polynomial, from which $a$ provably satisfies a monic linear recurrence of order $S=5$
derived below; a polynomial of degree $3$ satisfies $(z-1)^{3+1}$; so the difference
satisfies the product, and $5+3+1$ consecutive agreements settle it. No transfer matrix is
...
OEIS A199833 is ``Number of -n..n arrays of 4 elements with zero sum and no two neighbors summing to zero.''. It has offset $1$ and begins
\[
4, 40, 140, 336, 660, 1144, 1820, 2720,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = (16/3)*n\^{}3 - (4/3)*n = 4*A000447(n).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 14 2022 09:59:07, revision 18)
...
kills $a$, and $(z-1)^{3+1}$ kills $P$, so $A(z)(z-1)^{3+1}$ --- monic, of order
$5+3+1$ --- kills $r$. The model was evaluated exactly in integer arithmetic and $P$ in
exact rational arithmetic at every index from $n=1$ to $n=1+5+3+1$, and $r$ vanished
at all of them. A monic recurrence with $5+3+1$ consecutive zeros has every later term zero,
so $r\equiv0$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A199833.
