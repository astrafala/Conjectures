...
\title{The empirical recurrence for OEIS A221756, proved by transfer matrix}
...
\begin{abstract}
OEIS A221756 counts the distinct OCCUPANCY arrays obtained when every cell of an $n\times2$ grid
...
an empirical recurrence of order $4$ contributed by R.~H.~Hardin. It is true, and it is
...
one, on $S=32$ states, after which the conjectured recurrence is settled by a finite exact
computation.
\end{abstract}
...
OEIS A221756 is ``Number of 2 X n arrays of occupancy after each element moves to some horizontal or antidiagonal neighbor, without move-in move-out straight through or left turns.''. It has offset $1$ and begins
\[
0, 2, 1, 9, 17, 64, 171, 536,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + 6*a(n-2) + a(n-3) - 4*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2018 17:33:54, revision 8) this is still
...
$S=32$ reachable sets. In particular $a$ is $C$-finite.
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 6\,a(n-2) + a(n-3) - 4\,a(n-4)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221756.
