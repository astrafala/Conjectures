...
\title{The empirical recurrence for OEIS A125316, proved from an exact model}
...
\begin{abstract}
OEIS A125316 carries an empirical recurrence of order $5$. It is true, and it is
...
OEIS A125316 is ``Number of base 6 circular n-digit numbers with adjacent digits differing by 3 or less.''. It has offset $0$ and begins
\[
1, 6, 30, 138, 694, 3526, 18012, 92140,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
[Empirical] a(base,n)=a(base-1,n)+A025012(n+1) for base>=3.int(n/2)+1. a(n) = 6*a(n-1) - 3*a(n-2) - 8*a(n-3) + a(n-4) + 2*a(n-5) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:01:48, revision 17)
...
when $|u-v|\le3$, loops included, and let $M$ be its adjacency matrix, so $M_{uv}=1$ when
$|u-v|\le3$ and $0$ otherwise. A circular $n$-digit string is a cyclic sequence
$d_1,\dots,d_n$ in which every neighbouring pair, the pair $(d_n,d_1)$ included, differs by at
most $3$ --- that is, a CLOSED walk of length $n$ in $G$ together with the choice of where it
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A125316.
