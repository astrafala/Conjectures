...
\title{The empirical closed form for OEIS A219293, proved}
...
\begin{abstract}
OEIS A219293 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=16$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219293 is ``Number of n X 2 arrays of the minimum value of corresponding elements and their horizontal, vertical or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..2 n X 2 array.''. It has offset $1$ and begins
\[
3, 7, 17, 35, 62, 103, 165, 257,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/12)*n\^{}4 - (2/3)*n\^{}3 + (53/12)*n\^{}2 - (17/6)*n - 3 for n > 2.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2018 06:34:48, revision 14)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=16$
...
states number $S=16$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219293 records is correct for all $n>2$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219293.
