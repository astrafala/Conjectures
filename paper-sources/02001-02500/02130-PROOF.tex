...
\title{The empirical recurrence for OEIS A203982, proved by counting patterns}
...
\begin{abstract}
OEIS A203982 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
$15$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A203982 is ``Number of (n+1)X7 0..2 arrays with no 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
6144, 186624, 5668704, 178855776, 5671161216, 180709558848, 5764846339584, 184042295652096,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:55:52, revision 9) the statement is
...
For every $n$ with $m=n+1-1\ge15$,
\[
3!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-15}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-15}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $15$ matrix--vector products
...
corresponding to $n=15+1$ onwards, and the run continued until $w$ was the zero vector,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203982.
