...
\title{A further conjecture on OEIS A204565,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A204565 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $8, 42, 256, 1682, 11448, 79162, 551216, 3849762,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=11\,a(n-1) - 31\,a(n-2) + 21\,a(n-3) ,
\]
a linear recurrence of order $3$ with characteristic polynomial
\[
  q(x)=x^{3} - 11 x^{2} + 31 x - 21 .
\]
...
\begin{quote}\raggedright a(n) = 1/3 + 3\^{}n + (2*7\^{}n)/3.\end{quote}
