...
\title{A further conjecture on OEIS A229472,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A229472 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $0, 1, 2, 6, 20, 70, 246, 854,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=8\,a(n-1) - 22\,a(n-2) + 24\,a(n-3) - 9\,a(n-4) ,
\]
a linear recurrence of order $4$ with characteristic polynomial
\[
  q(x)=x^{4} - 8 x^{3} + 22 x^{2} - 24 x + 9 .
\]
...
\begin{quote}\raggedright a(n) = (27+3\^{}n) * (n-1) / 54 for n>2.\end{quote}
...
indices, and $a$ and $f$ were checked to agree at $n=7,\dots,10$,
