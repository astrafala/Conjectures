...
\title{The empirical column recurrences for the table OEIS A264163}
...
\begin{abstract}
OEIS A264163 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264163 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 0,1 0,-1 0,2 1,0 -1,0 or 2,0.''. It has offset $1$, is
read by antidiagonals, and begins
\[
4, 10, 10, 29, 72, 29, 89, 412,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 2*a(n-1) +2*a(n-2) +2*a(n-3) +4*a(n-5) -a(n-8)\\
k=2: [order 84]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 12:05:44, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 0,1 0,-1 0,2 1,0 -1,0 or 2,0.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;2\,a(n-1) + 2\,a(n-2) + 2\,a(n-3) + 4\,a(n-5) - a(n-8)
\]
...
model reproduces $10$ published values, $31$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264163.
