...
\title{The empirical recurrence for OEIS A197057, proved by transfer matrix}
...
\begin{abstract}
OEIS A197057 counts arrays of width $4$ in which every entry is equal to the NUMBER of its own
...
OEIS A197057 is ``Number of n X 4 0..4 arrays with each element equal to the number its horizontal and vertical neighbors equal to 2.''. It has offset $1$ and begins
\[
1, 4, 10, 18, 28, 47, 93, 189,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) +a(n-3) +2*a(n-4) +3*a(n-5) +2*a(n-6) -a(n-7) -3*a(n-8) -2*a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (May 11 2018 09:53:57, revision 9) this is still
...
that are equal to $2$: writing $v=x(i,j)$,
\[
v\;=\;\#\{\text{neighbours } y \text{ of } (i,j)\ :\ y=2\} .
\]
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=5164.
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-3) + 2\,a(n-4) + 3\,a(n-5) + 2\,a(n-6) - a(n-7) - 3\,a(n-8) - 2\,a(n-9)
\]
for every $n>8$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A197057.
