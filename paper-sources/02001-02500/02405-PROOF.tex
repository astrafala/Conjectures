...
\title{The empirical column recurrences for the table OEIS A251652}
...
\begin{abstract}
OEIS A251652 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A251652 is ``T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with no row, column, diagonal or antidiagonal in any 3X3 subblock summing to 0 2 4 5 7 or 9.''. It has offset $1$, is
read by antidiagonals, and begins
\[
376, 448, 448, 738, 908, 738, 1248, 1928,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:29:02, revision 6) these are still
...
The sequence is ``Number of (n+2)X3 0..3 arrays with no row, column, diagonal or antidiagonal in any 3X3 subblock summing to 0 2 4 5 7 or 9.''. Its rows are the vertices of a digraph on
$S=4161$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + 2\,a(n-2) + 5\,a(n-3) - 7\,a(n-4) - 12\,a(n-5) + 6\,a(n-6) + 6\,a(n-7)
\]
...
model reproduces $9$ published values, $35$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251652.
