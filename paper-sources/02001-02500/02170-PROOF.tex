...
\title{The empirical closed form for OEIS A219520, proved}
...
\begin{abstract}
OEIS A219520 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=38$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219520 is ``Number of 3 X n arrays of the minimum value of corresponding elements and their horizontal, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 3 X n array.''. It has offset $1$ and begins
\[
4, 6, 11, 24, 43, 70, 111, 172,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/24)*n\^{}4 - (1/12)*n\^{}3 - (13/24)*n\^{}2 + (151/12)*n - 22 for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2018 07:59:31, revision 7)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=38$
...
states number $S=38$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219520 records is correct for all $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219520.
