...
\title{A further conjecture on OEIS A198976,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A198976 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 4, 34, 481, 8731, 174454, 3603244, 75251971,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=32\,a(n-1) - 262\,a(n-2) + 672\,a(n-3) - 441\,a(n-4) ,
\]
a linear recurrence of order $4$ with characteristic polynomial
\[
  q(x)=x^{4} - 32 x^{3} + 262 x^{2} - 672 x + 441 .
\]
...
\begin{quote}\raggedright a(n) = (189 + 56*3\^{}n + 18*7\^{}n + 21\^{}n) / 504.\end{quote}
...
\[
  p(x)=x^{4} - 32 x^{3} + 262 x^{2} - 672 x + 441 ,
\]
