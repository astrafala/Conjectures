...
\title{The empirical recurrence for OEIS A203983, proved by counting patterns}
...
\begin{abstract}
OEIS A203983 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
$45$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A203983 is ``Number of (n+1)X8 0..2 arrays with no 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
24576, 1119744, 51018336, 2447270496, 118333620576, 5764846339584, 281455764775776, 13760034485954400,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:55:59, revision 9) the statement is
...
For every $n$ with $m=n+1-1\ge45$,
\[
3!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-45}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-45}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $45$ matrix--vector products
...
corresponding to $n=45+1$ onwards, and the run continued until $w$ was the zero vector,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203983.
