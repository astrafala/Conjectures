...
\title{The empirical recurrence for OEIS A270257, proved by an exact quasi-polynomial}
...
\begin{abstract}
OEIS A270257 counts triangular arrays of a FIXED shape over the alphabet $0..n$, and carries an
empirical recurrence of order $4$ contributed by R.~H.~Hardin. It is true. The array
...
OEIS A270257 is ``Number of 2X2X2 triangular 0..n arrays with some element plus some adjacent element totalling n exactly once.''. It has offset $1$ and begins
\[
0, 12, 24, 48, 72, 108,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) -2*a(n-3) +a(n-4)
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 14 2016 09:18:59, revision 4) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array is the triangle with $2$ rows: cells $(i,j)$ for $0\le i<2$ and $0\le j\le i$,
so $T=3$ of them. Two cells are adjacent when they are neighbours in that triangle --- side
by side in a row, or one directly below the other on either side --- and there are
$3\binom{2}{2}=3$ adjacent pairs. Each cell carries a value in
...
involve $n$, so the sum was formed once by running over all $2^{3}$ subsets and
...
The degree of $A$ is $2$ and that of $C$ is $0$. Both are at most $T-1$,
...
(E-1)^{3}(E+1)^{1},
\]
a monic operator of order $4$, and by no proper divisor of it, since the two
...
$t^{4}-\sum_i c_it^{4-i}$; dividing it by the operator above in
...
First, the closed form reproduces all $47$ terms the entry publishes, exactly, in integer
arithmetic.

Second, the arrays themselves were enumerated for the smallest alphabets --- every assignment
of $0..n$ to the $3$ cells written out and its satisfied pairs counted, with no
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270257.
