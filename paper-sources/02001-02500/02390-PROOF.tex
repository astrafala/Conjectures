...
\title{The empirical recurrence for OEIS A263795, proved by transfer matrix}
...
\begin{abstract}
OEIS A263795 counts arrays of width $5$ over $\{0,1\}$ whose rows, read as base-$2$
numbers, are divisible by $3$ and whose columns, read the same way, are divisible by
...
is then a walk count on $S=433$ states.
\end{abstract}
...
OEIS A263795 is ``Number of (n+1)X(4+1) 0..1 arrays with each row and column divisible by 3, read as a binary number with top and left being the most significant bits, and rows and columns lexicographically nonincreasing.''. It has offset $1$ and begins
\[
3, 3, 7, 7, 17, 18, 56, 66,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 15:38:44, revision 6) this is still
...
$3$ and every $\gamma_j$ be divisible by $3$.

The entry adds that the rows and the columns are each in lexicographic nonincreasing order. Comparing
...
So the vector $\bigl(\gamma_1\bmod 3,\dots,\gamma_{5}\bmod
3\bigr)$ is a state: at most $3^{5}$ of them, and each array row
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=433.
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 9\,a(n-2) - 9\,a(n-3) - 36\,a(n-4) + 36\,a(n-5) + 84\,a(n-6) - 84\,a(n-7) - 126\,a(n-8) + 126\,a(n-9) + 126\,a(n-10) - 126\,a(n-11) - 84\,a(n-12) + 84\,a(n-13) + 36\,a(n-14) - 36\,a(n-15) - 9\,a(n-16) + 9\,a(n-17) + a(n-18) - a(n-19)
\]
for every $n>17$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263795.
