...
\title{The empirical recurrence for OEIS A218051, proved by transfer matrix}
...
\begin{abstract}
OEIS A218051 counts the distinct arrays obtained by applying a fixed local operation to every cell
...
$S=20$ states, after which the conjectured recurrence is settled by a finite exact
computation.
\end{abstract}
...
OEIS A218051 is ``Number of n X 1 arrays of the minimum value of corresponding elements and their horizontal, vertical, diagonal or antidiagonal neighbors in a random 0..3 n X 1 array.''. It has offset $1$ and begins
\[
4, 4, 16, 36, 94, 236, 602, 1528,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + a(n-2) + 2*a(n-4) + a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 19 2018 09:11:29, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

Let $R=\{0,\dots,3\}^{1}$ be the set of rows and let the neighbours of
...
\]
There are $S=20$ reachable sets, generated from $D_0$ outward, and $a$ is $C$-finite.
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + a(n-2) + 2\,a(n-4) + a(n-5)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A218051.
