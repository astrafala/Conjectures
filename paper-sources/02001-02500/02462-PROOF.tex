...
\title{A further conjecture on OEIS A198717,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A198717 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 4, 34, 499, 10507, 272410, 7817980, 234638905,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=55\,a(n-1) - 918\,a(n-2) + 5818\,a(n-3) - 13417\,a(n-4) + 8463\,a(n-5) ,
\]
a linear recurrence of order $5$ with characteristic polynomial
\[
  q(x)=x^{5} - 55 x^{4} + 918 x^{3} - 5818 x^{2} + 13417 x - 8463 .
\]
...
\begin{quote}\raggedright a(n) = 11/30 + 3\^{}n/8 + 7\^{}(n-1)/6 + 13\^{}(n-1)/12 + 31\^{}(n-1)/120.\end{quote}
...
\[
  p(x)=x^{5} - 55 x^{4} + 918 x^{3} - 5818 x^{2} + 13417 x - 8463 ,
\]
