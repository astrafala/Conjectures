...
\title{The empirical recurrence for OEIS A267717, proved by transfer matrix}
...
\begin{abstract}
OEIS A267717 counts arrays in which the REPEATED VALUES of each row, and of each column, stand in a
...
value so far in each column of the scan, so the count is a walk count on $S=4712$ states.
\end{abstract}
...
OEIS A267717 is ``Number of nX6 0..1 arrays with every repeated value in every row and column greater than the previous repeated value.''. It has offset $1$ and begins
\[
16, 256, 1080, 4266, 10489, 26906, 54887, 114392,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2016 20:15:48, revision 4) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=4712,\quad\tau=\mathbf{1} .
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267717.
