...
\title{The empirical recurrence for OEIS A279322, proved by counting patterns with a budget}
...
\begin{abstract}
OEIS A279322 imposes three things at once: a condition on each cell that reaches one row up and
...
from a finite exact computation on $S=28$ states.
\end{abstract}
...
OEIS A279322 is ``Number of n X 1 0..2 arrays with no element equal to a strict majority of its king-move neighbors, with the exception of exactly one element, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
0, 0, 2, 4, 14, 40, 120, 352,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) - 8*a(n-3) - 4*a(n-4) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 21 2018 11:41:28, revision 9) the statement is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A279322.
