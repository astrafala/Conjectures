...
\title{The empirical recurrence for OEIS A209844, proved by counting patterns}
...
\begin{abstract}
OEIS A209844 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
$24$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A209844 is ``Number of (n+1) X 5 0..3 arrays with every 2 X 2 subblock having one or two distinct values, and new values 0..3 introduced in row major order.''. It has offset $1$ and begins
\[
1331, 59563, 2713195, 128454587, 6168792219, 299013108267, 14575079152523, 713014417713003,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 17 2025 16:12:57, revision 13) the statement is
...
For every $n$ with $m=n+1-1\ge24$,
\[
4!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-24}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-24}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $24$ matrix--vector products
...
corresponding to $n=23+1$ onwards, and the run continued until $w$ was the zero vector,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A209844.
