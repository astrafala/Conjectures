...
\title{The empirical recurrence for OEIS A265956, proved by transfer matrix}
...
\begin{abstract}
OEIS A265956 counts arrays of width $2$ in which every entry is equal to the NUMBER of its own
...
OEIS A265956 is ``Number of nX2 integer arrays with each element equal to the number of horizontal, vertical, diagonal and antidiagonal neighbors less than itself.''. It has offset $1$ and begins
\[
3, 75, 235, 4267, 72764, 527311, 7155883, 100785131,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 19 2015 07:47:54, revision 5) this is still
...
neighbours in the named directions, of which there are $8$, so every entry of the array
lies in $\{0,1,\dots,8\}$ and the count is finite. That bound is used below and
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=1861.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A265956.
