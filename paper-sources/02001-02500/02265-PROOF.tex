...
\title{The empirical recurrence for OEIS A208198, proved by counting patterns}
...
\begin{abstract}
OEIS A208198 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
$19$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A208198 is ``Number of nX5 0..3 arrays with new values 0..3 introduced in row major order and no element equal to more than two of its immediate leftward or upward or left-upward diagonal neighbors.''. It has offset $1$ and begins
\[
51, 41272, 39725054, 38372808893, 37069738362057, 35810971705853725, 34594950233928279876, 33420220819293699707599,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 14:42:44, revision 9) the statement is
...
For every $n$ with $m=n+0-1\ge19$,
\[
4!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-19}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-19}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $19$ matrix--vector products
...
corresponding to $n=20+1$ onwards, and the run continued until $w$ was the zero vector,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208198.
