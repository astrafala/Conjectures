...
\title{The conjectured generating function for OEIS A202051, proved}
...
\begin{abstract}
OEIS A202051 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=343$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $365$ coefficients in exact arithmetic
...
OEIS A202051 is ``Number of (n+2) X 9 binary arrays avoiding patterns 001 and 110 in rows and columns.''. It has offset $1$ and begins
\[
1926, 7848, 25650, 71964, 180054, 411696,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 26 2018 08:45:26, revision 11) this is still
...
\]
and there are $S=343$ states. The digraph is built by reading the entry's own English, and the
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=10$ and $\deg D_2=10$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $353$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{353}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $365$
of them --- more than the $353+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202051.
