...
\title{The empirical recurrence for OEIS A224562, proved by transfer matrix}
...
\begin{abstract}
OEIS A224562 counts the $(n+3)\times5$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=366$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224562 is ``Number of (n+3) X 5 0..1 matrices with each 4 X 4 subblock idempotent.''. It has offset $1$ and begins
\[
370, 340, 428, 517, 611, 775, 1044, 1383,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - a(n-2) + a(n-3) - 2*a(n-5) + a(n-6) - 2*a(n-7) + 2*a(n-8) + a(n-11) - a(n-12) for n>15.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 01 2018 09:17:36, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $4\times4$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $4\times5$ matrix a \emph{band} when every one of its $2$ contiguous
$4\times4$ subblocks is a window. What the entry counts is the matrices of $5$ columns
whose every $4$ consecutive rows form a band.

Take as states the $3\times5$ strips that occur as the top $3$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=366$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{15}$ strips of $3$ rows and $5$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $4\times3$ entries, so a band is exactly a walk of $2$ windows in
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) - a(n-2) + a(n-3) - 2\,a(n-5) + a(n-6) - 2\,a(n-7) + 2\,a(n-8) + a(n-11) - a(n-12)
\]
for every $n>15$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224562.
