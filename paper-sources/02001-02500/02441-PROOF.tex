...
\title{The empirical recurrence for OEIS A229473, proved by counting defective colourings}
...
\begin{abstract}
OEIS A229473 counts the colourings of an array with $K=4$ colours that are proper except at
...
numbers. What is left is a walk count on $S=60$ vertices, and the empirical recurrence of
...
OEIS A229473 is ``Number of defective 4-colorings of an n X 2 0..3 array connected horizontally, antidiagonally and vertically with exactly one mistake, and colors introduced in row-major 0..3 order.''. It has offset $1$ and begins
\[
1, 6, 42, 248, 1312, 6528, 31232, 145408,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 8*a(n-1) - 16*a(n-2) for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 17 2018 14:17:48, revision 10) the statement is
...
The arrays have $2$ columns and $L=n$ rows, entries in $\{0,\dots,4-1\}$.
...
Assembling the $4$ chains block-diagonally into $N$ of size $S=60$ with the weights of
...
\[
4!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)=\mathbf{v}^{\!\top}G^{\,j-2}w,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229473.
