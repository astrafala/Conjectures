...
\title{The empirical recurrence for OEIS A214139, proved by transfer matrix}
...
\begin{abstract}
OEIS A214139 counts the proper colourings of a grid $n\times7$, wrapped in the direction of width $7$, with at most $5$ colours and
counted up to renaming the colours, and carries an empirical recurrence of order $9$
...
down to $S=259$ and makes the exact residual test affordable.
\end{abstract}
...
OEIS A214139 is ``Number of 0..4 colorings on an nX7 array circular in the 7 direction with new values 0..4 introduced in row major order.''. It has offset $1$ and begins
\[
147, 522515, 2000093424, 7658621867351, 29325981412599886, 112293482777565310571, 429988214977169477452677, 1646487940037763716348859705,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 16:53:26, revision 8) this is still
...
A row of the array is a proper colouring of the cycle $C_{7}$, since the wrapping makes
its first and last cells adjacent, and two consecutive rows must differ in every one of the
$7$ positions. So $L_i(n)$ is the number of walks of $n-1$ steps in the digraph whose
vertices are the proper colourings of $C_{7}$ over $i$ colours and whose edges join
colourings differing everywhere:
\[
L_i(n)\;=\;\iota^{\!\top}M_i^{\,n-1}\iota ,\qquad \iota=(1,\dots,1)^{\!\top} .
\]
There are $(i-1)^{7}+(-1)^{7}(i-1)$ such colourings, which at $i=5$ is already more
...
state count is $S=259$ rather than the number of colourings.
...
\begin{theorem}
\[
a(n)\;=\;4156\,a(n-1) - 1272609\,a(n-2) + 80884549\,a(n-3) - 1222656178\,a(n-4) - 19887912212\,a(n-5) + 527430399664\,a(n-6) - 2643328241280\,a(n-7) + 3940919419904\,a(n-8) - 1580167634944\,a(n-9)
\]
for every $n>9$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A214139.
