...
\title{The empirical recurrence for OEIS A280399, proved by counting patterns with a budget}
...
\begin{abstract}
OEIS A280399 imposes three things at once: a condition on each cell that reaches one column up and
...
OEIS A280399 is ``Number of 1 X n 0..2 arrays with no element unequal to a strict majority of its horizontal and antidiagonal neighbors, with the exception of exactly one element, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
0, 0, 2, 2, 8, 14, 36, 74,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) + 3*a(n-2) - 4*a(n-3) - 4*a(n-4) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 13 2019 10:16:48, revision 8) the statement is
...
The arrays have $1$ rows and $L=n$ columns; write $x_{t,u}$ for the entry in
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A280399.
