...
\title{The empirical closed form for OEIS A220034, proved}
...
\begin{abstract}
OEIS A220034 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=92$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
monic recurrence with characteristic polynomial $(x-1)^{4}$; that the walk count
...
OEIS A220034 is ``Number of 4 X n arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 4 X n array.''. It has offset $1$ and begins
\[
5, 8, 15, 34, 61, 95, 137, 187,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/6)*n\^{}3 + (1/2)*n\^{}2 + (43/3)*n - 45 for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 07:39:59, revision 7)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=92$
...
states number $S=92$. The last filtered row is emitted with no row below it, so it is not a
...
\[
q(x)=(x-1)^{4},\qquad\text{that is}\qquad a(n)=4\,a(n-1) - 6\,a(n-2) + 4\,a(n-3) - a(n-4).
\]

\begin{theorem}
The closed form OEIS A220034 records is correct for all $n>5$.
\end{theorem}
...
$1 + 45 - 1$, which is more than the $4$ consecutive agreements a
common recurrence of order $4$ needs. Two sequences satisfying the same monic recurrence
of order $4$ and agreeing at $4$ consecutive indices past both their thresholds are
equal from there on.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220034.
