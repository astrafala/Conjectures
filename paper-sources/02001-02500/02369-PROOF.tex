...
\title{The empirical recurrence for OEIS A221685, proved from an exact model}
...
\begin{abstract}
OEIS A221685 carries an empirical recurrence of order $3$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = 18*n\^{}2 + 57*n - 99 for n>4. a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2018 09:14:27, revision 7)
...
\]
annihilates $a$, and $S=\deg A=13$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=13$, and
