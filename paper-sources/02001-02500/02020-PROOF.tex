...
\title{The empirical closed form for OEIS A220036, proved}
...
\begin{abstract}
OEIS A220036 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=657$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
monic recurrence with characteristic polynomial $(x-1)^{6}$; that the walk count
...
OEIS A220036 is ``Number of 6 X n arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 6 X n array.''. It has offset $1$ and begins
\[
7, 12, 28, 83, 187, 358, 613, 962,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/120)*n\^{}5 - (1/12)*n\^{}4 + (47/24)*n\^{}3 - (23/12)*n\^{}2 + (1771/30)*n - 323 for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 08:13:35, revision 7)
this is still recorded as empirical, and it is still open.

\section{What is being counted}

Fix the width $W=6$ and the value range $0..1$. The underlying arrays are the
$6\times n$ arrays over that range whose rows are lexicographically nonincreasing read downwards
...
rows --- but for each of the $5$ adjacent column pairs there are only two possibilities:
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=657$
...
row at a time, holding the last two rows and the $5$ bits of Lemma~\ref{lem:local}, can
...
states number $S=657$. The last filtered row is emitted with no row below it, so it is not a
...
\[
q(x)=(x-1)^{6},\qquad\text{that is}\qquad a(n)=6\,a(n-1) - 15\,a(n-2) + 20\,a(n-3) - 15\,a(n-4) + 6\,a(n-5) - a(n-6).
\]

\begin{theorem}
The closed form OEIS A220036 records is correct for all $n>8$.
\end{theorem}
...
Second, the polynomial and the walk count agree at every index from $9$ up to
$1 + 37 - 1$, which is more than the $6$ consecutive agreements a
common recurrence of order $6$ needs. Two sequences satisfying the same monic recurrence
of order $6$ and agreeing at $6$ consecutive indices past both their thresholds are
equal from there on.
\end{proof}
...
First, the model reproduces all $37$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220036.
