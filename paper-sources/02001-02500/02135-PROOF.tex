...
\title{The empirical recurrence for OEIS A269469, proved from an exact model}
...
\begin{abstract}
OEIS A269469 carries an empirical recurrence of order $6$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}5 + 5*n\^{}4 + 7*n\^{}3 + 2*n\^{}2 - n. a(n) = 6*a(n-1) - 15*a(n-2) + 20*a(n-3) - 15*a(n-4) + 6*a(n-5) - a(n-6) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 23 2019 07:18:50, revision 7)
