...
\title{The empirical recurrence for OEIS A124699, proved from an exact model}
...
\begin{abstract}
OEIS A124699 carries an empirical recurrence of order $6$. It is true, and it is
...
OEIS A124699 is ``Number of base 6 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 6, 16, 36, 92, 236, 622, 1658,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
[Empirical] a(base,n)=a(base-1,n)+A002426(n+1) for base>=1.int(n/2)+1 a(n) = 6*a(n-1) - 10*a(n-2) + 9*a(n-4) - 2*a(n-5) - a(n-6) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:00:09, revision 20)
...
when $|u-v|\le1$, loops included, and let $M$ be its adjacency matrix, so $M_{uv}=1$ when
$|u-v|\le1$ and $0$ otherwise. A circular $n$-digit string is a cyclic sequence
$d_1,\dots,d_n$ in which every neighbouring pair, the pair $(d_n,d_1)$ included, differs by at
most $1$ --- that is, a CLOSED walk of length $n$ in $G$ together with the choice of where it
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) - 10\,a(n-2) + 9\,a(n-4) - 2\,a(n-5) - a(n-6)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A124699.
