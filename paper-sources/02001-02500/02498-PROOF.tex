...
\title{A further conjecture on OEIS A202049,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A202049 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $966, 3304, 9170, 22092, 47950, 95984, 180054, 320180,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=8\,a(n-1) - 28\,a(n-2) + 56\,a(n-3) - 70\,a(n-4) + 56\,a(n-5) - 28\,a(n-6) + 8\,a(n-7) - 1\,a(n-8) ,
\]
a linear recurrence of order $8$ with characteristic polynomial
\[
  q(x)=x^{8} - 8 x^{7} + 28 x^{6} - 56 x^{5} + 70 x^{4} - 56 x^{3} + 28 x^{2} - 8 x + 1 .
\]
...
with at least $8+2$ further terms after that index.

\section{The further conjecture}

Separately from the conjecture already settled, the entry records

\begin{quote}\raggedright Empirical: a(n) = (1/180)*n\^{}7 + (7/36)*n\^{}6 + (511/180)*n\^{}5 + (805/36)*n\^{}4 + (4606/45)*n\^{}3 + (2443/9)*n\^{}2 + (1854/5)*n + 196.\end{quote}
...
\[
  p(x)=x^{8} - 8 x^{7} + 28 x^{6} - 56 x^{5} + 70 x^{4} - 56 x^{3} + 28 x^{2} - 8 x + 1 ,
\]
of order $D=8$. Since $q\mid p$ and $a$ satisfies the recurrence of $q$, $a$ satisfies that of
...
indices, and $a$ and $f$ were checked to agree at $n=9,\dots,16$,
