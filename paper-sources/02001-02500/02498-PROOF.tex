...
\title{A further conjecture on OEIS A229666,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A229666 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $0, 4, 61, 652, 5048, 33152, 197248, 1098752,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=12\,a(n-1) - 48\,a(n-2) + 64\,a(n-3) ,
\]
a linear recurrence of order $3$ with characteristic polynomial
\[
  q(x)=x^{3} - 12 x^{2} + 48 x - 64 .
\]
...
\begin{quote}\raggedright a(n) = 2\^{}(2*n-7) * (106 - 145*n + 50*n\^{}2) for n>3.\end{quote}
