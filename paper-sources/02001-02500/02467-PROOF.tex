...
\title{A further conjecture on OEIS A121138,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A121138 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 1, 3, 10, 46, 192, 840, 3584,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=8\,a(n-1) - 12\,a(n-2) - 32\,a(n-3) + 64\,a(n-4) ,
\]
a linear recurrence of order $4$ with characteristic polynomial
\[
  q(x)=x^{4} - 8 x^{3} + 12 x^{2} + 32 x - 64 .
\]
...
\begin{quote}\raggedright Empirical (for n>=4): a(n) = 2\^{}(n-8)*((n+6)*2\^{}n + 8 - 8*(-1)\^{}n). - \_Vaclav Kotesovec\_, Nov 29 2012\end{quote}
