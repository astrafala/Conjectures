...
\title{The empirical recurrence for OEIS A210121, proved by counting patterns}
...
\begin{abstract}
OEIS A210121 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
$36$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A210121 is ``Number of (n+1)X5 0..3 arrays with every 2X2 subblock having two or four distinct values, and new values 0..3 introduced in row major order.''. It has offset $1$ and begins
\[
1702, 67585, 2737637, 112952832, 4697571356, 196165653701, 8207720539895, 343746384294748,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 15:44:13, revision 9) the statement is
...
For every $n$ with $m=n+1-1\ge36$,
\[
4!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-36}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-36}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $36$ matrix--vector products
...
corresponding to $n=36+1$ onwards, and the run continued until $w$ was the zero vector,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A210121.
