...
\title{The empirical recurrence for OEIS A224574, proved by transfer matrix}
...
\begin{abstract}
OEIS A224574 counts the $(n+5)\times10$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=25223$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224574 is ``Number of (n+5) X 10 0..1 matrices with each 6 X 6 subblock idempotent.''. It has offset $1$ and begins
\[
22096, 7034, 7295, 8143, 8880, 9480, 9946, 11672,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 01 2018 12:36:51, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $6\times6$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $6\times10$ matrix a \emph{band} when every one of its $5$ contiguous
$6\times6$ subblocks is a window. What the entry counts is the matrices of $10$ columns
whose every $6$ consecutive rows form a band.

Take as states the $5\times10$ strips that occur as the top $5$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=25223$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{50}$ strips of $5$ rows and $10$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $6\times5$ entries, so a band is exactly a walk of $5$ windows in
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) - 6\,a(n-2) + 3\,a(n-3) + 3\,a(n-4) - 6\,a(n-5) + 5\,a(n-6) - 4\,a(n-7) + 3\,a(n-8) - 3\,a(n-10) + 3\,a(n-11) - a(n-12)
\]
for every $n>16$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224574.
