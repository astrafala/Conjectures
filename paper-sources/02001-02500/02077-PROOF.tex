...
\title{The empirical recurrence for OEIS A209116, proved from an exact model}
...
\begin{abstract}
OEIS A209116 carries an empirical recurrence of order $6$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by a Burnside sum over the necklace group, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=25$, derived below and not
...
OEIS A209116 is ``Number of 4-bead necklaces labeled with numbers -n..n not allowing reversal, with sum zero and avoiding the patterns z z+1 z+2 and z z-1 z-2.''. It has offset $1$ and begins
\[
5, 22, 55, 120, 221, 362, 559, 816,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) - 3*a(n-2) + 2*a(n-3) - 3*a(n-4) + 3*a(n-5) - a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 08 2018 21:30:53, revision 13)
...
$S=25$, which includes the pre-period $n_0=12$ the conditions with an offset need. The
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A209116.
