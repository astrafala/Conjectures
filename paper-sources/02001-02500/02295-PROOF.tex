...
\title{The empirical column recurrences for the table OEIS A252305}
...
\begin{abstract}
OEIS A252305 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the rows $n=3$.
\end{abstract}
...
OEIS A252305 is ``T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 1 3 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 1 3 6 or 7.''. It has offset $1$, is
read by antidiagonals, and begins
\[
864, 1126, 1048, 1675, 1031, 1303, 2670, 1370,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:03:04, revision 6) these are still
...
\subsection*{Row $n$$=3$}
The sequence is ``Number of (5)X(n+2) 0..3 arrays with every 3X3 subblock row and diagonal sum equal to 0 1 3 6 or 7 and every 3X3 column and antidiagonal sum not equal to 0 1 3 6 or 7.''. Its rows are the vertices of a digraph on
$S=42638$ vertices, edges being the admissible consecutive pairs, and the count is
...
model reproduces $11$ published values, $48$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252305.
