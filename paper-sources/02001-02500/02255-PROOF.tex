...
\title{The empirical recurrence for OEIS A223319, proved by transfer matrix}
...
\begin{abstract}
OEIS A223319 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$6$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is icosahedron graph. The entry carries an empirical recurrence of order $3$
contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical. A slice of
the array would be a state, but there are $37500$ legal slices here and that is too many to
...
then be run on the ORBITS of slices instead of on the slices, and $S=313$ of those suffice.
\end{abstract}
...
OEIS A223319 is ``Rolling icosahedron footprints: number of n X 6 0..11 arrays starting with 0 where 0..11 label vertices of an icosahedron and every array movement to a horizontal or antidiagonal neighbor moves along an icosahedral edge.''. It has offset $1$ and begins
\[
3125, 3570125, 4176940625, 4903804407125, 5759605530667625, 6765094542682458125, 7946162712131677450625, 9333430848774501484101125,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 1366*a(n-1) - 232713*a(n-2) + 9253764*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 19 2018 07:36:46, revision 7) this is still
...
There are $37500$ legal $row$s, which is more than can be carried one at a time. They
need not be.
...
\]
and the count the entry publishes is that divided by $12$, by Lemma~\ref{lem:fibre}. Here $S=|\mathcal{O}|=313$, down from $37500$.
...
$\widehat{M}^{j+3}-\sum_ic_i\widehat{M}^{j+3-i}$. For the bound,
...
\begin{theorem}
\[
a(n)\;=\;1366\,a(n-1) - 232713\,a(n-2) + 9253764\,a(n-3)
\]
for every $n>3$.
\end{theorem}

\begin{proof}
The vector $w=q(\widehat{M})\mathbf{1}$ was formed by $3$ matrix--vector products in
...
$n=3+1$ onwards and the run continued until the working vector was identically zero, which
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223319.
