...
\title{The empirical recurrence for OEIS A224573, proved by transfer matrix}
...
\begin{abstract}
OEIS A224573 counts the $(n+5)\times9$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=22247$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224573 is ``Number of (n+5)X9 0..1 matrices with each 6X6 subblock idempotent.''. It has offset $1$ and begins
\[
18868, 6411, 6659, 7394, 8143, 8714, 9164, 10807,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:28:33, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $6\times6$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $6\times9$ matrix a \emph{band} when every one of its $4$ contiguous
$6\times6$ subblocks is a window. What the entry counts is the matrices of $9$ columns
whose every $6$ consecutive rows form a band.

Take as states the $5\times9$ strips that occur as the top $5$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=22247$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{45}$ strips of $5$ rows and $9$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $6\times5$ entries, so a band is exactly a walk of $4$ windows in
...
until the working vector was identically zero, which settles every larger $j$ at once. The bound is exact: at $n=20$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224573.
