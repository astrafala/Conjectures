...
\title{The empirical recurrence for OEIS A125370, proved from an exact model}
...
\begin{abstract}
OEIS A125370 carries an empirical recurrence of order $3$. It is true, and it is
...
OEIS A125370 is ``Number of base 7 circular n-digit numbers with adjacent digits differing by 5 or less.''. It has offset $0$ and begins
\[
1, 7, 47, 307, 2067, 13927, 93887, 632947,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
[Empirical] a(base,n)=a(base-1,n)+F(5) for base>=5.int(n/2)+1 and F(d) is the largest coefficient in (1+x+...+x\^{}(2d))\^{}n a(n) = 7*a(n-1) - a(n-2) - 5*a(n-3) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:04:21, revision 18)
this is still recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a trace}

Let $G$ be the graph on the digits $0,1,\dots,6$ with an edge between $u$ and $v$ exactly
when $|u-v|\le5$, loops included, and let $M$ be its adjacency matrix, so $M_{uv}=1$ when
$|u-v|\le5$ and $0$ otherwise. A circular $n$-digit string is a cyclic sequence
$d_1,\dots,d_n$ in which every neighbouring pair, the pair $(d_n,d_1)$ included, differs by at
most $5$ --- that is, a CLOSED walk of length $n$ in $G$ together with the choice of where it
starts. Closed walks of length $n$ beginning at $u$ number $(M^n)_{uu}$, so for $n\ge1$
\[
a(n)\;=\;\sum_{u=0}^{6}(M^{n})_{uu}\;=\;\operatorname{tr}M^{n} .
\]
...
string, where the trace is $7$; that is the entry's convention at one index and the two agree
at every $n\ge1$, on every published term.

\section{The bound}

$\operatorname{tr}M^{n}$ is the $n$-th power sum of the eigenvalues of $M$, so by
Cayley--Hamilton the characteristic polynomial of $M$ --- monic of degree exactly $S=7$,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A125370.
