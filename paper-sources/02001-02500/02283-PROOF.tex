...
\title{The empirical recurrence for OEIS A269410, proved from an exact model}
...
\begin{abstract}
OEIS A269410 carries an empirical recurrence of order $5$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}4 + 4*n\^{}3 + (7/2)*n\^{}2 + (1/2)*n. a(n) = 5*a(n-1) - 10*a(n-2) + 10*a(n-3) - 5*a(n-4) + a(n-5) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 21 2019 10:06:51, revision 7)
