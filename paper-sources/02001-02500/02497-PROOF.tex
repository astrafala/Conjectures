...
\title{The empirical recurrence for OEIS A240390, proved by transfer matrix}
...
\begin{abstract}
OEIS A240390 counts arrays over $\{0,1,2,3\}$ in which no entry may equal, modulo $4$, a given
...
count is then a walk count on $S=146$ states.
\end{abstract}
...
OEIS A240390 is ``Number of nX3 0..3 arrays with no element equal to zero plus the sum of elements to its left or one plus the sum of elements above it or two plus the sum of the elements diagonally to its northwest, modulo 4.''. It has offset $1$ and begins
\[
1, 7, 32, 258, 1232, 10291, 48826, 405404,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 09:53:36, revision 6) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=146.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A240390.
