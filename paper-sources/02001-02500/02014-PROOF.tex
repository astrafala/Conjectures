...
\title{The empirical recurrence for OEIS A267718, proved by transfer matrix}
...
\begin{abstract}
OEIS A267718 counts arrays in which the REPEATED VALUES of each row, and of each column, stand in a
...
value so far in each column of the scan, so the count is a walk count on $S=12159$ states.
\end{abstract}
...
OEIS A267718 is ``Number of nX7 0..1 arrays with every repeated value in every row and column greater than the previous repeated value.''. It has offset $1$ and begins
\[
20, 400, 1866, 7879, 20226, 54887, 116036, 255009,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2016 20:16:29, revision 4) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=12159,\quad\tau=\mathbf{1} .
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267718.
