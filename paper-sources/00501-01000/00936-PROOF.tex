...
\title{The cross-base identity for OEIS A126414, proved}
...
\begin{abstract}
OEIS A126414 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126414 is ``Number of base 27 n-digit numbers with adjacent digits differing by two or less.''. It has offset $0$ and begins
\[
1, 27, 129, 625, 3043, 14861, 72723, 356399,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:12:20, revision 9)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $27$ and the permitted difference is $d=2$, so the first clause
claims $a_{27}(n)-a_{26}(n)=5^{\,n-1}$ whenever $2(n-1)+1\le27$, that is for
$1\le n\le14$, and the second claims $a_{27}(n)-a_{26}(n)=5^{\,n-1}-2$ at the single
$n$ with $2(n-1)=27$.
...
arithmetic, $L_{2}$ at base $27$ reproduces all $21$ terms A126414 publishes.
...
First, the model reproduces all $21$ terms A126414 publishes, exactly, in integer
...
$1\le n\le14$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{2}(27,n)$ & $L_{2}(26,n)$ & $5^{\,n-1}$ \\ \hline
1 & 27 & 26 & 1 \\
2 & 129 & 124 & 5 \\
3 & 625 & 600 & 25 \\
4 & 3043 & 2918 & 125 \\
