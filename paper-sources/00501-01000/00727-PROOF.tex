...
\title{The cross-base identity for OEIS A125362, proved}
...
\begin{abstract}
OEIS A125362 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125362 is ``Number of base 25 circular n-digit numbers with adjacent digits differing by 4 or less.''. It has offset $0$ and begins
\[
1, 25, 205, 1345, 10325, 80475, 642601, 5201179,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:27:25, revision 11)
...
entry itself as the largest coefficient of $(1+x+\dots+x^{2d})^{n}$. The entry writes that summand as $\mathrm{A025014}(n+1)$, the $(n+1)$st term of OEIS A025014; that sequence is the central coefficient of $(1+x+\dots+x^{8})^{n}$ and its published terms agree with $F_{4}(n)$ term by term, which is the identification used here. For this entry
the base is $25$ and the permitted difference is $d=4$, so the claim is that
\[
a_{25}(n)-a_{24}(n)\;=\;F_{4}(n)
\qquad\text{for every } n \text{ with } 4\lfloor n/2\rfloor+1\le25,
\]
that is, for $1\le n\le13$.
...
together with a choice of where it starts. Evaluated exactly in integer arithmetic, $T_{4}$
at base $25$ reproduces all $19$ terms A125362 publishes, and at base $24$ all the
...
polynomial. In particular $a_{25}(n)-a_{24}(n)=F_{4}(n)$ for $1\le n\le13$, which
...
First, the model reproduces all $19$ terms A125362 publishes, exactly, in integer
...
$1\le n\le13$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{4}(25,n)$ & $T_{4}(24,n)$ & $F_{4}(n)$ \\ \hline
1 & 25 & 24 & 1 \\
2 & 205 & 196 & 9 \\
3 & 1345 & 1284 & 61 \\
4 & 10325 & 9836 & 489 \\
5 & 80475 & 76524 & 3951 \\
6 & 642601 & 609940 & 32661 \\
7 & 5201179 & 4928052 & 273127 \\
8 & 42555989 & 40249964 & 2306025 \\
\end{tabular}
\end{center}

\noindent and the difference of the second and third columns is the fourth at every row shown
and at every row up to $n=13$.
