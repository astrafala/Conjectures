...
\title{The cross-base identity for OEIS A125372, proved}
...
\begin{abstract}
OEIS A125372 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125372 is ``Number of base-9 circular n-digit numbers with adjacent digits differing by 5 or less.''. It has offset $0$ and begins
\[
1, 9, 69, 489, 3773, 29359, 229371, 1793675,\ \dots
\]
The entry states, as an empirical claim and never marked settled:
\begin{quote}
[Empirical] a(base, n) = a(base-1, n) + F(5) for base >= 5*floor(n/2) + 1 and F(d) is the largest coefficient in (1 + x + ... + x\textasciicircum{}(2d))\textasciicircum{}n.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:29:41, revision 41)
...
the base is $9$ and the permitted difference is $d=5$, so the claim is that
\[
a_{9}(n)-a_{8}(n)\;=\;F_{5}(n)
\qquad\text{for every } n \text{ with } 5\lfloor n/2\rfloor+1\le9,
\]
...
at base $9$ reproduces all $20$ terms A125372 publishes, and at base $8$ all the
...
polynomial. In particular $a_{9}(n)-a_{8}(n)=F_{5}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $20$ terms A125372 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{5}(9,n)$ & $T_{5}(8,n)$ & $F_{5}(n)$ \\ \hline
1 & 9 & 8 & 1 \\
2 & 69 & 58 & 11 \\
3 & 489 & 398 & 91 \\
\end{tabular}
