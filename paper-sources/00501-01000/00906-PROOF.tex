...
\title{The cross-base identity for OEIS A126478, proved}
...
\begin{abstract}
OEIS A126478 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126478 is ``Number of base 10 n-digit numbers with adjacent digits differing by three or less.''. It has offset $0$ and begins
\[
1, 10, 58, 350, 2130, 12990, 79258, 483646,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:13:04, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $10$ and the permitted difference is $d=3$, so the first clause
claims $a_{10}(n)-a_{9}(n)=7^{\,n-1}$ whenever $3(n-1)+1\le10$, that is for
$1\le n\le4$, and the second claims $a_{10}(n)-a_{9}(n)=7^{\,n-1}-2$ at the single
$n$ with $3(n-1)=10$.
...
arithmetic, $L_{3}$ at base $10$ reproduces all $20$ terms A126478 publishes.
...
First, the model reproduces all $20$ terms A126478 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{3}(10,n)$ & $L_{3}(9,n)$ & $7^{\,n-1}$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 58 & 51 & 7 \\
3 & 350 & 301 & 49 \\
4 & 2130 & 1787 & 343 \\
