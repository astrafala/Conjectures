...
\title{The cross-base identity for OEIS A126395, proved}
...
\begin{abstract}
OEIS A126395 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126395 is ``Number of base 8 n-digit numbers with adjacent digits differing by two or less.''. It has offset $0$ and begins
\[
1, 8, 34, 150, 668, 2986, 13362, 59816,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:10:36, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $8$ and the permitted difference is $d=2$, so the first clause
claims $a_{8}(n)-a_{7}(n)=5^{\,n-1}$ whenever $2(n-1)+1\le8$, that is for
$1\le n\le4$, and the second claims $a_{8}(n)-a_{7}(n)=5^{\,n-1}-2$ at the single
$n$ with $2(n-1)=8$.
...
arithmetic, $L_{2}$ at base $8$ reproduces all $22$ terms A126395 publishes.
...
First, the model reproduces all $22$ terms A126395 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{2}(8,n)$ & $L_{2}(7,n)$ & $5^{\,n-1}$ \\ \hline
1 & 8 & 7 & 1 \\
2 & 34 & 29 & 5 \\
3 & 150 & 125 & 25 \\
4 & 668 & 543 & 125 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=5$, where $b=8=2(n-1)$: there $L_{2}(8,5)-L_{2}(7,5)=623$ and $5^{4}-2=623$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126395}.
\end{enumerate}
