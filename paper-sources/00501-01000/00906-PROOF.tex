...
\title{The cross-base identity for OEIS A126380, proved}
...
\begin{abstract}
OEIS A126380 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126380 is ``Number of base 26 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 26, 76, 224, 662, 1960, 5810, 17238,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:10:10, revision 11)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $26$ and the permitted difference is $d=1$, so the first clause
claims $a_{26}(n)-a_{25}(n)=3^{\,n-1}$ whenever $(n-1)+1\le26$, that is for
$1\le n\le26$, and the second claims $a_{26}(n)-a_{25}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=26$.
...
arithmetic, $L_{1}$ at base $26$ reproduces all $25$ terms A126380 publishes.
...
First, the model reproduces all $25$ terms A126380 publishes, exactly, in integer
...
$1\le n\le26$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(26,n)$ & $L_{1}(25,n)$ & $3^{\,n-1}$ \\ \hline
1 & 26 & 25 & 1 \\
2 & 76 & 73 & 3 \\
3 & 224 & 215 & 9 \\
4 & 662 & 635 & 27 \\
...
and at every row up to $n=26$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=27$, where $b=26=(n-1)$: there $L_{1}(26,27)-L_{1}(25,27)=2541865828327$ and $3^{26}-2=2541865828327$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126380}.
\end{enumerate}
