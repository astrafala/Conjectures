...
\title{The cross-base identity for OEIS A126473, proved}
...
\begin{abstract}
OEIS A126473 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126473 is ``Number of strings over a 5 symbol alphabet with adjacent symbols differing by three or less.''. It has offset $0$ and begins
\[
1, 5, 23, 107, 497, 2309, 10727, 49835,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 13 2026 18:15:16, revision 57)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $5$ and the permitted difference is $d=3$, so the first clause
claims $a_{5}(n)-a_{4}(n)=7^{\,n-1}$ whenever $3(n-1)+1\le5$, that is for
$1\le n\le2$, and the second claims $a_{5}(n)-a_{4}(n)=7^{\,n-1}-2$ at the single
$n$ with $3(n-1)=5$.
...
arithmetic, $L_{3}$ at base $5$ reproduces all $24$ terms A126473 publishes.
...
First, the model reproduces all $24$ terms A126473 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{3}(5,n)$ & $L_{3}(4,n)$ & $7^{\,n-1}$ \\ \hline
1 & 5 & 4 & 1 \\
2 & 23 & 16 & 7 \\
\end{tabular}
\end{center}
