...
\title{The cross-base identity for OEIS A126372, proved}
...
\begin{abstract}
OEIS A126372 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126372 is ``Number of base 18 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 18, 52, 152, 446, 1312, 3866, 11406,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:27, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $18$ and the permitted difference is $d=1$, so the first clause
claims $a_{18}(n)-a_{17}(n)=3^{\,n-1}$ whenever $(n-1)+1\le18$, that is for
$1\le n\le18$, and the second claims $a_{18}(n)-a_{17}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=18$.
...
arithmetic, $L_{1}$ at base $18$ reproduces all $25$ terms A126372 publishes.
...
First, the model reproduces all $25$ terms A126372 publishes, exactly, in integer
...
$1\le n\le18$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(18,n)$ & $L_{1}(17,n)$ & $3^{\,n-1}$ \\ \hline
1 & 18 & 17 & 1 \\
2 & 52 & 49 & 3 \\
3 & 152 & 143 & 9 \\
4 & 446 & 419 & 27 \\
...
and at every row up to $n=18$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=19$, where $b=18=(n-1)$: there $L_{1}(18,19)-L_{1}(17,19)=387420487$ and $3^{18}-2=387420487$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126372}.
\end{enumerate}
