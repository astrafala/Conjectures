...
\title{The cross-base identity for OEIS A125344, proved}
...
\begin{abstract}
OEIS A125344 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125344 is ``Number of base 7 circular n-digit numbers with adjacent digits differing by 4 or less.''. It has offset $0$ and begins
\[
1, 7, 43, 247, 1527, 9497, 59245, 369803,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:03:15, revision 16)
...
the base is $7$ and the permitted difference is $d=4$, so the claim is that
\[
a_{7}(n)-a_{6}(n)\;=\;F_{4}(n)
\qquad\text{for every } n \text{ with } 4\lfloor n/2\rfloor+1\le7,
\]
...
at base $7$ reproduces all $21$ terms A125344 publishes, and at base $6$ all the
...
polynomial. In particular $a_{7}(n)-a_{6}(n)=F_{4}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $21$ terms A125344 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{4}(7,n)$ & $T_{4}(6,n)$ & $F_{4}(n)$ \\ \hline
1 & 7 & 6 & 1 \\
2 & 43 & 34 & 9 \\
3 & 247 & 186 & 61 \\
\end{tabular}
