...
\title{The cross-base identity for OEIS A124807, proved}
...
\begin{abstract}
OEIS A124807 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124807 is ``Number of base-6 circular n-digit numbers with adjacent digits differing by 2 or less.''. It has offset $0$ and begins
\[
1, 6, 24, 84, 332, 1336, 5478, 22658,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:15:09, revision 22)
...
the base is $6$ and the permitted difference is $d=2$, so the claim is that
\[
a_{6}(n)-a_{5}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le6,
\]
...
at base $6$ reproduces all $23$ terms A124807 publishes, and at base $5$ all the
...
polynomial. In particular $a_{6}(n)-a_{5}(n)=F_{2}(n)$ for $1\le n\le5$, which
...
First, the model reproduces all $23$ terms A124807 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(6,n)$ & $T_{2}(5,n)$ & $F_{2}(n)$ \\ \hline
1 & 6 & 5 & 1 \\
2 & 24 & 19 & 5 \\
3 & 84 & 65 & 19 \\
4 & 332 & 247 & 85 \\
