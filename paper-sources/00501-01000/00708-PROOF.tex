...
\title{The cross-base identity for OEIS A125319, proved}
...
\begin{abstract}
OEIS A125319 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125319 is ``Number of base 9 circular n-digit numbers with adjacent digits differing by 3 or less.''. It has offset $0$ and begins
\[
1, 9, 51, 249, 1387, 7879, 45663, 267367,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:02:05, revision 17)
...
the base is $9$ and the permitted difference is $d=3$, so the claim is that
\[
a_{9}(n)-a_{8}(n)\;=\;F_{3}(n)
\qquad\text{for every } n \text{ with } 3\lfloor n/2\rfloor+1\le9,
\]
...
at base $9$ reproduces all $21$ terms A125319 publishes, and at base $8$ all the
...
polynomial. In particular $a_{9}(n)-a_{8}(n)=F_{3}(n)$ for $1\le n\le5$, which
...
First, the model reproduces all $21$ terms A125319 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{3}(9,n)$ & $T_{3}(8,n)$ & $F_{3}(n)$ \\ \hline
1 & 9 & 8 & 1 \\
2 & 51 & 44 & 7 \\
3 & 249 & 212 & 37 \\
4 & 1387 & 1156 & 231 \\
