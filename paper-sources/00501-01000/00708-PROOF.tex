...
\title{The empirical recurrence for OEIS A223286, proved by transfer matrix}
...
\begin{abstract}
OEIS A223286 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$5$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is dodecahedron graph. The entry carries an empirical recurrence of order $12$
contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical. A slice of
the array would be a state, but there are $3200000$ legal slices here and that is too many to
carry one by one. The condition names only the edge relation, so the automorphism group of $G$
--- of order $120$ --- acts on the arrays and commutes with the transfer matrix; the walk may
then be run on the ORBITS of slices instead of on the slices, and $S=26800$ of those suffice.
\end{abstract}
...
OEIS A223286 is ``Rolling icosahedron face footprints: number of 5Xn 0..19 arrays starting with 0 where 0..19 label faces of an icosahedron and every array movement to a horizontal, diagonal or antidiagonal neighbor moves across an icosahedral edge.''. It has offset $1$ and begins
\[
160000, 1875, 81057, 1048923, 28271577, 473368227, 10801441521, 200475062187,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 21:27:45, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

Let $G$ be the graph on the label set $\{0,1,\dots,19\}$ whose vertices are the twenty faces of a regular icosahedron, two of them adjacent when they share an edge. That is the dodecahedron graph: $20$ vertices, $3$-regular, $30$ edges.
...
each an image consistent with every vertex already placed, and has order $|\Gamma|=120$.
...
arrays with no condition on any cell, divided by $20$.
\end{lemma}
...
arrays with $x(0,0)=w$, with inverse given by $\sigma^{-1}$. The $20$ fibres of
...
There are $3200000$ legal $column$s, which is more than can be carried one at a time. They
need not be.
...
\]
and the count the entry publishes is that divided by $20$, by Lemma~\ref{lem:fibre}. Here $S=|\mathcal{O}|=26800$, down from $3200000$.
...
$\widehat{M}^{j+12}-\sum_ic_i\widehat{M}^{j+12-i}$. For the bound,
...
\begin{theorem}
\[
a(n)\;=\;13\,a(n-1) + 278\,a(n-2) - 2372\,a(n-3) - 11584\,a(n-4) + 98256\,a(n-5) + 68096\,a(n-6) - 1208064\,a(n-7) + 753664\,a(n-8) + 4509696\,a(n-9) - 4485120\,a(n-10) - 4571136\,a(n-11) + 4718592\,a(n-12)
\]
for every $n>13$.
\end{theorem}

\begin{proof}
The vector $w=q(\widehat{M})\mathbf{1}$ was formed by $12$ matrix--vector products in
...
$n=13+1$ onwards and the run continued until the working vector was identically zero, which
...
inverses---and every one of its $120$ elements verified to preserve the edge set.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223286.
