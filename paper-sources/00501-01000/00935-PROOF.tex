...
\title{The cross-base identity for OEIS A126399, proved}
...
\begin{abstract}
OEIS A126399 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126399 is ``Number of base 12 n-digit numbers with adjacent digits differing by two or less.''. It has offset $0$ and begins
\[
1, 12, 54, 250, 1168, 5486, 25848, 122024,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:10:57, revision 15)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $12$ and the permitted difference is $d=2$, so the first clause
claims $a_{12}(n)-a_{11}(n)=5^{\,n-1}$ whenever $2(n-1)+1\le12$, that is for
$1\le n\le6$, and the second claims $a_{12}(n)-a_{11}(n)=5^{\,n-1}-2$ at the single
$n$ with $2(n-1)=12$.
...
arithmetic, $L_{2}$ at base $12$ reproduces all $22$ terms A126399 publishes.
...
First, the model reproduces all $22$ terms A126399 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{2}(12,n)$ & $L_{2}(11,n)$ & $5^{\,n-1}$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 54 & 49 & 5 \\
3 & 250 & 225 & 25 \\
4 & 1168 & 1043 & 125 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=7$, where $b=12=2(n-1)$: there $L_{2}(12,7)-L_{2}(11,7)=15623$ and $5^{6}-2=15623$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126399}.
\end{enumerate}
