...
\title{The cross-base identity for OEIS A126533, proved}
...
\begin{abstract}
OEIS A126533 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126533 is ``Number of base 12 n-digit numbers with adjacent digits differing by five or less.''. It has offset $0$ and begins
\[
1, 12, 102, 902, 7982, 70654, 625402, 5535834,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:17:27, revision 17)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $12$ and the permitted difference is $d=5$, so the first clause
claims $a_{12}(n)-a_{11}(n)=11^{\,n-1}$ whenever $5(n-1)+1\le12$, that is for
$1\le n\le3$, and the second claims $a_{12}(n)-a_{11}(n)=11^{\,n-1}-2$ at the single
$n$ with $5(n-1)=12$.
...
arithmetic, $L_{5}$ at base $12$ reproduces all $19$ terms A126533 publishes.
...
First, the model reproduces all $19$ terms A126533 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{5}(12,n)$ & $L_{5}(11,n)$ & $11^{\,n-1}$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 102 & 91 & 11 \\
3 & 902 & 781 & 121 \\
\end{tabular}
