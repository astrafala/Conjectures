...
\title{The cross-base identity for OEIS A124719, proved}
...
\begin{abstract}
OEIS A124719 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124719 is ``Number of base 26 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 26, 76, 176, 472, 1256, 3442, 9518,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:14:29, revision 14)
...
the base is $26$ and the permitted difference is $d=1$, so the claim is that
\[
a_{26}(n)-a_{25}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le26,
\]
...
at base $26$ reproduces all $25$ terms A124719 publishes, and at base $25$ all the
...
polynomial. In particular $a_{26}(n)-a_{25}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124719 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(26,n)$ & $T_{1}(25,n)$ & $F_{1}(n)$ \\ \hline
1 & 26 & 25 & 1 \\
2 & 76 & 73 & 3 \\
3 & 176 & 169 & 7 \\
4 & 472 & 453 & 19 \\
