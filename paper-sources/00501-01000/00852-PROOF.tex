...
\title{The cross-base identity for OEIS A126379, proved}
...
\begin{abstract}
OEIS A126379 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126379 is ``Number of base 25 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 25, 73, 215, 635, 1879, 5567, 16509,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:10:04, revision 9)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $25$ and the permitted difference is $d=1$, so the first clause
claims $a_{25}(n)-a_{24}(n)=3^{\,n-1}$ whenever $(n-1)+1\le25$, that is for
$1\le n\le25$, and the second claims $a_{25}(n)-a_{24}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=25$.
...
arithmetic, $L_{1}$ at base $25$ reproduces all $25$ terms A126379 publishes.
...
First, the model reproduces all $25$ terms A126379 publishes, exactly, in integer
...
$1\le n\le25$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(25,n)$ & $L_{1}(24,n)$ & $3^{\,n-1}$ \\ \hline
1 & 25 & 24 & 1 \\
2 & 73 & 70 & 3 \\
3 & 215 & 206 & 9 \\
4 & 635 & 608 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=26$, where $b=25=(n-1)$: there $L_{1}(25,26)-L_{1}(24,26)=847288609441$ and $3^{25}-2=847288609441$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126379}.
\end{enumerate}
