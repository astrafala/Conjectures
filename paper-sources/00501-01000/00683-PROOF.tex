...
\title{The cross-base identity for OEIS A124955, proved}
...
\begin{abstract}
OEIS A124955 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124955 is ``Number of base 27 circular n-digit numbers with adjacent digits differing by 2 or less.''. It has offset $0$ and begins
\[
1, 27, 129, 483, 2117, 9337, 42249, 193493,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:18:06, revision 16)
...
the base is $27$ and the permitted difference is $d=2$, so the claim is that
\[
a_{27}(n)-a_{26}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le27,
\]
that is, for $1\le n\le27$.
...
at base $27$ reproduces all $21$ terms A124955 publishes, and at base $26$ all the
...
polynomial. In particular $a_{27}(n)-a_{26}(n)=F_{2}(n)$ for $1\le n\le27$, which
...
First, the model reproduces all $21$ terms A124955 publishes, exactly, in integer
...
$1\le n\le27$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(27,n)$ & $T_{2}(26,n)$ & $F_{2}(n)$ \\ \hline
1 & 27 & 26 & 1 \\
2 & 129 & 124 & 5 \\
3 & 483 & 464 & 19 \\
4 & 2117 & 2032 & 85 \\
