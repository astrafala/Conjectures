...
\title{The cross-base identity for OEIS A124697, proved}
...
\begin{abstract}
OEIS A124697 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124697 is ``Number of base 4 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 4, 10, 22, 54, 134, 340, 872,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:11:17, revision 32)
...
the base is $4$ and the permitted difference is $d=1$, so the claim is that
\[
a_{4}(n)-a_{3}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le4,
\]
...
at base $4$ reproduces all $30$ terms A124697 publishes, and at base $3$ all the
...
polynomial. In particular $a_{4}(n)-a_{3}(n)=F_{1}(n)$ for $1\le n\le7$, which
...
First, the model reproduces all $30$ terms A124697 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(4,n)$ & $T_{1}(3,n)$ & $F_{1}(n)$ \\ \hline
1 & 4 & 3 & 1 \\
2 & 10 & 7 & 3 \\
3 & 22 & 15 & 7 \\
4 & 54 & 35 & 19 \\
