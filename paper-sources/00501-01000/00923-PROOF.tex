...
\title{The empirical recurrence for OEIS A217978, proved by transfer matrix}
...
\begin{abstract}
OEIS A217978 counts the distinct arrays obtained by applying a fixed local operation to every cell
of an array over a bounded alphabet, and carries an empirical recurrence of order $7$
...
$S=38$ states, after which the conjectured recurrence is settled by a finite exact
computation.
\end{abstract}
...
OEIS A217978 is ``Number of n X 4 arrays of the minimum value of corresponding elements and their horizontal, vertical, diagonal or antidiagonal neighbors in a random 0..1 n X 4 array.''. It has offset $1$ and begins
\[
6, 6, 36, 98, 362, 1180, 4046, 13594,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + 5*a(n-2) + a(n-3) - 5*a(n-4) - 8*a(n-5) + a(n-6) + 2*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2018 15:46:17, revision 7) this is still
...
\]
There are $S=38$ reachable sets, generated from $D_0$ outward, and $a$ is $C$-finite.
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 5\,a(n-2) + a(n-3) - 5\,a(n-4) - 8\,a(n-5) + a(n-6) + 2\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A217978.
