...
\title{The cross-base identity for OEIS A125474, proved}
...
\begin{abstract}
OEIS A125474 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125474 is ``Number of base 17 circular n-digit numbers with adjacent digits differing by 9 or less.''. It has offset $0$ and begins
\[
1, 17, 233, 2897, 39897, 556757, 7818749, 110011877,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:08:32, revision 14)
...
the base is $17$ and the permitted difference is $d=9$, so the claim is that
\[
a_{17}(n)-a_{16}(n)\;=\;F_{9}(n)
\qquad\text{for every } n \text{ with } 9\lfloor n/2\rfloor+1\le17,
\]
...
at base $17$ reproduces all $17$ terms A125474 publishes, and at base $16$ all the
...
polynomial. In particular $a_{17}(n)-a_{16}(n)=F_{9}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $17$ terms A125474 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{9}(17,n)$ & $T_{9}(16,n)$ & $F_{9}(n)$ \\ \hline
1 & 17 & 16 & 1 \\
2 & 233 & 214 & 19 \\
3 & 2897 & 2626 & 271 \\
\end{tabular}
