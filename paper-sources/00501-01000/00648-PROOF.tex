...
\title{The cross-base identity for OEIS A124702, proved}
...
\begin{abstract}
OEIS A124702 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124702 is ``Number of base 9 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 9, 25, 57, 149, 389, 1045, 2837,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:00:25, revision 20)
...
the base is $9$ and the permitted difference is $d=1$, so the claim is that
\[
a_{9}(n)-a_{8}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le9,
\]
...
at base $9$ reproduces all $26$ terms A124702 publishes, and at base $8$ all the
...
polynomial. In particular $a_{9}(n)-a_{8}(n)=F_{1}(n)$ for $1\le n\le17$, which
...
First, the model reproduces all $26$ terms A124702 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(9,n)$ & $T_{1}(8,n)$ & $F_{1}(n)$ \\ \hline
1 & 9 & 8 & 1 \\
2 & 25 & 22 & 3 \\
3 & 57 & 50 & 7 \\
4 & 149 & 130 & 19 \\
