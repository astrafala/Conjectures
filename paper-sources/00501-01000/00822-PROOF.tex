...
\title{The cross-base identity for OEIS A125489, proved}
...
\begin{abstract}
OEIS A125489 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125489 is ``Number of base 32 circular n-digit numbers with adjacent digits differing by 9 or less.''. It has offset $0$ and begins
\[
1, 32, 518, 6962, 108578, 1728482, 28149368, 464599244,\ \dots
\]
The entry states, as an empirical claim and never marked settled:
\begin{quote}
[Empirical] a(base,n)=a(base-1,n)+F(9) for base>=9.int(n/2)+1 and F(d) is the largest coefficient in (1+x+...+x\textasciicircum{}(2d))\textasciicircum{}n
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:41:55, revision 12)
...
the base is $32$ and the permitted difference is $d=9$, so the claim is that
\[
a_{32}(n)-a_{31}(n)\;=\;F_{9}(n)
\qquad\text{for every } n \text{ with } 9\lfloor n/2\rfloor+1\le32,
\]
...
together with a choice of where it starts. Evaluated exactly in integer arithmetic, $T_{9}$
at base $32$ reproduces all $16$ terms A125489 publishes, and at base $31$ all the
...
polynomial. In particular $a_{32}(n)-a_{31}(n)=F_{9}(n)$ for $1\le n\le7$, which
...
First, the model reproduces all $16$ terms A125489 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{9}(32,n)$ & $T_{9}(31,n)$ & $F_{9}(n)$ \\ \hline
1 & 32 & 31 & 1 \\
2 & 518 & 499 & 19 \\
3 & 6962 & 6691 & 271 \\
4 & 108578 & 103999 & 4579 \\
5 & 1728482 & 1650331 & 78151 \\
6 & 28149368 & 26785795 & 1363573 \\
7 & 464599244 & 440527111 & 24072133 \\
\end{tabular}
\end{center}
