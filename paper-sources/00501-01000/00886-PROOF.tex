...
\title{The cross-base identity for OEIS A126476, proved}
...
\begin{abstract}
OEIS A126476 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126476 is ``Number of base 8 n-digit numbers with adjacent digits differing by three or less.''. It has offset $0$ and begins
\[
1, 8, 44, 252, 1446, 8300, 47642, 273466,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:12:53, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $8$ and the permitted difference is $d=3$, so the first clause
claims $a_{8}(n)-a_{7}(n)=7^{\,n-1}$ whenever $3(n-1)+1\le8$, that is for
$1\le n\le3$, and the second claims $a_{8}(n)-a_{7}(n)=7^{\,n-1}-2$ at the single
$n$ with $3(n-1)=8$.
...
arithmetic, $L_{3}$ at base $8$ reproduces all $21$ terms A126476 publishes.
...
First, the model reproduces all $21$ terms A126476 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{3}(8,n)$ & $L_{3}(7,n)$ & $7^{\,n-1}$ \\ \hline
1 & 8 & 7 & 1 \\
2 & 44 & 37 & 7 \\
3 & 252 & 203 & 49 \\
\end{tabular}
