...
\title{The cross-base identity for OEIS A126475, proved}
...
\begin{abstract}
OEIS A126475 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126475 is ``Number of base 7 n-digit numbers with adjacent digits differing by three or less.''. It has offset $0$ and begins
\[
1, 7, 37, 203, 1111, 6083, 33305, 182349,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:12:48, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $7$ and the permitted difference is $d=3$, so the first clause
claims $a_{7}(n)-a_{6}(n)=7^{\,n-1}$ whenever $3(n-1)+1\le7$, that is for
$1\le n\le3$, and the second claims $a_{7}(n)-a_{6}(n)=7^{\,n-1}-2$ at the single
$n$ with $3(n-1)=7$.
...
arithmetic, $L_{3}$ at base $7$ reproduces all $21$ terms A126475 publishes.
...
First, the model reproduces all $21$ terms A126475 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{3}(7,n)$ & $L_{3}(6,n)$ & $7^{\,n-1}$ \\ \hline
1 & 7 & 6 & 1 \\
2 & 37 & 30 & 7 \\
3 & 203 & 154 & 49 \\
\end{tabular}
