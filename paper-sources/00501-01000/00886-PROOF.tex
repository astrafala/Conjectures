...
\title{The cross-base identity for OEIS A126358, proved}
...
\begin{abstract}
OEIS A126358 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126358 is ``Number of base 4 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 4, 10, 26, 68, 178, 466, 1220,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:42:00, revision 49)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $4$ and the permitted difference is $d=1$, so the first clause
claims $a_{4}(n)-a_{3}(n)=3^{\,n-1}$ whenever $(n-1)+1\le4$, that is for
$1\le n\le4$, and the second claims $a_{4}(n)-a_{3}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=4$.
...
arithmetic, $L_{1}$ at base $4$ reproduces all $29$ terms A126358 publishes.
...
First, the model reproduces all $29$ terms A126358 publishes, exactly, in integer
...
$1\le n\le4$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(4,n)$ & $L_{1}(3,n)$ & $3^{\,n-1}$ \\ \hline
1 & 4 & 3 & 1 \\
2 & 10 & 7 & 3 \\
3 & 26 & 17 & 9 \\
4 & 68 & 41 & 27 \\
...
and at every row up to $n=4$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=5$, where $b=4=(n-1)$: there $L_{1}(4,5)-L_{1}(3,5)=79$ and $3^{4}-2=79$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126358}.
\end{enumerate}
