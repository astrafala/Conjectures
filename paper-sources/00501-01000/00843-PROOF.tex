...
\title{The empirical closed form for OEIS A219351, proved}
...
\begin{abstract}
OEIS A219351 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=455$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219351 is ``Number of nX5 arrays of the minimum value of corresponding elements and their horizontal, vertical, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 nX5 array.''. It has offset $1$ and begins
\[
5, 5, 27, 81, 247, 689, 1880, 4994,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 17:59:12, revision 8)
...
$n\times5$ arrays over that range whose rows are lexicographically nondecreasing read downwards
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=455$
...
states number $S=455$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219351 records is correct for all $n>15$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $33$.
Second, the polynomial and the walk count agree at every index from $16$ up to
$1 + 28 - 1$, which is more than the $18$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219351.
