...
\title{The cross-base identity for OEIS A125373, proved}
...
\begin{abstract}
OEIS A125373 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125373 is ``Number of base 10 circular n-digit numbers with adjacent digits differing by 5 or less.''. It has offset $0$ and begins
\[
1, 10, 80, 580, 4660, 37960, 311378, 2559658,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:30:02, revision 28)
...
the base is $10$ and the permitted difference is $d=5$, so the claim is that
\[
a_{10}(n)-a_{9}(n)\;=\;F_{5}(n)
\qquad\text{for every } n \text{ with } 5\lfloor n/2\rfloor+1\le10,
\]
...
at base $10$ reproduces all $20$ terms A125373 publishes, and at base $9$ all the
...
polynomial. In particular $a_{10}(n)-a_{9}(n)=F_{5}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $20$ terms A125373 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{5}(10,n)$ & $T_{5}(9,n)$ & $F_{5}(n)$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 80 & 69 & 11 \\
3 & 580 & 489 & 91 \\
\end{tabular}
