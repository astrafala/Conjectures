...
\title{The empirical recurrence for OEIS A280437, proved by counting patterns with a budget}
...
\begin{abstract}
OEIS A280437 imposes three things at once: a condition on each cell that reaches one row up and
...
OEIS A280437 is ``Number of n X 5 0..1 arrays with no element equal to more than one of its horizontal and vertical neighbors, with the exception of exactly one element, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
5, 38, 77, 138, 225, 360, 574, 920,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) - 4*a(n-2) - 2*a(n-3) + 4*a(n-4) - a(n-6) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 13 2019 10:01:15, revision 7) the statement is
...
one of the $32$ published indices and agrees with the entry's DATA exactly, the quotient
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A280437.
