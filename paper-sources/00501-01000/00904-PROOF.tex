...
\title{The cross-base identity for OEIS A126505, proved}
...
\begin{abstract}
OEIS A126505 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126505 is ``Number of base 10 n-digit numbers with adjacent digits differing by four or less.''. It has offset $0$ and begins
\[
1, 10, 70, 510, 3720, 27142, 198034, 1444904,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:15:10, revision 20)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $10$ and the permitted difference is $d=4$, so the first clause
claims $a_{10}(n)-a_{9}(n)=9^{\,n-1}$ whenever $4(n-1)+1\le10$, that is for
$1\le n\le3$, and the second claims $a_{10}(n)-a_{9}(n)=9^{\,n-1}-2$ at the single
$n$ with $4(n-1)=10$.
...
arithmetic, $L_{4}$ at base $10$ reproduces all $20$ terms A126505 publishes.
...
First, the model reproduces all $20$ terms A126505 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{4}(10,n)$ & $L_{4}(9,n)$ & $9^{\,n-1}$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 70 & 61 & 9 \\
3 & 510 & 429 & 81 \\
\end{tabular}
