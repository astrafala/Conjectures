...
\title{The cross-base identity for OEIS A126385, proved}
...
\begin{abstract}
OEIS A126385 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126385 is ``Number of base 31 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 31, 91, 269, 797, 2365, 7025, 20883,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:44:24, revision 7)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $31$ and the permitted difference is $d=1$, so the first clause
claims $a_{31}(n)-a_{30}(n)=3^{\,n-1}$ whenever $(n-1)+1\le31$, that is for
$1\le n\le31$, and the second claims $a_{31}(n)-a_{30}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=31$.
...
arithmetic, $L_{1}$ at base $31$ reproduces all $24$ terms A126385 publishes.
...
First, the model reproduces all $24$ terms A126385 publishes, exactly, in integer
...
$1\le n\le31$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(31,n)$ & $L_{1}(30,n)$ & $3^{\,n-1}$ \\ \hline
1 & 31 & 30 & 1 \\
2 & 91 & 88 & 3 \\
3 & 269 & 260 & 9 \\
4 & 797 & 770 & 27 \\
...
and at every row up to $n=31$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=32$, where $b=31=(n-1)$: there $L_{1}(31,32)-L_{1}(30,32)=617673396283945$ and $3^{31}-2=617673396283945$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126385}.
\end{enumerate}
