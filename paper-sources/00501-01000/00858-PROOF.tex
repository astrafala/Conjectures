...
\title{The empirical closed form for OEIS A219818, proved}
...
\begin{abstract}
OEIS A219818 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=1298$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219818 is ``Number of 4Xn arrays of the minimum value of corresponding elements and their horizontal, vertical or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 4Xn array.''. It has offset $1$ and begins
\[
10, 33, 169, 648, 2179, 7049, 22017, 65842,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:19:17, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=1298$
...
states number $S=1298$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219818 records is correct for all $n>11$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219818.
