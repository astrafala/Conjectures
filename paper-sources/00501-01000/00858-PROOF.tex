...
\title{The cross-base identity for OEIS A126368, proved}
...
\begin{abstract}
OEIS A126368 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126368 is ``Number of base 14 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 14, 40, 116, 338, 988, 2894, 8490,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:10, revision 12)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $14$ and the permitted difference is $d=1$, so the first clause
claims $a_{14}(n)-a_{13}(n)=3^{\,n-1}$ whenever $(n-1)+1\le14$, that is for
$1\le n\le14$, and the second claims $a_{14}(n)-a_{13}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=14$.
...
arithmetic, $L_{1}$ at base $14$ reproduces all $25$ terms A126368 publishes.
...
First, the model reproduces all $25$ terms A126368 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(14,n)$ & $L_{1}(13,n)$ & $3^{\,n-1}$ \\ \hline
1 & 14 & 13 & 1 \\
2 & 40 & 37 & 3 \\
3 & 116 & 107 & 9 \\
4 & 338 & 311 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=15$, where $b=14=(n-1)$: there $L_{1}(14,15)-L_{1}(13,15)=4782967$ and $3^{14}-2=4782967$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126368}.
\end{enumerate}
