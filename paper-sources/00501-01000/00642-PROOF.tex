...
\title{The cross-base identity for OEIS A124713, proved}
...
\begin{abstract}
OEIS A124713 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124713 is ``Number of base 20 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 20, 58, 134, 358, 950, 2596, 7160,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:01:20, revision 16)
...
the base is $20$ and the permitted difference is $d=1$, so the claim is that
\[
a_{20}(n)-a_{19}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le20,
\]
that is, for $1\le n\le39$.
...
at base $20$ reproduces all $25$ terms A124713 publishes, and at base $19$ all the
...
polynomial. In particular $a_{20}(n)-a_{19}(n)=F_{1}(n)$ for $1\le n\le39$, which
...
First, the model reproduces all $25$ terms A124713 publishes, exactly, in integer
...
$1\le n\le39$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(20,n)$ & $T_{1}(19,n)$ & $F_{1}(n)$ \\ \hline
1 & 20 & 19 & 1 \\
2 & 58 & 55 & 3 \\
3 & 134 & 127 & 7 \\
4 & 358 & 339 & 19 \\
