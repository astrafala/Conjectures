...
\title{The cross-base identity for OEIS A124708, proved}
...
\begin{abstract}
OEIS A124708 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124708 is ``Number of base 15 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 15, 43, 99, 263, 695, 1891, 5195,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:00:52, revision 16)
...
the base is $15$ and the permitted difference is $d=1$, so the claim is that
\[
a_{15}(n)-a_{14}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le15,
\]
...
at base $15$ reproduces all $26$ terms A124708 publishes, and at base $14$ all the
...
polynomial. In particular $a_{15}(n)-a_{14}(n)=F_{1}(n)$ for $1\le n\le29$, which
...
First, the model reproduces all $26$ terms A124708 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(15,n)$ & $T_{1}(14,n)$ & $F_{1}(n)$ \\ \hline
1 & 15 & 14 & 1 \\
2 & 43 & 40 & 3 \\
3 & 99 & 92 & 7 \\
4 & 263 & 244 & 19 \\
