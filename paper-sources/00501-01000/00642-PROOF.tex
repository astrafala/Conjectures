...
\title{The cross-base identity for OEIS A124851, proved}
...
\begin{abstract}
OEIS A124851 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124851 is ``Number of base 9 circular n-digit numbers with adjacent digits differing by 2 or less.''. It has offset $0$ and begins
\[
1, 9, 39, 141, 587, 2479, 10731, 47063,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:15:28, revision 20)
...
the base is $9$ and the permitted difference is $d=2$, so the claim is that
\[
a_{9}(n)-a_{8}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le9,
\]
...
at base $9$ reproduces all $22$ terms A124851 publishes, and at base $8$ all the
...
polynomial. In particular $a_{9}(n)-a_{8}(n)=F_{2}(n)$ for $1\le n\le9$, which
...
First, the model reproduces all $22$ terms A124851 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(9,n)$ & $T_{2}(8,n)$ & $F_{2}(n)$ \\ \hline
1 & 9 & 8 & 1 \\
2 & 39 & 34 & 5 \\
3 & 141 & 122 & 19 \\
4 & 587 & 502 & 85 \\
