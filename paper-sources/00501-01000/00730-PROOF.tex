...
\title{The cross-base identity for OEIS A125398, proved}
...
\begin{abstract}
OEIS A125398 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125398 is ``Number of base 10 circular n-digit numbers with adjacent digits differing by 6 or less.''. It has offset $0$ and begins
\[
1, 10, 88, 724, 6412, 57120, 510214, 4559754,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:05:32, revision 17)
...
the base is $10$ and the permitted difference is $d=6$, so the claim is that
\[
a_{10}(n)-a_{9}(n)\;=\;F_{6}(n)
\qquad\text{for every } n \text{ with } 6\lfloor n/2\rfloor+1\le10,
\]
...
at base $10$ reproduces all $19$ terms A125398 publishes, and at base $9$ all the
...
polynomial. In particular $a_{10}(n)-a_{9}(n)=F_{6}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $19$ terms A125398 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{6}(10,n)$ & $T_{6}(9,n)$ & $F_{6}(n)$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 88 & 75 & 13 \\
3 & 724 & 597 & 127 \\
\end{tabular}
