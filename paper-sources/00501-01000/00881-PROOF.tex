...
\title{The cross-base identity for OEIS A126485, proved}
...
\begin{abstract}
OEIS A126485 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126485 is ``Number of base 17 n-digit numbers with adjacent digits differing by three or less.''. It has offset $0$ and begins
\[
1, 17, 107, 693, 4531, 29785, 196419, 1297863,\ \dots
\]
The entry states, as an empirical claim and never marked settled:
\begin{quote}
Empirical: a(base,n) = a(base-1,n) + 7\textasciicircum{}(n-1) for base >= 3*n-2; a(base,n) = a(base-1,n) + 7\textasciicircum{}(n-1)-2 when base = 3*n-3.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:13:37, revision 15)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $17$ and the permitted difference is $d=3$, so the first clause
claims $a_{17}(n)-a_{16}(n)=7^{\,n-1}$ whenever $3(n-1)+1\le17$, that is for
$1\le n\le6$, and the second claims $a_{17}(n)-a_{16}(n)=7^{\,n-1}-2$ at the single
$n$ with $3(n-1)=17$.
...
arithmetic, $L_{3}$ at base $17$ reproduces all $20$ terms A126485 publishes.
...
First, the model reproduces all $20$ terms A126485 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{3}(17,n)$ & $L_{3}(16,n)$ & $7^{\,n-1}$ \\ \hline
1 & 17 & 16 & 1 \\
2 & 107 & 100 & 7 \\
3 & 693 & 644 & 49 \\
4 & 4531 & 4188 & 343 \\
5 & 29785 & 27384 & 2401 \\
6 & 196419 & 179612 & 16807 \\
\end{tabular}
\end{center}
