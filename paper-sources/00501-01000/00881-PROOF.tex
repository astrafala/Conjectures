...
\title{The cross-base identity for OEIS A126365, proved}
...
\begin{abstract}
OEIS A126365 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126365 is ``Number of base 11 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 11, 31, 89, 257, 745, 2165, 6303,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:08:54, revision 12)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $11$ and the permitted difference is $d=1$, so the first clause
claims $a_{11}(n)-a_{10}(n)=3^{\,n-1}$ whenever $(n-1)+1\le11$, that is for
$1\le n\le11$, and the second claims $a_{11}(n)-a_{10}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=11$.
...
arithmetic, $L_{1}$ at base $11$ reproduces all $25$ terms A126365 publishes.
...
First, the model reproduces all $25$ terms A126365 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(11,n)$ & $L_{1}(10,n)$ & $3^{\,n-1}$ \\ \hline
1 & 11 & 10 & 1 \\
2 & 31 & 28 & 3 \\
3 & 89 & 80 & 9 \\
4 & 257 & 230 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=12$, where $b=11=(n-1)$: there $L_{1}(11,12)-L_{1}(10,12)=177145$ and $3^{11}-2=177145$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126365}.
\end{enumerate}
