...
\title{The cross-base identity for OEIS A126366, proved}
...
\begin{abstract}
OEIS A126366 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126366 is ``Number of base 12 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 12, 34, 98, 284, 826, 2408, 7032,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:00, revision 12)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $12$ and the permitted difference is $d=1$, so the first clause
claims $a_{12}(n)-a_{11}(n)=3^{\,n-1}$ whenever $(n-1)+1\le12$, that is for
$1\le n\le12$, and the second claims $a_{12}(n)-a_{11}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=12$.
...
arithmetic, $L_{1}$ at base $12$ reproduces all $25$ terms A126366 publishes.
...
First, the model reproduces all $25$ terms A126366 publishes, exactly, in integer
...
$1\le n\le12$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(12,n)$ & $L_{1}(11,n)$ & $3^{\,n-1}$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 34 & 31 & 3 \\
3 & 98 & 89 & 9 \\
4 & 284 & 257 & 27 \\
...
and at every row up to $n=12$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=13$, where $b=12=(n-1)$: there $L_{1}(12,13)-L_{1}(11,13)=531439$ and $3^{12}-2=531439$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126366}.
\end{enumerate}
