...
\title{The empirical recurrence for OEIS A218237, proved by transfer matrix}
...
\begin{abstract}
OEIS A218237 counts the distinct indicator arrays obtained from the $n\times3$ arrays over
...
indicator row by row turns it into one, on $S=99$ states, after which the conjectured
...
OEIS A218237 is ``Hilltop maps: number of n X 3 binary arrays indicating the locations of corresponding elements not exceeded by any king-move neighbor in a random 0..3 n X 3 array.''. It has offset $1$ and begins
\[
7, 63, 511, 4095, 32753, 262017, 2096129, 16769025,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 7*a(n-1) + 7*a(n-2) + 7*a(n-3) + 7*a(n-4) + 7*a(n-5) + 7*a(n-6) + 7*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 10 2018 05:46:44, revision 10) this is still
...
\]
There are $S=99$ reachable sets, generated from $D_0$ outward, and $a$ is $C$-finite. Note
...
\begin{theorem}
\[
a(n)\;=\;7\,a(n-1) + 7\,a(n-2) + 7\,a(n-3) + 7\,a(n-4) + 7\,a(n-5) + 7\,a(n-6) + 7\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A218237.
