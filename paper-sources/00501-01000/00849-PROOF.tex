...
\title{The cross-base identity for OEIS A126375, proved}
...
\begin{abstract}
OEIS A126375 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126375 is ``Number of base 21 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 21, 61, 179, 527, 1555, 4595, 13593,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:43, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $21$ and the permitted difference is $d=1$, so the first clause
claims $a_{21}(n)-a_{20}(n)=3^{\,n-1}$ whenever $(n-1)+1\le21$, that is for
$1\le n\le21$, and the second claims $a_{21}(n)-a_{20}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=21$.
...
arithmetic, $L_{1}$ at base $21$ reproduces all $25$ terms A126375 publishes.
...
First, the model reproduces all $25$ terms A126375 publishes, exactly, in integer
...
$1\le n\le21$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(21,n)$ & $L_{1}(20,n)$ & $3^{\,n-1}$ \\ \hline
1 & 21 & 20 & 1 \\
2 & 61 & 58 & 3 \\
3 & 179 & 170 & 9 \\
4 & 527 & 500 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=22$, where $b=21=(n-1)$: there $L_{1}(21,22)-L_{1}(20,22)=10460353201$ and $3^{21}-2=10460353201$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126375}.
\end{enumerate}
