...
\title{The cross-base identity for OEIS A125448, proved}
...
\begin{abstract}
OEIS A125448 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125448 is ``Number of base 13 circular n-digit numbers with adjacent digits differing by 8 or less.''. It has offset $0$ and begins
\[
1, 13, 149, 1597, 18421, 213703, 2485463, 28921087,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:07:38, revision 14)
...
the base is $13$ and the permitted difference is $d=8$, so the claim is that
\[
a_{13}(n)-a_{12}(n)\;=\;F_{8}(n)
\qquad\text{for every } n \text{ with } 8\lfloor n/2\rfloor+1\le13,
\]
...
at base $13$ reproduces all $18$ terms A125448 publishes, and at base $12$ all the
...
polynomial. In particular $a_{13}(n)-a_{12}(n)=F_{8}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $18$ terms A125448 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{8}(13,n)$ & $T_{8}(12,n)$ & $F_{8}(n)$ \\ \hline
1 & 13 & 12 & 1 \\
2 & 149 & 132 & 17 \\
3 & 1597 & 1380 & 217 \\
\end{tabular}
