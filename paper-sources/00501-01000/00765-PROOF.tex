...
\title{A lattice count, not a walk count: OEIS A225314}
...
\begin{abstract}
OEIS A225314 carries an empirical recurrence of order $24$ contributed by R.~H.~Hardin. It is
...
$S=5880$ the order of a linear recurrence it must satisfy. Whether the conjectured recurrence
...
OEIS A225314 is ``Number of 7Xn -1,1 arrays such that the sum over i=1..7,j=1..n of i*x(i,j) is zero and rows are nondecreasing (ways to put n thrusters pointing east or west at each of 7 positions 1..n distance from the hinge of a south-pointing gate without turning the gate).''. It has offset $1$ and begins
\[
8, 87, 474, 1787, 5304, 13333, 29638, 60007,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:43:04, revision 8) this is still
...
becomes $\sum_{i=1}^{7} i\,k_i = T(n)$ with $T(n)=14n$. The arrays are gone: what is left is the lattice points of the box $\{0,\dots,n\}^{7}$ on one hyperplane.
...
where $\sigma(S)$ is the sum of $S$ and $D(m)$ counts the solutions of $\sum i\,k_i=m$ in non-negative integers with no upper bound --- the classical denumerant for the parts $1,\dots,7$. $D$ is a quasi-polynomial in $m$ of degree $6$ with period dividing $\mathrm{lcm}(1,\dots,7)=420$; each argument above is an integer-linear function of $n$, which preserves the degree and can only shrink the period, and the parity case at worst doubles it. So $a$ is a quasi-polynomial of degree at most $6$ and period dividing $840$, hence satisfies a linear recurrence of order at most $S=5880$.
...
so $a$ is $C$-finite of order at most $S=5880$, and the alignment between the model and the
entry's own $n$ is fixed by matching against every published term.

\section{The proof}

\begin{lemma}\label{lem:crit}
Let $q$ be the characteristic polynomial of the conjectured recurrence, of order $24$,
and let $u_j=a(j+24)-\sum_i c_i\,a(j+24-i)$ be its residual. Then $u$ satisfies the
same linear recurrence of order $S=5880$ that $a$ does, so if $u_j=0$ for $S$ consecutive
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-2) + 2\,a(n-3) - 3\,a(n-4) - 5\,a(n-5) + 4\,a(n-7) + a(n-8) - 2\,a(n-9) + a(n-10) + 3\,a(n-11) - 3\,a(n-13) - a(n-14) + 2\,a(n-15) - a(n-16) - 4\,a(n-17) + 5\,a(n-19) + 3\,a(n-20) - 2\,a(n-21) - 3\,a(n-22) + a(n-24)
\]
for every $n>23$.
\end{theorem}
...
$S=5880$ consecutive residuals, and the residuals were formed in exact integer arithmetic.
They vanish from the index corresponding to $n=23+1$ onwards and go on vanishing for more
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A225314.
