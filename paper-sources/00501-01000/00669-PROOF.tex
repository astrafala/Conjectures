...
\title{The cross-base identity for OEIS A125320, proved}
...
\begin{abstract}
OEIS A125320 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125320 is ``Number of base 10 circular n-digit numbers with adjacent digits differing by 3 or less.''. It has offset $0$ and begins
\[
1, 10, 58, 286, 1618, 9330, 54994, 328058,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:02:10, revision 16)
...
the base is $10$ and the permitted difference is $d=3$, so the claim is that
\[
a_{10}(n)-a_{9}(n)\;=\;F_{3}(n)
\qquad\text{for every } n \text{ with } 3\lfloor n/2\rfloor+1\le10,
\]
...
at base $10$ reproduces all $20$ terms A125320 publishes, and at base $9$ all the
...
polynomial. In particular $a_{10}(n)-a_{9}(n)=F_{3}(n)$ for $1\le n\le7$, which
...
First, the model reproduces all $20$ terms A125320 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{3}(10,n)$ & $T_{3}(9,n)$ & $F_{3}(n)$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 58 & 51 & 7 \\
3 & 286 & 249 & 37 \\
4 & 1618 & 1387 & 231 \\
