...
\title{The cross-base identity for OEIS A124701, proved}
...
\begin{abstract}
OEIS A124701 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124701 is ``Number of base 8 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 8, 22, 50, 130, 338, 904, 2444,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:00:20, revision 20)
...
the base is $8$ and the permitted difference is $d=1$, so the claim is that
\[
a_{8}(n)-a_{7}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le8,
\]
...
at base $8$ reproduces all $26$ terms A124701 publishes, and at base $7$ all the
...
polynomial. In particular $a_{8}(n)-a_{7}(n)=F_{1}(n)$ for $1\le n\le15$, which
...
First, the model reproduces all $26$ terms A124701 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(8,n)$ & $T_{1}(7,n)$ & $F_{1}(n)$ \\ \hline
1 & 8 & 7 & 1 \\
2 & 22 & 19 & 3 \\
3 & 50 & 43 & 7 \\
4 & 130 & 111 & 19 \\
