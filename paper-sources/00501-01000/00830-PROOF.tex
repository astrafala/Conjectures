...
\title{The cross-base identity for OEIS A125451, proved}
...
\begin{abstract}
OEIS A125451 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125451 is ``Number of base 16 circular n-digit numbers with adjacent digits differing by 8 or less.''. It has offset $0$ and begins
\[
1, 16, 200, 2248, 28184, 358936, 4608164, 59321572,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:07:55, revision 14)
...
the base is $16$ and the permitted difference is $d=8$, so the claim is that
\[
a_{16}(n)-a_{15}(n)\;=\;F_{8}(n)
\qquad\text{for every } n \text{ with } 8\lfloor n/2\rfloor+1\le16,
\]
...
at base $16$ reproduces all $17$ terms A125451 publishes, and at base $15$ all the
...
polynomial. In particular $a_{16}(n)-a_{15}(n)=F_{8}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $17$ terms A125451 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{8}(16,n)$ & $T_{8}(15,n)$ & $F_{8}(n)$ \\ \hline
1 & 16 & 15 & 1 \\
2 & 200 & 183 & 17 \\
3 & 2248 & 2031 & 217 \\
\end{tabular}
