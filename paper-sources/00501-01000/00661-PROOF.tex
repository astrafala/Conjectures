...
\title{The cross-base identity for OEIS A124712, proved}
...
\begin{abstract}
OEIS A124712 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124712 is ``Number of base 19 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 19, 55, 127, 339, 899, 2455, 6767,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:01:15, revision 17)
...
the base is $19$ and the permitted difference is $d=1$, so the claim is that
\[
a_{19}(n)-a_{18}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le19,
\]
that is, for $1\le n\le37$.
...
at base $19$ reproduces all $25$ terms A124712 publishes, and at base $18$ all the
...
polynomial. In particular $a_{19}(n)-a_{18}(n)=F_{1}(n)$ for $1\le n\le37$, which
...
First, the model reproduces all $25$ terms A124712 publishes, exactly, in integer
...
$1\le n\le37$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(19,n)$ & $T_{1}(18,n)$ & $F_{1}(n)$ \\ \hline
1 & 19 & 18 & 1 \\
2 & 55 & 52 & 3 \\
3 & 127 & 120 & 7 \\
4 & 339 & 320 & 19 \\
