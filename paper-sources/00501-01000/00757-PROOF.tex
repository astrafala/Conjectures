...
\title{The cross-base identity for OEIS A125426, proved}
...
\begin{abstract}
OEIS A125426 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125426 is ``Number of base 14 circular n-digit numbers with adjacent digits differing by 7 or less.''. It has offset $0$ and begins
\[
1, 14, 154, 1526, 16842, 188734, 2131486, 24133858,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:07:05, revision 14)
...
the base is $14$ and the permitted difference is $d=7$, so the claim is that
\[
a_{14}(n)-a_{13}(n)\;=\;F_{7}(n)
\qquad\text{for every } n \text{ with } 7\lfloor n/2\rfloor+1\le14,
\]
...
at base $14$ reproduces all $18$ terms A125426 publishes, and at base $13$ all the
...
polynomial. In particular $a_{14}(n)-a_{13}(n)=F_{7}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $18$ terms A125426 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{7}(14,n)$ & $T_{7}(13,n)$ & $F_{7}(n)$ \\ \hline
1 & 14 & 13 & 1 \\
2 & 154 & 139 & 15 \\
3 & 1526 & 1357 & 169 \\
\end{tabular}
