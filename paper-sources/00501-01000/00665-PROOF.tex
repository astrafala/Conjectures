...
\title{The cross-base identity for OEIS A124716, proved}
...
\begin{abstract}
OEIS A124716 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124716 is ``Number of base 23 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 23, 67, 155, 415, 1103, 3019, 8339,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:01:37, revision 16)
...
the base is $23$ and the permitted difference is $d=1$, so the claim is that
\[
a_{23}(n)-a_{22}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le23,
\]
...
at base $23$ reproduces all $25$ terms A124716 publishes, and at base $22$ all the
...
polynomial. In particular $a_{23}(n)-a_{22}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124716 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(23,n)$ & $T_{1}(22,n)$ & $F_{1}(n)$ \\ \hline
1 & 23 & 22 & 1 \\
2 & 67 & 64 & 3 \\
3 & 155 & 148 & 7 \\
4 & 415 & 396 & 19 \\
