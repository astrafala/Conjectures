...
\title{The cross-base identity for OEIS A124858, proved}
...
\begin{abstract}
OEIS A124858 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124858 is ``Number of base 12 circular n-digit numbers with adjacent digits differing by 2 or less.''. It has offset $0$ and begins
\[
1, 12, 54, 198, 842, 3622, 15984, 71468,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:15:49, revision 16)
...
the base is $12$ and the permitted difference is $d=2$, so the claim is that
\[
a_{12}(n)-a_{11}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le12,
\]
...
at base $12$ reproduces all $22$ terms A124858 publishes, and at base $11$ all the
...
polynomial. In particular $a_{12}(n)-a_{11}(n)=F_{2}(n)$ for $1\le n\le11$, which
...
First, the model reproduces all $22$ terms A124858 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(12,n)$ & $T_{2}(11,n)$ & $F_{2}(n)$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 54 & 49 & 5 \\
3 & 198 & 179 & 19 \\
4 & 842 & 757 & 85 \\
