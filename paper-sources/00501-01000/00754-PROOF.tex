...
\title{The cross-base identity for OEIS A125423, proved}
...
\begin{abstract}
OEIS A125423 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125423 is ``Number of base 11 circular n-digit numbers with adjacent digits differing by 7 or less.''. It has offset $0$ and begins
\[
1, 11, 109, 1019, 10157, 101661, 1019443, 10225933,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:06:48, revision 17)
...
the base is $11$ and the permitted difference is $d=7$, so the claim is that
\[
a_{11}(n)-a_{10}(n)\;=\;F_{7}(n)
\qquad\text{for every } n \text{ with } 7\lfloor n/2\rfloor+1\le11,
\]
...
at base $11$ reproduces all $18$ terms A125423 publishes, and at base $10$ all the
...
polynomial. In particular $a_{11}(n)-a_{10}(n)=F_{7}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $18$ terms A125423 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{7}(11,n)$ & $T_{7}(10,n)$ & $F_{7}(n)$ \\ \hline
1 & 11 & 10 & 1 \\
2 & 109 & 94 & 15 \\
3 & 1019 & 850 & 169 \\
\end{tabular}
