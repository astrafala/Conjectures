...
\title{The cross-base identity for OEIS A125345, proved}
...
\begin{abstract}
OEIS A125345 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125345 is ``Number of base 8 circular n-digit numbers with adjacent digits differing by 4 or less.''. It has offset $0$ and begins
\[
1, 8, 52, 308, 2012, 13308, 88582, 590710,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:03:20, revision 17)
...
the base is $8$ and the permitted difference is $d=4$, so the claim is that
\[
a_{8}(n)-a_{7}(n)\;=\;F_{4}(n)
\qquad\text{for every } n \text{ with } 4\lfloor n/2\rfloor+1\le8,
\]
...
at base $8$ reproduces all $20$ terms A125345 publishes, and at base $7$ all the
...
polynomial. In particular $a_{8}(n)-a_{7}(n)=F_{4}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $20$ terms A125345 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{4}(8,n)$ & $T_{4}(7,n)$ & $F_{4}(n)$ \\ \hline
1 & 8 & 7 & 1 \\
2 & 52 & 43 & 9 \\
3 & 308 & 247 & 61 \\
\end{tabular}
