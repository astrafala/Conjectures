...
\title{The empirical closed form for OEIS A219770, proved}
...
\begin{abstract}
OEIS A219770 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=1107$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219770 is ``Number of nX5 arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 nX5 array.''. It has offset $1$ and begins
\[
5, 12, 65, 291, 1229, 4795, 17186, 57060,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:16:09, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=1107$
...
states number $S=1107$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219770 records is correct for all $n>10$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219770.
