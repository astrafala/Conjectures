...
\title{The cross-base identity for OEIS A124803, proved}
...
\begin{abstract}
OEIS A124803 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124803 is ``Number of base 31 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 31, 91, 211, 567, 1511, 4147, 11483,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:14:57, revision 16)
...
the base is $31$ and the permitted difference is $d=1$, so the claim is that
\[
a_{31}(n)-a_{30}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le31,
\]
...
at base $31$ reproduces all $25$ terms A124803 publishes, and at base $30$ all the
...
polynomial. In particular $a_{31}(n)-a_{30}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124803 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(31,n)$ & $T_{1}(30,n)$ & $F_{1}(n)$ \\ \hline
1 & 31 & 30 & 1 \\
2 & 91 & 88 & 3 \\
3 & 211 & 204 & 7 \\
4 & 567 & 548 & 19 \\
