...
\title{The cross-base identity for OEIS A126536, proved}
...
\begin{abstract}
OEIS A126536 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126536 is ``Number of base 15 n-digit numbers with adjacent digits differing by five or less.''. It has offset $0$ and begins
\[
1, 15, 135, 1265, 11945, 112969, 1068723, 10111095,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:17:38, revision 15)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $15$ and the permitted difference is $d=5$, so the first clause
claims $a_{15}(n)-a_{14}(n)=11^{\,n-1}$ whenever $5(n-1)+1\le15$, that is for
$1\le n\le3$, and the second claims $a_{15}(n)-a_{14}(n)=11^{\,n-1}-2$ at the single
$n$ with $5(n-1)=15$.
...
arithmetic, $L_{5}$ at base $15$ reproduces all $19$ terms A126536 publishes.
...
First, the model reproduces all $19$ terms A126536 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{5}(15,n)$ & $L_{5}(14,n)$ & $11^{\,n-1}$ \\ \hline
1 & 15 & 14 & 1 \\
2 & 135 & 124 & 11 \\
3 & 1265 & 1144 & 121 \\
\end{tabular}
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=4$, where $b=15=5(n-1)$: there $L_{5}(15,4)-L_{5}(14,4)=1329$ and $11^{3}-2=1329$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126536}.
\end{enumerate}
