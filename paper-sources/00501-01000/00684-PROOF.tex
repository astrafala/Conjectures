...
\title{The cross-base identity for OEIS A124726, proved}
...
\begin{abstract}
OEIS A124726 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124726 is ``Number of base 27 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 27, 79, 183, 491, 1307, 3583, 9911,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:14:35, revision 14)
...
the base is $27$ and the permitted difference is $d=1$, so the claim is that
\[
a_{27}(n)-a_{26}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le27,
\]
...
at base $27$ reproduces all $25$ terms A124726 publishes, and at base $26$ all the
...
polynomial. In particular $a_{27}(n)-a_{26}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124726 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(27,n)$ & $T_{1}(26,n)$ & $F_{1}(n)$ \\ \hline
1 & 27 & 26 & 1 \\
2 & 79 & 76 & 3 \\
3 & 183 & 176 & 7 \\
4 & 491 & 472 & 19 \\
