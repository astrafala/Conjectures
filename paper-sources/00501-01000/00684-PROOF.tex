...
\title{The cross-base identity for OEIS A125346, proved}
...
\begin{abstract}
OEIS A125346 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125346 is ``Number of base 9 circular n-digit numbers with adjacent digits differing by 4 or less.''. It has offset $0$ and begins
\[
1, 9, 61, 369, 2501, 17259, 120367, 842739,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:03:25, revision 19)
...
the base is $9$ and the permitted difference is $d=4$, so the claim is that
\[
a_{9}(n)-a_{8}(n)\;=\;F_{4}(n)
\qquad\text{for every } n \text{ with } 4\lfloor n/2\rfloor+1\le9,
\]
...
at base $9$ reproduces all $20$ terms A125346 publishes, and at base $8$ all the
...
polynomial. In particular $a_{9}(n)-a_{8}(n)=F_{4}(n)$ for $1\le n\le5$, which
...
First, the model reproduces all $20$ terms A125346 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{4}(9,n)$ & $T_{4}(8,n)$ & $F_{4}(n)$ \\ \hline
1 & 9 & 8 & 1 \\
2 & 61 & 52 & 9 \\
3 & 369 & 308 & 61 \\
4 & 2501 & 2012 & 489 \\
