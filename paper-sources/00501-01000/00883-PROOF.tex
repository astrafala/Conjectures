...
\title{The cross-base identity for OEIS A126374, proved}
...
\begin{abstract}
OEIS A126374 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126374 is ``Number of base 20 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 20, 58, 170, 500, 1474, 4352, 12864,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:38, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $20$ and the permitted difference is $d=1$, so the first clause
claims $a_{20}(n)-a_{19}(n)=3^{\,n-1}$ whenever $(n-1)+1\le20$, that is for
$1\le n\le20$, and the second claims $a_{20}(n)-a_{19}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=20$.
...
arithmetic, $L_{1}$ at base $20$ reproduces all $25$ terms A126374 publishes.
...
First, the model reproduces all $25$ terms A126374 publishes, exactly, in integer
...
$1\le n\le20$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(20,n)$ & $L_{1}(19,n)$ & $3^{\,n-1}$ \\ \hline
1 & 20 & 19 & 1 \\
2 & 58 & 55 & 3 \\
3 & 170 & 161 & 9 \\
4 & 500 & 473 & 27 \\
...
and at every row up to $n=20$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=21$, where $b=20=(n-1)$: there $L_{1}(20,21)-L_{1}(19,21)=3486784399$ and $3^{20}-2=3486784399$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126374}.
\end{enumerate}
