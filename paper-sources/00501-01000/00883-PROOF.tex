...
\title{The cross-base identity for OEIS A126401, proved}
...
\begin{abstract}
OEIS A126401 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126401 is ``Number of base 14 n-digit numbers with adjacent digits differing by two or less.''. It has offset $0$ and begins
\[
1, 14, 64, 300, 1418, 6736, 32098, 153274,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:11:08, revision 12)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $14$ and the permitted difference is $d=2$, so the first clause
claims $a_{14}(n)-a_{13}(n)=5^{\,n-1}$ whenever $2(n-1)+1\le14$, that is for
$1\le n\le7$, and the second claims $a_{14}(n)-a_{13}(n)=5^{\,n-1}-2$ at the single
$n$ with $2(n-1)=14$.
...
arithmetic, $L_{2}$ at base $14$ reproduces all $22$ terms A126401 publishes.
...
First, the model reproduces all $22$ terms A126401 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{2}(14,n)$ & $L_{2}(13,n)$ & $5^{\,n-1}$ \\ \hline
1 & 14 & 13 & 1 \\
2 & 64 & 59 & 5 \\
3 & 300 & 275 & 25 \\
4 & 1418 & 1293 & 125 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=8$, where $b=14=2(n-1)$: there $L_{2}(14,8)-L_{2}(13,8)=78123$ and $5^{7}-2=78123$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126401}.
\end{enumerate}
