...
\title{The cross-base identity for OEIS A124784, proved}
...
\begin{abstract}
OEIS A124784 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124784 is ``Number of base 29 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 29, 85, 197, 529, 1409, 3865, 10697,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:14:46, revision 14)
...
the base is $29$ and the permitted difference is $d=1$, so the claim is that
\[
a_{29}(n)-a_{28}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le29,
\]
...
at base $29$ reproduces all $25$ terms A124784 publishes, and at base $28$ all the
...
polynomial. In particular $a_{29}(n)-a_{28}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124784 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(29,n)$ & $T_{1}(28,n)$ & $F_{1}(n)$ \\ \hline
1 & 29 & 28 & 1 \\
2 & 85 & 82 & 3 \\
3 & 197 & 190 & 7 \\
4 & 529 & 510 & 19 \\
