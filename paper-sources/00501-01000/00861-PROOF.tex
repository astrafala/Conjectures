...
\title{The cross-base identity for OEIS A126371, proved}
...
\begin{abstract}
OEIS A126371 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126371 is ``Number of base 17 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 17, 49, 143, 419, 1231, 3623, 10677,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 16:52:42, revision 22)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $17$ and the permitted difference is $d=1$, so the first clause
claims $a_{17}(n)-a_{16}(n)=3^{\,n-1}$ whenever $(n-1)+1\le17$, that is for
$1\le n\le17$, and the second claims $a_{17}(n)-a_{16}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=17$.
...
arithmetic, $L_{1}$ at base $17$ reproduces all $25$ terms A126371 publishes.
...
First, the model reproduces all $25$ terms A126371 publishes, exactly, in integer
...
$1\le n\le17$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(17,n)$ & $L_{1}(16,n)$ & $3^{\,n-1}$ \\ \hline
1 & 17 & 16 & 1 \\
2 & 49 & 46 & 3 \\
3 & 143 & 134 & 9 \\
4 & 419 & 392 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=18$, where $b=17=(n-1)$: there $L_{1}(17,18)-L_{1}(16,18)=129140161$ and $3^{17}-2=129140161$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126371}.
\end{enumerate}
