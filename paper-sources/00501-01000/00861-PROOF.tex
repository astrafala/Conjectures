...
\title{The empirical closed form for OEIS A219596, proved}
...
\begin{abstract}
OEIS A219596 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=179$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
monic recurrence with characteristic polynomial $(x-1)^{10}$; that the walk count
...
OEIS A219596 is ``Number of 3 X n arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 3 X n array.''. It has offset $1$ and begins
\[
6, 21, 84, 233, 550, 1188, 2415, 4684,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 00:34:04, revision 9)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=179$
...
states number $S=179$. The last filtered row is emitted with no row below it, so it is not a
...
\[
q(x)=(x-1)^{10},\qquad\text{that is}\qquad a(n)=10\,a(n-1) - 45\,a(n-2) + 120\,a(n-3) - 210\,a(n-4) + 252\,a(n-5) - 210\,a(n-6) + 120\,a(n-7) - 45\,a(n-8) + 10\,a(n-9) - a(n-10).
\]

\begin{theorem}
The closed form OEIS A219596 records is correct for all $n>3$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $13$.
Second, the polynomial and the walk count agree at every index from $4$ up to
$1 + 31 - 1$, which is more than the $10$ consecutive agreements a
common recurrence of order $10$ needs. Two sequences satisfying the same monic recurrence
of order $10$ and agreeing at $10$ consecutive indices past both their thresholds are
equal from there on.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219596.
