...
\title{A lattice count, not a walk count: OEIS A225312}
...
\begin{abstract}
OEIS A225312 carries an empirical recurrence of order $18$ contributed by R.~H.~Hardin. It is
...
$S=600$ the order of a linear recurrence it must satisfy. Whether the conjectured recurrence
...
OEIS A225312 is ``Number of 5 X n -1,1 arrays such that the sum over i=1..5,j=1..n of i*x(i,j) is zero and rows are nondecreasing (ways to put n thrusters pointing east or west at each of 5 positions 1..n distance from the hinge of a south-pointing gate without turning the gate).''. It has offset $1$ and begins
\[
0, 15, 0, 113, 0, 427, 0, 1165,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-2) - 2*a(n-4) - 2*a(n-6) + 4*a(n-8) - 4*a(n-10) + 2*a(n-12) + 2*a(n-14) - 3*a(n-16) + a(n-18).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 05 2018 08:12:24, revision 8) this is still
...
becomes $\sum_{i=1}^{5} i\,k_i = T(n)$ with $T(n)=\tfrac{15}{2}n$. The arrays are gone: what is left is the lattice points of the box $\{0,\dots,n\}^{5}$ on one hyperplane.
...
where $\sigma(S)$ is the sum of $S$ and $D(m)$ counts the solutions of $\sum i\,k_i=m$ in non-negative integers with no upper bound --- the classical denumerant for the parts $1,\dots,5$. $D$ is a quasi-polynomial in $m$ of degree $4$ with period dividing $\mathrm{lcm}(1,\dots,5)=60$; each argument above is an integer-linear function of $n$, which preserves the degree and can only shrink the period, and the parity case at worst doubles it. So $a$ is a quasi-polynomial of degree at most $4$ and period dividing $120$, hence satisfies a linear recurrence of order at most $S=600$.
...
so $a$ is $C$-finite of order at most $S=600$, and the alignment between the model and the
entry's own $n$ is fixed by matching against every published term.

\section{The proof}

\begin{lemma}\label{lem:crit}
Let $q$ be the characteristic polynomial of the conjectured recurrence, of order $18$,
and let $u_j=a(j+18)-\sum_i c_i\,a(j+18-i)$ be its residual. Then $u$ satisfies the
same linear recurrence of order $S=600$ that $a$ does, so if $u_j=0$ for $S$ consecutive
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-2) - 2\,a(n-4) - 2\,a(n-6) + 4\,a(n-8) - 4\,a(n-10) + 2\,a(n-12) + 2\,a(n-14) - 3\,a(n-16) + a(n-18)
\]
for every $n>17$.
\end{theorem}
...
$S=600$ consecutive residuals, and the residuals were formed in exact integer arithmetic.
They vanish from the index corresponding to $n=17+1$ onwards and go on vanishing for more
...
First, the model reproduces all $51$ terms the entry publishes, exactly, in integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A225312.
