...
\title{The cross-base identity for OEIS A126503, proved}
...
\begin{abstract}
OEIS A126503 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126503 is ``Number of base 8 n-digit numbers with adjacent digits differing by four or less.''. It has offset $0$ and begins
\[
1, 8, 52, 348, 2322, 15500, 103462, 690610,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:14:59, revision 15)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $8$ and the permitted difference is $d=4$, so the first clause
claims $a_{8}(n)-a_{7}(n)=9^{\,n-1}$ whenever $4(n-1)+1\le8$, that is for
$1\le n\le2$, and the second claims $a_{8}(n)-a_{7}(n)=9^{\,n-1}-2$ at the single
$n$ with $4(n-1)=8$.
...
arithmetic, $L_{4}$ at base $8$ reproduces all $20$ terms A126503 publishes.
...
First, the model reproduces all $20$ terms A126503 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{4}(8,n)$ & $L_{4}(7,n)$ & $9^{\,n-1}$ \\ \hline
1 & 8 & 7 & 1 \\
2 & 52 & 43 & 9 \\
\end{tabular}
\end{center}
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=3$, where $b=8=4(n-1)$: there $L_{4}(8,3)-L_{4}(7,3)=79$ and $9^{2}-2=79$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126503}.
\end{enumerate}
