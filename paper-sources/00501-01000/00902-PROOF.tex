...
\title{The cross-base identity for OEIS A126398, proved}
...
\begin{abstract}
OEIS A126398 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126398 is ``Number of base 11 n-digit numbers with adjacent digits differing by two or less.''. It has offset $0$ and begins
\[
1, 11, 49, 225, 1043, 4861, 22723, 106401,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:10:52, revision 15)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $11$ and the permitted difference is $d=2$, so the first clause
claims $a_{11}(n)-a_{10}(n)=5^{\,n-1}$ whenever $2(n-1)+1\le11$, that is for
$1\le n\le6$, and the second claims $a_{11}(n)-a_{10}(n)=5^{\,n-1}-2$ at the single
$n$ with $2(n-1)=11$.
...
arithmetic, $L_{2}$ at base $11$ reproduces all $22$ terms A126398 publishes.
...
First, the model reproduces all $22$ terms A126398 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{2}(11,n)$ & $L_{2}(10,n)$ & $5^{\,n-1}$ \\ \hline
1 & 11 & 10 & 1 \\
2 & 49 & 44 & 5 \\
3 & 225 & 200 & 25 \\
4 & 1043 & 918 & 125 \\
