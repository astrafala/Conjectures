...
\title{The cross-base identity for OEIS A126383, proved}
...
\begin{abstract}
OEIS A126383 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126383 is ``Number of base 29 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 29, 85, 251, 743, 2203, 6539, 19425,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:10:25, revision 9)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $29$ and the permitted difference is $d=1$, so the first clause
claims $a_{29}(n)-a_{28}(n)=3^{\,n-1}$ whenever $(n-1)+1\le29$, that is for
$1\le n\le29$, and the second claims $a_{29}(n)-a_{28}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=29$.
...
arithmetic, $L_{1}$ at base $29$ reproduces all $24$ terms A126383 publishes.
...
First, the model reproduces all $24$ terms A126383 publishes, exactly, in integer
...
$1\le n\le29$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(29,n)$ & $L_{1}(28,n)$ & $3^{\,n-1}$ \\ \hline
1 & 29 & 28 & 1 \\
2 & 85 & 82 & 3 \\
3 & 251 & 242 & 9 \\
4 & 743 & 716 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=30$, where $b=29=(n-1)$: there $L_{1}(29,30)-L_{1}(28,30)=68630377364881$ and $3^{29}-2=68630377364881$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126383}.
\end{enumerate}
