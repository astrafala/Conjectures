...
\title{The empirical closed form for OEIS A220004, proved}
...
\begin{abstract}
OEIS A220004 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=28$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220004 is ``Number of n X 2 arrays of the minimum value of corresponding elements and their horizontal, vertical or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 n X 2 array.''. It has offset $1$ and begins
\[
4, 7, 20, 58, 141, 308, 637, 1271,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 29 2018 10:34:20, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=28$
...
states number $S=28$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220004 records is correct for all $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220004.
