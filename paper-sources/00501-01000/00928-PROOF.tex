...
\title{The cross-base identity for OEIS A126384, proved}
...
\begin{abstract}
OEIS A126384 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126384 is ``Number of base 30 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 30, 88, 260, 770, 2284, 6782, 20154,\ \dots
\]
The entry states, as an empirical claim and never marked settled:
\begin{quote}
[Empirical] a(base,n)=a(base-1,n)+3\textasciicircum{}(n-1) for base>=n; a(base,n)=a(base-1,n)+3\textasciicircum{}(n-1)-2 when base=n-1
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:10:31, revision 9)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $30$ and the permitted difference is $d=1$, so the first clause
claims $a_{30}(n)-a_{29}(n)=3^{\,n-1}$ whenever $(n-1)+1\le30$, that is for
$1\le n\le30$, and the second claims $a_{30}(n)-a_{29}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=30$.
...
arithmetic, $L_{1}$ at base $30$ reproduces all $24$ terms A126384 publishes.
...
First, the model reproduces all $24$ terms A126384 publishes, exactly, in integer
...
$1\le n\le30$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(30,n)$ & $L_{1}(29,n)$ & $3^{\,n-1}$ \\ \hline
1 & 30 & 29 & 1 \\
2 & 88 & 85 & 3 \\
3 & 260 & 251 & 9 \\
4 & 770 & 743 & 27 \\
...
and at every row up to $n=30$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=31$, where $b=30=(n-1)$: there $L_{1}(30,31)-L_{1}(29,31)=205891132094647$ and $3^{30}-2=205891132094647$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126384}.
\end{enumerate}
