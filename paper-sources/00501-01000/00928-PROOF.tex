...
\title{The cross-base identity for OEIS A126502, proved}
...
\begin{abstract}
OEIS A126502 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126502 is ``Number of base 7 n-digit numbers with adjacent digits differing by four or less.''. It has offset $0$ and begins
\[
1, 7, 43, 269, 1679, 10483, 65449, 408623,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:14:53, revision 15)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $7$ and the permitted difference is $d=4$, so the first clause
claims $a_{7}(n)-a_{6}(n)=9^{\,n-1}$ whenever $4(n-1)+1\le7$, that is for
$1\le n\le2$, and the second claims $a_{7}(n)-a_{6}(n)=9^{\,n-1}-2$ at the single
$n$ with $4(n-1)=7$.
...
arithmetic, $L_{4}$ at base $7$ reproduces all $20$ terms A126502 publishes.
...
First, the model reproduces all $20$ terms A126502 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{4}(7,n)$ & $L_{4}(6,n)$ & $9^{\,n-1}$ \\ \hline
1 & 7 & 6 & 1 \\
2 & 43 & 34 & 9 \\
\end{tabular}
\end{center}
