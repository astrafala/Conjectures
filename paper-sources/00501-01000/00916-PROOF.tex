...
\title{The cross-base identity for OEIS A126507, proved}
...
\begin{abstract}
OEIS A126507 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126507 is ``Number of base 12 n-digit numbers with adjacent digits differing by four or less.''. It has offset $0$ and begins
\[
1, 12, 88, 672, 5168, 39796, 306520, 2361008,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:15:20, revision 17)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $12$ and the permitted difference is $d=4$, so the first clause
claims $a_{12}(n)-a_{11}(n)=9^{\,n-1}$ whenever $4(n-1)+1\le12$, that is for
$1\le n\le3$, and the second claims $a_{12}(n)-a_{11}(n)=9^{\,n-1}-2$ at the single
$n$ with $4(n-1)=12$.
...
arithmetic, $L_{4}$ at base $12$ reproduces all $20$ terms A126507 publishes.
...
First, the model reproduces all $20$ terms A126507 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{4}(12,n)$ & $L_{4}(11,n)$ & $9^{\,n-1}$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 88 & 79 & 9 \\
3 & 672 & 591 & 81 \\
\end{tabular}
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=4$, where $b=12=4(n-1)$: there $L_{4}(12,4)-L_{4}(11,4)=727$ and $9^{3}-2=727$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126507}.
\end{enumerate}
