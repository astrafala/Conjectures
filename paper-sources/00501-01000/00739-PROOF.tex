...
\title{The cross-base identity for OEIS A125347, proved}
...
\begin{abstract}
OEIS A125347 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125347 is ``Number of base 10 circular n-digit numbers with adjacent digits differing by 4 or less.''. It has offset $0$ and begins
\[
1, 10, 70, 430, 2990, 21210, 152752, 1107368,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:03:31, revision 13)
...
the base is $10$ and the permitted difference is $d=4$, so the claim is that
\[
a_{10}(n)-a_{9}(n)\;=\;F_{4}(n)
\qquad\text{for every } n \text{ with } 4\lfloor n/2\rfloor+1\le10,
\]
...
at base $10$ reproduces all $20$ terms A125347 publishes, and at base $9$ all the
...
polynomial. In particular $a_{10}(n)-a_{9}(n)=F_{4}(n)$ for $1\le n\le5$, which
...
First, the model reproduces all $20$ terms A125347 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{4}(10,n)$ & $T_{4}(9,n)$ & $F_{4}(n)$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 70 & 61 & 9 \\
3 & 430 & 369 & 61 \\
4 & 2990 & 2501 & 489 \\
