...
\title{The empirical recurrence for OEIS A187858, proved from an exact model}
...
\begin{abstract}
OEIS A187858 carries an empirical recurrence of order $3$. It is true, and it is
...
OEIS A187858 is ``Number of 3-step one space for components leftwards or up, two space for components rightwards or down asymmetric quasi-queen's tours (antidiagonal moves become knight moves) on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 2, 81, 254, 578, 1030, 1610, 2318,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 64*n\^{}2 - 252*n + 238 for n>3. a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 26 2018 08:45:41, revision 10)
...
SHAPE when one is a translate of the other. There are exactly $64$ shapes: self-avoiding
sequences of $3$ distinct cells in $\mathbf{Z}^{2}$ with each consecutive difference in
...
Write $w^\ast$ for the largest bounding-box side over all $64$ shapes. Once $n\ge w^\ast$
...
vanish from $n=6+1$ onwards, and the run exceeds $S+3$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187858.
