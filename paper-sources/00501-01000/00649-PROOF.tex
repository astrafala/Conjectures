...
\title{The cross-base identity for OEIS A124783, proved}
...
\begin{abstract}
OEIS A124783 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124783 is ``Number of base 28 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 28, 82, 190, 510, 1358, 3724, 10304,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:14:41, revision 14)
...
the base is $28$ and the permitted difference is $d=1$, so the claim is that
\[
a_{28}(n)-a_{27}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le28,
\]
...
at base $28$ reproduces all $25$ terms A124783 publishes, and at base $27$ all the
...
polynomial. In particular $a_{28}(n)-a_{27}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124783 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(28,n)$ & $T_{1}(27,n)$ & $F_{1}(n)$ \\ \hline
1 & 28 & 27 & 1 \\
2 & 82 & 79 & 3 \\
3 & 190 & 183 & 7 \\
4 & 510 & 491 & 19 \\
