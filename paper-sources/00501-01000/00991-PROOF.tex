...
\title{The cross-base identity for OEIS A126511, proved}
...
\begin{abstract}
OEIS A126511 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126511 is ``Number of base 16 n-digit numbers with adjacent digits differing by four or less.''. It has offset $0$ and begins
\[
1, 16, 124, 996, 8084, 65928, 538668, 4404564,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:15:42, revision 14)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $16$ and the permitted difference is $d=4$, so the first clause
claims $a_{16}(n)-a_{15}(n)=9^{\,n-1}$ whenever $4(n-1)+1\le16$, that is for
$1\le n\le4$, and the second claims $a_{16}(n)-a_{15}(n)=9^{\,n-1}-2$ at the single
$n$ with $4(n-1)=16$.
...
arithmetic, $L_{4}$ at base $16$ reproduces all $20$ terms A126511 publishes.
...
First, the model reproduces all $20$ terms A126511 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{4}(16,n)$ & $L_{4}(15,n)$ & $9^{\,n-1}$ \\ \hline
1 & 16 & 15 & 1 \\
2 & 124 & 115 & 9 \\
3 & 996 & 915 & 81 \\
4 & 8084 & 7355 & 729 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=5$, where $b=16=4(n-1)$: there $L_{4}(16,5)-L_{4}(15,5)=6559$ and $9^{4}-2=6559$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126511}.
\end{enumerate}
