...
\title{The cross-base identity for OEIS A124700, proved}
...
\begin{abstract}
OEIS A124700 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124700 is ``Number of base 7 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 7, 19, 43, 111, 287, 763, 2051,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:00:14, revision 19)
...
the base is $7$ and the permitted difference is $d=1$, so the claim is that
\[
a_{7}(n)-a_{6}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le7,
\]
...
at base $7$ reproduces all $26$ terms A124700 publishes, and at base $6$ all the
...
polynomial. In particular $a_{7}(n)-a_{6}(n)=F_{1}(n)$ for $1\le n\le13$, which
...
First, the model reproduces all $26$ terms A124700 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(7,n)$ & $T_{1}(6,n)$ & $F_{1}(n)$ \\ \hline
1 & 7 & 6 & 1 \\
2 & 19 & 16 & 3 \\
3 & 43 & 36 & 7 \\
4 & 111 & 92 & 19 \\
