...
\title{The cross-base identity for OEIS A126377, proved}
...
\begin{abstract}
OEIS A126377 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126377 is ``Number of base 23 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 23, 67, 197, 581, 1717, 5081, 15051,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:54, revision 9)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $23$ and the permitted difference is $d=1$, so the first clause
claims $a_{23}(n)-a_{22}(n)=3^{\,n-1}$ whenever $(n-1)+1\le23$, that is for
$1\le n\le23$, and the second claims $a_{23}(n)-a_{22}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=23$.
...
arithmetic, $L_{1}$ at base $23$ reproduces all $25$ terms A126377 publishes.
...
First, the model reproduces all $25$ terms A126377 publishes, exactly, in integer
...
$1\le n\le23$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(23,n)$ & $L_{1}(22,n)$ & $3^{\,n-1}$ \\ \hline
1 & 23 & 22 & 1 \\
2 & 67 & 64 & 3 \\
3 & 197 & 188 & 9 \\
4 & 581 & 554 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=24$, where $b=23=(n-1)$: there $L_{1}(23,24)-L_{1}(22,24)=94143178825$ and $3^{23}-2=94143178825$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126377}.
\end{enumerate}
