...
\title{The cross-base identity for OEIS A125469, proved}
...
\begin{abstract}
OEIS A125469 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125469 is ``Number of base 12 circular n-digit numbers with adjacent digits differing by 9 or less.''. It has offset $0$ and begins
\[
1, 12, 138, 1542, 17782, 205222, 2369460, 27358168,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:08:06, revision 20)
...
the base is $12$ and the permitted difference is $d=9$, so the claim is that
\[
a_{12}(n)-a_{11}(n)\;=\;F_{9}(n)
\qquad\text{for every } n \text{ with } 9\lfloor n/2\rfloor+1\le12,
\]
...
at base $12$ reproduces all $18$ terms A125469 publishes, and at base $11$ all the
...
polynomial. In particular $a_{12}(n)-a_{11}(n)=F_{9}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $18$ terms A125469 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{9}(12,n)$ & $T_{9}(11,n)$ & $F_{9}(n)$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 138 & 119 & 19 \\
3 & 1542 & 1271 & 271 \\
\end{tabular}
