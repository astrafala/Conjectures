...
\title{The empirical recurrence for OEIS A221818, proved by transfer matrix}
...
\begin{abstract}
OEIS A221818 counts the distinct OCCUPANCY arrays obtained when every cell of an $n\times2$ grid
...
an empirical recurrence of order $12$ contributed by R.~H.~Hardin. It is true, and it is
...
one, on $S=137$ states, after which the conjectured recurrence is settled by a finite exact
computation.
\end{abstract}
...
OEIS A221818 is ``Number of 2 X n arrays of occupancy after each element moves to some horizontal, diagonal or antidiagonal neighbor, without move-in move-out straight through or left turns.''. It has offset $1$ and begins
\[
0, 4, 16, 91, 459, 2429, 12678, 66380,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 20 2023 07:59:47, revision 6) this is still
...
\item $d'=d$, that is, the token arriving at a cell and the token leaving it move in the same direction --- the move goes STRAIGHT THROUGH;
\end{itemize}
...
Write $c_i\in\Delta^{2}$ for the row of offsets chosen by the cells of row $i$, where
...
$S=137$ reachable sets. In particular $a$ is $C$-finite.
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 17\,a(n-2) + 25\,a(n-3) + 7\,a(n-4) + 15\,a(n-5) + 10\,a(n-6) - 19\,a(n-7) + 8\,a(n-8) + 9\,a(n-9) + 2\,a(n-10) - 3\,a(n-11) + a(n-12)
\]
for every $n>12$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221818.
