...
\title{The cross-base identity for OEIS A125473, proved}
...
\begin{abstract}
OEIS A125473 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125473 is ``Number of base 16 circular n-digit numbers with adjacent digits differing by 9 or less.''. It has offset $0$ and begins
\[
1, 16, 214, 2626, 35338, 480386, 6560422, 89700102,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:08:27, revision 14)
...
the base is $16$ and the permitted difference is $d=9$, so the claim is that
\[
a_{16}(n)-a_{15}(n)\;=\;F_{9}(n)
\qquad\text{for every } n \text{ with } 9\lfloor n/2\rfloor+1\le16,
\]
...
at base $16$ reproduces all $17$ terms A125473 publishes, and at base $15$ all the
...
polynomial. In particular $a_{16}(n)-a_{15}(n)=F_{9}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $17$ terms A125473 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{9}(16,n)$ & $T_{9}(15,n)$ & $F_{9}(n)$ \\ \hline
1 & 16 & 15 & 1 \\
2 & 214 & 195 & 19 \\
3 & 2626 & 2355 & 271 \\
\end{tabular}
