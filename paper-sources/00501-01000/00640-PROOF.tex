...
\title{The cross-base identity for OEIS A124706, proved}
...
\begin{abstract}
OEIS A124706 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124706 is ``Number of base 13 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 13, 37, 85, 225, 593, 1609, 4409,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:00:47, revision 16)
...
the base is $13$ and the permitted difference is $d=1$, so the claim is that
\[
a_{13}(n)-a_{12}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le13,
\]
...
at base $13$ reproduces all $26$ terms A124706 publishes, and at base $12$ all the
...
polynomial. In particular $a_{13}(n)-a_{12}(n)=F_{1}(n)$ for $1\le n\le25$, which
...
First, the model reproduces all $26$ terms A124706 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(13,n)$ & $T_{1}(12,n)$ & $F_{1}(n)$ \\ \hline
1 & 13 & 12 & 1 \\
2 & 37 & 34 & 3 \\
3 & 85 & 78 & 7 \\
4 & 225 & 206 & 19 \\
