...
\title{The cross-base identity for OEIS A124707, proved}
...
\begin{abstract}
OEIS A124707 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124707 is ``Number of base 14 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 14, 40, 92, 244, 644, 1750, 4802,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:13:11, revision 23)
...
the base is $14$ and the permitted difference is $d=1$, so the claim is that
\[
a_{14}(n)-a_{13}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le14,
\]
...
at base $14$ reproduces all $28$ terms A124707 publishes, and at base $13$ all the
...
polynomial. In particular $a_{14}(n)-a_{13}(n)=F_{1}(n)$ for $1\le n\le27$, which
...
First, the model reproduces all $28$ terms A124707 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(14,n)$ & $T_{1}(13,n)$ & $F_{1}(n)$ \\ \hline
1 & 14 & 13 & 1 \\
2 & 40 & 37 & 3 \\
3 & 92 & 85 & 7 \\
4 & 244 & 225 & 19 \\
