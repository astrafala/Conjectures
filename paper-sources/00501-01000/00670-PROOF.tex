...
\title{The cross-base identity for OEIS A125321, proved}
...
\begin{abstract}
OEIS A125321 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125321 is ``Number of base 11 circular n-digit numbers with adjacent digits differing by 3 or less.''. It has offset $0$ and begins
\[
1, 11, 65, 323, 1849, 10781, 64325, 388749,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:02:15, revision 17)
...
the base is $11$ and the permitted difference is $d=3$, so the claim is that
\[
a_{11}(n)-a_{10}(n)\;=\;F_{3}(n)
\qquad\text{for every } n \text{ with } 3\lfloor n/2\rfloor+1\le11,
\]
...
at base $11$ reproduces all $20$ terms A125321 publishes, and at base $10$ all the
...
polynomial. In particular $a_{11}(n)-a_{10}(n)=F_{3}(n)$ for $1\le n\le7$, which
...
First, the model reproduces all $20$ terms A125321 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{3}(11,n)$ & $T_{3}(10,n)$ & $F_{3}(n)$ \\ \hline
1 & 11 & 10 & 1 \\
2 & 65 & 58 & 7 \\
3 & 323 & 286 & 37 \\
4 & 1849 & 1618 & 231 \\
