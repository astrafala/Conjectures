...
\title{The empirical recurrence for OEIS A188149, proved from an exact model}
...
\begin{abstract}
OEIS A188149 carries an empirical recurrence of order $3$. It is true, and it is
...
OEIS A188149 is ``Number of 4-step self-avoiding walks on an n X n square summed over all starting positions.''. It has offset $1$ and begins
\[
0, 8, 80, 232, 456, 752, 1120, 1560,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 36*n\^{}2 - 100*n + 56 for n>2. a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 26 2018 11:37:37, revision 10)
...
SHAPE when one is a translate of the other. There are exactly $36$ shapes: self-avoiding
...
Write $w^\ast$ for the largest bounding-box side over all $36$ shapes. Once $n\ge w^\ast$
...
$z^{4}(z-1)^{3}$, monic of order $S=7$. Both numbers are read off the enumerated
...
vanish from $n=5+1$ onwards, and the run exceeds $S+3$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188149.
