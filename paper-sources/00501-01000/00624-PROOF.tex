...
\title{The cross-base identity for OEIS A124799, proved}
...
\begin{abstract}
OEIS A124799 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124799 is ``Number of base 30 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 30, 88, 204, 548, 1460, 4006, 11090,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:14:52, revision 14)
...
entry itself as the largest coefficient of $(1+x+\dots+x^{2d})^{n}$. The entry writes that summand as $\mathrm{A002426}(n+1)$, the $(n+1)$st term of OEIS A002426; that sequence is the central coefficient of $(1+x+\dots+x^{2})^{n}$ and its published terms agree with $F_{1}(n)$ term by term, which is the identification used here. For this entry
the base is $30$ and the permitted difference is $d=1$, so the claim is that
\[
a_{30}(n)-a_{29}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le30,
\]
that is, for $1\le n\le40$.
...
together with a choice of where it starts. Evaluated exactly in integer arithmetic, $T_{1}$
at base $30$ reproduces all $25$ terms A124799 publishes, and at base $29$ all the
...
polynomial. In particular $a_{30}(n)-a_{29}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124799 publishes, exactly, in integer
...
$1\le n\le40$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(30,n)$ & $T_{1}(29,n)$ & $F_{1}(n)$ \\ \hline
1 & 30 & 29 & 1 \\
2 & 88 & 85 & 3 \\
3 & 204 & 197 & 7 \\
4 & 548 & 529 & 19 \\
