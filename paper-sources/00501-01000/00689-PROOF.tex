...
\title{The cross-base identity for OEIS A125317, proved}
...
\begin{abstract}
OEIS A125317 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125317 is ``Number of base 7 circular n-digit numbers with adjacent digits differing by 3 or less.''. It has offset $0$ and begins
\[
1, 7, 37, 175, 925, 4977, 27067, 147777,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:01:54, revision 16)
...
the base is $7$ and the permitted difference is $d=3$, so the claim is that
\[
a_{7}(n)-a_{6}(n)\;=\;F_{3}(n)
\qquad\text{for every } n \text{ with } 3\lfloor n/2\rfloor+1\le7,
\]
...
at base $7$ reproduces all $21$ terms A125317 publishes, and at base $6$ all the
...
polynomial. In particular $a_{7}(n)-a_{6}(n)=F_{3}(n)$ for $1\le n\le5$, which
...
First, the model reproduces all $21$ terms A125317 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{3}(7,n)$ & $T_{3}(6,n)$ & $F_{3}(n)$ \\ \hline
1 & 7 & 6 & 1 \\
2 & 37 & 30 & 7 \\
3 & 175 & 138 & 37 \\
4 & 925 & 694 & 231 \\
