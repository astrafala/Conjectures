...
\title{The empirical closed form for OEIS A219287, proved}
...
\begin{abstract}
OEIS A219287 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=228$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219287 is ``Number of n X 4 arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 n X 4 array.''. It has offset $1$ and begins
\[
4, 9, 42, 149, 423, 1092, 2623, 5937,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 25 2018 08:37:31, revision 8)
this is still recorded as empirical, and it is still open.

\section{What is being counted}

Fix the width $W=4$ and the value range $0..1$. The underlying arrays are the
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=228$
...
states number $S=228$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219287 records is correct for all $n>6$.
\end{theorem}
...
$1 + 30 - 1$, which is more than the $9$ consecutive agreements a
...
First, the model reproduces all $30$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219287.
