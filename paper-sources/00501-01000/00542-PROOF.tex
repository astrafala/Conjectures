...
\title{A proof of the conjectured generating function for OEIS A266072}
...
\begin{abstract}
OEIS A266072 carries a conjectured generating function. We prove it. The entry states, as fact,
...
\begin{quote}\small
a(n) = n-(-1)\textasciicircum{}n*(n-1).
\end{quote}
and separately carries the comment
\begin{quote}\small
Conjectured g.f.: (1 + x - x\textasciicircum{}2 + 3*x\textasciicircum{}3)/(-1 + x\textasciicircum{}2)\textasciicircum{}2. - \emph{Michael De Vlieger}, Dec 21 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-02-16, revision 30) the second
...
\left(- n - 2\right)\,a(n) + \left(- n - 3\right)\,a(n+1) + n\,a(n+2) + \left(n + 1\right)\,a(n+3) \;=\; 0
\end{equation}

The leading coefficient of \eqref{eq:rec} is $n + 1$. Its only integer zeros are $n\in\{-1\}$, all below the range claimed here.
...
For all $n\ge 0$,
\[
\sum_{n\ge 0} a(n)x^{n} \;=\; \frac{3 x^{3} - x^{2} + x + 1}{\left(x^{2} - 1\right)^{2}}
\]
\end{theorem}
...
\url{https://oeis.org}, 2026. Sequence A266072.
