...
\title{The cross-base identity for OEIS A125449, proved}
...
\begin{abstract}
OEIS A125449 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125449 is ``Number of base 14 circular n-digit numbers with adjacent digits differing by 8 or less.''. It has offset $0$ and begins
\[
1, 14, 166, 1814, 21646, 260634, 3150976, 38131968,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:07:44, revision 14)
...
the base is $14$ and the permitted difference is $d=8$, so the claim is that
\[
a_{14}(n)-a_{13}(n)\;=\;F_{8}(n)
\qquad\text{for every } n \text{ with } 8\lfloor n/2\rfloor+1\le14,
\]
...
at base $14$ reproduces all $17$ terms A125449 publishes, and at base $13$ all the
...
polynomial. In particular $a_{14}(n)-a_{13}(n)=F_{8}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $17$ terms A125449 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{8}(14,n)$ & $T_{8}(13,n)$ & $F_{8}(n)$ \\ \hline
1 & 14 & 13 & 1 \\
2 & 166 & 149 & 17 \\
3 & 1814 & 1597 & 217 \\
\end{tabular}
