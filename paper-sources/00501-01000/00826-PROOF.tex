...
\title{The cross-base identity for OEIS A125450, proved}
...
\begin{abstract}
OEIS A125450 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125450 is ``Number of base 15 circular n-digit numbers with adjacent digits differing by 8 or less.''. It has offset $0$ and begins
\[
1, 15, 183, 2031, 24907, 309275, 3863091, 48336527,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:07:49, revision 14)
...
the base is $15$ and the permitted difference is $d=8$, so the claim is that
\[
a_{15}(n)-a_{14}(n)\;=\;F_{8}(n)
\qquad\text{for every } n \text{ with } 8\lfloor n/2\rfloor+1\le15,
\]
...
at base $15$ reproduces all $17$ terms A125450 publishes, and at base $14$ all the
...
polynomial. In particular $a_{15}(n)-a_{14}(n)=F_{8}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $17$ terms A125450 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{8}(15,n)$ & $T_{8}(14,n)$ & $F_{8}(n)$ \\ \hline
1 & 15 & 14 & 1 \\
2 & 183 & 166 & 17 \\
3 & 2031 & 1814 & 217 \\
\end{tabular}
