...
\title{The cross-base identity for OEIS A124717, proved}
...
\begin{abstract}
OEIS A124717 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124717 is ``Number of base 24 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 24, 70, 162, 434, 1154, 3160, 8732,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:14:17, revision 14)
...
the base is $24$ and the permitted difference is $d=1$, so the claim is that
\[
a_{24}(n)-a_{23}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le24,
\]
...
at base $24$ reproduces all $25$ terms A124717 publishes, and at base $23$ all the
...
polynomial. In particular $a_{24}(n)-a_{23}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124717 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(24,n)$ & $T_{1}(23,n)$ & $F_{1}(n)$ \\ \hline
1 & 24 & 23 & 1 \\
2 & 70 & 67 & 3 \\
3 & 162 & 155 & 7 \\
4 & 434 & 415 & 19 \\
