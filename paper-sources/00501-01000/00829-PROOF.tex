...
\title{The cross-base identity for OEIS A126370, proved}
...
\begin{abstract}
OEIS A126370 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126370 is ``Number of base 16 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 16, 46, 134, 392, 1150, 3380, 9948,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:21, revision 12)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $16$ and the permitted difference is $d=1$, so the first clause
claims $a_{16}(n)-a_{15}(n)=3^{\,n-1}$ whenever $(n-1)+1\le16$, that is for
$1\le n\le16$, and the second claims $a_{16}(n)-a_{15}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=16$.
...
arithmetic, $L_{1}$ at base $16$ reproduces all $25$ terms A126370 publishes.
...
First, the model reproduces all $25$ terms A126370 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(16,n)$ & $L_{1}(15,n)$ & $3^{\,n-1}$ \\ \hline
1 & 16 & 15 & 1 \\
2 & 46 & 43 & 3 \\
3 & 134 & 125 & 9 \\
4 & 392 & 365 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=17$, where $b=16=(n-1)$: there $L_{1}(16,17)-L_{1}(15,17)=43046719$ and $3^{16}-2=43046719$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126370}.
\end{enumerate}
