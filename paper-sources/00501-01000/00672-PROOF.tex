...
\title{The cross-base identity for OEIS A124828, proved}
...
\begin{abstract}
OEIS A124828 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124828 is ``Number of base 7 circular n-digit numbers with adjacent digits differing by 2 or less.''. It has offset $0$ and begins
\[
1, 7, 29, 103, 417, 1717, 7229, 30793,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:15:15, revision 19)
...
the base is $7$ and the permitted difference is $d=2$, so the claim is that
\[
a_{7}(n)-a_{6}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le7,
\]
...
at base $7$ reproduces all $22$ terms A124828 publishes, and at base $6$ all the
...
polynomial. In particular $a_{7}(n)-a_{6}(n)=F_{2}(n)$ for $1\le n\le7$, which
...
First, the model reproduces all $22$ terms A124828 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(7,n)$ & $T_{2}(6,n)$ & $F_{2}(n)$ \\ \hline
1 & 7 & 6 & 1 \\
2 & 29 & 24 & 5 \\
3 & 103 & 84 & 19 \\
4 & 417 & 332 & 85 \\
