...
\title{The cross-base identity for OEIS A124805, proved}
...
\begin{abstract}
OEIS A124805 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124805 is ``Number of circular n-letter words over the alphabet {0,1,2,3} with adjacent letters differing by at most 2.''. It has offset $0$ and begins
\[
1, 4, 14, 46, 162, 574, 2042, 7270,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 30 2025 14:08:06, revision 33)
...
the base is $4$ and the permitted difference is $d=2$, so the claim is that
\[
a_{4}(n)-a_{3}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le4,
\]
...
at base $4$ reproduces all $24$ terms A124805 publishes, and at base $3$ all the
...
polynomial. In particular $a_{4}(n)-a_{3}(n)=F_{2}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $24$ terms A124805 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(4,n)$ & $T_{2}(3,n)$ & $F_{2}(n)$ \\ \hline
1 & 4 & 3 & 1 \\
2 & 14 & 9 & 5 \\
3 & 46 & 27 & 19 \\
\end{tabular}
