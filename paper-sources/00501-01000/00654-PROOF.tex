...
\title{The cross-base identity for OEIS A124711, proved}
...
\begin{abstract}
OEIS A124711 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124711 is ``Number of base 18 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 18, 52, 120, 320, 848, 2314, 6374,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:01:09, revision 16)
...
the base is $18$ and the permitted difference is $d=1$, so the claim is that
\[
a_{18}(n)-a_{17}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le18,
\]
that is, for $1\le n\le35$.
...
at base $18$ reproduces all $25$ terms A124711 publishes, and at base $17$ all the
...
polynomial. In particular $a_{18}(n)-a_{17}(n)=F_{1}(n)$ for $1\le n\le35$, which
...
First, the model reproduces all $25$ terms A124711 publishes, exactly, in integer
...
$1\le n\le35$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(18,n)$ & $T_{1}(17,n)$ & $F_{1}(n)$ \\ \hline
1 & 18 & 17 & 1 \\
2 & 52 & 49 & 3 \\
3 & 120 & 113 & 7 \\
4 & 320 & 301 & 19 \\
