...
\title{The cross-base identity for OEIS A126382, proved}
...
\begin{abstract}
OEIS A126382 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126382 is ``Number of base 28 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 28, 82, 242, 716, 2122, 6296, 18696,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:10:20, revision 9)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $28$ and the permitted difference is $d=1$, so the first clause
claims $a_{28}(n)-a_{27}(n)=3^{\,n-1}$ whenever $(n-1)+1\le28$, that is for
$1\le n\le28$, and the second claims $a_{28}(n)-a_{27}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=28$.
...
arithmetic, $L_{1}$ at base $28$ reproduces all $24$ terms A126382 publishes.
...
First, the model reproduces all $24$ terms A126382 publishes, exactly, in integer
...
$1\le n\le28$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(28,n)$ & $L_{1}(27,n)$ & $3^{\,n-1}$ \\ \hline
1 & 28 & 27 & 1 \\
2 & 82 & 79 & 3 \\
3 & 242 & 233 & 9 \\
4 & 716 & 689 & 27 \\
...
and at every row up to $n=28$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=29$, where $b=28=(n-1)$: there $L_{1}(28,29)-L_{1}(27,29)=22876792454959$ and $3^{28}-2=22876792454959$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126382}.
\end{enumerate}
