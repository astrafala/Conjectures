...
\title{The empirical closed form for OEIS A220148, proved}
...
\begin{abstract}
OEIS A220148 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=70$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220148 is ``Number of n X 3 arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 n X 3 array.''. It has offset $1$ and begins
\[
6, 23, 77, 242, 727, 2062, 5493, 13773,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 09:52:44, revision 7)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=70$
...
states number $S=70$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220148 records is correct for all $n>0$.
\end{theorem}
...
$1 + 28 - 1$, which is more than the $12$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220148.
