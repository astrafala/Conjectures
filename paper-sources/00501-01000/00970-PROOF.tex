...
\title{The empirical closed form for OEIS A219847, proved}
...
\begin{abstract}
OEIS A219847 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=65$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219847 is ``Number of nX3 arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 nX3 array.''. It has offset $1$ and begins
\[
6, 17, 65, 225, 669, 1815, 4617, 11125,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:21:28, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=65$
...
states number $S=65$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219847 records is correct for all $n>3$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $15$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219847.
