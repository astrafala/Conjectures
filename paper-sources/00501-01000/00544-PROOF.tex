...
\title{The empirical recurrence for OEIS A266844, proved from an exact model}
...
\begin{abstract}
OEIS A266844 carries an empirical recurrence of order $3$ contributed by Colin Barker. It is true, and it is
...
\begin{quote}
Conjectures from \_Colin Barker\_, Jan 05 2016 and Apr 18 2019: (Start) a(n) = 10*a(n-1)+10000*a(n-2)-100000*a(n-3) for n>2.
\end{quote}
...
For this entry the automaton is rule 70, the period is $p=2$ and the pre-period is $n_0=0$.
The certificate is
\begin{gather*}
r=0:\quad \texttt{10}^{\,j} \cdot \texttt{1} \cdot \texttt{00}^{\,j} \\
r=1:\quad \texttt{1} \cdot \texttt{10}^{\,j} \cdot \texttt{1} \cdot \texttt{00}^{\,j} \cdot \texttt{0}
\end{gather*}
