...
\title{The empirical closed form for OEIS A219521, proved}
...
\begin{abstract}
OEIS A219521 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=183$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219521 is ``Number of 4Xn arrays of the minimum value of corresponding elements and their horizontal, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 4Xn array.''. It has offset $1$ and begins
\[
5, 9, 22, 68, 166, 380, 821, 1700,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:07:58, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=183$
...
states number $S=183$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219521 records is correct for all $n>9$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219521.
