...
\title{The cross-base identity for OEIS A126483, proved}
...
\begin{abstract}
OEIS A126483 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126483 is ``Number of base 15 n-digit numbers with adjacent digits differing by three or less.''. It has offset $0$ and begins
\[
1, 15, 93, 595, 3845, 24983, 162805, 1062635,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:13:25, revision 12)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $15$ and the permitted difference is $d=3$, so the first clause
claims $a_{15}(n)-a_{14}(n)=7^{\,n-1}$ whenever $3(n-1)+1\le15$, that is for
$1\le n\le5$, and the second claims $a_{15}(n)-a_{14}(n)=7^{\,n-1}-2$ at the single
$n$ with $3(n-1)=15$.
...
arithmetic, $L_{3}$ at base $15$ reproduces all $20$ terms A126483 publishes.
...
First, the model reproduces all $20$ terms A126483 publishes, exactly, in integer
...
$1\le n\le5$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{3}(15,n)$ & $L_{3}(14,n)$ & $7^{\,n-1}$ \\ \hline
1 & 15 & 14 & 1 \\
2 & 93 & 86 & 7 \\
3 & 595 & 546 & 49 \\
4 & 3845 & 3502 & 343 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=6$, where $b=15=3(n-1)$: there $L_{3}(15,6)-L_{3}(14,6)=16805$ and $7^{5}-2=16805$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126483}.
\end{enumerate}
