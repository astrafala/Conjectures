...
\title{The cross-base identity for OEIS A124952, proved}
...
\begin{abstract}
OEIS A124952 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124952 is ``Number of base 24 circular n-digit numbers with adjacent digits differing by 2 or less.''. It has offset $0$ and begins
\[
1, 24, 114, 426, 1862, 8194, 36996, 169088,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:17:43, revision 16)
...
the base is $24$ and the permitted difference is $d=2$, so the claim is that
\[
a_{24}(n)-a_{23}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le24,
\]
that is, for $1\le n\le23$.
...
at base $24$ reproduces all $22$ terms A124952 publishes, and at base $23$ all the
...
polynomial. In particular $a_{24}(n)-a_{23}(n)=F_{2}(n)$ for $1\le n\le23$, which
...
First, the model reproduces all $22$ terms A124952 publishes, exactly, in integer
...
$1\le n\le23$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(24,n)$ & $T_{2}(23,n)$ & $F_{2}(n)$ \\ \hline
1 & 24 & 23 & 1 \\
2 & 114 & 109 & 5 \\
3 & 426 & 407 & 19 \\
4 & 1862 & 1777 & 85 \\
