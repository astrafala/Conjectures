...
\title{The cross-base identity for OEIS A126360, proved}
...
\begin{abstract}
OEIS A126360 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126360 is ``Number of base 6 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 6, 16, 44, 122, 340, 950, 2658,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:08:38, revision 26)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $6$ and the permitted difference is $d=1$, so the first clause
claims $a_{6}(n)-a_{5}(n)=3^{\,n-1}$ whenever $(n-1)+1\le6$, that is for
$1\le n\le6$, and the second claims $a_{6}(n)-a_{5}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=6$.
...
arithmetic, $L_{1}$ at base $6$ reproduces all $26$ terms A126360 publishes.
...
First, the model reproduces all $26$ terms A126360 publishes, exactly, in integer
...
$1\le n\le6$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(6,n)$ & $L_{1}(5,n)$ & $3^{\,n-1}$ \\ \hline
1 & 6 & 5 & 1 \\
2 & 16 & 13 & 3 \\
3 & 44 & 35 & 9 \\
4 & 122 & 95 & 27 \\
...
and at every row up to $n=6$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=7$, where $b=6=(n-1)$: there $L_{1}(6,7)-L_{1}(5,7)=727$ and $3^{6}-2=727$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126360}.
\end{enumerate}
