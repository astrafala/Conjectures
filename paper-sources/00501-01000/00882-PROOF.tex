...
\title{The cross-base identity for OEIS A126363, proved}
...
\begin{abstract}
OEIS A126363 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126363 is ``Number of base 9 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 9, 25, 71, 203, 583, 1679, 4845,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:08:49, revision 12)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $9$ and the permitted difference is $d=1$, so the first clause
claims $a_{9}(n)-a_{8}(n)=3^{\,n-1}$ whenever $(n-1)+1\le9$, that is for
$1\le n\le9$, and the second claims $a_{9}(n)-a_{8}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=9$.
...
arithmetic, $L_{1}$ at base $9$ reproduces all $26$ terms A126363 publishes.
...
First, the model reproduces all $26$ terms A126363 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(9,n)$ & $L_{1}(8,n)$ & $3^{\,n-1}$ \\ \hline
1 & 9 & 8 & 1 \\
2 & 25 & 22 & 3 \\
3 & 71 & 62 & 9 \\
4 & 203 & 176 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=10$, where $b=9=(n-1)$: there $L_{1}(9,10)-L_{1}(8,10)=19681$ and $3^{9}-2=19681$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126363}.
\end{enumerate}
