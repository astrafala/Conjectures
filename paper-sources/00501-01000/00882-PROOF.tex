...
\title{The empirical recurrence for OEIS A279128, proved by counting patterns with a budget}
...
\begin{abstract}
OEIS A279128 imposes three things at once: a condition on each cell that reaches one row up and
...
OEIS A279128 is ``Number of n X 2 0..1 arrays with no element equal to a strict majority of its horizontal and vertical neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
1, 0, 9, 34, 87, 194, 400, 790,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) - 7*a(n-2) - 2*a(n-3) + 10*a(n-4) - 2*a(n-5) - 5*a(n-6) + a(n-7) + a(n-8) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 26 2018 07:29:01, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 7\,a(n-2) - 2\,a(n-3) + 10\,a(n-4) - 2\,a(n-5) - 5\,a(n-6) + a(n-7) + a(n-8)
\end{equation}
for all $n>9$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A279128.
