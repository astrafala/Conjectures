...
\title{The cross-base identity for OEIS A126369, proved}
...
\begin{abstract}
OEIS A126369 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126369 is ``Number of base 15 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 15, 43, 125, 365, 1069, 3137, 9219,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:16, revision 12)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $15$ and the permitted difference is $d=1$, so the first clause
claims $a_{15}(n)-a_{14}(n)=3^{\,n-1}$ whenever $(n-1)+1\le15$, that is for
$1\le n\le15$, and the second claims $a_{15}(n)-a_{14}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=15$.
...
arithmetic, $L_{1}$ at base $15$ reproduces all $25$ terms A126369 publishes.
...
First, the model reproduces all $25$ terms A126369 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(15,n)$ & $L_{1}(14,n)$ & $3^{\,n-1}$ \\ \hline
1 & 15 & 14 & 1 \\
2 & 43 & 40 & 3 \\
3 & 125 & 116 & 9 \\
4 & 365 & 338 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=16$, where $b=15=(n-1)$: there $L_{1}(15,16)-L_{1}(14,16)=14348905$ and $3^{15}-2=14348905$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126369}.
\end{enumerate}
