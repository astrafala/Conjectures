...
\title{The cross-base identity for OEIS A126386, proved}
...
\begin{abstract}
OEIS A126386 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126386 is ``Number of base 32 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 32, 94, 278, 824, 2446, 7268, 21612,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:44:30, revision 7)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $32$ and the permitted difference is $d=1$, so the first clause
claims $a_{32}(n)-a_{31}(n)=3^{\,n-1}$ whenever $(n-1)+1\le32$, that is for
$1\le n\le32$, and the second claims $a_{32}(n)-a_{31}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=32$.
...
arithmetic, $L_{1}$ at base $32$ reproduces all $24$ terms A126386 publishes.
...
First, the model reproduces all $24$ terms A126386 publishes, exactly, in integer
...
$1\le n\le32$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(32,n)$ & $L_{1}(31,n)$ & $3^{\,n-1}$ \\ \hline
1 & 32 & 31 & 1 \\
2 & 94 & 91 & 3 \\
3 & 278 & 269 & 9 \\
4 & 824 & 797 & 27 \\
...
and at every row up to $n=32$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=33$, where $b=32=(n-1)$: there $L_{1}(32,33)-L_{1}(31,33)=1853020188851839$ and $3^{32}-2=1853020188851839$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126386}.
\end{enumerate}
