...
\title{The cross-base identity for OEIS A125399, proved}
...
\begin{abstract}
OEIS A125399 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125399 is ``Number of base 11 circular n-digit numbers with adjacent digits differing by 6 or less.''. It has offset $0$ and begins
\[
1, 11, 101, 851, 7861, 73361, 687911, 6458645,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:05:38, revision 14)
...
the base is $11$ and the permitted difference is $d=6$, so the claim is that
\[
a_{11}(n)-a_{10}(n)\;=\;F_{6}(n)
\qquad\text{for every } n \text{ with } 6\lfloor n/2\rfloor+1\le11,
\]
...
at base $11$ reproduces all $19$ terms A125399 publishes, and at base $10$ all the
...
polynomial. In particular $a_{11}(n)-a_{10}(n)=F_{6}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $19$ terms A125399 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{6}(11,n)$ & $T_{6}(10,n)$ & $F_{6}(n)$ \\ \hline
1 & 11 & 10 & 1 \\
2 & 101 & 88 & 13 \\
3 & 851 & 724 & 127 \\
\end{tabular}
