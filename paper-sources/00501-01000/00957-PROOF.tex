...
\title{The cross-base identity for OEIS A126531, proved}
...
\begin{abstract}
OEIS A126531 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126531 is ``Number of base 10 n-digit numbers with adjacent digits differing by five or less.''. It has offset $0$ and begins
\[
1, 10, 80, 660, 5430, 44692, 367826, 3027314,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:17:16, revision 15)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $10$ and the permitted difference is $d=5$, so the first clause
claims $a_{10}(n)-a_{9}(n)=11^{\,n-1}$ whenever $5(n-1)+1\le10$, that is for
$1\le n\le2$, and the second claims $a_{10}(n)-a_{9}(n)=11^{\,n-1}-2$ at the single
$n$ with $5(n-1)=10$.
...
arithmetic, $L_{5}$ at base $10$ reproduces all $20$ terms A126531 publishes.
...
First, the model reproduces all $20$ terms A126531 publishes, exactly, in integer
...
$1\le n\le2$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{5}(10,n)$ & $L_{5}(9,n)$ & $11^{\,n-1}$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 80 & 69 & 11 \\
\end{tabular}
\end{center}
...
and at every row up to $n=2$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=3$, where $b=10=5(n-1)$: there $L_{5}(10,3)-L_{5}(9,3)=119$ and $11^{2}-2=119$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126531}.
\end{enumerate}
