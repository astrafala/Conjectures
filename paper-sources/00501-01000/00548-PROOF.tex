...
\title{The reduction of OEIS A396797 modulo $6$}
...
\begin{abstract}
OEIS A396797 is defined by a functional equation for its ordinary generating function and
...
them modulo $6$ as well; and modulo $6$ the series $x/(1-x)$ satisfies the equation
...
$\mathbb{Z}/6\mathbb{Z}$; nothing is truncated or estimated.
\end{abstract}
...
OEIS A396797 is ``G.f. satisfies A(x) = x + A\^{}3(x)*A\^{}4(x).''. It has offset $1$ and begins
\[
1, 1, 7, 79, 1135, 18985,\ \dots
\]
...
Conjecture: [x\^{}n] A\^{}k(x) == k\^{}(n-1) (mod 6) for n >= 1 and all integer k. --- \emph{Paul D. Hanna}
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 17 2026 00:54:04, revision 22) they are
...
Lemma~\ref{lem:jaN}. On the left the coefficient is $1$. On the right, $x$ contributes nothing and the product $A^{\,3}A^{\,4}$ contributes nothing either: every monomial of $\bigl[x^{N}\bigr]\bigl(A^{\,3}A^{\,4}\bigr)$ is a product of one coefficient of $A^{\,3}$ and one of $A^{\,4}$ with indices $i,j\ge1$ and $i+j=N$, so both indices are at most $N-1$. Hence the net coefficient of $a(N)$ is $1$.
...
Write $f(x)=\dfrac{x}{1-x}$ over $\mathbb{Z}/6\mathbb{Z}$.
...
Over $\mathbb{Z}/6\mathbb{Z}$, $f$ satisfies $f=x+f^{\,3}f^{\,4}$.
\end{lemma}

\begin{proof}
By Lemma~\ref{lem:iter}, $f^{\,3}f^{\,4}
=\dfrac{x^{2}}{(1-3x)\bigl(1-4x\bigr)}$, while
$f-x=\dfrac{x}{1-x}-x=\dfrac{x^{2}}{1-x}$. So the claim is that
$(1-3x)(1-4x)$ and $1-x$ agree in $(\mathbb{Z}/6\mathbb{Z})[x]$. Expanding,
\[
(1-3x)\bigl(1-4x\bigr)=1-7x+12x^{2} ,
\]
and modulo $6$ we have $7\equiv1$ and $12\equiv0$. Hence both sides equal
$1-x$, and the two rational functions agree.
\end{proof}


\begin{theorem}\label{thm:main}
Over $\mathbb{Z}/6\mathbb{Z}$ the reduction of $A$ is $x/(1-x)$. Equivalently
$a(n)\equiv1\pmod{6}$ for every $n\ge1$, and more generally
\[
\bigl[x^{n}\bigr]A^{\,j}(x)\;\equiv\;j^{\,n-1}\pmod{6}
\]
...
Reduction modulo $6$ carries composition of power series with unit linear coefficient to
...
\begin{remark}
The proof used only that $(1-3x)(1-4x)\equiv1-x$ modulo $6$, that is, that
$6$ divides both $2\cdot3$ and $3\cdot4$. The largest modulus with that property
is $\gcd\bigl(2p,\,p(p+1)\bigr)$, which is $2p$ for odd $p$ and $p$ for even $p$; here
$p=3$ gives $6$. This is exactly why the statement is made modulo $6$ and not modulo
a larger number.
\end{remark}
...
continued to $n=24$ and every term reduced modulo $6$; all residues are
$1$.

Third, the composition of the proof was carried out independently as a formal power series
over $\mathbb{Z}/6\mathbb{Z}$ to order $30$, starting from the coefficients of $x/(1-x)$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A396797.
