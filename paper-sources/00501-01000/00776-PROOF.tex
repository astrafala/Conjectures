...
\title{The cross-base identity for OEIS A125446, proved}
...
\begin{abstract}
OEIS A125446 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125446 is ``Number of base 11 circular n-digit numbers with adjacent digits differing by 8 or less.''. It has offset $0$ and begins
\[
1, 11, 115, 1163, 12199, 128101, 1345957, 14142615,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:07:27, revision 17)
...
the base is $11$ and the permitted difference is $d=8$, so the claim is that
\[
a_{11}(n)-a_{10}(n)\;=\;F_{8}(n)
\qquad\text{for every } n \text{ with } 8\lfloor n/2\rfloor+1\le11,
\]
...
at base $11$ reproduces all $18$ terms A125446 publishes, and at base $10$ all the
...
polynomial. In particular $a_{11}(n)-a_{10}(n)=F_{8}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $18$ terms A125446 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{8}(11,n)$ & $T_{8}(10,n)$ & $F_{8}(n)$ \\ \hline
1 & 11 & 10 & 1 \\
2 & 115 & 98 & 17 \\
3 & 1163 & 946 & 217 \\
\end{tabular}
