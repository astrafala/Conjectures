...
\title{The cross-base identity for OEIS A126541, proved}
...
\begin{abstract}
OEIS A126541 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126541 is ``Number of base 20 n-digit numbers with adjacent digits differing by five or less.''. It has offset $0$ and begins
\[
1, 20, 190, 1870, 18600, 185922, 1862204, 18667904,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:18:06, revision 11)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $20$ and the permitted difference is $d=5$, so the first clause
claims $a_{20}(n)-a_{19}(n)=11^{\,n-1}$ whenever $5(n-1)+1\le20$, that is for
$1\le n\le4$, and the second claims $a_{20}(n)-a_{19}(n)=11^{\,n-1}-2$ at the single
$n$ with $5(n-1)=20$.
...
arithmetic, $L_{5}$ at base $20$ reproduces all $18$ terms A126541 publishes.
...
First, the model reproduces all $18$ terms A126541 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{5}(20,n)$ & $L_{5}(19,n)$ & $11^{\,n-1}$ \\ \hline
1 & 20 & 19 & 1 \\
2 & 190 & 179 & 11 \\
3 & 1870 & 1749 & 121 \\
4 & 18600 & 17269 & 1331 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=5$, where $b=20=5(n-1)$: there $L_{5}(20,5)-L_{5}(19,5)=14639$ and $11^{4}-2=14639$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126541}.
\end{enumerate}
