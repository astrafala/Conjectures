...
\title{The cross-base identity for OEIS A126367, proved}
...
\begin{abstract}
OEIS A126367 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126367 is ``Number of base 13 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 13, 37, 107, 311, 907, 2651, 7761,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:05, revision 12)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $13$ and the permitted difference is $d=1$, so the first clause
claims $a_{13}(n)-a_{12}(n)=3^{\,n-1}$ whenever $(n-1)+1\le13$, that is for
$1\le n\le13$, and the second claims $a_{13}(n)-a_{12}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=13$.
...
arithmetic, $L_{1}$ at base $13$ reproduces all $25$ terms A126367 publishes.
...
First, the model reproduces all $25$ terms A126367 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(13,n)$ & $L_{1}(12,n)$ & $3^{\,n-1}$ \\ \hline
1 & 13 & 12 & 1 \\
2 & 37 & 34 & 3 \\
3 & 107 & 98 & 9 \\
4 & 311 & 284 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=14$, where $b=13=(n-1)$: there $L_{1}(13,14)-L_{1}(12,14)=1594321$ and $3^{13}-2=1594321$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126367}.
\end{enumerate}
