...
\title{The cross-base identity for OEIS A124957, proved}
...
\begin{abstract}
OEIS A124957 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124957 is ``Number of base 29 circular n-digit numbers with adjacent digits differing by 2 or less.''. It has offset $0$ and begins
\[
1, 29, 139, 521, 2287, 10099, 45751, 209763,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:18:30, revision 16)
...
entry itself as the largest coefficient of $(1+x+\dots+x^{2d})^{n}$. The entry writes that summand as $\mathrm{A005191}(n+1)$, the $(n+1)$st term of OEIS A005191; that sequence is the central coefficient of $(1+x+\dots+x^{4})^{n}$ and its published terms agree with $F_{2}(n)$ term by term, which is the identification used here. For this entry
the base is $29$ and the permitted difference is $d=2$, so the claim is that
\[
a_{29}(n)-a_{28}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le29,
\]
that is, for $1\le n\le29$.
...
together with a choice of where it starts. Evaluated exactly in integer arithmetic, $T_{2}$
at base $29$ reproduces all $21$ terms A124957 publishes, and at base $28$ all the
...
polynomial. In particular $a_{29}(n)-a_{28}(n)=F_{2}(n)$ for $1\le n\le29$, which
...
First, the model reproduces all $21$ terms A124957 publishes, exactly, in integer
...
$1\le n\le29$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(29,n)$ & $T_{2}(28,n)$ & $F_{2}(n)$ \\ \hline
1 & 29 & 28 & 1 \\
2 & 139 & 134 & 5 \\
3 & 521 & 502 & 19 \\
4 & 2287 & 2202 & 85 \\
