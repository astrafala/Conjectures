...
\title{The cross-base identity for OEIS A124804, proved}
...
\begin{abstract}
OEIS A124804 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124804 is ``Number of base 32 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 32, 94, 218, 586, 1562, 4288, 11876,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:15:03, revision 14)
...
the base is $32$ and the permitted difference is $d=1$, so the claim is that
\[
a_{32}(n)-a_{31}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le32,
\]
...
at base $32$ reproduces all $25$ terms A124804 publishes, and at base $31$ all the
...
polynomial. In particular $a_{32}(n)-a_{31}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124804 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(32,n)$ & $T_{1}(31,n)$ & $F_{1}(n)$ \\ \hline
1 & 32 & 31 & 1 \\
2 & 94 & 91 & 3 \\
3 & 218 & 211 & 7 \\
4 & 586 & 567 & 19 \\
