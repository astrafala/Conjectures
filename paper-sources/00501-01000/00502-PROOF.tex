...
\title{The empirical recurrence for OEIS A283591, proved from an exact model}
...
\begin{abstract}
OEIS A283591 carries an empirical recurrence of order $2$ contributed by Colin Barker. It is true, and it is
...
which $a$ provably satisfies a MONIC linear recurrence of order $S=4$, derived below and not
...
OEIS A283591 is ``Decimal representation of the x-axis, from the left edge to the origin, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 657", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 0, 5, 14, 30, 62, 126, 254,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Conjectures from \_Colin Barker\_, Mar 13 2017: (Start) a(n) = 3*a(n-1) - 2*a(n-2) for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:38, revision 12)
...
\[
C(n+1)\;=\;C(n)\qquad\text{as configurations of the whole plane, for every } n\ge 3.
\]
This was checked directly, and one instance of it is all that is needed: the automaton is a
deterministic map on configurations, so $C(n_0+1)=C(n_0)$ forces $C(n_0+k\cdot1)=C(n_0)$ for
...
neighbourhood to a live cell flips the ENTIRE plane, and rule 657 is read
...
w(n+1)\;=\;L_r + w(n) + R_r
\]
with $L_r$ and $R_r$ fixed words depending only on the residue $r$ of $n$ modulo $1$ --- an
...
\[
a(n+1)\;=\;v(L_r)\,B^{|w(n)|+|R_r|} + a(n)\,B^{|R_r|} + v(R_r).
\]

\section{The bound}

In the shift $T=S^{\,1}$ that is a first-order recurrence with a geometric forcing term and a
constant, killed by $(T-B^{|R_r|})(T-B^{|L_r|+|R_r|})(T-1)$. Taking the DISTINCT roots that
occur across the residue classes, pulling each back to $S^{\,1}-\lambda$, and allowing the
pre-period to raise the numerator's degree gives a monic annihilator of order $S=4$.
...
vanish from $n=4+1$ onwards, and the run exceeds $S+2$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A283591.
