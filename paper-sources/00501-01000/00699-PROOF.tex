...
\title{The cross-base identity for OEIS A125318, proved}
...
\begin{abstract}
OEIS A125318 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125318 is ``Number of base 8 circular n-digit numbers with adjacent digits differing by 3 or less.''. It has offset $0$ and begins
\[
1, 8, 44, 212, 1156, 6428, 36338, 206942,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:01:59, revision 17)
...
the base is $8$ and the permitted difference is $d=3$, so the claim is that
\[
a_{8}(n)-a_{7}(n)\;=\;F_{3}(n)
\qquad\text{for every } n \text{ with } 3\lfloor n/2\rfloor+1\le8,
\]
...
at base $8$ reproduces all $21$ terms A125318 publishes, and at base $7$ all the
...
polynomial. In particular $a_{8}(n)-a_{7}(n)=F_{3}(n)$ for $1\le n\le5$, which
...
First, the model reproduces all $21$ terms A125318 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{3}(8,n)$ & $T_{3}(7,n)$ & $F_{3}(n)$ \\ \hline
1 & 8 & 7 & 1 \\
2 & 44 & 37 & 7 \\
3 & 212 & 175 & 37 \\
4 & 1156 & 925 & 231 \\
