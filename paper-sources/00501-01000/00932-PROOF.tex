...
\title{The cross-base identity for OEIS A126396, proved}
...
\begin{abstract}
OEIS A126396 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126396 is ``Number of base 9 n-digit numbers with adjacent digits differing by two or less.''. It has offset $0$ and begins
\[
1, 9, 39, 175, 793, 3611, 16475, 75229,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:10:41, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $9$ and the permitted difference is $d=2$, so the first clause
claims $a_{9}(n)-a_{8}(n)=5^{\,n-1}$ whenever $2(n-1)+1\le9$, that is for
$1\le n\le5$, and the second claims $a_{9}(n)-a_{8}(n)=5^{\,n-1}-2$ at the single
$n$ with $2(n-1)=9$.
...
arithmetic, $L_{2}$ at base $9$ reproduces all $22$ terms A126396 publishes.
...
First, the model reproduces all $22$ terms A126396 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{2}(9,n)$ & $L_{2}(8,n)$ & $5^{\,n-1}$ \\ \hline
1 & 9 & 8 & 1 \\
2 & 39 & 34 & 5 \\
3 & 175 & 150 & 25 \\
4 & 793 & 668 & 125 \\
