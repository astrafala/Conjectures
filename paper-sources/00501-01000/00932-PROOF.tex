...
\title{The cross-base identity for OEIS A126506, proved}
...
\begin{abstract}
OEIS A126506 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126506 is ``Number of base 11 n-digit numbers with adjacent digits differing by four or less.''. It has offset $0$ and begins
\[
1, 11, 79, 591, 4441, 33391, 251077, 1887941,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:15:15, revision 17)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $11$ and the permitted difference is $d=4$, so the first clause
claims $a_{11}(n)-a_{10}(n)=9^{\,n-1}$ whenever $4(n-1)+1\le11$, that is for
$1\le n\le3$, and the second claims $a_{11}(n)-a_{10}(n)=9^{\,n-1}-2$ at the single
$n$ with $4(n-1)=11$.
...
arithmetic, $L_{4}$ at base $11$ reproduces all $20$ terms A126506 publishes.
...
First, the model reproduces all $20$ terms A126506 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{4}(11,n)$ & $L_{4}(10,n)$ & $9^{\,n-1}$ \\ \hline
1 & 11 & 10 & 1 \\
2 & 79 & 70 & 9 \\
3 & 591 & 510 & 81 \\
\end{tabular}
