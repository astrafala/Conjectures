...
\title{The cross-base identity for OEIS A126418, proved}
...
\begin{abstract}
OEIS A126418 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126418 is ``Number of base 31 n-digit numbers with adjacent digits differing by two or less.''. It has offset $0$ and begins
\[
1, 31, 149, 725, 3543, 17361, 85223, 418899,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:47:41, revision 7)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $31$ and the permitted difference is $d=2$, so the first clause
claims $a_{31}(n)-a_{30}(n)=5^{\,n-1}$ whenever $2(n-1)+1\le31$, that is for
$1\le n\le16$, and the second claims $a_{31}(n)-a_{30}(n)=5^{\,n-1}-2$ at the single
$n$ with $2(n-1)=31$.
...
arithmetic, $L_{2}$ at base $31$ reproduces all $21$ terms A126418 publishes.
...
First, the model reproduces all $21$ terms A126418 publishes, exactly, in integer
...
$1\le n\le16$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{2}(31,n)$ & $L_{2}(30,n)$ & $5^{\,n-1}$ \\ \hline
1 & 31 & 30 & 1 \\
2 & 149 & 144 & 5 \\
3 & 725 & 700 & 25 \\
4 & 3543 & 3418 & 125 \\
