...
\title{The cross-base identity for OEIS A124703, proved}
...
\begin{abstract}
OEIS A124703 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124703 is ``Number of base 10 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 10, 28, 64, 168, 440, 1186, 3230,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:00:30, revision 19)
...
the base is $10$ and the permitted difference is $d=1$, so the claim is that
\[
a_{10}(n)-a_{9}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le10,
\]
...
at base $10$ reproduces all $26$ terms A124703 publishes, and at base $9$ all the
...
polynomial. In particular $a_{10}(n)-a_{9}(n)=F_{1}(n)$ for $1\le n\le19$, which
...
First, the model reproduces all $26$ terms A124703 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(10,n)$ & $T_{1}(9,n)$ & $F_{1}(n)$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 28 & 25 & 3 \\
3 & 64 & 57 & 7 \\
4 & 168 & 149 & 19 \\
