...
\title{The cross-base identity for OEIS A126373, proved}
...
\begin{abstract}
OEIS A126373 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126373 is ``Number of base 19 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 19, 55, 161, 473, 1393, 4109, 12135,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:32, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $19$ and the permitted difference is $d=1$, so the first clause
claims $a_{19}(n)-a_{18}(n)=3^{\,n-1}$ whenever $(n-1)+1\le19$, that is for
$1\le n\le19$, and the second claims $a_{19}(n)-a_{18}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=19$.
...
arithmetic, $L_{1}$ at base $19$ reproduces all $25$ terms A126373 publishes.
...
First, the model reproduces all $25$ terms A126373 publishes, exactly, in integer
...
$1\le n\le19$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(19,n)$ & $L_{1}(18,n)$ & $3^{\,n-1}$ \\ \hline
1 & 19 & 18 & 1 \\
2 & 55 & 52 & 3 \\
3 & 161 & 152 & 9 \\
4 & 473 & 446 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=20$, where $b=19=(n-1)$: there $L_{1}(19,20)-L_{1}(18,20)=1162261465$ and $3^{19}-2=1162261465$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126373}.
\end{enumerate}
