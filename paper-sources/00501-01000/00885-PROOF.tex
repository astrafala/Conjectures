...
\title{The cross-base identity for OEIS A126376, proved}
...
\begin{abstract}
OEIS A126376 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126376 is ``Number of base 22 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 22, 64, 188, 554, 1636, 4838, 14322,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:48, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $22$ and the permitted difference is $d=1$, so the first clause
claims $a_{22}(n)-a_{21}(n)=3^{\,n-1}$ whenever $(n-1)+1\le22$, that is for
$1\le n\le22$, and the second claims $a_{22}(n)-a_{21}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=22$.
...
arithmetic, $L_{1}$ at base $22$ reproduces all $25$ terms A126376 publishes.
...
First, the model reproduces all $25$ terms A126376 publishes, exactly, in integer
...
$1\le n\le22$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(22,n)$ & $L_{1}(21,n)$ & $3^{\,n-1}$ \\ \hline
1 & 22 & 21 & 1 \\
2 & 64 & 61 & 3 \\
3 & 188 & 179 & 9 \\
4 & 554 & 527 & 27 \\
...
and at every row up to $n=22$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=23$, where $b=22=(n-1)$: there $L_{1}(22,23)-L_{1}(21,23)=31381059607$ and $3^{22}-2=31381059607$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126376}.
\end{enumerate}
