...
\title{The cross-base identity for OEIS A124705, proved}
...
\begin{abstract}
OEIS A124705 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124705 is ``Number of base 12 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 12, 34, 78, 206, 542, 1468, 4016,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:00:41, revision 22)
...
the base is $12$ and the permitted difference is $d=1$, so the claim is that
\[
a_{12}(n)-a_{11}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le12,
\]
...
at base $12$ reproduces all $26$ terms A124705 publishes, and at base $11$ all the
...
polynomial. In particular $a_{12}(n)-a_{11}(n)=F_{1}(n)$ for $1\le n\le23$, which
...
First, the model reproduces all $26$ terms A124705 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(12,n)$ & $T_{1}(11,n)$ & $F_{1}(n)$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 34 & 31 & 3 \\
3 & 78 & 71 & 7 \\
4 & 206 & 187 & 19 \\
