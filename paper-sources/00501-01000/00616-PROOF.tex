...
\title{The cross-base identity for OEIS A124715, proved}
...
\begin{abstract}
OEIS A124715 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124715 is ``Number of base 22 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 22, 64, 148, 396, 1052, 2878, 7946,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:01:31, revision 16)
...
the base is $22$ and the permitted difference is $d=1$, so the claim is that
\[
a_{22}(n)-a_{21}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le22,
\]
...
at base $22$ reproduces all $25$ terms A124715 publishes, and at base $21$ all the
...
polynomial. In particular $a_{22}(n)-a_{21}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124715 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(22,n)$ & $T_{1}(21,n)$ & $F_{1}(n)$ \\ \hline
1 & 22 & 21 & 1 \\
2 & 64 & 61 & 3 \\
3 & 148 & 141 & 7 \\
4 & 396 & 377 & 19 \\
