...
\title{The cross-base identity for OEIS A125447, proved}
...
\begin{abstract}
OEIS A125447 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125447 is ``Number of base 12 circular n-digit numbers with adjacent digits differing by 8 or less.''. It has offset $0$ and begins
\[
1, 12, 132, 1380, 15260, 169252, 1879782, 20881502,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:07:33, revision 20)
...
the base is $12$ and the permitted difference is $d=8$, so the claim is that
\[
a_{12}(n)-a_{11}(n)\;=\;F_{8}(n)
\qquad\text{for every } n \text{ with } 8\lfloor n/2\rfloor+1\le12,
\]
...
at base $12$ reproduces all $18$ terms A125447 publishes, and at base $11$ all the
...
polynomial. In particular $a_{12}(n)-a_{11}(n)=F_{8}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $18$ terms A125447 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{8}(12,n)$ & $T_{8}(11,n)$ & $F_{8}(n)$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 132 & 115 & 17 \\
3 & 1380 & 1163 & 217 \\
\end{tabular}
