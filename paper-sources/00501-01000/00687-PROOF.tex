...
\title{The cross-base identity for OEIS A124852, proved}
...
\begin{abstract}
OEIS A124852 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124852 is ``Number of base 10 circular n-digit numbers with adjacent digits differing by 2 or less.''. It has offset $0$ and begins
\[
1, 10, 44, 160, 672, 2860, 12482, 55198,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:15:35, revision 19)
...
the base is $10$ and the permitted difference is $d=2$, so the claim is that
\[
a_{10}(n)-a_{9}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le10,
\]
...
at base $10$ reproduces all $22$ terms A124852 publishes, and at base $9$ all the
...
polynomial. In particular $a_{10}(n)-a_{9}(n)=F_{2}(n)$ for $1\le n\le9$, which
...
First, the model reproduces all $22$ terms A124852 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(10,n)$ & $T_{2}(9,n)$ & $F_{2}(n)$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 44 & 39 & 5 \\
3 & 160 & 141 & 19 \\
4 & 672 & 587 & 85 \\
