...
\title{The cross-base identity for OEIS A125349, proved}
...
\begin{abstract}
OEIS A125349 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125349 is ``Number of base 12 circular n-digit numbers with adjacent digits differing by 4 or less.''. It has offset $0$ and begins
\[
1, 12, 88, 552, 3968, 29112, 218008, 1650528,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:03:42, revision 13)
...
the base is $12$ and the permitted difference is $d=4$, so the claim is that
\[
a_{12}(n)-a_{11}(n)\;=\;F_{4}(n)
\qquad\text{for every } n \text{ with } 4\lfloor n/2\rfloor+1\le12,
\]
that is, for $1\le n\le5$.
...
at base $12$ reproduces all $20$ terms A125349 publishes, and at base $11$ all the
...
polynomial. In particular $a_{12}(n)-a_{11}(n)=F_{4}(n)$ for $1\le n\le5$, which
...
First, the model reproduces all $20$ terms A125349 publishes, exactly, in integer
...
$1\le n\le5$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{4}(12,n)$ & $T_{4}(11,n)$ & $F_{4}(n)$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 88 & 79 & 9 \\
3 & 552 & 491 & 61 \\
4 & 3968 & 3479 & 489 \\
