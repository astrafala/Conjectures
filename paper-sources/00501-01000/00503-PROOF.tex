...
\title{The empirical recurrence for OEIS A280808, proved by counting patterns with a budget}
...
\begin{abstract}
OEIS A280808 imposes three things at once: a condition on each cell that reaches one row up and
...
from a finite exact computation on $S=12291$ states.
\end{abstract}
...
OEIS A280808 is ``Number of nX6 0..1 arrays with no element equal to more than two of its horizontal, vertical and antidiagonal neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
0, 284, 6540, 37644, 155560, 547197, 1736603, 5150299,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 08 2017 09:45:35, revision 4) the statement is
...
Assembling the $2$ chains block-diagonally into $N$ of size $S=12291$ with the weights of
...
index corresponding to $n=53+1$ onwards, and the run continued until $S+1$ consecutive
zeros had been seen.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A280808.
