...
\title{The cross-base identity for OEIS A126381, proved}
...
\begin{abstract}
OEIS A126381 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126381 is ``Number of base 27 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 27, 79, 233, 689, 2041, 6053, 17967,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:10:15, revision 9)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $27$ and the permitted difference is $d=1$, so the first clause
claims $a_{27}(n)-a_{26}(n)=3^{\,n-1}$ whenever $(n-1)+1\le27$, that is for
$1\le n\le27$, and the second claims $a_{27}(n)-a_{26}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=27$.
...
arithmetic, $L_{1}$ at base $27$ reproduces all $25$ terms A126381 publishes.
...
First, the model reproduces all $25$ terms A126381 publishes, exactly, in integer
...
$1\le n\le27$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(27,n)$ & $L_{1}(26,n)$ & $3^{\,n-1}$ \\ \hline
1 & 27 & 26 & 1 \\
2 & 79 & 76 & 3 \\
3 & 233 & 224 & 9 \\
4 & 689 & 662 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=28$, where $b=27=(n-1)$: there $L_{1}(27,28)-L_{1}(26,28)=7625597484985$ and $3^{27}-2=7625597484985$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126381}.
\end{enumerate}
