...
\title{The cross-base identity for OEIS A125397, proved}
...
\begin{abstract}
OEIS A125397 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125397 is ``Number of base 9 circular n-digit numbers with adjacent digits differing by 6 or less.''. It has offset $0$ and begins
\[
1, 9, 75, 597, 4999, 41959, 352605, 2963557,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:05:26, revision 18)
...
the base is $9$ and the permitted difference is $d=6$, so the claim is that
\[
a_{9}(n)-a_{8}(n)\;=\;F_{6}(n)
\qquad\text{for every } n \text{ with } 6\lfloor n/2\rfloor+1\le9,
\]
...
at base $9$ reproduces all $20$ terms A125397 publishes, and at base $8$ all the
...
polynomial. In particular $a_{9}(n)-a_{8}(n)=F_{6}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $20$ terms A125397 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{6}(9,n)$ & $T_{6}(8,n)$ & $F_{6}(n)$ \\ \hline
1 & 9 & 8 & 1 \\
2 & 75 & 62 & 13 \\
3 & 597 & 470 & 127 \\
\end{tabular}
