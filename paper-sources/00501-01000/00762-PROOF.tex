...
\title{Recovering the unwritten recurrences of the table OEIS A256029}
...
\begin{abstract}
OEIS A256029 is a two-dimensional table $T(n,k)$ whose empirical recurrences are, for several of
...
OEIS A256029 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 0 or 1 and no antidiagonal sum 0 or 1 and no row sum 1 and no column sum 1.''. It is read by antidiagonals and begins
\[
33, 68, 68, 154, 164, 154, 352, 484,\ \dots
\]
The entry carries, among its empirical claims:
\begin{quote}\small
k=2: [order 12]
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:44:22, revision 6) these are still
...
$c_{1}$ & $3$ & $c_{4}$ & $-9$ & $c_{7}$ & $7$ & $c_{10}$ & $-3$ \\
$c_{2}$ & $-1$ & $c_{5}$ & $-5$ & $c_{8}$ & $7$ & $c_{11}$ & $-2$ \\
$c_{3}$ & $5$ & $c_{6}$ & $-3$ & $c_{9}$ & $4$ & $c_{12}$ & $-2$ \\
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A256029.
