...
\title{The cross-base identity for OEIS A124709, proved}
...
\begin{abstract}
OEIS A124709 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124709 is ``Number of base 16 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 16, 46, 106, 282, 746, 2032, 5588,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:00:58, revision 16)
...
the base is $16$ and the permitted difference is $d=1$, so the claim is that
\[
a_{16}(n)-a_{15}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le16,
\]
...
at base $16$ reproduces all $25$ terms A124709 publishes, and at base $15$ all the
...
polynomial. In particular $a_{16}(n)-a_{15}(n)=F_{1}(n)$ for $1\le n\le31$, which
...
First, the model reproduces all $25$ terms A124709 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(16,n)$ & $T_{1}(15,n)$ & $F_{1}(n)$ \\ \hline
1 & 16 & 15 & 1 \\
2 & 46 & 43 & 3 \\
3 & 106 & 99 & 7 \\
4 & 282 & 263 & 19 \\
