...
\title{The cross-base identity for OEIS A125400, proved}
...
\begin{abstract}
OEIS A125400 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125400 is ``Number of base 12 circular n-digit numbers with adjacent digits differing by 6 or less.''. It has offset $0$ and begins
\[
1, 12, 114, 978, 9326, 90242, 879720, 8596348,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:05:43, revision 14)
...
the base is $12$ and the permitted difference is $d=6$, so the claim is that
\[
a_{12}(n)-a_{11}(n)\;=\;F_{6}(n)
\qquad\text{for every } n \text{ with } 6\lfloor n/2\rfloor+1\le12,
\]
...
at base $12$ reproduces all $19$ terms A125400 publishes, and at base $11$ all the
...
polynomial. In particular $a_{12}(n)-a_{11}(n)=F_{6}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $19$ terms A125400 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{6}(12,n)$ & $T_{6}(11,n)$ & $F_{6}(n)$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 114 & 101 & 13 \\
3 & 978 & 851 & 127 \\
\end{tabular}
