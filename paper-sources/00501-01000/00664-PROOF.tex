...
\title{The cross-base identity for OEIS A124843, proved}
...
\begin{abstract}
OEIS A124843 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124843 is ``Number of base 8 circular n-digit numbers with adjacent digits differing by 2 or less.''. It has offset $0$ and begins
\[
1, 8, 34, 122, 502, 2098, 8980, 38928,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:15:21, revision 20)
...
the base is $8$ and the permitted difference is $d=2$, so the claim is that
\[
a_{8}(n)-a_{7}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le8,
\]
...
at base $8$ reproduces all $22$ terms A124843 publishes, and at base $7$ all the
...
polynomial. In particular $a_{8}(n)-a_{7}(n)=F_{2}(n)$ for $1\le n\le7$, which
...
First, the model reproduces all $22$ terms A124843 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(8,n)$ & $T_{2}(7,n)$ & $F_{2}(n)$ \\ \hline
1 & 8 & 7 & 1 \\
2 & 34 & 29 & 5 \\
3 & 122 & 103 & 19 \\
4 & 502 & 417 & 85 \\
