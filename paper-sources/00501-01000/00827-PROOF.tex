...
\title{The cross-base identity for OEIS A126364, proved}
...
\begin{abstract}
OEIS A126364 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126364 is ``Number of base 10 n-digit numbers with adjacent digits differing by one or less.''. It has offset $1$ and begins
\[
10, 28, 80, 230, 664, 1922, 5574, 16188,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:42:24, revision 40)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $10$ and the permitted difference is $d=1$, so the first clause
claims $a_{10}(n)-a_{9}(n)=3^{\,n-1}$ whenever $(n-1)+1\le10$, that is for
$1\le n\le10$, and the second claims $a_{10}(n)-a_{9}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=10$.
...
arithmetic, $L_{1}$ at base $10$ reproduces all $27$ terms A126364 publishes.
...
First, the model reproduces all $27$ terms A126364 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(10,n)$ & $L_{1}(9,n)$ & $3^{\,n-1}$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 28 & 25 & 3 \\
3 & 80 & 71 & 9 \\
4 & 230 & 203 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=11$, where $b=10=(n-1)$: there $L_{1}(10,11)-L_{1}(9,11)=59047$ and $3^{10}-2=59047$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126364}.
\end{enumerate}
