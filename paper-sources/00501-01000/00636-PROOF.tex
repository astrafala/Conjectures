...
\title{The cross-base identity for OEIS A124704, proved}
...
\begin{abstract}
OEIS A124704 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124704 is ``Number of base 11 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 11, 31, 71, 187, 491, 1327, 3623,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:00:36, revision 20)
...
the base is $11$ and the permitted difference is $d=1$, so the claim is that
\[
a_{11}(n)-a_{10}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le11,
\]
...
at base $11$ reproduces all $26$ terms A124704 publishes, and at base $10$ all the
...
polynomial. In particular $a_{11}(n)-a_{10}(n)=F_{1}(n)$ for $1\le n\le21$, which
...
First, the model reproduces all $26$ terms A124704 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(11,n)$ & $T_{1}(10,n)$ & $F_{1}(n)$ \\ \hline
1 & 11 & 10 & 1 \\
2 & 31 & 28 & 3 \\
3 & 71 & 64 & 7 \\
4 & 187 & 168 & 19 \\
