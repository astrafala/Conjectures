...
\title{The cross-base identity for OEIS A124806, proved}
...
\begin{abstract}
OEIS A124806 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124806 is ``Number of circular n-letter words over the alphabet {0,1,2,3,4} with adjacent letters differing by at most 2.''. It has offset $0$ and begins
\[
1, 5, 19, 65, 247, 955, 3733, 14649,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 30 2025 14:08:32, revision 24)
...
the base is $5$ and the permitted difference is $d=2$, so the claim is that
\[
a_{5}(n)-a_{4}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le5,
\]
...
at base $5$ reproduces all $24$ terms A124806 publishes, and at base $4$ all the
...
polynomial. In particular $a_{5}(n)-a_{4}(n)=F_{2}(n)$ for $1\le n\le5$, which
...
First, the model reproduces all $24$ terms A124806 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(5,n)$ & $T_{2}(4,n)$ & $F_{2}(n)$ \\ \hline
1 & 5 & 4 & 1 \\
2 & 19 & 14 & 5 \\
3 & 65 & 46 & 19 \\
4 & 247 & 162 & 85 \\
