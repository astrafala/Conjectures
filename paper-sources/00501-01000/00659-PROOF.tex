...
\title{The cross-base identity for OEIS A124710, proved}
...
\begin{abstract}
OEIS A124710 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124710 is ``Number of base 17 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 17, 49, 113, 301, 797, 2173, 5981,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:01:04, revision 16)
...
the base is $17$ and the permitted difference is $d=1$, so the claim is that
\[
a_{17}(n)-a_{16}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le17,
\]
that is, for $1\le n\le33$.
...
at base $17$ reproduces all $25$ terms A124710 publishes, and at base $16$ all the
...
polynomial. In particular $a_{17}(n)-a_{16}(n)=F_{1}(n)$ for $1\le n\le33$, which
...
First, the model reproduces all $25$ terms A124710 publishes, exactly, in integer
...
$1\le n\le33$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(17,n)$ & $T_{1}(16,n)$ & $F_{1}(n)$ \\ \hline
1 & 17 & 16 & 1 \\
2 & 49 & 46 & 3 \\
3 & 113 & 106 & 7 \\
4 & 301 & 282 & 19 \\
