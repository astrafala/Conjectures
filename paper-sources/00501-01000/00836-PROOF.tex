...
\title{The empirical closed form for OEIS A219168, proved}
...
\begin{abstract}
OEIS A219168 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=182$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219168 is ``Number of nX3 arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..2 nX3 array.''. It has offset $1$ and begins
\[
6, 23, 146, 644, 2399, 8024, 24670, 70595,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 17:56:13, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=182$
...
states number $S=182$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219168 records is correct for all $n>6$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $23$.
Second, the polynomial and the walk count agree at every index from $7$ up to
$1 + 25 - 1$, which is more than the $17$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219168.
