...
\title{The cross-base identity for OEIS A126362, proved}
...
\begin{abstract}
OEIS A126362 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126362 is ``Number of base 8 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 8, 22, 62, 176, 502, 1436, 4116,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:08:43, revision 23)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $8$ and the permitted difference is $d=1$, so the first clause
claims $a_{8}(n)-a_{7}(n)=3^{\,n-1}$ whenever $(n-1)+1\le8$, that is for
$1\le n\le8$, and the second claims $a_{8}(n)-a_{7}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=8$.
...
arithmetic, $L_{1}$ at base $8$ reproduces all $26$ terms A126362 publishes.
...
First, the model reproduces all $26$ terms A126362 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(8,n)$ & $L_{1}(7,n)$ & $3^{\,n-1}$ \\ \hline
1 & 8 & 7 & 1 \\
2 & 22 & 19 & 3 \\
3 & 62 & 53 & 9 \\
4 & 176 & 149 & 27 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=9$, where $b=8=(n-1)$: there $L_{1}(8,9)-L_{1}(7,9)=6559$ and $3^{8}-2=6559$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126362}.
\end{enumerate}
