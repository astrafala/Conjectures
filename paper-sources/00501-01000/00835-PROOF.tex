...
\title{The cross-base identity for OEIS A125475, proved}
...
\begin{abstract}
OEIS A125475 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125475 is ``Number of base 18 circular n-digit numbers with adjacent digits differing by 9 or less.''. It has offset $0$ and begins
\[
1, 18, 252, 3168, 44472, 634368, 9124092, 131601852,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:40:39, revision 12)
...
the base is $18$ and the permitted difference is $d=9$, so the claim is that
\[
a_{18}(n)-a_{17}(n)\;=\;F_{9}(n)
\qquad\text{for every } n \text{ with } 9\lfloor n/2\rfloor+1\le18,
\]
...
at base $18$ reproduces all $17$ terms A125475 publishes, and at base $17$ all the
...
polynomial. In particular $a_{18}(n)-a_{17}(n)=F_{9}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $17$ terms A125475 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{9}(18,n)$ & $T_{9}(17,n)$ & $F_{9}(n)$ \\ \hline
1 & 18 & 17 & 1 \\
2 & 252 & 233 & 19 \\
3 & 3168 & 2897 & 271 \\
\end{tabular}
