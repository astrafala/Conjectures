...
\title{The cross-base identity for OEIS A126378, proved}
...
\begin{abstract}
OEIS A126378 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126378 is ``Number of base 24 n-digit numbers with adjacent digits differing by one or less.''. It has offset $0$ and begins
\[
1, 24, 70, 206, 608, 1798, 5324, 15780,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:09:59, revision 12)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $24$ and the permitted difference is $d=1$, so the first clause
claims $a_{24}(n)-a_{23}(n)=3^{\,n-1}$ whenever $(n-1)+1\le24$, that is for
$1\le n\le24$, and the second claims $a_{24}(n)-a_{23}(n)=3^{\,n-1}-2$ at the single
$n$ with $(n-1)=24$.
...
arithmetic, $L_{1}$ at base $24$ reproduces all $25$ terms A126378 publishes.
...
First, the model reproduces all $25$ terms A126378 publishes, exactly, in integer
...
$1\le n\le24$, both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{1}(24,n)$ & $L_{1}(23,n)$ & $3^{\,n-1}$ \\ \hline
1 & 24 & 23 & 1 \\
2 & 70 & 67 & 3 \\
3 & 206 & 197 & 9 \\
4 & 608 & 581 & 27 \\
...
and at every row up to $n=24$.

Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=25$, where $b=24=(n-1)$: there $L_{1}(24,25)-L_{1}(23,25)=282429536479$ and $3^{24}-2=282429536479$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126378}.
\end{enumerate}
