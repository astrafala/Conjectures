...
\title{The cross-base identity for OEIS A125371, proved}
...
\begin{abstract}
OEIS A125371 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125371 is ``Number of base 8 circular n-digit numbers with adjacent digits differing by 5 or less.''. It has offset $0$ and begins
\[
1, 8, 58, 398, 2902, 21238, 155716, 1142016,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:04:26, revision 20)
...
the base is $8$ and the permitted difference is $d=5$, so the claim is that
\[
a_{8}(n)-a_{7}(n)\;=\;F_{5}(n)
\qquad\text{for every } n \text{ with } 5\lfloor n/2\rfloor+1\le8,
\]
...
at base $8$ reproduces all $20$ terms A125371 publishes, and at base $7$ all the
...
polynomial. In particular $a_{8}(n)-a_{7}(n)=F_{5}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $20$ terms A125371 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{5}(8,n)$ & $T_{5}(7,n)$ & $F_{5}(n)$ \\ \hline
1 & 8 & 7 & 1 \\
2 & 58 & 47 & 11 \\
3 & 398 & 307 & 91 \\
\end{tabular}
