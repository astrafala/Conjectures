...
\title{The cross-base identity for OEIS A124714, proved}
...
\begin{abstract}
OEIS A124714 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124714 is ``Number of base 21 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 21, 61, 141, 377, 1001, 2737, 7553,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:01:25, revision 16)
...
the base is $21$ and the permitted difference is $d=1$, so the claim is that
\[
a_{21}(n)-a_{20}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le21,
\]
...
at base $21$ reproduces all $25$ terms A124714 publishes, and at base $20$ all the
...
polynomial. In particular $a_{21}(n)-a_{20}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124714 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(21,n)$ & $T_{1}(20,n)$ & $F_{1}(n)$ \\ \hline
1 & 21 & 20 & 1 \\
2 & 61 & 58 & 3 \\
3 & 141 & 134 & 7 \\
4 & 377 & 358 & 19 \\
