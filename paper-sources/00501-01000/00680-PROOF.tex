...
\title{The cross-base identity for OEIS A125341, proved}
...
\begin{abstract}
OEIS A125341 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125341 is ``Number of base 31 circular n-digit numbers with adjacent digits differing by 3 or less.''. It has offset $0$ and begins
\[
1, 31, 205, 1063, 6469, 39801, 250945, 1602569,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:24:39, revision 11)
...
entry itself as the largest coefficient of $(1+x+\dots+x^{2d})^{n}$. The entry writes that summand as $\mathrm{A025012}(n+1)$, the $(n+1)$st term of OEIS A025012; that sequence is the central coefficient of $(1+x+\dots+x^{6})^{n}$ and its published terms agree with $F_{3}(n)$ term by term, which is the identification used here. For this entry
the base is $31$ and the permitted difference is $d=3$, so the claim is that
\[
a_{31}(n)-a_{30}(n)\;=\;F_{3}(n)
\qquad\text{for every } n \text{ with } 3\lfloor n/2\rfloor+1\le31,
\]
that is, for $1\le n\le21$.
...
together with a choice of where it starts. Evaluated exactly in integer arithmetic, $T_{3}$
at base $31$ reproduces all $20$ terms A125341 publishes, and at base $30$ all the
...
polynomial. In particular $a_{31}(n)-a_{30}(n)=F_{3}(n)$ for $1\le n\le21$, which
...
First, the model reproduces all $20$ terms A125341 publishes, exactly, in integer
...
$1\le n\le21$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{3}(31,n)$ & $T_{3}(30,n)$ & $F_{3}(n)$ \\ \hline
1 & 31 & 30 & 1 \\
2 & 205 & 198 & 7 \\
3 & 1063 & 1026 & 37 \\
4 & 6469 & 6238 & 231 \\
5 & 39801 & 38350 & 1451 \\
6 & 250945 & 241614 & 9331 \\
7 & 1602569 & 1541878 & 60691 \\
8 & 10342789 & 9944222 & 398567 \\
\end{tabular}
\end{center}

\noindent and the difference of the second and third columns is the fourth at every row shown
and at every row up to $n=21$.
