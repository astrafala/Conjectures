...
\title{The empirical closed form for OEIS A220155, proved}
...
\begin{abstract}
OEIS A220155 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=340$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220155 is ``Number of 3Xn arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 3Xn array.''. It has offset $1$ and begins
\[
10, 26, 77, 254, 645, 1413, 2841, 5412,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:32:10, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=340$
...
states number $S=340$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220155 records is correct for all $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220155.
