...
\title{The empirical closed form for OEIS A219681, proved}
...
\begin{abstract}
OEIS A219681 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=74$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219681 is ``Number of nX3 arrays of the minimum value of corresponding elements and their horizontal, vertical or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 nX3 array.''. It has offset $1$ and begins
\[
6, 8, 31, 99, 269, 655, 1506, 3356,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:12:46, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=74$
...
states number $S=74$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219681 records is correct for all $n>4$.
\end{theorem}
...
$1 + 30 - 1$, which is more than the $12$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219681.
