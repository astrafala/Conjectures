...
\title{The empirical recurrence for OEIS A202256, proved from an exact model}
...
\begin{abstract}
OEIS A202256 carries an empirical recurrence of order $19$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=71$, derived below and not
...
OEIS A202256 is ``Number of zero-sum -n..n arrays of 6 elements with adjacent element differences also in -n..n.''. It has offset $1$ and begins
\[
33, 387, 2003, 6963, 18841, 43293, 88301, 164873,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 15 2025 10:22:04, revision 15)
...
\]
annihilates $a$, and $S=\deg A=71$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=71$, and
...
$q(z)=z^{19}-\sum_ic_iz^{19-i}$ the characteristic polynomial of the conjectured
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-2) + 2\,a(n-3) - 3\,a(n-5) - 3\,a(n-6) - 2\,a(n-7) + a(n-8) + 4\,a(n-9) + 4\,a(n-10) + a(n-11) - 2\,a(n-12) - 3\,a(n-13) - 3\,a(n-14) + 2\,a(n-16) + 2\,a(n-17) - a(n-19)
\]
for every $n>0$.
\end{theorem}
...
vanish from $n=0+1$ onwards, and the run exceeds $S+19$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202256.
