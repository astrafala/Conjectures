...
\title{The cross-base identity for OEIS A126397, proved}
...
\begin{abstract}
OEIS A126397 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126397 is ``Number of base 10 n-digit numbers with adjacent digits differing by two or less.''. It has offset $0$ and begins
\[
1, 10, 44, 200, 918, 4236, 19598, 90790,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:10:46, revision 14)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $10$ and the permitted difference is $d=2$, so the first clause
claims $a_{10}(n)-a_{9}(n)=5^{\,n-1}$ whenever $2(n-1)+1\le10$, that is for
$1\le n\le5$, and the second claims $a_{10}(n)-a_{9}(n)=5^{\,n-1}-2$ at the single
$n$ with $2(n-1)=10$.
...
arithmetic, $L_{2}$ at base $10$ reproduces all $22$ terms A126397 publishes.
...
First, the model reproduces all $22$ terms A126397 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{2}(10,n)$ & $L_{2}(9,n)$ & $5^{\,n-1}$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 44 & 39 & 5 \\
3 & 200 & 175 & 25 \\
4 & 918 & 793 & 125 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=6$, where $b=10=2(n-1)$: there $L_{2}(10,6)-L_{2}(9,6)=3123$ and $5^{5}-2=3123$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126397}.
\end{enumerate}
