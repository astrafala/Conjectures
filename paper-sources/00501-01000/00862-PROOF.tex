...
\title{The empirical recurrence for OEIS A223320, proved by transfer matrix}
...
\begin{abstract}
OEIS A223320 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$7$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is icosahedron graph. The entry carries an empirical recurrence of order $8$
contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical. A slice of
the array would be a state, but there are $187500$ legal slices here and that is too many to
...
then be run on the ORBITS of slices instead of on the slices, and $S=1563$ of those suffice.
\end{abstract}
...
OEIS A223320 is ``Rolling icosahedron footprints: number of nX7 0..11 arrays starting with 0 where 0..11 label vertices of an icosahedron and every array movement to a horizontal or antidiagonal neighbor moves along an icosahedral edge.''. It has offset $1$ and begins
\[
15625, 46411625, 142980696625, 443803619416625, 1379464144963464625, 4288761014162797342625, 13334277936752000032650625, 41458126974532997739670258625,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 21:30:50, revision 8) this is still
...
There are $187500$ legal $row$s, which is more than can be carried one at a time. They
need not be.
...
\]
and the count the entry publishes is that divided by $12$, by Lemma~\ref{lem:fibre}. Here $S=|\mathcal{O}|=1563$, down from $187500$.
...
\begin{theorem}
\[
a(n)\;=\;4313\,a(n-1) - 4293540\,a(n-2) + 1835925129\,a(n-3) - 400542201531\,a(n-4) + 46506128233194\,a(n-5) - 2762797384141236\,a(n-6) + 72983536818080616\,a(n-7) - 643864107456947520\,a(n-8)
\]
for every $n>8$.
\end{theorem}
...
$n=8+1$ onwards and the run continued until the working vector was identically zero, which
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223320.
