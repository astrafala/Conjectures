...
\title{The cross-base identity for OEIS A126529, proved}
...
\begin{abstract}
OEIS A126529 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126529 is ``Number of base 8 n-digit numbers with adjacent digits differing by five or less.''. It has offset $0$ and begins
\[
1, 8, 58, 426, 3124, 22914, 168066, 1232708,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:17:04, revision 15)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $8$ and the permitted difference is $d=5$, so the first clause
claims $a_{8}(n)-a_{7}(n)=11^{\,n-1}$ whenever $5(n-1)+1\le8$, that is for
$1\le n\le2$, and the second claims $a_{8}(n)-a_{7}(n)=11^{\,n-1}-2$ at the single
$n$ with $5(n-1)=8$.
...
arithmetic, $L_{5}$ at base $8$ reproduces all $20$ terms A126529 publishes.
...
First, the model reproduces all $20$ terms A126529 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{5}(8,n)$ & $L_{5}(7,n)$ & $11^{\,n-1}$ \\ \hline
1 & 8 & 7 & 1 \\
2 & 58 & 47 & 11 \\
\end{tabular}
\end{center}
