...
\title{The cross-base identity for OEIS A124953, proved}
...
\begin{abstract}
OEIS A124953 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124953 is ``Number of base 25 circular n-digit numbers with adjacent digits differing by 2 or less.''. It has offset $0$ and begins
\[
1, 25, 119, 445, 1947, 8575, 38747, 177223,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:17:51, revision 16)
...
the base is $25$ and the permitted difference is $d=2$, so the claim is that
\[
a_{25}(n)-a_{24}(n)\;=\;F_{2}(n)
\qquad\text{for every } n \text{ with } 2\lfloor n/2\rfloor+1\le25,
\]
that is, for $1\le n\le25$.
...
at base $25$ reproduces all $22$ terms A124953 publishes, and at base $24$ all the
...
polynomial. In particular $a_{25}(n)-a_{24}(n)=F_{2}(n)$ for $1\le n\le25$, which
...
First, the model reproduces all $22$ terms A124953 publishes, exactly, in integer
...
$1\le n\le25$, with both sides computed from the definition and not from the theorem:

\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{2}(25,n)$ & $T_{2}(24,n)$ & $F_{2}(n)$ \\ \hline
1 & 25 & 24 & 1 \\
2 & 119 & 114 & 5 \\
3 & 445 & 426 & 19 \\
4 & 1947 & 1862 & 85 \\
