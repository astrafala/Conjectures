...
\title{The cross-base identity for OEIS A125322, proved}
...
\begin{abstract}
OEIS A125322 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125322 is ``Number of base 12 circular n-digit numbers with adjacent digits differing by 3 or less.''. It has offset $0$ and begins
\[
1, 12, 72, 360, 2080, 12232, 73656, 449440,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:02:21, revision 13)
...
the base is $12$ and the permitted difference is $d=3$, so the claim is that
\[
a_{12}(n)-a_{11}(n)\;=\;F_{3}(n)
\qquad\text{for every } n \text{ with } 3\lfloor n/2\rfloor+1\le12,
\]
...
at base $12$ reproduces all $20$ terms A125322 publishes, and at base $11$ all the
...
polynomial. In particular $a_{12}(n)-a_{11}(n)=F_{3}(n)$ for $1\le n\le7$, which
...
First, the model reproduces all $20$ terms A125322 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{3}(12,n)$ & $T_{3}(11,n)$ & $F_{3}(n)$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 72 & 65 & 7 \\
3 & 360 & 323 & 37 \\
4 & 2080 & 1849 & 231 \\
