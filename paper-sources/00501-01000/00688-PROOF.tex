...
\title{The empirical closed form for OEIS A220199, proved}
...
\begin{abstract}
OEIS A220199 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=410$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220199 is ``Number of nX3 arrays of the minimum value of corresponding elements and their horizontal, vertical or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 nX3 array.''. It has offset $1$ and begins
\[
10, 36, 238, 1013, 3916, 15067, 58288, 218422,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:33:56, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=410$
...
states number $S=410$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220199 records is correct for all $n>10$.
\end{theorem}
...
$1 + 23 - 1$, which is more than the $30$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220199.
