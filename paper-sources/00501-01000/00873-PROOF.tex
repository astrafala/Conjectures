...
\title{The cross-base identity for OEIS A126477, proved}
...
\begin{abstract}
OEIS A126477 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126477 is ``Number of base 9 n-digit numbers with adjacent digits differing by three or less.''. It has offset $0$ and begins
\[
1, 9, 51, 301, 1787, 10619, 63111, 375091,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:12:58, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $9$ and the permitted difference is $d=3$, so the first clause
claims $a_{9}(n)-a_{8}(n)=7^{\,n-1}$ whenever $3(n-1)+1\le9$, that is for
$1\le n\le3$, and the second claims $a_{9}(n)-a_{8}(n)=7^{\,n-1}-2$ at the single
$n$ with $3(n-1)=9$.
...
arithmetic, $L_{3}$ at base $9$ reproduces all $20$ terms A126477 publishes.
...
First, the model reproduces all $20$ terms A126477 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{3}(9,n)$ & $L_{3}(8,n)$ & $7^{\,n-1}$ \\ \hline
1 & 9 & 8 & 1 \\
2 & 51 & 44 & 7 \\
3 & 301 & 252 & 49 \\
\end{tabular}
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=4$, where $b=9=3(n-1)$: there $L_{3}(9,4)-L_{3}(8,4)=341$ and $7^{3}-2=341$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126477}.
\end{enumerate}
