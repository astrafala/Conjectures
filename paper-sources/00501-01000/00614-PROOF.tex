...
\title{The empirical closed form for OEIS A219383, proved}
...
\begin{abstract}
OEIS A219383 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=235$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219383 is ``Number of 3Xn arrays of the minimum value of corresponding elements and their horizontal, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..2 3Xn array.''. It has offset $1$ and begins
\[
10, 23, 84, 315, 957, 2792, 7472, 18910,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:00:57, revision 10)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=235$
...
states number $S=235$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219383 records is correct for all $n>16$.
\end{theorem}
...
Second, the polynomial and the walk count agree at every index from $17$ up to
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219383.
