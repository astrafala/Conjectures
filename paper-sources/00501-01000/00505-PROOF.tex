...
\title{The empirical recurrence for OEIS A187156, proved from an exact model}
...
\begin{abstract}
OEIS A187156 carries an empirical recurrence of order $3$. It is true, and it is
...
OEIS A187156 is ``Number of 3-step one space at a time bishop's tours on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 20, 64, 132, 224, 340, 480,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 12*n\^{}2 - 40*n + 32 for n>1. a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 28 2018 10:43:59, revision 10)
...
SHAPE when one is a translate of the other. There are exactly $12$ shapes: self-avoiding
...
Write $w^\ast$ for the largest bounding-box side over all $12$ shapes. Once $n\ge w^\ast$
...
$z^{3}(z-1)^{3}$, monic of order $S=6$. Both numbers are read off the enumerated
...
First, the model reproduces all $43$ terms the entry publishes, exactly, in integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187156.
