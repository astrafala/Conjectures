...
\title{The cross-base identity for OEIS A126480, proved}
...
\begin{abstract}
OEIS A126480 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126480 is ``Number of base 12 n-digit numbers with adjacent digits differing by three or less.''. It has offset $0$ and begins
\[
1, 12, 72, 448, 2816, 17780, 112440, 711504,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:48:21, revision 19)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $12$ and the permitted difference is $d=3$, so the first clause
claims $a_{12}(n)-a_{11}(n)=7^{\,n-1}$ whenever $3(n-1)+1\le12$, that is for
$1\le n\le4$, and the second claims $a_{12}(n)-a_{11}(n)=7^{\,n-1}-2$ at the single
$n$ with $3(n-1)=12$.
...
arithmetic, $L_{3}$ at base $12$ reproduces all $20$ terms A126480 publishes.
...
First, the model reproduces all $20$ terms A126480 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{3}(12,n)$ & $L_{3}(11,n)$ & $7^{\,n-1}$ \\ \hline
1 & 12 & 11 & 1 \\
2 & 72 & 65 & 7 \\
3 & 448 & 399 & 49 \\
4 & 2816 & 2473 & 343 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=5$, where $b=12=3(n-1)$: there $L_{3}(12,5)-L_{3}(11,5)=2399$ and $7^{4}-2=2399$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126480}.
\end{enumerate}
