...
\title{The empirical recurrence for OEIS A221401, proved by transfer matrix}
...
\begin{abstract}
OEIS A221401 counts the distinct OCCUPANCY arrays obtained when every cell of an $n\times2$ grid
...
an empirical recurrence of order $8$ contributed by R.~H.~Hardin. It is true, and it is
...
one, on $S=99$ states, after which the conjectured recurrence is settled by a finite exact
computation.
\end{abstract}
...
OEIS A221401 is ``Number of 2 X n arrays of occupancy after each element moves to some horizontal, diagonal or antidiagonal neighbor, with no 2-loops and with no occupancy greater than 2.''. It has offset $1$ and begins
\[
0, 1, 6, 56, 324, 2196, 13328, 83680,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 7*a(n-1) + 6*a(n-2) - 74*a(n-3) + 8*a(n-4) + 132*a(n-5) - 72*a(n-6) - 24*a(n-7) + 16*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 05 2018 08:21:05, revision 8) this is still
...
zero offset on either side of the pair carries no condition at all. The entry asks in addition that no occupancy exceed $2$, a condition on the array recorded rather than on the moves.
...
$S=99$ reachable sets. In particular $a$ is $C$-finite.
...
\begin{theorem}
\[
a(n)\;=\;7\,a(n-1) + 6\,a(n-2) - 74\,a(n-3) + 8\,a(n-4) + 132\,a(n-5) - 72\,a(n-6) - 24\,a(n-7) + 16\,a(n-8)
\]
for every $n>8$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221401.
