...
\title{The empirical recurrence for OEIS A279129, proved by counting patterns with a budget}
...
\begin{abstract}
OEIS A279129 imposes three things at once: a condition on each cell that reaches one row up and
...
numbers. The empirical recurrence of order $11$ contributed by R.~H.~Hardin then follows
...
OEIS A279129 is ``Number of nX3 0..1 arrays with no element equal to a strict majority of its horizontal and vertical neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
0, 9, 66, 256, 820, 2551, 7491, 21131,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) -11*a(n-2) +5*a(n-3) +a(n-5) +9*a(n-6) -6*a(n-7) +3*a(n-8) -5*a(n-9) -a(n-11) for n>17
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 06 2016 18:57:14, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) - 11\,a(n-2) + 5\,a(n-3) + a(n-5) + 9\,a(n-6) - 6\,a(n-7) + 3\,a(n-8) - 5\,a(n-9) - a(n-11)
\end{equation}
for all $n>17$.
...
\[
2!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)=\mathbf{v}^{\!\top}G^{\,j-11}w,
...
$11$ applications of $G$ in exact integer arithmetic and $\mathbf{v}^{\!\top}G^{m}w$
evaluated for $m=0,1,\dots$, again exactly. Every one of those integers vanishes from the
index corresponding to $n=17+1$ onwards, and the run continued until $S+1$ consecutive
zeros had been seen.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A279129.
