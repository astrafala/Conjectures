...
\title{The cross-base identity for OEIS A125422, proved}
...
\begin{abstract}
OEIS A125422 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125422 is ``Number of base 10 circular n-digit numbers with adjacent digits differing by 7 or less.''. It has offset $0$ and begins
\[
1, 10, 94, 850, 8022, 75830, 717388, 6787364,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:06:43, revision 17)
...
the base is $10$ and the permitted difference is $d=7$, so the claim is that
\[
a_{10}(n)-a_{9}(n)\;=\;F_{7}(n)
\qquad\text{for every } n \text{ with } 7\lfloor n/2\rfloor+1\le10,
\]
...
at base $10$ reproduces all $19$ terms A125422 publishes, and at base $9$ all the
...
polynomial. In particular $a_{10}(n)-a_{9}(n)=F_{7}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $19$ terms A125422 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{7}(10,n)$ & $T_{7}(9,n)$ & $F_{7}(n)$ \\ \hline
1 & 10 & 9 & 1 \\
2 & 94 & 79 & 15 \\
3 & 850 & 681 & 169 \\
\end{tabular}
