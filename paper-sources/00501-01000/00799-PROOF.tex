...
\title{The cross-base identity for OEIS A125470, proved}
...
\begin{abstract}
OEIS A125470 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A125470 is ``Number of base 13 circular n-digit numbers with adjacent digits differing by 9 or less.''. It has offset $0$ and begins
\[
1, 13, 157, 1813, 21997, 267483, 3255907, 39636855,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:08:11, revision 17)
...
the base is $13$ and the permitted difference is $d=9$, so the claim is that
\[
a_{13}(n)-a_{12}(n)\;=\;F_{9}(n)
\qquad\text{for every } n \text{ with } 9\lfloor n/2\rfloor+1\le13,
\]
...
at base $13$ reproduces all $17$ terms A125470 publishes, and at base $12$ all the
...
polynomial. In particular $a_{13}(n)-a_{12}(n)=F_{9}(n)$ for $1\le n\le3$, which
...
First, the model reproduces all $17$ terms A125470 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{9}(13,n)$ & $T_{9}(12,n)$ & $F_{9}(n)$ \\ \hline
1 & 13 & 12 & 1 \\
2 & 157 & 138 & 19 \\
3 & 1813 & 1542 & 271 \\
\end{tabular}
