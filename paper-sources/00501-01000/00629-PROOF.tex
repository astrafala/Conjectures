...
\title{The cross-base identity for OEIS A124718, proved}
...
\begin{abstract}
OEIS A124718 carries an empirical identity relating its counts to those of the entry one base
...
OEIS A124718 is ``Number of base 25 circular n-digit numbers with adjacent digits differing by 1 or less.''. It has offset $0$ and begins
\[
1, 25, 73, 169, 453, 1205, 3301, 9125,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:14:24, revision 14)
...
the base is $25$ and the permitted difference is $d=1$, so the claim is that
\[
a_{25}(n)-a_{24}(n)\;=\;F_{1}(n)
\qquad\text{for every } n \text{ with } \lfloor n/2\rfloor+1\le25,
\]
...
at base $25$ reproduces all $25$ terms A124718 publishes, and at base $24$ all the
...
polynomial. In particular $a_{25}(n)-a_{24}(n)=F_{1}(n)$ for $1\le n\le40$, which
...
First, the model reproduces all $25$ terms A124718 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $T_{1}(25,n)$ & $T_{1}(24,n)$ & $F_{1}(n)$ \\ \hline
1 & 25 & 24 & 1 \\
2 & 73 & 70 & 3 \\
3 & 169 & 162 & 7 \\
4 & 453 & 434 & 19 \\
