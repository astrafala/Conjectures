...
\title{The cross-base identity for OEIS A126515, proved}
...
\begin{abstract}
OEIS A126515 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126515 is ``Number of base 20 n-digit numbers with adjacent digits differing by four or less.''. It has offset $0$ and begins
\[
1, 20, 160, 1320, 11000, 92172, 774696, 6522504,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:16:04, revision 11)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $20$ and the permitted difference is $d=4$, so the first clause
claims $a_{20}(n)-a_{19}(n)=9^{\,n-1}$ whenever $4(n-1)+1\le20$, that is for
$1\le n\le5$, and the second claims $a_{20}(n)-a_{19}(n)=9^{\,n-1}-2$ at the single
$n$ with $4(n-1)=20$.
...
arithmetic, $L_{4}$ at base $20$ reproduces all $19$ terms A126515 publishes.
...
First, the model reproduces all $19$ terms A126515 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{4}(20,n)$ & $L_{4}(19,n)$ & $9^{\,n-1}$ \\ \hline
1 & 20 & 19 & 1 \\
2 & 160 & 151 & 9 \\
3 & 1320 & 1239 & 81 \\
4 & 11000 & 10271 & 729 \\
...
Third, the second clause was evaluated at its own index. For this entry the second clause bites at $n=6$, where $b=20=4(n-1)$: there $L_{4}(20,6)-L_{4}(19,6)=59047$ and $9^{5}-2=59047$.

\section*{References}
\begin{enumerate}
\item The OEIS Foundation Inc., \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org/A126515}.
\end{enumerate}
