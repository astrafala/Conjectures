...
\title{A proof of the conjectured recurrence for OEIS A257388}
...
\begin{abstract}
OEIS A257388 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 - 4 x$, a polynomial of degree $1$, which proves
...
OEIS A257388 is ``Number of 4-Motzkin paths of length n with no level steps at odd level''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 17, 72, 306, 1304, 5573, 23888,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4 x + \sqrt{\left(4 x - 1\right) \left(4 x^{2} + 4 x - 1\right)} - 1}{2 x^{2} \left(4 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +8*(-n-1)*a(n-1) +4*(3*n+1)*a(n-2) +8*(2*n-3)*a(n-3)=0. - \emph{R. J. Mathar}, Sep 24 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2020-06-30, revision 25) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 8 \left(n + 1\right),\qquad p_{2}(n)=4 \left(3 n + 1\right),\qquad p_{3}(n)=8 \left(2 n - 3\right)
\]
...
\section{The computation for A257388}
...
\[
(n + 2)\,a(n) -  8 \left(n + 1\right)\,a(n-1) + 4 \left(3 n + 1\right)\,a(n-2) + 8 \left(2 n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A257388, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 - 4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A257388.
