...
\title{A proof of the conjectured recurrence for OEIS A228330}
...
\begin{abstract}
OEIS A228330 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A228330 is ``Let h(m) denote the sequence whose n-th term is Sum\_\{k=0..n\} (k+1)\^{}m*T(n,k)\^{}2, where T(n,k) is the Catalan triangle A039598. This is h(4)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 20, 362, 6504, 114686, 1992536, 34231540, 583027920,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} n*(2*n+1)*(3467*n-4029)*a(n) + 8*(-36721*n\^{}3 + 109040*n\^{}2 - 137926*n + 69822)*a(n-1) + 4*(4*n-9)*(45706*n-7907)*(4*n-7)*a(n-2) = 0. - \emph{R. J. Mathar}, Sep 08 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-16, revision 44) it is still
...
\begin{quote}\small
Recurrence: n*(2*n+1)*(15*n\^{}3 - 30*n\^{}2 + 16*n - 2)*a(n) = 2*(4*n-5)*(4*n-3)*(15*n\^{}3 + 15*n\^{}2 + n - 1)*a(n-1).
\end{quote}
...
\section{The computation for A228330}

Reading both relations as operators and dividing the conjectured one by the stated one
leaves remainder $0$, with quotient
\[
Q \;=\; \left(\frac{- 91412 n - 167010}{15 n^{3} + 60 n^{2} + 76 n + 30}\right) + \left(\frac{3467 n + 2905}{15 n^{3} + 60 n^{2} + 76 n + 30}\right)\,N .
\]
...
\[
n \left(2 n + 1\right) \left(3467 n - 4029\right)\,a(n) -  8 \left(36721 n^{3} - 109040 n^{2} + 137926 n - 69822\right)\,a(n-1) + 4 \left(4 n - 9\right) \left(4 n - 7\right) \left(45706 n - 7907\right)\,a(n-2) \;=\;0
\]
...
\url{https://oeis.org}, 2026. Sequence A228330.
