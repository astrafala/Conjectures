...
\title{A proof of the conjectured recurrence for OEIS A158495}
...
\begin{abstract}
OEIS A158495 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 x^{2} - x$, a polynomial of degree $2$, which proves
...
OEIS A158495 is ``Expansion of ((1-4*x) + sqrt(1-4*x))/(2*(1-2*x))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -1, -3, -8, -21, -56, -154, -440,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4 x - \sqrt{1 - 4 x} - 1}{2 \left(2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +6*(-n+1)*a(n-1) +4*(2*n-3)*a(n-2)=0. - \emph{R. J. Mathar}, Dec 03 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2023-01-10, revision 20) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 6 \left(n - 1\right),\qquad p_{2}(n)=4 \left(2 n - 3\right)
\]
...
\section{The computation for A158495}
...
\[
n\,a(n) -  6 \left(n - 1\right)\,a(n-1) + 4 \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A158495, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=4 x^{2} - x$ is an
...
\url{https://oeis.org}, 2026. Sequence A158495.
