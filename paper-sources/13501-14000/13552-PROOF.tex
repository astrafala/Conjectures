...
\title{A proof of the conjectured recurrence for OEIS A190724}
...
\begin{abstract}
OEIS A190724 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $10 x$, a polynomial of degree $1$, which proves
...
OEIS A190724 is ``Row sums of Riordan matrix A118384''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 20, 106, 576, 3174, 17648, 98746,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{7 x - \sqrt{x^{2} - 6 x + 1} - 1}{2 \left(6 x - 1\right) \sqrt{x^{2} - 6 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(2*n+3)*a(n) +2*(-12*n\^{}2-15*n+22)*a(n-1) +(74*n\^{}2+73*n-274)*a(n-2) -6*(2*n+5)*(n-2)*a(n-3)=0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
...
\[
p_{0}(n)=n \left(2 n + 3\right),\qquad p_{1}(n)=- 2 \left(12 n^{2} + 15 n - 22\right),\qquad p_{2}(n)=74 n^{2} + 73 n - 274,\qquad p_{3}(n)=- 6 \left(n - 2\right) \left(2 n + 5\right)
\]
...
\section{The computation for A190724}
...
\[
n \left(2 n + 3\right)\,a(n) -  2 \left(12 n^{2} + 15 n - 22\right)\,a(n-1) + (74 n^{2} + 73 n - 274)\,a(n-2) -  6 \left(n - 2\right) \left(2 n + 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A190724, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=10 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A190724.
