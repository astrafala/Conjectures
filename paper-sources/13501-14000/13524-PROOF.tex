...
\title{A proof of the conjectured recurrence for OEIS A081207}
...
\begin{abstract}
OEIS A081207 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A081207 is ``Main diagonal of number square A081206''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 3, 7, 16, 37, 89, 216,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x + 1}{\sqrt{x^{4} - 2 x^{3} - x^{2} - 2 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-2)*a(n) +(-2*n\^{}2+5*n-1)*a(n-1) +(-n\^{}2+3*n-4)*a(n-2) +(-2*n\^{}2+7*n-4)*a(n-3) +(n-1)*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 12 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2021-12-08, revision 19) the
...
\[
p_{0}(n)=n \left(n - 2\right),\qquad p_{1}(n)=- 2 n^{2} + 5 n - 1,\qquad p_{2}(n)=- n^{2} + 3 n - 4,\qquad p_{3}(n)=- 2 n^{2} + 7 n - 4,\qquad p_{4}(n)=\left(n - 3\right) \left(n - 1\right)
\]
...
\section{The computation for A081207}
...
\[
n \left(n - 2\right)\,a(n) + (- 2 n^{2} + 5 n - 1)\,a(n-1) + (- n^{2} + 3 n - 4)\,a(n-2) + (- 2 n^{2} + 7 n - 4)\,a(n-3) + \left(n - 3\right) \left(n - 1\right)\,a(n-4) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A081207, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A081207.
