...
\title{A proof of the conjectured recurrence for OEIS A026135}
...
\begin{abstract}
OEIS A026135 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 - 2 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A026135 is ``Number of (s(0),s(1),...,s(n)) such that every s(i) is a nonnegative integer, s(0) = 1, |s(1) - s(0)| = 1, |s(i) - s(i-1)| <= 1 for i >= 2. Also sum of numbers in row n+1 of the array T defined in A026120''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 14, 39, 110, 312, 890,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} + \sqrt{\frac{- x - 1}{3 x - 1}} \left(x - 1\right)^{2} - 1}{2 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +3*(-n-1)*a(n-1) +(-n-2)*a(n-2) +3*(n-3)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 23 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2026-02-12, revision 22) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 3 \left(n + 1\right),\qquad p_{2}(n)=- n - 2,\qquad p_{3}(n)=3 \left(n - 3\right)
\]
...
\section{The computation for A026135}
...
\[
(n + 2)\,a(n) -  3 \left(n + 1\right)\,a(n-1) + (- n - 2)\,a(n-2) + 3 \left(n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A026135, agreeing on all 13 terms compared.
...
vanish, for all 24 values of $n$ from $n=3$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=2 - 2 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A026135.
