...
\title{A proof of the conjectured recurrence for OEIS A176755}
...
\begin{abstract}
OEIS A176755 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $8 x^{3} - 13 x^{2} + 2 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A176755 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=3, k=0 and l=-2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 4, 15, 52, 208, 846, 3579,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 16 x^{3} + 4 x^{2} + 5 x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(1-3*n)*a(n-1) +(n+3)*a(n-2) +2*(10*n-33)*a(n-3) +16*(4-n)*a(n-4) =0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=n + 3,\qquad p_{3}(n)=2 \left(10 n - 33\right),\qquad p_{4}(n)=- 16 \left(n - 4\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + (n + 3)\,a(n-2) + 2 \left(10 n - 33\right)\,a(n-3) -  16 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A176755, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=8 x^{3} - 13 x^{2} + 2 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176755.
