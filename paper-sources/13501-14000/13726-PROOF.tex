...
\title{A proof of the conjectured recurrence for OEIS A177170}
...
\begin{abstract}
OEIS A177170 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $18 x^{3} - 33 x^{2} + 12 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A177170 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=8, k=0 and l=-2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 8, 14, 90, 402, 2438, 13826, 86014,\ \dots
\]
The entry posts no generating function of its own. Its NAME states the
relation \texttt{the entry states the generating function as G.f f: f(z)=(1-sqrt(1-4*z*(a(0)-z*a(0)\textasciicircum{}2+z*a(1)+(k+l)*z\textasciicircum{}2/(1-z)+k*z\textasciicircum{}2/(1-z)\textasciicircum{}2)))/(2*z) (k=0, l=-2); a(0) and a(1) are read off the entry's own terms and the parameters are the values given on that line} to another entry, and that entry's exponential generating
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 36 x^{3} + 24 x^{2} + 5 x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-3*n+1)*a(n-1) +(-19*n+43)*a(n-2) +6*(10*n-31)*a(n-3) +36*(-n+4)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-06-14, revision 5) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=43 - 19 n,\qquad p_{3}(n)=6 \left(10 n - 31\right),\qquad p_{4}(n)=- 36 \left(n - 4\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + (43 - 19 n)\,a(n-2) + 6 \left(10 n - 31\right)\,a(n-3) -  36 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A177170, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=18 x^{3} - 33 x^{2} + 12 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A177170.
