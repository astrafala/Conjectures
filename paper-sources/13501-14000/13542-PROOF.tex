...
\title{A proof of the conjectured recurrence for OEIS A121320}
...
\begin{abstract}
OEIS A121320 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{3}$, a polynomial of degree $3$, which proves
...
OEIS A121320 is ``Number of vertices in all ordered (plane) trees with n edges that are at distance two from all the leaves above them''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 2, 6, 18, 59, 203,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} \left(- \sqrt{1 - 4 x} - 1\right)}{2 \sqrt{1 - 4 x} \left(x^{2} + x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+2)*a(n) +(5*n-12)*a(n-1) +(-3*n+8)*a(n-2) +2*(-2*n+5)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 22 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2020-04-07, revision 20) the
...
\[
p_{0}(n)=2 - n,\qquad p_{1}(n)=5 n - 12,\qquad p_{2}(n)=8 - 3 n,\qquad p_{3}(n)=- 2 \left(2 n - 5\right)
\]
...
\section{The computation for A121320}
...
\[
(2 - n)\,a(n) + (5 n - 12)\,a(n-1) + (8 - 3 n)\,a(n-2) -  2 \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A121320, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A121320.
