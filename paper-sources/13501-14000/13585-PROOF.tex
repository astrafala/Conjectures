...
\title{A proof of the conjectured recurrence for OEIS A082134}
...
\begin{abstract}
OEIS A082134 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 100 x^{2} + 29 x$, a polynomial of degree $2$, which proves
...
OEIS A082134 is ``Expansion of e.g.f. x*exp(3*x)*cosh(x)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 6, 30, 144, 680, 3168, 14560,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x \left(10 x^{2} - 6 x + 1\right)}{\left(2 x - 1\right)^{2} \left(4 x - 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+28)*a(n) + (n-282)*a(n-1) + 2*(-17*n+423)*a(n-2) + 8*(7*n-94)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 29 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 49) the
...
\[
p_{0}(n)=n + 28,\qquad p_{1}(n)=n - 282,\qquad p_{2}(n)=- 2 \left(17 n - 423\right),\qquad p_{3}(n)=8 \left(7 n - 94\right)
\]
...
\section{The computation for A082134}
...
\[
(n + 28)\,a(n) + (n - 282)\,a(n-1) -  2 \left(17 n - 423\right)\,a(n-2) + 8 \left(7 n - 94\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A082134, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 100 x^{2} + 29 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A082134.
