...
\title{A proof of the conjectured recurrence for OEIS A176754}
...
\begin{abstract}
OEIS A176754 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $6 x^{3} - 9 x^{2} + 1$, a polynomial of degree $3$, which proves
...
OEIS A176754 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=2, k=0 and l=-2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 2, 6, 18, 62, 218, 790,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 12 x^{3} + 5 x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-3*n+1)*a(n-1) +5*(n-1)*a(n-2) +6*(2*n-7)*a(n-3) +12*(-n+4)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 18 2016
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=5 \left(n - 1\right),\qquad p_{3}(n)=6 \left(2 n - 7\right),\qquad p_{4}(n)=- 12 \left(n - 4\right)
\]
...
\section{The computation for A176754}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \frac{\left(2 x - 1\right) \left(6 x^{2} + 3 x - 1\right)}{x - 1} .
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + 5 \left(n - 1\right)\,a(n-2) + 6 \left(2 n - 7\right)\,a(n-3) -  12 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A176754, agreeing on all 13 terms compared.
...
vanish, for all 22 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=6 x^{3} - 9 x^{2} + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176754.
