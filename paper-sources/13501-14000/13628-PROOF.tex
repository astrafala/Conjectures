...
\title{A proof of the conjectured recurrence for OEIS A128057}
...
\begin{abstract}
OEIS A128057 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A128057 is ``Expansion of (1+x)/sqrt(1+4x\^{}2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, -2, -2, 6, 6, -20, -20,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x + 1}{\sqrt{4 x^{2} + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(5*n-9)*a(n) +4*a(n-1) +4*(5*n-4)*(n-2)*a(n-2)=0. - \emph{R. J. Mathar}, Dec 02 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2021-09-03, revision 11) the
...
\[
p_{0}(n)=n \left(5 n - 9\right),\qquad p_{1}(n)=4,\qquad p_{2}(n)=4 \left(n - 2\right) \left(5 n - 4\right)
\]
...
\section{The computation for A128057}
...
\[
n \left(5 n - 9\right)\,a(n) + 4\,a(n-1) + 4 \left(n - 2\right) \left(5 n - 4\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A128057, agreeing on all 13 terms compared.
...
vanish, for all 30 values of $n$ from $n=2$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A128057.
