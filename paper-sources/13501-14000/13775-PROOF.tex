...
\title{A proof of the conjectured recurrence for OEIS A025271}
...
\begin{abstract}
OEIS A025271 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A025271 is ``a(n) = a(1)*a(n-1) + a(2)*a(n-2) + ...+ a(n-1)*a(1) for n >= 5, with initial values 1,1,2,1''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 2, 1, 6, 18, 52, 165,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(n+1)*a(n-1) +(n+8)*a(n-2) +42*(-2*n+7)*a(n-3) +16*(n-6)*a(n-4) +80*(n-7)*a(n-5) +336*(n-8)*a(n-6)=0. - \emph{R. J. Mathar}, Nov 21 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 25) it is still
...
\begin{quote}\small
Recurrence: n*a(n) = 2*(2*n-3)*a(n-1) - 16*(n-6)*a(n-4). - \emph{Vaclav Kotesovec}, Jan 25 2015
\end{quote}
...
\section{The computation for A025271}
...
\[
n\,a(n) + (n + 1)\,a(n-1) + (n + 8)\,a(n-2) -  42 \left(2 n - 7\right)\,a(n-3) + 16 \left(n - 6\right)\,a(n-4) + 80 \left(n - 7\right)\,a(n-5) + 336 \left(n - 8\right)\,a(n-6) \;=\;0
\]
holds for every $n\ge7$.
\end{theorem}
...
\url{https://oeis.org}, 2026. Sequence A025271.
