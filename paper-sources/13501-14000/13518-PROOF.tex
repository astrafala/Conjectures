...
\title{A proof of the conjectured recurrence for OEIS A188464}
...
\begin{abstract}
OEIS A188464 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x + 4$, a polynomial of degree $1$, which proves
...
OEIS A188464 is ``Diagonal sums of triangle A188463''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 8, 22, 62, 178, 519, 1533,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{3} + x^{2} - 3 x + \left(x - 1\right) \sqrt{x^{4} + 2 x^{2} - 4 x + 1} + 1}{2 x^{4}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+4)*a(n)-(4*n+9)*a(n-1) +(2*n-1)*a(n-2) -a(n-3) +(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 17 2011
\end{quote}
...
\[
p_{0}(n)=n + 4,\qquad p_{1}(n)=- 4 n - 9,\qquad p_{2}(n)=2 n - 1,\qquad p_{3}(n)=-1,\qquad p_{4}(n)=n - 3
\]
...
\section{The computation for A188464}
...
\[
(n + 4)\,a(n) + (- 4 n - 9)\,a(n-1) + (2 n - 1)\,a(n-2) - 1\,a(n-3) + (n - 3)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A188464, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x + 4$ is an
...
\url{https://oeis.org}, 2026. Sequence A188464.
