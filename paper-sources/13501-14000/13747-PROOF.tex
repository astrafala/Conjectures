...
\title{A proof of the conjectured recurrence for OEIS A177185}
...
\begin{abstract}
OEIS A177185 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 22 x^{4} + 64 x^{3} - 60 x^{2} + 15 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A177185 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=10, k=-1 and l=-1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 10, 17, 130, 595, 4073, 24459, 167500,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 44 x^{4} + 80 x^{3} - 27 x^{2} - 6 x + 1}{x^{2} - 2 x + 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(-7*n+2)*a(n-1) +3*(-7*n+17)*a(n-2) +(107*n-334)*a(n-3) +4*(-31*n+125)*a(n-4) +44*(n-5)*a(n-5)=0. - \emph{R. J. Mathar}, Feb 21 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-02-21, revision 5) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=2 - 7 n,\qquad p_{2}(n)=- 3 \left(7 n - 17\right),\qquad p_{3}(n)=107 n - 334,\qquad p_{4}(n)=- 4 \left(31 n - 125\right),\qquad p_{5}(n)=44 \left(n - 5\right)
\]
...
\section{The computation for A177185}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \frac{44 x^{4} - 80 x^{3} + 27 x^{2} + 6 x - 1}{\left(x - 1\right)^{2}} .
\]
...
\[
B(x)\;=\;- 22 x^{4} + 64 x^{3} - 60 x^{2} + 15 x + 1 ,
\]
...
\[
(n + 1)\,a(n) + (2 - 7 n)\,a(n-1) -  3 \left(7 n - 17\right)\,a(n-2) + (107 n - 334)\,a(n-3) -  4 \left(31 n - 125\right)\,a(n-4) + 44 \left(n - 5\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A177185, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 22 x^{4} + 64 x^{3} - 60 x^{2} + 15 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A177185.
