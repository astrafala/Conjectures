...
\title{A proof of the conjectured closed form for OEIS A049486}
...
\begin{abstract}
OEIS A049486 carries a conjectured closed form for its terms. We prove it. The entry
...
explicit rational function; here it collapses to the polynomial $x^{2} + x$, of degree
...
OEIS A049486 is ``Maximum length of non-crossing path on n X n square lattice''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 4, 10, 21, 34, 53, 74, 101,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;- \frac{x \left(x^{5} - x^{4} + 3 x^{3} + 2 x^{2} + 2 x + 1\right)}{\left(x - 1\right)^{3} \left(x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
(9 + (-1)\^{}n - 8*n + 4*n\^{}2)/2 for n > 2.
\end{quote}
As of the ``Last modified'' line on the live entry (2022-10-18, revision 27) the
...
\[
f(n)\;=\;\frac{\left(-1\right)^{n}}{2} + 2 n^{2} - 4 n + \frac{9}{2} .
\]
...
\section{The computation for A049486}

By Lemma~\ref{lem:gf} the conjectured formula has generating function
\[
F(x)\;=\;\frac{x \left(- 5 x^{3} - x - 2\right)}{x^{4} - 2 x^{3} + 2 x - 1} ,
\]
...
For every $n>2$,
\[
a(n)\;=\;\frac{\left(-1\right)^{n}}{2} + 2 n^{2} - 4 n + \frac{9}{2} .
\]
\end{theorem}
...
section of A049486, agreeing on all 13 terms compared.
...
with the entry's own value for every $n>2$ in range, agreeing in all 39 cases
...
\url{https://oeis.org}, 2026. Sequence A049486.
