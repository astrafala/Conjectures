...
\title{A proof of the conjectured recurrence for OEIS A182881}
...
\begin{abstract}
OEIS A182881 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A182881 is ``Number of (1,1)-steps in all weighted lattice paths in L\_n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 0, 2, 6, 18, 56, 162,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x^{3}}{\left(\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)\right)^{\frac{3}{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+3)*a(n) +(2*n-5)*a(n-1) +(n-2)*a(n-2) +(2*n-3)*a(n-3) +(-n+1)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 22) the
...
\[
p_{0}(n)=3 - n,\qquad p_{1}(n)=2 n - 5,\qquad p_{2}(n)=n - 2,\qquad p_{3}(n)=2 n - 3,\qquad p_{4}(n)=1 - n
\]
...
\section{The computation for A182881}
...
\[
(3 - n)\,a(n) + (2 n - 5)\,a(n-1) + (n - 2)\,a(n-2) + (2 n - 3)\,a(n-3) + (1 - n)\,a(n-4) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A182881, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A182881.
