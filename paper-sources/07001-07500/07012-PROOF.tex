...
\title{The empirical recurrence for OEIS A302666, proved by transfer matrix}
...
\begin{abstract}
OEIS A302666 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the six horizontal, diagonal and antidiagonal neighbours, a number $equal to$ itself lying in $\{1,2,3,6\}$, and it carries an empirical
recurrence of order $32$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A302666 is ``Number of nX4 0..1 arrays with every element equal to 1, 2, 3 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
2, 45, 164, 934, 4267, 21949, 106793, 529984,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 11 2018 11:56:58, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) + 12\,a(n-2) - 47\,a(n-3) - 101\,a(n-4) + 45\,a(n-5) + 381\,a(n-6) + 1292\,a(n-7) + 42\,a(n-8) - 5366\,a(n-9) - 7123\,a(n-10) + 6047\,a(n-11) + 22989\,a(n-12) + 15537\,a(n-13) - 22434\,a(n-14) - 51326\,a(n-15) - 29224\,a(n-16) + 38509\,a(n-17) + 82591\,a(n-18) + 44309\,a(n-19) - 44268\,a(n-20) - 87403\,a(n-21) - 42575\,a(n-22) + 28053\,a(n-23) + 52051\,a(n-24) + 24821\,a(n-25) - 7290\,a(n-26) - 13614\,a(n-27) - 6288\,a(n-28) + 190\,a(n-29) + 1280\,a(n-30) + 312\,a(n-31) + 20\,a(n-32)
\end{equation}
for all $n>35$.
...
has score in $\{1,2,3,6\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,2,3,6\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,2,3,6\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,2,3,6\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,2,3,6\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-32}$ whenever $n-32\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-32$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-32}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $32$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=35+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302666.
