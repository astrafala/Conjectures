...
\title{The empirical recurrence for OEIS A300091, proved by transfer matrix}
...
\begin{abstract}
OEIS A300091 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{0,2,3,5,6,7,8\}$, and it carries an empirical
...
OEIS A300091 is ``Number of n X 3 0..1 arrays with every element equal to 0, 2, 3, 5, 6, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 7, 7, 19, 35, 95, 225, 575,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 20 2021 12:25:19, revision
8) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 4\,a(n-2) - 8\,a(n-3) - 5\,a(n-4) + 35\,a(n-5) - 4\,a(n-6) - 26\,a(n-7) - 10\,a(n-8) - 2\,a(n-9) - 8\,a(n-10) + 88\,a(n-11) + 78\,a(n-12) - 176\,a(n-13) - 62\,a(n-14) + 40\,a(n-15) + 24\,a(n-16) + 52\,a(n-17) - 8\,a(n-18) - 8\,a(n-19)
\end{equation}
for all $n>20$.
...
has score in $\{0,2,3,5,6,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,2,3,5,6,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,2,3,5,6,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,2,3,5,6,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,2,3,5,6,7,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A300091.
