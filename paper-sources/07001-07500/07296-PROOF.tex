...
\title{The empirical recurrence for OEIS A250430, proved by transfer matrix}
...
\begin{abstract}
OEIS A250430 counts arrays over $\{0,1\}$ in which a statistic taken over short runs
...
window of consecutive rows, so the count is a walk count on $S=400$ states, and the
...
OEIS A250430 is ``Number of (n+1)X(6+1) 0..1 arrays with nondecreasing sum of every two consecutive values in every row and column.''. It has offset $1$ and begins
\[
400, 3000, 22500, 105000, 490000, 1715000, 6002500, 17287200,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 12:24:13, revision 10) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=400,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250430.
