...
\title{The empirical recurrence for OEIS A316285, proved by transfer matrix}
...
\begin{abstract}
OEIS A316285 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,3,6,8\}$, and it carries an empirical
...
OEIS A316285 is ``Number of nX4 0..1 arrays with every element unequal to 0, 1, 2, 3, 6 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
8, 49, 125, 354, 1372, 3933, 12454, 42946,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 28 2018 15:31:05, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;7\,a(n-1) - 15\,a(n-2) + 22\,a(n-3) - 91\,a(n-4) + 194\,a(n-5) - 128\,a(n-6) + 198\,a(n-7) - 559\,a(n-8) + 288\,a(n-9) + 210\,a(n-10) + 102\,a(n-11) - 58\,a(n-12) - 534\,a(n-13) + 420\,a(n-14) - 139\,a(n-15) + 238\,a(n-16) - 200\,a(n-17) + 56\,a(n-18) - 24\,a(n-19) + 18\,a(n-20) - 4\,a(n-21)
\end{equation}
for all $n>25$.
...
has score in $\{0,1,2,3,6,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,6,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,6,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,6,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,6,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A316285.
