...
\title{The empirical recurrence for OEIS A298254, proved by transfer matrix}
...
\begin{abstract}
OEIS A298254 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{2,3,5,8\}$, and it carries an empirical
...
OEIS A298254 is ``Number of nX3 0..1 arrays with every element equal to 2, 3, 5 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 3, 1, 4, 3, 7, 14, 35,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 15 2018 14:56:40, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 3\,a(n-2) - 7\,a(n-3) - 14\,a(n-4) + 2\,a(n-5) + 34\,a(n-6) + 16\,a(n-7) - 20\,a(n-8) - 48\,a(n-9) - 50\,a(n-10) + 36\,a(n-11) + 89\,a(n-12) + 39\,a(n-13) + 3\,a(n-14) - 9\,a(n-15) - 14\,a(n-16) + 2\,a(n-18)
\end{equation}
for all $n>19$.
...
has score in $\{2,3,5,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{2,3,5,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{2,3,5,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{2,3,5,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{2,3,5,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298254.
