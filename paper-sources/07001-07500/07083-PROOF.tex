...
\title{The empirical product formula for OEIS A140919, proved}
...
\begin{abstract}
OEIS A140919 counts $4\times9$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $4\times9\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $36$. The entry's own expression, cleared of its denominator,
...
OEIS A140919 is ``Number of 4 X 9 matrices with elements in 0..n with each row and each column in nondecreasing order. 4,9,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 715, 143143, 13026013, 677352676, 23029990984,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2025 11:09:18, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $4\times9$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$4\times9$ rectangle --- a plane partition in a $4\times9\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{4}\prod_{j=1}^{9}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $36$ with rational coefficients.
\end{theorem}
...
$4\cdot9=36$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,9,4)$ and claims
\[
a(n)\;=\;\prod_{i=0}^{4-1}\binom{s}{n+i}\,\frac{i!}{(s-i)^{4-1-i}}\;=\;
\frac{N(n)}{D(n)} .
\]

\begin{lemma}\label{lem:deg}
$N$ and $D$ are polynomials in $n$ of degrees $42$ and $6$ respectively.
\end{lemma}

\begin{proof}
With $s=n+9+4-1$ one has $s-(n+i)=9+4-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{9+4-1-i}
=\frac{(s-9-4+2+i)(s-9-4+3+i)\cdots s}{(9+4-1-i)!},
\]
a polynomial in $n$ of degree $9+4-1-i$. Multiplying over $i=0,\dots,4-1$ gives
$\deg N=\sum_{i=0}^{4-1}(9+4-1-i)=42$. The denominator is
$\prod_{i=0}^{4-1}(s-i)^{4-1-i}$, of degree
$\sum_{i=0}^{4-1}(4-1-i)=6$, and it does not vanish for $n\ge0$.
\end{proof}
...
of degree at most $36+6=42$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,42$, that is at $43$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $42$ has at most $42$ roots, so it is identically zero. Since $D(n)\neq0$
...
First, Theorem~\ref{thm:poly} reproduces all $16$ terms the entry publishes,
exactly, in integer arithmetic.

Second, the matrices themselves were counted for the smallest alphabets, directly from the
definition: the nondecreasing rows were listed and the chains of $4$ of them increasing
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,42$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140919.
