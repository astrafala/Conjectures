...
\title{The empirical recurrence for OEIS A234653, proved by transfer matrix}
...
\begin{abstract}
OEIS A234653 counts $4$-column arrays over $\{0,\dots,5\}$ in which every
$2\times2$ subblock has its diagonal sum differing from its antidiagonal sum by
exactly $4$, and it
carries an empirical recurrence of order $21$ contributed by R.~H.~Hardin. It is true,
...
the residual is $\mathbf{1}^{\!\top}M^{\,n-21}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234653 is ``Number of (n+1) X (3+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
3264, 9616, 28496, 94432, 315024, 1148176, 4196144,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 01:49:46, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;8\,a(n-1) + 83\,a(n-2) - 804\,a(n-3) - 2479\,a(n-4) + 34112\,a(n-5) + 22457\,a(n-6) - 798036\,a(n-7) + 453896\,a(n-8) + 11262032\,a(n-9) - 15161188\,a(n-10) - 98135856\,a(n-11) + 191644944\,a(n-12) + 513355968\,a(n-13) - 1327868352\,a(n-14) - 1430555904\,a(n-15) + 5245827840\,a(n-16) + 1148359680\,a(n-17) - 11006668800\,a(n-18) + 3284582400\,a(n-19) + 9455616000\,a(n-20) - 5971968000\,a(n-21)
\end{equation}
for all $n>21$, which is every $n$ at which a recurrence of order $21$ can be
stated.
...
For $n\ge21$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-21}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-21$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-21}\bigl(M^{21}-\sum_i c_i M^{21-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-21$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $21$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=21-21+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>21$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234653.
