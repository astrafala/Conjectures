...
\title{The empirical recurrence for OEIS A234453, proved by transfer matrix}
...
\begin{abstract}
OEIS A234453 counts $3$-column arrays over $\{0,\dots,5\}$ in which every
$2\times2$ subblock has its diagonal sum differing from its antidiagonal sum by
exactly $1$, and it
carries an empirical recurrence of order $26$ contributed by R.~H.~Hardin. It is true,
...
the residual is $\mathbf{1}^{\!\top}M^{\,n-26}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234453 is ``Number of (n+1) X (2+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
2400, 31112, 427956, 6049248, 86545688, 1246417268, 17999990956,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 18:38:42, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;24\,a(n-1) + 202\,a(n-2) - 8375\,a(n-3) + 18366\,a(n-4) + 832456\,a(n-5) - 4372543\,a(n-6) - 32845569\,a(n-7) + 252119346\,a(n-8) + 475222218\,a(n-9) - 6386106484\,a(n-10) + 1228195800\,a(n-11) + 78155524488\,a(n-12) - 93005666672\,a(n-13) - 459032184864\,a(n-14) + 814396694400\,a(n-15) + 1253941668864\,a(n-16) - 2968493257344\,a(n-17) - 1338994735872\,a(n-18) + 5126543725056\,a(n-19) - 147546633728\,a(n-20) - 4053307347968\,a(n-21) + 1013644265472\,a(n-22) + 1200673271808\,a(n-23) - 333045153792\,a(n-24) - 113446158336\,a(n-25) + 22613852160\,a(n-26)
\end{equation}
for all $n>25$, which is every $n$ at which a recurrence of order $26$ can be
stated.
...
For $n\ge26$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-26}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-26$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-26}\bigl(M^{26}-\sum_i c_i M^{26-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-26$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $26$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=25-26+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>25$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234453.
