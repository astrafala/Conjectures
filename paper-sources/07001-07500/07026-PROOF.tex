...
\title{The empirical recurrence for OEIS A235084, proved by transfer matrix}
...
\begin{abstract}
OEIS A235084 counts $6$-column arrays over $\{0,\dots,4\}$ in which every
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=5120$ vertices, which
...
OEIS A235084 is ``Number of (n+1) X (5+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
1078, 2002, 3610, 7046, 13446, 27218, 53882,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 23:23:55, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) + 4\,a(n-2) - 87\,a(n-3) + 90\,a(n-4) + 450\,a(n-5) - 830\,a(n-6) - 936\,a(n-7) + 2737\,a(n-8) + 408\,a(n-9) - 4078\,a(n-10) + 891\,a(n-11) + 2766\,a(n-12) - 816\,a(n-13) - 904\,a(n-14) + 180\,a(n-15) + 120\,a(n-16)
\end{equation}
...
Let $V$ consist of those $r\in\{0,\dots,4\}^{6}$ with $r_j\ne r_{j+1}$ for every
$j$ --- the rows in which no two horizontally adjacent entries are equal, and put $S=|V|=5120$. For $r,s\in V$ declare $r\to s$ an edge
when $\bigl|r_j+s_{j+1}-r_{j+1}-s_j\bigr|=4$ for every $j$ with
$1\le j\le6-1$, and $r_j\ne s_j$ for every $j$. Let $M\in\{0,1\}^{S\times S}$ be the adjacency matrix and $\mathbf{1}$
the all-ones vector.
...
\begin{proof}
An array with $n+1$ rows and $6$ columns is a sequence $r^{(0)},\dots,r^{(n)}$ of
...
the $25$ published indices; the values agree exactly. That is what ties the digraph to
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=5120$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A235084.
