...
\title{Eventual periodicity modulo $m$ with period dividing $\varphi(m)$:\\a proof of Bala's conjecture on OEIS A167137}
...
\[
a(0),\dots,a(7)\;=\;1, 1, 5, 31, 257, 2551, 30065, 407191,\ \dots
\]
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} Let k be a positive integer. The sequence obtained by reducing a(n) modulo k is eventually periodic with the period dividing phi(k) = A000010(k). For example, modulo 16 we obtain the sequence [1, 1, 5, 15, 1, 7, 1, 7, 1, 7, ...], with an apparent period of 2 beginning at a(4).
\end{quote}
As of the ``Last modified'' line on the live entry (2024-07-27, revision 31) the
...
\section{Application to A167137}\label{sec:apply}

\begin{corollary}\label{cor:this}
The conjecture quoted in Section~1 is true: for every $m\ge1$ the sequence
A167137$(n) \bmod m$ is eventually periodic with period dividing $\varphi(m)$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:main} it suffices to exhibit the entry's exponential generating
function in the form $G(e^{x}-1)$ with $G$ integral. The entry states ``E.g.f.: $P(\exp(x)-1)$ where $P(x)$ is the g.f.\ of the partition numbers'', and equivalently ``E.g.f.: $\exp(\sum_{n\ge1}\sigma(n)[\exp(x)-1]^{n}/n)$''. Both are $G(y)$ with $G$ the partition generating function. Therefore
\[
\sum_{n\ge0}a(n)\frac{x^{n}}{n!}=G(y),\qquad G(t)=\exp\!\Big(\sum_{n\ge1}\frac{\sigma(n)}{n}y^{n}\Big)=\prod_{j\ge1}\frac{1}{1-y^{j}} ,
\]
whose coefficients are the partition numbers, all integers. So $G\in\Z[[t]]$ and
Theorem~\ref{thm:main} applies.
\end{proof}

Concretely, the first coefficients of $G$ are
\[
c_{0},c_{1},c_{2},\dots \;=\; 1, 1, 2, 3, 5, 7, 11, \dots ,
\]
...
$22$ terms published on A167137 returns integers at every $k$, and the values
...
exact period  & $4$ & $6$ & $2$ & $2$ & $10$ & $2$ & $12$ & $2$ & $20$ & $6$\\
\bottomrule
\end{tabular}
\end{center}
...
\url{https://oeis.org}, 2026. Sequence A167137; conjecture of P.\ Bala.
