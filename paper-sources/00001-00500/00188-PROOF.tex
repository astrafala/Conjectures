...
\title{The empirical recurrence for OEIS A211323, proved by a one-dimensional reduction}
...
\begin{abstract}
OEIS A211323 counts symmetric $(n+1)\times(n+1)$ matrices over
$\{-3,\dots,3\}$ in which every $2\times2$ subblock sums to zero and has three distinct
...
on $S=14$ states, and the empirical recurrence of order $4$ contributed by R.~H.~Hardin
follows.
\end{abstract}
...
OEIS A211323 is ``Number of (n+1) X (n+1) -3..3 symmetric matrices with every 2 X 2 subblock having sum zero and three distinct values.''. It has offset $1$ and begins
\[
14, 24, 42, 76, 140, 262, 496, 948,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 4*a(n-2) - a(n-3) + 2*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 16 2018 05:22:23, revision 7) this is still
...
$S=14$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad \tau=\mathbf{1},\qquad S=14.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211323.
