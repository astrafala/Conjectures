...
\title{The empirical recurrence for OEIS A283606, proved from an exact model}
...
\begin{abstract}
OEIS A283606 carries an empirical recurrence of order $3$. It is true, and it is
...
which $a$ provably satisfies a MONIC linear recurrence of order $S=21$, derived below and not
...
OEIS A283606 is ``Decimal representation of the x-axis, from the left edge to the origin, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 670", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 3, 7, 15, 29, 63, 119, 255,\ \dots
\]
...
pre-period to raise the numerator's degree gives a monic annihilator of order $S=21$. The
...
vanish from $n=17+1$ onwards, and the run exceeds $S+3$, which by
Lemma~\ref{lem:crit} settles every larger index at once. The bound is exact: at $n=17$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A283606.
