...
\title{The empirical recurrence for OEIS A211114, proved by a one-dimensional reduction}
...
\begin{abstract}
OEIS A211114 counts symmetric $(n+1)\times(n+1)$ matrices over
$\{-2,\dots,2\}$ in which every $2\times2$ subblock sums to zero and has one or three distinct
...
on $S=12$ states, and the empirical recurrence of order $5$ contributed by R.~H.~Hardin
follows.
\end{abstract}
...
OEIS A211114 is ``Number of (n+1) X (n+1) -2..2 symmetric matrices with every 2 X 2 subblock having sum zero and one or three distinct values.''. It has offset $1$ and begins
\[
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 15 2018 12:03:54, revision 10) this is still
...
$S=12$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad \tau=\mathbf{1},\qquad S=12.
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 3\,a(n-2) - 6\,a(n-3) - 2\,a(n-4) + 4\,a(n-5)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211114.
