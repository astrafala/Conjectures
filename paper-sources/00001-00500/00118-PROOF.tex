...
\title{Eventual periodicity modulo $m$ with period dividing $\varphi(m)$:\\a proof of Bala's conjecture on OEIS A316143}
...
OEIS A316143 is ``Expansion of e.g.f. Product\_{k>=1} 1 / (1 - (exp(x)-1)\^{}k)\^{}2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 12, 92, 912, 10772, 148512, 2328692,\ \dots
\]
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} Let k be a positive integer. The sequence obtained by reducing a(n) modulo k is eventually periodic with the period dividing phi(k) = A000010(k). For example, modulo 7 we obtain the sequence [1, 2, 5, 1, 2, 6, 0, 2, 5, 1, 2, 6, 0, 2, 5, 1, 2, 6, 0, ...], with a preperiod of length 1 and an apparent period thereafter of 6 = phi(7). - \emph{Peter Bala}, Mar 03 2023
\end{quote}
As of the ``Last modified'' line on the live entry (2024-02-28, revision 20) the
...
\section{Application to A316143}\label{sec:apply}

\begin{corollary}\label{cor:this}
The conjecture quoted in Section~1 is true: for every $m\ge1$ the sequence
A316143$(n) \bmod m$ is eventually periodic with period dividing $\varphi(m)$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:main} it suffices to exhibit the entry's exponential generating
function in the form $G(e^{x}-1)$ with $G$ integral. The entry is ``Expansion of e.g.f.\ $\prod_{k\ge1}1/(1-(\exp(x)-1)^{k})^{2}$''. Therefore
\[
\sum_{n\ge0}a(n)\frac{x^{n}}{n!}=G(y),\qquad G(t)=\prod_{j\ge1}\frac{1}{\bigl(1-y^{j}\bigr)^{2}} ,
\]
...
\[
c_{0},c_{1},c_{2},\dots \;=\; 1, 2, 5, 10, 20, 36, 65, \dots ,
\]
...
$20$ terms published on A316143 returns integers at every $k$, and the values
...
exact period  & $1$ & $6$ & $2$ & $6$ & $10$ & $2$ & $12$ & $2$ & $4$ & $18$\\
\bottomrule
\end{tabular}
\end{center}
...
\url{https://oeis.org}, 2026. Sequence A316143; conjecture of P.\ Bala.
