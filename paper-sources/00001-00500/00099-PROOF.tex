...
\title{Eventual periodicity modulo $m$ with period dividing $\varphi(m)$:\\a proof of Bala's conjecture on OEIS A301921}
...
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 19, 159, 1651, 21303, 324619,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (2025-08-20, revision 18) the
...
\section{Application to A301921}\label{sec:apply}

\begin{corollary}\label{cor:this}
The conjecture quoted in Section~1 is true: for every $m\ge1$ the sequence
A301921$(n) \bmod m$ is eventually periodic with period dividing $\varphi(m)$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:main} it suffices to exhibit the entry's exponential generating
function in the form $G(e^{x}-1)$ with $G$ integral. The entry is defined by ``e.g.f.\ $1/(1-(\exp(x)-1)/(1-(\exp(x)-1)^{2}/(1-\cdots)))$'', a continued fraction whose every appearance of $x$ is inside $\exp(x)-1$. Therefore
\[
\sum_{n\ge0}a(n)\frac{x^{n}}{n!}=G(y),\qquad G(t)=\cfrac{1}{1-\cfrac{y}{1-\cfrac{y^{2}}{1-\cdots}}} ,
\]
whose coefficients are a continued fraction with integer entries, each convergent integral, all integers. So $G\in\Z[[t]]$ and
Theorem~\ref{thm:main} applies.
\end{proof}

Concretely, the first coefficients of $G$ are
\[
c_{0},c_{1},c_{2},\dots \;=\; 1, 1, 1, 2, 3, 5, 9, \dots ,
\]
...
$21$ terms published on A301921 returns integers at every $k$, and the values
...
\url{https://oeis.org}, 2026. Sequence A301921; conjecture of P.\ Bala.
