...
\title{The reduction of OEIS A396099 modulo $2$}
...
\begin{abstract}
OEIS A396099 is defined by a functional equation for its ordinary generating function and
...
them modulo $2$ as well; and modulo $2$ the series $x/(1-x)$ satisfies the equation
...
OEIS A396099 is ``G.f. satisfies A(x) = x + A\^{}2(x)*A\^{}3(x).''. It has offset $1$ and begins
\[
1, 1, 5, 39, 383, 4357,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 04 2026 11:37:17, revision 17) it is
still recorded as a conjecture, and nothing on the entry records
it as settled either way. The entry carries two further conjectures of the same contributor, on the residues modulo $4$; those are stronger than what is proved here and are not claimed. Section~4 says exactly where the argument stops short of them.
...
Lemma~\ref{lem:jaN}. On the left the coefficient is $1$. On the right, $x$ contributes nothing and the product $A^{\,2}A^{\,3}$ contributes nothing either: every monomial of $\bigl[x^{N}\bigr]\bigl(A^{\,2}A^{\,3}\bigr)$ is a product of one coefficient of $A^{\,2}$ and one of $A^{\,3}$ with indices $i,j\ge1$ and $i+j=N$, so both indices are at most $N-1$. Hence the net coefficient of $a(N)$ is $1$.
...
Over $\mathbb{Z}/2\mathbb{Z}$, $f$ satisfies $f=x+f^{\,2}f^{\,3}$.
\end{lemma}

\begin{proof}
By Lemma~\ref{lem:iter}, $f^{\,2}f^{\,3}
=\dfrac{x^{2}}{(1-2x)\bigl(1-3x\bigr)}$, while
$f-x=\dfrac{x}{1-x}-x=\dfrac{x^{2}}{1-x}$. So the claim is that
$(1-2x)(1-3x)$ and $1-x$ agree in $(\mathbb{Z}/2\mathbb{Z})[x]$. Expanding,
\[
(1-2x)\bigl(1-3x\bigr)=1-5x+6x^{2} ,
\]
and modulo $2$ we have $5\equiv1$ and $6\equiv0$. Hence both sides equal
...
$a(n)\equiv1\pmod{2}$ for every $n\ge1$, and more generally
\[
\bigl[x^{n}\bigr]A^{\,j}(x)\;\equiv\;j^{\,n-1}\pmod{2}
\]
for every $n\ge1$ and every integer $j$. This is the conjecture of Section~1.
\end{theorem}
...
Reduction modulo $2$ carries composition of power series with unit linear coefficient to
...
\begin{remark}
The proof used only that $(1-2x)(1-3x)\equiv1-x$ modulo $2$, that is, that
$2$ divides both $2\cdot2$ and $2\cdot3$. The largest modulus with that property
is $\gcd\bigl(2p,\,p(p+1)\bigr)$, which is $2p$ for odd $p$ and $p$ for even $p$; here
$p=2$ gives $2$. This is exactly why the statement is made modulo $2$ and not modulo
a larger number.
\end{remark}
...
continued to $n=24$ and every term reduced modulo $2$; all residues are
$1$.

Third, the composition of the proof was carried out independently as a formal power series
over $\mathbb{Z}/2\mathbb{Z}$ to order $30$, starting from the coefficients of $x/(1-x)$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A396099.
