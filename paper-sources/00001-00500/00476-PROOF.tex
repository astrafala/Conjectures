...
\title{The empirical recurrence for OEIS A184259, proved from an exact model}
...
\begin{abstract}
OEIS A184259 carries an empirical recurrence of order $74$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=273$, derived below and not
...
OEIS A184259 is ``Number of strings of numbers x(i=1..4) in 0..n with sum i\^{}3*x(i) equal to 64*n.''. It has offset $1$ and begins
\[
1, 2, 2, 3, 4, 5, 8, 13,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n)=2*a(n-1)-a(n-2)+a(n-8)-2*a(n-9)+a(n-10)+a(n-64)-2*a(n-65)+a(n-66)-a(n-72)+2*a(n-73)-a(n-74)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 02:29:21, revision 9)
...
\]
annihilates $a$, and $S=\deg A=273$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=273$, and
...
$q(z)=z^{74}-\sum_ic_iz^{74-i}$ the characteristic polynomial of the conjectured
...
vanish from $n=0+1$ onwards, and the run exceeds $S+74$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184259.
