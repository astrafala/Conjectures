...
\title{The empirical recurrence for OEIS A283403, proved from an exact model}
...
\begin{abstract}
OEIS A283403 carries an empirical recurrence of order $3$. It is true, and it is
...
which $a$ provably satisfies a MONIC linear recurrence of order $S=30$, derived below and not
...
OEIS A283403 is ``Decimal representation of the x-axis, from the origin to the right edge, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 633", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 0, 6, 8, 14, 24, 62, 56,\ \dots
\]
...
pre-period to raise the numerator's degree gives a monic annihilator of order $S=30$. The
...
vanish from $n=26+1$ onwards, and the run exceeds $S+3$, which by
Lemma~\ref{lem:crit} settles every larger index at once. The bound is exact: at $n=26$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A283403.
