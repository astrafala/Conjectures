...
\title{The empirical recurrence for OEIS A154145, proved from an exact model}
...
\begin{abstract}
OEIS A154145 carries an empirical recurrence of order $9$. It is true, and it is
...
\begin{quote}
Conjectures: (Start) a(n)= +a(n-1) +6*a(n-4) -6*a(n-5) -a(n-8) +a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 23 2024 14:53:42, revision 14)
this is still recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a Pell equation}

$k(k+1)/2 + 15$ is a perfect square exactly when $8$ times it is, and completing the square
turns that into
\[
(2k+1)^2-2(2m)^2\;=\;1-8\cdot15\;=\;-119,
\]
so writing $X=2k+1$ and $Y=2m$ the admissible $k$ are exactly the solutions of $X^2-2Y^2=-119$
...
finds them -- there are $4$ here -- and the search is checked rather than bounded by a quoted
theorem: it is run to a limit and then re-run to four times that limit, and the engine refuses
unless the two agree.

\section{The bound}

On the region $X\ge1$, $Y\ge0$ one has $Y=\sqrt{(X^2--119)/2}$, an increasing function of $X$,
so $T(X)=3X+4Y$ is strictly increasing: $T$ PRESERVES THE ORDER of solutions. Hence if the
sorted list of solutions has its $(n+4)$-th entry equal to $T$ of its $n$-th for $4$
consecutive $n$, it does so for every later $n$ -- the $4$ orbits interleave in a fixed cyclic
...
interleaving,
\[
a(n+2\cdot4)\;=\;6\,a(n+4)-a(n)+2 \qquad (n\ge n_0),
\]
so $(z-1)(z^{2\cdot4}-6z^{4}+1)$ annihilates $a$, monic of order $S=10$ once the
transient is allowed for. Nothing in that bound is estimated: $4$ is the number of fundamental
...
vanish from $n=1+1$ onwards, and the run exceeds $S+9$, which by
