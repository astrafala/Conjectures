...
\title{Eventual periodicity modulo $m$ with period dividing $\varphi(m)$:\\a proof of Bala's conjecture on OEIS A259533}
...
\[
a(0),\dots,a(7)\;=\;1, 4, 18, 94, 582, 4294, 37398, 378214,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 61) the
...
\section{Application to A259533}\label{sec:apply}

\begin{corollary}\label{cor:this}
The conjecture quoted in Section~1 is true: for every $m\ge1$ the sequence
A259533$(n) \bmod m$ is eventually periodic with period dividing $\varphi(m)$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:main} it suffices to exhibit the entry's exponential generating
function in the form $G(e^{x}-1)$ with $G$ integral. The posted e.g.f.\ is $e^{3x}/(2-e^{x})$; with $e^{x}=1+y$ this is $(1+y)^{3}/(1-y)$. Therefore
\[
\sum_{n\ge0}a(n)\frac{x^{n}}{n!}=G(y),\qquad G(t)=\frac{(1+y)^{3}}{1-y} ,
\]
...
\[
c_{0},c_{1},c_{2},\dots \;=\; 1, 4, 7, 8, 8, 8, 8, \dots ,
\]
...
$22$ terms published on A259533 returns integers at every $k$, and the values
...
exact period  & $4$ & $6$ & $1$ & $6$ & $10$ & $2$ & $12$ & $1$ & $20$ & $18$\\
\bottomrule
\end{tabular}
\end{center}
...
\url{https://oeis.org}, 2026. Sequence A259533; conjecture of P.\ Bala.
