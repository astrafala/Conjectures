...
\title{The empirical recurrence for OEIS A211115, proved by a one-dimensional reduction}
...
\begin{abstract}
OEIS A211115 counts symmetric $(n+1)\times(n+1)$ matrices over
$\{-2,\dots,2\}$ in which every $2\times2$ subblock sums to zero and has two or three distinct
...
on $S=25$ states, and the empirical recurrence of order $9$ contributed by R.~H.~Hardin
follows.
\end{abstract}
...
OEIS A211115 is ``Number of (n+1) X (n+1) -2..2 symmetric matrices with every 2 X 2 subblock having sum zero and two or three distinct values.''. It has offset $1$ and begins
\[
12, 26, 54, 106, 208, 398, 766, 1458,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-1) - 4*a(n-2) - 16*a(n-3) + 27*a(n-4) + 8*a(n-5) - 35*a(n-6) + 10*a(n-7) + 10*a(n-8) - 4*a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 15 2018 12:04:01, revision 8) this is still
...
\qquad u_0,\dots,u_{N-1}\in\{-2,\dots,2\}\ \text{all of the same parity},
\]
...
the same parity; and $|x(i,j)|=|u_i+u_j|/2\le2$ for all $i,j$ is equivalent to $|u_i|\le2$
...
$\{-2,\dots,2\}$ with all entries of one parity. What remains is to say, in terms of $u$,
...
the compatibility graph on the $\le 25$ pairs.
\end{corollary}
...
$S=25$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad \tau=\mathbf{1},\qquad S=25.
\]
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-1) - 4\,a(n-2) - 16\,a(n-3) + 27\,a(n-4) + 8\,a(n-5) - 35\,a(n-6) + 10\,a(n-7) + 10\,a(n-8) - 4\,a(n-9)
\]
for every $n>8$.
\end{theorem}
...
symmetric matrices were written out entry by entry over $\{-2,\dots,2\}$ and each
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211115.
