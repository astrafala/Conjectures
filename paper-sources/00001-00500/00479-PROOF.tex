...
\title{The conjectured generating function of OEIS A063219,\\ \large from the recurrence the entry
states}
...
\begin{abstract}
OEIS A063219 states a linear recurrence as a fact and carries a rational generating function as an
...
The first terms are $4, 13, 21, 31, 39, 47, 57, 65,\dots$, with offset $1$. The entry states, as a fact,

\begin{quote}\raggedright a(n) = a(n-1) + a(n-3) - a(n-4) for n>5.\end{quote}

\noindent and separately records the conjecture

\begin{quote}\raggedright Empirical g.f.: -x*(x\^{}4 - 6*x\^{}3 - 8*x\^{}2 - 9*x - 4) / ((x-1)\^{}2*(x\^{}2 + x + 1)).\end{quote}
...
Write $b_k=a(1+k)$ and
\[
  D(x)=1-\sum_{i} c_i x^{i}=x^{4} - x^{3} - x + 1 ,
\]
the polynomial of the stated recurrence, of degree $4$. Let $B(x)=\sum_{k\ge0}b_kx^k$.

The recurrence says $b_k=\sum_i c_ib_{k-i}$ for every $k\ge 5$. The coefficient of $x^k$ in
$D(x)B(x)$ is exactly $b_k-\sum_i c_ib_{k-i}$, so it vanishes for every $k\ge 5$ and
\[
  D(x)B(x)=P(x)=\sum_{k<5}\Bigl(b_k-\sum_i c_ib_{k-i}\Bigr)x^k
         =- x^{5} + 6 x^{4} + 8 x^{3} + 9 x^{2} + 4 x ,
\]
a polynomial whose every coefficient is fixed by finitely many published terms. Therefore
\[
  B(x)=\frac{P(x)}{D(x)}=- \frac{x \left(x^{4} - 6 x^{3} - 8 x^{2} - 9 x - 4\right)}{\left(x - 1\right)^{2} \left(x^{2} + x + 1\right)} ,
\]
which is the conjectured expression.

\begin{theorem}
The conjectured generating function of OEIS A063219 is correct, given the recurrence the entry
states.
\end{theorem}
...
rational functions, not a comparison of the $50$ terms the entry publishes -- those
enter only in fixing the $5$ coefficients of $P$.
\end{proof}
