...
\title{The empirical recurrence for OEIS A266792, proved from an exact model}
...
\begin{abstract}
OEIS A266792 carries an empirical recurrence of order $4$ contributed by Colin Barker. It is true, and it is
...
\begin{quote}
Conjectures from \_Colin Barker\_, Jan 04 2016 and Apr 18 2019: (Start) a(n) = 2*a(n-2)-a(n-4) for n>8.
\end{quote}
...
For this entry the automaton is rule 61, the period is $p=2$ and the pre-period is $n_0=5$.
The certificate is
\begin{gather*}
r=0:\quad w(n+p) = \texttt{0} \cdot w(n) \cdot \texttt{000} \\
r=1:\quad w(n+p) = \texttt{1} \cdot w(n) \cdot \texttt{111}
\end{gather*}
