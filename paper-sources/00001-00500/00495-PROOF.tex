...
\title{The empirical recurrence for OEIS A187161, proved from an exact model}
...
\begin{abstract}
OEIS A187161 carries an empirical recurrence of order $3$. It is true, and it is
...
OEIS A187161 is ``Number of 8-step one space at a time bishop's tours on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 0, 0, 472, 3504, 10576, 23340,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2172*n\^{}2 - 19816*n + 42860 for n>6. a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 23 2018 12:36:02, revision 13)
...
The number of steps is fixed at $8$ cells, hence $7$ moves, and it is the BOARD
...
SHAPE when one is a translate of the other. There are exactly $2172$ shapes: self-avoiding
sequences of $8$ distinct cells in $\mathbf{Z}^{2}$ with each consecutive difference in
...
Write $w^\ast$ for the largest bounding-box side over all $2172$ shapes. Once $n\ge w^\ast$
...
annihilates $a$ from $n=7$ onwards, and the annihilator in force on the whole sequence is
$z^{8}(z-1)^{3}$, monic of order $S=11$. Both numbers are read off the enumerated
...
vanish from $n=9+1$ onwards, and the run exceeds $S+3$, which by
Lemma~\ref{lem:crit} settles every larger index at once. The bound is exact: at $n=9$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187161.
