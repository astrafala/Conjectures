...
\title{The empirical recurrence for OEIS A229539, proved by counting defective colourings}
...
\begin{abstract}
OEIS A229539 counts the colourings of an array with $K=3$ colours that are proper except at
...
order $46$ contributed by R.~H.~Hardin follows from a finite exact computation.
\end{abstract}
...
OEIS A229539 is ``Number of defective 3-colorings of a 6 X n 0..2 array connected horizontally, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. It has offset $1$ and begins
\[
0, 1188, 11184, 185808, 2451728, 34811238, 463976296, 6218438820,\ \dots
\]
...
\[
3!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)=\mathbf{v}^{\!\top}G^{\,j-46}w,
\qquad w=h_{46}-\sum_i c_ih_{46-i} .
\]
...
index corresponding to $n=48+1$ onwards, and the run continued until $S+1$ consecutive
zeros had been seen.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229539.
