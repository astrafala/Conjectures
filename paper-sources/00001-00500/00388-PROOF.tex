...
\title{The empirical recurrence for OEIS A187165, proved from an exact model}
...
\begin{abstract}
OEIS A187165 carries an empirical recurrence of order $4$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = 150*n\^{}3 - 426*n\^{}2 + 312*n - 48 for n>2. a(n) = 4*a(n-1) - 6*a(n-2) + 4*a(n-3) - a(n-4) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 05 2022 22:11:44, revision 11)
...
SHAPE when one is a translate of the other. There are exactly $150$ shapes: self-avoiding
sequences of $4$ distinct cells in $\mathbf{Z}^{3}$ with each consecutive difference in
...
Write $w^\ast$ for the largest bounding-box side over all $150$ shapes. Once $n\ge w^\ast$
...
$z^{4}(z-1)^{4}$, monic of order $S=8$. Both numbers are read off the enumerated
