...
\title{The empirical recurrence for OEIS A154153, proved from an exact model}
...
\begin{abstract}
OEIS A154153 carries an empirical recurrence of order $5$. It is true, and it is
...
OEIS A154153 is ``Numbers k such that 28 plus the k-th triangular number is a perfect square.''. It has offset $1$ and begins
\[
6, 8, 47, 57, 278, 336, 1623, 1961,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Conjectures: (Start) a(n) = +a(n-1) +6*a(n-2) -6*a(n-3) -a(n-4) +a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 23 2024 14:53:42, revision 33)
this is still recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a Pell equation}

$k(k+1)/2 + 28$ is a perfect square exactly when $8$ times it is, and completing the square
turns that into
\[
(2k+1)^2-2(2m)^2\;=\;1-8\cdot28\;=\;-223,
\]
so writing $X=2k+1$ and $Y=2m$ the admissible $k$ are exactly the solutions of $X^2-2Y^2=-223$
...
On the region $X\ge1$, $Y\ge0$ one has $Y=\sqrt{(X^2--223)/2}$, an increasing function of $X$,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A154153.
