...
\title{Eventual periodicity modulo $m$ with period dividing $\varphi(m)$:\\a proof of Bala's conjecture on OEIS A000670}
...
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 13, 75, 541, 4683, 47293,\ \dots
\]
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} Let k be a positive integer. The sequence obtained by reducing a(n) modulo k is eventually periodic with the period dividing phi(k) = A000010(k). For example, modulo 16 we obtain the sequence [1, 1, 3, 13, 11, 13, 11, 13, 11, 13, ...], with an apparent period of 2 beginning at a(4). Cf. A354242.
\end{quote}
As of the ``Last modified'' line on the live entry (2026-08-14, revision 909) the
...
\section{Application to A000670}\label{sec:apply}

\begin{corollary}\label{cor:this}
The conjecture quoted in Section~1 is true: for every $m\ge1$ the sequence
A000670$(n) \bmod m$ is eventually periodic with period dividing $\varphi(m)$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:main} it suffices to exhibit the entry's exponential generating
function in the form $G(e^{x}-1)$ with $G$ integral. The posted e.g.f.\ is $1/(2-e^{x})$. Since $e^{x}=1+y$, the denominator is $2-(1+y)=1-y$. Therefore
\[
\sum_{n\ge0}a(n)\frac{x^{n}}{n!}=G(y),\qquad G(t)=\frac{1}{1-y} ,
\]
whose coefficients are a geometric series, all coefficients $1$, all integers. So $G\in\Z[[t]]$ and
Theorem~\ref{thm:main} applies.
\end{proof}

Concretely, the first coefficients of $G$ are
\[
c_{0},c_{1},c_{2},\dots \;=\; 1, 1, 1, 1, 1, 1, 1, \dots ,
\]
...
$22$ terms published on A000670 returns integers at every $k$, and the values
...
\url{https://oeis.org}, 2026. Sequence A000670; conjecture of P.\ Bala.
