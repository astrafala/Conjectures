...
\title{The Gauss congruences for OEIS A158690,\\ for the sequence and all of its shifts}
...
\begin{abstract}
OEIS A158690 carries a conjecture of P.~Bala that for every shift $i\ge0$ the sequence
...
\[
1, 1, 5, 55, 1073, 32671, 1431665,\ \dots
\]
...
\begin{quote}\small
2) Let i >= 0 and define a\_i(n) = a(n+i). Then for each i the Gauss congruences a\_i(n*p\^{}k) == a\_i(n*p\^{}(k-1)) ( mod p\^{}k ) hold for all prime p and positive integers n and k. --- \emph{Peter Bala}, Dec 18 2021
\end{quote}
...
records it as settled either way. The entry carries a second conjecture of the same contributor, on the period of the sequence modulo $k$; it is not the statement treated here.
...
A(x)\;=\;G\bigl(e^{x}-1\bigr),\qquad G(t)\;=\;\sum_{n\ge 0}\ \prod_{k=1}^{n}\Bigl(1-(1+t)^{-(2k-1)}\Bigr).
\end{equation}
...
Write $G(t)=\sum_{j\ge0}c_j t^{j}$ with $c_j\in\mathbb{Z}$; here $c_j = 1, 1, 5, 23, 137, \dots$, and
\[
c_0,\dots,c_6\;=\;1, 1, 2, 7, 33, 197, 1419.
\]
...
Let $a(n)$ be OEIS A158690 and $a_i(n)=a(n+i)$. For every $i\ge0$, every prime $p$ and all
integers $n,r\ge1$,
\[
