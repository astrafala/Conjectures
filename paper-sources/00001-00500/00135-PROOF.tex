...
\title{The reduction of OEIS A396807 modulo $10$}
...
\begin{abstract}
OEIS A396807 is defined by a functional equation for its ordinary generating function and
carries a congruence conjectures of P.~D.~Hanna. It is true. The equation determines the
coefficients one at a time, with $a(N)$ entering with coefficient $1$, so it determines
them modulo $10$ as well; and modulo $10$ the series $x/(1-x)$ satisfies the equation
exactly, because its compositional iterates are $x/(1-jx)$ and the relevant product of
denominators collapses. Uniqueness then identifies the reduction, which is the conjectures.
The whole argument is an identity between rational functions over
$\mathbb{Z}/10\mathbb{Z}$; nothing is truncated or estimated.
\end{abstract}
...
OEIS A396807 is ``G.f. satisfies A(x) = x + A\^{}5(x)*A\^{}6(x).''. It has offset $1$ and begins
\[
...
Conjecture: [x\^{}n] A\^{}k(x) == k\^{}(n-1) (mod 10) for n >= 1 and all integer k. --- \emph{Paul D. Hanna}
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 18 2026 00:43:54, revision 9) they are
still recorded as conjectures, and nothing on the entry records
...
Lemma~\ref{lem:jaN}. On the left the coefficient is $1$. On the right, $x$ contributes nothing and the product $A^{\,5}A^{\,6}$ contributes nothing either: every monomial of $\bigl[x^{N}\bigr]\bigl(A^{\,5}A^{\,6}\bigr)$ is a product of one coefficient of $A^{\,5}$ and one of $A^{\,6}$ with indices $i,j\ge1$ and $i+j=N$, so both indices are at most $N-1$. Hence the net coefficient of $a(N)$ is $1$.
...
\section{The reduction modulo $10$}

Write $f(x)=\dfrac{x}{1-x}$ over $\mathbb{Z}/10\mathbb{Z}$.
...
Over $\mathbb{Z}/10\mathbb{Z}$, $f$ satisfies $f=x+f^{\,5}f^{\,6}$.
\end{lemma}

\begin{proof}
By Lemma~\ref{lem:iter}, $f^{\,5}f^{\,6}
=\dfrac{x^{2}}{(1-5x)\bigl(1-6x\bigr)}$, while
$f-x=\dfrac{x}{1-x}-x=\dfrac{x^{2}}{1-x}$. So the claim is that
$(1-5x)(1-6x)$ and $1-x$ agree in $(\mathbb{Z}/10\mathbb{Z})[x]$. Expanding,
\[
(1-5x)\bigl(1-6x\bigr)=1-11x+30x^{2} ,
\]
and modulo $10$ we have $11\equiv1$ and $30\equiv0$. Hence both sides equal
$1-x$, and the two rational functions agree.
\end{proof}


\begin{theorem}\label{thm:main}
Over $\mathbb{Z}/10\mathbb{Z}$ the reduction of $A$ is $x/(1-x)$. Equivalently
$a(n)\equiv1\pmod{10}$ for every $n\ge1$, and more generally
\[
\bigl[x^{n}\bigr]A^{\,j}(x)\;\equiv\;j^{\,n-1}\pmod{10}
\]
for every $n\ge1$ and every integer $j$. This is the conjectures of Section~1.
\end{theorem}
...
Reduction modulo $10$ carries composition of power series with unit linear coefficient to
...
\begin{remark}
The proof used only that $(1-5x)(1-6x)\equiv1-x$ modulo $10$, that is, that
$10$ divides both $2\cdot5$ and $5\cdot6$. The largest modulus with that property
is $\gcd\bigl(2p,\,p(p+1)\bigr)$, which is $2p$ for odd $p$ and $p$ for even $p$; here
$p=5$ gives $10$. This is exactly why the statement is made modulo $10$ and not modulo
a larger number.
\end{remark}
...
its output compared against the entry's DATA at every one of the $19$ published
...
out $1$ at every $N$ up to $24$, as Lemma~\ref{lem:unique} requires. The same run was
continued to $n=24$ and every term reduced modulo $10$; all residues are
$1$.

Third, the composition of the proof was carried out independently as a formal power series
over $\mathbb{Z}/10\mathbb{Z}$ to order $30$, starting from the coefficients of $x/(1-x)$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A396807.
