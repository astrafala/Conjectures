...
\title{The conjectured generating function of OEIS A007388,\\ \large from the recurrence the entry
states}
...
\begin{abstract}
OEIS A007388 states a linear recurrence as a fact and carries a rational generating function as an
...
The first terms are $0, 2, 3, 2, 5, 2, 7, 2,\dots$, with offset $1$. The entry states, as a fact,

\begin{quote}\raggedright For n >= 13: a(n) = a(n-2) + a(n-7). - \_Sean A. Irvine\_, Jan 02 2018\end{quote}

\noindent and separately records the conjecture

\begin{quote}\raggedright Empirical g.f.: x\^{}2*(5*x\^{}10+3*x\^{}8-2*x\^{}5-2*x\^{}3-3*x-2) / (x\^{}7+x\^{}2-1). - \_Colin Barker\_, Mar 29 2014\end{quote}
...
Write $b_k=a(1+k)$ and
\[
  D(x)=1-\sum_{i} c_i x^{i}=- x^{7} - x^{2} + 1 ,
\]
the polynomial of the stated recurrence, of degree $7$. Let $B(x)=\sum_{k\ge0}b_kx^k$.

The recurrence says $b_k=\sum_i c_ib_{k-i}$ for every $k\ge 12$. The coefficient of $x^k$ in
$D(x)B(x)$ is exactly $b_k-\sum_i c_ib_{k-i}$, so it vanishes for every $k\ge 12$ and
\[
  D(x)B(x)=P(x)=\sum_{k<12}\Bigl(b_k-\sum_i c_ib_{k-i}\Bigr)x^k
         =- 5 x^{12} - 3 x^{10} + 2 x^{7} + 2 x^{5} + 3 x^{3} + 2 x^{2} ,
\]
a polynomial whose every coefficient is fixed by finitely many published terms. Therefore
\[
  B(x)=\frac{P(x)}{D(x)}=\frac{x^{2} \left(5 x^{10} + 3 x^{8} - 2 x^{5} - 2 x^{3} - 3 x - 2\right)}{x^{7} + x^{2} - 1} ,
\]
which is the conjectured expression.

\begin{theorem}
The conjectured generating function of OEIS A007388 is correct, given the recurrence the entry
states.
\end{theorem}
...
rational functions, not a comparison of the $53$ terms the entry publishes -- those
enter only in fixing the $12$ coefficients of $P$.
\end{proof}
