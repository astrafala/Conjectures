...
\begin{abstract}
OEIS A265225 carries an empirical recurrence of order $5$ contributed by Colin Barker. It is true, and it is
...
\begin{quote}
Conjectures from \_Colin Barker\_, Dec 08 2015 and Apr 20 2019: (Start) a(n) = a(n-1) + 2*a(n-2) - 2*a(n-3) - a(n-4) + a(n-5) for n>4.
\end{quote}
...
For this entry the automaton is rule 54, the period is $p=2$ and the pre-period is $n_0=0$.
The certificate is
\begin{gather*}
r=0:\quad w(n+p) = \varepsilon \cdot w(n) \cdot \texttt{0001} \\
r=1:\quad w(n+p) = \varepsilon \cdot w(n) \cdot \texttt{0111}
\end{gather*}
