...
\title{The empirical recurrence for OEIS A211326, proved by a one-dimensional reduction}
...
\begin{abstract}
OEIS A211326 counts symmetric $(n+1)\times(n+1)$ matrices over
$\{-3,\dots,3\}$ in which every $2\times2$ subblock sums to zero and has one, two or three distinct
...
on $S=77$ states, and the empirical recurrence of order $11$ contributed by R.~H.~Hardin
follows.
\end{abstract}
...
OEIS A211326 is ``Number of (n+1) X (n+1) -3..3 symmetric matrices with every 2 X 2 subblock having sum zero and one, two or three distinct values.''. It has offset $1$ and begins
\[
25, 63, 149, 357, 829, 1941, 4479, 10413,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 16 2018 09:42:35, revision 8) this is still
...
$S=77$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad \tau=\mathbf{1},\qquad S=77.
\]
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-1) - a(n-2) - 29\,a(n-3) + 33\,a(n-4) + 50\,a(n-5) - 88\,a(n-6) - 14\,a(n-7) + 73\,a(n-8) - 22\,a(n-9) - 10\,a(n-10) + 4\,a(n-11)
\]
for every $n>11$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211326.
