...
\title{The empirical recurrence for OEIS A188786, proved from an exact model}
...
\begin{abstract}
OEIS A188786 carries an empirical recurrence of order $5$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = 56*n\^{}4 - 112*n\^{}3 + 48*n\^{}2 for n>1. a(n) = 5*a(n-1) - 10*a(n-2) + 10*a(n-3) - 5*a(n-4) + a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 28 2018 12:49:00, revision 9)
...
sequences of $3$ distinct cells in $\mathbf{Z}^{4}$ with each consecutive difference in
...
$z^{3}(z-1)^{5}$, monic of order $S=8$. Both numbers are read off the enumerated
