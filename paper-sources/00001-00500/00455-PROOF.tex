...
\title{The empirical recurrence for OEIS A280807, proved by counting patterns with a budget}
...
\begin{abstract}
OEIS A280807 imposes three things at once: a condition on each cell that reaches one row up and
...
numbers. The empirical recurrence of order $37$ contributed by R.~H.~Hardin then follows
...
OEIS A280807 is ``Number of nX5 0..1 arrays with no element equal to more than two of its horizontal, vertical and antidiagonal neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
0, 72, 1578, 9715, 41931, 155560, 521862, 1635481,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 08 2017 09:44:49, revision 4) the statement is
...
$h_j=G^{j}N^{o}\mathbf{u}$, we have for $j\ge37$
\[
2!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)=\mathbf{v}^{\!\top}G^{\,j-37}w,
...
$37$ applications of $G$ in exact integer arithmetic and $\mathbf{v}^{\!\top}G^{m}w$
evaluated for $m=0,1,\dots$, again exactly. Every one of those integers vanishes from the
index corresponding to $n=46+1$ onwards, and the run continued until $S+1$ consecutive
zeros had been seen.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A280807.
