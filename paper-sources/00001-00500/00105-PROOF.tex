...
\title{The empirical recurrence for OEIS A211324, proved by a one-dimensional reduction}
...
\begin{abstract}
OEIS A211324 counts symmetric $(n+1)\times(n+1)$ matrices over
$\{-3,\dots,3\}$ in which every $2\times2$ subblock sums to zero and has one or three distinct
...
on $S=20$ states, and the empirical recurrence of order $7$ contributed by R.~H.~Hardin
follows.
\end{abstract}
...
OEIS A211324 is ``Number of (n+1) X (n+1) -3..3 symmetric matrices with every 2 X 2 subblock having sum zero and one or three distinct values.''. It has offset $1$ and begins
\[
15, 29, 55, 107, 209, 409, 805, 1583,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 2*a(n-2) - 11*a(n-3) + a(n-4) + 12*a(n-5) - 2*a(n-6) - 4*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 16 2018 09:12:44, revision 8) this is still
...
$S=20$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad \tau=\mathbf{1},\qquad S=20.
\]
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 2\,a(n-2) - 11\,a(n-3) + a(n-4) + 12\,a(n-5) - 2\,a(n-6) - 4\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211324.
