...
\title{The empirical recurrence for OEIS A211116, proved by a one-dimensional reduction}
...
\begin{abstract}
OEIS A211116 counts symmetric $(n+1)\times(n+1)$ matrices over
$\{-2,\dots,2\}$ in which every $2\times2$ subblock sums to zero and has one, two or three distinct
...
on $S=27$ states, and the empirical recurrence of order $9$ contributed by R.~H.~Hardin
follows.
\end{abstract}
...
OEIS A211116 is ``Number of (n+1) X (n+1) -2..2 symmetric matrices with every 2 X 2 subblock having sum zero and one, two or three distinct values.''. It has offset $1$ and begins
\[
13, 31, 75, 177, 415, 963, 2227, 5137,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 7*a(n-1) - 15*a(n-2) + a(n-3) + 33*a(n-4) - 28*a(n-5) - 12*a(n-6) + 16*a(n-7) + a(n-8) - 2*a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 16 2018 04:47:24, revision 8) this is still
...
$S=27$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad \tau=\mathbf{1},\qquad S=27.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211116.
