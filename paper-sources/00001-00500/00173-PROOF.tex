...
\title{The empirical recurrence for OEIS A211117, proved by a one-dimensional reduction}
...
\begin{abstract}
OEIS A211117 counts symmetric $(n+1)\times(n+1)$ matrices over
$\{-2,\dots,2\}$ in which every $2\times2$ subblock sums to zero and has two, three or four distinct
...
on $S=16$ states, and the empirical recurrence of order $6$ contributed by R.~H.~Hardin
follows.
\end{abstract}
...
OEIS A211117 is ``Number of (n+1) X (n+1) -2..2 symmetric matrices with every 2 X 2 subblock having sum zero and two, three or four distinct values.''. It has offset $1$ and begins
\[
12, 30, 72, 174, 422, 1028, 2510, 6134,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) - 11*a(n-2) + 3*a(n-3) + 8*a(n-4) - 3*a(n-5) - 2*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 16 2018 04:47:17, revision 8) this is still
...
$S=16$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad \tau=\mathbf{1},\qquad S=16.
\]
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) - 11\,a(n-2) + 3\,a(n-3) + 8\,a(n-4) - 3\,a(n-5) - 2\,a(n-6)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211117.
