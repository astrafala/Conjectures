...
\title{The empirical recurrence for OEIS A265428, proved from an exact model}
...
\begin{abstract}
OEIS A265428 carries an empirical recurrence of order $5$ contributed by Colin Barker. It is true, and it is
...
OEIS A265428 is ``Number of ON (black) cells in the n-th iteration of the "Rule 188" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 2, 2, 4, 4, 5, 5, 7,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Conjectures from \_Colin Barker\_, Dec 09 2015 and Apr 16 2019: (Start) a(n) = a(n-1) + a(n-4) - a(n-5) for n > 4.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:27, revision 27)
...
For this entry the automaton is rule 188, the period is $p=4$ and the pre-period is $n_0=0$.
The certificate is
\begin{gather*}
r=0:\quad w(n+p) = \texttt{0000} \cdot w(n) \cdot \texttt{1101} \\
r=1:\quad w(n+p) = \texttt{0000} \cdot w(n) \cdot \texttt{0111} \\
r=2:\quad w(n+p) = \texttt{0000} \cdot w(n) \cdot \texttt{1101} \\
r=3:\quad w(n+p) = \texttt{0000} \cdot w(n) \cdot \texttt{0111}
\end{gather*}
...
First, the model reproduces all $67$ terms the entry publishes, exactly, in integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A265428.
