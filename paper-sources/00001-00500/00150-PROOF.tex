...
\title{The empirical recurrence for OEIS A211258, proved by a one-dimensional reduction}
...
\begin{abstract}
OEIS A211258 counts symmetric $(n+1)\times(n+1)$ matrices over
$\{-6,\dots,6\}$ in which every $2\times2$ subblock sums to zero and has three or four distinct
...
on $S=70$ states, and the empirical recurrence of order $26$ contributed by R.~H.~Hardin
follows.
\end{abstract}
...
OEIS A211258 is ``Number of (n+1)X(n+1) -6..6 symmetric matrices with every 2X2 subblock having sum zero and three or four distinct values.''. It has offset $1$ and begins
\[
64, 322, 1602, 7994, 39770, 197618, 980042, 4851354,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 16:09:12, revision 8) this is still
...
$S=70$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad \tau=\mathbf{1},\qquad S=70.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211258.
