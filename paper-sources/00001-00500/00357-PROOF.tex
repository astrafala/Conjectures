...
\title{The empirical recurrence for OEIS A187167, proved from an exact model}
...
\begin{abstract}
OEIS A187167 carries an empirical recurrence of order $4$. It is true, and it is
...
OEIS A187167 is ``Number of 6-step self-avoiding walks on an n X n X n cube summed over all starting positions.''. It has offset $1$ and begins
\[
0, 240, 9504, 51504, 148224, 320328, 588924, 975216,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3534*n\^{}3 - 15366*n\^{}2 + 19536*n - 7056 for n>4. a(n) = 4*a(n-1) - 6*a(n-2) + 4*a(n-3) - a(n-4) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 20 2018 14:39:02, revision 14)
...
SHAPE when one is a translate of the other. There are exactly $3534$ shapes: self-avoiding
sequences of $6$ distinct cells in $\mathbf{Z}^{3}$ with each consecutive difference in
...
Write $w^\ast$ for the largest bounding-box side over all $3534$ shapes. Once $n\ge w^\ast$
...
$z^{6}(z-1)^{4}$, monic of order $S=10$. Both numbers are read off the enumerated
...
vanish from $n=8+1$ onwards, and the run exceeds $S+4$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187167.
