...
\title{The empirical closed form for OEIS A219941, proved}
...
\begin{abstract}
OEIS A219941 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=1892$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219941 is ``Number of 3 X n arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 3 X n array.''. It has offset $1$ and begins
\[
20, 76, 368, 2070, 8523, 30558, 100761, 313108,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2025 15:09:45, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=1892$
...
states number $S=1892$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219941 records is correct for all $n>13$.
\end{theorem}
...
$1 + 27 - 1$, which is more than the $27$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219941.
