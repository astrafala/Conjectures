...
\begin{abstract}
OEIS A266438 carries an empirical recurrence of order $5$ contributed by Michael De Vlieger. It is true, and it is
...
\begin{quote}
Conjectured g.f.: (-(1 + x)\^{}(-2) + (-3 + 3*x - 2*x\^{}2)/(-1 + x)\^{}3)/2. - \_Michael De Vlieger\_, Dec 29 2015 a(n) = a(n-1)+2*a(n-2)-2*a(n-3)-a(n-4)+a(n-5) for n>4.
\end{quote}
