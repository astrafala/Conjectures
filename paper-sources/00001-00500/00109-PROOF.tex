...
\title{Eventual periodicity modulo $m$ with period dividing $\varphi(m)$:\\a proof of Bala's conjecture on OEIS A004123}
...
\[
a(1),\dots,a(8)\;=\;1, 2, 10, 74, 730, 9002, 133210, 2299754,\ \dots
\]
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} Let k be a positive integer. The sequence obtained by reducing a(n) modulo k is eventually periodic with the period dividing phi(k) = A000010(k). For example, modulo 7 we obtain the sequence [1, 2, 3, 4, 2, 0, 0, 2, 3, 4, 2, 0, 0, 2, 3, 4, 2, 0, 0, ...] with an apparent period of 6 = phi(7) starting at a(2). Cf. A000670.
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 98) the
...
\section{Application to A004123}\label{sec:apply}

\begin{corollary}\label{cor:this}
The conjecture quoted in Section~1 is true: for every $m\ge1$ the sequence
A004123$(n) \bmod m$ is eventually periodic with period dividing $\varphi(m)$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:main} it suffices to exhibit the entry's exponential generating
function in the form $G(e^{x}-1)$ with $G$ integral. The entry's own formula line reads ``e.g.f.\ for sequence with offset $0$: $1/(3-2e^{x})$''. With $e^{x}=1+y$ the denominator is $3-2(1+y)=1-2y$. Therefore
\[
\sum_{n\ge0}a(n)\frac{x^{n}}{n!}=G(y),\qquad G(t)=\frac{1}{1-2y} ,
\]
whose coefficients are a geometric series with ratio $2y$, all integers. So $G\in\Z[[t]]$ and
Theorem~\ref{thm:main} applies. The entry has offset $1$ and its formula line specifies the e.g.f.\ ``for sequence with offset $0$'', so the entry's sequence is a shift of the one generated by $G$; a shift alters neither eventual periodicity nor the period.
\end{proof}

Concretely, the first coefficients of $G$ are
\[
c_{0},c_{1},c_{2},\dots \;=\; 1, 2, 4, 8, 16, 32, 64, \dots ,
\]
...
$18$ terms published on A004123 returns integers at every $k$, and the values
...
\url{https://oeis.org}, 2026. Sequence A004123; conjecture of P.\ Bala.
