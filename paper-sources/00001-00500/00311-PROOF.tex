...
\title{The empirical recurrence for OEIS A188785, proved from an exact model}
...
\begin{abstract}
OEIS A188785 carries an empirical recurrence of order $5$. It is true, and it is
...
OEIS A188785 is ``Number of 2-step self-avoiding walks on an n X n X n X n 4-cube summed over all starting positions.''. It has offset $1$ and begins
\[
0, 64, 432, 1536, 4000, 8640, 16464, 28672,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 8*n\^{}4 - 8*n\^{}3. a(n) = 5*a(n-1) - 10*a(n-2) + 10*a(n-3) - 5*a(n-4) + a(n-5) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 28 2018 12:49:07, revision 9)
...
The number of steps is fixed at $2$ cells, hence $1$ moves, and it is the BOARD
...
SHAPE when one is a translate of the other. There are exactly $8$ shapes: self-avoiding
sequences of $2$ distinct cells in $\mathbf{Z}^{4}$ with each consecutive difference in
...
Write $w^\ast$ for the largest bounding-box side over all $8$ shapes. Once $n\ge w^\ast$
...
annihilates $a$ from $n=1$ onwards, and the annihilator in force on the whole sequence is
$z^{2}(z-1)^{5}$, monic of order $S=7$. Both numbers are read off the enumerated
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188785.
