...
\title{The empirical recurrence for OEIS A187859, proved from an exact model}
...
\begin{abstract}
OEIS A187859 carries an empirical recurrence of order $3$. It is true, and it is
...
which $a$ provably satisfies a MONIC linear recurrence of order $S=10$, derived below and not
...
OEIS A187859 is ``Number of 4-step one space for components leftwards or up, two space for components rightwards or down asymmetric quasi-queen's tours (antidiagonal moves become knight moves) on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 216, 968, 2754, 5428, 9237, 14040,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 497*n\^{}2 - 2652*n + 3448 for n>5. a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 26 2018 08:56:39, revision 10)
...
SHAPE when one is a translate of the other. There are exactly $497$ shapes: self-avoiding
sequences of $4$ distinct cells in $\mathbf{Z}^{2}$ with each consecutive difference in
...
Write $w^\ast$ for the largest bounding-box side over all $497$ shapes. Once $n\ge w^\ast$
...
annihilates $a$ from $n=6$ onwards, and the annihilator in force on the whole sequence is
$z^{7}(z-1)^{3}$, monic of order $S=10$. Both numbers are read off the enumerated
...
vanish from $n=8+1$ onwards, and the run exceeds $S+3$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187859.
