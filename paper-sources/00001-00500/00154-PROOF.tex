...
\title{The empirical recurrence for OEIS A267214, proved from an exact model}
...
\begin{abstract}
OEIS A267214 carries an empirical recurrence of order $8$ contributed by Colin Barker. It is true, and it is
...
which $a$ provably satisfies a MONIC linear recurrence of order $S=31$, derived below and not
...
OEIS A267214 is ``Total number of OFF (white) cells after n iterations of the "Rule 109" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 2, 2, 7, 9, 13, 16, 26,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Conjectures from \_Colin Barker\_, Jan 13 2016 and Apr 19 2019: (Start) a(n) = a(n-1)+a(n-3)-a(n-5)-a(n-7)+a(n-8) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 16)
this is still recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The entry counts the running total of its OFF (white) cells over the first $n$ iterations of the automaton. Nothing is being enumerated over a window, so there is
...
For this entry the automaton is rule 109, the period is $p=12$ and the pre-period is $n_0=1$.
The certificate is
\begin{gather*}
r=0:\quad \texttt{010111110001}^{\,j} \cdot \texttt{0} \cdot \texttt{101000111110}^{\,j} \cdot \texttt{10} \\
r=1:\quad \texttt{111110001010}^{\,j} \cdot \texttt{1} \cdot \texttt{111101010001}^{\,j} \cdot \texttt{1111} \\
r=2:\quad \texttt{0} \cdot \texttt{100010101111}^{\,j} \cdot \texttt{1} \cdot \texttt{000111110101}^{\,j} \cdot \texttt{00010} \\
r=3:\quad \texttt{11} \cdot \texttt{101011111000}^{\,j} \cdot \texttt{1} \cdot \texttt{010100011111}^{\,j} \cdot \texttt{010111}
\end{gather*} The remaining 8 residue classes are of the same shape and were verified in the same way.
...
and the order $S=31$ used below covers all four wordings.
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-3) - a(n-5) - a(n-7) + a(n-8)
\]
for every $n>0$.
\end{theorem}
...
First, the model reproduces all $54$ terms the entry publishes, exactly, in integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267214.
