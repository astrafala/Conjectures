...
\title{The empirical recurrence for OEIS A211549, proved by a one-dimensional reduction}
...
\begin{abstract}
OEIS A211549 counts symmetric $(n+1)\times(n+1)$ matrices over
$\{-9,\dots,9\}$ in which every $2\times2$ subblock sums to zero and has one or two distinct
...
on $S=51$ states, and the empirical recurrence of order $3$ contributed by R.~H.~Hardin
follows.
\end{abstract}
...
OEIS A211549 is ``Number of (n+1) X (n+1) -9..9 symmetric matrices with every 2 X 2 subblock having sum zero and one or two distinct values.''. It has offset $1$ and begins
\[
31, 43, 61, 91, 139, 217, 343, 547,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 19 2018 05:18:50, revision 8) this is still
...
that every $2\times2$ subblock has one or two distinct entries.
...
Call two consecutive pairs COMPATIBLE when that number lies in $\{1,2\}$. The condition on the
...
$S=51$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad \tau=\mathbf{1},\qquad S=51.
\]
...
$2\times2$ subblock tested for summing to zero and for having one or two distinct entries. The
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211549.
