...
\title{The empirical recurrence for OEIS A187159, proved from an exact model}
...
\begin{abstract}
OEIS A187159 carries an empirical recurrence of order $3$. It is true, and it is
...
OEIS A187159 is ``Number of 6-step one space at a time bishop's tours on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 0, 56, 456, 1588, 3288, 5556,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 284*n\^{}2 - 1992*n + 3316 for n>4. a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 20 2018 09:44:27, revision 10)
...
SHAPE when one is a translate of the other. There are exactly $284$ shapes: self-avoiding
...
Write $w^\ast$ for the largest bounding-box side over all $284$ shapes. Once $n\ge w^\ast$
...
annihilates $a$ from $n=5$ onwards, and the annihilator in force on the whole sequence is
$z^{6}(z-1)^{3}$, monic of order $S=9$. Both numbers are read off the enumerated
...
vanish from $n=7+1$ onwards, and the run exceeds $S+3$, which by
Lemma~\ref{lem:crit} settles every larger index at once. The bound is exact: at $n=7$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187159.
