...
\title{The empirical recurrence for OEIS A187611, proved from an exact model}
...
\begin{abstract}
OEIS A187611 carries an empirical recurrence of order $3$. It is true, and it is
...
which $a$ provably satisfies a MONIC linear recurrence of order $S=16$, derived below and not
...
OEIS A187611 is ``Number of 7-step one space for components leftwards or up, two space for components rightwards or down asymmetric quasi-bishop's tours (antidiagonal moves become knight moves) on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 0, 0, 48, 616, 2936, 8530,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2344*n\^{}2 - 28880*n + 85282 for n>11. a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) for n>14.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 25 2018 11:48:56, revision 11)
...
The number of steps is fixed at $7$ cells, hence $6$ moves, and it is the BOARD
...
SHAPE when one is a translate of the other. There are exactly $2344$ shapes: self-avoiding
sequences of $7$ distinct cells in $\mathbf{Z}^{2}$ with each consecutive difference in
...
Write $w^\ast$ for the largest bounding-box side over all $2344$ shapes. Once $n\ge w^\ast$
...
annihilates $a$ from $n=12$ onwards, and the annihilator in force on the whole sequence is
$z^{13}(z-1)^{3}$, monic of order $S=16$. Both numbers are read off the enumerated
...
vanish from $n=14+1$ onwards, and the run exceeds $S+3$, which by
Lemma~\ref{lem:crit} settles every larger index at once. The bound is exact: at $n=14$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187611.
