...
\title{The empirical recurrence for OEIS A187515, proved from an exact model}
...
\begin{abstract}
OEIS A187515 carries an empirical recurrence of order $3$. It is true, and it is
...
OEIS A187515 is ``Number of 10-step S, E, and NW-moving king's tours on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 0, 398, 3868, 13796, 31548, 56952,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3603*n\^{}2 - 28504*n + 54377 for n>8. a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) for n>11.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 24 2018 11:08:39, revision 10)
...
The number of steps is fixed at $10$ cells, hence $9$ moves, and it is the BOARD
...
SHAPE when one is a translate of the other. There are exactly $3603$ shapes: self-avoiding
sequences of $10$ distinct cells in $\mathbf{Z}^{2}$ with each consecutive difference in
...
Write $w^\ast$ for the largest bounding-box side over all $3603$ shapes. Once $n\ge w^\ast$
...
annihilates $a$ from $n=9$ onwards, and the annihilator in force on the whole sequence is
$z^{10}(z-1)^{3}$, monic of order $S=13$. Both numbers are read off the enumerated
...
vanish from $n=11+1$ onwards, and the run exceeds $S+3$, which by
Lemma~\ref{lem:crit} settles every larger index at once. The bound is exact: at $n=11$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187515.
