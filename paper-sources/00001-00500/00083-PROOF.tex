...
\title{Eventual periodicity modulo $m$ with period dividing $\varphi(m)$:\\a proof of Bala's conjecture on OEIS A080253}
...
\[
a(0),\dots,a(7)\;=\;1, 3, 17, 147, 1697, 24483, 423857, 8560947,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 85) the
...
\section{Application to A080253}\label{sec:apply}

\begin{corollary}\label{cor:this}
The conjecture quoted in Section~1 is true: for every $m\ge1$ the sequence
A080253$(n) \bmod m$ is eventually periodic with period dividing $\varphi(m)$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:main} it suffices to exhibit the entry's exponential generating
function in the form $G(e^{x}-1)$ with $G$ integral. The posted e.g.f.\ is $e^{x}/(2-e^{2x})$. With $e^{x}=1+y$ the denominator is $2-(1+y)^{2}=1-2y-y^{2}$. Therefore
\[
\sum_{n\ge0}a(n)\frac{x^{n}}{n!}=G(y),\qquad G(t)=\frac{1+y}{1-2y-y^{2}} ,
\]
...
\[
c_{0},c_{1},c_{2},\dots \;=\; 1, 3, 7, 17, 41, 99, 239, \dots ,
\]
...
$18$ terms published on A080253 returns integers at every $k$, and the values
...
\url{https://oeis.org}, 2026. Sequence A080253; conjecture of P.\ Bala.
