...
\title{The empirical recurrence for OEIS A267530, proved from an exact model}
...
\begin{abstract}
OEIS A267530 carries an empirical recurrence of order $3$ contributed by Colin Barker. It is true, and it is
...
OEIS A267530 is ``Number of OFF (white) cells in the n-th iteration of the "Rule 141" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 2, 3, 4, 4, 5, 5, 6,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Conjectures from \_Colin Barker\_, Jan 17 2016 and Apr 20 2019: (Start) a(n) = a(n-1)+a(n-2)-a(n-3) for n>4.
\end{quote}
...
For this entry the automaton is rule 141, the period is $p=2$ and the pre-period is $n_0=2$.
The certificate is
\begin{gather*}
r=0:\quad \texttt{0} \cdot \texttt{01}^{\,j} \cdot \texttt{010} \cdot \texttt{11}^{\,j} \cdot \texttt{1} \\
r=1:\quad \texttt{00} \cdot \texttt{01}^{\,j} \cdot \texttt{010} \cdot \texttt{11}^{\,j} \cdot \texttt{11}
\end{gather*}
...
First, the model reproduces all $68$ terms the entry publishes, exactly, in integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267530.
