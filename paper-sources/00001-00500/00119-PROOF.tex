...
\title{Eventual periodicity modulo $m$ with period dividing $\varphi(m)$:\\a proof of Bala's conjecture on OEIS A316144}
...
OEIS A316144 is ``Expansion of e.g.f. Product\_{k>=1} ((1 + (exp(x)-1)\^{}k) / (1 - (exp(x)-1)\^{}k))\^{}2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 28, 268, 3148, 43564, 692428, 12390508,\ \dots
\]
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} Let k be a positive integer. The sequence obtained by reducing a(n) modulo k is eventually periodic with the period dividing phi(k) = A000010(k). For example, modulo 7 we obtain the sequence [1, 4, 0, 2, 5, 3, 2, 4, 0, 2, 5, 3, 2, 4, 0, 2, 5, 3, 2, ...], with a preperiod of length 1 and an apparent period thereafter of 6 = phi(7). - \emph{Peter Bala}, Mar 03 2023
\end{quote}
As of the ``Last modified'' line on the live entry (2024-02-28, revision 21) the
...
\section{Application to A316144}\label{sec:apply}

\begin{corollary}\label{cor:this}
The conjecture quoted in Section~1 is true: for every $m\ge1$ the sequence
A316144$(n) \bmod m$ is eventually periodic with period dividing $\varphi(m)$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:main} it suffices to exhibit the entry's exponential generating
function in the form $G(e^{x}-1)$ with $G$ integral. The entry is ``Expansion of e.g.f.\ $\prod_{k\ge1}((1+(\exp(x)-1)^{k})/(1-(\exp(x)-1)^{k}))^{2}$''. Therefore
\[
\sum_{n\ge0}a(n)\frac{x^{n}}{n!}=G(y),\qquad G(t)=\prod_{j\ge1}\left(\frac{1+y^{j}}{1-y^{j}}\right)^{2} ,
\]
...
\[
c_{0},c_{1},c_{2},\dots \;=\; 1, 4, 12, 32, 76, 168, 352, \dots ,
\]
...
$19$ terms published on A316144 returns integers at every $k$, and the values
...
exact period  & $4$ & $6$ & $1$ & $6$ & $10$ & $1$ & $12$ & $1$ & $20$ & $18$\\
\bottomrule
\end{tabular}
\end{center}
...
\url{https://oeis.org}, 2026. Sequence A316144; conjecture of P.\ Bala.
