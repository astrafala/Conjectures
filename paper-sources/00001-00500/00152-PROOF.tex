...
\title{The empirical recurrence for OEIS A284350, proved from an exact model}
...
\begin{abstract}
OEIS A284350 carries an empirical recurrence of order $9$. It is true, and it is
...
which $a$ provably satisfies a MONIC linear recurrence of order $S=39$, derived below and not
...
OEIS A284350 is ``Decimal representation of the x-axis, from the origin to the right edge, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 873", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 0, 6, 10, 10, 46, 46, 250,\ \dots
\]
...
pre-period to raise the numerator's degree gives a monic annihilator of order $S=39$. The
...
vanish from $n=23+1$ onwards, and the run exceeds $S+9$, which by
Lemma~\ref{lem:crit} settles every larger index at once. The bound is exact: at $n=23$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A284350.
