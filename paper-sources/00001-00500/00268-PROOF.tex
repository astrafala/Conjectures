...
\title{The empirical recurrence for OEIS A188787, proved from an exact model}
...
\begin{abstract}
OEIS A188787 carries an empirical recurrence of order $5$. It is true, and it is
...
OEIS A188787 is ``Number of 4-step self-avoiding walks on an n X n X n X n 4-cube summed over all starting positions.''. It has offset $1$ and begins
\[
0, 576, 8928, 41984, 125840, 296064, 597632, 1084928,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 392*n\^{}4 - 1128*n\^{}3 + 912*n\^{}2 - 192*n for n>2. a(n) = 5*a(n-1) - 10*a(n-2) + 10*a(n-3) - 5*a(n-4) + a(n-5) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 28 2018 12:49:04, revision 10)
...
The number of steps is fixed at $4$ cells, hence $3$ moves, and it is the BOARD
...
SHAPE when one is a translate of the other. There are exactly $392$ shapes: self-avoiding
sequences of $4$ distinct cells in $\mathbf{Z}^{4}$ with each consecutive difference in
$M$, counted up to translation, which a finite search enumerates once and for all.

A shape occupies a bounding box with sides $(w_1,\dots,w_{4})$, the range of its cells in each
...
\[
a(n)\;=\;\sum_{w} N_w \prod_{i=1}^{4}\max\bigl(0,\ n-w_i\bigr).
\]
...
Write $w^\ast$ for the largest bounding-box side over all $392$ shapes. Once $n\ge w^\ast$
every factor above is nonnegative, so the truncation does nothing and
$a(n)=\sum_w N_w\prod_i(n-w_i)$, a polynomial in $n$ of degree $4$. Hence $(z-1)^{5}$
annihilates $a$ from $n=3$ onwards, and the annihilator in force on the whole sequence is
$z^{4}(z-1)^{5}$, monic of order $S=9$. Both numbers are read off the enumerated
...
vanish from $n=7+1$ onwards, and the run exceeds $S+5$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188787.
