...
\title{The empirical recurrence for OEIS A283864, proved from an exact model}
...
\begin{abstract}
OEIS A283864 carries an empirical recurrence of order $5$. It is true, and it is
...
which $a$ provably satisfies a MONIC linear recurrence of order $S=33$, derived below and not
...
OEIS A283864 is ``Decimal representation of the x-axis, from the left edge to the origin, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 734", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
...
pre-period to raise the numerator's degree gives a monic annihilator of order $S=33$. The
...
vanish from $n=25+1$ onwards, and the run exceeds $S+5$, which by
Lemma~\ref{lem:crit} settles every larger index at once. The bound is exact: at $n=25$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A283864.
