...
\title{The empirical recurrence for OEIS A187166, proved from an exact model}
...
\begin{abstract}
OEIS A187166 carries an empirical recurrence of order $4$. It is true, and it is
...
OEIS A187166 is ``Number of 5-step self-avoiding walks on an n X n X n cube summed over all starting positions.''. It has offset $1$ and begins
\[
0, 144, 3240, 14256, 37470, 77184, 137754, 223536,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 726*n\^{}3 - 2640*n\^{}2 + 2688*n - 720 for n>3. a(n) = 4*a(n-1) - 6*a(n-2) + 4*a(n-3) - a(n-4) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 20 2018 14:38:43, revision 11)
...
SHAPE when one is a translate of the other. There are exactly $726$ shapes: self-avoiding
sequences of $5$ distinct cells in $\mathbf{Z}^{3}$ with each consecutive difference in
...
Write $w^\ast$ for the largest bounding-box side over all $726$ shapes. Once $n\ge w^\ast$
...
$z^{5}(z-1)^{4}$, monic of order $S=9$. Both numbers are read off the enumerated
...
vanish from $n=7+1$ onwards, and the run exceeds $S+4$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187166.
