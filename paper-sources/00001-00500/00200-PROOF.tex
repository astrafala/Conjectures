...
\title{The empirical recurrence for OEIS A211492, proved by a one-dimensional reduction}
...
\begin{abstract}
OEIS A211492 counts symmetric $(n+1)\times(n+1)$ matrices over
$\{-4,\dots,4\}$ in which every $2\times2$ subblock sums to zero and has one or three distinct
...
on $S=108$ states, and the empirical recurrence of order $9$ contributed by R.~H.~Hardin
follows.
\end{abstract}
...
OEIS A211492 is ``Number of (n+1) X (n+1) -4..4 symmetric matrices with every 2 X 2 subblock having sum zero and one or three distinct values.''. It has offset $1$ and begins
\[
29, 65, 139, 287, 593, 1189, 2397, 4755,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 5*a(n-2) - 20*a(n-3) - 5*a(n-4) + 45*a(n-5) - 5*a(n-6) - 40*a(n-7) + 6*a(n-8) + 12*a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 18 2018 08:22:28, revision 7) this is still
...
$S=108$. The two parity classes never mix, so the walk starts by choosing a parity and a first
value, and every state may end a sequence:
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad \tau=\mathbf{1},\qquad S=108.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211492.
