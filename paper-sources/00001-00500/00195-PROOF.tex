...
\title{The empirical recurrence for OEIS A267212, proved from an exact model}
...
\begin{abstract}
OEIS A267212 carries an empirical recurrence of order $8$ contributed by Colin Barker. It is true, and it is
...
OEIS A267212 is ``Total number of ON (black) cells after n iterations of the "Rule 109" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 2, 7, 9, 16, 23, 33, 38,\ \dots
\]
...
The entry counts the running total of its ON (black) cells over the first $n$ iterations of the automaton. Nothing is being enumerated over a window, so there is
...
First, the model reproduces all $52$ terms the entry publishes, exactly, in integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267212.
