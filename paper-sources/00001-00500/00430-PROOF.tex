...
\title{The empirical recurrence for OEIS A187612, proved from an exact model}
...
\begin{abstract}
OEIS A187612 carries an empirical recurrence of order $3$. It is true, and it is
...
OEIS A187612 is ``Number of 8-step one space for components leftwards or up, two space for components rightwards or down asymmetric quasi-bishop's tours (antidiagonal moves become knight moves) on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 0, 0, 0, 474, 3780, 12946,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 8156*n\^{}2 - 114640*n + 385419 for n>13. a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) for n>16.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 25 2018 10:12:29, revision 10)
...
SHAPE when one is a translate of the other. There are exactly $8156$ shapes: self-avoiding
...
Write $w^\ast$ for the largest bounding-box side over all $8156$ shapes. Once $n\ge w^\ast$
...
annihilates $a$ from $n=14$ onwards, and the annihilator in force on the whole sequence is
$z^{15}(z-1)^{3}$, monic of order $S=18$. Both numbers are read off the enumerated
...
vanish from $n=16+1$ onwards, and the run exceeds $S+3$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187612.
