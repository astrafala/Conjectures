...
\title{The empirical recurrence for OEIS A267049, proved from an exact model}
...
\begin{abstract}
OEIS A267049 carries an empirical recurrence of order $5$ contributed by Colin Barker. It is true, and it is
...
OEIS A267049 is ``Total number of OFF (white) cells after n iterations of the "Rule 91" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 1, 4, 7, 11, 14, 22, 25,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Conjectures from \_Colin Barker\_, Jan 11 2016 and Apr 19 2019: (Start) a(n) = a(n-1)+2*a(n-2)-2*a(n-3)-a(n-4)+a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 18)
...
For this entry the automaton is rule 91, the period is $p=2$ and the pre-period is $n_0=3$.
The certificate is
\begin{gather*}
r=0:\quad w(n+p) = \texttt{00} \cdot w(n) \cdot \texttt{00} \\
r=1:\quad w(n+p) = \texttt{11} \cdot w(n) \cdot \texttt{11}
\end{gather*}
...
annihilator of order $S=9$ used below.
...
vanish from $n=6+1$ onwards, and the run exceeds $S+5$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267049.
