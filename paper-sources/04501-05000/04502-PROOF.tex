...
\title{The conjectured generating function for OEIS A242545, proved}
...
\begin{abstract}
OEIS A242545 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=225$ vertices, so its generating function is a rational function whose numerator
...
OEIS A242545 is ``Number of length n+3 0..4 arrays with no four elements in a row with pattern aabb (possibly a=b) and new values 0..4 introduced in 0..4 order.''. It has offset $1$ and begins
\[
13, 43, 165, 690, 3050, 13988,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.:  -x*(13 -48*x +46*x\^{}2 -240*x\^{}3 -28*x\^{}4 +26*x\^{}5 +245*x\^{}6 +1169*x\^{}7 +1276*x\^{}8 +1240*x\^{}9 +992*x\^{}10 +296*x\^{}11) / ( (2*x\^{}2+2*x-1) *(x\^{}2+x-1) *(2*x\^{}2+1) *(x\^{}2+1) *(4*x\^{}2+1) *(4*x\^{}2+4*x-1) ). - \_R. J. Mathar\_, May 18 2014
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 01 2024 19:53:41, revision 8) this is still
...
\]
and there are $S=225$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $237$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{237}$ then $R$ has a zero of order greater than its own
...
of them --- more than the $237+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{12}$ read off $D_2$, the first few being $7,\ -14,\ 29,\ -53,\ -18,\ -28$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A242545.
