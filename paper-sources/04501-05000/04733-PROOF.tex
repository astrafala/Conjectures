...
\title{The empirical column recurrences for the table OEIS A231144}
...
\begin{abstract}
OEIS A231144 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231144 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with no element equal to a strict majority of its horizontal and vertical neighbors, with upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
18, 100, 100, 570, 1470, 570, 3234, 22870,\ \dots
\]
...
k=3: [order 46]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:17:21, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..2 arrays with no element equal to a strict majority of its horizontal and vertical neighbors, with upper left element zero.''. Its rows are the vertices of a digraph on
$S=70$ vertices, edges being the admissible consecutive pairs, and the count is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231144.
