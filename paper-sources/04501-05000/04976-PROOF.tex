...
\title{The empirical recurrence for OEIS A224404, proved by transfer matrix}
...
\begin{abstract}
OEIS A224404 counts arrays over $\{0,\dots,1\}$ whose rows, columns, diagonals or
antidiagonals are required to be monotone or unimodal, and carries an empirical recurrence of
order $3$ contributed by R.~H.~Hardin. A monotone condition is local, but a unimodal one
...
is a walk count on $S=7$ vertices and the recurrence follows from a finite exact
computation.
\end{abstract}
...
OEIS A224404 is ``Number of n X 3 0..1 arrays with rows unimodal and antidiagonals nondecreasing.''. It has offset $1$ and begins
\[
7, 28, 100, 358, 1288, 4636, 16684, 60040,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) - 2*a(n-2) + 2*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 30 2018 06:07:34, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) - 2\,a(n-2) + 2\,a(n-3)
\end{equation}
for all $n>3$.
...
bit vector), $|\mathcal V|=S=7$; let $(r,f)\to(s,f')$ be an edge when placing $s$ after $r$
...
$h_j=G^{j}N^{o}\mathbf{1}$, we have for $j\ge3$
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224404.
