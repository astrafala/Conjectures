...
\title{The empirical recurrence for OEIS A208264, proved by counting patterns}
...
\begin{abstract}
OEIS A208264 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
$3$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A208264 is ``Number of n X 3 0..1 arrays with new values 0..1 introduced in row major order and no element equal to more than two of its immediate leftward or upward or left-upward diagonal neighbors.''. It has offset $1$ and begins
\[
4, 25, 161, 1033, 6631, 42563, 273205, 1753657,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) + 3*a(n-2) - 2*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 29 2018 08:35:40, revision 8) the statement is
...
For every $n$ with $m=n+0-1\ge3$,
\[
2!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-3}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-3}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $3$ matrix--vector products
...
corresponding to $n=3+1$ onwards, and the run continued until $w$ was the zero vector,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208264.
