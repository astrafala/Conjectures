...
\title{The conjectured generating function for OEIS A282122, proved}
...
\begin{abstract}
OEIS A282122 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A282122 is ``Binary representation of the x-axis, from the origin to the right edge, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 430", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 11, 11, 1111, 1011, 111111,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 + 11*x - 90*x\^{}2) / ((1 - x)*(1 + x)*(1 - 10*x)*(1 + 10*x)).
\end{quote}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A282122.
