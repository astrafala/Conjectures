...
\title{The conjectured generating function for OEIS A277864, proved}
...
\begin{abstract}
OEIS A277864 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A277864 is ``Binary representation of the x-axis, from the left edge to the origin, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 3", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 1, 1, 1101, 1, 111101,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1+x-100*x\^{}2+1000*x\^{}3) / ((1-x)*(1+x)*(1-10*x)*(1+10*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:35, revision 13) this is still
...
\[
C(n+2)\;=\;C(n)\qquad\text{as configurations of the whole plane, for every } n\ge 0.
\]
...
neighbourhood to a live cell flips the ENTIRE plane, and rule 3 is read
...
The model reproduces all $27$ terms the entry publishes, exactly, in integer arithmetic:
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=3$ and $\deg D_2=4$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $11$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{11}$ then $R$ has a zero of order greater than its own
...
of them --- more than the $11+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A277864.
