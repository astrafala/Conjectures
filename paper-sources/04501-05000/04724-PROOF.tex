...
\title{The empirical recurrence for OEIS A240342, proved by transfer matrix}
...
\begin{abstract}
OEIS A240342 counts arrays over $\{0,1,2,3\}$ in which no entry may equal, modulo $4$, a given
constant plus the SUM of all the entries lying before it along one of several rays. The entry
carries an empirical recurrence of order $4$ contributed by R.~H.~Hardin. It is true, and
...
OEIS A240342 is ``Number of n X 2 0..3 arrays with no element equal to one plus the sum of elements to its left or one plus the sum of elements above it or three plus the sum of the elements diagonally to its northwest, modulo 4.''. It has offset $1$ and begins
\[
4, 18, 78, 334, 1418, 6002, 25374, 107230,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) - 7*a(n-2) - 4*a(n-3) + 8*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 27 2018 08:13:45, revision 9) this is still
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) - 7\,a(n-2) - 4\,a(n-3) + 8\,a(n-4)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A240342.
