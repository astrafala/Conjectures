...
\title{The empirical recurrence for OEIS A275143, proved by counting patterns}
...
\begin{abstract}
OEIS A275143 counts arrays in which no entry may repeat a value found at any of three stated
...
cutting $794$ pairs to $S=155$ states. The entry's empirical recurrence of order $3$,
...
OEIS A275143 is ``Number of 3 X n 0..2 arrays with no element equal to any value at offset (-2,-2) (-1,-2) or (0,-1) and new values introduced in order 0..2.''. It has offset $1$ and begins
\[
5, 36, 80, 224, 528, 1216, 2816, 6544,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) - 2*a(n-2) + a(n-3) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 01 2019 05:03:56, revision 7) the statement is
...
$o=1n_0+0-2$, we have for $j\ge3$
\[
3!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)
=\mathbf{v}^{\!\top}G^{\,j-3}w,\qquad
w=h_{3}-\sum_i c_i h_{3-i} .
\]
...
the survivors by the pattern of the new pair. The vector $w$ was formed by $3$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A275143.
