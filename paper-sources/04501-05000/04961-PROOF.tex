...
\title{The empirical column recurrences for the table OEIS A238654}
...
\begin{abstract}
OEIS A238654 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A238654 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no element equal to all horizontal neighbors or equal to all vertical neighbors, and new values 0..1 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 3, 8, 3, 5, 18,\ \dots
\]
...
k=3: a(n) = 2*a(n-1) +5*a(n-2) -a(n-4)\\
k=4: [order 20]\\
k=5: [order 68]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:53:46, revision 6) these are still
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with no element equal to all horizontal neighbors or equal to all vertical neighbors, and new values 0..1 introduced in row major order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+2)X(4) 0..1 arrays with no element equal to all horizontal neighbors or equal to all vertical neighbors, and new values 0..1 introduced in row major order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+2)X(5) 0..1 arrays with no element equal to all horizontal neighbors or equal to all vertical neighbors, and new values 0..1 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=42$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-1) + 5\,a(n-2) - a(n-4)
\]
...
model reproduces $27$ published values, $62$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A238654.
