...
\title{The empirical column recurrences for the table OEIS A264190}
...
\begin{abstract}
OEIS A264190 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264190 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having index change +-(.,.) 0,0 1,-1 or 2,2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 4, 4, 8, 24, 8, 16, 81,\ \dots
\]
...
k=1: a(n) = 2*a(n-1)\\
k=2: [order 14]\\
k=3: a(n) = 10*a(n-1) for n>4\\
k=4: [order 13]\\
k=5: [order 58]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 11:10:09, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having index change +-(.,.) 0,0 1,-1 or 2,2.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264190.
