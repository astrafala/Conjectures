...
\title{The conjectured generating function for OEIS A252971, proved}
...
\begin{abstract}
OEIS A252971 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=48$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $70$ coefficients in exact arithmetic
...
OEIS A252971 is ``Number of nX3 nonnegative integer arrays with upper left 0 and lower right n+3-5 and value increasing by 0 or 1 with every step right or down.''. It has offset $1$ and begins
\[
0, 1, 18, 153, 770, 2859,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.: x\^{}2*(1+8*x+18*x\^{}2-70*x\^{}3+94*x\^{}4-78*x\^{}5+43*x\^{}6-14*x\^{}7+2*x\^{}8) / (x-1)\^{}10. - \_R. J. Mathar\_, Nov 23 2015
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 23 2015 05:53:12, revision 9) this is still
...
\]
and there are $S=48$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $58$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{58}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $70$
of them --- more than the $58+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252971.
