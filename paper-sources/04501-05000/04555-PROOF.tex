...
\title{The conjectured generating function for OEIS A277866, proved}
...
\begin{abstract}
OEIS A277866 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (1+x-4*x\^{}2+8*x\^{}3) / ((1-x)*(1+x)*(1-2*x)*(1+2*x)).
\end{quote}
...
The model reproduces all $43$ terms the entry publishes, exactly, in integer arithmetic:
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A277866.
