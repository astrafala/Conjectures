...
\title{The empirical column recurrences for the table OEIS A279657}
...
\begin{abstract}
OEIS A279657 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A279657 is ``T(n,k) = Number of n X k 0..2 arrays with no element equal to a strict majority of its horizontal and antidiagonal neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 1, 0, 0, 3, 0, 3, 27,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 01 2021 16:40:19, revision 6) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of 1 X n 0..2 arrays with no element equal to a strict majority of its horizontal and antidiagonal neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards.''. Its rows are the vertices of a digraph on
$S=46$ vertices, edges being the admissible consecutive pairs, and the count is
...
model reproduces $9$ published values, $15$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A279657.
