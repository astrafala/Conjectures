...
\title{The empirical column recurrences for the table OEIS A302820}
...
\begin{abstract}
OEIS A302820 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A302820 is ``T(n,k) = number of n X k 0..1 arrays with every element equal to 0, 1, 2, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 4, 8, 4, 8, 20,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 08 2025 14:38:35, revision 7) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of 1 X n 0..1 arrays with every element equal to 0, 1, 2, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of 2 X n 0..1 arrays with every element equal to 0, 1, 2, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
model reproduces $18$ published values, $39$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302820.
