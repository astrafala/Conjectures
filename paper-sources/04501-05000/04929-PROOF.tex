...
\title{The empirical column recurrences for the table OEIS A203887}
...
\begin{abstract}
OEIS A203887 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A203887 is ``T(n,k)=Number of (n+1)X(k+1) 0..6 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. It has offset $1$, is
read by antidiagonals, and begins
\[
637, 8281, 8281, 107653, 254359, 107653, 1399489, 7501753,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for columns:\\
k=1: a(n) = 49*13\^{}n\\
k=2: a(n) = 25*a(n-1) +138*a(n-2)\\
k=3: a(n) = 79*a(n-1) -450*a(n-2) -144*a(n-3)\\
k=4: a(n) = 115*a(n-1) +17016*a(n-2) -763308*a(n-3) -1177416*a(n-4) +49714560*a(n-5) +35870688*a(n-6) -153031680*a(n-7)\\
k=5: (order 12 recurrence)\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:49:19, revision 9) these are still
...
The sequence is ``Number of (n+1)X(3) 0..6 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. Its rows are the vertices of a digraph on
$S=343$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;25\,a(n-1) + 138\,a(n-2)
\]
holds for every $n>1$.

\subsection*{Column $k=3$}
The sequence is ``Number of (n+1)X(4) 0..6 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. Its rows are the vertices of a digraph on
$S=2401$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;79\,a(n-1) - 450\,a(n-2) - 144\,a(n-3)
\]
holds for every $n>2$.
...
different counts. Across the columns settled here the model reproduces $14$ published
values, $145$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203887.
