...
\title{The conjectured generating function for OEIS A250775, proved}
...
\begin{abstract}
OEIS A250775 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $266$ coefficients in exact arithmetic
...
OEIS A250775 is ``Number of (7+1) X (n+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
576, 1026, 1622, 2396, 3412, 4798,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: 2*x*(288 - 927*x + 838*x\^{}2 - 256*x\^{}3) / ((1 - x)\^{}3*(1 - 2*x)). - \_Colin Barker\_, Nov 19 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 19 2018 09:13:52, revision 8) this is still
...
\]
a polynomial of degree at most $260$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{260}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $266$
of them --- more than the $260+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{4}$ read off $D_2$, the first few being $5,\ -9,\ 7,\ -2$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250775.
