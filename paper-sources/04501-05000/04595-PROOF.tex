...
\title{The empirical column recurrences for the table OEIS A303800}
...
\begin{abstract}
OEIS A303800 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A303800 is ``T(n,k) = Number of n X k 0..1 arrays with every element unequal to 0, 1, 2 or 4 king-move adjacent elements, with upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 4, 4, 4, 8, 12,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 31 2023 15:36:53, revision 6) these are still
...
column $k$$=1$ & $1$ & $4$ & $>2$ & $11$ \\
column $k$$=2$ & $5$ & $16$ & $>6$ & $10$ \\
column $k$$=3$ & $7$ & $64$ & $>10$ & $9$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of n X 1 0..1 arrays with every element unequal to 0, 1, 2 or 4 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 2 0..1 arrays with every element unequal to 0, 1, 2 or 4 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 3 0..1 arrays with every element unequal to 0, 1, 2 or 4 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
model reproduces $30$ published values, $70$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303800.
