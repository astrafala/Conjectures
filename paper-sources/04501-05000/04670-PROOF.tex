...
\title{The empirical product formula for OEIS A140914, proved}
...
\begin{abstract}
OEIS A140914 counts $5\times8$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $5\times8\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $40$. The entry's own expression, cleared of its denominator,
...
OEIS A140914 is ``Number of 5 X 8 matrices with elements in 0..n with each row and each column in nondecreasing order. 5,8,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 1287, 429429, 61408347, 4789851066, 235234907908,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2025 11:09:17, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $5\times8$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$5\times8$ rectangle --- a plane partition in a $5\times8\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{5}\prod_{j=1}^{8}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $40$ with rational coefficients.
\end{theorem}
...
$5\cdot8=40$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,8,5)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $50$ and $10$ respectively.
\end{lemma}

\begin{proof}
With $s=n+8+5-1$ one has $s-(n+i)=8+5-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{8+5-1-i}
=\frac{(s-8-5+2+i)(s-8-5+3+i)\cdots s}{(8+5-1-i)!},
\]
a polynomial in $n$ of degree $8+5-1-i$. Multiplying over $i=0,\dots,5-1$ gives
$\deg N=\sum_{i=0}^{5-1}(8+5-1-i)=50$. The denominator is
...
of degree at most $40+10=50$. It was evaluated in exact rational arithmetic at
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A140914.
