...
\title{The empirical column recurrences for the table OEIS A222944}
...
\begin{abstract}
OEIS A222944 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A222944 is ``T(n,k) = Number of n X k 0..4 arrays with no element equal to another at a city block distance of exactly two, and new values 0..4 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 3, 7, 3, 7, 33,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 21:01:54, revision 8) these are still
...
column $k$$=1$ & $3$ & $75$ & $>5$ & $9$ \\
column $k$$=2$ & $2$ & $644$ & $>4$ & $8$ \\
column $k$$=3$ & $1$ & $4387$ & $>3$ & $7$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of n X 1 0..4 arrays with no element equal to another at a city block distance of exactly two, and new values 0..4 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=75$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of n X 2 0..4 arrays with no element equal to another at a city block distance of exactly two, and new values 0..4 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=644$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of n X 3 0..4 arrays with no element equal to another at a city block distance of exactly two, and new values 0..4 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=4387$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;14\,a(n-1)
...
model reproduces $24$ published values, $77$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222944.
