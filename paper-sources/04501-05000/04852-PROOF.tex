...
\title{The empirical column recurrences for the table OEIS A282056}
...
\begin{abstract}
OEIS A282056 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A282056 is ``T(n,k) = Number of n X k 0..2 arrays with no element unequal to more than four of its king-move neighbors and with new values introduced in order 0 sequentially upwards.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 5, 14, 5, 14, 91,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 03 2021 12:59:42, revision 6) these are still
...
The sequence is ``Number of n X 1 0..2 arrays with no element unequal to more than four of its king-move neighbors and with new values introduced in order 0 sequentially upwards.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 2 0..2 arrays with no element unequal to more than four of its king-move neighbors and with new values introduced in order 0 sequentially upwards.''. Its rows are the vertices of a digraph on
...
model reproduces $17$ published values, $51$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A282056.
