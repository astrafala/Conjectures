...
\title{The empirical recurrence for OEIS A224129, proved by transfer matrix}
...
\begin{abstract}
OEIS A224129 counts arrays over $\{0,\dots,1\}$ whose rows, columns, diagonals or
antidiagonals are required to be monotone or unimodal, and carries an empirical recurrence of
order $4$ contributed by R.~H.~Hardin. A monotone condition is local, but a unimodal one
...
OEIS A224129 is ``Number of n X 4 0..1 arrays with rows nondecreasing and antidiagonals unimodal.''. It has offset $1$ and begins
\[
5, 25, 106, 419, 1635, 6392, 25047, 98221,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) - 5*a(n-2) + 2*a(n-3) + 4*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 27 2018 06:04:19, revision 11) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 5\,a(n-2) + 2\,a(n-3) + 4\,a(n-4)
\end{equation}
for all $n>4$.
...
$h_j=G^{j}N^{o}\mathbf{1}$, we have for $j\ge4$
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224129.
