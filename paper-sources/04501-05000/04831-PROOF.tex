...
\title{The empirical recurrence for OEIS A223182, proved by transfer matrix}
...
\begin{abstract}
OEIS A223182 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$4$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is icosahedron graph. The entry carries an empirical recurrence of order $5$
...
single slice is a state and the count is a walk count on a finite digraph with $S=896$ vertices.
\end{abstract}
...
OEIS A223182 is ``Rolling icosahedron footprints: number of n X 4 0..11 arrays starting with 0 where 0..11 label vertices of an icosahedron and every array movement to a horizontal, vertical or antidiagonal neighbor moves along an icosahedral edge.''. It has offset $1$ and begins
\[
125, 320, 2080, 14560, 103520, 738720, 5274720, 37664800,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 10*a(n-1) - 25*a(n-2) + 36*a(n-3) - 24*a(n-4) + 4*a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 17 2018 09:21:53, revision 8) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=896.
\]
...
\begin{theorem}
\[
a(n)\;=\;10\,a(n-1) - 25\,a(n-2) + 36\,a(n-3) - 24\,a(n-4) + 4\,a(n-5)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223182.
