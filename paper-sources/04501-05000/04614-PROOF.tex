...
\title{The empirical column recurrences for the table OEIS A230940}
...
\begin{abstract}
OEIS A230940 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A230940 is ``T(n,k)=Number of white-square subarrays of (n+2)X(k+2) 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 2, 2, 2, 6, 2, 8, 16,\ \dots
\]
...
k=6: [order 71]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:07:44, revision 6) these are still
...
column $k$$=1$ & $4$ & $61$ & $>3$ & $10$ \\
column $k$$=2$ & $3$ & $157$ & $>1$ & $9$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of white-square subarrays of (n+2)X3 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. Its rows are the vertices of a digraph on
$S=61$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of white-square subarrays of (n+2)X4 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. Its rows are the vertices of a digraph on
$S=157$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $19$ published values, $44$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230940.
