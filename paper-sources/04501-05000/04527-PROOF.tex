...
\title{The empirical recurrence for OEIS A230971, proved by transfer matrix}
...
\begin{abstract}
OEIS A230971 counts arrays of width $4$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
the count is a walk count on $S=5841$ states.
\end{abstract}
...
OEIS A230971 is ``Number of (n+2) X (2+2) 0..3 arrays x(i,j) with each element diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
16, 144, 1024, 9216, 85264, 767376, 6885376, 61968384,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 9*a(n-1) - 8*a(n-2) + 72*a(n-3) + 9*a(n-4) - 81*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 25 2018 08:53:08, revision 8) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=5841.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230971.
