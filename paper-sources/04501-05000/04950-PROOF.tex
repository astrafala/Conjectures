...
\title{The conjectured generating function for OEIS A250752, proved}
...
\begin{abstract}
OEIS A250752 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250752 is ``Number of (n+1) X (5+1) 0..2 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing x(i,j)+x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
294, 957, 3066, 9633, 29814, 91317,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: 3*x*(98 - 269*x + 186*x\^{}2) / ((1 - x)*(1 - 2*x)*(1 - 3*x)). - \_Colin Barker\_, Nov 17 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 17 2018 20:55:09, revision 8) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250752.
