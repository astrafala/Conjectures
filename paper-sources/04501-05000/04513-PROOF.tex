...
\title{The empirical column recurrences for the table OEIS A297595}
...
\begin{abstract}
OEIS A297595 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A297595 is ``T(n,k) = Number of n X k 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 1 or 5 neighboring 1s.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 1, 3, 5, 1, 4, 9,\ \dots
\]
...
k=7: [order 83]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 01 2019 14:54:00, revision 6) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of 1 X n 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 1 or 5 neighboring 1s.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of 2 X n 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 1 or 5 neighboring 1s.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=3$}
The sequence is ``Number of 3 X n 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 1 or 5 neighboring 1s.''. Its rows are the vertices of a digraph on
$S=12$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + a(n-2) + 8\,a(n-3) + 7\,a(n-4) - 8\,a(n-6) - 6\,a(n-7)
\]
...
model reproduces $30$ published values, $62$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A297595.
