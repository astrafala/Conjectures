...
\title{The conjectured generating function for OEIS A282417, proved}
...
\begin{abstract}
OEIS A282417 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A282417 is ``Decimal representation of the x-axis, from the left edge to the origin, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 469", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 2, 3, 12, 7, 56,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 + 2*x - 4*x\^{}2 - 2*x\^{}3) / ((1 - x)*(1 + x)*(1 - 2*x)*(1 + 2*x)*(1 - 2*x\^{}2)).
\end{quote}
...
\]
and there are $S=8$ states. The digraph is built by reading the entry's own English, and the
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=3$ and $\deg D_2=6$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $13$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{13}$ then $R$ has a zero of order greater than its own
...
of them --- more than the $13+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{6}$ read off $D_2$, the first few being $0,\ 7,\ 0,\ -14,\ 0,\ 8$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A282417.
