...
\title{The empirical column recurrences for the table OEIS A237324}
...
\begin{abstract}
OEIS A237324 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A237324 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with the maximum plus the upper median plus the minimum minus the lower median of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 348, 348, 1508, 2108, 1508, 6608, 12756,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 356*a(n-4) for n>5\\
k=2: a(n) = 1400*a(n-4) -6519*a(n-8) +126*a(n-12) for n>14\\
k=3: [order 32] for n>35\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:04:53, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with the maximum plus the upper median plus the minimum minus the lower median of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237324.
