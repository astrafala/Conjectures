...
\title{The empirical column recurrences for the table OEIS A206094}
...
\begin{abstract}
OEIS A206094 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A206094 is ``T(n,k) = number of (n+1) X (k+1) 0..2 arrays with every 2 X 3 or 3 X 2 subblock having exactly two counterclockwise and two clockwise edge increases.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 270, 270, 972, 780, 972, 3564, 2016,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:28:06, revision 9) these are still
...
column $k$$=1$ & $4$ & $81$ & $>4$ & $8$ \\
column $k$$=2$ & $4$ & $270$ & $>6$ & $7$ \\
column $k$$=3$ & $3$ & $972$ & $>6$ & $6$ \\
column $k$$=4$ & $3$ & $3564$ & $>7$ & $5$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1) X2 0..2 arrays with every 2 X 3 or 3 X 2 subblock having exactly two counterclockwise and two clockwise edge increases.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X3 0..2 arrays with every 2 X 3 or 3 X 2 subblock having exactly two counterclockwise and two clockwise edge increases.''. Its rows are the vertices of a digraph on
$S=270$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of (n+1) X4 0..2 arrays with every 2 X 3 or 3 X 2 subblock having exactly two counterclockwise and two clockwise edge increases.''. Its rows are the vertices of a digraph on
$S=972$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;18\,a(n-3)
...
The sequence is ``Number of (n+1) X5 0..2 arrays with every 2 X 3 or 3 X 2 subblock having exactly two counterclockwise and two clockwise edge increases.''. Its rows are the vertices of a digraph on
...
model reproduces $26$ published values, $114$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206094.
