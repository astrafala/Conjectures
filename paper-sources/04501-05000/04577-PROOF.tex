...
\title{The empirical column recurrences for the table OEIS A206215}
...
\begin{abstract}
OEIS A206215 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A206215 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every 2X3 or 3X2 subblock having exactly two equal edges, and new values 0..2 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
14, 45, 45, 162, 130, 162, 594, 336,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 4*a(n-1) -a(n-2) +12*a(n-3) -36*a(n-4) for n>5\\
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:47:57, revision 9) these are still
...
column $k$$=1$ & $4$ & $98$ & $>5$ & $9$ \\
column $k$$=2$ & $4$ & $300$ & $>6$ & $8$ \\
column $k$$=3$ & $3$ & $1034$ & $>6$ & $7$ \\
column $k$$=4$ & $3$ & $3698$ & $>7$ & $6$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X2 0..2 arrays with every 2X3 or 3X2 subblock having exactly two equal edges, and new values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=98$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-1) - a(n-2) + 12\,a(n-3) - 36\,a(n-4)
\]
holds from index $5+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+1)X3 0..2 arrays with every 2X3 or 3X2 subblock having exactly two equal edges, and new values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X4 0..2 arrays with every 2X3 or 3X2 subblock having exactly two equal edges, and new values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=1034$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;18\,a(n-3)
...
The sequence is ``Number of (n+1)X5 0..2 arrays with every 2X3 or 3X2 subblock having exactly two equal edges, and new values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=3698$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;34\,a(n-3)
...
model reproduces $30$ published values, $115$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206215.
