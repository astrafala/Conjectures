...
\title{The empirical column recurrences for the table OEIS A255228}
...
\begin{abstract}
OEIS A255228 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A255228 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 0 and no antidiagonal sum 0 and no row sum 2 and no column sum 2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
23, 22, 22, 31, 28, 31, 43, 37,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:41:02, revision 6) these are still
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with no 3x3 subblock diagonal sum 0 and no antidiagonal sum 0 and no row sum 2 and no column sum 2.''. Its rows are the vertices of a digraph on
...
model reproduces $11$ published values, $27$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A255228.
