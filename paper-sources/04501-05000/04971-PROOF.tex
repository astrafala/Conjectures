...
\title{The empirical column recurrences for the table OEIS A283691}
...
\begin{abstract}
OEIS A283691 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A283691 is ``T(n,k) = Number of n X k 0..1 arrays with no 1 equal to more than two of its horizontal, diagonal and antidiagonal neighbors.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 4, 4, 8, 16, 8, 16, 49,\ \dots
\]
...
k=6: [order 83].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 03 2022 21:13:12, revision 6) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of 1 X n 0..1 arrays with no 1 equal to more than two of its horizontal, diagonal and antidiagonal neighbors.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of 2 X n 0..1 arrays with no 1 equal to more than two of its horizontal, diagonal and antidiagonal neighbors.''. Its rows are the vertices of a digraph on
$S=20$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1) + a(n-2) + 3\,a(n-3) - 6\,a(n-4) - 8\,a(n-5)
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A283691.
