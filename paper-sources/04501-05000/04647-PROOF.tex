...
\title{The empirical column recurrences for the table OEIS A204076}
...
\begin{abstract}
OEIS A204076 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A204076 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..2 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
8, 38, 38, 188, 329, 188, 938, 2882,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:58:57, revision 9) these are still
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(4) 0..2 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=98$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;20\,a(n-1) - 73\,a(n-2) + 86\,a(n-3) - 32\,a(n-4)
\]
...
model reproduces $13$ published values, $70$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204076.
