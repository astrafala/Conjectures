...
\title{The empirical column recurrences for the table OEIS A235911}
...
\begin{abstract}
OEIS A235911 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A235911 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with the minimum plus the maximum unequal to the lower median plus the upper median in every 2X2 subblock.''. It has offset $1$, is
read by antidiagonals, and begins
\[
48, 260, 260, 1408, 2618, 1408, 7632, 26408,\ \dots
\]
...
k=6: [order 62]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:56:02, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..2 arrays with the minimum plus the maximum unequal to the lower median plus the upper median in every 2X2 subblock.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..2 arrays with the minimum plus the maximum unequal to the lower median plus the upper median in every 2X2 subblock.''. Its rows are the vertices of a digraph on
$S=27$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;8\,a(n-1) + 58\,a(n-2) - 368\,a(n-3) - 24\,a(n-4) + 192\,a(n-5)
\]
...
model reproduces $17$ published values, $101$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235911.
