...
\title{The empirical recurrence for OEIS A208316, proved by counting patterns}
...
\begin{abstract}
OEIS A208316 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $14$ to $S=5$. The entry's empirical recurrence of order
...
OEIS A208316 is ``Number of n X 2 0..2 arrays with new values 0..2 introduced in row major order and no element equal to more than two of its immediate leftward or upward or left-upward diagonal neighbors.''. It has offset $1$ and begins
\[
2, 13, 113, 982, 8534, 74164, 644516, 5601112,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 8*a(n-1) + 6*a(n-2) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 02 2018 04:53:43, revision 8) the statement is
...
Assembling the $3$ lumped chains into one block-diagonal matrix $N$ of size $S=5$ and
...
\end{equation}
The unlumped alphabet had $14$ rows in all; the lumped chain has $S=5$ states.
...
\eqref{eq:walk}, divided by $3!$, was evaluated at every one of the $19$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208316.
