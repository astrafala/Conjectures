...
\title{The empirical column recurrences for the table OEIS A263060}
...
\begin{abstract}
OEIS A263060 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A263060 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with each row and column not divisible by 3, read as a binary number with top and left being the most significant bits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 2, 2, 10, 33, 10, 10, 142,\ \dots
\]
...
k=7: [order 21]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 08 2015 15:02:03, revision 4) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with each row and column not divisible by 3, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..1 arrays with each row and column not divisible by 3, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
model reproduces $19$ published values, $64$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263060.
