...
\title{The conjectured generating function for OEIS A207021, proved}
...
\begin{abstract}
OEIS A207021 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=183$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $194$ coefficients in exact arithmetic
...
OEIS A207021 is ``Number of nX5 0..1 arrays avoiding 0 0 0 and 0 1 1 horizontally and 0 0 1 and 1 0 1 vertically.''. It has offset $1$ and begins
\[
13, 169, 624, 1586, 3315, 6123,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical G.f.: 13*x*(1+8*x-7*x\^{}2+2*x\^{}3)/(1-x)\^{}5. [\_Colin Barker\_, May 22 2012]
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 13:23:54, revision 12) this is still
...
\]
and there are $S=183$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $187$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{187}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $194$
of them --- more than the $187+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207021.
