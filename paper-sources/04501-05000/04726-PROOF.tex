...
\title{The empirical recurrence for OEIS A240513, proved by transfer matrix}
...
\begin{abstract}
OEIS A240513 counts arrays of width $2$ over $\{0,\dots,1\}$ under a condition on each cell
...
a walk count on $S=22$ of them, and the conjectured recurrence is then settled by a finite
exact computation.
\end{abstract}
...
OEIS A240513 is ``Number of n X 2 0..1 arrays with no element equal to exactly two horizontal and vertical neighbors, with new values 0..1 introduced in row major order.''. It has offset $1$ and begins
\[
2, 3, 6, 10, 21, 42, 86, 179,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + a(n-2) - a(n-3) - 2*a(n-4) + a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 09 2018 05:08:19, revision 11) this is still
...
terms. There are $S=22$ states, and $a$ is $C$-finite.
...
first term. There are $2^{2}=4$ possible rows over the entry's own alphabet.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A240513.
