...
\title{The empirical column recurrences for the table OEIS A208085}
...
\begin{abstract}
OEIS A208085 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A208085 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with every 2X2 subblock having the same number of equal edges as its horizontal neighbors and a different number from its vertical neighbors, and new values 0..1 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
8, 20, 12, 56, 20, 24, 164, 32,\ \dots
\]
...
k=1: a(n) = 3*a(n-2)\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 14:38:00, revision 10) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with every 2X2 subblock having the same number of equal edges as its horizontal neighbors and a different number from its vertical neighbors, and new values 0..1 introduced in row major order.''. Its rows are the vertices of a digraph on
...
model reproduces $9$ published values, $21$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208085.
