...
\title{The empirical column recurrences for the table OEIS A252243}
...
\begin{abstract}
OEIS A252243 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 4$.
\end{abstract}
...
OEIS A252243 is ``T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and column sum not equal to 1 3 4 6 or 7 and every 3X3 diagonal and antidiagonal sum equal to 1 3 4 6 or 7.''. It has offset $1$, is
read by antidiagonals, and begins
\[
342, 858, 858, 2174, 3896, 2174, 4708, 16056,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:58:32, revision 6) these are still
...
column $k$$=1$ & $6$ & $151$ & $>8$ & $8$ \\
column $k$$=4$ & $6$ & $44889$ & $>7$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X(3) 0..3 arrays with every 3X3 subblock row and column sum not equal to 1 3 4 6 or 7 and every 3X3 diagonal and antidiagonal sum equal to 1 3 4 6 or 7.''. Its rows are the vertices of a digraph on
$S=151$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;18\,a(n-3) - 32\,a(n-6)
\]
holds from index $8+1$ on.

\subsection*{Column $k$$=4$}
The sequence is ``Number of (n+2)X(6) 0..3 arrays with every 3X3 subblock row and column sum not equal to 1 3 4 6 or 7 and every 3X3 diagonal and antidiagonal sum equal to 1 3 4 6 or 7.''. Its rows are the vertices of a digraph on
$S=44889$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $18$ published values, $114$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252243.
