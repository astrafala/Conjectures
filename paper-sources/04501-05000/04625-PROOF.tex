...
\title{The empirical recurrence for OEIS A230813, proved by transfer matrix}
...
\begin{abstract}
OEIS A230813 counts arrays of width $2$ over $\{0,1,2\}$ in which every cell must see, among its
...
the count is a walk count on $S=64$ states.
\end{abstract}
...
OEIS A230813 is ``Number of n X 2 0..2 arrays x(i,j) with each element horizontally or vertically next to at least one element with value (x(i,j)+1) mod 3 and at least one element with value (x(i,j)-1) mod 3, and upper left element zero.''. It has offset $1$ and begins
\[
0, 0, 2, 0, 2, 12, 18, 56,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + a(n-2) + 4*a(n-3) - 4*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 17 2018 07:10:13, revision 9) this is still
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + a(n-2) + 4\,a(n-3) - 4\,a(n-4)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230813.
