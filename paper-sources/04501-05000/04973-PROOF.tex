...
\title{The empirical recurrence for OEIS A198287, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A198287 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
order $3$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A198287 is ``Number of 2n X 2 0..3 arrays with values 0..3 introduced in row major order and each element equal to exactly one horizontal and vertical neighbor.''. It has offset $1$ and begins
\[
2, 17, 272, 4667, 80702, 1397477, 24207692, 419373647,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 23*a(n-1) - 103*a(n-2) + 81*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 02 2018 06:27:04, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;23\,a(n-1) - 103\,a(n-2) + 81\,a(n-3)
\end{equation}
for all $n>3$.

\section{The condition, and what the canonical-form clause counts}

The arrays have $2$ columns and $L=2n+0$ rows. Write $x_{t,u}$ for the entry in
...
index,
\[
4!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-3}\,
\Bigl(h_{3}-\sum_i c_i h_{3-i}\Bigr)
\]
...
$w=h_{3}-\sum_i c_ih_{3-i}$; and it is enough to check $m=0,1,\dots,S-1$.
\end{lemma}
...
$3$ applications of $G$ in exact integer arithmetic and $\mathbf{v}^{\!\top}G^{m}w$
evaluated for $m=0,1,\dots$, again exactly. Every one of those integers vanishes from the
index corresponding to $n=3+1$ onwards, and the run was continued until $S+1$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198287.
