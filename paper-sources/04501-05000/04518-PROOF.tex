...
\title{Arrays counted up to renaming: the empirical recurrence for OEIS A206163}
...
\begin{abstract}
OEIS A206163 carries an empirical recurrence of order $6$ contributed by R.~H.~Hardin. It is
...
both in the state the arrays are exactly the walks in a digraph on $S=14$ vertices, so $a(n)$
...
OEIS A206163 is ``Number of (n+1) X 3 0..3 arrays with every 2 X 2 subblock in a row having an equal number of equal diagonal or equal antidiagonal elements, adjacent rows differing in this number, and new values 0..3 introduced in row major order.''. It has offset $1$ and begins
\[
85, 1070, 17142, 238182, 3814022, 53494742, 850283062, 12012206406,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 13 2018 14:08:56, revision 8) this is still
...
What is compared \emph{across} windows is carried too: the state holds the single number the last band scored, since a band is constant and only that one number is compared with the next. With $2$ subblocks to a band that is a bounded amount of bookkeeping and no more.
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 217\,a(n-2) - 33\,a(n-3) - 1320\,a(n-4) - 432\,a(n-5) + 1152\,a(n-6)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206163.
