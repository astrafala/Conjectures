...
\title{The empirical column recurrences for the table OEIS A261265}
...
\begin{abstract}
OEIS A261265 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A261265 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 00100101 or 01010101.''. It has offset $1$, is
read by antidiagonals, and begins
\[
36, 41, 41, 91, 52, 91, 166, 110,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1) +a(n-2) +2*a(n-4) +a(n-6) for n>7\\
k=2: a(n) = a(n-1) +2*a(n-3) +a(n-4) +2*a(n-5) for n>6\\
k=3: [order 12] for n>13\\
k=4: [order 16] for n>17\\
k=5: [order 25] for n>28\\
k=6: [order 31] for n>36\\
k=7: [order 55] for n>62\\
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 13 2015 06:40:25, revision 4) these are still
...
column $k$$=1$ & $6$ & $64$ & $>7$ & $10$ \\
column $k$$=2$ & $5$ & $256$ & $>6$ & $9$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 00100101 or 01010101.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;a(n-1) + a(n-2) + 2\,a(n-4) + a(n-6)
\]
holds from index $7+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+2)X(4) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 00100101 or 01010101.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + 2\,a(n-3) + a(n-4) + 2\,a(n-5)
\]
...
model reproduces $19$ published values, $59$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A261265.
