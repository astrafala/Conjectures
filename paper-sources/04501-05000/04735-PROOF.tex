...
\title{The conjectured generating function for OEIS A208375, proved}
...
\begin{abstract}
OEIS A208375 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=111$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $120$ coefficients in exact arithmetic
...
OEIS A208375 is ``Number of n X 4 0..1 arrays avoiding 0 0 0 and 1 1 1 horizontally and 0 0 1 and 1 0 1 vertically.''. It has offset $1$ and begins
\[
10, 100, 240, 420, 640, 900,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: 10*x*(1 + 7*x - 3*x\^{}2 - x\^{}3) / (1 - x)\^{}3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 02 2018 05:45:47, revision 9) this is still
...
\]
and there are $S=111$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $115$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{115}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $120$
of them --- more than the $115+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208375.
