...
\title{The empirical column recurrences for the table OEIS A234832}
...
\begin{abstract}
OEIS A234832 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234832 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with each 2X2 subblock having the number of clockwise edge increases less than or equal to the number of counterclockwise edge increases.''. It has offset $1$, is
read by antidiagonals, and begins
\[
69, 516, 516, 3843, 9207, 3843, 28602, 161631,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 9*a(n-1) -12*a(n-2) +3*a(n-3)\\
k=2: a(n) = 23*a(n-1) -102*a(n-2) +93*a(n-3) -9*a(n-4)\\
k=3: [order 13]\\
k=4: [order 28]\\
k=5: [order 92]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:28:34, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..2 arrays with each 2X2 subblock having the number of clockwise edge increases less than or equal to the number of counterclockwise edge increases.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..2 arrays with each 2X2 subblock having the number of clockwise edge increases less than or equal to the number of counterclockwise edge increases.''. Its rows are the vertices of a digraph on
$S=28$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;23\,a(n-1) - 102\,a(n-2) + 93\,a(n-3) - 9\,a(n-4)
\]
...
model reproduces $13$ published values, $72$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234832.
