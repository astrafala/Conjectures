...
\title{The conjectured generating function for OEIS A250659, proved}
...
\begin{abstract}
OEIS A250659 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250659 is ``Number of (5+1) X (n+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
159, 286, 445, 636, 859, 1114,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(159 - 191*x + 64*x\^{}2) / (1 - x)\^{}3.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 15 2018 15:04:28, revision 7) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250659.
