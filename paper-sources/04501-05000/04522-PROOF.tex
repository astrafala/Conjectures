...
\title{The empirical column recurrences for the table OEIS A229510}
...
\begin{abstract}
OEIS A229510 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A229510 is ``T(n,k) = number of defective 3-colorings of an n X k 0..2 array connected diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 0, 0, 0, 6, 0, 0, 48,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 27 2021 21:06:20, revision 7) these are still
...
column $k$$=1$ & $1$ & $12$ & $>2$ & $10$ \\
column $k$$=2$ & $2$ & $28$ & $>2$ & $9$ \\
column $k$$=3$ & $2$ & $72$ & $>3$ & $7$ \\
column $k$$=4$ & $2$ & $196$ & $>4$ & $6$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of defective 3-colorings of an n X 1 0..2 array connected diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of defective 3-colorings of an n X 2 0..2 array connected diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of defective 3-colorings of an n X 3 0..2 array connected diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of defective 3-colorings of an n X 4 0..2 array connected diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
$S=196$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $32$ published values, $96$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229510.
