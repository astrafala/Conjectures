...
\title{The empirical column recurrences for the table OEIS A264285}
...
\begin{abstract}
OEIS A264285 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264285 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 0,0 0,1 1,0 or -1,-2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 4, 1, 8, 10, 1, 16, 32,\ \dots
\]
...
k=7: [order 34]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 14:56:25, revision 4) these are still
...
column $k$$=1$ & $1$ & $16$ & $>1$ & $9$ \\
column $k$$=2$ & $4$ & $64$ & $>2$ & $8$ \\
column $k$$=3$ & $3$ & $256$ & $>1$ & $7$ \\
column $k$$=4$ & $6$ & $1024$ & $>4$ & $10$ \\
row $n$$=1$ & $6$ & $256$ & $>4$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 0,0 0,1 1,0 or -1,-2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having directed index change 0,0 0,1 1,0 or -1,-2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(4) arrays of permutations of 0..(n+1)*(4)-1 with each element having directed index change 0,0 0,1 1,0 or -1,-2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(5) arrays of permutations of 0..(n+1)*(5)-1 with each element having directed index change 0,0 0,1 1,0 or -1,-2.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having directed index change 0,0 0,1 1,0 or -1,-2.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $44$ published values, $128$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264285.
