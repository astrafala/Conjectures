...
\title{The empirical closed form for OEIS A188864, proved}
...
\begin{abstract}
OEIS A188864 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=128$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A188864 is ``Number of n X 7 binary arrays without the pattern 0 1 diagonally, vertically or antidiagonally.''. It has offset $1$ and begins
\[
128, 577, 1220, 1931, 2658, 3387, 4116, 4845,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 729*n - 987 for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (May 01 2018 09:50:02, revision 9) this is still
...
walk, so with $M$ the adjacency matrix of that digraph on $S=128$ vertices, and $u,w$ the
...
$n=7,\dots,8$ and agree at each. A monic recurrence of order $2$
determines $a(m)$ from $a(m-1),\dots,a(m-2)$, and for every $m\ge9$ both
$m>6$ and $m-2\ge7$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge7$.
Below $n=7$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=7$ on. The remaining indices $n=5,\dots,6$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188864.
