...
\title{The empirical column recurrences for the table OEIS A264655}
...
\begin{abstract}
OEIS A264655 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264655 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 1,1 2,2 -1,0 or 0,-1.''. It has offset $1$, is
...
k=1: a(n) = a(n-3)\\
k=2: a(n) = 2*a(n-1) +2*a(n-5) -2*a(n-6) +4*a(n-7) -a(n-10)\\
k=3: [order 15]\\
k=4: [order 18] for n>21\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 20 2015 06:59:22, revision 4) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 1,1 2,2 -1,0 or 0,-1.''. Its rows are the vertices of a digraph on
...
model reproduces $11$ published values, $11$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264655.
