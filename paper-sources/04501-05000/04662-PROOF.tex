...
\title{The empirical recurrence for OEIS A231413, proved by transfer matrix}
...
\begin{abstract}
OEIS A231413 counts $(n+1)\times2$ arrays over $\{0,\dots,2\}$ subject to a condition imposed on
...
OEIS A231413 is ``Number of (n+1) X (1+1) 0..2 arrays with no element equal to a strict majority of its horizontal, diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
9, 50, 285, 1617, 9188, 52193, 296511, 1684466,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) - 11*a(n-3) + 4*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 21 2018 09:43:32, revision 8) this is still
...
integers vanish from the index corresponding to $n=4+1$ onwards, and the run was
continued until the working vector was identically zero, which settles all larger $j$ at once.
By Lemma~\ref{lem:crit} the recurrence holds for every $n>4$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231413.
