...
\title{The empirical recurrence for OEIS A203446, proved by transfer matrix}
...
\begin{abstract}
OEIS A203446 counts arrays of width $3$ over $\{0,1\}$ through two derived arrays of PAIR SUMS ---
each cell added to its left neighbour, and each cell added to the one above it --- on which the
entry imposes an order condition. The entry carries an empirical recurrence of order $5$
contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical: the count is
a walk count on $S=23$ states.
\end{abstract}
...
OEIS A203446 is ``Number of (n+1) X 3 0..1 arrays with column and row pair sums b(i,j)=a(i,j)+a(i,j-1) and c(i,j)=a(i,j)+a(i-1,j) such that rows of b(i,j) and columns of c(i,j) are lexicographically nondecreasing.''. It has offset $1$ and begins
\[
16, 34, 80, 174, 376, 786, 1624, 3310,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) -4*a(n-2) -2*a(n-3) +5*a(n-4) -2*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 04 2018 09:22:41, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array $a$ has $3$ cells across, with entries in $\{0,1\}$. Two derived arrays are formed,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203446.
