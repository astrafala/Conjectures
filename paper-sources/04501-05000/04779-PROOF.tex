...
\title{The empirical column recurrences for the table OEIS A206238}
...
\begin{abstract}
OEIS A206238 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A206238 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with every 2X3 or 3X2 subblock having exactly two equal edges, and new values 0..3 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
15, 60, 60, 310, 256, 310, 1640, 1136,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:49:25, revision 9) these are still
...
The sequence is ``Number of (n+1)X4 0..3 arrays with every 2X3 or 3X2 subblock having exactly two equal edges, and new values 0..3 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=8102$ vertices, edges being the admissible consecutive pairs, and the count is
...
model reproduces $6$ published values, $28$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206238.
