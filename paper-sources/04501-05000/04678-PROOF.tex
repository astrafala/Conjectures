...
\title{The empirical column recurrences for the table OEIS A234175}
...
\begin{abstract}
OEIS A234175 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234175 is ``T(n,k) is the number of (n+1) X (k+1) 0..6 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
448, 4536, 4536, 47936, 70486, 47936, 517104, 1173940,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 154*a(n-2) -4144*a(n-4) +16176*a(n-6).\\
k=2: [order 35].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 21:02:45, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..6 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
$S=49$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;154\,a(n-2) - 4144\,a(n-4) + 16176\,a(n-6)
\]
...
The entry gives only the order for this line: ``k=2: [order 35].''. Its model is a walk on
$S=343$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=686$ exact terms returns the minimal one, of order
$35$ --- the stated order --- with integer coefficients
\[
(c_1,\dots,c_{35})=(33,\ 323,\ -20479,\ 66827,\ 4196961,\ -32408967,\ -381895309,\ 4395837462,\ 15113145790,\ -293548976068,\ -51289577932,\ 10922776315912,\ -16766805352904,\ -235952101172000,\ 626750889441184,\ 2967198544937088,\ -10881957631245888,\ -20625630149777664,\ 106077230575696640,\ 62012982701743616,\ -603993107147691008,\ 80220923857108992,\ 1983943732124129280,\ -1120944653090611200,\ -3571475321182953472,\ 3062535903549603840,\ 3148640202567516160,\ -3550878140723822592,\ -1004211355174305792,\ 1711073233311105024,\ -31999222443147264,\ -285978658647048192,\ 29150317966786560,\ 14870074192035840,\ -1882287872409600).
\]
Repeating with the first $35+4$ terms removed gives order $35$ again.
...
model reproduces $10$ published values, $75$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234175.
