...
\title{The empirical column recurrences for the table OEIS A259742}
...
\begin{abstract}
OEIS A259742 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A259742 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 or 01010101.''. It has offset $1$, is
read by antidiagonals, and begins
\[
20, 22, 22, 42, 36, 42, 62, 74,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-2) +3*a(n-4) +2*a(n-6) for n>7\\
k=2: a(n) = a(n-1) +2*a(n-3) +a(n-4)\\
k=3: a(n) = 2*a(n-2) +9*a(n-4) +4*a(n-6) -6*a(n-8) +4*a(n-10) for n>11\\
k=4: [order 10] for n>11\\
k=5: [order 18] for n>19\\
k=6: [order 20] for n>22\\
k=7: [order 34] for n>37\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:54:55, revision 8) these are still
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 or 01010101.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+2)X(4) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 or 01010101.''. Its rows are the vertices of a digraph on
...
model reproduces $19$ published values, $52$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A259742.
