...
\title{The empirical column recurrences for the table OEIS A222444}
...
\begin{abstract}
OEIS A222444 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A222444 is ``T(n,k) = number of n X k 0..3 arrays with entries increasing mod 4 by 0, 1 or 2 rightwards and downwards, starting with upper left zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 3, 3, 9, 21, 9, 27, 147,\ \dots
\]
...
k=7: [order 32]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:19, revision 18) these are still
...
The sequence is ``Number of n X 1 0..3 arrays with entries increasing mod 4 by 0, 1 or 2 rightwards and downwards, starting with upper left zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 2 0..3 arrays with entries increasing mod 4 by 0, 1 or 2 rightwards and downwards, starting with upper left zero.''. Its rows are the vertices of a digraph on
$S=2$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of n X 3 0..3 arrays with entries increasing mod 4 by 0, 1 or 2 rightwards and downwards, starting with upper left zero.''. Its rows are the vertices of a digraph on
$S=3$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;18\,a(n-1) - 27\,a(n-2)
\]
holds from index $2+1$ on.

\subsection*{Column $k$$=4$}
The sequence is ``Number of n X 4 0..3 arrays with entries increasing mod 4 by 0, 1 or 2 rightwards and downwards, starting with upper left zero.''. Its rows are the vertices of a digraph on
$S=5$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;45\,a(n-1) - 267\,a(n-2) + 263\,a(n-3)
\]
...
model reproduces $29$ published values, $115$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222444.
