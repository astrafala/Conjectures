...
\title{The empirical column recurrences for the table OEIS A262917}
...
\begin{abstract}
OEIS A262917 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A262917 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with each row divisible by 3 and each column divisible by 7, read as a binary number with top and left being the most significant bits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 1, 2, 1, 3, 3, 1, 6,\ \dots
\]
...
k=3: [order 15]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 04 2015 10:13:40, revision 4) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with each row divisible by 3 and each column divisible by 7, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
model reproduces $10$ published values, $18$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262917.
