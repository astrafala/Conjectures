...
\title{A condition on each element and its neighbours: OEIS A240645}
...
\begin{abstract}
OEIS A240645 carries an empirical recurrence of order $48$ contributed by R.~H.~Hardin. It is
...
OEIS A240645 is ``Number of nX4 0..1 arrays with no element equal to the same number of vertical neighbors as horizontal neighbors, with new values 0..1 introduced in row major order.''. It has offset $1$ and begins
\[
2, 3, 8, 18, 61, 168, 526, 1643,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 11:04:32, revision 6) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $4$ columns over the alphabet $\{0,1\}$, growing one row at a time. Its condition is that every element must equal a different number of its vertical neighbours than it does of its horizontal neighbours.
...
Vertices with identical futures are merged, which leaves $S=71$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A240645.
