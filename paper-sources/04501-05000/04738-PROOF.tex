...
\title{The empirical recurrence for OEIS A230825, proved by transfer matrix}
...
\begin{abstract}
OEIS A230825 counts arrays of width $2$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
the count is a walk count on $S=77$ states.
\end{abstract}
...
OEIS A230825 is ``Number of n X 2 0..3 arrays x(i,j) with each element horizontally, vertically or antidiagonally next to at least one element with value (x(i,j)+1) mod 4 and at least one element with value (x(i,j)-1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
0, 2, 2, 6, 16, 44, 124, 354,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 2*a(n-2) - 3*a(n-3) - a(n-4) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 22 2018 11:51:18, revision 8) this is still
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) - 2\,a(n-2) - 3\,a(n-3) - a(n-4)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230825.
