...
\title{The empirical recurrence for OEIS A196852, proved by transfer matrix}
...
\begin{abstract}
OEIS A196852 counts $n\times4$ arrays over $\{0,\dots,4\}$ subject to a condition imposed on
every CELL over a neighbour set the entry names, and it carries an empirical recurrence of
order $89$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than
...
walk, and $a(n)$ is a walk count on $S=2366$ vertices, which is $C$-finite. Whether the
...
OEIS A196852 is ``Number of nX4 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 3,1,0,2,4 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
6, 136, 1370, 16876, 199125, 2369631, 28194191, 335586650,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 06:46:16, revision 9) this is still
...
settled by the terminal weight, which evaluates it with $\varnothing$ below. Most of those pairs are indistinguishable, and exactly so. Every offset named above has $|dj|\le 1$, so whether the cell at column $j$ of the middle row is satisfied depends on the three rows only through their windows at $j-1,j,j+1$. Collect for each $j$ the indicator over the window of the row below, and call that tuple $P(r,s)$: the number of cells any $t$ violates is read off $P$, so is the number violated at the bottom edge, and the successor $(s,t)$ carries $P(s,t)$, which does not mention $r$. Hence two pairs $(r,s)$ and $(r',s)$ with the same $P$ have the same outgoing rows and the same successors, and the vertex is $(s,P)$. That leaves $S=2366$ vertices in place of $390625$. Hence,
with $M$ the adjacency matrix on $S=2366$ vertices, $\iota$ the starting vector and $\tau$ the
terminal weight,
\[
...
Let $q(t)=t^{89}-\sum_i c_it^{89-i}$ be the characteristic polynomial of the
...
\begin{proof}
Factor $M^{j}$ out of $M^{j+89}-\sum_ic_iM^{j+89-i}$. For the bound, $M^{S}$
...
\begin{proof}
The vector $w=q(M)\tau$ was formed by $89$ matrix--vector products in exact integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196852.
