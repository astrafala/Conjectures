...
\title{The empirical closed form for OEIS A203880, proved}
...
\begin{abstract}
OEIS A203880 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=49$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
monic linear recurrence whose characteristic polynomial is
$\prod_j(x-\lambda_j)^{\deg p_j+1}$; here that polynomial is $q(x)=(x-13)$, of degree
...
OEIS A203880 is ``Number of (n+1)X2 0..6 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. It has offset $1$ and begins
\[
637, 8281, 107653, 1399489, 18193357, 236513641, 3074677333, 39970805329,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 49*13\^{}n
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:48:49, revision 9) this is still
...
walk, so with $M$ the adjacency matrix of that digraph on $S=49$ vertices, and $u,w$ the
...
Here $q(x)=(x-13)$, of degree $1$, and the recurrence it encodes is
...
$n=1,\dots,1$ and agree at each. A monic recurrence of order $1$
determines $a(m)$ from $a(m-1),\dots,a(m-1)$, and for every $m\ge2$ both
$m>0$ and $m-1\ge1$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge1$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203880.
