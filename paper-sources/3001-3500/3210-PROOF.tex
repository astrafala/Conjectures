...
\title{The empirical closed form, generating function and recurrence for OEIS A195233, proved}
...
\begin{abstract}
OEIS A195233 counts the lower triangles of a $3\times3$ array over $\{0,\dots,n\}$ in which
...
OEIS A195233 is ``Number of lower triangles of a 3 X 3 0..n array with each element differing from all of its diagonal, vertical, antidiagonal and horizontal neighbors by one or less.''. It has offset $1$ and begins
\[
64, 155, 246, 337, 428, 519,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 06 2018 11:53:12, revision 9) these are still
...
Every $y\in\mathcal P$ has $\mathrm{sp}(y)\le 1\cdot2=2$, where $2$ is the
...
entry differ by at most $1\cdot2$.
\end{proof}
...
\[
a(n)\;=\;91\,n-27\qquad\text{for every }n\ge0,
\]
and $\mathcal P$ has exactly $91$ elements.
\end{theorem}

\begin{proof}
For $n\ge2$ every term of Lemma~\ref{lem:trans} is positive by
Lemma~\ref{lem:bound}, so
\[
a(n)=\sum_{y\in\mathcal P}\bigl(n+1-\mathrm{sp}(y)\bigr)
=|\mathcal P|\,(n+1)-\sum_{y}\mathrm{sp}(y),
\]
which is a linear function of $n$. Two values determine it, and $a(2)$ and
$a(2+1)$ were computed exactly, giving slope $91$ and the stated intercept; the slope
is $|\mathcal P|$. That the identity already holds from $n=0$ was then read off by
comparing it with the exact values of $a(n)$ below $2$, each computed the same way.
\end{proof}
...
vanishes, which for a linear function it does; it therefore holds for every $n\ge0+2$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195233.
