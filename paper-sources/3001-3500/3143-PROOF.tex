...
\title{The empirical recurrence for OEIS A269277, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A269277 counts arrays over $\{0,\dots,3\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=32$ vertices,
and the empirical recurrence of order $2$ contributed by R.~H.~Hardin then follows from
...
OEIS A269277 is ``Number of 2 X n 0..3 arrays with some element plus some horizontally or antidiagonally adjacent neighbor totalling three exactly once.''. It has offset $1$ and begins
\[
0, 108, 1368, 13896, 127512, 1104264, 9211608, 74853576,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 14*a(n-1) - 49*a(n-2) for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 21 2019 06:22:48, revision 8) the statement is
...
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,3\}^{2},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=32,
\]
...
the digraph dense, with $(3+1)^{2\cdot2}$ edges instead of the sparse set used here.
\end{remark}
...
\qquad w=h_{2}-\sum_i c_i h_{2-i},
\]
for $j\ge2$. Hence \eqref{eq:conj} holds from that point on if and only if
...
those integers vanishes from the index corresponding to $n=4+1$ onwards, and the run
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269277.
