...
\title{The empirical closed form, generating function and recurrence for OEIS A195234, proved}
...
\begin{abstract}
OEIS A195234 counts the lower triangles of a $4\times4$ array over $\{0,\dots,n\}$ in which
...
OEIS A195234 is ``Number of lower triangles of a 4 X 4 0..n array with each element differing from all of its diagonal, vertical, antidiagonal and horizontal neighbors by one or less.''. It has offset $1$ and begins
\[
1024, 3151, 5428, 7705, 9982, 12259,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 06 2018 11:53:46, revision 9) these are still
...
Every $y\in\mathcal P$ has $\mathrm{sp}(y)\le 1\cdot3=3$, where $3$ is the
...
entry differ by at most $1\cdot3$.
\end{proof}
...
\[
a(n)\;=\;2277\,n-1403\qquad\text{for every }n\ge2,
\]
and $\mathcal P$ has exactly $2277$ elements.
\end{theorem}

\begin{proof}
For $n\ge3$ every term of Lemma~\ref{lem:trans} is positive by
Lemma~\ref{lem:bound}, so
\[
a(n)=\sum_{y\in\mathcal P}\bigl(n+1-\mathrm{sp}(y)\bigr)
=|\mathcal P|\,(n+1)-\sum_{y}\mathrm{sp}(y),
\]
which is a linear function of $n$. Two values determine it, and $a(3)$ and
$a(3+1)$ were computed exactly, giving slope $2277$ and the stated intercept; the slope
is $|\mathcal P|$. That the identity already holds from $n=2$ was then read off by
comparing it with the exact values of $a(n)$ below $3$, each computed the same way.
\end{proof}
...
vanishes, which for a linear function it does; it therefore holds for every $n\ge2+2$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195234.
