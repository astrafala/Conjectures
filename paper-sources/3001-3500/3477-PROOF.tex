...
\title{The empirical product formula for OEIS A140923, proved}
...
\begin{abstract}
OEIS A140923 counts $8\times9$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $8\times9\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $72$. The entry's own expression, cleared of its denominator,
...
OEIS A140923 is ``Number of 8 X 9 matrices with elements in 0..n with each row and each column in nondecreasing order. 8,9,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 24310, 118195220, 198520691512, 151561524301616, 62305564808355232,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2025 11:11:31, revision 8) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $8\times9$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$8\times9$ rectangle --- a plane partition in a $8\times9\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{8}\prod_{j=1}^{9}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $72$ with rational coefficients.
\end{theorem}
...
$8\cdot9=72$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,9,8)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $100$ and $28$ respectively.
\end{lemma}

\begin{proof}
With $s=n+9+8-1$ one has $s-(n+i)=9+8-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{9+8-1-i}
=\frac{(s-9-8+2+i)(s-9-8+3+i)\cdots s}{(9+8-1-i)!},
\]
a polynomial in $n$ of degree $9+8-1-i$. Multiplying over $i=0,\dots,8-1$ gives
$\deg N=\sum_{i=0}^{8-1}(9+8-1-i)=100$. The denominator is
...
of degree at most $72+28=100$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,100$, that is at $101$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $100$ has at most $100$ roots, so it is identically zero. Since $D(n)\neq0$
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,100$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140923.
