...
\title{The empirical column recurrences for the table OEIS A264131}
...
\begin{abstract}
OEIS A264131 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264131 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having index change +-(.,.) 0,0 0,2 or 1,2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 5, 1, 25, 13, 1, 80, 169,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 11:05:11, revision 6) these are still
...
column $k$$=1$ & $1$ & $16$ & $>1$ & $8$ \\
column $k$$=2$ & $2$ & $64$ & $>0$ & $7$ \\
column $k$$=3$ & $3$ & $256$ & $>1$ & $6$ \\
column $k$$=4$ & $2$ & $1024$ & $>2$ & $5$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having index change +-(.,.) 0,0 0,2 or 1,2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having index change +-(.,.) 0,0 0,2 or 1,2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(4) arrays of permutations of 0..(n+1)*(4)-1 with each element having index change +-(.,.) 0,0 0,2 or 1,2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(5) arrays of permutations of 0..(n+1)*(5)-1 with each element having index change +-(.,.) 0,0 0,2 or 1,2.''. Its rows are the vertices of a digraph on
...
model reproduces $26$ published values, $73$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264131.
