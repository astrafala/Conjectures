...
\title{No column above the one before it in every row: OEIS A212925}
...
\begin{abstract}
OEIS A212925 carries an empirical recurrence of order $3$ contributed by R.~H.~Hardin. It is
...
OEIS A212925 is ``Number of n X 3 0..2 arrays with no column j greater than column j-1 in all rows.''. It has offset $1$ and begins
\[
10, 568, 18226, 518320, 14230810, 386357608, 10450787266, 282343443040,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 37*a(n-1) -279*a(n-2) +243*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 21 2018 15:52:19, revision 11) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays of $3$ columns whose rows are arbitrary over the alphabet $\{0,1,2\}$, and asks that no column $j$ be greater than column $j-1$ \emph{in all rows}.

That is not a condition on any bounded window: ``in all rows'' quantifies over the whole array, and a row arriving at the very end can still decide it. What makes it a walk anyway is that it is a conjunction of independent facts about single rows. For each of the $2$ adjacent column pairs keep one bit, recording whether every row so far has had column $j$ greater than column $j-1$. A new row can only clear bits, never set one, and the array is admissible exactly when every bit has been cleared by the time the array ends.

So the state is that bit mask and nothing else --- at most $2^{2}$ of them, however large the alphabet is, which is why entries whose terms run to twenty digits have models this small. Rows with the same bit pattern act identically, so each row contributes one edge and the multiplicity does the counting. Merging vertices with identical futures leaves $S=3$, and with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which masks may begin and end an array,
\[
...
\begin{theorem}
\[
a(n)\;=\;37\,a(n-1) - 279\,a(n-2) + 243\,a(n-3)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A212925.
