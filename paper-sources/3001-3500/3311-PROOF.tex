...
\title{The empirical closed form and generating function for OEIS A195002, proved}
...
\begin{abstract}
OEIS A195002 counts the lower triangles of a $6\times6$ array over $\{0,\dots,n\}$ in which
...
OEIS A195002 is ``Number of lower triangles of a 6 X 6 0..n array with each element differing from all of its horizontal and vertical neighbors by one.''. It has offset $1$ and begins
\[
2, 4608, 31042, 81014, 138956, 199988,\ \dots
\]
...
Empirical g.f.: 2*x*(1 + 2302*x + 10914*x\^{}2 + 11769*x\^{}3 + 3985*x\^{}4 + 1545*x\^{}5 + 152*x\^{}6 + 73*x\^{}7 + 2*x\^{}8 + x\^{}9) / (1 - x)\^{}2. - \_Colin Barker\_, May 06 2018
\end{quote}
As of the ``Last modified'' line on the live entry (May 06 2018 10:12:47, revision 9) these are still
...
\[
a(n)\;=\;61488\,n-169100\qquad\text{for every }n\ge9,
\]
and $\mathcal P$ has exactly $61488$ elements.
\end{theorem}
...
$a(10+1)$ were computed exactly, giving slope $61488$ and the stated intercept; the slope
...
First, the count reproduces all $30$ terms the entry publishes, exactly, in integer
arithmetic.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195002.
