...
\title{The empirical column recurrences for the table OEIS A264128}
...
\begin{abstract}
OEIS A264128 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264128 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having index change +-(.,.) 0,0 1,-2 or 2,-1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 4, 9, 4, 8, 45,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 11:03:29, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having index change +-(.,.) 0,0 1,-2 or 2,-1.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264128.
