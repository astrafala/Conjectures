...
\title{The conjectured generating function for OEIS A188707, proved}
...
\begin{abstract}
OEIS A188707 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A188707 is ``Number of 3 X n binary arrays without the pattern 0 0 diagonally or vertically.''. It has offset $1$ and begins
\[
5, 21, 90, 387, 1665, 7164,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(5 - 4*x)/(1 - 5*x + 3*x\^{}2). - \_Colin Barker\_, Mar 11 2012
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 21 2021 05:32:49, revision 39) this is still
...
numerator. Its coefficients are the integers $c_1,\dots,c_{2}$ read off $D_2$, the first few being $5,\ -3$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188707.
