...
\title{The empirical recurrence for OEIS A188691, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A188691 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $85$ contributed
...
OEIS A188691 is ``Number of nX5 binary arrays without the pattern 0 1 0 diagonally, vertically, antidiagonally or horizontally.''. It has offset $1$ and begins
\[
21, 441, 3609, 20662, 139307, 1181340, 9908740, 75877068,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 04:46:40, revision 9) the statement is
...
$o=1n_0+0-2$. Then for $j\ge85$,
\[
a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\;=\;\,\mathbf{v}^{\!\top}G^{\,j-85}w,
\qquad w=h_{85}-\sum_i c_i\,h_{85-i} .
\]
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-85}$ out of
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>86$, which is the statement of
Section~1.
\end{theorem}
...
the middle row. The vector $w$ was formed by $85$ applications of $G$ in exact integer
arithmetic and $\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly.
Every one of those integers vanishes from the index corresponding to $n=86+1$ onwards,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188691.
