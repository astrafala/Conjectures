...
\title{The conjectured generating function for OEIS A250737, proved}
...
\begin{abstract}
OEIS A250737 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $22$ coefficients in exact arithmetic
...
OEIS A250737 is ``Number of (n+1) X (3+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nonincreasing x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
18, 22, 30, 46, 78, 142,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: 2*x*(9 - 16*x) / ((1 - x)*(1 - 2*x)). - \_Colin Barker\_, Nov 16 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 16 2018 17:39:28, revision 8) this is still
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $22$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250737.
