...
\title{The empirical recurrence for OEIS A206210, proved by transfer matrix}
...
\begin{abstract}
OEIS A206210 counts, up to renaming the letters, the arrays of width $4$ over at most $3$
letters in which every $2\times3$ and $3\times2$ subblock has exactly $2$ equal edges, an edge of such a window being one of the six steps of its perimeter cycle and an equal edge one whose two cells agree. It carries an empirical recurrence of order $3$ contributed by
...
OEIS A206210 is ``Number of (n+1) X 4 0..2 arrays with every 2 X 3 or 3 X 2 subblock having exactly two equal edges, and new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
162, 336, 760, 2224, 5632, 14416, 40032, 101376,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 18*a(n-3) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 22 2023 14:45:05, revision 7) this is still
...
$w_i$ cleared of denominators and $S=1034$ states in all. The walk index $j$ and the entry's own
...
the integers. The bound is exact: at $n=6$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206210.
