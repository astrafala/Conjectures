...
\title{The empirical product formula for OEIS A140932, proved}
...
\begin{abstract}
OEIS A140932 counts $9\times10$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $9\times10\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $90$. The entry's own expression, cleared of its denominator,
...
OEIS A140932 is ``Number of 9 X 10 matrices with elements in 0..n with each row and each column in nondecreasing order. 9,10,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 92378, 1551580888, 8291930371088, 18748054569029968, 21427584214357481888,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 25 2025 12:15:46, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $9\times10$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$9\times10$ rectangle --- a plane partition in a $9\times10\times n$ box. The
correspondence is a bijection, being an involution on arrays, so
\[
a(n)\;=\;\mathrm{PP}(9,10,n).
\]
...
\[
a(n)\;=\;\prod_{i=1}^{9}\prod_{j=1}^{10}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $90$ with rational coefficients.
\end{theorem}
...
$9\cdot10=90$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,10,9)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $126$ and $36$ respectively.
\end{lemma}

\begin{proof}
With $s=n+10+9-1$ one has $s-(n+i)=10+9-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{10+9-1-i}
=\frac{(s-10-9+2+i)(s-10-9+3+i)\cdots s}{(10+9-1-i)!},
\]
a polynomial in $n$ of degree $10+9-1-i$. Multiplying over $i=0,\dots,9-1$ gives
$\deg N=\sum_{i=0}^{9-1}(10+9-1-i)=126$. The denominator is
...
of degree at most $90+36=126$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,126$, that is at $127$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $126$ has at most $126$ roots, so it is identically zero. Since $D(n)\neq0$
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,126$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140932.
