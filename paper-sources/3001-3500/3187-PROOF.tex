...
\title{The empirical closed form and generating function for OEIS A194999, proved}
...
\begin{abstract}
OEIS A194999 counts the lower triangles of a $3\times3$ array over $\{0,\dots,n\}$ in which
...
OEIS A194999 is ``Number of lower triangles of a 3 X 3 0..n array with each element differing from all of its horizontal and vertical neighbors by one.''. It has offset $1$ and begins
\[
2, 20, 42, 66, 90, 114,\ \dots
\]
...
Empirical g.f.: 2*x*(1 + 8*x + 2*x\^{}2 + x\^{}3) / (1 - x)\^{}2. - \_Colin Barker\_, May 06 2018
\end{quote}
As of the ``Last modified'' line on the live entry (May 06 2018 11:54:44, revision 9) these are still
...
\[
a(n)\;=\;24\,n-30\qquad\text{for every }n\ge3,
\]
and $\mathcal P$ has exactly $24$ elements.
\end{theorem}
...
$a(4+1)$ were computed exactly, giving slope $24$ and the stated intercept; the slope
...
First, the count reproduces all $52$ terms the entry publishes, exactly, in integer
arithmetic.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A194999.
