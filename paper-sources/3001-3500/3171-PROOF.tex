...
\title{The empirical recurrence for OEIS A189247, proved by transfer matrix}
...
\begin{abstract}
OEIS A189247 counts the permutations of the cells of an $n\times2$ array in which every cell
moves by one of a fixed short list of offsets. The entry carries an empirical recurrence of
order $3$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than
...
carried is which images inside a window of $7$ consecutive positions are already taken.
That makes $a$ a walk count on $S=10$ states, hence $C$-finite, and whether the published
...
OEIS A189247 is ``Number of n X 2 array permutations with each element making zero or one king moves.''. It has offset $1$ and begins
\[
2, 24, 152, 1168, 8448, 62272, 456320, 3350016,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) + 12*a(n-2) - 16*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 22 2025 11:37:35, revision 10) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

Write the cells of the array as pairs $(i,j)$ with $0\le i<n$ and $0\le j<2$. An
...
Equivalently, $a(n)$ is the permanent of the $n2\times n2$ matrix whose $(c,c')$ entry is
$1$ exactly when $c'-c\in\Delta$.

\section{A window of 7 positions}

Number the cells in row-major order, $k=i\cdot2+j$. An offset $(\delta_1,\delta_2)$ moves a
cell by $\delta_12+\delta_2$ places in that order, so every image lies between $lo=-3$ and
$hi=3$ places from its source. Process the cells in order and let each one choose its image.
Once the cell at position $k$ has been dealt with, the position $k+-3$ can never be chosen
again by anybody, so it must already be taken; and no position beyond $k+3$ can have been
taken yet. What has to be remembered is therefore exactly which of the $7$ positions
$k+-3,\dots,k+3$ are taken --- a subset of a set of size $7$, and nothing else.

That is the state. One step of the walk is one row, that is $2$ cell choices, and the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A189247.
