...
\title{The empirical recurrence for OEIS A205312, proved by transfer matrix}
...
\begin{abstract}
OEIS A205312 counts, up to renaming the letters, the arrays of width $3$ over at most $2$
letters in which all the $2\times2$ subblocks of one array have the SAME number of equal edges, an equal edge being one of the four perimeter edges whose two cells agree; the common value is not named, so the count splits by it. It carries an empirical recurrence of order $3$ contributed by
...
OEIS A205312 is ``Number of (n+1) X 3 0..1 arrays with every 2 X 2 subblock having the same number of equal edges, and new values 0..1 introduced in row major order.''. It has offset $1$ and begins
\[
20, 84, 376, 1708, 7784, 35500, 161928, 738636,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) -7*a(n-2) +2*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 09:26:18, revision 8) this is still
...
$w_i$ cleared of denominators and $S=13$ states in all. The walk index $j$ and the entry's own
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205312.
