...
\title{The empirical recurrence for OEIS A208704, proved by counting patterns}
...
\begin{abstract}
OEIS A208704 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $9$ to $S=5$. The entry's empirical recurrence of order
$1$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A208704 is ``Number of nX3 0..1 arrays with new values 0..1 introduced in row major order and no element equal to more than two of its immediate leftward or upward or right-upward antidiagonal neighbors.''. It has offset $1$ and begins
\[
4, 28, 196, 1372, 9604, 67228, 470596, 3294172,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 7*a(n-1).
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 06 2016 12:51:09, revision 26) the statement is
...
Assembling the $2$ lumped chains into one block-diagonal matrix $N$ of size $S=5$ and
...
\end{equation}
The unlumped alphabet had $9$ rows in all; the lumped chain has $S=5$ states.

\section{The criterion, and the computation}

Let $q(t)=t^{1}-\sum_i c_i\,t^{1-i}$ be the characteristic polynomial of
\eqref{eq:conj}.

\begin{lemma}\label{lem:crit}
For every $n$ with $m=n+0-1\ge1$,
\[
2!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-1}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-1}$ out of
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>1$, which is the statement of
Section~1.
\end{theorem}
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $1$ matrix--vector products
...
corresponding to $n=1+1$ onwards, and the run continued until $w$ was the zero vector,
...
\eqref{eq:walk}, divided by $2!$, was evaluated at every one of the $20$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208704.
