...
\title{The conjectured generating function for OEIS A240001, proved}
...
\begin{abstract}
OEIS A240001 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=14$ vertices, so its generating function is a rational function whose numerator
...
OEIS A240001 is ``Number of 2 X n 0..3 arrays with no element equal to one plus the sum of elements to its left or two plus the sum of the elements above it or one plus the sum of the elements diagonally to its northwest, modulo 4.''. It has offset $1$ and begins
\[
5, 13, 25, 42, 65, 95,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(5 - 7*x + 3*x\^{}2) / (1 - x)\^{}4.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 27 2018 06:21:06, revision 8) this is still
...
\]
and there are $S=14$ states. The digraph is built by reading the entry's own English, and the
model reproduces all $44$ terms the entry publishes, exactly, in integer arithmetic --- a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A240001.
