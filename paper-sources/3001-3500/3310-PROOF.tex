...
\title{The conjectured generating function for OEIS A205248, proved}
...
\begin{abstract}
OEIS A205248 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A205248 is ``Number of (n+1) X 2 0..1 arrays with the number of clockwise edge increases in every 2 X 2 subblock the same.''. It has offset $1$ and begins
\[
16, 40, 112, 328, 976, 2920,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: 8*x*(2 - 3*x)/((1 - x)*(1 - 3*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 26 2025 21:50:32, revision 35) this is still
...
The family this entry belongs to is read by one model, described by its author as: ``Edge counts on the 2 X 2 subblocks: clockwise increases, and its relatives.''
...
numerator. Its coefficients are the integers $c_1,\dots,c_{2}$ read off $D_2$, the first few being $4,\ -3$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205248.
