...
\title{The empirical recurrence for OEIS A238906, proved by transfer matrix}
...
\begin{abstract}
OEIS A238906 counts $(n+1)\times2$ arrays over $\{0,\dots,2\}$ subject to a condition imposed on
...
OEIS A238906 is ``Number of (n+1) X (1+1) 0..2 arrays with no element equal to all horizontal neighbors or equal to all vertical neighbors, and new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
3, 9, 54, 261, 1341, 6768, 34335, 173925,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) + 6*a(n-2) - 3*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 24 2018 10:10:55, revision 8) this is still
...
$\{0,\dots,2\}^{2}$. The neighbourhood of a cell in row $u_i$ meets $u_{i-1}$,
$u_i$ and $u_{i+1}$ and nothing else. Take as vertices the pairs
$(r,s)\in(\{0,\dots,2\}^{2}\cup\{\varnothing\})\times\{0,\dots,2\}^{2}$,
...
integers vanish from the index corresponding to $n=3+1$ onwards, and the run was
continued until the working vector was identically zero, which settles all larger $j$ at once.
By Lemma~\ref{lem:crit} the recurrence holds for every $n>3$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A238906.
