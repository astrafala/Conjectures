...
\title{The empirical closed form, generating function and recurrence for OEIS A195249, proved}
...
\begin{abstract}
OEIS A195249 counts the lower triangles of a $3\times3$ array over $\{0,\dots,n\}$ in which
adjacent entries differ by at most $2$, and carries closed form, generating function and recurrence recorded as empirical. The shape here is FIXED
...
OEIS A195249 is ``Number of lower triangles of a 3 X 3 0..n array with each element differing from all of its diagonal, vertical, antidiagonal and horizontal neighbors by two or less.''. It has offset $1$ and begins
\[
64, 729, 1682, 2729, 3776, 4823,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 06 2018 12:18:32, revision 9) these are still
...
horizontal, antidiagonal, vertical, diagonal offsets, which here gives $10$ adjacent pairs. Each cell carries a value in
...
Every $y\in\mathcal P$ has $\mathrm{sp}(y)\le 2\cdot2=4$, where $2$ is the
diameter of the neighbour graph.
\end{lemma}

\begin{proof}
The neighbour graph is connected, so any two cells are joined by a path of at most $2$
edges, and each edge changes the value by at most $2$. Hence the largest and the smallest
entry differ by at most $2\cdot2$.
\end{proof}
...
\[
a(n)\;=\;1047\,n-1459\qquad\text{for every }n\ge3,
\]
and $\mathcal P$ has exactly $1047$ elements.
\end{theorem}
...
$a(4+1)$ were computed exactly, giving slope $1047$ and the stated intercept; the slope
...
vanishes, which for a linear function it does; it therefore holds for every $n\ge3+2$.
...
First, the count reproduces all $38$ terms the entry publishes, exactly, in integer
arithmetic.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195249.
