...
\title{The empirical closed form for OEIS A188863, proved}
...
\begin{abstract}
OEIS A188863 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
OEIS A188863 is ``Number of n X 6 binary arrays without the pattern 0 1 diagonally, vertically or antidiagonally.''. It has offset $1$ and begins
\[
64, 239, 466, 707, 950, 1193, 1436, 1679,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 243*n - 265 for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (May 01 2018 09:12:52, revision 8) this is still
...
$n=6,\dots,7$ and agree at each. A monic recurrence of order $2$
determines $a(m)$ from $a(m-1),\dots,a(m-2)$, and for every $m\ge8$ both
$m>5$ and $m-2\ge6$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge6$.
Below $n=6$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=6$ on. The remaining indices $n=4,\dots,5$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188863.
