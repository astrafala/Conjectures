...
\title{The empirical recurrence for OEIS A263551, proved by transfer matrix}
...
\begin{abstract}
OEIS A263551 counts arrays of width $2$ over $\{0,1,\dots,4\}$ whose rows, read as base-$5$
numbers, are divisible by $7$ and whose columns, read the same way, are divisible by
$7$. The entry carries an empirical recurrence of order $3$ contributed by
...
is then a walk count on $S=7$ states.
\end{abstract}
...
OEIS A263551 is ``Number of (n+1) X (1+1) 0..4 arrays with each row and column divisible by 7, read as a base-5 number with top and left being the most significant digits.''. It has offset $1$ and begins
\[
4, 10, 40, 148, 592, 2344, 9376, 37456,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) + 2*a(n-2) - 8*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 26 2021 14:12:07, revision 14) this is still
...
\gamma_j^{(i)}\bmod 7\;=\;
\bigl(5\,(\gamma_j^{(i-1)}\bmod 7)+x(i,j)\bigr)\bmod 7 .
\]
\end{lemma}
...
So the vector $\bigl(\gamma_1\bmod 7,\dots,\gamma_{2}\bmod
7\bigr)$ is a state: at most $7^{2}$ of them, and each array row
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263551.
