...
\title{The empirical recurrence for OEIS A223284, proved by transfer matrix}
...
\begin{abstract}
OEIS A223284 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$3$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is dodecahedron graph. The entry carries an empirical recurrence of order $3$
...
single slice is a state and the count is a walk count on a finite digraph with $S=876$ vertices.
\end{abstract}
...
OEIS A223284 is ``Rolling icosahedron face footprints: number of 3 X n 0..19 arrays starting with 0 where 0..19 label faces of an icosahedron and every array movement to a horizontal, diagonal or antidiagonal neighbor moves across an icosahedral edge.''. It has offset $1$ and begins
\[
400, 75, 849, 4995, 38457, 261819, 1881441, 13196979,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-1) + 18*a(n-2) - 24*a(n-3) for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 18 2018 16:03:31, revision 9) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=876.
\]
...
of length $3$ over $20$ vertices is one of $20^{3}$ words, but only $S-1$ of them are
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223284.
