...
\title{The empirical recurrence for OEIS A198900, proved by counting patterns}
...
\begin{abstract}
OEIS A198900 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $55$ to $S=9$. The entry's empirical recurrence of order
...
OEIS A198900 is ``Number of n X 2 0..4 arrays with values 0..4 introduced in row major order and no element equal to any horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
1, 4, 33, 380, 4801, 62004, 804833, 10459180,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 17*a(n-1) -55*a(n-2) +39*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 18 2018 14:58:25, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;17\,a(n-1) - 55\,a(n-2) + 39\,a(n-3)
\end{equation}
for all $n>3$.
...
Assembling the $5$ lumped chains into one block-diagonal matrix $N$ of size $S=9$ and
...
\end{equation}
The unlumped alphabet had $55$ rows in all; the lumped chain has $S=9$ states.
...
\[
5!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-3}q(N)\mathbf{1} .
\]
...
\eqref{eq:walk}, divided by $5!$, was evaluated at every one of the $18$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198900.
