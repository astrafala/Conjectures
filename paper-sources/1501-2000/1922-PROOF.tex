...
\title{The empirical recurrence for OEIS A205747, proved by transfer matrix}
...
\begin{abstract}
OEIS A205747 counts, up to renaming the letters, the arrays of width $3$ over at most $3$
letters in which horizontally neighbouring $2\times2$ subblocks have the same number of equal edges and vertically neighbouring ones have different numbers. It carries an empirical recurrence of order $11$ contributed by
...
OEIS A205747 is ``Number of (n+1)X3 0..2 arrays with every 2X2 subblock having the same number of equal edges as its horizontal neighbors and a different number from its vertical neighbors, and new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
48, 284, 1936, 12384, 82992, 542718, 3602578, 23691830,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:14:27, revision 9) this is still
...
$w_i$ cleared of denominators and $S=326$ states in all. The walk index $j$ and the entry's own
...
\begin{theorem}
\[
a(n)\;=\;33\,a(n-2) + 76\,a(n-3) + 13\,a(n-4) - 284\,a(n-5) - 597\,a(n-6) + 484\,a(n-7) + 1376\,a(n-8) + 36\,a(n-9) - 752\,a(n-10) - 256\,a(n-11)
\]
for every $n>12$.
\end{theorem}
...
the integers. The bound is exact: at $n=12$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205747.
