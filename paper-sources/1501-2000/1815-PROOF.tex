...
\title{The empirical recurrence for OEIS A198719, proved by counting patterns}
...
\begin{abstract}
OEIS A198719 counts arrays subject to a local condition \emph{and} to the clause ``new values
$0..6$ introduced in row major order'', which says the array is written in canonical
...
$7!\,a(n)=\sum_i\binom{7}{i}D_{7-i}L_i(n)$ with $D_m$ the derangement numbers,
...
state space down from $4676$ to $S=83$. The entry's empirical recurrence of order
$13$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A198719 is ``Number of nX4 0..6 arrays with values 0..6 introduced in row major order and no element equal to any horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
5, 499, 232841, 173549032, 141168480719, 116492275674072, 96356630422085931, 79732515488691835557,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 08:00:55, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;1081\,a(n-1) - 231028\,a(n-2) + 18466747\,a(n-3) - 729356901\,a(n-4) + 16017483310\,a(n-5) - 207602672167\,a(n-6) + 1633653422655\,a(n-7) - 7862297151184\,a(n-8) + 22902587520657\,a(n-9) - 39383752489643\,a(n-10) + 38480449661142\,a(n-11) - 19517387256516\,a(n-12) + 3939042601848\,a(n-13)
\end{equation}
for all $n>13$.

\section{What the canonical-form clause counts}

Fix the shape: $4$ columns and $L=n+0$ rows, so $L\cdot4$ cells. Call a
...
entry's requirement is that no cell equals more than $0$ of its neighbours at the
offsets
\[
\mathcal N=\{(0,-1),\ (-1,0)\}
\]
...
\#\bigl\{(d_1,d_2)\in\mathcal N:\ x_{t+d_1,\,u+d_2}=x_{t,u}\bigr\}\;\le\;0
...
The clause ``new values $0..6$ introduced in row major order'' selects, from each class
...
$v+1$. Every pattern with at most $K=7$ classes has exactly one such representative, and
...
\qquad
7!\,\sum_{j=0}^{7}N_j(n)=\sum_{i=0}^{7}\binom{7}{i}D_{7-i}\,L_i(n),
\]
...
$j\le 7$ and exchanging the order of summation,
\[
\sum_{j=0}^{7}N_j=\sum_{i=0}^{7}L_i\sum_{j=i}^{7}\frac{(-1)^{j-i}}{i!\,(j-i)!}
=\sum_{i=0}^{7}\frac{L_i}{i!}\sum_{t=0}^{7-i}\frac{(-1)^t}{t!} ,
\]
and multiplying by $7!$ turns the coefficient of $L_i$ into
$\frac{7!}{i!}\sum_{t\le 7-i}(-1)^t/t!=\binom{7}{i}(7-i)!\sum_{t\le 7-i}(-1)^t/t!
=\binom{7}{i}D_{7-i}$, an integer.
\end{proof}

\section{Each $L_i$ is a walk count, and the chain lumps}

Fix $i$ and let $V_i=\{0,\dots,i-1\}^{4}$ be the possible rows. For $r,s\in V_i$
...
Assembling the $7$ lumped chains into one block-diagonal matrix $N$ of size $S=83$ and
putting the weights of Lemma~\ref{lem:inv} into the start vector,
$\mathbf{v}=\bigl(\binom{7}{i}D_{7-i}\varepsilon_P\bigr)_{i,P}$, gives
\begin{equation}\label{eq:walk}
7!\;a(n)\;=\;\mathbf{v}^{\!\top}N^{\,n+0-1}\mathbf{1} .
\end{equation}
The unlumped alphabet had $4676$ rows in all; the lumped chain has $S=83$ states.

\section{The criterion, and the computation}

Let $q(t)=t^{13}-\sum_i c_i\,t^{13-i}$ be the characteristic polynomial of
\eqref{eq:conj}.

\begin{lemma}\label{lem:crit}
For every $n$ with $m=n+0-1\ge13$,
\[
7!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-13}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-13}$ out of
$N^{m}-\sum_i c_iN^{m-i}$. The scalar $7!$ is nonzero, so it does not affect whether
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>13$, which is the statement of
Section~1.
\end{theorem}
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $13$ matrix--vector products
...
corresponding to $n=13+1$ onwards, and the run continued until $w$ was the zero vector,
...
\eqref{eq:walk}, divided by $7!$, was evaluated at every one of the $12$
...
show up as a mismatch, and the quotient by $7!$ came out an integer at every index.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198719.
