...
\title{The empirical recurrence for OEIS A184024, proved by splitting on the common sum}
...
\begin{abstract}
OEIS A184024 counts arrays over $\{0,\dots,3\}$ in which all $2\times2$ subblock sums are
...
computation on $S=13312$ vertices.
\end{abstract}
...
OEIS A184024 is ``1/8 the number of (n+1)X5 0..3 arrays with all 2X2 subblock sums the same.''. It has offset $1$ and begins
\[
748, 1347, 2597, 5802, 13603, 35256, 94469, 271176,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 02:15:23, revision 9) the statement is
...
$V=\{0,\dots,3\}^{5}$ be the possible rows and let $M_c$ be the adjacency
...
Writing $N=\bigoplus_{c=0}^{12}M_c$, of size $S=13312$, and $\mathbf{v}$ for the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184024.
