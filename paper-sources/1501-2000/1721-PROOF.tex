...
\title{The empirical recurrence for OEIS A184023, proved by splitting on the common sum}
...
\begin{abstract}
OEIS A184023 counts arrays over $\{0,\dots,3\}$ in which all $2\times2$ subblock sums are
...
computation on $S=3328$ vertices.
\end{abstract}
...
OEIS A184023 is ``1/8 the number of (n+1)X4 0..3 arrays with all 2X2 subblock sums the same.''. It has offset $1$ and begins
\[
242, 491, 1052, 2597, 6650, 18701, 53948, 165929,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 02:15:16, revision 9) the statement is
...
$V=\{0,\dots,3\}^{4}$ be the possible rows and let $M_c$ be the adjacency
...
Writing $N=\bigoplus_{c=0}^{12}M_c$, of size $S=3328$, and $\mathbf{v}$ for the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184023.
