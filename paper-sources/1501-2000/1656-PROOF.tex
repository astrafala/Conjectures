...
\title{The empirical recurrence for OEIS A208267, proved by counting patterns}
...
\begin{abstract}
OEIS A208267 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
$18$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A208267 is ``Number of nX6 0..1 arrays with new values 0..1 introduced in row major order and no element equal to more than two of its immediate leftward or upward or left-upward diagonal neighbors.''. It has offset $1$ and begins
\[
32, 1129, 42563, 1592731, 59655167, 2234126207, 83670667271, 3133560234217,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 14:44:06, revision 9) the statement is
...
For every $n$ with $m=n+0-1\ge18$,
\[
2!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-18}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-18}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $18$ matrix--vector products
...
corresponding to $n=18+1$ onwards, and the run continued until $w$ was the zero vector,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208267.
