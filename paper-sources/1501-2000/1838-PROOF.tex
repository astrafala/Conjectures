...
\title{The empirical recurrence for OEIS A230826, proved by transfer matrix}
...
\begin{abstract}
OEIS A230826 counts arrays of width $3$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
carries an empirical recurrence of order $12$ contributed by R.~H.~Hardin. It is true, and
it is decidable rather than empirical: a cell's neighbours lie in three consecutive rows, so
the count is a walk count on $S=449$ states.
\end{abstract}
...
OEIS A230826 is ``Number of nX3 0..3 arrays x(i,j) with each element horizontally, vertically or antidiagonally next to at least one element with value (x(i,j)+1) mod 4 and at least one element with value (x(i,j)-1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
0, 2, 8, 10, 38, 130, 456, 1610,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:03:22, revision 6) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=449.
\]
...
\begin{theorem}
\[
a(n)\;=\;10\,a(n-1) - 37\,a(n-2) + 59\,a(n-3) - 25\,a(n-4) - 31\,a(n-5) + 33\,a(n-6) - 22\,a(n-7) - 4\,a(n-8) + 46\,a(n-9) - 2\,a(n-10) - 18\,a(n-11) - 4\,a(n-12)
\]
for every $n>17$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230826.
