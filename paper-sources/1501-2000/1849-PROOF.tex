...
\title{The empirical recurrence for OEIS A269038, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A269038 counts arrays over $\{0,\dots,2\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=162$ vertices,
and the empirical recurrence of order $12$ contributed by R.~H.~Hardin then follows from
...
OEIS A269038 is ``Number of 4Xn 0..2 arrays with some element plus some horizontally, diagonally or antidiagonally adjacent neighbor totalling two exactly once.''. It has offset $1$ and begins
\[
0, 504, 2136, 18384, 115656, 785124, 4998648, 31834476,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 18 2016 09:02:07, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) + 23\,a(n-2) - 102\,a(n-3) - 288\,a(n-4) + 250\,a(n-5) + 787\,a(n-6) - 238\,a(n-7) - 741\,a(n-8) + 124\,a(n-9) + 196\,a(n-10) - 16\,a(n-11) - 16\,a(n-12)
\end{equation}
for all $n>14$.
...
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,2\}^{4},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=162,
\]
...
the digraph dense, with $(2+1)^{2\cdot4}$ edges instead of the sparse set used here.
\end{remark}
...
Let $q(t)=t^{12}-\sum_i c_i\,t^{12-i}$ be the characteristic polynomial of
...
$o=1n_0+0-1$,
\[
a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\;=\;\,\mathbf{v}^{\!\top}G^{\,j-12}w,
\qquad w=h_{12}-\sum_i c_i h_{12-i},
\]
for $j\ge12$. Hence \eqref{eq:conj} holds from that point on if and only if
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>14$, which is the statement of
Section~1.
\end{theorem}

\begin{proof}
The digraph was built directly from the definition of a marked pair. The vector $w$ was
formed by $12$ applications of $G$ in exact integer arithmetic and
$\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly. Every one of
those integers vanishes from the index corresponding to $n=14+1$ onwards, and the run
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269038.
