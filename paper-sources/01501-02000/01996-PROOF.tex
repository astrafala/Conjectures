...
\title{The empirical recurrence for OEIS A269585, proved from an exact model}
...
\begin{abstract}
OEIS A269585 carries an empirical recurrence of order $6$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}5 + 5*n\^{}4 + 7*n\^{}3 + 12*n\^{}2 + 6*n + 1. a(n) = 6*a(n-1) - 15*a(n-2) + 20*a(n-3) - 15*a(n-4) + 6*a(n-5) - a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 24 2019 11:44:26, revision 7)
