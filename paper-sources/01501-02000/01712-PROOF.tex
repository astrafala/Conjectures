...
\title{The empirical closed form for OEIS A219472, proved}
...
\begin{abstract}
OEIS A219472 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=56$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219472 is ``Number of 2 X n arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..3 2 X n array.''. It has offset $1$ and begins
\[
10, 23, 68, 211, 547, 1248, 2663, 5432,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2018 05:17:21, revision 10)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=56$
...
states number $S=56$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219472 records is correct for all $n>3$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $12$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219472.
