...
\title{The empirical recurrence for OEIS A184022, proved by splitting on the common sum}
...
\begin{abstract}
OEIS A184022 counts arrays over $\{0,\dots,3\}$ in which all $2\times2$ subblock sums are
...
OEIS A184022 is ``1/8 the number of (n+1)X3 0..3 arrays with all 2X2 subblock sums the same.''. It has offset $1$ and begins
\[
88, 204, 491, 1347, 3781, 11541, 35771, 117321,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 02:15:10, revision 9) the statement is
...
The arrays have $3$ columns and $L=n+1$ rows, entries in
$\{0,\dots,3\}$, and every $2\times2$ subblock has the same sum. Since $L\ge2$ and
$3\ge2$, every such array contains at least one $2\times2$ subblock, and that subblock's
...
$V=\{0,\dots,3\}^{3}$ be the possible rows and let $M_c$ be the adjacency
matrix of the digraph in which $r\to s$ when
$r_j+r_{j+1}+s_j+s_{j+1}=c$ for every $j$ with $1\le j\le 3-1$. Then
...
Writing $N=\bigoplus_{c=0}^{12}M_c$, of size $S=832$, and $\mathbf{v}$ for the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184022.
