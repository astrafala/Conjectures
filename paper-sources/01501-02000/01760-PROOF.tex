...
\title{The empirical recurrence for OEIS A249848, proved from an exact model}
...
\begin{abstract}
OEIS A249848 carries an empirical recurrence of order $9$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}8 + (13/21)*n\^{}7 + (101/30)*n\^{}6 + (71/30)*n\^{}5 + (2/3)*n\^{}4 + 2*n\^{}3 - (1/30)*n\^{}2 + (1/70)*n. a(n) = 9*a(n-1) - 36*a(n-2) + 84*a(n-3) - 126*a(n-4) + 126*a(n-5) - 84*a(n-6) + 36*a(n-7) - 9*a(n-8) + a(n-9) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 18 2017 18:10:59, revision 8)
