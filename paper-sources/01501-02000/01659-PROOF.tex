...
\title{The empirical closed form for OEIS A199835, proved from an exact model}
...
\begin{abstract}
OEIS A199835 carries an empirical closed form: a polynomial in $n$ of degree $2$, recorded on the
entry and never marked settled. It is true. The entry's count is given exactly by an Ehrhart
quasi-polynomial, from which $a$ provably satisfies a monic linear recurrence of order $S=7$
derived below; a polynomial of degree $2$ satisfies $(z-1)^{2+1}$; so the difference
satisfies the product, and $7+2+1$ consecutive agreements settle it. No transfer matrix is
...
OEIS A199835 is ``Number of -n..n arrays x(0..5) of 6 elements with zero sum and no two neighbors summing to zero.''. It has offset $1$ and begins
\[
10, 426, 3556, 15708, 49302, 124982, 273728, 538968,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = n*(2*n+1)*(2*n-1)*(66*n\^{}2-35*n+19)/15.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 22 2025 13:24:45, revision 9)
...
\]
annihilates $a$, and $S=\deg A=7$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=7$, and
...
kills $a$, and $(z-1)^{2+1}$ kills $P$, so $A(z)(z-1)^{2+1}$ --- monic, of order
$7+2+1$ --- kills $r$. The model was evaluated exactly in integer arithmetic and $P$ in
exact rational arithmetic at every index from $n=1$ to $n=1+7+2+1$, and $r$ vanished
at all of them. A monic recurrence with $7+2+1$ consecutive zeros has every later term zero,
so $r\equiv0$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A199835.
