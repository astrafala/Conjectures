...
\title{The empirical recurrence for OEIS A250144, proved from an exact model}
...
\begin{abstract}
OEIS A250144 carries an empirical recurrence of order $8$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = (10/63)*n\^{}7 + (163/90)*n\^{}6 + (1421/180)*n\^{}5 + (625/36)*n\^{}4 + (751/36)*n\^{}3 + (2489/180)*n\^{}2 + (178/35)*n + 1. a(n) = 8*a(n-1) - 28*a(n-2) + 56*a(n-3) - 70*a(n-4) + 56*a(n-5) - 28*a(n-6) + 8*a(n-7) - a(n-8) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 12 2018 03:01:13, revision 11)
