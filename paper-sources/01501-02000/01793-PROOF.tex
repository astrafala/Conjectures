...
\title{The empirical recurrence for OEIS A221938, proved by transfer matrix}
...
\begin{abstract}
OEIS A221938 counts the distinct OCCUPANCY arrays obtained when every cell of an $n\times2$ grid
...
an empirical recurrence of order $9$ contributed by R.~H.~Hardin. It is true, and it is
...
one, on $S=153$ states, after which the conjectured recurrence is settled by a finite exact
computation.
\end{abstract}
...
OEIS A221938 is ``Number of 2 X n arrays of occupancy after each element stays put or moves to some horizontal or antidiagonal neighbor, without move-in move-out straight through or left turns.''. It has offset $1$ and begins
\[
1, 17, 105, 783, 5622, 40608, 293084, 2115379,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 25*a(n-2) + 40*a(n-3) + a(n-4) - 42*a(n-5) - 20*a(n-6) + 10*a(n-7) + 4*a(n-8) -a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 11 2018 13:09:33, revision 8) this is still
...
(-1,0), (-1,1), (0,0), (1,-1), (1,0),
\]
a move being available only when it lands inside the grid. The offset $(0,0)$ is among them: the entry lets a token stay put. The entry writes the array with $n$ as the number of COLUMNS. Transposing it exchanges the two coordinates of every offset, which is done once so that the walk always runs down the rows; a transposition is a reflection and reverses the sense of a turn, so the left-hand test is negated in the transposed frame rather than being applied in the wrong one. The OCCUPANCY array
...
$S=153$ reachable sets. In particular $a$ is $C$-finite.
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 25\,a(n-2) + 40\,a(n-3) + a(n-4) - 42\,a(n-5) - 20\,a(n-6) + 10\,a(n-7) + 4\,a(n-8) - a(n-9)
\]
for every $n>9$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221938.
