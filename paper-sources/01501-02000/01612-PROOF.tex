...
\title{The empirical recurrence for OEIS A280435, proved by counting patterns with a budget}
...
\begin{abstract}
OEIS A280435 imposes three things at once: a condition on each cell that reaches one row up and
...
OEIS A280435 is ``Number of nX3 0..1 arrays with no element equal to more than one of its horizontal and vertical neighbors, with the exception of exactly one element, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
1, 10, 21, 42, 77, 136, 236, 404,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) -a(n-2) -3*a(n-3) +a(n-4) +a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 04 2017 13:12:34, revision 6) the statement is
...
\[
2!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)=\mathbf{v}^{\!\top}G^{\,j-5}w,
...
one of the $33$ published indices and agrees with the entry's DATA exactly, the quotient
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A280435.
