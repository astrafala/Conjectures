...
\title{The empirical closed form for OEIS A219708, proved}
...
\begin{abstract}
OEIS A219708 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=14$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219708 is ``Number of n X 2 arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 n X 2 array.''. It has offset $1$ and begins
\[
4, 13, 44, 129, 338, 813, 1822, 3840,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/40320)*n\^{}8 + (1/2016)*n\^{}7 + (7/2880)*n\^{}6 + (5/144)*n\^{}5 + (167/5760)*n\^{}4 + (259/288)*n\^{}3 + (4723/10080)*n\^{}2 - (73/168)*n + 3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 27 2018 06:12:01, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=14$
...
states number $S=14$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219708 records is correct for all $n>0$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $0$.
Second, the polynomial and the walk count agree at every index from $1$ up to
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219708.
