...
\title{The empirical closed form for OEIS A219572, proved}
...
\begin{abstract}
OEIS A219572 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
monic recurrence with characteristic polynomial $(x-1)^{9}$; that the walk count
...
OEIS A219572 is ``Number of n X 2 arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 n X 2 array.''. It has offset $1$ and begins
\[
4, 13, 59, 181, 469, 1107, 2447, 5120,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/20160)*n\^{}8 + (1/2520)*n\^{}7 + (1/480)*n\^{}6 + (43/360)*n\^{}5 - (297/320)*n\^{}4 + (1271/180)*n\^{}3 - (75973/5040)*n\^{}2 + (2081/105)*n - 12 for n>1.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 11 2018 15:09:33, revision 7)
...
\[
q(x)=(x-1)^{9},\qquad\text{that is}\qquad a(n)=9\,a(n-1) - 36\,a(n-2) + 84\,a(n-3) - 126\,a(n-4) + 126\,a(n-5) - 84\,a(n-6) + 36\,a(n-7) - 9\,a(n-8) + a(n-9).
\]

\begin{theorem}
The closed form OEIS A219572 records is correct for all $n>1$.
\end{theorem}
...
$1 + 31 - 1$, which is more than the $9$ consecutive agreements a
common recurrence of order $9$ needs. Two sequences satisfying the same monic recurrence
of order $9$ and agreeing at $9$ consecutive indices past both their thresholds are
equal from there on.
\end{proof}
...
First, the model reproduces all $31$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219572.
