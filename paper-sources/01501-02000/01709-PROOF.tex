...
\title{The empirical closed form for OEIS A220926, proved}
...
\begin{abstract}
OEIS A220926 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=32$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220926 is ``Number of n X 2 arrays of the minimum value of corresponding elements and their horizontal, vertical or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..3 n X 2 array.''. It has offset $1$ and begins
\[
4, 13, 41, 114, 260, 545, 1104, 2182,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/20160)*n\^{}8 - (1/2520)*n\^{}7 - (1/1440)*n\^{}6 + (7/45)*n\^{}5 - (3953/2880)*n\^{}4 + (2093/360)*n\^{}3 + (1049/560)*n\^{}2 - (12377/420)*n + 28 for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 03 2018 08:11:59, revision 9)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=32$
...
states number $S=32$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220926 records is correct for all $n>3$.
\end{theorem}
...
$1 + 32 - 1$, which is more than the $9$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220926.
