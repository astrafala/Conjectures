...
\title{The empirical recurrence for OEIS A200889, proved from an exact model}
...
\begin{abstract}
OEIS A200889 carries an empirical recurrence of order $7$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = (2/45)*n\^{}6 + (19/15)*n\^{}5 + (217/36)*n\^{}4 + (71/6)*n\^{}3 + (2057/180)*n\^{}2 + (27/5)*n + 1. a(n) = 7*a(n-1) - 21*a(n-2) + 35*a(n-3) - 35*a(n-4) + 21*a(n-5) - 7*a(n-6) + a(n-7) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 16 2017 12:23:14, revision 10)
...
\]
annihilates $a$, and $S=\deg A=8$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=8$, and
