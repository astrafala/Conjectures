...
\title{The empirical recurrence for OEIS A297454, proved by transfer matrix}
...
\begin{abstract}
OEIS A297454 counts arrays of width $5$ over $\{0,1\}$ in which every cell of one value has an allowed number of neighbours of another. The entry carries an
...
OEIS A297454 is ``Number of n X 5 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 0 or 2 neighboring 1's.''. It has offset $1$ and begins
\[
13, 156, 1143, 10579, 97005, 862112, 7800563, 70510494,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 07 2021 13:55:44, revision 6) this is still
...
The condition is that every cell equal to $1$ has exactly $0$ or exactly $2$ neighbours equal to $1$:
\[
x(i,j)=1\ \Longrightarrow\ \#\{(a,b)\in S(i,j):x(a,b)=1\}\in\{0,2\}.
\]
...
Thus
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=398,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A297454.
