...
\title{The empirical recurrence for OEIS A267785, proved by transfer matrix}
...
\begin{abstract}
OEIS A267785 counts arrays in which the REPEATED VALUES of each row, and of each column, stand in a
...
value so far in each column of the scan, so the count is a walk count on $S=6433$ states.
\end{abstract}
...
OEIS A267785 is ``Number of nX5 0..1 arrays with every repeated value in every row greater than or equal to, and in every column greater than, the previous repeated value.''. It has offset $1$ and begins
\[
28, 784, 5004, 34173, 122770, 460598, 1234716, 3380133,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 20 2016 12:29:01, revision 4) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=6433,\quad\tau=\mathbf{1} .
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267785.
