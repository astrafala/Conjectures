...
\title{The empirical recurrence for OEIS A229582, proved by counting defective colourings}
...
\begin{abstract}
OEIS A229582 counts the colourings of an array with $K=3$ colours that are proper except at
...
numbers. What is left is a walk count on $S=196$ vertices, and the empirical recurrence of
order $2$ contributed by R.~H.~Hardin follows from a finite exact computation.
\end{abstract}
...
OEIS A229582 is ``Number of defective 3-colorings of an n X 4 0..2 array connected horizontally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. It has offset $1$ and begins
\[
6, 116, 1620, 19764, 224532, 2440692, 25745364, 265720500,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 18*a(n-1) - 81*a(n-2) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 19 2018 08:49:48, revision 8) the statement is
...
The arrays have $4$ columns and $L=n$ rows, entries in $\{0,\dots,3-1\}$.
...
\[
3!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)=\mathbf{v}^{\!\top}G^{\,j-2}w,
...
$2$ applications of $G$ in exact integer arithmetic and $\mathbf{v}^{\!\top}G^{m}w$
evaluated for $m=0,1,\dots$, again exactly. Every one of those integers vanishes from the
index corresponding to $n=3+1$ onwards, and the run continued until $S+1$ consecutive
zeros had been seen.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229582.
