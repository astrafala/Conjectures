...
\title{The empirical recurrence for OEIS A249884, proved from an exact model}
...
\begin{abstract}
OEIS A249884 carries an empirical recurrence of order $8$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}7 + (7/2)*n\^{}6 + 7*n\^{}5 + (35/4)*n\^{}4 + (49/6)*n\^{}3 + (21/4)*n\^{}2 + (4/3)*n. a(n) = 8*a(n-1) - 28*a(n-2) + 56*a(n-3) - 70*a(n-4) + 56*a(n-5) - 28*a(n-6) + 8*a(n-7) - a(n-8) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2018 05:46:31, revision 8)
