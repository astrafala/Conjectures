...
\title{The empirical closed form for OEIS A219803, proved}
...
\begin{abstract}
OEIS A219803 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=27$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219803 is ``Number of 2 X n arrays of the minimum value of corresponding elements and their horizontal, vertical, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 2 X n array.''. It has offset $1$ and begins
\[
4, 4, 10, 21, 39, 68, 114, 186,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/120)*n\^{}5 - (1/8)*n\^{}4 + (29/24)*n\^{}3 - (35/8)*n\^{}2 + (737/60)*n - 12 for n>2.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 28 2018 10:43:21, revision 8)
this is still recorded as empirical, and it is still open.

\section{What is being counted}

Fix the width $W=2$ and the value range $0..3$. The underlying arrays are the
$2\times n$ arrays over that range whose rows are lexicographically nonincreasing read downwards
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=27$
...
states number $S=27$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219803 records is correct for all $n>2$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $8$.
Second, the polynomial and the walk count agree at every index from $3$ up to
$1 + 39 - 1$, which is more than the $6$ consecutive agreements a
...
First, the model reproduces all $39$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219803.
