...
\title{The empirical recurrence for OEIS A188238, proved from an exact model}
...
\begin{abstract}
OEIS A188238 carries an empirical recurrence of order $11$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=29$, derived below and not
...
OEIS A188238 is ``Number of nondecreasing arrangements of 5 numbers in -(n+3)..(n+3) with sum zero and not more than two numbers equal.''. It has offset $1$ and begins
\[
58, 119, 221, 374, 598, 903, 1317, 1852,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - a(n-3) - 2*a(n-5) + 2*a(n-6) + a(n-8) - 2*a(n-10) + a(n-11).
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 27 2018 08:53:37, revision 8)
...
\]
annihilates $a$, and $S=\deg A=29$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=29$, and
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188238.
