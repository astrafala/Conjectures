...
\title{The empirical recurrence for OEIS A269436, proved from an exact model}
...
\begin{abstract}
OEIS A269436 carries an empirical recurrence of order $5$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}4 + 4*n\^{}3 + (11/2)*n\^{}2 + (7/2)*n + 1. a(n) = 5*a(n-1) - 10*a(n-2) + 10*a(n-3) - 5*a(n-4) + a(n-5) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 22 2019 07:46:26, revision 8)
...
$a$ and $S=5$. Nothing is fitted and no threshold is guessed: the $N_m$ are counted, not
estimated.
