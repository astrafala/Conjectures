...
\title{The empirical recurrence for OEIS A209074, proved from an exact model}
...
\begin{abstract}
OEIS A209074 carries an empirical recurrence of order $10$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by a Burnside sum over the necklace group, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=46$, derived below and not
...
OEIS A209074 is ``Number of 5-bead necklaces labeled with numbers -n..n not allowing reversal, with sum zero and avoiding the pattern z z+1 z+2.''. It has offset $1$ and begins
\[
8, 70, 276, 766, 1722, 3378, 6006, 9928,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) - 3*a(n-2) + 2*a(n-3) - 2*a(n-4) + 2*a(n-6) - 2*a(n-7) + 3*a(n-8) - 3*a(n-9) + a(n-10).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 08 2018 07:59:47, revision 13)
this is still recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a Burnside sum}

The entry counts $5$-bead necklaces labelled from $-n..n$ with sum zero under a further
...
orbit. For a rotation by $d$ with $c=\gcd(5,d)$ the fixed labellings are exactly the
$c$-periodic words, the whole sum is $(5/c)$ times the sum of one period, and the condition on
the $5$-periodic extension is the condition on the length-$c$ cyclic word; for a reflection
they are the palindromes, determined by $\lceil(5+1)/2\rceil$ values, and the condition is
...
$S=46$, which includes the pre-period $n_0=15$ the conditions with an offset need. The
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 3\,a(n-2) + 2\,a(n-3) - 2\,a(n-4) + 2\,a(n-6) - 2\,a(n-7) + 3\,a(n-8) - 3\,a(n-9) + a(n-10)
\]
for every $n>10$.
\end{theorem}
...
vanish from $n=10+1$ onwards, and the run exceeds $S+10$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A209074.
