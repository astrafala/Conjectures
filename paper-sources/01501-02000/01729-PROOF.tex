...
\title{The empirical closed form for OEIS A219884, proved}
...
\begin{abstract}
OEIS A219884 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=106$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219884 is ``Number of 3 X n arrays of the minimum value of corresponding elements and their horizontal, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 3 X n array.''. It has offset $1$ and begins
\[
10, 21, 47, 129, 292, 600, 1158, 2148,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 28 2018 12:27:45, revision 7)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=106$
...
states number $S=106$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219884 records is correct for all $n>6$.
\end{theorem}
...
$1 + 32 - 1$, which is more than the $10$ consecutive agreements a
...
First, the model reproduces all $32$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219884.
