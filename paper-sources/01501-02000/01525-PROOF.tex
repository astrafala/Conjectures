...
\title{The empirical recurrence for OEIS A269496, proved from an exact model}
...
\begin{abstract}
OEIS A269496 carries an empirical recurrence of order $6$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}5 + 5*n\^{}4 + 10*n\^{}3 + 4*n\^{}2 + 3*n + 1. a(n) = 6*a(n-1) - 15*a(n-2) + 20*a(n-3) - 15*a(n-4) + 6*a(n-5) - a(n-6) for n>6. (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 21 2025 05:20:10, revision 12)
