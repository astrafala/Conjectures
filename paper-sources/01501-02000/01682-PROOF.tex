...
\title{The empirical recurrence for OEIS A208565, proved by counting patterns}
...
\begin{abstract}
OEIS A208565 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
$72$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A208565 is ``Number of nX6 0..2 arrays with new values 0..2 introduced in row major order and no element equal to more than one of its immediate leftward or upward or left-upward diagonal neighbors.''. It has offset $1$ and begins
\[
122, 21632, 4098492, 788922798, 152454784170, 29489112993258, 5705351522369620, 1103893186338679596,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 14:55:47, revision 9) the statement is
...
For every $n$ with $m=n+0-1\ge72$,
\[
3!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-72}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-72}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $72$ matrix--vector products
...
corresponding to $n=73+1$ onwards, and the run continued until $w$ was the zero vector,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208565.
