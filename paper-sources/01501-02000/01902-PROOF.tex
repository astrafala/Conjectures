...
\title{The empirical recurrence for OEIS A198979, proved by counting patterns}
...
\begin{abstract}
OEIS A198979 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $12201$ to $S=213$. The entry's empirical recurrence of order
...
OEIS A198979 is ``Number of nX5 0..5 arrays with values 0..5 introduced in row major order and no element equal to any horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
15, 8731, 12662226, 19622872903, 30525244435834, 47495195899845037, 73900197917712351603, 114985169155743145765664,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 08:02:46, revision 9) the statement is
...
Assembling the $6$ lumped chains into one block-diagonal matrix $N$ of size $S=213$ and
...
\end{equation}
The unlumped alphabet had $12201$ rows in all; the lumped chain has $S=213$ states.
...
\[
6!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-20}q(N)\mathbf{1} .
\]
...
\eqref{eq:walk}, divided by $6!$, was evaluated at every one of the $10$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198979.
