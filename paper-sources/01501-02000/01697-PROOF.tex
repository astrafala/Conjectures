...
\title{The empirical closed form for OEIS A219687, proved}
...
\begin{abstract}
OEIS A219687 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=104$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219687 is ``Number of 3 X n arrays of the minimum value of corresponding elements and their horizontal, vertical or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 3 X n array.''. It has offset $1$ and begins
\[
6, 9, 31, 87, 208, 452, 922, 1799,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2018 15:17:01, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=104$
...
states number $S=104$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219687 records is correct for all $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219687.
