...
\title{The empirical recurrence for OEIS A199902, proved from an exact model}
...
\begin{abstract}
OEIS A199902 carries an empirical recurrence of order $7$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = (151/180)*n\^{}6 + (163/15)*n\^{}5 + (377/9)*n\^{}4 + (395/6)*n\^{}3 + (7429/180)*n\^{}2 + (93/10)*n + 1. a(n) = 7*a(n-1) - 21*a(n-2) + 35*a(n-3) - 35*a(n-4) + 21*a(n-5) - 7*a(n-6) + a(n-7) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (May 17 2018 06:35:05, revision 8)
...
\]
annihilates $a$, and $S=\deg A=9$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=9$, and
