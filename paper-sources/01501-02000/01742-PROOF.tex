...
\title{The empirical recurrence for OEIS A235568, proved by transfer matrix}
...
\begin{abstract}
OEIS A235568 counts arrays of width $4$ over $\{0,1\}$ by reducing every $2\times2$ subblock to a
...
about it is one bit per adjacent pair of columns, so the count is a walk count on $S=237$
states.
\end{abstract}
...
OEIS A235568 is ``Number of (n+1)X(3+1) 0..1 arrays with the difference of the upper and lower median value of each 2X2 subblock in lexicographically nondecreasing order rowwise and columnwise.''. It has offset $1$ and begins
\[
144, 1120, 8046, 53728, 336836, 2000524, 11372978, 62383454,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:39:36, revision 6) this is still
...
f(B)\;=\;v_3-v_2,
\]
the upper median minus the lower median. An array with $R$ rows and $4$ columns has $(R-1)\times3$ subblocks, and they
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=237.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235568.
