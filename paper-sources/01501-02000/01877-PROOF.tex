...
\title{The empirical recurrence for OEIS A250084, proved from an exact model}
...
\begin{abstract}
OEIS A250084 carries an empirical recurrence of order $8$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = (2/5)*n\^{}7 + (19/3)*n\^{}6 + (47/2)*n\^{}5 + 37*n\^{}4 + (183/5)*n\^{}3 + (83/3)*n\^{}2 + (23/2)*n + 1. a(n) = 8*a(n-1) - 28*a(n-2) + 56*a(n-3) - 70*a(n-4) + 56*a(n-5) - 28*a(n-6) + 8*a(n-7) - a(n-8) for  n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 11 2018 09:55:50, revision 8)
