...
\title{The empirical recurrence for OEIS A200456, proved from an exact model}
...
\begin{abstract}
OEIS A200456 carries an empirical recurrence of order $12$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=36$, derived below and not
...
OEIS A200456 is ``Number of -n..n arrays x(0..3) of 4 elements with zero sum and nonzero first and second differences.''. It has offset $1$ and begins
\[
4, 44, 142, 342, 678, 1148, 1832, 2744,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) +2*a(n-3) -2*a(n-4) +a(n-5) -2*a(n-6) +a(n-7) -2*a(n-8) +2*a(n-9) +a(n-11) -a(n-12).
\end{quote}
As of the ``Last modified'' line on the live entry (May 21 2018 09:45:29, revision 8)
...
\]
annihilates $a$, and $S=\deg A=36$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=36$, and
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 2\,a(n-3) - 2\,a(n-4) + a(n-5) - 2\,a(n-6) + a(n-7) - 2\,a(n-8) + 2\,a(n-9) + a(n-11) - a(n-12)
\]
for every $n>0$.
\end{theorem}
...
vanish from $n=0+1$ onwards, and the run exceeds $S+12$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A200456.
