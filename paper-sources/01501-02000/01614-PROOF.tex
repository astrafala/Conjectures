...
\title{The empirical recurrence for OEIS A188213, proved from an exact model}
...
\begin{abstract}
OEIS A188213 carries an empirical recurrence of order $13$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=41$, derived below and not
...
OEIS A188213 is ``Number of nondecreasing arrangements of 6 numbers in -(n+4)..(n+4) with sum zero.''. It has offset $1$ and begins
\[
338, 676, 1242, 2137, 3486, 5444, 8196, 11963,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) - 2*a(n-2) - a(n-3) + 2*a(n-5) - a(n-6) - a(n-7) + 2*a(n-8) - a(n-10) - 2*a(n-11) + 3*a(n-12) - a(n-13).
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 27 2018 08:18:13, revision 8)
...
\]
annihilates $a$, and $S=\deg A=41$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=41$, and
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188213.
