...
\title{The empirical recurrence for OEIS A269016, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A269016 counts arrays over $\{0,\dots,1\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=128$ vertices,
and the empirical recurrence of order $20$ contributed by R.~H.~Hardin then follows from
...
OEIS A269016 is ``Number of 6Xn binary arrays with some element plus some horizontally, diagonally or antidiagonally adjacent neighbor totalling two exactly once.''. It has offset $1$ and begins
\[
0, 420, 4952, 91048, 1053432, 14382480, 164351184, 2008761644,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 17 2016 12:08:31, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;8\,a(n-1) + 160\,a(n-2) - 980\,a(n-3) - 9416\,a(n-4) + 38056\,a(n-5) + 226410\,a(n-6) - 670648\,a(n-7) - 2592568\,a(n-8) + 5911340\,a(n-9) + 15294568\,a(n-10) - 26316136\,a(n-11) - 50480497\,a(n-12) + 59619248\,a(n-13) + 94091288\,a(n-14) - 64298400\,a(n-15) - 91176864\,a(n-16) + 26645760\,a(n-17) + 36563904\,a(n-18) - 3642624\,a(n-19) - 4981824\,a(n-20)
\end{equation}
for all $n>20$.
...
column $t$ and row $u$. Call two cells \emph{adjacent} when they differ by one of the
offsets
\[
\mathcal N=\{(1,0),\ (1,1),\ (1,-1)\},
\]
written as (column shift, row shift), and call an adjacent pair $\{p,q\}$
\emph{marked} when
\[
x_{p}+x_{q}=2 .
\]
Each unordered adjacency is represented by exactly one offset, all with a nonnegative column
shift, so every pair is named once and only once: the pairs inside a column are seen when
that column is laid down, and the pairs joining two columns when the later of the two is. The
entry's requirement is
\begin{equation}\label{eq:glob}
\#\{\text{marked pairs in the whole array}\}\;=1.
\end{equation}

This is not a condition on any bounded window, so no transfer matrix over columns alone can
express it. It becomes one as soon as the running total is part of the state. Let
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,1\}^{6},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=128,
\]
and for columns $r,s$ write $\beta(r,s)$ for the number of marked pairs joining them plus the
...
them --- as a third, absorbing value of $c$ --- would make every pair of columns an edge and
the digraph dense, with $(1+1)^{2\cdot6}$ edges instead of the sparse set used here.
\end{remark}
...
of $L$ columns satisfying \eqref{eq:glob} are exactly the walks of length $L-1$ from a
...
$c_t$ is the number of marked pairs in the first $t$ columns: it holds at $t=1$ by the
definition of $\mathbf{v}$, and the step from $t$ to $t+1$ adds exactly the pairs joining
columns $t$ and $t+1$ together with those inside column $t+1$, which are precisely the marked
pairs of the first $t+1$ columns not already counted --- each unordered pair being named by
...
column, so $L$ columns correspond to a walk of length $L-1$.
\end{proof}
...
$o=1n_0+0-1$,
\[
a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\;=\;\,\mathbf{v}^{\!\top}G^{\,j-20}w,
\qquad w=h_{20}-\sum_i c_i h_{20-i},
\]
for $j\ge20$. Hence \eqref{eq:conj} holds from that point on if and only if
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>20$, which is the statement of
Section~1.
\end{theorem}

\begin{proof}
The digraph was built directly from the definition of a marked pair. The vector $w$ was
formed by $20$ applications of $G$ in exact integer arithmetic and
$\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly. Every one of
those integers vanishes from the index corresponding to $n=20+1$ onwards, and the run
...
first column, changes the counts immediately. Indeed a first version of the computation
placed the walk at length $L-2$ rather than $L-1$ --- the state carries a column, not a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269016.
