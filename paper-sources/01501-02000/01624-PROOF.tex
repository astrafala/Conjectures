...
\title{The empirical closed form for OEIS A220147, proved}
...
\begin{abstract}
OEIS A220147 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=10$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220147 is ``Number of n X 2 arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 n X 2 array.''. It has offset $1$ and begins
\[
3, 11, 26, 52, 95, 163, 266, 416,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/12)*n\^{}4 - (1/6)*n\^{}3 + (29/12)*n\^{}2 + (2/3)*n.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 09:49:10, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=10$
...
states number $S=10$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220147 records is correct for all $n>0$.
\end{theorem}
...
$1 + 39 - 1$, which is more than the $5$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220147.
