...
\title{The empirical recurrence for OEIS A202078, proved from an exact model}
...
\begin{abstract}
OEIS A202078 carries an empirical recurrence of order $13$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=17$, derived below and not
...
OEIS A202078 is ``Number of arrays of 7 integers in -n..n with sum zero and the sum of every adjacent pair being odd.''. It has offset $1$ and begins
\[
6, 96, 1014, 3752, 15010, 35604, 95342, 181834,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 09:26:30, revision 9)
...
\]
annihilates $a$, and $S=\deg A=17$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=17$, and
...
vanish from $n=0+1$ onwards, and the run exceeds $S+13$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202078.
