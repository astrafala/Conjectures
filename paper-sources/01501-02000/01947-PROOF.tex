...
\title{The empirical recurrence for OEIS A221466, proved from an exact model}
...
\begin{abstract}
OEIS A221466 carries an empirical recurrence of order $7$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = 1*n\^{}6 + 4*n\^{}5 + 3*n\^{}4. a(n) = 7*a(n-1) - 21*a(n-2) + 35*a(n-3) - 35*a(n-4) + 21*a(n-5) - 7*a(n-6) + a(n-7) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 05 2018 15:06:02, revision 8)
