...
\title{The empirical recurrence for OEIS A188336, proved from an exact model}
...
\begin{abstract}
OEIS A188336 carries an empirical recurrence of order $16$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=53$, derived below and not
...
OEIS A188336 is ``Number of nondecreasing arrangements of 6 nonzero numbers in -(n+4)..(n+4) with sum zero.''. It has offset $1$ and begins
\[
197, 424, 828, 1488, 2519, 4050, 6252, 9314,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1)-a(n-3)-a(n-5)+2*a(n-8)-a(n-11)-a(n-13)+2*a(n-15)-a(n-16).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 13 2025 23:11:07, revision 14)
...
\]
annihilates $a$, and $S=\deg A=53$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=53$, and
...
$q(z)=z^{16}-\sum_ic_iz^{16-i}$ the characteristic polynomial of the conjectured
...
vanish from $n=0+1$ onwards, and the run exceeds $S+16$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188336.
