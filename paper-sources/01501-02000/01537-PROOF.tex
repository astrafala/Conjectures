...
\title{The empirical recurrence for OEIS A201045, proved from an exact model}
...
\begin{abstract}
OEIS A201045 carries an empirical recurrence of order $7$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = (2101/120)*n\^{}6 + (2101/40)*n\^{}5 + (1753/24)*n\^{}4 + (1405/24)*n\^{}3 + (569/20)*n\^{}2 + (119/15)*n + 1. a(n) = 7*a(n-1) - 21*a(n-2) + 35*a(n-3) - 35*a(n-4) + 21*a(n-5) - 7*a(n-6) + a(n-7) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (May 22 2018 03:49:35, revision 9)
