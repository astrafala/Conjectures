...
\title{The empirical recurrence for OEIS A269439, proved from an exact model}
...
\begin{abstract}
OEIS A269439 carries an empirical recurrence of order $8$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}7 + 7*n\^{}6 + 16*n\^{}5 + (71/3)*n\^{}4 + (139/6)*n\^{}3 + (43/3)*n\^{}2 + (35/6)*n + 1. a(n) = 8*a(n-1) - 28*a(n-2) + 56*a(n-3) - 70*a(n-4) + 56*a(n-5) - 28*a(n-6) + 8*a(n-7) - a(n-8) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 22 2019 06:25:35, revision 8)
