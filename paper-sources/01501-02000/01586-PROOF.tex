...
\title{The empirical recurrence for OEIS A254705, proved from an exact model}
...
\begin{abstract}
OEIS A254705 carries an empirical recurrence of order $9$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = (31/84)*n\^{}8 + (323/35)*n\^{}7 + (473/10)*n\^{}6 + 94*n\^{}5 + (967/12)*n\^{}4 + (377/10)*n\^{}3 + (7927/210)*n\^{}2 + (365/14)*n + 1. a(n) = 9*a(n-1) - 36*a(n-2) + 84*a(n-3) - 126*a(n-4) + 126*a(n-5) - 84*a(n-6) + 36*a(n-7) - 9*a(n-8) + a(n-9) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 17 2018 14:50:53, revision 7)
...
$a$ and $S=12$. Nothing is fitted and no threshold is guessed: the $N_m$ are counted, not
estimated.
