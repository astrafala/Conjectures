...
\title{The empirical recurrence for OEIS A203723, proved by transfer matrix}
...
\begin{abstract}
OEIS A203723 counts arrays of width $4$ over $\{0,1,2\}$ through two derived arrays of PAIR SUMS ---
each cell added to its left neighbour, and each cell added to the one above it --- on which the
entry imposes an order condition. The entry carries an empirical recurrence of order $20$
contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical: the count is
a walk count on $S=82$ states.
\end{abstract}
...
OEIS A203723 is ``Number of (n+1) X 4 0..2 arrays with column and row pair sums b(i,j)=a(i,j)+a(i,j-1) and c(i,j)=a(i,j)+a(i-1,j) nondecreasing in column and row directions, respectively.''. It has offset $1$ and begins
\[
326, 1632, 6249, 20290, 58048, 151649, 367595, 839916,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 21 2025 11:03:44, revision 13) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203723.
