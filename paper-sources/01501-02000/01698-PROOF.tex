...
\title{A lattice count, not a walk count: OEIS A225311}
...
\begin{abstract}
OEIS A225311 carries an empirical recurrence of order $10$ contributed by R.~H.~Hardin. It is
...
$S=96$ the order of a linear recurrence it must satisfy. Whether the conjectured recurrence
...
OEIS A225311 is ``Number of 4 X n -1,1 arrays such that the sum over i=1..4,j=1..n of i*x(i,j) is zero and rows are nondecreasing (ways to put n thrusters pointing east or west at each of 4 positions 1..n distance from the hinge of a south-pointing gate without turning the gate).''. It has offset $1$ and begins
\[
2, 7, 16, 31, 52, 83, 122, 175,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + a(n-2) - 2*a(n-5) + a(n-8) + a(n-9) - a(n-10).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 05 2018 06:13:39, revision 7) this is still
...
becomes $\sum_{i=1}^{4} i\,k_i = T(n)$ with $T(n)=5n$. The arrays are gone: what is left is the lattice points of the box $\{0,\dots,n\}^{4}$ on one hyperplane.
...
where $\sigma(S)$ is the sum of $S$ and $D(m)$ counts the solutions of $\sum i\,k_i=m$ in non-negative integers with no upper bound --- the classical denumerant for the parts $1,\dots,4$. $D$ is a quasi-polynomial in $m$ of degree $3$ with period dividing $\mathrm{lcm}(1,\dots,4)=12$; each argument above is an integer-linear function of $n$, which preserves the degree and can only shrink the period, and the parity case at worst doubles it. So $a$ is a quasi-polynomial of degree at most $3$ and period dividing $24$, hence satisfies a linear recurrence of order at most $S=96$.
...
so $a$ is $C$-finite of order at most $S=96$, and the alignment between the model and the
entry's own $n$ is fixed by matching against every published term.

\section{The proof}

\begin{lemma}\label{lem:crit}
Let $q$ be the characteristic polynomial of the conjectured recurrence, of order $10$,
and let $u_j=a(j+10)-\sum_i c_i\,a(j+10-i)$ be its residual. Then $u$ satisfies the
same linear recurrence of order $S=96$ that $a$ does, so if $u_j=0$ for $S$ consecutive
...
$S=96$ consecutive residuals, and the residuals were formed in exact integer arithmetic.
They vanish from the index corresponding to $n=9+1$ onwards and go on vanishing for more
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A225311.
