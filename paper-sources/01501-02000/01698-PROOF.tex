...
\title{The empirical closed form for OEIS A219829, proved}
...
\begin{abstract}
OEIS A219829 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=54$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219829 is ``Number of 2Xn arrays of the minimum value of corresponding elements and their horizontal, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..3 2Xn array.''. It has offset $1$ and begins
\[
10, 13, 38, 87, 176, 370, 721, 1374,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:20:31, revision 8)
...
$2\times n$ arrays over that range whose rows are lexicographically nondecreasing read downwards
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=54$
...
states number $S=54$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219829 records is correct for all $n>8$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $17$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219829.
