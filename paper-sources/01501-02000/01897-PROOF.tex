...
\title{The empirical recurrence for OEIS A230984, proved by transfer matrix}
...
\begin{abstract}
OEIS A230984 counts the white-square subarrays of arrays of width $4$ over $\{0,1\}$ in which
no cell matches a strict majority of its diagonal neighbours. The entry carries an empirical recurrence of order $8$ contributed by
...
OEIS A230984 is ``Number of white square subarrays of (n+1) X (3+1) binary arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with upper left element zero.''. It has offset $1$ and begins
\[
2, 4, 11, 24, 59, 139, 332, 796,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + 3*a(n-2) - 3*a(n-3) - 6*a(n-4) + 2*a(n-5) + 4*a(n-6) - a(n-7) - a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 18 2018 06:22:57, revision 8) this is still
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 3\,a(n-2) - 3\,a(n-3) - 6\,a(n-4) + 2\,a(n-5) + 4\,a(n-6) - a(n-7) - a(n-8)
\]
for every $n>8$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230984.
