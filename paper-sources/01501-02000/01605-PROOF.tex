...
\title{The empirical closed form for OEIS A219940, proved}
...
\begin{abstract}
OEIS A219940 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=137$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219940 is ``Number of 2 X n arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 2 X n array.''. It has offset $1$ and begins
\[
10, 23, 64, 196, 459, 918, 1660, 2802,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/15)*n\^{}5 - (3/4)*n\^{}4 + (32/3)*n\^{}3 - (135/4)*n\^{}2 + (1583/30)*n - 34 for n>2.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 03:30:26, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=137$
...
states number $S=137$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219940 records is correct for all $n>2$.
\end{theorem}
...
$1 + 34 - 1$, which is more than the $6$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219940.
