...
\title{The empirical closed form for OEIS A219933, proved}
...
\begin{abstract}
OEIS A219933 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=22$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219933 is ``Number of nX2 arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 nX2 array.''. It has offset $1$ and begins
\[
4, 23, 76, 208, 519, 1212, 2670, 5574,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/20160)*n\^{}8 + (1/1680)*n\^{}7 + (1/1440)*n\^{}6 + (23/240)*n\^{}5 - (613/2880)*n\^{}4 + (211/120)*n\^{}3 + (11149/5040)*n\^{}2 + (61/420)*n
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:25:16, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=22$
...
states number $S=22$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219933 records is correct for all $n>0$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219933.
