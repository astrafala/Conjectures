...
\title{The empirical recurrence for OEIS A269643, proved from an exact model}
...
\begin{abstract}
OEIS A269643 carries an empirical recurrence of order $7$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}6 + 6*n\^{}5 + 5*n\^{}4 + 12*n\^{}3 - 12*n\^{}2 + 9*n - 7 for n>2. a(n) = 7*a(n-1) - 21*a(n-2) + 35*a(n-3) - 35*a(n-4) + 21*a(n-5) - 7*a(n-6) + a(n-7) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 25 2019 11:45:06, revision 8)
