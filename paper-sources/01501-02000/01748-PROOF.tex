...
\title{The empirical recurrence for OEIS A270258, proved by an exact quasi-polynomial}
...
\begin{abstract}
OEIS A270258 counts triangular arrays of a FIXED shape over the alphabet $0..n$, and carries an
empirical recurrence of order $10$ contributed by R.~H.~Hardin. It is true. The array
...
OEIS A270258 is ``Number of 3X3X3 triangular 0..n arrays with some element plus some adjacent element totalling n exactly once.''. It has offset $1$ and begins
\[
0, 66, 768, 4428, 14976, 42750,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) +3*a(n-2) -8*a(n-3) -2*a(n-4) +12*a(n-5) -2*a(n-6) -8*a(n-7) +3*a(n-8) +2*a(n-9) -a(n-10)
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 14 2016 09:20:40, revision 6) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array is the triangle with $3$ rows: cells $(i,j)$ for $0\le i<3$ and $0\le j\le i$,
so $T=6$ of them. Two cells are adjacent when they are neighbours in that triangle --- side
by side in a row, or one directly below the other on either side --- and there are
$3\binom{3}{2}=9$ adjacent pairs. Each cell carries a value in
...
A(n)=9n^{5} - 27n^{4} + 36n^{3} - \tfrac{33}{2}n^{2} - 12n + \tfrac{21}{2},
\]
\[
C(n)=6n^{3} - \tfrac{45}{2}n^{2} + 27n - \tfrac{21}{2}.
\]
\end{theorem}
...
involve $n$, so the sum was formed once by running over all $2^{9}$ subsets and
...
The degree of $A$ is $5$ and that of $C$ is $3$. Both are at most $T-1$,
...
\[
a(n)=\begin{cases}
9n^{5} - 27n^{4} + 42n^{3} - 39n^{2} + 15n, & n\ \text{even},\\[2pt]
9n^{5} - 27n^{4} + 30n^{3} + 6n^{2} - 39n + 21, & n\ \text{odd},
\end{cases}
\]
...
(E-1)^{6}(E+1)^{4},
\]
a monic operator of order $10$, and by no proper divisor of it, since the two
...
$t^{10}-\sum_i c_it^{10-i}$; dividing it by the operator above in
...
First, the closed form reproduces all $27$ terms the entry publishes, exactly, in integer
arithmetic.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270258.
