...
\title{The empirical recurrence for OEIS A219398, proved by transfer matrix}
...
\begin{abstract}
OEIS A219398 counts the distinct indicator arrays obtained from the $n\times3$ arrays over
...
indicator row by row turns it into one, on $S=17$ states, after which the conjectured
...
OEIS A219398 is ``Unchanging value maps: number of n X 3 binary arrays indicating the locations of corresponding elements unequal to no king-move neighbor in a random 0..1 n X 3 array.''. It has offset $1$ and begins
\[
4, 4, 16, 43, 122, 345, 979, 2791,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) - 2*a(n-2) + 3*a(n-3) + 4*a(n-4) + 2*a(n-5) - 5*a(n-6) - 2*a(n-7) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 00:24:58, revision 10) this is still
...
\]
There are $S=17$ reachable sets, generated from $D_0$ outward, and $a$ is $C$-finite. Note
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 2\,a(n-2) + 3\,a(n-3) + 4\,a(n-4) + 2\,a(n-5) - 5\,a(n-6) - 2\,a(n-7)
\]
for every $n>8$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219398.
