...
\title{The empirical recurrence for OEIS A269622, proved from an exact model}
...
\begin{abstract}
OEIS A269622 carries an empirical recurrence of order $7$. It is true, and it is
...
which $a$ provably satisfies a MONIC linear recurrence of order $S=32$, derived below and not
...
OEIS A269622 is ``Number of length-6 0..n arrays with no repeated value differing from the previous repeated value by other than plus two, zero or minus 1.''. It has offset $1$ and begins
\[
51, 624, 3611, 14125, 43013, 110099, 248143, 507521,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = n\^{}6 + 6*n\^{}5 + 9*n\^{}4 + 22*n\^{}3 + 9*n\^{}2 + 9*n - 7 for n>2. a(n) = 7*a(n-1) - 21*a(n-2) + 35*a(n-3) - 35*a(n-4) + 21*a(n-5) - 7*a(n-6) + a(n-7) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 25 2019 08:27:48, revision 7)
...
condition relates each repeated value to the previous one. The length is fixed at $6$ and the
...
$6$ positions into $m$ blocks of equal value, listed in increasing value --- together with
...
most $6$, valid as soon as $n$ exceeds the total forced amount. That total is at most
$(m-1)+(\text{constrained pairs})(k+1)\le L+L(k+1)$, so with
\[
n_0 \;=\; L + L(k+1) \;=\; 24
\]
the sequence agrees with a polynomial of degree at most $6$ for every $n\ge n_0$. It is
therefore annihilated by $(z-1)^{6+1}$ from that point, and by $z^{n_0+1}(z-1)^{6+1}$
everywhere, a monic recurrence of order $S=32$. The annihilator was then asked for terms the
...
vanish from $n=9+1$ onwards, and the run exceeds $S+7$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269622.
