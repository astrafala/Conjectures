...
\title{The empirical closed form for OEIS A219817, proved}
...
\begin{abstract}
OEIS A219817 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=156$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219817 is ``Number of 3 X n arrays of the minimum value of corresponding elements and their horizontal, vertical or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 3 X n array.''. It has offset $1$ and begins
\[
6, 16, 62, 165, 377, 795, 1588, 3039,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 28 2018 10:43:32, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=156$
...
states number $S=156$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219817 records is correct for all $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219817.
