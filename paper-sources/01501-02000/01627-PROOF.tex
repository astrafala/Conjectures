...
\title{The empirical closed form for OEIS A220033, proved}
...
\begin{abstract}
OEIS A220033 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=34$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220033 is ``Number of 3 X n arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 3 X n array.''. It has offset $1$ and begins
\[
4, 6, 10, 19, 30, 42, 55, 69,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/2)*n\^{}2 + (13/2)*n - 15 for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 06:14:14, revision 7)
this is still recorded as empirical, and it is still open.

\section{What is being counted}

Fix the width $W=3$ and the value range $0..1$. The underlying arrays are the
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=34$
...
states number $S=34$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220033 records is correct for all $n>3$.
\end{theorem}
...
$1 + 52 - 1$, which is more than the $3$ consecutive agreements a
...
First, the model reproduces all $52$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220033.
