...
\title{The empirical recurrence for OEIS A220538, proved by transfer matrix}
...
\begin{abstract}
OEIS A220538 counts the distinct indicator arrays obtained from the $n\times2$ arrays over
...
indicator row by row turns it into one, on $S=19$ states, after which the conjectured
...
OEIS A220538 is ``Equals one maps: number of 2 X n binary arrays indicating the locations of corresponding elements equal to exactly one of their horizontal, diagonal and antidiagonal neighbors in a random 0..1 2 X n array.''. It has offset $1$ and begins
\[
1, 4, 5, 25, 55, 166, 446, 1240,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) -4*a(n-2) +3*a(n-3) -a(n-4) -7*a(n-5) -7*a(n-6) +7*a(n-8) -6*a(n-9) for n>12.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 31 2018 16:11:59, revision 8) this is still
...
\]
There are $S=19$ reachable sets, generated from $D_0$ outward, and $a$ is $C$-finite. Note
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) - 4\,a(n-2) + 3\,a(n-3) - a(n-4) - 7\,a(n-5) - 7\,a(n-6) + 7\,a(n-8) - 6\,a(n-9)
\]
for every $n>12$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220538.
