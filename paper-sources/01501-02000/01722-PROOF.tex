...
\title{Recovering the unwritten recurrences of the table OEIS A232406}
...
\begin{abstract}
OEIS A232406 is a two-dimensional table $T(n,k)$ whose empirical recurrences are, for several of
its lines, given only as an ORDER --- ``k=2: [order 9]'' and the like --- with the
...
OEIS A232406 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or vertically, with no adjacent elements equal.''. It is read by antidiagonals and begins
\[
4, 4, 4, 8, 16, 8, 16, 20,\ \dots
\]
The entry carries, among its empirical claims:
\begin{quote}\small
k=2: [order 9]
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 07:24:50, revision 6) these are still
...
$c_{1}$ & $0$ & $c_{4}$ & $-3$ & $c_{7}$ & $6$ &  &  \\
$c_{2}$ & $5$ & $c_{5}$ & $-12$ & $c_{8}$ & $6$ &  &  \\
$c_{3}$ & $4$ & $c_{6}$ & $-3$ & $c_{9}$ & $2$ &  &  \\
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232406.
