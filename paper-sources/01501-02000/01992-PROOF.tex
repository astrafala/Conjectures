...
\title{The empirical recurrence for OEIS A198656, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A198656 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
$8!\,a(n)=\sum_i\binom{8}{i}D_{8-i}L_i(n)$ with $D_m$ the derangement numbers and
...
lumpable, cutting $8772$ pairs down to $S=98$ states. The entry's empirical recurrence of
order $24$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A198656 is ``Number of nX2 0..7 arrays with values 0..7 introduced in row major order and each element equal to at least one horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
1, 3, 14, 76, 488, 3521, 28030, 242883,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 07:58:13, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;50\,a(n-1) - 973\,a(n-2) + 8874\,a(n-3) - 29927\,a(n-4) - 92230\,a(n-5) + 939984\,a(n-6) - 1157338\,a(n-7) - 7025352\,a(n-8) + 14473974\,a(n-9) + 29278108\,a(n-10) - 54017762\,a(n-11) - 108264239\,a(n-12) + 78120984\,a(n-13) + 276328733\,a(n-14) + 56831952\,a(n-15) - 336850101\,a(n-16) - 307076936\,a(n-17) + 88644128\,a(n-18) + 275952224\,a(n-19) + 124163560\,a(n-20) - 45062112\,a(n-21) - 62248400\,a(n-22) - 20707200\,a(n-23) - 2210000\,a(n-24)
\end{equation}
for all $n>25$.
...
class, so with $K=8$ available values the entry counts admissible patterns with at most $K$
classes.
...
\[
8!\,\sum_{j\le 8}N_j(n)\;=\;\sum_{i=0}^{8}\binom{8}{i}D_{8-i}\,L_i(n),
...
$j!N_j=\sum_i(-1)^{j-i}\binom{j}{i}L_i$. Summing over $j\le8$ and exchanging the
order of summation, the coefficient of $L_i$ is
$\sum_{t\le8-i}(-1)^t/(i!\,t!)$, which multiplied by $8!$ is
$\binom{8}{i}(8-i)!\sum_{t\le8-i}(-1)^t/t!=\binom{8}{i}D_{8-i}$.
\end{proof}
...
Assembling the $8$ lumped chains block-diagonally into $N$ of size $S=98$, with the
...
8!\;a(n)\;=\;\mathbf{v}^{\!\top}N^{\,L-2}\mathbf{u},\qquad L=n .
\end{equation}
...
index,
\[
8!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-24}\,
\Bigl(h_{24}-\sum_i c_i h_{24-i}\Bigr)
\]
...
$w=h_{24}-\sum_i c_ih_{24-i}$; and it is enough to check $m=0,1,\dots,S-1$.
\end{lemma}

\begin{proof}
Substitute \eqref{eq:walk2} and factor. The scalar $8!$ is nonzero, so it does not
...
index corresponding to $n=25+1$ onwards, and the run was continued until $S+1$
...
\eqref{eq:walk2}, divided by $8!$, was evaluated at every one of the $20$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A198656.
