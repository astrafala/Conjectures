...
\title{The empirical closed form for OEIS A219350, proved}
...
\begin{abstract}
OEIS A219350 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=81$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219350 is ``Number of n X 4 arrays of the minimum value of corresponding elements and their horizontal, vertical, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 n X 4 array.''. It has offset $1$ and begins
\[
4, 4, 15, 35, 81, 174, 364, 740,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/40320)*n\^{}8 - (1/1120)*n\^{}7 + (5/192)*n\^{}6 - (2/5)*n\^{}5 + (7429/1920)*n\^{}4 - (3037/160)*n\^{}3 + (34483/2016)*n\^{}2 + (71507/280)*n - 791 for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2018 05:16:30, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=81$
...
states number $S=81$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219350 records is correct for all $n>7$.
\end{theorem}
...
$1 + 33 - 1$, which is more than the $9$ consecutive agreements a
...
First, the model reproduces all $33$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219350.
