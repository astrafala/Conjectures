...
\title{The empirical recurrence for OEIS A275395, proved by counting patterns}
...
\begin{abstract}
OEIS A275395 counts arrays in which no entry may repeat a value found at any of three stated
...
cutting $98$ pairs to $S=23$ states. The entry's empirical recurrence of order $9$,
...
OEIS A275395 is ``Number of n X 2 0..2 arrays with no element equal to any value at offset (-2,-1) (-2,0) or (-1,1) and new values introduced in order 0..2.''. It has offset $1$ and begins
\[
2, 9, 16, 31, 63, 129, 260, 534,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) + a(n-2) - 3*a(n-3) + a(n-4) + 4*a(n-5) - 3*a(n-6) + a(n-7) -  2*a(n-8) + a(n-9) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 04 2019 09:08:25, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + a(n-2) - 3\,a(n-3) + a(n-4) + 4\,a(n-5) - 3\,a(n-6) + a(n-7) - 2\,a(n-8) + a(n-9)
\end{equation}
for all $n>10$.
...
Fix $i$, let $V_i=\{0,\dots,i-1\}^{2}$ and take $\mathcal V=V_i\times V_i$ as vertices.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A275395.
