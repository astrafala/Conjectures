...
\title{The empirical recurrence for OEIS A231242, proved by transfer matrix}
...
\begin{abstract}
OEIS A231242 counts arrays of width $4$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
the count is a walk count on $S=2869$ states.
\end{abstract}
...
OEIS A231242 is ``Number of n X 4 0..3 arrays x(i,j) with each element horizontally, diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
0, 44, 464, 5992, 76136, 968824, 12322784, 156753000,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 10*a(n-1) + 30*a(n-2) + 53*a(n-3) + 71*a(n-4) - a(n-5) + 6*a(n-6) - 8*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 27 2018 15:34:15, revision 7) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=2869.
\]
...
\begin{theorem}
\[
a(n)\;=\;10\,a(n-1) + 30\,a(n-2) + 53\,a(n-3) + 71\,a(n-4) - a(n-5) + 6\,a(n-6) - 8\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231242.
