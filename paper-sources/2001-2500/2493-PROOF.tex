...
\title{The empirical recurrence for OEIS A263459, proved by transfer matrix}
...
\begin{abstract}
OEIS A263459 counts arrays of width $2$ over $\{0,1,\dots,4\}$ whose rows, read as base-$5$
numbers, are divisible by $11$ and whose columns, read the same way, are divisible by
$11$. The entry carries an empirical recurrence of order $6$ contributed by
...
OEIS A263459 is ``Number of (n+1) X (1+1) 0..4 arrays with each row and column divisible by 11, read as a base-5 number with top and left being the most significant digits.''. It has offset $1$ and begins
\[
2, 3, 8, 23, 67, 200, 597, 1790,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + a(n-5) - 3*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 01 2019 20:13:45, revision 9) this is still
...
\gamma_j^{(i)}\bmod 11\;=\;
\bigl(5\,(\gamma_j^{(i-1)}\bmod 11)+x(i,j)\bigr)\bmod 11 .
\]
\end{lemma}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263459.
