...
\title{The empirical column recurrences for the table OEIS A264364}
...
\begin{abstract}
OEIS A264364 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264364 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 0,0 0,2 1,0 or -1,-2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 3, 1, 9, 6, 1, 18, 36,\ \dots
\]
...
k=6: [order 30]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 12 2015 07:01:27, revision 4) these are still
...
column $k$$=1$ & $1$ & $16$ & $>1$ & $9$ \\
column $k$$=2$ & $3$ & $64$ & $>1$ & $8$ \\
column $k$$=3$ & $6$ & $256$ & $>4$ & $10$ \\
column $k$$=4$ & $6$ & $1024$ & $>5$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 0,0 0,2 1,0 or -1,-2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having directed index change 0,0 0,2 1,0 or -1,-2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(4) arrays of permutations of 0..(n+1)*(4)-1 with each element having directed index change 0,0 0,2 1,0 or -1,-2.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1) + 7\,a(n-2) + 3\,a(n-3) - 5\,a(n-4) + 3\,a(n-5) - a(n-6)
\]
holds from index $4+1$ on.

\subsection*{Column $k$$=4$}
The sequence is ``Number of (n+1)X(5) arrays of permutations of 0..(n+1)*(5)-1 with each element having directed index change 0,0 0,2 1,0 or -1,-2.''. Its rows are the vertices of a digraph on
$S=1024$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1) + 28\,a(n-2) + 57\,a(n-3) + 10\,a(n-4) - 24\,a(n-5) + 8\,a(n-6)
\]
...
model reproduces $37$ published values, $126$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264364.
