...
\title{The empirical recurrence for OEIS A223285, proved by transfer matrix}
...
\begin{abstract}
OEIS A223285 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$4$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is dodecahedron graph. The entry carries an empirical recurrence of order $6$
...
single slice is a state and the count is a walk count on a finite digraph with $S=9958$ vertices.
\end{abstract}
...
OEIS A223285 is ``Rolling icosahedron face footprints: number of 4 X n 0..19 arrays starting with 0 where 0..19 label faces of an icosahedron and every array movement to a horizontal, diagonal or antidiagonal neighbor moves across an icosahedral edge.''. It has offset $1$ and begins
\[
8000, 375, 8295, 72279, 1024071, 10979127, 137621799, 1576368663,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-1) + 92*a(n-2) - 56*a(n-3) - 920*a(n-4) + 192*a(n-5) + 1152*a(n-6) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 18 2018 15:44:23, revision 9) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=9958.
\]
...
of length $4$ over $20$ vertices is one of $20^{4}$ words, but only $S-1$ of them are
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-1) + 92\,a(n-2) - 56\,a(n-3) - 920\,a(n-4) + 192\,a(n-5) + 1152\,a(n-6)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223285.
