...
\title{The empirical recurrence for OEIS A253021, proved by transfer matrix}
...
\begin{abstract}
OEIS A253021 counts arrays of width $6$ over an alphabet of $4$ letters in which every three
...
brings the state count down to $S=1619$.
\end{abstract}
...
OEIS A253021 is ``Number of (n+2) X (4+2) 0..3 arrays with every consecutive three elements in every row and column not having exactly two distinct values, and in every diagonal and antidiagonal having exactly two distinct values, and new values 0 upwards introduced in row major order.''. It has offset $1$ and begins
\[
63, 91, 120, 155, 230, 296, 369, 546,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-3) - a(n-6) - a(n-9) for n>12.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 08 2018 11:41:51, revision 8) this is still
...
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,
\]
with $S=1619$ states in all. The walk index $j$ and the entry's own $n$ differ by a constant,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253021.
