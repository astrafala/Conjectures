...
\title{The empirical column recurrences for the table OEIS A260207}
...
\begin{abstract}
OEIS A260207 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A260207 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00000000 00000001 or 00010001.''. It has offset $1$, is
read by antidiagonals, and begins
\[
26, 44, 44, 106, 80, 106, 220, 207,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 2*a(n-1) +a(n-2) -2*a(n-4) -2*a(n-5) for n>7\\
k=2: a(n) = a(n-1) +a(n-2) +5*a(n-3) +2*a(n-4) +a(n-5) -a(n-7) -a(n-8) for n>9\\
k=3: [order 11] for n>12\\
k=4: [order 18] for n>19\\
k=5: [order 32] for n>34\\
k=6: [order 45] for n>46\\
k=7: [order 80] for n>83\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 12:03:22, revision 9) these are still
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00000000 00000001 or 00010001.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;2\,a(n-1) + a(n-2) - 2\,a(n-4) - 2\,a(n-5)
\]
holds from index $7+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+2)X(4) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00000000 00000001 or 00010001.''. Its rows are the vertices of a digraph on
...
model reproduces $19$ published values, $67$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A260207.
