...
\title{The empirical column recurrences for the table OEIS A260927}
...
\begin{abstract}
OEIS A260927 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A260927 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 00101011 or 01010101.''. It has offset $1$, is
read by antidiagonals, and begins
\[
36, 48, 48, 104, 68, 104, 186, 146,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-2) +3*a(n-4) +2*a(n-5) +2*a(n-6) +3*a(n-7) +a(n-8) for n>10\\
k=2: a(n) = a(n-2) +6*a(n-4) +4*a(n-6) -a(n-8) for n>10\\
k=3: a(n) = a(n-2) +11*a(n-4) +13*a(n-6) -2*a(n-8) -2*a(n-10) +4*a(n-12) for n>15\\
k=4: [order 20] for n>22\\
k=5: [order 27] for n>28\\
k=6: [order 40] for n>42\\
k=7: [order 54] for n>59\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 12:12:54, revision 6) these are still
...
column $k$$=1$ & $8$ & $64$ & $>10$ & $10$ \\
column $k$$=2$ & $8$ & $256$ & $>10$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 00101011 or 01010101.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;a(n-2) + 3\,a(n-4) + 2\,a(n-5) + 2\,a(n-6) + 3\,a(n-7) + a(n-8)
\]
holds from index $10+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+2)X(4) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00010101 00101011 or 01010101.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-2) + 6\,a(n-4) + 4\,a(n-6) - a(n-8)
\]
holds from index $10+1$ on.
...
model reproduces $20$ published values, $63$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A260927.
