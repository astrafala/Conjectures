...
\title{The empirical recurrence for OEIS A208266, proved by counting patterns}
...
\begin{abstract}
OEIS A208266 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $33$ to $S=17$. The entry's empirical recurrence of order
...
OEIS A208266 is ``Number of nX5 0..1 arrays with new values 0..1 introduced in row major order and no element equal to more than two of its immediate leftward or upward or left-upward diagonal neighbors.''. It has offset $1$ and begins
\[
16, 317, 6631, 137845, 2867739, 59655167, 1240971177, 25815151595,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 21*a(n-1) +9*a(n-2) -278*a(n-3) +73*a(n-4) +790*a(n-5) -662*a(n-6) +29*a(n-7) +69*a(n-8) -10*a(n-9)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 14:43:58, revision 9) the statement is
...
Assembling the $2$ lumped chains into one block-diagonal matrix $N$ of size $S=17$ and
...
\end{equation}
The unlumped alphabet had $33$ rows in all; the lumped chain has $S=17$ states.
...
\[
2!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-9}q(N)\mathbf{1} .
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208266.
