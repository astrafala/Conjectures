...
\title{The empirical recurrence for OEIS A223200, proved by transfer matrix}
...
\begin{abstract}
OEIS A223200 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$6$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is octahedron graph. The entry carries an empirical recurrence of order $7$
...
single slice is a state and the count is a walk count on a finite digraph with $S=6145$ vertices.
\end{abstract}
...
OEIS A223200 is ``Rolling cube footprints: number of nX6 0..5 arrays starting with 0 where 0..5 label faces of a cube and every array movement to a horizontal or vertical neighbor moves across a corresponding cube edge.''. It has offset $1$ and begins
\[
1024, 995328, 1020002304, 1061444124672, 1110327429169152, 1163614255186968576, 1220273386789986631680, 1280001086812985197854720,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 21:20:32, revision 8) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=6145.
\]
...
\begin{theorem}
\[
a(n)\;=\;1920\,a(n-1) - 1191936\,a(n-2) + 335020032\,a(n-3) - 48670703616\,a(n-4) + 3778497478656\,a(n-5) - 147884313935872\,a(n-6) + 2269391999729664\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223200.
