...
\title{The empirical recurrence for OEIS A232517, proved by transfer matrix}
...
\begin{abstract}
OEIS A232517 counts arrays of width $3$ over $\{0,1,2\}$ in which every cell sees its own value plus and minus one among its neighbours. The entry carries an
...
OEIS A232517 is ``Number of (2+1)X(n+1) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally, diagonally or antidiagonally, with no adjacent elements equal.''. It has offset $1$ and begins
\[
4, 52, 32, 326, 250, 2138, 1956, 13976,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-2) +a(n-3) +3*a(n-4) +a(n-5) -2*a(n-6) -2*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 07:28:44, revision 8) this is still
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-2) + a(n-3) + 3\,a(n-4) + a(n-5) - 2\,a(n-6) - 2\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232517.
