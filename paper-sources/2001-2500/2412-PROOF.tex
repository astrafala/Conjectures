...
\title{The empirical recurrence for OEIS A262236, proved by transfer matrix}
...
\begin{abstract}
OEIS A262236 counts arrays of width $4$ over $\{0,1\}$ whose rows, read as base-$2$
numbers, are divisible by $11$ and whose columns, read the same way, are divisible by
$11$. The entry carries an empirical recurrence of order $7$ contributed by
...
is then a walk count on $S=11$ states.
\end{abstract}
...
OEIS A262236 is ``Number of (n+3) X (1+3) 0..1 arrays with each row and column divisible by 11, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
2, 3, 6, 12, 24, 47, 94, 187,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) -2*a(n-2) -a(n-5) +3*a(n-6) -2*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 25 2021 10:56:19, revision 9) this is still
...
So the vector $\bigl(\gamma_1\bmod 11,\dots,\gamma_{4}\bmod
11\bigr)$ is a state: at most $11^{4}$ of them, and each array row
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 2\,a(n-2) - a(n-5) + 3\,a(n-6) - 2\,a(n-7)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262236.
