...
\title{The empirical column recurrences for the table OEIS A234738}
...
\begin{abstract}
OEIS A234738 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234738 is ``T(n,k) is the number of (n+1) X (k+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 8 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
168, 672, 672, 2184, 2184, 2184, 8736, 6048,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 20:26:02, revision 7) these are still
...
The sequence is ``Number of (n+1) X (2) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 8 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
values, $53$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234738.
