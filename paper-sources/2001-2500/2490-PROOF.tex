...
\title{The empirical column recurrences for the table OEIS A231997}
...
\begin{abstract}
OEIS A231997 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231997 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with no element having a strict majority of its horizontal and antidiagonal neighbors equal to one.''. It has offset $1$, is
read by antidiagonals, and begins
\[
4, 6, 9, 16, 32, 25, 39, 121,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:47:04, revision 6) these are still
...
column $k$$=1$ & $3$ & $11$ & $>2$ & $8$ \\
column $k$$=2$ & $6$ & $42$ & $>6$ & $10$ \\
column $k$$=3$ & $6$ & $127$ & $>6$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) 0..1 arrays with no element having a strict majority of its horizontal and antidiagonal neighbors equal to one.''. Its rows are the vertices of a digraph on
$S=11$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+1)X(3) 0..1 arrays with no element having a strict majority of its horizontal and antidiagonal neighbors equal to one.''. Its rows are the vertices of a digraph on
$S=42$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1) + 12\,a(n-2) - 5\,a(n-3) - 19\,a(n-4) + 2\,a(n-5) + 4\,a(n-6)
\]
holds from index $6+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+1)X(4) 0..1 arrays with no element having a strict majority of its horizontal and antidiagonal neighbors equal to one.''. Its rows are the vertices of a digraph on
$S=127$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;9\,a(n-1) - 3\,a(n-2) - 25\,a(n-3) + 9\,a(n-4) + 3\,a(n-5) - a(n-6)
\]
holds from index $6+1$ on.
...
The entry gives only the order for this line: ``k=4: [order 14]''. Its model is a walk on
$S=406$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=812$ exact terms returns the minimal one, of order
$14$ --- the stated order --- with integer coefficients
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $15$ & $c_{5}$ & $2289$ & $c_{9}$ & $1713$ & $c_{13}$ & $6$ \\
$c_{2}$ & $10$ & $c_{6}$ & $-2814$ & $c_{10}$ & $-1638$ & $c_{14}$ & $-4$ \\
$c_{3}$ & $-363$ & $c_{7}$ & $-3711$ & $c_{11}$ & $-219$ &  &  \\
$c_{4}$ & $300$ & $c_{8}$ & $4416$ & $c_{12}$ & $181$ &  &  \\
\end{tabular}\end{center}
Repeating with the first $14+4$ terms removed gives order $14$ again.

\subsection*{Row $n=2$, recovered}
The entry gives only the order for this line: ``n=2: [order 15]''. Its model is a walk on
$S=39$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=78$ exact terms returns the minimal one, of order
$15$ --- the stated order --- with integer coefficients
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $3$ & $c_{5}$ & $-7$ & $c_{9}$ & $9$ & $c_{13}$ & $-1$ \\
$c_{2}$ & $-3$ & $c_{6}$ & $11$ & $c_{10}$ & $-5$ & $c_{14}$ & $1$ \\
$c_{3}$ & $11$ & $c_{7}$ & $-35$ & $c_{11}$ & $13$ & $c_{15}$ & $-1$ \\
$c_{4}$ & $5$ & $c_{8}$ & $-11$ & $c_{12}$ & $3$ &  &  \\
\end{tabular}\end{center}
...
model reproduces $28$ published values, $123$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231997.
