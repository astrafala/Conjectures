...
\title{The empirical recurrence for OEIS A269279, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A269279 counts arrays over $\{0,\dots,3\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=512$ vertices,
and the empirical recurrence of order $8$ contributed by R.~H.~Hardin then follows from
...
OEIS A269279 is ``Number of 4Xn 0..3 arrays with some element plus some horizontally or antidiagonally adjacent neighbor totalling three exactly once.''. It has offset $1$ and begins
\[
0, 20412, 1365336, 74853576, 3719884392, 174924572760, 7934992835112, 350946381867480,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 21 2016 08:59:59, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;96\,a(n-1) - 3108\,a(n-2) + 40720\,a(n-3) - 265326\,a(n-4) + 931200\,a(n-5) - 1766452\,a(n-6) + 1678992\,a(n-7) - 622521\,a(n-8)
\end{equation}
for all $n>12$.
...
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,3\}^{4},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=512,
\]
...
the digraph dense, with $(3+1)^{2\cdot4}$ edges instead of the sparse set used here.
\end{remark}
...
$o=1n_0+0-1$,
\[
a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\;=\;\,\mathbf{v}^{\!\top}G^{\,j-8}w,
\qquad w=h_{8}-\sum_i c_i h_{8-i},
\]
for $j\ge8$. Hence \eqref{eq:conj} holds from that point on if and only if
...
formed by $8$ applications of $G$ in exact integer arithmetic and
...
every one of the $13$ published indices, with no shift fitted: the number of columns is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269279.
