...
\title{The conjectured generating function for OEIS A202049, proved}
...
\begin{abstract}
OEIS A202049 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=211$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $229$ coefficients in exact arithmetic
...
OEIS A202049 is ``Number of (n+2) X 7 binary arrays avoiding patterns 001 and 110 in rows and columns.''. It has offset $1$ and begins
\[
966, 3304, 9170, 22092, 47950, 95984,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 26 2018 03:34:53, revision 9) this is still
...
\]
and there are $S=211$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $219$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{219}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $229$
of them --- more than the $219+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{8}$ read off $D_2$, the first few being $8,\ -28,\ 56,\ -70,\ 56,\ -28$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202049.
