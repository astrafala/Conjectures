...
\title{The empirical recurrence for OEIS A247283, derived from the entry's generating function}
...
\begin{abstract}
OEIS A247283 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A247283 is ``Positions of subrecords in A048673.''. It has offset $1$ and begins
\[
5, 7, 9, 15, 18, 27, 36, 54,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2020 20:21:14, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{6} - x^{4} + x^{3} - 7 x^{2} - 5 x}{2 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - 2 x^{2}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 3 x^{6} + x^{4} - x^{3} + 7 x^{2} + 5 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A247283.
