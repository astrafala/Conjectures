...
\title{The empirical recurrence for OEIS A183494, proved by transfer matrix}
...
\begin{abstract}
OEIS A183494 counts arrays of width $1$ over $\{0,\dots,2\}$ under a condition on each cell
...
a walk count on $S=12$ of them, and the conjectured recurrence is then settled by a finite
exact computation.
\end{abstract}
...
OEIS A183494 is ``Half the number of nX1 0..2 arrays with no element equal to the sum mod 3 of its horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
1, 3, 4, 8, 18, 32, 64, 132,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n)=2*a(n-1)+2*a(n-3)-4*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 21 2025 04:14:41, revision 13) this is still
...
terms. There are $S=12$ states, and $a$ is $C$-finite.
...
first term. There are $3^{1}=3$ possible rows over the entry's own alphabet. The state count $S=12$ is exactly $3\cdot4$,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183494.
