...
\title{The empirical recurrence for OEIS A190035, derived from the entry's generating function}
...
\begin{abstract}
OEIS A190035 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A190035 is ``Number of nondecreasing arrangements of n+2 numbers in 0..3 with the last equal to 3 and each after the second equal to the sum of one or two of the preceding three.''. It has offset $1$ and begins
\[
5, 7, 10, 14, 18, 22, 26, 30,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 25 2025 10:12:06, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} + x^{3} - 3 x^{2} + 5 x}{x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A190035.
