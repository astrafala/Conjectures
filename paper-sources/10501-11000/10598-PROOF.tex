...
\title{The empirical recurrence for OEIS A251518, proved by transfer matrix}
...
\begin{abstract}
OEIS A251518 counts $3$-column arrays over $\{0,\dots,3\}$ in which every
$2\times2$ subblock sums to one of $\{2, 3, 4, 5, 6, 7, 8, 9, 10\}$, and carries an empirical
recurrence of order $6$ contributed by R.~H.~Hardin. It is true, and it is not an
...
OEIS A251518 is ``Number of (n+1)X(2+1) 0..3 arrays with every 2X2 subblock summing to 2 3 4 5 6 7 8 9 or 10.''. It has offset $1$
and begins
\[
3798, 227174, 13580282, 811863390, 48535019898, 2901533826974, 173460282721706, 10369849728580798,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 59*a(n-1) +67*a(n-2) -1223*a(n-3) +782*a(n-4) +1020*a(n-5) -520*a(n-6)
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:27:08, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;59\,a(n-1) + 67\,a(n-2) - 1223\,a(n-3) + 782\,a(n-4) + 1020\,a(n-5) - 520\,a(n-6)
\end{equation}
for all $n>5$, which is every $n$ at which a recurrence of order $6$ can be
stated.
...
Let $q(t)=t^{6}-\sum_{i}c_i\,t^{6-i}$ be the characteristic polynomial of
\eqref{eq:conj}, with the $c_i$ read off that equation.

\begin{lemma}\label{lem:crit}
For every $n\ge6$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n-6}\,q(M)\,\mathbf{1} .
\]
Hence \eqref{eq:conj} holds for every $n\ge6$ if and only if
...
=\mathbf{1}^{\!\top}M^{\,n-6}\Bigl(M^{6}-\sum_i c_i M^{6-i}\Bigr)\mathbf{1},
\]
which is the stated identity. The equivalence follows by letting $n-6=j$ run over
...
\begin{proof}
The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $6$ matrix--vector
...
$j=5-6+1$ onwards, and the run was continued past $S$ consecutive zeros, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251518.
