...
\title{The empirical recurrence for OEIS A190039, derived from the entry's generating function}
...
\begin{abstract}
OEIS A190039 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A190039 is ``Number of nondecreasing arrangements of n+2 numbers in 0..7 with the last equal to 7 and each after the second equal to the sum of one or two of the preceding three.''. It has offset $1$ and begins
\[
11, 17, 22, 31, 43, 60, 80, 100,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 04 2018 08:11:12, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{7} + 5 x^{6} + 3 x^{5} + 4 x^{4} - x^{3} - 5 x^{2} + 11 x}{x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{7} + 5 x^{6} + 3 x^{5} + 4 x^{4} - x^{3} - 5 x^{2} + 11 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A190039.
