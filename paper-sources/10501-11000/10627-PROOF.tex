...
\title{The empirical recurrence for OEIS A266846, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266846 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A266846 is ``Decimal representation of the n-th iteration of the "Rule 70" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 6, 20, 104, 336, 1696, 5440, 27264,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{2} + 4 x + 1}{32 x^{3} - 16 x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;32 x^{3} - 16 x^{2} - 2 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266846.
