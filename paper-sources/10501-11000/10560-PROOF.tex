...
\title{The empirical recurrence for OEIS A159715, derived from the entry's generating function}
...
\begin{abstract}
OEIS A159715 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A159715 is ``Number of permutations of 2 indistinguishable copies of 1..n arranged in a circle with exactly 1 local maximum.''. It has offset $2$ and begins
\[
4, 18, 72, 270, 972, 3402, 11664, 39366,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 30 2026 20:07:17, revision 25) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{3} + 4 x^{2}}{9 x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;9 x^{2} - 6 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A159715.
