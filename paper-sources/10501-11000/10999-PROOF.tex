...
\title{The empirical recurrence for OEIS A186455, proved by transfer matrix}
...
\begin{abstract}
OEIS A186455 counts the $(n+1)\times3$ arrays over $\{0,\dots,1\}$ in which
...
a state, the count is a walk count on $S=21$ of them, and the conjectured recurrence is then
...
OEIS A186455 is ``Number of (n+1) X 3 binary arrays with every 2 X 2 subblock commuting with each of its horizontal and vertical 2 X 2 subblock neighbors.''. It has offset $1$ and begins
\[
21, 30, 51, 88, 128, 197, 292, 457,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - 2*a(n-3) + 2*a(n-4) - a(n-5) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 17 2018 14:13:39, revision 9) this is still
...
$21$, which the model reproduces along with every later term.
...
\]
on the $S=21$ admissible $2$-tuples. In particular $a$ is $C$-finite.
...
size of the construction is therefore $2^{2\cdot3}$ before the condition prunes
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A186455.
