...
\title{The empirical recurrence for OEIS A301781, proved by transfer matrix}
...
\begin{abstract}
OEIS A301781 counts $5$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the four horizontal and vertical neighbours, a number $equal to$ itself lying in $\{0,1,2,3\}$, and it carries an empirical
recurrence of order $8$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A301781 is ``Number of nX5 0..1 arrays with every element equal to 0, 1, 2 or 3 horizontally or vertically adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
16, 512, 13616, 369936, 10033408, 272151040, 7381982784, 200232929792,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 24*a(n-1) +92*a(n-2) -156*a(n-3) -1088*a(n-4) -480*a(n-5) +3200*a(n-6) +3200*a(n-7) -4608*a(n-8)
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 26 2018 16:26:04, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;24\,a(n-1) + 92\,a(n-2) - 156\,a(n-3) - 1088\,a(n-4) - 480\,a(n-5) + 3200\,a(n-6) + 3200\,a(n-7) - 4608\,a(n-8)
\end{equation}
for all $n>8$.
...
has score in $\{0,1,2,3\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-8}$ whenever $n-8\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-8$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-8}$ out of
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>8$. This is the statement of
Section~1.
\end{theorem}

\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $8$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=8+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A301781.
