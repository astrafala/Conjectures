...
\title{The empirical recurrence for OEIS A187607, derived from the entry's generating function}
...
\begin{abstract}
OEIS A187607 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 25 2018 11:39:10, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{5 x^{6} - 19 x^{5} - 9 x^{4} - 9 x^{3}}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 5 x^{6} + 19 x^{5} + 9 x^{4} + 9 x^{3},
\]
