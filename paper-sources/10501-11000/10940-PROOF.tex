...
\title{The empirical recurrence for OEIS A266753, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266753 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A266753 is ``Decimal representation of the n-th iteration of the "Rule 163" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 4, 18, 74, 298, 1194, 4778, 19114,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 49) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{2} - x + 1}{4 x^{2} - 5 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266753.
