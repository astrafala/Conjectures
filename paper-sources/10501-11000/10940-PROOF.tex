...
\title{The empirical recurrence for OEIS A237228, proved by transfer matrix}
...
\begin{abstract}
OEIS A237228 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=268$
...
OEIS A237228 is ``Number of (n+1) X (2+1) 0..2 arrays with the maximum plus the minimum of every 2 X 2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$ and begins
\[
268, 984, 3284, 10888, 36180, 120540, 401520, 1336540,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + 4*a(n-2) + 5*a(n-3) + 21*a(n-4) + 12*a(n-5) + 4*a(n-6) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 22 2018 01:37:53, revision 9) this is still
...
differ by $1$. The entry's first term $268$ counts those, and the model was made to
...
\]
over the $S=268$ pairs whose own subblock row already satisfies the condition along itself.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237228.
