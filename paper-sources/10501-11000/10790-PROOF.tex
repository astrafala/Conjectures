...
\title{The empirical recurrence for OEIS A269464, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269464 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269464 is ``Number of length-n 0..5 arrays with no repeated value equal to the previous repeated value.''. It has offset $1$ and begins
\[
6, 36, 210, 1230, 7170, 41730, 242370, 1405530,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 22 2019 08:20:05, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 54 x^{3} - 18 x^{2} + 6 x}{50 x^{3} + 10 x^{2} - 9 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;50 x^{3} + 10 x^{2} - 9 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269464.
