...
\title{The empirical recurrence for OEIS A223278, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223278 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 18 2018 11:23:25, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 108 x^{2} + 27 x}{36 x^{2} - 17 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;36 x^{2} - 17 x + 1
\]
