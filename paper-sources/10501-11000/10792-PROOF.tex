...
\title{The empirical recurrence for OEIS A269466, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269466 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269466 is ``Number of length-n 0..7 arrays with no repeated value equal to the previous repeated value.''. It has offset $1$ and begins
\[
8, 64, 504, 3976, 31304, 246232, 1934856, 15190840,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 22 2019 11:34:58, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 104 x^{3} - 40 x^{2} + 8 x}{98 x^{3} + 28 x^{2} - 13 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;98 x^{3} + 28 x^{2} - 13 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269466.
