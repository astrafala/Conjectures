...
\title{A condition on each element and its neighbours: OEIS A240701}
...
\begin{abstract}
OEIS A240701 carries an empirical recurrence of order $7$ contributed by R.~H.~Hardin. It is
...
OEIS A240701 is ``Number of n X 3 0..3 arrays with no element equal to a different number of vertical neighbors than horizontal neighbors, with new values 0..3 introduced in row major order.''. It has offset $1$ and begins
\[
2, 27, 461, 7980, 138562, 2406061, 41780245, 725496762,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 15*a(n-1) + 40*a(n-2) + 22*a(n-3) - 60*a(n-4) -  40*a(n-5) - 21*a(n-6) + 45*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 29 2018 05:05:23, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $3$ columns over the alphabet $\{0,1,2,3\}$, growing one row at a time. Its condition is that every element must equal the same number of its vertical neighbours as it does of its horizontal neighbours.
...
Vertices with identical futures are merged, which leaves $S=10$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
...
\begin{theorem}
\[
a(n)\;=\;15\,a(n-1) + 40\,a(n-2) + 22\,a(n-3) - 60\,a(n-4) - 40\,a(n-5) - 21\,a(n-6) + 45\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A240701.
