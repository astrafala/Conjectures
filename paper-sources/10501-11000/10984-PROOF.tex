...
\title{The empirical recurrence for OEIS A269712, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269712 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A269712 is ``Number of active (ON, black) cells at stage 2\^{}n-1 of the two-dimensional cellular automaton defined by "Rule 20", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 4, 12, 28, 60, 124, 252, 508,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{2} + x + 1}{2 x^{2} - 3 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269712.
