...
\title{The empirical recurrence for OEIS A267886, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267886 carries an empirical recurrence of order $2$. The entry separately records a
...
\begin{quote}
G.f.: (1-x+11*x\^{}2+8*x\^{}3-16*x\^{}4) / ((1-x)*(1-4*x)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 16 x^{4} + 8 x^{3} + 11 x^{2} - x + 1}{4 x^{2} - 5 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 16 x^{4} + 8 x^{3} + 11 x^{2} - x + 1,
\]
