...
\title{The empirical recurrence for OEIS A267920, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267920 carries an empirical recurrence of order $2$. The entry separately records a
...
\begin{quote}
G.f.: (1+1000*x\^{}2) / ((1-x)*(1-100*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:30, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{1000 x^{2} + 1}{100 x^{2} - 101 x + 1},
\]
