...
\title{The empirical recurrence for OEIS A267888, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267888 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A267888 is ``Decimal representation of the n-th iteration of the "Rule 237" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 2, 31, 127, 511, 2047, 8191, 32767,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 24 2025 12:31:05, revision 23) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 20 x^{3} + 25 x^{2} - 3 x + 1}{4 x^{2} - 5 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267888.
