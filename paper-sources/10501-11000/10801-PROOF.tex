...
\title{The empirical recurrence for OEIS A269688, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269688 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269688 is ``Number of length-n 0..6 arrays with no repeated value differing from the previous repeated value by plus or minus one modulo 6+1.''. It has offset $1$ and begins
\[
7, 49, 343, 2387, 16541, 114205, 786079, 5396363,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 27 2019 16:01:39, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 28 x^{3} - 35 x^{2} + 7 x}{30 x^{3} + 31 x^{2} - 12 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;30 x^{3} + 31 x^{2} - 12 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;12\,a(n-1) - 31\,a(n-2) - 30\,a(n-3)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269688.
