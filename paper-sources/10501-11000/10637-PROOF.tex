...
\title{The empirical recurrence for OEIS A063094, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063094 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A063094 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 26 ).''. It has offset $1$ and begins
\[
2, 9, 15, 23, 29, 37, 43, 51,\ \dots
\]
...
\begin{quote}
G.f.: x*(2 + 7*x + 4*x\^{}2 + x\^{}3) / ((1 - x)\^{}2*(1 + x)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} + 4 x^{3} + 7 x^{2} + 2 x}{x^{3} - x^{2} - x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063094.
