...
\title{The empirical recurrence for OEIS A269675, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269675 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269675 is ``Number of length-n 0..5 arrays with no repeated value differing from the previous repeated value by other than plus or minus one modulo 5+1.''. It has offset $1$ and begins
\[
6, 36, 210, 1212, 6918, 39156, 220050, 1229292,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 26 2019 05:46:34, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 36 x^{3} - 18 x^{2} + 6 x}{35 x^{3} + 13 x^{2} - 9 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;35 x^{3} + 13 x^{2} - 9 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269675.
