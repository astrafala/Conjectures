...
\title{The empirical recurrence for OEIS A235882, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235882 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 19 2018 09:21:03, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 13122 x^{3} + 12081 x^{2} - 2571 x}{6 x^{3} - 11 x^{2} + 6 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;13122 x^{3} - 12081 x^{2} + 2571 x,
\]
