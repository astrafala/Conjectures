...
\title{The empirical recurrence for OEIS A252831, derived from the entry's generating function}
...
\begin{abstract}
OEIS A252831 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A252831 is ``Number of n X 3 nonnegative integer arrays with upper left 0 and every value within 2 of its king move distance from the upper left and every value increasing by 0 or 1 with every step right, diagonally se or down.''. It has offset $1$ and begins
\[
4, 13, 44, 127, 288, 529, 850, 1251,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 06 2018 20:06:47, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{6} - 26 x^{5} - 30 x^{4} - 17 x^{3} - x^{2} - 4 x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{6} + 26 x^{5} + 30 x^{4} + 17 x^{3} + x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252831.
