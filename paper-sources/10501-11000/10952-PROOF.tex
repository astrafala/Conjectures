...
\title{The empirical recurrence for OEIS A267613, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267613 carries an empirical recurrence of order $2$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 900 x^{3} + 1010 x^{2} - 100 x + 1}{100 x^{2} - 101 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 900 x^{3} + 1010 x^{2} - 100 x + 1,
\]
