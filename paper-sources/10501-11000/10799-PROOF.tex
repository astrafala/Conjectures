...
\title{The empirical recurrence for OEIS A269686, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269686 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269686 is ``Number of length-n 0..4 arrays with no repeated value differing from the previous repeated value by plus or minus one modulo 4+1.''. It has offset $1$ and begins
\[
5, 25, 125, 615, 2995, 14465, 69405, 331255,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 27 2019 10:58:55, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 10 x^{3} - 15 x^{2} + 5 x}{12 x^{3} + 13 x^{2} - 8 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;12 x^{3} + 13 x^{2} - 8 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269686.
