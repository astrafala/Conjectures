...
\title{The empirical recurrence for OEIS A200249, derived from the entry's generating function}
...
\begin{abstract}
OEIS A200249 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A200249 is ``Number of 0..5 arrays x(0..n-1) of n elements with each no smaller than the sum of its previous elements modulo 6.''. It has offset $1$ and begins
\[
6, 21, 75, 267, 951, 3387, 12063, 42963,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 20 2018 13:54:36, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{2} - 6 x}{2 x^{2} + 3 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A200249.
