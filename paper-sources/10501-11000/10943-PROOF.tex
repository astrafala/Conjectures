...
\title{The empirical recurrence for OEIS A267536, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267536 carries an empirical recurrence of order $2$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{3} - x^{2} + x + 1}{4 x^{2} - 5 x + 1},
\]
