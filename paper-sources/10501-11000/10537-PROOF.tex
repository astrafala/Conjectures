...
\title{The empirical recurrence for OEIS A267209, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267209 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267209 is ``Binary representation of the middle column of the "Rule 109" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 11, 111, 1110, 11101, 111011, 1110110, 11101101,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{3} + x^{2} + x + 1}{10 x^{4} - x^{3} - 10 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;10 x^{4} - x^{3} - 10 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267209.
