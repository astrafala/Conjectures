...
\title{The empirical recurrence for OEIS A263660, derived from the entry's generating function}
...
\begin{abstract}
OEIS A263660 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 02 2019 11:29:54, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{6} - 3 x^{4} - 2 x^{3} - 2 x^{2} - x}{2 x^{2} - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 2 x^{6} + 3 x^{4} + 2 x^{3} + 2 x^{2} + x,
\]
