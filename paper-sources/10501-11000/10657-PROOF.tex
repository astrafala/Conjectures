...
\title{The empirical recurrence for OEIS A183247, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183247 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A183247 is ``Number of permutations of 1..2*n+4 with each element displaced by at least n.''. It has offset $1$ and begins
\[
265, 1708, 13365, 114124, 998112, 8751552, 76915200, 677461056,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 27 2018 08:49:01, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{100844 x^{6} + 29191 x^{5} + 7150 x^{4} - 17394 x^{3} + 4122 x^{2} - 265 x}{360 x^{3} - 157 x^{2} + 22 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 360 x^{3} + 157 x^{2} - 22 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;22\,a(n-1) - 157\,a(n-2) + 360\,a(n-3)
\]
for every $n>6$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 100844 x^{6} - 29191 x^{5} - 7150 x^{4} + 17394 x^{3} - 4122 x^{2} + 265 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183247.
