...
\title{The empirical recurrence for OEIS A267157, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267157 carries an empirical recurrence of order $3$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{10} + x^{4} + x^{3} - x^{2} + 1}{2 x^{3} - x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{3} - x^{2} - 2 x + 1
\]
