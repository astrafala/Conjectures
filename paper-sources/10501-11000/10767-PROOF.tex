...
\title{A condition on the multiset of every subblock: OEIS A183597}
...
\begin{abstract}
OEIS A183597 carries an empirical recurrence of order $7$ contributed by R.~H.~Hardin. It is
...
work. The entry does not count the arrays themselves but $1/6$ of them, so every count below is divided by $6$.
\end{abstract}
...
OEIS A183597 is ``1/6 the number of (n+1) X 4 0..2 arrays with every 2 X 2 subblock containing all three values.''. It has offset $1$ and begins
\[
120, 1479, 17174, 201770, 2366412, 27768032, 325834456, 3823553752,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 12*a(n-1) + 20*a(n-2) - 238*a(n-3) - 420*a(n-4) + 324*a(n-5) + 400*a(n-6) - 224*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 30 2018 09:09:25, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $4$ columns over the alphabet $\{0,1,2\}$, growing one row at a time, and asks that every $2\times2$ subblock holds exactly $3$ distinct values.
...
The entry counts not the arrays themselves but $1/6$ of them, so every count below is divided by $6$; the division comes out exact at every term the entry publishes, which is itself a check on the model.
...
\begin{theorem}
\[
a(n)\;=\;12\,a(n-1) + 20\,a(n-2) - 238\,a(n-3) - 420\,a(n-4) + 324\,a(n-5) + 400\,a(n-6) - 224\,a(n-7)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183597.
