...
\title{The empirical recurrence for OEIS A298569, proved by transfer matrix}
...
\begin{abstract}
OEIS A298569 counts $2$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{0,2,4,5,7\}$, and it carries an empirical
...
OEIS A298569 is ``Number of nX2 0..1 arrays with every element equal to 0, 2, 4, 5 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 4, 4, 8, 36, 52, 100, 356,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +8*a(n-3) -4*a(n-4)
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 22 2018 06:12:56, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 8\,a(n-3) - 4\,a(n-4)
\end{equation}
for all $n>4$.
...
has score in $\{0,2,4,5,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,2,4,5,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,2,4,5,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,2,4,5,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,2,4,5,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298569.
