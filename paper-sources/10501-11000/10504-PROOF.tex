...
\title{The empirical recurrence for OEIS A265284, derived from the entry's generating function}
...
\begin{abstract}
OEIS A265284 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A265284 is ``Total number of ON (black) cells after n iterations of the "Rule 94" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 4, 8, 14, 20, 28, 36, 46,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:27, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} - 2 x - 1}{x^{4} - 2 x^{3} + 2 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A265284.
