...
\title{The empirical recurrence for OEIS A152933, derived from the entry's generating function}
...
\begin{abstract}
OEIS A152933 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A152933 is ``Number of sets (in the Hausdorff metric geometry) at each location between two sets defining a polygonal configuration consisting of k 6-gonal polygonal components chained with string components of length 2 as k varies.''. It has offset $1$ and begins
\[
18, 1197, 80361, 5394960, 362185569, 24314987763, 1632363850242, 109587212856081,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 03 2023 10:52:47, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{9 x^{2} - 18 x}{9 x^{2} + 67 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 9 x^{2} - 67 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A152933.
