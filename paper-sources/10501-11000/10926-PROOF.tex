...
\title{The empirical recurrence for OEIS A228256, derived from the entry's generating function}
...
\begin{abstract}
OEIS A228256 carries an empirical recurrence of order $2$. The entry separately records a
...
\begin{quote}
G.f.: x*(33 + 268*x + 1994*x\^{}2 + 6844*x\^{}3 - 35854*x\^{}4 - 86008*x\^{}5 - 163840*x\^{}6 - 57600*x\^{}7) / ((1 - 4*x)*(1 - 5*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 10 2018 09:53:23, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 57600 x^{8} - 163840 x^{7} - 86008 x^{6} - 35854 x^{5} + 6844 x^{4} + 1994 x^{3} + 268 x^{2} + 33 x}{20 x^{2} - 9 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;20 x^{2} - 9 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 57600 x^{8} - 163840 x^{7} - 86008 x^{6} - 35854 x^{5} + 6844 x^{4} + 1994 x^{3} + 268 x^{2} + 33 x,
\]
