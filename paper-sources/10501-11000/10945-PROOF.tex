...
\title{The empirical recurrence for OEIS A267539, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267539 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 45) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} - x^{2} + 1}{2 x^{2} - 3 x + 1},
\]
