...
\title{The empirical recurrence for OEIS A183702, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183702 carries an empirical recurrence of order $2$. The entry separately records a
...
\begin{quote}
G.f.: 24*x*(8 + 15*x) / (1 - 11*x - 24*x\^{}2).
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 04 2018 10:09:07, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 360 x^{2} - 192 x}{24 x^{2} + 11 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 24 x^{2} - 11 x + 1
\]
