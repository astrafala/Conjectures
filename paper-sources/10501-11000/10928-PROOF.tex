...
\title{The empirical recurrence for OEIS A235643, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235643 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A235643 is ``Total number of sides of a tetraflake-like fractal after n iterations, a(1) = 16 (see comments).''. It has offset $1$ and begins
\[
16, 68, 296, 1300, 5728, 25268, 111512, 492196,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 28 2026 15:48:08, revision 24) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 28 x^{2} + 16 x}{7 x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;7 x^{2} - 6 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235643.
