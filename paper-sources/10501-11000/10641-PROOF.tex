...
\title{The empirical recurrence for OEIS A221022, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221022 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A221022 is ``Equals one maps: number of n X 4 binary arrays indicating the locations of corresponding elements equal to exactly one of their horizontal, vertical and antidiagonal neighbors in a random 0..3 n X 4 array.''. It has offset $1$ and begins
\[
8, 168, 3474, 62944, 1038208, 16735744, 268269568, 4294303744,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 03 2018 08:12:42, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{1664 x^{5} + 4216 x^{4} + 626 x^{3} + 8 x^{2} + 8 x}{64 x^{2} - 20 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;1664 x^{5} + 4216 x^{4} + 626 x^{3} + 8 x^{2} + 8 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221022.
