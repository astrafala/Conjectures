...
\title{The empirical recurrence for OEIS A063120, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063120 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A063120 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 52 ).''. It has offset $1$ and begins
\[
5, 18, 32, 46, 60, 74, 88, 102,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} + 8 x^{2} + 5 x}{x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063120.
