...
\title{The empirical recurrence for OEIS A183905, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183905 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A183905 is ``Number of nondecreasing arrangements of n+2 numbers in 0..2 with each number being the sum mod 3 of two others.''. It has offset $1$ and begins
\[
1, 4, 10, 17, 25, 34, 44, 55,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 05 2018 12:04:57, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{4} - x^{3} - x^{2} - x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183905.
