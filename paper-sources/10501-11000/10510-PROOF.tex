...
\title{The empirical recurrence for OEIS A266251, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266251 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A266251 is ``Number of OFF (white) cells in the n-th iteration of the "Rule 9" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 3, 3, 5, 5, 6, 9, 6,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{7} + 2 x^{6} - x^{5} - x^{4} - x^{3} + 3 x^{2} + 3 x}{x^{4} - 2 x^{2} + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266251.
