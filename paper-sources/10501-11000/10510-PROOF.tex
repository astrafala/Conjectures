...
\title{The empirical recurrence for OEIS A235877, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235877 carries an empirical recurrence of order $2$. The entry separately records a
...
\begin{quote}
G.f.: 3*x*(7 - 18*x) / ((1 - 2*x)*(1 - 3*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 19 2018 08:07:54, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 54 x^{2} + 21 x}{6 x^{2} - 5 x + 1},
\]
