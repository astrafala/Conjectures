...
\title{The empirical recurrence for OEIS A267779, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267779 carries an empirical recurrence of order $3$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{11000 x^{3} + 900 x^{2} + x + 1}{100 x^{3} - x^{2} - 100 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;100 x^{3} - x^{2} - 100 x + 1
\]
