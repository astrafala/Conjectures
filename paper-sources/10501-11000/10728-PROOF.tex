...
\title{The empirical recurrence for OEIS A224667, derived from the entry's generating function}
...
\begin{abstract}
OEIS A224667 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A224667 is ``Number of 5 X 5 0..n matrices with each 2 X 2 subblock idempotent.''. It has offset $1$ and begins
\[
196, 260, 332, 412, 500, 596, 700, 812,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 02 2018 12:09:31, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 140 x^{3} + 328 x^{2} - 196 x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224667.
