...
\title{A proof of the conjectured recurrence for OEIS A097189}
...
\begin{abstract}
OEIS A097189 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $6 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A097189 is ``Row sums of triangle A097186, in which the n-th row polynomial R\_n(y) is formed from the initial (n+1) terms of g.f. A057083(y)\textasciicircum{}(n+1), where R\_n(1/3) = 3\textasciicircum{}n for all n >= 0''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 7, 55, 451, 3781, 32131, 275563, 2378971,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{3}{- 9 x + 2 \left(1 - 9 x\right)^{\frac{2}{3}} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-1)*a(n) - (19*n-18)*(n-1)*a(n-1) + 9*(11*n\textasciicircum{}2-31*n+22)*a(n-2) - 9*(3*n-4)*(3*n-5)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 16 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 31) the
...
\[
p_{0}(n)=n \left(n - 1\right),\qquad p_{1}(n)=- \left(n - 1\right) \left(19 n - 18\right),\qquad p_{2}(n)=9 \left(11 n^{2} - 31 n + 22\right),\qquad p_{3}(n)=- 9 \left(3 n - 5\right) \left(3 n - 4\right)
\]
...
\section{The computation for A097189}

The generating function \eqref{eq:gf} is algebraic over $\mathbb{Q}(x)$;
its minimal polynomial is
\[
P(x,y) \;=\; 9 \left(81 x^{3} y^{3} - 99 x^{2} y^{3} + 81 x^{2} y^{2} + 19 x y^{3} - 18 x y^{2} + 27 x y - y^{3} + y^{2} - 3 y + 3\right) ,
\]
...
\[
n \left(n - 1\right)\,a(n) -  \left(n - 1\right) \left(19 n - 18\right)\,a(n-1) + 9 \left(11 n^{2} - 31 n + 22\right)\,a(n-2) -  9 \left(3 n - 5\right) \left(3 n - 4\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A097189, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=6 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A097189.
