...
\title{The empirical recurrence for OEIS A228219, derived from the entry's generating function}
...
\begin{abstract}
OEIS A228219 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A228219 is ``Number of second differences of arrays of length 4 of numbers in 0..n.''. It has offset $1$ and begins
\[
15, 49, 103, 177, 271, 385, 519, 673,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 16 2018 10:01:30, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{3} - 4 x^{2} - 15 x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A228219.
