...
\title{The empirical recurrence for OEIS A266219, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266219 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A266219 is ``Binary representation of the middle column of the "Rule 7" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 11, 110, 1101, 11010, 110101, 1101010, 11010101,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 32) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{2} + x + 1}{10 x^{3} - x^{2} - 10 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266219.
