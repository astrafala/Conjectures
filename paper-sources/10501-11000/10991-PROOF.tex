...
\title{The empirical recurrence for OEIS A267884, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267884 carries an empirical recurrence of order $1$. The entry separately records a
...
G.f.: x*(3+2*x+x\^{}2) / (1-x).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 24 2025 12:30:54, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{3} - 2 x^{2} - 3 x}{x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - x
\]
