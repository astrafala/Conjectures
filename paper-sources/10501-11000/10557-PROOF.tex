...
\title{The empirical recurrence for OEIS A267689, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267689 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 27) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{8 x^{4} - 2 x^{3} + x^{2} - 1}{8 x^{4} - 6 x^{3} - 7 x^{2} + 6 x - 1},
\]
