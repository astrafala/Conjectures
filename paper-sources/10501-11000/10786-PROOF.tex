...
\title{The empirical recurrence for OEIS A268941, derived from the entry's generating function}
...
\begin{abstract}
OEIS A268941 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A268941 is ``Number of length-n 0..5 arrays with no repeated value unequal to the previous repeated value plus one mod 5+1.''. It has offset $1$ and begins
\[
6, 36, 210, 1206, 6834, 38322, 213042, 1175850,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 17 2019 02:55:43, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 30 x^{3} - 18 x^{2} + 6 x}{30 x^{3} + 14 x^{2} - 9 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;30 x^{3} + 14 x^{2} - 9 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268941.
