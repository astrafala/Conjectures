...
\title{The empirical recurrence for OEIS A267297, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267297 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A267297 is ``Square triangular numbers that are the sum of 2 nonzero nonconsecutive triangular numbers.''. It has offset $1$ and begins
\[
1225, 41616, 1413721, 1631432881, 55420693056, 1882672131025, 63955431761796, 2172602007770041,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 03 2016 22:52:42, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 46611179 x^{6} + 1630019160 x^{5} - 1583407981 x^{4} - 36 x^{3} + 1259 x^{2} - 1225 x}{x^{3} - 35 x^{2} + 35 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{3} + 35 x^{2} - 35 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;46611179 x^{6} - 1630019160 x^{5} + 1583407981 x^{4} + 36 x^{3} - 1259 x^{2} + 1225 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267297.
