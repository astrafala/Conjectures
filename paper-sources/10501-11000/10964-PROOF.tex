...
\title{The empirical recurrence for OEIS A267877, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267877 carries an empirical recurrence of order $2$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 32 x^{5} + 14 x^{3} + 25 x^{2} - 5 x + 1}{4 x^{2} - 5 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 32 x^{5} + 14 x^{3} + 25 x^{2} - 5 x + 1,
\]
