...
\title{The empirical recurrence for OEIS A269696, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269696 carries an empirical recurrence of order $1$. The entry separately records a
...
OEIS A269696 is ``Number of active (ON, black) cells at stage 2\^{}n-1 of the two-dimensional cellular automaton defined by "Rule 6", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 5, 20, 80, 320, 1280, 5120, 20480,\ \dots
\]
...
G.f.: (1+x) / (1-4*x).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:31, revision 28) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x - 1}{4 x - 1},
\]
...
$n>1$. The bound is exact: at $n=1$ the recurrence fails on the entry's own published terms.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269696.
