...
\title{The empirical recurrence for OEIS A188150, derived from the entry's generating function}
...
\begin{abstract}
OEIS A188150 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A188150 is ``Number of 5-step self-avoiding walks on an n X n square summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 104, 432, 972, 1712, 2652, 3792,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 27 2018 09:22:26, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{12 x^{6} + 12 x^{5} - 120 x^{4} - 104 x^{3}}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 12 x^{6} - 12 x^{5} + 120 x^{4} + 104 x^{3},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188150.
