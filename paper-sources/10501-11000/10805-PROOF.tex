...
\title{The empirical recurrence for OEIS A270080, derived from the entry's generating function}
...
\begin{abstract}
OEIS A270080 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A270080 is ``Number of active (ON, black) cells at stage 2\^{}n-1 of the two-dimensional cellular automaton defined by "Rule 62", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 5, 20, 72, 280, 1080, 4216, 16632,\ \dots
\]
...
\begin{quote}
G.f.: (1-2*x-x\^{}2-6*x\^{}3+16*x\^{}4-32*x\^{}5) / ((1-x)*(1-2*x)*(1-4*x)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{32 x^{5} - 16 x^{4} + 6 x^{3} + x^{2} + 2 x - 1}{8 x^{3} - 14 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 32 x^{5} + 16 x^{4} - 6 x^{3} - x^{2} - 2 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270080.
