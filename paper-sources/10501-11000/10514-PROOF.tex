...
\title{The empirical recurrence for OEIS A259290, derived from the entry's generating function}
...
\begin{abstract}
OEIS A259290 carries an empirical recurrence of order $2$. The entry separately records a
...
\begin{quote}
G.f.: 2*x*(5 + 8*x) / (1 - x - 4*x\^{}2).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 25 2018 06:22:27, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 16 x^{2} - 10 x}{4 x^{2} + x - 1},
\]
