...
\title{The empirical recurrence for OEIS A063150, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063150 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A063150 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 82 ).''. It has offset $1$ and begins
\[
9, 30, 50, 72, 92, 114, 134, 156,\ \dots
\]
...
\begin{quote}
G.f.: x*(9 + 21*x + 11*x\^{}2 + x\^{}3) / ((1 - x)\^{}2*(1 + x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 22 2019 12:18:12, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} + 11 x^{3} + 21 x^{2} + 9 x}{x^{3} - x^{2} - x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063150.
