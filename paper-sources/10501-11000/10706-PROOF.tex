...
\title{The empirical recurrence for OEIS A187860, derived from the entry's generating function}
...
\begin{abstract}
OEIS A187860 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A187860 is ``Number of 5-step one space for components leftwards or up, two space for components rightwards or down asymmetric quasi-queen's tours (antidiagonal moves become knight moves) on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 486, 3320, 11986, 26836, 50378, 81124,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 26 2018 08:59:52, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{655 x^{10} - 977 x^{9} + 1488 x^{8} - 2508 x^{7} - 352 x^{6} - 3484 x^{5} - 1862 x^{4} - 486 x^{3}}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 655 x^{10} + 977 x^{9} - 1488 x^{8} + 2508 x^{7} + 352 x^{6} + 3484 x^{5} + 1862 x^{4} + 486 x^{3},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187860.
