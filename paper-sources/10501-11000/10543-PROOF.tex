...
\title{The empirical recurrence for OEIS A267359, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267359 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267359 is ``Binary representation of the n-th iteration of the "Rule 125" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 11, 11100, 101111, 111110000, 1000111111, 1111101000000, 10001111111111,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 30) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 1000000 x^{8} + 11000000 x^{7} + 890000 x^{6} - 10990000 x^{5} + 108900 x^{4} - 8900 x^{3} + 1099 x^{2} + 11 x + 1}{10000 x^{4} - 10001 x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;10000 x^{4} - 10001 x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 1000000 x^{8} + 11000000 x^{7} + 890000 x^{6} - 10990000 x^{5} + 108900 x^{4} - 8900 x^{3} + 1099 x^{2} + 11 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267359.
