...
\title{The empirical recurrence for OEIS A180486, derived from the entry's generating function}
...
\begin{abstract}
OEIS A180486 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A180486 is ``Numbers of the form ceiling(A179896(j)/A018252(j)) where A179896(j) mod A018252(j) <> 0.''. It has offset $1$ and begins
\[
...
G.f.: x*(5 - 2*x) / (1 - x)\^{}2.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 20 2026 19:39:44, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{2} + 5 x}{x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A180486.
