...
\title{The empirical recurrence for OEIS A269687, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269687 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269687 is ``Number of length-n 0..5 arrays with no repeated value differing from the previous repeated value by plus or minus one modulo 5+1.''. It has offset $1$ and begins
\[
6, 36, 216, 1284, 7584, 44556, 260616, 1518804,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 27 2019 18:05:41, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 18 x^{3} - 24 x^{2} + 6 x}{20 x^{3} + 21 x^{2} - 10 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;20 x^{3} + 21 x^{2} - 10 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269687.
