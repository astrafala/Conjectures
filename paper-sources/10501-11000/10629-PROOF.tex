...
\title{The empirical recurrence for OEIS A267045, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267045 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
a(n) = 2*a(n-1)+a(n-2)-2*a(n-3) for n>4. [Typo corrected by \_Karl V. Keller, Jr.\_, Mar 16 2022]
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} - x^{2} + 1}{2 x^{3} - x^{2} - 2 x + 1},
\]
