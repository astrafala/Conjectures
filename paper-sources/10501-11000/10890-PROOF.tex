...
\title{The empirical recurrence for OEIS A063140, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063140 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A063140 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 72 ).''. It has offset $1$ and begins
\[
5, 28, 52, 76, 100, 124, 148, 172,\ \dots
\]
...
\begin{quote}
G.f.: x*(5+18*x+x\^{}2)/(1-x)\^{}2. (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 02 2025 10:03:21, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} + 18 x^{2} + 5 x}{x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063140.
