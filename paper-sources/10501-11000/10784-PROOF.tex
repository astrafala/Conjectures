...
\title{The empirical recurrence for OEIS A267926, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267926 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A267926 is ``Decimal representation of the n-th iteration of the "Rule 246" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 7, 23, 119, 375, 1911, 6007, 30583,\ \dots
\]
...
\begin{quote}
G.f.: (1+6*x) / ((1-x)*(1-4*x)*(1+4*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:30, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{6 x + 1}{16 x^{3} - 16 x^{2} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;16 x^{3} - 16 x^{2} - x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267926.
