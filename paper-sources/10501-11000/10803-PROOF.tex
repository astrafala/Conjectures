...
\title{The empirical recurrence for OEIS A269907, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269907 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269907 is ``Number of active (ON, black) cells at stage 2\^{}n-1 of the two-dimensional cellular automaton defined by "Rule 1", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 4, 44, 220, 956, 3964, 16124, 65020,\ \dots
\]
...
\begin{quote}
G.f.: (1-3*x+30*x\^{}2-40*x\^{}3) / ((1-x)*(1-2*x)*(1-4*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:31, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{40 x^{3} - 30 x^{2} + 3 x - 1}{8 x^{3} - 14 x^{2} + 7 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269907.
