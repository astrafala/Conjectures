...
\title{The empirical recurrence for OEIS A221082, proved by transfer matrix}
...
\begin{abstract}
OEIS A221082 carries an empirical recurrence of order $2$ contributed by R.~H.~Hardin. It is
true, and it is decidable rather than empirical. The entry's condition is local to a bounded window of consecutive lines, so
the admissible configurations of such a window are the vertices of a finite digraph and the
arrays counted are exactly the walks in it; hence $a(n)$ is a walk count on $S=82$ vertices
...
OEIS A221082 is ``Number of n X 3 arrays of occupancy after each element moves to some horizontal or antidiagonal neighbor.''. It has offset $1$ and begins
\[
2, 24, 270, 3024, 33858, 379080, 4244238, 47519136,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 12*a(n-1) - 9*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 03 2018 08:13:01, revision 7) this is still
...
admissible array is then exactly a walk, so with $M$ the adjacency matrix on $S=82$ vertices
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221082.
