...
\title{The empirical recurrence for OEIS A269685, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269685 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269685 is ``Number of length-n 0..3 arrays with no repeated value differing from the previous repeated value by plus or minus one modulo 3+1.''. It has offset $1$ and begins
\[
4, 16, 64, 248, 944, 3544, 13168, 48536,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 27 2019 07:52:36, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{3} - 8 x^{2} + 4 x}{6 x^{3} + 7 x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;6 x^{3} + 7 x^{2} - 6 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269685.
