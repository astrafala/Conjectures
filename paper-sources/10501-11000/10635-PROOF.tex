...
\title{The empirical recurrence for OEIS A267226, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267226 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A267226 is ``Number of length-n 0..2 arrays with no following elements greater than or equal to the first repeated value.''. It has offset $1$ and begins
\[
3, 9, 21, 47, 103, 223, 479, 1023,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 06 2018 04:16:32, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{4} + 6 x^{2} - 3 x}{4 x^{3} - 8 x^{2} + 5 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267226.
