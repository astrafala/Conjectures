...
\title{A proof of the conjectured recurrence for OEIS A386830}
...
\begin{abstract}
OEIS A386830 carries a conjectured linear recurrence with polynomial coefficients, of
...
$- 30 x$, of degree $1$, which proves the recurrence for every $n>1$.
\end{abstract}
...
OEIS A386830 is ``a(n) = Sum\_\{k=0..n\} 3\^{}k * 2\^{}(n-k) * binomial(3*n+1,k) * binomial(3*n-k,n-k)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 18, 459, 12942, 382671, 11632248, 360048924, 11287595862,\ \dots
\]
...
a(n)\;=\;[x^{n}]\,f(x)\,g(x)^{n},
\qquad f(x)=\frac{- 3 x - 1}{2 x - 1},\qquad g(x)=\frac{27 x^{3} + 27 x^{2} + 9 x + 1}{4 x^{2} - 4 x + 1} .
\end{equation}
...
\begin{quote}\small
Conjecture D-finite with recurrence +8*n*(2*n-3)*a(n) +6*(-108*n\^{}2+207*n-80)*a(n-1) +405*(3*n-2)*(3*n-4)*a(n-2)=0. - \emph{R. J. Mathar}, Aug 19 2025
\end{quote}
As of the ``Last modified'' line on the live entry (2025-08-19, revision 10) the
...
For A386830 this yields
\[
3645 t^{2} y^{3} - 648 t y^{3} - 810 t y^{2} + 16 y^{3} + 24 y^{2} - 15 y - 25 \;=\;0 ,
\]
...
\[
8 n \left(2 n - 3\right)\,a(n) -  6 \left(108 n^{2} - 207 n + 80\right)\,a(n-1) + 405 \left(3 n - 4\right) \left(3 n - 2\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
The result was then compared term by term with the DATA section of A386830, agreeing on
...
all 15 values of $n$ from $n=2$ upward.
...
the identity $B=- 30 x$ is an identity of functions and the theorem holds for all
$n>1$ simultaneously.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A386830.
