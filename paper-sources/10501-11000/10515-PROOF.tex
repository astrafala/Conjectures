...
\title{The empirical recurrence for OEIS A063153, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063153 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
G.f.: x*(7 + 19*x + 9*x\^{}2 + x\^{}3) / ((1 - x)\^{}2*(1 + x)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} + 9 x^{3} + 19 x^{2} + 7 x}{x^{3} - x^{2} - x + 1},
\]
