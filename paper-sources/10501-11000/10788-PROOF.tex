...
\title{The empirical recurrence for OEIS A269461, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269461 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269461 is ``Number of length-n 0..2 arrays with no repeated value equal to the previous repeated value.''. It has offset $1$ and begins
\[
3, 9, 24, 66, 174, 462, 1206, 3150,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 21 2018 17:20:03, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 9 x^{3} + 3 x}{8 x^{3} - 2 x^{2} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;8 x^{3} - 2 x^{2} - 3 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269461.
