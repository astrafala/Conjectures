...
\title{The empirical recurrence for OEIS A183245, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183245 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A183245 is ``Number of permutations of 1..2*n+2 with each element displaced by at least n.''. It has offset $1$ and begins
\[
9, 29, 112, 436, 1708, 6724, 26572, 105316,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 27 2018 08:49:17, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{17 x^{3} - 34 x^{2} + 9 x}{12 x^{2} - 7 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;12 x^{2} - 7 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183245.
