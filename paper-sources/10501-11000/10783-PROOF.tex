...
\title{The empirical recurrence for OEIS A267925, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267925 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A267925 is ``Binary representation of the n-th iteration of the "Rule 246" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 111, 10111, 1110111, 101110111, 11101110111, 1011101110111, 111011101110111,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:30, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{110 x + 1}{10000 x^{3} - 10000 x^{2} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;10000 x^{3} - 10000 x^{2} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 10000\,a(n-2) - 10000\,a(n-3)
\]
for every $n>1$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267925.
