...
\title{The empirical recurrence for OEIS A250799, proved by transfer matrix}
...
\begin{abstract}
OEIS A250799 counts arrays over $\{0,1\}$ in which a statistic taken over short runs
...
window of consecutive rows, so the count is a walk count on $S=8$ states, and the
...
OEIS A250799 is ``Number of (2+1) X (n+1) 0..1 arrays with nondecreasing min(x(i,j),x(i,j-1)) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
24, 66, 162, 376, 838, 1818, 3868, 8114,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 4*a(n-2) - a(n-3) + 2*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 20 2018 10:57:45, revision 7) this is still
...
So a single row is a state. Let $M$ be the matrix on the $2^{3}=8$ rows with
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250799.
