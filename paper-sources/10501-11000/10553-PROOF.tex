...
\title{The empirical recurrence for OEIS A267678, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267678 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267678 is ``Decimal representation of the n-th iteration of the "Rule 197" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 2, 20, 104, 468, 1960, 8020, 32424,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{16 x^{5} + 12 x^{4} - 4 x^{3} - 15 x^{2} + 4 x - 1}{8 x^{4} - 6 x^{3} - 7 x^{2} + 6 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 8 x^{4} + 6 x^{3} + 7 x^{2} - 6 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 16 x^{5} - 12 x^{4} + 4 x^{3} + 15 x^{2} - 4 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267678.
