...
\title{A proof of the conjectured recurrence for OEIS A115256}
...
\begin{abstract}
OEIS A115256 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A115256 is ``Diagonal sums of correlation triangle of central binomial coefficients''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 8, 25, 90, 312, 1145, 4186,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{1}{\sqrt{1 - 4 x} \sqrt{1 - 4 x^{2}} \left(x^{3} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + 2*(-2*n+1)*a(n-1) + 4*(-n+1)*a(n-2) + 3*(5*n-8)*a(n-3) + 2*(2*n-1)*a(n-4) + 4*(n-1)*a(n-5) + 8*(-2*n+3)*a(n-6) = 0. - \emph{R. J. Mathar}, Jun 22 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-22, revision 21) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(2 n - 1\right),\qquad p_{2}(n)=- 4 \left(n - 1\right),\qquad p_{3}(n)=3 \left(5 n - 8\right),\qquad p_{4}(n)=2 \left(2 n - 1\right),\qquad p_{5}(n)=4 \left(n - 1\right),\qquad p_{6}(n)=- 8 \left(2 n - 3\right)
\]
...
\[
n\,a(n) -  2 \left(2 n - 1\right)\,a(n-1) -  4 \left(n - 1\right)\,a(n-2) + 3 \left(5 n - 8\right)\,a(n-3) + 2 \left(2 n - 1\right)\,a(n-4) + 4 \left(n - 1\right)\,a(n-5) -  8 \left(2 n - 3\right)\,a(n-6) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A115256, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A115256.
