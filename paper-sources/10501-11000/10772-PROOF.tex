...
\title{The empirical recurrence for OEIS A267460, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267460 carries an empirical recurrence of order $3$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{4} + 2 x^{2} + 2 x}{x^{3} - x^{2} - x + 1},
\]
