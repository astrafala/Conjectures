...
\title{The empirical recurrence for OEIS A266444, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266444 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A266444 is ``Middle column of the "Rule 25" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{13} + x^{11} + x^{10} - x^{9} - x^{4} + x^{2} - 1}{x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - x^{2}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{13} - x^{11} - x^{10} + x^{9} + x^{4} - x^{2} + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266444.
