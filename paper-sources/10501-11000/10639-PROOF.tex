...
\title{The empirical recurrence for OEIS A063109, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063109 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
G.f.: x*(3 + 7*x + 3*x\^{}2 + x\^{}3) / ((1 - x)\^{}2*(1 + x)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} + 3 x^{3} + 7 x^{2} + 3 x}{x^{3} - x^{2} - x + 1},
\]
