...
\title{The empirical recurrence for OEIS A265207, derived from the entry's generating function}
...
\begin{abstract}
OEIS A265207 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A265207 is ``Draw a square and follow these steps: Take a square and place at its edges isosceles right triangles with the edge as hypotenuse. Draw a square at every new edge of the triangles. Repeat for all the new squares of the same size. New figures are only placed on empty space. The structure is symmetric about the first square. The sequence gives the numbers of squares of equal size in successive rings around the center.''. It has offset $1$ and begins
\[
1, 8, 20, 36, 60, 92, 140, 204,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 07 2015 10:56:28, revision 37) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{4} + 10 x^{3} + 7 x^{2} + x}{2 x^{3} - 2 x^{2} - x + 1},
\]
...
$n>4$. The bound is exact: at $n=4$ the recurrence fails on the entry's own published terms.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A265207.
