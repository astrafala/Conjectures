...
\title{The empirical recurrence for OEIS A039827, derived from the entry's generating function}
...
\begin{abstract}
OEIS A039827 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A039827 is ``Number of different coefficient values in expansion of Product (1+q\^{}i+q\^{}(2i)), i=1 to n.''. It has offset $1$ and begins
\[
1, 2, 3, 6, 11, 19, 26, 34,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 22 2016 21:26:48, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{11} + 2 x^{10} - x^{9} - 2 x^{8} + 4 x^{7} - x^{6} - 2 x^{4} + x^{2} - x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{11} - 2 x^{10} + x^{9} + 2 x^{8} - 4 x^{7} + x^{6} + 2 x^{4} - x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A039827.
