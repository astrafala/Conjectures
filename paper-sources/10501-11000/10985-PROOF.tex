...
\title{The empirical recurrence for OEIS A269716, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269716 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A269716 is ``Number of active (ON, black) cells at stage 2\^{}n-1 of the two-dimensional cellular automaton defined by "Rule 22", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 5, 20, 88, 368, 1504, 6080, 24448,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{8 x^{3} - 2 x^{2} - x + 1}{8 x^{2} - 6 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269716.
