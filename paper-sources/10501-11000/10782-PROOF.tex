...
\title{The empirical recurrence for OEIS A267882, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267882 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
G.f.: (1-2*x+4*x\^{}2-x\^{}5) / (1-x)\^{}3. (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 08 2025 09:54:46, revision 21) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} - 4 x^{2} + 2 x - 1}{x^{3} - 3 x^{2} + 3 x - 1},
\]
