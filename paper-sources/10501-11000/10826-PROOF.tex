...
\title{The empirical recurrence for OEIS A232670, proved by transfer matrix}
...
\begin{abstract}
OEIS A232670 counts arrays of width $2$ over $\{0,\dots,2\}$ under a condition on each cell
and its neighbours, and carries an empirical recurrence of order $8$ contributed by
...
a walk count on $S=90$ of them, and the conjectured recurrence is then settled by a finite
exact computation.
\end{abstract}
...
OEIS A232670 is ``Number of (n+1) X (1+1) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally, vertically, diagonally or antidiagonally.''. It has offset $1$ and begins
\[
36, 212, 1568, 11878, 87680, 651678, 4846056, 35990386,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + 21*a(n-2) + 118*a(n-3) + 170*a(n-4) + 166*a(n-5) - 52*a(n-6) - 48*a(n-7) - 64*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 05 2018 12:00:09, revision 7) this is still
...
terms. There are $S=90$ states, and $a$ is $C$-finite.

At $n=1$ the array has one row, and
the condition is tested there with nothing above or below it; the entry's first term is
$36$, which the model reproduces along with every later term. That is the cheapest check
...
first term. There are $3^{2}=9$ possible rows over the entry's own alphabet. The state count $S=90$ is exactly $9\cdot10$,
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 21\,a(n-2) + 118\,a(n-3) + 170\,a(n-4) + 166\,a(n-5) - 52\,a(n-6) - 48\,a(n-7) - 64\,a(n-8)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232670.
