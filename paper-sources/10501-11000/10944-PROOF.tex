...
\title{The empirical recurrence for OEIS A267538, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267538 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A267538 is ``Binary representation of the middle column of the "Rule 143" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 11, 110, 1100, 11001, 110011, 1100111, 11001111,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} - x^{2} + 1}{10 x^{2} - 11 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;10 x^{2} - 11 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267538.
