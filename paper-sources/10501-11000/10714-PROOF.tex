...
\title{The empirical recurrence for OEIS A059158, derived from the entry's generating function}
...
\begin{abstract}
OEIS A059158 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A059158 is ``A hierarchical sequence (S(W'3{2,2}cc) - see A059126).''. It has offset $0$ and begins
\[
24, 432, 7008, 112320, 1797504, 28760832, 460174848, 7362800640,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = 18*a(n-1) - 32*a(n-2) for n > 1.
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: 24 / ((2*x-1)*(16*x-1)).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 02 2023 16:24:51, revision 20) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{24}{32 x^{2} - 18 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;32 x^{2} - 18 x + 1
\]
...
D(x)\,G(x)\;=\;24,
\]
a POLYNOMIAL of degree $0$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $0$, so by
...
they stop at index $0$ because that polynomial has degree $0$. A reader who trusts
...
indices beyond $n=0$ and holds at every one, and at $n=0$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A059158.
