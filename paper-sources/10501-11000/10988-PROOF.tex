...
\title{The empirical recurrence for OEIS A208428, derived from the entry's generating function}
...
\begin{abstract}
OEIS A208428 carries an empirical recurrence of order $1$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 29 2023 13:14:31, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{3} - 6 x^{2} - 2 x}{4 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - 4 x
\]
