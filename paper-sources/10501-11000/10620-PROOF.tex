...
\title{The empirical recurrence for OEIS A263244, derived from the entry's generating function}
...
\begin{abstract}
OEIS A263244 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:27, revision 32) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{1100 x^{3} - 90 x^{2} + 100 x + 1}{10000 x^{3} - 10000 x^{2} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;1100 x^{3} - 90 x^{2} + 100 x + 1,
\]
