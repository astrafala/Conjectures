...
\title{The empirical recurrence for OEIS A266613, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266613 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 63) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} - x^{4} + 1}{2 x^{3} - x^{2} - 2 x + 1},
\]
