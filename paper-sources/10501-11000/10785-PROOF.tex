...
\title{The empirical recurrence for OEIS A268939, derived from the entry's generating function}
...
\begin{abstract}
OEIS A268939 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A268939 is ``Number of length-n 0..3 arrays with no repeated value unequal to the previous repeated value plus one mod 3+1.''. It has offset $1$ and begins
\[
4, 16, 60, 220, 788, 2780, 9684, 33404,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 16 2019 15:53:38, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 12 x^{3} - 4 x^{2} + 4 x}{12 x^{3} + 2 x^{2} - 5 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268939.
