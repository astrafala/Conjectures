...
\title{The empirical recurrence for OEIS A119652, derived from the entry's generating function}
...
\begin{abstract}
OEIS A119652 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A119652 is ``Number of different values of <= n standard American coins (pennies, nickels, dimes and quarters).''. It has offset $1$ and begins
\[
4, 13, 27, 46, 69, 94, 119, 144,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 19 2021 10:40:36, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{6} + 4 x^{5} + 5 x^{4} + 5 x^{3} + 5 x^{2} + 4 x}{x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{6} + 4 x^{5} + 5 x^{4} + 5 x^{3} + 5 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A119652.
