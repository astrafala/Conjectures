...
\title{The empirical recurrence for OEIS A063133, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063133 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
G.f.: x*(5 + 15*x + 7*x\^{}2 + x\^{}3) / ((1 - x)\^{}2*(1 + x))
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} + 7 x^{3} + 15 x^{2} + 5 x}{x^{3} - x^{2} - x + 1},
\]
