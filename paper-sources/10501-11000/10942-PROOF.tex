...
\title{A proof of the conjectured recurrence for OEIS A094905}
...
\begin{abstract}
OEIS A094905 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A094905 is ``Expansion of e.g.f.: exp(6*x)/(1-6*x)\textasciicircum{}(1/6)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 7, 55, 541, 7585, 157231, 4452247, 157484725,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{e^{6 x}}{\sqrt[6]{1 - 6 x}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjectured to be D-finite with recurrence: a(n) +(-6*n-1)*a(n-1) +36*(n-1)*a(n-2) = 0.
\end{quote}
As of the ``Last modified'' line on the live entry (2022-12-15, revision 14) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- 6 n - 1,\qquad p_{2}(n)=36 \left(n - 1\right)
\]
...
\section{The computation for A094905}
...
\[
1\,a(n) + (- 6 n - 1)\,a(n-1) + 36 \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A094905, agreeing on all 13 terms compared.
...
vanish, for all 13 values of $n$ from $n=3$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A094905.
