...
\title{The empirical recurrence for OEIS A063122, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063122 carries an empirical recurrence of order $2$. The entry separately records a
...
\begin{quote}
G.f.: x*(4 + 13*x + x\^{}2) / (1 - x)\^{}2.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 18 2023 21:51:02, revision 21) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} + 13 x^{2} + 4 x}{x^{2} - 2 x + 1},
\]
