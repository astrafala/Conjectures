...
\title{The empirical recurrence for OEIS A190036, derived from the entry's generating function}
...
\begin{abstract}
OEIS A190036 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A190036 is ``Number of nondecreasing arrangements of n+2 numbers in 0..4 with the last equal to 4 and each after the second equal to the sum of one or two of the preceding three.''. It has offset $1$ and begins
\[
7, 12, 18, 27, 39, 53, 69, 87,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 04 2018 05:18:42, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} - 2 x^{4} - 3 x^{3} + 9 x^{2} - 7 x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + 2 x^{4} + 3 x^{3} - 9 x^{2} + 7 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A190036.
