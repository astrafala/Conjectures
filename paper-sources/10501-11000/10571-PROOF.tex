...
\title{A proof of the conjectured recurrence for OEIS A176753}
...
\begin{abstract}
OEIS A176753 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 x^{3} - 5 x^{2} - 2 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A176753 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=1, k=0 and l=-2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 0, -1, -4, -12, -34, -93,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{4 x \left(- 2 x^{2} - x + 1\right) + x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(1-3*n)*a(n-1) +(9*n-13)*a(n-2) +2*(2*n-9)*a(n-3) +8*(4-n)*a(n-4)=0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2014-01-20, revision 6) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=9 n - 13,\qquad p_{3}(n)=2 \left(2 n - 9\right),\qquad p_{4}(n)=- 8 \left(n - 4\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + (9 n - 13)\,a(n-2) + 2 \left(2 n - 9\right)\,a(n-3) -  8 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A176753, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=4 x^{3} - 5 x^{2} - 2 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176753.
