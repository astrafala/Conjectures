...
\title{The empirical recurrence for OEIS A269708, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269708 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A269708 is ``Number of active (ON, black) cells at stage 2\^{}n-1 of the two-dimensional cellular automaton defined by "Rule 14", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 5, 20, 76, 292, 1132, 4420, 17356,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:31, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{3} - 3 x^{2} - 2 x + 1}{12 x^{2} - 7 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269708.
