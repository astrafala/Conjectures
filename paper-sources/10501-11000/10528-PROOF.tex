...
\title{The empirical recurrence for OEIS A266872, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266872 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 32) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{1000 x^{3} + 100 x^{2} + 10 x + 1}{10000 x^{4} - 10001 x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;1000 x^{3} + 100 x^{2} + 10 x + 1,
\]
