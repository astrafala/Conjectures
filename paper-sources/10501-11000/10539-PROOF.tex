...
\title{The empirical recurrence for OEIS A063146, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063146 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A063146 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 78 ).''. It has offset $1$ and begins
\[
11, 38, 66, 94, 122, 150, 178, 206,\ \dots
\]
...
\begin{quote}
G.f.: x*(11 + 16*x + x\^{}2) / (1 - x)\^{}2.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 22 2019 12:18:12, revision 15) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} + 16 x^{2} + 11 x}{x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063146.
