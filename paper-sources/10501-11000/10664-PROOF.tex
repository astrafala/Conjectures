...
\title{The empirical recurrence for OEIS A187156, derived from the entry's generating function}
...
\begin{abstract}
OEIS A187156 carries an empirical recurrence of order $3$. The entry separately records a
...
G.f.: 4*x\^{}3*(5 + x) / (1 - x)\^{}3.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 28 2018 10:43:59, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{4} - 20 x^{3}}{x^{3} - 3 x^{2} + 3 x - 1},
\]
