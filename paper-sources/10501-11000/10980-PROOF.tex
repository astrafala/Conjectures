...
\title{The empirical recurrence for OEIS A267938, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267938 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A267938 is ``Decimal representation of the n-th iteration of the "Rule 251" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 5, 31, 127, 511, 2047, 8191, 32767,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 24 2025 12:23:25, revision 23) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{3} + 10 x^{2} + 1}{4 x^{2} - 5 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267938.
