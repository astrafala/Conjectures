...
\title{The empirical recurrence for OEIS A253122, derived from the entry's generating function}
...
\begin{abstract}
OEIS A253122 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A253122 is ``Number of length n+2 0..1 arrays with the sum of medians of adjacent triples multiplied by some arrangement of +-1 equal to zero.''. It has offset $1$ and begins
\[
4, 12, 16, 40, 64, 144, 256, 544,\ \dots
\]
...
\begin{quote}
G.f.: 4*x*(1 + x - 4*x\^{}2) / ((1 - 2*x)*(1 - 2*x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 09 2018 07:34:42, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 16 x^{3} + 4 x^{2} + 4 x}{4 x^{3} - 2 x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253122.
