...
\title{The empirical recurrence for OEIS A119651, derived from the entry's generating function}
...
\begin{abstract}
OEIS A119651 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A119651 is ``Number of different values of exactly n standard American coins (pennies, nickels, dimes and quarters).''. It has offset $1$ and begins
\[
4, 10, 20, 34, 52, 73, 96, 120,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 24 2019 14:33:28, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{8} + 2 x^{7} + 3 x^{6} + 4 x^{5} + 4 x^{4} + 4 x^{3} + 2 x^{2} + 4 x}{x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{8} + 2 x^{7} + 3 x^{6} + 4 x^{5} + 4 x^{4} + 4 x^{3} + 2 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A119651.
