...
\title{The empirical recurrence for OEIS A222459, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A222459 counts arrays over $\{0,\dots,7\}$ in which every cell is constrained by the
...
with $S=512$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A222459 is ``Number of n X 3 0..7 arrays with no entry increasing mod 8 by 7 rightwards or downwards, starting with upper left zero.''. It has offset $1$ and begins
\[
49, 12943, 3418807, 903055069, 238535974201, 63007686842527, 16643060295393343, 4396153388210813341,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 270*a(n-1) - 1547*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 15 2018 10:12:31, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;270\,a(n-1) - 1547\,a(n-2)
\end{equation}
for all $n>2$.
...
Let $V=\{0,\dots,7\}^{3}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=512$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
publishes, $n=11$, the arrays have $3\cdot11$ cells over
$8$ values, so there are about $10^{29}$ of them to sift. The
walk count replaces that by $512$ states and one matrix--vector product per row.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=512$ pairs and each
of the $(7+1)^{3}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222459.
