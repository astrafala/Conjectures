...
\title{The empirical recurrence for OEIS A211479, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211479 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A211479 is ``Number of -1..1 arrays x(i) of n+1 elements i=1..n+1 with x(i)+x(j), x(i+1)+x(j+1), -(x(i)+x(j+1)), and -(x(i+1)+x(j)) having two, three or four distinct values for every i<=n and j<=n.''. It has offset $1$ and begins
\[
8, 22, 58, 150, 382, 962, 2402, 5958,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 18 2018 07:47:27, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{3} - 10 x^{2} + 8 x}{2 x^{3} + 3 x^{2} - 4 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{3} + 3 x^{2} - 4 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211479.
