...
\title{The empirical recurrence for OEIS A063136, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063136 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A063136 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 68 ).''. It has offset $1$ and begins
\[
7, 24, 42, 60, 78, 96, 114, 132,\ \dots
\]
...
\begin{quote}
G.f.: x*(7 + 10*x + x\^{}2) / (1 - x)\^{}2.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 22 2019 12:18:12, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} + 10 x^{2} + 7 x}{x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063136.
