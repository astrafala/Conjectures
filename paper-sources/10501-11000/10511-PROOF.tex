...
\title{The empirical recurrence for OEIS A063121, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063121 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
G.f.: x*(4 + 9*x + 4*x\^{}2 + x\^{}3) / ((1 - x)\^{}2*(1 + x)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} + 4 x^{3} + 9 x^{2} + 4 x}{x^{3} - x^{2} - x + 1},
\]
