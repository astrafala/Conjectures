...
\title{The empirical recurrence for OEIS A266662, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266662 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A266662 is ``Number of ON (black) cells in the n-th iteration of the "Rule 47" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 2, 2, 5, 2, 9, 2, 13,\ \dots
\]
...
\begin{quote}
G.f.: (1+2*x+x\^{}3-x\^{}4+x\^{}5) / ((1-x)\^{}2*(1+x)\^{}2).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} - x^{4} + x^{3} + 2 x + 1}{x^{4} - 2 x^{2} + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266662.
