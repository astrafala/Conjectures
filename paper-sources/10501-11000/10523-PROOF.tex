...
\title{The empirical recurrence for OEIS A164004, derived from the entry's generating function}
...
\begin{abstract}
OEIS A164004 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A164004 is ``Zero together with row 4 of the array in A163280.''. It has offset $0$ and begins
\[
0, 5, 10, 18, 28, 40, 54, 70,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 25 2023 06:42:20, revision 21) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} - 3 x^{3} + 5 x^{2} - 5 x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A164004.
