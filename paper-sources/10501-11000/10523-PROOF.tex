...
\title{The empirical recurrence for OEIS A046960, derived from the entry's generating function}
...
\begin{abstract}
OEIS A046960 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A046960 is ``Bessel function J\_0(n) is a monotonically decreasing positive sequence.''. It has offset $0$ and begins
\[
1, 2, 6, 12, 21, 37, 40, 84,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 05 2021 21:45:23, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{666 x^{10} + 41 x^{7} - 13 x^{6} + 7 x^{5} + 3 x^{4} + 2 x^{3} + 3 x^{2} + 1}{x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;666 x^{10} + 41 x^{7} - 13 x^{6} + 7 x^{5} + 3 x^{4} + 2 x^{3} + 3 x^{2} + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A046960.
