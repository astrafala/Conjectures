...
\title{The empirical recurrence for OEIS A267804, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267804 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267804 is ``Binary representation of the n-th iteration of the "Rule 214" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 111, 10111, 1100111, 101110111, 11001100111, 1011101110111, 110011001100111,\ \dots
\]
...
\begin{quote}
G.f.: (1+111*x+110*x\^{}2-10000*x\^{}3) / ((1-x)*(1+x)*(1-100*x)*(1+100*x)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 10000 x^{3} + 110 x^{2} + 111 x + 1}{10000 x^{4} - 10001 x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 10000 x^{3} + 110 x^{2} + 111 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267804.
