...
\title{The empirical recurrence for OEIS A266447, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266447 carries an empirical recurrence of order $4$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{7} - 2 x^{6} + x^{5} + x^{4} + x^{3} + x + 1}{x^{4} - 2 x^{2} + 1},
\]
