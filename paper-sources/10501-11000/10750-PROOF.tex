...
\title{The empirical recurrence for OEIS A183246, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183246 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A183246 is ``Number of permutations of 1..2*n+3 with each element displaced by at least n.''. It has offset $1$ and begins
\[
44, 206, 1168, 6984, 41808, 250464, 1501248, 9001344,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 27 2018 08:48:54, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{248 x^{4} + 164 x^{3} - 234 x^{2} + 44 x}{24 x^{2} - 10 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;24 x^{2} - 10 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;248 x^{4} + 164 x^{3} - 234 x^{2} + 44 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183246.
