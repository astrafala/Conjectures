...
\title{The empirical recurrence for OEIS A266220, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266220 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + x^{4} + 3 x^{3} - 2 x^{2} + 2 x + 1}{x^{4} - 2 x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{5} + x^{4} + 3 x^{3} - 2 x^{2} + 2 x + 1,
\]
