...
\title{The empirical recurrence for OEIS A181192, derived from the entry's generating function}
...
\begin{abstract}
OEIS A181192 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A181192 is ``Number of n X 5 matrices containing a permutation of 1..n*5 in increasing order rowwise, columnwise, diagonally and (downwards) antidiagonally.''. It has offset $1$ and begins
\[
1, 14, 290, 6392, 141696, 3142704, 69705920, 1546100352,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 27 2018 05:00:53, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{3} - 10 x^{2} + x}{8 x^{3} + 40 x^{2} - 24 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;8 x^{3} + 40 x^{2} - 24 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A181192.
