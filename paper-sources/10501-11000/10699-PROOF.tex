...
\title{The empirical recurrence for OEIS A297432, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A297432 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A297432 is ``Number of 2 X n 0..1 arrays with every 1 horizontally, diagonally or antidiagonally adjacent to 2 neighboring 1s.''. It has offset $1$ and begins
\[
1, 2, 7, 12, 18, 33, 73, 139,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) + a(n-3) + 4*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 28 2019 18:52:01, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + a(n-3) + 4\,a(n-4)
\end{equation}
for all $n>5$.
...
every one of the $33$ published indices, with no shift fitted: the number of columns is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A297432.
