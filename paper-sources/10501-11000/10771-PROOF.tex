...
\title{The empirical recurrence for OEIS A266680, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266680 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A266680 is ``Binary representation of the middle column of the "Rule 175" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 11, 110, 1101, 11011, 110111, 1101111, 11011111,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 34) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} - x^{2} + 1}{10 x^{2} - 11 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266680.
