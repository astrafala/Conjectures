...
\title{The empirical recurrence for OEIS A267935, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267935 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 24 2025 12:17:47, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 16 x^{4} - 4 x^{3} + 26 x^{2} - 4 x + 1}{4 x^{2} - 5 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{2} - 5 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 16 x^{4} - 4 x^{3} + 26 x^{2} - 4 x + 1,
\]
