...
\title{A proof of the conjectured recurrence for OEIS A191625}
...
\begin{abstract}
OEIS A191625 carries a conjectured linear recurrence with polynomial coefficients, of
order 9, contributed by R.~J.~Mathar. We prove it. The generating function of the
...
closed form. Here it equals $2 x^{8} - 5 x^{6} - 14 x^{5} + 5 x^{4} + 6 x^{3} - 7 x^{2} + 1$, a polynomial of degree $8$, which proves
the recurrence for every $n>8$.
\end{abstract}
...
OEIS A191625 is ``Number of n-step two-sided prudent self-avoiding walks ending at the northeast corner of their box''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 4, 10, 24, 58, 140, 340,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4 \left(x - 1\right) \left(x^{2} - 1\right) \left(x^{3} + x^{2} - x - \sqrt{\left(x^{4} - 1\right) \left(x^{2} + 2 x - 1\right)} + 1\right) - \left(- x^{3} - x^{2} + x + \sqrt{\left(x^{4} - 1\right) \left(x^{2} + 2 x - 1\right)} + 1\right) \left(- x^{3} + 3 x^{2} + x + \sqrt{\left(x^{4} - 1\right) \left(x^{2} + 2 x - 1\right)} - 1\right)}{\left(- x^{3} - x^{2} + x + \sqrt{\left(x^{4} - 1\right) \left(x^{2} + 2 x - 1\right)} + 1\right) \left(- x^{3} + 3 x^{2} + x + \sqrt{\left(x^{4} - 1\right) \left(x^{2} + 2 x - 1\right)} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(n+1)*a(n) -4*n*a(n-1) +(n-5)*a(n-2) +2*(4*n-3)*a(n-3) +3*(-n+3)*a(n-4) +2*(n-8)*a(n-5) +(-n+13)*a(n-6) +8*(-n+7)*a(n-7) +2*(n-7)*a(n-8) +2*(n-9)*a(n-9)=0. - \emph{R. J. Mathar}, Sep 16 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2022-02-23, revision 29) the
statement is still recorded as a conjecture, and no proof appears among the entry's links,
formulas or programs.

Writing the conjectured relation in the form $\sum_{i=0}^{9}p_{i}(n)\,a(n-i)=0$,
the coefficient polynomials are
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 4 n,\qquad p_{2}(n)=n - 5,\qquad p_{3}(n)=2 \left(4 n - 3\right),\qquad p_{4}(n)=- 3 \left(n - 3\right),\qquad p_{5}(n)=2 \left(n - 8\right),\qquad p_{6}(n)=13 - n,\qquad p_{7}(n)=- 8 \left(n - 7\right),\qquad p_{8}(n)=2 \left(n - 7\right),\qquad p_{9}(n)=2 \left(n - 9\right)
\]
...
\section{The computation for A191625}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x - 1\right) \left(x + 1\right) \left(x^{2} + 1\right) \left(x^{2} + 2 x - 1\right) .
\]
...
\[
B(x)\;=\;2 x^{8} - 5 x^{6} - 14 x^{5} + 5 x^{4} + 6 x^{3} - 7 x^{2} + 1 ,
\]
...
\[
(n + 1)\,a(n) -  4 n\,a(n-1) + (n - 5)\,a(n-2) + 2 \left(4 n - 3\right)\,a(n-3) -  3 \left(n - 3\right)\,a(n-4) + 2 \left(n - 8\right)\,a(n-5) + (13 - n)\,a(n-6) -  8 \left(n - 7\right)\,a(n-7) + 2 \left(n - 7\right)\,a(n-8) + 2 \left(n - 9\right)\,a(n-9) \;=\;0
\]
holds for every $n>8$.
\end{theorem}
...
$n\le8$ some of the shifted terms $a(n-i)$ fall outside the range of the sequence,
...
section of A191625, agreeing on all 13 terms compared.
...
vanish, for all 22 values of $n$ from $n=9$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{8} - 5 x^{6} - 14 x^{5} + 5 x^{4} + 6 x^{3} - 7 x^{2} + 1$ is an
identity of functions and the theorem holds for all $n>8$ simultaneously.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A191625.
