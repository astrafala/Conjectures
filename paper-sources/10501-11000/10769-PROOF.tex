...
\title{The empirical recurrence for OEIS A266327, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266327 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A266327 is ``Binary representation of the n-th iteration of the "Rule 20" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 11, 1, 11, 1, 11, 1, 11,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 11 x - 1}{x^{2} - 1},
\]
...
compared against all $74$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266327.
