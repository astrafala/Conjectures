...
\title{The empirical recurrence for OEIS A263806, derived from the entry's generating function}
...
\begin{abstract}
OEIS A263806 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A263806 is ``Decimal representation of the n-th iteration of the "Rule 157" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 3, 7, 47, 95, 703, 1407, 11007,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:27, revision 33) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{32 x^{4} - 32 x^{3} + 16 x^{2} - 1}{32 x^{4} - 48 x^{3} + 14 x^{2} + 3 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 32 x^{4} + 48 x^{3} - 14 x^{2} - 3 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 14\,a(n-2) - 48\,a(n-3) + 32\,a(n-4)
\]
for every $n>4$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 32 x^{4} + 32 x^{3} - 16 x^{2} + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263806.
