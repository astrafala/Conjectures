...
\title{The empirical recurrence for OEIS A266593, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266593 carries an empirical recurrence of order $4$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{5} - 2 x^{4} + x^{2} + x + 1}{x^{4} - 2 x^{2} + 1},
\]
