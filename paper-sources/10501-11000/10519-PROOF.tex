...
\title{The empirical recurrence for OEIS A129393, derived from the entry's generating function}
...
\begin{abstract}
OEIS A129393 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A129393 is ``Row sums of A129392.''. It has offset $0$ and begins
\[
1, 5, -15, 29, -23, -43, 225, -499,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 30 2026 22:35:46, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{2} - 7 x - 1}{4 x^{3} - x^{2} - 2 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 4 x^{3} + x^{2} + 2 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A129393.
