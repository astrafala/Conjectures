...
\title{The empirical recurrence for OEIS A267777, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267777 carries an empirical recurrence of order $2$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{11000 x^{2} - 100 x + 1}{100 x^{2} - 101 x + 1},
\]
...
$n>2$. The bound is exact: at $n=2$ the recurrence fails on the entry's own published terms.
\end{proof}
