...
\title{The empirical recurrence for OEIS A266285, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266285 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 12 2025 08:08:16, revision 28) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{3} + x + 1}{x^{4} - 2 x^{2} + 1},
\]
