...
\title{The empirical recurrence for OEIS A267596, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267596 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 30 2025 13:08:05, revision 32) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{3} + 9 x^{2} - 3 x + 1}{4 x^{2} - 5 x + 1},
\]
