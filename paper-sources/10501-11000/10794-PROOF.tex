...
\title{The empirical recurrence for OEIS A269673, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269673 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269673 is ``Number of length-n 0..3 arrays with no repeated value differing from the previous repeated value by other than plus or minus one modulo 3+1.''. It has offset $1$ and begins
\[
4, 16, 60, 224, 820, 2976, 10700, 38224,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 26 2019 04:26:21, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 16 x^{3} - 4 x^{2} + 4 x}{15 x^{3} + x^{2} - 5 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;15 x^{3} + x^{2} - 5 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269673.
