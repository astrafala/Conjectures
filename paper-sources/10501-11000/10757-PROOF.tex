...
\title{The empirical recurrence for OEIS A228254, derived from the entry's generating function}
...
\begin{abstract}
OEIS A228254 carries an empirical recurrence of order $2$. The entry separately records a
...
\begin{quote}
G.f.: x*(17 + 58*x + 8*x\^{}2 - 188*x\^{}3 - 264*x\^{}4) / ((1 - 2*x)*(1 - 3*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 10 2018 08:51:42, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 264 x^{5} - 188 x^{4} + 8 x^{3} + 58 x^{2} + 17 x}{6 x^{2} - 5 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 264 x^{5} - 188 x^{4} + 8 x^{3} + 58 x^{2} + 17 x,
\]
