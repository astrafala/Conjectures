...
\title{The empirical recurrence for OEIS A117547, derived from the entry's generating function}
...
\begin{abstract}
OEIS A117547 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A117547 is ``Numbers k such that 2*k\^{}2+1 is a perfect power.''. It has offset $1$ and begins
\[
0, 2, 11, 12, 70, 408, 2378, 13860,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = 6*a(n-1) - a(n-2) for n > 5.
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: x\^{}2*(2-x-52*x\^{}2+9*x\^{}3)/ (1-6*x+x\^{}2). (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2026 21:57:37, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{9 x^{5} - 52 x^{4} - x^{3} + 2 x^{2}}{x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{2} - 6 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;9 x^{5} - 52 x^{4} - x^{3} + 2 x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A117547.
