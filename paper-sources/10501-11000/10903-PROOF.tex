...
\title{The empirical recurrence for OEIS A063166, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063166 carries an empirical recurrence of order $2$. The entry separately records a
...
\begin{quote}
G.f.: x*(7 + 20*x + x\^{}2) / (1 - x)\^{}2.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 22 2019 12:18:12, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} + 20 x^{2} + 7 x}{x^{2} - 2 x + 1},
\]
