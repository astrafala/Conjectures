...
\title{A proof of the conjectured recurrence for OEIS A274295}
...
\begin{abstract}
OEIS A274295 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $24 x^{7} + 14 x^{6} + 55 x^{5} - 40 x^{4} + 17 x^{3} + 10 x^{2} - 10 x + 2$, a polynomial of degree $7$, which proves
...
OEIS A274295 is ``a(n) = n+binomial(2*n-6,n-3)+binomial(2*n-5,n-3)+binomial(n-1,n-3)+Sum\_\{i=1..n-3\} (binomial(n+i-3,n-3)+2*n-i-5)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 6, 16, 43, 120, 369,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{3}}{\sqrt{1 - 4 x}} - \frac{23 x^{2}}{\left(x - 1\right)^{3}} + \frac{x^{2}}{\sqrt{1 - 4 x}} - 4 x + \frac{31 x}{\left(x - 1\right)^{3}} - 11 - \frac{12}{\left(x - 1\right)^{3}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+2)*a(n) +(7*n-18)*a(n-1) +14*(-n+3)*a(n-2) +2*(3*n-2)*a(n-3) +(11*n-90)*a(n-4) +(-13*n+102)*a(n-5) +2*(2*n-17)*a(n-6)=0. - \emph{R. J. Mathar}, Oct 07 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 30) the
...
\[
p_{0}(n)=2 - n,\qquad p_{1}(n)=7 n - 18,\qquad p_{2}(n)=- 14 \left(n - 3\right),\qquad p_{3}(n)=2 \left(3 n - 2\right),\qquad p_{4}(n)=11 n - 90,\qquad p_{5}(n)=102 - 13 n,\qquad p_{6}(n)=2 \left(2 n - 17\right)
\]
...
\section{The computation for A274295}
...
\[
B(x)\;=\;24 x^{7} + 14 x^{6} + 55 x^{5} - 40 x^{4} + 17 x^{3} + 10 x^{2} - 10 x + 2 ,
\]
...
\[
(2 - n)\,a(n) + (7 n - 18)\,a(n-1) -  14 \left(n - 3\right)\,a(n-2) + 2 \left(3 n - 2\right)\,a(n-3) + (11 n - 90)\,a(n-4) + (102 - 13 n)\,a(n-5) + 2 \left(2 n - 17\right)\,a(n-6) \;=\;0
\]
holds for every $n>7$.
\end{theorem}
...
section of A274295, agreeing on all 13 terms compared.
...
vanish, for all 22 values of $n$ from $n=8$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=24 x^{7} + 14 x^{6} + 55 x^{5} - 40 x^{4} + 17 x^{3} + 10 x^{2} - 10 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A274295.
