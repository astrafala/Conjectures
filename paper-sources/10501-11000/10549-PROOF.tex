...
\title{The empirical recurrence for OEIS A267465, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267465 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267465 is ``Number of length-n 0..2 arrays with no following elements larger than the first repeated value.''. It has offset $1$ and begins
\[
3, 9, 24, 62, 160, 418, 1112, 3018,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 05 2018 09:35:22, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 7 x^{4} + 21 x^{3} - 15 x^{2} + 3 x}{12 x^{4} - 28 x^{3} + 23 x^{2} - 8 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;12 x^{4} - 28 x^{3} + 23 x^{2} - 8 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 7 x^{4} + 21 x^{3} - 15 x^{2} + 3 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267465.
