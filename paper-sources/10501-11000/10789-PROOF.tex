...
\title{The empirical recurrence for OEIS A269462, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269462 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269462 is ``Number of length-n 0..3 arrays with no repeated value equal to the previous repeated value.''. It has offset $1$ and begins
\[
4, 16, 60, 228, 852, 3180, 11796, 43644,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 22 2019 08:19:32, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 20 x^{3} - 4 x^{2} + 4 x}{18 x^{3} - 5 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;18 x^{3} - 5 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269462.
