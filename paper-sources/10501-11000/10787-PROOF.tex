...
\title{The empirical recurrence for OEIS A268943, derived from the entry's generating function}
...
\begin{abstract}
OEIS A268943 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A268943 is ``Number of length-n 0..7 arrays with no repeated value unequal to the previous repeated value plus one mod 7+1.''. It has offset $1$ and begins
\[
8, 64, 504, 3928, 30344, 232696, 1773384, 13443064,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 17 2019 04:20:36, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 56 x^{3} - 40 x^{2} + 8 x}{56 x^{3} + 34 x^{2} - 13 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;56 x^{3} + 34 x^{2} - 13 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;13\,a(n-1) - 34\,a(n-2) - 56\,a(n-3)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268943.
