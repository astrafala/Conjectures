...
\title{The empirical closed form for OEIS A219498, proved}
...
\begin{abstract}
OEIS A219498 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=33$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219498 is ``Number of n X 4 arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 n X 4 array.''. It has offset $1$ and begins
\[
4, 7, 18, 35, 58, 88, 126, 173,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/6)*n\^{}3 + 1*n\^{}2 + (23/6)*n - 7 for n>2.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2018 06:35:00, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=33$
...
states number $S=33$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219498 records is correct for all $n>2$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219498.
