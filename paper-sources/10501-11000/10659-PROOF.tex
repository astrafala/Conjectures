...
\title{A proof of the conjectured recurrence for OEIS A025757}
...
\begin{abstract}
OEIS A025757 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $2592 x^{3}$, a polynomial of degree $3$, which proves
...
OEIS A025757 is ``4th-order Vatalan numbers (generalization of Catalan numbers)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 7, 69, 783, 9597, 123495, 1643397,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4}{\sqrt[4]{1 - 16 x} + 3} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 5*n*(n-1)*(n-2)*a(n) -(239*n-600)*(n-1)*(n-2)*a(n-1) +24*(n-2)*(158*n\textasciicircum{}2-953*n+1445)*a(n-2) +16*(-1232*n\textasciicircum{}3+13056*n\textasciicircum{}2-45949*n+53730)*a(n-3) -128*(4*n-15)*(2*n-7)*(4*n-13)*a(n-4)=0. - \emph{R. J. Mathar}, Jul 28 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2026-01-03, revision 36) the
...
\[
p_{0}(n)=5 n \left(n - 2\right) \left(n - 1\right),\qquad p_{1}(n)=- \left(n - 2\right) \left(n - 1\right) \left(239 n - 600\right),\qquad p_{2}(n)=24 \left(n - 2\right) \left(158 n^{2} - 953 n + 1445\right),\qquad p_{3}(n)=- 16 \left(1232 n^{3} - 13056 n^{2} + 45949 n - 53730\right),\qquad p_{4}(n)=- 128 \left(2 n - 7\right) \left(4 n - 15\right) \left(4 n - 13\right)
\]
...
\[
5 n \left(n - 2\right) \left(n - 1\right)\,a(n) -  \left(n - 2\right) \left(n - 1\right) \left(239 n - 600\right)\,a(n-1) + 24 \left(n - 2\right) \left(158 n^{2} - 953 n + 1445\right)\,a(n-2) -  16 \left(1232 n^{3} - 13056 n^{2} + 45949 n - 53730\right)\,a(n-3) -  128 \left(2 n - 7\right) \left(4 n - 15\right) \left(4 n - 13\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A025757, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2592 x^{3}$ is an
...
\url{https://oeis.org}, 2026. Sequence A025757.
