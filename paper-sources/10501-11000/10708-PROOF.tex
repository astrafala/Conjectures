...
\title{A proof of the conjectured recurrence for OEIS A226751}
...
\begin{abstract}
OEIS A226751 carries a conjectured linear recurrence with polynomial coefficients, of
order 2. The entry posts no generating function; it defines the sequence by an
...
$204 x$, of degree $1$, which proves the recurrence for every $n>1$.
\end{abstract}
...
OEIS A226751 is ``G.f.: 1 / (1 + 6*x*G(x) - 7*x*G(x)\^{}2), where G(x) = 1 + x*G(x)\^{}3 is the g.f. of A001764''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 9, 48, 289, 1761, 10932, 68664,\ \dots
\]
...
a(n)\;=\;[x^{n}]\,f(x)\,g(x)^{n},
\qquad f(x)=- \frac{1}{2 x^{2} - x - 1},\qquad g(x)=\frac{1}{x^{2} - 2 x + 1} .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} 18*n*(2*n-1)*(55*n-76)*a(n) +(-11605*n\^{}3+28521*n\^{}2-20870*n+4536)*a(n-1) -24*(55*n-21)*(3*n-4)*(3*n-2)*a(n-2)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-08-15, revision 29) the
...
For A226751 this yields
\[
216 t^{2} y^{3} + 211 t y^{3} - 162 t y^{2} - 36 y^{3} + 24 y^{2} + 11 y + 1 \;=\;0 ,
\]
...
\[
18 n \left(2 n - 1\right) \left(55 n - 76\right)\,a(n) + (- 11605 n^{3} + 28521 n^{2} - 20870 n + 4536)\,a(n-1) -  24 \left(3 n - 4\right) \left(3 n - 2\right) \left(55 n - 21\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
The result was then compared term by term with the DATA section of A226751, agreeing on
...
all 21 values of $n$ from $n=2$ upward.
...
the identity $B=204 x$ is an identity of functions and the theorem holds for all
$n>1$ simultaneously.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A226751.
