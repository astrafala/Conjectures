...
\title{The empirical recurrence for OEIS A007780, derived from the entry's generating function}
...
\begin{abstract}
OEIS A007780 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A007780 is ``Losing initial configurations in 2-hole Tchuka Ruma.''. It has offset $1$ and begins
\[
1, 2, 3, 6, 9, 11, 18, 20,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 25 2018 14:09:38, revision 20) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{18 x^{16} + 243 x^{15} + 75 x^{14} + 9 x^{12} + 27 x^{11} + 30 x^{10} + 27 x^{9} + 13 x^{8} + 9 x^{7} + 7 x^{6} - 2 x^{2} - x}{3 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - 3 x^{2}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 18 x^{16} - 243 x^{15} - 75 x^{14} - 9 x^{12} - 27 x^{11} - 30 x^{10} - 27 x^{9} - 13 x^{8} - 9 x^{7} - 7 x^{6} + 2 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A007780.
