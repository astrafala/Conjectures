...
\title{The empirical recurrence for OEIS A207142, derived from the entry's generating function}
...
\begin{abstract}
OEIS A207142 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A207142 is ``Number of (n+1) X 2 0..2 arrays with every 2 X 2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
50, 30, 80, 62, 159, 122, 315, 242,\ \dots
\]
...
\begin{quote}
G.f.: x*(50 - 20*x - 50*x\^{}2 + 22*x\^{}3 - 3*x\^{}4 - x\^{}5 - x\^{}6 + x\^{}7) / ((1 - x)*(1 - 2*x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2018 12:25:11, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{8} - x^{7} - x^{6} - 3 x^{5} + 22 x^{4} - 50 x^{3} - 20 x^{2} + 50 x}{2 x^{3} - 2 x^{2} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{3} - 2 x^{2} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{8} - x^{7} - x^{6} - 3 x^{5} + 22 x^{4} - 50 x^{3} - 20 x^{2} + 50 x,
\]
...
compared against all $37$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207142.
