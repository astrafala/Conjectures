...
\title{The empirical recurrence for OEIS A251421, derived from the entry's generating function}
...
\begin{abstract}
OEIS A251421 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A251421 is ``Number of length n+2 0..1 arrays with the sum of the maximum minus twice the median plus the minimum of adjacent triples multiplied by some arrangement of +-1 equal to zero.''. It has offset $1$ and begins
\[
2, 12, 12, 40, 56, 144, 240, 544,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 20 2018 06:51:59, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 16 x^{3} + 8 x^{2} + 2 x}{4 x^{3} - 2 x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{3} - 2 x^{2} - 2 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251421.
