...
\title{The empirical recurrence for OEIS A233123, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233123 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 19 2018 09:40:13, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{2} + x}{48 x^{2} - 16 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;48 x^{2} - 16 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233123.
