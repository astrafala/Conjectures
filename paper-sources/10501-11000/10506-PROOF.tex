...
\title{The empirical recurrence for OEIS A265723, derived from the entry's generating function}
...
\begin{abstract}
OEIS A265723 carries an empirical recurrence of order $4$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{3} + 4 x^{2} + 3 x}{x^{4} - 2 x^{2} + 1},
\]
