...
\title{The empirical recurrence for OEIS A267921, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267921 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:30, revision 18) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{8 x^{2} + 1}{4 x^{2} - 5 x + 1},
\]
