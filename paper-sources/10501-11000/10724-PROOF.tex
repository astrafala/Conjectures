...
\title{The empirical recurrence for OEIS A204707, proved by transfer matrix}
...
\begin{abstract}
OEIS A204707 counts arrays over $\{0,\dots,1\}$ under a condition on the permanent of every
...
a bounded window of consecutive rows, so the count is a walk count on $S=6$ states and is
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 09 2018 06:49:39, revision 9) this is still
...
\]
on the $S=6$ states that survive. Rows carrying no edge at all are dropped: such a row could
...
first term is $13$, and the model reproduces it along with every later term; that is the
