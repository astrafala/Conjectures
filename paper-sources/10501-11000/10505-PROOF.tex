...
\title{The empirical recurrence for OEIS A265722, derived from the entry's generating function}
...
\begin{abstract}
OEIS A265722 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A265722 is ``Number of ON (black) cells in the n-th iteration of the "Rule 1" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 0, 1, 4, 1, 8, 1, 12,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{3} - x^{2} + 1}{x^{4} - 2 x^{2} + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A265722.
