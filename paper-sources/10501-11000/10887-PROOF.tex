...
\title{The empirical recurrence for OEIS A063130, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063130 carries an empirical recurrence of order $2$. The entry separately records a
...
G.f.: x*(1+x)*(7+x) / (1-x)\^{}2.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 03 2025 13:12:56, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} + 8 x^{2} + 7 x}{x^{2} - 2 x + 1},
\]
