...
\title{The empirical recurrence for OEIS A267940, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267940 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:30, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 10000 x^{3} + 10100 x^{2} - 90 x + 1}{100 x^{2} - 101 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 10000 x^{3} + 10100 x^{2} - 90 x + 1,
\]
