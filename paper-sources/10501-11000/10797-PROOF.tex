...
\title{The empirical recurrence for OEIS A269684, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269684 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 27 2019 07:51:53, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{2} + 3 x}{2 x^{3} + 3 x^{2} - 4 x + 1},
\]
