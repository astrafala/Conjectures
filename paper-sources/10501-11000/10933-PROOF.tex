...
\title{The empirical recurrence for OEIS A262242, derived from the entry's generating function}
...
\begin{abstract}
OEIS A262242 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A262242 is ``Triangular numbers representable as 2\^{}x + 2\^{}y.''. It has offset $1$ and begins
\[
3, 6, 10, 36, 66, 136, 528, 2080,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 05 2021 16:55:05, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{240 x^{7} + 28 x^{6} - 70 x^{5} + 24 x^{4} - 2 x^{3} - 12 x^{2} + 3 x}{8 x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;8 x^{2} - 6 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;240 x^{7} + 28 x^{6} - 70 x^{5} + 24 x^{4} - 2 x^{3} - 12 x^{2} + 3 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262242.
