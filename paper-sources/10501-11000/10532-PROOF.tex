...
\title{The empirical recurrence for OEIS A267035, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267035 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267035 is ``Binary representation of the n-th iteration of the "Rule 85" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 11, 10000, 111111, 100000000, 1111111111, 1000000000000, 11111111111111,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{1100 x^{3} - x^{2} + 11 x + 1}{10000 x^{4} - 10001 x^{2} + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267035.
