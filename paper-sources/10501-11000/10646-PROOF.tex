...
\title{The empirical recurrence for OEIS A126474, derived from the entry's generating function}
...
\begin{abstract}
OEIS A126474 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A126474 is ``Number of arrays in [1..6]\^{}n with adjacent elements differing by three or less.''. It has offset $0$ and begins
\[
1, 6, 30, 154, 788, 4034, 20650, 105708,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:12:43, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{2} + x + 1}{2 x^{3} - x^{2} - 5 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{3} - x^{2} - 5 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A126474.
