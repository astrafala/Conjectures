...
\title{The empirical recurrence for OEIS A267589, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267589 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A267589 is ``Decimal representation of the middle column of the "Rule 169" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 2, 5, 10, 20, 41, 83, 167,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} - x^{3} + x^{2} - x + 1}{2 x^{2} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{2} - 3 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267589.
