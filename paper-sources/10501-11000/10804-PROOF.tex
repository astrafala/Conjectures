...
\title{The empirical recurrence for OEIS A269911, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269911 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269911 is ``Number of active (ON, black) cells at stage 2\^{}n-1 of the two-dimensional cellular automaton defined by "Rule 3", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 5, 45, 221, 957, 3965, 16125, 65021,\ \dots
\]
...
\begin{quote}
G.f.: (1-2*x+24*x\^{}2-32*x\^{}3) / ((1-x)*(1-2*x)*(1-4*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:31, revision 20) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{32 x^{3} - 24 x^{2} + 2 x - 1}{8 x^{3} - 14 x^{2} + 7 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269911.
