...
\title{The empirical recurrence for OEIS A267183, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267183 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267183 is ``Row 3 of the square array in A267181.''. It has offset $0$ and begins
\[
1, 3, 4, 0, 4, 5, 1, 5,\ \dots
\]
...
\begin{quote}
G.f.: (1+2*x+x\^{}2-5*x\^{}3+2*x\^{}4) / ((1-x)\^{}2*(1+x+x\^{}2)). (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 29 2016 11:21:58, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{4} - 5 x^{3} + x^{2} + 2 x + 1}{x^{4} - x^{3} - x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267183.
