...
\title{The empirical recurrence for OEIS A063116, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063116 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 12 2019 12:33:29, revision 24) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} + 12 x^{2} + 3 x}{x^{2} - 2 x + 1},
\]
