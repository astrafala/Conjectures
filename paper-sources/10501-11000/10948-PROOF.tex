...
\title{The empirical recurrence for OEIS A267595, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267595 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 07 2025 06:51:49, revision 33) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 100 x^{3} + 201 x^{2} - 91 x + 1}{100 x^{2} - 101 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 100 x^{3} + 201 x^{2} - 91 x + 1,
\]
