...
\title{The empirical recurrence for OEIS A267812, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267812 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A267812 is ``Decimal representation of the n-th iteration of the "Rule 217" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 1, 27, 119, 495, 2015, 8127, 32639,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 20) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 32 x^{4} + 64 x^{3} - 34 x^{2} + 6 x - 1}{8 x^{3} - 14 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;32 x^{4} - 64 x^{3} + 34 x^{2} - 6 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267812.
