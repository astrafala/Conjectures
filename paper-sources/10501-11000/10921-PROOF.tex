...
\title{The empirical recurrence for OEIS A211482, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211482 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A211482 is ``Number of ordered triples (w,x,y) with all terms in {-n,...,0,...,n} and w*x+w*y+x*y=w*x*y.''. It has offset $0$ and begins
\[
1, 10, 22, 35, 50, 62, 80, 92,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 15 2017 10:59:08, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{7} + 6 x^{6} - 3 x^{5} + 2 x^{4} + x^{3} + 3 x^{2} + 8 x + 1}{x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 6 x^{7} + 6 x^{6} - 3 x^{5} + 2 x^{4} + x^{3} + 3 x^{2} + 8 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211482.
