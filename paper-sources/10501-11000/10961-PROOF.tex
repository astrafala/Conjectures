...
\title{The empirical recurrence for OEIS A267872, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267872 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:30, revision 23) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{3} + 4 x^{2} - x + 1}{x^{2} - 2 x + 1},
\]
...
compared against all $63$ terms the entry publishes: it reproduces every one. This is
