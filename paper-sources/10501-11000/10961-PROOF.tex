...
\title{The empirical recurrence for OEIS A257443, proved by transfer matrix}
...
\begin{abstract}
OEIS A257443 counts $(n+2)\times6$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
...
OEIS A257443 is ``Number of (n+2) X (4+2) 0..1 arrays with no 3 x 3 subblock diagonal sum 0 and no antidiagonal sum 3 and no row sum 1 and no column sum 1.''. It has offset $1$ and begins
\[
49, 51, 59, 71, 89, 116, 155, 212,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - a(n-2) + a(n-3) - a(n-4) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 21 2018 07:49:39, revision 8) this is still
...
exactly. Those integers vanish from the index corresponding to $n=6+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>6$. The bound is exact: at $n=6$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A257443.
