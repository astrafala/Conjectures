...
\title{The empirical recurrence for OEIS A187608, derived from the entry's generating function}
...
\begin{abstract}
OEIS A187608 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A187608 is ``Number of 4-step one space for components leftwards or up, two space for components rightwards or down asymmetric quasi-bishop's tours (antidiagonal moves become knight moves) on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 0, 28, 144, 340, 675, 1120,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 25 2018 11:48:52, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{29 x^{8} - 59 x^{7} + 8 x^{6} - 60 x^{5} - 28 x^{4}}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 29 x^{8} + 59 x^{7} - 8 x^{6} + 60 x^{5} + 28 x^{4},
\]
...
compared against all $41$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187608.
