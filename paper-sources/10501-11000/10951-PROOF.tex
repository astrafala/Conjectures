...
\title{The empirical recurrence for OEIS A267604, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267604 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A267604 is ``Decimal representation of the middle column of the "Rule 175" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 3, 6, 13, 27, 55, 111, 223,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 58) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} - x^{2} + 1}{2 x^{2} - 3 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267604.
