...
\title{The empirical recurrence for OEIS A234429, derived from the entry's generating function}
...
\begin{abstract}
OEIS A234429 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A234429 is ``Numbers which are the digital sum of the square of some prime.''. It has offset $1$ and begins
\[
4, 7, 9, 10, 13, 16, 19, 22,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 17 2025 09:52:10, revision 63) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{5} - x^{4} - x^{3} - x^{2} + 4 x}{x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234429.
