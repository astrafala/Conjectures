...
\title{The empirical recurrence for OEIS A267459, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267459 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267459 is ``Total number of ON (black) cells after n iterations of the "Rule 133" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 2, 3, 4, 7, 10, 15, 20,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{4} + x^{2} - 1}{x^{4} - 2 x^{3} + 2 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267459.
