...
\title{A proof of the conjectured identity for OEIS A273832}
...
\begin{abstract}
OEIS A273832 carries a conjectured identity expressing its terms through the terms of
A269911. We prove it. Each entry involved posts a generating function, so the claim is
a single identity between generating functions: writing $F_{a}$ for the generating
function of A273832 and $F_{j}$ for those of the entries on the right, the conjecture
...
OEIS A273832 is ``Number of active (ON, black) cells at stage 2\^{}n-1 of the two-dimensional cellular automaton defined by "Rule 961", based on the 5-celled von Neumann neighborhood''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 45, 221, 957, 3965, 16125, 65021,\ \dots
\]
Its generating function, as posted on the entry, is
\begin{equation}\label{eq:gf}
F_{a}(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 8 x^{4} + 46 x^{3} - 31 x^{2} + 3 x - 1}{\left(x - 1\right) \left(2 x - 1\right) \left(4 x - 1\right)} ,
\end{equation}
and the entry carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = A269911(n) for n>1. - \emph{R. J. Mathar}, Jun 02 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 20) the
...
\[
F_{A269911}(x) \;=\; \frac{32 x^{3} - 24 x^{2} + 2 x - 1}{\left(x - 1\right) \left(2 x - 1\right) \left(4 x - 1\right)}
\]
...
\section{The computation for A273832}

Assembling the right-hand side by Lemmas~\ref{lem:shift} and~\ref{lem:theta} gives
\[
R(x)\;=\;\frac{32 x^{3} - 24 x^{2} + 2 x - 1}{\left(x - 1\right) \left(2 x - 1\right) \left(4 x - 1\right)} ,
\]
...
\url{https://oeis.org}, 2026. Sequence A273832.
