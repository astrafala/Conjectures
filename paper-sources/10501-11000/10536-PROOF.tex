...
\title{The empirical recurrence for OEIS A267186, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267186 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267186 is ``Column 3 of the square array in A267181.''. It has offset $0$ and begins
\[
0, 2, 3, 0, 5, 6, 2, 6,\ \dots
\]
...
\begin{quote}
G.f.: x*(2+x-3*x\^{}2+3*x\^{}3-x\^{}5-x\^{}6) / ((1-x)\^{}2*(1+x+x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 30 2016 02:31:19, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{7} - x^{6} + 3 x^{4} - 3 x^{3} + x^{2} + 2 x}{x^{4} - x^{3} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{7} - x^{6} + 3 x^{4} - 3 x^{3} + x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267186.
