...
\title{The empirical recurrence for OEIS A269689, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269689 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269689 is ``Number of length-n 0..7 arrays with no repeated value differing from the previous repeated value by plus or minus one modulo 7+1.''. It has offset $1$ and begins
\[
8, 64, 512, 4080, 32416, 256880, 2031072, 16027696,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 27 2019 16:07:51, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 40 x^{3} - 48 x^{2} + 8 x}{42 x^{3} + 43 x^{2} - 14 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;42 x^{3} + 43 x^{2} - 14 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;14\,a(n-1) - 43\,a(n-2) - 42\,a(n-3)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269689.
