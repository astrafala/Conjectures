...
\title{The empirical recurrence for OEIS A266976, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266976 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A266976 is ``Decimal representation of the n-th iteration of the "Rule 78" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 6, 28, 104, 464, 1696, 7488, 27264,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 16 x^{3} + 4 x + 1}{32 x^{3} - 16 x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266976.
