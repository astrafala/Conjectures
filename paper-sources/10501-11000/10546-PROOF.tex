...
\title{The empirical recurrence for OEIS A267457, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267457 carries an empirical recurrence of order $4$. The entry separately records a
...
\begin{quote}
G.f.: (1+2*x)*(1-13*x\^{}2+32*x\^{}4) / ((1-x)*(1+x)*(1-4*x)*(1+4*x)). (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 28 2025 04:55:11, revision 31) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{64 x^{5} + 32 x^{4} - 26 x^{3} - 13 x^{2} + 2 x + 1}{16 x^{4} - 17 x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;16 x^{4} - 17 x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;64 x^{5} + 32 x^{4} - 26 x^{3} - 13 x^{2} + 2 x + 1,
\]
