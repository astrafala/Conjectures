...
\title{The empirical recurrence for OEIS A046962, derived from the entry's generating function}
...
\begin{abstract}
OEIS A046962 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A046962 is ``Bessel function |J\_0(n)| is a monotonically decreasing positive sequence.''. It has offset $0$ and begins
\[
1, 2, 5, 6, 9, 12, 15, 18,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 05 2021 21:45:35, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{333 x^{16} + 19 x^{10} - 16 x^{9} + 16 x^{8} + 2 x^{4} - 2 x^{3} + 2 x^{2} + 1}{x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;333 x^{16} + 19 x^{10} - 16 x^{9} + 16 x^{8} + 2 x^{4} - 2 x^{3} + 2 x^{2} + 1,
\]
...
compared against all $45$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A046962.
