...
\title{The empirical recurrence for OEIS A267885, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267885 carries an empirical recurrence of order $2$. The entry separately records a
...
\begin{quote}
G.f.: (1-x+1011*x\^{}2+9000*x\^{}3-10000*x\^{}4) / ((1-x)*(1-100*x)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 10000 x^{4} + 9000 x^{3} + 1011 x^{2} - x + 1}{100 x^{2} - 101 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 10000 x^{4} + 9000 x^{3} + 1011 x^{2} - x + 1,
\]
