...
\title{The empirical recurrence for OEIS A189274, derived from the entry's generating function}
...
\begin{abstract}
OEIS A189274 carries an empirical recurrence of order $1$. The entry separately records a
...
OEIS A189274 is ``Number of nX3 array permutations with each element not moved or moved diagonally or antidiagonally by one.''. It has offset $1$ and begins
\[
1, 9, 45, 225, 1125, 5625, 28125, 140625,\ \dots
\]
...
\begin{quote}
G.f.: x*(1 + 4*x)/(1 - 5*x). - \_Charles R Greathouse IV\_, May 20 2026
\end{quote}
As of the ``Last modified'' line on the live entry (May 20 2026 13:50:02, revision 25) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{2} - x}{5 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - 5 x
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A189274.
