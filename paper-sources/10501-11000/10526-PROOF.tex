...
\title{The empirical recurrence for OEIS A046963, derived from the entry's generating function}
...
\begin{abstract}
OEIS A046963 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A046963 is ``Bessel function |Y\_0(n)| is a monotonically decreasing positive sequence.''. It has offset $0$ and begins
\[
1, 4, 26, 29, 51, 73, 95, 117,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 05 2021 21:45:41, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{333 x^{11} + 19 x^{4} - 19 x^{3} + 19 x^{2} + 2 x + 1}{x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;333 x^{11} + 19 x^{4} - 19 x^{3} + 19 x^{2} + 2 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A046963.
