...
\title{The empirical recurrence for OEIS A269465, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269465 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269465 is ``Number of length-n 0..6 arrays with no repeated value equal to the previous repeated value.''. It has offset $1$ and begins
\[
7, 49, 336, 2310, 15834, 108402, 741090, 5060706,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 22 2019 08:35:30, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 77 x^{3} - 28 x^{2} + 7 x}{72 x^{3} + 18 x^{2} - 11 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;72 x^{3} + 18 x^{2} - 11 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;11\,a(n-1) - 18\,a(n-2) - 72\,a(n-3)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269465.
