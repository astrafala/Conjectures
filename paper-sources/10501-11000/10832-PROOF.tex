...
\title{A condition on each element and its neighbours: OEIS A183436}
...
\begin{abstract}
OEIS A183436 carries an empirical recurrence of order $6$ contributed by R.~H.~Hardin. It is
...
OEIS A183436 is ``Number of n X 3 binary arrays with every 1 having exactly one king-move neighbor equal to 1.''. It has offset $1$ and begins
\[
3, 13, 29, 89, 273, 751, 2221, 6485,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + 3*a(n-2) + 8*a(n-3) - a(n-4) - 2*a(n-5) - 2*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 29 2018 05:30:41, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $3$ columns over the alphabet $\{0,1\}$, growing one row at a time. Its condition is that every entry equal to $1$ must have exactly $1$ of its eight king-move neighbours equal to $1$.
...
Vertices with identical futures are merged, which leaves $S=8$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 3\,a(n-2) + 8\,a(n-3) - a(n-4) - 2\,a(n-5) - 2\,a(n-6)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183436.
