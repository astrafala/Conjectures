...
\title{The empirical recurrence for OEIS A253361, proved by transfer matrix}
...
\begin{abstract}
OEIS A253361 counts arrays of width $4$ over $\{0,1\}$ in which the largest entry on the main diagonal minus the smallest on the antidiagonal of every $2\times2$ and $3\times3$ subblock differs from the same number at the neighbouring subblock. The entry carries an
...
$4$ consecutive rows and the count is a walk count on $S=386$ states.
\end{abstract}
...
OEIS A253361 is ``Number of (n+2) X (2+2) 0..1 arrays with every 2 X 2 and 3 X 3 subblock diagonal maximum minus antidiagonal minimum unequal to its neighbors horizontally and vertically.''. It has offset $1$ and begins
\[
272, 424, 960, 1536, 3136, 5664, 10496, 20000,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-2) - 8*a(n-4) + 8*a(n-6) + 4*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 11 2018 09:00:12, revision 8) this is still
...
Thus
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=386,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253361.
