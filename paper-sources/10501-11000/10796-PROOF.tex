...
\title{The empirical recurrence for OEIS A269677, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269677 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 26 2019 11:10:11, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 64 x^{3} - 40 x^{2} + 8 x}{63 x^{3} + 33 x^{2} - 13 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;63 x^{3} + 33 x^{2} - 13 x + 1
\]
