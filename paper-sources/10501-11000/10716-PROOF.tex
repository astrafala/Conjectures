...
\title{The empirical recurrence for OEIS A189808, derived from the entry's generating function}
...
\begin{abstract}
OEIS A189808 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A189808 is ``Number of right triangles on a (n+1) X 4 grid.''. It has offset $1$ and begins
\[
28, 94, 200, 342, 524, 732, 972, 1236,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 03 2018 07:02:34, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{11} - 4 x^{10} + 8 x^{8} - 6 x^{7} + 14 x^{6} - 4 x^{5} + 4 x^{4} - 2 x^{3} - 10 x^{2} - 28 x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 4 x^{11} + 4 x^{10} - 8 x^{8} + 6 x^{7} - 14 x^{6} + 4 x^{5} - 4 x^{4} + 2 x^{3} + 10 x^{2} + 28 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A189808.
