...
\title{The empirical recurrence for OEIS A189809, derived from the entry's generating function}
...
\begin{abstract}
OEIS A189809 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A189809 is ``Number of right triangles on an (n+1) X 5 grid.''. It has offset $1$ and begins
\[
46, 158, 342, 596, 926, 1308, 1754, 2250,\ \dots
\]
...
\begin{quote}
G.f.: 2*x*(23 + 10*x + 3*x\^{}2 - x\^{}3 + 3*x\^{}4 - 12*x\^{}5 + 6*x\^{}6 - 7*x\^{}7 - x\^{}8 + 2*x\^{}9 - 6*x\^{}10 + 4*x\^{}12 - 4*x\^{}13 + 2*x\^{}16 - 2*x\^{}17) / (1 - x)\^{}3.
\end{quote}
As of the ``Last modified'' line on the live entry (May 03 2018 07:02:54, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{18} - 4 x^{17} + 8 x^{14} - 8 x^{13} + 12 x^{11} - 4 x^{10} + 2 x^{9} + 14 x^{8} - 12 x^{7} + 24 x^{6} - 6 x^{5} + 2 x^{4} - 6 x^{3} - 20 x^{2} - 46 x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 4 x^{18} + 4 x^{17} - 8 x^{14} + 8 x^{13} - 12 x^{11} + 4 x^{10} - 2 x^{9} - 14 x^{8} + 12 x^{7} - 24 x^{6} + 6 x^{5} - 2 x^{4} + 6 x^{3} + 20 x^{2} + 46 x,
\]
a POLYNOMIAL of degree $18$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $18$, so by
Lemma~\ref{lem:key} the residual $a(n)-\sum_ic_ia(n-i)$ vanishes for every
$n>18$. The bound is exact: at $n=18$ the recurrence fails on the entry's own published terms.
\end{proof}
...
they stop at index $18$ because that polynomial has degree $18$. A reader who trusts
...
indices beyond $n=18$ and holds at every one, and at $n=18$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A189809.
