...
\title{The empirical recurrence for OEIS A211525, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211525 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A211525 is ``Number of -1..1 arrays x(i) of n+1 elements i=1..n+1 with set{t,u,v in 0,1}((x[i+t]+x[j+u]+x[k+v])*(-1)\^{}(t+u+v)) having two or four distinct values for every i,j,k<=n.''. It has offset $1$ and begins
\[
8, 14, 24, 44, 80, 152, 288, 560,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 25 2024 09:31:52, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 20 x^{3} - 2 x^{2} + 8 x}{4 x^{3} - 2 x^{2} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211525.
