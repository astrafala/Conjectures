...
\title{The empirical recurrence for OEIS A267934, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267934 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 24 2025 12:23:17, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 10000 x^{4} - 900 x^{3} + 11010 x^{2} - 100 x + 1}{100 x^{2} - 101 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 10000 x^{4} - 900 x^{3} + 11010 x^{2} - 100 x + 1,
\]
