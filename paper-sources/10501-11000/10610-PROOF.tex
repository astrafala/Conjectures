...
\title{The empirical recurrence for OEIS A252978, derived from the entry's generating function}
...
\begin{abstract}
OEIS A252978 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A252978 is ``Number of n X 3 nonnegative integer arrays with upper left 0 and lower right its king-move distance away minus 2 and every value increasing by 0 or 1 with every step right, diagonally se or down.''. It has offset $1$ and begins
\[
1, 1, 1, 33, 266, 851, 1836, 3221,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 07 2018 11:39:59, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 48 x^{7} - 151 x^{6} - 169 x^{5} - 32 x^{4} - x^{3} + 2 x^{2} - x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;48 x^{7} + 151 x^{6} + 169 x^{5} + 32 x^{4} + x^{3} - 2 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252978.
