...
\title{Parities, differences and neighbours: OEIS A229397}
...
\begin{abstract}
OEIS A229397 carries an empirical recurrence of order $4$ contributed by R.~H.~Hardin. It is
...
OEIS A229397 is ``Number of n X 3 0..2 arrays with top left element 0, horizontal differences mod 3 never 1, vertical differences mod 3 never -1, and antidiagonal differences never 0.''. It has offset $1$ and begins
\[
4, 13, 44, 153, 536, 1881, 6604, 23189,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-1) - 6*a(n-2) + 3*a(n-3) - a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 14 2018 16:11:59, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $3$ columns over the alphabet $\{0,1,2\}$, growing one row at a time, and asks that the top left entry is $0$, and the step to the right may never change the entry by $1$ modulo $3$, the step downwards may never change the entry by $-1$ modulo $3$, the step antidiagonally may never change the entry by $0$ modulo $3$.
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-1) - 6\,a(n-2) + 3\,a(n-3) - a(n-4)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229397.
