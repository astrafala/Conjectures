...
\title{The empirical recurrence for OEIS A269613, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269613 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A269613 is ``Number of length-n 0..2 arrays with no repeated value differing from the previous repeated value by other than plus two, zero or minus 1.''. It has offset $1$ and begins
\[
3, 9, 27, 78, 222, 624, 1740, 4824,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 21 2018 17:19:50, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{3} - 3 x^{2} + 3 x}{4 x^{3} + 2 x^{2} - 4 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269613.
