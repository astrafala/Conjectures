...
\title{The empirical recurrence for OEIS A206198, derived from the entry's generating function}
...
\begin{abstract}
OEIS A206198 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A206198 is ``Number of (n+1) X (n+1) 0..3 arrays with every 2 X 3 or 3 X 2 subblock having exactly one clockwise edge increases.''. It has offset $1$ and begins
\[
256, 1060, 5136, 32920, 118936, 484704, 3270240, 11893600,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:46:35, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{21760 x^{7} + 28896 x^{6} - 12936 x^{5} - 7320 x^{4} - 5136 x^{3} - 1060 x^{2} - 256 x}{100 x^{3} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - 100 x^{3}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 21760 x^{7} - 28896 x^{6} + 12936 x^{5} + 7320 x^{4} + 5136 x^{3} + 1060 x^{2} + 256 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206198.
