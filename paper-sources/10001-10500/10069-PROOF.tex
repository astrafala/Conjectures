...
\title{The empirical recurrence for OEIS A178353, derived from the entry's generating function}
...
\begin{abstract}
OEIS A178353 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A178353 is ``Number of collinear point 4-tuples in a 7 X 7 X 7 X ... n-dimensional cubic grid.''. It has offset $0$ and begins
\[
0, 35, 660, 9767, 135240, 1856075, 25897500, 371453327,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = 57*a(n-1) - 1270*a(n-2) + 13830*a(n-3) - 73609*a(n-4) + 153153*a(n-5).
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: x*(-35+1335*x-16597*x\^{}2+67329*x\^{}3) / ( (11*x-1)*(13*x-1)*(17*x-1)*(9*x-1)*(7*x-1) ). (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 17 2025 23:12:08, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{67329 x^{4} - 16597 x^{3} + 1335 x^{2} - 35 x}{153153 x^{5} - 73609 x^{4} + 13830 x^{3} - 1270 x^{2} + 57 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 153153 x^{5} + 73609 x^{4} - 13830 x^{3} + 1270 x^{2} - 57 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 67329 x^{4} + 16597 x^{3} - 1335 x^{2} + 35 x,
\]
...
\[
153153 x^{5} - 73609 x^{4} + 13830 x^{3} - 1270 x^{2} + 57 x - 1,
\]
...
compared against all $10$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A178353.
