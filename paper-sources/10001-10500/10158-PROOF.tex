...
\title{The empirical recurrence for OEIS A267467, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267467 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A267467 is ``Number of length-n 0..4 arrays with no following elements larger than the first repeated value.''. It has offset $1$ and begins
\[
5, 25, 115, 515, 2285, 10119, 44901, 200119,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 05 2018 09:35:43, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 326 x^{6} + 920 x^{5} - 870 x^{4} + 365 x^{3} - 70 x^{2} + 5 x}{480 x^{6} - 1216 x^{5} + 1174 x^{4} - 565 x^{3} + 145 x^{2} - 19 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;480 x^{6} - 1216 x^{5} + 1174 x^{4} - 565 x^{3} + 145 x^{2} - 19 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;19\,a(n-1) - 145\,a(n-2) + 565\,a(n-3) - 1174\,a(n-4) + 1216\,a(n-5) - 480\,a(n-6)
\]
for every $n>6$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 326 x^{6} + 920 x^{5} - 870 x^{4} + 365 x^{3} - 70 x^{2} + 5 x,
\]
...
\[
480 x^{6} - 1216 x^{5} + 1174 x^{4} - 565 x^{3} + 145 x^{2} - 19 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267467.
