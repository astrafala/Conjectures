...
\title{The empirical recurrence for OEIS A146126, derived from the entry's generating function}
...
\begin{abstract}
OEIS A146126 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A146126 is ``Number of n X n arrays of squares of integers with every (n-2) X (n-2) subblock summing to 5 and every element equal to at least one neighbor.''. It has offset $4$ and begins
\[
0, 235, 9716, 43201, 125064, 290427, 583948, 1060251,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 23 2018 12:28:50, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 18 x^{12} - 430 x^{11} + 1212 x^{10} - 1132 x^{9} - 3869 x^{8} + 3029 x^{7} - 8541 x^{6} - 235 x^{5}}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;18 x^{12} + 430 x^{11} - 1212 x^{10} + 1132 x^{9} + 3869 x^{8} - 3029 x^{7} + 8541 x^{6} + 235 x^{5},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A146126.
