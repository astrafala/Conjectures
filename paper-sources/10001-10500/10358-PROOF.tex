...
\title{The empirical recurrence for OEIS A072134, derived from the entry's generating function}
...
\begin{abstract}
OEIS A072134 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A072134 is ``Second generation sequence in which each number is skipped that can be written as sum of distinct previous entries. To make the first generation we start with all natural numbers: this gives the powers of 2 (A000079). For the second generation we start with the natural numbers from which are removed the numbers of the first generation.''. It has offset $0$ and begins
\[
3, 5, 6, 7, 17, 19, 53, 55,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 09 2017 10:38:47, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{5} + 9 x^{4} + 11 x^{3} - 2 x^{2} - 8 x - 3}{3 x^{3} + 3 x^{2} - x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 3 x^{5} - 9 x^{4} - 11 x^{3} + 2 x^{2} + 8 x + 3,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A072134.
