...
\title{The empirical recurrence for OEIS A252178, derived from the entry's generating function}
...
\begin{abstract}
OEIS A252178 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A252178 is ``Number of length 2+2 0..n arrays with the sum of the maximum minus the minimum of adjacent triples multiplied by some arrangement of +-1 equal to zero.''. It has offset $1$ and begins
\[
12, 49, 132, 285, 536, 917, 1464, 2217,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 01 2018 12:02:25, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 5 x^{4} - 7 x^{3} + 11 x^{2} - 12 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} - 5 x^{4} + 7 x^{3} - 11 x^{2} + 12 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252178.
