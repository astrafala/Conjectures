...
\title{The empirical recurrence for OEIS A267210, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267210 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267210 is ``Decimal representation of the middle column of the "Rule 109" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 3, 7, 14, 29, 59, 118, 237,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{3} + x^{2} + x + 1}{2 x^{4} - x^{3} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{4} - x^{3} - 2 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267210.
