...
\title{The empirical recurrence for OEIS A145647, derived from the entry's generating function}
...
\begin{abstract}
OEIS A145647 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A145647 is ``First differences of A145646.''. It has offset $1$ and begins
\[
2734, 4556, 6379, 8202, 10024, 11846, 13669, 15492,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 01 2022 00:43:54, revision 21) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 912 x^{4} + 2735 x^{3} - 912 x^{2} + 2734 x}{x^{4} - 2 x^{3} + 2 x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 912 x^{4} + 2735 x^{3} - 912 x^{2} + 2734 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A145647.
