...
\title{The empirical recurrence for OEIS A205343, derived from the entry's generating function}
...
\begin{abstract}
OEIS A205343 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A205343 is ``Number of length 6 nonnegative integer arrays starting and ending with 0 with adjacent elements unequal but differing by no more than n.''. It has offset $1$ and begins
\[
0, 24, 138, 454, 1130, 2370, 4424, 7588,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 13:13:36, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{4} - 18 x^{3} - 24 x^{2}}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205343.
