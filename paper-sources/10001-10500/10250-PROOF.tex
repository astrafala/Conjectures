...
\title{The empirical recurrence for OEIS A210374, derived from the entry's generating function}
...
\begin{abstract}
OEIS A210374 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A210374 is ``Number of 2 X 2 matrices with all terms in {0,1,...,n} and (sum of terms) = n+2.''. It has offset $0$ and begins
\[
0, 4, 19, 40, 68, 104, 149, 204,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 16 2024 13:11:50, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{6 x^{4} - 12 x^{3} + 3 x^{2} + 4 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;6 x^{4} - 12 x^{3} + 3 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A210374.
