...
\title{The empirical recurrence for OEIS A227085, derived from the entry's generating function}
...
\begin{abstract}
OEIS A227085 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A227085 is ``Number of n X 2 binary arrays indicating whether each 2 X 2 subblock of a larger binary array has lexicographically increasing rows and columns, for some larger (n+1) X 3 binary array with rows and columns of the latter in lexicographically nondecreasing order.''. It has offset $1$ and begins
\[
4, 12, 29, 62, 122, 225, 393, 655,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 07 2018 03:04:26, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 5 x^{5} - 12 x^{4} + 17 x^{3} - 12 x^{2} + 4 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + 5 x^{5} - 12 x^{4} + 17 x^{3} - 12 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A227085.
