...
\title{The empirical recurrence for OEIS A184679, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184679 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 14 2018 08:21:36, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 96 x^{3} - 12 x^{2} + 28 x}{12 x^{3} - 5 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;12 x^{3} - 5 x + 1
\]
