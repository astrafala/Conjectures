...
\title{The empirical recurrence for OEIS A154142, derived from the entry's generating function}
...
\begin{abstract}
OEIS A154142 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 07 2025 12:44:28, revision 34) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + 10 x^{4} - 3 x^{3} - 10 x^{2}}{x^{5} - x^{4} - 6 x^{3} + 6 x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{5} - 10 x^{4} + 3 x^{3} + 10 x^{2},
\]
