...
\title{The empirical recurrence for OEIS A103627, derived from the entry's generating function}
...
\begin{abstract}
OEIS A103627 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A103627 is ``Let S(n) = {n,1,n}; sequence gives concatenation S(0), S(1), S(2), ...''. It has offset $0$ and begins
\[
0, 1, 0, 1, 1, 1, 2, 1,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = 2*a(n-3) - a(n-6) for n>5. (End)
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: x*(1 + x\^{}2 - x\^{}3 + x\^{}4) / ((1 - x)\^{}2*(1 + x + x\^{}2)\^{}2).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 22 2025 16:00:46, revision 42) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} - x^{4} + x^{3} + x}{x^{6} - 2 x^{3} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{6} - 2 x^{3} + 1
\]
...
compared against all $88$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A103627.
