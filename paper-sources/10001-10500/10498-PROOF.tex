...
\title{The empirical recurrence for OEIS A300500, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A300500 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A300500 is ``Number of nX2 0..1 arrays with every element equal to 0, 1, 3 or 4 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
2, 3, 2, 3, 6, 6, 11, 26,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) -a(n-2) +4*a(n-3) -4*a(n-4) +a(n-5) -2*a(n-6) +a(n-7)
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 07 2018 07:29:41, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) - a(n-2) + 4\,a(n-3) - 4\,a(n-4) + a(n-5) - 2\,a(n-6) + a(n-7)
\end{equation}
for all $n>8$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A300500.
