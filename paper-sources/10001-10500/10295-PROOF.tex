...
\title{The empirical recurrence for OEIS A208121, proved by transfer matrix}
...
\begin{abstract}
OEIS A208121 counts arrays over $\{0,\dots,1\}$ of width $6$ that avoid the patterns
$(0,0,1), (0,1,1)$ along one axis and $(0,0,0), (0,1,0)$ along the other, and carries an empirical recurrence of
order $8$ contributed by R.~H.~Hardin. It is true. The patterns have length $3$, so
...
$\mathbf{1}^{\!\top}P^{\,n-2-8}q(P)\mathbf{1}$, which vanishes identically
...
OEIS A208121 is ``Number of 6 X n 0..1 arrays avoiding 0 0 0 and 0 1 0 horizontally and 0 0 1 and 0 1 1 vertically.''. It has offset
$1$ and begins
\[
16, 256, 784, 2401, 17689, 130321, 577600,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 7*a(n-1) - 21*a(n-2) + 84*a(n-3) - 756*a(n-5) + 1701*a(n-6) - 5103*a(n-7) + 6561*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 28 2018 10:54:51, revision
8) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;7\,a(n-1) - 21\,a(n-2) + 84\,a(n-3) - 756\,a(n-5) + 1701\,a(n-6) - 5103\,a(n-7) + 6561\,a(n-8)
\end{equation}
for all $n>8$. The entry's array has a fixed number of rows and a growing number of
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-8}$ whenever $n-8\ge2$. Hence
\eqref{eq:conj} holds for every $n>t$ exactly when $u_j=0$ for every
$j>t-2-8$. Moreover $(u_j)$ satisfies the monic linear recurrence whose
...
$P^{\,n-2-8}$ gives the identity. The equivalence follows by letting the exponent
...
\begin{proof}
The vector $q(P)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $8$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated in turn. They vanish from the
index corresponding to $n=8+1$ onwards, and the run was continued past $S$ consecutive
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208121.
