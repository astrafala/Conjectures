...
\title{The empirical recurrence for OEIS A107750, derived from the entry's generating function}
...
\begin{abstract}
OEIS A107750 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A107750 is ``If n=0 then 0, else smallest number greater than its predecessor and having either more or fewer zeros in its binary representation.''. It has offset $0$ and begins
\[
0, 1, 2, 3, 4, 5, 7, 8,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = a(n-1) + a(n-3) - a(n-4) for n>3. (End)
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: x*(1+x)*(1+x\^{}2-x\^{}3+x\^{}4) / ((1-x)\^{}2*(1+x+x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 15 2025 23:04:29, revision 34) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} + x^{3} + x^{2} + x}{x^{4} - x^{3} - x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A107750.
