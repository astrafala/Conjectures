...
\title{The empirical recurrence for OEIS A071282, derived from the entry's generating function}
...
\begin{abstract}
OEIS A071282 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A071282 is ``Denominators of Peirce sequence of order 3.''. It has offset $0$ and begins
\[
2, 3, 1, 3, 2, 3, 2, 3,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{5} - 2 x^{4} - 3 x^{3} - x^{2} - 3 x - 2}{x^{6} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - x^{6}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{5} + 2 x^{4} + 3 x^{3} + x^{2} + 3 x + 2,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A071282.
