...
\title{The empirical recurrence for OEIS A221968, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221968 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A221968 is ``Number of -n..n arrays of length 5 with the sum ahead of each element differing from the sum following that element by n or less.''. It has offset $1$ and begins
\[
63, 705, 3647, 12609, 34111, 78273, 159615, 297857,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 14 2018 08:57:28, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 7 x^{5} + 42 x^{4} + 362 x^{3} + 327 x^{2} + 63 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + 7 x^{5} + 42 x^{4} + 362 x^{3} + 327 x^{2} + 63 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221968.
