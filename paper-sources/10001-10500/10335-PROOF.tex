...
\title{The empirical recurrence for OEIS A269495, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269495 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A269495 is ``Number of length-4 0..n arrays with no repeated value differing from the previous repeated value by one.''. It has offset $1$ and begins
\[
14, 77, 250, 617, 1286, 2389, 4082, 6545,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 24 2025 11:24:54, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 3 x^{4} - 5 x^{3} - 7 x^{2} - 14 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} - 3 x^{4} + 5 x^{3} + 7 x^{2} + 14 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269495.
