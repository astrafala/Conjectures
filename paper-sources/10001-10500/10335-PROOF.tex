...
\title{The empirical recurrence for OEIS A192473, derived from the entry's generating function}
...
\begin{abstract}
OEIS A192473 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A192473 is ``Coefficient of x in the reduction by x\^{}2->x+1 of the polynomial p(n,x)=1+x\^{}n+x\^{}(2n+2).''. It has offset $1$ and begins
\[
4, 9, 23, 58, 149, 385, 1000, 2605,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 07 2019 22:02:01, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} + x^{3} + 7 x^{2} - 4 x}{x^{4} - 2 x^{3} - 3 x^{2} + 4 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{4} + 2 x^{3} + 3 x^{2} - 4 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A192473.
