...
\title{The empirical recurrence for OEIS A250388, derived from the entry's generating function}
...
\begin{abstract}
OEIS A250388 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A250388 is ``Number of length 2+3 0..n arrays with no four consecutive terms having the maximum of any two terms equal to the minimum of the remaining two terms.''. It has offset $1$ and begins
\[
6, 78, 432, 1560, 4350, 10206, 21168, 40032,\ \dots
\]
...
\begin{quote}
G.f.: 6*x*(1 + x)*(1 + 6*x + 3*x\^{}2) / (1 - x)\^{}6.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 19 2025 13:03:50, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{18 x^{4} + 54 x^{3} + 42 x^{2} + 6 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;18 x^{4} + 54 x^{3} + 42 x^{2} + 6 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250388.
