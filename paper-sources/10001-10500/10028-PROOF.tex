...
\title{The empirical recurrence for OEIS A221464, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221464 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A221464 is ``Number of 0..n arrays of length 5 with each element unequal to at least one neighbor, starting with 0.''. It has offset $1$ and begins
\[
3, 32, 135, 384, 875, 1728, 3087, 5120,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 05 2018 13:47:00, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} - 5 x^{3} - 17 x^{2} - 3 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} - 6 x^{4} - 12 x^{3} + 14 x^{2} + 3 x,
\]
...
So the conjecture is not minimal: its polynomial $D(x)$ factors as the reduced denominator times $x - 1$, and the sequence already satisfies a recurrence of order 5. The conjecture is true, but it is a consequence of a shorter one the entry does not state.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221464.
