...
\title{The empirical recurrence for OEIS A252932, derived from the entry's generating function}
...
\begin{abstract}
OEIS A252932 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A252932 is ``Number of n X 2 nonnegative integer arrays with upper left 0 and every value within 3 of its king move distance from the upper left and every value increasing by 0 or 1 with every step right, diagonally se or down.''. It has offset $1$ and begins
\[
2, 5, 13, 34, 83, 176, 329, 558,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 07 2018 11:51:06, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} + 7 x^{5} + 4 x^{4} + 5 x^{3} - 3 x^{2} + 2 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} + 7 x^{5} + 4 x^{4} + 5 x^{3} - 3 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252932.
