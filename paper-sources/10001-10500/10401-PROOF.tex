...
\title{A proof of the conjectured recurrence for OEIS A261681}
...
\begin{abstract}
OEIS A261681 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $6 x^{2} - 2 x + 4$, a polynomial of degree $2$, which proves
...
OEIS A261681 is ``a(n) = 2\^{}n + binomial(n, floor(n/2)) - 1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 10, 21, 41, 83, 162,\ \dots
\]
The entry posts no generating function of its own. Its NAME states the
relation \texttt{a(n) = A000079(n) + A014495(n).} to another entry, and that entry's exponential generating
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x \left(x - 1\right) + \left(1 - x\right) \sqrt{1 - 4 x^{2}} + \left(x + 1\right) \left(2 x - 1\right)}{2 x \left(x - 1\right) \left(2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+1)*(n-4)*a(n) +(3*n\^{}2-9*n-8)*a(n-1) +2*(n\^{}2-9*n+16)*a(n-2) +4*(-3*n\^{}2+18*n-25)*a(n-3) +8*(n-3)\^{}2*a(n-4)=0. - \emph{R. J. Mathar}, Jan 04 2017
\end{quote}
...
\[
p_{0}(n)=- \left(n - 4\right) \left(n + 1\right),\qquad p_{1}(n)=3 n^{2} - 9 n - 8,\qquad p_{2}(n)=2 \left(n^{2} - 9 n + 16\right),\qquad p_{3}(n)=- 4 \left(3 n^{2} - 18 n + 25\right),\qquad p_{4}(n)=8 \left(n - 3\right)^{2}
\]
...
\section{The computation for A261681}
...
\[
- \left(n - 4\right) \left(n + 1\right)\,a(n) + (3 n^{2} - 9 n - 8)\,a(n-1) + 2 \left(n^{2} - 9 n + 16\right)\,a(n-2) -  4 \left(3 n^{2} - 18 n + 25\right)\,a(n-3) + 8 \left(n - 3\right)^{2}\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A261681, agreeing on all 13 terms compared.
...
vanish, for all 30 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=6 x^{2} - 2 x + 4$ is an
...
\url{https://oeis.org}, 2026. Sequence A261681.
