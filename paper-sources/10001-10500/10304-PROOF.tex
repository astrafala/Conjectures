...
\title{The empirical recurrence for OEIS A266252, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266252 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 15) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{7} - 2 x^{6} + x^{5} + x^{4} + x^{3} - 3 x^{2} - 3 x}{x^{5} - x^{4} - 2 x^{3} + 2 x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{7} + 2 x^{6} - x^{5} - x^{4} - x^{3} + 3 x^{2} + 3 x,
\]
