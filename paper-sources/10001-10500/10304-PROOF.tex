...
\title{The empirical recurrence for OEIS A096222, derived from the entry's generating function}
...
\begin{abstract}
OEIS A096222 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A096222 is ``Number of different rectangles when a piece of paper is folded n times in alternate directions.''. It has offset $0$ and begins
\[
1, 3, 9, 30, 100, 360, 1296, 4896,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 19 2020 11:51:33, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 24 x^{3} + 9 x^{2} + 3 x - 1}{64 x^{4} - 48 x^{3} + 6 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 64 x^{4} + 48 x^{3} - 6 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A096222.
