...
\title{The empirical recurrence for OEIS A251269, proved by transfer matrix}
...
\begin{abstract}
OEIS A251269 counts $3\times(n+1)$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
every $2\times2$ subblock, and carries an empirical recurrence of order $5$ contributed
...
on $S=8$ vertices whose edges are the admissible consecutive pairs, and the count is a
...
OEIS A251269 is ``Number of (2+1) X (n+1) 0..1 arrays with no 2 X 2 subblock having x11-x00 less than x10-x01.''. It has offset $1$ and begins
\[
35, 114, 313, 772, 1775, 3894, 8277, 17224,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) - 14*a(n-2) + 16*a(n-3) - 9*a(n-4) + 2*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 27 2018 11:37:11, revision 7) the statement is
...
$V=\{0,1,\dots,1\}^{3}$, so $|V|=S=8$. A $2\times2$ subblock of the array
...
For every $n$ with $n+1-1\ge5$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n+1-1-5}\,q(M)\,\mathbf{1} .
\]
...
=\mathbf{1}^{\!\top}M^{\,m-5}\,q(M)\,\mathbf{1}
\]
with $m=n+1-1$, which is the stated identity; the equivalence follows by letting
$m-5=j$ run over $\ge0$. For the last clause put $w=q(M)\mathbf{1}$. By the
...
$S^2$ pairs. The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was then formed by $5$
...
every one of the $27$ published indices; the values agree exactly. That is what ties
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251269.
