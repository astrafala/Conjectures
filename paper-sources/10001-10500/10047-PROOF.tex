...
\title{The empirical recurrence for OEIS A195279, derived from the entry's generating function}
...
\begin{abstract}
OEIS A195279 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A195279 is ``Number of lower triangles of a 3 X 3 integer array with each element differing from all of its vertical and horizontal neighbors by n or less and triangles differing by a constant counted only once.''. It has offset $1$ and begins
\[
171, 2125, 11319, 39609, 107811, 248261, 507375, 948209,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 01 2018 05:35:00, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 7 x^{5} + 150 x^{4} + 1134 x^{3} + 1099 x^{2} + 171 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + 7 x^{5} + 150 x^{4} + 1134 x^{3} + 1099 x^{2} + 171 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195279.
