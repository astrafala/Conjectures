...
\title{The empirical recurrence for OEIS A270085, derived from the entry's generating function}
...
\begin{abstract}
OEIS A270085 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A270085 is ``Partial sums of the number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 65", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 5, 10, 46, 55, 151, 168, 356,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{10} - 8 x^{8} + 4 x^{7} + 4 x^{6} - 3 x^{4} + 24 x^{3} + 2 x^{2} + 4 x + 1}{x^{7} - x^{6} - 3 x^{5} + 3 x^{4} + 3 x^{3} - 3 x^{2} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;4 x^{10} - 8 x^{8} + 4 x^{7} + 4 x^{6} - 3 x^{4} + 24 x^{3} + 2 x^{2} + 4 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270085.
