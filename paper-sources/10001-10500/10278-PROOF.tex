...
\title{The empirical recurrence for OEIS A245158, derived from the entry's generating function}
...
\begin{abstract}
OEIS A245158 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A245158 is ``Number of length n 0..3 arrays with new values introduced in order from both ends.''. It has offset $1$ and begins
\[
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 03 2018 10:32:27, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{11 x^{6} - 31 x^{5} + 43 x^{4} - 29 x^{3} + 9 x^{2} - x}{16 x^{5} - 52 x^{4} + 64 x^{3} - 37 x^{2} + 10 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 16 x^{5} + 52 x^{4} - 64 x^{3} + 37 x^{2} - 10 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;10\,a(n-1) - 37\,a(n-2) + 64\,a(n-3) - 52\,a(n-4) + 16\,a(n-5)
\]
for every $n>6$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 11 x^{6} + 31 x^{5} - 43 x^{4} + 29 x^{3} - 9 x^{2} + x,
\]
...
\[
16 x^{5} - 52 x^{4} + 64 x^{3} - 37 x^{2} + 10 x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A245158.
