...
\title{The empirical recurrence for OEIS A251936, derived from the entry's generating function}
...
\begin{abstract}
OEIS A251936 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A251936 is ``Number of length 2+2 0..n arrays with the sum of the maximum minus the median of adjacent triples multiplied by some arrangement of +-1 equal to zero.''. It has offset $1$ and begins
\[
10, 43, 120, 265, 506, 875, 1408, 2145,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 01 2018 04:40:13, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 5 x^{4} - 5 x^{3} + 7 x^{2} - 10 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} - 5 x^{4} + 5 x^{3} - 7 x^{2} + 10 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251936.
