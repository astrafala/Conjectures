...
\title{The empirical recurrence for OEIS A183907, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183907 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A183907 is ``Number of nondecreasing arrangements of n+2 numbers in 0..4 with each number being the sum mod 5 of two others.''. It has offset $1$ and begins
\[
1, 2, 20, 66, 148, 275, 457, 705,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 05 2018 18:22:00, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{6} - 3 x^{5} + 24 x^{4} - 20 x^{3} + 3 x^{2} - x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;4 x^{6} + 3 x^{5} - 24 x^{4} + 20 x^{3} - 3 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183907.
