...
\title{The empirical recurrence for OEIS A209008, derived from the entry's generating function}
...
\begin{abstract}
OEIS A209008 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A209008 is ``Number of 4-bead necklaces labeled with numbers -n..n not allowing reversal, with sum zero and first and second differences in -n..n.''. It has offset $1$ and begins
\[
1, 3, 5, 10, 16, 26, 38, 55,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 07 2018 19:31:37, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 3 x^{4} - 2 x^{3} + x}{x^{5} - 3 x^{4} + 2 x^{3} + 2 x^{2} - 3 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A209008.
