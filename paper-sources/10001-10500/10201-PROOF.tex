...
\title{The empirical recurrence for OEIS A053697, derived from the entry's generating function}
...
\begin{abstract}
OEIS A053697 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A053697 is ``a(n+1)=a(n)+a\^{}(n), where the addition is in base 11 and where a\^{}(n) is obtained from a(n) by replacing each digit with its multiplicative inverse modulo 11. Zero digits, if any, are deleted.''. It has offset $1$ and begins
\[
1, 2, 8, 14, 27, 94, 137, 284,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 31 2017 22:27:16, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{10 x^{9} - 10 x^{7} + 10 x^{6} - x^{5} + 2 x^{4} + 7 x^{3} + 2 x^{2} + x}{10 x^{5} - 10 x^{3} - x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;10 x^{5} - 10 x^{3} - x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;10 x^{9} - 10 x^{7} + 10 x^{6} - x^{5} + 2 x^{4} + 7 x^{3} + 2 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A053697.
