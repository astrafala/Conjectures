...
\title{The empirical recurrence for OEIS A211562, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211562 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A211562 is ``Number of nonnegative integer arrays of length n+2 with new values 0 upwards introduced in order, and containing the value n-1.''. It has offset $1$ and begins
\[
5, 14, 36, 81, 162, 295, 499, 796,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 19 2018 05:50:26, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{5} + 9 x^{4} - 16 x^{3} + 11 x^{2} - 5 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{5} - 9 x^{4} + 16 x^{3} - 11 x^{2} + 5 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211562.
