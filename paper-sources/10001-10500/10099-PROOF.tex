...
\title{The empirical recurrence for OEIS A249845, derived from the entry's generating function}
...
\begin{abstract}
OEIS A249845 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A249845 is ``Number of length 1+4 0..n arrays with no five consecutive terms having the maximum of any two terms equal to the minimum of the remaining three terms.''. It has offset $1$ and begins
\[
10, 110, 560, 1920, 5170, 11830, 24080, 44880,\ \dots
\]
...
\begin{quote}
G.f.: 10*x*(1 + x)*(1 + 4*x + x\^{}2) / (1 - x)\^{}6.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 18 2017 11:15:42, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{10 x^{4} + 50 x^{3} + 50 x^{2} + 10 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;10 x^{4} + 50 x^{3} + 50 x^{2} + 10 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A249845.
