...
\title{The empirical recurrence for OEIS A063147, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063147 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A063147 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 79 ).''. It has offset $1$ and begins
\[
6, 19, 33, 45, 59, 73, 85, 99,\ \dots
\]
...
\begin{quote}
G.f.: x*(6 + 13*x + 14*x\^{}2 + 6*x\^{}3 + x\^{}4) / ((1 - x)\^{}2*(1 + x + x\^{}2)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + 6 x^{4} + 14 x^{3} + 13 x^{2} + 6 x}{x^{4} - x^{3} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} + 6 x^{4} + 14 x^{3} + 13 x^{2} + 6 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063147.
