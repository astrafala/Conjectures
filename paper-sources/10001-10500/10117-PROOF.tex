...
\title{The empirical recurrence for OEIS A266606, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266606 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A266606 is ``Binary representation of the n-th iteration of the "Rule 39" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 110, 10, 1111101, 100, 11111111011, 1000, 111111111110111,\ \dots
\]
...
\begin{quote}
G.f.: (1+110*x-10001*x\^{}2+9891*x\^{}3+10000*x\^{}4-20000*x\^{}5) / ((1-x)*(1+x)*(1-100*x)*(1+100*x)*(1-10*x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 30) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{20000 x^{5} - 10000 x^{4} - 9891 x^{3} + 10001 x^{2} - 110 x - 1}{100000 x^{6} - 110010 x^{4} + 10011 x^{2} - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 20000 x^{5} + 10000 x^{4} + 9891 x^{3} - 10001 x^{2} + 110 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266606.
