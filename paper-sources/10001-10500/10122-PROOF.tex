...
\title{The empirical recurrence for OEIS A302675, proved by transfer matrix}
...
\begin{abstract}
OEIS A302675 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
...
recurrence of order $2$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A302675 is ``Number of nX3 0..1 arrays with every element equal to 0, 1, 4 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
3, 3, 8, 7, 11, 18, 29, 47,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +a(n-2) for n>5
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 11 2018 12:24:58, revision
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-2}$ whenever $n-2\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-2$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-2}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $2$ matrix--vector
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302675.
