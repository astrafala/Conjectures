...
\title{The empirical recurrence for OEIS A243515, derived from the entry's generating function}
...
\begin{abstract}
OEIS A243515 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A243515 is ``Number of length n+2 0..6 arrays with no three elements in a row with pattern aba or abb (with a!=b) and new values 0..6 introduced in 0..6 order.''. It has offset $1$ and begins
\[
3, 5, 10, 25, 77, 279, 1134, 4979,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 02 2018 12:26:53, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 60 x^{5} + 141 x^{4} - 106 x^{3} + 31 x^{2} - 3 x}{30 x^{5} - 91 x^{4} + 102 x^{3} - 52 x^{2} + 12 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 30 x^{5} + 91 x^{4} - 102 x^{3} + 52 x^{2} - 12 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;12\,a(n-1) - 52\,a(n-2) + 102\,a(n-3) - 91\,a(n-4) + 30\,a(n-5)
\]
for every $n>5$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;60 x^{5} - 141 x^{4} + 106 x^{3} - 31 x^{2} + 3 x,
\]
...
\[
30 x^{5} - 91 x^{4} + 102 x^{3} - 52 x^{2} + 12 x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A243515.
