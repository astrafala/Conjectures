...
\title{The empirical recurrence for OEIS A184035, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184035 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A184035 is ``1/16 the number of (n+1) X 6 0..3 arrays with all 2 X 2 subblocks having the same four values.''. It has offset $1$ and begins
\[
169, 181, 202, 244, 322, 478, 778, 1378,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 05:56:47, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 652 x^{4} + 341 x^{3} + 326 x^{2} - 169 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;652 x^{4} - 341 x^{3} - 326 x^{2} + 169 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184035.
