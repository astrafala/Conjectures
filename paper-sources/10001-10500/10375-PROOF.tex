...
\title{The empirical recurrence for OEIS A253000, derived from the entry's generating function}
...
\begin{abstract}
OEIS A253000 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A253000 is ``Number of n X 4 nonnegative integer arrays with upper left 0 and lower right its king-move distance away minus 3 and every value within 3 of its king move distance from the upper left and every value increasing by 0 or 1 with every step right, diagonally se or down.''. It has offset $1$ and begins
\[
1, 1, 1, 1, 69, 1132, 7235, 25233,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 08 2018 05:28:56, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{183 x^{10} + 1154 x^{9} + 2810 x^{8} + 3118 x^{7} + 859 x^{6} + 68 x^{5} - x^{4} + 3 x^{3} - 3 x^{2} + x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;183 x^{10} + 1154 x^{9} + 2810 x^{8} + 3118 x^{7} + 859 x^{6} + 68 x^{5} - x^{4} + 3 x^{3} - 3 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253000.
