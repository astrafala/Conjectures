...
\title{The empirical recurrence for OEIS A201371, derived from the entry's generating function}
...
\begin{abstract}
OEIS A201371 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 22 2018 20:37:13, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{4} - 9 x^{3} + 5 x^{2} - 2 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
