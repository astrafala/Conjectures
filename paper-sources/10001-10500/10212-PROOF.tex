...
\title{The empirical recurrence for OEIS A126269, derived from the entry's generating function}
...
\begin{abstract}
OEIS A126269 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A126269 is ``Numbers k such that hcl(k,k) < hcl(k,k-1) where hcl(k,i) is the Huffman code length; see comments.''. It has offset $3$ and begins
\[
3, 4, 9, 10, 21, 22, 45, 46,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 13 2025 13:42:53, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{6} + 4 x^{4} + 3 x^{3}}{2 x^{4} - 3 x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{4} - 3 x^{2} + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A126269.
