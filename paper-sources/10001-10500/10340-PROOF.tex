...
\title{The empirical recurrence for OEIS A200431, derived from the entry's generating function}
...
\begin{abstract}
OEIS A200431 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 20 2018 14:18:17, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{12 x^{3} + 20 x^{2}}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
