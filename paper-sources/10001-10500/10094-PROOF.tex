...
\title{The empirical recurrence for OEIS A224671, derived from the entry's generating function}
...
\begin{abstract}
OEIS A224671 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A224671 is ``Number of (n+1) X 4 0..2 matrices with each 2 X 2 subblock idempotent.''. It has offset $1$ and begins
\[
41, 76, 108, 170, 257, 398, 617, 967,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 17 2018 05:32:30, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} - 41 x^{5} + 77 x^{4} + 9 x^{3} - 88 x^{2} + 41 x}{x^{5} - 2 x^{4} - x^{3} + 5 x^{2} - 4 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{5} - 2 x^{4} - x^{3} + 5 x^{2} - 4 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} - 41 x^{5} + 77 x^{4} + 9 x^{3} - 88 x^{2} + 41 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224671.
