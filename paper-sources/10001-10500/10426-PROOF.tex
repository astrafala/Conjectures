...
\title{The empirical recurrence for OEIS A267227, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267227 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267227 is ``Number of length-n 0..3 arrays with no following elements greater than or equal to the first repeated value.''. It has offset $1$ and begins
\[
4, 16, 54, 176, 564, 1790, 5646, 17732,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 06 2018 11:26:16, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{5} - 2 x^{4} + 26 x^{3} - 20 x^{2} + 4 x}{18 x^{4} - 39 x^{3} + 29 x^{2} - 9 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;18 x^{4} - 39 x^{3} + 29 x^{2} - 9 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 6 x^{5} - 2 x^{4} + 26 x^{3} - 20 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267227.
