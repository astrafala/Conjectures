...
\title{The empirical closed form for OEIS A219769, proved}
...
\begin{abstract}
OEIS A219769 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=135$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219769 is ``Number of n X 4 arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 n X 4 array.''. It has offset $1$ and begins
\[
4, 9, 33, 100, 283, 732, 1745, 3881,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/10080)*n\^{}8 - (1/120)*n\^{}6 + (73/240)*n\^{}5 - (487/160)*n\^{}4 + (1015/48)*n\^{}3 - (191389/2520)*n\^{}2 + (2351/20)*n - 5 for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 27 2018 08:56:40, revision 7)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=135$
...
states number $S=135$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219769 records is correct for all $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219769.
