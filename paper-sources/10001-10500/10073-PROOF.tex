...
\title{The empirical recurrence for OEIS A188785, derived from the entry's generating function}
...
\begin{abstract}
OEIS A188785 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 28 2018 12:49:07, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 16 x^{4} - 112 x^{3} - 64 x^{2}}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
