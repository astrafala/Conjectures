...
\title{The empirical recurrence for OEIS A269512, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269512 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A269512 is ``First differences of number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 342", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
4, 3, 16, -7, 36, -21, 64, -47,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} - x^{5} + 6 x^{4} + 2 x^{3} + 15 x^{2} + 7 x + 4}{x^{7} + x^{6} - x^{5} - x^{4} - x^{3} - x^{2} + x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} - x^{5} + 6 x^{4} + 2 x^{3} + 15 x^{2} + 7 x + 4,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269512.
