...
\title{The empirical recurrence for OEIS A063141, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063141 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A063141 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 73 ).''. It has offset $1$ and begins
\[
5, 18, 30, 42, 54, 68, 78, 92,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = a(n-2) + a(n-3) - a(n-5) for n>6.
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: x*(5 + 18*x + 25*x\^{}2 + 19*x\^{}3 + 6*x\^{}4 + x\^{}5) / ((1 - x)\^{}2*(1 + x)*(1 + x + x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 22 2019 12:18:12, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} + 6 x^{5} + 19 x^{4} + 25 x^{3} + 18 x^{2} + 5 x}{x^{5} - x^{3} - x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{5} - x^{3} - x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} + 6 x^{5} + 19 x^{4} + 25 x^{3} + 18 x^{2} + 5 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063141.
