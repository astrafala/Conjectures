...
\title{The empirical recurrence for OEIS A259714, derived from the entry's generating function}
...
\begin{abstract}
OEIS A259714 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A259714 is ``a(n) = Sum\_{k=1..n-1}((k mod 5)*a(n-k)), a(1) = 1.''. It has offset $1$ and begins
\[
1, 1, 3, 8, 21, 50, 129, 327,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 19 2015 13:45:34, revision 23) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} - x}{x^{5} + 4 x^{4} + 3 x^{3} + 2 x^{2} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{5} - 4 x^{4} - 3 x^{3} - 2 x^{2} - x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + 2\,a(n-2) + 3\,a(n-3) + 4\,a(n-4) + a(n-5)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A259714.
