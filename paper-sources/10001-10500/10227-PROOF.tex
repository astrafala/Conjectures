...
\title{The empirical recurrence for OEIS A267047, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267047 carries an empirical recurrence of order $5$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{6} - 2 x^{5} - 2 x^{4} - 2 x - 1}{x^{5} - x^{4} - 2 x^{3} + 2 x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 3 x^{6} + 2 x^{5} + 2 x^{4} + 2 x + 1,
\]
