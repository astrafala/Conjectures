...
\title{The empirical recurrence for OEIS A212346, derived from the entry's generating function}
...
\begin{abstract}
OEIS A212346 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A212346 is ``Sequence of coefficients of x\^{}0 in marked mesh pattern generating function Q\_{n,132}\^{}(0,4,0,0)(x).''. It has offset $0$ and begins
\[
1, 1, 2, 5, 14, 28, 48, 75,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:22, revision 34) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{6} - 5 x^{5} + 3 x^{4} - x^{3} + 4 x^{2} - 3 x + 1}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{6} - 5 x^{5} + 3 x^{4} - x^{3} + 4 x^{2} - 3 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A212346.
