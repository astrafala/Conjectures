...
\title{The empirical recurrence for OEIS A227265, derived from the entry's generating function}
...
\begin{abstract}
OEIS A227265 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 07 2018 16:51:51, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} - 6 x^{3} + 7 x^{2} - 3 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
