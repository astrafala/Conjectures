...
\title{The empirical recurrence for OEIS A252239, proved by transfer matrix}
...
\begin{abstract}
OEIS A252239 counts $(n+2)\times6$ arrays over $\{0,\dots,3\}$ subject to a condition imposed on
every $3\times3$ subblock, and it carries an empirical recurrence of order $6$
...
$a(n)$ is a walk count on $S=44889$ vertices. Such a sequence is $C$-finite, so whether a given
...
OEIS A252239 is ``Number of (n+2) X (4+2) 0..3 arrays with every 3 X 3 subblock row and column sum not equal to 1 3 4 6 or 7 and every 3 X 3 diagonal and antidiagonal sum equal to 1 3 4 6 or 7.''. It has offset $1$ and begins
\[
4708, 43848, 331884, 1179792, 11206944, 84935088, 301990464, 2868905088,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 260*a(n-3) - 1024*a(n-6) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 20 2018 10:16:55, revision 7) this is still
...
\[
\text{every row, column sum} \notin \{1,3,4,6,7\}\ \text{ and }\ \text{every diagonal, antidiagonal sum} \in \{1,3,4,6,7\}
\]
...
\begin{theorem}
\[
a(n)\;=\;260\,a(n-3) - 1024\,a(n-6)
\]
for every $n>7$.
\end{theorem}

\begin{proof}
The vector $w=q(M)\mathbf{1}$ was formed by $6$ matrix--vector products in exact
integer arithmetic and $\mathbf{1}^{\!\top}M^{j}w$ evaluated for $j=0,1,\dots$, again
exactly. Those integers vanish from the index corresponding to $n=7+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>7$. The bound is exact: at $n=7$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252239.
