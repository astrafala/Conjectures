...
\title{The empirical recurrence for OEIS A063111, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063111 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A063111 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 43 ).''. It has offset $1$ and begins
\[
3, 10, 18, 24, 32, 40, 46, 54,\ \dots
\]
...
\begin{quote}
G.f.: x*(3 + 7*x + 8*x\^{}2 + 3*x\^{}3 + x\^{}4) / ((1 - x)\^{}2*(1 + x + x\^{}2)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + 3 x^{4} + 8 x^{3} + 7 x^{2} + 3 x}{x^{4} - x^{3} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} + 3 x^{4} + 8 x^{3} + 7 x^{2} + 3 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063111.
