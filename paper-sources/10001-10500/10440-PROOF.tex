...
\title{The empirical recurrence for OEIS A301791, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A301791 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A301791 is ``Number of 2Xn 0..1 arrays with every element equal to 0 or 1 horizontally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
2, 5, 6, 9, 14, 22, 35, 56,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) -a(n-3) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 03 2018 04:16:39, revision 9) the statement is
...
\#\{\text{neighbours with a value }equal to\ x_{t,u}\}\in\{0,1\}
\end{equation}
...
publishes, $n=37$, the arrays have $2\cdot37$ cells over
...
The threshold below is exact rather than merely sufficient: at $n=5$ the conjectured
...
Every one of those integers vanishes from the index corresponding to $n=5+1$ onwards,
...
every one of the $37$ published indices, with no shift fitted: the number of columns is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A301791.
