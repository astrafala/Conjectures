...
\title{The empirical recurrence for OEIS A267003, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267003 carries an empirical recurrence of order $6$. The entry separately records a
...
\begin{quote}
G.f.: (1+2*x)*(1+3*x-23*x\^{}2+32*x\^{}3-48*x\^{}4) / ((1-x)*(1+x)*(1-4*x)*(1+4*x)*(1-8*x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 19 2025 10:02:57, revision 36) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{96 x^{5} - 16 x^{4} + 14 x^{3} + 17 x^{2} - 5 x - 1}{128 x^{6} - 152 x^{4} + 25 x^{2} - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 96 x^{5} + 16 x^{4} - 14 x^{3} - 17 x^{2} + 5 x + 1,
\]
