...
\title{The empirical recurrence for OEIS A227252, derived from the entry's generating function}
...
\begin{abstract}
OEIS A227252 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A227252 is ``Number of n X 2 binary arrays indicating whether each 2 X 2 subblock of a larger binary array has lexicographically nondecreasing rows and columns, for some larger (n+1) X 3 binary array with rows and columns of the latter in lexicographically nondecreasing order.''. It has offset $1$ and begins
\[
2, 3, 9, 23, 53, 113, 225, 421,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 07 2018 14:21:37, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{7} - 7 x^{6} + 20 x^{5} - 26 x^{4} + 21 x^{3} - 9 x^{2} + 2 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{7} - 7 x^{6} + 20 x^{5} - 26 x^{4} + 21 x^{3} - 9 x^{2} + 2 x,
\]
...
compared against all $38$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A227252.
