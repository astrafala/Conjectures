...
\title{The empirical recurrence for OEIS A199705, derived from the entry's generating function}
...
\begin{abstract}
OEIS A199705 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 16 2018 06:43:15, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{3} - 2 x^{2} - 6 x}{x^{4} - 2 x^{3} + 2 x - 1},
\]
