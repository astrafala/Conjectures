...
\title{The empirical recurrence for OEIS A178299, derived from the entry's generating function}
...
\begin{abstract}
OEIS A178299 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A178299 is ``Number of collinear point 4-tuples in a 6 X 6 X 6 X... n-dimensional cubic grid.''. It has offset $0$ and begins
\[
0, 15, 234, 2820, 31176, 333840, 3546144, 37807680,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = 36*a(n-1) - 476*a(n-2) + 2736*a(n-3) - 5760*a(n-4).
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: 3*x*(5-102*x+512*x\^{}2)/((12*x-1)*(6*x-1)*(8*x-1)*(10*x-1)). (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 17 2025 23:12:12, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{1536 x^{3} - 306 x^{2} + 15 x}{5760 x^{4} - 2736 x^{3} + 476 x^{2} - 36 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;5760 x^{4} - 2736 x^{3} + 476 x^{2} - 36 x + 1
\]
...
\[
5760 x^{4} - 2736 x^{3} + 476 x^{2} - 36 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A178299.
