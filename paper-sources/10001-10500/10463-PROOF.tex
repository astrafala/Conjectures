...
\title{A proof of the conjectured recurrence for OEIS A257290}
...
\begin{abstract}
OEIS A257290 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $9 x^{2} - 9 x + 2$, a polynomial of degree $2$, which proves
...
OEIS A257290 is ``Number of 3-Motzkin paths of length n with no level steps at even level''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 1, 3, 11, 39, 140, 504,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 3 x - \sqrt{\left(3 x - 1\right) \left(4 x^{2} + 3 x - 1\right)} + 1}{2 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +3*(-2*n-1)*a(n-1) +5*(n-1)*a(n-2) +6*(2*n-5)*a(n-3)=0. - \emph{R. J. Mathar}, Sep 24 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-02-14, revision 27) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 3 \left(2 n + 1\right),\qquad p_{2}(n)=5 \left(n - 1\right),\qquad p_{3}(n)=6 \left(2 n - 5\right)
\]
...
\section{The computation for A257290}
...
\[
(n + 2)\,a(n) -  3 \left(2 n + 1\right)\,a(n-1) + 5 \left(n - 1\right)\,a(n-2) + 6 \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A257290, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=9 x^{2} - 9 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A257290.
