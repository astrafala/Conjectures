...
\title{The empirical recurrence for OEIS A250362, derived from the entry's generating function}
...
\begin{abstract}
OEIS A250362 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A250362 is ``Number of length 4 arrays x(i), i=1..4 with x(i) in i..i+n and no value appearing more than 3 times.''. It has offset $1$ and begins
\[
16, 81, 255, 623, 1293, 2397, 4091, 6555,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 13 2018 08:38:08, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 2 x^{5} + 2 x^{4} - 10 x^{3} - x^{2} - 16 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} - 2 x^{5} - 2 x^{4} + 10 x^{3} + x^{2} + 16 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250362.
