...
\title{The empirical recurrence for OEIS A250337, derived from the entry's generating function}
...
\begin{abstract}
OEIS A250337 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A250337 is ``Number of length 1+5 0..n arrays with every six consecutive terms having the maximum of some three terms equal to the minimum of the remaining three terms.''. It has offset $1$ and begins
\[
44, 429, 2056, 6785, 17796, 39949, 80144, 147681,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 12 2018 11:13:11, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 6 x^{5} + 4 x^{4} + 142 x^{3} + 165 x^{2} + 44 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + 6 x^{5} + 4 x^{4} + 142 x^{3} + 165 x^{2} + 44 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250337.
