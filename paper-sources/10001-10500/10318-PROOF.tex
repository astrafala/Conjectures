...
\title{The empirical recurrence for OEIS A178351, derived from the entry's generating function}
...
\begin{abstract}
OEIS A178351 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A178351 is ``Number of collinear point 5-tuples in a 7 X 7 X 7 X ... n-dimensional cubic grid.''. It has offset $0$ and begins
\[
0, 21, 364, 4833, 58360, 676941, 7731364, 88086873,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 23 2025 08:10:41, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2663 x^{3} - 476 x^{2} + 21 x}{9009 x^{4} - 3800 x^{3} + 590 x^{2} - 40 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;9009 x^{4} - 3800 x^{3} + 590 x^{2} - 40 x + 1
\]
...
\[
9009 x^{4} - 3800 x^{3} + 590 x^{2} - 40 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A178351.
