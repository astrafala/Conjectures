...
\title{The empirical recurrence for OEIS A204215, derived from the entry's generating function}
...
\begin{abstract}
OEIS A204215 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A204215 is ``Number of length 7 nonnegative integer arrays starting and ending with 0 with adjacent elements differing by no more than n.''. It has offset $1$ and begins
\[
51, 473, 2208, 7167, 18583, 41363, 82440, 151125,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 06 2018 12:02:59, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 6 x^{5} - 6 x^{4} + 135 x^{3} + 167 x^{2} + 51 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + 6 x^{5} - 6 x^{4} + 135 x^{3} + 167 x^{2} + 51 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204215.
