...
\title{The empirical recurrence for OEIS A154148, derived from the entry's generating function}
...
\begin{abstract}
OEIS A154148 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A154148 is ``Numbers k such that 21 plus the k-th triangular number is a perfect square.''. It has offset $1$ and begins
\[
5, 7, 40, 50, 237, 295, 1384, 1722,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 23 2024 14:53:42, revision 20) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{6 x^{5} + 2 x^{4} - 3 x^{3} - 2 x^{2} - 5 x}{x^{5} - x^{4} - 6 x^{3} + 6 x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 6 x^{5} - 2 x^{4} + 3 x^{3} + 2 x^{2} + 5 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A154148.
