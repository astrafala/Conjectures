...
\title{The empirical recurrence for OEIS A263613, derived from the entry's generating function}
...
\begin{abstract}
OEIS A263613 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A263613 is ``Palindromic numbers in base 4 that are cubes.''. It has offset $1$ and begins
\[
0, 1, 1331, 1030301, 1003003001, 1000300030001, 1000030000300001, 1000003000003000001,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 04 2024 15:31:14, revision 36) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 7000000 x^{6} + 6446000 x^{5} - 336330 x^{4} + 220 x^{3} + x^{2}}{1000000 x^{4} - 1111000 x^{3} + 112110 x^{2} - 1111 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1000000 x^{4} - 1111000 x^{3} + 112110 x^{2} - 1111 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 7000000 x^{6} + 6446000 x^{5} - 336330 x^{4} + 220 x^{3} + x^{2},
\]
...
\[
1000000 x^{4} - 1111000 x^{3} + 112110 x^{2} - 1111 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263613.
