...
\title{The empirical recurrence for OEIS A266595, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266595 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A266595 is ``Number of OFF (white) cells in the n-th iteration of the "Rule 37" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 2, 2, 5, 6, 5, 10, 5,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{5} + 2 x^{4} + x^{3} + 2 x^{2} + 2 x}{x^{4} - 2 x^{2} + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266595.
