...
\title{The empirical recurrence for OEIS A190038, derived from the entry's generating function}
...
\begin{abstract}
OEIS A190038 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A190038 is ``Number of nondecreasing arrangements of n+2 numbers in 0..6 with the last equal to 6 and each after the second equal to the sum of one or two of the preceding three.''. It has offset $1$ and begins
\[
10, 18, 30, 47, 72, 107, 151, 203,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 04 2018 05:18:35, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{8} + x^{7} - 2 x^{6} - 3 x^{5} - x^{4} - 6 x^{3} + 12 x^{2} - 10 x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{8} - x^{7} + 2 x^{6} + 3 x^{5} + x^{4} + 6 x^{3} - 12 x^{2} + 10 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A190038.
