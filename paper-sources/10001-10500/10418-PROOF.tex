...
\title{The empirical recurrence for OEIS A266981, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266981 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A266981 is ``Number of ON (black) cells in the n-th iteration of the "Rule 79" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 2, 2, 5, 3, 8, 4, 11,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 12 2025 08:08:48, revision 33) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} + 2 x + 1}{x^{4} - 2 x^{2} + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266981.
