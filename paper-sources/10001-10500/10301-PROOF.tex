...
\title{The empirical recurrence for OEIS A266221, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266221 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A266221 is ``Total number of ON (black) cells after n iterations of the "Rule 7" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 3, 3, 10, 10, 21, 21, 36,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} - x^{4} - 3 x^{3} + 2 x^{2} - 2 x - 1}{x^{5} - x^{4} - 2 x^{3} + 2 x^{2} + x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266221.
