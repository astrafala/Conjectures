...
\title{The empirical closed form for OEIS A220005, proved}
...
\begin{abstract}
OEIS A220005 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=281$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220005 is ``Number of nX3 arrays of the minimum value of corresponding elements and their horizontal, vertical or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 nX3 array.''. It has offset $1$ and begins
\[
10, 19, 109, 560, 2410, 9094, 32575, 114987,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:27:34, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=281$
...
states number $S=281$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220005 records is correct for all $n>10$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220005.
