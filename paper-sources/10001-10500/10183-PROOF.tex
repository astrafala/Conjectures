...
\title{The empirical recurrence for OEIS A270082, derived from the entry's generating function}
...
\begin{abstract}
OEIS A270082 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A270082 is ``First differences of number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 62", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
4, 7, 8, 12, 12, 24, 4, 28,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:31, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{11} + 4 x^{9} + 4 x^{8} - x^{7} - 12 x^{6} + 5 x^{5} + 5 x^{3} + 4 x^{2} + 7 x + 4}{x^{6} - x^{4} - x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 4 x^{11} + 4 x^{9} + 4 x^{8} - x^{7} - 12 x^{6} + 5 x^{5} + 5 x^{3} + 4 x^{2} + 7 x + 4,
\]
...
compared against all $64$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270082.
