...
\title{The empirical recurrence for OEIS A239367, derived from the entry's generating function}
...
\begin{abstract}
OEIS A239367 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A239367 is ``The bisection of A238315 that remains constant with changes in the offset of the exponent of the second term.''. It has offset $1$ and begins
\[
1, 5, 13, 29, 69, 197, 669, 2509,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 30 2014 13:11:50, revision 23) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 7 x^{3} - 2 x^{2} + x}{4 x^{4} - 13 x^{3} + 15 x^{2} - 7 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{5} - 9 x^{4} + 2 x^{3} + 8 x^{2} - 6 x + 1
\]
...
So the conjecture is not minimal: its polynomial $D(x)$ factors as the reduced denominator times $x + 1$, and the sequence already satisfies a recurrence of order 4. The conjecture is true, but it is a consequence of a shorter one the entry does not state.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A239367.
