...
\title{The empirical recurrence for OEIS A269909, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269909 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A269909 is ``First differences of number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 1", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
3, -3, 43, -43, 115, -115, 219, -219,\ \dots
\]
...
\begin{quote}
G.f.: (3+34*x\^{}2-5*x\^{}4) / ((1-x)\^{}2*(1+x)\^{}3).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 5 x^{4} + 34 x^{2} + 3}{x^{5} + x^{4} - 2 x^{3} - 2 x^{2} + x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269909.
