...
\title{A proof of the conjectured recurrence for OEIS A212696}
...
\begin{abstract}
OEIS A212696 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A212696 is ``Central coefficient of the triangle A097609''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 3, 4, 25, 66, 287, 960,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{2} \left(3 \sqrt{1 - 4 x} - 1\right) + 2 \sqrt{6 x + \sqrt{1 - 4 x} + 1}}{4 \sqrt{1 - 4 x} \sqrt{6 x + \sqrt{1 - 4 x} + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*n*(n-1)*(2*n+1)*(5*n-8)*a(n) -(n-1)*(115*n\^{}3-344*n\^{}2+299*n-82) *a(n-1) -4*(2*n-3)*(5*n\^{}3+27*n\^{}2-74*n+30)*a(n-2) +36*(n-1)*(5*n-3)*(2*n-3)*(2*n-5) *a(n-3)=0. - \emph{R. J. Mathar}, Oct 08 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2024-07-16, revision 44) the
...
\[
p_{0}(n)=2 n \left(n - 1\right) \left(2 n + 1\right) \left(5 n - 8\right),\qquad p_{1}(n)=- \left(n - 1\right) \left(115 n^{3} - 344 n^{2} + 299 n - 82\right),\qquad p_{2}(n)=- 4 \left(2 n - 3\right) \left(5 n^{3} + 27 n^{2} - 74 n + 30\right),\qquad p_{3}(n)=36 \left(n - 1\right) \left(2 n - 5\right) \left(2 n - 3\right) \left(5 n - 3\right)
\]
...
\section{The computation for A212696}

The generating function \eqref{eq:gf} is algebraic over $\mathbb{Q}(x)$;
its minimal polynomial is
\[
P(x,y) \;=\; 144 x^{4} y^{4} - 8 x^{3} y^{4} - 36 x^{3} y^{2} - 23 x^{2} y^{4} + 17 x^{2} y^{2} + 9 x^{2} + 4 x y^{4} + 2 x y^{2} - 4 x y - 2 x - y^{2} + y ,
\]
...
\[
2 n \left(n - 1\right) \left(2 n + 1\right) \left(5 n - 8\right)\,a(n) -  \left(n - 1\right) \left(115 n^{3} - 344 n^{2} + 299 n - 82\right)\,a(n-1) -  4 \left(2 n - 3\right) \left(5 n^{3} + 27 n^{2} - 74 n + 30\right)\,a(n-2) + 36 \left(n - 1\right) \left(2 n - 5\right) \left(2 n - 3\right) \left(5 n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A212696, agreeing on all 13 terms compared.
...
vanish, for all 25 values of $n$ from $n=3$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A212696.
