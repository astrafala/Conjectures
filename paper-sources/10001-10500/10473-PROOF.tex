...
\title{The empirical recurrence for OEIS A268622, derived from the entry's generating function}
...
\begin{abstract}
OEIS A268622 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 21 2018 09:46:40, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{3} - 12 x^{2} + 9 x}{9 x^{2} - 6 x + 1},
\]
