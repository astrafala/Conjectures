...
\title{The empirical recurrence for OEIS A199899, derived from the entry's generating function}
...
\begin{abstract}
OEIS A199899 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 23 2018 07:28:24, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} + 5 x^{3} - 11 x^{2} + 15 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
