...
\title{The empirical recurrence for OEIS A267187, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267187 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A267187 is ``Column 4 of the square array in A267181.''. It has offset $0$ and begins
\[
0, 3, 1, 4, 0, 6, 4, 7,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 29 2016 10:51:36, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{9} - x^{8} + 3 x^{5} - 4 x^{4} + 3 x^{3} - 2 x^{2} + 3 x}{x^{5} - x^{4} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{9} - x^{8} + 3 x^{5} - 4 x^{4} + 3 x^{3} - 2 x^{2} + 3 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267187.
