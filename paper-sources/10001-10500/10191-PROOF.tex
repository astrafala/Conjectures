...
\title{The empirical recurrence for OEIS A020881, derived from the entry's generating function}
...
\begin{abstract}
OEIS A020881 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A020881 is ``Number of strong restricted edge-subgraphs in Moebius ladder M\_n.''. It has offset $2$ and begins
\[
18, 102, 418, 2006, 8946, 41222, 187202, 855606,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 24 x^{5} + 16 x^{4} + 66 x^{3} + 18 x^{2}}{4 x^{4} - 4 x^{3} - 11 x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{4} - 4 x^{3} - 11 x^{2} - 2 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 11\,a(n-2) + 4\,a(n-3) - 4\,a(n-4)
\]
for every $n>5$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 24 x^{5} + 16 x^{4} + 66 x^{3} + 18 x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A020881.
