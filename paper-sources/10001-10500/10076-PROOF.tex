...
\title{The empirical recurrence for OEIS A197201, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A197201 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A197201 is ``Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 4,2,1,0,3 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
1, 5, 16, 47, 154, 496, 1601, 5183,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) + 4*a(n-2) + 2*a(n-3) - 4*a(n-4) - 8*a(n-5) + a(n-7) + 2*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 01 2018 14:31:16, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 4\,a(n-2) + 2\,a(n-3) - 4\,a(n-4) - 8\,a(n-5) + a(n-7) + 2\,a(n-8)
\end{equation}
for all $n>9$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(4,2,1,0,3)
\end{equation}
for every cell $(t,u)$.
...
publishes, $n=26$, the arrays have $2\cdot26$ cells over
$5$ values, so there are about $10^{36}$ of them to sift. The
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A197201.
