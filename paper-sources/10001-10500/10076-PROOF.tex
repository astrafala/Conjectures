...
\title{The empirical recurrence for OEIS A266788, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266788 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A266788 is ``Decimal representation of the n-th iteration of the "Rule 61" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 3, 12, 87, 120, 1679, 976, 29823,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 29) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 256 x^{10} + 768 x^{9} - 112 x^{8} - 688 x^{7} + 328 x^{6} + 40 x^{5} + 28 x^{4} - 12 x^{3} + 13 x^{2} - 3 x - 1}{128 x^{6} - 152 x^{4} + 25 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 128 x^{6} + 152 x^{4} - 25 x^{2} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;25\,a(n-2) - 152\,a(n-4) + 128\,a(n-6)
\]
for every $n>10$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;256 x^{10} - 768 x^{9} + 112 x^{8} + 688 x^{7} - 328 x^{6} - 40 x^{5} - 28 x^{4} + 12 x^{3} - 13 x^{2} + 3 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266788.
