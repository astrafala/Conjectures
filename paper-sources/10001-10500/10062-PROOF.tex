...
\title{The empirical recurrence for OEIS A154143, derived from the entry's generating function}
...
\begin{abstract}
OEIS A154143 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 02 2026 16:38:00, revision 29) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{5} + 2 x^{4} - 3 x^{3} - 2 x^{2} - 3 x}{x^{5} - x^{4} - 6 x^{3} + 6 x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 4 x^{5} - 2 x^{4} + 3 x^{3} + 2 x^{2} + 3 x,
\]
