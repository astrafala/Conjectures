...
\title{The empirical recurrence for OEIS A261954, derived from the entry's generating function}
...
\begin{abstract}
OEIS A261954 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A261954 is ``Start with a single equilateral triangle for n=0; for the odd n-th generation add a triangle at each expandable side of the triangles of the (n-1)-th generation (this is the "side to side" version); for the even n-th generation use the "side to vertex" version; a(n) is the number of triangles added in the n-th generation.''. It has offset $0$ and begins
\[
1, 3, 3, 6, 12, 15, 21, 18,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 13 2017 17:38:04, revision 23) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{7 x^{6} + 6 x^{5} + 8 x^{4} + 3 x^{3} + 2 x^{2} + 3 x + 1}{x^{6} - x^{4} - x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;7 x^{6} + 6 x^{5} + 8 x^{4} + 3 x^{3} + 2 x^{2} + 3 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A261954.
