...
\title{The empirical recurrence for OEIS A199531, derived from the entry's generating function}
...
\begin{abstract}
OEIS A199531 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 15 2018 20:43:35, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{3} + 24 x^{2} + 12 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
