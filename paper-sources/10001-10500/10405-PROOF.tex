...
\title{The empirical recurrence for OEIS A054149, derived from the entry's generating function}
...
\begin{abstract}
OEIS A054149 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A054149 is ``T(2n-1,n), array T as in A054144.''. It has offset $1$ and begins
\[
1, 14, 84, 410, 1820, 7648, 31008, 122584,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 11 2015 12:12:45, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{4} - 8 x^{3} + 6 x^{2} + x}{4 x^{4} - 16 x^{3} + 20 x^{2} - 8 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{4} - 16 x^{3} + 20 x^{2} - 8 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A054149.
