...
\title{The empirical recurrence for OEIS A270009, derived from the entry's generating function}
...
\begin{abstract}
OEIS A270009 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A270009 is ``First differences of number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 5", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
7, -8, 49, -49, 121, -121, 225, -225,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} - x^{5} - 3 x^{4} + 2 x^{3} + 27 x^{2} - x + 7}{x^{5} + x^{4} - 2 x^{3} - 2 x^{2} + x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{5} + x^{4} - 2 x^{3} - 2 x^{2} + x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} - x^{5} - 3 x^{4} + 2 x^{3} + 27 x^{2} - x + 7,
\]
...
compared against all $43$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270009.
