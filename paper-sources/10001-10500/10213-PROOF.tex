...
\title{The empirical recurrence for OEIS A128424, derived from the entry's generating function}
...
\begin{abstract}
OEIS A128424 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A128424 is ``a(n) = floor(sqrt(a(n-1)\^{}2 + a(n-2)\^{}2 + a(n-1)*a(n-2))), a(1)=1, a(2)=3.''. It has offset $1$ and begins
\[
1, 3, 3, 5, 7, 10, 14, 20,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 03 2018 08:43:40, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{7} + x^{6} - 2 x^{5} + x^{4} - 2 x^{3} + x^{2} + x}{x^{4} - x^{3} + x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{4} - x^{3} + x^{2} - 2 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{7} + x^{6} - 2 x^{5} + x^{4} - 2 x^{3} + x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A128424.
