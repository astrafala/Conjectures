...
\title{The empirical recurrence for OEIS A266725, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266725 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A266725 is ``Total number of OFF (white) cells after n iterations of the "Rule 59" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 1, 4, 5, 12, 13, 24, 25,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 11 2025 05:28:11, revision 27) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} + x^{3} - 3 x^{2} - x}{x^{5} - x^{4} - 2 x^{3} + 2 x^{2} + x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266725.
