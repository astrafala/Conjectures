...
\title{The empirical recurrence for OEIS A266616, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266616 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A266616 is ``Number of OFF (white) cells in the n-th iteration of the "Rule 41" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 3, 3, 4, 8, 3, 11, 4,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{5} + 5 x^{4} + x^{3} + 3 x^{2} + 3 x}{x^{6} - x^{4} - x^{2} + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266616.
