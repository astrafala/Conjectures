...
\title{The empirical recurrence for OEIS A190026, proved by transfer matrix}
...
\begin{abstract}
OEIS A190026 counts the $(n+2)\times4$ arrays over $\{0,\dots,1\}$ in which
...
a state, the count is a walk count on $S=112$ of them, and the conjectured recurrence is then
...
OEIS A190026 is ``Number of (n+2) X 4 binary arrays with every 3 X 3 subblock commuting with each horizontal and vertical neighbor 3 X 3 subblock.''. It has offset $1$ and begins
\[
112, 45, 79, 155, 235, 295, 355, 423,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) -a(n-2) +a(n-3) -2*a(n-4) +a(n-5) +a(n-6) -a(n-7) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (May 03 2018 07:02:18, revision 8) this is still
...
$112$, which the model reproduces along with every later term.
...
\]
on the $S=112$ admissible $3$-tuples. In particular $a$ is $C$-finite.
...
size of the construction is therefore $2^{3\cdot4}$ before the condition prunes
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A190026.
