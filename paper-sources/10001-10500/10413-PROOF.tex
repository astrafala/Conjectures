...
\title{The empirical recurrence for OEIS A266724, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266724 carries an empirical recurrence of order $4$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} - x^{3} + 3 x^{2} + x}{x^{4} - 2 x^{2} + 1},
\]
