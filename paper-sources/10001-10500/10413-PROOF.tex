...
\title{The empirical recurrence for OEIS A063135, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063135 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A063135 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 67 ).''. It has offset $1$ and begins
\[
5, 16, 28, 38, 50, 62, 72, 84,\ \dots
\]
...
\begin{quote}
G.f.: x*(5 + 11*x + 12*x\^{}2 + 5*x\^{}3 + x\^{}4) / ((1 - x)\^{}2*(1 + x + x\^{}2)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + 5 x^{4} + 12 x^{3} + 11 x^{2} + 5 x}{x^{4} - x^{3} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} + 5 x^{4} + 12 x^{3} + 11 x^{2} + 5 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063135.
