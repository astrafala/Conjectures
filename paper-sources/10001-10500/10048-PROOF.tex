...
\title{The empirical recurrence for OEIS A245872, derived from the entry's generating function}
...
\begin{abstract}
OEIS A245872 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A245872 is ``Number of length 3+2 0..n arrays with some pair in every consecutive three terms totalling exactly n.''. It has offset $1$ and begins
\[
16, 103, 256, 549, 960, 1579, 2368, 3433,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2018 03:05:12, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 2 x^{5} - 2 x^{4} + 34 x^{3} + 71 x^{2} + 16 x}{x^{6} - 2 x^{5} - x^{4} + 4 x^{3} - x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + 2 x^{5} - 2 x^{4} + 34 x^{3} + 71 x^{2} + 16 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A245872.
