...
\title{The empirical recurrence for OEIS A235951, proved by transfer matrix}
...
\begin{abstract}
OEIS A235951 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=7353$
...
OEIS A235951 is ``Number of (n+1) X (5+1) 0..2 arrays colored with the upper median value of each 2 X 2 subblock.''. It has offset $1$ and begins
\[
7353, 5034, 2478, 3496, 5358, 6728, 9260, 14016,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-4) for n>13.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 20 2018 10:03:45, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The $2\times2$ subblocks of an $(n+1)\times6$ array form an
$n\times5$ grid. Colour each of them by $\sigma=c$, where $a\le b\le c\le d$ are its four entries sorted --- its minimum, lower median, upper median and maximum. ``Coloured with'' is the entry's way of
...
\]
over the $S=7353$ pairs whose own subblock row is properly coloured.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235951.
