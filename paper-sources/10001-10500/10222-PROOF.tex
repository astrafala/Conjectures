...
\title{The empirical recurrence for OEIS A183906, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183906 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 05 2018 20:38:32, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{6} - x^{5} - 8 x^{4} + 9 x^{3} - 3 x^{2} + 2 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{6} - x^{5} - 8 x^{4} + 9 x^{3} - 3 x^{2} + 2 x,
\]
