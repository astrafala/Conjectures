...
\title{The empirical recurrence for OEIS A210376, derived from the entry's generating function}
...
\begin{abstract}
OEIS A210376 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A210376 is ``Number of 2 X 2 matrices with all terms in {0,1,...,n} and (sum of terms) = n + 4.''. It has offset $0$ and begins
\[
0, 0, 10, 40, 85, 140, 206, 284,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 16 2024 13:16:36, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{6 x^{6} - 15 x^{4} + 10 x^{2}}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A210376.
