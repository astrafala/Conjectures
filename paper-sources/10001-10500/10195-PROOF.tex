...
\title{The empirical recurrence for OEIS A063087, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063087 carries an empirical recurrence of order $4$. The entry separately records a
...
\begin{quote}
G.f.: x*(1 + 3*x + 4*x\^{}2 + x\^{}3 + x\^{}4) / ((1 - x)\^{}2*(1 + x + x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 23 2025 00:28:44, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + x^{4} + 4 x^{3} + 3 x^{2} + x}{x^{4} - x^{3} - x + 1},
\]
