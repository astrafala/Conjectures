...
\title{The empirical recurrence for OEIS A269913, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269913 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A269913 is ``First differences of number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 3", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
4, -4, 44, -44, 116, -116, 220, -220,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{4} + 32 x^{2} + 4}{x^{5} + x^{4} - 2 x^{3} - 2 x^{2} + x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269913.
