...
\title{The empirical recurrence for OEIS A227637, derived from the entry's generating function}
...
\begin{abstract}
OEIS A227637 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A227637 is ``Number of n X 2 0,1 arrays indicating 2 X 2 subblocks of some larger (n+1) X 3 binary array having determinant equal to one, with rows and columns of the latter in nondecreasing lexicographic order.''. It has offset $1$ and begins
\[
2, 5, 11, 23, 44, 78, 130, 206,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 09 2018 09:23:32, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + x^{5} + 2 x^{4} - 6 x^{3} + 5 x^{2} - 2 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} - x^{5} - 2 x^{4} + 6 x^{3} - 5 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A227637.
