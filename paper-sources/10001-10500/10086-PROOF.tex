...
\title{The empirical recurrence for OEIS A267229, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267229 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A267229 is ``Number of length-n 0..5 arrays with no following elements greater than or equal to the first repeated value.''. It has offset $1$ and begins
\[
6, 36, 195, 1030, 5375, 27854, 143695, 738990,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 11 2025 18:16:34, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 120 x^{7} - 172 x^{6} + 969 x^{5} - 1010 x^{4} + 435 x^{3} - 84 x^{2} + 6 x}{600 x^{6} - 1490 x^{5} + 1399 x^{4} - 650 x^{3} + 160 x^{2} - 20 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;600 x^{6} - 1490 x^{5} + 1399 x^{4} - 650 x^{3} + 160 x^{2} - 20 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;20\,a(n-1) - 160\,a(n-2) + 650\,a(n-3) - 1399\,a(n-4) + 1490\,a(n-5) - 600\,a(n-6)
\]
for every $n>7$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 120 x^{7} - 172 x^{6} + 969 x^{5} - 1010 x^{4} + 435 x^{3} - 84 x^{2} + 6 x,
\]
...
\[
600 x^{6} - 1490 x^{5} + 1399 x^{4} - 650 x^{3} + 160 x^{2} - 20 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267229.
