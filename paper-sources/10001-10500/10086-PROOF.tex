...
\title{The empirical recurrence for OEIS A200887, derived from the entry's generating function}
...
\begin{abstract}
OEIS A200887 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A200887 is ``Number of 0..n arrays x(0..3) of 4 elements without any interior element greater than both neighbors.''. It has offset $1$ and begins
\[
12, 51, 144, 325, 636, 1127, 1856, 2889,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 16 2017 09:57:36, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 5 x^{4} - 9 x^{3} + 9 x^{2} - 12 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} - 5 x^{4} + 9 x^{3} - 9 x^{2} + 12 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A200887.
