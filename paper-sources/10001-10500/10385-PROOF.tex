...
\title{The empirical recurrence for OEIS A256429, derived from the entry's generating function}
...
\begin{abstract}
OEIS A256429 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A256429 is ``a(n) is the total number of pentagrams in a variant of pentagram fractal after n iterations.''. It has offset $0$ and begins
\[
1, 6, 11, 16, 41, 106, 211, 416,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 29 2015 11:32:25, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{2} + 3 x + 1}{5 x^{4} - 7 x^{3} + 4 x^{2} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;5 x^{4} - 7 x^{3} + 4 x^{2} - 3 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 4\,a(n-2) + 7\,a(n-3) - 5\,a(n-4)
\]
for every $n>2$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A256429.
