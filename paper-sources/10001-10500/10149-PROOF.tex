...
\title{The empirical recurrence for OEIS A267158, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267158 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A267158 is ``Number of ON (black) cells in the n-th iteration of the "Rule 107" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 1, 3, 5, 6, 8, 7, 13,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 15) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{12} + x^{11} + x^{10} - x^{9} - 4 x^{8} + x^{7} - x^{6} + 2 x^{5} + 2 x^{4} + 4 x^{3} + 2 x^{2} + x + 1}{x^{6} - x^{4} - x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{6} - x^{4} - x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{12} + x^{11} + x^{10} - x^{9} - 4 x^{8} + x^{7} - x^{6} + 2 x^{5} + 2 x^{4} + 4 x^{3} + 2 x^{2} + x + 1,
\]
...
compared against all $72$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267158.
