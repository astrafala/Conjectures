...
\title{The empirical recurrence for OEIS A185762, proved by transfer matrix}
...
\begin{abstract}
OEIS A185762 counts arrays over $\{0,\dots,1\}$ under a condition on the trace of every
$2\times2$ subblock, and carries an empirical recurrence of order $3$ contributed by
R.~H.~Hardin. It is true, and it is decidable rather than empirical. The condition is local to
a bounded window of consecutive rows, so the count is a walk count on $S=40$ states and is
...
OEIS A185762 is ``Number of (n+1) X 3 binary arrays with no 2 X 2 subblock trace equal to any horizontal or vertical neighbor 2 X 2 subblock trace.''. It has offset $1$ and begins
\[
40, 120, 368, 1128, 3456, 10592, 32456, 99464,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + 6*a(n-2) + a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 16 2018 11:13:48, revision 9) this is still
...
entry's first term is $40$, and the model reproduces it along with every later term; that
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A185762.
