...
\title{The empirical recurrence for OEIS A063089, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063089 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A063089 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 21 ).''. It has offset $1$ and begins
\[
1, 6, 12, 16, 22, 28, 32, 38,\ \dots
\]
...
\begin{quote}
G.f.: x*(1 + x)*(1 + 4*x + 2*x\^{}2 + x\^{}3) / ((1 - x)\^{}2*(1 + x + x\^{}2)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + 3 x^{4} + 6 x^{3} + 5 x^{2} + x}{x^{4} - x^{3} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} + 3 x^{4} + 6 x^{3} + 5 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063089.
