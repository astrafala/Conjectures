...
\title{The empirical recurrence for OEIS A205342, derived from the entry's generating function}
...
\begin{abstract}
OEIS A205342 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A205342 is ``Number of length 5 nonnegative integer arrays starting and ending with 0 with adjacent elements unequal but differing by no more than n.''. It has offset $1$ and begins
\[
2, 11, 35, 82, 160, 277, 441, 660,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2018 11:53:16, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{3} + 3 x^{2} + 2 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205342.
