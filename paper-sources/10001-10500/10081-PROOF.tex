...
\title{The empirical recurrence for OEIS A225976, derived from the entry's generating function}
...
\begin{abstract}
OEIS A225976 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A225976 is ``Number of n X 2 binary arrays whose sum with another n X 2 binary array containing no more than two 1s has rows and columns in lexicographically nondecreasing order.''. It has offset $1$ and begins
\[
4, 15, 48, 138, 350, 790, 1616, 3049,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 05 2018 14:50:54, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} + 2 x^{5} - 5 x^{4} + 18 x^{3} - 9 x^{2} + 4 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} + 2 x^{5} - 5 x^{4} + 18 x^{3} - 9 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A225976.
