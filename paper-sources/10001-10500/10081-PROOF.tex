...
\title{The empirical recurrence for OEIS A200166, derived from the entry's generating function}
...
\begin{abstract}
OEIS A200166 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 19 2021 09:58:21, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{36 x^{4} + 30 x^{3} + 28 x^{2} + 2 x}{x^{5} - 3 x^{4} + 2 x^{3} + 2 x^{2} - 3 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;36 x^{4} + 30 x^{3} + 28 x^{2} + 2 x,
\]
