...
\title{The empirical recurrence for OEIS A266842, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266842 carries an empirical recurrence of order $6$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{48 x^{5} - x^{3} + x^{2} - 2 x - 1}{64 x^{6} - 84 x^{4} + 21 x^{2} - 1},
\]
