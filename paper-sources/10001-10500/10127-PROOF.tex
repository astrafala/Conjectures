...
\title{The empirical recurrence for OEIS A154150, derived from the entry's generating function}
...
\begin{abstract}
OEIS A154150 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A154150 is ``Numbers k such that 24 plus the k-th triangular number is a perfect square.''. It has offset $1$ and begins
\[
1, 15, 24, 94, 145, 551, 848, 3214,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 23 2024 14:53:42, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{5} + 14 x^{4} - 3 x^{3} - 14 x^{2} - x}{x^{5} - x^{4} - 6 x^{3} + 6 x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 2 x^{5} - 14 x^{4} + 3 x^{3} + 14 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A154150.
