...
\title{The empirical recurrence for OEIS A249708, derived from the entry's generating function}
...
\begin{abstract}
OEIS A249708 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 09 2018 14:20:29, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 5 x^{4} - 3 x^{3} + x^{2} - 14 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} - 5 x^{4} + 3 x^{3} - x^{2} + 14 x,
\]
