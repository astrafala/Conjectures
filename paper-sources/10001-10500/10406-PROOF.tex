...
\title{A proof of the conjectured recurrence for OEIS A025758}
...
\begin{abstract}
OEIS A025758 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $768000 x^{4}$, a polynomial of degree $4$, which proves
...
OEIS A025758 is ``5th-order Vatalan numbers (generalization of Catalan numbers)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 11, 171, 3056, 58916, 1191376, 24896436,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{5}{\sqrt[5]{1 - 25 x} + 4} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 41*n*(n-1)*(n-2)*(n-3)*a(n) -3*(1367*n-4100)*(n-1)*(n-2)*(n-3)*a(n-1) +50*(n-2)*(n-3)*(3077*n\textasciicircum{}2-21533*n+38136)*a(n-2) -250*(n-3)*(10265*n\textasciicircum{}3-123135*n\textasciicircum{}2+494446*n-664572)*a(n-3) +1875*(8575*n\textasciicircum{}4-154250*n\textasciicircum{}3+1039765*n\textasciicircum{}2-3112730*n+3491808)*a(n-4) -625*(5*n-22)*(5*n-21)*(5*n-24)*(5*n-23)*a(n-5)=0. - \emph{R. J. Mathar}, Jul 28 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2026-01-05, revision 44) the
...
\[
p_{0}(n)=41 n \left(n - 3\right) \left(n - 2\right) \left(n - 1\right),\qquad p_{1}(n)=- 3 \left(n - 3\right) \left(n - 2\right) \left(n - 1\right) \left(1367 n - 4100\right),\qquad p_{2}(n)=50 \left(n - 3\right) \left(n - 2\right) \left(3077 n^{2} - 21533 n + 38136\right),\qquad p_{3}(n)=- 250 \left(n - 3\right) \left(10265 n^{3} - 123135 n^{2} + 494446 n - 664572\right),\qquad p_{4}(n)=1875 \left(8575 n^{4} - 154250 n^{3} + 1039765 n^{2} - 3112730 n + 3491808\right),\qquad p_{5}(n)=- 625 \left(5 n - 24\right) \left(5 n - 23\right) \left(5 n - 22\right) \left(5 n - 21\right)
\]
...
\section{The computation for A025758}

The generating function \eqref{eq:gf} is algebraic over $\mathbb{Q}(x)$;
its minimal polynomial is
\[
P(x,y) \;=\; 25 \left(x y^{5} - 41 y^{5} + 256 y^{4} - 640 y^{3} + 800 y^{2} - 500 y + 125\right) ,
\]
...
\[
41 n \left(n - 3\right) \left(n - 2\right) \left(n - 1\right)\,a(n) -  3 \left(n - 3\right) \left(n - 2\right) \left(n - 1\right) \left(1367 n - 4100\right)\,a(n-1) + 50 \left(n - 3\right) \left(n - 2\right) \left(3077 n^{2} - 21533 n + 38136\right)\,a(n-2) -  250 \left(n - 3\right) \left(10265 n^{3} - 123135 n^{2} + 494446 n - 664572\right)\,a(n-3) + 1875 \left(8575 n^{4} - 154250 n^{3} + 1039765 n^{2} - 3112730 n + 3491808\right)\,a(n-4) -  625 \left(5 n - 24\right) \left(5 n - 23\right) \left(5 n - 22\right) \left(5 n - 21\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A025758, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=768000 x^{4}$ is an
...
\url{https://oeis.org}, 2026. Sequence A025758.
