...
\title{The empirical recurrence for OEIS A227121, derived from the entry's generating function}
...
\begin{abstract}
OEIS A227121 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 07 2018 08:39:09, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{4} - 8 x^{3} + 8 x^{2} - 3 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
