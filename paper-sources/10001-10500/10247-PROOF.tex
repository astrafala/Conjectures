...
\title{The empirical recurrence for OEIS A205186, derived from the entry's generating function}
...
\begin{abstract}
OEIS A205186 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A205186 is ``Number of (n+1) X (n+1) 0..1 arrays with the number of clockwise edge increases in every 2 X 2 subblock differing from each horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
16, 40, 124, 256, 736, 1408, 3904, 7168,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2018 05:11:36, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 64 x^{4} - 4 x^{3} + 40 x^{2} + 16 x}{16 x^{4} - 8 x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;16 x^{4} - 8 x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 64 x^{4} - 4 x^{3} + 40 x^{2} + 16 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205186.
