...
\title{The empirical recurrence for OEIS A268945, derived from the entry's generating function}
...
\begin{abstract}
OEIS A268945 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A268945 is ``Number of length-4 0..n arrays with no repeated value unequal to the previous repeated value plus one mod n+1.''. It has offset $1$ and begins
\[
10, 63, 220, 565, 1206, 2275, 3928, 6345,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 17 2019 04:20:43, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 5 x^{4} - 5 x^{3} - 13 x^{2} - 10 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} - 5 x^{4} + 5 x^{3} + 13 x^{2} + 10 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268945.
