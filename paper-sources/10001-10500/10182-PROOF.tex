...
\title{The empirical recurrence for OEIS A270010, derived from the entry's generating function}
...
\begin{abstract}
OEIS A270010 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A270010 is ``Number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 7", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 9, 0, 49, 0, 121, 0, 225,\ \dots
\]
...
\begin{quote}
G.f.: (1+9*x-3*x\^{}2+22*x\^{}3+3*x\^{}4+x\^{}5-x\^{}6) / ((1-x)\^{}3*(1+x)\^{}3).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:31, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} - x^{5} - 3 x^{4} - 22 x^{3} + 3 x^{2} - 9 x - 1}{x^{6} - 3 x^{4} + 3 x^{2} - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + x^{5} + 3 x^{4} + 22 x^{3} - 3 x^{2} + 9 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270010.
