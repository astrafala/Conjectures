...
\title{The empirical recurrence for OEIS A203286, derived from the entry's generating function}
...
\begin{abstract}
OEIS A203286 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A203286 is ``Number of arrays of 2n nondecreasing integers in -3..3 with sum zero and equal numbers greater than zero and less than zero.''. It has offset $1$ and begins
\[
4, 12, 28, 57, 104, 176, 280, 425,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 13 2025 06:00:29, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 4 x^{5} - 5 x^{4} + 4 x^{2} - 4 x}{x^{6} - 4 x^{5} + 5 x^{4} - 5 x^{2} + 4 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} - 4 x^{5} + 5 x^{4} - 4 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203286.
