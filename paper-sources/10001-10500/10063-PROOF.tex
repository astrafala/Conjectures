...
\title{The empirical recurrence for OEIS A262148, derived from the entry's generating function}
...
\begin{abstract}
OEIS A262148 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A262148 is ``Denominators of domain values whose images via the flowsnake Q-function follow a spiral trajectory.''. It has offset $1$ and begins
\[
56, 56, 392, 392, 56, 392, 2744, 2744,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 13 2016 10:25:01, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n-1}\;=\;\frac{- 392 x^{5} - 56 x^{4} - 392 x^{3} - 392 x^{2} - 56 x - 56}{49 x^{6} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - 49 x^{6}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;392 x^{5} + 56 x^{4} + 392 x^{3} + 392 x^{2} + 56 x + 56,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262148.
