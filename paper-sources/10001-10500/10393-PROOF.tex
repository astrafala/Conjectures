...
\title{The empirical recurrence for OEIS A187303, derived from the entry's generating function}
...
\begin{abstract}
OEIS A187303 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A187303 is ``Number of 8-step one space leftwards or up, two space rightwards or down asymmetric rook's tours on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 72, 2028, 12778, 37124, 81362, 145100,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 23 2018 14:27:04, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{26 x^{16} - 30 x^{15} + 310 x^{14} - 274 x^{13} + 1410 x^{12} - 1158 x^{11} + 2674 x^{10} - 3214 x^{9} + 392 x^{8} - 6296 x^{7} - 4802 x^{6} - 6910 x^{5} - 1812 x^{4} - 72 x^{3}}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 26 x^{16} + 30 x^{15} - 310 x^{14} + 274 x^{13} - 1410 x^{12} + 1158 x^{11} - 2674 x^{10} + 3214 x^{9} - 392 x^{8} + 6296 x^{7} + 4802 x^{6} + 6910 x^{5} + 1812 x^{4} + 72 x^{3},
\]
a POLYNOMIAL of degree $16$ --- the denominator of $G$ divides $D(x)$ times its numerator,
which is precisely the condition for the conjectured recurrence to be implied by the
generating function. Its coefficients vanish beyond degree $16$, so by
Lemma~\ref{lem:key} the residual $a(n)-\sum_ic_ia(n-i)$ vanishes for every
$n>16$. The bound is exact: at $n=16$ the recurrence fails on the entry's own published terms.
\end{proof}
...
they stop at index $16$ because that polynomial has degree $16$. A reader who trusts
...
indices beyond $n=16$ and holds at every one, and at $n=16$ itself it fails --- the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187303.
