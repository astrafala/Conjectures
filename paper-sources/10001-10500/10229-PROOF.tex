...
\title{The empirical recurrence for OEIS A267184, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267184 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A267184 is ``Row 4 of the square array in A267181.''. It has offset $0$ and begins
\[
1, 4, 2, 5, 0, 5, 3, 6,\ \dots
\]
...
\begin{quote}
G.f.: (1+3*x-2*x\^{}2+3*x\^{}3-6*x\^{}4+2*x\^{}5) / ((1-x)\^{}2*(1+x)*(1+x\^{}2)). (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 29 2024 17:34:40, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{5} - 6 x^{4} + 3 x^{3} - 2 x^{2} + 3 x + 1}{x^{5} - x^{4} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{5} - x^{4} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{5} - 6 x^{4} + 3 x^{3} - 2 x^{2} + 3 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267184.
