...
\title{The empirical recurrence for OEIS A248121, derived from the entry's generating function}
...
\begin{abstract}
OEIS A248121 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A248121 is ``a(n) = floor(1 / (1/n - Pi\^{}2/6 + Sum\_{h=1..n} 1/h\^{}2)).''. It has offset $1$ and begins
\[
2, 9, 20, 34, 53, 76, 102, 133,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 11 2024 01:57:01, revision 18) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} - 4 x^{3} - 5 x^{2} - 2 x}{x^{5} - 2 x^{4} + x^{3} - x^{2} + 2 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{5} + 2 x^{4} - x^{3} + x^{2} - 2 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A248121.
