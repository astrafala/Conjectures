...
\title{The empirical recurrence for OEIS A201347, derived from the entry's generating function}
...
\begin{abstract}
OEIS A201347 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 27 2020 19:12:22, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} - 2 x^{4} + 4 x^{3} - 4 x^{2} + 2 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} - 2 x^{4} + 4 x^{3} - 4 x^{2} + 2 x,
\]
