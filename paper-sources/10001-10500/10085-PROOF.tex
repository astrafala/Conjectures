...
\title{The empirical recurrence for OEIS A267160, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267160 carries an empirical recurrence of order $6$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{12} - x^{11} - x^{10} + x^{9} + 4 x^{8} - x^{7} + x^{6} - x^{5} + x^{4} + 2 x^{2} + 2 x}{x^{6} - x^{4} - x^{2} + 1},
\]
