...
\title{The empirical recurrence for OEIS A201695, derived from the entry's generating function}
...
\begin{abstract}
OEIS A201695 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A201695 is ``Number of n X 3 0..2 arrays with rows and columns lexicographically nondecreasing read forwards and nonincreasing read backwards.''. It has offset $1$ and begins
\[
3, 18, 116, 395, 989, 2068, 3838, 6541,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 23 2018 20:03:39, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 9 x^{5} + 35 x^{4} - 56 x^{3} - 3 x^{2} - 3 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;9 x^{5} - 35 x^{4} + 56 x^{3} + 3 x^{2} + 3 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A201695.
