...
\title{The empirical recurrence for OEIS A247284, derived from the entry's generating function}
...
\begin{abstract}
OEIS A247284 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A247284 is ``Subrecords in A048673: maximum value between two consecutive records in A048673.''. It has offset $1$ and begins
\[
4, 6, 13, 18, 38, 63, 113, 188,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 14 2025 14:58:26, revision 15) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 10 x^{7} + 10 x^{6} - x^{5} - x^{4} - 5 x^{3} + 2 x^{2} + 4 x}{3 x^{3} - 3 x^{2} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;3 x^{3} - 3 x^{2} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 10 x^{7} + 10 x^{6} - x^{5} - x^{4} - 5 x^{3} + 2 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A247284.
