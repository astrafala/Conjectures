...
\title{The empirical recurrence for OEIS A267049, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267049 carries an empirical recurrence of order $5$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{6} + 2 x^{5} + 2 x^{4} - x^{3} - 3 x^{2} - x}{x^{5} - x^{4} - 2 x^{3} + 2 x^{2} + x - 1},
\]
