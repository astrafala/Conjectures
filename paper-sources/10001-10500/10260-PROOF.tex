...
\title{The empirical recurrence for OEIS A221599, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221599 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A221599 is ``Number of 0..n arrays of length 7 with each element differing from at least one neighbor by 1 or less.''. It has offset $1$ and begins
\[
128, 1113, 4240, 11011, 22912, 41401, 67936, 103975,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 09 2018 08:56:37, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 28 x^{6} - 16 x^{5} + 217 x^{4} + 556 x^{3} + 601 x^{2} + 128 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 28 x^{6} - 16 x^{5} + 217 x^{4} + 556 x^{3} + 601 x^{2} + 128 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221599.
