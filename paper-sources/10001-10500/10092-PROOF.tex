...
\title{The empirical recurrence for OEIS A228798, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A228798 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
with $S=8$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A228798 is ``Number of 3 X n binary arrays with top left element equal to 1 and no two ones adjacent horizontally or nw-se.''. It has offset $1$ and begins
\[
4, 5, 36, 97, 411, 1394, 5223, 18708,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) + 8*a(n-2) + 6*a(n-3) - a(n-4) - a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 13 2018 08:24:48, revision 11) the statement is
...
$\mathcal V=V\times V$, of size $S=8$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
walk count replaces that by $8$ states and one matrix--vector product per column.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=8$ pairs and each
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A228798.
