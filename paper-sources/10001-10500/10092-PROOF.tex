...
\title{The empirical recurrence for OEIS A202455, derived from the entry's generating function}
...
\begin{abstract}
OEIS A202455 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A202455 is ``Number of (n+2) X 4 binary arrays with consecutive windows of three bits considered as a binary number nondecreasing in every row and column.''. It has offset $1$ and begins
\[
729, 1194, 1876, 2835, 4137, 5854, 8064, 10851,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 01 2018 07:54:34, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 427 x^{5} + 1895 x^{4} - 3196 x^{3} + 2451 x^{2} - 729 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;427 x^{5} - 1895 x^{4} + 3196 x^{3} - 2451 x^{2} + 729 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202455.
