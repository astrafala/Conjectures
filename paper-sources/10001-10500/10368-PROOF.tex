...
\title{The empirical recurrence for OEIS A181270, derived from the entry's generating function}
...
\begin{abstract}
OEIS A181270 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A181270 is ``Number of 2 X n binary matrices M with rows in strictly increasing order and rows of M*Mtranspose (mod 2) in strictly increasing order.''. It has offset $1$ and begins
\[
1, 2, 11, 44, 188, 752, 3056, 12224,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 27 2018 08:48:45, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{8 x^{4} - x^{3} - 2 x^{2} + x}{16 x^{3} - 4 x^{2} - 4 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;16 x^{3} - 4 x^{2} - 4 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A181270.
