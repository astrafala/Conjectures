...
\title{The empirical recurrence for OEIS A222277, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A222277 counts arrays over $\{0,\dots,5\}$ in which every cell is constrained by the
...
OEIS A222277 is ``Number of n X 4 0..5 arrays with no entry increasing mod 6 by 5 rightwards or downwards, starting with upper left zero.''. It has offset $1$ and begins
\[
125, 46305, 17153945, 6354787485, 2354171487645, 872117822449905, 323081602357856985, 119687637492011211885,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 399*a(n-1) - 10749*a(n-2) + 64767*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 14 2018 15:16:44, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;399\,a(n-1) - 10749\,a(n-2) + 64767\,a(n-3)
\end{equation}
for all $n>3$.
...
x_{t,u+1}\ne x_{t,u}+5\ (\mathrm{mod}\ 6)\ \text{ and }\ x_{t+1,u}\ne x_{t,u}+5\ (\mathrm{mod}\ 6)
\end{equation}
...
Let $V=\{0,\dots,5\}^{4}$ be the possible rows and take
...
and $4$ cells in a row, so $|V|=6^{4}=1296$, and the vertex set is
$V\times V$, of size $1296^2=1296$. That is the whole of the state: two consecutive
...
publishes, $n=11$, the arrays have $4\cdot11$ cells over
$6$ values, so there are about $10^{34}$ of them to sift. The
...
of the $(5+1)^{4}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222277.
