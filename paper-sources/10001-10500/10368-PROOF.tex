...
\title{The empirical recurrence for OEIS A250883, derived from the entry's generating function}
...
\begin{abstract}
OEIS A250883 carries an empirical recurrence of order $4$. The entry separately records a
...
\[
155080, 553736, 1350002, 2681528, 4685964, 7500960, 11264166, 16113232,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 22 2018 14:35:18, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 16384 x^{4} + 65538 x^{3} - 66584 x^{2} + 155080 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 16384 x^{4} + 65538 x^{3} - 66584 x^{2} + 155080 x,
\]
