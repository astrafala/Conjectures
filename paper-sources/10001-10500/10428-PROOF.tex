...
\title{The empirical recurrence for OEIS A165204, derived from the entry's generating function}
...
\begin{abstract}
OEIS A165204 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A165204 is ``Hankel transform of A165203.''. It has offset $0$ and begins
\[
1, -5, -21, -21, 89, 377, 377, -1597,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 15 2018 20:46:11, revision 15) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{3} + 2 x^{2} - 8 x + 1}{x^{4} - 3 x^{3} + 8 x^{2} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{4} - 3 x^{3} + 8 x^{2} - 3 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 8\,a(n-2) + 3\,a(n-3) - a(n-4)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A165204.
