...
\title{The empirical recurrence for OEIS A267354, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267354 carries an empirical recurrence of order $4$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{4} - x^{3} + 2 x^{2} + x}{x^{4} - 2 x^{2} + 1},
\]
