...
\title{The empirical recurrence for OEIS A189807, derived from the entry's generating function}
...
\begin{abstract}
OEIS A189807 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A189807 is ``Number of right triangles on an (n+1) X 3 grid.''. It has offset $1$ and begins
\[
14, 44, 94, 158, 238, 330, 434, 550,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 03 2018 07:02:28, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{6} - 2 x^{5} + 6 x^{4} - 4 x^{3} - 2 x^{2} - 14 x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 4 x^{6} + 2 x^{5} - 6 x^{4} + 4 x^{3} + 2 x^{2} + 14 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A189807.
