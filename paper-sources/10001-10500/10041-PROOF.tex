...
\title{The empirical recurrence for OEIS A228222, derived from the entry's generating function}
...
\begin{abstract}
OEIS A228222 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A228222 is ``Number of second differences of arrays of length 7 of numbers in 0..n.''. It has offset $1$ and begins
\[
127, 2059, 14053, 58141, 176851, 439927, 951049, 1854553,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 10 2018 08:39:26, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} - 475 x^{5} + 2168 x^{4} + 3604 x^{3} + 1297 x^{2} + 127 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} - 475 x^{5} + 2168 x^{4} + 3604 x^{3} + 1297 x^{2} + 127 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A228222.
