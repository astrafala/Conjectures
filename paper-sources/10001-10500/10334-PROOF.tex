...
\title{The empirical recurrence for OEIS A269468, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269468 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 23 2019 07:18:35, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{3} - 16 x^{2} - 10 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
