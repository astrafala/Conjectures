...
\title{The empirical recurrence for OEIS A221685, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221685 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A221685 is ``Number of 0..n arrays of length 6 with each element differing from at least one neighbor by 1 or less, starting with 0.''. It has offset $1$ and begins
\[
32, 113, 241, 418, 636, 891, 1182, 1509,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2018 09:14:27, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{7} + 4 x^{6} + 8 x^{5} - 2 x^{4} + 2 x^{3} - 17 x^{2} - 32 x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{7} - 4 x^{6} - 8 x^{5} + 2 x^{4} - 2 x^{3} + 17 x^{2} + 32 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221685.
