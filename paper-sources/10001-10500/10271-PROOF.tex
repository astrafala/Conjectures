...
\title{The empirical recurrence for OEIS A229014, derived from the entry's generating function}
...
\begin{abstract}
OEIS A229014 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A229014 is ``Number of arrays of median of three adjacent elements of some length 6 0..n array, with no adjacent equal elements in the latter.''. It has offset $1$ and begins
\[
2, 31, 130, 359, 794, 1527, 2666, 4335,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 13 2018 11:11:33, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + x^{4} + 5 x^{3} - 21 x^{2} - 2 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{5} - x^{4} - 5 x^{3} + 21 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229014.
