...
\title{The empirical recurrence for OEIS A259661, derived from the entry's generating function}
...
\begin{abstract}
OEIS A259661 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:26, revision 55) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{1}{10 x^{4} - 11 x^{3} + 11 x^{2} - 11 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;10 x^{4} - 11 x^{3} + 11 x^{2} - 11 x + 1
\]
