...
\title{The empirical recurrence for OEIS A265380, derived from the entry's generating function}
...
\begin{abstract}
OEIS A265380 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A265380 is ``Binary representation of the middle column of the "Rule 158" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 11, 111, 1110, 11101, 111011, 1110111, 11101110,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:27, revision 40) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{2} + x + 1}{10 x^{5} - x^{4} - 10 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;10 x^{5} - x^{4} - 10 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A265380.
