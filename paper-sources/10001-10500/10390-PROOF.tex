...
\title{The empirical recurrence for OEIS A263805, derived from the entry's generating function}
...
\begin{abstract}
OEIS A263805 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:27, revision 30) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{100000 x^{4} - 100000 x^{3} + 10000 x^{2} - 1}{100000 x^{4} - 110000 x^{3} + 9990 x^{2} + 11 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 100000 x^{4} + 110000 x^{3} - 9990 x^{2} - 11 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 100000 x^{4} + 100000 x^{3} - 10000 x^{2} + 1,
\]
...
\[
100000 x^{4} - 110000 x^{3} + 9990 x^{2} + 11 x - 1,
\]
