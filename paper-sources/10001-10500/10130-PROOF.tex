...
\title{The empirical recurrence for OEIS A178352, derived from the entry's generating function}
...
\begin{abstract}
OEIS A178352 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A178352 is ``Number of collinear point triples in a 6 X 6 X 6 X ... n-dimensional cubic grid.''. It has offset $0$ and begins
\[
0, 20, 372, 5696, 85584, 1324160, 21306432, 354876416,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = 54*a(n-1) - 1124*a(n-2) + 11304*a(n-3) - 55008*a(n-4) + 103680*a(n-5).
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: 4*x*(-5+177*x-2022*x\^{}2+7488*x\^{}3) / ( (12*x-1)*(6*x-1)*(8*x-1)*(18*x-1)*(10*x-1) ). (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 17 2025 23:12:16, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{29952 x^{4} - 8088 x^{3} + 708 x^{2} - 20 x}{103680 x^{5} - 55008 x^{4} + 11304 x^{3} - 1124 x^{2} + 54 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 103680 x^{5} + 55008 x^{4} - 11304 x^{3} + 1124 x^{2} - 54 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 29952 x^{4} + 8088 x^{3} - 708 x^{2} + 20 x,
\]
...
\[
103680 x^{5} - 55008 x^{4} + 11304 x^{3} - 1124 x^{2} + 54 x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A178352.
