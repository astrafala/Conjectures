...
\title{The empirical recurrence for OEIS A269906, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269906 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A269906 is ``Number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 1", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 4, 1, 44, 1, 116, 1, 220,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{5} - x^{4} - 32 x^{3} + 2 x^{2} - 4 x - 1}{x^{6} - 3 x^{4} + 3 x^{2} - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269906.
