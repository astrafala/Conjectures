...
\title{A proof of the conjectured recurrence for OEIS A127632}
...
\begin{abstract}
OEIS A127632 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $18 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A127632 is ``Expansion of c(x*c(x)), where c(x) is the g.f. for A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 11, 44, 185, 804, 3579,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{\sqrt{2 \sqrt{1 - 4 x} - 1} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 3*n*(n-1)*(4*n-9)*(n+1)*a(n) - 8*n*(n-1)*(20*n\^{}2-67*n+48)*a(n-1) + 8*(4*n-5)*(n-1)*(22*n\^{}2-94*n+99)*a(n-2) - 8*(4*n-11)*(4*n-5)*(4*n-9)*(2*n-5)*a(n-3) = 0. - \emph{R. J. Mathar}, May 04 2018
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 129) the
...
\[
p_{0}(n)=3 n \left(n - 1\right) \left(n + 1\right) \left(4 n - 9\right),\qquad p_{1}(n)=- 8 n \left(n - 1\right) \left(20 n^{2} - 67 n + 48\right),\qquad p_{2}(n)=8 \left(n - 1\right) \left(4 n - 5\right) \left(22 n^{2} - 94 n + 99\right),\qquad p_{3}(n)=- 8 \left(2 n - 5\right) \left(4 n - 11\right) \left(4 n - 9\right) \left(4 n - 5\right)
\]
...
\[
3 n \left(n - 1\right) \left(n + 1\right) \left(4 n - 9\right)\,a(n) -  8 n \left(n - 1\right) \left(20 n^{2} - 67 n + 48\right)\,a(n-1) + 8 \left(n - 1\right) \left(4 n - 5\right) \left(22 n^{2} - 94 n + 99\right)\,a(n-2) -  8 \left(2 n - 5\right) \left(4 n - 11\right) \left(4 n - 9\right) \left(4 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A127632, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=18 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A127632.
