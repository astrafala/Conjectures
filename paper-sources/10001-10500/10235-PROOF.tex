...
\title{The empirical recurrence for OEIS A192431, derived from the entry's generating function}
...
\begin{abstract}
OEIS A192431 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A192431 is ``Coefficient of x in the reduction by x\^{}2->x+1 of the polynomial p(n,x) defined below in Comments.''. It has offset $0$ and begins
\[
0, 1, 4, 15, 52, 185, 648, 2287,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 07 2019 22:01:55, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{3} + 2 x^{2} + x}{x^{4} + 2 x^{3} - 6 x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{4} + 2 x^{3} - 6 x^{2} - 2 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A192431.
