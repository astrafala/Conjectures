...
\title{The empirical recurrence for OEIS A252935, derived from the entry's generating function}
...
\begin{abstract}
OEIS A252935 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A252935 is ``Number of n X 5 nonnegative integer arrays with upper left 0 and every value within 3 of its king move distance from the upper left and every value increasing by 0 or 1 with every step right, diagonally se or down.''. It has offset $1$ and begins
\[
15, 83, 494, 3067, 17962, 86488, 320270, 917811,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 07 2018 11:51:50, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{128 x^{12} + 2477 x^{11} + 15733 x^{10} + 49399 x^{9} + 86878 x^{8} + 70316 x^{7} + 31149 x^{6} + 8341 x^{5} + 1529 x^{4} + 252 x^{3} + 23 x^{2} + 15 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;128 x^{12} + 2477 x^{11} + 15733 x^{10} + 49399 x^{9} + 86878 x^{8} + 70316 x^{7} + 31149 x^{6} + 8341 x^{5} + 1529 x^{4} + 252 x^{3} + 23 x^{2} + 15 x,
\]
...
compared against all $25$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252935.
