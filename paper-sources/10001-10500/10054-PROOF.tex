...
\title{The empirical recurrence for OEIS A145646, derived from the entry's generating function}
...
\begin{abstract}
OEIS A145646 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A145646 is ``Wavelength (in ångströms) of the series limit of the Hydrogen spectrum for main quantum number n.''. It has offset $1$ and begins
\[
911, 3645, 8201, 14580, 22782, 32806, 44652, 58321,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 28 2026 15:47:56, revision 25) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} - 913 x^{4} - 910 x^{3} - 912 x^{2} - 911 x}{x^{5} - 3 x^{4} + 4 x^{3} - 4 x^{2} + 3 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{5} + 3 x^{4} - 4 x^{3} + 4 x^{2} - 3 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{5} + 913 x^{4} + 910 x^{3} + 912 x^{2} + 911 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A145646.
