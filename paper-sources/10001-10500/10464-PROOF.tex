...
\title{The empirical recurrence for OEIS A211476, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211476 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A211476 is ``Number of -1..1 arrays x(i) of n+1 elements i=1..n+1 with x(i)+x(j), x(i+1)+x(j+1), -(x(i)+x(j+1)), and -(x(i+1)+x(j)) having one or three distinct values for every i<=n and j<=n.''. It has offset $1$ and begins
\[
7, 17, 37, 79, 165, 339, 693, 1403,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 18 2018 05:31:11, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 12 x^{4} - 8 x^{3} + 10 x^{2} + 7 x}{4 x^{4} + 2 x^{3} - 4 x^{2} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{4} + 2 x^{3} - 4 x^{2} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 12 x^{4} - 8 x^{3} + 10 x^{2} + 7 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211476.
