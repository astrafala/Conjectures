...
\title{The empirical recurrence for OEIS A237631, proved by transfer matrix}
...
\begin{abstract}
OEIS A237631 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=260$
...
OEIS A237631 is ``Number of (n+1) X (2+1) 0..2 arrays with the upper median of every 2 X 2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$ and begins
\[
260, 636, 1220, 2356, 4500, 8594, 16448, 31672,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-3) + 10*a(n-4) - 12*a(n-7) - 6*a(n-8) for n>12.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 24 2018 06:02:35, revision 8) this is still
...
differ by $1$. The entry's first term $260$ counts those, and the model was made to
...
\]
over the $S=260$ pairs whose own subblock row already satisfies the condition along itself.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237631.
