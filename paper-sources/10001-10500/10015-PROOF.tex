...
\title{The empirical recurrence for OEIS A270012, derived from the entry's generating function}
...
\begin{abstract}
OEIS A270012 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A270012 is ``Partial sums of the number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 7", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 10, 10, 59, 59, 180, 180, 405,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + x^{5} + 3 x^{4} + 22 x^{3} - 3 x^{2} + 9 x + 1}{x^{7} - x^{6} - 3 x^{5} + 3 x^{4} + 3 x^{3} - 3 x^{2} - x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270012.
