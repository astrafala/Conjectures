...
\title{The empirical recurrence for OEIS A212836, derived from the entry's generating function}
...
\begin{abstract}
OEIS A212836 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 21 2018 10:07:09, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} - 6 x^{3} + 9 x^{2} + 2 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
