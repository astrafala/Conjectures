...
\title{The empirical recurrence for OEIS A269782, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269782 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A269782 is ``Number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 65", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 4, 5, 36, 9, 96, 17, 188,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:31, revision 29) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{10} + 8 x^{8} - 4 x^{7} - 4 x^{6} + 3 x^{4} - 24 x^{3} - 2 x^{2} - 4 x - 1}{x^{6} - 3 x^{4} + 3 x^{2} - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269782.
