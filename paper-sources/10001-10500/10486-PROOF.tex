...
\title{The empirical recurrence for OEIS A252999, derived from the entry's generating function}
...
\begin{abstract}
OEIS A252999 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A252999 is ``Number of n X 3 nonnegative integer arrays with upper left 0 and lower right its king-move distance away minus 3 and every value within 3 of its king move distance from the upper left and every value increasing by 0 or 1 with every step right, diagonally se or down.''. It has offset $1$ and begins
\[
0, 0, 0, 1, 34, 279, 1028, 2601,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 08 2018 05:29:01, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{28 x^{8} + 112 x^{7} + 149 x^{6} + 30 x^{5} + x^{4}}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;28 x^{8} + 112 x^{7} + 149 x^{6} + 30 x^{5} + x^{4},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252999.
