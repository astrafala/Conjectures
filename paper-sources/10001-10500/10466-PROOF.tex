...
\title{The empirical recurrence for OEIS A211850, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211850 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A211850 is ``Number of nonnegative integer arrays of length 2n+5 with new values 0 upwards introduced in order, no three adjacent elements all unequal, and containing the value n+1.''. It has offset $1$ and begins
\[
63, 147, 286, 494, 785, 1173, 1672, 2296,\ \dots
\]
...
\begin{quote}
G.f.: x*(63 - 105*x + 76*x\^{}2 - 20*x\^{}3) / (1 - x)\^{}4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 20 2018 07:52:14, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 20 x^{4} + 76 x^{3} - 105 x^{2} + 63 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 20 x^{4} + 76 x^{3} - 105 x^{2} + 63 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211850.
