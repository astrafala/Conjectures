...
\title{The empirical recurrence for OEIS A027116, derived from the entry's generating function}
...
\begin{abstract}
OEIS A027116 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A027116 is ``a(n) = A027113(n, n+3).''. It has offset $3$ and begins
\[
7, 23, 56, 120, 241, 467, 886, 1660,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 23 2019 20:32:55, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{6} - x^{5} - 2 x^{4} - 7 x^{3}}{x^{5} - x^{4} - 2 x^{2} + 3 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{5} + x^{4} + 2 x^{2} - 3 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A027116.
