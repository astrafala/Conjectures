...
\title{The empirical recurrence for OEIS A063117, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063117 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A063117 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 49 ).''. It has offset $1$ and begins
\[
1, 10, 20, 28, 38, 48, 56, 66,\ \dots
\]
...
\begin{quote}
G.f.: x*(1 + 9*x + 10*x\^{}2 + 7*x\^{}3 + x\^{}4) / ((1 - x)\^{}2*(1 + x + x\^{}2)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + 7 x^{4} + 10 x^{3} + 9 x^{2} + x}{x^{4} - x^{3} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} + 7 x^{4} + 10 x^{3} + 9 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063117.
