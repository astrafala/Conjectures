...
\title{The empirical recurrence for OEIS A201813, derived from the entry's generating function}
...
\begin{abstract}
OEIS A201813 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A201813 is ``Number of arrays of 5 integers in -n..n with sum zero and equal numbers of elements greater than zero and less than zero.''. It has offset $1$ and begins
\[
51, 221, 631, 1401, 2651, 4501, 7071, 10481,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 25 2018 10:16:29, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} + 53 x^{3} + 17 x^{2} + 51 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{4} + 53 x^{3} + 17 x^{2} + 51 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A201813.
