...
\title{The empirical recurrence for OEIS A270006, derived from the entry's generating function}
...
\begin{abstract}
OEIS A270006 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A270006 is ``Number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 5", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 8, 0, 49, 0, 121, 0, 225,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:31, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{7} + x^{6} + 2 x^{5} - 3 x^{4} - 25 x^{3} + 3 x^{2} - 8 x - 1}{x^{6} - 3 x^{4} + 3 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{6} + 3 x^{4} - 3 x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{7} - x^{6} - 2 x^{5} + 3 x^{4} + 25 x^{3} - 3 x^{2} + 8 x + 1,
\]
...
compared against all $59$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270006.
