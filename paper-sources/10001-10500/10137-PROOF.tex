...
\title{The empirical recurrence for OEIS A267002, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267002 carries an empirical recurrence of order $6$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 31) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{1100000 x^{5} - 10000 x^{4} + 9990 x^{3} + 10001 x^{2} - 101 x - 1}{10000000 x^{6} - 10011000 x^{4} + 11001 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 10000000 x^{6} + 10011000 x^{4} - 11001 x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 1100000 x^{5} + 10000 x^{4} - 9990 x^{3} - 10001 x^{2} + 101 x + 1,
\]
...
\[
10000000 x^{6} - 10011000 x^{4} + 11001 x^{2} - 1,
\]
