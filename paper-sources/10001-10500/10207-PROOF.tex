...
\title{The empirical recurrence for OEIS A265724, derived from the entry's generating function}
...
\begin{abstract}
OEIS A265724 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A265724 is ``Total number of OFF (white) cells after n iterations of the "Rule 1" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 3, 7, 10, 18, 21, 33, 36,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{3} - 4 x^{2} - 3 x}{x^{5} - x^{4} - 2 x^{3} + 2 x^{2} + x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A265724.
