...
\title{The empirical recurrence for OEIS A304216, proved by transfer matrix}
...
\begin{abstract}
OEIS A304216 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,3,8\}$, and it carries an empirical
...
OEIS A304216 is ``Number of n X 3 0..1 arrays with every element unequal to 0, 1, 3 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
3, 7, 10, 17, 32, 56, 93, 159,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +a(n-3) +2*a(n-4) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 30 2023 16:37:45, revision
...
has score in $\{0,1,3,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,3,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,3,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,3,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,3,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A304216.
