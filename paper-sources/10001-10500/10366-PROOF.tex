...
\title{The empirical recurrence for OEIS A167440, derived from the entry's generating function}
...
\begin{abstract}
OEIS A167440 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A167440 is ``5th GegenbauerC polynomial evaluated at powers of 2 (multiplied by 5).''. It has offset $1$ and begins
\[
2, 724, 30248, 1028176, 33390752, 1072431424, 34349253248, 1099427742976,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 28 2026 15:47:56, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 512 x^{3} - 640 x^{2} - 2 x}{512 x^{3} - 336 x^{2} + 42 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 512 x^{3} + 336 x^{2} - 42 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A167440.
