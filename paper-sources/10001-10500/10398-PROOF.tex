...
\title{The empirical closed form for OEIS A219701, proved}
...
\begin{abstract}
OEIS A219701 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=978$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219701 is ``Number of nX5 arrays of the minimum value of corresponding elements and their horizontal, vertical or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 nX5 array.''. It has offset $1$ and begins
\[
5, 9, 45, 158, 518, 1648, 5007, 14438,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:13:56, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=978$
...
states number $S=978$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219701 records is correct for all $n>11$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $29$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219701.
