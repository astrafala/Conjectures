...
\title{The empirical recurrence for OEIS A266663, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266663 carries an empirical recurrence of order $5$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + x^{4} - x^{3} - 2 x - 1}{x^{5} - x^{4} - 2 x^{3} + 2 x^{2} + x - 1},
\]
