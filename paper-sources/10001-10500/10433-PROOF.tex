...
\title{The empirical recurrence for OEIS A201722, derived from the entry's generating function}
...
\begin{abstract}
OEIS A201722 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A201722 is ``Number of n X 1 0..4 arrays with rows and columns lexicographically nondecreasing and no element equal to the number of horizontal and vertical neighbors equal to itself.''. It has offset $1$ and begins
\[
4, 10, 20, 35, 56, 83, 116, 155,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 23 2018 15:13:46, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} - x^{4} - 2 x^{3} + 2 x^{2} - 4 x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} + x^{4} + 2 x^{3} - 2 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A201722.
