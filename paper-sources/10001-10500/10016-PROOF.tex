...
\title{The empirical recurrence for OEIS A270079, derived from the entry's generating function}
...
\begin{abstract}
OEIS A270079 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A270079 is ``Number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 62", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 5, 12, 20, 32, 44, 68, 72,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:31, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{12} - 4 x^{10} - 4 x^{9} + x^{8} + 12 x^{7} - 6 x^{6} - 4 x^{4} - 4 x^{3} - 6 x^{2} - 4 x - 1}{x^{7} - x^{6} - x^{5} + x^{4} - x^{3} + x^{2} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{7} + x^{6} + x^{5} - x^{4} + x^{3} - x^{2} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 4 x^{12} + 4 x^{10} + 4 x^{9} - x^{8} - 12 x^{7} + 6 x^{6} + 4 x^{4} + 4 x^{3} + 6 x^{2} + 4 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270079.
