...
\title{The empirical recurrence for OEIS A267042, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267042 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A267042 is ``Decimal representation of the n-th iteration of the "Rule 91" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 5, 10, 99, 124, 1935, 496, 32319,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 26) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 1344 x^{8} + 320 x^{7} + 1332 x^{6} - 276 x^{5} + 2 x^{4} + 6 x^{3} + 11 x^{2} - 5 x - 1}{64 x^{6} - 84 x^{4} + 21 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 64 x^{6} + 84 x^{4} - 21 x^{2} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;21\,a(n-2) - 84\,a(n-4) + 64\,a(n-6)
\]
for every $n>8$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;1344 x^{8} - 320 x^{7} - 1332 x^{6} + 276 x^{5} - 2 x^{4} - 6 x^{3} - 11 x^{2} + 5 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267042.
