...
\title{The empirical recurrence for OEIS A183885, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183885 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A183885 is ``Number of arrangements of n+3 numbers in 0..2 with each number being the sum mod 3 of three others.''. It has offset $1$ and begins
\[
1, 121, 631, 2059, 6399, 19483, 58807, 176859,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = 6*a(n-1) - 12*a(n-2) + 10*a(n-3) - 3*a(n-4) for n>6.
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: x*(1 + 115*x - 83*x\^{}2 - 285*x\^{}3 + 410*x\^{}4 - 150*x\^{}5) / ((1 - x)\^{}3*(1 - 3*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 05 2018 09:51:47, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 150 x^{6} + 410 x^{5} - 285 x^{4} - 83 x^{3} + 115 x^{2} + x}{3 x^{4} - 10 x^{3} + 12 x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;3 x^{4} - 10 x^{3} + 12 x^{2} - 6 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 150 x^{6} + 410 x^{5} - 285 x^{4} - 83 x^{3} + 115 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183885.
