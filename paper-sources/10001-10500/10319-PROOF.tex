...
\title{The empirical recurrence for OEIS A266795, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266795 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A266795 is ``Total number of OFF (white) cells after n iterations of the "Rule 61" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 1, 4, 6, 11, 15, 23, 27,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{8} + 2 x^{7} - x^{6} - x^{5} + x^{4} - 3 x^{2} - x}{x^{5} - x^{4} - 2 x^{3} + 2 x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{8} - 2 x^{7} + x^{6} + x^{5} - x^{4} + 3 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266795.
