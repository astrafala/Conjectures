...
\title{The empirical recurrence for OEIS A142248, derived from the entry's generating function}
...
\begin{abstract}
OEIS A142248 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A142248 is ``Odd numbers in A138123.''. It has offset $1$ and begins
\[
1, 3, 7, 11, 17, 29, 47, 75,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2025 11:55:01, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{6} - 3 x^{5} + 3 x^{3} + 2 x^{2} + x}{x^{5} + x^{4} - x^{3} - x^{2} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 2 x^{6} - 3 x^{5} + 3 x^{3} + 2 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A142248.
