...
\title{The empirical recurrence for OEIS A236535, derived from the entry's generating function}
...
\begin{abstract}
OEIS A236535 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A236535 is ``a(n)*Pi is the total length of irregular spiral (center points: 2, 3, 1; pattern 1) after n rotations.''. It has offset $1$ and begins
\[
2, 5, 8, 10, 13, 16, 18, 21,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 12 2014 16:46:10, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{3} + 3 x^{2} + 2 x}{x^{4} - x^{3} - x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A236535.
