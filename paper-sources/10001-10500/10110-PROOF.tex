...
\title{The empirical recurrence for OEIS A234222, proved by transfer matrix}
...
\begin{abstract}
OEIS A234222 counts $4$-column arrays over $\{0,\dots,4\}$ in which every
$2\times2$ subblock has its diagonal sum differing from its antidiagonal sum by
exactly $3$, and it
carries an empirical recurrence of order $11$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=625$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-11}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234222 is ``Number of (n+1) X (3+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
1824, 5612, 17448, 56936, 188040, 648536, 2264904,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 02:01:33, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;9\,a(n-1) + 20\,a(n-2) - 390\,a(n-3) + 460\,a(n-4) + 5520\,a(n-5) - 14240\,a(n-6) - 26400\,a(n-7) + 115776\,a(n-8) - 13824\,a(n-9) - 299520\,a(n-10) + 276480\,a(n-11)
\end{equation}
for all $n>10$, which is every $n$ at which a recurrence of order $11$ can be
stated.
...
Let $V$ consist of all of $\{0,\dots,4\}^{4}$, and put $S=|V|=625$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge11$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-11}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-11$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-11}\bigl(M^{11}-\sum_i c_i M^{11-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-11$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $11$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=10-11+1$ onwards, and the run was continued past
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234222.
