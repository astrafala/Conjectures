...
\title{The empirical recurrence for OEIS A063247, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063247 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A063247 is ``Dimension of the space of weight 2n cuspidal newforms for Gamma\_0( 100 ).''. It has offset $1$ and begins
\[
1, 5, 8, 11, 14, 18, 20, 24,\ \dots
\]
...
\begin{quote}
G.f.: x*(1 + 5*x + 7*x\^{}2 + 5*x\^{}3 + x\^{}4) / ((1 - x)\^{}2*(1 + x)*(1 + x + x\^{}2)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + 5 x^{4} + 7 x^{3} + 5 x^{2} + x}{x^{5} - x^{3} - x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} + 5 x^{4} + 7 x^{3} + 5 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063247.
