...
\title{The empirical recurrence for OEIS A063125, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063125 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A063125 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 57 ).''. It has offset $1$ and begins
\[
5, 18, 32, 44, 58, 72, 84, 98,\ \dots
\]
...
\begin{quote}
G.f.: x*(1 + x)*(5 + 8*x + 6*x\^{}2 + x\^{}3) / ((1 - x)\^{}2*(1 + x + x\^{}2)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + 7 x^{4} + 14 x^{3} + 13 x^{2} + 5 x}{x^{4} - x^{3} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} + 7 x^{4} + 14 x^{3} + 13 x^{2} + 5 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063125.
