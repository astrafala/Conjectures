...
\title{The empirical recurrence for OEIS A250162, derived from the entry's generating function}
...
\begin{abstract}
OEIS A250162 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A250162 is ``Number of length n+1 0..3 arrays with the sum of adjacent differences multiplied by some arrangement of +-1 equal to zero.''. It has offset $1$ and begins
\[
4, 20, 96, 436, 1880, 7836, 32032, 129572,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 12 2018 14:36:33, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{20 x^{3} - 12 x^{2} + 4 x}{8 x^{4} - 22 x^{3} + 21 x^{2} - 8 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;8 x^{4} - 22 x^{3} + 21 x^{2} - 8 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250162.
