...
\title{The empirical recurrence for OEIS A266589, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266589 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A266589 is ``Binary representation of the n-th iteration of the "Rule 37" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 10, 1110, 1000001, 111000, 11100000111, 11100000, 111110000011111,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 27) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{1010100000 x^{7} - 10100000 x^{6} - 1009091010 x^{5} + 10091010 x^{4} - 898991 x^{3} + 8991 x^{2} - 10 x - 1}{1000000 x^{6} - 1010100 x^{4} + 10101 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 1000000 x^{6} + 1010100 x^{4} - 10101 x^{2} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;10101\,a(n-2) - 1010100\,a(n-4) + 1000000\,a(n-6)
\]
for every $n>7$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 1010100000 x^{7} + 10100000 x^{6} + 1009091010 x^{5} - 10091010 x^{4} + 898991 x^{3} - 8991 x^{2} + 10 x + 1,
\]
...
$n>7$. The bound is exact: at $n=7$ the recurrence fails on the entry's own published terms.
\end{proof}
...
\[
1000000 x^{6} - 1010100 x^{4} + 10101 x^{2} - 1,
\]
...
compared against all $17$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266589.
