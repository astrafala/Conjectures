...
\title{The empirical recurrence for OEIS A211526, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211526 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 18 2018 10:08:31, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 12 x^{4} + 22 x^{3} + 6 x^{2} - 8 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 4 x^{4} + 6 x^{3} - 3 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;12 x^{4} - 22 x^{3} - 6 x^{2} + 8 x,
\]
