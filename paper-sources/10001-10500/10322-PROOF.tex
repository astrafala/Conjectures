...
\title{The empirical recurrence for OEIS A184063, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184063 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A184063 is ``Number of (n+1) X 2 binary arrays with rows and columns in nondecreasing order and with no 2 X 2 subblock sum differing from a horizontal or vertical neighbor subblock sum by more than one.''. It has offset $1$ and begins
\[
7, 11, 20, 34, 54, 81, 116, 160,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 11 2018 08:10:24, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} - 8 x^{4} + 18 x^{3} - 17 x^{2} + 7 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} - 8 x^{4} + 18 x^{3} - 17 x^{2} + 7 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184063.
