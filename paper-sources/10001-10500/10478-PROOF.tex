...
\title{The empirical recurrence for OEIS A247000, derived from the entry's generating function}
...
\begin{abstract}
OEIS A247000 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A247000 is ``Maximal number of palindromes in a circular binary word of length n.''. It has offset $1$ and begins
\[
1, 2, 4, 6, 7, 9, 10, 12,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 30 2026 16:40:42, revision 32) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{14} - x^{13} + x^{12} - x^{11} + x^{10} - x^{9} + x^{8} - x^{7} + x^{4} + 2 x^{3} + x^{2} + x}{x^{4} - x^{3} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{4} - x^{3} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{14} - x^{13} + x^{12} - x^{11} + x^{10} - x^{9} + x^{8} - x^{7} + x^{4} + 2 x^{3} + x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A247000.
