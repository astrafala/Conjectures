...
\title{The empirical recurrence for OEIS A270013, derived from the entry's generating function}
...
\begin{abstract}
OEIS A270013 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A270013 is ``First differences of number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 7", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
8, -9, 49, -49, 121, -121, 225, -225,\ \dots
\]
...
\begin{quote}
G.f.: (8-x+24*x\^{}2+2*x\^{}3-x\^{}5) / ((1-x)\^{}2*(1+x)\^{}3).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 2 x^{3} + 24 x^{2} - x + 8}{x^{5} + x^{4} - 2 x^{3} - 2 x^{2} + x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270013.
