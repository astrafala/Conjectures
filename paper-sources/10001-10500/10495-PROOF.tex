...
\title{The empirical recurrence for OEIS A253688, derived from the entry's generating function}
...
\begin{abstract}
OEIS A253688 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A253688 is ``The total number of pentagons in a variant of pentagon expansion (vertex-to-vertex, two consecutive vertices and one isolated vertex) after n iterations.''. It has offset $1$ and begins
\[
1, 4, 10, 21, 39, 64, 94, 129,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 15 2015 12:38:39, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{9} + 2 x^{8} + 2 x^{7} - 2 x^{6} - 4 x^{5} - 3 x^{4} - 2 x^{3} - 2 x^{2} - x}{x^{4} - 2 x^{3} + 2 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{9} - 2 x^{8} - 2 x^{7} + 2 x^{6} + 4 x^{5} + 3 x^{4} + 2 x^{3} + 2 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253688.
