...
\title{The empirical recurrence for OEIS A223417, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A223417 counts arrays over $\{0,\dots,6\}$ in which every cell is constrained by the
...
with $S=49$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A223417 is ``3-level binary fanout graph coloring a rectangular array: number of nX1 0..6 arrays where 0..6 label nodes of a graph with edges 0,1 1,3 1,4 0,2 2,5 2,6 and every array movement to a horizontal or vertical neighbor moves along an edge of this graph.''. It has offset $1$ and begins
\[
7, 12, 26, 48, 104, 192, 416, 768,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-2) for n>3
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 21:39:02, revision 8) the statement is
...
row $t$ and column $u$, each in $\{0,\dots,6\}$. With the neighbour offsets
\[
\mathcal N=\{(0,1),\ (1,0)\},
\]
...
\{x_{t,u},x_{t+d_1,u+d_2}\}\in E\ \text{ for every offset in }\mathcal N,\quad E=\{\{0,1\},\{0,2\},\{1,3\},\{1,4\},\{2,5\},\{2,6\}\}
\end{equation}
for every cell $(t,u)$.
...
Let $V=\{0,\dots,6\}^{1}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=49$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
publishes, $n=30$, the arrays have $1\cdot30$ cells over
$7$ values, so there are about $10^{25}$ of them to sift. The
walk count replaces that by $49$ states and one matrix--vector product per row.

The threshold below is exact rather than merely sufficient: at $n=3$ the conjectured
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=49$ pairs and each
of the $(6+1)^{1}$ possible next rows, the condition was tested on every cell of
...
every one of the $30$ published indices, with no shift fitted: the number of rows is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223417.
