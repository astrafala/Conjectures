...
\title{The empirical recurrence for OEIS A266717, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266717 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A266717 is ``Binary representation of the n-th iteration of the "Rule 59" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 101, 110, 1111011, 1100, 11111110111, 11000, 111111111101111,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 36) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 1000000 x^{6} + 110000 x^{5} + 990100 x^{4} - 99900 x^{3} + 9901 x^{2} - 101 x - 1}{100000 x^{6} - 110010 x^{4} + 10011 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 100000 x^{6} + 110010 x^{4} - 10011 x^{2} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;10011\,a(n-2) - 110010\,a(n-4) + 100000\,a(n-6)
\]
for every $n>6$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;1000000 x^{6} - 110000 x^{5} - 990100 x^{4} + 99900 x^{3} - 9901 x^{2} + 101 x + 1,
\]
...
\[
100000 x^{6} - 110010 x^{4} + 10011 x^{2} - 1,
\]
...
compared against all $18$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266717.
