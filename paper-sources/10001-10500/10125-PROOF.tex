...
\title{The empirical recurrence for OEIS A247727, derived from the entry's generating function}
...
\begin{abstract}
OEIS A247727 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A247727 is ``Number of length 1+3 0..n arrays with no disjoint pairs in any consecutive four terms having the same sum.''. It has offset $1$ and begins
\[
8, 48, 168, 440, 960, 1848, 3248, 5328,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 14 2025 04:35:25, revision 41) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{3} - 8 x^{2} - 8 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A247727.
