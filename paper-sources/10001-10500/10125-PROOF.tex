...
\title{The empirical recurrence for OEIS A154144, derived from the entry's generating function}
...
\begin{abstract}
OEIS A154144 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A154144 is ``Indices k such that 13 plus the k-th triangular number is a perfect square.''. It has offset $1$ and begins
\[
2, 8, 23, 53, 138, 312, 807, 1821,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 23 2024 14:53:42, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{5} + 6 x^{4} - 3 x^{3} - 6 x^{2} - 2 x}{x^{5} - x^{4} - 6 x^{3} + 6 x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 3 x^{5} - 6 x^{4} + 3 x^{3} + 6 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A154144.
