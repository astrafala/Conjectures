...
\title{The empirical recurrence for OEIS A251743, derived from the entry's generating function}
...
\begin{abstract}
OEIS A251743 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A251743 is ``Pairs of nodes in a complete binary tree that are at an absolute height difference of less than 2 from each other.''. It has offset $1$ and begins
\[
3, 13, 49, 185, 713, 2793, 11049, 43945,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 30 2025 11:10:49, revision 24) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{8 x^{2} - 3 x}{8 x^{3} - 14 x^{2} + 7 x - 1},
\]
...
So the conjecture is not minimal: its polynomial $D(x)$ factors as the reduced denominator times $- x - 1$, and the sequence already satisfies a recurrence of order 3. The conjecture is true, but it is a consequence of a shorter one the entry does not state.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251743.
