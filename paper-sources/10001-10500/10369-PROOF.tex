...
\title{The empirical recurrence for OEIS A181278, derived from the entry's generating function}
...
\begin{abstract}
OEIS A181278 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A181278 is ``Number of 2 X n binary matrices M with rows in strictly increasing order and rows of M*Mtranspose (mod 2) in strictly decreasing order.''. It has offset $1$ and begins
\[
0, 3, 11, 48, 188, 768, 3056, 12288,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 27 2018 08:49:24, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{4} - x^{3} + 3 x^{2}}{16 x^{3} - 4 x^{2} - 4 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A181278.
