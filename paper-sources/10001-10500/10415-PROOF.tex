...
\title{The empirical recurrence for OEIS A063159, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063159 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A063159 is ``Dimension of the space of weight 2n cusp forms for Gamma\_0( 91 ).''. It has offset $1$ and begins
\[
7, 26, 46, 62, 82, 102, 118, 138,\ \dots
\]
...
\begin{quote}
G.f.: x*(1 + x)*(7 + 12*x + 8*x\^{}2 + x\^{}3) / ((1 - x)\^{}2*(1 + x + x\^{}2)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + 9 x^{4} + 20 x^{3} + 19 x^{2} + 7 x}{x^{4} - x^{3} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} + 9 x^{4} + 20 x^{3} + 19 x^{2} + 7 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A063159.
