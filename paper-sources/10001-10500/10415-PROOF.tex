...
\title{The empirical recurrence for OEIS A266794, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266794 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A266794 is ``Number of OFF (white) cells in the n-th iteration of the "Rule 61" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 1, 3, 2, 5, 4, 8, 4,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 21) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{8} - 2 x^{7} + x^{6} + x^{5} - x^{4} + 3 x^{2} + x}{x^{4} - 2 x^{2} + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266794.
