...
\title{The empirical recurrence for OEIS A184042, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184042 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A184042 is ``1/9 the number of (n+1) X 4 0..2 arrays with all 2 X 2 subblocks having the same four values.''. It has offset $1$ and begins
\[
25, 31, 41, 61, 97, 169, 305, 577,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 09:18:55, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 88 x^{4} + 52 x^{3} + 44 x^{2} - 25 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;88 x^{4} - 52 x^{3} - 44 x^{2} + 25 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184042.
