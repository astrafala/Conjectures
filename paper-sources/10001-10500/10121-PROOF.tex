...
\title{The empirical recurrence for OEIS A154140, derived from the entry's generating function}
...
\begin{abstract}
OEIS A154140 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 23 2024 14:53:42, revision 29) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{5} + 2 x^{4} - 3 x^{3} - 2 x^{2} - 2 x}{x^{5} - x^{4} - 6 x^{3} + 6 x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 3 x^{5} - 2 x^{4} + 3 x^{3} + 2 x^{2} + 2 x,
\]
