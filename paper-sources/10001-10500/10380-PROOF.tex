...
\title{The empirical recurrence for OEIS A253504, derived from the entry's generating function}
...
\begin{abstract}
OEIS A253504 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A253504 is ``Number of (n+2) X (2+2) 0..1 arrays with every 2 X 2 and 3X3 subblock diagonal maximum minus antidiagonal minimum nondecreasing horizontally and vertically.''. It has offset $1$ and begins
\[
260, 472, 784, 1196, 1712, 2340, 3088, 3964,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 16 2018 08:34:19, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{6} + 4 x^{5} - 148 x^{4} + 456 x^{3} - 568 x^{2} + 260 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;4 x^{6} + 4 x^{5} - 148 x^{4} + 456 x^{3} - 568 x^{2} + 260 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253504.
