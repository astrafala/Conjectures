...
\title{The empirical recurrence for OEIS A270008, derived from the entry's generating function}
...
\begin{abstract}
OEIS A270008 carries an empirical recurrence of order $7$. The entry separately records a
...
OEIS A270008 is ``Partial sums of the number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 5", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 9, 9, 58, 58, 179, 179, 404,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{7} - x^{6} - 2 x^{5} + 3 x^{4} + 25 x^{3} - 3 x^{2} + 8 x + 1}{x^{7} - x^{6} - 3 x^{5} + 3 x^{4} + 3 x^{3} - 3 x^{2} - x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270008.
