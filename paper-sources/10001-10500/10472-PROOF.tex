...
\title{The empirical recurrence for OEIS A228220, derived from the entry's generating function}
...
\begin{abstract}
OEIS A228220 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A228220 is ``Number of second differences of arrays of length 5 of numbers in 0..n.''. It has offset $1$ and begins
\[
31, 199, 625, 1429, 2731, 4651, 7309, 10825,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 10 2018 05:36:34, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} + 15 x^{3} + 75 x^{2} + 31 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{4} + 15 x^{3} + 75 x^{2} + 31 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A228220.
