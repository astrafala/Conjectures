...
\title{The empirical recurrence for OEIS A267055, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267055 carries an empirical recurrence of order $6$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{32 x^{5} + 16 x^{3} + x^{2} - 3 x - 1}{64 x^{6} - 84 x^{4} + 21 x^{2} - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 32 x^{5} - 16 x^{3} - x^{2} + 3 x + 1,
\]
