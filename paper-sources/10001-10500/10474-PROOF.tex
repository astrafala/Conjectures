...
\title{The empirical recurrence for OEIS A229013, derived from the entry's generating function}
...
\begin{abstract}
OEIS A229013 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A229013 is ``Number of arrays of median of three adjacent elements of some length-5 0..n array, with no adjacent equal elements in the latter.''. It has offset $1$ and begins
\[
2, 15, 46, 101, 186, 307, 470, 681,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 13 2018 11:00:25, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} - 2 x^{3} + 7 x^{2} + 2 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229013.
