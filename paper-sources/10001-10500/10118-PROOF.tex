...
\title{The empirical recurrence for OEIS A266607, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266607 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A266607 is ``Decimal representation of the n-th iteration of the "Rule 39" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 6, 2, 125, 4, 2043, 8, 32759,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = 19*a(n-2)-50*a(n-4)+32*a(n-6) for n>5.
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: (1+6*x-17*x\^{}2+11*x\^{}3+16*x\^{}4-32*x\^{}5) / ((1-x)*(1+x)*(1-4*x)*(1+4*x)*(1-2*x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 25) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{32 x^{5} - 16 x^{4} - 11 x^{3} + 17 x^{2} - 6 x - 1}{32 x^{6} - 50 x^{4} + 19 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 32 x^{6} + 50 x^{4} - 19 x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 32 x^{5} + 16 x^{4} + 11 x^{3} - 17 x^{2} + 6 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266607.
