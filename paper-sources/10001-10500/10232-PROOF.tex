...
\title{The empirical recurrence for OEIS A267233, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267233 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 05 2018 17:10:08, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{3} - 17 x^{2} - 6 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
