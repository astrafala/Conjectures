...
\title{The empirical recurrence for OEIS A229015, derived from the entry's generating function}
...
\begin{abstract}
OEIS A229015 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A229015 is ``Number of arrays of median of three adjacent elements of some length 7 0..n array, with no adjacent equal elements in the latter.''. It has offset $1$ and begins
\[
2, 57, 332, 1145, 3002, 6635, 13040, 23515,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 13 2018 12:06:45, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{6} + 2 x^{5} - 32 x^{4} + 20 x^{3} + 45 x^{2} + 2 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} + 2 x^{5} - 32 x^{4} + 20 x^{3} + 45 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229015.
