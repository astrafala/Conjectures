...
\title{The empirical recurrence for OEIS A253130, derived from the entry's generating function}
...
\begin{abstract}
OEIS A253130 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A253130 is ``Number of length 2+2 0..n arrays with the sum of medians of adjacent triples multiplied by some arrangement of +-1 equal to zero.''. It has offset $1$ and begins
\[
12, 53, 152, 345, 676, 1197, 1968, 3057,\ \dots
\]
...
\begin{quote}
G.f.: x*(3 - x)*(4 - x + 2*x\^{}2 - x\^{}3) / (1 - x)\^{}5.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 09 2018 07:27:36, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 5 x^{4} - 7 x^{3} + 7 x^{2} - 12 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} - 5 x^{4} + 7 x^{3} - 7 x^{2} + 12 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253130.
