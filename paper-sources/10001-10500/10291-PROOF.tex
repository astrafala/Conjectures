...
\title{The empirical recurrence for OEIS A233162, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233162 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:25, revision 22) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{29 x^{4} - 35 x^{3} + 11 x^{2} - x}{48 x^{3} - 44 x^{2} + 12 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 48 x^{3} + 44 x^{2} - 12 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;12\,a(n-1) - 44\,a(n-2) + 48\,a(n-3)
\]
for every $n>4$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 29 x^{4} + 35 x^{3} - 11 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233162.
