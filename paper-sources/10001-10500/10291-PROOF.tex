...
\title{The empirical recurrence for OEIS A024849, derived from the entry's generating function}
...
\begin{abstract}
OEIS A024849 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A024849 is ``a(n) = least m such that if r and s in {|F(h+1)-tau*F(h)|: h = 1,2,...,n} satisfy r < s, then r < k/m < s for some integer k, where F = A000045 (Fibonacci numbers) and tau = (1+sqrt(5))/2 (golden ratio).''. It has offset $2$ and begins
\[
2, 4, 6, 9, 14, 23, 36, 59,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 16 2026 09:57:23, revision 21) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{7} - x^{6} - 3 x^{5} - 2 x^{4} + 2 x^{3} + 2 x^{2}}{x^{4} + x^{3} - 2 x^{2} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{4} + x^{3} - 2 x^{2} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{7} - x^{6} - 3 x^{5} - 2 x^{4} + 2 x^{3} + 2 x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A024849.
