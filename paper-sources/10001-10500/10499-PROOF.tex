...
\title{The empirical recurrence for OEIS A262258, derived from the entry's generating function}
...
\begin{abstract}
OEIS A262258 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A262258 is ``a(n) = the number of hills (arch length of 1 with no covering arches) for semi-meander solutions with n arches and floor((n+2)/2) arch group returns to the x axis.''. It has offset $1$ and begins
\[
1, 2, 2, 4, 6, 10, 16, 24,\ \dots
\]
...
\begin{quote}
G.f.: x*(1+2*x-2*x\^{}2-4*x\^{}3+2*x\^{}4+2*x\^{}5) / (1-2*x\^{}2)\^{}2. (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 11 2025 07:58:58, revision 49) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{6} + 2 x^{5} - 4 x^{4} - 2 x^{3} + 2 x^{2} + x}{4 x^{4} - 4 x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{4} - 4 x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{6} + 2 x^{5} - 4 x^{4} - 2 x^{3} + 2 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262258.
