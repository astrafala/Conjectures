...
\title{The empirical recurrence for OEIS A126326, derived from the entry's generating function}
...
\begin{abstract}
OEIS A126326 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A126326 is ``a(1) = 1; for n>1, a(n) = smallest number which is not a sum or product or power of any subset of the numbers a(1) to a(n-1).''. It has offset $1$ and begins
\[
1, 2, 5, 9, 13, 31, 35, 92,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 21 2019 12:30:26, revision 21) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{210 x^{16} + 26 x^{15} - 472 x^{14} + 184 x^{13} - 114 x^{12} + 166 x^{11} - 10 x^{10} + 6 x^{9} - 2 x^{8} - 9 x^{7} + 5 x^{6} - x^{5} + 5 x^{4} + 3 x^{3} + 2 x^{2} + x}{2 x^{6} - 4 x^{4} - 2 x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;210 x^{16} + 26 x^{15} - 472 x^{14} + 184 x^{13} - 114 x^{12} + 166 x^{11} - 10 x^{10} + 6 x^{9} - 2 x^{8} - 9 x^{7} + 5 x^{6} - x^{5} + 5 x^{4} + 3 x^{3} + 2 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A126326.
