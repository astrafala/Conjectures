...
\title{The empirical recurrence for OEIS A266671, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266671 carries an empirical recurrence of order $6$. The entry separately records a
...
\begin{quote}
G.f.: (1+3*x-17*x\^{}2+20*x\^{}3+16*x\^{}4-128*x\^{}5) / ((1-x)*(1+x)*(1-4*x)*(1+4*x)*(1-8*x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 28) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{128 x^{5} - 16 x^{4} - 20 x^{3} + 17 x^{2} - 3 x - 1}{128 x^{6} - 152 x^{4} + 25 x^{2} - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 128 x^{5} + 16 x^{4} + 20 x^{3} - 17 x^{2} + 3 x + 1,
\]
