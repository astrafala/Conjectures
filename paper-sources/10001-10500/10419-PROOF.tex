...
\title{The empirical recurrence for OEIS A266984, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266984 carries an empirical recurrence of order $4$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{4} + 14 x^{3} + 7 x^{2} + x + 1}{16 x^{4} - 17 x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 8 x^{4} + 14 x^{3} + 7 x^{2} + x + 1,
\]
