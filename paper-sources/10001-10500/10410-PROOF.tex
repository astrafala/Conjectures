...
\title{The empirical recurrence for OEIS A266661, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266661 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A266661 is ``Decimal representation of the n-th iteration of the "Rule 47" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 6, 3, 124, 3, 2044, 3, 32764,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:28, revision 33) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{32 x^{5} - 32 x^{4} + 22 x^{3} - 14 x^{2} + 6 x + 1}{16 x^{4} - 17 x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;32 x^{5} - 32 x^{4} + 22 x^{3} - 14 x^{2} + 6 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266661.
