...
\title{The empirical recurrence for OEIS A185460, proved by transfer matrix}
...
\begin{abstract}
OEIS A185460 counts arrays over $\{0,\dots,1\}$ under a condition on the determinant of every
...
a bounded window of consecutive rows, so the count is a walk count on $S=34$ states and is
...
OEIS A185460 is ``Number of (n+1) X 3 binary arrays with no 2 X 2 subblock determinant equal to any horizontal or vertical neighbor 2 X 2 subblock determinant.''. It has offset $1$ and begins
\[
34, 92, 240, 672, 1646, 4452, 11050, 29506,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-2) + 4*a(n-3) + 4*a(n-4) - 8*a(n-5) - a(n-6) + 3*a(n-8) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 15 2018 08:59:14, revision 8) this is still
...
entry's first term is $34$, and the model reproduces it along with every later term; that
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-2) + 4\,a(n-3) + 4\,a(n-4) - 8\,a(n-5) - a(n-6) + 3\,a(n-8)
\]
for every $n>10$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A185460.
