...
\title{The empirical recurrence for OEIS A210377, derived from the entry's generating function}
...
\begin{abstract}
OEIS A210377 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A210377 is ``Number of 2 X 2 matrices with all terms in {0,1,...,n} and (sum of terms) = n + 5.''. It has offset $0$ and begins
\[
0, 0, 4, 31, 80, 146, 224, 315,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 16 2024 13:17:34, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{6 x^{7} - 4 x^{5} - 20 x^{4} + 15 x^{3} + 4 x^{2}}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;6 x^{7} - 4 x^{5} - 20 x^{4} + 15 x^{3} + 4 x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A210377.
