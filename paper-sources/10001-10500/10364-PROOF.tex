...
\title{The empirical recurrence for OEIS A152934, derived from the entry's generating function}
...
\begin{abstract}
OEIS A152934 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A152934 is ``Number of sets (in the Hausdorff metric geometry) at each location between two sets defining a polygonal configuration consisting of two m-gonal polygonal components chained with string components of length 3 as m varies.''. It has offset $2$ and begins
\[
289, 1962, 13429, 92025, 630730, 4323069, 29630737, 203092074,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 03 2023 10:54:04, revision 21) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 45 x^{4} + 350 x^{3} - 289 x^{2}}{x^{3} - 8 x^{2} + 8 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{3} + 8 x^{2} - 8 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;45 x^{4} - 350 x^{3} + 289 x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A152934.
