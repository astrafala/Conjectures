...
\title{The empirical recurrence for OEIS A253547, derived from the entry's generating function}
...
\begin{abstract}
OEIS A253547 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A253547 is ``Total number of star-shaped dodecagons appearing in a variant of hexagon expansion after n iterations.''. It has offset $1$ and begins
\[
0, 0, 0, 1, 3, 9, 16, 23,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 21 2025 11:15:51, revision 24) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{9} + 4 x^{8} - 3 x^{6} - x^{5} - x^{4}}{x^{4} - 2 x^{3} + 2 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{9} - 4 x^{8} + 3 x^{6} + x^{5} + x^{4},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253547.
