...
\title{The empirical recurrence for OEIS A269620, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269620 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A269620 is ``Number of length-4 0..n arrays with no repeated value differing from the previous repeated value by other than plus two, zero or minus 1.''. It has offset $1$ and begins
\[
15, 78, 249, 612, 1275, 2370, 4053, 6504,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 25 2019 08:28:12, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{4} - 9 x^{3} - 3 x^{2} - 15 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 3 x^{4} + 9 x^{3} + 3 x^{2} + 15 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269620.
