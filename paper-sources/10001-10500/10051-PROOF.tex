...
\title{The empirical recurrence for OEIS A200433, derived from the entry's generating function}
...
\begin{abstract}
OEIS A200433 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A200433 is ``Number of -n..n arrays x(0..5) of 6 elements with zero sum and no two or three adjacent elements summing to zero.''. It has offset $1$ and begins
\[
0, 212, 2232, 11008, 36952, 98052, 221984, 448224,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 21 2018 08:48:21, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{144 x^{5} + 796 x^{4} + 960 x^{3} + 212 x^{2}}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;144 x^{5} + 796 x^{4} + 960 x^{3} + 212 x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A200433.
