...
\title{The empirical recurrence for OEIS A200432, derived from the entry's generating function}
...
\begin{abstract}
OEIS A200432 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 20 2018 15:05:53, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 30 x^{4} - 120 x^{3} - 80 x^{2}}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
