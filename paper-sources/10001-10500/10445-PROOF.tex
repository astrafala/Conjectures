...
\title{The empirical recurrence for OEIS A243513, derived from the entry's generating function}
...
\begin{abstract}
OEIS A243513 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A243513 is ``Number of length n+2 0..4 arrays with no three elements in a row with pattern aba or abb (with a!=b) and new values 0..4 introduced in 0..4 order.''. It has offset $1$ and begins
\[
3, 5, 10, 24, 65, 187, 552, 1646,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 02 2018 12:27:31, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{3} + 10 x^{2} - 3 x}{3 x^{3} - 7 x^{2} + 5 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 3 x^{3} + 7 x^{2} - 5 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A243513.
