...
\title{The empirical recurrence for OEIS A267271, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267271 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A267271 is ``Decimal representation of the n-th iteration of the "Rule 115" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 5, 12, 111, 96, 1919, 768, 31743,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = 25*a(n-2) - 152*a(n-4) + 128*a(n-6) for n > 6.
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: (1 + 5*x - 13*x\^{}2 - 14*x\^{}3 - 52*x\^{}4 - 96*x\^{}5 + 64*x\^{}6) / ((1-x)*(1+x)*(1-4*x)*(1+4*x)*(1-8*x\^{}2)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 64 x^{6} + 96 x^{5} + 52 x^{4} + 14 x^{3} + 13 x^{2} - 5 x - 1}{128 x^{6} - 152 x^{4} + 25 x^{2} - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;64 x^{6} - 96 x^{5} - 52 x^{4} - 14 x^{3} - 13 x^{2} + 5 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267271.
