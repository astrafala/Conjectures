...
\title{The empirical recurrence for OEIS A237632, proved by transfer matrix}
...
\begin{abstract}
OEIS A237632 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=702$
...
OEIS A237632 is ``Number of (n+1) X (3+1) 0..2 arrays with the upper median of every 2 X 2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$ and begins
\[
702, 1220, 1310, 1488, 2086, 2900, 4132, 5720,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-4) for n>13.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 24 2018 06:02:19, revision 8) this is still
...
differ by $1$. The entry's first term $702$ counts those, and the model was made to
...
\]
over the $S=702$ pairs whose own subblock row already satisfies the condition along itself.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237632.
