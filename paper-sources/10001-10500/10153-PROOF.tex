...
\title{The empirical recurrence for OEIS A202330, derived from the entry's generating function}
...
\begin{abstract}
OEIS A202330 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A202330 is ``Number of (n+1) X 4 binary arrays with consecutive windows of two bits considered as a binary number nondecreasing in every row and column.''. It has offset $1$ and begins
\[
36, 82, 162, 289, 478, 746, 1112, 1597,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 27 2018 14:42:35, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 13 x^{5} + 61 x^{4} - 112 x^{3} + 98 x^{2} - 36 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;13 x^{5} - 61 x^{4} + 112 x^{3} - 98 x^{2} + 36 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202330.
