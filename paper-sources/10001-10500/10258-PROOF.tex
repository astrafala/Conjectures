...
\title{The empirical recurrence for OEIS A184045, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184045 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 12:06:12, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 600 x^{4} + 308 x^{3} + 300 x^{2} - 153 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;600 x^{4} - 308 x^{3} - 300 x^{2} + 153 x,
\]
