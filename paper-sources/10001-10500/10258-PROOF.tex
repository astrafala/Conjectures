...
\title{The empirical recurrence for OEIS A211911, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211911 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A211911 is ``Number of lower triangular n X n arrays colored with integers 0 upwards introduced in row major order, with no element equal to any horizontal, vertical, diagonal or antidiagonal neighbor, and containing the value n(n+1)/2-2.''. It has offset $1$ and begins
\[
0, 1, 6, 25, 70, 156, 301, 526,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 20 2018 09:36:57, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 5 x^{5} - 5 x^{4} - x^{3} - x^{2}}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{6} - 5 x^{5} + 5 x^{4} + x^{3} + x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211911.
