...
\title{The empirical recurrence for OEIS A229446, derived from the entry's generating function}
...
\begin{abstract}
OEIS A229446 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A229446 is ``Number of 3 X n 0..2 arrays with horizontal differences mod 3 never 1, vertical differences mod 3 never -1, and rows and columns lexicographically nondecreasing.''. It has offset $1$ and begins
\[
8, 13, 22, 37, 60, 93, 138, 197,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 17 2018 08:31:48, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 5 x^{4} + 18 x^{3} - 19 x^{2} + 8 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 5 x^{4} + 18 x^{3} - 19 x^{2} + 8 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229446.
