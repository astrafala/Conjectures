...
\title{The empirical recurrence for OEIS A267294, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267294 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A267294 is ``Decimal representation of the n-th iteration of the "Rule 121" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 1, 26, 59, 476, 751, 7024, 16319,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = a(n-2)+256*a(n-4)-256*a(n-6) for n>12.
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: (1 +x +25*x\^{}2 +58*x\^{}3 +194*x\^{}4 +436*x\^{}5 +148*x\^{}6 +720*x\^{}7 -7088*x\^{}8 +1856*x\^{}9 -15040*x\^{}10 -2816*x\^{}11 +21760*x\^{}12) / ((1-x)*(1+x)*(1-4*x)*(1+4*x)*(1+16*x\^{}2)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{21760 x^{12} - 2816 x^{11} - 15040 x^{10} + 1856 x^{9} - 7088 x^{8} + 720 x^{7} + 148 x^{6} + 436 x^{5} + 194 x^{4} + 58 x^{3} + 25 x^{2} + x + 1}{256 x^{6} - 256 x^{4} - x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;256 x^{6} - 256 x^{4} - x^{2} + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;21760 x^{12} - 2816 x^{11} - 15040 x^{10} + 1856 x^{9} - 7088 x^{8} + 720 x^{7} + 148 x^{6} + 436 x^{5} + 194 x^{4} + 58 x^{3} + 25 x^{2} + x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267294.
