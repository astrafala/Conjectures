...
\title{The empirical recurrence for OEIS A227021, derived from the entry's generating function}
...
\begin{abstract}
OEIS A227021 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A227021 is ``Number of n X 2 (0,1,2) arrays of permanents of 2 X 2 subblocks of some (n+1) X 3 binary array with rows and columns of the latter in lexicographically nondecreasing order.''. It has offset $1$ and begins
\[
7, 26, 72, 171, 368, 729, 1343, 2325,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 06 2018 11:30:19, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{8} - 6 x^{6} + 7 x^{5} - 11 x^{4} + 21 x^{3} - 16 x^{2} + 7 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{8} - 6 x^{6} + 7 x^{5} - 11 x^{4} + 21 x^{3} - 16 x^{2} + 7 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A227021.
