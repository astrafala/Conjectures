...
\title{The empirical recurrence for OEIS A243514, derived from the entry's generating function}
...
\begin{abstract}
OEIS A243514 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 02 2018 12:29:32, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 16 x^{4} + 33 x^{3} - 19 x^{2} + 3 x}{8 x^{4} - 22 x^{3} + 21 x^{2} - 8 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 16 x^{4} + 33 x^{3} - 19 x^{2} + 3 x,
\]
