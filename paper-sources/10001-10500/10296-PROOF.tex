...
\title{The empirical recurrence for OEIS A063099, derived from the entry's generating function}
...
\begin{abstract}
OEIS A063099 carries an empirical recurrence of order $4$. The entry separately records a
...
\begin{quote}
G.f.: x*(2 + 5*x + 6*x\^{}2 + 2*x\^{}3 + x\^{}4) / ((1 - x)\^{}2*(1 + x + x\^{}2)).
\end{quote}
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{5} + 2 x^{4} + 6 x^{3} + 5 x^{2} + 2 x}{x^{4} - x^{3} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} + 2 x^{4} + 6 x^{3} + 5 x^{2} + 2 x,
\]
