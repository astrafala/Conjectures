...
\title{The empirical recurrence for OEIS A221516, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221516 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A221516 is ``Number of 0..n arrays of length 5 with each element differing from at least one neighbor by 2 or more, starting with 0.''. It has offset $1$ and begins
\[
0, 3, 30, 130, 381, 884, 1765, 3174,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 06 2018 05:35:41, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{8} - 2 x^{7} + 6 x^{6} - x^{5} - 10 x^{4} - 15 x^{3} - 3 x^{2}}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{8} + 2 x^{7} - 6 x^{6} + x^{5} + 10 x^{4} + 15 x^{3} + 3 x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221516.
