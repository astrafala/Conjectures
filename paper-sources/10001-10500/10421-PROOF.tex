...
\title{The empirical recurrence for OEIS A267048, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267048 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A267048 is ``Number of OFF (white) cells in the n-th iteration of the "Rule 91" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
0, 1, 3, 3, 4, 3, 8, 3,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:29, revision 17) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{6} - 2 x^{5} - 2 x^{4} + x^{3} + 3 x^{2} + x}{x^{4} - 2 x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{6} - 2 x^{5} - 2 x^{4} + x^{3} + 3 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267048.
