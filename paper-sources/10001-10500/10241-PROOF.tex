...
\title{The empirical closed form for OEIS A220030, proved}
...
\begin{abstract}
OEIS A220030 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=95$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220030 is ``Number of n X 6 arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 n X 6 array.''. It has offset $1$ and begins
\[
6, 15, 42, 95, 192, 358, 626, 1038,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/120)*n\^{}5 + (1/8)*n\^{}4 + (5/24)*n\^{}3 + (15/8)*n\^{}2 + (227/60)*n - 4 for n>1.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 05:51:31, revision 7)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=95$
...
states number $S=95$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220030 records is correct for all $n>1$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220030.
