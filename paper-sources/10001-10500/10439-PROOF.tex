...
\title{A proof of the conjectured recurrence for OEIS A163824}
...
\begin{abstract}
OEIS A163824 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 6 x^{3} + 2 x^{2} - 6 x$, a polynomial of degree $3$, which proves
...
OEIS A163824 is ``Number of permutations of length n in the 2x2 double-chevron permutation grid class''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 6, 24, 106, 470, 2038,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x \sqrt{1 - 4 x} \left(x - 1\right) + \left(2 x - 1\right) \left(3 x - 1\right)}{\sqrt{1 - 4 x} \left(2 x - 1\right) \left(3 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n\^{}2-6*n+11)*a(n) +(-9*n\^{}3+56*n\^{}2-119*n+60)*a(n-1) +2*(13*n\^{}3-83*n\^{}2+193*n-150)*a(n-2) -12*(2*n-5)*(n\^{}2-4*n+6)*a(n-3) =0 . - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2016-06-09, revision 25) the
...
\[
p_{0}(n)=n \left(n^{2} - 6 n + 11\right),\qquad p_{1}(n)=- 9 n^{3} + 56 n^{2} - 119 n + 60,\qquad p_{2}(n)=2 \left(13 n^{3} - 83 n^{2} + 193 n - 150\right),\qquad p_{3}(n)=- 12 \left(2 n - 5\right) \left(n^{2} - 4 n + 6\right)
\]
...
\section{The computation for A163824}
...
\[
n \left(n^{2} - 6 n + 11\right)\,a(n) + (- 9 n^{3} + 56 n^{2} - 119 n + 60)\,a(n-1) + 2 \left(13 n^{3} - 83 n^{2} + 193 n - 150\right)\,a(n-2) -  12 \left(2 n - 5\right) \left(n^{2} - 4 n + 6\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A163824, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 6 x^{3} + 2 x^{2} - 6 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A163824.
