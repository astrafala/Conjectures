...
\title{The empirical recurrence for OEIS A187165, derived from the entry's generating function}
...
\begin{abstract}
OEIS A187165 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A187165 is ``Number of 4-step self-avoiding walks on an n X n X n cube summed over all starting positions.''. It has offset $1$ and begins
\[
0, 96, 1104, 3984, 9612, 18888, 32712, 51984,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 05 2022 22:11:44, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{24 x^{6} - 84 x^{5} + 144 x^{4} + 720 x^{3} + 96 x^{2}}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;24 x^{6} - 84 x^{5} + 144 x^{4} + 720 x^{3} + 96 x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187165.
