...
\title{The empirical recurrence for OEIS A018918, derived from the entry's generating function}
...
\begin{abstract}
OEIS A018918 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A018918 is ``Define the generalized Pisot sequence T(a(0),a(1)) by: a(n+2) is the greatest integer such that a(n+2)/a(n+1) < a(n+1)/a(n). This is T(3,6).''. It has offset $0$ and begins
\[
3, 6, 11, 20, 36, 64, 113, 199,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 08 2022 08:44:44, revision 42) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{3} + 2 x^{2} - 3 x + 3}{x^{4} - 2 x^{3} + 3 x^{2} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{4} - 2 x^{3} + 3 x^{2} - 3 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A018918.
