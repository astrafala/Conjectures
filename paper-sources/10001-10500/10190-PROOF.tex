...
\title{The empirical recurrence for OEIS A249852, derived from the entry's generating function}
...
\begin{abstract}
OEIS A249852 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A249852 is ``a(n) is the total number of pentagons on the left or the right of the vertical symmetry axis of a pentagon expansion (vertex to vertex) after n iterations.''. It has offset $0$ and begins
\[
0, 2, 7, 14, 23, 35, 50, 67,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 23 2025 11:20:59, revision 29) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} - x^{3} - x^{2} - 2 x}{x^{5} - 3 x^{4} + 4 x^{3} - 4 x^{2} + 3 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A249852.
