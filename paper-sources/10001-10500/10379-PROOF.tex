...
\title{The empirical recurrence for OEIS A203650, proved by transfer matrix}
...
\begin{abstract}
OEIS A203650 counts $(n+1)\times3$ arrays over $\{0,\dots,4\}$ subject to a condition imposed on
every $2\times2$ subblock, and carries an empirical recurrence of order $2$ contributed
...
number of walks: $a(n)=\tfrac1{25}\mathbf{1}^{\!\top}M^{n}\mathbf{1}$. Writing $q$
...
OEIS A203650 is ``1/25 the number of (n+1) X 3 0..4 arrays with every 2 X 2 subblock having equal diagonal elements or equal antidiagonal elements.''. It has offset $1$ and begins
\[
81, 1517, 28057, 519445, 9616161, 178019197, 3295578857, 61009378085,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 17*a(n-1) +28*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 04 2018 12:08:44, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;17\,a(n-1) + 28\,a(n-2)
\end{equation}
for all $n>1$.
...
\[
a(n)\;=\;\tfrac1{25}\mathbf{1}^{\!\top}M^{n}\mathbf{1} .
\]
\end{lemma}
...
length $k$ from $r$ to $s$. The entry's index $n$ corresponds to $L=n+1$ rows. The entry counts $\tfrac1{25}$ of those arrays, a constant factor that passes through every step below unchanged.
\end{proof}
...
For every $n$ with $n+1-1\ge2$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\tfrac1{25}\mathbf{1}^{\!\top}M^{\,n+1-1-2}\,q(M)\,\mathbf{1} .
\]
...
\begin{proof}
By Lemma~\ref{lem:walk}, $a(n-i)=\tfrac1{25}\mathbf{1}^{\!\top}M^{\,n+1-1-i}\mathbf{1}$, so
\[
a(n)-\sum_i c_i a(n-i)
=\tfrac1{25}\mathbf{1}^{\!\top}\Bigl(M^{m}-\sum_i c_i M^{m-i}\Bigr)\mathbf{1}
=\tfrac1{25}\mathbf{1}^{\!\top}M^{\,m-2}\,q(M)\,\mathbf{1}
\]
with $m=n+1-1$, which is the stated identity; the scalar $\tfrac1{25}$ never vanishes, so it does not affect whether the left side is zero, and the equivalence follows by letting
$m-2=j$ run over $\ge0$. For the last clause put $w=q(M)\mathbf{1}$. By the
...
$S^2$ pairs. The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was then formed by $2$
...
index corresponding to $n=1+1$ onwards, and the run continued until $w$ itself was the
...
$\tfrac1{25}\mathbf{1}^{\!\top}M^{L-1}\mathbf{1}$ compared against the entry's DATA at
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203650.
