...
\title{The empirical recurrence for OEIS A178297, derived from the entry's generating function}
...
\begin{abstract}
OEIS A178297 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A178297 is ``Number of collinear point 6-tuples in a 7 X 7 X 7 X... n-dimensional cubic grid.''. It has offset $0$ and begins
\[
0, 7, 116, 1459, 16520, 177727, 1861436, 19230379,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = 27*a(n-1) - 239*a(n-2) + 693*a(n-3).
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: x*(-7+73*x)/((11*x-1)*(7*x-1)*(9*x-1)). (End)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 17 2025 23:12:20, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{73 x^{2} - 7 x}{693 x^{3} - 239 x^{2} + 27 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 693 x^{3} + 239 x^{2} - 27 x + 1
\]
...
compared against all $9$ terms the entry publishes: it reproduces every one. This is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A178297.
