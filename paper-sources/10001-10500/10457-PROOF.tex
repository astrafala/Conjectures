...
\title{The empirical recurrence for OEIS A201975, derived from the entry's generating function}
...
\begin{abstract}
OEIS A201975 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A201975 is ``Number of n X 2 0..3 arrays with rows and columns lexicographically nondecreasing read forwards and nonincreasing read backwards.''. It has offset $1$ and begins
\[
4, 30, 72, 131, 208, 304, 420, 557,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 25 2018 11:14:01, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{7 x^{4} - 24 x^{3} + 14 x^{2} + 4 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;7 x^{4} - 24 x^{3} + 14 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A201975.
