...
\title{A condition on each element and its neighbours: OEIS A183435}
...
\begin{abstract}
OEIS A183435 carries an empirical recurrence of order $3$ contributed by R.~H.~Hardin. It is
...
OEIS A183435 is ``Number of n X 2 binary arrays with every 1 having exactly one king-move neighbor equal to 1.''. It has offset $1$ and begins
\[
2, 7, 13, 28, 69, 149, 330, 755,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + a(n-2) + 4*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 28 2018 03:12:38, revision 12) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $2$ columns over the alphabet $\{0,1\}$, growing one row at a time. Its condition is that every entry equal to $1$ must have exactly $1$ of its eight king-move neighbours equal to $1$.
...
Vertices with identical futures are merged, which leaves $S=3$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183435.
