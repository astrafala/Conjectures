...
\title{A proof of the conjectured recurrence for OEIS A172025}
...
\begin{abstract}
OEIS A172025 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A172025 is ``Expansion of (2/(3*sqrt(1-4*z)-1+4*z))*((1-sqrt(1-4*z))/(2*z))\textasciicircum{}k with k=3''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 16, 62, 239, 920, 3544, 13672,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = [x\textasciicircum{}n] 1/((1 - x\textasciicircum{}2)*(1 - x)\textasciicircum{}(n+3)).
\end{quote}
that is,
\begin{equation}\label{eq:def}
a(n)\;=\;[x^{n}]\,f(x)\,g(x)^{n},
\qquad f(x)=\frac{1}{x^{5} - 3 x^{4} + 2 x^{3} + 2 x^{2} - 3 x + 1},\qquad g(x)=- \frac{1}{x - 1} .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} 2*n*(n+3)*a(n) + (-7*n\textasciicircum{}2 - 17*n - 8)*a(n-1) -2*(n+2)*(2*n+1)*a(n-2) = 0. - \emph{R. J. Mathar}, Feb 19 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-03-13, revision 46) the
...
For A172025 this yields
\[
4 t^{5} y^{2} + 7 t^{4} y^{2} - 2 t^{3} y^{2} + 4 t y - y + 1 \;=\;0 ,
\]
...
\[
2 n \left(n + 3\right)\,a(n) + (- 7 n^{2} - 17 n - 8)\,a(n-1) -  2 \left(n + 2\right) \left(2 n + 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
The result was then compared term by term with the DATA section of A172025, agreeing on
...
all 22 values of $n$ from $n=2$ upward.
...
\url{https://oeis.org}, 2026. Sequence A172025.
