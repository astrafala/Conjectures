...
\title{The empirical recurrence for OEIS A203292, derived from the entry's generating function}
...
\begin{abstract}
OEIS A203292 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A203292 is ``Number of arrays of 4 nondecreasing integers in -n..n with sum zero and equal numbers greater than zero and less than zero.''. It has offset $1$ and begins
\[
3, 6, 12, 21, 35, 54, 80, 113,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 04 2018 04:29:16, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 3 x^{4} - 3 x^{2} + 3 x}{x^{5} - 3 x^{4} + 2 x^{3} + 2 x^{2} - 3 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203292.
