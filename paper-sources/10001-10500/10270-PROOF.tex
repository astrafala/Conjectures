...
\title{The empirical recurrence for OEIS A229004, derived from the entry's generating function}
...
\begin{abstract}
OEIS A229004 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A229004 is ``Indices of Bell numbers divisible by 3.''. It has offset $1$ and begins
\[
4, 8, 9, 11, 17, 21, 22, 24,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 30 2026 16:40:41, revision 30) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{5} + 2 x^{4} + x^{3} + 4 x^{2} + 4 x}{x^{5} - x^{4} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{5} + 2 x^{4} + x^{3} + 4 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229004.
