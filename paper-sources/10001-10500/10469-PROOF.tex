...
\title{The empirical recurrence for OEIS A221574, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221574 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A221574 is ``Number of 0..n arrays of length 3 with each element differing from at least one neighbor by something other than 1.''. It has offset $1$ and begins
\[
2, 9, 26, 59, 114, 197, 314, 471,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 08 2018 15:19:04, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} + 2 x^{3} + x^{2} + 2 x}{x^{4} - 4 x^{3} + 6 x^{2} - 4 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221574.
