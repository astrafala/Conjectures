...
\title{The empirical recurrence for OEIS A157792, derived from the entry's generating function}
...
\begin{abstract}
OEIS A157792 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A157792 is ``Least number of lattice points on one side from which every point of a square n X n lattice is visible.''. It has offset $1$ and begins
\[
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:09, revision 18) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{12} - x^{10} - x^{8} + x^{7} - x^{6} + x^{5} - x^{4} + x^{3} + x}{x^{4} - x^{3} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{12} - x^{10} - x^{8} + x^{7} - x^{6} + x^{5} - x^{4} + x^{3} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A157792.
