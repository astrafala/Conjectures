...
\title{The empirical recurrence for OEIS A065759, derived from the entry's generating function}
...
\begin{abstract}
OEIS A065759 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A065759 is ``For a number k of length L, let f(k) be the sum of the products of the first i digits of k multiplied by the last L-i digits, for i from 1 to L-1, e.g., f(1234) = 1*234 + 12*34 + 123*4 = 1134. Sequence gives k such that f(k) = k.''. It has offset $1$ and begins
\[
0, 655, 1461, 1642, 2361, 3442, 6550, 14610,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 12 2023 20:46:36, revision 38) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n-1}\;=\;\frac{- 3442 x^{5} - 2361 x^{4} - 1642 x^{3} - 1461 x^{2} - 655 x}{10 x^{5} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - 10 x^{5}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3442 x^{5} + 2361 x^{4} + 1642 x^{3} + 1461 x^{2} + 655 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A065759.
