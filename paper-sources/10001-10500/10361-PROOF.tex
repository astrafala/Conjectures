...
\title{The empirical recurrence for OEIS A184034, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184034 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A184034 is ``1/16 the number of (n+1) X 5 0..3 arrays with all 2 X 2 subblocks having the same four values.''. It has offset $1$ and begins
\[
91, 103, 124, 166, 244, 400, 700, 1300,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 05:25:21, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 340 x^{4} + 185 x^{3} + 170 x^{2} - 91 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;340 x^{4} - 185 x^{3} - 170 x^{2} + 91 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184034.
