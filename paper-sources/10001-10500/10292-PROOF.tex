...
\title{The empirical recurrence for OEIS A027984, derived from the entry's generating function}
...
\begin{abstract}
OEIS A027984 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A027984 is ``a(n) = Sum\_{k=0..n} T(n, k)*T(n, n+k), T given by A027960.''. It has offset $0$ and begins
\[
1, 6, 20, 58, 161, 436, 1165, 3088,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 10 2025 08:58:42, revision 15) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{3} + x^{2} - 2 x - 1}{x^{4} - 2 x^{3} - 3 x^{2} + 4 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A027984.
