...
\title{The empirical recurrence for OEIS A220639, derived from the entry's generating function}
...
\begin{abstract}
OEIS A220639 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A220639 is ``Number of ways to reciprocally link elements of an n X 3 array either to themselves or to exactly one king-move neighbor.''. It has offset $0$ and begins
\[
1, 3, 40, 369, 3755, 37320, 373177, 3725843,\ \dots
\]
...
\begin{quote}
G.f.: -(x\^{}4-3*x\^{}3+6*x\^{}2+5*x-1)/((x-1)*(3*x\^{}5-7*x\^{}4+9*x\^{}3+29*x\^{}2+7*x-1)). - \_Alois P. Heinz\_, Jun 03 2014
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 03 2023 15:23:24, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} + 3 x^{3} - 6 x^{2} - 5 x + 1}{3 x^{6} - 10 x^{5} + 16 x^{4} + 20 x^{3} - 22 x^{2} - 8 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;3 x^{6} - 10 x^{5} + 16 x^{4} + 20 x^{3} - 22 x^{2} - 8 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;8\,a(n-1) + 22\,a(n-2) - 20\,a(n-3) - 16\,a(n-4) + 10\,a(n-5) - 3\,a(n-6)
\]
for every $n>4$.
\end{theorem}
...
\[
3 x^{6} - 10 x^{5} + 16 x^{4} + 20 x^{3} - 22 x^{2} - 8 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220639.
