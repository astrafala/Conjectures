...
\title{The empirical recurrence for OEIS A267262, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267262 carries an empirical recurrence of order $5$. The entry separately records a
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{8} + 2 x^{7} - x^{6} - x^{5} - 2 x^{2} - x}{x^{5} - x^{4} - 2 x^{3} + 2 x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{8} - 2 x^{7} + x^{6} + x^{5} + 2 x^{2} + x,
\]
