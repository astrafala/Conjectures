...
\title{A proof of the conjectured recurrence for OEIS A163493}
...
\begin{abstract}
OEIS A163493 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 x^{3} - 2 x^{2} + x$, a polynomial of degree $3$, which proves
...
OEIS A163493 is ``Number of binary strings of length n which have the same number of 00 and 01 substrings''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 2, 3, 6, 9, 15, 30,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- \sqrt{\left(x - 1\right) \left(2 x - 1\right) \left(2 x^{2} + x + 1\right)} + \left(x - 1\right) \left(2 x + 1\right)}{2 \sqrt{\left(x - 1\right) \left(2 x - 1\right) \left(2 x^{2} + x + 1\right)} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -n*a(n) +(2+n)*a(n-1) +(3n-12)*a(n-2) +(12-n)*a(n-3) +(2n-18)*a(n-4)+(56-12n)*a(n-5) +(8n-40)*a(n-6)=0. - \emph{R. J. Mathar}, Nov 28 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2026-03-13, revision 62) the
...
\[
p_{0}(n)=- n,\qquad p_{1}(n)=n + 2,\qquad p_{2}(n)=3 \left(n - 4\right),\qquad p_{3}(n)=12 - n,\qquad p_{4}(n)=2 \left(n - 9\right),\qquad p_{5}(n)=- 4 \left(3 n - 14\right),\qquad p_{6}(n)=8 \left(n - 5\right)
\]
...
\section{The computation for A163493}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x - 1\right) \left(2 x - 1\right) \left(2 x^{2} + x + 1\right) .
\]
...
\[
- n\,a(n) + (n + 2)\,a(n-1) + 3 \left(n - 4\right)\,a(n-2) + (12 - n)\,a(n-3) + 2 \left(n - 9\right)\,a(n-4) -  4 \left(3 n - 14\right)\,a(n-5) + 8 \left(n - 5\right)\,a(n-6) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A163493, agreeing on all 13 terms compared.
...
vanish, for all 31 values of $n$ from $n=6$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=4 x^{3} - 2 x^{2} + x$ is an
...
\url{https://oeis.org}, 2026. Sequence A163493.
