...
\title{The empirical recurrence for OEIS A187157, derived from the entry's generating function}
...
\begin{abstract}
OEIS A187157 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 20 2018 09:28:07, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{4 x^{5} - 68 x^{4} - 8 x^{3}}{x^{3} - 3 x^{2} + 3 x - 1},
\]
