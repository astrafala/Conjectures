...
\title{The empirical recurrence for OEIS A269607, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269607 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 25 2019 08:28:57, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{4} + 8 x^{3} - 22 x^{2} - 8 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{4} - 8 x^{3} + 22 x^{2} + 8 x,
\]
