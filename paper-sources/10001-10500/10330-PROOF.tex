...
\title{The empirical recurrence for OEIS A268262, derived from the entry's generating function}
...
\begin{abstract}
OEIS A268262 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A268262 is ``Number of length-(3+1) 0..n arrays with new repeated values introduced in sequential order starting with zero.''. It has offset $1$ and begins
\[
9, 42, 143, 378, 837, 1634, 2907, 4818,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 13 2019 08:48:49, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{5} + 7 x^{4} - 23 x^{3} + 3 x^{2} - 9 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{5} - 7 x^{4} + 23 x^{3} - 3 x^{2} + 9 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268262.
