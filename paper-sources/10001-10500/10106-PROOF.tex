...
\title{The empirical recurrence for OEIS A254700, derived from the entry's generating function}
...
\begin{abstract}
OEIS A254700 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A254700 is ``Number of length 2+4 0..n arrays with every five consecutive terms having the maximum of some two terms equal to the minimum of the remaining three terms.''. It has offset $1$ and begins
\[
34, 267, 1116, 3341, 8142, 17255, 33048, 58617,\ \dots
\]
...
\begin{quote}
G.f.: x*(34 + 63*x + 24*x\^{}2 - 30*x\^{}3 + 6*x\^{}4 - x\^{}5) / (1 - x)\^{}6.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 02 2025 12:14:58, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 6 x^{5} - 30 x^{4} + 24 x^{3} + 63 x^{2} + 34 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + 6 x^{5} - 30 x^{4} + 24 x^{3} + 63 x^{2} + 34 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A254700.
