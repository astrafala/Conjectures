...
\title{The empirical recurrence for OEIS A005908, derived from the entry's generating function}
...
\begin{abstract}
OEIS A005908 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A005908 is ``a(n) = floor( phi*a(n-1) ) + floor( phi*a(n-2) ), where phi is the golden ratio.''. It has offset $0$ and begins
\[
0, 1, 1, 2, 4, 9, 20, 46,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 15 2022 09:51:12, revision 27) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} - x^{3} - x^{2} + x}{x^{5} + x^{4} - x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;x^{5} + x^{4} - x^{2} - 2 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A005908.
