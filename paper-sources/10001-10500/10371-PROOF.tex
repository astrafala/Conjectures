...
\title{The empirical recurrence for OEIS A183897, derived from the entry's generating function}
...
\begin{abstract}
OEIS A183897 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A183897 is ``Number of nondecreasing arrangements of n+3 numbers in 0..2 with each number being the sum mod 3 of three others.''. It has offset $1$ and begins
\[
1, 7, 17, 25, 34, 44, 55, 67,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 05 2018 09:52:28, revision 14) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{5} + 6 x^{4} + x^{3} - 4 x^{2} - x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;3 x^{5} - 6 x^{4} - x^{3} + 4 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183897.
