...
\title{The empirical recurrence for OEIS A228664, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A228664 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $7$ contributed
...
OEIS A228664 is ``Number of 5 X n binary arrays with top left value 1 and no two ones adjacent horizontally, diagonally or antidiagonally.''. It has offset $1$ and begins
\[
16, 29, 574, 2228, 24109, 128740, 1096624, 6780585,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) + 34*a(n-2) - 76*a(n-3) - 134*a(n-4) + 258*a(n-5) + 45*a(n-6) - 102*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 12 2018 09:07:47, revision 12) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) + 34\,a(n-2) - 76\,a(n-3) - 134\,a(n-4) + 258\,a(n-5) + 45\,a(n-6) - 102\,a(n-7)
\end{equation}
for all $n>8$.
...
\text{no two cells both equal to }1\text{ are adjacent along an offset of }\mathcal N
\end{equation}
for every cell $(t,u)$.
...
publishes, $n=19$, the arrays have $5\cdot19$ cells over
...
$o=1n_0+0-2$. Then for $j\ge7$,
\[
a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\;=\;\,\mathbf{v}^{\!\top}G^{\,j-7}w,
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-7}$ out of
...
the middle column. The vector $w$ was formed by $7$ applications of $G$ in exact integer
arithmetic and $\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly.
Every one of those integers vanishes from the index corresponding to $n=8+1$ onwards,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A228664.
