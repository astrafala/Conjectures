...
\title{The empirical recurrence for OEIS A229795, derived from the entry's generating function}
...
\begin{abstract}
OEIS A229795 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A229795 is ``Number of 2 X 2 0..n arrays with rows and columns in lexicographically nondecreasing order.''. It has offset $1$ and begins
\[
7, 29, 86, 205, 421, 777, 1324, 2121,\ \dots
\]
...
\begin{quote}
G.f.: x*(7 - 6*x + 11*x\^{}2 - 5*x\^{}3 + x\^{}4) / (1 - x)\^{}5.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 21 2018 08:46:40, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 5 x^{4} - 11 x^{3} + 6 x^{2} - 7 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{5} - 5 x^{4} + 11 x^{3} - 6 x^{2} + 7 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229795.
