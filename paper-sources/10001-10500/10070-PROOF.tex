...
\title{The empirical recurrence for OEIS A211924, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211924 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A211924 is ``Number of lower triangular n X n arrays colored with integers 0 upwards introduced in row major order, with no element equal to any horizontal, vertical or antidiagonal neighbor, and containing the value n(n+1)/2-2.''. It has offset $1$ and begins
\[
0, 2, 9, 31, 80, 171, 322, 554,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 07:52:15, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 5 x^{5} - 6 x^{4} + x^{3} - 2 x^{2}}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{7} + 6 x^{6} - 11 x^{5} + 7 x^{4} - 3 x^{3} + 2 x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211924.
