...
\title{The empirical recurrence for OEIS A202254, derived from the entry's generating function}
...
\begin{abstract}
OEIS A202254 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 03 2018 05:35:49, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 4 x^{4} + 2 x^{3} + 10 x^{2} + 7 x}{x^{5} - 3 x^{4} + 2 x^{3} + 2 x^{2} - 3 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{5} + 4 x^{4} + 2 x^{3} + 10 x^{2} + 7 x,
\]
