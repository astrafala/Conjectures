...
\title{The empirical recurrence for OEIS A124870, derived from the entry's generating function}
...
\begin{abstract}
OEIS A124870 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A124870 is ``Denominator of real part of (3*i - 1)\^{}(-n).''. It has offset $0$ and begins
\[
1, 10, 25, 500, 2500, 25000, 31250, 1250000,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 27 2025 09:49:10, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{31250 x^{6} - 500 x^{3} - 25 x^{2} - 10 x - 1}{2500 x^{4} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - 2500 x^{4}
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 31250 x^{6} + 500 x^{3} + 25 x^{2} + 10 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A124870.
