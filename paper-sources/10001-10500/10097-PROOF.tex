...
\title{The empirical recurrence for OEIS A208995, derived from the entry's generating function}
...
\begin{abstract}
OEIS A208995 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 07 2018 19:31:24, revision 16) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{5} + 3 x^{4} - x^{3} + x^{2} + 2 x}{x^{5} - 3 x^{4} + 2 x^{3} + 2 x^{2} - 3 x + 1},
\]
