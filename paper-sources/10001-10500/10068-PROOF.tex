...
\title{The empirical recurrence for OEIS A208971, derived from the entry's generating function}
...
\begin{abstract}
OEIS A208971 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A208971 is ``Number of 4-bead necklaces labeled with numbers -n..n allowing reversal, with sum zero and first and second differences in -n..n.''. It has offset $1$ and begins
\[
1, 3, 4, 8, 11, 18, 24, 35,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 07 2018 08:51:21, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{6} + 2 x^{5} + x^{4} - 3 x^{3} + x^{2} + x}{x^{6} - 2 x^{5} - x^{4} + 4 x^{3} - x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{6} + 2 x^{5} + x^{4} - 3 x^{3} + x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208971.
