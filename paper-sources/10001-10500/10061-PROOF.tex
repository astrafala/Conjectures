...
\title{The empirical recurrence for OEIS A261950, derived from the entry's generating function}
...
\begin{abstract}
OEIS A261950 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A261950 is ``Start with a single equilateral triangle for n=0; for the odd n-th generation add a triangle at each expandable vertex of the triangles of the (n-1)-th generation (this is the "side to vertex" version); for the even n-th generation use the "vertex to vertex" version; a(n) is the number of triangles added in the n-th generation.''. It has offset $0$ and begins
\[
1, 3, 9, 12, 30, 18, 45, 27,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 13 2015 13:14:34, revision 30) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{7 x^{6} + 3 x^{5} + 20 x^{4} + 9 x^{3} + 8 x^{2} + 3 x + 1}{x^{6} - x^{4} - x^{2} + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;7 x^{6} + 3 x^{5} + 20 x^{4} + 9 x^{3} + 8 x^{2} + 3 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A261950.
