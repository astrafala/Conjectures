...
\title{The empirical recurrence for OEIS A266250, derived from the entry's generating function}
...
\begin{abstract}
OEIS A266250 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A266250 is ``Total number of ON (black) cells after n iterations of the "Rule 9" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 1, 3, 5, 9, 14, 18, 27,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{7} + 2 x^{6} - x^{5} - x^{4} - 2 x^{3} - 1}{x^{5} - x^{4} - 2 x^{3} + 2 x^{2} + x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{7} - 2 x^{6} + x^{5} + x^{4} + 2 x^{3} + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A266250.
