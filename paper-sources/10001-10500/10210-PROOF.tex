...
\title{The empirical recurrence for OEIS A146123, derived from the entry's generating function}
...
\begin{abstract}
OEIS A146123 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A146123 is ``Number of n X n arrays of squares of integers with every (n-3) X (n-3) subblock summing to 4 and every element equal to at least one neighbor.''. It has offset $5$ and begins
\[
1, 540, 4120, 12188, 31000, 67216, 129464, 228136,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 23 2018 12:28:33, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{204 x^{12} - 1564 x^{11} + 4405 x^{10} - 5865 x^{9} + 3022 x^{8} - 1430 x^{7} - 535 x^{6} - x^{5}}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{5} + 5 x^{4} - 10 x^{3} + 10 x^{2} - 5 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 204 x^{12} + 1564 x^{11} - 4405 x^{10} + 5865 x^{9} - 3022 x^{8} + 1430 x^{7} + 535 x^{6} + x^{5},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A146123.
