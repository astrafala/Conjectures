...
\title{The empirical recurrence for OEIS A267234, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267234 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A267234 is ``Number of length-5 0..n arrays with no following elements greater than or equal to the first repeated value.''. It has offset $1$ and begins
\[
7, 103, 564, 1980, 5375, 12327, 25088, 46704,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 10 2019 15:22:16, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{4} + 51 x^{3} + 61 x^{2} + 7 x}{x^{6} - 6 x^{5} + 15 x^{4} - 20 x^{3} + 15 x^{2} - 6 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267234.
