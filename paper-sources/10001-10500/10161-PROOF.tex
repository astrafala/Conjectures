...
\title{The empirical recurrence for OEIS A267855, derived from the entry's generating function}
...
\begin{abstract}
OEIS A267855 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A267855 is ``Decimal representation of the n-th iteration of the "Rule 230" elementary cellular automaton starting with a single ON (black) cell.''. It has offset $0$ and begins
\[
1, 6, 20, 120, 368, 1888, 5952, 30592,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 128 x^{5} + 32 x^{4} + 24 x^{3} + 4 x^{2} + 6 x + 1}{256 x^{6} - 16 x^{4} - 16 x^{2} + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;256 x^{6} - 16 x^{4} - 16 x^{2} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;16\,a(n-2) + 16\,a(n-4) - 256\,a(n-6)
\]
for every $n>5$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 128 x^{5} + 32 x^{4} + 24 x^{3} + 4 x^{2} + 6 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A267855.
