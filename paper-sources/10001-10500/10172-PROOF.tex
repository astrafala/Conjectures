...
\title{The empirical recurrence for OEIS A225894, derived from the entry's generating function}
...
\begin{abstract}
OEIS A225894 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A225894 is ``Number of n X 2 binary arrays whose sum with another n X 2 binary array containing no more than a single 1 has rows and columns in lexicographically nondecreasing order.''. It has offset $1$ and begins
\[
4, 12, 33, 78, 162, 304, 527, 858,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 05 2018 08:26:18, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{5} + 7 x^{4} - 13 x^{3} + 8 x^{2} - 4 x}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{5} - 7 x^{4} + 13 x^{3} - 8 x^{2} + 4 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A225894.
