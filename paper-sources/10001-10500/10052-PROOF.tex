...
\title{The empirical recurrence for OEIS A140829, derived from the entry's generating function}
...
\begin{abstract}
OEIS A140829 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A140829 is ``a(0)=1; for n >= 1, a(n) = ceiling(Fibonacci(n)/a(n-1)).''. It has offset $0$ and begins
\[
1, 1, 1, 2, 2, 3, 3, 5,\ \dots
\]
The entry states, as a conjecture and never marked settled:
\begin{quote}
a(n) = -a(n-1) + a(n-2) + a(n-3) + a(n-4) + a(n-5) for n>12. (End)
\end{quote}
and, separately and NOT as a conjecture:
\begin{quote}
G.f.: (1 + x + x\^{}3)*(1 + x - x\^{}9) / ((1 + x)*(1 - x\^{}2 - x\^{}4)).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 10 2026 13:34:26, revision 24) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{12} + x^{10} + x^{9} - x^{4} - x^{3} - x^{2} - 2 x - 1}{x^{5} + x^{4} + x^{3} + x^{2} - x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{5} - x^{4} - x^{3} - x^{2} + x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- x^{12} - x^{10} - x^{9} + x^{4} + x^{3} + x^{2} + 2 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A140829.
