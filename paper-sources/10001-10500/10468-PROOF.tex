...
\title{A condition on the multiset of every subblock: OEIS A183586}
...
\begin{abstract}
OEIS A183586 carries an empirical recurrence of order $2$ contributed by R.~H.~Hardin. It is
...
on $S=2$ vertices and is $C$-finite, and whether the conjectured recurrence annihilates it
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 29 2018 15:06:38, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $2$ columns over the alphabet $\{0,1,2,3\}$, growing one row at a time, and asks that every $2\times2$ subblock holds exactly $3$ distinct values.
...
Merging vertices with identical futures leaves $S=2$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
