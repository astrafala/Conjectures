...
\title{The empirical recurrence for OEIS A233165, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233165 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 09 2018 14:38:41, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{67584 x^{3} - 4096 x^{2} + 48 x}{2048 x^{2} - 96 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2048 x^{2} - 96 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;67584 x^{3} - 4096 x^{2} + 48 x,
\]
