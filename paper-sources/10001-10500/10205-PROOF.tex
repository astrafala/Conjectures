...
\title{The empirical recurrence for OEIS A089640, derived from the entry's generating function}
...
\begin{abstract}
OEIS A089640 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A089640 is ``Enumeration of partial sums of 1 + [1,2] + [2,3] + [1,2] + [2,3] + ...''. It has offset $1$ and begins
\[
1, 1, 1, 1, 3, 4, 4, 6,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 18 2026 11:18:29, revision 28) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{3} - x^{2} - x}{x^{5} + 2 x^{4} + x^{3} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{5} - 2 x^{4} - x^{3} + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A089640.
