...
\title{The empirical recurrence for OEIS A265428, derived from the entry's generating function}
...
\begin{abstract}
OEIS A265428 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 16 2025 08:33:27, revision 27) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} + 2 x^{3} + x + 1}{x^{5} - x^{4} - x + 1},
\]
