...
\title{The empirical recurrence for OEIS A039824, derived from the entry's generating function}
...
\begin{abstract}
OEIS A039824 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A039824 is ``Number of different coefficient values in expansion of Product (1+q\^{}1+q\^{}3...+q\^{}(2i-1)), i=1 to n.''. It has offset $1$ and begins
\[
1, 2, 4, 11, 20, 31, 46, 61,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 04 2017 11:28:20, revision 19) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{9} + 4 x^{8} - 2 x^{7} + 3 x^{5} - 4 x^{4} - x^{3} + x^{2} - x}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2 x^{9} - 4 x^{8} + 2 x^{7} - 3 x^{5} + 4 x^{4} + x^{3} - x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A039824.
