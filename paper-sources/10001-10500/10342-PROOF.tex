...
\title{The empirical recurrence for OEIS A269879, derived from the entry's generating function}
...
\begin{abstract}
OEIS A269879 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A269879 is ``First differences of number of active (ON, black) cells in n-th stage of growth of two-dimensional cellular automaton defined by "Rule 43", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
4, 0, 32, -24, 84, -72, 160, -144,\ \dots
\]
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{24 x^{2} + 4 x + 4}{x^{5} + x^{4} - 2 x^{3} - 2 x^{2} + x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269879.
