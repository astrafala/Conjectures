...
\title{A condition on every subblock: the empirical recurrence for OEIS A181255}
...
\begin{abstract}
OEIS A181255 carries an empirical recurrence of order $7$ contributed by R.~H.~Hardin. It is
...
OEIS A181255 is ``Number of (n+2) X 3 binary matrices with every 3 X 3 block having exactly four 1's.''. It has offset $1$ and begins
\[
126, 336, 906, 2484, 7218, 21024, 61398, 182520,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 12*a(n-3) - 36*a(n-4) - 27*a(n-6) + 81*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 12 2023 10:53:41, revision 19) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $3$ columns over the alphabet $\{0,1\}$, growing one row at a time, and the condition it imposes is that every $3\times3$ subblock must contain exactly $4$ ones.

That condition is decided by a bounded window. A $3\times3$ subblock occupies $3$ consecutive rows, so a window of $3$ consecutive rows decides everything that the arrival of a new row settles; nothing outside the window is consulted. Take those windows as the vertices of a digraph, with an edge from $u$ to $v$ when $v$'s new row may follow $u$, and merge vertices with identical futures; that leaves $S=12$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which windows may begin and end an array,
\[
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 12\,a(n-3) - 36\,a(n-4) - 27\,a(n-6) + 81\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A181255.
