...
\title{A proof of the conjectured recurrence for OEIS A210064}
...
\begin{abstract}
OEIS A210064 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A210064 is ``Total number of 231 patterns in the set of permutations avoiding 123''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;0, 0, 1, 11, 81, 500, 2794, 14649,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{x \left(1 - 4 x\right)^{\frac{3}{2}} + \left(1 - 4 x\right)^{\frac{5}{2}} + \left(x - 1\right) \left(4 x - 1\right)^{2}}{2 \left(1 - 4 x\right)^{\frac{7}{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-3)*a(n) +2*(-4*n\^{}2+11*n-2)*a(n-1) +8*(n-1)*(2*n-3)*a(n-2)=0. - \emph{R. J. Mathar}, Oct 08 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-12-05, revision 29) the
...
\[
p_{0}(n)=n \left(n - 3\right),\qquad p_{1}(n)=- 2 \left(4 n^{2} - 11 n + 2\right),\qquad p_{2}(n)=8 \left(n - 1\right) \left(2 n - 3\right)
\]
...
\section{The computation for A210064}
...
\[
n \left(n - 3\right)\,a(n) -  2 \left(4 n^{2} - 11 n + 2\right)\,a(n-1) + 8 \left(n - 1\right) \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A210064, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A210064.
