...
\title{A proof of the conjectured recurrence for OEIS A165537}
...
\begin{abstract}
OEIS A165537 carries a conjectured linear recurrence with polynomial coefficients, of
order 7, contributed by R.~J.~Mathar. We prove it. The generating function of the
...
closed form. Here it equals $- 14 x^{5} + 40 x^{4} - 43 x^{3} + 20 x^{2} - 3 x$, a polynomial of degree $5$, which proves
...
OEIS A165537 is ``Number of permutations of length n which avoid the patterns 4213 and 2143''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 6, 22, 88, 366, 1556,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4 x^{4} - 20 x^{3} + 24 x^{2} - 10 x + \left(2 x - 1\right) \left(- 2 x^{3} + 7 x^{2} - 5 x + \left(x - 1\right) \sqrt{5 x^{2} - 6 x + 1} + 1\right) + 1}{4 x^{4} - 20 x^{3} + 24 x^{2} - 10 x + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + 2*(-9*n+7)*a(n-1) + (121*n-204)*a(n-2) + 28*(-14*n+37)*a(n-3) + 16*(42*n-151)*a(n-4) + 4*(-153*n+694)*a(n-5) + 4*(67*n-364)*a(n-6) + 40*(-n+6)*a(n-7) = 0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-06-28, revision 36) the
statement is still recorded as a conjecture, and no proof appears among the entry's links,
formulas or programs.

Writing the conjectured relation in the form $\sum_{i=0}^{7}p_{i}(n)\,a(n-i)=0$,
the coefficient polynomials are
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(9 n - 7\right),\qquad p_{2}(n)=121 n - 204,\qquad p_{3}(n)=- 28 \left(14 n - 37\right),\qquad p_{4}(n)=16 \left(42 n - 151\right),\qquad p_{5}(n)=- 4 \left(153 n - 694\right),\qquad p_{6}(n)=4 \left(67 n - 364\right),\qquad p_{7}(n)=- 40 \left(n - 6\right)
\]
...
\section{The computation for A165537}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x - 1\right) \left(5 x - 1\right) .
\]
...
\[
B(x)\;=\;- 14 x^{5} + 40 x^{4} - 43 x^{3} + 20 x^{2} - 3 x ,
\]
...
\[
n\,a(n) -  2 \left(9 n - 7\right)\,a(n-1) + (121 n - 204)\,a(n-2) -  28 \left(14 n - 37\right)\,a(n-3) + 16 \left(42 n - 151\right)\,a(n-4) -  4 \left(153 n - 694\right)\,a(n-5) + 4 \left(67 n - 364\right)\,a(n-6) -  40 \left(n - 6\right)\,a(n-7) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A165537, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 14 x^{5} + 40 x^{4} - 43 x^{3} + 20 x^{2} - 3 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A165537.
