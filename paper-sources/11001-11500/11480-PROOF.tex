...
\title{A proof of the conjectured recurrence for OEIS A279014}
...
\begin{abstract}
OEIS A279014 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $49 x^{4} - 159 x^{3}$, a polynomial of degree $4$, which proves
...
OEIS A279014 is ``a(n) = Sum\_\{k=0..n\} fibonacci(k+1)*binomial(2*n-1,n-k)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 8, 33, 138, 581, 2455, 10395,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x \left(2 x - \sqrt{1 - 4 x} - 1\right)}{4 x^{2} - 3 x \sqrt{1 - 4 x} + 3 x + \sqrt{1 - 4 x} - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +n*(3*n-62)*a(n) +(3*n\^{}2+362*n-247)*a(n-1) +(-171*n\^{}2+220*n+162)*a(n-2) +(417*n\^{}2-2570*n+3551)*a(n-3) +2*(27*n-59)*(2*n-7)*a(n-4)=0. - \emph{R. J. Mathar}, Mar 12 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2025-02-07, revision 27) the
...
\[
p_{0}(n)=n \left(3 n - 62\right),\qquad p_{1}(n)=3 n^{2} + 362 n - 247,\qquad p_{2}(n)=- 171 n^{2} + 220 n + 162,\qquad p_{3}(n)=417 n^{2} - 2570 n + 3551,\qquad p_{4}(n)=2 \left(2 n - 7\right) \left(27 n - 59\right)
\]
...
\section{The computation for A279014}
...
\[
n \left(3 n - 62\right)\,a(n) + (3 n^{2} + 362 n - 247)\,a(n-1) + (- 171 n^{2} + 220 n + 162)\,a(n-2) + (417 n^{2} - 2570 n + 3551)\,a(n-3) + 2 \left(2 n - 7\right) \left(27 n - 59\right)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A279014, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=49 x^{4} - 159 x^{3}$ is an
...
\url{https://oeis.org}, 2026. Sequence A279014.
