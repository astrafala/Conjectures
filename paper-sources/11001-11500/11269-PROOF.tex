...
\title{A proof of the conjectured recurrence for OEIS A218185}
...
\begin{abstract}
OEIS A218185 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 16 x^{4}$, a polynomial of degree $4$, which proves
...
OEIS A218185 is ``Number of rows with the value true in the truth tables of all bracketed formulas with n distinct propositions p\_1, ..., p\_n connected by the binary connective of m-implication (case 3)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 4, 19, 100, 566, 3384,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;x + \frac{\sqrt{1 - 8 x} \sqrt{1 - 4 x}}{4} - \frac{\sqrt{1 - 8 x}}{4} + \frac{\sqrt{1 - 4 x}}{4} - \frac{1}{4} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-1)*(n-2)*a(n) -12*(2*n-5)*(n-1)*(n-2)*a(n-1) +4 *(n-2)*(52*n\^{}2-312*n+465) *a(n-2) -48*(2*n-7)*(8*n\^{}2-56*n+95)*a(n-3) +256*(n-5) *(2*n-7) *(2*n-9)*a(n-4)=0. - \emph{R. J. Mathar}, Oct 08 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 36) the
...
\[
p_{0}(n)=n \left(n - 2\right) \left(n - 1\right),\qquad p_{1}(n)=- 12 \left(n - 2\right) \left(n - 1\right) \left(2 n - 5\right),\qquad p_{2}(n)=4 \left(n - 2\right) \left(52 n^{2} - 312 n + 465\right),\qquad p_{3}(n)=- 48 \left(2 n - 7\right) \left(8 n^{2} - 56 n + 95\right),\qquad p_{4}(n)=256 \left(n - 5\right) \left(2 n - 9\right) \left(2 n - 7\right)
\]
...
\[
n \left(n - 2\right) \left(n - 1\right)\,a(n) -  12 \left(n - 2\right) \left(n - 1\right) \left(2 n - 5\right)\,a(n-1) + 4 \left(n - 2\right) \left(52 n^{2} - 312 n + 465\right)\,a(n-2) -  48 \left(2 n - 7\right) \left(8 n^{2} - 56 n + 95\right)\,a(n-3) + 256 \left(n - 5\right) \left(2 n - 9\right) \left(2 n - 7\right)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A218185, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 16 x^{4}$ is an
...
\url{https://oeis.org}, 2026. Sequence A218185.
