...
\title{A proof of the conjectured recurrence for OEIS A176332}
...
\begin{abstract}
OEIS A176332 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A176332 is ``Row sums of triangle A176331''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 16, 56, 202, 741, 2752,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4}{\sqrt{1 - 4 x} \left(\left(\sqrt{1 - 4 x} - 1\right)^{2} + 4\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 6*n*a(n) + 2*(-17*n+10)*a(n-1) + (47*n-60)*a(n-2) + 10*(-3*n+5)*a(n-3) + 4*(2*n-5)*a(n-4) = 0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-14, revision 68) the
...
\[
p_{0}(n)=6 n,\qquad p_{1}(n)=- 2 \left(17 n - 10\right),\qquad p_{2}(n)=47 n - 60,\qquad p_{3}(n)=- 10 \left(3 n - 5\right),\qquad p_{4}(n)=4 \left(2 n - 5\right)
\]
...
\section{The computation for A176332}
...
\[
6 n\,a(n) -  2 \left(17 n - 10\right)\,a(n-1) + (47 n - 60)\,a(n-2) -  10 \left(3 n - 5\right)\,a(n-3) + 4 \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A176332, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A176332.
