...
\title{A proof of the conjectured recurrence for OEIS A182904}
...
\begin{abstract}
OEIS A182904 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A182904 is ``Number of weighted lattice paths in L\_n having no peaks. The members of L\_n are paths of weight n that start at (0,0), end on the horizontal axis and whose steps are of the following four kinds: an (1,0)-step with weight 1, an (1,0)-step with weight 2, a (1,1)-step with weight 2, and a (1,-1)-step with weight 1. The weight of a path is the sum of the weights of its steps. A peak is a (1,1)-step followed by a (1,-1)-step''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 4, 9, 21, 48, 112,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1}{\sqrt{x^{6} - 2 x^{5} - x^{4} - x^{2} - 2 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(n-2)*a(n-1) +(-7*n+10)*a(n-2) +3*(-n+2)*a(n-3) +(-n+2)*a(n-4) +(-5*n+14)*a(n-5) +(-5*n+18)*a(n-6) +3*(n-4)*a(n-7)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-06-14, revision 7) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=n - 2,\qquad p_{2}(n)=10 - 7 n,\qquad p_{3}(n)=- 3 \left(n - 2\right),\qquad p_{4}(n)=2 - n,\qquad p_{5}(n)=14 - 5 n,\qquad p_{6}(n)=18 - 5 n,\qquad p_{7}(n)=3 \left(n - 4\right)
\]
...
\[
n\,a(n) + (n - 2)\,a(n-1) + (10 - 7 n)\,a(n-2) -  3 \left(n - 2\right)\,a(n-3) + (2 - n)\,a(n-4) + (14 - 5 n)\,a(n-5) + (18 - 5 n)\,a(n-6) + 3 \left(n - 4\right)\,a(n-7) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A182904, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A182904.
