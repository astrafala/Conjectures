...
\title{A proof of the conjectured recurrence for OEIS A261196}
...
\begin{abstract}
OEIS A261196 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A261196 is ``Expansion of sqrt(8*x + sqrt(1 + 64*x\^{}2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 8, -32, -160, 896, 5376, -33792,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\sqrt{8 x + \sqrt{64 x^{2} + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: n*(n-1)*a(n) +(n-1)*(n-2)*a(n-1) +16*(2*n-3)*(2*n-5)*a(n-2) +16*(2*n-5)*(2*n-7)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 07 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2022-09-08, revision 30) the
...
\[
p_{0}(n)=n \left(n - 1\right),\qquad p_{1}(n)=\left(n - 2\right) \left(n - 1\right),\qquad p_{2}(n)=16 \left(2 n - 5\right) \left(2 n - 3\right),\qquad p_{3}(n)=16 \left(2 n - 7\right) \left(2 n - 5\right)
\]
...
\[
n \left(n - 1\right)\,a(n) + \left(n - 2\right) \left(n - 1\right)\,a(n-1) + 16 \left(2 n - 5\right) \left(2 n - 3\right)\,a(n-2) + 16 \left(2 n - 7\right) \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A261196, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A261196.
