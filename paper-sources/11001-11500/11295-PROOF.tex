...
\title{The empirical recurrence for OEIS A214907, derived from the entry's generating function}
...
\begin{abstract}
OEIS A214907 carries an empirical recurrence of order $14$. The entry separately records a
...
OEIS A214907 is ``Number of 3 X 3 X 3 triangular 0..n arrays with every horizontal row nondecreasing and having the same average value.''. It has offset $1$ and begins
\[
2, 6, 14, 29, 50, 88, 136, 209,\ \dots
\]
...
\begin{quote}
G.f.: x*(2 + 6*x + 10*x\^{}2 + 13*x\^{}3 + 10*x\^{}4 + 10*x\^{}5 + 8*x\^{}6 + 7*x\^{}7 + 6*x\^{}8 + x\^{}9 - 2*x\^{}10 - 2*x\^{}11 + x\^{}13) / ((1 - x)\^{}5*(1 + x)\^{}3*(1 + x\^{}2)*(1 + x + x\^{}2)\^{}2). - \_Colin Barker\_, Jul 22 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 22 2018 08:44:31, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{14} + 2 x^{12} + 2 x^{11} - x^{10} - 6 x^{9} - 7 x^{8} - 8 x^{7} - 10 x^{6} - 10 x^{5} - 13 x^{4} - 10 x^{3} - 6 x^{2} - 2 x}{x^{14} - 2 x^{12} - 2 x^{11} + 4 x^{9} + 3 x^{8} - 3 x^{6} - 4 x^{5} + 2 x^{3} + 2 x^{2} - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{14} + 2 x^{12} + 2 x^{11} - 4 x^{9} - 3 x^{8} + 3 x^{6} + 4 x^{5} - 2 x^{3} - 2 x^{2} + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-2) + 2\,a(n-3) - 4\,a(n-5) - 3\,a(n-6) + 3\,a(n-8) + 4\,a(n-9) - 2\,a(n-11) - 2\,a(n-12) + a(n-14)
\]
for every $n>14$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{14} - 2 x^{12} - 2 x^{11} + x^{10} + 6 x^{9} + 7 x^{8} + 8 x^{7} + 10 x^{6} + 10 x^{5} + 13 x^{4} + 10 x^{3} + 6 x^{2} + 2 x,
\]
...
\[
x^{14} - 2 x^{12} - 2 x^{11} + 4 x^{9} + 3 x^{8} - 3 x^{6} - 4 x^{5} + 2 x^{3} + 2 x^{2} - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A214907.
