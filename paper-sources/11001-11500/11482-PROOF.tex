...
\title{A proof of the conjectured recurrence for OEIS A211278}
...
\begin{abstract}
OEIS A211278 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x$, a polynomial of degree $1$, which proves
...
OEIS A211278 is ``a(n) is the number of n-lettered words in the alphabet \{1, 2, 3\} with as many occurrences of the substring (consecutive subword) [1, 1] as of [2, 3]''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 7, 16, 38, 95, 248, 668,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{\frac{- x - 1}{3 x - 1}} + 1}{2 \left(x - 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) - 4*n*a(n-1) +2*(n+3)*a(n-2) + 4*(n-3)*a(n-3) + 3*(-n+2)*a(n-4) = 0. - \emph{R. J. Mathar}, Jun 09 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 35) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 4 n,\qquad p_{2}(n)=2 \left(n + 3\right),\qquad p_{3}(n)=4 \left(n - 3\right),\qquad p_{4}(n)=- 3 \left(n - 2\right)
\]
...
\section{The computation for A211278}
...
\[
n\,a(n) -  4 n\,a(n-1) + 2 \left(n + 3\right)\,a(n-2) + 4 \left(n - 3\right)\,a(n-3) -  3 \left(n - 2\right)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A211278, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x$ is an
...
\url{https://oeis.org}, 2026. Sequence A211278.
