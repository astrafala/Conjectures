...
\title{A proof of the conjectured recurrence for OEIS A000483}
...
\begin{abstract}
OEIS A000483 carries a conjectured linear recurrence with polynomial coefficients, of
order 4, contributed by its contributor. We prove it. The generating function of the
...
closed form. Here it equals $- 20 x^{3} - 45 x^{2}$, a polynomial of degree $3$, which proves
...
OEIS A000483 is ``Associated Stirling numbers: second-order reciprocal Stirling numbers (Fekete) a(n) = [[n, 3]]. The number of 3-orbit permutations of an n-set with at least 2 elements in each orbit''. It has offset $6$ and begins
\[
a(6),\dots,a(13)\;=\;15, 210, 2380, 26432, 303660, 3678840, 47324376, 647536032,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 6} a(n)\,\frac{x^{n}}{n!}\;=\;- \frac{\left(x + \log{\left(1 - x \right)}\right)^{3}}{6} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 3*(-n+4)*a(n) +(9*n\^{}2-59*n+90)*a(n-1) +(-9*n\^{}3+96*n\^{}2-348*n+436)*a(n-2) +(n-3)*(3*n\^{}3-45*n\^{}2+237*n-430)*a(n-3) +5*(n-5)*(n-6)*(n-3)*(n-4)*a(n-4)=0. - \emph{R. J. Mathar}, Jul 18 2015
\end{quote}
...
\[
p_{0}(n)=- 3 \left(n - 4\right),\qquad p_{1}(n)=9 n^{2} - 59 n + 90,\qquad p_{2}(n)=- 9 n^{3} + 96 n^{2} - 348 n + 436,\qquad p_{3}(n)=\left(n - 3\right) \left(3 n^{3} - 45 n^{2} + 237 n - 430\right),\qquad p_{4}(n)=5 \left(n - 6\right) \left(n - 5\right) \left(n - 4\right) \left(n - 3\right)
\]
...
\section{The computation for A000483}
...
\[
- 3 \left(n - 4\right)\,a(n) + (9 n^{2} - 59 n + 90)\,a(n-1) + (- 9 n^{3} + 96 n^{2} - 348 n + 436)\,a(n-2) + \left(n - 3\right) \left(3 n^{3} - 45 n^{2} + 237 n - 430\right)\,a(n-3) + 5 \left(n - 6\right) \left(n - 5\right) \left(n - 4\right) \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A000483, agreeing on all 13 terms compared.
...
vanish, for all 16 values of $n$ from $n=10$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 20 x^{3} - 45 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A000483.
