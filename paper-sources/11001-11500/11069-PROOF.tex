...
\title{The empirical recurrence for OEIS A237634, proved by transfer matrix}
...
\begin{abstract}
OEIS A237634 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=4621$
...
OEIS A237634 is ``Number of (n+1)X(5+1) 0..2 arrays with the upper median of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$ and begins
\[
4621, 4500, 2086, 2816, 4622, 5656, 7756, 11312,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-4) for n>13
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:19:54, revision 6) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The $2\times2$ subblocks of an $(n+1)\times6$ array form an
$n\times5$ grid, and each carries the value $\sigma=c$, where $a\le b\le c\le d$ are its four entries sorted --- its minimum, lower median, upper median and maximum. The entry asks that the values of
...
At $n=1$ the array has two rows, so its subblock grid is a single row of $5$ subblocks with
no vertical neighbours; the condition there is exactly that consecutive values along that row
differ by $1$. The entry's first term $4621$ counts those, and the model was made to
...
\]
over the $S=4621$ pairs whose own subblock row already satisfies the condition along itself.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237634.
