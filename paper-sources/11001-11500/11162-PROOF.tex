...
\title{The empirical recurrence for OEIS A223442, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A223442 counts arrays over $\{0,\dots,14\}$ in which every cell is constrained by the
...
with $S=225$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
Let $V=\{0,\dots,14\}^{1}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=225$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
publishes, $n=28$, the arrays have $1\cdot28$ cells over
$15$ values, so there are about $10^{32}$ of them to sift. The
walk count replaces that by $225$ states and one matrix--vector product per row.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=225$ pairs and each
of the $(14+1)^{1}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223442.
