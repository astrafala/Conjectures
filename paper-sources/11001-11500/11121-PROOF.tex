...
\title{A proof of the conjectured recurrence for OEIS A085387}
...
\begin{abstract}
OEIS A085387 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A085387 is ``E.g.f exp(x)*cosh(x+x\textasciicircum{}2/2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 7, 23, 71, 242, 925,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;e^{x} \cosh{\left(\frac{x \left(x + 2\right)}{2} \right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) -3*a(n-1) +2*a(n-2) +(-2*n+3)*a(n-3) +-(n-3)\textasciicircum{}2*a(n-4) +(n-3)*(n-4)*a(n- 5)=0.
\end{quote}
As of the ``Last modified'' line on the live entry (2013-10-19, revision 8) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=-3,\qquad p_{2}(n)=2,\qquad p_{3}(n)=3 - 2 n,\qquad p_{4}(n)=- \left(n - 3\right)^{2},\qquad p_{5}(n)=\left(n - 4\right) \left(n - 3\right)
\]
...
\section{The computation for A085387}
...
\[
1\,a(n) - 3\,a(n-1) + 2\,a(n-2) + (3 - 2 n)\,a(n-3) -  \left(n - 3\right)^{2}\,a(n-4) + \left(n - 4\right) \left(n - 3\right)\,a(n-5) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A085387, agreeing on all 13 terms compared.
...
vanish, for all 19 values of $n$ from $n=6$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A085387.
