...
\title{A proof of the conjectured recurrence for OEIS A176858}
...
\begin{abstract}
OEIS A176858 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 8 x^{4} + 23 x^{3} - 21 x^{2} + 3 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A176858 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=4, k=-1 and l=0''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 6, 25, 94, 419, 1884, 8866,\ \dots
\]
The entry posts no generating function of its own. Its NAME states the
relation \texttt{the entry states the generating function as G.f f: f(z)=(1-sqrt(1-4*z*(a(0)-z*a(0)\textasciicircum{}2+z*a(1)+(k+l)*z\textasciicircum{}2/(1-z)+k*z\textasciicircum{}2/(1-z)\textasciicircum{}2)))/(2*z) (k=-1, l=0); a(0) and a(1) are read off the entry's own terms and the parameters are the values given on that line} to another entry, and that entry's exponential generating
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 16 x^{4} + 28 x^{3} - 3 x^{2} - 6 x + 1}{x^{2} - 2 x + 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(-7*n+2)*a(n-1) +3*(n+1)*a(n-2) +(31*n-104)*a(n-3) +2*(-22*n+89)*a(n-4) +16*(n-5)*a(n-5)=0. - \emph{R. J. Mathar}, Mar 01 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-06-18, revision 8) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=2 - 7 n,\qquad p_{2}(n)=3 \left(n + 1\right),\qquad p_{3}(n)=31 n - 104,\qquad p_{4}(n)=- 2 \left(22 n - 89\right),\qquad p_{5}(n)=16 \left(n - 5\right)
\]
...
\section{The computation for A176858}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \frac{16 x^{4} - 28 x^{3} + 3 x^{2} + 6 x - 1}{\left(x - 1\right)^{2}} .
\]
...
\[
(n + 1)\,a(n) + (2 - 7 n)\,a(n-1) + 3 \left(n + 1\right)\,a(n-2) + (31 n - 104)\,a(n-3) -  2 \left(22 n - 89\right)\,a(n-4) + 16 \left(n - 5\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A176858, agreeing on all 13 terms compared.
...
vanish, for all 18 values of $n$ from $n=5$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 8 x^{4} + 23 x^{3} - 21 x^{2} + 3 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176858.
