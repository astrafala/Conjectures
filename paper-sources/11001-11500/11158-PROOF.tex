...
\title{The empirical recurrence for OEIS A195956, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A195956 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A195956 is ``Number of n X 2 0..4 arrays with each element equal to the number its horizontal and vertical neighbors unequal to itself.''. It has offset $1$ and begins
\[
1, 1, 6, 5, 5, 14, 21, 29,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +3*a(n-3) -a(n-4) -2*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (May 08 2018 06:20:16, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 3\,a(n-3) - a(n-4) - 2\,a(n-6)
\end{equation}
for all $n>7$.
...
x_{t,u}=\#\{\text{neighbours with a value }different from\ x_{t,u}\}
\end{equation}
for every cell $(t,u)$.
...
$5$ values, so there are about $10^{51}$ of them to sift. The
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195956.
