...
\title{The empirical recurrence for OEIS A283410, proved by transfer matrix}
...
\begin{abstract}
OEIS A283410 counts $n\times3$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
...
walk, and $a(n)$ is a walk count on $S=69$ vertices, which is $C$-finite. Whether the
...
OEIS A283410 is ``Number of n X 3 0..1 arrays with no 1 equal to more than three of its horizontal, vertical and antidiagonal neighbors.''. It has offset $1$ and begins
\[
8, 61, 416, 2944, 20744, 146228, 1030788, 7266120,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-1) + 13*a(n-2) + 10*a(n-3) + a(n-4) + 2*a(n-5) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 21 2019 08:19:11, revision 7) this is still
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-1) + 13\,a(n-2) + 10\,a(n-3) + a(n-4) + 2\,a(n-5)
\]
for every $n>7$.
\end{theorem}
...
By Lemma~\ref{lem:crit} the recurrence holds for every $n>7$. The bound is exact: at $n=7$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A283410.
