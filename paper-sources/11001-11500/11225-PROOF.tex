...
\title{A proof of the conjectured recurrence for OEIS A270530}
...
\begin{abstract}
OEIS A270530 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A270530 is ``a(n) = Sum\_\{k=0..n\}((binomial(2*k,k)/(k+1)*binomial(2*n+2,n-k)))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 5, 23, 105, 484, 2267, 10821, 52705,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x \sqrt{\frac{- 5 x - 2 \sqrt{1 - 4 x} + 2}{x^{3} \left(4 x - 1\right)}} \sqrt{1 - 4 x} + 1}{2 x \sqrt{1 - 4 x}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*n*(2*n+3)*(n+1)*a(n) -n*(77*n\^{}2+27*n-4)*a(n-1) +(549*n\^{}3-987*n\^{}2+686*n-168)*a(n-2) -20*(2*n-3)*(43*n\^{}2-104*n+70)*a(n-3) +500*(2*n-5)*(n-2)*(2*n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 07 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-04-10, revision 18) the
...
\[
p_{0}(n)=2 n \left(n + 1\right) \left(2 n + 3\right),\qquad p_{1}(n)=- n \left(77 n^{2} + 27 n - 4\right),\qquad p_{2}(n)=549 n^{3} - 987 n^{2} + 686 n - 168,\qquad p_{3}(n)=- 20 \left(2 n - 3\right) \left(43 n^{2} - 104 n + 70\right),\qquad p_{4}(n)=500 \left(n - 2\right) \left(2 n - 5\right) \left(2 n - 3\right)
\]
...
\section{The computation for A270530}

The generating function \eqref{eq:gf} is algebraic over $\mathbb{Q}(x)$;
its minimal polynomial is
\[
P(x,y) \;=\; 16 x^{6} y^{4} - 8 x^{5} y^{4} + x^{4} y^{4} + 12 x^{3} y^{2} - 7 x^{2} y^{2} + x y^{2} + 4 x y - y + 1 ,
\]
...
\[
2 n \left(n + 1\right) \left(2 n + 3\right)\,a(n) -  n \left(77 n^{2} + 27 n - 4\right)\,a(n-1) + (549 n^{3} - 987 n^{2} + 686 n - 168)\,a(n-2) -  20 \left(2 n - 3\right) \left(43 n^{2} - 104 n + 70\right)\,a(n-3) + 500 \left(n - 2\right) \left(2 n - 5\right) \left(2 n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A270530, agreeing on all 13 terms compared.
...
vanish, for all 17 values of $n$ from $n=4$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A270530.
