...
\title{A proof of the conjectured recurrence for OEIS A025270}
...
\begin{abstract}
OEIS A025270 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 20 x^{4} - 9 x^{3} + 2 x^{2} + x$, a polynomial of degree $4$, which proves
...
OEIS A025270 is ``a(n) = a(1)*a(n-1) + a(2)*a(n-2) + ...+ a(n-1)*a(1) for n >= 5, with initial values 1,2,1,1''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 1, 1, 6, 17, 60, 201,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{1}{2} - \frac{\sqrt{20 x^{4} + 12 x^{3} - 4 x^{2} - 4 x + 1}}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +2*(-2*n+3)*a(n-1) +4*(-n+3)*a(n-2) +6*(2*n-9)*a(n-3) +20*(n-6)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 21 2014
\end{quote}
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(2 n - 3\right),\qquad p_{2}(n)=- 4 \left(n - 3\right),\qquad p_{3}(n)=6 \left(2 n - 9\right),\qquad p_{4}(n)=20 \left(n - 6\right)
\]
...
\section{The computation for A025270}
...
\[
n\,a(n) -  2 \left(2 n - 3\right)\,a(n-1) -  4 \left(n - 3\right)\,a(n-2) + 6 \left(2 n - 9\right)\,a(n-3) + 20 \left(n - 6\right)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A025270, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 20 x^{4} - 9 x^{3} + 2 x^{2} + x$ is an
...
\url{https://oeis.org}, 2026. Sequence A025270.
