...
\title{A proof of the conjectured recurrence for OEIS A128732}
...
\begin{abstract}
OEIS A128732 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $20 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A128732 is ``Number DL's in all skew Dyck paths of semilength n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 5, 23, 106, 493, 2312,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{x \left(x + \sqrt{5 x^{2} - 6 x + 1} - 1\right)}{5 x^{2} - 6 x + \left(x + 1\right) \sqrt{5 x^{2} - 6 x + 1} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +2*n*(3*n-1)*a(n) -n*(39*n-37)*a(n-1) +4*(12*n\^{}2-22*n-15)*a(n-2) -5*(3*n+2)*(n-3)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 17 2016
\end{quote}
...
\[
p_{0}(n)=2 n \left(3 n - 1\right),\qquad p_{1}(n)=- n \left(39 n - 37\right),\qquad p_{2}(n)=4 \left(12 n^{2} - 22 n - 15\right),\qquad p_{3}(n)=- 5 \left(n - 3\right) \left(3 n + 2\right)
\]
...
\section{The computation for A128732}
...
\[
2 n \left(3 n - 1\right)\,a(n) -  n \left(39 n - 37\right)\,a(n-1) + 4 \left(12 n^{2} - 22 n - 15\right)\,a(n-2) -  5 \left(n - 3\right) \left(3 n + 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A128732, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=20 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A128732.
