...
\title{A proof of the conjectured recurrence for OEIS A168505}
...
\begin{abstract}
OEIS A168505 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{3} - 3 x^{2} - x + 1$, a polynomial of degree $3$, which proves
...
OEIS A168505 is ``Expansion of 1/(1-x/(1+x/(1-x/(1-x/(1+x/(1-x/(1-x/(1+x/(1-... (continued fraction)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 0, -1, -2, -2, 0, 5,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x + \sqrt{\left(x + 1\right) \left(4 x^{2} - 3 x + 1\right)} - 1}{2 x \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) - 3*n*a(n-1) + 3*(n-1)*a(n-2) + 3*(n-4)*a(n-3) + 2*(-2*n+7)*a(n-4) = 0. - \emph{R. J. Mathar}, Feb 10 2015
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 3 n,\qquad p_{2}(n)=3 \left(n - 1\right),\qquad p_{3}(n)=3 \left(n - 4\right),\qquad p_{4}(n)=- 2 \left(2 n - 7\right)
\]
...
\section{The computation for A168505}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x + 1\right) \left(4 x^{2} - 3 x + 1\right) .
\]
...
\[
(n + 1)\,a(n) -  3 n\,a(n-1) + 3 \left(n - 1\right)\,a(n-2) + 3 \left(n - 4\right)\,a(n-3) -  2 \left(2 n - 7\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A168505, agreeing on all 13 terms compared.
...
vanish, for all 34 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{3} - 3 x^{2} - x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A168505.
