...
\title{A proof of the conjectured recurrence for OEIS A000276}
...
\begin{abstract}
OEIS A000276 carries a conjectured linear recurrence with polynomial coefficients, of
...
to the polynomial $x^{2} + 3 x$, of degree $2$, which proves the recurrence for every
$n>2$.
\end{abstract}
...
OEIS A000276 is ``Associated Stirling numbers''. It has offset $4$ and begins
\[
a(4),\dots,a(11)\;=\;3, 20, 130, 924, 7308, 64224, 623376, 6636960,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 4} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{\left(x + \log{\left(1 - x \right)}\right)^{2}}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) +2*(-n+2)*a(n-1) +(n\textasciicircum{}2-6*n+10)*a(n-2) +(n-3)*(n-4)*a(n-3)=0.
\end{quote}
As of the ``Last modified'' line on the live entry (2026-07-11, revision 60) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- 2 \left(n - 2\right),\qquad p_{2}(n)=n^{2} - 6 n + 10,\qquad p_{3}(n)=\left(n - 4\right) \left(n - 3\right)
\]
...
\section{The computation for A000276}
...
\[
1\,a(n) -  2 \left(n - 2\right)\,a(n-1) + (n^{2} - 6 n + 10)\,a(n-2) + \left(n - 4\right) \left(n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A000276, agreeing on all 13 terms compared.
...
vanish, for all 14 values of $n$ from $n=10$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=x^{2} + 3 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A000276.
