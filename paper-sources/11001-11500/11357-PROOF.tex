...
\title{A proof of the conjectured recurrence for OEIS A270661}
...
\begin{abstract}
OEIS A270661 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{6} + 6 x^{5} - 9 x^{4} - 4 x^{3} + 9 x^{2} + 6 x + 1$, a polynomial of degree $6$, which proves
...
OEIS A270661 is ``a(n) = Sum\_\{k=0..n\}((binomial(2*k,k)*Sum\_\{i=0..n-k\}(binomial(k+1,n-k-i)*binomial(k+i,k)))/(k+1)\^{}2)*(n+1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 5, 14, 45, 164, 662, 2862, 12957,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(\sqrt{\frac{4 x \left(x + 1\right) + x - 1}{x - 1}} - 1\right) \left(- x^{2} + 2 x + 1\right)}{2 x \left(x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) -4* n*a(n-1) +(-13*n+33)*a(n-2) +2*(8*n-17)*a(n-3) +(19*n-69)*a(n-4) +4*(-4*n+15)*a(n-5) +7*(-n+5)*a(n-6) +2*(2*n-13)*a(n-7)=0. - \emph{R. J. Mathar}, Jun 07 2016
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 4 n,\qquad p_{2}(n)=33 - 13 n,\qquad p_{3}(n)=2 \left(8 n - 17\right),\qquad p_{4}(n)=19 n - 69,\qquad p_{5}(n)=- 4 \left(4 n - 15\right),\qquad p_{6}(n)=- 7 \left(n - 5\right),\qquad p_{7}(n)=2 \left(2 n - 13\right)
\]
...
\[
B(x)\;=\;- x^{6} + 6 x^{5} - 9 x^{4} - 4 x^{3} + 9 x^{2} + 6 x + 1 ,
\]
...
\[
(n + 1)\,a(n) -  4 n\,a(n-1) + (33 - 13 n)\,a(n-2) + 2 \left(8 n - 17\right)\,a(n-3) + (19 n - 69)\,a(n-4) -  4 \left(4 n - 15\right)\,a(n-5) -  7 \left(n - 5\right)\,a(n-6) + 2 \left(2 n - 13\right)\,a(n-7) \;=\;0
\]
holds for every $n>6$.
\end{theorem}
...
section of A270661, agreeing on all 13 terms compared.
...
vanish, for all 17 values of $n$ from $n=7$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{6} + 6 x^{5} - 9 x^{4} - 4 x^{3} + 9 x^{2} + 6 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A270661.
