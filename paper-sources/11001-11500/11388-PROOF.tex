...
\title{A condition on the multiset of every subblock: OEIS A236271}
...
\begin{abstract}
OEIS A236271 carries an empirical recurrence of order $4$ contributed by R.~H.~Hardin. It is
...
on $S=4$ vertices and is $C$-finite, and whether the conjectured recurrence annihilates it
...
OEIS A236271 is ``Number of (n+1) X (1+1) 0..2 arrays with the upper median unequal to the lower median in every 2 X 2 subblock.''. It has offset $1$ and begins
\[
42, 208, 1048, 5328, 27168, 138720, 708640, 3620768,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-1) + 4*a(n-2) - 16*a(n-3) - 8*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 21 2018 13:45:30, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $2$ columns over the alphabet $\{0,1,2\}$, growing one row at a time, and asks that in every $2\times2$ subblock the two middle order statistics are different: writing the $4$ entries of the block in nondecreasing order $v_1\le\cdots\le v_{4}$, the lower median is $v_{2}$ and the upper median $v_{3}$, and these must be unequal.
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-1) + 4\,a(n-2) - 16\,a(n-3) - 8\,a(n-4)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A236271.
