...
\title{A proof of the conjectured recurrence for OEIS A053532}
...
\begin{abstract}
OEIS A053532 carries a conjectured linear recurrence with polynomial coefficients, of
order 4. We prove it. The entry's exponential generating function is
...
OEIS A053532 is ``Expansion of e.g.f.: (1-x)\textasciicircum{}(-1/2)*exp(-x/2 -x\textasciicircum{}2/4 -x\textasciicircum{}3/6)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 0, 0, 3, 12, 60, 360,\ \dots
\]
The entry does not list an exponential generating function separately: it
is defined as the expansion of one, so the identity below is the definition of the sequence
rather than a claim about it,
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{e^{- \frac{x \left(2 x^{2} + 3 x + 6\right)}{12}}}{\sqrt{1 - x}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} D-finite with recurrence 2*a(n) +2*(-n+1)*a(n-1) -(n-1)*(n-2)*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Jul 06 2020
\end{quote}
...
\[
p_{0}(n)=2,\qquad p_{1}(n)=- 2 \left(n - 1\right),\qquad p_{2}(n)=0,\qquad p_{3}(n)=0,\qquad p_{4}(n)=- \left(n - 3\right) \left(n - 2\right) \left(n - 1\right)
\]
...
\section{The computation for A053532}
...
\[
2\,a(n) -  2 \left(n - 1\right)\,a(n-1) -  \left(n - 3\right) \left(n - 2\right) \left(n - 1\right)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A053532, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A053532.
