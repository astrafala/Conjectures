...
\title{A proof of the conjectured recurrence for OEIS A025249}
...
\begin{abstract}
OEIS A025249 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $12 x^{3} - 2 x^{2} + 4 x$, a polynomial of degree $3$, which proves
...
OEIS A025249 is ``a(n) = (1/2)*s(n+3), where s = A025248''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 3, 7, 13, 29, 71, 163,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{- 4 x^{3} - x - \sqrt{- 8 x^{3} + x^{2} - 2 x + 1} + 1}{4 x^{3}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(n+3)*a(n) +(-2*n-3)*a(n-1) +n*a(n-2) +4*(-2*n+3)*a(n-3)=0. - \emph{R. J. Mathar}, Feb 25 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2022-06-15, revision 12) the
...
\[
p_{0}(n)=n + 3,\qquad p_{1}(n)=- 2 n - 3,\qquad p_{2}(n)=n,\qquad p_{3}(n)=- 4 \left(2 n - 3\right)
\]
...
\section{The computation for A025249}
...
\[
(n + 3)\,a(n) + (- 2 n - 3)\,a(n-1) + n\,a(n-2) -  4 \left(2 n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A025249, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=12 x^{3} - 2 x^{2} + 4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A025249.
