...
\title{A proof of the conjectured recurrence for OEIS A157125}
...
\begin{abstract}
OEIS A157125 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{3} - 6$, a polynomial of degree $3$, which proves
...
OEIS A157125 is ``A transform of the Catalan numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -1, -1, 0, 2, 1, -1, -4,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x - 1\right) \left(\sqrt{x^{2} + 1} - \sqrt{4 x^{3} + x^{2} + 1}\right)}{2 x^{3} \sqrt{x^{2} + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+3)*(n-2)*a(n) -4*a(n-1) +2*(n\^{}2-n-4)*a(n-2) +2*(2*n\^{}2-7*n+2)*a(n-3) +(n+1)*(n-4)*a(n-4) +2*(n-1)*(2*n-7)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 15 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2024-10-05, revision 11) the
...
\[
p_{0}(n)=\left(n - 2\right) \left(n + 3\right),\qquad p_{1}(n)=-4,\qquad p_{2}(n)=2 \left(n^{2} - n - 4\right),\qquad p_{3}(n)=2 \left(2 n^{2} - 7 n + 2\right),\qquad p_{4}(n)=\left(n - 4\right) \left(n + 1\right),\qquad p_{5}(n)=2 \left(n - 1\right) \left(2 n - 7\right)
\]
...
\[
\left(n - 2\right) \left(n + 3\right)\,a(n) - 4\,a(n-1) + 2 \left(n^{2} - n - 4\right)\,a(n-2) + 2 \left(2 n^{2} - 7 n + 2\right)\,a(n-3) + \left(n - 4\right) \left(n + 1\right)\,a(n-4) + 2 \left(n - 1\right) \left(2 n - 7\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A157125, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{3} - 6$ is an
...
\url{https://oeis.org}, 2026. Sequence A157125.
