...
\title{A proof of the conjectured recurrence for OEIS A025756}
...
\begin{abstract}
OEIS A025756 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $24 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A025756 is ``3rd-order Vatalan numbers (generalization of Catalan numbers)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 22, 139, 949, 6808, 50548,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{3}{\sqrt[3]{1 - 9 x} + 2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-1)*a(n) -(n-1)*(19*n-36)*a(n-1) +9*(11*n\textasciicircum{}2-51*n+60)*a(n-2) -9*(3*n-7)*(3*n-8)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 14 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2026-02-17, revision 59) the
...
\[
p_{0}(n)=n \left(n - 1\right),\qquad p_{1}(n)=- \left(n - 1\right) \left(19 n - 36\right),\qquad p_{2}(n)=9 \left(11 n^{2} - 51 n + 60\right),\qquad p_{3}(n)=- 9 \left(3 n - 8\right) \left(3 n - 7\right)
\]
...
\[
n \left(n - 1\right)\,a(n) -  \left(n - 1\right) \left(19 n - 36\right)\,a(n-1) + 9 \left(11 n^{2} - 51 n + 60\right)\,a(n-2) -  9 \left(3 n - 8\right) \left(3 n - 7\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A025756, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=24 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A025756.
