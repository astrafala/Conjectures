...
\title{A proof of the conjectured recurrence for OEIS A141351}
...
\begin{abstract}
OEIS A141351 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{3} - 6 x^{2} + 1$, a polynomial of degree $3$, which proves
...
OEIS A141351 is ``a(n) = C(n) + 1 - 0\^{}n where C(n) = A000108(n)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 3, 6, 15, 43, 133, 430,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{2} - \frac{\left(x - 1\right) \left(\sqrt{1 - 4 x} - 1\right)}{2}}{x \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-3*n+1)*a(n-1) +(9*n-13)*a(n-2) +2*(-2*n+5)*a(n-3)=0. - \emph{R. J. Mathar}, Oct 15 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-24, revision 26) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=9 n - 13,\qquad p_{3}(n)=- 2 \left(2 n - 5\right)
\]
...
\section{The computation for A141351}
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + (9 n - 13)\,a(n-2) -  2 \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A141351, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{3} - 6 x^{2} + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A141351.
