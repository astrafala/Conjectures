...
\title{A proof of the conjectured recurrence for OEIS A190725}
...
\begin{abstract}
OEIS A190725 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x$, a polynomial of degree $1$, which proves
...
OEIS A190725 is ``Diagonal sums of Riordan matrix A118384''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 14, 69, 355, 1872, 10037, 54459,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{3 x - \sqrt{x^{2} - 6 x + 1} + 3}{2 \sqrt{x^{2} - 6 x + 1} \left(x^{2} + 3 x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
conjecture: n*(2*n-3)*a(n) +(-6*n\^{}2+15*n-8)*a(n-1) +2*(-16*n\^{}2+32*n-11)*a(n-2) +(-6*n\^{}2+9*n-2)*a(n-3) +(2*n-1)*(n-2)*a(n-4) =0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-27, revision 16) the
...
\[
p_{0}(n)=n \left(2 n - 3\right),\qquad p_{1}(n)=- 6 n^{2} + 15 n - 8,\qquad p_{2}(n)=- 2 \left(16 n^{2} - 32 n + 11\right),\qquad p_{3}(n)=- 6 n^{2} + 9 n - 2,\qquad p_{4}(n)=\left(n - 2\right) \left(2 n - 1\right)
\]
...
\section{The computation for A190725}
...
\[
n \left(2 n - 3\right)\,a(n) + (- 6 n^{2} + 15 n - 8)\,a(n-1) -  2 \left(16 n^{2} - 32 n + 11\right)\,a(n-2) + (- 6 n^{2} + 9 n - 2)\,a(n-3) + \left(n - 2\right) \left(2 n - 1\right)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A190725, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A190725.
