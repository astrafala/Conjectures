...
closed form. Here it equals $2 x^{2} - 10 x + 2$, a polynomial of degree $2$, which proves
...
\begin{quote}\small
\textbf{Conjecture:} +(n+2)*a(n) +(-11*n-14)*a(n-1) +(37*n+20)*a(n-2) +(-25*n-6)*a(n-3) +2*(-21*n+41)*a(n-4) +4*(-2*n+5)*a(n-5)=0. - \emph{R. J. Mathar}, Mar 06 2017
\end{quote}
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 11 n - 14,\qquad p_{2}(n)=37 n + 20,\qquad p_{3}(n)=- 25 n - 6,\qquad p_{4}(n)=- 2 \left(21 n - 41\right),\qquad p_{5}(n)=- 4 \left(2 n - 5\right)
\]
...
\[
(n + 2)\,a(n) + (- 11 n - 14)\,a(n-1) + (37 n + 20)\,a(n-2) + (- 25 n - 6)\,a(n-3) -  2 \left(21 n - 41\right)\,a(n-4) -  4 \left(2 n - 5\right)\,a(n-5) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{2} - 10 x + 2$ is an
