...
\title{A proof of the conjectured recurrence for OEIS A273351}
...
\begin{abstract}
OEIS A273351 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A273351 is ``Number of up steps in all bargraphs of semiperimeter n (n>=2)''. It has offset $2$ and begins
\[
a(2),\dots,a(9)\;=\;1, 3, 10, 32, 102, 326, 1046, 3370,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 2} a(n)\,x^{n}\;=\;\frac{- x^{3} - x^{2} - x - \left(x + 1\right) \sqrt{x^{4} + 2 x^{2} - 4 x + 1} + 1}{2 \sqrt{x^{4} + 2 x^{2} - 4 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-3)*(n-2)\^{}2*a(n) -(n-3)*(2*n-3)*(2*n\^{}2-6*n+3) *a(n-1) +(2*n\^{}4-16*n\^{}3+41*n\^{}2-36*n+5) *a(n-2) +2*(n-1)*(2*n-5) *a(n-3) +(n-2)*(n-5)*(n-1)\^{}2 *a(n-4)=0. - \emph{R. J. Mathar}, Jun 06 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 23) the
...
\[
p_{0}(n)=n \left(n - 3\right) \left(n - 2\right)^{2},\qquad p_{1}(n)=- \left(n - 3\right) \left(2 n - 3\right) \left(2 n^{2} - 6 n + 3\right),\qquad p_{2}(n)=2 n^{4} - 16 n^{3} + 41 n^{2} - 36 n + 5,\qquad p_{3}(n)=2 \left(n - 1\right) \left(2 n - 5\right),\qquad p_{4}(n)=\left(n - 5\right) \left(n - 2\right) \left(n - 1\right)^{2}
\]
...
\section{The computation for A273351}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x - 1\right) \left(x^{3} + x^{2} + 3 x - 1\right) .
\]
...
\[
n \left(n - 3\right) \left(n - 2\right)^{2}\,a(n) -  \left(n - 3\right) \left(2 n - 3\right) \left(2 n^{2} - 6 n + 3\right)\,a(n-1) + (2 n^{4} - 16 n^{3} + 41 n^{2} - 36 n + 5)\,a(n-2) + 2 \left(n - 1\right) \left(2 n - 5\right)\,a(n-3) + \left(n - 5\right) \left(n - 2\right) \left(n - 1\right)^{2}\,a(n-4) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A273351, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A273351.
