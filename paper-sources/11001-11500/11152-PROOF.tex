...
\title{A proof of the conjectured recurrence for OEIS A110521}
...
\begin{abstract}
OEIS A110521 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 3 x^{3} + 48 x^{2} + 12 x$, a polynomial of degree $3$, which proves
...
OEIS A110521 is ``Expansion of 1/(1 - (x + x\^{}2)c(3x)), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 5, 30, 224, 1871, 16771, 157668,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{6}{\left(x + 1\right) \left(\sqrt{1 - 12 x} - 1\right) + 6} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*n*a(n) + 2*(16-11n)*a(n-1) + 2*(42-11n)*a(n-2) + (32-21n)*a(n-3) + 5*(20-7n)*a(n-4) + 6*(7-2n)*a(n-5) = 0. - \emph{R. J. Mathar}, Dec 10 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2017-08-30, revision 11) the
...
\[
p_{0}(n)=2 n,\qquad p_{1}(n)=- 2 \left(11 n - 16\right),\qquad p_{2}(n)=- 2 \left(11 n - 42\right),\qquad p_{3}(n)=32 - 21 n,\qquad p_{4}(n)=- 5 \left(7 n - 20\right),\qquad p_{5}(n)=- 6 \left(2 n - 7\right)
\]
...
\section{The computation for A110521}
...
\[
2 n\,a(n) -  2 \left(11 n - 16\right)\,a(n-1) -  2 \left(11 n - 42\right)\,a(n-2) + (32 - 21 n)\,a(n-3) -  5 \left(7 n - 20\right)\,a(n-4) -  6 \left(2 n - 7\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A110521, agreeing on all 13 terms compared.
...
vanish, for all 15 values of $n$ from $n=5$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 3 x^{3} + 48 x^{2} + 12 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A110521.
