...
\title{A proof of the conjectured recurrence for OEIS A108296}
...
\begin{abstract}
OEIS A108296 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x + 3$, a polynomial of degree $1$, which proves
...
OEIS A108296 is ``Diagonal sums of the number triangle associated to A086617''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 3, 5, 8, 14, 24,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} + \sqrt{- 3 x^{4} - 4 x^{3} - 2 x^{2} + 1} - 1}{2 x^{3} \left(x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+3)*a(n) +(-n-2)*a(n-1) +2*(-n-1)*a(n-2) +2*(-n+3)*a(n-3) +(n+1)*a(n-4) +3*(n-2)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 16 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2024-09-02, revision 10) the
...
\[
p_{0}(n)=n + 3,\qquad p_{1}(n)=- n - 2,\qquad p_{2}(n)=- 2 \left(n + 1\right),\qquad p_{3}(n)=- 2 \left(n - 3\right),\qquad p_{4}(n)=n + 1,\qquad p_{5}(n)=3 \left(n - 2\right)
\]
...
\section{The computation for A108296}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \left(x + 1\right) \left(3 x^{3} + x^{2} + x - 1\right) .
\]
...
\[
(n + 3)\,a(n) + (- n - 2)\,a(n-1) -  2 \left(n + 1\right)\,a(n-2) -  2 \left(n - 3\right)\,a(n-3) + (n + 1)\,a(n-4) + 3 \left(n - 2\right)\,a(n-5) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A108296, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=x + 3$ is an
...
\url{https://oeis.org}, 2026. Sequence A108296.
