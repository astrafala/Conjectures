...
\title{A proof of the conjectured recurrence for OEIS A055392}
...
\begin{abstract}
OEIS A055392 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $- 49018 x^{5} + 3048 x^{4} + 360 x^{3} + 200 x^{2}$, a polynomial of degree $5$, which proves
...
OEIS A055392 is ``Number of bracketings of 0\#0\#0\#...\#0 giving result 0, where 0\#0 = 1, 0\#1 = 1\#0 = 1\#1 = 0''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 0, 2, 1, 12, 14, 100, 180,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{\sqrt{3 - 2 \sqrt{1 - 4 x}}}{2} + \frac{1}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 500*n*(n-1)*a(n) +100*(n-1)*(5*n -12)*a(n-1) +20*(25*n\^{}2 -463*n +846)*a(n-2) +(-140161*n\^{}2 +966559*n -1637508)*a(n-3) +2*(250*n\^{}2 -26509*n +105084)*a(n-4) +98036*(4*n -19)*(4*n -21)*a(n-5) = 0. - \emph{R. J. Mathar}, Nov 26 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 36) the
...
\[
p_{0}(n)=500 n \left(n - 1\right),\qquad p_{1}(n)=100 \left(n - 1\right) \left(5 n - 12\right),\qquad p_{2}(n)=20 \left(25 n^{2} - 463 n + 846\right),\qquad p_{3}(n)=- 140161 n^{2} + 966559 n - 1637508,\qquad p_{4}(n)=2 \left(250 n^{2} - 26509 n + 105084\right),\qquad p_{5}(n)=98036 \left(4 n - 21\right) \left(4 n - 19\right)
\]
...
\section{The computation for A055392}
...
\[
B(x)\;=\;- 49018 x^{5} + 3048 x^{4} + 360 x^{3} + 200 x^{2} ,
\]
...
\[
500 n \left(n - 1\right)\,a(n) + 100 \left(n - 1\right) \left(5 n - 12\right)\,a(n-1) + 20 \left(25 n^{2} - 463 n + 846\right)\,a(n-2) + (- 140161 n^{2} + 966559 n - 1637508)\,a(n-3) + 2 \left(250 n^{2} - 26509 n + 105084\right)\,a(n-4) + 98036 \left(4 n - 21\right) \left(4 n - 19\right)\,a(n-5) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A055392, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 49018 x^{5} + 3048 x^{4} + 360 x^{3} + 200 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A055392.
