...
\title{A proof of the conjectured recurrence for OEIS A217447}
...
\begin{abstract}
OEIS A217447 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A217447 is ``Number of n x n permutation matrices that disconnect their zeros''. It has offset $2$ and begins
\[
a(2),\dots,a(9)\;=\;2, 6, 12, 32, 120, 580, 3392, 23244,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} 2*a(n) + 2*(-n-1)*a(n-1) + (6*n-11)*a(n-2) + (-5*n+14)*a(n-3) + 3*a(n-4) + (n-6)*a(n-5) = 0. - \emph{R. J. Mathar}, Nov 30 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 21) it is still
...
\begin{quote}\small
Recurrence (for n>4): (2*n\^{}2 - 16*n + 31)*a(n) = (2*n\^{}3 - 16*n\^{}2 + 33*n - 6)*a(n-1) - (2*n-7)*(2*n\^{}2 - 12*n + 15)*a(n-2) + (n-4)*(2*n\^{}2 - 12*n + 17)*a(n-3). - \emph{Vaclav Kotesovec}, Jan 31 2014
\end{quote}
...
\section{The computation for A217447}

Reading both relations as operators and dividing the conjectured one by the stated one
leaves remainder $0$, with quotient
\[
Q \;=\; \left(- \frac{1}{2 n^{2} - 1}\right) + \left(\frac{- 4 n^{2} - 4 n + 6}{4 n^{4} + 8 n^{3} - 4 n - 1}\right)\,N + \left(\frac{2}{2 n^{2} + 4 n + 1}\right)\,N^{2} .
\]
...
\[
2\,a(n) -  2 \left(n + 1\right)\,a(n-1) + (6 n - 11)\,a(n-2) + (14 - 5 n)\,a(n-3) + 3\,a(n-4) + (n - 6)\,a(n-5) \;=\;0
\]
...
\url{https://oeis.org}, 2026. Sequence A217447.
