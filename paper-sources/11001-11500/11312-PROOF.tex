...
\title{A proof of the conjectured recurrence for OEIS A226434}
...
\begin{abstract}
OEIS A226434 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $30912 x^{6} - 114464 x^{5} + 153750 x^{4} - 50826 x^{3} + 2908 x^{2}$, a polynomial of degree $6$, which proves
...
OEIS A226434 is ``The number of sum decomposable permutations which avoid the patterns 3124 and 4312''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;0, 1, 3, 10, 37, 146, 595, 2456,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;- \frac{x \left(8 x^{4} - 16 x^{3} + 19 x^{2} - 8 x - \sqrt{1 - 4 x} \left(2 x^{3} + x^{2} - 4 x + 1\right) + 1\right)}{12 x^{4} - 31 x^{3} + 27 x^{2} - 9 x + \sqrt{1 - 4 x} \left(4 x^{4} - 13 x^{3} + 15 x^{2} - 7 x + 1\right) + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(95*n+537)*(n+2)*a(n) +(95*n\^{}2-16421*n-14748) *a(n-1) +(-6403*n\^{}2+124495*n-60066) *a(n-2) +(21565*n\^{}2-354883*n+596496) *a(n-3) +2*(-5092*n\^{}2+138877*n-395970)  *a(n-4) +8*(-2470*n\^{}2+11113*n+12744) *a(n-5) +192*(38*n-67)*(2*n-13)*a(n-6)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 9) the
...
\[
p_{0}(n)=\left(n + 2\right) \left(95 n + 537\right),\qquad p_{1}(n)=95 n^{2} - 16421 n - 14748,\qquad p_{2}(n)=- 6403 n^{2} + 124495 n - 60066,\qquad p_{3}(n)=21565 n^{2} - 354883 n + 596496,\qquad p_{4}(n)=- 2 \left(5092 n^{2} - 138877 n + 395970\right),\qquad p_{5}(n)=- 8 \left(2470 n^{2} - 11113 n - 12744\right),\qquad p_{6}(n)=192 \left(2 n - 13\right) \left(38 n - 67\right)
\]
...
\section{The computation for A226434}
...
\[
B(x)\;=\;30912 x^{6} - 114464 x^{5} + 153750 x^{4} - 50826 x^{3} + 2908 x^{2} ,
\]
...
\[
\left(n + 2\right) \left(95 n + 537\right)\,a(n) + (95 n^{2} - 16421 n - 14748)\,a(n-1) + (- 6403 n^{2} + 124495 n - 60066)\,a(n-2) + (21565 n^{2} - 354883 n + 596496)\,a(n-3) -  2 \left(5092 n^{2} - 138877 n + 395970\right)\,a(n-4) -  8 \left(2470 n^{2} - 11113 n - 12744\right)\,a(n-5) + 192 \left(2 n - 13\right) \left(38 n - 67\right)\,a(n-6) \;=\;0
\]
holds for every $n>6$.
\end{theorem}
...
section of A226434, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=30912 x^{6} - 114464 x^{5} + 153750 x^{4} - 50826 x^{3} + 2908 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A226434.
