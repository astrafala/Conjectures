...
\title{The empirical recurrence for OEIS A222276, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A222276 counts arrays over $\{0,\dots,5\}$ in which every cell is constrained by the
...
with $S=46656$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A222276 is ``Number of n X 3 0..5 arrays with no entry increasing mod 6 by 5 rightwards or downwards, starting with upper left zero.''. It has offset $1$ and begins
\[
25, 2205, 194485, 17153945, 1513010465, 133450391205, 11770577485085, 1038187247574145,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 92*a(n-1) - 335*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 14 2018 15:13:51, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;92\,a(n-1) - 335\,a(n-2)
\end{equation}
for all $n>3$.
...
Let $V=\{0,\dots,5\}^{3}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=46656$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
and $3$ cells in a row, so $|V|=6^{3}=216$, and the vertex set is
$V\times V$, of size $216^2=46656$. That is the whole of the state: two consecutive
...
publishes, $n=13$, the arrays have $3\cdot13$ cells over
$6$ values, so there are about $10^{30}$ of them to sift. The
walk count replaces that by $46656$ states and one matrix--vector product per row.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=46656$ pairs and each
of the $(5+1)^{3}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222276.
