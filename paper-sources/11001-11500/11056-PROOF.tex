...
\title{A proof of the conjectured recurrence for OEIS A126931}
...
\begin{abstract}
OEIS A126931 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 - 2 x$, a polynomial of degree $1$, which proves
...
OEIS A126931 is ``a(n) = A127359(n+1)/2 - A127359(n)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 10, 33, 110, 366, 1220, 4065,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{- 6 x + \sqrt{1 - 4 x^{2}} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 3*(n+1)*a(n) +10*(-n-1)*a(n-1) +12*(-n+2)*a(n-2) +40*(n-2)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 26 2012
\end{quote}
...
\[
p_{0}(n)=3 \left(n + 1\right),\qquad p_{1}(n)=- 10 \left(n + 1\right),\qquad p_{2}(n)=- 12 \left(n - 2\right),\qquad p_{3}(n)=40 \left(n - 2\right)
\]
...
\section{The computation for A126931}
...
\[
3 \left(n + 1\right)\,a(n) -  10 \left(n + 1\right)\,a(n-1) -  12 \left(n - 2\right)\,a(n-2) + 40 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A126931, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=3 - 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A126931.
