...
\title{The empirical recurrence for OEIS A230185, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A230185 counts arrays over $\{0,\dots,2\}$ in which every cell is constrained by the
...
OEIS A230185 is ``Number of 2 X n 0..2 arrays x(i,j) with each element horizontally or antidiagonally next to at least one element with value 2-x(i,j).''. It has offset $1$ and begins
\[
0, 9, 27, 159, 825, 4395, 23307, 123729,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) + 7*a(n-2) + a(n-3) - 6*a(n-4) - 5*a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 21 2018 14:57:43, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) + 7\,a(n-2) + a(n-3) - 6\,a(n-4) - 5\,a(n-5)
\end{equation}
for all $n>6$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230185.
