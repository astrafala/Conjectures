...
\title{A proof of the conjectured recurrence for OEIS A168503}
...
\begin{abstract}
OEIS A168503 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $6 x^{2} - 6 x + 1$, a polynomial of degree $2$, which proves
...
OEIS A168503 is ``Expansion of 1/(1-x/(1-2x/(1-3x/(1-x/(1-2x/(1-3x/(1-... (continued fraction)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 15, 87, 531, 3339, 21483,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{- 24 x^{3} + 36 x^{2} - 12 x + 1} - 1}{6 x \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(-13*n+5)*a(n-1) +6*(8*n-13)*a(n-2) +12*(-5*n+13)*a(n-3) +12*(2*n-7)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 10 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-02-07, revision 20) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=5 - 13 n,\qquad p_{2}(n)=6 \left(8 n - 13\right),\qquad p_{3}(n)=- 12 \left(5 n - 13\right),\qquad p_{4}(n)=12 \left(2 n - 7\right)
\]
...
\[
(n + 1)\,a(n) + (5 - 13 n)\,a(n-1) + 6 \left(8 n - 13\right)\,a(n-2) -  12 \left(5 n - 13\right)\,a(n-3) + 12 \left(2 n - 7\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A168503, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=6 x^{2} - 6 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A168503.
