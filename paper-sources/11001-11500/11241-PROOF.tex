...
\title{A proof of the conjectured recurrence for OEIS A162481}
...
\begin{abstract}
OEIS A162481 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x + 1$, a polynomial of degree $1$, which proves
...
OEIS A162481 is ``Expansion of (1/(1-x)\^{}3)*c(x/(1-x)\^{}3), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 14, 54, 235, 1119, 5658, 29800,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{4 x - \left(1 - x\right)^{3}}{\left(x - 1\right)^{3}}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 2*(1-4*n)*a(n-1) + 2*(5*n-3)*a(n-2) + 4*(2-n)*a(n-3) + (n-3)*a(n-4) = 0. - \emph{R. J. Mathar}, Dec 11 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-09, revision 23) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(4 n - 1\right),\qquad p_{2}(n)=2 \left(5 n - 3\right),\qquad p_{3}(n)=- 4 \left(n - 2\right),\qquad p_{4}(n)=n - 3
\]
...
\section{The computation for A162481}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \frac{x^{3} - 3 x^{2} + 7 x - 1}{\left(x - 1\right)^{3}} .
\]
...
\[
(n + 1)\,a(n) -  2 \left(4 n - 1\right)\,a(n-1) + 2 \left(5 n - 3\right)\,a(n-2) -  4 \left(n - 2\right)\,a(n-3) + (n - 3)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A162481, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A162481.
