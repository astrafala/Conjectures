...
\title{A proof of the conjectured recurrence for OEIS A225034}
...
\begin{abstract}
OEIS A225034 carries a conjectured linear recurrence with polynomial coefficients, of
order 2, contributed by Bruno Berselli. We prove it. The generating function of the
...
closed form. Here it equals $x + 1$, a polynomial of degree $1$, which proves
...
OEIS A225034 is ``a(n) is the number of binary words containing n 1's and at most n 0's that do not contain the substring 101''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 7, 18, 48, 131, 363, 1017,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 \left(1 - x^{2}\right)}{3 x^{2} - 4 x + \sqrt{\left(x^{2} - 1\right) \left(x^{2} - 4 x \left(x - 1\right) - 1\right)} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) - (2*n+3)*a(n-1) - 3*(n-2)*a(n-2) = 0 for n>1. - \emph{Bruno Berselli}, May 02 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2022-05-24, revision 75) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 n - 3,\qquad p_{2}(n)=- 3 \left(n - 2\right)
\]
...
\section{The computation for A225034}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \left(x - 1\right)^{2} \left(x + 1\right) \left(3 x - 1\right) .
\]
...
\[
(n + 1)\,a(n) + (- 2 n - 3)\,a(n-1) -  3 \left(n - 2\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A225034, agreeing on all 13 terms compared.
...
vanish, for all 28 values of $n$ from $n=2$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A225034.
