...
\title{A proof of the conjectured recurrence for OEIS A188460}
...
\begin{abstract}
OEIS A188460 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x + 5$, a polynomial of degree $1$, which proves
...
OEIS A188460 is ``Diagonal sums of number triangle A119308''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 4, 9, 20, 45, 103, 238,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{3} - 2 x + \sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)} \left(x - 1\right) + 1}{2 x^{5}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+5)*(n+1)*a(n) -(2*n+5)*(n+2)*a(n-1) -(n+2)*(n+1)*a(n-2) -(n+1)*(2*n+1)*a(n-3) +(n-2)*(n+2)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 13 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2015-02-13, revision 15) the
...
\[
p_{0}(n)=\left(n + 1\right) \left(n + 5\right),\qquad p_{1}(n)=- \left(n + 2\right) \left(2 n + 5\right),\qquad p_{2}(n)=- \left(n + 1\right) \left(n + 2\right),\qquad p_{3}(n)=- \left(n + 1\right) \left(2 n + 1\right),\qquad p_{4}(n)=\left(n - 2\right) \left(n + 2\right)
\]
...
\section{The computation for A188460}
...
\[
\left(n + 1\right) \left(n + 5\right)\,a(n) -  \left(n + 2\right) \left(2 n + 5\right)\,a(n-1) -  \left(n + 1\right) \left(n + 2\right)\,a(n-2) -  \left(n + 1\right) \left(2 n + 1\right)\,a(n-3) + \left(n - 2\right) \left(n + 2\right)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A188460, agreeing on all 13 terms compared.
...
vanish, for all 27 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=3 x + 5$ is an
...
\url{https://oeis.org}, 2026. Sequence A188460.
