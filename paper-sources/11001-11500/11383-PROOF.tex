...
\title{A proof of the conjectured recurrence for OEIS A108308}
...
\begin{abstract}
OEIS A108308 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{2} + 24 x - 2$, a polynomial of degree $2$, which proves
...
OEIS A108308 is ``Expansion of 1/(1-x\^{}2*c(2*x)), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 1, 2, 9, 44, 245, 1462,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4}{x \sqrt{1 - 8 x} - x + 4} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n-1)*a(n) + (41-17*n)*a(n-1) + 4*(2*n-5)*a(n-2) + (n-1)*a(n-3) + 4*(5-2*n)*a(n-4) = 0. - \emph{R. J. Mathar}, Dec 10 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-05, revision 13) the
...
\[
p_{0}(n)=2 \left(n - 1\right),\qquad p_{1}(n)=41 - 17 n,\qquad p_{2}(n)=4 \left(2 n - 5\right),\qquad p_{3}(n)=n - 1,\qquad p_{4}(n)=- 4 \left(2 n - 5\right)
\]
...
\section{The computation for A108308}
...
\[
2 \left(n - 1\right)\,a(n) + (41 - 17 n)\,a(n-1) + 4 \left(2 n - 5\right)\,a(n-2) + (n - 1)\,a(n-3) -  4 \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A108308, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{2} + 24 x - 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A108308.
