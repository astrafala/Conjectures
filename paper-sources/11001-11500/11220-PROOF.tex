...
\title{A proof of the conjectured recurrence for OEIS A114194}
...
\begin{abstract}
OEIS A114194 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{2} - 16 x + 6$, a polynomial of degree $2$, which proves
...
OEIS A114194 is ``Expansion of 1/(1+x*(2-x)*c(-2*x)), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -2, 9, -46, 261, -1578, 9969, -65030,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4}{x - \left(x - 2\right) \sqrt{8 x + 1} + 2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 6*(n+1)*a(n) + (37n-29)*a(n-1) + 2*(45-41n)*a(n-2) + (47n-87)*a(n-3) + 4*(5-2n)*a(n-4) = 0. - \emph{R. J. Mathar}, Dec 10 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2024-05-24, revision 14) the
...
\[
p_{0}(n)=6 \left(n + 1\right),\qquad p_{1}(n)=37 n - 29,\qquad p_{2}(n)=- 2 \left(41 n - 45\right),\qquad p_{3}(n)=47 n - 87,\qquad p_{4}(n)=- 4 \left(2 n - 5\right)
\]
...
\section{The computation for A114194}
...
\[
6 \left(n + 1\right)\,a(n) + (37 n - 29)\,a(n-1) -  2 \left(41 n - 45\right)\,a(n-2) + (47 n - 87)\,a(n-3) -  4 \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A114194, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{2} - 16 x + 6$ is an
...
\url{https://oeis.org}, 2026. Sequence A114194.
