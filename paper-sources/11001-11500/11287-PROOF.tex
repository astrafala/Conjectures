...
\title{A proof of the conjectured recurrence for OEIS A244886}
...
\begin{abstract}
OEIS A244886 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x^{4} + 3 x^{3} + x^{2} - x$, a polynomial of degree $4$, which proves
...
OEIS A244886 is ``G.f.: (1-x+sqrt(1-2*x-3*x\^{}2))/(1-3*x+x\^{}2+x\^{}3+(1-x\^{}2)*sqrt(1-2*x-3*x\^{}2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 4, 9, 22, 56, 147,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x + \sqrt{- 3 x^{2} - 2 x + 1} + 1}{x^{3} + x^{2} - 3 x - \left(x^{2} - 1\right) \sqrt{- 3 x^{2} - 2 x + 1} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: n*a(n) +(-5*n+3)*a(n-1) +2*(n)*a(n-2) +(13*n-30)*a(n-3) +3*(-1)*a(n-4) +(-8*n+21)*a(n-5) +3*(-n+3)*a(n-6)=0. - \emph{R. J. Mathar}, Jan 24 2020
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 34) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=3 - 5 n,\qquad p_{2}(n)=2 n,\qquad p_{3}(n)=13 n - 30,\qquad p_{4}(n)=-3,\qquad p_{5}(n)=21 - 8 n,\qquad p_{6}(n)=- 3 \left(n - 3\right)
\]
...
\section{The computation for A244886}
...
\[
n\,a(n) + (3 - 5 n)\,a(n-1) + 2 n\,a(n-2) + (13 n - 30)\,a(n-3) - 3\,a(n-4) + (21 - 8 n)\,a(n-5) -  3 \left(n - 3\right)\,a(n-6) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A244886, agreeing on all 13 terms compared.
...
vanish, for all 24 values of $n$ from $n=6$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=3 x^{4} + 3 x^{3} + x^{2} - x$ is an
...
\url{https://oeis.org}, 2026. Sequence A244886.
