...
\title{A proof of the conjectured recurrence for OEIS A270363}
...
\begin{abstract}
OEIS A270363 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $126 x^{3} + 24 x^{2} - 24 x$, a polynomial of degree $3$, which proves
...
OEIS A270363 is ``a(n) = (n+1)*Sum\_\{k=0..(n-1)/2\}((binomial(2*n-3*k-2,n-k-1))/(n-k))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 2, 3, 10, 30, 101, 350, 1250,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{\left(\sqrt{1 - 4 x} - 1\right) \left(6 x + \sqrt{1 - 4 x} - 1\right)}{\left(4 x + \sqrt{1 - 4 x} - 1\right) \left(x \sqrt{1 - 4 x} - x + 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(7*n\^{}2-17*n-2) *a(n) +(-35*n\^{}3+99*n\^{}2+20*n-120) *a(n-1) +2* (2*n-5) *(7*n\^{}2-3*n-12)*a(n-2) +n*(7*n\^{}2-17*n-2) *a(n-3) +2*-(2*n-5) *(7*n\^{}2-3*n-12) *a(n-4)=0. - \emph{R. J. Mathar}, Mar 22 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-06-04, revision 24) the
...
\[
p_{0}(n)=n \left(7 n^{2} - 17 n - 2\right),\qquad p_{1}(n)=- 35 n^{3} + 99 n^{2} + 20 n - 120,\qquad p_{2}(n)=2 \left(2 n - 5\right) \left(7 n^{2} - 3 n - 12\right),\qquad p_{3}(n)=n \left(7 n^{2} - 17 n - 2\right),\qquad p_{4}(n)=- 2 \left(2 n - 5\right) \left(7 n^{2} - 3 n - 12\right)
\]
...
\section{The computation for A270363}
...
\[
n \left(7 n^{2} - 17 n - 2\right)\,a(n) + (- 35 n^{3} + 99 n^{2} + 20 n - 120)\,a(n-1) + 2 \left(2 n - 5\right) \left(7 n^{2} - 3 n - 12\right)\,a(n-2) + n \left(7 n^{2} - 17 n - 2\right)\,a(n-3) -  2 \left(2 n - 5\right) \left(7 n^{2} - 3 n - 12\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A270363, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=126 x^{3} + 24 x^{2} - 24 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A270363.
