...
\title{A proof of the conjectured recurrence for OEIS A174169}
...
\begin{abstract}
OEIS A174169 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 - 6 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A174169 is ``A generalized Chebyshev transform of the Motzkin numbers A001006''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, -1, -2, 0, 0, -3, 1,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{3 x^{2} - x - \sqrt{\left(3 x^{2} - 3 x + 1\right) \left(3 x^{2} + x + 1\right)} + 1}{2 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) -(2*n+1)*a(n-1) +3*(n-1)*a(n-2) +3*(5-2*n)*a(n-3) +9*(n-4)*a(n-4)=0. - \emph{R. J. Mathar}, Sep 30 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-17, revision 21) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 2 n - 1,\qquad p_{2}(n)=3 \left(n - 1\right),\qquad p_{3}(n)=- 3 \left(2 n - 5\right),\qquad p_{4}(n)=9 \left(n - 4\right)
\]
...
\section{The computation for A174169}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(3 x^{2} - 3 x + 1\right) \left(3 x^{2} + x + 1\right) .
\]
...
\[
(n + 2)\,a(n) + (- 2 n - 1)\,a(n-1) + 3 \left(n - 1\right)\,a(n-2) -  3 \left(2 n - 5\right)\,a(n-3) + 9 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A174169, agreeing on all 13 terms compared.
...
vanish, for all 37 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=2 - 6 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A174169.
