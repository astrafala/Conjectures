...
\title{A proof of the conjectured recurrence for OEIS A122920}
...
\begin{abstract}
OEIS A122920 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 12 x$, a polynomial of degree $1$, which proves
...
OEIS A122920 is ``Diagonal sums of number triangle A122919''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 12, 39, 129, 436, 1498,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x - 1\right) \left(2 x^{2} + 2 x + \sqrt{1 - 4 x} - 1\right)}{2 x^{3} \left(x + 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*n*(n+3)*a(n) - (7*n\^{}2+9*n+4)*a(n-1) - 2*(n+1)*(2*n+1)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 05 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2017-07-08, revision 36) the
...
\[
p_{0}(n)=2 n \left(n + 3\right),\qquad p_{1}(n)=- 7 n^{2} - 9 n - 4,\qquad p_{2}(n)=- 2 \left(n + 1\right) \left(2 n + 1\right)
\]
...
\section{The computation for A122920}
...
\[
2 n \left(n + 3\right)\,a(n) + (- 7 n^{2} - 9 n - 4)\,a(n-1) -  2 \left(n + 1\right) \left(2 n + 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A122920, agreeing on all 13 terms compared.
...
vanish, for all 27 values of $n$ from $n=2$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 12 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A122920.
