...
\title{A proof of the conjectured recurrence for OEIS A184120}
...
\begin{abstract}
OEIS A184120 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $1 - 2 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A184120 is ``Expansion of (1/(1+4x+2x\^{}2))*c(x/(1+4x+2x\^{}2)), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -3, 8, -23, 70, -218, 688, -2195,\ \dots
\]
...
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(2n+1)*a(n-1) +4*(n-1)*a(n-2) +4*(2n-5)*a(n-3) +4*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 17 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2022-09-08, revision 26) the
...
section of A184120, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=1 - 2 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A184120.
