...
\title{A proof of the conjectured recurrence for OEIS A160823}
...
\begin{abstract}
OEIS A160823 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{3} + 3 x^{2} - 3 x + 2$, a polynomial of degree $3$, which proves
...
OEIS A160823 is ``A transform of the large Schroeder numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 5, 13, 27, 69, 161,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} + x + \sqrt{x^{4} + 6 x^{3} - 5 x^{2} - 2 x + 1} - 1}{2 x^{2} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) -3*(n+1)*a(n-1) +3(2-n)*a(n-2) +(11*n-20)*a(n-3) +(11-5*n)*a(n-4) + (4-n)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 16 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2022-09-08, revision 12) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 3 \left(n + 1\right),\qquad p_{2}(n)=- 3 \left(n - 2\right),\qquad p_{3}(n)=11 n - 20,\qquad p_{4}(n)=11 - 5 n,\qquad p_{5}(n)=4 - n
\]
...
\section{The computation for A160823}
...
\[
(n + 2)\,a(n) -  3 \left(n + 1\right)\,a(n-1) -  3 \left(n - 2\right)\,a(n-2) + (11 n - 20)\,a(n-3) + (11 - 5 n)\,a(n-4) + (4 - n)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A160823, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{3} + 3 x^{2} - 3 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A160823.
