...
\title{The empirical recurrence for OEIS A302078, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A302078 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A302078 is ``Number of n X 5 0..1 arrays with every element equal to 0, 1, 3 or 4 horizontally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
8, 35, 154, 692, 3048, 13712, 61536, 277248,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) +17*a(n-2) -38*a(n-3) -68*a(n-4) +136*a(n-5) +64*a(n-6) -128*a(n-7) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 10 2023 13:14:51, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 17\,a(n-2) - 38\,a(n-3) - 68\,a(n-4) + 136\,a(n-5) + 64\,a(n-6) - 128\,a(n-7)
\end{equation}
for all $n>9$.
...
\#\{\text{neighbours with a value }equal to\ x_{t,u}\}\in\{0,1,3,4\}
\end{equation}
...
The threshold below is exact rather than merely sufficient: at $n=9$ the conjectured
...
the middle row. The vector $w$ was formed by $7$ applications of $G$ in exact integer
arithmetic and $\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly.
Every one of those integers vanishes from the index corresponding to $n=9+1$ onwards,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302078.
