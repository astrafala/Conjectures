...
\title{The empirical recurrence for OEIS A234260, proved by transfer matrix}
...
\begin{abstract}
OEIS A234260 counts $3$-column arrays over $\{0,\dots,2\}$ in which every
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=27$ vertices, which
...
OEIS A234260 is ``Number of (n+1) X (2+1) 0..2 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
46, 88, 170, 358, 754, 1690, 3746,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) + 6*a(n-2) - 24*a(n-3) + 4*a(n-4) + 36*a(n-5) - 24*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 21:33:55, revision 13) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 6\,a(n-2) - 24\,a(n-3) + 4\,a(n-4) + 36\,a(n-5) - 24\,a(n-6)
\end{equation}
...
Let $V$ consist of all of $\{0,\dots,2\}^{3}$, and put $S=|V|=27$. For $r,s\in V$ declare $r\to s$ an edge
...
$j=0,1,\dots$. They vanish from $j=5-6+1$ onwards, and the run was continued past
...
the $27$ published indices; the values agree exactly. That is what ties the digraph to
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=27$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A234260.
