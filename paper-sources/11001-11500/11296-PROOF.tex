...
\title{The empirical closed form for OEIS A219516, proved}
...
\begin{abstract}
OEIS A219516 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=954$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219516 is ``Number of nX5 arrays of the minimum value of corresponding elements and their horizontal, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 nX5 array.''. It has offset $1$ and begins
\[
5, 6, 43, 166, 620, 2125, 6994, 22202,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:07:29, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=954$
...
states number $S=954$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219516 records is correct for all $n>14$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219516.
