...
\title{A proof of the conjectured recurrence for OEIS A005560}
...
\begin{abstract}
OEIS A005560 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A005560 is ``Number of walks on square lattice. Column y=2 of A052174''. It has offset $2$ and begins
\[
a(2),\dots,a(9)\;=\;1, 3, 15, 45, 189, 588, 2352, 7560,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = C(n+3, ceiling(n/2))*C(n+2, floor(n/2)) - C(n+3, ceiling((n-1)/2))*C(n+2, floor((n-1)/2)). - \emph{Paul D. Hanna}, Apr 16 2004
\end{quote}
that is,
\begin{equation}\label{eq:cf}
a(n)\;=\;{\binom{n}{\left\lfloor{\frac{n}{2}}\right\rfloor - 1}} {\binom{n + 1}{\left\lceil{\frac{n}{2}}\right\rceil - 1}} - {\binom{n}{\left\lfloor{\frac{n}{2} - \frac{3}{2}}\right\rfloor}} {\binom{n + 1}{\left\lceil{\frac{n}{2} - \frac{3}{2}}\right\rceil}} .
\end{equation}
...
assumed: the formula as posted gives $a(n+2)$, and is used here shifted accordingly.

The entry separately carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n-1)*(n-2)*(2*n+1)*(n+5)*(n+4)*a(n) -4*n*(n+1)*(2*n\^{}2+4*n+19)*a(n-1) -16*n\^{}2*(n-1)*(2*n+3)*(n+1)*a(n-2)=0. - \emph{R. J. Mathar}, Apr 02 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2025-02-03, revision 39) the
...
\left(n - 2\right) \left(n - 1\right) \left(n + 4\right) \left(n + 5\right) \left(2 n + 1\right)\,a(n) -  4 n \left(n + 1\right) \left(2 n^{2} + 4 n + 19\right)\,a(n-1) -  16 n^{2} \left(n - 1\right) \left(n + 1\right) \left(2 n + 3\right)\,a(n-2)
\end{equation}
...
a(2m)\;=\;e_{0}(m)\;=\;\frac{3 {\binom{2 m + 1}{m + 2}} \left(2 m\right)!}{\left(m - 1\right)! \left(m + 2\right)!},
\qquad
a(2m+1)\;=\;e_{1}(m)\;=\;\frac{6 \left(2 m + 1\right)!^{2}}{\left(m + 2\right) m!^{2} \left(m - 1\right)! \left(m + 3\right)!},
\end{equation}
...
\section{The computation for A005560}
...
entry \href{https://oeis.org/A005560}{A005560}, 2025-02-03.
