...
\title{The empirical closed form for OEIS A219211, proved}
...
\begin{abstract}
OEIS A219211 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=13$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219211 is ``Number of n X 2 arrays of the minimum value of corresponding elements and their horizontal, vertical, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..2 n X 2 array.''. It has offset $1$ and begins
\[
3, 3, 7, 12, 21, 35, 57, 91,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/24)*n\^{}4 - (5/12)*n\^{}3 + (59/24)*n\^{}2 - (37/12)*n + 1 for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 11 2018 10:09:09, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=13$
...
states number $S=13$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219211 records is correct for all $n>3$.
\end{theorem}
...
$1 + 42 - 1$, which is more than the $5$ consecutive agreements a
...
First, the model reproduces all $42$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219211.
