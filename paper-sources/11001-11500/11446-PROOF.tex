...
\title{A proof of the conjectured recurrence for OEIS A177172}
...
\begin{abstract}
OEIS A177172 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $22 x^{3} - 41 x^{2} + 16 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A177172 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=10, k=0 and l=-2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 10, 18, 134, 626, 4254, 25850, 177270,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 44 x^{3} + 32 x^{2} + 5 x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-3*n+1)*a(n-1) +(-27*n+59)*a(n-2) +2*(38*n-117)*a(n-3) +44*(-n+4)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 21 2016
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=59 - 27 n,\qquad p_{3}(n)=2 \left(38 n - 117\right),\qquad p_{4}(n)=- 44 \left(n - 4\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + (59 - 27 n)\,a(n-2) + 2 \left(38 n - 117\right)\,a(n-3) -  44 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A177172, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=22 x^{3} - 41 x^{2} + 16 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A177172.
