...
\title{The empirical closed form for OEIS A219376, proved}
...
\begin{abstract}
OEIS A219376 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=141$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219376 is ``Number of nX3 arrays of the minimum value of corresponding elements and their horizontal, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..2 nX3 array.''. It has offset $1$ and begins
\[
6, 14, 84, 344, 1271, 4116, 12452, 35741,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:00:22, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=141$
...
states number $S=141$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219376 records is correct for all $n>6$.
\end{theorem}
...
$1 + 26 - 1$, which is more than the $17$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219376.
