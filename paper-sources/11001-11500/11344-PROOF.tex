...
\title{The empirical recurrence for OEIS A207718, proved by transfer matrix}
...
\begin{abstract}
OEIS A207718 counts arrays over $\{0,\dots,1\}$ of width $3$ that avoid the patterns
$(0,0,1), (1,0,0)$ along one axis and $(0,0,0), (0,0,1)$ along the other, and carries an empirical recurrence of
order $5$ contributed by R.~H.~Hardin. It is true. The patterns have length $3$, so
...
$\mathbf{1}^{\!\top}P^{\,n-2-5}q(P)\mathbf{1}$, which vanishes identically
...
OEIS A207718 is ``Number of 3 X n 0..1 arrays avoiding 0 0 0 and 0 0 1 horizontally and 0 0 1 and 1 0 0 vertically.''. It has offset
$1$ and begins
\[
6, 36, 114, 450, 1644, 6186, 23010,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) + 7*a(n-2) - a(n-3) - 4*a(n-4) + a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 05 2018 11:26:14, revision
8) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 7\,a(n-2) - a(n-3) - 4\,a(n-4) + a(n-5)
\end{equation}
...
$(0,0,1), (1,0,0)$ as $3$ consecutive entries. There are $6$ admissible columns; that condition
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-5}$ whenever $n-5\ge2$. Hence
\eqref{eq:conj} holds for every $n>t$ exactly when $u_j=0$ for every
$j>t-2-5$. Moreover $(u_j)$ satisfies the monic linear recurrence whose
...
$P^{\,n-2-5}$ gives the identity. The equivalence follows by letting the exponent
...
\begin{proof}
The vector $q(P)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $5$ matrix--vector
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207718.
