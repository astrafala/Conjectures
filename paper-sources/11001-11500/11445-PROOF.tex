...
\title{A proof of the conjectured recurrence for OEIS A063395}
...
\begin{abstract}
OEIS A063395 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A063395 is ``T(2n,n) with T(n,m) as in A063394''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 19, 131, 979, 7683, 62099, 511619,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4 x \left(x - 1\right) + \left(2 x - 1\right) \sqrt{9 x^{2} - 10 x + 1}}{\left(x - 1\right) \sqrt{9 x^{2} - 10 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+1)*a(n) +(11*n-17)*a(n-1) +(-19*n+43)*a(n-2) +9*(n-3)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 10 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2020-07-29, revision 10) the
...
\[
p_{0}(n)=1 - n,\qquad p_{1}(n)=11 n - 17,\qquad p_{2}(n)=43 - 19 n,\qquad p_{3}(n)=9 \left(n - 3\right)
\]
...
\section{The computation for A063395}
...
\[
(1 - n)\,a(n) + (11 n - 17)\,a(n-1) + (43 - 19 n)\,a(n-2) + 9 \left(n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A063395, agreeing on all 13 terms compared.
...
vanish, for all 19 values of $n$ from $n=3$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A063395.
