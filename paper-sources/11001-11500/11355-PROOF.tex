...
\title{The empirical closed form for OEIS A219853, proved}
...
\begin{abstract}
OEIS A219853 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=18$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219853 is ``Number of 2 X n arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 2 X n array.''. It has offset $1$ and begins
\[
6, 7, 17, 33, 56, 86, 125, 173,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (9/2)*n\^{}2 - (39/2)*n + 41 for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 28 2018 12:16:04, revision 7)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=18$
...
states number $S=18$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219853 records is correct for all $n>4$.
\end{theorem}
...
$1 + 47 - 1$, which is more than the $3$ consecutive agreements a
...
First, the model reproduces all $47$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219853.
