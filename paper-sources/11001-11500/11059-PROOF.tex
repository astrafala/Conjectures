...
\title{A proof of the conjectured recurrence for OEIS A101478}
...
\begin{abstract}
OEIS A101478 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $48 x^{3}$, a polynomial of degree $3$, which proves
...
OEIS A101478 is ``G.f. satisfies A(x) = x*(1+A(x))\textasciicircum{}4/(1+A(x)\textasciicircum{}2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 4, 21, 124, 782, 5144, 34845,\ \dots
\]
The entry posts the generating function
\begin{quote}\small
\texttt{(1-(1-8*x)\textasciicircum{}(1/4))/(1+(1-8*x)\textasciicircum{}(1/4))-1}
\end{quote}
...
Adding the correcting polynomial $1$ leaves the function algebraic and changes
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt[4]{1 - 8 x}}{\sqrt[4]{1 - 8 x} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-1)*(n+1)*a(n) -12*n*(n-1)*(2*n-3)*a(n-1) +12*(n-1)*(16*n\textasciicircum{}2-64*n+65)*a(n-2) -16*(2*n-5)*(4*n-9)*(4*n-11)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 10 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-12-18, revision 32) the
...
\[
p_{0}(n)=n \left(n - 1\right) \left(n + 1\right),\qquad p_{1}(n)=- 12 n \left(n - 1\right) \left(2 n - 3\right),\qquad p_{2}(n)=12 \left(n - 1\right) \left(16 n^{2} - 64 n + 65\right),\qquad p_{3}(n)=- 16 \left(2 n - 5\right) \left(4 n - 11\right) \left(4 n - 9\right)
\]
...
\[
n \left(n - 1\right) \left(n + 1\right)\,a(n) -  12 n \left(n - 1\right) \left(2 n - 3\right)\,a(n-1) + 12 \left(n - 1\right) \left(16 n^{2} - 64 n + 65\right)\,a(n-2) -  16 \left(2 n - 5\right) \left(4 n - 11\right) \left(4 n - 9\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A101478, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=48 x^{3}$ is an
...
\url{https://oeis.org}, 2026. Sequence A101478.
