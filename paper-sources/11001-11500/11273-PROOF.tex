...
\title{A proof of the conjectured recurrence for OEIS A126323}
...
\begin{abstract}
OEIS A126323 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $80641332 x^{3} - 72161172 x^{2} + 15693336 x$, a polynomial of degree $3$, which proves
...
OEIS A126323 is ``Number of branches of length 1 in all hex trees with n edges''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 3, 2, 15, 80, 399, 1956, 9546,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(3 x - 1\right)^{2} \left(5 x^{2} - 9 x + \left(3 x - 2\right) \sqrt{5 x^{2} - 6 x + 1} + 2\right)}{2 x^{2} \sqrt{5 x^{2} - 6 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+2)*(1176*n\^{}2+350135*n-2095015)*a(n) +3*(-392*n\^{}3+1110577*n\^{}2-6230546*n-1231580)*a(n-1) +(43512*n\^{}3-9103273*n\^{}2+68264245*n-103131090) *a(n-2) -15*(n-4)*(2744*n\^{}2-398547*n+1917624)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 17 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-12-30, revision 7) the
...
\[
p_{0}(n)=- \left(n + 2\right) \left(1176 n^{2} + 350135 n - 2095015\right),\qquad p_{1}(n)=- 3 \left(392 n^{3} - 1110577 n^{2} + 6230546 n + 1231580\right),\qquad p_{2}(n)=43512 n^{3} - 9103273 n^{2} + 68264245 n - 103131090,\qquad p_{3}(n)=- 15 \left(n - 4\right) \left(2744 n^{2} - 398547 n + 1917624\right)
\]
...
\section{The computation for A126323}
...
\[
B(x)\;=\;80641332 x^{3} - 72161172 x^{2} + 15693336 x ,
\]
...
\[
- \left(n + 2\right) \left(1176 n^{2} + 350135 n - 2095015\right)\,a(n) -  3 \left(392 n^{3} - 1110577 n^{2} + 6230546 n + 1231580\right)\,a(n-1) + (43512 n^{3} - 9103273 n^{2} + 68264245 n - 103131090)\,a(n-2) -  15 \left(n - 4\right) \left(2744 n^{2} - 398547 n + 1917624\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A126323, agreeing on all 13 terms compared.
...
vanish, for all 19 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=80641332 x^{3} - 72161172 x^{2} + 15693336 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A126323.
