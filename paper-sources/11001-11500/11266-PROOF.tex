...
\title{The empirical recurrence for OEIS A235287, proved by transfer matrix}
...
\begin{abstract}
OEIS A235287 counts $7$-column arrays over $\{0,\dots,3\}$ in which every
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=16384$ vertices, which
...
OEIS A235287 is ``Number of (n+1) X (6+1) 0..3 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
472, 572, 680, 908, 1168, 1724, 2408,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) +3*a(n-2) -15*a(n-3) +4*a(n-4) +18*a(n-5) -12*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 18 2022 23:38:07, revision 9) the statement is
...
Let $V$ consist of all of $\{0,\dots,3\}^{7}$, and put $S=|V|=16384$. For $r,s\in V$ declare $r\to s$ an edge
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235287.
