...
\title{The empirical recurrence for OEIS A251374, proved by transfer matrix}
...
\begin{abstract}
OEIS A251374 counts $2$-column arrays over $\{0,\dots,3\}$ in which every
$2\times2$ subblock sums to one of $\{5, 6, 7\}$, and carries an empirical
recurrence of order $4$ contributed by R.~H.~Hardin. It is true, and it is not an
...
\mathbf{1}$ for the adjacency matrix $M$ of that digraph, on $S=16$ vertices. Writing
...
OEIS A251374 is ``Number of (n+1) X (1+1) 0..3 arrays with every 2 X 2 subblock summing to 5 6 or 7.''. It has offset $1$
and begins
\[
124, 1048, 9124, 80296, 709396, 6276040, 55551652, 491795368,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 10*a(n-1) - 3*a(n-2) - 66*a(n-3) + 24*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 29 2018 10:37:16, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;10\,a(n-1) - 3\,a(n-2) - 66\,a(n-3) + 24\,a(n-4)
\end{equation}
for all $n>3$, which is every $n$ at which a recurrence of order $4$ can be
stated.

\section{The rows form a digraph}

Let $V=\{0,1,\dots,3\}^{2}$, the set of possible rows, so $|V|=S=16$. For
$r,s\in V$ declare $r\to s$ an edge when, for every $j$ with $1\le j\le 2-1$,
\begin{itemize}
\item $r_j+r_{j+1}+s_j+s_{j+1}\in\{5, 6, 7\}$;
...
\begin{proof}
An array with $n+1$ rows and $2$ columns is precisely a sequence
...
Let $q(t)=t^{4}-\sum_{i}c_i\,t^{4-i}$ be the characteristic polynomial of
\eqref{eq:conj}, with the $c_i$ read off that equation.

\begin{lemma}\label{lem:crit}
For every $n\ge4$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n-4}\,q(M)\,\mathbf{1} .
\]
Hence \eqref{eq:conj} holds for every $n\ge4$ if and only if
...
=\mathbf{1}^{\!\top}M^{\,n-4}\Bigl(M^{4}-\sum_i c_i M^{4-i}\Bigr)\mathbf{1},
\]
which is the stated identity. The equivalence follows by letting $n-4=j$ run over
...
\begin{proof}
The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $4$ matrix--vector
...
$j=3-4+1$ onwards, and the run was continued past $S$ consecutive zeros, which by
...
bound $j<S=16$ rather than to a sampled prefix, in exact integer arithmetic, and returned
zero throughout.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A251374.
