...
\title{The empirical recurrence for OEIS A208642, proved by transfer matrix}
...
\begin{abstract}
OEIS A208642 counts $7\times n$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
...
OEIS A208642 is ``Number of 7 X n 0..1 arrays with new values 0..1 introduced in row major order and no element equal to more than one of its immediate leftward or upward or right-upward antidiagonal neighbors.''. It has offset $1$ and begins
\[
64, 610, 2552, 7548, 18888, 43284, 94320, 199299,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 8*a(n-1) - 27*a(n-2) + 50*a(n-3) - 55*a(n-4) + 36*a(n-5) - 13*a(n-6) + 2*a(n-7) for n>11.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 05 2018 13:55:01, revision 9) this is still
...
$\{0,\dots,1\}^{7}$. The neighbourhood of a cell in column $u_i$ meets $u_{i-1}$,
...
$7$ cells, given $r$ above and $t$ below. An array with $L$ columns is then a walk of length
...
By Lemma~\ref{lem:crit} the recurrence holds for every $n>11$. The bound is exact: at $n=11$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208642.
