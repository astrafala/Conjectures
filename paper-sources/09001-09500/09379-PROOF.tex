...
\title{The empirical recurrence for OEIS A236149, proved by transfer matrix}
...
\begin{abstract}
OEIS A236149 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=414$
...
OEIS A236149 is ``Number of (n+1) X (2+1) 0..2 arrays colored with the sets of distinct values in every 2 X 2 subblock.''. It has offset $1$ and begins
\[
414, 2502, 16128, 104436, 676980, 4389660, 28462080, 184549536,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 19*a(n-2) + 24*a(n-3) - a(n-4) - 22*a(n-5) - 11*a(n-6) - 2*a(n-7) - 2*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 20 2018 14:20:25, revision 7) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The $2\times2$ subblocks of an $(n+1)\times3$ array form an
$n\times2$ grid. Colour each of them by the SET of values occurring in it. ``Coloured with'' is the entry's way of
...
\]
over the $S=414$ pairs whose own subblock row is properly coloured.
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 19\,a(n-2) + 24\,a(n-3) - a(n-4) - 22\,a(n-5) - 11\,a(n-6) - 2\,a(n-7) - 2\,a(n-8)
\]
for every $n>8$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A236149.
