...
\title{The empirical recurrence for OEIS A299361, proved by transfer matrix}
...
\begin{abstract}
OEIS A299361 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{0,2,3,4,5,8\}$, and it carries an empirical
...
OEIS A299361 is ``Number of nX3 0..1 arrays with every element equal to 0, 2, 3, 4, 5 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 13, 23, 125, 420, 1943, 9012, 42321,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 08 2018 13:58:50, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) + 2\,a(n-2) - 38\,a(n-3) - 24\,a(n-4) + 163\,a(n-5) + 33\,a(n-6) - 337\,a(n-7) + 66\,a(n-8) + 312\,a(n-9) - 116\,a(n-10) - 92\,a(n-11) + 6\,a(n-12)
\end{equation}
for all $n>14$.
...
has score in $\{0,2,3,4,5,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,2,3,4,5,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,2,3,4,5,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,2,3,4,5,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,2,3,4,5,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A299361.
