...
\title{The empirical recurrence for OEIS A252526, proved by transfer matrix}
...
\begin{abstract}
OEIS A252526 counts $(n+2)\times4$ arrays over $\{0,\dots,3\}$ subject to a condition imposed on
...
OEIS A252526 is ``Number of (n+2) X (2+2) 0..3 arrays with every 3 X 3 subblock row and diagonal sum equal to 0 2 3 6 or 7 and every 3 X 3 column and antidiagonal sum not equal to 0 2 3 6 or 7.''. It has offset $1$ and begins
\[
730, 337, 334, 426, 676, 964, 1344, 2312,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-3) - 8*a(n-6) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 04 2018 11:03:47, revision 10) this is still
...
\[
\text{every row, diagonal sum} \in \{0,2,3,6,7\}\ \text{ and }\ \text{every column, antidiagonal sum} \notin \{0,2,3,6,7\}
\]
...
exactly. Those integers vanish from the index corresponding to $n=10+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>10$. The bound is exact: at $n=10$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
First, the transfer model reproduces all $32$ terms the entry publishes, exactly, in
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252526.
