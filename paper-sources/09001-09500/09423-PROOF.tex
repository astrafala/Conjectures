...
\title{The empirical recurrence for OEIS A280856, proved by transfer matrix}
...
\begin{abstract}
OEIS A280856 counts $n\times5$ arrays over $\{0,\dots,2\}$ subject to a condition imposed on
...
OEIS A280856 is ``Number of nX5 0..2 arrays with no element equal to more than one of its king-move neighbors and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
30, 442, 256, 216, 282, 491, 968, 1857,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) +a(n-2) -a(n-3) +2*a(n-4) -a(n-5) +4*a(n-6) -a(n-7) -a(n-8) -2*a(n-9) -3*a(n-12) for n>18
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 09 2017 09:35:45, revision 4) this is still
...
$(r,s)\in(\{0,\dots,2\}^{5}\cup\{\varnothing\})\times\{0,\dots,2\}^{5}$,
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-2) - a(n-3) + 2\,a(n-4) - a(n-5) + 4\,a(n-6) - a(n-7) - a(n-8) - 2\,a(n-9) - 3\,a(n-12)
\]
for every $n>18$.
\end{theorem}
...
By Lemma~\ref{lem:crit} the recurrence holds for every $n>18$. The bound is exact: at $n=18$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
First, the transfer model reproduces all $31$ terms the entry publishes, exactly, in
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A280856.
