...
\title{The empirical recurrence for OEIS A251294, proved by transfer matrix}
...
\begin{abstract}
OEIS A251294 counts $3$-column arrays over $\{0,\dots,3\}$ in which every
$2\times2$ subblock sums to one of $\{3, 6, 9\}$, and carries an empirical
recurrence of order $7$ contributed by R.~H.~Hardin. It is true, and it is not an
...
OEIS A251294 is ``Number of (n+1) X (2+1) 0..3 arrays with every 2 X 2 subblock summing to 3 6 or 9.''. It has offset $1$
and begins
\[
444, 3140, 22536, 164480, 1221748, 9240612, 71163896, 557726448,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 27*a(n-1) - 260*a(n-2) + 970*a(n-3) - 405*a(n-4) - 3449*a(n-5) - 722*a(n-6) + 840*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 28 2018 08:04:13, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;27\,a(n-1) - 260\,a(n-2) + 970\,a(n-3) - 405\,a(n-4) - 3449\,a(n-5) - 722\,a(n-6) + 840\,a(n-7)
\end{equation}
...
Let $q(t)=t^{7}-\sum_{i}c_i\,t^{7-i}$ be the characteristic polynomial of
\eqref{eq:conj}, with the $c_i$ read off that equation.

\begin{lemma}\label{lem:crit}
For every $n\ge7$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\mathbf{1}^{\!\top}M^{\,n-7}\,q(M)\,\mathbf{1} .
\]
Hence \eqref{eq:conj} holds for every $n\ge7$ if and only if
...
=\mathbf{1}^{\!\top}M^{\,n-7}\Bigl(M^{7}-\sum_i c_i M^{7-i}\Bigr)\mathbf{1},
\]
which is the stated identity. The equivalence follows by letting $n-7=j$ run over
...
\begin{proof}
The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $7$ matrix--vector
...
$j=6-7+1$ onwards, and the run was continued past $S$ consecutive zeros, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251294.
