...
\title{The empirical recurrence for OEIS A230726, proved by transfer matrix}
...
\begin{abstract}
OEIS A230726 carries an empirical recurrence of order $6$ contributed by R.~H.~Hardin. It is
true, and it is decidable rather than empirical. The entry's condition is local to a bounded window of consecutive lines, so
the admissible configurations of such a window are the vertices of a finite digraph and the
arrays counted are exactly the walks in it; hence $a(n)$ is a walk count on $S=4944$ vertices
...
OEIS A230726 is ``Number of n X 4 0..5 arrays x(i,j) with each element horizontally or antidiagonally next to at least one element with value (x(i,j)+1) mod 6, and upper left element zero.''. It has offset $1$ and begins
\[
0, 2, 30, 396, 6212, 95412, 1459082, 22379112,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 16*a(n-1) - 12*a(n-2) + 87*a(n-3) - 918*a(n-4) + 61*a(n-5) - a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 22 2018 11:50:59, revision 9) this is still
...
admissible array is then exactly a walk, so with $M$ the adjacency matrix on $S=4944$ vertices
...
\begin{theorem}
\[
a(n)\;=\;16\,a(n-1) - 12\,a(n-2) + 87\,a(n-3) - 918\,a(n-4) + 61\,a(n-5) - a(n-6)
\]
for every $n>7$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230726.
