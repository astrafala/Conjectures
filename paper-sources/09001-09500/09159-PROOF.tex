...
\title{The empirical recurrence for OEIS A206121, proved by transfer matrix}
...
\begin{abstract}
OEIS A206121 counts arrays over $\{0,\dots,2\}$ under a condition on the number of clockwise edge increases
...
a bounded window of consecutive rows, so the count is a walk count on $S=50$ states and is
...
OEIS A206121 is ``Number of (n+1) X 2 0..2 arrays with every 2 X 2 subblock having nonzero determinant and having the same number of clockwise edge increases as its horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
50, 170, 558, 1906, 6386, 21940, 74714, 258496,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + 13*a(n-2) - 9*a(n-3) - 50*a(n-4) - 43*a(n-5) + 4*a(n-6) - 4*a(n-8) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 13 2018 12:00:06, revision 8) this is still
...
\]
on the $S=50$ admissible pairs.

At $n=1$ the array has two rows, so the grid is a single row of $1$
subblock; the entry's first term is $50$, which the model
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 13\,a(n-2) - 9\,a(n-3) - 50\,a(n-4) - 43\,a(n-5) + 4\,a(n-6) - 4\,a(n-8)
\]
for every $n>9$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206121.
