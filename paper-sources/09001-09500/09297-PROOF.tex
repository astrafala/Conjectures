...
\title{The empirical recurrence for OEIS A237229, proved by transfer matrix}
...
\begin{abstract}
OEIS A237229 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=928$
...
OEIS A237229 is ``Number of (n+1)X(3+1) 0..2 arrays with the maximum plus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$ and begins
\[
928, 3284, 12014, 29456, 97862, 269844, 926086, 2450048,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-2) +42*a(n-4) -21*a(n-6) -207*a(n-8) for n>13
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 09:59:43, revision 6) this is still
...
differ by $1$. The entry's first term $928$ counts those, and the model was made to
...
\]
over the $S=928$ pairs whose own subblock row already satisfies the condition along itself.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237229.
