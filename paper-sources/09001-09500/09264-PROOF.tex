...
\title{The empirical recurrence for OEIS A237379, proved by transfer matrix}
...
\begin{abstract}
OEIS A237379 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=1472$
...
OEIS A237379 is ``Number of (n+1)X(3+1) 0..2 arrays with the maximum plus the minimum minus the upper median minus the lower median of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$ and begins
\[
1472, 12408, 104248, 912912, 7660840, 64312576, 541081120, 4737777088,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 10380*a(n-4) -26935616*a(n-8) for n>11
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:07:52, revision 6) this is still
...
differ by $1$. The entry's first term $1472$ counts those, and the model was made to
...
\]
over the $S=1472$ pairs whose own subblock row already satisfies the condition along itself.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237379.
