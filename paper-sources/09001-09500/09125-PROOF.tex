...
\title{The empirical recurrence for OEIS A252153, proved by transfer matrix}
...
\begin{abstract}
OEIS A252153 counts $(n+2)\times6$ arrays over $\{0,\dots,3\}$ subject to a condition imposed on
every $3\times3$ subblock, and it carries an empirical recurrence of order $8$
...
$a(n)$ is a walk count on $S=650996$ vertices. Such a sequence is $C$-finite, so whether a given
...
OEIS A252153 is ``Number of (n+2)X(4+2) 0..3 arrays with every 3X3 subblock row and column sum not equal to 0 1 3 6 or 7 and every 3X3 diagonal and antidiagonal sum equal to 0 1 3 6 or 7.''. It has offset $1$ and begins
\[
3583, 2702, 4988, 8425, 15460, 29825, 58792, 116276,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) -2*a(n-2) +6*a(n-3) -18*a(n-4) +12*a(n-5) -8*a(n-6) +24*a(n-7) -16*a(n-8) for n>13
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:52:25, revision 6) this is still
...
\[
\text{every row, column sum} \notin \{0,1,3,6,7\}\ \text{ and }\ \text{every diagonal, antidiagonal sum} \in \{0,1,3,6,7\}
\]
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 2\,a(n-2) + 6\,a(n-3) - 18\,a(n-4) + 12\,a(n-5) - 8\,a(n-6) + 24\,a(n-7) - 16\,a(n-8)
\]
for every $n>13$.
\end{theorem}

\begin{proof}
The vector $w=q(M)\mathbf{1}$ was formed by $8$ matrix--vector products in exact
integer arithmetic and $\mathbf{1}^{\!\top}M^{j}w$ evaluated for $j=0,1,\dots$, again
exactly. Those integers vanish from the index corresponding to $n=13+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>13$. The bound is exact: at $n=13$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252153.
