...
\title{The empirical recurrence for OEIS A305485, proved by transfer matrix}
...
\begin{abstract}
OEIS A305485 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{1,2,3,6,8\}$, and it carries an empirical
...
OEIS A305485 is ``Number of nX4 0..1 arrays with every element unequal to 1, 2, 3, 6 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
2, 33, 27, 65, 195, 125, 268, 1176,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2018 08:59:41, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 4\,a(n-3) - 26\,a(n-4) + 29\,a(n-6) + 55\,a(n-7) - 18\,a(n-8) - 111\,a(n-9) - 15\,a(n-10) + 60\,a(n-11) + 72\,a(n-12) - 14\,a(n-13) - 36\,a(n-14) - 13\,a(n-15) + 5\,a(n-16) + 6\,a(n-17)
\end{equation}
for all $n>22$.
...
has score in $\{1,2,3,6,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,2,3,6,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,2,3,6,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,2,3,6,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,2,3,6,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A305485.
