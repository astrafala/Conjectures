...
\title{The empirical recurrence for OEIS A235092, proved by transfer matrix}
...
\begin{abstract}
OEIS A235092 counts $3$-column arrays over $\{0,\dots,5\}$ in which every
...
carries an empirical recurrence of order $16$ contributed by R.~H.~Hardin. It is true,
...
the residual is $\mathbf{1}^{\!\top}M^{\,n-16}q(M)\mathbf{1}$, so the recurrence
...
OEIS A235092 is ``Number of (n+1) X (2+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
256, 656, 1728, 4820, 13624, 39884, 117488,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 21:20:20, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) + 20\,a(n-2) - 178\,a(n-3) - 45\,a(n-4) + 2028\,a(n-5) - 1568\,a(n-6) - 11316\,a(n-7) + 15017\,a(n-8) + 32278\,a(n-9) - 57324\,a(n-10) - 42818\,a(n-11) + 104201\,a(n-12) + 14456\,a(n-13) - 82056\,a(n-14) + 10920\,a(n-15) + 16380\,a(n-16)
\end{equation}
for all $n>16$, which is every $n$ at which a recurrence of order $16$ can be
stated.
...
For $n\ge16$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-16}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-16$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-16}\bigl(M^{16}-\sum_i c_i M^{16-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-16$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $16$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=16-16+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>16$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235092.
