...
\title{The empirical recurrence for OEIS A302517, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A302517 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A302517 is ``Number of 4Xn 0..1 arrays with every element equal to 0, 1, 4 or 5 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
8, 6, 9, 28, 36, 80, 235, 541,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +a(n-2) +3*a(n-3) +5*a(n-4) -a(n-5) -5*a(n-6) -4*a(n-7) for n>10
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 09 2018 09:11:26, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + a(n-2) + 3\,a(n-3) + 5\,a(n-4) - a(n-5) - 5\,a(n-6) - 4\,a(n-7)
\end{equation}
for all $n>10$.
...
publishes, $n=30$, the arrays have $4\cdot30$ cells over
...
The threshold below is exact rather than merely sufficient: at $n=10$ the conjectured
...
Every one of those integers vanishes from the index corresponding to $n=10+1$ onwards,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302517.
