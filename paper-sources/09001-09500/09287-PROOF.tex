...
\title{The empirical recurrence for OEIS A203184, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A203184 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
with $S=25$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A203184 is ``Number of n X 1 0..4 arrays with every nonzero element less than or equal to some horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
1, 5, 25, 95, 325, 1121, 3985, 14288,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) -10*a(n-2) +20*a(n-3) -15*a(n-4) +21*a(n-5) -7*a(n-6) +8*a(n-7) -a(n-8) +a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 01 2026 16:12:52, revision 16) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 10\,a(n-2) + 20\,a(n-3) - 15\,a(n-4) + 21\,a(n-5) - 7\,a(n-6) + 8\,a(n-7) - a(n-8) + a(n-9)
\end{equation}
for all $n>10$.
...
Let $V=\{0,\dots,4\}^{1}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=25$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
publishes, $n=27$, the arrays have $1\cdot27$ cells over
$5$ values, so there are about $10^{18}$ of them to sift. The
walk count replaces that by $25$ states and one matrix--vector product per row.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=25$ pairs and each
of the $(4+1)^{1}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203184.
