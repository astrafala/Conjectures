...
\title{The empirical recurrence for OEIS A316690, proved by transfer matrix}
...
\begin{abstract}
OEIS A316690 counts $5$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,6,7,8\}$, and it carries an empirical
recurrence of order $7$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A316690 is ``Number of nX5 0..1 arrays with every element unequal to 0, 1, 2, 6, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
16, 8, 21, 44, 88, 359, 1125, 3117,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = a(n-1) +3*a(n-2) +12*a(n-3) +2*a(n-4) -9*a(n-5) -27*a(n-6) -6*a(n-7) for n>11
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 10 2018 15:04:33, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 3\,a(n-2) + 12\,a(n-3) + 2\,a(n-4) - 9\,a(n-5) - 27\,a(n-6) - 6\,a(n-7)
\end{equation}
for all $n>11$.
...
has score in $\{0,1,2,6,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,6,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,6,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,6,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,6,7,8\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-7}$ whenever $n-7\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-7$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-7}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $7$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=11+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A316690.
