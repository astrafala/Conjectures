...
\title{A condition on each element and its neighbours: OEIS A241073}
...
\begin{abstract}
OEIS A241073 carries an empirical recurrence of order $6$ contributed by R.~H.~Hardin. It is
...
OEIS A241073 is ``Number of n X 2 0..2 arrays with no element equal to exactly one horizontal or vertical neighbor, with new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
1, 4, 12, 39, 131, 444, 1516, 5195,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) - 8*a(n-2) - 4*a(n-3) + a(n-4) + 14*a(n-5) - 8*a(n-6) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 29 2018 10:18:43, revision 10) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $2$ columns over the alphabet $\{0,1,2\}$, growing one row at a time. Its condition is that no element may equal exactly $1$ of its horizontal and vertical neighbours.
...
Vertices with identical futures are merged, which leaves $S=7$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A241073.
