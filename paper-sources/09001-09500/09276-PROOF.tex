...
\title{The empirical recurrence for OEIS A252221, proved by transfer matrix}
...
\begin{abstract}
OEIS A252221 counts arrays of width $3$ over $\{0,1,2\}$ in which every $3\times3$ window has
...
OEIS A252221 is ``Number of (n+2) X (1+2) 0..2 arrays with every 3 X 3 subblock row and column sum 2 3 or 4 and every diagonal and antidiagonal sum not 2 3 or 4.''. It has offset $1$ and begins
\[
228, 368, 812, 1442, 2458, 4738, 9922, 20070,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - a(n-2) + a(n-3) + 3*a(n-4) - 2*a(n-5) - 2*a(n-7) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 02 2018 07:15:15, revision 8) this is still
...
\text{each row} & \text{must be 2 or 3 or 4} \\
\text{each column} & \text{must be 2 or 3 or 4} \\
\text{the main diagonal} & \text{must NOT be 2 or 3 or 4} \\
\text{the antidiagonal} & \text{must NOT be 2 or 3 or 4} \\
\end{array}
\]
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) - a(n-2) + a(n-3) + 3\,a(n-4) - 2\,a(n-5) - 2\,a(n-7)
\]
for every $n>10$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252221.
