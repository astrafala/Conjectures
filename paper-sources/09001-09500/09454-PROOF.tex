...
\title{The empirical recurrence for OEIS A230393, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A230393 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A230393 is ``Number of n X 2 0..4 arrays x(i,j) with each element horizontally or vertically next to at least one element with value (x(i,j)+1) mod 5, and upper left element zero.''. It has offset $1$ and begins
\[
0, 0, 0, 0, 2, 8, 32, 120,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) - 4*a(n-3) - 9*a(n-4) - 6*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 19 2025 17:23:57, revision 12) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;4\,a(n-1) - 4\,a(n-3) - 9\,a(n-4) - 6\,a(n-5)
\end{equation}
for all $n>6$.
...
\exists\ \text{a neighbour with value }(x_{t,u}+1)\bmod 5
\end{equation}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230393.
