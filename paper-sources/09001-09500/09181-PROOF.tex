...
\title{The empirical recurrence for OEIS A202882, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A202882 counts arrays over $\{0,\dots,2\}$ in which every cell is constrained by the
...
with $S=9$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A202882 is ``Number of n X 1 0..2 arrays with every nonzero element less than or equal to some horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
1, 3, 9, 22, 51, 121, 292, 704,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) -3*a(n-2) +4*a(n-3) -a(n-4) +a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 18 2023 11:21:38, revision 16) the statement is
...
Let $V=\{0,\dots,2\}^{1}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=9$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
publishes, $n=29$, the arrays have $1\cdot29$ cells over
$3$ values, so there are about $10^{13}$ of them to sift. The
walk count replaces that by $9$ states and one matrix--vector product per row.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=9$ pairs and each
of the $(2+1)^{1}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202882.
