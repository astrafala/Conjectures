...
\title{The empirical recurrence for OEIS A297315, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A297315 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $9$ contributed
...
OEIS A297315 is ``Number of 2 X n 0..1 arrays with every 1 horizontally or antidiagonally adjacent to 1 or 2 neighboring 1s.''. It has offset $1$ and begins
\[
1, 7, 23, 66, 207, 654, 2049, 6422,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) - 2*a(n-2) + 8*a(n-3) - 3*a(n-4) + 7*a(n-5) - 5*a(n-6) - a(n-7) - a(n-8) + a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 26 2019 14:13:33, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) - 2\,a(n-2) + 8\,a(n-3) - 3\,a(n-4) + 7\,a(n-5) - 5\,a(n-6) - a(n-7) - a(n-8) + a(n-9)
\end{equation}
for all $n>10$.
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-9}$ out of
...
the middle column. The vector $w$ was formed by $9$ applications of $G$ in exact integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A297315.
