...
\title{The empirical recurrence for OEIS A223461, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A223461 counts arrays over $\{0,\dots,20\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $13$ contributed
...
with $S=441$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A223461 is ``6X6X6 triangular graph without horizontal edges coloring a rectangular array: number of nX2 0..20 arrays where 0..20 label nodes of a graph with edges 0,1 0,2 1,3 1,4 2,4 2,5 3,6 3,7 4,7 4,8 5,8 5,9 6,10 6,11 7,11 7,12 8,12 8,13 9,13 9,14 10,15 10,16 11,16 11,17 12,17 12,18 13,18 13,19 14,19 14,20 and every array movement to a horizontal or vertical neighbor moves along an edge of this graph.''. It has offset $1$ and begins
\[
60, 400, 2840, 20588, 150952, 1113460, 8242836, 61154044,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 21:42:48, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;20\,a(n-1) - 137\,a(n-2) + 299\,a(n-3) + 529\,a(n-4) - 2784\,a(n-5) + 973\,a(n-6) + 6475\,a(n-7) - 4402\,a(n-8) - 4888\,a(n-9) + 2878\,a(n-10) + 428\,a(n-11) - 408\,a(n-12) + 52\,a(n-13)
\end{equation}
for all $n>13$.
...
row $t$ and column $u$, each in $\{0,\dots,20\}$. With the neighbour offsets
\[
\mathcal N=\{(0,1),\ (1,0)\},
\]
...
\{x_{t,u},x_{t+d_1,u+d_2}\}\in E\ \text{ for every offset in }\mathcal N,\quad E=\{\{0,1\},\{0,2\},\{1,3\},\{1,4\},\{2,4\},\{2,5\},\{3,6\},\{3,7\},\{4,7\},\{4,8\},\{5,8\},\{5,9\},\{6,10\},\{6,11\},\{7,11\},\{7,12\},\{8,12\},\{8,13\},\{9,13\},\{9,14\},\{10,15\},\{10,16\},\{11,16\},\{11,17\},\{12,17\},\{12,18\},\{13,18\},\{13,19\},\{14,19\},\{14,20\}\}
\end{equation}
for every cell $(t,u)$.
...
Let $V=\{0,\dots,20\}^{2}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=441$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
and $2$ columns. In the walk, the cells of row $t$ for $2\le t\le L-1$ are tested
...
$21$ values, so there are about $10^{50}$ of them to sift. The
walk count replaces that by $441$ states and one matrix--vector product per row.
...
$o=1n_0+0-2$. Then for $j\ge13$,
\[
a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\;=\;\,\mathbf{v}^{\!\top}G^{\,j-13}w,
\qquad w=h_{13}-\sum_i c_i\,h_{13-i} .
\]
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-13}$ out of
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=441$ pairs and each
of the $(20+1)^{2}$ possible next rows, the condition was tested on every cell of
the middle row. The vector $w$ was formed by $13$ applications of $G$ in exact integer
arithmetic and $\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly.
Every one of those integers vanishes from the index corresponding to $n=13+1$ onwards,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223461.
