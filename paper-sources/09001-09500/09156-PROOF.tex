...
\title{The empirical recurrence for OEIS A298707, proved by transfer matrix}
...
\begin{abstract}
OEIS A298707 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{1,3,4,6,7\}$, and it carries an empirical
...
OEIS A298707 is ``Number of nX3 0..1 arrays with every element equal to 1, 3, 4, 6 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 3, 0, 2, 6, 20, 22, 92,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 25 2018 08:33:05, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;a(n-1) + 2\,a(n-2) + 12\,a(n-3) - 5\,a(n-4) - 23\,a(n-5) - 50\,a(n-6) - 2\,a(n-7) + 70\,a(n-8) + 84\,a(n-9) + 33\,a(n-10) - 86\,a(n-11) - 24\,a(n-12) + 2\,a(n-13) + 44\,a(n-14) - 14\,a(n-15) - 8\,a(n-17)
\end{equation}
for all $n>18$.
...
has score in $\{1,3,4,6,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,3,4,6,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,3,4,6,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,3,4,6,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,3,4,6,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298707.
