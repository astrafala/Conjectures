...
\title{The empirical recurrence for OEIS A234261, proved by transfer matrix}
...
\begin{abstract}
OEIS A234261 counts $4$-column arrays over $\{0,\dots,2\}$ in which every
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=81$ vertices, which
...
OEIS A234261 is ``Number of (n+1) X (3+1) 0..2 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
104, 170, 292, 560, 1100, 2324, 4924,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) + 6*a(n-2) - 24*a(n-3) + 4*a(n-4) + 36*a(n-5) - 24*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 21:33:46, revision 12) the statement is
...
Let $V$ consist of all of $\{0,\dots,2\}^{4}$, and put $S=|V|=81$. For $r,s\in V$ declare $r\to s$ an edge
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234261.
