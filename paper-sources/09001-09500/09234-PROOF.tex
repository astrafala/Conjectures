...
\title{The empirical recurrence for OEIS A301659, proved by transfer matrix}
...
\begin{abstract}
OEIS A301659 counts $5$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the four horizontal and vertical neighbours, a number $equal to$ itself lying in $\{0,1,4\}$, and it carries an empirical
recurrence of order $11$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A301659 is ``Number of nX5 0..1 arrays with every element equal to 0, 1 or 4 horizontally or vertically adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
8, 9, 15, 24, 46, 99, 223, 528,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) -a(n-2) +a(n-3) -4*a(n-4) -4*a(n-6) +3*a(n-7) +3*a(n-8) +a(n-9) +a(n-10) -2*a(n-11)
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 25 2018 11:26:45, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) - a(n-2) + a(n-3) - 4\,a(n-4) - 4\,a(n-6) + 3\,a(n-7) + 3\,a(n-8) + a(n-9) + a(n-10) - 2\,a(n-11)
\end{equation}
for all $n>11$.
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-11}$ whenever $n-11\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-11$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-11}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $11$ matrix--vector
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A301659.
