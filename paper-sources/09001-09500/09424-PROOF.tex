...
\title{The empirical recurrence for OEIS A298180, proved by transfer matrix}
...
\begin{abstract}
OEIS A298180 counts $5$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{3,5,6,8\}$, and it carries an empirical
...
OEIS A298180 is ``Number of nX5 0..1 arrays with every element equal to 3, 5, 6 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 3, 3, 6, 15, 27, 54, 117,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) +a(n-2) +a(n-3) -2*a(n-4) -5*a(n-5) -6*a(n-6) -4*a(n-7) +14*a(n-8) +6*a(n-9) -2*a(n-10)
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 14 2018 09:28:51, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + a(n-2) + a(n-3) - 2\,a(n-4) - 5\,a(n-5) - 6\,a(n-6) - 4\,a(n-7) + 14\,a(n-8) + 6\,a(n-9) - 2\,a(n-10)
\end{equation}
for all $n>10$.
...
has score in $\{3,5,6,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{3,5,6,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{3,5,6,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{3,5,6,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{3,5,6,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298180.
