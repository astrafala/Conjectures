...
\title{The empirical recurrence for OEIS A258964, proved by transfer matrix}
...
\begin{abstract}
OEIS A258964 counts $(n+2)\times8$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
...
OEIS A258964 is ``Number of (n+2) X (6+2) 0..1 arrays with no 3 X 3 subblock diagonal sum 0 and no antidiagonal sum 3 and no row sum 0 or 3 and no column sum 0 or 3.''. It has offset $1$ and begins
\[
1374, 951, 412, 503, 466, 694, 788, 952,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-2) + 4*a(n-4) - 4*a(n-6) for n>12.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 24 2018 08:30:16, revision 8) this is still
...
exactly. Those integers vanish from the index corresponding to $n=12+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>12$. The bound is exact: at $n=12$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
First, the transfer model reproduces all $35$ terms the entry publishes, exactly, in
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A258964.
