...
\title{Dimensions of weight-$n$ newforms for $\Gamma_{1}(32)$:\\
a proof of Luschny's conjecture on OEIS A063305}
...
\begin{abstract}
OEIS A063305 tabulates the dimension of the space of weight-$n$ cuspidal newforms for
$\Gamma_{1}(32)$. In March 2012 P.~Luschny conjectured the closed form $a(n)=18n-41$ for
$n>2$. We prove it. The route is the classical dimension formula for $S_{k}(\Gamma_{1}(M))$
together with the Atkin--Lehner--Li newform sieve; for $N=32$ the sieve leaves only a short
explicit combination, and the result is $\dim S_{k}(\Gamma_{1}(32))^{\mathrm{new}}=18k-23$
for $k\ge2$. There is no parity term here, and the reason is visible in the sieve: the
level $M=4$, which is the only level carrying a half-integer correction in
Proposition~\ref{prop:dim}, does not survive the sieve at $N=32$. We also record that the
...
OEIS A063305 (N.~J.~A. Sloane, 14 July 2001) has offset 2 and begins
\[
  -1,\;13,\;31,\;49,\;67,\;85,\;103,\;121,\;139,\;\dots
\]
The entry carries

\begin{quote}
\textbf{Conjecture:} $a(n)=18n-41$ for $n>2$. --- Peter Luschny, Mar 2012
\end{quote}
...
\section{The computation at level 32}

\begin{lemma}\label{lem:sieve}
Since $32=2^{5}$, the divisors $M\mid32$ with $\bar\mu(32/M)\ne0$ are exactly
$M=32,16,8$, with $\bar\mu(1)=1$, $\bar\mu(2)=-2$, $\bar\mu(4)=1$. Hence
\[
  \dim S_{k}(\Gamma_{1}(32))^{\mathrm{new}}=\dim S_{k}(\Gamma_{1}(32))
  -2\dim S_{k}(\Gamma_{1}(16))+\dim S_{k}(\Gamma_{1}(8)).
\]
\end{lemma}

The relevant invariants are $g_{1}(32)=17$, $c_{1}(32)=32$; $g_{1}(16)=2$, $c_{1}(16)=14$;
$g_{1}(8)=0$, $c_{1}(8)=6$. None of the three levels is 3 or 4, so no exceptional term
occurs and $\dim S_{k}(\Gamma_{1}(32))^{\mathrm{new}}$ is an affine function of $k$ for
$k\ge3$.

\begin{theorem}\label{thm:main}
For every $k\ge2$,
\[
  \dim S_{k}(\Gamma_{1}(32))^{\mathrm{new}}=18k-23.
\]
\end{theorem}
...
For every $n>2$, $a(n)=18n-41$, which is Luschny's conjecture. Indeed
$a(n)=\dim S_{n-1}(\Gamma_{1}(32))^{\mathrm{new}}=18(n-1)-23=18n-41$.
\end{corollary}
...
(iv) confirms the shift \eqref{eq:shift} against all 49 published terms
$n=3,\dots,51$, and independently against the two companion entries; and (v) confirms
Luschny's formula against all 49 published terms, and against
$\dim S_{n-1}(\Gamma_{1}(32))^{\mathrm{new}}$ for $n=3,\dots,399$, with no discrepancy.
...
Sequences}, \url{https://oeis.org/A063305}, consulted 24 August 2026.
\end{thebibliography}
\end{document}
