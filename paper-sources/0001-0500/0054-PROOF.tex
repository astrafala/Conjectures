...
\title{Eventual periodicity modulo $m$ with period dividing $\varphi(m)$:\\a proof of Bala's conjecture on OEIS A355409}
...
\section{Application to A355409}\label{sec:apply}

\begin{corollary}\label{cor:this}
The conjecture quoted in Section~1 is true: for every $m\ge1$ the sequence
A355409$(n) \bmod m$ is eventually periodic with period dividing $\varphi(m)$.
\end{corollary}

\begin{proof}
By Theorem~\ref{thm:main} it suffices to exhibit the entry's exponential generating
function in the form $G(e^{x}-1)$ with $G$ integral. The posted e.g.f.\ is $1/(1+e^{2x}-e^{3x})$. With $e^{x}=1+y$ the denominator is $1+(1+y)^{2}-(1+y)^{3}=1-y-2y^{2}-y^{3}$. Therefore
\[
\sum_{n\ge0}a(n)\frac{x^{n}}{n!}=G(y),\qquad G(t)=\frac{1}{1-y-2y^{2}-y^{3}} ,
\]
...
\[
c_{0},c_{1},c_{2},\dots \;=\; 1, 1, 3, 6, 13, 28, 60, \dots ,
\]
...
$19$ terms published on A355409 returns integers at every $k$, and the values
...
\url{https://oeis.org}, 2026. Sequence A355409; conjecture of P.\ Bala.
