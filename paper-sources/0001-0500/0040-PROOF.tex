...
\title{The reduction of OEIS A158690 modulo $k$:\\ a proof that the period divides
$\varphi(k)$}
...
\begin{abstract}
OEIS A158690 carries a conjecture of P.~Bala that for every positive integer $k$ the
...
OEIS A158690 is ``Expansion of the basic hypergeometric series 1 + (1 - exp(-t)) + (1 - exp(-t))*(1 - exp(-3*t)) + (1 - exp(-t))*(1 - exp(-3*t))*(1 - exp(-5*t)) + ... as a series in t.''. It has offset $0$ and begins
\[
1, 1, 5, 55, 1073, 32671, 1431665, 85363615,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
Expansion of the basic hypergeometric series 1 + (1 - exp(-t)) + (1 - exp(-t))*(1 - exp(-3*t)) + ...
\end{quote}
that is, writing $A(x)=\sum_{n\ge 0}a(n)x^{n}/n!$,
\begin{equation}\label{eq:egf}
A(x)\;=\;\sum_{n\ge 0}\ \prod_{k=1}^{n}\Bigl(1-e^{-(2k-1)x}\Bigr).
\end{equation}
This is the only input taken from the entry, and it was checked against every one of the
$15$ terms of the entry's DATA section before use, not against a sample.

The entry separately carries the following comment:
\begin{quote}\small
\textbf{Conjecture} 1) Taking the sequence modulo an integer k gives an eventually periodic sequence with period dividing phi(k). For example, the sequence taken modulo 16 begins [1, 1, 5, 7, 1, 15, 1, 15, 1, 15, 1, 15, ...] with an apparent pre-period of length 4 and a period of length 2. --- \emph{Peter Bala}, Dec 18 2021
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 05 2022 13:53:39, revision
46) the statement is still recorded as a conjecture, and nothing on the entry
...
G(t)\;=\;\sum_{n\ge 0}\ \prod_{k=1}^{n}\Bigl(1-(1+t)^{-(2k-1)}\Bigr).
\end{equation}
...
\begin{proof}
$G$ is each factor $1-(1+t)^{-(2k-1)}$ is an integer series with zero constant term, so the $n$-th product is divisible by $t^{n}$ and only finitely many terms of the sum affect any one coefficient.
\end{proof}

Write $G(t)=\sum_{j\ge0}c_j t^{j}$, so $c_j\in\mathbb{Z}$; here $c_j = 1, 1, 5, 23, 137, \dots$, and the
first few are
\[
c_0,\dots,c_8 \;=\; 1, 1, 2, 7, 33, 197, 1419, 11966, 115575.
\]
...
Let $a(n)$ be OEIS A158690. For every integer $k\ge1$,
\[
...
against the entry's DATA at every one of the $15$ published indices; the values
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A158690.
