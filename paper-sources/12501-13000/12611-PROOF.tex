...
\title{A proof of the conjectured recurrence for OEIS A003440}
...
\begin{abstract}
OEIS A003440 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{3} + 3 x^{2} - 3 x + 2$, a polynomial of degree $3$, which proves
...
OEIS A003440 is ``Number of binary vectors with restricted repetitions''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 7, 17, 42, 104, 259,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} + \sqrt{\frac{x^{2} + x + 1}{x^{2} - 3 x + 1}} \left(x - 1\right)^{2} - 1}{2 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +3*(-n-1)*a(n-1) +(n-2)*a(n-2) +(-n+1)*a(n-3) +3*(n-4)*a(n-4) +(-n+5)*a(n-5)=0. - \emph{R. J. Mathar}, Jun 07 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2023-07-06, revision 50) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 3 \left(n + 1\right),\qquad p_{2}(n)=n - 2,\qquad p_{3}(n)=1 - n,\qquad p_{4}(n)=3 \left(n - 4\right),\qquad p_{5}(n)=5 - n
\]
...
\[
(n + 2)\,a(n) -  3 \left(n + 1\right)\,a(n-1) + (n - 2)\,a(n-2) + (1 - n)\,a(n-3) + 3 \left(n - 4\right)\,a(n-4) + (5 - n)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A003440, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{3} + 3 x^{2} - 3 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A003440.
