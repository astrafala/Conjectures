...
\title{A proof of the conjectured recurrence for OEIS A210496}
...
\begin{abstract}
OEIS A210496 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x^{7} - 30 x^{6} + 16 x^{5} - 32 x^{4} + 8 x^{3} + 16 x^{2} - 11 x + 2$, a polynomial of degree $7$, which proves
...
OEIS A210496 is ``Number of set partitions of [n] avoiding the patterns \{1123, 1211\}''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 5, 13, 33, 81, 196,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 3 x^{5} + 15 x^{4} - 14 x^{3} + 2 x^{2} + 3 x + \sqrt{\left(x - 1\right) \left(4 x^{2} + x - 1\right)} \left(1 - x^{2}\right) \left(x - 1\right)^{2} - 1}{2 x^{2} \left(x - 1\right)^{2} \left(x^{2} - 3 x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +2*(-3*n-4)*a(n-1) +4*(2*n+3)*a(n-2) +(13*n-40)*a(n-3) +5*(-7*n+18)*a(n-4) +6*(2*n-1)*a(n-5) +22*(n-7)*a(n-6) +(-19*n+134)*a(n-7) +2*(2*n-15)*a(n-8)=0. - \emph{R. J. Mathar}, Oct 08 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 32) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 2 \left(3 n + 4\right),\qquad p_{2}(n)=4 \left(2 n + 3\right),\qquad p_{3}(n)=13 n - 40,\qquad p_{4}(n)=- 5 \left(7 n - 18\right),\qquad p_{5}(n)=6 \left(2 n - 1\right),\qquad p_{6}(n)=22 \left(n - 7\right),\qquad p_{7}(n)=134 - 19 n,\qquad p_{8}(n)=2 \left(2 n - 15\right)
\]
...
\section{The computation for A210496}
...
\[
B(x)\;=\;3 x^{7} - 30 x^{6} + 16 x^{5} - 32 x^{4} + 8 x^{3} + 16 x^{2} - 11 x + 2 ,
\]
...
\[
(n + 2)\,a(n) -  2 \left(3 n + 4\right)\,a(n-1) + 4 \left(2 n + 3\right)\,a(n-2) + (13 n - 40)\,a(n-3) -  5 \left(7 n - 18\right)\,a(n-4) + 6 \left(2 n - 1\right)\,a(n-5) + 22 \left(n - 7\right)\,a(n-6) + (134 - 19 n)\,a(n-7) + 2 \left(2 n - 15\right)\,a(n-8) \;=\;0
\]
holds for every $n>7$.
\end{theorem}
...
section of A210496, agreeing on all 13 terms compared.
...
vanish, for all 30 values of $n$ from $n=8$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=3 x^{7} - 30 x^{6} + 16 x^{5} - 32 x^{4} + 8 x^{3} + 16 x^{2} - 11 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A210496.
