...
\title{A proof of the conjectured recurrence for OEIS A059279}
...
\begin{abstract}
OEIS A059279 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{2} - 2 x + 1$, a polynomial of degree $2$, which proves
...
OEIS A059279 is ``G.f. is ((1-x)/(1-2*x)) * G(x*(1-x)/(1-2*x)) where G(x) is g.f. for Catalan numbers A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 6, 20, 72, 276, 1112, 4656,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 4 x^{2} + 6 x - 1}{2 x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(1-4*n)*a(n-1) + 4*(4*n-5)*a(n-2) +4*(5-2*n)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2024-03-03, revision 24) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(4 n - 1\right),\qquad p_{2}(n)=4 \left(4 n - 5\right),\qquad p_{3}(n)=- 4 \left(2 n - 5\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(4 n - 1\right)\,a(n-1) + 4 \left(4 n - 5\right)\,a(n-2) -  4 \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A059279, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{2} - 2 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A059279.
