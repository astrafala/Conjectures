...
\title{A proof of the conjectured recurrence for OEIS A217333}
...
\begin{abstract}
OEIS A217333 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 x^{2} - 6 x + 3$, a polynomial of degree $2$, which proves
...
OEIS A217333 is ``G.f.: exp( Sum\_\{n>=1\} x\^{}n/n * Sum\_\{k=0..n\} binomial(n,k)\^{}2 * x\^{}k/(1-x)\^{}k )''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 5, 12, 29, 72, 182,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x - \sqrt{4 x^{4} - 4 x^{3} + 4 x^{2} - 4 x + 1} + 1}{2 x^{3}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+3)*a(n) +2*(-2*n-3)*a(n-1) +4*n*a(n-2) +2*(-2*n+3)*a(n-3) +4*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, May 17 2019
\end{quote}
As of the ``Last modified'' line on the live entry (2020-06-30, revision 22) the
...
\[
p_{0}(n)=n + 3,\qquad p_{1}(n)=- 2 \left(2 n + 3\right),\qquad p_{2}(n)=4 n,\qquad p_{3}(n)=- 2 \left(2 n - 3\right),\qquad p_{4}(n)=4 \left(n - 3\right)
\]
...
\[
(n + 3)\,a(n) -  2 \left(2 n + 3\right)\,a(n-1) + 4 n\,a(n-2) -  2 \left(2 n - 3\right)\,a(n-3) + 4 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A217333, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=4 x^{2} - 6 x + 3$ is an
...
\url{https://oeis.org}, 2026. Sequence A217333.
