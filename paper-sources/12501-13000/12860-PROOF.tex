...
\title{A proof of the conjectured identity for OEIS A110320}
...
\begin{abstract}
OEIS A110320 carries a conjectured identity expressing its terms through the terms of
A051286, A051292. We prove it. Each entry involved posts a generating function, so the claim is
a single identity between generating functions: writing $F_{a}$ for the generating
function of A110320 and $F_{j}$ for those of the entries on the right, the conjecture
...
closed form and equals $0$, of degree $-1$, which proves the identity for
every $n>-1$.
\end{abstract}
...
OEIS A110320 is ``Number of blocks in all RNA secondary structures with n nodes (an RNA secondary structure can be viewed as a restricted noncrossing partition)''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 5, 13, 32, 80, 201, 505,\ \dots
\]
Its generating function, as posted on the entry, is
\begin{equation}\label{eq:gf}
F_{a}(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{- x^{2} - x - \sqrt{x^{4} - 2 x^{3} - x^{2} - 2 x + 1} + 1}{2 x^{2} \sqrt{x^{4} - 2 x^{3} - x^{2} - 2 x + 1}} ,
\end{equation}
and the entry carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = (A051292(n+2)-A051286(n+1))/2. - \emph{Gerald McGarvey}, Jan 14 2007
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 51) the
...
\[
F_{A051292}(x) \;=\; \frac{- x^{2} + \sqrt{x^{4} - 2 x^{3} - x^{2} - 2 x + 1} + 1}{\sqrt{x^{4} - 2 x^{3} - x^{2} - 2 x + 1}} \qquad F_{A051286}(x) \;=\; \frac{1}{\sqrt{x^{4} - 2 x^{3} - x^{2} - 2 x + 1}}
\]
...
\section{The computation for A110320}

Assembling the right-hand side by Lemmas~\ref{lem:shift} and~\ref{lem:theta} gives
\[
R(x)\;=\;\frac{- x^{2} - x - \sqrt{x^{4} - 2 x^{3} - x^{2} - 2 x + 1} + 1}{2 x^{2} \sqrt{x^{4} - 2 x^{3} - x^{2} - 2 x + 1}} ,
\]
...
both sides were formed from the entries' own values and found to agree, for all 28
values of $n$ from $n=1$ upward. This is what Corollary~\ref{cor:criterion}
...
\url{https://oeis.org}, 2026. Sequence A110320.
