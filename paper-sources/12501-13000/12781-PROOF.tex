...
\title{A proof of the conjectured recurrence for OEIS A262768}
...
\begin{abstract}
OEIS A262768 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 96 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A262768 is ``a(n) = binomial(2*n+2,n) - 2*binomial(2*n,n) + binomial(2*n-2,n)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 4, 20, 85, 344, 1365, 5368,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x^{3} + x^{2} \sqrt{1 - 4 x} - 3 x^{2} - 2 x - \sqrt{1 - 4 x} + 1}{2 x^{2} \sqrt{1 - 4 x}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -2*(n+2)*(49*n-95)*a(n) + 325*(n-1)*(n+1)*a(n-1) + 2*(67*n-38)*(2*n-5)*a(n-2) = 0. - \emph{R. J. Mathar}, Jun 07 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-18, revision 22) the
...
\[
p_{0}(n)=- 2 \left(n + 2\right) \left(49 n - 95\right),\qquad p_{1}(n)=325 \left(n - 1\right) \left(n + 1\right),\qquad p_{2}(n)=2 \left(2 n - 5\right) \left(67 n - 38\right)
\]
...
\section{The computation for A262768}
...
\[
- 2 \left(n + 2\right) \left(49 n - 95\right)\,a(n) + 325 \left(n - 1\right) \left(n + 1\right)\,a(n-1) + 2 \left(2 n - 5\right) \left(67 n - 38\right)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A262768, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 96 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A262768.
