...
\title{A proof of the conjectured recurrence for OEIS A257178}
...
\begin{abstract}
OEIS A257178 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 - 3 x$, a polynomial of degree $1$, which proves
...
OEIS A257178 is ``Number of 3-Motzkin paths of length n with no level steps at odd level''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 10, 33, 110, 369, 1247, 4245,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{3 x + \sqrt{\left(3 x - 1\right) \left(4 x^{2} + 3 x - 1\right)} - 1}{2 x^{2} \left(3 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +6*(-n-1)*a(n-1) +(5*n+4)*a(n-2) +6*(2*n-3)*a(n-3)=0. - \emph{R. J. Mathar}, Sep 24 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2020-06-30, revision 23) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 6 \left(n + 1\right),\qquad p_{2}(n)=5 n + 4,\qquad p_{3}(n)=6 \left(2 n - 3\right)
\]
...
\section{The computation for A257178}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(x + 1\right) \left(3 x - 1\right) \left(4 x - 1\right) .
\]
...
\[
(n + 2)\,a(n) -  6 \left(n + 1\right)\,a(n-1) + (5 n + 4)\,a(n-2) + 6 \left(2 n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A257178, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 - 3 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A257178.
