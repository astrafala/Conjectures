...
\title{A proof of the conjectured recurrence for OEIS A191993}
...
\begin{abstract}
OEIS A191993 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A191993 is ``a(n) = 3\^{}(n-1) + C(2*n, n)/2''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;2, 6, 19, 62, 207, 705, 2445, 8622,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{- 20 x^{2} + 9 x + \sqrt{- 36 x^{3} + 33 x^{2} - 10 x + 1} - 1}{2 \left(12 x^{2} - 7 x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-3)*a(n) - (7*n\^{}2 -23*n +12)*a(n-1) +6*(2*n-3)*(n-2)*a(n-2)=0. - \emph{R. J. Mathar}, Oct 18 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2025-08-22, revision 28) the
...
\[
p_{0}(n)=n \left(n - 3\right),\qquad p_{1}(n)=- 7 n^{2} + 23 n - 12,\qquad p_{2}(n)=6 \left(n - 2\right) \left(2 n - 3\right)
\]
...
\section{The computation for A191993}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \left(3 x - 1\right)^{2} \left(4 x - 1\right) .
\]
...
\[
n \left(n - 3\right)\,a(n) + (- 7 n^{2} + 23 n - 12)\,a(n-1) + 6 \left(n - 2\right) \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A191993, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A191993.
