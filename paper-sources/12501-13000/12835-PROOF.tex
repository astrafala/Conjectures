...
\title{A proof of the conjectured recurrence for OEIS A141222}
...
\begin{abstract}
OEIS A141222 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A141222 is ``Expansion of -1/(2*x) + (2*x-1)\^{}2/(2*x*(1-4x)\^{}(3/2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 5, 22, 95, 406, 1722, 7260, 30459,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{1}{2 x} + \frac{\left(2 x - 1\right)^{2}}{2 x \left(1 - 4 x\right)^{\frac{3}{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 2*(-3*n-1)*a(n-1) + 4*(2*n-3)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-02-09, revision 33) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n + 1\right),\qquad p_{2}(n)=4 \left(2 n - 3\right)
\]
...
\section{The computation for A141222}
...
\[
(n + 1)\,a(n) -  2 \left(3 n + 1\right)\,a(n-1) + 4 \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A141222, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A141222.
