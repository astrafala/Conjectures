...
\title{A proof of the conjectured recurrence for OEIS A001724}
...
\begin{abstract}
OEIS A001724 carries a conjectured linear recurrence with polynomial coefficients, of
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{- 420 \log{\left(1 - x \right)}^{4} + 1066 \log{\left(1 - x \right)}^{3} - 753 \log{\left(1 - x \right)}^{2} + 156 \log{\left(1 - x \right)} - 6}{6 \left(x - 1\right)^{9}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence a(n) +5*(-n-6)*a(n-1) +5*(2*n\textasciicircum{}2+22*n+61)*a(n-2) - 5*(n+5)*(2*n\textasciicircum{}2+20*n+51)*a(n-3) +(5*n\textasciicircum{}4+90*n\textasciicircum{}3+610*n\textasciicircum{}2+1845*n+2101)*a(n-4) -(n+4)\textasciicircum{}5*a(n- 5)=0.
\end{quote}
As of the ``Last modified'' line on the live entry (2026-07-20, revision 43) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- 5 \left(n + 6\right),\qquad p_{2}(n)=5 \left(2 n^{2} + 22 n + 61\right),\qquad p_{3}(n)=- 5 \left(n + 5\right) \left(2 n^{2} + 20 n + 51\right),\qquad p_{4}(n)=5 n^{4} + 90 n^{3} + 610 n^{2} + 1845 n + 2101,\qquad p_{5}(n)=- \left(n + 4\right)^{5}
\]
...
\section{The computation for A001724}
...
\[
1\,a(n) -  5 \left(n + 6\right)\,a(n-1) + 5 \left(2 n^{2} + 22 n + 61\right)\,a(n-2) -  5 \left(n + 5\right) \left(2 n^{2} + 20 n + 51\right)\,a(n-3) + (5 n^{4} + 90 n^{3} + 610 n^{2} + 1845 n + 2101)\,a(n-4) -  \left(n + 4\right)^{5}\,a(n-5) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A001724, agreeing on all 13 terms compared.
...
vanish, for all 12 values of $n$ from $n=6$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A001724.
