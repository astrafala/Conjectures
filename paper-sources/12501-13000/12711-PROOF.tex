...
\title{A proof of the conjectured recurrence for OEIS A111779}
...
\begin{abstract}
OEIS A111779 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A111779 is ``Sixth column (m=5) of unsigned triangle A111595''. It has offset $5$ and begins
\[
a(5),\dots,a(12)\;=\;1, 30, 651, 12600, 235494, 4396140, 83471850, 1627358040,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 5} a(n)\,\frac{x^{n}}{n!}\;=\;- \frac{x^{5}}{120 \sqrt{1 - x^{2}} \left(x - 1\right)^{5}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(-n+5)*a(n) +5*n*a(n-1) +n*(n-1)\^{}2*a(n-2)=0. - \emph{R. J. Mathar}, Jun 08 2016
\end{quote}
...
\[
p_{0}(n)=5 - n,\qquad p_{1}(n)=5 n,\qquad p_{2}(n)=n \left(n - 1\right)^{2}
\]
...
\section{The computation for A111779}
...
\[
(5 - n)\,a(n) + 5 n\,a(n-1) + n \left(n - 1\right)^{2}\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A111779, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A111779.
