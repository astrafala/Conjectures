...
\title{A proof of the conjectured recurrence for OEIS A120010}
...
\begin{abstract}
OEIS A120010 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{2} + x$, a polynomial of degree $2$, which proves
...
OEIS A120010 is ``G.f.: A(x) = (1-sqrt(1-4*x))/2 o x/(1-x) o (x-x\^{}2), a composition of functions involving the Catalan function and its inverse''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 1, 2, 6, 18, 53, 158,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{1}{2} - \frac{\sqrt{\frac{5 x^{2} - 5 x + 1}{x^{2} - x + 1}}}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +2*(4-3*n)*a(n-1) +(11*n-26)*a(n-2) +10*(3-n)*a(n-3) +5*(n-4)*a(n-4)= 0. - \emph{R. J. Mathar}, Nov 14 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2021-12-08, revision 23) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(3 n - 4\right),\qquad p_{2}(n)=11 n - 26,\qquad p_{3}(n)=- 10 \left(n - 3\right),\qquad p_{4}(n)=5 \left(n - 4\right)
\]
...
\[
n\,a(n) -  2 \left(3 n - 4\right)\,a(n-1) + (11 n - 26)\,a(n-2) -  10 \left(n - 3\right)\,a(n-3) + 5 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A120010, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{2} + x$ is an
...
\url{https://oeis.org}, 2026. Sequence A120010.
