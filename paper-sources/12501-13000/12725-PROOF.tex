...
\title{A proof of the conjectured recurrence for OEIS A194724}
...
\begin{abstract}
OEIS A194724 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $72 x^{2} - 9 x$, a polynomial of degree $2$, which proves
...
OEIS A194724 is ``Number of quaternary words either empty or beginning with the first character of the alphabet, that can be built by inserting n doublets into the initially empty word''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 7, 58, 523, 4966, 48838, 492724,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{3 \left(\sqrt{1 - 12 x} + 1\right)}{2 \left(2 \sqrt{1 - 12 x} + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +2*(-14*n+9)*a(n-1) +96*(2*n-3)*a(n-2)=0. - \emph{R. J. Mathar}, Mar 14 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2026-03-13, revision 25) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(14 n - 9\right),\qquad p_{2}(n)=96 \left(2 n - 3\right)
\]
...
\section{The computation for A194724}
...
\[
n\,a(n) -  2 \left(14 n - 9\right)\,a(n-1) + 96 \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A194724, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=72 x^{2} - 9 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A194724.
