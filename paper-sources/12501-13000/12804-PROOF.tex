...
\title{A proof of the conjectured recurrence for OEIS A257515}
...
\begin{abstract}
OEIS A257515 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $10 x^{5} + 2 x^{4} + 2 x^{3} + 10 x^{2} + 2 x + 2$, a polynomial of degree $5$, which proves
...
OEIS A257515 is ``Number of 3-generalized 2-Motzkin paths of length n with no level steps H=(3,0) at odd level''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 1, 2, 2, 4, 9, 12,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x^{3} + \sqrt{\left(2 x^{3} - 1\right) \left(2 x^{3} + 4 x^{2} - 1\right)} - 1}{2 x^{2} \left(2 x^{3} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +(n+1)*a(n-1) +(n+4)*a(n-2) +4*(-2*n+3)*a(n-3) +4*(-6*n+17)*a(n-4) +4*(-3*n+10)*a(n-5) +4*(3*n-11)*a(n-6) +4*(11*n-50)*a(n-7) +20*(n-6)*a(n-8)=0. - \emph{R. J. Mathar}, Jun 07 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2020-05-01, revision 19) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=n + 1,\qquad p_{2}(n)=n + 4,\qquad p_{3}(n)=- 4 \left(2 n - 3\right),\qquad p_{4}(n)=- 4 \left(6 n - 17\right),\qquad p_{5}(n)=- 4 \left(3 n - 10\right),\qquad p_{6}(n)=4 \left(3 n - 11\right),\qquad p_{7}(n)=4 \left(11 n - 50\right),\qquad p_{8}(n)=20 \left(n - 6\right)
\]
...
\section{The computation for A257515}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(2 x^{3} - 1\right) \left(2 x^{3} + 4 x^{2} - 1\right) .
\]
...
\[
B(x)\;=\;10 x^{5} + 2 x^{4} + 2 x^{3} + 10 x^{2} + 2 x + 2 ,
\]
...
\[
(n + 2)\,a(n) + (n + 1)\,a(n-1) + (n + 4)\,a(n-2) -  4 \left(2 n - 3\right)\,a(n-3) -  4 \left(6 n - 17\right)\,a(n-4) -  4 \left(3 n - 10\right)\,a(n-5) + 4 \left(3 n - 11\right)\,a(n-6) + 4 \left(11 n - 50\right)\,a(n-7) + 20 \left(n - 6\right)\,a(n-8) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A257515, agreeing on all 13 terms compared.
...
vanish, for all 28 values of $n$ from $n=8$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=10 x^{5} + 2 x^{4} + 2 x^{3} + 10 x^{2} + 2 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A257515.
