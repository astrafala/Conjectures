...
\title{A proof of the conjectured recurrence for OEIS A157418}
...
\begin{abstract}
OEIS A157418 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 64 x^{2} - 22 x$, a polynomial of degree $2$, which proves
...
OEIS A157418 is ``a(n) is the number of ways to insert single pairs of parenthesis to completely separate n identical objects in a straight line such that at least one of the objects at the two ends is not enclosed''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 5, 16, 55, 202, 773, 3052,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{- 2 x^{2} - x - \left(x + 2\right) \sqrt{- 4 x^{2} - 4 x + 1} + 2}{2 \left(x + 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(7*n-29)*a(n) +(-21*n\^{}2+136*n-192)*a(n-1) +2*(-28*n\^{}2+165*n-210)*a(n-2) -4*(n-3)*(7*n-22)*a(n-3)=0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-06-11, revision 24) the
...
\[
p_{0}(n)=n \left(7 n - 29\right),\qquad p_{1}(n)=- 21 n^{2} + 136 n - 192,\qquad p_{2}(n)=- 2 \left(28 n^{2} - 165 n + 210\right),\qquad p_{3}(n)=- 4 \left(n - 3\right) \left(7 n - 22\right)
\]
...
\section{The computation for A157418}
...
\[
n \left(7 n - 29\right)\,a(n) + (- 21 n^{2} + 136 n - 192)\,a(n-1) -  2 \left(28 n^{2} - 165 n + 210\right)\,a(n-2) -  4 \left(n - 3\right) \left(7 n - 22\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A157418, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 64 x^{2} - 22 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A157418.
