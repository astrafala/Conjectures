...
\title{A proof of the conjectured recurrence for OEIS A182892}
...
\begin{abstract}
OEIS A182892 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $5 x^{6} + 6 x^{5} + 16 x^{4} - 7 x^{3} - 2 x^{2}$, a polynomial of degree $6$, which proves
...
OEIS A182892 is ``Number of weighted lattice paths in L\_n having no (1,0)-steps of weight 2 at level 0''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 1, 3, 7, 15, 35, 83,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1}{x^{2} + \sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(n-2)*a(n-1) +2*(-9*n+16)*a(n-2) +5*(2*n-5)*a(n-3) +(10*n-33) *a(n-4) +2*(26*n-109)*a(n-5) +(13*n-37)*a(n-6) +(13*n-63) *a(n-7) +10*(-n+7) *a(n-8)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 21) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=n - 2,\qquad p_{2}(n)=- 2 \left(9 n - 16\right),\qquad p_{3}(n)=5 \left(2 n - 5\right),\qquad p_{4}(n)=10 n - 33,\qquad p_{5}(n)=2 \left(26 n - 109\right),\qquad p_{6}(n)=13 n - 37,\qquad p_{7}(n)=13 n - 63,\qquad p_{8}(n)=- 10 \left(n - 7\right)
\]
...
\section{The computation for A182892}
...
\[
B(x)\;=\;5 x^{6} + 6 x^{5} + 16 x^{4} - 7 x^{3} - 2 x^{2} ,
\]
...
\[
n\,a(n) + (n - 2)\,a(n-1) -  2 \left(9 n - 16\right)\,a(n-2) + 5 \left(2 n - 5\right)\,a(n-3) + (10 n - 33)\,a(n-4) + 2 \left(26 n - 109\right)\,a(n-5) + (13 n - 37)\,a(n-6) + (13 n - 63)\,a(n-7) -  10 \left(n - 7\right)\,a(n-8) \;=\;0
\]
holds for every $n>6$.
\end{theorem}
...
section of A182892, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=5 x^{6} + 6 x^{5} + 16 x^{4} - 7 x^{3} - 2 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A182892.
