...
\title{A proof of the conjectured recurrence for OEIS A186940}
...
\begin{abstract}
OEIS A186940 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{3} + 9 x^{2} - 7 x + 3$, a polynomial of degree $3$, which proves
...
OEIS A186940 is ``Diagonal sums of number triangle A114709''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 3, 6, 31, 126, 589, 2772,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x^{2} + 3 x - \sqrt{x^{2} - 6 x + 1} + 1}{2 x \left(x^{3} - 3 x^{2} + x + 3\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 3*(n+1)*a(n) +(10-17*n)*a(n-1) -6*(n+1)*a(n-2) +10*(2*n-1)*a(n-3) +9*(1-n)*a(n-4) +(n-2)*a(n-5) =0. - \emph{R. J. Mathar}, Nov 17 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2022-09-03, revision 19) the
...
\[
p_{0}(n)=3 \left(n + 1\right),\qquad p_{1}(n)=10 - 17 n,\qquad p_{2}(n)=- 6 \left(n + 1\right),\qquad p_{3}(n)=10 \left(2 n - 1\right),\qquad p_{4}(n)=- 9 \left(n - 1\right),\qquad p_{5}(n)=n - 2
\]
...
\section{The computation for A186940}
...
\[
3 \left(n + 1\right)\,a(n) + (10 - 17 n)\,a(n-1) -  6 \left(n + 1\right)\,a(n-2) + 10 \left(2 n - 1\right)\,a(n-3) -  9 \left(n - 1\right)\,a(n-4) + (n - 2)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A186940, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{3} + 9 x^{2} - 7 x + 3$ is an
...
\url{https://oeis.org}, 2026. Sequence A186940.
