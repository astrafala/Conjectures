...
\title{A proof of the conjectured recurrence for OEIS A177169}
...
\begin{abstract}
OEIS A177169 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $16 x^{3} - 29 x^{2} + 10 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A177169 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=7, k=0 and l=-2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 7, 12, 71, 308, 1752, 9518, 55995,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 32 x^{3} + 20 x^{2} + 5 x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-3*n+1)*a(n-1) +5*(-3*n+7)*a(n-2) +2*(26*n-81)*a(n-3) +32*(-n+4)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=- 5 \left(3 n - 7\right),\qquad p_{3}(n)=2 \left(26 n - 81\right),\qquad p_{4}(n)=- 32 \left(n - 4\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) -  5 \left(3 n - 7\right)\,a(n-2) + 2 \left(26 n - 81\right)\,a(n-3) -  32 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A177169, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=16 x^{3} - 29 x^{2} + 10 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A177169.
