...
\title{A proof of the conjectured recurrence for OEIS A176126}
...
\begin{abstract}
OEIS A176126 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{8} + 2 x^{7} - 2 x^{6} + 3 x^{5} + 2 x^{4} + 3 x^{3} - 2 x^{2} + 2 x - 1$, a polynomial of degree $8$, which proves
...
OEIS A176126 is ``Numerator of -A127276(n)/A001788(n)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;-1, -1, 1, 2, 4, 13, 19, 13,\ \dots
\]
The entry posts no generating function of its own. Its NAME states the
relation \texttt{the entry states, not as a conjecture, that a(n) = 3*a(n-4) - 3*a(n-8) + a(n-12); that recurrence and the entry's own initial terms determine the generating function below, after the factor (1+x)\textasciicircum{}3 common to numerator and denominator is cancelled} to another entry, and that entry's exponential generating
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{\left(- x^{4} + x^{3} + 2 x^{2} + x - 1\right) \left(x^{4} - x^{3} + 3 x^{2} - x + 1\right)}{\left(x - 1\right)^{3} \left(x^{2} + 1\right)^{3}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = +3*a(n-1) -6*a(n-2) +10*a(n-3) -12*a(n-4) +12*a(n-5) -10*a(n-6) +6*a(n-7) -3*a(n-8) +a(n-9) with g.f. (x\textasciicircum{}4-x\textasciicircum{}3+3*x\textasciicircum{}2-x+1)*(x\textasciicircum{}4-x\textasciicircum{}3-2*x\textasciicircum{}2-x+1) / ( (x-1)\textasciicircum{}3*(x\textasciicircum{}2+1)\textasciicircum{}3 ). - \emph{R. J. Mathar}, Dec 12 2010
\end{quote}
As of the ``Last modified'' line on the live entry (2022-08-27, revision 18) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=-3,\qquad p_{2}(n)=6,\qquad p_{3}(n)=-10,\qquad p_{4}(n)=12,\qquad p_{5}(n)=-12,\qquad p_{6}(n)=10,\qquad p_{7}(n)=-6,\qquad p_{8}(n)=3,\qquad p_{9}(n)=-1
\]
...
\section{The computation for A176126}
...
\[
B(x)\;=\;- x^{8} + 2 x^{7} - 2 x^{6} + 3 x^{5} + 2 x^{4} + 3 x^{3} - 2 x^{2} + 2 x - 1 ,
\]
...
\[
1\,a(n) - 3\,a(n-1) + 6\,a(n-2) - 10\,a(n-3) + 12\,a(n-4) - 12\,a(n-5) + 10\,a(n-6) - 6\,a(n-7) + 3\,a(n-8) - 1\,a(n-9) \;=\;0
\]
holds for every $n>8$.
\end{theorem}
...
section of A176126, agreeing on all 13 terms compared.
...
vanish, for all 71 values of $n$ from $n=9$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{8} + 2 x^{7} - 2 x^{6} + 3 x^{5} + 2 x^{4} + 3 x^{3} - 2 x^{2} + 2 x - 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176126.
