...
\title{A proof of the conjectured recurrence for OEIS A105849}
...
\begin{abstract}
OEIS A105849 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{2} - 5 x + 2$, a polynomial of degree $2$, which proves
...
OEIS A105849 is ``Row sums of number triangle A105848''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 9, 28, 91, 308, 1079, 3888,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x - \sqrt{\frac{4 x^{3} - 8 x^{2} + 6 x - 1}{2 x - 1}} + 1}{2 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +2*(-4*n-3)*a(n-1) +4*(5*n-2)*a(n-2) +2*(-10*n+17)*a(n-3) +8*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 16 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2012-11-16, revision 4) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 2 \left(4 n + 3\right),\qquad p_{2}(n)=4 \left(5 n - 2\right),\qquad p_{3}(n)=- 2 \left(10 n - 17\right),\qquad p_{4}(n)=8 \left(n - 3\right)
\]
...
\[
(n + 2)\,a(n) -  2 \left(4 n + 3\right)\,a(n-1) + 4 \left(5 n - 2\right)\,a(n-2) -  2 \left(10 n - 17\right)\,a(n-3) + 8 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A105849, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{2} - 5 x + 2$ is an
...
\url{https://oeis.org}, 2026. Sequence A105849.
