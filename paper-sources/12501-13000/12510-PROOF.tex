...
\title{The empirical recurrence for OEIS A235904, proved by transfer matrix}
...
\begin{abstract}
OEIS A235904 counts arrays of width $2$ over $\{0,1,2\}$ in which the smallest and largest of the
...
wider, so the count is a walk count on $S=9$ states.
\end{abstract}
...
OEIS A235904 is ``Number of (n+1) X (1+1) 0..2 arrays with the minimum plus the maximum unequal to the lower median plus the upper median in every 2 X 2 subblock.''. It has offset $1$ and begins
\[
48, 260, 1408, 7632, 41344, 224064, 1213952, 6578432,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + 20*a(n-2) - 8*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 20 2018 06:04:59, revision 10) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The arrays have $2$ columns and a growing number of rows, with entries in $\{0,1,2\}$. Take any
...
$\mathbf{1}$, and the states are all $3^{2}=9$ rows. Thus
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235904.
