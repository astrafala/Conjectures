...
\title{A proof of the conjectured recurrence for OEIS A177203}
...
\begin{abstract}
OEIS A177203 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 18 x^{4} + 54 x^{3} - 54 x^{2} + 15 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A177203 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=10, k=-1 and l=1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 10, 19, 136, 649, 4375, 26893, 184438,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 36 x^{4} + 72 x^{3} - 27 x^{2} - 6 x + 1}{x^{2} - 2 x + 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(2-7n)*a(n-1) +3(17-7n)*a(n-2) +9(11n-34)*a(n-3) +108(4-n)*a(n-4) +36(n-5)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 27 2011
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=2 - 7 n,\qquad p_{2}(n)=- 3 \left(7 n - 17\right),\qquad p_{3}(n)=9 \left(11 n - 34\right),\qquad p_{4}(n)=- 108 \left(n - 4\right),\qquad p_{5}(n)=36 \left(n - 5\right)
\]
...
\section{The computation for A177203}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \frac{\left(6 x^{2} - 9 x + 1\right) \left(6 x^{2} - 3 x - 1\right)}{\left(x - 1\right)^{2}} .
\]
...
\[
B(x)\;=\;- 18 x^{4} + 54 x^{3} - 54 x^{2} + 15 x + 1 ,
\]
...
\[
(n + 1)\,a(n) + (2 - 7 n)\,a(n-1) -  3 \left(7 n - 17\right)\,a(n-2) + 9 \left(11 n - 34\right)\,a(n-3) -  108 \left(n - 4\right)\,a(n-4) + 36 \left(n - 5\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A177203, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 18 x^{4} + 54 x^{3} - 54 x^{2} + 15 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A177203.
