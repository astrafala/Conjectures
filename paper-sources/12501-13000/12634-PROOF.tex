...
\title{A proof of the conjectured recurrence for OEIS A128750}
...
\begin{abstract}
OEIS A128750 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A128750 is ``Number of skew Dyck paths of semilength n having no ascents of length 1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 2, 4, 14, 44, 150, 520,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x^{2} - 1\right) \left(\sqrt{\frac{x^{2} + 4 x - 1}{x^{2} - 1}} - 1\right)}{2 x \left(x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(-4*n+3)*a(n-1) +(-2*n-1)*a(n-2) +(4*n-11)*a(n-3) +(n-4)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 15 2012
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=3 - 4 n,\qquad p_{2}(n)=- 2 n - 1,\qquad p_{3}(n)=4 n - 11,\qquad p_{4}(n)=n - 4
\]
...
\section{The computation for A128750}
...
\[
(n + 1)\,a(n) + (3 - 4 n)\,a(n-1) + (- 2 n - 1)\,a(n-2) + (4 n - 11)\,a(n-3) + (n - 4)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A128750, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A128750.
