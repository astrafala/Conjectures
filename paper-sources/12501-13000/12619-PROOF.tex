...
\title{A proof of the conjectured recurrence for OEIS A166287}
...
\begin{abstract}
OEIS A166287 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 3 x^{3}$, a polynomial of degree $3$, which proves
...
OEIS A166287 is ``Number of peak plateaux in all Dyck paths of semilength n with no UUU's and no DDD's (U=(1,1), D=(1,-1))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 0, 1, 3, 8, 21, 53,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2} + x + \sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)} - 1}{2 \sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(2*n-5)*(2*n-7)*a(n) -(2*n-7)*(6*n\^{}2-17*n+8)*a(n-1) +(n-2)*(4*n\^{}2-16*n-1)*a(n-2) +(-4*n\^{}3+32*n\^{}2-71*n+44)*a(n-3) +(2*n-3) (6*n\^{}2-37*n+54)*a(n-4) -(n-4)*(2*n-3)*(2*n-5)*a(n-5)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-23, revision 12) the
...
\[
p_{0}(n)=n \left(2 n - 7\right) \left(2 n - 5\right),\qquad p_{1}(n)=- \left(2 n - 7\right) \left(6 n^{2} - 17 n + 8\right),\qquad p_{2}(n)=\left(n - 2\right) \left(4 n^{2} - 16 n - 1\right),\qquad p_{3}(n)=- 4 n^{3} + 32 n^{2} - 71 n + 44,\qquad p_{4}(n)=\left(2 n - 3\right) \left(6 n^{2} - 37 n + 54\right),\qquad p_{5}(n)=- \left(n - 4\right) \left(2 n - 5\right) \left(2 n - 3\right)
\]
...
\section{The computation for A166287}
...
\[
n \left(2 n - 7\right) \left(2 n - 5\right)\,a(n) -  \left(2 n - 7\right) \left(6 n^{2} - 17 n + 8\right)\,a(n-1) + \left(n - 2\right) \left(4 n^{2} - 16 n - 1\right)\,a(n-2) + (- 4 n^{3} + 32 n^{2} - 71 n + 44)\,a(n-3) + \left(2 n - 3\right) \left(6 n^{2} - 37 n + 54\right)\,a(n-4) -  \left(n - 4\right) \left(2 n - 5\right) \left(2 n - 3\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A166287, agreeing on all 13 terms compared.
...
vanish, for all 26 values of $n$ from $n=5$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 3 x^{3}$ is an
...
\url{https://oeis.org}, 2026. Sequence A166287.
