...
\title{A proof of the conjectured recurrence for OEIS A174013}
...
\begin{abstract}
OEIS A174013 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 24 x^{5} - 6 x^{4} + 12 x^{3} - x^{2} + 3 x + 1$, a polynomial of degree $5$, which proves
...
OEIS A174013 is ``Sequence whose Hankel transform is a (1,1) Somos-4 sequence''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 6, 12, 22, 37, 56,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{2} - 3 x - \sqrt{5 x^{4} + 6 x^{3} - x^{2} - 2 x + 1} + 1}{2 x \left(x^{2} + x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +n*a(n-1) +12*(-n+1)*a(n-2) +3*(3*n-8)*a(n-3) +6*(6*n-23)*a(n-4) +11*(-n+2)*a(n-5) +(-49*n+253)*a(n-6) +20*(-n+6)*a(n-7)=0. - \emph{R. J. Mathar}, Jan 12 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-02-08, revision 14) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=n,\qquad p_{2}(n)=- 12 \left(n - 1\right),\qquad p_{3}(n)=3 \left(3 n - 8\right),\qquad p_{4}(n)=6 \left(6 n - 23\right),\qquad p_{5}(n)=- 11 \left(n - 2\right),\qquad p_{6}(n)=253 - 49 n,\qquad p_{7}(n)=- 20 \left(n - 6\right)
\]
...
\[
B(x)\;=\;- 24 x^{5} - 6 x^{4} + 12 x^{3} - x^{2} + 3 x + 1 ,
\]
...
\[
(n + 1)\,a(n) + n\,a(n-1) -  12 \left(n - 1\right)\,a(n-2) + 3 \left(3 n - 8\right)\,a(n-3) + 6 \left(6 n - 23\right)\,a(n-4) -  11 \left(n - 2\right)\,a(n-5) + (253 - 49 n)\,a(n-6) -  20 \left(n - 6\right)\,a(n-7) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A174013, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 24 x^{5} - 6 x^{4} + 12 x^{3} - x^{2} + 3 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A174013.
