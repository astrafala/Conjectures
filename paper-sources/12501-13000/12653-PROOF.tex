...
\title{A proof of the conjectured recurrence for OEIS A026030}
...
\begin{abstract}
OEIS A026030 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $24 x^{3} - 24 x^{2} + 6 x$, a polynomial of degree $3$, which proves
...
OEIS A026030 is ``a(n) = T(2n,n-1), where T is defined in A026022''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 4, 15, 56, 209, 780, 2912, 10880,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;- \frac{2 x \left(2 x - 1\right)}{\sqrt{1 - 4 x} \left(x - 1\right) \left(3 x - 1\right) - \left(2 x - 1\right) \left(x^{2} - 4 x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+5)*a(n) -2*(5*n+16)*a(n-1) +(35*n+47)*a(n-2) +2*(-25*n+14)*a(n-3) + 12*(2*n-5)*a(n-4) = 0. - \emph{R. J. Mathar}, Jun 15 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2026-01-21, revision 42) the
...
\[
p_{0}(n)=n + 5,\qquad p_{1}(n)=- 2 \left(5 n + 16\right),\qquad p_{2}(n)=35 n + 47,\qquad p_{3}(n)=- 2 \left(25 n - 14\right),\qquad p_{4}(n)=12 \left(2 n - 5\right)
\]
...
\section{The computation for A026030}
...
\[
(n + 5)\,a(n) -  2 \left(5 n + 16\right)\,a(n-1) + (35 n + 47)\,a(n-2) -  2 \left(25 n - 14\right)\,a(n-3) + 12 \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A026030, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=24 x^{3} - 24 x^{2} + 6 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A026030.
