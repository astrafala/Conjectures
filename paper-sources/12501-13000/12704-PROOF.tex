...
\title{A proof of the conjectured recurrence for OEIS A176752}
...
\begin{abstract}
OEIS A176752 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{3} - x^{2} - 4 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A176752 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=0, k=0 and l=-2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, -2, -6, -14, -26, -30, 30,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 4 x^{3} - 8 x^{2} + 5 x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(-3*n+1)*a(n-1) +(13*n-21)*a(n-2) +2*(-2*n+3)*a(n-3) +4*(-n+4)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 18 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-02-18, revision 4) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=13 n - 21,\qquad p_{3}(n)=- 2 \left(2 n - 3\right),\qquad p_{4}(n)=- 4 \left(n - 4\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + (13 n - 21)\,a(n-2) -  2 \left(2 n - 3\right)\,a(n-3) -  4 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A176752, agreeing on all 13 terms compared.
...
vanish, for all 23 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{3} - x^{2} - 4 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176752.
