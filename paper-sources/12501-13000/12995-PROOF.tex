...
\title{A proof of the conjectured recurrence for OEIS A191313}
...
\begin{abstract}
OEIS A191313 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 28 x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A191313 is ``Sum of the abscissae of the first returns to the horizontal axis (assumed to be 0 if there are no such returns) in all dispersed Dyck paths of length n (i.e., Motzkin paths of length n with no (1,0) steps at positive heights)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 2, 5, 15, 30, 71, 134,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x \left(4 x + \sqrt{1 - 4 x^{2}} - 1\right)}{\sqrt{1 - 4 x^{2}} \left(x - 1\right)^{2} \left(- 2 x + \sqrt{1 - 4 x^{2}} + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(3*n-13)*a(n) +2*(-6*n\^{}2+29*n-18)*a(n-1) +(3*n\^{}2-13*n+24)*a(n-2) +2*(21*n\^{}2-124*n+150)*a(n-3) +4*(-15*n\^{}2+92*n-132) *a(n-4) +8*(n-3)*(3*n-10) *a(n-5)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2022-07-15, revision 23) the
...
\[
p_{0}(n)=n \left(3 n - 13\right),\qquad p_{1}(n)=- 2 \left(6 n^{2} - 29 n + 18\right),\qquad p_{2}(n)=3 n^{2} - 13 n + 24,\qquad p_{3}(n)=2 \left(21 n^{2} - 124 n + 150\right),\qquad p_{4}(n)=- 4 \left(15 n^{2} - 92 n + 132\right),\qquad p_{5}(n)=8 \left(n - 3\right) \left(3 n - 10\right)
\]
...
\section{The computation for A191313}
...
\[
n \left(3 n - 13\right)\,a(n) -  2 \left(6 n^{2} - 29 n + 18\right)\,a(n-1) + (3 n^{2} - 13 n + 24)\,a(n-2) + 2 \left(21 n^{2} - 124 n + 150\right)\,a(n-3) -  4 \left(15 n^{2} - 92 n + 132\right)\,a(n-4) + 8 \left(n - 3\right) \left(3 n - 10\right)\,a(n-5) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A191313, agreeing on all 13 terms compared.
...
vanish, for all 30 values of $n$ from $n=5$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 28 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A191313.
