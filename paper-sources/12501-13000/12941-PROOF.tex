...
\title{A proof of the conjectured recurrence for OEIS A247170}
...
\begin{abstract}
OEIS A247170 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 9 x^{4} - 21 x^{3} - 12 x^{2}$, a polynomial of degree $4$, which proves
...
OEIS A247170 is ``Expansion of (-3/2+(x\textasciicircum{}3+3*x)/(sqrt(x\textasciicircum{}4-4*x\textasciicircum{}3-2*x\textasciicircum{}2+1)*2*x))''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;0, 2, 3, 2, 10, 11, 21, 50,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{x^{2} - 3 \sqrt{x^{4} - 4 x^{3} - 2 x^{2} + 1} + 3}{2 \sqrt{x^{4} - 4 x^{3} - 2 x^{2} + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjectured: +n*(2*n-7)*a(n) +(n-1)*(2*n-9)*a(n-1) +2*(-2*n\textasciicircum{}2+9*n-6)*a(n-2) +2*(-6*n\textasciicircum{}2+33*n-38)*a(n-3) +3*(-2*n\textasciicircum{}2+15*n-26)*a(n-4) +(2*n-5)*(n-5)*a(n-5) = 0.
\end{quote}
As of the ``Last modified'' line on the live entry (2022-09-08, revision 24) the
...
\[
p_{0}(n)=n \left(2 n - 7\right),\qquad p_{1}(n)=\left(n - 1\right) \left(2 n - 9\right),\qquad p_{2}(n)=- 2 \left(2 n^{2} - 9 n + 6\right),\qquad p_{3}(n)=- 2 \left(6 n^{2} - 33 n + 38\right),\qquad p_{4}(n)=- 3 \left(2 n^{2} - 15 n + 26\right),\qquad p_{5}(n)=\left(n - 5\right) \left(2 n - 5\right)
\]
...
\section{The computation for A247170}
...
\[
n \left(2 n - 7\right)\,a(n) + \left(n - 1\right) \left(2 n - 9\right)\,a(n-1) -  2 \left(2 n^{2} - 9 n + 6\right)\,a(n-2) -  2 \left(6 n^{2} - 33 n + 38\right)\,a(n-3) -  3 \left(2 n^{2} - 15 n + 26\right)\,a(n-4) + \left(n - 5\right) \left(2 n - 5\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A247170, agreeing on all 13 terms compared.
...
vanish, for all 29 values of $n$ from $n=6$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 9 x^{4} - 21 x^{3} - 12 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A247170.
