...
\title{A proof of the conjectured recurrence for OEIS A178072}
...
\begin{abstract}
OEIS A178072 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 24 x^{5} + 34 x^{4} - 36 x^{3} - 42 x^{2} + 4 x$, a polynomial of degree $5$, which proves
...
OEIS A178072 is ``Sequence with Hankel transform equal to the Somos-4 sequence A006769(n+2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, -1, -1, -1, -1, 1, 8,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{x^{2} + 2 x + \sqrt{x^{4} + 6 x^{2} - 4 x + 1} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*n*(n+1)*a(n) +2*n*(n+1)*a(n-1) -3*(3*n-1)*(3*n-4)*a(n-2) +(61*n\^{}2-191*n+36)*a(n-3) +6*(-2*n\^{}2+2*n-1)*a(n-4) +2*(5*n-1)*(4*n-15)*a(n-5) +n*(n-5)*a(n-6) +(5*n-1)*(n-6)*a(n-7)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-05-11, revision 35) the
...
\[
p_{0}(n)=2 n \left(n + 1\right),\qquad p_{1}(n)=2 n \left(n + 1\right),\qquad p_{2}(n)=- 3 \left(3 n - 4\right) \left(3 n - 1\right),\qquad p_{3}(n)=61 n^{2} - 191 n + 36,\qquad p_{4}(n)=- 6 \left(2 n^{2} - 2 n + 1\right),\qquad p_{5}(n)=2 \left(4 n - 15\right) \left(5 n - 1\right),\qquad p_{6}(n)=n \left(n - 5\right),\qquad p_{7}(n)=\left(n - 6\right) \left(5 n - 1\right)
\]
...
\section{The computation for A178072}
...
\[
B(x)\;=\;- 24 x^{5} + 34 x^{4} - 36 x^{3} - 42 x^{2} + 4 x ,
\]
...
\[
2 n \left(n + 1\right)\,a(n) + 2 n \left(n + 1\right)\,a(n-1) -  3 \left(3 n - 4\right) \left(3 n - 1\right)\,a(n-2) + (61 n^{2} - 191 n + 36)\,a(n-3) -  6 \left(2 n^{2} - 2 n + 1\right)\,a(n-4) + 2 \left(4 n - 15\right) \left(5 n - 1\right)\,a(n-5) + n \left(n - 5\right)\,a(n-6) + \left(n - 6\right) \left(5 n - 1\right)\,a(n-7) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A178072, agreeing on all 13 terms compared.
...
vanish, for all 27 values of $n$ from $n=7$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 24 x^{5} + 34 x^{4} - 36 x^{3} - 42 x^{2} + 4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A178072.
