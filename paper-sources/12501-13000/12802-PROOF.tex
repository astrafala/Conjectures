...
\title{A proof of the conjectured recurrence for OEIS A026163}
...
\begin{abstract}
OEIS A026163 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $224 x$, a polynomial of degree $1$, which proves
...
OEIS A026163 is ``Sum\{T(k,k-1)\}, k = 1,2,...,n, where T is the array in A026148''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 6, 16, 45, 126, 356, 1008,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{\left(2 x - 1\right) \left(x^{4} - 4 x^{3} - 2 x^{2} + 4 x + \sqrt{1 - 3 x} \left(x - 1\right) \sqrt{x + 1} \left(x^{2} + 2 x - 1\right) - 1\right)}{2 x^{7}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+7)*(3*n-31)*a(n) +3*(-n\^{}2-35*n-76)*a(n-1) +2*(32*n\^{}2+27*n-459)*a(n-2) +(-47*n\^{}2+286*n-204)*a(n-3) -3*(37*n-51)*(n-2)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 23 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2020-05-10, revision 15) the
...
\[
p_{0}(n)=- \left(n + 7\right) \left(3 n - 31\right),\qquad p_{1}(n)=- 3 \left(n^{2} + 35 n + 76\right),\qquad p_{2}(n)=2 \left(32 n^{2} + 27 n - 459\right),\qquad p_{3}(n)=- 47 n^{2} + 286 n - 204,\qquad p_{4}(n)=- 3 \left(n - 2\right) \left(37 n - 51\right)
\]
...
\[
- \left(n + 7\right) \left(3 n - 31\right)\,a(n) -  3 \left(n^{2} + 35 n + 76\right)\,a(n-1) + 2 \left(32 n^{2} + 27 n - 459\right)\,a(n-2) + (- 47 n^{2} + 286 n - 204)\,a(n-3) -  3 \left(n - 2\right) \left(37 n - 51\right)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A026163, agreeing on all 13 terms compared.
...
vanish, for all 23 values of $n$ from $n=5$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=224 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A026163.
