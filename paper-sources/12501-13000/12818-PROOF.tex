...
\title{A proof of the conjectured recurrence for OEIS A127363}
...
\begin{abstract}
OEIS A127363 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 3 x$, a polynomial of degree $1$, which proves
...
OEIS A127363 is ``a(n) = Sum\_\{k=0..n\} C(n,floor(k/2))*(-4)\^{}(n-k)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -3, 14, -57, 246, -1038, 4424, -18777,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x - \sqrt{1 - 4 x^{2}} + 1}{2 \sqrt{1 - 4 x^{2}} \left(x - 2 \sqrt{1 - 4 x^{2}} + 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 4*n*a(n) +(17*n-8)*a(n-1) +2*(-8*n-1)*a(n-2) +68*(-n+2)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-07-31, revision 14) the
...
\[
p_{0}(n)=4 n,\qquad p_{1}(n)=17 n - 8,\qquad p_{2}(n)=- 2 \left(8 n + 1\right),\qquad p_{3}(n)=- 68 \left(n - 2\right)
\]
...
\section{The computation for A127363}
...
\[
4 n\,a(n) + (17 n - 8)\,a(n-1) -  2 \left(8 n + 1\right)\,a(n-2) -  68 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A127363, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 3 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A127363.
