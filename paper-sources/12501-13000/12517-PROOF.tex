...
\title{A proof of the conjectured recurrence for OEIS A117186}
...
\begin{abstract}
OEIS A117186 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{2} - 2 x - 1$, a polynomial of degree $2$, which proves
...
OEIS A117186 is ``Expansion of (1+x)c(x\textasciicircum{}2)/((1-xc(x\textasciicircum{}2))*sqrt(1-4x\textasciicircum{}2)), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 9, 21, 38, 86, 157,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{\left(x + 1\right) \left(\sqrt{1 - 4 x^{2}} - 1\right)}{x \sqrt{1 - 4 x^{2}} \left(2 x + \sqrt{1 - 4 x^{2}} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+1)*a(n) +(n+1)*a(n-1) +2*(3*n-2)*a(n-2) -4*n*a(n-3) +8*(3-n)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2021-07-09, revision 12) the
...
\[
p_{0}(n)=- n - 1,\qquad p_{1}(n)=n + 1,\qquad p_{2}(n)=2 \left(3 n - 2\right),\qquad p_{3}(n)=- 4 n,\qquad p_{4}(n)=- 8 \left(n - 3\right)
\]
...
\section{The computation for A117186}
...
\[
(- n - 1)\,a(n) + (n + 1)\,a(n-1) + 2 \left(3 n - 2\right)\,a(n-2) -  4 n\,a(n-3) -  8 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A117186, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{2} - 2 x - 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A117186.
