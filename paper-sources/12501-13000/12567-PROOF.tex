...
\title{A proof of the conjectured recurrence for OEIS A048775}
...
\begin{abstract}
OEIS A048775 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{3} + 5 x^{2} + 2 x$, a polynomial of degree $3$, which proves
...
OEIS A048775 is ``Number of (partially defined) monotone maps from intervals of 1..n to 1..n''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 7, 31, 121, 456, 1709, 6427, 24301,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;- \frac{1}{\left(x - 1\right)^{2}} - \frac{1}{2 x} + \frac{1}{2 x \sqrt{1 - 4 x}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + (-7*n-2)*a(n-1) + 3*(5*n-3)*a(n-2) + (-13*n+20)*a(n-3) + 2*(2*n-5)*a(n-4) = 0. - \emph{R. J. Mathar}, Nov 30 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-06-28, revision 57) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 7 n - 2,\qquad p_{2}(n)=3 \left(5 n - 3\right),\qquad p_{3}(n)=20 - 13 n,\qquad p_{4}(n)=2 \left(2 n - 5\right)
\]
...
\section{The computation for A048775}
...
\[
(n + 1)\,a(n) + (- 7 n - 2)\,a(n-1) + 3 \left(5 n - 3\right)\,a(n-2) + (20 - 13 n)\,a(n-3) + 2 \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A048775, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{3} + 5 x^{2} + 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A048775.
