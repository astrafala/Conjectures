...
\title{A proof of the conjectured recurrence for OEIS A191782}
...
\begin{abstract}
OEIS A191782 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{2} + 44 x$, a polynomial of degree $2$, which proves
...
OEIS A191782 is ``Sum of the lengths of the first ascents in all n-length left factors of Dyck paths''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 3, 6, 13, 24, 49, 90, 181,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{\left(4 x^{3} + \left(\sqrt{1 - 4 x^{2}} - 1\right)^{2}\right) \left(\sqrt{1 - 4 x^{2}} - 1\right)}{4 x^{3} \left(x - 1\right) \left(2 x + \sqrt{1 - 4 x^{2}} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+3)*(6*n-17)*a(n) +2*(6*n\^{}2-2*n-57)*a(n-1) +3*(6*n\^{}2-17*n+27)*a(n-2) -2*(2*n-3)*(12*n-25)*a(n-3) +4*(6*n-11)*(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2019-02-10, revision 21) the
...
\[
p_{0}(n)=- \left(n + 3\right) \left(6 n - 17\right),\qquad p_{1}(n)=2 \left(6 n^{2} - 2 n - 57\right),\qquad p_{2}(n)=3 \left(6 n^{2} - 17 n + 27\right),\qquad p_{3}(n)=- 2 \left(2 n - 3\right) \left(12 n - 25\right),\qquad p_{4}(n)=4 \left(n - 3\right) \left(6 n - 11\right)
\]
...
\section{The computation for A191782}
...
\[
- \left(n + 3\right) \left(6 n - 17\right)\,a(n) + 2 \left(6 n^{2} - 2 n - 57\right)\,a(n-1) + 3 \left(6 n^{2} - 17 n + 27\right)\,a(n-2) -  2 \left(2 n - 3\right) \left(12 n - 25\right)\,a(n-3) + 4 \left(n - 3\right) \left(6 n - 11\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A191782, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=x^{2} + 44 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A191782.
