...
\title{A proof of the conjectured recurrence for OEIS A128096}
...
\begin{abstract}
OEIS A128096 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3090 x^{3} - 7800 x^{2} + 31530 x$, a polynomial of degree $3$, which proves
...
OEIS A128096 is ``Number of steps that touch the x-axis in all peakless Motzkin paths of length n''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 5, 12, 27, 62, 144, 336,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{4 \left(- x^{2} - \sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)} + 1\right)}{\left(x^{2} - x + \sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)} + 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -2*(n+4)*(1088*n-4241)*a(n) +(6616*n\^{}2-12361*n-59102)*a(n-1) +2*(-1176*n\^{}2+6520*n-4985)*a(n-2) +(2088*n\^{}2-6071*n+16522) *a(n-3) +2*(-3352*n\^{}2+22882*n-35653)*a(n-4) +(2264*n-6277)*(n-6) *a(n-5)=0. - \emph{R. J. Mathar}, Jun 17 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-06-17, revision 5) the
...
\[
p_{0}(n)=- 2 \left(n + 4\right) \left(1088 n - 4241\right),\qquad p_{1}(n)=6616 n^{2} - 12361 n - 59102,\qquad p_{2}(n)=- 2 \left(1176 n^{2} - 6520 n + 4985\right),\qquad p_{3}(n)=2088 n^{2} - 6071 n + 16522,\qquad p_{4}(n)=- 2 \left(3352 n^{2} - 22882 n + 35653\right),\qquad p_{5}(n)=\left(n - 6\right) \left(2264 n - 6277\right)
\]
...
\section{The computation for A128096}
...
\[
- 2 \left(n + 4\right) \left(1088 n - 4241\right)\,a(n) + (6616 n^{2} - 12361 n - 59102)\,a(n-1) -  2 \left(1176 n^{2} - 6520 n + 4985\right)\,a(n-2) + (2088 n^{2} - 6071 n + 16522)\,a(n-3) -  2 \left(3352 n^{2} - 22882 n + 35653\right)\,a(n-4) + \left(n - 6\right) \left(2264 n - 6277\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A128096, agreeing on all 13 terms compared.
...
vanish, for all 25 values of $n$ from $n=6$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=3090 x^{3} - 7800 x^{2} + 31530 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A128096.
