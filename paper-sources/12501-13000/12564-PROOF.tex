...
\title{A proof of the conjectured recurrence for OEIS A115967}
...
\begin{abstract}
OEIS A115967 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x^{2} + x$, a polynomial of degree $2$, which proves
...
OEIS A115967 is ``Expansion of 1/(2*sqrt(1-2*x-3*x\^{}2) - 1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 8, 28, 104, 384, 1428, 5316,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 \sqrt{- 3 x^{2} - 2 x + 1} - 1}{12 x^{2} + 8 x - 3} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 3*n*a(n) + (-14*n+9)*a(n-1) + (-5*n+3)*a(n-2) + 12*(4*n-9)* a(n-3) + 36*(n-3)*a(n-4) = 0. - \emph{R. J. Mathar}, Nov 15 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 32) the
...
\[
p_{0}(n)=3 n,\qquad p_{1}(n)=9 - 14 n,\qquad p_{2}(n)=3 - 5 n,\qquad p_{3}(n)=12 \left(4 n - 9\right),\qquad p_{4}(n)=36 \left(n - 3\right)
\]
...
\section{The computation for A115967}
...
\[
3 n\,a(n) + (9 - 14 n)\,a(n-1) + (3 - 5 n)\,a(n-2) + 12 \left(4 n - 9\right)\,a(n-3) + 36 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A115967, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=3 x^{2} + x$ is an
...
\url{https://oeis.org}, 2026. Sequence A115967.
