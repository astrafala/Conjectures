...
\title{A proof of the conjectured recurrence for OEIS A191319}
...
\begin{abstract}
OEIS A191319 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A191319 is ``Sum of pyramid weights of all dispersed Dyck paths of length n (i.e., of all Motzkin paths of length n with no (1,0) steps at positive heights)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 2, 7, 14, 37, 74,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{2}}{\sqrt{1 - 4 x^{2}} \left(2 x - 1\right) \left(x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+2)*a(n) +2*a(n-1) +5*(n-2)*a(n-2) -2*a(n-3) +4*(-n+2)*a(n-4)=0. - \emph{R. J. Mathar}, Dec 07 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 25) the
...
\[
p_{0}(n)=2 - n,\qquad p_{1}(n)=2,\qquad p_{2}(n)=5 \left(n - 2\right),\qquad p_{3}(n)=-2,\qquad p_{4}(n)=- 4 \left(n - 2\right)
\]
...
\section{The computation for A191319}
...
\[
(2 - n)\,a(n) + 2\,a(n-1) + 5 \left(n - 2\right)\,a(n-2) - 2\,a(n-3) -  4 \left(n - 2\right)\,a(n-4) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A191319, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A191319.
