...
\title{A proof of the conjectured recurrence for OEIS A114590}
...
\begin{abstract}
OEIS A114590 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $4 - 4 x$, a polynomial of degree $1$, which proves
...
OEIS A114590 is ``Number of peaks at even levels in all hill-free Dyck paths of semilength n+2 (a hill in a Dyck path is a peak at level 1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 8, 28, 103, 382, 1432, 5408,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x^{2} - \sqrt{1 - 4 x} \left(2 x + 1\right) + 1}{2 x^{2} \sqrt{1 - 4 x} \left(x + 2\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n+2)*a(n) +(-7*n-9)*a(n-1) -18*a(n-2) +2*(-7*n+19)*a(n-3) +4*(-2*n+3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 15 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2014-02-13, revision 13) the
...
\[
p_{0}(n)=2 \left(n + 2\right),\qquad p_{1}(n)=- 7 n - 9,\qquad p_{2}(n)=-18,\qquad p_{3}(n)=- 2 \left(7 n - 19\right),\qquad p_{4}(n)=- 4 \left(2 n - 3\right)
\]
...
\section{The computation for A114590}
...
\[
2 \left(n + 2\right)\,a(n) + (- 7 n - 9)\,a(n-1) - 18\,a(n-2) -  2 \left(7 n - 19\right)\,a(n-3) -  4 \left(2 n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A114590, agreeing on all 13 terms compared.
...
vanish, for all 21 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=4 - 4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A114590.
