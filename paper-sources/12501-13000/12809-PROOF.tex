...
\title{A proof of the conjectured recurrence for OEIS A000986}
...
\begin{abstract}
OEIS A000986 carries a conjectured linear recurrence with polynomial coefficients, of
order 5. The entry separately states a recurrence of order 4 that is not
...
OEIS A000986 is ``Number of n X n symmetric matrices with (0,1) entries and all row sums 2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 1, 4, 18, 112, 820, 6912,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} 2*a(n) +2*(-2*n+1)*a(n-1) +2*(n\^{}2-2*n-1)*a(n-2) -2*(n-2)*(n-4)*a(n-3) +(n-1)*(n-2)*(n-3)*a(n-4) -(n-2)*(n-3)*(n-4)*a(n-5)=0. - \emph{R. J. Mathar}, Aug 04 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 65) it is still
...
\begin{quote}\small
Recurrence: 2*a(n) = 4*(n-1)*a(n-1) - 2*(n-3)*(n-1)*a(n-2) - (n-3)*(n-2)*(n-1)*a(n-4). - \emph{Vaclav Kotesovec}, Feb 13 2014
\end{quote}
...
\section{The computation for A000986}
...
\[
2\,a(n) -  2 \left(2 n - 1\right)\,a(n-1) + 2 \left(n^{2} - 2 n - 1\right)\,a(n-2) -  2 \left(n - 4\right) \left(n - 2\right)\,a(n-3) + \left(n - 3\right) \left(n - 2\right) \left(n - 1\right)\,a(n-4) -  \left(n - 4\right) \left(n - 3\right) \left(n - 2\right)\,a(n-5) \;=\;0
\]
...
\url{https://oeis.org}, 2026. Sequence A000986.
