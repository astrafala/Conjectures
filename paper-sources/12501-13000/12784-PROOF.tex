...
\title{A proof of the conjectured recurrence for OEIS A071715}
...
\begin{abstract}
OEIS A071715 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $20068 x + 47768$, a polynomial of degree $1$, which proves
...
OEIS A071715 is ``Expansion of (1+x*C\^{}4)*C\^{}4, where C = (1-(1-4*x)\^{}(1/2))/(2*x) is g.f. for Catalan numbers, A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 5, 22, 92, 373, 1482, 5810, 22576,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(16 x^{3} + \left(\sqrt{1 - 4 x} - 1\right)^{4}\right) \left(\sqrt{1 - 4 x} - 1\right)^{4}}{256 x^{7}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (891*n+6824)*(n+7)*a(n) + (891*n\^{}2-38453*n-250970)*a(n-1) + 4*(-5040*n\^{}2-4271*n+8095)*a(n-2) + 4*(1170*n+3847)*(2*n-1)*a(n-3) = 0. - \emph{R. J. Mathar}, Aug 25 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-26, revision 15) the
...
\[
p_{0}(n)=\left(n + 7\right) \left(891 n + 6824\right),\qquad p_{1}(n)=891 n^{2} - 38453 n - 250970,\qquad p_{2}(n)=- 4 \left(5040 n^{2} + 4271 n - 8095\right),\qquad p_{3}(n)=4 \left(2 n - 1\right) \left(1170 n + 3847\right)
\]
...
\section{The computation for A071715}
...
\[
\left(n + 7\right) \left(891 n + 6824\right)\,a(n) + (891 n^{2} - 38453 n - 250970)\,a(n-1) -  4 \left(5040 n^{2} + 4271 n - 8095\right)\,a(n-2) + 4 \left(2 n - 1\right) \left(1170 n + 3847\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A071715, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=20068 x + 47768$ is an
...
\url{https://oeis.org}, 2026. Sequence A071715.
