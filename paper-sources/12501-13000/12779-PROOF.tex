...
\title{A proof of the conjectured recurrence for OEIS A294159}
...
\begin{abstract}
OEIS A294159 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{3} + x^{2} - x$, a polynomial of degree $3$, which proves
...
OEIS A294159 is ``Alternating row sums of triangle A291844''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 6, 18, 55, 171, 538,\ \dots
\]
The entry posts no generating function of its own. Its NAME states the
relation \texttt{the entry states that the generating function satisfies G.f. y(x) satisfies: 0 = (x\textasciicircum{}3 + x\textasciicircum{}2 + 3*x - 1)*(y\textasciicircum{}2 - y) + x} to another entry, and that entry's exponential generating
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{3} + x^{2} + 3 x - \sqrt{\left(x - 1\right) \left(x^{3} + x^{2} + 3 x - 1\right)} \left(x + 1\right) - 1}{2 \left(x^{3} + x^{2} + 3 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} D-finite with recurrence n*a(n) +(-3*n+1)*a(n-1) +2*(-n+3)*a(n-2) +2*(n-5)*a(n-3) +(n-4)*a(n-4) +(n-5)*a(n-5)=0. - \emph{R. J. Mathar}, Jun 17 2020
\end{quote}
As of the ``Last modified'' line on the live entry (2020-06-17, revision 11) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=1 - 3 n,\qquad p_{2}(n)=- 2 \left(n - 3\right),\qquad p_{3}(n)=2 \left(n - 5\right),\qquad p_{4}(n)=n - 4,\qquad p_{5}(n)=n - 5
\]
...
\section{The computation for A294159}
...
\[
n\,a(n) + (1 - 3 n)\,a(n-1) -  2 \left(n - 3\right)\,a(n-2) + 2 \left(n - 5\right)\,a(n-3) + (n - 4)\,a(n-4) + (n - 5)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A294159, agreeing on all 13 terms compared.
...
vanish, for all 25 values of $n$ from $n=5$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{3} + x^{2} - x$ is an
...
\url{https://oeis.org}, 2026. Sequence A294159.
