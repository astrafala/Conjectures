...
\title{A proof of the conjectured recurrence for OEIS A105524}
...
\begin{abstract}
OEIS A105524 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $24 x + 24$, a polynomial of degree $1$, which proves
...
OEIS A105524 is ``Diagonal sums of number triangle A105522''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -2, 2, 0, -1, -2, 4, 4,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 4 x^{2} - 2 x + \left(2 x + 1\right) \sqrt{4 x^{2} + 1} - 1}{x^{2} \left(\sqrt{4 x^{2} + 1} - 3\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n+2)*(13*n\^{}2-35*n+6)*a(n) +24*(n-8)*a(n-1) +(91*n\^{}3-375*n\^{}2+152*n+372)*a(n-2) +12*(8-n)*a(n-3) -4*(n-3)*(13*n\^{}2-9*n-16)*a(n-4) = 0. - \emph{R. J. Mathar}, Nov 09 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-01, revision 10) the
...
\[
p_{0}(n)=2 \left(n + 2\right) \left(13 n^{2} - 35 n + 6\right),\qquad p_{1}(n)=24 \left(n - 8\right),\qquad p_{2}(n)=91 n^{3} - 375 n^{2} + 152 n + 372,\qquad p_{3}(n)=- 12 \left(n - 8\right),\qquad p_{4}(n)=- 4 \left(n - 3\right) \left(13 n^{2} - 9 n - 16\right)
\]
...
\section{The computation for A105524}
...
\[
2 \left(n + 2\right) \left(13 n^{2} - 35 n + 6\right)\,a(n) + 24 \left(n - 8\right)\,a(n-1) + (91 n^{3} - 375 n^{2} + 152 n + 372)\,a(n-2) -  12 \left(n - 8\right)\,a(n-3) -  4 \left(n - 3\right) \left(13 n^{2} - 9 n - 16\right)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A105524, agreeing on all 13 terms compared.
...
vanish, for all 35 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=24 x + 24$ is an
...
\url{https://oeis.org}, 2026. Sequence A105524.
