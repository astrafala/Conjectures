...
\title{A proof of the conjectured recurrence for OEIS A132864}
...
\begin{abstract}
OEIS A132864 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $30 x$, a polynomial of degree $1$, which proves
...
OEIS A132864 is ``Expansion of 1/(1-4x*c(5x)), where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 36, 424, 5716, 83544, 1288296, 20637264,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{5}{2 \sqrt{1 - 20 x} + 3} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + 2*(15-2*n)*a(n-1) + 160*(3-2*n)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2021-06-05, revision 28) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(2 n - 15\right),\qquad p_{2}(n)=- 160 \left(2 n - 3\right)
\]
...
\section{The computation for A132864}
...
\[
n\,a(n) -  2 \left(2 n - 15\right)\,a(n-1) -  160 \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A132864, agreeing on all 13 terms compared.
...
vanish, for all 17 values of $n$ from $n=2$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=30 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A132864.
