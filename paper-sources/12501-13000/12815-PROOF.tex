...
\title{A proof of the conjectured recurrence for OEIS A108600}
...
\begin{abstract}
OEIS A108600 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{5} + 15 x^{4} + 28 x^{3} - 12 x^{2} - 3 x + 1$, a polynomial of degree $5$, which proves
...
OEIS A108600 is ``Number of freely braided permutations of length n; the freely braided permutations are those that avoid 3421, 4231, 4312 and 4321''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 6, 20, 71, 260, 971,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x^{2} + 3 x - \sqrt{1 - 4 x} \left(x + 1\right) - 1}{x^{2} + 4 x + \sqrt{1 - 4 x} \left(x^{2} - 1\right) - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (1-n)*a(n) +(7*n-10)*a(n-1) +2*(1-4*n)*a(n-2) +8*(11-2*n)*a(n-3) +(n-1)*a(n-4) +3*(2-n)*a(n-5) +2*(11-2*n)*a(n-6)=0. - \emph{R. J. Mathar}, Aug 24 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-12, revision 9) the
...
\[
p_{0}(n)=1 - n,\qquad p_{1}(n)=7 n - 10,\qquad p_{2}(n)=- 2 \left(4 n - 1\right),\qquad p_{3}(n)=- 8 \left(2 n - 11\right),\qquad p_{4}(n)=n - 1,\qquad p_{5}(n)=- 3 \left(n - 2\right),\qquad p_{6}(n)=- 2 \left(2 n - 11\right)
\]
...
\section{The computation for A108600}
...
\[
B(x)\;=\;- x^{5} + 15 x^{4} + 28 x^{3} - 12 x^{2} - 3 x + 1 ,
\]
...
\[
(1 - n)\,a(n) + (7 n - 10)\,a(n-1) -  2 \left(4 n - 1\right)\,a(n-2) -  8 \left(2 n - 11\right)\,a(n-3) + (n - 1)\,a(n-4) -  3 \left(n - 2\right)\,a(n-5) -  2 \left(2 n - 11\right)\,a(n-6) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A108600, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{5} + 15 x^{4} + 28 x^{3} - 12 x^{2} - 3 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A108600.
