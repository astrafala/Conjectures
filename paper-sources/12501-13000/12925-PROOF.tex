...
\title{A proof of the conjectured recurrence for OEIS A126568}
...
\begin{abstract}
OEIS A126568 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 15 x^{2} - 2 x$, a polynomial of degree $2$, which proves
...
OEIS A126568 is ``Binomial transform of A026641''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 7, 29, 127, 572, 2623, 12182,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 11 x^{4} + 12 x^{3} + x^{2} - 3 x + \sqrt{5 x^{2} - 6 x + 1} \left(5 x^{3} - 3 x^{2} - 1\right) + 1}{10 x^{5} - 32 x^{4} + 31 x^{3} - 20 x^{2} + 13 x + \sqrt{5 x^{2} - 6 x + 1} \left(- 4 x^{4} + 8 x^{3} + 3 x^{2} - 7 x + 2\right) - 2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 4*n*a(n) +2*(2*n-7)*a(n-1) +(-163*n+267)*a(n-2) +10*(23*n-58)*a(n-3) +75*(-n+3)*a(n-4) = 0. - \emph{R. J. Mathar}, Jun 30 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2026-07-02, revision 27) the
...
\[
p_{0}(n)=4 n,\qquad p_{1}(n)=2 \left(2 n - 7\right),\qquad p_{2}(n)=267 - 163 n,\qquad p_{3}(n)=10 \left(23 n - 58\right),\qquad p_{4}(n)=- 75 \left(n - 3\right)
\]
...
\section{The computation for A126568}
...
\[
4 n\,a(n) + 2 \left(2 n - 7\right)\,a(n-1) + (267 - 163 n)\,a(n-2) + 10 \left(23 n - 58\right)\,a(n-3) -  75 \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A126568, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 15 x^{2} - 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A126568.
