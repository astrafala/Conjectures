...
\title{A proof of the conjectured recurrence for OEIS A022917}
...
\begin{abstract}
OEIS A022917 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A022917 is ``Multinomial coefficient n!/ ([n/4]!, [(n+1)/4]!, [(n+2)/4]!, [(n+3)/4]!)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 6, 24, 60, 180, 630,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} -(126*n-1)*(n+3)*(n+2)*(n+1)*a(n) +4*(-208*n\^{}4-51*n\^{}3+793*n\^{}2+376*n-6)*a(n-1) +16*(334*n\^{}3-114*n\^{}2-5*n-369)*a(n-2) +64*(n-2)*(334*n\^{}2+264*n-213)*a(n-3) +768*(n-2)*(n-3)*(42*n\^{}2+153*n+113)*a(n-4) +1024*(208*n+177)*(n-2)*(n-3)*(n-4)*a(n-5)=0. - \emph{R. J. Mathar}, Aug 06 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2019-10-11, revision 30) it is still
...
\section{The computation for A022917}

Reading both relations as operators and dividing the conjectured one by the stated one
leaves remainder $0$, with quotient
\[
Q \;=\; \left(\frac{- 832 n - 4868}{4 n^{3} + 68 n^{2} + 384 n + 719}\right) + \left(\frac{- 126 n - 629}{4 n^{3} + 68 n^{2} + 384 n + 719}\right)\,N .
\]
...
\[
- \left(n + 1\right) \left(n + 2\right) \left(n + 3\right) \left(126 n - 1\right)\,a(n) -  4 \left(208 n^{4} + 51 n^{3} - 793 n^{2} - 376 n + 6\right)\,a(n-1) + 16 \left(334 n^{3} - 114 n^{2} - 5 n - 369\right)\,a(n-2) + 64 \left(n - 2\right) \left(334 n^{2} + 264 n - 213\right)\,a(n-3) + 768 \left(n - 3\right) \left(n - 2\right) \left(42 n^{2} + 153 n + 113\right)\,a(n-4) + 1024 \left(n - 4\right) \left(n - 3\right) \left(n - 2\right) \left(208 n + 177\right)\,a(n-5) \;=\;0
\]
...
\url{https://oeis.org}, 2026. Sequence A022917.
