...
\title{A proof of the conjectured recurrence for OEIS A278023}
...
\begin{abstract}
OEIS A278023 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $156 x^{4} - 6 x^{3} - 2 x^{2} + 2 x$, a polynomial of degree $4$, which proves
...
OEIS A278023 is ``G.f.: 2*x*(1-x*sqrt(1-4*x))/((1+2*x\^{}2+sqrt(1-4*x))*sqrt(1-4*x))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 2, 8, 30, 109, 401, 1495,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{2 x \left(x \sqrt{1 - 4 x} - 1\right)}{\sqrt{1 - 4 x} \left(2 x^{2} + \sqrt{1 - 4 x} + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +n*(3*n\^{}2-12*n+11) *a(n) -(3*n-5) *(3*n\^{}2-9*n+4) *a(n-1) -2*(2*n-5) *(3*n\^{}2-6*n+2) *a(n-2) +n *(3*n\^{}2-12*n+11) *a(n-3) -2 *(2*n-5) *(3*n\^{}2-6*n+2) *a(n-4)=0. - \emph{R. J. Mathar}, Jun 24 2018
\end{quote}
As of the ``Last modified'' line on the live entry (2018-06-24, revision 13) the
...
\[
p_{0}(n)=n \left(3 n^{2} - 12 n + 11\right),\qquad p_{1}(n)=- \left(3 n - 5\right) \left(3 n^{2} - 9 n + 4\right),\qquad p_{2}(n)=- 2 \left(2 n - 5\right) \left(3 n^{2} - 6 n + 2\right),\qquad p_{3}(n)=n \left(3 n^{2} - 12 n + 11\right),\qquad p_{4}(n)=- 2 \left(2 n - 5\right) \left(3 n^{2} - 6 n + 2\right)
\]
...
\section{The computation for A278023}
...
\[
n \left(3 n^{2} - 12 n + 11\right)\,a(n) -  \left(3 n - 5\right) \left(3 n^{2} - 9 n + 4\right)\,a(n-1) -  2 \left(2 n - 5\right) \left(3 n^{2} - 6 n + 2\right)\,a(n-2) + n \left(3 n^{2} - 12 n + 11\right)\,a(n-3) -  2 \left(2 n - 5\right) \left(3 n^{2} - 6 n + 2\right)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A278023, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=156 x^{4} - 6 x^{3} - 2 x^{2} + 2 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A278023.
