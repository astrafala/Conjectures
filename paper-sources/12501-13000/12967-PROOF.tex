...
\title{The empirical recurrence for OEIS A268797, recovered from its tail}
...
\begin{abstract}
OEIS A268797 records ``Empirical recurrence of order $68$'' and keeps the recurrence itself in
a linked file rather than in the entry, so the conjecture cannot be read off the entry at all.
It can still be settled. The entry counts arrays under a condition local to a bounded window of
lines, which makes the count a walk count on a finite digraph and forces a monic integer
recurrence of order at most the number $S'=719$ of states with distinct futures. Berlekamp--Massey on
...
OEIS A268797 is ``Number of nX7 0..2 arrays with some element plus some horizontally, vertically or antidiagonally adjacent neighbor totalling two exactly once.''. It has offset $1$ and begins
\[
576, 4360, 41248, 561116, 7801688, 104971262, 1384570516, 17967375416,\ \dots
\]
The entry states:
\begin{quote}
Empirical recurrence of order 68 (see link above)
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 13 2016 13:45:04, revision 4)
...
merging $S'=719$ states remain, and it is that bound every computation below uses.

\begin{lemma}\label{lem:cf}
$a$ satisfies a monic linear recurrence with integer coefficients of order at most $S'=719$.
\end{lemma}
...
Applied to $2S'=1438$ exact terms from $n=4$ on, it returns order $68$ --- the
...
\begin{theorem}
The recurrence above holds for every $n\ge72$, and it is the recurrence OEIS A268797 records.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268797.
