...
\title{A proof of the conjectured recurrence for OEIS A143017}
...
\begin{abstract}
OEIS A143017 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A143017 is ``Number of \{2-1-3, 2'\^{}e-31\}-avoiding permutations of size n (see definition in the Elizalde paper)''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 4, 9, 22, 56, 147, 396,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} -8*n*(n+1)*a(n) + 4*n*(2*n+5)*a(n-1) + 4*n*(n+7)*a(n-2) + 2*(70*n\^{}2-395*n+564)*a(n-3) + 2*(25*n\^{}2-143*n+222)*a(n-4) + 4*(49*n-228)*(n-5)*a(n-5) - 45*(n-5)*(n-6)*a(n-6) = 0. - \emph{R. J. Mathar}, Mar 14 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2026-03-25, revision 33) it is still
...
\begin{quote}\small
Recurrence (of order 4): 4*n*(n+1)*(91*n\^{}2 - 217*n + 102)*a(n) = 6*n*(182*n\^{}3 - 525*n\^{}2 + 365*n - 78)*a(n-1) - 4*(91*n\^{}4 - 399*n\^{}3 - 136*n\^{}2 + 990*n - 450)*a(n-2) + 12*(n-3)*(182*n\^{}3 - 525*n\^{}2 + 92*n + 140)*a(n-3) - 5*(n-4)*(n-3)*(91*n\^{}2 - 35*n - 24)*a(n-4). - \emph{Vaclav Kotesovec}, Mar 20 2014
\end{quote}
...
\section{The computation for A143017}

Reading both relations as operators and dividing the conjectured one by the stated one
leaves remainder $0$, with quotient
\[
Q \;=\; \left(- \frac{9}{91 n^{2} + 693 n + 1292}\right) + \left(\frac{- 364 n^{2} - 3136 n - 6456}{8281 n^{4} + 142688 n^{3} + 912863 n^{2} + 2569168 n + 2682192}\right)\,N + \left(- \frac{2}{91 n^{2} + 875 n + 2076}\right)\,N^{2} .
\]
...
\[
- 8 n \left(n + 1\right)\,a(n) + 4 n \left(2 n + 5\right)\,a(n-1) + 4 n \left(n + 7\right)\,a(n-2) + 2 \left(70 n^{2} - 395 n + 564\right)\,a(n-3) + 2 \left(25 n^{2} - 143 n + 222\right)\,a(n-4) + 4 \left(n - 5\right) \left(49 n - 228\right)\,a(n-5) -  45 \left(n - 6\right) \left(n - 5\right)\,a(n-6) \;=\;0
\]
...
entry's own values and found to vanish, for all 21 values of $n$ from
$n=7$ upward.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A143017.
