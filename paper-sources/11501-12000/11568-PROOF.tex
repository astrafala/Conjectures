...
\title{A proof of the conjectured recurrence for OEIS A219314}
...
\begin{abstract}
OEIS A219314 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{3} - 4 x^{2} + x$, a polynomial of degree $3$, which proves
...
OEIS A219314 is ``Composition of the inverse binomial transform of Fibonacci numbers and the Catalan transform of Fibonacci numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 0, 3, 3, 13, 26, 77,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x^{2} - x - \left(2 x + 1\right) \sqrt{- 3 x^{2} - 2 x + 1} + 1}{2 \left(x + 1\right) \left(4 x^{2} + 2 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) -2*n*a(n-1) +11*(-n+2)*a(n-2) +4*(2*n-5)*a(n-3) +8*(5*n-17)*a(n-4) +24*(n-4)*a(n-5)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2023-04-12, revision 20) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 n,\qquad p_{2}(n)=- 11 \left(n - 2\right),\qquad p_{3}(n)=4 \left(2 n - 5\right),\qquad p_{4}(n)=8 \left(5 n - 17\right),\qquad p_{5}(n)=24 \left(n - 4\right)
\]
...
\section{The computation for A219314}
...
\[
n\,a(n) -  2 n\,a(n-1) -  11 \left(n - 2\right)\,a(n-2) + 4 \left(2 n - 5\right)\,a(n-3) + 8 \left(5 n - 17\right)\,a(n-4) + 24 \left(n - 4\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A219314, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{3} - 4 x^{2} + x$ is an
...
\url{https://oeis.org}, 2026. Sequence A219314.
