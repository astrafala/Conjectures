...
\title{A proof of the conjectured recurrence for OEIS A176006}
...
\begin{abstract}
OEIS A176006 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A176006 is ``The number of branching configurations of RNA (see Sankoff, 1985) with n or fewer hairpins''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 4, 10, 32, 122, 516, 2322,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x + \sqrt{x^{2} - 6 x + 1} - 3}{2 \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture : n*a(n) +(9-7*n)*a(n-1) +(7*n-12)*a(n-2) +(3-n)*a(n-3)=0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-07-06, revision 41) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=9 - 7 n,\qquad p_{2}(n)=7 n - 12,\qquad p_{3}(n)=3 - n
\]
...
\section{The computation for A176006}
...
\[
n\,a(n) + (9 - 7 n)\,a(n-1) + (7 n - 12)\,a(n-2) + (3 - n)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A176006, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A176006.
