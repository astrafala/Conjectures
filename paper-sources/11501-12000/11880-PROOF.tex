...
\title{A proof of the conjectured recurrence for OEIS A135335}
...
\begin{abstract}
OEIS A135335 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $10 x^{4} - 15 x^{3} + 10 x^{2}$, a polynomial of degree $4$, which proves
...
OEIS A135335 is ``Number of Dyck paths of semilength n having no DDUU's starting at level 2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 5, 13, 36, 106, 327,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x \sqrt{1 - 4 x} + 3 x + 2 \sqrt{1 - 4 x} - 2}{x^{2} \sqrt{1 - 4 x} - 3 x^{2} - 2 x \sqrt{1 - 4 x} + 2 x + \sqrt{1 - 4 x} - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(-7*n+6)*a(n-1) +2*(8*n-9)*a(n-2) +6*(-3*n+4)*a(n-3) +3*(3*n-4)*a(n-4) +2*(-2*n+3)*a(n-5)=0. - \emph{R. J. Mathar}, Jun 17 2016
\end{quote}
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=6 - 7 n,\qquad p_{2}(n)=2 \left(8 n - 9\right),\qquad p_{3}(n)=- 6 \left(3 n - 4\right),\qquad p_{4}(n)=3 \left(3 n - 4\right),\qquad p_{5}(n)=- 2 \left(2 n - 3\right)
\]
...
\section{The computation for A135335}
...
\[
n\,a(n) + (6 - 7 n)\,a(n-1) + 2 \left(8 n - 9\right)\,a(n-2) -  6 \left(3 n - 4\right)\,a(n-3) + 3 \left(3 n - 4\right)\,a(n-4) -  2 \left(2 n - 3\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A135335, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=10 x^{4} - 15 x^{3} + 10 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A135335.
