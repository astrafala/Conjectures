...
closed form. Here it equals $- 16 x^{5} + 48 x^{4} - 64 x^{3} + 48 x^{2} - 20 x + 4$, a polynomial of degree $5$, which proves
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x - 1\right) \left(\sqrt{\frac{- 4 x^{4} + \left(1 - x\right)^{4}}{\left(x - 1\right)^{4}}} - 1\right) \left(2 x^{2} - 2 x + 1\right)}{2 x^{4}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+4)*a(n) +(-7*n-18)*a(n-1) +2*(11*n+12)*a(n-2) +8*(-5*n+2)*a(n-3) +(41*n-76)*a(n-4) +(-19*n+62)*a(n-5) +4*(-n+6)*a(n-6) +6*(n-6)*a(n-7)=0. - \emph{R. J. Mathar}, Feb 05 2015
\end{quote}
...
\[
p_{0}(n)=n + 4,\qquad p_{1}(n)=- 7 n - 18,\qquad p_{2}(n)=2 \left(11 n + 12\right),\qquad p_{3}(n)=- 8 \left(5 n - 2\right),\qquad p_{4}(n)=41 n - 76,\qquad p_{5}(n)=62 - 19 n,\qquad p_{6}(n)=- 4 \left(n - 6\right),\qquad p_{7}(n)=6 \left(n - 6\right)
\]
...
\[
B(x)\;=\;- 16 x^{5} + 48 x^{4} - 64 x^{3} + 48 x^{2} - 20 x + 4 ,
\]
...
\[
(n + 4)\,a(n) + (- 7 n - 18)\,a(n-1) + 2 \left(11 n + 12\right)\,a(n-2) -  8 \left(5 n - 2\right)\,a(n-3) + (41 n - 76)\,a(n-4) + (62 - 19 n)\,a(n-5) -  4 \left(n - 6\right)\,a(n-6) + 6 \left(n - 6\right)\,a(n-7) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
cancellation in $K$, not by series truncation, so the identity $B=- 16 x^{5} + 48 x^{4} - 64 x^{3} + 48 x^{2} - 20 x + 4$ is an
