...
\title{A proof of the conjectured recurrence for OEIS A101596}
...
\begin{abstract}
OEIS A101596 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A101596 is ``Expansion of g.f. c(2*x)\^{}4, where c(x) is the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 8, 56, 384, 2640, 18304, 128128, 905216,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4 x \left(2 x + \sqrt{1 - 8 x} - 2\right) - \sqrt{1 - 8 x} + 1}{32 x^{4}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+4)*a(n) - 4*(3n+7)*a(n-1) + 16*(2n+1)*a(n-2) = 0. - \emph{R. J. Mathar}, Dec 13 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2026-01-25, revision 28) the
...
\[
p_{0}(n)=n + 4,\qquad p_{1}(n)=- 4 \left(3 n + 7\right),\qquad p_{2}(n)=16 \left(2 n + 1\right)
\]
...
\section{The computation for A101596}
...
\[
(n + 4)\,a(n) -  4 \left(3 n + 7\right)\,a(n-1) + 16 \left(2 n + 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A101596, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A101596.
