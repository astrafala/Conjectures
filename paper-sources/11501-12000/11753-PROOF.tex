...
\title{A proof of the conjectured recurrence for OEIS A191526}
...
\begin{abstract}
OEIS A191526 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $20 - 20 x$, a polynomial of degree $1$, which proves
...
OEIS A191526 is ``Number left factors of Dyck paths of length n and having no hills; a hill is a (1,1)-step starting at level 0 and followed by a (1,-1)-step''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 1, 2, 4, 7, 13, 24,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- \sqrt{1 - 4 x^{2}} \left(x + 1\right) + \left(x - 1\right) \left(2 x - 1\right)}{2 x \left(2 x - 1\right) \left(x^{2} + 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -2*(n+1)*(3*n-10)*a(n) +12*(n-5)*a(n-1) +(21*n\^{}2-97*n+122)*a(n-2) +6*(n-5)*a(n-3) +4*(n-2)*(3*n-7)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2024-10-20, revision 15) the
...
\[
p_{0}(n)=- 2 \left(n + 1\right) \left(3 n - 10\right),\qquad p_{1}(n)=12 \left(n - 5\right),\qquad p_{2}(n)=21 n^{2} - 97 n + 122,\qquad p_{3}(n)=6 \left(n - 5\right),\qquad p_{4}(n)=4 \left(n - 2\right) \left(3 n - 7\right)
\]
...
\section{The computation for A191526}
...
\[
- 2 \left(n + 1\right) \left(3 n - 10\right)\,a(n) + 12 \left(n - 5\right)\,a(n-1) + (21 n^{2} - 97 n + 122)\,a(n-2) + 6 \left(n - 5\right)\,a(n-3) + 4 \left(n - 2\right) \left(3 n - 7\right)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A191526, agreeing on all 13 terms compared.
...
vanish, for all 32 values of $n$ from $n=4$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=20 - 20 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A191526.
