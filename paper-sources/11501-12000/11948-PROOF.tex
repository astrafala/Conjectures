...
\title{A proof of the conjectured recurrence for OEIS A134389}
...
\begin{abstract}
OEIS A134389 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $5 x^{2} - 2 x - 1$, a polynomial of degree $2$, which proves
...
OEIS A134389 is ``A transform of floor((n+2)/2) with Hankel transform floor((n+2)/2)*(cos(Pi*n/2) + sin(Pi*n/2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 11, 24, 51, 108, 226,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x \sqrt{1 - 4 x^{2}} + \left(x - 2\right) \left(2 x - 1\right)}{2 \left(2 x - 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n-1)*a(n) - 2*n*a(n-1) + 4*(4-n)*a(n-2) + 8*(n-3)*a(n-3) = 0. - \emph{R. J. Mathar}, Oct 25 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2021-04-11, revision 13) the
...
\[
p_{0}(n)=n - 1,\qquad p_{1}(n)=- 2 n,\qquad p_{2}(n)=- 4 \left(n - 4\right),\qquad p_{3}(n)=8 \left(n - 3\right)
\]
...
\section{The computation for A134389}
...
\[
(n - 1)\,a(n) -  2 n\,a(n-1) -  4 \left(n - 4\right)\,a(n-2) + 8 \left(n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A134389, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=5 x^{2} - 2 x - 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A134389.
