...
\title{A proof of the conjectured recurrence for OEIS A226302}
...
\begin{abstract}
OEIS A226302 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A226302 is ``a(n) = P\_n(-1), where P\_n(x) is a certain polynomial arising in the enumeration of tatami mat coverings''. It has offset $2$ and begins
\[
a(2),\dots,a(9)\;=\;1, -1, 2, -4, 6, -14, 20, -48,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} (-n+2)*a(n) +(-n+2)*a(n-1) +2*(3*n-11)*a(n-2) +2*(3*n-14)*a(n-3) +4*(-2*n+9)*a(n-4) +8*(-n+6)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 06 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 30) it is still
...
\begin{quote}\small
Recurrence (for n>5): (n-5)*(n-2)*a(n) = -2*(n-4)*a(n-1) + 2*(n-5)*(3*n-10)*a(n-2) + 4*(n-4)*a(n-3) - 8*(n-5)*(n-4)*a(n-4). - \emph{Vaclav Kotesovec}, Jun 14 2015
\end{quote}
...
\section{The computation for A226302}
...
\[
(2 - n)\,a(n) + (2 - n)\,a(n-1) + 2 \left(3 n - 11\right)\,a(n-2) + 2 \left(3 n - 14\right)\,a(n-3) -  4 \left(2 n - 9\right)\,a(n-4) -  8 \left(n - 6\right)\,a(n-5) \;=\;0
\]
...
recurrence annihilates the sequence. The integer poles of the coefficients of $Q$ all lie below $n=7$, so the lemma applies throughout the stated range.
\end{proof}
...
entry's own values and found to vanish, for all 30 values of $n$ from
$n=7$ upward.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A226302.
