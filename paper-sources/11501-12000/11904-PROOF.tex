...
\title{The empirical recurrence for OEIS A235949, proved by transfer matrix}
...
\begin{abstract}
OEIS A235949 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=905$
...
OEIS A235949 is ``Number of (n+1) X (3+1) 0..2 arrays colored with the upper median value of each 2 X 2 subblock.''. It has offset $1$ and begins
\[
905, 1372, 1494, 1780, 2478, 3504, 4884, 6888,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-4) for n>13.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 20 2018 10:22:38, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The $2\times2$ subblocks of an $(n+1)\times4$ array form an
$n\times3$ grid. Colour each of them by $\sigma=c$, where $a\le b\le c\le d$ are its four entries sorted --- its minimum, lower median, upper median and maximum. ``Coloured with'' is the entry's way of
...
\]
over the $S=905$ pairs whose own subblock row is properly coloured.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235949.
