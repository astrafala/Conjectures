...
\title{A proof of the conjectured recurrence for OEIS A174195}
...
\begin{abstract}
OEIS A174195 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $57576 x^{3} + 13998 x^{2} + 168734 x + 166200$, a polynomial of degree $3$, which proves
...
OEIS A174195 is ``Number of permutations of 1..n that almost avoid 231''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 6, 24, 111, 531, 2519,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{24 x^{4} - 45 x^{3} + 6 x^{2} + 5 x + \left(1 - 4 x\right)^{\frac{3}{2}} \left(x^{3} - 4 x^{2} + x + 1\right) - 1}{2 x^{2} \left(1 - 4 x\right)^{\frac{3}{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n+2)*(2013*n\^{}2-10435*n+41550)*a(n) +(4026*n\^{}3 -128025*n\^{}2 +34955*n+59010)*a(n-1) -2 *(2*n-5)*(20130*n\^{}2 -110855*n +93819) *a(n-2)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 19) the
...
\[
p_{0}(n)=2 \left(n + 2\right) \left(2013 n^{2} - 10435 n + 41550\right),\qquad p_{1}(n)=4026 n^{3} - 128025 n^{2} + 34955 n + 59010,\qquad p_{2}(n)=- 2 \left(2 n - 5\right) \left(20130 n^{2} - 110855 n + 93819\right)
\]
...
\section{The computation for A174195}
...
\[
B(x)\;=\;57576 x^{3} + 13998 x^{2} + 168734 x + 166200 ,
\]
...
\[
2 \left(n + 2\right) \left(2013 n^{2} - 10435 n + 41550\right)\,a(n) + (4026 n^{3} - 128025 n^{2} + 34955 n + 59010)\,a(n-1) -  2 \left(2 n - 5\right) \left(20130 n^{2} - 110855 n + 93819\right)\,a(n-2) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A174195, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=57576 x^{3} + 13998 x^{2} + 168734 x + 166200$ is an
...
\url{https://oeis.org}, 2026. Sequence A174195.
