...
\title{The empirical recurrence for OEIS A252832, proved by transfer matrix}
...
\begin{abstract}
OEIS A252832 carries an empirical recurrence of order $3$ contributed by R. H. Hardin. It is
true, and it is decidable rather than empirical. The entry's condition is local to a bounded window of consecutive lines, so
the admissible configurations of such a window are the vertices of a finite digraph and the
arrays counted are exactly the walks in it; hence $a(n)$ is a walk count on $S=146$ vertices
...
\begin{quote}
Empirical: a(n) = 480*n\^{}2 - 3394*n + 6449 for n>5. a(n) = 3*a(n-1) - 3*a(n-2) + a(n-3) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 06 2018 16:36:54, revision 7) this is still
...
admissible array is then exactly a walk, so with $M$ the adjacency matrix on $S=146$ vertices
