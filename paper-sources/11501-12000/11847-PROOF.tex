...
\begin{quote}\small
Conjecture D-finite with recurrence a(n) -3*a(n-1) +(-2*n\textasciicircum{}2+8*n-3)*a(n-2) +(n-2)*(4*n- 15)*a(n-3) +(n-3)*(n\textasciicircum{}3-9*n\textasciicircum{}2+22*n-7)*a(n-4) -(n-3)*(n-4)*(n-5)\textasciicircum{}2*a(n-5)=0.
\end{quote}
...
vanish, for all 13 values of $n$ from $n=8$ upward. This is what
