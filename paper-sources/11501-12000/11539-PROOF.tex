...
\title{A proof of the conjectured recurrence for OEIS A190166}
...
\begin{abstract}
OEIS A190166 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A190166 is ``Number of (1,0)-steps at levels 0,2,4,... in all peakless Motzkin paths of length n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 2, 3, 6, 14, 34, 83,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x}{\sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)} \left(x^{2} - x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+1)*a(n) +(3*n-4)*a(n-1) +2*(-n+1)*a(n-2) +3*(n-2)*a(n-3) +2*(-n+3)*a(n-4) +(3*n-8)*a(n-5) +(-n+3)*a(n-6)=0. - \emph{R. J. Mathar}, Apr 09 2019
\end{quote}
As of the ``Last modified'' line on the live entry (2022-05-29, revision 11) the
...
\[
p_{0}(n)=1 - n,\qquad p_{1}(n)=3 n - 4,\qquad p_{2}(n)=- 2 \left(n - 1\right),\qquad p_{3}(n)=3 \left(n - 2\right),\qquad p_{4}(n)=- 2 \left(n - 3\right),\qquad p_{5}(n)=3 n - 8,\qquad p_{6}(n)=3 - n
\]
...
\section{The computation for A190166}
...
\[
(1 - n)\,a(n) + (3 n - 4)\,a(n-1) -  2 \left(n - 1\right)\,a(n-2) + 3 \left(n - 2\right)\,a(n-3) -  2 \left(n - 3\right)\,a(n-4) + (3 n - 8)\,a(n-5) + (3 - n)\,a(n-6) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A190166, agreeing on all 13 terms compared.
...
vanish, for all 28 values of $n$ from $n=6$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A190166.
