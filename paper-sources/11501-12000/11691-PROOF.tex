...
\title{The empirical recurrence for OEIS A223291, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A223291 counts arrays over $\{0,\dots,8\}$ in which every cell is constrained by the
...
OEIS A223291 is ``4-loop graph coloring a rectangular array: number of n X 2 0..8 arrays where 0..8 label nodes of a graph with edges 0,1 1,2 2,0 0,3 3,4 4,0 0,5 5,6 6,0 0,7 7,8 8,0 and every array movement to a horizontal or vertical neighbor moves along an edge of this graph.''. It has offset $1$ and begins
\[
24, 168, 1368, 11304, 93528, 773928, 6404184, 52994088,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 9*a(n-1) - 6*a(n-2).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 19 2018 07:36:20, revision 8) the statement is
...
Let $V=\{0,\dots,8\}^{2}$ be the possible rows and take
...
The size of $\mathcal V$ is worth seeing concretely. There are $8+1=9$ values
and $2$ cells in a row, so $|V|=9^{2}=81$, and the vertex set is
...
$9$ values, so there are about $10^{36}$ of them to sift. The
...
of the $(8+1)^{2}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223291.
