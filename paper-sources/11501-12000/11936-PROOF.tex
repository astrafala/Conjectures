...
closed form. Here it equals $- 27 x^{3} + 2 x$, a polynomial of degree $3$, which proves
...
\begin{quote}\small
\textbf{Conjecture:} +4*n*a(n) +2*(-23*n+22)*a(n-1) +156*(n-2)*a(n-2) +9*(-7*n+38)*a(n-3) +162*(-2*n+5)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
...
\[
p_{0}(n)=4 n,\qquad p_{1}(n)=- 2 \left(23 n - 22\right),\qquad p_{2}(n)=156 \left(n - 2\right),\qquad p_{3}(n)=- 9 \left(7 n - 38\right),\qquad p_{4}(n)=- 162 \left(2 n - 5\right)
\]
...
\[
4 n\,a(n) -  2 \left(23 n - 22\right)\,a(n-1) + 156 \left(n - 2\right)\,a(n-2) -  9 \left(7 n - 38\right)\,a(n-3) -  162 \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
cancellation in $K$, not by series truncation, so the identity $B=- 27 x^{3} + 2 x$ is an
