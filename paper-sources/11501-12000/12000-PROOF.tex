...
\title{The empirical recurrence for OEIS A304421, proved by transfer matrix}
...
\begin{abstract}
OEIS A304421 counts $2$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{1,2,3,4,5\}$, and it carries an empirical
recurrence of order $3$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A304421 is ``Number of nX2 0..1 arrays with every element unequal to 1, 2, 3, 4 or 5 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 7, 25, 98, 383, 1493, 5824, 22717,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) +3*a(n-2) +2*a(n-3)
\end{quote}
As of the ``Last modified'' line on the live entry (May 12 2018 09:34:44, revision
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-3}$ whenever $n-3\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-3$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-3}$ out of
...
\begin{theorem}
The recurrence \eqref{eq:conj} holds for every $n>3$. This is the statement of
Section~1.
\end{theorem}

\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $3$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=3+1$ onwards, and the run was continued
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A304421.
