...
\title{The empirical recurrence for OEIS A234220, proved by transfer matrix}
...
\begin{abstract}
OEIS A234220 counts $2$-column arrays over $\{0,\dots,4\}$ in which every
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=25$ vertices, which
...
OEIS A234220 is ``Number of (n+1) X (1+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 3 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
104, 436, 1824, 7696, 32384, 137536, 582144,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) + 20*a(n-2) - 80*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 21:34:28, revision 12) the statement is
...
Let $V$ consist of all of $\{0,\dots,4\}^{2}$, and put $S=|V|=25$. For $r,s\in V$ declare $r\to s$ an edge
...
$j=0,1,\dots$. They vanish from $j=2-3+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>2$.
\end{proof}
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=25$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A234220.
