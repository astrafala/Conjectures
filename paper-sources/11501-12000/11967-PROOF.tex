...
\title{The empirical recurrence for OEIS A237633, proved by transfer matrix}
...
\begin{abstract}
OEIS A237633 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=1786$
...
OEIS A237633 is ``Number of (n+1)X(4+1) 0..2 arrays with the upper median of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$ and begins
\[
1786, 2356, 1488, 2012, 2816, 4440, 5976, 10104,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-4) for n>14
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:19:47, revision 6) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The $2\times2$ subblocks of an $(n+1)\times5$ array form an
$n\times4$ grid, and each carries the value $\sigma=c$, where $a\le b\le c\le d$ are its four entries sorted --- its minimum, lower median, upper median and maximum. The entry asks that the values of
...
differ by $1$. The entry's first term $1786$ counts those, and the model was made to
...
\]
over the $S=1786$ pairs whose own subblock row already satisfies the condition along itself.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237633.
