...
\title{A proof of the conjectured recurrence for OEIS A159772}
...
\begin{abstract}
OEIS A159772 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A159772 is ``Number of n-leaf binary trees that do not contain (()((((()())())())())) as a subtree''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 2, 5, 14, 41, 124, 384,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} 2*(n-1)*(2*n-3)*a(n) +(-43*n\^{}2+172*n-177)*a(n-1) +4*(44*n\^{}2-266*n+411)*a(n-2) +8*(-43*n\^{}2+358*n-741)*a(n-3) +96*(3*n\^{}2-29*n+69)*a(n-4) -128*(n-4)*(n-6)*a(n-5) +512*(n-6)*(n-7)*a(n-6)=0. - \emph{R. J. Mathar}, May 30 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 51) it is still
...
\section{The computation for A159772}

Reading both relations as operators and dividing the conjectured one by the stated one
leaves remainder $0$, with quotient
\[
Q \;=\; \left(\frac{8}{13 n^{2} + 55 n + 54}\right) + \left(\frac{- 52 n^{2} - 272 n - 348}{169 n^{4} + 1768 n^{3} + 6743 n^{2} + 11084 n + 6588}\right)\,N + \left(\frac{1}{13 n^{2} + 81 n + 122}\right)\,N^{2} .
\]
...
\[
2 \left(n - 1\right) \left(2 n - 3\right)\,a(n) + (- 43 n^{2} + 172 n - 177)\,a(n-1) + 4 \left(44 n^{2} - 266 n + 411\right)\,a(n-2) -  8 \left(43 n^{2} - 358 n + 741\right)\,a(n-3) + 96 \left(3 n^{2} - 29 n + 69\right)\,a(n-4) -  128 \left(n - 6\right) \left(n - 4\right)\,a(n-5) + 512 \left(n - 7\right) \left(n - 6\right)\,a(n-6) \;=\;0
\]
...
\url{https://oeis.org}, 2026. Sequence A159772.
