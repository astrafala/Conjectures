...
\title{A proof of the conjectured recurrence for OEIS A182401}
...
\begin{abstract}
OEIS A182401 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A182401 is ``Number of paths from (0,0) to (n,0), never going below the x-axis, using steps U=(1,1), H=(1,0) and D=(1,-1), where the H steps come in five colors''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 5, 26, 140, 777, 4425, 25755, 152675,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 5 x - \sqrt{21 x^{2} - 10 x + 1} + 1}{2 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +5*(-2*n-1)*a(n-1) +21*(n-1)*a(n-2)=0. - \emph{R. J. Mathar}, Jul 24 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-22, revision 64) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 5 \left(2 n + 1\right),\qquad p_{2}(n)=21 \left(n - 1\right)
\]
...
\section{The computation for A182401}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(3 x - 1\right) \left(7 x - 1\right) .
\]
...
\[
(n + 2)\,a(n) -  5 \left(2 n + 1\right)\,a(n-1) + 21 \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A182401, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A182401.
