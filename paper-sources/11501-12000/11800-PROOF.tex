...
\title{A proof of the conjectured recurrence for OEIS A151483}
...
\begin{abstract}
OEIS A151483 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3$, a polynomial of degree $0$, which proves
...
OEIS A151483 is ``Number of walks within N\^{}2 (the first quadrant of Z\^{}2) starting at (0,0), ending on the vertical axis and consisting of n steps taken from \{(-1, 0), (-1, 1), (0, -1), (0, 1), (1, -1), (1, 0)\}''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 12, 48, 192, 832, 3712,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 4 x^{2} + 8 x + \left(1 - 6 x\right) \sqrt{- 12 x^{2} - 4 x + 1} - 1}{32 x^{3}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(n+3)*a(n) -4*n*a(n-1) +12*(-n+1)*a(n-2)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 23) the
...
\[
p_{0}(n)=n + 3,\qquad p_{1}(n)=- 4 n,\qquad p_{2}(n)=- 12 \left(n - 1\right)
\]
...
\section{The computation for A151483}
...
\[
(n + 3)\,a(n) -  4 n\,a(n-1) -  12 \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A151483, agreeing on all 13 terms compared.
...
vanish, for all 26 values of $n$ from $n=2$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A151483.
