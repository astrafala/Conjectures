...
\title{A proof of the conjectured recurrence for OEIS A025258}
...
\begin{abstract}
OEIS A025258 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{5} + x^{4} - 6 x^{3} + 3 x^{2} - x$, a polynomial of degree $5$, which proves
...
OEIS A025258 is ``a(n) = a(1)*a(n-1) + a(2)*a(n-2) + ... + a(n-3)*a(3) for n >= 4, with initial terms 1, -1, 1, 1''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, -1, 1, 1, 0, 0, 2, 3,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{2 x \left(x^{2} - x + 1\right)}{- x^{2} + x + \sqrt{x^{4} - 6 x^{3} + 3 x^{2} - 2 x + 1} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -n*a(n) +(2*n-3)*a(n-1) +3*(-n+3)*a(n-2) +3*(2*n-9)*a(n-3) +(-n+6)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 25 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-12-16, revision 16) the
...
\[
p_{0}(n)=- n,\qquad p_{1}(n)=2 n - 3,\qquad p_{2}(n)=- 3 \left(n - 3\right),\qquad p_{3}(n)=3 \left(2 n - 9\right),\qquad p_{4}(n)=6 - n
\]
...
\[
- n\,a(n) + (2 n - 3)\,a(n-1) -  3 \left(n - 3\right)\,a(n-2) + 3 \left(2 n - 9\right)\,a(n-3) + (6 - n)\,a(n-4) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A025258, agreeing on all 13 terms compared.
...
vanish, for all 34 values of $n$ from $n=7$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{5} + x^{4} - 6 x^{3} + 3 x^{2} - x$ is an
...
\url{https://oeis.org}, 2026. Sequence A025258.
