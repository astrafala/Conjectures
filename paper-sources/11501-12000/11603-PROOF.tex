...
\title{A proof of the conjectured recurrence for OEIS A014533}
...
\begin{abstract}
OEIS A014533 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A014533 is ``Form array in which n-th row is obtained by expanding (1 + x + x\^{}2)\^{}n and taking the 4th column from the center''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 5, 21, 77, 266, 882, 2850, 9042,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{\left(x + \sqrt{- 3 x^{2} - 2 x + 1} - 1\right)^{4}}{16 x^{7} \sqrt{- 3 x^{2} - 2 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+7)*(n-1)*a(n) + (n+3)*(2*n+5)*a(n-1) + 3*(n+3)*(n+2)*a(n-2) = 0. - \emph{R. J. Mathar}, Feb 25 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-07-02, revision 38) the
...
\[
p_{0}(n)=- \left(n - 1\right) \left(n + 7\right),\qquad p_{1}(n)=\left(n + 3\right) \left(2 n + 5\right),\qquad p_{2}(n)=3 \left(n + 2\right) \left(n + 3\right)
\]
...
\section{The computation for A014533}
...
\[
- \left(n - 1\right) \left(n + 7\right)\,a(n) + \left(n + 3\right) \left(2 n + 5\right)\,a(n-1) + 3 \left(n + 2\right) \left(n + 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A014533, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A014533.
