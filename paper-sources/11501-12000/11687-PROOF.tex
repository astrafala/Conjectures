...
\title{A proof of the conjectured recurrence for OEIS A111053}
...
\begin{abstract}
OEIS A111053 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 60 x^{4} + 6 x^{3} - 26 x^{2} + 6 x$, a polynomial of degree $4$, which proves
...
OEIS A111053 is ``Number of permutations which avoid the patterns 1324 and (2143 with Bruhat restriction \{2<->3\}). Also the number of permutations whose graphs are acyclic''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 6, 22, 89, 379, 1661, 7405,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{\left(1 - 5 x\right) \sqrt{1 - 4 x} - \left(x - 1\right) \left(2 x^{2} + 4 x - 1\right)}{2 \left(x^{3} - 2 x^{2} + 5 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n+5)*a(n) +3*(20-13*n-3*n\^{}2)*a(n-1) +2*(11*n\^{}2+40*n-150)*a(n-2) +3*(40-11*n-3*n\^{}2)*a(n-3) +2*(n+6)*(2*n-5)*a(n-4)=0. - \emph{R. J. Mathar}, Aug 14 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-20, revision 47) the
...
\[
p_{0}(n)=n \left(n + 5\right),\qquad p_{1}(n)=- 3 \left(3 n^{2} + 13 n - 20\right),\qquad p_{2}(n)=2 \left(11 n^{2} + 40 n - 150\right),\qquad p_{3}(n)=- 3 \left(3 n^{2} + 11 n - 40\right),\qquad p_{4}(n)=2 \left(n + 6\right) \left(2 n - 5\right)
\]
...
\section{The computation for A111053}
...
\[
n \left(n + 5\right)\,a(n) -  3 \left(3 n^{2} + 13 n - 20\right)\,a(n-1) + 2 \left(11 n^{2} + 40 n - 150\right)\,a(n-2) -  3 \left(3 n^{2} + 11 n - 40\right)\,a(n-3) + 2 \left(n + 6\right) \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A111053, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 60 x^{4} + 6 x^{3} - 26 x^{2} + 6 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A111053.
