...
\title{The empirical recurrence for OEIS A229841, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A229841 counts arrays over $\{0,\dots,2\}$ in which every cell is constrained by the
...
OEIS A229841 is ``Number of n X 2 0..2 arrays x(i,j) with each element horizontally or vertically next to at least one element with value 2-x(i,j).''. It has offset $1$ and begins
\[
3, 15, 81, 423, 2247, 11925, 63291, 335943,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) + 2*a(n-2) - a(n-3) - 5*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 17 2018 05:45:33, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) + 2\,a(n-2) - a(n-3) - 5\,a(n-4)
\end{equation}
for all $n>5$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229841.
