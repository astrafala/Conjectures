...
closed form. Here it equals $40 x^{2} - 26 x + 4$, a polynomial of degree $2$, which proves
...
\begin{quote}\small
\textbf{Conjecture:} +4*(n+1)*a(n) +(-81*n+23)*a(n-1) +10*(51*n-70)*a(n-2) +500*(-2*n+5)*a(n-3)=0. - \emph{R. J. Mathar}, Feb 05 2015
\end{quote}
...
\[
p_{0}(n)=4 \left(n + 1\right),\qquad p_{1}(n)=23 - 81 n,\qquad p_{2}(n)=10 \left(51 n - 70\right),\qquad p_{3}(n)=- 500 \left(2 n - 5\right)
\]
...
\[
4 \left(n + 1\right)\,a(n) + (23 - 81 n)\,a(n-1) + 10 \left(51 n - 70\right)\,a(n-2) -  500 \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
cancellation in $K$, not by series truncation, so the identity $B=40 x^{2} - 26 x + 4$ is an
