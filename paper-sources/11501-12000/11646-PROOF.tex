...
\title{A proof of the conjectured recurrence for OEIS A132900}
...
\begin{abstract}
OEIS A132900 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A132900 is ``Colored Motzkin paths where each of the steps has three possible colors''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 18, 108, 729, 5103, 37179, 277749,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 3 x - \sqrt{- 27 x^{2} - 6 x + 1} + 1}{18 x^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) - 3*(2*n+1)*a(n-1) + 27*(1-n)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 14 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2024-02-05, revision 35) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 3 \left(2 n + 1\right),\qquad p_{2}(n)=- 27 \left(n - 1\right)
\]
...
\section{The computation for A132900}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \left(3 x + 1\right) \left(9 x - 1\right) .
\]
...
\[
(n + 2)\,a(n) -  3 \left(2 n + 1\right)\,a(n-1) -  27 \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A132900, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A132900.
