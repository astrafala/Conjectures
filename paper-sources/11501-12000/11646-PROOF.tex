...
\title{A proof of the conjectured recurrence for OEIS A177166}
...
\begin{abstract}
OEIS A177166 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $18 x^{3} - 34 x^{2} + 14 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A177166 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=9, k=0 and l=-1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 9, 17, 114, 533, 3406, 20281, 132987,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 36 x^{3} + 28 x^{2} + 5 x - 1}{x - 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2(1-3n)*a(n-1) +(51-23n)*a(n-2) +4*(16n-49)*a(n-3) +36*(4-n)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 27 2011
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=51 - 23 n,\qquad p_{3}(n)=4 \left(16 n - 49\right),\qquad p_{4}(n)=- 36 \left(n - 4\right)
\]
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + (51 - 23 n)\,a(n-2) + 4 \left(16 n - 49\right)\,a(n-3) -  36 \left(n - 4\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A177166, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=18 x^{3} - 34 x^{2} + 14 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A177166.
