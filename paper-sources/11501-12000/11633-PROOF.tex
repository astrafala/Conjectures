...
\title{The empirical closed form for OEIS A219846, proved}
...
\begin{abstract}
OEIS A219846 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=8$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219846 is ``Number of n X 2 arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 n X 2 array.''. It has offset $1$ and begins
\[
3, 7, 16, 33, 62, 108, 177, 276,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/24)*n\^{}4 + (1/12)*n\^{}3 + (23/24)*n\^{}2 - (1/12)*n + 2.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 28 2018 10:43:39, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=8$
...
states number $S=8$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219846 records is correct for all $n>0$.
\end{theorem}
...
$1 + 41 - 1$, which is more than the $5$ consecutive agreements a
...
First, the model reproduces all $41$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219846.
