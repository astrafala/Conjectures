...
\title{A proof of the conjectured recurrence for OEIS A133307}
...
\begin{abstract}
OEIS A133307 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A133307 is ``a(n) = (1/n)*Sum\_\{i=0..n-1\} C(n,i)*C(n,i+1)*6\^{}i*7\^{}(n-i), a(0)=1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 7, 91, 1477, 26845, 522739, 10663471, 224939113,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x - \sqrt{x^{2} - 26 x + 1} + 1}{12 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 13*(-2*n+1)*a(n-1) + (n-2)*a(n-2) = 0. - \emph{R. J. Mathar}, May 23 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-19, revision 33) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 13 \left(2 n - 1\right),\qquad p_{2}(n)=n - 2
\]
...
\section{The computation for A133307}
...
\[
(n + 1)\,a(n) -  13 \left(2 n - 1\right)\,a(n-1) + (n - 2)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A133307, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A133307.
