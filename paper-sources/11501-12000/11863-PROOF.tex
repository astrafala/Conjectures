...
\title{A proof of the conjectured recurrence for OEIS A243764}
...
\begin{abstract}
OEIS A243764 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $24576 x^{6} - 49152 x^{5} + 16224 x^{4} - 1260 x^{3}$, a polynomial of degree $6$, which proves
the recurrence for every $n>6$.
\end{abstract}
...
OEIS A243764 is ``Expansion of -((2*sqrt(1-4*x)-2)*x)/(sqrt(8*x+4*sqrt(1-4*x)-3)-1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;-1, 1, 3, 4, 13, 36, 122, 408,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x \left(1 - \sqrt{1 - 4 x}\right)}{\sqrt{8 x + 4 \sqrt{1 - 4 x} - 3} - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 245*n*(n-1)*(n-2)*a(n) -140*(n-1)*(n-2)*(28*n-81)*a(n-1) +4*(n-2)*(5812*n\^{}2-41488*n+72261)*a(n-2) +16*(-3368*n\^{}3+45144*n\^{}2-191968*n+260859)*a(n-3) +128*(-152*n\^{}3-768*n\^{}2+16748*n-45483)*a(n-4) +2048*(2*n-11)*(74*n\^{}2-715*n+1629)*a(n-5) -98304*(n-7)*(2*n-11)*(2*n-13)*a(n-6)=0. - \emph{R. J. Mathar}, Jun 07 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2024-11-09, revision 28) the
...
\[
p_{0}(n)=245 n \left(n - 2\right) \left(n - 1\right),\qquad p_{1}(n)=- 140 \left(n - 2\right) \left(n - 1\right) \left(28 n - 81\right),\qquad p_{2}(n)=4 \left(n - 2\right) \left(5812 n^{2} - 41488 n + 72261\right),\qquad p_{3}(n)=- 16 \left(3368 n^{3} - 45144 n^{2} + 191968 n - 260859\right),\qquad p_{4}(n)=- 128 \left(152 n^{3} + 768 n^{2} - 16748 n + 45483\right),\qquad p_{5}(n)=2048 \left(2 n - 11\right) \left(74 n^{2} - 715 n + 1629\right),\qquad p_{6}(n)=- 98304 \left(n - 7\right) \left(2 n - 13\right) \left(2 n - 11\right)
\]
...
\[
B(x)\;=\;24576 x^{6} - 49152 x^{5} + 16224 x^{4} - 1260 x^{3} ,
\]
...
\[
245 n \left(n - 2\right) \left(n - 1\right)\,a(n) -  140 \left(n - 2\right) \left(n - 1\right) \left(28 n - 81\right)\,a(n-1) + 4 \left(n - 2\right) \left(5812 n^{2} - 41488 n + 72261\right)\,a(n-2) -  16 \left(3368 n^{3} - 45144 n^{2} + 191968 n - 260859\right)\,a(n-3) -  128 \left(152 n^{3} + 768 n^{2} - 16748 n + 45483\right)\,a(n-4) + 2048 \left(2 n - 11\right) \left(74 n^{2} - 715 n + 1629\right)\,a(n-5) -  98304 \left(n - 7\right) \left(2 n - 13\right) \left(2 n - 11\right)\,a(n-6) \;=\;0
\]
holds for every $n>6$.
\end{theorem}
...
$n\le6$ some of the shifted terms $a(n-i)$ fall outside the range of the sequence,
...
section of A243764, agreeing on all 13 terms compared.
...
vanish, for all 21 values of $n$ from $n=7$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=24576 x^{6} - 49152 x^{5} + 16224 x^{4} - 1260 x^{3}$ is an
...
\url{https://oeis.org}, 2026. Sequence A243764.
