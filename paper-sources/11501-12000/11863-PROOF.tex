...
\title{The empirical recurrence for OEIS A206342, proved by transfer matrix}
...
\begin{abstract}
OEIS A206342 counts the $(n+1)\times8$ arrays over $\{0,\dots,3\}$ in which
neighbouring $2\times2$ subblocks, each of nonzero determinant, commute as matrices, and carries an empirical
recurrence
of order $3$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than
empirical. The condition ties together $2+1$ consecutive rows, so $2$ consecutive rows are
a state, the count is a walk count on $S=4111$ of them, and the conjectured recurrence is then
...
OEIS A206342 is ``Number of (n+1)X8 0..3 arrays with every 2X2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
4111, 2042, 12005, 6078, 36015, 18258, 108141, 54762,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) +3*a(n-2) -3*a(n-3) for n>16
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:52:38, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The contiguous $2\times2$ subblocks of an $(n+1)\times8$ array form a grid of $n$
rows and $7$ columns. Two subblocks that are neighbours in that
...
$AB=BA$ as matrices over $\mathbf{Z}$. The entry adds that every subblock have nonzero determinant; that is a condition on one subblock alone and is checked where the subblock is formed.

At $n=1$ the array has $2$ rows and the grid is a single row of $7$
subblocks, with only the horizontal pairs to test; the entry's first term is
$4111$, which the model reproduces along with every later term.
...
\]
on the $S=4111$ admissible $2$-tuples. In particular $a$ is $C$-finite.

This state does not compress. What a step needs of the past is the previous subblock row, and
for $8\ge2$ that row already determines the $2$ array rows it was built from, so nothing
is lost by carrying the rows and nothing would be gained by carrying the matrices instead. The
size of the construction is therefore $4^{2\cdot8}$ before the condition prunes
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206342.
