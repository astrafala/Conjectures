...
\title{A proof of the conjectured recurrence for OEIS A116391}
...
\begin{abstract}
OEIS A116391 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A116391 is ``Expansion of 1/((1+x)*(sqrt(1-4*x\^{}2)-x))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 3, 2, 11, 14, 47, 78,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{1}{\left(x + 1\right) \left(x - \sqrt{1 - 4 x^{2}}\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: n*a(n) +(n)*a(n-1) +3*(-3*n+4)*a(n-2) +3*(-3*n+4)*a(n-3) +20*(n-3)*a(n-4) +20*(n-3)*a(n-5)=0. - \emph{R. J. Mathar}, Jan 23 2020
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 16) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=n,\qquad p_{2}(n)=- 3 \left(3 n - 4\right),\qquad p_{3}(n)=- 3 \left(3 n - 4\right),\qquad p_{4}(n)=20 \left(n - 3\right),\qquad p_{5}(n)=20 \left(n - 3\right)
\]
...
\section{The computation for A116391}
...
\[
n\,a(n) + n\,a(n-1) -  3 \left(3 n - 4\right)\,a(n-2) -  3 \left(3 n - 4\right)\,a(n-3) + 20 \left(n - 3\right)\,a(n-4) + 20 \left(n - 3\right)\,a(n-5) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A116391, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A116391.
