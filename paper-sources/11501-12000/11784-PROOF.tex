...
\title{The empirical recurrence for OEIS A229406, proved by transfer matrix}
...
\begin{abstract}
OEIS A229406 counts arrays of width $2$ over $\{0,1,2,3\}$ containing no occurrence of certain short
...
rather than empirical: no pattern is longer than $2$ letters and no direction moves the row
index by more than one, so every occurrence lies inside $2$ consecutive array rows and the
count is a walk count on $S=16$ states.
\end{abstract}
...
OEIS A229406 is ``Number of n X 2 0..3 arrays avoiding 11 horizontally, 22 vertically, 33 antidiagonally and 00 diagonally.''. It has offset $1$ and begins
\[
15, 163, 1756, 18712, 199595, 2127718, 22684613, 241841143,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 10*a(n-1) + 13*a(n-2) - 67*a(n-3) + 38*a(n-4) - 2*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 15 2018 05:30:04, revision 8) this is still
...
\text{along a row, left to right} & \mathtt{11} \\
\text{down a column} & \mathtt{22} \\
\text{down an antidiagonal, ne to sw} & \mathtt{33} \\
\text{down a diagonal, nw to se} & \mathtt{00} \\
\end{array}
\]


\section{Every occurrence lies in $2$ consecutive rows}

\begin{lemma}\label{lem:win}
Each of the directions above changes the row index by $0$ or $1$ from one cell of a run to the
next, and no pattern is longer than $2$ letters. Hence every occurrence lies inside $2$
...
So the state is the window of the last $1$ array rows, a row still to come being written
...
\begin{theorem}
\[
a(n)\;=\;10\,a(n-1) + 13\,a(n-2) - 67\,a(n-3) + 38\,a(n-4) - 2\,a(n-5)
\]
for every $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229406.
