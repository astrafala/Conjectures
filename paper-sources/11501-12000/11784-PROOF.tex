...
\title{A proof of the conjectured recurrence for OEIS A098519}
...
\begin{abstract}
OEIS A098519 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A098519 is ``E.g.f. exp(x)*BesselI(1,2*sqrt(3)*x)/sqrt(3)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 2, 12, 40, 185, 726, 3157,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{2 x}{11 x^{2} + 2 x + \left(x - 1\right) \sqrt{- 11 x^{2} - 2 x + 1} - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n-1)*(n+1)*a(n) - n*(2*n-1)*a(n-1) - 11*n*(n-1)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 23 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-02, revision 22) the
...
\[
p_{0}(n)=\left(n - 1\right) \left(n + 1\right),\qquad p_{1}(n)=- n \left(2 n - 1\right),\qquad p_{2}(n)=- 11 n \left(n - 1\right)
\]
...
\section{The computation for A098519}
...
\[
\left(n - 1\right) \left(n + 1\right)\,a(n) -  n \left(2 n - 1\right)\,a(n-1) -  11 n \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A098519, agreeing on all 13 terms compared.
...
vanish, for all 25 values of $n$ from $n=2$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A098519.
