...
\title{A proof of the conjectured recurrence for OEIS A128652}
...
\begin{abstract}
OEIS A128652 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $20 x^{3} - 40 x^{2} + 14 x$, a polynomial of degree $3$, which proves
...
OEIS A128652 is ``Number of square permutations of length n''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 6, 24, 104, 464, 2088, 9392,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;- \frac{x \left(4 x^{2} \sqrt{1 - 4 x} - 10 x^{2} + 6 x - 1\right)}{\left(4 x - 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(n-3)*(n-8)*a(n) +2*(-4*n\^{}2+43*n-96)*a(n-1) +8*(2*n-7)*(n-7)*a(n-2)=0. - \emph{R. J. Mathar}, Oct 16 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 35) the
...
\[
p_{0}(n)=\left(n - 8\right) \left(n - 3\right),\qquad p_{1}(n)=- 2 \left(4 n^{2} - 43 n + 96\right),\qquad p_{2}(n)=8 \left(n - 7\right) \left(2 n - 7\right)
\]
...
\section{The computation for A128652}
...
\[
\left(n - 8\right) \left(n - 3\right)\,a(n) -  2 \left(4 n^{2} - 43 n + 96\right)\,a(n-1) + 8 \left(n - 7\right) \left(2 n - 7\right)\,a(n-2) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A128652, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=20 x^{3} - 40 x^{2} + 14 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A128652.
