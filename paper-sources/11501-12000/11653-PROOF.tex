...
\title{A proof of the conjectured recurrence for OEIS A228770}
...
\begin{abstract}
OEIS A228770 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $62284518 x^{5} - 133858314 x^{4} + 117291960 x^{3} - 39977022 x^{2} + 4869274 x$, a polynomial of degree $5$, which proves
the recurrence for every $n>5$.
\end{abstract}
...
OEIS A228770 is ``The number of sum indecomposable permutations which avoid the patterns 3124 and 4312''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 3, 12, 51, 217, 912, 3785,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;- \frac{x \left(24 x^{5} - 71 x^{4} + 84 x^{3} - 45 x^{2} + 11 x + \sqrt{1 - 4 x} \left(4 x^{5} - 25 x^{4} + 40 x^{3} - 29 x^{2} + 9 x - 1\right) - 1\right)}{8 x^{6} - 54 x^{5} + 117 x^{4} - 114 x^{3} + 54 x^{2} - 12 x - \sqrt{1 - 4 x} \left(12 x^{5} - 43 x^{4} + 58 x^{3} - 36 x^{2} + 10 x - 1\right) + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+1)*(39961*n-2474598)*a(n) +(-39961*n\^{}2-25975201*n+4949196) *a(n-1) +3*(1460811*n\^{}2+27429105*n-41310802) *a(n-2) +3 *(-8653921*n\^{}2-4750029*n+74360724) *a(n-3) +4*(15005713*n\^{}2-82481258*n+83094771) *a(n-4) +12*(-4937548*n\^{}2+40726604*n-73155719) *a(n-5) +16*(652718*n-2110173)*(2*n-13) *a(n-6)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 15) the
...
\[
p_{0}(n)=- \left(n + 1\right) \left(39961 n - 2474598\right),\qquad p_{1}(n)=- 39961 n^{2} - 25975201 n + 4949196,\qquad p_{2}(n)=3 \left(1460811 n^{2} + 27429105 n - 41310802\right),\qquad p_{3}(n)=- 3 \left(8653921 n^{2} + 4750029 n - 74360724\right),\qquad p_{4}(n)=4 \left(15005713 n^{2} - 82481258 n + 83094771\right),\qquad p_{5}(n)=- 12 \left(4937548 n^{2} - 40726604 n + 73155719\right),\qquad p_{6}(n)=16 \left(2 n - 13\right) \left(652718 n - 2110173\right)
\]
...
\section{The computation for A228770}
...
\[
B(x)\;=\;62284518 x^{5} - 133858314 x^{4} + 117291960 x^{3} - 39977022 x^{2} + 4869274 x ,
\]
...
\[
- \left(n + 1\right) \left(39961 n - 2474598\right)\,a(n) + (- 39961 n^{2} - 25975201 n + 4949196)\,a(n-1) + 3 \left(1460811 n^{2} + 27429105 n - 41310802\right)\,a(n-2) -  3 \left(8653921 n^{2} + 4750029 n - 74360724\right)\,a(n-3) + 4 \left(15005713 n^{2} - 82481258 n + 83094771\right)\,a(n-4) -  12 \left(4937548 n^{2} - 40726604 n + 73155719\right)\,a(n-5) + 16 \left(2 n - 13\right) \left(652718 n - 2110173\right)\,a(n-6) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
$n\le5$ some of the shifted terms $a(n-i)$ fall outside the range of the sequence,
...
section of A228770, agreeing on all 13 terms compared.
...
vanish, for all 24 values of $n$ from $n=7$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=62284518 x^{5} - 133858314 x^{4} + 117291960 x^{3} - 39977022 x^{2} + 4869274 x$ is an
identity of functions and the theorem holds for all $n>5$ simultaneously.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A228770.
