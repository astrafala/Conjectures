...
\title{A proof of the conjectured recurrence for OEIS A050168}
...
\begin{abstract}
OEIS A050168 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A050168 is ``a(0) = 1; for n > 0, a(n) = binomial(n, floor(n/2)) + binomial(n-1, floor(n/2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 3, 5, 9, 16, 30, 55,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(x + 1\right) \left(\sqrt{\frac{- 2 x - 1}{2 x - 1}} - 1\right)}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + (n-3)*a(n-1) + 2*(-2*n+1)*a(n-2) + 4*(-n+3)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 26 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 52) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=n - 3,\qquad p_{2}(n)=- 2 \left(2 n - 1\right),\qquad p_{3}(n)=- 4 \left(n - 3\right)
\]
...
\section{The computation for A050168}
...
\[
(n + 1)\,a(n) + (n - 3)\,a(n-1) -  2 \left(2 n - 1\right)\,a(n-2) -  4 \left(n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A050168, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A050168.
