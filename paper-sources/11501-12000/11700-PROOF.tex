...
\title{A proof of the conjectured recurrence for OEIS A143954}
...
\begin{abstract}
OEIS A143954 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{2}$, a polynomial of degree $2$, which proves
...
OEIS A143954 is ``Number of peaks in the peak plateaux of all Dyck paths of semilength n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 5, 19, 68, 243, 880,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{x}{2 \left(x - 1\right)^{2}} + \frac{x}{2 \sqrt{1 - 4 x} \left(x - 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+1)*a(n) +2*(3*n-4)*a(n-1) +(-9*n+13)*a(n-2) +2*(2*n-3)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 16 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-23, revision 24) the
...
\[
p_{0}(n)=1 - n,\qquad p_{1}(n)=2 \left(3 n - 4\right),\qquad p_{2}(n)=13 - 9 n,\qquad p_{3}(n)=2 \left(2 n - 3\right)
\]
...
\section{The computation for A143954}
...
\[
(1 - n)\,a(n) + 2 \left(3 n - 4\right)\,a(n-1) + (13 - 9 n)\,a(n-2) + 2 \left(2 n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A143954, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A143954.
