...
\title{A proof of the conjectured recurrence for OEIS A135925}
...
\begin{abstract}
OEIS A135925 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 8628263825301312 x^{13} - 56627815207112768 x^{12} - 82029355854002112 x^{11} + 107522236010828544 x^{10} + 267411638401515712 x^{9} + 26860851665524032 x^{8} - 96203187527862528 x^{7} + 17483921103890240 x^{6}$, a polynomial of degree $13$, which proves
the recurrence for every $n>13$.
\end{abstract}
...
OEIS A135925 is ``G.f.: (x\^{}2+x-1)*(1+2*x+S)*(1-5*x+2*x\^{}2+(3*x-1)*S)\^{}2*(-1+3*x+2*x\^{}2+(1-x)*S), where S=sqrt(1-4*x)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 0, 0, 0, 0, -16, 0,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\left(2 x + \sqrt{1 - 4 x} + 1\right) \left(x^{2} + x - 1\right) \left(2 x^{2} - 5 x + \sqrt{1 - 4 x} \left(3 x - 1\right) + 1\right)^{2} \left(2 x^{2} + 3 x + \sqrt{1 - 4 x} \left(1 - x\right) - 1\right) ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: +4*(n-1)*(7678757997599*n-100709801435251)*a(n) +(-291262905844111*n\^{}2+4745710679447853*n-12935393149282124)*a(n-1) +3*(268523622217993*n\^{}2-4978734571371597*n+20251709258114720)*a(n-2) +2*(-196528865827703*n\^{}2+4147266222561252*n-21888137659762186)*a(n-3) -12*(22463548140013*n-247087251730058)*(2*n-25)*a(n-4)=0. - \emph{R. J. Mathar}, Jan 23 2020
\end{quote}
As of the ``Last modified'' line on the live entry (2020-01-30, revision 13) the
...
\[
p_{0}(n)=4 \left(n - 1\right) \left(7678757997599 n - 100709801435251\right),\qquad p_{1}(n)=- 291262905844111 n^{2} + 4745710679447853 n - 12935393149282124,\qquad p_{2}(n)=3 \left(268523622217993 n^{2} - 4978734571371597 n + 20251709258114720\right),\qquad p_{3}(n)=- 2 \left(196528865827703 n^{2} - 4147266222561252 n + 21888137659762186\right),\qquad p_{4}(n)=- 12 \left(2 n - 25\right) \left(22463548140013 n - 247087251730058\right)
\]
...
\section{The computation for A135925}
...
\[
B(x)\;=\;- 8628263825301312 x^{13} - 56627815207112768 x^{12} - 82029355854002112 x^{11} + 107522236010828544 x^{10} + 267411638401515712 x^{9} + 26860851665524032 x^{8} - 96203187527862528 x^{7} + 17483921103890240 x^{6} ,
\]
a polynomial of degree $13$.

\begin{theorem}
The conjectured recurrence
\[
4 \left(n - 1\right) \left(7678757997599 n - 100709801435251\right)\,a(n) + (- 291262905844111 n^{2} + 4745710679447853 n - 12935393149282124)\,a(n-1) + 3 \left(268523622217993 n^{2} - 4978734571371597 n + 20251709258114720\right)\,a(n-2) -  2 \left(196528865827703 n^{2} - 4147266222561252 n + 21888137659762186\right)\,a(n-3) -  12 \left(2 n - 25\right) \left(22463548140013 n - 247087251730058\right)\,a(n-4) \;=\;0
\]
...
$n\le13$ some of the shifted terms $a(n-i)$ fall outside the range of the sequence,
...
section of A135925, agreeing on all 13 terms compared.
...
vanish, for all 17 values of $n$ from $n=14$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 8628263825301312 x^{13} - 56627815207112768 x^{12} - 82029355854002112 x^{11} + 107522236010828544 x^{10} + 267411638401515712 x^{9} + 26860851665524032 x^{8} - 96203187527862528 x^{7} + 17483921103890240 x^{6}$ is an
identity of functions and the theorem holds for all $n>13$ simultaneously.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A135925.
