...
\title{The empirical recurrence for OEIS A222460, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A222460 counts arrays over $\{0,\dots,7\}$ in which every cell is constrained by the
...
OEIS A222460 is ``Number of n X 4 0..7 arrays with no entry increasing mod 8 by 7 rightwards or downwards, starting with upper left zero.''. It has offset $1$ and begins
\[
343, 556549, 903055069, 1465295106499, 2377584520856755, 3857863258420747009, 6259760185235726701945, 10157072698503130798653535,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 1689*a(n-1) - 108775*a(n-2) + 1672631*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 15 2018 10:12:17, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;1689\,a(n-1) - 108775\,a(n-2) + 1672631\,a(n-3)
\end{equation}
for all $n>3$.
...
row $t$ and column $u$, each in $\{0,\dots,7\}$. With the neighbour offsets
\[
\mathcal N=\{(0,1),\ (1,0)\},
\]
...
x_{t,u+1}\ne x_{t,u}+7\ (\mathrm{mod}\ 8)\ \text{ and }\ x_{t+1,u}\ne x_{t,u}+7\ (\mathrm{mod}\ 8)
\end{equation}
...
Let $V=\{0,\dots,7\}^{4}$ be the possible rows and take
...
The size of $\mathcal V$ is worth seeing concretely. There are $7+1=8$ values
and $4$ cells in a row, so $|V|=8^{4}=4096$, and the vertex set is
...
publishes, $n=10$, the arrays have $4\cdot10$ cells over
$8$ values, so there are about $10^{36}$ of them to sift. The
...
of the $(7+1)^{4}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222460.
