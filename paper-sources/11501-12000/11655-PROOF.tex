...
\title{A proof of the conjectured recurrence for OEIS A114589}
...
\begin{abstract}
OEIS A114589 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $6 x + 8$, a polynomial of degree $1$, which proves
...
OEIS A114589 is ``Number of hill-free Dyck paths of semilength n+3 and having no peaks at even levels (a hill in a Dyck path is a peak at level 1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 3, 7, 17, 43, 110, 286,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x^{3} - 2 x^{2} - x - \sqrt{- 3 x^{2} - 2 x + 1} + 1}{2 x^{4} \left(x^{2} + 2 x + 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n+4)*a(n) +2*(-n-1)*a(n-1) +3*(-3*n-4)*a(n-2) +(-8*n-11)*a(n-3) +3*(-n-1)*a(n-4)=0. - \emph{R. J. Mathar}, Jul 02 2018
\end{quote}
As of the ``Last modified'' line on the live entry (2024-09-09, revision 19) the
...
\[
p_{0}(n)=2 \left(n + 4\right),\qquad p_{1}(n)=- 2 \left(n + 1\right),\qquad p_{2}(n)=- 3 \left(3 n + 4\right),\qquad p_{3}(n)=- 8 n - 11,\qquad p_{4}(n)=- 3 \left(n + 1\right)
\]
...
\section{The computation for A114589}
...
\[
2 \left(n + 4\right)\,a(n) -  2 \left(n + 1\right)\,a(n-1) -  3 \left(3 n + 4\right)\,a(n-2) + (- 8 n - 11)\,a(n-3) -  3 \left(n + 1\right)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A114589, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=6 x + 8$ is an
...
\url{https://oeis.org}, 2026. Sequence A114589.
