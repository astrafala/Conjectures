...
\title{A proof of the conjectured recurrence for OEIS A073155}
...
\begin{abstract}
OEIS A073155 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x + 1$, a polynomial of degree $1$, which proves
...
OEIS A073155 is ``Leftmost column sequence of triangle A073153''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 14, 56, 237, 1046, 4762,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{- 4 x \left(x + 1\right)^{2} + 1}}{2 x \left(x + 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + (-3*n+4)*a(n-1) + 2*(-6*n+7)*a(n-2) + 2*(-6*n+11)*a(n-3) + 2*(-2*n+5)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 26 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 29) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=4 - 3 n,\qquad p_{2}(n)=- 2 \left(6 n - 7\right),\qquad p_{3}(n)=- 2 \left(6 n - 11\right),\qquad p_{4}(n)=- 2 \left(2 n - 5\right)
\]
...
\section{The computation for A073155}
...
\[
(n + 1)\,a(n) + (4 - 3 n)\,a(n-1) -  2 \left(6 n - 7\right)\,a(n-2) -  2 \left(6 n - 11\right)\,a(n-3) -  2 \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A073155, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=3 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A073155.
