...
\title{A proof of the conjectured recurrence for OEIS A025256}
...
\begin{abstract}
OEIS A025256 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 3 x^{5} + x^{4} + 6 x^{3} + x^{2} + x$, a polynomial of degree $5$, which proves
...
OEIS A025256 is ``a(n) = a(1)*a(n-1) + a(2)*a(n-2) + ... + a(n-3)*a(3) for n >= 4, with initial terms 1, 1, 3, 3''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 3, 3, 6, 18, 42, 105,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{x^{2}}{2} + \frac{x}{2} - \frac{\sqrt{x^{4} - 10 x^{3} - x^{2} - 2 x + 1}}{2} + \frac{1}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(-2*n+3)*a(n-1) +(-n+3)*a(n-2) +5*(-2*n+9)*a(n-3) +(n-6)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 25 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-16, revision 13) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=3 - 2 n,\qquad p_{2}(n)=3 - n,\qquad p_{3}(n)=- 5 \left(2 n - 9\right),\qquad p_{4}(n)=n - 6
\]
...
\section{The computation for A025256}
...
\[
n\,a(n) + (3 - 2 n)\,a(n-1) + (3 - n)\,a(n-2) -  5 \left(2 n - 9\right)\,a(n-3) + (n - 6)\,a(n-4) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A025256, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 3 x^{5} + x^{4} + 6 x^{3} + x^{2} + x$ is an
...
\url{https://oeis.org}, 2026. Sequence A025256.
