...
\title{A proof of the conjectured recurrence for OEIS A236407}
...
\begin{abstract}
OEIS A236407 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 6 x^{3} + 16 x^{2} - 6 x$, a polynomial of degree $3$, which proves
...
OEIS A236407 is ``a(n) = 2*Sum\_\{k=0..n-1\} C(n-1,k)*C(n+k,k) + n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 3, 10, 41, 196, 1007, 5342, 28821,\ \dots
\]
The entry posts no generating function of its own. Its NAME states the
relation \texttt{a(n) = A002003(n) + n.} to another entry, and that entry's exponential generating
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x \sqrt{x^{2} - 6 x + 1} + \frac{\left(x - \sqrt{x^{2} - 6 x + 1} + 1\right) \left(x^{2} - 2 x + 1\right)}{2}}{\sqrt{x^{2} - 6 x + 1} \left(x^{2} - 2 x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-3)*a(n) -4*(2*n-1)*(n-3)*a(n-1) +2*(7*n\^{}2-28*n+20)*a(n-2) -4*(n-1)*(2*n-7)*a(n-3) +(n-1)*(n-4)*a(n-4)=0. - \emph{R. J. Mathar}, Feb 01 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 18) the
...
\[
p_{0}(n)=n \left(n - 3\right),\qquad p_{1}(n)=- 4 \left(n - 3\right) \left(2 n - 1\right),\qquad p_{2}(n)=2 \left(7 n^{2} - 28 n + 20\right),\qquad p_{3}(n)=- 4 \left(n - 1\right) \left(2 n - 7\right),\qquad p_{4}(n)=\left(n - 4\right) \left(n - 1\right)
\]
...
\section{The computation for A236407}
...
\[
n \left(n - 3\right)\,a(n) -  4 \left(n - 3\right) \left(2 n - 1\right)\,a(n-1) + 2 \left(7 n^{2} - 28 n + 20\right)\,a(n-2) -  4 \left(n - 1\right) \left(2 n - 7\right)\,a(n-3) + \left(n - 4\right) \left(n - 1\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A236407, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 6 x^{3} + 16 x^{2} - 6 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A236407.
