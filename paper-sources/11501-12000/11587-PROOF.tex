...
\title{The empirical closed form for OEIS A220045, proved}
...
\begin{abstract}
OEIS A220045 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=31$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220045 is ``Number of 2 X n arrays of the minimum value of corresponding elements and their horizontal, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 2 X n array.''. It has offset $1$ and begins
\[
10, 13, 25, 50, 90, 152, 249, 397,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/120)*n\^{}5 - (1/24)*n\^{}4 + (5/24)*n\^{}3 + (109/24)*n\^{}2 - (1063/60)*n + 39 for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 08:23:27, revision 7)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=31$
...
states number $S=31$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220045 records is correct for all $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220045.
