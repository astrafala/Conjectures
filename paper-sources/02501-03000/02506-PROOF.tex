...
\title{A value repeated at its own distance: OEIS A209369}
...
\begin{abstract}
OEIS A209369 carries an empirical recurrence of order $9$ contributed by R.~H.~Hardin. It is
...
walks in it. Hence $a(n)$ is a walk count on $S=112$ vertices and is $C$-finite, and whether the
...
OEIS A209369 is ``Number of n X 3 1..3 arrays with no element with value z exactly a city block distance of z from another element with value z.''. It has offset $1$ and begins
\[
19, 84, 115, 242, 690, 1922, 5460, 15488,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) +5*a(n-3) +2*a(n-4) +6*a(n-5) +5*a(n-6) -2*a(n-8) -a(n-9) for n>12.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 22 2025 21:42:30, revision 10) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $3$ columns over the alphabet $\{1,2,3\}$, distance measured as $|\Delta \text{row}|+|\Delta \text{column}|$, and asks that no element of value $z$ may have another element of value $z$ exactly $z$ steps away. An element of value $0$ is $0$ steps from itself and so always has such a partner. That is not an interpretation imposed on the entry: it is the reading the entry's own published terms select, and reading ``another'' as strictly some other cell disagrees with them at the very first term.
...
Merging vertices with identical futures leaves $S=112$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which states may begin and end an array,
\[
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 5\,a(n-3) + 2\,a(n-4) + 6\,a(n-5) + 5\,a(n-6) - 2\,a(n-8) - a(n-9)
\]
for every $n>12$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A209369.
