...
\title{A further conjecture on OEIS A203979,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A203979 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $96, 864, 7776, 69984, 629856, 5668704, 51018336, 459165024,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=9\,a(n-1) ,
\]
a linear recurrence of order $1$ with characteristic polynomial
\[
  q(x)=x - 9 .
\]
...
with at least $1+2$ further terms after that index.

\section{The further conjecture}

Separately from the conjecture already settled, the entry records

\begin{quote}\raggedright Empirical: a(n) = 96*9\^{}(n-1)\end{quote}
...
\[
  p(x)=x - 9 ,
\]
of order $D=1$. Since $q\mid p$ and $a$ satisfies the recurrence of $q$, $a$ satisfies that of
...
indices, and $a$ and $f$ were checked to agree at $n=2,\dots,2$,
