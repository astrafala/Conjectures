...
\title{A further conjecture on OEIS A243515,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A243515 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $3, 5, 10, 25, 77, 279, 1134, 4979,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=12\,a(n-1) - 52\,a(n-2) + 102\,a(n-3) - 91\,a(n-4) + 30\,a(n-5) ,
\]
a linear recurrence of order $5$ with characteristic polynomial
\[
  q(x)=x^{5} - 12 x^{4} + 52 x^{3} - 102 x^{2} + 91 x - 30 .
\]
...
\begin{quote}\raggedright a(n) = (147 + 2\^{}(5+n) + 4*3\^{}(1+n) + 5\^{}n + 36*n) / 96.\end{quote}
...
\[
  p(x)=x^{5} - 12 x^{4} + 52 x^{3} - 102 x^{2} + 91 x - 30 ,
\]
