...
\title{A further conjecture on OEIS A268768,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A268768 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $3, 12, 32, 100, 248, 620, 1456, 3380,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=2\,a(n-1) + 3\,a(n-2) - 4\,a(n-3) - 4\,a(n-4) ,
\]
a linear recurrence of order $4$ with characteristic polynomial
\[
  q(x)=x^{4} - 2 x^{3} - 3 x^{2} + 4 x + 4 .
\]
...
\begin{quote}\raggedright a(n) = (4/27)*(7*((-1)\^{}n-2\^{}n) + 3*((-1)\^{}n + 2\^{}(2+n))*n) for n>1.\end{quote}
