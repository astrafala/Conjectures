...
\title{The empirical recurrence for OEIS A263285, proved by transfer matrix}
...
\begin{abstract}
OEIS A263285 counts arrays of width $5$ over $\{0,1\}$ whose rows, read as base-$2$
numbers, are divisible by $11$ and whose columns, read the same way, are not divisible by
...
OEIS A263285 is ``Number of (n+3)X(2+3) 0..1 arrays with each row divisible by 11 and column not divisible by 11, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
42, 156, 482, 1512, 4610, 14140, 42420, 129444,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 13 2015 12:20:39, revision 5) this is still
...
$11$ and every $\gamma_j$ be not divisible by $11$.
...
which every entry is not divisible by $11$ --- so the end vector is not the all-ones
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263285.
