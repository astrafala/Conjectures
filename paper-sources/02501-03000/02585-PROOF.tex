...
\title{A further conjecture on OEIS A266981,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A266981 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 2, 2, 5, 3, 8, 4, 11,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=2\,a(n-2) - 1\,a(n-4) ,
\]
...
\begin{quote}\raggedright a(n) = (3+(-1)\^{}n-2*(-2+(-1)\^{}n)*n)/4.\end{quote}
