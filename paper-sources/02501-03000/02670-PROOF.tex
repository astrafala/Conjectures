...
\title{The empirical column recurrences for the table OEIS A235437}
...
\begin{abstract}
OEIS A235437 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A235437 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with the difference between each 2X2 subblock maximum and minimum lexicographically nondecreasing rowwise and columnwise.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 583, 583, 3987, 10241, 3987, 26091, 169041,\ \dots
\]
...
k=2: [order 39]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:33:44, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..2 arrays with the difference between each 2X2 subblock maximum and minimum lexicographically nondecreasing rowwise and columnwise.''. Its rows are the vertices of a digraph on
$S=29$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;8\,a(n-1) - a(n-2) - 64\,a(n-3) - 14\,a(n-4) + 88\,a(n-5) + 8\,a(n-6) - 24\,a(n-7)
\]
...
model reproduces $10$ published values, $61$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235437.
