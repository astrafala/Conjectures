...
\title{A further conjecture on OEIS A222892,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A222892 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $3, 34, 991, 84521, 11047723, 1639681049, 251412693667, 38863548783281,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=211\,a(n-1) - 9385\,a(n-2) + 110913\,a(n-3) - 253890\,a(n-4) ,
\]
a linear recurrence of order $4$ with characteristic polynomial
\[
  q(x)=x^{4} - 211 x^{3} + 9385 x^{2} - 110913 x + 253890 .
\]
...
\begin{quote}\raggedright a(n) = 3\^{}n + 5*2\^{}(n-1)*7\^{}(n-2) + 14*5\^{}(n-3)*31\^{}(n-2) + 10*39\^{}(n-2) for n>2.\end{quote}
...
\[
  p(x)=x^{4} - 211 x^{3} + 9385 x^{2} - 110913 x + 253890 ,
\]
