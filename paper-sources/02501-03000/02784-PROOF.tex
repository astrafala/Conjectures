...
\title{The empirical column recurrences for the table OEIS A252254}
...
\begin{abstract}
OEIS A252254 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=3, 4$.
\end{abstract}
...
OEIS A252254 is ``T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and column sum not equal to 0 3 5 6 or 7 and every 3X3 diagonal and antidiagonal sum equal to 0 3 5 6 or 7.''. It has offset $1$, is
read by antidiagonals, and begins
\[
318, 260, 260, 278, 239, 278, 352, 242,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: [linear recurrence of order 18] for n>25\\
k=2: [order 12] for n>16\\
k=3: a(n) = a(n-1) +3*a(n-3) -3*a(n-4) -a(n-6) +a(n-7) for n>9\\
k=4: a(n) = a(n-1) +3*a(n-3) -3*a(n-4) -a(n-6) +a(n-7) for n>9\\
k=5: a(n) = a(n-1) +3*a(n-3) -3*a(n-4) -a(n-6) +a(n-7) for n>9\\
k=6: a(n) = a(n-1) +3*a(n-3) -3*a(n-4) -a(n-6) +a(n-7) for n>9\\
k=7: a(n) = a(n-1) +3*a(n-3) -3*a(n-4) -a(n-6) +a(n-7) for n>9\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:58:52, revision 6) these are still
...
column $k$$=3$ & $7$ & $15861$ & $>9$ & $10$ \\
column $k$$=4$ & $7$ & $134934$ & $>9$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X(5) 0..3 arrays with every 3X3 subblock row and column sum not equal to 0 3 5 6 or 7 and every 3X3 diagonal and antidiagonal sum equal to 0 3 5 6 or 7.''. Its rows are the vertices of a digraph on
$S=15861$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+2)X(6) 0..3 arrays with every 3X3 subblock row and column sum not equal to 0 3 5 6 or 7 and every 3X3 diagonal and antidiagonal sum equal to 0 3 5 6 or 7.''. Its rows are the vertices of a digraph on
$S=134934$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + 3\,a(n-3) - 3\,a(n-4) - a(n-6) + a(n-7)
...
model reproduces $20$ published values, $60$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252254.
