...
\title{A further conjecture on OEIS A251130,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A251130 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $40, 133, 369, 919, 2129, 4699, 10033, 20947,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=5\,a(n-1) - 8\,a(n-2) + 2\,a(n-3) + 7\,a(n-4) - 7\,a(n-5) + 2\,a(n-6) ,
\]
a linear recurrence of order $6$ with characteristic polynomial
\[
  q(x)=x^{6} - 5 x^{5} + 8 x^{4} - 2 x^{3} - 7 x^{2} + 7 x - 2 .
\]
...
\begin{quote}\raggedright a(n) = (-957 + (-1)\^{}n + 529*2\^{}(1+n) - 540*n - 126*n\^{}2 - 12*n\^{}3) / 12.\end{quote}
...
\[
  p(x)=x^{6} - 5 x^{5} + 8 x^{4} - 2 x^{3} - 7 x^{2} + 7 x - 2 ,
\]
