...
\title{A further conjecture on OEIS A214142,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A214142 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 1, 4, 11, 40, 147, 568, 2227,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=6\,a(n-1) - 7\,a(n-2) - 6\,a(n-3) + 8\,a(n-4) ,
\]
a linear recurrence of order $4$ with characteristic polynomial
\[
  q(x)=x^{4} - 6 x^{3} + 7 x^{2} + 6 x - 8 .
\]
...
\begin{quote}\raggedright Empirical: a(n) = 2\^{}n/6 - 11*(-1)\^{}n/30 + 4\^{}n/30 + 1/6. - \_R. J. Mathar\_, Jul 07 2012\end{quote}
