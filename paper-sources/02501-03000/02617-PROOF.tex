...
\title{The empirical column recurrences for the table OEIS A237384}
...
\begin{abstract}
OEIS A237384 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A237384 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with the maximum plus the minimum minus the upper median minus the lower median of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 344, 344, 1472, 2064, 1472, 6368, 12408,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 336*a(n-4) for n>5\\
k=2: a(n) = 1320*a(n-4) for n>6\\
k=3: a(n) = 10380*a(n-4) -26935616*a(n-8) for n>11\\
k=4: [order 16] for n>20\\
k=5: [order 36] for n>41\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:08:25, revision 6) these are still
...
column $k$$=1$ & $4$ & $81$ & $>5$ & $8$ \\
column $k$$=2$ & $4$ & $344$ & $>6$ & $7$ \\
column $k$$=3$ & $8$ & $1472$ & $>11$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with the maximum plus the minimum minus the upper median minus the lower median of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays with the maximum plus the minimum minus the upper median minus the lower median of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
$S=344$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;1320\,a(n-4)
...
The sequence is ``Number of (n+1)X(4) 0..2 arrays with the maximum plus the minimum minus the upper median minus the lower median of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
$S=1472$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;10380\,a(n-4) - 26935616\,a(n-8)
\]
...
model reproduces $25$ published values, $153$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237384.
