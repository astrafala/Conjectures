...
\title{The empirical recurrence for OEIS A262844, proved by transfer matrix}
...
\begin{abstract}
OEIS A262844 counts arrays of width $4$ over $\{0,1\}$ whose rows, read as base-$2$
numbers, are divisible by $7$ and whose columns, read the same way, are not divisible by
$7$. The entry carries an empirical recurrence of order $15$ contributed by
...
OEIS A262844 is ``Number of (n+2) X (2+2) 0..1 arrays with each row divisible by 7 and column not divisible by 7, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
6, 34, 132, 396, 1264, 3962, 11886, 35914,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 11 2024 09:44:46, revision 10) this is still
...
$7$ and every $\gamma_j$ be not divisible by $7$.
...
\gamma_j^{(i)}\bmod 7\;=\;
\bigl(2\,(\gamma_j^{(i-1)}\bmod 7)+x(i,j)\bigr)\bmod 7 .
\]
\end{lemma}
...
So the vector $\bigl(\gamma_1\bmod 7,\dots,\gamma_{4}\bmod
7\bigr)$ is a state: at most $7^{4}$ of them, and each array row
...
which every entry is not divisible by $7$ --- so the end vector is not the all-ones
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) - 4\,a(n-2) + 20\,a(n-3) - 68\,a(n-4) + 68\,a(n-5) - 162\,a(n-6) + 444\,a(n-7) - 444\,a(n-8) + 644\,a(n-9) - 1244\,a(n-10) + 1244\,a(n-11) - 1277\,a(n-12) + 1376\,a(n-13) - 1376\,a(n-14) + 1032\,a(n-15)
\]
for every $n>12$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262844.
