...
\title{The empirical recurrence for OEIS A224571, proved by transfer matrix}
...
\begin{abstract}
OEIS A224571 counts the $(n+5)\times7$ matrices over $\{0,\dots,1\}$ all of whose
$6\times6$ contiguous subblocks are idempotent, and carries an empirical recurrence of order
$24$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical.
The condition ties together $6$ consecutive rows and $6$ consecutive columns at once, so a
single row is not a state; the strip of the last $5$ rows is, and the admissible strips
are read off the overlap graph of the admissible windows instead of being enumerated. That
makes $a(n)$ a walk count on $S=21915$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224571 is ``Number of (n+5)X7 0..1 matrices with each 6X6 subblock idempotent.''. It has offset $1$ and begins
\[
18044, 5668, 5696, 6411, 7034, 7386, 7843, 9237,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:28:26, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $6\times6$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $6\times7$ matrix a \emph{band} when every one of its $2$ contiguous
$6\times6$ subblocks is a window. What the entry counts is the matrices of $7$ columns
whose every $6$ consecutive rows form a band.

Take as states the $5\times7$ strips that occur as the top $5$ rows of a band,
together with those that occur as its bottom $5$ rows, and put an edge from $u$ to $v$
when the band whose first $5$ rows are $u$ and whose last $5$ rows are $v$ exists.
A band is determined by that pair, so the edge is unique when it exists. Reading a counted
matrix from the top, its rows $1..5$, then $2..6$, and so on, form a walk, and every
walk arises from exactly one matrix; a matrix with $n+5$ rows gives a walk of $n$ edges.
Since the first $5$ rows of a counted matrix with at least $6$ rows are the top of a
band, and since no further condition falls on where a walk starts or ends,
\[
a(n)\;=\;\iota^{\!\top}M^{\,n}\tau ,\qquad \iota=\tau=(1,1,\dots,1)^{\!\top},
\]
with $M$ the adjacency matrix on $S=21915$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{35}$ strips of $5$ rows and $7$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $6\times5$ entries, so a band is exactly a walk of $2$ windows in
the graph that joins a window to those whose first $5$ columns are its last $5$
columns. Enumerating those walks produces the bands, and with them the states and the edges,
directly.

\section{The admissible windows}

The vertices of the overlap graph are the $0/1$ matrices $B$ of size $6\times6$ with
$B^2=B$. There are $2^{36}$ matrices to sift, which is already awkward at $6=6$ and
...
Generating from Lemma~\ref{lem:idem} gives $96608$ admissible $6\times6$ windows. For
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) - 2\,a(n-2) + 2\,a(n-3) - 2\,a(n-4) + 3\,a(n-5) - 2\,a(n-6) + a(n-7) - a(n-9) + a(n-10) - 3\,a(n-11) + 3\,a(n-12) - 4\,a(n-13) + 3\,a(n-14) - 2\,a(n-15) + 2\,a(n-16) - a(n-17) + a(n-18) + a(n-19) - a(n-20) + a(n-21) - a(n-22) + a(n-23) - a(n-24)
\]
for every $n>33$.
\end{theorem}
...
until the working vector was identically zero, which settles every larger $j$ at once. The bound is exact: at $n=33$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224571.
