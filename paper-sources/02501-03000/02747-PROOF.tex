...
\title{The empirical column recurrences for the table OEIS A230989}
...
\begin{abstract}
OEIS A230989 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A230989 is ``T(n,k)=Number of white square subarrays of (n+1)X(k+1) binary arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 1, 1, 2, 1, 2, 3, 4,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:10:09, revision 6) these are still
...
The sequence is ``Number of white square subarrays of (n+1)X2 binary arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of white square subarrays of (n+1)X3 binary arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with upper left element zero.''. Its rows are the vertices of a digraph on
$S=19$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $21$ published values, $34$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230989.
