...
\title{A further conjecture on OEIS A268053,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A268053 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 16, 422, 14554, 560778, 22501266, 915745002, 37450557314,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=62\,a(n-1) - 969\,a(n-2) + 4568\,a(n-3) - 5740\,a(n-4) ,
\]
a linear recurrence of order $4$ with characteristic polynomial
\[
  q(x)=x^{4} - 62 x^{3} + 969 x^{2} - 4568 x + 5740 .
\]
...
with at least $4+2$ further terms after that index.

\section{The further conjecture}

Separately from the conjecture already settled, the entry records

\begin{quote}\raggedright a(n) = (38745*2\^{}n + 127428*5\^{}n + 17425*2\^{}(2+n)*7\^{}n + 9800*41\^{}n) / 2092230.\end{quote}
...
\[
  p(x)=x^{4} - 62 x^{3} + 969 x^{2} - 4568 x + 5740 ,
\]
of order $D=4$. Since $q\mid p$ and $a$ satisfies the recurrence of $q$, $a$ satisfies that of
...
indices, and $a$ and $f$ were checked to agree at $n=5,\dots,8$,
