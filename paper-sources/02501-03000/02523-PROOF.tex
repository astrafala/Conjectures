...
\title{A further conjecture on OEIS A251134,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A251134 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $2129, 4786, 8050, 14753, 25597, 46023, 82431, 151567,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=6\,a(n-1) - 13\,a(n-2) + 10\,a(n-3) + 5\,a(n-4) - 14\,a(n-5) + 9\,a(n-6) - 2\,a(n-7) ,
\]
a linear recurrence of order $7$ with characteristic polynomial
\[
  q(x)=x^{7} - 6 x^{6} + 13 x^{5} - 10 x^{4} - 5 x^{3} + 14 x^{2} - 9 x + 2 .
\]
...
with at least $7+2$ further terms after that index.

\section{The further conjecture}

Separately from the conjecture already settled, the entry records

\begin{quote}\raggedright a(n) = (2*(5472+1058*(-1)\^{}n+2809*2\^{}n) + 6142*n + 1759*n\^{}2 + 266*n\^{}3 + 17*n\^{}4) / 12 for n>3.\end{quote}
...
\[
  p(x)=x^{7} - 6 x^{6} + 13 x^{5} - 10 x^{4} - 5 x^{3} + 14 x^{2} - 9 x + 2 ,
\]
of order $D=7$. Since $q\mid p$ and $a$ satisfies the recurrence of $q$, $a$ satisfies that of
...
indices, and $a$ and $f$ were checked to agree at $n=11,\dots,17$,
