...
\title{The empirical recurrence for OEIS A224408, proved by transfer matrix}
...
\begin{abstract}
OEIS A224408 counts arrays over $\{0,\dots,1\}$ whose rows, columns, diagonals or
...
is a walk count on $S=29$ vertices and the recurrence follows from a finite exact
computation.
\end{abstract}
...
OEIS A224408 is ``Number of n X 7 0..1 arrays with rows unimodal and antidiagonals nondecreasing.''. It has offset $1$ and begins
\[
29, 267, 1568, 7490, 32814, 139638, 590254, 2496332,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 8*a(n-1) - 20*a(n-2) + 18*a(n-3) - 4*a(n-4) + 2*a(n-5) + 8*a(n-6) + 32*a(n-7) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 30 2018 08:20:16, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;8\,a(n-1) - 20\,a(n-2) + 18\,a(n-3) - 4\,a(n-4) + 2\,a(n-5) + 8\,a(n-6) + 32\,a(n-7)
\end{equation}
for all $n>8$.
...
bit vector), $|\mathcal V|=S=29$; let $(r,f)\to(s,f')$ be an edge when placing $s$ after $r$
...
every one of the $20$ published indices, with no shift fitted. The values agree exactly.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224408.
