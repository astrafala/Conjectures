...
\title{The empirical recurrence for OEIS A224686, proved by transfer matrix}
...
\begin{abstract}
OEIS A224686 counts the $(n+4)\times8$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=3847$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224686 is ``Number of (n+4) X 8 0..1 matrices with each 5 X 5 subblock idempotent.''. It has offset $1$ and begins
\[
3465, 1720, 1896, 2166, 2429, 2646, 3214, 4122,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) - 2*a(n-2) - 2*a(n-3) + 3*a(n-4) - 2*a(n-6) + 2*a(n-8) - a(n-9) for n>13.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 03 2018 03:49:56, revision 7) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $5\times5$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $5\times8$ matrix a \emph{band} when every one of its $4$ contiguous
$5\times5$ subblocks is a window. What the entry counts is the matrices of $8$ columns
whose every $5$ consecutive rows form a band.

Take as states the $4\times8$ strips that occur as the top $4$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=3847$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{32}$ strips of $4$ rows and $8$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $5\times4$ entries, so a band is exactly a walk of $4$ windows in
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 2\,a(n-2) - 2\,a(n-3) + 3\,a(n-4) - 2\,a(n-6) + 2\,a(n-8) - a(n-9)
\]
for every $n>13$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224686.
