...
\title{A further conjecture on OEIS A266256,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A266256 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 1, 2, 5, 2, 9, 2, 13,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=2\,a(n-2) - 1\,a(n-4) ,
\]
...
\begin{quote}\raggedright a(n) = (-2*((-1)\^{}n-1)*n+3*(-1)\^{}n+1)/2 for n>0.\end{quote}
