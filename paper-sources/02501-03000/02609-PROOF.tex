...
\title{The empirical recurrence for OEIS A214234, proved by transfer matrix}
...
\begin{abstract}
OEIS A214234 counts the proper colourings of a grid $n\times5$, wrapped in the direction of width $5$, with at most $8$ colours and
...
down to $S=59$ and makes the exact residual test affordable.
\end{abstract}
...
OEIS A214234 is ``Number of 0..7 colorings of an nX5 array circular in the 5 direction with new values 0..7 introduced in row major order.''. It has offset $1$ and begins
\[
11, 7749, 35910351, 283323771619, 2440058302217191, 21284607690377676689, 186014154011114838330931, 1626099752980474200170964309,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 16:56:53, revision 8) this is still
...
A row of the array is a proper colouring of the cycle $C_{5}$, since the wrapping makes
its first and last cells adjacent, and two consecutive rows must differ in every one of the
$5$ positions. So $L_i(n)$ is the number of walks of $n-1$ steps in the digraph whose
vertices are the proper colourings of $C_{5}$ over $i$ colours and whose edges join
colourings differing everywhere:
\[
L_i(n)\;=\;\iota^{\!\top}M_i^{\,n-1}\iota ,\qquad \iota=(1,\dots,1)^{\!\top} .
\]
There are $(i-1)^{5}+(-1)^{5}(i-1)$ such colourings, which at $i=8$ is already more
...
state count is $S=59$ rather than the number of colourings.
...
\begin{theorem}
\[
a(n)\;=\;10629\,a(n-1) - 17350472\,a(n-2) + 7613269646\,a(n-3) - 1150563633112\,a(n-4) + 68845322541884\,a(n-5) - 1602676444724896\,a(n-6) + 14117645719614888\,a(n-7) - 45952444522339344\,a(n-8) + 33030412555840128\,a(n-9)
\]
for every $n>9$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A214234.
