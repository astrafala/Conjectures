...
\title{An exact count for OEIS A228463: windowed maxima over a growing alphabet}
...
\begin{abstract}
OEIS A228463 counts the arrays obtainable as the maxima of $3$ adjacent elements of a length
...
The entry's first terms are $27, 183, 736, 2227, 5615, 12453, 25096, 46941,\dots$ and it states

\begin{quote}\raggedright Empirical: a(n) = (2/45)*n\^{}6 + (8/15)*n\^{}5 + (115/36)*n\^{}4 + 8*n\^{}3 + (1667/180)*n\^{}2 + (149/30)*n + 1.\end{quote}

\section{The object}

Fix $k=8$ and $w=3$, and put $L=k-w+1=6$. For $a=(a_1,\dots,a_k)\in\{0,\dots,n\}^k$
let
\[
...
\[
  \lvert\Phi(\{0,\dots,n\}^k)\rvert=\sum_{m=1}^{6}A_m\binom{n+1}{m}
  \qquad\text{for every } n\ge 0 .
\]
Enumerating the weak orderings of $6$ elements and applying Lemma~\ref{lem:greedy} to each
gives $A_{1}=1, A_{2}=25, A_{3}=105, A_{4}=162, A_{5}=112, A_{6}=32$, so
\[
  \lvert\Phi(\{0,\dots,n\}^k)\rvert=1\binom{n+1}{1} + 25\binom{n+1}{2} + 105\binom{n+1}{3} + 162\binom{n+1}{4} + 112\binom{n+1}{5} + 32\binom{n+1}{6}=\frac{2 n^{6}}{45} + \frac{8 n^{5}}{15} + \frac{115 n^{4}}{36} + 8 n^{3} + \frac{1667 n^{2}}{180} + \frac{149 n}{30} + 1 .
\]

\section{Conclusion}

The right-hand side is the polynomial the entry states, so the empirical claim of OEIS A228463 is
true for every $n\ge 0$. The argument is finite and exact: it enumerates the weak orderings of
$6$ elements, of which there are 437 achievable, and performs no fitting to the
