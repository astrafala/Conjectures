...
\title{The empirical column recurrences for the table OEIS A237234}
...
\begin{abstract}
OEIS A237234 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A237234 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with the maximum plus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 268, 268, 928, 984, 928, 2784, 3284,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:00:11, revision 6) these are still
...
column $k$$=1$ & $6$ & $81$ & $>7$ & $8$ \\
column $k$$=2$ & $6$ & $268$ & $>8$ & $10$ \\
column $k$$=3$ & $8$ & $928$ & $>13$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with the maximum plus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;8\,a(n-2) + 24\,a(n-4) + 8\,a(n-6)
\]
holds from index $7+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+1)X(3) 0..2 arrays with the maximum plus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
$S=268$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + 4\,a(n-2) + 5\,a(n-3) + 21\,a(n-4) + 12\,a(n-5) + 4\,a(n-6)
\]
holds from index $8+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+1)X(4) 0..2 arrays with the maximum plus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
$S=928$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;5\,a(n-2) + 42\,a(n-4) - 21\,a(n-6) - 207\,a(n-8)
\]
...
model reproduces $28$ published values, $143$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237234.
