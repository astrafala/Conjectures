...
\title{The empirical column recurrences for the table OEIS A224745}
...
\begin{abstract}
OEIS A224745 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A224745 is ``T(n,k) is the number of (n+1) X (k+1) 0..1 matrices with each 2 X 2 permanent equal.''. It has offset $1$, is
read by antidiagonals, and begins
\[
16, 38, 38, 89, 152, 89, 218, 565,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 5*a(n-1) -6*a(n-2) -3*a(n-3) +7*a(n-4) -2*a(n-5)\\
k=2: a(n) = 7*a(n-1) -7*a(n-2) -34*a(n-3) +59*a(n-4) +27*a(n-5) -87*a(n-6) +36*a(n-7)\\
k=3: [order 9]\\
k=4: [order 13]\\
k=5: [order 20]\\
k=6: [order 34]\\
k=7: [order 55]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 05 2019 08:19:58, revision 10) these are still
...
The sequence is ``Number of (n+1) X2 0..1 matrices with each 2 X 2 permanent equal.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X3 0..1 matrices with each 2 X 2 permanent equal.''. Its rows are the vertices of a digraph on
$S=14$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;7\,a(n-1) - 7\,a(n-2) - 34\,a(n-3) + 59\,a(n-4) + 27\,a(n-5) - 87\,a(n-6) + 36\,a(n-7)
\]
...
model reproduces $19$ published values, $77$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224745.
