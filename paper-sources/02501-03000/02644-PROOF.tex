...
\title{A further conjecture on OEIS A253452,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A253452 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $3639, 8554, 11147, 15730, 26831, 52912, 114717, 266911,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=6\,a(n-1) - 11\,a(n-2) + 6\,a(n-3) ,
\]
a linear recurrence of order $3$ with characteristic polynomial
\[
  q(x)=x^{3} - 6 x^{2} + 11 x - 6 .
\]
...
\begin{quote}\raggedright Empirical: a(n) = 529*3\^{}(n-3) + 1039*2\^{}(n-1) + 5372 for n>6.\end{quote}
...
indices, and $a$ and $f$ were checked to agree at $n=10,\dots,12$,
