...
\title{The empirical recurrence for OEIS A252721, proved by transfer matrix}
...
\begin{abstract}
OEIS A252721 counts arrays of width $4$ over an alphabet of $4$ letters in which every three
...
brings the state count down to $S=131$.
\end{abstract}
...
OEIS A252721 is ``Number of (2+2) X (n+2) 0..3 arrays with every consecutive three elements in every row and diagonal having exactly two distinct values, and in every column and antidiagonal not having exactly two distinct values, and new values 0 upwards introduced in row major order.''. It has offset $1$ and begins
\[
45, 46, 50, 63, 69, 91, 107, 131,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-2) + 2*a(n-3) - 2*a(n-5) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 05 2018 15:41:01, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array has width $4$ and entries in $\{0,1,\dots,3\}$. Four families of triples
...
\end{array}
\]
A triple counts only when all three of its cells lie inside the array. The entry writes the array with $n$ as the number of COLUMNS. The walk is run down the transpose, which exchanges rows with columns and carries each diagonal direction to itself, so the two conditions are exchanged once and the diagonal ones left alone.
...
$a=N_1+\dots+N_{4}$, so
\[
a\;=\;\sum_{i=1}^{4}w_i\,L_i ,\qquad w=\mathbf{1}^{\!\top}F^{-1},
\]
...
That digraph has up to $i^{2\cdot4}$ vertices, which is more than can be written down for
...
Assembling the $i=1,\dots,4$ pieces as a disjoint union, with $\iota$ carrying the weights
$w_i$ cleared of denominators,
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,
\]
with $S=131$ states in all. The walk index $j$ and the entry's own $n$ differ by a constant,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252721.
