...
\title{A further conjecture on OEIS A266668,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A266668 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 5, 4, 119, 16, 2015, 64, 32639,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=-2\,a(n-1) + 17\,a(n-2) + 34\,a(n-3) - 16\,a(n-4) - 32\,a(n-5) ,
\]
a linear recurrence of order $5$ with characteristic polynomial
\[
  q(x)=x^{5} + 2 x^{4} - 17 x^{3} - 34 x^{2} + 16 x + 32 .
\]
...
\begin{quote}\raggedright a(n) = (-1/2-(-4)\^{}n+(-2)\^{}n+(-1)\^{}n/2+4\^{}n).\end{quote}
...
\[
  p(x)=x^{5} + 2 x^{4} - 17 x^{3} - 34 x^{2} + 16 x + 32 ,
\]
