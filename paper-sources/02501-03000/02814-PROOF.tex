...
\title{The empirical recurrence for OEIS A274854, proved by counting patterns}
...
\begin{abstract}
OEIS A274854 counts arrays in which no entry may repeat a value found at any of three stated
...
cutting $6818$ pairs to $S=1223$ states. The entry's empirical recurrence of order $5$,
...
OEIS A274854 is ``Number of n X 4 0..2 arrays with no element equal to any value at offset (-1,0) (0,-1) or (-2,-2) and new values introduced in order 0..2.''. It has offset $1$ and begins
\[
4, 27, 43, 118, 330, 923, 2585, 7253,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) - 9*a(n-2) + 10*a(n-3) - 6*a(n-4) + a(n-5) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 31 2019 08:06:44, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 9\,a(n-2) + 10\,a(n-3) - 6\,a(n-4) + a(n-5)
\end{equation}
for all $n>7$.
...
$o=1n_0+0-2$, we have for $j\ge5$
\[
3!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)
=\mathbf{v}^{\!\top}G^{\,j-5}w,\qquad
w=h_{5}-\sum_i c_i h_{5-i} .
\]
...
the survivors by the pattern of the new pair. The vector $w$ was formed by $5$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A274854.
