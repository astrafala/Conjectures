...
\title{The empirical column recurrences for the table OEIS A233883}
...
\begin{abstract}
OEIS A233883 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A233883 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every 2X2 subblock having the sum of the squares of all six edge and diagonal differences equal to 11 (11 maximizes T(1,1)).''. It has offset $1$, is
read by antidiagonals, and begins
\[
24, 76, 76, 240, 300, 240, 760, 1224,\ \dots
\]
...
k=5: [order 52]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 09:03:42, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..2 arrays with every 2X2 subblock having the sum of the squares of all six edge and diagonal differences equal to 11.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..2 arrays with every 2X2 subblock having the sum of the squares of all six edge and diagonal differences equal to 11.''. Its rows are the vertices of a digraph on
...
model reproduces $19$ published values, $85$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233883.
