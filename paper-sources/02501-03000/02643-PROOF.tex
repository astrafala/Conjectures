...
\title{The empirical recurrence for OEIS A224565, proved by transfer matrix}
...
\begin{abstract}
OEIS A224565 counts the $(n+3)\times8$ matrices over $\{0,\dots,1\}$ all of whose
$4\times4$ contiguous subblocks are idempotent, and carries an empirical recurrence of order
$8$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical.
The condition ties together $4$ consecutive rows and $4$ consecutive columns at once, so a
single row is not a state; the strip of the last $3$ rows is, and the admissible strips
are read off the overlap graph of the admissible windows instead of being enumerated. That
makes $a(n)$ a walk count on $S=990$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224565 is ``Number of (n+3) X 8 0..1 matrices with each 4 X 4 subblock idempotent.''. It has offset $1$ and begins
\[
990, 611, 728, 861, 974, 1233, 1614, 2099,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) - 2*a(n-2) - 2*a(n-3) + 4*a(n-4) - 3*a(n-5) + 2*a(n-7) - a(n-8) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 01 2018 09:17:56, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $4\times4$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $4\times8$ matrix a \emph{band} when every one of its $5$ contiguous
$4\times4$ subblocks is a window. What the entry counts is the matrices of $8$ columns
whose every $4$ consecutive rows form a band.

Take as states the $3\times8$ strips that occur as the top $3$ rows of a band,
together with those that occur as its bottom $3$ rows, and put an edge from $u$ to $v$
when the band whose first $3$ rows are $u$ and whose last $3$ rows are $v$ exists.
A band is determined by that pair, so the edge is unique when it exists. Reading a counted
matrix from the top, its rows $1..3$, then $2..4$, and so on, form a walk, and every
walk arises from exactly one matrix; a matrix with $n+3$ rows gives a walk of $n$ edges.
Since the first $3$ rows of a counted matrix with at least $4$ rows are the top of a
band, and since no further condition falls on where a walk starts or ends,
\[
a(n)\;=\;\iota^{\!\top}M^{\,n}\tau ,\qquad \iota=\tau=(1,1,\dots,1)^{\!\top},
\]
with $M$ the adjacency matrix on $S=990$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{24}$ strips of $3$ rows and $8$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $4\times3$ entries, so a band is exactly a walk of $5$ windows in
the graph that joins a window to those whose first $3$ columns are its last $3$
columns. Enumerating those walks produces the bands, and with them the states and the edges,
directly.

\section{The admissible windows}

The vertices of the overlap graph are the $0/1$ matrices $B$ of size $4\times4$ with
$B^2=B$. There are $2^{16}$ matrices to sift, which is already awkward at $4=4$ and
...
Generating from Lemma~\ref{lem:idem} gives $452$ admissible $4\times4$ windows. For
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) - 2\,a(n-2) - 2\,a(n-3) + 4\,a(n-4) - 3\,a(n-5) + 2\,a(n-7) - a(n-8)
\]
for every $n>10$.
\end{theorem}
...
until the working vector was identically zero, which settles every larger $j$ at once. The bound is exact: at $n=10$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224565.
