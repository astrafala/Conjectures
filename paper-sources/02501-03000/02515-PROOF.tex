...
\title{A further conjecture on OEIS A235891,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A235891 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $17152, 18712, 21928, 28744, 43912, 80392, 177928, 471304,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=7\,a(n-1) - 14\,a(n-2) + 8\,a(n-3) ,
\]
a linear recurrence of order $3$ with characteristic polynomial
\[
  q(x)=x^{3} - 7 x^{2} + 14 x - 8 .
\]
...
\begin{quote}\raggedright a(n) = 4*(3906 + 189*2\^{}n + 4\^{}n).\end{quote}
