...
\title{The empirical recurrence for OEIS A268094, proved by transfer matrix}
...
\begin{abstract}
OEIS A268094 counts arrays in which the REPEATED VALUES of each row, and of each column, stand in a
...
OEIS A268094 is ``Number of 2 X n 0..2 arrays with every repeated value in every row not one larger and in every column one larger mod 3 than the previous repeated value, and upper left element zero.''. It has offset $1$ and begins
\[
3, 27, 243, 2028, 16428, 129792, 1009200, 7756992,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 14*a(n-1) - 44*a(n-2) - 88*a(n-3) + 352*a(n-4) + 320*a(n-5) - 256*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 11 2019 15:23:14, revision 8) this is still
...
NOT one larger modulo 3 than the previous repeated value of that row, and that each repeated value of a column,
read downwards, be one larger modulo 3 than the previous repeated value of that column. The FIRST repeated
value of a scan has no predecessor and is unconstrained; a scan with no two equal neighbours
imposes nothing at all. The entry also fixes the top left entry at $0$, which restricts which slices may begin an array and nothing else. The entry writes the array with $n$ as the number of COLUMNS, so the array grows one COLUMN at a time: the column scan is the one that runs inside a slice and the row scan is the one that runs across slices.
...
\begin{theorem}
\[
a(n)\;=\;14\,a(n-1) - 44\,a(n-2) - 88\,a(n-3) + 352\,a(n-4) + 320\,a(n-5) - 256\,a(n-6)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A268094.
