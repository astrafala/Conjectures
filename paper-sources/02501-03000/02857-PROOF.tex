...
\title{The empirical recurrence for OEIS A224558, proved by transfer matrix}
...
\begin{abstract}
OEIS A224558 counts the $(n+2)\times9$ matrices over $\{0,\dots,1\}$ all of whose
$3\times3$ contiguous subblocks are idempotent, and carries an empirical recurrence of order
$6$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than empirical.
...
makes $a(n)$ a walk count on $S=611$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224558 is ``Number of (n+2) X 9 0..1 matrices with each 3 X 3 subblock idempotent.''. It has offset $1$ and begins
\[
669, 492, 620, 749, 988, 1298, 1668, 2201,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 6*a(n-2) + 5*a(n-3) - 4*a(n-4) + 3*a(n-5) - a(n-6) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 01 2018 09:17:24, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $3\times3$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $3\times9$ matrix a \emph{band} when every one of its $7$ contiguous
$3\times3$ subblocks is a window. What the entry counts is the matrices of $9$ columns
whose every $3$ consecutive rows form a band.

Take as states the $2\times9$ strips that occur as the top $2$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=611$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{18}$ strips of $2$ rows and $9$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $3\times2$ entries, so a band is exactly a walk of $7$ windows in
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) - 6\,a(n-2) + 5\,a(n-3) - 4\,a(n-4) + 3\,a(n-5) - a(n-6)
\]
for every $n>8$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224558.
