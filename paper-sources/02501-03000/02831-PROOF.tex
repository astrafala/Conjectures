...
\title{The empirical column recurrences for the table OEIS A231199}
...
\begin{abstract}
OEIS A231199 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231199 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with no element equal to a strict majority of its horizontal, vertical and antidiagonal neighbors, with values 0..2 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
9, 67, 67, 538, 1413, 538, 4264, 31665,\ \dots
\]
...
k=3: [order 62]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:18:09, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with no element equal to a strict majority of its horizontal, vertical and antidiagonal neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=105$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;8\,a(n-1) - 4\,a(n-3) + 4\,a(n-4) - 12\,a(n-5) - 16\,a(n-6)
\]
...
$S=763$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=1526$ exact terms returns the minimal one, of order
$18$ --- the stated order --- with integer coefficients
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $22$ & $c_{6}$ & $-4420$ & $c_{11}$ & $179816$ & $c_{16}$ & $368640$ \\
$c_{2}$ & $7$ & $c_{7}$ & $892$ & $c_{12}$ & $-55176$ & $c_{17}$ & $246784$ \\
$c_{3}$ & $-24$ & $c_{8}$ & $1572$ & $c_{13}$ & $-492608$ & $c_{18}$ & $-61440$ \\
$c_{4}$ & $218$ & $c_{9}$ & $5428$ & $c_{14}$ & $-667584$ &  &  \\
$c_{5}$ & $-1474$ & $c_{10}$ & $124572$ & $c_{15}$ & $-56192$ &  &  \\
...
model reproduces $8$ published values, $36$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231199.
