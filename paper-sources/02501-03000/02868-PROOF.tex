...
\title{The empirical recurrence for OEIS A231356, proved by transfer matrix}
...
\begin{abstract}
OEIS A231356 counts $(n+1)\times2$ arrays over $\{0,\dots,3\}$ subject to a condition imposed on
...
OEIS A231356 is ``Number of (n+1) X (1+1) 0..3 arrays with no element unequal to a strict majority of its horizontal, vertical and antidiagonal neighbors, with values 0..3 introduced in row major order.''. It has offset $1$ and begins
\[
1, 2, 4, 12, 33, 102, 329, 1075,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 5*a(n-2) - 5*a(n-3) - 24*a(n-4) - 2*a(n-5) + 15*a(n-6) + 9*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 28 2018 14:59:21, revision 8) this is still
...
$\{0,\dots,3\}^{2}$. The neighbourhood of a cell in row $u_i$ meets $u_{i-1}$,
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 5\,a(n-2) - 5\,a(n-3) - 24\,a(n-4) - 2\,a(n-5) + 15\,a(n-6) + 9\,a(n-7)
\]
for every $n>7$.
\end{theorem}
...
integers vanish from the index corresponding to $n=7+1$ onwards, and the run was
continued until the working vector was identically zero, which settles all larger $j$ at once.
By Lemma~\ref{lem:crit} the recurrence holds for every $n>7$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231356.
