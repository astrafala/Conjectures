...
\title{A further conjecture on OEIS A266303,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A266303 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 2, 1, 6, 1, 10, 1, 14,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=2\,a(n-2) - 1\,a(n-4) ,
\]
...
\begin{quote}\raggedright a(n) = (1+(-1)\^{}n-2*(-1+(-1)\^{}n)*n)/2.\end{quote}
