...
\title{The empirical column recurrences for the table OEIS A229606}
...
\begin{abstract}
OEIS A229606 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A229606 is ``T(n,k) = number of defective 3-colorings of an n X k 0..2 array connected horizontally and vertically with exactly two mistakes, and colors introduced in row-major 0..2 order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 0, 0, 1, 6, 1, 3, 39,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 27 2021 21:30:13, revision 7) these are still
...
The sequence is ``Number of defective 3-colorings of an n X 1 0..2 array connected horizontally and vertically with exactly two mistakes, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
$S=18$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of defective 3-colorings of an n X 2 0..2 array connected horizontally and vertically with exactly two mistakes, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of defective 3-colorings of an n X 3 0..2 array connected horizontally and vertically with exactly two mistakes, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
$S=108$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;15\,a(n-1) - 81\,a(n-2) + 185\,a(n-3) - 162\,a(n-4) + 60\,a(n-5) - 8\,a(n-6)
\]
...
model reproduces $25$ published values, $75$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229606.
