...
\title{The empirical column recurrences for the table OEIS A203878}
...
\begin{abstract}
OEIS A203878 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=4$.
\end{abstract}
...
OEIS A203878 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with no 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..3 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
7, 55, 55, 487, 1135, 487, 4375, 23815,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:48:34, revision 9) these are still
...
The sequence is ``Number of (n+1)X(5) 0..3 arrays with no 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..3 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=1300$ vertices, edges being the admissible consecutive pairs, and the count is
...
model reproduces $8$ published values, $89$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203878.
