...
\title{The empirical recurrence for OEIS A209521, proved by counting patterns}
...
\begin{abstract}
OEIS A209521 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $204$ to $S=15$. The entry's empirical recurrence of order
$9$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A209521 is ``Number of nX2 0..7 arrays with every 2X2 subblock containing exactly one value repeat, and new values 0..7 introduced in row major order.''. It has offset $1$ and begins
\[
2, 6, 57, 700, 10677, 193676, 4054358, 95534880,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 15:18:15, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;68\,a(n-1) - 1410\,a(n-2) + 5696\,a(n-3) + 105944\,a(n-4) - 757536\,a(n-5) - 1207008\,a(n-6) + 7568640\,a(n-7) + 13260672\,a(n-8) + 4354560\,a(n-9)
\end{equation}
for all $n>10$.
...
Assembling the $8$ lumped chains into one block-diagonal matrix $N$ of size $S=15$ and
...
\end{equation}
The unlumped alphabet had $204$ rows in all; the lumped chain has $S=15$ states.
...
For every $n$ with $m=n+0-1\ge9$,
\[
8!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-9}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-9}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $9$ matrix--vector products
...
\eqref{eq:walk}, divided by $8!$, was evaluated at every one of the $17$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A209521.
