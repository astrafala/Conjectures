...
\title{A further conjecture on OEIS A280399,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A280399 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $0, 0, 2, 2, 8, 14, 36, 74,\dots$, with offset $1$. The sequence satisfies
\[
...
\begin{quote}\raggedright a(n) = (8*((-1)\^{}(1+n)+2\^{}n) + 3*(-8*(-1)\^{}n+2\^{}n)*n) / 108 for n>1.\end{quote}
