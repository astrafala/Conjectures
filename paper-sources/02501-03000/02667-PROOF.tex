...
\title{The empirical recurrence for OEIS A209809, proved by counting patterns}
...
\begin{abstract}
OEIS A209809 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $354$ to $S=38$. The entry's empirical recurrence of order
$14$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A209809 is ``Number of (n+1)X4 0..3 arrays with every 2X2 subblock having two distinct values, and new values 0..3 introduced in row major order.''. It has offset $1$ and begins
\[
173, 2721, 43209, 712829, 11912459, 201392025, 3428713825, 58668627061,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 15:30:28, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;26\,a(n-1) - 68\,a(n-2) - 1905\,a(n-3) + 6356\,a(n-4) + 40342\,a(n-5) - 117881\,a(n-6) - 302966\,a(n-7) + 604526\,a(n-8) + 940543\,a(n-9) - 1163096\,a(n-10) - 1137756\,a(n-11) + 834912\,a(n-12) + 402624\,a(n-13) - 124416\,a(n-14)
\end{equation}
for all $n>14$.

\section{What the canonical-form clause counts}

Fix the shape: $4$ columns and $L=n+1$ rows, so $L\cdot4$ cells. Call a
...
Assembling the $4$ lumped chains into one block-diagonal matrix $N$ of size $S=38$ and
...
\end{equation}
The unlumped alphabet had $354$ rows in all; the lumped chain has $S=38$ states.
...
For every $n$ with $m=n+1-1\ge14$,
\[
4!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-14}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-14}$ out of
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $14$ matrix--vector products
...
corresponding to $n=14+1$ onwards, and the run continued until $w$ was the zero vector,
...
\eqref{eq:walk}, divided by $4!$, was evaluated at every one of the $15$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A209809.
