...
\title{The empirical column recurrences for the table OEIS A232316}
...
\begin{abstract}
OEIS A232316 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A232316 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with every element equal to some horizontal or antidiagonal neighbor, with top left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 5, 5, 16, 24, 13, 52, 139,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 07:18:13, revision 8) these are still
...
column $k$$=1$ & $2$ & $20$ & $>1$ & $8$ \\
column $k$$=2$ & $2$ & $72$ & $>1$ & $7$ \\
column $k$$=3$ & $4$ & $272$ & $>3$ & $6$ \\
column $k$$=4$ & $6$ & $1056$ & $>7$ & $10$ \\
row $n$$=1$ & $4$ & $20$ & $>5$ & $9$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) 0..1 arrays with every element equal to some horizontal or antidiagonal neighbor, with top left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..1 arrays with every element equal to some horizontal or antidiagonal neighbor, with top left element zero.''. Its rows are the vertices of a digraph on
$S=72$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;5\,a(n-1) - a(n-2)
...
The sequence is ``Number of (n+1)X(4) 0..1 arrays with every element equal to some horizontal or antidiagonal neighbor, with top left element zero.''. Its rows are the vertices of a digraph on
$S=272$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;10\,a(n-1) - 11\,a(n-2) - 5\,a(n-3) - a(n-4)
\]
holds from index $3+1$ on.

\subsection*{Column $k$$=4$}
The sequence is ``Number of (n+1)X(5) 0..1 arrays with every element equal to some horizontal or antidiagonal neighbor, with top left element zero.''. Its rows are the vertices of a digraph on
$S=1056$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;19\,a(n-1) - 44\,a(n-2) + 43\,a(n-3) - 19\,a(n-4) + 4\,a(n-5) - 2\,a(n-6)
\]
holds from index $7+1$ on.

\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) 0..1 arrays with every element equal to some horizontal or antidiagonal neighbor, with top left element zero.''. Its rows are the vertices of a digraph on
$S=20$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1) + a(n-2) - 2\,a(n-4)
\]
...
model reproduces $40$ published values, $168$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232316.
