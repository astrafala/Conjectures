...
\title{The empirical column recurrences for the table OEIS A237249}
...
\begin{abstract}
OEIS A237249 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A237249 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with the maximum plus the upper median minus the lower median minus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 344, 344, 1488, 2064, 1488, 6496, 12600,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 352*a(n-4) for n>5\\
k=2: a(n) = 1410*a(n-4) for n>6\\
k=3: a(n) = 11576*a(n-4) -33493888*a(n-8) for n>11\\
k=4: [order 16] for n>20\\
k=5: [order 36] for n>41\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:01:33, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with the maximum plus the upper median minus the lower median minus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays with the maximum plus the upper median minus the lower median minus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(4) 0..2 arrays with the maximum plus the upper median minus the lower median minus the minimum of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. Its rows are the vertices of a digraph on
$S=1488$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237249.
