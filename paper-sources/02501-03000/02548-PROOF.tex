...
\title{The empirical recurrence for OEIS A263667, proved by transfer matrix}
...
\begin{abstract}
OEIS A263667 counts arrays of width $2$ over $\{0,1,\dots,6\}$ whose rows, read as base-$7$
numbers, are divisible by $11$ and whose columns, read the same way, are divisible by
$11$. The entry carries an empirical recurrence of order $11$ contributed by
...
OEIS A263667 is ``Number of (n+1)X(1+1) 0..6 arrays with each row and column divisible by 11, read as a base-7 number with top and left being the most significant digits.''. It has offset $1$ and begins
\[
3, 12, 57, 283, 1415, 7097, 35513, 177572,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-1) +67*a(n-10) -335*a(n-11)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 23 2015 12:28:50, revision 5) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array has $2$ cells across and entries $x(i,j)\in\{0,1,\dots,6\}$, the digits of
base $7$. Row $i$ of an array with $R$ rows is the number
\[
\rho_i\;=\;\sum_{j=1}^{2}x(i,j)\,7^{2-j} ,
\]
the LEFT digit being the most significant, and column $j$ is
\[
\gamma_j\;=\;\sum_{i=1}^{R}x(i,j)\,7^{R-i} ,
\]
...
\[
\gamma_j^{(i)}\;=\;7\,\gamma_j^{(i-1)}+x(i,j),
\qquad\text{hence}\qquad
\gamma_j^{(i)}\bmod 11\;=\;
\bigl(7\,(\gamma_j^{(i-1)}\bmod 11)+x(i,j)\bigr)\bmod 11 .
\]
\end{lemma}

\begin{proof}
Adding a row at the BOTTOM shifts every earlier digit one place up in significance, which is
multiplication by $7$, and puts the new digit in the units place. Reduction modulo
$11$ commutes with both operations.
\end{proof}

So the vector $\bigl(\gamma_1\bmod 11,\dots,\gamma_{2}\bmod
11\bigr)$ is a state: at most $11^{2}$ of them, and each array row
...
enumerate every array of that size, turn each row and each column into a base-$7$ number with
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263667.
