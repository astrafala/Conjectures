...
\title{A further conjecture on OEIS A229750,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A229750 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 60, 598, 2347, 6809, 17404, 41872, 97565,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=9\,a(n-1) - 33\,a(n-2) + 63\,a(n-3) - 66\,a(n-4) + 36\,a(n-5) - 8\,a(n-6) ,
\]
a linear recurrence of order $6$ with characteristic polynomial
\[
  q(x)=x^{6} - 9 x^{5} + 33 x^{4} - 63 x^{3} + 66 x^{2} - 36 x + 8 .
\]
...
\begin{quote}\raggedright a(n) = (2*(1732+117*2\^{}n) + (-4084+279*2\^{}n)*n + (484+9*2\^{}n)*n\^{}2) / 8 for n>2.\end{quote}
...
\[
  p(x)=x^{6} - 9 x^{5} + 33 x^{4} - 63 x^{3} + 66 x^{2} - 36 x + 8 ,
\]
