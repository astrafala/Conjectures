...
\title{The conjectured generating function for OEIS A239032, proved}
...
\begin{abstract}
OEIS A239032 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=30$ vertices, so its generating function is a rational function whose numerator
...
OEIS A239032 is ``Number of 5 X n 0..2 arrays with no element equal to the sum of elements to its left or one plus the sum of the elements above it, modulo 3.''. It has offset $1$ and begins
\[
4, 16, 54, 149, 354, 757,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 25 2018 08:13:27, revision 8) this is still
...
\]
and there are $S=30$ states. The digraph is built by reading the entry's own English, and the
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=8$ and $\deg D_2=7$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $38$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{38}$ then $R$ has a zero of order greater than its own
...
of them --- more than the $38+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A239032.
