...
\title{A further conjecture on OEIS A266844,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A266844 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 110, 10100, 1101000, 101010000, 11010100000, 1010101000000, 110101010000000,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=10\,a(n-1) + 10000\,a(n-2) - 100000\,a(n-3) ,
\]
a linear recurrence of order $3$ with characteristic polynomial
\[
  q(x)=x^{3} - 10 x^{2} - 10000 x + 100000 .
\]
...
\begin{quote}\raggedright a(n) = (2\^{}(n-1)*(-9*(-50)\^{}n-2*5\^{}n+209*50\^{}n))/99.\end{quote}
...
indices, and $a$ and $f$ were checked to agree at $n=3,\dots,5$,
