...
\title{A further conjecture on OEIS A280400,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A280400 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $0, 2, 6, 16, 34, 76, 158, 336,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=3\,a(n-1) + 1\,a(n-2) - 7\,a(n-3) + 4\,a(n-5) ,
\]
a linear recurrence of order $5$ with characteristic polynomial
\[
  q(x)=x^{5} - 3 x^{4} - x^{3} + 7 x^{2} - 4 .
\]
...
\begin{quote}\raggedright Empirical: a(n) = 2\^{}(n-1)*n/9 +2\^{}(n-1)*47/27 +(-1)\^{}n*2*n/9 -10*(-1)\^{}n/27-2. - \_R. J. Mathar\_, Jan 04 2017\end{quote}
