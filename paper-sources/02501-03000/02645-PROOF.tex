...
\title{The empirical closed form for OEIS A224137, proved}
...
\begin{abstract}
OEIS A224137 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$\prod_j(x-\lambda_j)^{\deg p_j+1}$; here that polynomial is $q(x)=(x-1)^{7}$, of degree
$7$. Two things are then checked exactly: that the walk count satisfies the same
recurrence from some index on, which the Cayley--Hamilton theorem reduces to a finite
computation, and that the two sequences agree at $7$ consecutive indices past that
...
OEIS A224137 is ``Number of 6 X n 0..1 arrays with rows nondecreasing and antidiagonals unimodal.''. It has offset $1$ and begins
\[
64, 729, 2692, 6392, 12874, 24095, 42832, 72888,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = (2/45)*n\^{}6 + (8/15)*n\^{}5 + (181/36)*n\^{}4 + 28*n\^{}3 + (19247/180)*n\^{}2 + (3952/15)*n - 120 for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 27 2018 08:14:31, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;\frac{2 n^{6}}{45} + \frac{8 n^{5}}{15} + \frac{181 n^{4}}{36} + 28 n^{3} + \frac{19247 n^{2}}{180} + \frac{3952 n}{15} - 120.
\]
...
Here $q(x)=(x-1)^{7}$, of degree $7$; the closed form is a polynomial in $n$, so this is $(x-1)^{7}$, and the recurrence it encodes is
...
\begin{proof}
The vector $w'=q(M)w$ was formed by $7$ matrix--vector products in exact integer
...
identically. Both were then evaluated in exact arithmetic at the $7$ consecutive indices
$n=12,\dots,18$ and agree at each. A monic recurrence of order $7$
determines $a(m)$ from $a(m-1),\dots,a(m-7)$, and for every $m\ge19$ both
$m>11$ and $m-7\ge12$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge12$.
Below $n=12$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=12$ on. The remaining indices $n=5,\dots,11$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224137.
