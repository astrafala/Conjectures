...
\title{The empirical recurrence for OEIS A231248, proved by transfer matrix}
...
\begin{abstract}
OEIS A231248 counts arrays of width $3$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
the count is a walk count on $S=349$ states.
\end{abstract}
...
OEIS A231248 is ``Number of 3 X n 0..3 arrays x(i,j) with each element horizontally, diagonally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
0, 6, 78, 464, 3090, 22460, 162766, 1169472,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 7*a(n-1) - 3*a(n-2) + 27*a(n-3) + 28*a(n-4) + 14*a(n-5) - 8*a(n-6) - 8*a(n-7) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 28 2018 06:54:45, revision 8) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=349.
\]
...
\begin{theorem}
\[
a(n)\;=\;7\,a(n-1) - 3\,a(n-2) + 27\,a(n-3) + 28\,a(n-4) + 14\,a(n-5) - 8\,a(n-6) - 8\,a(n-7)
\]
for every $n>8$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231248.
