...
\title{The empirical recurrence for OEIS A224034, proved by transfer matrix}
...
\begin{abstract}
OEIS A224034 counts arrays over $\{0,\dots,1\}$ whose rows, columns, diagonals or
antidiagonals are required to be monotone or unimodal, and carries an empirical recurrence of
order $7$ contributed by R.~H.~Hardin. A monotone condition is local, but a unimodal one
...
OEIS A224034 is ``Number of nX4 0..1 arrays with antidiagonals unimodal and rows and diagonals nondecreasing.''. It has offset $1$ and begins
\[
5, 19, 58, 168, 488, 1422, 4160, 12214,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 5*a(n-1) -6*a(n-2) -a(n-3) +2*a(n-4) +2*a(n-5) -2*a(n-6) +4*a(n-7)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 23:12:10, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;5\,a(n-1) - 6\,a(n-2) - a(n-3) + 2\,a(n-4) + 2\,a(n-5) - 2\,a(n-6) + 4\,a(n-7)
\end{equation}
for all $n>7$.
...
$h_j=G^{j}N^{o}\mathbf{1}$, we have for $j\ge7$
\[
a(n_0+j)-\sum_i c_i\,a(n_0+j-i)=\,\mathbf{v}^{\!\top}G^{\,j-7}w,
\qquad w=h_{7}-\sum_i c_ih_{7-i} .
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224034.
