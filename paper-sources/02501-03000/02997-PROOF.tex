...
\title{The empirical recurrence for OEIS A224544, proved by transfer matrix}
...
\begin{abstract}
OEIS A224544 counts the $(n+1)\times3$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=8$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224544 is ``Number of (n+1) X 3 0..1 matrices with each 2 X 2 subblock idempotent.''. It has offset $1$ and begins
\[
16, 32, 52, 86, 137, 218, 345, 547,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 5*a(n-2) + a(n-3) + 2*a(n-4) - a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 30 2018 15:49:21, revision 7) this is still
...
\]
with $M$ the adjacency matrix on $S=8$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{3}$ strips of $1$ rows and $3$ columns, far too
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224544.
