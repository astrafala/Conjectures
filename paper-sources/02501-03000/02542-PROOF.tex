...
\title{An exact count for OEIS A228744: windowed medians over a growing alphabet}
...
\begin{abstract}
OEIS A228744 counts the arrays obtainable as the medians of $3$ adjacent elements of a length
...
The entry's first terms are $88, 1071, 6130, 23627, 71278, 181885, 410828, 845517,\dots$ and it states

\begin{quote}\raggedright Empirical: a(n) = (5/126)*n\^{}7 + (4/3)*n\^{}6 + (829/90)*n\^{}5 + (93/4)*n\^{}4 + (509/18)*n\^{}3 + (221/12)*n\^{}2 + (453/70)*n + 1.\end{quote}

\section{The object}

Fix $k=9$, $w=3$ and $L=k-w+1=7$. For $a\in\{0,\dots,n\}^k$ let
\[
...
Carrying that out over the weak orderings of $7$ elements gives
\[
  \lvert\Phi(\{0,\dots,n\}^k)\rvert
  = 1\binom{n+1}{1} + 86\binom{n+1}{2} + 810\binom{n+1}{3} + 2370\binom{n+1}{4} + 2812\binom{n+1}{5} + 1360\binom{n+1}{6} + 200\binom{n+1}{7} \;+\; 0
  = \frac{5 n^{7}}{126} + \frac{4 n^{6}}{3} + \frac{829 n^{5}}{90} + \frac{93 n^{4}}{4} + \frac{509 n^{3}}{18} + \frac{221 n^{2}}{12} + \frac{453 n}{70} + 1 .
\]

\section{Conclusion}

The right-hand side is the polynomial the entry states, so the empirical claim of OEIS A228744 is
true for every $n\ge 0$. The computation is finite and exact -- it enumerates the weak orderings
of $7$ elements and, for the few that need it, the gap patterns -- and performs no fitting to
the published terms. Those 26 terms are reproduced by the formula as a check, not as
evidence.
