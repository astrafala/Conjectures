...
\title{The empirical column recurrences for the table OEIS A231137}
...
\begin{abstract}
OEIS A231137 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231137 is ``T(n,k)=Number of (n+1)X(k+1) black-square subarrays of 0..2 arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 6, 9, 6, 16, 40,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:16:46, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 black-square subarrays of 0..2 arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 black-square subarrays of 0..2 arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=72$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;22\,a(n-2) - 36\,a(n-4) + 16\,a(n-6)
\]
...
model reproduces $17$ published values, $44$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231137.
