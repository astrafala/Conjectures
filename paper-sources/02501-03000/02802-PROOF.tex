...
\title{A further conjecture on OEIS A282124,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A282124 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 3, 3, 15, 11, 63, 43, 255,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=5\,a(n-2) - 4\,a(n-4) ,
\]
...
\begin{quote}\raggedright a(n) = (-1 + 2*(-1)\^{}n - (-1)\^{}n*2\^{}(1+n) + 2\^{}(2+n)) / 3.\end{quote}
