...
\title{A further conjecture on OEIS A283649,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A283649 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 0, 111, 1011, 111, 101111, 11111, 10111111,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=1\,a(n-1) + 100\,a(n-2) - 100\,a(n-3) ,
\]
a linear recurrence of order $3$ with characteristic polynomial
\[
  q(x)=x^{3} - x^{2} - 100 x + 100 .
\]
...
\begin{quote}\raggedright a(n) = (-10 + 23*2\^{}(1+n)*5\^{}n - 9*(-2)\^{}n*5\^{}(1+n)) / 90 for n>2.\end{quote}
...
indices, and $a$ and $f$ were checked to agree at $n=6,\dots,8$,
