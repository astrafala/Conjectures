...
\title{The empirical closed form for OEIS A207108, proved}
...
\begin{abstract}
OEIS A207108 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=169$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
monic linear recurrence whose characteristic polynomial is
$\prod_j(x-\lambda_j)^{\deg p_j+1}$; here that polynomial is $q(x)=(x-1)^{6}$, of degree
$6$. Two things are then checked exactly: that the walk count satisfies the same
recurrence from some index on, which the Cayley--Hamilton theorem reduces to a finite
computation, and that the two sequences agree at $6$ consecutive indices past that
...
OEIS A207108 is ``Number of n X 5 0..1 arrays avoiding 0 0 0 and 0 1 1 horizontally and 0 0 1 and 1 1 0 vertically.''. It has offset $1$ and begins
\[
13, 169, 677, 1835, 4047, 7837, 13863, 22931,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = (7/60)*n\^{}5 + (8/3)*n\^{}4 + (185/12)*n\^{}3 + (19/3)*n\^{}2 - (218/15)*n + 3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2018 03:32:55, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;\frac{7 n^{5}}{60} + \frac{8 n^{4}}{3} + \frac{185 n^{3}}{12} + \frac{19 n^{2}}{3} - \frac{218 n}{15} + 3.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=169$ vertices, and $u,w$ the
...
Here $q(x)=(x-1)^{6}$, of degree $6$; the closed form is a polynomial in $n$, so this is $(x-1)^{6}$, and the recurrence it encodes is
...
\begin{proof}
The vector $w'=q(M)w$ was formed by $6$ matrix--vector products in exact integer
...
identically. Both were then evaluated in exact arithmetic at the $6$ consecutive indices
$n=7,\dots,12$ and agree at each. A monic recurrence of order $6$
determines $a(m)$ from $a(m-1),\dots,a(m-6)$, and for every $m\ge13$ both
$m>6$ and $m-6\ge7$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge7$.
...
First, the walk model reproduces every one of the $31$ terms the entry publishes,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207108.
