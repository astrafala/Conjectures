...
\title{The empirical column recurrences for the table OEIS A236055}
...
\begin{abstract}
OEIS A236055 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A236055 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays colored with the difference of the maximum and minimum in each 2X2 subblock.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 292, 292, 1112, 1068, 1112, 3760, 3516,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 9*a(n-2) +4*a(n-3) +24*a(n-4) +12*a(n-5) +8*a(n-6) for n>8\\
k=2: a(n) = a(n-1) +4*a(n-2) +5*a(n-3) +21*a(n-4) +12*a(n-5) +4*a(n-6) for n>9\\
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 09:03:12, revision 6) these are still
...
column $k$$=1$ & $6$ & $81$ & $>8$ & $8$ \\
column $k$$=2$ & $6$ & $292$ & $>9$ & $10$ \\
column $k$$=3$ & $8$ & $1112$ & $>13$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays colored with the difference of the maximum and minimum in each 2X2 subblock.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays colored with the difference of the maximum and minimum in each 2X2 subblock.''. Its rows are the vertices of a digraph on
$S=292$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of (n+1)X(4) 0..2 arrays colored with the difference of the maximum and minimum in each 2X2 subblock.''. Its rows are the vertices of a digraph on
$S=1112$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $28$ published values, $148$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A236055.
