...
\title{The empirical recurrence for OEIS A222776, proved by transfer matrix}
...
\begin{abstract}
OEIS A222776 counts arrays in which no entry may equal the entries at a short list of fixed offsets
...
$2$ consecutive rows, $S=175$ states in all.
\end{abstract}
...
OEIS A222776 is ``Number of n X 1 0..6 arrays with no element equal to another at a city block distance of exactly two, and new values 0..6 introduced in row major order.''. It has offset $1$ and begins
\[
1, 2, 3, 7, 20, 67, 255, 1079,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 16*a(n-1) - 95*a(n-2) + 260*a(n-3) - 324*a(n-4) + 144*a(n-5) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 16 2018 08:58:49, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

Entries lie in $\{0,1,\dots,6\}$ and the condition is that
\[
...
list above is that rule read out. The closing clause ``new values $0..6$ introduced in row
...
$N_1+\dots+N_{7}$, so $a=\sum_i w_iL_i$ with $w=\mathbf{1}^{\!\top}F^{-1}$, $F$ the
...
of $t$ rows. Assembling $i=1,\dots,7$ as a disjoint union with $\iota$ carrying the
weights cleared of denominators and $\tau=\mathbf{1}$,
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=175.
\]
...
\begin{theorem}
\[
a(n)\;=\;16\,a(n-1) - 95\,a(n-2) + 260\,a(n-3) - 324\,a(n-4) + 144\,a(n-5)
\]
for every $n>7$.
\end{theorem}
...
enumerate every array over $\{0,\dots,6\}$, keep those whose values are introduced in the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222776.
