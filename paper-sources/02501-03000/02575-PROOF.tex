...
\title{A further conjecture on OEIS A266723,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A266723 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 3, 5, 11, 13, 23, 25, 39,\dots$, with offset $0$. The sequence satisfies
\[
...
\begin{quote}\raggedright a(n) = (3+(-1)\^{}n-2*(-3+(-1)\^{}n)*n+2*n\^{}2)/4.\end{quote}
