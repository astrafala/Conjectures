...
\title{The empirical column recurrences for the table OEIS A274858}
...
\begin{abstract}
OEIS A274858 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 3, 4$.
\end{abstract}
...
OEIS A274858 is ``T(n,k) = Number of n X k 0..2 arrays with no element equal to any value at offset (-1,0) (0,-1) or (-2,-2) and new values introduced in order 0..2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 1, 1, 2, 3, 2, 4, 9,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 15 2023 18:51:28, revision 6) these are still
...
column $k$$=1$ & $1$ & $5$ & $>2$ & $11$ \\
column $k$$=3$ & $3$ & $155$ & $>4$ & $9$ \\
column $k$$=4$ & $5$ & $1223$ & $>7$ & $8$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of n X 1 0..2 arrays with no element equal to any value at offset (-1,0) (0,-1) or (-2,-2) and new values introduced in order 0..2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 3 0..2 arrays with no element equal to any value at offset (-1,0) (0,-1) or (-2,-2) and new values introduced in order 0..2.''. Its rows are the vertices of a digraph on
$S=155$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of n X 4 0..2 arrays with no element equal to any value at offset (-1,0) (0,-1) or (-2,-2) and new values introduced in order 0..2.''. Its rows are the vertices of a digraph on
$S=1223$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $28$ published values, $65$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A274858.
