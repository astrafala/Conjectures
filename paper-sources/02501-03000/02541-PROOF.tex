...
\title{A further conjecture on OEIS A266249,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A266249 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 0, 2, 2, 4, 5, 4, 9,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=2\,a(n-2) - 1\,a(n-4) ,
\]
...
\begin{quote}\raggedright a(n) = (-2*(-1)\^{}n*n+2*n+9*(-1)\^{}n-1)/2 for n>3.\end{quote}
