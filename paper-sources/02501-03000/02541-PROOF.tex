...
\title{The empirical recurrence for OEIS A184025, proved by splitting on the common sum}
...
\begin{abstract}
OEIS A184025 counts arrays over $\{0,\dots,3\}$ in which all $2\times2$ subblock sums are
...
computation on $S=53248$ vertices.
\end{abstract}
...
OEIS A184025 is ``1/8 the number of (n+1)X6 0..3 arrays with all 2X2 subblock sums the same.''. It has offset $1$ and begins
\[
2312, 3781, 6650, 13603, 29432, 70867, 177386, 477919,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 02:15:30, revision 9) the statement is
...
The arrays have $6$ columns and $L=n+1$ rows, entries in
$\{0,\dots,3\}$, and every $2\times2$ subblock has the same sum. Since $L\ge2$ and
$6\ge2$, every such array contains at least one $2\times2$ subblock, and that subblock's
...
$V=\{0,\dots,3\}^{6}$ be the possible rows and let $M_c$ be the adjacency
matrix of the digraph in which $r\to s$ when
$r_j+r_{j+1}+s_j+s_{j+1}=c$ for every $j$ with $1\le j\le 6-1$. Then
...
Writing $N=\bigoplus_{c=0}^{12}M_c$, of size $S=53248$, and $\mathbf{v}$ for the
...
First, the right-hand side of \eqref{eq:walk} was evaluated at every one of the $22$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184025.
