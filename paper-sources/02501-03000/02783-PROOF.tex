...
\title{A further conjecture on OEIS A277560,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A277560 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 11, 0, 1111, 0, 111111, 0, 11111111,\dots$, with offset $0$. The sequence satisfies
\[
...
\begin{quote}\raggedright a(n) = (-1)*(-1 + (-1)\^{}n)*(-1 - 10\^{}n)/18 for n>0. (End)\end{quote}
...
indices, and $a$ and $f$ were checked to agree at the required window,
