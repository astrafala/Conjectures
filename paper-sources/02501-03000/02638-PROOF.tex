...
\title{A further conjecture on OEIS A280392,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A280392 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $0, 2, 14, 74, 358, 1666, 7582, 33978,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=10\,a(n-1) - 33\,a(n-2) + 40\,a(n-3) - 16\,a(n-4) ,
\]
a linear recurrence of order $4$ with characteristic polynomial
\[
  q(x)=x^{4} - 10 x^{3} + 33 x^{2} - 40 x + 16 .
\]
...
\begin{quote}\raggedright Empirical: a(n) = 4\^{}n*n/18 -4*n/9 +2*4\^{}n/27 -2/27. - \_R. J. Mathar\_, Jan 04 2017\end{quote}
