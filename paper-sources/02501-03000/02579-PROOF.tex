...
\title{A further conjecture on OEIS A266793,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A266793 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 3, 5, 10, 14, 21, 26, 37,\dots$, with offset $0$. The sequence satisfies
\[
...
\begin{quote}\raggedright a(n) = (1+7*(-1)\^{}n-2*(-3+(-1)\^{}n)*n+2*n\^{}2)/4 for n>3.\end{quote}
