...
\title{The empirical column recurrences for the table OEIS A206118}
...
\begin{abstract}
OEIS A206118 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A206118 is ``T(n,k) = number of (n+1) X (k+1) 0..3 arrays with every 2 X 3 or 3 X 2 subblock having exactly two counterclockwise and two clockwise edge increases.''. It has offset $1$, is
read by antidiagonals, and begins
\[
256, 1140, 1140, 6028, 5228, 6028, 31756, 22624,\ \dots
\]
The entry carries the following block:
\begin{quote}\small

\end{quote}
As of the ``Last modified'' line on the live entry (Dec 11 2015 11:52:11, revision 7) these are still
...
column $k$$=2$ & $6$ & $1140$ & $>8$ & $8$ \\
column $k$$=3$ & $3$ & $6028$ & $>6$ & $5$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1) X3 0..3 arrays with every 2 X 3 or 3 X 2 subblock having exactly two counterclockwise and two clockwise edge increases.''. Its rows are the vertices of a digraph on
$S=1140$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-1) + 19\,a(n-2) + 6\,a(n-3) - 90\,a(n-4) + 19\,a(n-5) + 45\,a(n-6)
\]
holds from index $8+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+1) X4 0..3 arrays with every 2 X 3 or 3 X 2 subblock having exactly two counterclockwise and two clockwise edge increases.''. Its rows are the vertices of a digraph on
$S=6028$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;115\,a(n-3)
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206118.
