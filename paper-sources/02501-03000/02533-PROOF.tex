...
\title{A further conjecture on OEIS A265723,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A265723 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $0, 3, 4, 3, 8, 3, 12, 3,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=2\,a(n-2) - 1\,a(n-4) ,
\]
...
\begin{quote}\raggedright a(n) = 1/2*((2*(-1)\^{}n+2)*n-3*((-1)\^{}n-1)).\end{quote}
