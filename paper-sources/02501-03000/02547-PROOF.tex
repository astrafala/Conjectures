...
\title{An exact count for OEIS A228741: windowed medians over a growing alphabet}
...
\begin{abstract}
OEIS A228741 counts the arrays obtainable as the medians of $3$ adjacent elements of a length
$6$ array with entries in $\{0,\dots,n\}$, and carries an empirical polynomial for that
...
The entry's first terms are $16, 77, 232, 545, 1096, 1981, 3312, 5217,\dots$ and it states

\begin{quote}\raggedright Empirical: a(n) = (n+1)*(2*n\^{}3+10*n\^{}2+9*n+3)/3.\end{quote}

\section{The object}

Fix $k=6$, $w=3$ and $L=k-w+1=4$. For $a\in\{0,\dots,n\}^k$ let
\[
...
Carrying that out over the weak orderings of $4$ elements gives
\[
  \lvert\Phi(\{0,\dots,n\}^k)\rvert
  = 1\binom{n+1}{1} + 14\binom{n+1}{2} + 32\binom{n+1}{3} + 16\binom{n+1}{4} \;+\; 0
  = \frac{2 n^{4}}{3} + 4 n^{3} + \frac{19 n^{2}}{3} + 4 n + 1 .
\]

\section{Conclusion}

The right-hand side is the polynomial the entry states, so the empirical claim of OEIS A228741 is
true for every $n\ge 0$. The computation is finite and exact -- it enumerates the weak orderings
of $4$ elements and, for the few that need it, the gap patterns -- and performs no fitting to
the published terms. Those 34 terms are reproduced by the formula as a check, not as
evidence.
