...
\title{A further conjecture on OEIS A280474,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A280474 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 6, 30, 158, 846, 4446, 22734, 113310,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=15\,a(n-1) - 87\,a(n-2) + 245\,a(n-3) - 348\,a(n-4) + 240\,a(n-5) - 64\,a(n-6) ,
\]
a linear recurrence of order $6$ with characteristic polynomial
\[
  q(x)=x^{6} - 15 x^{5} + 87 x^{4} - 245 x^{3} + 348 x^{2} - 240 x + 64 .
\]
...
\begin{quote}\raggedright a(n) = (8*(4\^{}n-1) + 21*4\^{}n*n + 6*(32+4\^{}n)*n\^{}2) / 324.\end{quote}
...
\[
  p(x)=x^{6} - 15 x^{5} + 87 x^{4} - 245 x^{3} + 348 x^{2} - 240 x + 64 ,
\]
