...
\title{The empirical recurrence for OEIS A240631, proved by transfer matrix}
...
\begin{abstract}
OEIS A240631 counts arrays of width $3$ over $\{0,\dots,1\}$ under a condition on each cell
...
a walk count on $S=74$ of them, and the conjectured recurrence is then settled by a finite
exact computation.
\end{abstract}
...
OEIS A240631 is ``Number of n X 3 0..1 arrays with no element equal to exactly three horizontal or vertical neighbors, with new values 0..1 introduced in row major order.''. It has offset $1$ and begins
\[
4, 25, 129, 637, 3147, 15590, 77269, 382933,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) + 4*a(n-2) + 2*a(n-3) + 7*a(n-4) + 6*a(n-5) - a(n-6) - a(n-7) - a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 29 2018 05:03:12, revision 8) this is still
...
terms. There are $S=74$ states, and $a$ is $C$-finite.
...
first term. There are $2^{3}=8$ possible rows over the entry's own alphabet.
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) + 4\,a(n-2) + 2\,a(n-3) + 7\,a(n-4) + 6\,a(n-5) - a(n-6) - a(n-7) - a(n-8)
\]
for every $n>8$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A240631.
