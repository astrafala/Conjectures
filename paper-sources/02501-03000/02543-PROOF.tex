...
\title{A further conjecture on OEIS A266251,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A266251 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $0, 3, 3, 5, 5, 6, 9, 6,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=2\,a(n-2) - 1\,a(n-4) ,
\]
...
\begin{quote}\raggedright a(n) = (2*(-1)\^{}n*n+2*n-9*(-1)\^{}n+3)/2 for n>3.\end{quote}
...
indices, and $a$ and $f$ were checked to agree at $n=8,\dots,11$,
