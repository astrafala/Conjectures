...
\title{The empirical closed form for OEIS A223640, proved}
...
\begin{abstract}
OEIS A223640 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=45056$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A223640 is ``Number of n X 4 0..1 arrays with rows, columns, diagonals and antidiagonals unimodal.''. It has offset $1$ and begins
\[
11, 121, 726, 2962, 9808, 28450, 74599, 179991,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 22 2018 06:25:22, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;\frac{n^{8}}{112} - \frac{19 n^{7}}{210} + \frac{107 n^{6}}{90} - \frac{197 n^{5}}{40} + \frac{2219 n^{4}}{144} + \frac{5093 n^{3}}{120} - \frac{868411 n^{2}}{2520} + \frac{417721 n}{420} - 1091.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=45056$ vertices, and $u,w$ the
...
$n=14,\dots,22$ and agree at each. A monic recurrence of order $9$
determines $a(m)$ from $a(m-1),\dots,a(m-9)$, and for every $m\ge23$ both
$m>13$ and $m-9\ge14$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge14$.
Below $n=14$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=14$ on. The remaining indices $n=5,\dots,13$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223640.
