...
\title{A further conjecture on OEIS A243514,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A243514 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $3, 5, 10, 25, 76, 263, 978, 3773,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=8\,a(n-1) - 21\,a(n-2) + 22\,a(n-3) - 8\,a(n-4) ,
\]
a linear recurrence of order $4$ with characteristic polynomial
\[
  q(x)=x^{4} - 8 x^{3} + 21 x^{2} - 22 x + 8 .
\]
...
\begin{quote}\raggedright a(n) = (26 + 9*2\^{}n + 4\^{}n + 6*n) / 18.\end{quote}
