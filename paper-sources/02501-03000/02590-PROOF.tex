...
\title{A further conjecture on OEIS A267046,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A267046 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 2, 2, 4, 5, 8, 5, 12,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=2\,a(n-2) - 1\,a(n-4) ,
\]
...
\begin{quote}\raggedright a(n) = 3/2+(7*(-1)\^{}n)/2+n-(-1)\^{}n*n for n>2.\end{quote}
