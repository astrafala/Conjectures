...
\title{The conjectured generating function for OEIS A250854, proved}
...
\begin{abstract}
OEIS A250854 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=64$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $80$ coefficients in exact arithmetic
...
OEIS A250854 is ``Number of (2+1) X (n+1) 0..3 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
543, 2457, 8037, 21436, 49599, 103293,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 21 2018 11:26:42, revision 8) this is still
...
\]
and there are $S=64$ states. The digraph is built by reading the entry's own English, and the
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=7$ and $\deg D_2=7$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $71$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{71}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $80$
of them --- more than the $71+1$ the argument requires --- agree.
\end{proof}
...
\[
a(n)\;=\;\sum_{i\ge1}c_i\,a(n-i)\qquad\text{for every }n>7,
\]
a constant-coefficient linear recurrence of order $7$, valid past the degree of the
numerator. Its coefficients are the integers $c_1,\dots,c_{7}$ read off $D_2$, the first few being $7,\ -21,\ 35,\ -35,\ 21,\ -7$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250854.
