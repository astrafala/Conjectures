...
\title{The empirical recurrence for OEIS A209522, proved by counting patterns}
...
\begin{abstract}
OEIS A209522 counts arrays subject to a local condition \emph{and} to the clause ``new values
$0..7$ introduced in row major order'', which says the array is written in canonical
...
$8!\,a(n)=\sum_i\binom{8}{i}D_{8-i}L_i(n)$ with $D_m$ the derangement numbers,
...
state space down from $1296$ to $S=35$. The entry's empirical recurrence of order
$20$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A209522 is ``Number of nX3 0..7 arrays with every 2X2 subblock containing exactly one value repeat, and new values 0..7 introduced in row major order.''. It has offset $1$ and begins
\[
5, 57, 2027, 107693, 7700397, 687132348, 71519658191, 8190834795146,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 15:18:23, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;272\,a(n-1) - 24648\,a(n-2) + 828436\,a(n-3) + 149527\,a(n-4) - 622108500\,a(n-5) + 12598172586\,a(n-6) - 33558174264\,a(n-7) - 1578553176492\,a(n-8) + 17819275800656\,a(n-9) - 28667330106760\,a(n-10) - 594252767603616\,a(n-11) + 3792849444610432\,a(n-12) - 4923895161267584\,a(n-13) - 25653638667831936\,a(n-14) + 97101168572132352\,a(n-15) - 88244422976636928\,a(n-16) - 79190406866239488\,a(n-17) + 174755710679777280\,a(n-18) - 81018004244004864\,a(n-19) + 7489524376535040\,a(n-20)
\end{equation}
for all $n>21$.

\section{What the canonical-form clause counts}

Fix the shape: $3$ columns and $L=n+0$ rows, so $L\cdot3$ cells. Call a
...
The clause ``new values $0..7$ introduced in row major order'' selects, from each class
...
$v+1$. Every pattern with at most $K=8$ classes has exactly one such representative, and
...
\qquad
8!\,\sum_{j=0}^{8}N_j(n)=\sum_{i=0}^{8}\binom{8}{i}D_{8-i}\,L_i(n),
\]
...
$j\le 8$ and exchanging the order of summation,
\[
\sum_{j=0}^{8}N_j=\sum_{i=0}^{8}L_i\sum_{j=i}^{8}\frac{(-1)^{j-i}}{i!\,(j-i)!}
=\sum_{i=0}^{8}\frac{L_i}{i!}\sum_{t=0}^{8-i}\frac{(-1)^t}{t!} ,
\]
and multiplying by $8!$ turns the coefficient of $L_i$ into
$\frac{8!}{i!}\sum_{t\le 8-i}(-1)^t/t!=\binom{8}{i}(8-i)!\sum_{t\le 8-i}(-1)^t/t!
=\binom{8}{i}D_{8-i}$, an integer.
\end{proof}

\section{Each $L_i$ is a walk count, and the chain lumps}

Fix $i$ and let $V_i=\{0,\dots,i-1\}^{3}$ be the possible rows. For $r,s\in V_i$
...
Assembling the $8$ lumped chains into one block-diagonal matrix $N$ of size $S=35$ and
putting the weights of Lemma~\ref{lem:inv} into the start vector,
$\mathbf{v}=\bigl(\binom{8}{i}D_{8-i}\varepsilon_P\bigr)_{i,P}$, gives
\begin{equation}\label{eq:walk}
8!\;a(n)\;=\;\mathbf{v}^{\!\top}N^{\,n+0-1}\mathbf{1} .
\end{equation}
The unlumped alphabet had $1296$ rows in all; the lumped chain has $S=35$ states.
...
For every $n$ with $m=n+0-1\ge20$,
\[
8!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-20}q(N)\mathbf{1} .
\]
...
\begin{proof}
Substitute \eqref{eq:walk} into the left side and factor $N^{m-20}$ out of
$N^{m}-\sum_i c_iN^{m-i}$. The scalar $8!$ is nonzero, so it does not affect whether
...
column. The vector $w=q(N)\mathbf{1}$ was then formed by $20$ matrix--vector products
...
corresponding to $n=21+1$ onwards, and the run continued until $w$ was the zero vector,
...
\eqref{eq:walk}, divided by $8!$, was evaluated at every one of the $13$
...
show up as a mismatch, and the quotient by $8!$ came out an integer at every index.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A209522.
