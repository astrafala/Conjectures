...
\title{The empirical column recurrences for the table OEIS A303182}
...
\begin{abstract}
OEIS A303182 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A303182 is ``T(n,k) = Number of n X k 0..1 arrays with every element equal to 0, 1, 2, 5 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 4, 8, 4, 8, 17,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 25 2024 20:47:15, revision 6) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of 1 X n 0..1 arrays with every element equal to 0, 1, 2, 5 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of 2 X n 0..1 arrays with every element equal to 0, 1, 2, 5 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=3$}
The sequence is ``Number of 3 X n 0..1 arrays with every element equal to 0, 1, 2, 5 or 6 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
model reproduces $27$ published values, $60$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A303182.
