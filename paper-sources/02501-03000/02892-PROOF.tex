...
\title{The empirical recurrence for OEIS A214105, proved by transfer matrix}
...
\begin{abstract}
OEIS A214105 counts the proper colourings of a grid $6\times(n+1)$, wrapped in the growing direction, with at most $3$ colours and
counted up to renaming the colours, and carries an empirical recurrence of order $40$
...
down to $S=1538$ and makes the exact residual test affordable.
\end{abstract}
...
OEIS A214105 is ``Number of 0..2 colorings of a 6X(n+1) array circular in the n+1 direction with new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
243, 32, 31307, 38880, 6214031, 19920544, 1400634371, 7553900384,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 16:51:27, revision 8) this is still
...
A column of the array is a proper colouring of the path $P_{6}$ and two consecutive
columns must differ in every one of the $6$ positions; the wrapping makes the last column
adjacent to the first. A colouring of the array is therefore a CLOSED walk of $m=n+1$ steps
in the digraph $T_i$ whose vertices are the proper colourings of $P_{6}$ over $i$ colours
...
$S_i$ acts on the colourings of $P_{6}$ preserving $T_i$; two colourings lie in one orbit
...
state count is $S=1538$ rather than the number of colourings.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A214105.
