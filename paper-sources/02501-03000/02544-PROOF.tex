...
\title{An exact count for OEIS A229014: windowed medians over a growing alphabet}
...
\begin{abstract}
OEIS A229014 counts the arrays obtainable as the medians of $3$ adjacent elements of a length
...
The entry's first terms are $2, 31, 130, 359, 794, 1527, 2666, 4335,\dots$ and it states

\begin{quote}\raggedright Empirical: a(n) = (2/3)*n\^{}4 + (10/3)*n\^{}3 - (5/3)*n\^{}2 + (2/3)*n - 1.\end{quote}
...
\[
  \lvert\Phi(\{0,\dots,n\}^k)\rvert
  = 2\binom{n+1}{2} + 16\binom{n+1}{3} + 12\binom{n+1}{4} \;+\; 1\binom{n-1}{1} + 8\binom{n-2}{0} + 20\binom{n-2}{1} + 12\binom{n-2}{2} + 12\binom{n-3}{0} + 40\binom{n-3}{1} + 44\binom{n-3}{2} + 16\binom{n-3}{3} + 4\binom{n-4}{0} + 16\binom{n-4}{1} + 24\binom{n-4}{2} + 16\binom{n-4}{3} + 4\binom{n-4}{4}
  = \frac{2 n^{4}}{3} + \frac{10 n^{3}}{3} - \frac{5 n^{2}}{3} + \frac{2 n}{3} - 1 .
\]

\section{Conclusion}

The right-hand side is the polynomial the entry states, so the empirical claim of OEIS A229014 is
...
the published terms. Those 35 terms are reproduced by the formula as a check, not as
evidence.
