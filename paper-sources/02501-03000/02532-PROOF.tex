...
\title{A further conjecture on OEIS A214167,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A214167 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 1, 4, 11, 41, 161, 694, 3151,\dots$, with offset $1$. The sequence satisfies
\[
  a(n)=10\,a(n-1) - 30\,a(n-2) + 20\,a(n-3) + 31\,a(n-4) - 30\,a(n-5) ,
\]
a linear recurrence of order $5$ with characteristic polynomial
\[
  q(x)=x^{5} - 10 x^{4} + 30 x^{3} - 20 x^{2} - 31 x + 30 .
\]
...
with at least $5+2$ further terms after that index.

\section{The further conjecture}

Separately from the conjecture already settled, the entry records

\begin{quote}\raggedright Empirical: a(n) = 2\^{}n/9 +3/16 -53*(-1)\^{}n/144 +3\^{}n/16 +5\^{}n/144. - \_R. J. Mathar\_, Jul 09 2012\end{quote}
...
\[
  p(x)=x^{5} - 10 x^{4} + 30 x^{3} - 20 x^{2} - 31 x + 30 ,
\]
of order $D=5$. Since $q\mid p$ and $a$ satisfies the recurrence of $q$, $a$ satisfies that of
...
indices, and $a$ and $f$ were checked to agree at $n=6,\dots,10$,
