...
\title{A further conjecture on OEIS A266450,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A266450 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $0, 2, 5, 9, 14, 19, 28, 33,\dots$, with offset $0$. The sequence satisfies
\[
...
\begin{quote}\raggedright a(n) = (2*n\^{}2+2*((-1)\^{}n+3)*n-7*(-1)\^{}n-1)/4 for n>2.\end{quote}
