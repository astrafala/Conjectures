...
\title{A further conjecture on OEIS A266593,\\ \large settled by the proof already in hand}
...
\begin{abstract}
OEIS A266593 carries more than one conjecture. One of them has a proof; this paper settles a
...
The entry's first terms are $1, 1, 3, 2, 3, 6, 3, 10,\dots$, with offset $0$. The sequence satisfies
\[
  a(n)=2\,a(n-2) - 1\,a(n-4) ,
\]
...
\begin{quote}\raggedright a(n) = (-2*(-1)\^{}n*n+2*n+7*(-1)\^{}n-1)/2 for n>1.\end{quote}
