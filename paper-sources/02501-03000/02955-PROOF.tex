...
\title{The empirical column recurrences for the table OEIS A229637}
...
\begin{abstract}
OEIS A229637 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the rows $n=1, 2$.
\end{abstract}
...
OEIS A229637 is ``T(n,k) = number of defective 3-colorings of an n X k 0..2 array connected horizontally, diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..2 order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 0, 0, 1, 6, 0, 3, 40,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 27 2021 22:20:40, revision 8) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of defective 3-colorings of an 1 X n 0..2 array connected horizontally, diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=2$}
The sequence is ``Number of defective 3-colorings of an 2 X n 0..2 array connected horizontally, diagonally and antidiagonally with exactly two mistakes, and colors introduced in row-major 0..2 order.''. Its rows are the vertices of a digraph on
...
model reproduces $19$ published values, $49$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229637.
