...
\title{An exact count for OEIS A229013: windowed medians over a growing alphabet}
...
\begin{abstract}
OEIS A229013 counts the arrays obtainable as the medians of $3$ adjacent elements of a length
$5$ array with entries in $\{0,\dots,n\}$, and carries an empirical polynomial for that
...
The entry's first terms are $2, 15, 46, 101, 186, 307, 470, 681,\dots$ and it states

\begin{quote}\raggedright Empirical: a(n) = n\^{}3 + 3*n\^{}2 - 3*n + 1.\end{quote}

\section{The object}

Fix $k=5$, $w=3$ and $L=k-w+1=3$. For $a\in\{0,\dots,n\}^k$ let
\[
...
Carrying that out over the weak orderings of $3$ elements gives
\[
  \lvert\Phi(\{0,\dots,n\}^k)\rvert
  = 2\binom{n+1}{2} + 4\binom{n+1}{3} \;+\; 1\binom{n-1}{1} + 4\binom{n-2}{0} + 8\binom{n-2}{1} + 4\binom{n-2}{2} + 4\binom{n-3}{0} + 10\binom{n-3}{1} + 8\binom{n-3}{2} + 2\binom{n-3}{3}
  = n^{3} + 3 n^{2} - 3 n + 1 .
\]

\section{Conclusion}

The right-hand side is the polynomial the entry states, so the empirical claim of OEIS A229013 is
true for every $n\ge 0$. The computation is finite and exact -- it enumerates the weak orderings
of $3$ elements and, for the few that need it, the gap patterns -- and performs no fitting to
the published terms. Those 41 terms are reproduced by the formula as a check, not as
evidence.
