...
\title{The empirical recurrence for OEIS A224035, proved by transfer matrix}
...
\begin{abstract}
OEIS A224035 counts arrays over $\{0,\dots,1\}$ whose rows, columns, diagonals or
...
is a walk count on $S=192$ vertices and the recurrence follows from a finite exact
computation.
\end{abstract}
...
OEIS A224035 is ``Number of n X 5 0..1 arrays with antidiagonals unimodal and rows and diagonals nondecreasing.''. It has offset $1$ and begins
\[
6, 26, 85, 252, 734, 2149, 6321, 18673,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) - 10*a(n-2) + a(n-3) + 6*a(n-4) + a(n-5) - 4*a(n-6) + 4*a(n-7) - 4*a(n-8) + 8*a(n-9) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 26 2018 09:50:36, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) - 10\,a(n-2) + a(n-3) + 6\,a(n-4) + a(n-5) - 4\,a(n-6) + 4\,a(n-7) - 4\,a(n-8) + 8\,a(n-9)
\end{equation}
for all $n>10$.
...
bit vector), $|\mathcal V|=S=192$; let $(r,f)\to(s,f')$ be an edge when placing $s$ after $r$
...
every one of the $25$ published indices, with no shift fitted. The values agree exactly.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224035.
