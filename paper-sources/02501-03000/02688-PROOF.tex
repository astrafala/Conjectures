...
\title{The conjectured generating function for OEIS A228279, proved}
...
\begin{abstract}
OEIS A228279 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $31$ coefficients in exact arithmetic
...
OEIS A228279 is ``Number of n X 4 binary arrays with top left value 1 and no two ones adjacent horizontally, vertically or nw-se diagonally.''. It has offset $1$ and begins
\[
3, 6, 35, 133, 587, 2448,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(1 - x)*(3 + 6*x + 5*x\^{}2 - 2*x\^{}3) / (1 - x - 10*x\^{}2 - 15*x\^{}3 - 4*x\^{}4 + 6*x\^{}5 + x\^{}6 - 3*x\^{}7 + x\^{}8). - \_Colin Barker\_, Mar 16 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 16 2018 13:15:16, revision 15) this is still
...
The family this entry belongs to is read by one model, described by its author as: ``Cell conditions over an explicit alphabet, two-line window, no relabelling.''
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=5$ and $\deg D_2=8$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $23$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{23}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $31$
of them --- more than the $23+1$ the argument requires --- agree.
\end{proof}
...
\]
a constant-coefficient linear recurrence of order $8$, valid past the degree of the
numerator. Its coefficients are the integers $c_1,\dots,c_{8}$ read off $D_2$, the first few being $1,\ 10,\ 15,\ 4,\ -6,\ -1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A228279.
