...
\title{The empirical recurrence for OEIS A280809, proved by counting patterns with a budget}
...
\begin{abstract}
OEIS A280809 imposes three things at once: a condition on each cell that reaches one row up and
...
numbers. The empirical recurrence of order $49$ contributed by R.~H.~Hardin then follows
...
OEIS A280809 is ``Number of nX7 0..1 arrays with no element equal to more than two of its horizontal, vertical and antidiagonal neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
0, 1047, 24490, 133067, 521862, 1736603, 5219491, 14672850,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 08 2017 09:46:28, revision 4) the statement is
...
$h_j=G^{j}N^{o}\mathbf{u}$, we have for $j\ge49$
\[
2!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)=\mathbf{v}^{\!\top}G^{\,j-49}w,
...
$49$ applications of $G$ in exact integer arithmetic and $\mathbf{v}^{\!\top}G^{m}w$
evaluated for $m=0,1,\dots$, again exactly. Every one of those integers vanishes from the
index corresponding to $n=60+1$ onwards, and the run continued until $S+1$ consecutive
zeros had been seen.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A280809.
