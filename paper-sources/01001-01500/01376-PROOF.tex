...
\title{The empirical recurrence for OEIS A199850, proved from an exact model}
...
\begin{abstract}
OEIS A199850 carries an empirical recurrence of order $13$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=45$, derived below and not
...
OEIS A199850 is ``Number of -n..n arrays x(0..5) of 6 elements with zero sum and no element more than one greater than the previous.''. It has offset $1$ and begins
\[
63, 192, 428, 845, 1532, 2600, 4188, 6465,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) -2*a(n-2) -a(n-3) +2*a(n-5) -a(n-6) -a(n-7) +2*a(n-8) -a(n-10) -2*a(n-11) +3*a(n-12) -a(n-13) for n>14.
\end{quote}
As of the ``Last modified'' line on the live entry (May 16 2018 16:34:43, revision 10)
...
\]
annihilates $a$, and $S=\deg A=45$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=45$, and
...
$q(z)=z^{13}-\sum_ic_iz^{13-i}$ the characteristic polynomial of the conjectured
...
vanish from $n=14+1$ onwards, and the run exceeds $S+13$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A199850.
