...
\title{The empirical recurrence for OEIS A199851, proved from an exact model}
...
\begin{abstract}
OEIS A199851 carries an empirical recurrence of order $22$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=85$, derived below and not
...
OEIS A199851 is ``Number of -n..n arrays x(0..6) of 7 elements with zero sum and no element more than one greater than the previous.''. It has offset $1$ and begins
\[
153, 575, 1393, 2958, 5754, 10426, 17879, 29268,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 08:20:55, revision 9)
...
\]
annihilates $a$, and $S=\deg A=85$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=85$, and
...
$q(z)=z^{22}-\sum_ic_iz^{22-i}$ the characteristic polynomial of the conjectured
...
vanish from $n=24+1$ onwards, and the run exceeds $S+22$, which by
Lemma~\ref{lem:crit} settles every larger index at once. The bound is exact: at $n=24$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A199851.
