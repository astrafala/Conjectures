...
\title{The empirical closed form for OEIS A220031, proved}
...
\begin{abstract}
OEIS A220031 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=203$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
monic recurrence with characteristic polynomial $(x-1)^{7}$; that the walk count
...
OEIS A220031 is ``Number of n X 7 arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 n X 7 array.''. It has offset $1$ and begins
\[
7, 18, 55, 137, 302, 613, 1165, 2094,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/720)*n\^{}6 + (7/240)*n\^{}5 + (11/144)*n\^{}4 + (25/48)*n\^{}3 + (263/90)*n\^{}2 + (29/20)*n - 4 for n>2.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 06:14:21, revision 7)
this is still recorded as empirical, and it is still open.

\section{What is being counted}

Fix the width $W=7$ and the value range $0..1$. The underlying arrays are the
$n\times7$ arrays over that range whose rows are lexicographically nondecreasing read downwards
...
rows --- but for each of the $6$ adjacent column pairs there are only two possibilities:
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=203$
...
row at a time, holding the last two rows and the $6$ bits of Lemma~\ref{lem:local}, can
...
states number $S=203$. The last filtered row is emitted with no row below it, so it is not a
...
\[
q(x)=(x-1)^{7},\qquad\text{that is}\qquad a(n)=7\,a(n-1) - 21\,a(n-2) + 35\,a(n-3) - 35\,a(n-4) + 21\,a(n-5) - 7\,a(n-6) + a(n-7).
\]

\begin{theorem}
The closed form OEIS A220031 records is correct for all $n>2$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $9$.
Second, the polynomial and the walk count agree at every index from $3$ up to
$1 + 34 - 1$, which is more than the $7$ consecutive agreements a
common recurrence of order $7$ needs. Two sequences satisfying the same monic recurrence
of order $7$ and agreeing at $7$ consecutive indices past both their thresholds are
equal from there on.
\end{proof}
...
First, the model reproduces all $34$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220031.
