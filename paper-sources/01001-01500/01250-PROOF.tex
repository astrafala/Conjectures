...
\title{The empirical recurrence for OEIS A184709, proved from an exact model}
...
\begin{abstract}
OEIS A184709 carries an empirical recurrence of order $39$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=275$, derived below and not
...
OEIS A184709 is ``Number of strings of numbers x(i=1..9) in 0..n with sum i*x(i) equal to n*9.''. It has offset $1$ and begins
\[
8, 91, 580, 2574, 8939, 26146, 67105, 155645,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 02:52:31, revision 9)
...
\]
annihilates $a$, and $S=\deg A=275$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=275$, and
...
$q(z)=z^{39}-\sum_ic_iz^{39-i}$ the characteristic polynomial of the conjectured
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-2) + a(n-3) - a(n-4) - 2\,a(n-5) - a(n-7) + a(n-8) - a(n-9) + a(n-10) + a(n-11) + 3\,a(n-12) + a(n-13) - a(n-14) - a(n-15) - 2\,a(n-16) - a(n-17) - 3\,a(n-18) + 3\,a(n-21) + a(n-22) + 2\,a(n-23) + a(n-24) + a(n-25) - a(n-26) - 3\,a(n-27) - a(n-28) - a(n-29) + a(n-30) - a(n-31) + a(n-32) + 2\,a(n-34) + a(n-35) - a(n-36) - a(n-37) - a(n-38) + a(n-39)
\]
for every $n>0$.
\end{theorem}
...
vanish from $n=0+1$ onwards, and the run exceeds $S+39$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184709.
