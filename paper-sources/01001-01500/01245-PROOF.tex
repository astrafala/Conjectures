...
\title{The empirical closed form for OEIS A219628, proved}
...
\begin{abstract}
OEIS A219628 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=25$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219628 is ``Number of 2 X n arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..2 2 X n array.''. It has offset $1$ and begins
\[
6, 11, 24, 54, 103, 178, 290, 453,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/8)*n\^{}4 - (11/12)*n\^{}3 + (63/8)*n\^{}2 - (145/12)*n + 3 for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2018 12:14:30, revision 7)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=25$
...
states number $S=25$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219628 records is correct for all $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219628.
