...
\title{The empirical recurrence for OEIS A269541, proved from an exact model}
...
\begin{abstract}
OEIS A269541 carries an empirical recurrence of order $8$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}7 + 7*n\^{}6 + 6*n\^{}5 + 20*n\^{}4 - 12*n\^{}3 + 22*n\^{}2 - 18*n + 4. a(n) = 8*a(n-1) - 28*a(n-2) + 56*a(n-3) - 70*a(n-4) + 56*a(n-5) - 28*a(n-6) + 8*a(n-7) - a(n-8) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 24 2019 08:37:19, revision 7)
...
everywhere, a monic recurrence of order $S=30$. The annihilator was then asked for terms the
...
vanish from $n=8+1$ onwards, and the run exceeds $S+8$, which by
