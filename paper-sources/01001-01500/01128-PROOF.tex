...
\title{The empirical degree for OEIS A201351, proved from an exact model}
...
\begin{abstract}
OEIS A201351 carries the empirical claim that its terms agree with a polynomial of degree $63$.
...
recurrence of order $S=2631$, so both are settled by a finite exact computation.
\end{abstract}
...
OEIS A201351 is ``Number of nX6 0..1 arrays with rows and columns lexicographically nondecreasing and every element equal to at least one horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
5, 36, 273, 2732, 29540, 310036, 3030673, 27104947,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) is a polynomial of degree 63 for n>22
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 09:05:14, revision 9)
...
with $M$ the adjacency matrix of that digraph on $S=2631$ vertices and $\iota,\tau$ the vectors
...
$(z-1)^{63+1}$, and $\Delta^{d}a$ constant is the one with $(z-1)^{63}$. Both are
...
\begin{theorem}
$a(n)$ agrees with a polynomial in $n$ of degree exactly $63$ for every $n\ge23$.
\end{theorem}

\begin{proof}
The residual of $(z-1)^{63+1}$ was formed at each index in exact integer arithmetic from
the model. It is nonzero at $n=86$ and vanishes from $n=86+1$ onwards, with the run of
zeros continuing past $S+63+1$, which by the annihilation bound settles every later index at
once. So $\Delta^{63+1}a$ vanishes on $n\ge86-63=23$, and
Lemma~\ref{lem:diff} gives the polynomial there.

For the degree: the residual of $(z-1)^{63}$ has no run of $S+63$ zeros anywhere in the
computed range, so $\Delta^{63}a$ is not eventually zero and the degree is not smaller
than $63$. The entry claims it for $n>22$, and the threshold above is exactly $22+63+1=86$, so the range proved is the range claimed and neither is slack.
\end{proof}
...
\url{https://oeis.org}, 2026. Sequence A201351.
