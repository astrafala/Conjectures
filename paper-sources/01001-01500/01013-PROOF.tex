...
\title{The cross-base identity for OEIS A126532, proved}
...
\begin{abstract}
OEIS A126532 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126532 is ``Number of base 11 n-digit numbers with adjacent digits differing by five or less.''. It has offset $0$ and begins
\[
1, 11, 91, 781, 6691, 57343, 491429, 4211559,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:17:21, revision 17)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $11$ and the permitted difference is $d=5$, so the first clause
claims $a_{11}(n)-a_{10}(n)=11^{\,n-1}$ whenever $5(n-1)+1\le11$, that is for
$1\le n\le3$, and the second claims $a_{11}(n)-a_{10}(n)=11^{\,n-1}-2$ at the single
$n$ with $5(n-1)=11$.
...
arithmetic, $L_{5}$ at base $11$ reproduces all $19$ terms A126532 publishes.
...
First, the model reproduces all $19$ terms A126532 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{5}(11,n)$ & $L_{5}(10,n)$ & $11^{\,n-1}$ \\ \hline
1 & 11 & 10 & 1 \\
2 & 91 & 80 & 11 \\
3 & 781 & 660 & 121 \\
\end{tabular}
