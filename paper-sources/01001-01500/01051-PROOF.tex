...
\title{The empirical recurrence for OEIS A229533, proved by counting defective colourings}
...
\begin{abstract}
OEIS A229533 counts the colourings of an array with $K=3$ colours that are proper except at
...
numbers. What is left is a walk count on $S=4632$ vertices, and the empirical recurrence of
order $38$ contributed by R.~H.~Hardin follows from a finite exact computation.
\end{abstract}
...
OEIS A229533 is ``Number of defective 3-colorings of an n X 7 0..2 array connected horizontally, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..2 order.''. It has offset $1$ and begins
\[
96, 1152, 33136, 833108, 19965136, 463976296, 10551803060, 236116939092,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 27 2021 21:18:48, revision 10) the statement is
...
The arrays have $7$ columns and $L=n$ rows, entries in $\{0,\dots,3-1\}$.
...
Assembling the $3$ chains block-diagonally into $N$ of size $S=4632$ with the weights of
...
\[
3!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)=\mathbf{v}^{\!\top}G^{\,j-38}w,
\qquad w=h_{38}-\sum_i c_ih_{38-i} .
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229533.
