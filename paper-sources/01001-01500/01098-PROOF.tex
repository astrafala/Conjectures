...
\title{The empirical degree for OEIS A201535, proved from an exact model}
...
\begin{abstract}
OEIS A201535 carries the empirical claim that its terms agree with a polynomial of degree $80$.
...
recurrence of order $S=842$, so both are settled by a finite exact computation.
\end{abstract}
...
OEIS A201535 is ``Number of nX4 0..2 arrays with rows and columns lexicographically nondecreasing and every element equal to at least one horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
6, 69, 633, 7799, 91425, 983981, 9526987, 82847704,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) is a polynomial of degree 80 for n>43
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 09:12:09, revision 9)
...
with $M$ the adjacency matrix of that digraph on $S=842$ vertices and $\iota,\tau$ the vectors
...
$(z-1)^{80+1}$, and $\Delta^{d}a$ constant is the one with $(z-1)^{80}$. Both are
...
\begin{theorem}
$a(n)$ agrees with a polynomial in $n$ of degree exactly $80$ for every $n\ge44$.
\end{theorem}

\begin{proof}
The residual of $(z-1)^{80+1}$ was formed at each index in exact integer arithmetic from
the model. It is nonzero at $n=124$ and vanishes from $n=124+1$ onwards, with the run of
zeros continuing past $S+80+1$, which by the annihilation bound settles every later index at
once. So $\Delta^{80+1}a$ vanishes on $n\ge124-80=44$, and
Lemma~\ref{lem:diff} gives the polynomial there.

For the degree: the residual of $(z-1)^{80}$ has no run of $S+80$ zeros anywhere in the
computed range, so $\Delta^{80}a$ is not eventually zero and the degree is not smaller
than $80$. The entry claims it for $n>43$, and the threshold above is exactly $43+80+1=124$, so the range proved is the range claimed and neither is slack.
\end{proof}
...
\url{https://oeis.org}, 2026. Sequence A201535.
