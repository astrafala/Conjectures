...
\title{The empirical recurrence for OEIS A221425, proved by transfer matrix}
...
\begin{abstract}
OEIS A221425 counts the distinct OCCUPANCY arrays obtained when every cell of an $n\times2$ grid
...
one, on $S=57$ states, after which the conjectured recurrence is settled by a finite exact
computation.
\end{abstract}
...
OEIS A221425 is ``Number of n X 2 arrays of occupancy after each element moves to some horizontal or vertical neighbor, without consecutive moves in the same direction.''. It has offset $1$ and begins
\[
1, 9, 42, 217, 1116, 5722, 29383, 150821,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) + 6*a(n-2) - 5*a(n-4) + a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 05 2018 08:20:45, revision 9) this is still
...
$S=57$ reachable sets. In particular $a$ is $C$-finite.
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) + 6\,a(n-2) - 5\,a(n-4) + a(n-5)
\]
for every $n>5$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221425.
