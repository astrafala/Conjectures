...
\title{Every odd-indexed term of OEIS A389472 after the first is even}
...
\begin{abstract}
OEIS A389472 is defined by the functional equation $A(x)=A(x^{2}+x^{3})/x^{2}-1$ for its
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 17 2025 00:08:59, revision 7) the statement is
...
\eqref{eq:init} and compared against the entry's DATA at every one of the $41$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A389472.
