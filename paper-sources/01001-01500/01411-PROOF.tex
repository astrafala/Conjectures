...
\title{The empirical closed form for OEIS A219811, proved}
...
\begin{abstract}
OEIS A219811 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=94$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219811 is ``Number of nX3 arrays of the minimum value of corresponding elements and their horizontal, vertical or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 nX3 array.''. It has offset $1$ and begins
\[
6, 15, 62, 169, 414, 987, 2359, 5569,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:18:34, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=94$
...
states number $S=94$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219811 records is correct for all $n>4$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $16$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219811.
