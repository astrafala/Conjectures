...
\title{The empirical recurrence for OEIS A267475, proved from an exact model}
...
\begin{abstract}
OEIS A267475 carries an empirical recurrence of order $8$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}7 + (69/20)*n\^{}6 + (17/2)*n\^{}5 + (97/6)*n\^{}4 + 20*n\^{}3 + (893/60)*n\^{}2 + 6*n + 1. a(n) = 8*a(n-1) - 28*a(n-2) + 56*a(n-3) - 70*a(n-4) + 56*a(n-5) - 28*a(n-6) + 8*a(n-7) - a(n-8) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 05 2018 09:36:41, revision 7)
