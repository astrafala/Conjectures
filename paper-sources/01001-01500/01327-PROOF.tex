...
\title{The empirical recurrence for OEIS A183955, proved from an exact model}
...
\begin{abstract}
OEIS A183955 carries an empirical recurrence of order $22$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=81$, derived below and not
...
OEIS A183955 is ``Number of strings of numbers x(i=1..4) in 0..n with sum i\^{}2*x(i) equal to n*16.''. It has offset $1$ and begins
\[
1, 1, 4, 8, 14, 21, 32, 48,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - a(n-2) + a(n-4) - 2*a(n-5) + a(n-6) + a(n-16) - 2*a(n-17) + a(n-18) - a(n-20) + 2*a(n-21)-a(n-22).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 22 2022 05:15:02, revision 13)
...
\]
annihilates $a$, and $S=\deg A=81$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=81$, and
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) - a(n-2) + a(n-4) - 2\,a(n-5) + a(n-6) + a(n-16) - 2\,a(n-17) + a(n-18) - a(n-20) + 2\,a(n-21) - a(n-22)
\]
for every $n>0$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183955.
