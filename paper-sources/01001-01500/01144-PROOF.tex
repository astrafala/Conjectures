...
\title{The empirical recurrence for OEIS A184707, proved from an exact model}
...
\begin{abstract}
OEIS A184707 carries an empirical recurrence of order $28$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=145$, derived below and not
...
OEIS A184707 is ``Number of strings of numbers x(i=1..7) in 0..n with sum i*x(i) equal to n*7.''. It has offset $1$ and begins
\[
5, 35, 154, 504, 1349, 3142, 6575, 12688,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 02:52:17, revision 9)
...
\]
annihilates $a$, and $S=\deg A=145$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=145$, and
...
$q(z)=z^{28}-\sum_ic_iz^{28-i}$ the characteristic polynomial of the conjectured
...
vanish from $n=0+1$ onwards, and the run exceeds $S+28$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184707.
