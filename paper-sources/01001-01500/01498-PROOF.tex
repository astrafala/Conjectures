...
\title{The empirical recurrence for OEIS A248436, proved from an exact model}
...
\begin{abstract}
OEIS A248436 carries an empirical recurrence of order $18$. It is true, and it is
...
OEIS A248436 is ``Number of length 3+2 0..n arrays with every three consecutive terms having the sum of some two elements equal to twice the third.''. It has offset $1$ and begins
\[
2, 9, 24, 69, 118, 185, 252, 357,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 08 2018 10:46:29, revision 10)
...
$q(z)=z^{18}-\sum_ic_iz^{18-i}$ the characteristic polynomial of the conjectured
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) - a(n-2) + a(n-3) - a(n-4) + 2\,a(n-5) - a(n-6) + a(n-7) - a(n-11) + a(n-12) - 2\,a(n-13) + a(n-14) - a(n-15) + a(n-16) - a(n-17) + a(n-18)
\]
for every $n>0$.
\end{theorem}
...
vanish from $n=0+1$ onwards, and the run exceeds $S+18$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A248436.
