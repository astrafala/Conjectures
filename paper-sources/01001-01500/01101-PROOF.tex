...
\title{The empirical recurrence for OEIS A183956, proved from an exact model}
...
\begin{abstract}
OEIS A183956 carries an empirical recurrence of order $55$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=208$, derived below and not
...
OEIS A183956 is ``Number of strings of numbers x(i=1..5) in 0..n with sum i\^{}2*x(i) equal to n*25.''. It has offset $1$ and begins
\[
2, 5, 10, 27, 53, 94, 161, 259,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 22 2022 12:13:54, revision 11)
...
\]
annihilates $a$, and $S=\deg A=208$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=208$, and
...
$q(z)=z^{55}-\sum_ic_iz^{55-i}$ the characteristic polynomial of the conjectured
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-4) - a(n-5) + a(n-9) - a(n-10) - a(n-13) + a(n-14) + a(n-16) - a(n-17) - a(n-20) + a(n-21) - a(n-34) + a(n-35) + a(n-38) - a(n-39) - a(n-41) + a(n-42) + a(n-45) - a(n-46) + a(n-50) - a(n-51) - a(n-54) + a(n-55)
\]
for every $n>0$.
\end{theorem}
...
vanish from $n=0+1$ onwards, and the run exceeds $S+55$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183956.
