...
\title{The empirical closed form for OEIS A219629, proved}
...
\begin{abstract}
OEIS A219629 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=277$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219629 is ``Number of 3Xn arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..2 3Xn array.''. It has offset $1$ and begins
\[
10, 27, 130, 582, 2109, 6637, 19166, 51491,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:12:06, revision 8)
...
$3\times n$ arrays over that range whose rows are lexicographically nondecreasing read downwards
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=277$
...
states number $S=277$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219629 records is correct for all $n>9$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $26$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219629.
