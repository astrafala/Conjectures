...
\title{The empirical recurrence for OEIS A269412, proved from an exact model}
...
\begin{abstract}
OEIS A269412 carries an empirical recurrence of order $7$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}6 + 6*n\^{}5 + 8*n\^{}4 + (5/3)*n\^{}3 - n\^{}2 + (1/3)*n. a(n) = 7*a(n-1) - 21*a(n-2) + 35*a(n-3) - 35*a(n-4) + 21*a(n-5) - 7*a(n-6) + a(n-7) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 21 2019 12:22:35, revision 8)
