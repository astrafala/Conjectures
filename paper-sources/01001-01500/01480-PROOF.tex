...
\title{The empirical closed form for OEIS A219500, proved}
...
\begin{abstract}
OEIS A219500 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=206$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219500 is ``Number of n X 6 arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 n X 6 array.''. It has offset $1$ and begins
\[
6, 11, 35, 88, 179, 325, 550, 885,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/120)*n\^{}5 + (1/24)*n\^{}4 + (13/24)*n\^{}3 + (59/24)*n\^{}2 + (59/20)*n - 17 for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2018 06:35:10, revision 8)
...
$n\times6$ arrays over that range whose rows are lexicographically nondecreasing read downwards
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=206$
...
states number $S=206$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219500 records is correct for all $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219500.
