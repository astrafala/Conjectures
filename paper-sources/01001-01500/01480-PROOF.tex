...
\title{The empirical recurrence for OEIS A221115, proved by transfer matrix}
...
\begin{abstract}
OEIS A221115 counts the distinct OCCUPANCY arrays obtained when every cell of an $n\times3$ grid
...
determinising the machine that emits the occupancy row by row turns it into one, on $S=1061$
...
OEIS A221115 is ``Number of 3 X n arrays of occupancy after each element moves to some horizontal or antidiagonal neighbor, without 2-loops.''. It has offset $1$ and begins
\[
0, 0, 8, 178, 2952, 45842, 693362, 10369358,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 07 2025 19:21:33, revision 6) this is still
...
(-1,0), (-1,1), (1,-1), (1,0),
\]
a move being available only when it lands inside the grid. The offset $(0,0)$ is NOT among them: every token must move. The entry writes the array with $n$ as the number of COLUMNS; transposing it exchanges the two coordinates of every offset, which is done once so that the walk always runs down the rows. The OCCUPANCY array
...
\]
There are $S=1061$ reachable sets. In particular $a$ is $C$-finite.
...
\begin{theorem}
\[
a(n)\;=\;24\,a(n-1) - 144\,a(n-2) + 46\,a(n-3) + 1135\,a(n-4) - 908\,a(n-5) - 3663\,a(n-6) + 2438\,a(n-7) + 6528\,a(n-8) - 4340\,a(n-9) - 5525\,a(n-10) + 5164\,a(n-11) - 674\,a(n-12) - 528\,a(n-13) - 32\,a(n-14)
\]
for every $n>15$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221115.
