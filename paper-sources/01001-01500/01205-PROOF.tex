...
\title{The empirical recurrence for OEIS A184710, proved from an exact model}
...
\begin{abstract}
OEIS A184710 carries an empirical recurrence of order $51$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=353$, derived below and not
...
OEIS A184710 is ``Number of strings of numbers x(i=1..10) in 0..n with Sum\_{i=1..10} i*x(i) = n*10.''. It has offset $1$ and begins
\[
10, 142, 1066, 5439, 21310, 69331, 195760, 495251,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 05 2025 22:03:23, revision 8)
...
\]
annihilates $a$, and $S=\deg A=353$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=353$, and
...
$q(z)=z^{51}-\sum_ic_iz^{51-i}$ the characteristic polynomial of the conjectured
...
vanish from $n=0+1$ onwards, and the run exceeds $S+51$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184710.
