...
\title{The empirical recurrence for OEIS A221420, proved by transfer matrix}
...
\begin{abstract}
OEIS A221420 counts the distinct OCCUPANCY arrays obtained when every cell of an $n\times2$ grid
...
determinising the machine that emits the occupancy row by row turns it into one, on $S=27$
...
OEIS A221420 is ``Number of 2 X n arrays of occupancy after each element stays put or moves to some horizontal or antidiagonal neighbor, with every occupancy equal to zero or two.''. It has offset $1$ and begins
\[
0, 4, 10, 29, 84, 242, 696, 2000,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 3*a(n-2) - 2*a(n-4) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 05 2018 08:20:51, revision 8) this is still
...
\]
There are $S=27$ reachable sets. In particular $a$ is $C$-finite.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221420.
