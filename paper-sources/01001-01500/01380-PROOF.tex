...
\title{The empirical degree for OEIS A201534, proved from an exact model}
...
\begin{abstract}
OEIS A201534 carries the empirical claim that its terms agree with a polynomial of degree $26$.
...
recurrence of order $S=143$, so both are settled by a finite exact computation.
\end{abstract}
...
OEIS A201534 is ``Number of nX3 0..2 arrays with rows and columns lexicographically nondecreasing and every element equal to at least one horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
3, 25, 111, 633, 3354, 16393, 72811, 293831,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) is a polynomial of degree 26 for n>13
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 09:12:01, revision 9)
...
with $M$ the adjacency matrix of that digraph on $S=143$ vertices and $\iota,\tau$ the vectors
...
$(z-1)^{26+1}$, and $\Delta^{d}a$ constant is the one with $(z-1)^{26}$. Both are
...
\begin{theorem}
$a(n)$ agrees with a polynomial in $n$ of degree exactly $26$ for every $n\ge14$.
\end{theorem}

\begin{proof}
The residual of $(z-1)^{26+1}$ was formed at each index in exact integer arithmetic from
the model. It is nonzero at $n=40$ and vanishes from $n=40+1$ onwards, with the run of
zeros continuing past $S+26+1$, which by the annihilation bound settles every later index at
once. So $\Delta^{26+1}a$ vanishes on $n\ge40-26=14$, and
Lemma~\ref{lem:diff} gives the polynomial there.

For the degree: the residual of $(z-1)^{26}$ has no run of $S+26$ zeros anywhere in the
computed range, so $\Delta^{26}a$ is not eventually zero and the degree is not smaller
than $26$. The entry claims it for $n>13$, and the threshold above is exactly $13+26+1=40$, so the range proved is the range claimed and neither is slack.
\end{proof}
...
\url{https://oeis.org}, 2026. Sequence A201534.
