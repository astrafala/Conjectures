...
\title{The empirical recurrence for OEIS A250061, proved from an exact model}
...
\begin{abstract}
OEIS A250061 carries an empirical recurrence of order $9$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}8 + (18/7)*n\^{}7 + 6*n\^{}6 + 9*n\^{}5 + 3*n\^{}4 - n\^{}3 + (1/2)*n\^{}2 - (1/14)*n. a(n) = 9*a(n-1) - 36*a(n-2) + 84*a(n-3) - 126*a(n-4) + 126*a(n-5) - 84*a(n-6) + 36*a(n-7) - 9*a(n-8) + a(n-9) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 22 2017 06:36:08, revision 8)
