...
\title{The cross-base identity for OEIS A126528, proved}
...
\begin{abstract}
OEIS A126528 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126528 is ``Number of base 7 n-digit numbers with adjacent digits differing by five or less.''. It has offset $0$ and begins
\[
1, 7, 47, 317, 2137, 14407, 97127, 654797,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 10 2026 17:52:42, revision 38)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $7$ and the permitted difference is $d=5$, so the first clause
claims $a_{7}(n)-a_{6}(n)=11^{\,n-1}$ whenever $5(n-1)+1\le7$, that is for
$1\le n\le2$, and the second claims $a_{7}(n)-a_{6}(n)=11^{\,n-1}-2$ at the single
$n$ with $5(n-1)=7$.
...
arithmetic, $L_{5}$ at base $7$ reproduces all $22$ terms A126528 publishes.
...
First, the model reproduces all $22$ terms A126528 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{5}(7,n)$ & $L_{5}(6,n)$ & $11^{\,n-1}$ \\ \hline
1 & 7 & 6 & 1 \\
2 & 47 & 36 & 11 \\
\end{tabular}
\end{center}
