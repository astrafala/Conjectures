...
\title{Every term of OEIS A393856 is congruent to $1$ modulo $5$}
...
\begin{abstract}
OEIS A393856 is defined by the functional equation $A\bigl(x-xA(4x)/4\bigr)=x$ for its
ordinary generating function, and carries a conjecture of P.~D.~Hanna that every term is
congruent to $1$ modulo $5$. It is true, and the reason is that $4\equiv-1$ modulo
$5$. The functional equation determines the coefficients one at a time, so it determines
them modulo $5$ as well; and modulo $5$ the series $x/(1-x)$, all of whose coefficients
...
$\mathbb{Z}/5\mathbb{Z}$ and involves no estimation and no truncation.
\end{abstract}
...
OEIS A393856 is ``G.f. A(x) satisfies: A( x - x*A(4*x)/4 ) = x.''. It has offset $1$ and begins
\[
1, 1, 6, 121, 8466, 2227706,\ \dots
\]
So, writing $A(x)=\sum_{n\ge1}a(n)x^{n}$, the entry defines $A$ by
\begin{equation}\label{eq:fe}
A\!\left(x-\frac{x\,A(4x)}{4}\right)\;=\;x .
\end{equation}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 12 2026 10:39:38, revision 14) the statement is
...
a(n)\;\equiv\;1 \pmod{5}\qquad\text{for every } n\ge1 .
\end{equation}
...
B(x)\;:=\;x-\frac{x\,A(4x)}{4}\;=\;x-\sum_{n\ge1}a(n)\,4^{\,n-1}x^{n+1},
\end{equation}
so $B$ has integer coefficients even though \eqref{eq:fe} appears to divide by $4$.
...
\begin{proof}
Let $\bar A(x)=\dfrac{x}{1-x}=\sum_{n\ge1}x^{n}$ in $(\mathbb{Z}/5\mathbb{Z})[[x]]$, so all its
coefficients are $1$ and $\bar A(x)=x+O(x^{2})$. By Corollary~\ref{cor:mod} it suffices to
check that $\bar A$ satisfies \eqref{eq:fe} modulo $5$.

Form $\bar B$ from $\bar A$ by \eqref{eq:B}. Its coefficients are
$\bigl[x^{n+1}\bigr]\bar B=-4^{\,n-1}$, and by \eqref{eq:minus}
$4^{\,n-1}\equiv(-1)^{n-1}$, so
\[
...
So $\bar A$ satisfies \eqref{eq:fe} modulo $5$, and Corollary~\ref{cor:mod} gives
$a(n)\equiv\bigl[x^{n}\bigr]\bar A=1\pmod{5}$ for every $n\ge1$.
\end{proof}

\begin{remark}
The proof used nothing about $4$ beyond \eqref{eq:minus}. The same argument shows that
...
the quantity $4^{\,n-1}$ must be $(-1)^{n-1}$ for the telescoping above, and that is
...
Second, the same run was continued to $n=24$ and every term reduced modulo $5$; all
...
independently as a formal power series over $\mathbb{Z}/5\mathbb{Z}$, coefficient by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A393856.
