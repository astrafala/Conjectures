...
\title{The cross-base identity for OEIS A126530, proved}
...
\begin{abstract}
OEIS A126530 carries an empirical identity, in two clauses, relating its counts to those of the
...
OEIS A126530 is ``Number of base 9 n-digit numbers with adjacent digits differing by five or less.''. It has offset $0$ and begins
\[
1, 9, 69, 541, 4231, 33101, 258953, 2025831,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 12 2026 19:17:10, revision 15)
this is still recorded as empirical, and nothing on the entry records it as proved.

For this entry the base is $9$ and the permitted difference is $d=5$, so the first clause
claims $a_{9}(n)-a_{8}(n)=11^{\,n-1}$ whenever $5(n-1)+1\le9$, that is for
$1\le n\le2$, and the second claims $a_{9}(n)-a_{8}(n)=11^{\,n-1}-2$ at the single
$n$ with $5(n-1)=9$.
...
arithmetic, $L_{5}$ at base $9$ reproduces all $20$ terms A126530 publishes.
...
First, the model reproduces all $20$ terms A126530 publishes, exactly, in integer
...
\begin{center}
\begin{tabular}{rrrr}
$n$ & $L_{5}(9,n)$ & $L_{5}(8,n)$ & $11^{\,n-1}$ \\ \hline
1 & 9 & 8 & 1 \\
2 & 69 & 58 & 11 \\
\end{tabular}
\end{center}
