...
\title{The empirical closed form for OEIS A219916, proved}
...
\begin{abstract}
OEIS A219916 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=88$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219916 is ``Number of 3 X n arrays of the minimum value of corresponding elements and their horizontal, diagonal, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 3 X n array.''. It has offset $1$ and begins
\[
6, 7, 24, 64, 147, 308, 606, 1144,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 29 2018 08:05:18, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=88$
...
states number $S=88$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219916 records is correct for all $n>5$.
\end{theorem}
...
$1 + 33 - 1$, which is more than the $10$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219916.
