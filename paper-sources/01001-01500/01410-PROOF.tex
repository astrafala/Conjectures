...
\title{The empirical recurrence for OEIS A199534, proved from an exact model}
...
\begin{abstract}
OEIS A199534 carries an empirical recurrence of order $7$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = (5887/180)*n\^{}6 + (5887/60)*n\^{}5 + (620/9)*n\^{}4 + (11/12)*n\^{}3 + (433/180)*n\^{}2 - (91/30)*n. a(n) = 7*a(n-1) - 21*a(n-2) + 35*a(n-3) - 35*a(n-4) + 21*a(n-5) - 7*a(n-6) + a(n-7) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (May 16 2018 05:38:02, revision 9)
