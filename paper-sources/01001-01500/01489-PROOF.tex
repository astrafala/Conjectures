...
\title{The empirical closed form for OEIS A219622, proved}
...
\begin{abstract}
OEIS A219622 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=123$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219622 is ``Number of nX3 arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..2 nX3 array.''. It has offset $1$ and begins
\[
6, 24, 130, 566, 2134, 7259, 22715, 66250,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:11:25, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=123$
...
states number $S=123$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219622 records is correct for all $n>3$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $20$.
Second, the polynomial and the walk count agree at every index from $4$ up to
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219622.
