...
\title{The empirical closed form for OEIS A219715, proved}
...
\begin{abstract}
OEIS A219715 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=57$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219715 is ``Number of 2 X n arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 2 X n array.''. It has offset $1$ and begins
\[
10, 13, 45, 125, 283, 561, 1030, 1776,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/10)*n\^{}5 - (53/24)*n\^{}4 + (117/4)*n\^{}3 - (4099/24)*n\^{}2 + (10233/20)*n - 594 for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 27 2018 06:12:07, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=57$
...
states number $S=57$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219715 records is correct for all $n>4$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219715.
