...
\title{The empirical recurrence for OEIS A200842, proved from an exact model}
...
\begin{abstract}
OEIS A200842 carries an empirical recurrence of order $8$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = (34/315)*n\^{}7 + (163/90)*n\^{}6 + (1981/180)*n\^{}5 + (557/18)*n\^{}4 + (7807/180)*n\^{}3 + (1361/45)*n\^{}2 + (333/35)*n + 1. a(n) = 8*a(n-1) - 28*a(n-2) + 56*a(n-3) - 70*a(n-4) + 56*a(n-5) - 28*a(n-6) + 8*a(n-7) - a(n-8) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 08:49:33, revision 11)
