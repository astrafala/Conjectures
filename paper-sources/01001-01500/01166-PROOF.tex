...
\title{The empirical closed form for OEIS A220011, proved}
...
\begin{abstract}
OEIS A220011 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=42$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220011 is ``Number of 2 X n arrays of the minimum value of corresponding elements and their horizontal, vertical or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 2 X n array.''. It has offset $1$ and begins
\[
4, 7, 19, 43, 85, 154, 263, 431,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/60)*n\^{}5 - (1/4)*n\^{}4 + (31/12)*n\^{}3 - (35/4)*n\^{}2 + (102/5)*n - 17 for n>2.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 03:30:36, revision 11)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=42$
...
states number $S=42$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220011 records is correct for all $n>2$.
\end{theorem}
...
$1 + 38 - 1$, which is more than the $6$ consecutive agreements a
...
First, the model reproduces all $38$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220011.
