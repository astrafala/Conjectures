...
\title{The empirical closed form for OEIS A219212, proved}
...
\begin{abstract}
OEIS A219212 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=112$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219212 is ``Number of n X 3 arrays of the minimum value of corresponding elements and their horizontal, vertical, diagonal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..2 n X 3 array.''. It has offset $1$ and begins
\[
6, 7, 37, 113, 345, 955, 2508, 6303,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 26 2025 14:42:23, revision 6)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=112$
...
states number $S=112$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219212 records is correct for all $n>11$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $28$.
Second, the polynomial and the walk count agree at every index from $12$ up to
$1 + 28 - 1$, which is more than the $17$ consecutive agreements a
...
First, the model reproduces all $28$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219212.
