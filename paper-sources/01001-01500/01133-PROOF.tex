...
\title{The empirical degree for OEIS A201620, proved from an exact model}
...
\begin{abstract}
OEIS A201620 carries the empirical claim that its terms agree with a polynomial of degree $63$.
...
recurrence of order $S=375$, so both are settled by a finite exact computation.
\end{abstract}
...
OEIS A201620 is ``Number of nX3 0..3 arrays with rows and columns lexicographically nondecreasing and every element equal to at least one horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
4, 56, 378, 3699, 31802, 256836, 1877283, 12466658,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) is a polynomial of degree 63 for n>35
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 09:13:00, revision 9)
...
with $M$ the adjacency matrix of that digraph on $S=375$ vertices and $\iota,\tau$ the vectors
...
\begin{theorem}
$a(n)$ agrees with a polynomial in $n$ of degree exactly $63$ for every $n\ge36$.
\end{theorem}

\begin{proof}
The residual of $(z-1)^{63+1}$ was formed at each index in exact integer arithmetic from
the model. It is nonzero at $n=99$ and vanishes from $n=99+1$ onwards, with the run of
zeros continuing past $S+63+1$, which by the annihilation bound settles every later index at
once. So $\Delta^{63+1}a$ vanishes on $n\ge99-63=36$, and
...
than $63$. The entry claims it for $n>35$, and the threshold above is exactly $35+63+1=99$, so the range proved is the range claimed and neither is slack.
\end{proof}
...
\url{https://oeis.org}, 2026. Sequence A201620.
