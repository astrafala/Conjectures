...
\title{The empirical closed form for OEIS A219579, proved}
...
\begin{abstract}
OEIS A219579 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=101$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219579 is ``Number of 2 X n arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..3 2 X n array.''. It has offset $1$ and begins
\[
4, 13, 51, 132, 277, 514, 881, 1429,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/40)*n\^{}5 - (5/24)*n\^{}4 + (79/24)*n\^{}3 - (91/24)*n\^{}2 + (161/60)*n - 1 for n>1.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 26 2018 08:07:23, revision 7)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=101$
...
states number $S=101$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219579 records is correct for all $n>1$.
\end{theorem}
...
$1 + 36 - 1$, which is more than the $6$ consecutive agreements a
...
First, the model reproduces all $36$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219579.
