...
\title{The empirical recurrence for OEIS A249847, proved from an exact model}
...
\begin{abstract}
OEIS A249847 carries an empirical recurrence of order $8$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = n\^{}7 + (6/5)*n\^{}6 + 3*n\^{}5 + 3*n\^{}4 + (5/6)*n\^{}3 + (4/5)*n\^{}2 + (1/6)*n. a(n) = 8*a(n-1) - 28*a(n-2) + 56*a(n-3) - 70*a(n-4) + 56*a(n-5) - 28*a(n-6) + 8*a(n-7) - a(n-8) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 18 2017 18:10:49, revision 10)
