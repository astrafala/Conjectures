...
\title{The empirical recurrence for OEIS A250085, proved from an exact model}
...
\begin{abstract}
OEIS A250085 carries an empirical recurrence of order $9$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = (3/28)*n\^{}8 + (121/35)*n\^{}7 + (85/4)*n\^{}6 + (501/10)*n\^{}5 + (249/4)*n\^{}4 + (1717/30)*n\^{}3 + (1159/28)*n\^{}2 + (1597/105)*n + 1. a(n) = 9*a(n-1) - 36*a(n-2) + 84*a(n-3) - 126*a(n-4) + 126*a(n-5) - 84*a(n-6) + 36*a(n-7) - 9*a(n-8) + a(n-9) for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 12 2018 03:00:27, revision 11)
...
$a$ and $S=10$. Nothing is fitted and no threshold is guessed: the $N_m$ are counted, not
estimated.
