...
\title{The empirical closed form for OEIS A219288, proved}
...
\begin{abstract}
OEIS A219288 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=1821$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219288 is ``Number of nX5 arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 nX5 array.''. It has offset $1$ and begins
\[
5, 12, 79, 423, 1751, 6546, 22970, 76311,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 17:57:29, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=1821$
...
states number $S=1821$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219288 records is correct for all $n>14$.
\end{theorem}
...
$1 + 25 - 1$, which is more than the $18$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219288.
