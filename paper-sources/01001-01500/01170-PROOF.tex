...
\title{The empirical recurrence for OEIS A202257, proved from an exact model}
...
\begin{abstract}
OEIS A202257 carries an empirical recurrence of order $25$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=127$, derived below and not
...
OEIS A202257 is ``Number of zero-sum -n..n arrays of 7 elements with adjacent element differences also in -n..n.''. It has offset $1$ and begins
\[
75, 1415, 10239, 45523, 150523, 408211, 960501, 2031401,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 15 2025 10:22:20, revision 18)
...
\]
annihilates $a$, and $S=\deg A=127$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=127$, and
...
$q(z)=z^{25}-\sum_ic_iz^{25-i}$ the characteristic polynomial of the conjectured
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-2) + a(n-3) - a(n-4) - 2\,a(n-5) - a(n-6) + a(n-8) + a(n-9) + a(n-10) + a(n-12) - a(n-13) - a(n-15) - a(n-16) - a(n-17) + a(n-19) + 2\,a(n-20) + a(n-21) - a(n-22) - a(n-23) - a(n-24) + a(n-25)
\]
for every $n>0$.
\end{theorem}
...
vanish from $n=0+1$ onwards, and the run exceeds $S+25$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A202257.
