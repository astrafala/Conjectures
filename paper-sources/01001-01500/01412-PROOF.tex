...
\title{The empirical recurrence for OEIS A221526, proved from an exact model}
...
\begin{abstract}
OEIS A221526 carries an empirical recurrence of order $7$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = 1*n\^{}6 - 20*n\^{}4 + 83*n\^{}3 - 182*n\^{}2 + 236*n - 148 for n>3. a(n) = 7*a(n-1) - 21*a(n-2) + 35*a(n-3) - 35*a(n-4) + 21*a(n-5) - 7*a(n-6) + a(n-7) for n>10.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 06 2018 07:25:13, revision 8)
...
\]
annihilates $a$, and $S=\deg A=19$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=19$, and
...
vanish from $n=10+1$ onwards, and the run exceeds $S+7$, which by
