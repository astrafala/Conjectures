...
\title{The empirical closed form for OEIS A219286, proved}
...
\begin{abstract}
OEIS A219286 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=30$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219286 is ``Number of n X 3 arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 n X 3 array.''. It has offset $1$ and begins
\[
3, 6, 19, 42, 79, 136, 220, 339,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/24)*n\^{}4 + (1/4)*n\^{}3 - (1/24)*n\^{}2 + (27/4)*n - 11 for n>2.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 25 2018 08:16:36, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=30$
...
states number $S=30$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219286 records is correct for all $n>2$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219286.
