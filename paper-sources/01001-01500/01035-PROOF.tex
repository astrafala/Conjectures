...
\title{Every term of OEIS A393859 is congruent to $1$ modulo $8$}
...
\begin{abstract}
OEIS A393859 is defined by the functional equation $A\bigl(x-xA(7x)/7\bigr)=x$ for its
ordinary generating function, and carries a conjecture of P.~D.~Hanna that every term is
congruent to $1$ modulo $8$. It is true, and the reason is that $7\equiv-1$ modulo
$8$. The functional equation determines the coefficients one at a time, so it determines
them modulo $8$ as well; and modulo $8$ the series $x/(1-x)$, all of whose coefficients
...
$\mathbb{Z}/8\mathbb{Z}$ and involves no estimation and no truncation.
\end{abstract}
...
OEIS A393859 is ``G.f. A(x) satisfies A( x - x*A(7*x)/7 ) = x.''. It has offset $1$ and begins
\[
1, 1, 9, 481, 167937, 404407577,\ \dots
\]
So, writing $A(x)=\sum_{n\ge1}a(n)x^{n}$, the entry defines $A$ by
\begin{equation}\label{eq:fe}
A\!\left(x-\frac{x\,A(7x)}{7}\right)\;=\;x .
\end{equation}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 12 2026 10:39:26, revision 8) the statement is
...
a(n)\;\equiv\;1 \pmod{8}\qquad\text{for every } n\ge1 .
\end{equation}
...
B(x)\;:=\;x-\frac{x\,A(7x)}{7}\;=\;x-\sum_{n\ge1}a(n)\,7^{\,n-1}x^{n+1},
\end{equation}
so $B$ has integer coefficients even though \eqref{eq:fe} appears to divide by $7$.
...
\section{The reduction modulo $8$}

Everything now happens in $(\mathbb{Z}/8\mathbb{Z})[[x]]$. The one fact that drives the proof is
\begin{equation}\label{eq:minus}
7\;\equiv\;-1 \pmod{8} .
\end{equation}

\begin{theorem}\label{thm:main}
$a(n)\equiv1\pmod{8}$ for every $n\ge1$. This is the conjecture of Section~1.
\end{theorem}

\begin{proof}
Let $\bar A(x)=\dfrac{x}{1-x}=\sum_{n\ge1}x^{n}$ in $(\mathbb{Z}/8\mathbb{Z})[[x]]$, so all its
coefficients are $1$ and $\bar A(x)=x+O(x^{2})$. By Corollary~\ref{cor:mod} it suffices to
check that $\bar A$ satisfies \eqref{eq:fe} modulo $8$.

Form $\bar B$ from $\bar A$ by \eqref{eq:B}. Its coefficients are
$\bigl[x^{n+1}\bigr]\bar B=-7^{\,n-1}$, and by \eqref{eq:minus}
$7^{\,n-1}\equiv(-1)^{n-1}$, so
\[
...
So $\bar A$ satisfies \eqref{eq:fe} modulo $8$, and Corollary~\ref{cor:mod} gives
$a(n)\equiv\bigl[x^{n}\bigr]\bar A=1\pmod{8}$ for every $n\ge1$.
\end{proof}

\begin{remark}
The proof used nothing about $7$ beyond \eqref{eq:minus}. The same argument shows that
...
the quantity $7^{\,n-1}$ must be $(-1)^{n-1}$ for the telescoping above, and that is
...
Second, the same run was continued to $n=24$ and every term reduced modulo $8$; all
...
independently as a formal power series over $\mathbb{Z}/8\mathbb{Z}$, coefficient by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A393859.
