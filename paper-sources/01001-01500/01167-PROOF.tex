...
\title{The empirical closed form for OEIS A220029, proved}
...
\begin{abstract}
OEIS A220029 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=44$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220029 is ``Number of n X 5 arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 n X 5 array.''. It has offset $1$ and begins
\[
5, 12, 30, 61, 111, 187, 297, 450,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/24)*n\^{}4 + (5/12)*n\^{}3 + (11/24)*n\^{}2 + (61/12)*n - 4 for n>1.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 05:51:38, revision 7)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=44$
...
states number $S=44$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220029 records is correct for all $n>1$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220029.
