...
\title{The empirical closed form for OEIS A220035, proved}
...
\begin{abstract}
OEIS A220035 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=247$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220035 is ``Number of 5 X n arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 5 X n array.''. It has offset $1$ and begins
\[
6, 10, 21, 55, 111, 192, 302, 442,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/24)*n\^{}4 - (1/12)*n\^{}3 + (95/24)*n\^{}2 + (289/12)*n - 132 for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 07:40:07, revision 7)
...
$5\times n$ arrays over that range whose rows are lexicographically nonincreasing read downwards
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=247$
...
states number $S=247$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220035 records is correct for all $n>6$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $11$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220035.
