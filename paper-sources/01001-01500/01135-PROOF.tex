...
\title{The empirical recurrence for OEIS A199849, proved from an exact model}
...
\begin{abstract}
OEIS A199849 carries an empirical recurrence of order $11$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=34$, derived below and not
...
OEIS A199849 is ``Number of -n..n arrays x(0..4) of 5 elements with zero sum and no element more than one greater than the previous.''. It has offset $1$ and begins
\[
26, 66, 132, 239, 399, 630, 944, 1367,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) -a(n-3) -2*a(n-5) +2*a(n-6) +a(n-8) -2*a(n-10) +a(n-11) for n>12.
\end{quote}
As of the ``Last modified'' line on the live entry (May 16 2018 16:34:59, revision 9)
...
\]
annihilates $a$, and $S=\deg A=34$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=34$, and
...
vanish from $n=12+1$ onwards, and the run exceeds $S+11$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A199849.
