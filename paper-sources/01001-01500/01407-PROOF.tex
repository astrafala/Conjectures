...
\title{The empirical recurrence for OEIS A200061, proved from an exact model}
...
\begin{abstract}
OEIS A200061 carries an empirical recurrence of order $22$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=97$, derived below and not
...
OEIS A200061 is ``Number of -n..n arrays x(0..6) of 7 elements with zero sum and elements alternately strictly increasing and strictly decreasing.''. It has offset $1$ and begins
\[
14, 472, 4224, 20978, 74338, 211242, 514168, 1115572,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 08:25:49, revision 9)
...
\]
annihilates $a$, and $S=\deg A=97$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=97$, and
...
vanish from $n=0+1$ onwards, and the run exceeds $S+22$, which by
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A200061.
