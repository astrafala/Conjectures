...
\title{The empirical closed form for OEIS A220037, proved}
...
\begin{abstract}
OEIS A220037 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=1739$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A220037 is ``Number of 7 X n arrays of the minimum value of corresponding elements and their horizontal or diagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 7 X n array.''. It has offset $1$ and begins
\[
8, 14, 36, 119, 297, 626, 1165, 1963,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/720)*n\^{}6 - (7/240)*n\^{}5 + (107/144)*n\^{}4 - (209/48)*n\^{}3 + (1609/45)*n\^{}2 + (1913/60)*n - 813 for n>9.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 30 2018 08:13:28, revision 7)
...
$7\times n$ arrays over that range whose rows are lexicographically nonincreasing read downwards
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=1739$
...
states number $S=1739$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A220037 records is correct for all $n>9$.
\end{theorem}
...
$1 + 35 - 1$, which is more than the $7$ consecutive agreements a
...
First, the model reproduces all $35$ terms the entry publishes, exactly. This is what
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220037.
