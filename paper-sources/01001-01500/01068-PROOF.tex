...
\title{The empirical closed form for OEIS A219768, proved}
...
\begin{abstract}
OEIS A219768 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=23$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219768 is ``Number of n X 3 arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and columns, 0..1 n X 3 array.''. It has offset $1$ and begins
\[
3, 6, 14, 29, 56, 101, 171, 274,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/24)*n\^{}4 + (1/4)*n\^{}3 - (25/24)*n\^{}2 + (23/4)*n - 4 for n>1.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 27 2018 08:56:55, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=23$
...
states number $S=23$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219768 records is correct for all $n>1$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219768.
