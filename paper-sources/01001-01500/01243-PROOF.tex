...
\title{The empirical closed form for OEIS A219501, proved}
...
\begin{abstract}
OEIS A219501 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=498$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219501 is ``Number of n X 7 arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..1 n X 7 array.''. It has offset $1$ and begins
\[
7, 13, 45, 126, 281, 550, 995, 1703,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/720)*n\^{}6 + (1/240)*n\^{}5 + (23/144)*n\^{}4 + (13/16)*n\^{}3 + (331/180)*n\^{}2 + (311/60)*n - 27 for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 26 2023 16:51:36, revision 5)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=498$
...
states number $S=498$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219501 records is correct for all $n>3$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $10$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219501.
