...
\title{The empirical recurrence for OEIS A240429, proved by transfer matrix}
...
\begin{abstract}
OEIS A240429 counts arrays over $\{0,1,2,3\}$ in which no entry may equal, modulo $4$, a given
...
count is then a walk count on $S=145$ states.
\end{abstract}
...
OEIS A240429 is ``Number of nX3 0..3 arrays with no element equal to zero plus the sum of elements to its left or zero plus the sum of elements above it or two plus the sum of the elements diagonally to its northwest, modulo 4.''. It has offset $1$ and begins
\[
4, 24, 206, 970, 8084, 38406, 319164, 1514122,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 09:57:47, revision 6) this is still
...
\text{northwest} & (i-1,j-1),(i-2,j-2),\dots & 2 \\
\end{array}
\]
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=145.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A240429.
