...
\title{The empirical recurrence for OEIS A279300, proved by counting patterns with a budget}
...
\begin{abstract}
OEIS A279300 imposes three things at once: a condition on each cell that reaches one row up and
...
from a finite exact computation on $S=42$ states.
\end{abstract}
...
OEIS A279300 is ``Number of n X 1 0..2 arrays with no element equal to a strict majority of its king-move neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
0, 1, 0, 3, 6, 24, 72, 232,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) - 6*a(n-2) - 16*a(n-3) + 12*a(n-4) + 24*a(n-5) + 8*a(n-6) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 21 2018 11:09:31, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) - 6\,a(n-2) - 16\,a(n-3) + 12\,a(n-4) + 24\,a(n-5) + 8\,a(n-6)
\end{equation}
for all $n>8$.

\section{The condition, the budget, and the canonical form}

The arrays have $1$ columns and $L=n$ rows; write $x_{t,u}$ for the entry in
...
Fix $i$, let $V_i=\{0,\dots,i-1\}^{1}$, and take the triples $(p,c,v)$ with
...
Assembling the $3$ chains block-diagonally into $N$ of size $S=42$ with the weights of
...
one of the $28$ published indices and agrees with the entry's DATA exactly, the quotient
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A279300.
