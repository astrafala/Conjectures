...
\title{The empirical recurrence for OEIS A229577, proved by counting defective colourings}
...
\begin{abstract}
OEIS A229577 counts the colourings of an array with $K=4$ colours that are proper except at
...
numbers. What is left is a walk count on $S=37400$ vertices, and the empirical recurrence of
order $4$ contributed by R.~H.~Hardin follows from a finite exact computation.
\end{abstract}
...
OEIS A229577 is ``Number of defective 4-colorings of an n X 7 0..3 array connected horizontally, vertically, diagonally and antidiagonally with exactly one mistake, and colors introduced in row-major 0..3 order.''. It has offset $1$ and begins
\[
246, 1312, 1584, 2064, 2852, 4048, 6108, 9992,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) - 13*a(n-2) + 12*a(n-3) - 4*a(n-4) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 19 2018 08:51:01, revision 7) the statement is
...
The arrays have $7$ columns and $L=n$ rows, entries in $\{0,\dots,4-1\}$.
...
Fix $i$ and take as vertices the pairs $(r,c)$ with $r\in\{0,\dots,i-1\}^{7}$ a row
...
Assembling the $4$ chains block-diagonally into $N$ of size $S=37400$ with the weights of
...
\[
4!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)=\mathbf{v}^{\!\top}G^{\,j-4}w,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A229577.
