...
\title{The empirical recurrence for OEIS A254702, proved from an exact model}
...
\begin{abstract}
OEIS A254702 carries an empirical recurrence of order $8$. It is true, and it is
...
\begin{quote}
Empirical: a(n) = (1/35)*n\^{}7 + (8/5)*n\^{}6 + (56/5)*n\^{}5 + 25*n\^{}4 + (249/10)*n\^{}3 + (77/5)*n\^{}2 + (481/70)*n + 1. a(n) = 8*a(n-1) - 28*a(n-2) + 56*a(n-3) - 70*a(n-4) + 56*a(n-5) - 28*a(n-6) + 8*a(n-7) - a(n-8) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 17 2018 09:43:44, revision 7)
