...
\title{The empirical recurrence for OEIS A184258, proved from an exact model}
...
\begin{abstract}
OEIS A184258 carries an empirical recurrence of order $29$. It is true, and it is
decidable rather than empirical. The entry's sequence is given exactly by an Ehrhart quasi-polynomial, from
which $a$ provably satisfies a MONIC linear recurrence of order $S=109$, derived below and not
...
OEIS A184258 is ``Number of strings of numbers x(i=1..3) in 0..n with sum i\^{}3*x(i) equal to 27*n.''. It has offset $1$ and begins
\[
1, 1, 2, 2, 2, 3, 3, 3,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) - a(n-2) + a(n-27) - 2*a(n-28) + a(n-29).
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 12 2018 11:20:38, revision 8)
...
\]
annihilates $a$, and $S=\deg A=109$. A congruence condition modulo $m$ is counted one residue
...
exactly $\deg A$. The bound in force is therefore $zA(z)$, of order $S=109$, and
...
$q(z)=z^{29}-\sum_ic_iz^{29-i}$ the characteristic polynomial of the conjectured
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) - a(n-2) + a(n-27) - 2\,a(n-28) + a(n-29)
\]
for every $n>0$.
\end{theorem}
...
vanish from $n=0+1$ onwards, and the run exceeds $S+29$, which by
...
First, the model reproduces all $69$ terms the entry publishes, exactly, in integer
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184258.
