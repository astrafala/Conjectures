...
\title{The empirical closed form for OEIS A219589, proved}
...
\begin{abstract}
OEIS A219589 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=12$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219589 is ``Number of n X 2 arrays of the minimum value of corresponding elements and their horizontal or vertical neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 n X 2 array.''. It has offset $1$ and begins
\[
3, 7, 21, 46, 87, 151, 247, 386,\ \dots
\]
and states:
\begin{quote}
Empirical: a(n) = (1/12)*n\^{}4 - (1/3)*n\^{}3 + (47/12)*n\^{}2 - (14/3)*n + 2 for n>1.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 12 2018 09:41:04, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=12$
...
states number $S=12$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219589 records is correct for all $n>1$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219589.
