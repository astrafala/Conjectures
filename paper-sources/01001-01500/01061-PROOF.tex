...
\title{The empirical closed form for OEIS A219854, proved}
...
\begin{abstract}
OEIS A219854 states an explicit closed form for $a(n)$ and records it as empirical. It is true, and
...
$S=130$ vertices, exact and finite. The closed form is a polynomial in $n$, so it satisfies the
...
OEIS A219854 is ``Number of 3Xn arrays of the minimum value of corresponding elements and their horizontal or antidiagonal neighbors in a random, but sorted with lexicographically nondecreasing rows and nonincreasing columns, 0..2 3Xn array.''. It has offset $1$ and begins
\[
10, 16, 65, 218, 591, 1437, 3195, 6668,\ \dots
\]
and states:
\begin{quote}
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 18:22:11, revision 8)
...
$a(n)$ is a walk count: there are a matrix $M$ with non-negative integer entries on $S=130$
...
states number $S=130$. The last filtered row is emitted with no row below it, so it is not a
...
\begin{theorem}
The closed form OEIS A219854 records is correct for all $n>9$.
\end{theorem}
...
$S$ consecutive zeros force every later one. The last index at which it fails is $19$.
Second, the polynomial and the walk count agree at every index from $10$ up to
$1 + 30 - 1$, which is more than the $10$ consecutive agreements a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A219854.
