...
\title{The empirical recurrence for OEIS A281062, proved by counting patterns with a budget}
...
\begin{abstract}
OEIS A281062 imposes three things at once: a condition on each cell that reaches one column up and
...
from a finite exact computation on $S=49155$ states.
\end{abstract}
...
OEIS A281062 is ``Number of 7Xn 0..1 arrays with no element equal to more than one of its horizontal and antidiagonal neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
0, 1686, 15028, 27635, 58005, 119205, 221212, 394884,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) -12*a(n-2) +5*a(n-3) +12*a(n-4) -12*a(n-5) -3*a(n-6) +6*a(n-7) -a(n-9) for n>20
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 13 2017 17:03:24, revision 4) the statement is
...
Assembling the $2$ chains block-diagonally into $N$ of size $S=49155$ with the weights of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A281062.
