...
\title{The empirical closed form for OEIS A250657, proved}
...
\begin{abstract}
OEIS A250657 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
OEIS A250657 is ``Number of (3+1)X(n+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
39, 70, 109, 156, 211, 274, 345, 424,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*n\^{}2 + 19*n + 16
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 12:39:41, revision 6) this is still
...
$n=3,\dots,5$ and agree at each. A monic recurrence of order $3$
determines $a(m)$ from $a(m-1),\dots,a(m-3)$, and for every $m\ge6$ both
$m>2$ and $m-3\ge3$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge3$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250657.
