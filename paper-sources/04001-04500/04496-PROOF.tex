...
\title{The conjectured generating function for OEIS A286773, proved}
...
\begin{abstract}
OEIS A286773 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (1 - x - 3*x\^{}2 + 8*x\^{}3 + 4*x\^{}4 - 8*x\^{}5) / ((1 - x)*(1 + x)*(1 - 2*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:39, revision 12) this is still
