...
\title{The conjectured generating function for OEIS A250774, proved}
...
\begin{abstract}
OEIS A250774 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $138$ coefficients in exact arithmetic
...
OEIS A250774 is ``Number of (6+1) X (n+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
288, 514, 820, 1234, 1812, 2666,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: 2*x*(144 - 463*x + 421*x\^{}2 - 128*x\^{}3) / ((1 - x)\^{}3*(1 - 2*x)). - \_Colin Barker\_, Nov 19 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 19 2018 09:13:45, revision 8) this is still
...
\]
a polynomial of degree at most $132$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{132}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $138$
of them --- more than the $132+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250774.
