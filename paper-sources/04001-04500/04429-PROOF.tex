...
\title{The conjectured generating function for OEIS A250846, proved}
...
\begin{abstract}
OEIS A250846 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250846 is ``Number of (n+1) X (1+1) 0..3 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
100, 543, 2670, 12311, 54410, 233683,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(100 - 457*x + 740*x\^{}2 - 384*x\^{}3) / ((1 - x)*(1 - 2*x)*(1 - 3*x)*(1 - 4*x)) (conjectured). - \_Colin Barker\_, Jan 18 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 18 2018 10:35:44, revision 7) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250846.
