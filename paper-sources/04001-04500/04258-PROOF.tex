...
\title{The empirical recurrence for OEIS A223283, proved by transfer matrix}
...
\begin{abstract}
OEIS A223283 counts the ways of writing the vertices of a fixed graph $G$ into an array of width
$2$ so that any two cells adjacent in one of the named directions carry vertices joined by an
edge of $G$; here $G$ is dodecahedron graph. The entry carries an empirical recurrence of order $2$
...
single slice is a state and the count is a walk count on a finite digraph with $S=154$ vertices.
\end{abstract}
...
OEIS A223283 is ``Rolling icosahedron face footprints: number of 2 X n 0..19 arrays starting with 0 where 0..19 label faces of an icosahedron and every array movement to a horizontal, diagonal or antidiagonal neighbor moves across an icosahedral edge.''. It has offset $1$ and begins
\[
20, 15, 87, 351, 1575, 6831, 29943, 130815,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) + 6*a(n-2) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 18 2018 17:33:51, revision 10) this is still
...
consecutive ones. So a $column$, a word of length $2$ over the vertices of $G$, carries
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=154.
\]
...
of length $2$ over $20$ vertices is one of $20^{2}$ words, but only $S-1$ of them are
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223283.
