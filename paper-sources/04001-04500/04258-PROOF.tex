...
\title{The empirical column recurrences for the table OEIS A253482}
...
\begin{abstract}
OEIS A253482 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A253482 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every 2X2 subblock ne-sw antidiagonal difference unequal to its neighbors horizontally and nw+se diagonal sum unequal to its neighbors vertically.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 558, 558, 3888, 6804, 3888, 27000, 85464,\ \dots
\]
...
k=2: [order 88]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 14:03:46, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..2 arrays with every 2X2 subblock ne-sw antidiagonal difference unequal to its neighbors horizontally and nw+se diagonal sum unequal to its neighbors vertically.''. Its rows are the vertices of a digraph on
$S=91$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;3\,a(n-1) + 24\,a(n-2) + 24\,a(n-3)
\]
...
model reproduces $7$ published values, $34$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253482.
