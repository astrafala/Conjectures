...
\title{The empirical product formula for OEIS A140941, proved}
...
\begin{abstract}
OEIS A140941 counts $9\times11$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $9\times11\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $99$. The entry's own expression, cleared of its denominator,
...
OEIS A140941 is ``Number of 9 X 11 matrices with elements in 0..n with each row and each column in nondecreasing order. 9,11,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 167960, 4936848280, 44648855844320, 165848175033726640, 303282730418598205184,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 28 2025 10:59:31, revision 8) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $9\times11$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$9\times11$ rectangle --- a plane partition in a $9\times11\times n$ box. The
correspondence is a bijection, being an involution on arrays, so
\[
a(n)\;=\;\mathrm{PP}(9,11,n).
\]
...
\[
a(n)\;=\;\prod_{i=1}^{9}\prod_{j=1}^{11}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $99$ with rational coefficients.
\end{theorem}
...
$9\cdot11=99$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,11,9)$ and claims
\[
a(n)\;=\;\prod_{i=0}^{9-1}\binom{s}{n+i}\,\frac{i!}{(s-i)^{9-1-i}}\;=\;
\frac{N(n)}{D(n)} .
\]

\begin{lemma}\label{lem:deg}
$N$ and $D$ are polynomials in $n$ of degrees $135$ and $36$ respectively.
\end{lemma}

\begin{proof}
With $s=n+11+9-1$ one has $s-(n+i)=11+9-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{11+9-1-i}
=\frac{(s-11-9+2+i)(s-11-9+3+i)\cdots s}{(11+9-1-i)!},
\]
a polynomial in $n$ of degree $11+9-1-i$. Multiplying over $i=0,\dots,9-1$ gives
$\deg N=\sum_{i=0}^{9-1}(11+9-1-i)=135$. The denominator is
$\prod_{i=0}^{9-1}(s-i)^{9-1-i}$, of degree
$\sum_{i=0}^{9-1}(9-1-i)=36$, and it does not vanish for $n\ge0$.
\end{proof}
...
of degree at most $99+36=135$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,135$, that is at $136$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $135$ has at most $135$ roots, so it is identically zero. Since $D(n)\neq0$
...
First, Theorem~\ref{thm:poly} reproduces all $7$ terms the entry publishes,
exactly, in integer arithmetic.

Second, the matrices themselves were counted for the smallest alphabets, directly from the
definition: the nondecreasing rows were listed and the chains of $9$ of them increasing
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,135$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140941.
