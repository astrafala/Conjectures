...
\title{The empirical recurrence for OEIS A189447, proved by transfer matrix}
...
\begin{abstract}
OEIS A189447 counts the permutations of the cells of an $n\times7$ array in which every cell
...
That makes $a$ a walk count on $S=2310$ states, hence $C$-finite, and whether the published
...
OEIS A189447 is ``Number of n X 7 array permutations with each element moving zero or one space horizontally or diagonally.''. It has offset $1$ and begins
\[
21, 3136, 313136, 33735505, 3586575040, 381987110556, 40668908306516, 4329976369716644,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 05:14:12, revision 9) this is still
...
cell by $\delta_17+\delta_2$ places in that order, so every image lies between $lo=-8$ and
$hi=8$ places from its source. Process the cells in order and let each one choose its image.
Once the cell at position $k$ has been dealt with, the position $k+-8$ can never be chosen
...
$k+-8,\dots,k+8$ are taken --- a subset of a set of size $17$, and nothing else.
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=2310,
\]
...
\begin{theorem}
\[
a(n)\;=\;119\,a(n-1) - 1238\,a(n-2) - 11504\,a(n-3) + 132848\,a(n-4) + 180912\,a(n-5) - 203520\,a(n-6) + 1542400\,a(n-7) + 2403200\,a(n-8) - 768000\,a(n-9) + 640000\,a(n-10)
\]
for every $n>14$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A189447.
