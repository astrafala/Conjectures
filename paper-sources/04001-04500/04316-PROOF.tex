...
\title{The empirical column recurrences for the table OEIS A263989}
...
\begin{abstract}
OEIS A263989 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A263989 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having index change +-(.,.) 0,0 or 1,2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 1, 4, 4, 1, 8, 16,\ \dots
\]
...
k=4: a(n) = 6*a(n-1)\\
k=5: a(n) = 9*a(n-1)\\
k=6: a(n) = 15*a(n-1) for n>2\\
k=7: a(n) = 25*a(n-1) for n>2\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 10:41:22, revision 6) these are still
...
column $k$$=1$ & $1$ & $16$ & $>1$ & $9$ \\
column $k$$=2$ & $1$ & $64$ & $>0$ & $8$ \\
column $k$$=3$ & $1$ & $256$ & $>0$ & $7$ \\
column $k$$=4$ & $1$ & $1024$ & $>1$ & $7$ \\
row $n$$=1$ & $1$ & $256$ & $>1$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having index change +-(.,.) 0,0 or 1,2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having index change +-(.,.) 0,0 or 1,2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(4) arrays of permutations of 0..(n+1)*(4)-1 with each element having index change +-(.,.) 0,0 or 1,2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(5) arrays of permutations of 0..(n+1)*(5)-1 with each element having index change +-(.,.) 0,0 or 1,2.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having index change +-(.,.) 0,0 or 1,2.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $41$ published values, $90$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263989.
