...
\title{The empirical recurrence for OEIS A275138, proved by counting patterns}
...
\begin{abstract}
OEIS A275138 counts arrays in which no entry may repeat a value found at any of three stated
...
cutting $6818$ pairs to $S=1223$ states. The entry's empirical recurrence of order $2$,
...
OEIS A275138 is ``Number of n X 4 0..2 arrays with no element equal to any value at offset (-2,-2) (-1,-2) or (0,-1) and new values introduced in order 0..2.''. It has offset $1$ and begins
\[
4, 48, 224, 1088, 5248, 25344, 122368, 590848,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) + 4*a(n-2) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 31 2019 19:36:41, revision 10) the statement is
...
$o=1n_0+0-2$, we have for $j\ge2$
\[
3!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)
=\mathbf{v}^{\!\top}G^{\,j-2}w,\qquad
w=h_{2}-\sum_i c_i h_{2-i} .
\]
...
the survivors by the pattern of the new pair. The vector $w$ was formed by $2$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A275138.
