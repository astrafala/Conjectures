...
\title{The empirical column recurrences for the table OEIS A264676}
...
\begin{abstract}
OEIS A264676 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264676 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change -1,-1 1,0 -1,-2 -2,-2 or 0,1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 1, 1, 1, 2, 0, 1, 10,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 20 2015 19:52:19, revision 4) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change -1,-1 1,0 -1,-2 -2,-2 or 0,1.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having directed index change -1,-1 1,0 -1,-2 -2,-2 or 0,1.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $21$ published values, $22$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264676.
