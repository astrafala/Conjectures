...
\title{The empirical column recurrences for the table OEIS A203934}
...
\begin{abstract}
OEIS A203934 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A203934 is ``T(n,k)=Number of (n+1)X(k+1) 0..5 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. It has offset $1$, is
read by antidiagonals, and begins
\[
396, 4356, 4356, 47916, 106596, 47916, 527076, 2543436,\ \dots
\]
The entry carries the following block:
\begin{quote}\small

\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:52:55, revision 9) these are still
...
The sequence is ``Number of (n+1)X(3) 0..5 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;21\,a(n-1) + 70\,a(n-2)
\]
holds for every $n>1$.

\subsection*{Column $k=3$}
The sequence is ``Number of (n+1)X(4) 0..5 arrays with every 2X2 subblock having equal diagonal elements or equal antidiagonal elements.''. Its rows are the vertices of a digraph on
$S=1296$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;61\,a(n-1) - 315\,a(n-2) + 50\,a(n-3)
\]
holds for every $n>2$.
...
values, $89$ decimal digits in all, every one of them exactly; a misreading of the entry
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203934.
