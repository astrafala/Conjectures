...
\title{The empirical recurrence for OEIS A208044, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A208044 counts arrays in which every cell is constrained by its neighbours on \emph{both}
...
order $2$, contributed by R.~H.~Hardin, then follows from a finite exact computation.
\end{abstract}
...
OEIS A208044 is ``Number of n X 3 0..3 arrays with new values 0..3 introduced in row major order and no element equal to any horizontal, vertical or antidiagonal neighbor (colorings ignoring permutations of colors).''. It has offset $1$ and begins
\[
2, 8, 44, 244, 1356, 7540, 41932, 233204,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 7*a(n-1) - 8*a(n-2) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 07 2018 02:31:01, revision 11) the statement is
...
equal to x_{t,u}\bigr\}\ \text{is not }=0
\end{equation}
for every cell $(t,u)$.
...
index,
\[
4!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}G^{\,j-2}\,
\Bigl(h_{2}-\sum_i c_i h_{2-i}\Bigr)
\]
...
$w=h_{2}-\sum_i c_ih_{2-i}$; and it is enough to check $m=0,1,\dots,S-1$.
\end{lemma}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208044.
