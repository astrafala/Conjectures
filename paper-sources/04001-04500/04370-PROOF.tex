...
\title{The empirical column recurrences for the table OEIS A253468}
...
\begin{abstract}
OEIS A253468 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A253468 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every 2X2 subblock diagonal maximum minus antidiagonal minimum unequal to its neighbors horizontally, vertically, diagonally and antidiagonally.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 540, 540, 3640, 800, 3640, 24208, 1400,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for diagonal and column k:\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 14:03:11, revision 6) these are still
...
column $k$$=1$ & $8$ & $91$ & $>8$ & $10$ \\
column $k$$=2$ & $6$ & $568$ & $>7$ & $10$ \\
column $k$$=3$ & $4$ & $3722$ & $>6$ & $6$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X2 0..2 arrays with every 2X2 subblock diagonal maximum minus antidiagonal minimum unequal to its neighbors horizontally, vertically, diagonally and antidiagonally.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;2\,a(n-1) + 27\,a(n-2) + 30\,a(n-3) - 7\,a(n-4) - 8\,a(n-5) - 75\,a(n-6) - 48\,a(n-7) - 16\,a(n-8)
\]
holds from index $8+1$ on.

\subsection*{Column $k$$=2$}
The sequence is ``Number of (n+1)X3 0..2 arrays with every 2X2 subblock diagonal maximum minus antidiagonal minimum unequal to its neighbors horizontally, vertically, diagonally and antidiagonally.''. Its rows are the vertices of a digraph on
$S=568$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of (n+1)X4 0..2 arrays with every 2X2 subblock diagonal maximum minus antidiagonal minimum unequal to its neighbors horizontally, vertically, diagonally and antidiagonally.''. Its rows are the vertices of a digraph on
$S=3722$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;5\,a(n-2) - 4\,a(n-4)
...
model reproduces $26$ published values, $130$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253468.
