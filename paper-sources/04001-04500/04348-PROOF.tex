...
\title{The conjectured generating function for OEIS A252873, proved}
...
\begin{abstract}
OEIS A252873 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=102$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $126$ coefficients in exact arithmetic
...
OEIS A252873 is ``Number of n X 5 nonnegative integer arrays with upper left 0 and lower right n+5-4 and value increasing by 0 or 1 with every step right or down.''. It has offset $1$ and begins
\[
6, 61, 413, 1953, 7313, 23203,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.: -x*(6 -5*x +72*x\^{}2 -225*x\^{}3 +460*x\^{}4 -612*x\^{}5 +537*x\^{}6 -315*x\^{}7 +121*x\^{}8 -28*x\^{}9 +3*x\^{}10) / (x-1)\^{}11. - \_R. J. Mathar\_, Nov 21 2015
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 22 2015 13:19:29, revision 9) this is still
...
\]
and there are $S=102$ states. The digraph is built by reading the entry's own English, and the
model reproduces all $25$ terms the entry publishes, exactly, in integer arithmetic --- a
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=11$ and $\deg D_2=11$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $113$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{113}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $126$
of them --- more than the $113+1$ the argument requires --- agree.
\end{proof}
...
\]
a constant-coefficient linear recurrence of order $11$, valid past the degree of the
numerator. Its coefficients are the integers $c_1,\dots,c_{11}$ read off $D_2$, the first few being $11,\ -55,\ 165,\ -330,\ 462,\ -462$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
First, the model reproduces every one of the $25$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252873.
