...
\title{The empirical column recurrences for the table OEIS A235310}
...
\begin{abstract}
OEIS A235310 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A235310 is ``T(n,k) is the number of (n+1) X (k+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 6 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
70, 210, 210, 518, 518, 518, 1554, 1106,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 18 2022 23:19:28, revision 7) these are still
...
The sequence is ``Number of (n+1) X (2) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 6 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235310.
