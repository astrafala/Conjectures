...
\title{The empirical column recurrences for the table OEIS A234564}
...
\begin{abstract}
OEIS A234564 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234564 is ``T(n,k) is the number of (n+1) X (k+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
70, 220, 220, 618, 546, 618, 1954, 1240,\ \dots
\]
...
k=1: a(n) = 3*a(n-1) +18*a(n-2) -54*a(n-3) -80*a(n-4) +240*a(n-5).\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 20:59:54, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234564.
