...
\title{The empirical recurrence for OEIS A209539, proved by transfer matrix}
...
\begin{abstract}
OEIS A209539 counts the permutations of the cells of an $n\times4$ array in which every cell
moves by one of a fixed short list of offsets. The entry carries an empirical recurrence of
order $1$ contributed by R.~H.~Hardin. It is true, and it is decidable rather than
...
That makes $a$ a walk count on $S=486$ states, hence $C$-finite, and whether the published
...
OEIS A209539 is ``Number of nX4 array permutations with every element making zero or one right-handed knight moves (out 2, right 1).''. It has offset $1$ and begins
\[
1, 4, 72, 729, 6480, 63504, 635040, 6350400,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 10*a(n-1) for n>6
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 15:19:42, revision 9) this is still
...
cell by $\delta_14+\delta_2$ places in that order, so every image lies between $lo=-9$ and
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=486,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A209539.
