...
\title{The conjectured generating function for OEIS A284238, proved}
...
\begin{abstract}
OEIS A284238 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (16*x\^{}5 - 24*x\^{}4 + 8*x\^{}3 + 2*x\^{}2 - x + 1)/((x - 1)*(2*x - 1)*(2*x + 1)). (End)
\end{quote}
