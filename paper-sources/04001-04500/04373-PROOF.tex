...
\title{The conjectured generating function for OEIS A280370, proved}
...
\begin{abstract}
OEIS A280370 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (1 + 2*x + x\^{}2 - 9*x\^{}3 - 2*x\^{}4 + 10*x\^{}5) / ((1 - x)*(1 + x)*(1 - 2*x)*(1 + 2*x)).
\end{quote}
