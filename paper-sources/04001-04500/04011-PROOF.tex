...
\title{The empirical column recurrences for the table OEIS A261380}
...
\begin{abstract}
OEIS A261380 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A261380 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00001011 00010101 or 01010101.''. It has offset $1$, is
read by antidiagonals, and begins
\[
36, 39, 39, 60, 41, 60, 97, 82,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-2) +3*a(n-4) +2*a(n-6) for n>12\\
k=2: a(n) = a(n-2) +6*a(n-4) +4*a(n-6) -a(n-8) for n>9\\
k=3: a(n) = a(n-2) +11*a(n-4) +13*a(n-6) -2*a(n-8) -2*a(n-10) +4*a(n-12) for n>17\\
k=4: [order 20] for n>22\\
k=5: [order 26] for n>29\\
k=6: [order 40] for n>44\\
k=7: [order 54] for n>59\\
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 17 2015 08:33:10, revision 4) these are still
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00001011 00010101 or 01010101.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+2)X(4) 0..1 arrays with each 3X3 subblock having clockwise perimeter pattern 00001011 00010101 or 01010101.''. Its rows are the vertices of a digraph on
...
model reproduces $20$ published values, $57$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A261380.
