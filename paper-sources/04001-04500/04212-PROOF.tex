...
\title{The empirical recurrence for OEIS A231779, proved by transfer matrix}
...
\begin{abstract}
OEIS A231779 counts arrays of width $2$ over $\{0,1,2\}$ in which no cell is beaten by a strict majority of its neighbours. The entry carries an
...
$3$ consecutive rows and the count is a walk count on $S=67$ states.
\end{abstract}
...
OEIS A231779 is ``Number of n X 2 0..2 arrays with no element having a strict majority of its horizontal and vertical neighbors equal to itself plus one mod 3, with upper left element zero (rock paper and scissors drawn positions).''. It has offset $1$ and begins
\[
1, 17, 74, 315, 1343, 5734, 24495, 104655,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) -8*a(n-2) +2*a(n-3) +3*a(n-4) -a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 18 2018 06:59:23, revision 8) this is still
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) - 8\,a(n-2) + 2\,a(n-3) + 3\,a(n-4) - a(n-5)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231779.
