...
\title{The conjectured generating function for OEIS A280367, proved}
...
\begin{abstract}
OEIS A280367 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A280367 is ``Binary representation of the x-axis, from the left edge to the origin, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 259", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 1, 11, 1000, 11, 111000,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 + x - 90*x\^{}2 + 899*x\^{}3 - 1000*x\^{}4 + 10100*x\^{}5) / ((1 - x)*(1 + x)*(1 - 10*x)*(1 + 10*x)).
\end{quote}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A280367.
