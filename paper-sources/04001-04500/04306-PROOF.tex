...
\title{The conjectured generating function for OEIS A283590, proved}
...
\begin{abstract}
OEIS A283590 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A283590 is ``Binary representation of the x-axis, from the origin to the right edge, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 657", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 0, 101, 111, 1111, 11111,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 - 11*x + 111*x\^{}2 - 1000*x\^{}3 + 900*x\^{}4) / ((1 - x)*(1 - 10*x)).
\end{quote}
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=4$ and $\deg D_2=2$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $10$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{10}$ then $R$ has a zero of order greater than its own
...
of them --- more than the $10+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{2}$ read off $D_2$, the first few being $11,\ -10$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A283590.
