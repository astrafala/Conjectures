...
\title{The empirical recurrence for OEIS A241125, proved by transfer matrix}
...
\begin{abstract}
OEIS A241125 counts arrays of width $2$ over $\{0,\dots,2\}$ under a condition on each cell
...
a walk count on $S=112$ of them, and the conjectured recurrence is then settled by a finite
exact computation.
\end{abstract}
...
OEIS A241125 is ``Number of n X 2 0..2 arrays with no element equal to exactly two horizontal and vertical neighbors, with new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
2, 9, 54, 325, 1965, 11876, 71793, 434007,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) + 2*a(n-2) -  9*a(n-3) - 10*a(n-4) + 8*a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 29 2018 20:16:25, revision 9) this is still
...
terms. There are $S=112$ states, and $a$ is $C$-finite.
...
first term. There are $3^{2}=9$ possible rows over the entry's own alphabet.
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) + 2\,a(n-2) - 9\,a(n-3) - 10\,a(n-4) + 8\,a(n-5)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A241125.
