...
\title{The empirical column recurrences for the table OEIS A234702}
...
\begin{abstract}
OEIS A234702 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234702 is ``T(n,k) is the number of (n+1) X (k+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 5 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
408, 2628, 2628, 16848, 13952, 16848, 109224, 74308,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 6*a(n-1) +90*a(n-2) -540*a(n-3) -2016*a(n-4) +12096*a(n-5).\\
k=2: [order 15].\\
k=3: [order 31].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 20:35:23, revision 6) these are still
...
The sequence is ``Number of (n+1) X (2) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 5 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
The entry gives only the order for this line: ``k=2: [order 15].''. Its model is a walk on
$S=512$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=1024$ exact terms returns the minimal one, of order
$15$ --- the stated order --- with integer coefficients
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $15$ & $c_{5}$ & $219360$ & $c_{9}$ & $161734320$ & $c_{13}$ & $-8926848000$ \\
$c_{2}$ & $125$ & $c_{6}$ & $-507740$ & $c_{10}$ & $-1006299360$ & $c_{14}$ & $-75188736000$ \\
$c_{3}$ & $-2865$ & $c_{7}$ & $-8437740$ & $c_{11}$ & $-1062547200$ & $c_{15}$ & $121927680000$ \\
$c_{4}$ & $-1490$ & $c_{8}$ & $34905816$ & $c_{12}$ & $13887417600$ &  &  \\
...
model reproduces $8$ published values, $48$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234702.
