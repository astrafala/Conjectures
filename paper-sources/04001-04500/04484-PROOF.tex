...
\title{The empirical recurrence for OEIS A232590, proved by transfer matrix}
...
\begin{abstract}
OEIS A232590 counts arrays of width $2$ over $\{0,1,2\}$ in which every cell sees its own value plus and minus one among its neighbours. The entry carries an
...
OEIS A232590 is ``Number of (1+1)X(n+1) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally, with no adjacent elements equal.''. It has offset $1$ and begins
\[
0, 10, 2, 26, 20, 70, 90, 210,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = -a(n-1) +2*a(n-2) +4*a(n-3) +3*a(n-4) +a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 07:31:19, revision 8) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232590.
