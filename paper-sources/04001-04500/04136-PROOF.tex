...
\title{The conjectured generating function for OEIS A277929, proved}
...
\begin{abstract}
OEIS A277929 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (1+x-5*x\^{}2+10*x\^{}3+4*x\^{}4-8*x\^{}5) / ((1-x)*(1+x)*(1-2*x)*(1+2*x)). (End)
\end{quote}
