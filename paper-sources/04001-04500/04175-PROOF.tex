...
\title{The empirical recurrence for OEIS A241329, proved by transfer matrix}
...
\begin{abstract}
OEIS A241329 counts $2\times n$ arrays over $\{0,\dots,3\}$ subject to a condition imposed on
every CELL over a neighbour set the entry names, and counted UP TO RELABELLING of the
alphabet; it carries an empirical recurrence of order $6$ contributed by
...
OEIS A241329 is ``Number of 2 X n 0..3 arrays with no element equal to fewer vertical neighbors than horizontal neighbors, with new values 0..3 introduced in row major order.''. It has offset $1$ and begins
\[
2, 8, 59, 530, 4877, 45057, 416533, 3851085,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 12*a(n-1) - 26*a(n-2) + 2*a(n-3) + 28*a(n-4) - 6*a(n-5) - 9*a(n-6) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 29 2018 15:51:05, revision 8) this is still
...
$\{0,\dots,3\}^{2}$. The neighbourhood of a cell in column $u_i$ meets $u_{i-1}$,
$u_i$ and $u_{i+1}$ and nothing else. Take as vertices the pairs
$(r,s)\in(\{0,\dots,3\}^{2}\cup\{\varnothing\})\times\{0,\dots,3\}^{2}$,
...
\[
a(n)\;=\;\frac{1}{24}\,\iota^{\!\top}M^{\,n-1}\tau ,
\]
...
\begin{theorem}
\[
a(n)\;=\;12\,a(n-1) - 26\,a(n-2) + 2\,a(n-3) + 28\,a(n-4) - 6\,a(n-5) - 9\,a(n-6)
\]
for every $n>8$.
\end{theorem}
...
By Lemma~\ref{lem:crit} the recurrence holds for every $n>8$. The bound is exact: at $n=8$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A241329.
