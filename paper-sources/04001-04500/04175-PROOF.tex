...
\title{The empirical column recurrences for the table OEIS A265960}
...
\begin{abstract}
OEIS A265960 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A265960 is ``T(n,k) = Number of n X k integer arrays with each element equal to the number of horizontal, vertical, diagonal and antidiagonal neighbors less than itself.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 3, 3, 5, 75, 5, 11, 235,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1) +a(n-2) +3*a(n-3) +a(n-4).\\
k=2: [order 30].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 22 2021 14:35:58, revision 6) these are still
...
The sequence is ``Number of n X 1 integer arrays with each element equal to the number of horizontal, vertical, diagonal and antidiagonal neighbors less than itself.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A265960.
