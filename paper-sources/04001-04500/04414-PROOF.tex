...
\title{The empirical column recurrences for the table OEIS A205318}
...
\begin{abstract}
OEIS A205318 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A205318 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with every 2X2 subblock having the same number of equal edges, and new values 0..1 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
8, 20, 20, 56, 84, 56, 164, 376,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:00:37, revision 10) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with every 2X2 subblock having the same number of equal edges, and new values 0..1 introduced in row major order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..1 arrays with every 2X2 subblock having the same number of equal edges, and new values 0..1 introduced in row major order.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X4 0..1 arrays with every 2X2 subblock having the same number of equal edges, and new values 0..1 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=21$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
The sequence is ``Number of (n+1)X5 0..1 arrays with every 2X2 subblock having the same number of equal edges, and new values 0..1 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=37$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;17\,a(n-1) - 81\,a(n-2) + 157\,a(n-3) - 140\,a(n-4) + 56\,a(n-5) - 8\,a(n-6)
\]
...
model reproduces $32$ published values, $146$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205318.
