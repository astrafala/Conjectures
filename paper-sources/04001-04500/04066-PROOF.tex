...
\title{The conjectured generating function for OEIS A252925, proved}
...
\begin{abstract}
OEIS A252925 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $92$ coefficients in exact arithmetic
...
OEIS A252925 is ``Number of n X 3 nonnegative integer arrays with upper left 0 and lower right n+3-6 and value increasing by 0 or 1 with every step right or down.''. It has offset $1$ and begins
\[
0, 0, 1, 33, 413, 2859,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.:  -x\^{}3*(1 +20*x +62*x\^{}2 -222*x\^{}3 +300*x\^{}4 -297*x\^{}5 +255*x\^{}6 -167*x\^{}7 +72*x\^{}8 -18*x\^{}9 +2*x\^{}10) / (x-1)\^{}13. - \_R. J. Mathar\_, Nov 21 2015
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 22 2015 13:19:46, revision 9) this is still
...
\]
a polynomial of degree at most $77$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{77}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $92$
of them --- more than the $77+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252925.
