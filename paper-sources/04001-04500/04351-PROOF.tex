...
\title{The empirical recurrence for OEIS A224557, proved by transfer matrix}
...
\begin{abstract}
OEIS A224557 counts the $(n+2)\times8$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=379$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224557 is ``Number of (n+2) X 8 0..1 matrices with each 3 X 3 subblock idempotent.''. It has offset $1$ and begins
\[
418, 351, 451, 551, 740, 992, 1298, 1742,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 6*a(n-2) + 5*a(n-3) - 4*a(n-4) + 3*a(n-5) - a(n-6) for n>8.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 31 2018 09:27:05, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $3\times3$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $3\times8$ matrix a \emph{band} when every one of its $6$ contiguous
$3\times3$ subblocks is a window. What the entry counts is the matrices of $8$ columns
whose every $3$ consecutive rows form a band.

Take as states the $2\times8$ strips that occur as the top $2$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=379$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{16}$ strips of $2$ rows and $8$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $3\times2$ entries, so a band is exactly a walk of $6$ windows in
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224557.
