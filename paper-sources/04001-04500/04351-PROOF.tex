...
\title{The conjectured generating function for OEIS A232582, proved}
...
\begin{abstract}
OEIS A232582 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=90$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $97$ coefficients in exact arithmetic
...
OEIS A232582 is ``Number of (n+1) X (1+1) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally, with no adjacent elements equal.''. It has offset $1$ and begins
\[
0, 2, 4, 6, 10, 18,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.: -2*x\^{}2 / ( -1+2*x-x\^{}2+x\^{}3 ). - \_R. J. Mathar\_, Nov 23 2014
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 21 2019 15:26:43, revision 19) this is still
...
\]
and there are $S=90$ states. The digraph is built by reading the entry's own English, and the
...
The family this entry belongs to is read by one model, described by its author as: ``Cell conditions over a named neighbour set, decided in a three-row window.''
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=2$ and $\deg D_2=3$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $92$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{92}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $97$
of them --- more than the $92+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{3}$ read off $D_2$, the first few being $2,\ -1,\ 1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232582.
