...
\title{The empirical column recurrences for the table OEIS A231343}
...
\begin{abstract}
OEIS A231343 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231343 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with no element unequal to a strict majority of its horizontal and vertical neighbors, with values 0..3 introduced in row major order.''. It has offset $1$, is
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:25:22, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..3 arrays with no element unequal to a strict majority of its horizontal and vertical neighbors, with values 0..3 introduced in row major order.''. Its rows are the vertices of a digraph on
...
model reproduces $10$ published values, $22$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231343.
