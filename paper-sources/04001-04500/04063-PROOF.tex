...
\title{The conjectured generating function for OEIS A252926, proved}
...
\begin{abstract}
OEIS A252926 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=140$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $175$ coefficients in exact arithmetic
...
OEIS A252926 is ``Number of n X 4 nonnegative integer arrays with upper left 0 and lower right n+4-6 and value increasing by 0 or 1 with every step right or down.''. It has offset $1$ and begins
\[
0, 1, 33, 615, 6997, 53950,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.: x\^{}2*(-1 -16*x -190*x\^{}2 -350*x\^{}3 +1419*x\^{}4 -3792*x\^{}5 +7860*x\^{}6 -11238*x\^{}7 +11247*x\^{}8 -8133*x\^{}9 +4280*x\^{}10 -1600*x\^{}11 +401*x\^{}12 -60*x\^{}13+4*x\^{}14) / (x-1)\^{}17. - \_R. J. Mathar\_, Nov 23 2015
\end{quote}
As of the ``Last modified'' line on the live entry (May 25 2024 21:53:34, revision 8) this is still
...
\]
and there are $S=140$ states. The digraph is built by reading the entry's own English, and the
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=16$ and $\deg D_2=17$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $156$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{156}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $175$
of them --- more than the $156+1$ the argument requires --- agree.
\end{proof}
...
\]
a constant-coefficient linear recurrence of order $17$, valid past the degree of the
numerator. Its coefficients are the integers $c_1,\dots,c_{17}$ read off $D_2$, the first few being $17,\ -136,\ 680,\ -2380,\ 6188,\ -12376$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252926.
