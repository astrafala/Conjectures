...
\title{The empirical recurrence for OEIS A231138, proved by transfer matrix}
...
\begin{abstract}
OEIS A231138 counts arrays of width $2$ over $\{0,1,2\}$ in which no cell matches a strict majority of its neighbours. The entry carries an
...
$3$ consecutive rows and the count is a walk count on $S=70$ states.
\end{abstract}
...
OEIS A231138 is ``Number of (n+1) X (1+1) 0..2 arrays with no element equal to a strict majority of its horizontal and vertical neighbors, with upper left element zero.''. It has offset $1$ and begins
\[
18, 100, 570, 3234, 18376, 104386, 593022, 3368932,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) - 11*a(n-3) + 4*a(n-4).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 26 2018 15:44:18, revision 9) this is still
...
\]
Strict means more than half, so a tie is allowed and a cell with no neighbours is under no condition. The entry adds that the top-left cell is $0$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231138.
