...
\title{The empirical column recurrences for the table OEIS A257447}
...
\begin{abstract}
OEIS A257447 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A257447 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with no 3x3 subblock diagonal sum 0 and no antidiagonal sum 3 and no row sum 1 and no column sum 1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
30, 36, 36, 39, 38, 39, 49, 41,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1) +2*a(n-3) -a(n-4) -a(n-6) for n>12\\
k=2: a(n) = 2*a(n-1) -a(n-2) +a(n-3) -a(n-4) for n>8\\
k=3: a(n) = 2*a(n-1) -a(n-2) +a(n-3) -a(n-4) for n>6\\
k=4: a(n) = 2*a(n-1) -a(n-2) +a(n-3) -a(n-4) for n>6\\
k=5: a(n) = 2*a(n-1) -a(n-2) +a(n-3) -a(n-4) for n>5\\
k=6: a(n) = 2*a(n-1) -a(n-2) +a(n-3) -a(n-4) for n>5\\
k=7: a(n) = 2*a(n-1) -a(n-2) +a(n-3) -a(n-4)\\
k=8: a(n) = 2*a(n-1) -a(n-2) +a(n-3) -a(n-4) for n>5\\
k=9: a(n) = 2*a(n-1) -a(n-2) +a(n-3) -a(n-4)\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:47:16, revision 6) these are still
...
column $k$$=1$ & $6$ & $64$ & $>12$ & $11$ \\
column $k$$=2$ & $4$ & $256$ & $>8$ & $10$ \\
column $k$$=3$ & $4$ & $1024$ & $>6$ & $9$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X(3) 0..1 arrays with no 3x3 subblock diagonal sum 0 and no antidiagonal sum 3 and no row sum 1 and no column sum 1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+2)X(4) 0..1 arrays with no 3x3 subblock diagonal sum 0 and no antidiagonal sum 3 and no row sum 1 and no column sum 1.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+2)X(5) 0..1 arrays with no 3x3 subblock diagonal sum 0 and no antidiagonal sum 3 and no row sum 1 and no column sum 1.''. Its rows are the vertices of a digraph on
...
model reproduces $30$ published values, $73$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A257447.
