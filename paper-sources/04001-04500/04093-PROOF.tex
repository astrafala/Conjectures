...
\title{The empirical column recurrences for the table OEIS A264313}
...
\begin{abstract}
OEIS A264313 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264313 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having directed index change 0,0 0,1 1,0 -1,-2 or 0,2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 6, 1, 20, 20, 1, 52, 139,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1)\\
k=2: a(n) = 2*a(n-1) +4*a(n-2) +4*a(n-3)\\
k=3: [order 15]\\
k=4: [order 21]\\
k=5: [order 28]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 11 2015 09:58:37, revision 4) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having directed index change 0,0 0,1 1,0 -1,-2 or 0,2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) arrays of permutations of 0..(n+1)*(3)-1 with each element having directed index change 0,0 0,1 1,0 -1,-2 or 0,2.''. Its rows are the vertices of a digraph on
$S=64$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-1) + 4\,a(n-2) + 4\,a(n-3)
\]
holds from index $1+1$ on.
...
$c_{1}$ & $7$ & $c_{5}$ & $34$ & $c_{9}$ & $125$ & $c_{13}$ & $17$ \\
$c_{2}$ & $6$ & $c_{6}$ & $54$ & $c_{10}$ & $-120$ & $c_{14}$ & $-6$ \\
$c_{3}$ & $-11$ & $c_{7}$ & $-16$ & $c_{11}$ & $83$ & $c_{15}$ & $1$ \\
$c_{4}$ & $-46$ & $c_{8}$ & $-85$ & $c_{12}$ & $-41$ &  &  \\
\end{tabular}\end{center}
Repeating with the first $15+4$ terms removed gives order $15$ again.

\subsection*{Column $k=4$, recovered}
The entry gives only the order for this line: ``k=4: [order 21]''. Its model is a walk on
...
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $16$ & $c_{7}$ & $622$ & $c_{13}$ & $10635$ & $c_{19}$ & $450$ \\
$c_{2}$ & $-15$ & $c_{8}$ & $-669$ & $c_{14}$ & $-9753$ & $c_{20}$ & $-162$ \\
$c_{3}$ & $0$ & $c_{9}$ & $1942$ & $c_{15}$ & $6511$ & $c_{21}$ & $27$ \\
$c_{4}$ & $-269$ & $c_{10}$ & $-5477$ & $c_{16}$ & $-4210$ &  &  \\
$c_{5}$ & $567$ & $c_{11}$ & $10021$ & $c_{17}$ & $2145$ &  &  \\
$c_{6}$ & $-632$ & $c_{12}$ & $-10882$ & $c_{18}$ & $-946$ &  &  \\
\end{tabular}\end{center}
...
model reproduces $15$ published values, $28$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264313.
