...
\title{The empirical column recurrences for the table OEIS A234882}
...
\begin{abstract}
OEIS A234882 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234882 is ``T(n,k) is the number of (n+1) X (k+1) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
32, 68, 68, 128, 148, 128, 268, 268,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 5*a(n-2) -4*a(n-4).\\
k=2: [order 9].\\
k=3: [order 11].\\
k=4: [order 19].\\
k=5: [order 23].\\
...
k=7: [order 35].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 20:29:15, revision 7) these are still
...
The sequence is ``Number of (n+1) X (2) 0..4 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
The entry gives only the order for this line: ``k=2: [order 9].''. Its model is a walk on
...
$c_{1}$ & $2$ & $c_{4}$ & $-29$ & $c_{7}$ & $-73$ &  &  \\
$c_{2}$ & $10$ & $c_{5}$ & $68$ & $c_{8}$ & $0$ &  &  \\
$c_{3}$ & $-21$ & $c_{6}$ & $22$ & $c_{9}$ & $22$ &  &  \\
\end{tabular}\end{center}
Repeating with the first $9+4$ terms removed gives order $9$ again.

\subsection*{Column $k=3$, recovered}
The entry gives only the order for this line: ``k=3: [order 11].''. Its model is a walk on
...
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $2$ & $c_{4}$ & $-38$ & $c_{7}$ & $-130$ & $c_{10}$ & $0$ \\
$c_{2}$ & $11$ & $c_{5}$ & $87$ & $c_{8}$ & $-16$ & $c_{11}$ & $-16$ \\
$c_{3}$ & $-23$ & $c_{6}$ & $46$ & $c_{9}$ & $78$ &  &  \\
\end{tabular}\end{center}
...
model reproduces $9$ published values, $29$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234882.
