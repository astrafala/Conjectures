...
\title{The empirical column recurrences for the table OEIS A234443}
...
\begin{abstract}
OEIS A234443 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A234443 is ``T(n,k) is the number of (n+1) X (k+1) 0..2 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1 (constant-stress 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
32, 120, 120, 448, 582, 448, 1680, 2804,\ \dots
\]
...
k=4: [order 45].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 21:04:20, revision 8) these are still
...
The sequence is ``Number of (n+1) X (2) 0..2 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X (3) 0..2 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1 (constant-stress 1 X 1 tilings).''. Its rows are the vertices of a digraph on
$S=27$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;5\,a(n-1) + 24\,a(n-2) - 130\,a(n-3) + 36\,a(n-4) + 80\,a(n-5) - 32\,a(n-6)
\]
...
The entry gives only the order for this line: ``k=3: [order 16].''. Its model is a walk on
...
$c_{1}$ & $0$ & $c_{5}$ & $0$ & $c_{9}$ & $0$ & $c_{13}$ & $0$ \\
$c_{2}$ & $119$ & $c_{6}$ & $76112$ & $c_{10}$ & $424128$ & $c_{14}$ & $58736$ \\
$c_{3}$ & $0$ & $c_{7}$ & $0$ & $c_{11}$ & $0$ & $c_{15}$ & $0$ \\
$c_{4}$ & $-4884$ & $c_{8}$ & $-295366$ & $c_{12}$ & $-242512$ & $c_{16}$ & $-4992$ \\
\end{tabular}\end{center}
Repeating with the first $16+4$ terms removed gives order $16$ again.

\subsection*{Column $k=4$, recovered}
The entry gives only the order for this line: ``k=4: [order 45].''. Its model is a walk on
...
$45$ --- the stated order --- with integer coefficients
\[
(c_1,\dots,c_{45})=(19,\ 174,\ -5225,\ -579,\ 557372,\ -1736348,\ -28633611,\ 147097160,\ 701879450,\ -5215474532,\ -6448644414,\ 90991389225,\ -13155453655,\ -911859068299,\ 785985063499,\ 5750480317192,\ -7719561708652,\ -23967944021653,\ 42175716887542,\ 67283824235584,\ -150636045987035,\ -125392746919991,\ 373892366562458,\ 141881972068085,\ -664306804785765,\ -55299787514109,\ 857132068732641,\ -106700542512030,\ -806720994198218,\ 225446251980190,\ 551556767133038,\ -222526751973132,\ -270104055145308,\ 138509404758088,\ 92036096585608,\ -57527289381280,\ -20591630958048,\ 15900557729248,\ 2635188748768,\ -2803132131904,\ -102351217472,\ 284333800832,\ -15435123840,\ -12584140800,\ 1510096896).
\]
Repeating with the first $45+4$ terms removed gives order $45$ again.
...
model reproduces $18$ published values, $89$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234443.
