...
\title{The empirical column recurrences for the table OEIS A252114}
...
\begin{abstract}
OEIS A252114 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A252114 is ``T(n,k)=Number of (n+2)X(k+2) 0..3 arrays with every 3X3 subblock row and column sum equal to 0 2 3 6 or 7 and every 3X3 diagonal and antidiagonal sum not equal to 0 2 3 6 or 7.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1417, 1929, 1929, 1568, 1424, 1568, 1600, 724,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: [linear recurrence of order 25] for n>33\\
k=2: a(n) = 6*a(n-3) -8*a(n-6) for n>13\\
k=3: a(n) = 6*a(n-3) -8*a(n-6) for n>11\\
k=4: a(n) = 4*a(n-3) for n>8\\
k=5: a(n) = 12*a(n-3) -32*a(n-6) for n>11\\
k=6: a(n) = 12*a(n-3) -32*a(n-6) for n>11\\
k=7: a(n) = 8*a(n-3) for n>8\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 10:48:25, revision 6) these are still
...
column $k$$=2$ & $6$ & $5654$ & $>13$ & $8$ \\
column $k$$=3$ & $6$ & $56522$ & $>11$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+2)X(4) 0..3 arrays with every 3X3 subblock row and column sum equal to 0 2 3 6 or 7 and every 3X3 diagonal and antidiagonal sum not equal to 0 2 3 6 or 7.''. Its rows are the vertices of a digraph on
$S=5654$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+2)X(5) 0..3 arrays with every 3X3 subblock row and column sum equal to 0 2 3 6 or 7 and every 3X3 diagonal and antidiagonal sum not equal to 0 2 3 6 or 7.''. Its rows are the vertices of a digraph on
$S=56522$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;6\,a(n-3) - 8\,a(n-6)
...
model reproduces $18$ published values, $65$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252114.
