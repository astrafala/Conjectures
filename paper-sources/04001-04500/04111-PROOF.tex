...
\title{The empirical closed form for OEIS A183902, proved}
...
\begin{abstract}
OEIS A183902 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=8$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
monic linear recurrence whose characteristic polynomial is
$\prod_j(x-\lambda_j)^{\deg p_j+1}$; here that polynomial is $q(x)=(x-1)^{8}$, of degree
$8$. Two things are then checked exactly: that the walk count satisfies the same
recurrence from some index on, which the Cayley--Hamilton theorem reduces to a finite
computation, and that the two sequences agree at $8$ consecutive indices past that
...
OEIS A183902 is ``Number of nondecreasing arrangements of n+3 numbers in 0..7 with each number being the sum mod 8 of three others.''. It has offset $1$ and begins
\[
5, 22, 243, 1160, 3347, 7416, 14376, 25584,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = (1/5040)*n\^{}7 + (7/720)*n\^{}6 + (29/144)*n\^{}5 + (329/144)*n\^{}4 + (1259/90)*n\^{}3 + (5167/180)*n\^{}2 - (6955/21)*n + 312 for n>4
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 02:08:53, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;\frac{n^{7}}{5040} + \frac{7 n^{6}}{720} + \frac{29 n^{5}}{144} + \frac{329 n^{4}}{144} + \frac{1259 n^{3}}{90} + \frac{5167 n^{2}}{180} - \frac{6955 n}{21} + 312.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=8$ vertices, and $u,w$ the
...
Here $q(x)=(x-1)^{8}$, of degree $8$; the closed form is a polynomial in $n$, so this is $(x-1)^{8}$, and the recurrence it encodes is
...
\begin{proof}
The vector $w'=q(M)w$ was formed by $8$ matrix--vector products in exact integer
...
identically. Both were then evaluated in exact arithmetic at the $8$ consecutive indices
$n=13,\dots,20$ and agree at each. A monic recurrence of order $8$
determines $a(m)$ from $a(m-1),\dots,a(m-8)$, and for every $m\ge21$ both
$m>12$ and $m-8\ge13$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge13$.
Below $n=13$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=13$ on. The remaining indices $n=5,\dots,12$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A183902.
