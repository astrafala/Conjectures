...
\title{The conjectured generating function for OEIS A290528, proved}
...
\begin{abstract}
OEIS A290528 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A290528 is ``Decimal representation of the diagonal from the origin to the corner of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 833", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 1, 5, 15, 3, 63,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 - 2*x\^{}2 + 10*x\^{}3 - 28*x\^{}4 + 48*x\^{}6 - 32*x\^{}7) / ((1 - x)*(1 - 2*x)*(1 + 2*x)*(1 - 2*x\^{}2)).
\end{quote}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{5}$ read off $D_2$, the first few being $1,\ 6,\ -6,\ -8,\ 8$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A290528.
