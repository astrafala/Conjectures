...
\title{The empirical recurrence for OEIS A206232, proved by transfer matrix}
...
\begin{abstract}
OEIS A206232 counts, up to renaming the letters, the arrays of width $3$ over at most $4$
letters in which every $2\times3$ and $3\times2$ subblock has exactly $2$ equal edges, an edge of such a window being one of the six steps of its perimeter cycle and an equal edge one whose two cells agree. It carries an empirical recurrence of order $8$ contributed by
...
OEIS A206232 is ``Number of (n+1)X3 0..3 arrays with every 2X3 or 3X2 subblock having exactly two equal edges, and new values 0..3 introduced in row major order.''. It has offset $1$ and begins
\[
60, 256, 1136, 5728, 31652, 170728, 943584, 5175034,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) +20*a(n-2) -14*a(n-3) -133*a(n-4) +95*a(n-5) +123*a(n-6) +9*a(n-7) -102*a(n-8) for n>10
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:49:04, revision 9) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The condition is that every $2\times3$ and $3\times2$ subblock has exactly $2$ equal edges, an edge of such a window being one of the six steps of its perimeter cycle and an equal edge one whose two cells agree.
...
$w_i$ cleared of denominators and $S=1560$ states in all. The walk index $j$ and the entry's own
...
\begin{theorem}
\[
a(n)\;=\;3\,a(n-1) + 20\,a(n-2) - 14\,a(n-3) - 133\,a(n-4) + 95\,a(n-5) + 123\,a(n-6) + 9\,a(n-7) - 102\,a(n-8)
\]
for every $n>10$.
\end{theorem}
...
the integers. The bound is exact: at $n=10$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206232.
