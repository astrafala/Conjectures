...
\title{The empirical column recurrences for the table OEIS A264195}
...
\begin{abstract}
OEIS A264195 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264195 is ``T(n,k)=Number of (n+1)X(k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having index change +-(.,.) 0,0 0,2 or 2,1.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 4, 2, 16, 18, 4, 36, 180,\ \dots
\]
...
k=5: [order 58]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 10 2015 11:12:03, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) arrays of permutations of 0..(n+1)*(2)-1 with each element having index change +-(.,.) 0,0 0,2 or 2,1.''. Its rows are the vertices of a digraph on
...
model reproduces $8$ published values, $13$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264195.
