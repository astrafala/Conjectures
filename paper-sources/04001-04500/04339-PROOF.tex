...
\title{The empirical recurrence for OEIS A208317, proved by counting patterns}
...
\begin{abstract}
OEIS A208317 counts arrays subject to a local condition \emph{and} to the clause ``new values
...
state space down from $36$ to $S=10$. The entry's empirical recurrence of order
...
OEIS A208317 is ``Number of n X 3 0..2 arrays with new values 0..2 introduced in row major order and no element equal to more than two of its immediate leftward or upward or left-upward diagonal neighbors.''. It has offset $1$ and begins
\[
5, 113, 2852, 71956, 1815496, 45806048, 1155714080, 29159359552,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 24*a(n-1) + 32*a(n-2) - 24*a(n-3) for n>4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 02 2018 04:53:33, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;24\,a(n-1) + 32\,a(n-2) - 24\,a(n-3)
\end{equation}
for all $n>4$.
...
Assembling the $3$ lumped chains into one block-diagonal matrix $N$ of size $S=10$ and
...
\end{equation}
The unlumped alphabet had $36$ rows in all; the lumped chain has $S=10$ states.
...
\[
3!\Bigl(a(n)-\sum_i c_i\,a(n-i)\Bigr)\;=\;\mathbf{v}^{\!\top}N^{\,m-3}q(N)\mathbf{1} .
\]
...
\eqref{eq:walk}, divided by $3!$, was evaluated at every one of the $16$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208317.
