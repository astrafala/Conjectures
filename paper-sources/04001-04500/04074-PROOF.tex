...
\title{The conjectured generating function for OEIS A278667, proved}
...
\begin{abstract}
OEIS A278667 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=19$ vertices, so its generating function is a rational function whose numerator
...
OEIS A278667 is ``Decimal representation of the x-axis, from the origin to the right edge, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 62", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 3, 3, 3, 3, 19,\ \dots
\]
...
\]
and there are $S=19$ states. The digraph is built by reading the entry's own English, and the
...
numerator. Its coefficients are the integers $c_1,\dots,c_{5}$ read off $D_2$, the first few being $2,\ 0,\ 0,\ 1,\ -2$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A278667.
