...
\title{The conjectured generating function for OEIS A283349, proved}
...
\begin{abstract}
OEIS A283349 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (1 - x - x\^{}2 + 2*x\^{}3 + x\^{}4 + 4*x\^{}6) / ((1 - x)*(1 - 2*x)*(1 + 2*x)).
\end{quote}
