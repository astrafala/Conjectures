...
\title{The empirical recurrence for OEIS A262450, proved by transfer matrix}
...
\begin{abstract}
OEIS A262450 counts arrays of width $4$ over $\{0,1\}$ whose rows, read as base-$2$
numbers, are divisible by $15$ and whose columns, read the same way, are divisible by
$15$. The entry carries an empirical recurrence of order $5$ contributed by
...
is then a walk count on $S=15$ states.
\end{abstract}
...
OEIS A262450 is ``Number of (n+3) X (1+3) 0..1 arrays with each row and column divisible by 15, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
2, 3, 5, 9, 18, 35, 69, 137,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + a(n-4) - 2*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 31 2018 20:23:36, revision 11) this is still
...
$15$ and every $\gamma_j$ be divisible by $15$.
...
So the vector $\bigl(\gamma_1\bmod 15,\dots,\gamma_{4}\bmod
15\bigr)$ is a state: at most $15^{4}$ of them, and each array row
...
which every entry is divisible by $15$ --- so the end vector is not the all-ones
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262450.
