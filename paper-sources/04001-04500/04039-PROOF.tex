...
\title{The conjectured generating function for OEIS A252872, proved}
...
\begin{abstract}
OEIS A252872 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=60$ vertices, so its generating function is a rational function whose numerator
...
OEIS A252872 is ``Number of n X 4 nonnegative integer arrays with upper left 0 and lower right n+4-4 and value increasing by 0 or 1 with every step right or down.''. It has offset $1$ and begins
\[
3, 26, 153, 615, 1953, 5281,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.: -x*(3-x+27*x\^{}2-78*x\^{}3+120*x\^{}4-110*x\^{}5+59*x\^{}6-17*x\^{}7+2*x\^{}8) / (x-1)\^{}9. - \_R. J. Mathar\_, Nov 23 2015
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 07 2019 11:32:36, revision 9) this is still
...
\]
and there are $S=60$ states. The digraph is built by reading the entry's own English, and the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252872.
