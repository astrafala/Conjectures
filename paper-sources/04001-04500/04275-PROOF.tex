...
\title{The empirical column recurrences for the table OEIS A206343}
...
\begin{abstract}
OEIS A206343 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A206343 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with every 2X2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. It has offset $1$, is
read by antidiagonals, and begins
\[
192, 96, 96, 455, 181, 455, 296, 232,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = a(n-1) +3*a(n-2) -3*a(n-3) for n>10\\
k=2: a(n) = a(n-1) +3*a(n-2) -3*a(n-3) for n>11\\
k=3: a(n) = a(n-1) +3*a(n-2) -3*a(n-3) for n>12\\
k=4: a(n) = a(n-1) +3*a(n-2) -3*a(n-3) for n>13\\
k=5: a(n) = a(n-1) +3*a(n-2) -3*a(n-3) for n>14\\
k=6: a(n) = a(n-1) +3*a(n-2) -3*a(n-3) for n>15\\
k=7: a(n) = a(n-1) +3*a(n-2) -3*a(n-3) for n>16\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 12:52:45, revision 9) these are still
...
column $k$$=1$ & $3$ & $192$ & $>10$ & $9$ \\
column $k$$=2$ & $3$ & $96$ & $>11$ & $8$ \\
column $k$$=3$ & $3$ & $455$ & $>12$ & $7$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X2 0..3 arrays with every 2X2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
$S=192$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+1)X3 0..3 arrays with every 2X2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X4 0..3 arrays with every 2X2 subblock having nonzero determinant and commuting with every horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
$S=455$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;a(n-1) + 3\,a(n-2) - 3\,a(n-3)
...
model reproduces $24$ published values, $83$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A206343.
