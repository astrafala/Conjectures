...
\title{The empirical column recurrences for the table OEIS A203984}
...
\begin{abstract}
OEIS A203984 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A203984 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with no 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..2 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
6, 24, 24, 96, 144, 96, 384, 864,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:56:07, revision 9) these are still
...
column $k$$=4$ & $4$ & $276$ & $>4$ & $8$ \\
column $k$$=5$ & $6$ & $794$ & $>6$ & $8$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(5) 0..2 arrays with no 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=276$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+1)X(6) 0..2 arrays with no 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=794$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $16$ published values, $123$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203984.
