...
\title{The conjectured generating function for OEIS A250770, proved}
...
\begin{abstract}
OEIS A250770 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250770 is ``Number of (2+1) X (n+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing absolute value of x(i,j)-x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
18, 34, 62, 114, 214, 410,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: 2*x*(9 - 19*x + 8*x\^{}2) / ((1 - x)\^{}2*(1 - 2*x)). - \_Colin Barker\_, Nov 19 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 19 2018 07:18:47, revision 11) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250770.
