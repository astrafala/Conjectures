...
\title{The empirical column recurrences for the table OEIS A233129}
...
\begin{abstract}
OEIS A233129 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
k=7: [order 14]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 12 2024 19:36:17, revision 8) these are still
...
The sequence is ``Number of n X 1 0..5 arrays with no element x(i,j) adjacent to itself or value 5-x(i,j) horizontally or vertically, top left element zero, and 1 appearing before 2 3 and 4, and 2 appearing before 3 in row major order.''. Its rows are the vertices of a digraph on
$S=3$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of n X 2 0..5 arrays with no element x(i,j) adjacent to itself or value 5-x(i,j) horizontally or vertically, top left element zero, and 1 appearing before 2 3 and 4, and 2 appearing before 3 in row major order.''. Its rows are the vertices of a digraph on
$S=2$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;16\,a(n-1) - 48\,a(n-2)
\]
holds from index $2+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of n X 3 0..5 arrays with no element x(i,j) adjacent to itself or value 5-x(i,j) horizontally or vertically, top left element zero, and 1 appearing before 2 3 and 4, and 2 appearing before 3 in row major order.''. Its rows are the vertices of a digraph on
$S=3$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;48\,a(n-1) - 448\,a(n-2) + 1024\,a(n-3)
\]
holds from index $3+1$ on.
...
model reproduces $23$ published values, $82$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233129.
