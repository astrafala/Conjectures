...
\title{The empirical closed form for OEIS A203979, proved}
...
\begin{abstract}
OEIS A203979 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=98$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
monic linear recurrence whose characteristic polynomial is
$\prod_j(x-\lambda_j)^{\deg p_j+1}$; here that polynomial is $q(x)=(x-9)$, of degree
...
OEIS A203979 is ``Number of (n+1)X4 0..2 arrays with no 2X2 subblock having equal diagonal elements or equal antidiagonal elements, and new values 0..2 introduced in row major order.''. It has offset $1$ and begins
\[
96, 864, 7776, 69984, 629856, 5668704, 51018336, 459165024,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 96*9\^{}(n-1)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:55:44, revision 9) this is still
...
walk, so with $M$ the adjacency matrix of that digraph on $S=98$ vertices, and $u,w$ the
...
Here $q(x)=(x-9)$, of degree $1$, and the recurrence it encodes is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203979.
