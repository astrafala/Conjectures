...
\title{The empirical closed form for OEIS A269497, proved}
...
\begin{abstract}
OEIS A269497 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=26$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A269497 is ``Number of length-6 0..n arrays with no repeated value differing from the previous repeated value by one.''. It has offset $1$ and begins
\[
40, 591, 3680, 14695, 44904, 114695, 257536, 524655,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = n\^{}6 + 6*n\^{}5 + 15*n\^{}4 + 8*n\^{}3 + 5*n\^{}2 + 6*n - 1.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 03 2016 03:51:54, revision 6) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;n^{6} + 6 n^{5} + 15 n^{4} + 8 n^{3} + 5 n^{2} + 6 n - 1.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=26$ vertices, and $u,w$ the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269497.
