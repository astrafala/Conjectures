...
\title{The conjectured generating function for OEIS A252874, proved}
...
\begin{abstract}
OEIS A252874 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=161$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $189$ coefficients in exact arithmetic
...
OEIS A252874 is ``Number of n X 6 nonnegative integer arrays with upper left 0 and lower right n+6-4 and value increasing by 0 or 1 with every step right or down.''. It has offset $1$ and begins
\[
10, 120, 949, 5281, 23203, 85801,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.: -x*(10 -10*x +169*x\^{}2 -556*x\^{}3 +1402*x\^{}4 -2404*x\^{}5 +2895*x\^{}6 -2526*x\^{}7 +1599*x\^{}8 -716*x\^{}9 +214*x\^{}10 -38*x\^{}11 +3*x\^{}12) / (x-1)\^{}13. - \_R. J. Mathar\_, Nov 21 2015
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 22 2015 13:19:24, revision 9) this is still
...
\]
and there are $S=161$ states. The digraph is built by reading the entry's own English, and the
model reproduces all $24$ terms the entry publishes, exactly, in integer arithmetic --- a
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=13$ and $\deg D_2=13$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $174$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{174}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $189$
of them --- more than the $174+1$ the argument requires --- agree.
\end{proof}
...
\]
a constant-coefficient linear recurrence of order $13$, valid past the degree of the
numerator. Its coefficients are the integers $c_1,\dots,c_{13}$ read off $D_2$, the first few being $13,\ -78,\ 286,\ -715,\ 1287,\ -1716$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
First, the model reproduces every one of the $24$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A252874.
