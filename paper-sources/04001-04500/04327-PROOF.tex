...
\title{The empirical column recurrences for the table OEIS A204754}
...
\begin{abstract}
OEIS A204754 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A204754 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with every 3X3 subblock having three equal elements in a row horizontally, vertically, diagonally or antidiagonally exactly three ways, and new values 0..1 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
40, 61, 61, 102, 46, 102, 188, 31,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for columns:\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 11:28:46, revision 9) these are still
...
The sequence is ``Number of (n+2)X(4) 0..1 arrays with every 3X3 subblock having three equal elements in a row horizontally, vertically, diagonally or antidiagonally exactly three ways, and new values 0..1 introduced in row major order.''. Its rows are the vertices of a digraph on
...
model reproduces $10$ published values, $23$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A204754.
