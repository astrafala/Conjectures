...
\title{The conjectured generating function for OEIS A282800, proved}
...
\begin{abstract}
OEIS A282800 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\[
1, 1, 111, 100, 11111, 10100,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 + 2*x)*(1 - x + 12*x\^{}2 - 25*x\^{}3 + 50*x\^{}4) / ((1 - x)*(1 + x)*(1 - 10*x)*(1 + 10*x)).
\end{quote}
