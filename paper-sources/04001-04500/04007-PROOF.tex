...
\title{The conjectured generating function for OEIS A220563, proved}
...
\begin{abstract}
OEIS A220563 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=5$ vertices, so its generating function is a rational function whose numerator
...
OEIS A220563 is ``Number of ways to reciprocally link elements of an 2 X n array either to themselves or to exactly one horizontal or antidiagonal neighbor.''. It has offset $1$ and begins
\[
1, 5, 14, 47, 149, 481,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(1 + 3*x - x\^{}3) / ((1 + x)*(1 - 3*x - x\^{}2 + x\^{}3)). - \_Colin Barker\_, Jul 31 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 01 2024 23:39:20, revision 23) this is still
...
\]
and there are $S=5$ states. The digraph is built by reading the entry's own English, and the
...
The family this entry belongs to is read by one model, described by its author as: ``Transfer matrix for the Hardin RECIPROCAL LINK families.''
...
numerator. Its coefficients are the integers $c_1,\dots,c_{4}$ read off $D_2$, the first few being $2,\ 4,\ 0,\ -1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A220563.
