...
\title{The empirical recurrence for OEIS A189145, proved by transfer matrix}
...
\begin{abstract}
OEIS A189145 counts the permutations of the cells of an $n\times2$ array in which every cell
...
That makes $a$ a walk count on $S=24$ states, hence $C$-finite, and whether the published
...
OEIS A189145 is ``Number of n X 2 array permutations with each element making zero or one knight moves.''. It has offset $1$ and begins
\[
1, 1, 4, 16, 36, 81, 225, 625,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-1) -3*a(n-2) +6*a(n-3) -6*a(n-5) +3*a(n-6) -3*a(n-7) +a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (May 06 2026 21:58:45, revision 28) this is still
...
cell by $\delta_12+\delta_2$ places in that order, so every image lies between $lo=-5$ and
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A189145.
