...
\title{The conjectured generating function for OEIS A250756, proved}
...
\begin{abstract}
OEIS A250756 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250756 is ``Number of (1+1) X (n+1) 0..2 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing x(i,j)+x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
32, 72, 129, 203, 294, 402,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(32 - 24*x + 9*x\^{}2) / (1 - x)\^{}3.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 17 2018 20:52:53, revision 8) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250756.
