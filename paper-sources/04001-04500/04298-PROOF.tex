...
\title{The conjectured generating function for OEIS A259216, proved}
...
\begin{abstract}
OEIS A259216 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A259216 is ``Number of (n+1) X (2+1) 0..1 arrays with each 2 X 2 subblock having clockwise pattern 0000 0011 or 0101.''. It has offset $1$ and begins
\[
13, 23, 40, 71, 127, 230,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(13 - 16*x - 16*x\^{}2) / ((1 - 2*x)*(1 - x - x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 24 2026 09:41:49, revision 20) this is still
...
numerator. Its coefficients are the integers $c_1,\dots,c_{3}$ read off $D_2$, the first few being $3,\ -1,\ -2$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A259216.
