...
\title{The empirical column recurrences for the table OEIS A238515}
...
\begin{abstract}
OEIS A238515 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A238515 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with no element equal to all horizontal neighbors or equal to all vertical neighbors, and new values 0..3 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
4, 25, 25, 295, 401, 295, 3113, 14407,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 10*a(n-1) +13*a(n-2) -44*a(n-3) -7*a(n-4) +14*a(n-5)\\
k=2: [order 14]\\
k=3: [order 9]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:52:50, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..3 arrays with no element equal to all horizontal neighbors or equal to all vertical neighbors, and new values 0..3 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=204$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;10\,a(n-1) + 13\,a(n-2) - 44\,a(n-3) - 7\,a(n-4) + 14\,a(n-5)
\]
...
model reproduces $7$ published values, $28$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A238515.
