...
\title{The empirical column recurrences for the table OEIS A235212}
...
\begin{abstract}
OEIS A235212 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A235212 is ``T(n,k) is the number of (n+1) X (k+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 5, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset $1$, is
read by antidiagonals, and begins
\[
240, 1172, 1172, 5696, 4616, 5696, 27812, 18444,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 3*a(n-1) +35*a(n-2) -81*a(n-3) -264*a(n-4) +144*a(n-5) +384*a(n-6).\\
k=2: [order 29].\\
k=3: [order 62].\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 00:09:30, revision 7) these are still
...
The sequence is ``Number of (n+1) X (2) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 5, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. Its rows are the vertices of a digraph on
$S=56$ vertices, edges being the admissible consecutive pairs, and the count is
...
The entry gives only the order for this line: ``k=2: [order 29].''. Its model is a walk on
$S=392$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=784$ exact terms returns the minimal one, of order
$29$ --- the stated order --- with integer coefficients
\[
(c_1,\dots,c_{29})=(15,\ 23,\ -1373,\ 3547,\ 50847,\ -236177,\ -957405,\ 6739749,\ 8784137,\ -112122375,\ -8734283,\ 1205207433,\ -682793243,\ -8845512699,\ 8015164257,\ 45997463512,\ -47860413548,\ -173398534628,\ 175140255652,\ 476432770592,\ -402722060848,\ -936013467040,\ 551524974720,\ 1245278007168,\ -366681768960,\ -999476834304,\ 6506053632,\ 362586931200,\ 89712230400).
\]
...
model reproduces $8$ published values, $43$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235212.
