...
\title{The empirical column recurrences for the table OEIS A205193}
...
\begin{abstract}
OEIS A205193 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=3, 4, 5, 6$.
\end{abstract}
...
OEIS A205193 is ``T(n,k) = Number of (n+1) X (k+1) 0..1 arrays with the number of clockwise edge increases in every 2 X 2 subblock differing from each horizontal or vertical neighbor.''. It has offset $1$, is
read by antidiagonals, and begins
\[
16, 24, 24, 48, 40, 48, 72, 64,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 12 2022 19:52:57, revision 7) these are still
...
column $k$$=3$ & $4$ & $48$ & $>5$ & $8$ \\
column $k$$=4$ & $4$ & $72$ & $>6$ & $7$ \\
column $k$$=5$ & $6$ & $144$ & $>9$ & $8$ \\
column $k$$=6$ & $6$ & $216$ & $>10$ & $8$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1) X4 0..1 arrays with the number of clockwise edge increases in every 2 X 2 subblock differing from each horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
$S=48$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of (n+1) X5 0..1 arrays with the number of clockwise edge increases in every 2 X 2 subblock differing from each horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X6 0..1 arrays with the number of clockwise edge increases in every 2 X 2 subblock differing from each horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
$S=144$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;2\,a(n-2) + a(n-6)
...
The sequence is ``Number of (n+1) X7 0..1 arrays with the number of clockwise edge increases in every 2 X 2 subblock differing from each horizontal or vertical neighbor.''. Its rows are the vertices of a digraph on
$S=216$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $31$ published values, $95$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A205193.
