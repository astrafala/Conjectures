...
\title{The empirical closed form for OEIS A188865, proved}
...
\begin{abstract}
OEIS A188865 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$\prod_j(x-\lambda_j)^{\deg p_j+1}$; here that polynomial is $q(x)=(x-1)^{2}$, of degree
$2$. Two things are then checked exactly: that the walk count satisfies the same
recurrence from some index on, which the Cayley--Hamilton theorem reduces to a finite
computation, and that the two sequences agree at $2$ consecutive indices past that
...
OEIS A188865 is ``Number of n X 8 binary arrays without the pattern 0 1 diagonally, vertically or antidiagonally.''. It has offset $1$ and begins
\[
256, 1393, 3194, 5275, 7442, 9627, 11814, 14001,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2187*n - 3495 for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 28 2018 11:41:22, revision 9) this is still
...
Here $q(x)=(x-1)^{2}$, of degree $2$; the closed form is a polynomial in $n$, so this is $(x-1)^{2}$, and the recurrence it encodes is
...
\begin{proof}
The vector $w'=q(M)w$ was formed by $2$ matrix--vector products in exact integer
arithmetic, and $u^{\!\top}M^{j}w'$ evaluated for $j=0,1,\dots$, again exactly. Those integers
vanish from the point corresponding to $n=8$ onwards, and the run was continued until the
working vector was identically zero, which settles every larger $j$ at once. So $a$ satisfies
\eqref{eq:rec} for every $n>7$. By Lemma~\ref{lem:ann} $f$ satisfies \eqref{eq:rec}
identically. Both were then evaluated in exact arithmetic at the $2$ consecutive indices
$n=8,\dots,9$ and agree at each. A monic recurrence of order $2$
determines $a(m)$ from $a(m-1),\dots,a(m-2)$, and for every $m\ge10$ both
$m>7$ and $m-2\ge8$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge8$.
Below $n=8$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=8$ on. The remaining indices $n=6,\dots,7$ were
...
First, the walk model reproduces every one of the $37$ terms the entry publishes,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A188865.
