...
\title{The empirical column recurrences for the table OEIS A263288}
...
\begin{abstract}
OEIS A263288 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A263288 is ``T(n,k)=Number of (n+3)X(k+3) 0..1 arrays with each row divisible by 11 and column not divisible by 11, read as a binary number with top and left being the most significant bits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 42, 0, 504, 156, 0, 6960, 3904,\ \dots
\]
...
k=2: [order 29]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 13 2015 12:22:38, revision 4) these are still
...
The sequence is ``Number of (n+3)X4 0..1 arrays with each row divisible by 11 and column not divisible by 11, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
model reproduces $7$ published values, $7$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263288.
