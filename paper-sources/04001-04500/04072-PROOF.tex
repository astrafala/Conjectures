...
\title{The empirical recurrence for OEIS A274729, proved by counting patterns}
...
\begin{abstract}
OEIS A274729 counts arrays in which no entry may repeat a value found at any of three stated
...
cutting $6818$ pairs to $S=1223$ states. The entry's empirical recurrence of order $6$,
...
OEIS A274729 is ``Number of 4 X n 0..2 arrays with no element equal to any value at offset (-1,-2) (-2,-1) or (-1,0) and new values introduced in order 0..2.''. It has offset $1$ and begins
\[
4, 48, 260, 1632, 10368, 66132, 421904, 2691884,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 8*a(n-1) - 10*a(n-2) - 4*a(n-3) + 13*a(n-4) - 7*a(n-5) + a(n-6) for n>7.
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 30 2019 10:55:43, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;8\,a(n-1) - 10\,a(n-2) - 4\,a(n-3) + 13\,a(n-4) - 7\,a(n-5) + a(n-6)
\end{equation}
for all $n>7$.
...
$o=1n_0+0-2$, we have for $j\ge6$
\[
3!\Bigl(a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\Bigr)
=\mathbf{v}^{\!\top}G^{\,j-6}w,\qquad
w=h_{6}-\sum_i c_i h_{6-i} .
\]
...
the survivors by the pattern of the new pair. The vector $w$ was formed by $6$
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A274729.
