...
\title{The empirical recurrence for OEIS A224687, proved by transfer matrix}
...
\begin{abstract}
OEIS A224687 counts the $(n+4)\times9$ matrices over $\{0,\dots,1\}$ all of whose
...
makes $a(n)$ a walk count on $S=4680$ vertices, hence $C$-finite, and whether the conjectured
...
OEIS A224687 is ``Number of (n+4) X 9 0..1 matrices with each 5 X 5 subblock idempotent.''. It has offset $1$ and begins
\[
4405, 1938, 2136, 2429, 2700, 2926, 3540, 4514,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 4*a(n-1) - 6*a(n-2) + 4*a(n-3) - a(n-4) + a(n-5) - 3*a(n-6) + 3*a(n-7) - a(n-8) for n>12.
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 03 2018 11:37:31, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

Call a $5\times5$ matrix over $\{0,\dots,1\}$ a \emph{window} when it is idempotent, and
call a $5\times9$ matrix a \emph{band} when every one of its $5$ contiguous
$5\times5$ subblocks is a window. What the entry counts is the matrices of $9$ columns
whose every $5$ consecutive rows form a band.

Take as states the $4\times9$ strips that occur as the top $4$ rows of a band,
...
\]
with $M$ the adjacency matrix on $S=4680$ states. In particular $a$ is $C$-finite. The
alignment was checked against the entry's published terms rather than assumed.

There are $(1 + 1)^{36}$ strips of $4$ rows and $9$ columns, far too
many to test one by one, and they are not tested one by one. Two consecutive windows of a band
overlap in $5\times4$ entries, so a band is exactly a walk of $5$ windows in
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A224687.
