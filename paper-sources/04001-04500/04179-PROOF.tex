...
\title{The empirical closed form, generating function and recurrence for OEIS A195236, proved}
...
\begin{abstract}
OEIS A195236 counts the lower triangles of a $6\times6$ array over $\{0,\dots,n\}$ in which
...
OEIS A195236 is ``Number of lower triangles of a 6 X 6 0..n array with each element differing from all of its diagonal, vertical, antidiagonal and horizontal neighbors by one or less.''. It has offset $1$ and begins
\[
2097152, 10322065, 20729610, 31374671, 42029278, 52683885,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 06 2018 12:18:41, revision 13) these are still
...
Every $y\in\mathcal P$ has $\mathrm{sp}(y)\le 1\cdot5=5$, where $5$ is the
...
entry differ by at most $1\cdot5$.
\end{proof}
...
\[
a(n)\;=\;10654607\,n-11243757\qquad\text{for every }n\ge4,
\]
and $\mathcal P$ has exactly $10654607$ elements.
\end{theorem}

\begin{proof}
For $n\ge5$ every term of Lemma~\ref{lem:trans} is positive by
Lemma~\ref{lem:bound}, so
\[
a(n)=\sum_{y\in\mathcal P}\bigl(n+1-\mathrm{sp}(y)\bigr)
=|\mathcal P|\,(n+1)-\sum_{y}\mathrm{sp}(y),
\]
which is a linear function of $n$. Two values determine it, and $a(5)$ and
$a(5+1)$ were computed exactly, giving slope $10654607$ and the stated intercept; the slope
is $|\mathcal P|$. That the identity already holds from $n=4$ was then read off by
comparing it with the exact values of $a(n)$ below $5$, each computed the same way.
\end{proof}
...
vanishes, which for a linear function it does; it therefore holds for every $n\ge4+2$.
...
First, the count reproduces all $22$ terms the entry publishes, exactly, in integer
arithmetic.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A195236.
