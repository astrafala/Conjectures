...
\title{The empirical recurrence for OEIS A284244, proved from an exact model}
...
\begin{abstract}
OEIS A284244 carries an empirical recurrence of order $6$. It is true, and it is
...
\[
1, 0, 110, 1110, 11110, 101110, 101110, 10111110,\ \dots
\]
...
vanish from $n=16+1$ onwards, and the run exceeds $S+6$, which by
Lemma~\ref{lem:crit} settles every larger index at once. The bound is exact: at $n=16$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
