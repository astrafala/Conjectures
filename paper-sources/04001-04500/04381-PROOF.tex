...
\title{The conjectured generating function for OEIS A242543, proved}
...
\begin{abstract}
OEIS A242543 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=36$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $46$ coefficients in exact arithmetic
...
OEIS A242543 is ``Number of length n+3 0..2 arrays with no four elements in a row with pattern aabb (possibly a=b) and new values 0..2 introduced in 0..2 order.''. It has offset $1$ and begins
\[
12, 32, 88, 242, 660, 1800,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical: G.f.: -2*x*(6+4*x+12*x\^{}2+9*x\^{}3) / ( (2*x\^{}2+2*x-1)*(2*x\^{}2+1) ). - \_R. J. Mathar\_, May 18 2014
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 11:17:06, revision 8) this is still
...
\]
and there are $S=36$ states. The digraph is built by reading the entry's own English, and the
...
The family this entry belongs to is read by one model, described by its author as: ``One-dimensional words under a window condition, with values introduced in order.''
...
\]
a polynomial of degree at most $40$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{40}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $46$
of them --- more than the $40+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{4}$ read off $D_2$, the first few being $2,\ 0,\ 4,\ 4$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A242543.
