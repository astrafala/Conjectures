...
\title{The conjectured generating function for OEIS A285644, proved}
...
\begin{abstract}
OEIS A285644 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\[
1, 1, 100, 11, 10000, 1111,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 + x - x\^{}2 - 90*x\^{}3 + 100*x\^{}5 + 100000*x\^{}6 - 100000*x\^{}8) / ((1 - x)*(1 + x)*(1 - 10*x)*(1 + 10*x)).
\end{quote}
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=8$ and $\deg D_2=4$; its expansion as a
