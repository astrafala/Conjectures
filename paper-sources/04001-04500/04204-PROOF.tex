...
\title{The empirical column recurrences for the table OEIS A253749}
...
\begin{abstract}
OEIS A253749 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A253749 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every 2X2 subblock ne-sw antidiagonal difference nondecreasing horizontally and nw+se diagonal sum nondecreasing vertically.''. It has offset $1$, is
read by antidiagonals, and begins
\[
81, 450, 450, 1998, 2723, 1998, 7803, 10182,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 14:12:53, revision 8) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with every 2X2 subblock ne-sw antidiagonal difference nondecreasing horizontally and nw+se diagonal sum nondecreasing vertically.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) 0..2 arrays with every 2X2 subblock ne-sw antidiagonal difference nondecreasing horizontally and nw+se diagonal sum nondecreasing vertically.''. Its rows are the vertices of a digraph on
$S=81$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $15$ published values, $65$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253749.
