...
\title{The empirical column recurrences for the table OEIS A231070}
...
\begin{abstract}
OEIS A231070 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231070 is ``T(n,k)=Number of black square subarrays of (n+1)X(k+1) binary arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with upper left element zero.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 1, 1, 2, 3, 2, 3, 4,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:14:49, revision 6) these are still
...
column $k$$=1$ & $2$ & $10$ & $>2$ & $11$ \\
column $k$$=2$ & $6$ & $18$ & $>5$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of black square subarrays of (n+1)X2 binary arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with upper left element zero.''. Its rows are the vertices of a digraph on
$S=10$ vertices, edges being the admissible consecutive pairs, and the count is
...
The sequence is ``Number of black square subarrays of (n+1)X3 binary arrays with no element equal to a strict majority of its diagonal and antidiagonal neighbors, with upper left element zero.''. Its rows are the vertices of a digraph on
...
model reproduces $21$ published values, $36$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231070.
