...
\title{The empirical column recurrences for the table OEIS A250956}
...
\begin{abstract}
OEIS A250956 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A250956 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with no 2X2 subblock having its maximum diagonal element less than its minimum antidiagonal element.''. It has offset $1$, is
read by antidiagonals, and begins
\[
15, 56, 56, 209, 392, 209, 780, 2744,\ \dots
\]
...
k=7: [order 16]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 12:54:06, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..1 arrays with no 2X2 subblock having its maximum diagonal element less than its minimum antidiagonal element.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..1 arrays with no 2X2 subblock having its maximum diagonal element less than its minimum antidiagonal element.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(4) 0..1 arrays with no 2X2 subblock having its maximum diagonal element less than its minimum antidiagonal element.''. Its rows are the vertices of a digraph on
$S=16$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;15\,a(n-1) - 24\,a(n-2) - 8\,a(n-3)
\]
holds from index $2+1$ on.

\subsection*{Column $k$$=4$}
The sequence is ``Number of (n+1)X(5) 0..1 arrays with no 2X2 subblock having its maximum diagonal element less than its minimum antidiagonal element.''. Its rows are the vertices of a digraph on
$S=32$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;27\,a(n-1) - 57\,a(n-2) - 46\,a(n-3) + 30\,a(n-4)
\]
...
The entry gives only the order for this line: ``k=5: [order 7]''. Its model is a walk on
...
$c_{1}$ & $57$ & $c_{3}$ & $637$ & $c_{5}$ & $-3639$ & $c_{7}$ & $429$ \\
$c_{2}$ & $-516$ & $c_{4}$ & $2799$ & $c_{6}$ & $-100$ &  &  \\
\end{tabular}\end{center}
Repeating with the first $7+4$ terms removed gives order $7$ again.

\subsection*{Column $k=6$, recovered}
The entry gives only the order for this line: ``k=6: [order 9]''. Its model is a walk on
...
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $103$ & $c_{4}$ & $25412$ & $c_{7}$ & $261$ &  &  \\
$c_{2}$ & $-1452$ & $c_{5}$ & $-90159$ & $c_{8}$ & $-20412$ &  &  \\
$c_{3}$ & $1999$ & $c_{6}$ & $84285$ & $c_{9}$ & $4212$ &  &  \\
\end{tabular}\end{center}
...
model reproduces $29$ published values, $162$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250956.
