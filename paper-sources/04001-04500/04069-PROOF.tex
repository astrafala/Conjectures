...
\title{The empirical recurrence for OEIS A238768, proved by transfer matrix}
...
\begin{abstract}
OEIS A238768 counts arrays over $\{0,1,2,3\}$ in which no entry may equal, modulo $4$, a given
constant plus the SUM of all the entries lying before it along one of several rays. The entry
carries an empirical recurrence of order $3$ contributed by R.~H.~Hardin. It is true, and
...
count is then a walk count on $S=4$ states.
\end{abstract}
...
OEIS A238768 is ``Number of n X 1 0..3 arrays with no element equal to the sum modulo 4 of elements to its left or elements above it.''. It has offset $1$ and begins
\[
3, 6, 14, 32, 72, 164, 372, 844,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + 2*a(n-2) + 2*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 24 2018 09:25:34, revision 7) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array has $1$ cells across one way, entries $x(i,j)\in\{0,1,2,3\}$, and the entry names these
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A238768.
