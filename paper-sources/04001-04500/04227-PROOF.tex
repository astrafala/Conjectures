...
\title{The empirical column recurrences for the table OEIS A283572}
...
\begin{abstract}
OEIS A283572 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A283572 is ``T(n,k) = Number of n X k 0..1 arrays with no 1 equal to more than one of its horizontal, diagonal and antidiagonal neighbors, with the exception of exactly one element.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 0, 0, 1, 4, 0, 2, 26,\ \dots
\]
...
k=6: [order 54]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 28 2022 03:27:52, revision 6) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of 1 X n 0..1 arrays with no 1 equal to more than one of its horizontal, diagonal and antidiagonal neighbors, with the exception of exactly one element.''. Its rows are the vertices of a digraph on
...
\[
a(n)\;=\;2\,a(n-1) + a(n-2) - 3\,a(n-4) - 2\,a(n-5) - a(n-6)
\]
holds from index $6+1$ on.

\subsection*{Row $n$$=2$}
The sequence is ``Number of 2 X n 0..1 arrays with no 1 equal to more than one of its horizontal, diagonal and antidiagonal neighbors, with the exception of exactly one element.''. Its rows are the vertices of a digraph on
...
model reproduces $19$ published values, $42$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A283572.
