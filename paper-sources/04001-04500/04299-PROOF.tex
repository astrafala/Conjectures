...
\title{The conjectured generating function for OEIS A281473, proved}
...
\begin{abstract}
OEIS A281473 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=561$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $580$ coefficients in exact arithmetic
...
OEIS A281473 is ``Number of 5 X n  0..1 arrays with no element equal to more than two of its horizontal, diagonal or antidiagonal neighbors and with new values introduced in order 0 sequentially upwards.''. It has offset $1$ and begins
\[
16, 264, 133, 207, 278, 358,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: x*(16 + 232*x - 379*x\^{}2 + 189*x\^{}3 - 251*x\^{}4 + 124*x\^{}5 - 288*x\^{}6 + 132*x\^{}7 - 24*x\^{}8 + x\^{}9 + 2*x\^{}10) / ((1 - x)*(1 - x - x\^{}3 - x\^{}5)). - \_Colin Barker\_, Feb 19 2019
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 19 2019 14:41:23, revision 7) this is still
...
\]
and there are $S=561$ states. The digraph is built by reading the entry's own English, and the
...
The family this entry belongs to is read by one model, described by its author as: ``Cell conditions counted up to relabelling of the alphabet.''
...
\]
a polynomial of degree at most $572$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{572}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $580$
of them --- more than the $572+1$ the argument requires --- agree.
\end{proof}
...
numerator. Its coefficients are the integers $c_1,\dots,c_{6}$ read off $D_2$, the first few being $2,\ -1,\ 1,\ -1,\ 1,\ -1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A281473.
