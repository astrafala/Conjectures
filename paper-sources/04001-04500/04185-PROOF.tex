...
\title{The conjectured generating function for OEIS A207596, proved}
...
\begin{abstract}
OEIS A207596 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=241$ vertices, so its generating function is a rational function whose numerator
...
OEIS A207596 is ``Number of n X 5 0..1 arrays avoiding 0 0 0 and 0 1 0 horizontally and 0 0 1 and 1 0 1 vertically.''. It has offset $1$ and begins
\[
15, 225, 825, 1995, 3915, 6765,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: 15*x*(1 + 11*x + x\^{}2 - x\^{}3) / (1 - x)\^{}4.
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 25 2018 06:08:36, revision 8) this is still
...
\]
and there are $S=241$ states. The digraph is built by reading the entry's own English, and the
...
\]
a polynomial of degree at most $245$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{245}$ then $R$ has a zero of order greater than its own
...
of them --- more than the $245+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207596.
