...
\title{The empirical column recurrences for the table OEIS A232589}
...
\begin{abstract}
OEIS A232589 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
exact computation, with the Cayley--Hamilton theorem bounding the work. This note settles the columns $k=1, 2, 3, 4$ and the rows $n=1$.
\end{abstract}
...
OEIS A232589 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally, with no adjacent elements equal.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 10, 2, 2, 34, 4, 26, 12,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 07:31:12, revision 6) these are still
...
column $k$$=1$ & $3$ & $90$ & $>2$ & $9$ \\
column $k$$=2$ & $4$ & $756$ & $>3$ & $8$ \\
column $k$$=3$ & $3$ & $6642$ & $>3$ & $7$ \\
column $k$$=4$ & $5$ & $59292$ & $>4$ & $10$ \\
row $n$$=1$ & $5$ & $90$ & $>5$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally, with no adjacent elements equal.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(3) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally, with no adjacent elements equal.''. Its rows are the vertices of a digraph on
$S=756$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;4\,a(n-1) - a(n-2) - a(n-3) + 2\,a(n-4)
\]
holds from index $3+1$ on.

\subsection*{Column $k$$=3$}
The sequence is ``Number of (n+1)X(4) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally, with no adjacent elements equal.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X(5) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally, with no adjacent elements equal.''. Its rows are the vertices of a digraph on
$S=59292$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;5\,a(n-1) + 9\,a(n-2) + 2\,a(n-3) + a(n-4) + 2\,a(n-5)
\]
holds from index $4+1$ on.

\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) 0..2 arrays with every element next to itself plus and minus one within the range 0..2 horizontally or antidiagonally, with no adjacent elements equal.''. Its rows are the vertices of a digraph on
$S=90$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;-a(n-1) + 2\,a(n-2) + 4\,a(n-3) + 3\,a(n-4) + a(n-5)
\]
...
model reproduces $44$ published values, $136$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A232589.
