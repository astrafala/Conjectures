...
\title{The conjectured generating function for OEIS A284237, proved}
...
\begin{abstract}
OEIS A284237 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (2*x\^{}5 - 3*x\^{}4 + x\^{}3 + 2*x\^{}2 - 2*x + 1)/((x - 1)*(x + 1)*(2*x - 1)). (End)
\end{quote}
