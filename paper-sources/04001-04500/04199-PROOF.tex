...
\title{The empirical column recurrences for the table OEIS A279977}
...
\begin{abstract}
OEIS A279977 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A279977 is ``T(n,k) is the number of n X k 0..1 arrays with no element equal to a strict majority of its horizontal and antidiagonal neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 1, 0, 0, 3, 0, 3, 9,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 20 2023 18:48:55, revision 8) these are still
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of 1 X n 0..1 arrays with no element equal to a strict majority of its horizontal and antidiagonal neighbors, with the exception of exactly two elements, and with new values introduced in order 0 sequentially upwards.''. Its rows are the vertices of a digraph on
...
model reproduces $10$ published values, $15$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A279977.
