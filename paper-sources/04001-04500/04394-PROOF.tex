...
\title{The empirical recurrence for OEIS A263592, proved by transfer matrix}
...
\begin{abstract}
OEIS A263592 carries an empirical recurrence of order $8$ contributed by R.~H.~Hardin. It is
true, and it is decidable rather than empirical. The entry's condition is local to a bounded window of consecutive lines, so
the admissible configurations of such a window are the vertices of a finite digraph and the
arrays counted are exactly the walks in it; hence $a(n)$ is a walk count on $S=293$ vertices
...
OEIS A263592 is ``Number of length n arrays of permutations of 0..n-1 with each element moved by -3 to 3 places and the median of every three consecutive elements nondecreasing.''. It has offset $1$ and begins
\[
1, 2, 6, 16, 41, 108, 280, 729,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + a(n-2) + 7*a(n-3) + 2*a(n-4) + 4*a(n-5) - a(n-7) - a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 02 2019 07:24:06, revision 8) this is still
...
admissible array is then exactly a walk, so with $M$ the adjacency matrix on $S=293$ vertices
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-2) + 7\,a(n-3) + 2\,a(n-4) + 4\,a(n-5) - a(n-7) - a(n-8)
\]
for every $n>0$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263592.
