...
\title{The empirical recurrence for OEIS A231105, proved by transfer matrix}
...
\begin{abstract}
OEIS A231105 counts arrays of width $5$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
the count is a walk count on $S=44145$ states.
\end{abstract}
...
OEIS A231105 is ``Number of n X 5 0..3 arrays x(i,j) with each element horizontally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
0, 20, 286, 4674, 75346, 1217338, 19662930, 317613674,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 14*a(n-1) + 35*a(n-2) - 4*a(n-3) + 4*a(n-4) + 32*a(n-5) - 32*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 26 2018 13:07:01, revision 8) this is still
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=44145.
\]
...
\begin{theorem}
\[
a(n)\;=\;14\,a(n-1) + 35\,a(n-2) - 4\,a(n-3) + 4\,a(n-4) + 32\,a(n-5) - 32\,a(n-6)
\]
for every $n>6$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231105.
