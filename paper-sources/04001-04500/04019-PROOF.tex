...
\title{The empirical column recurrences for the table OEIS A269097}
...
\begin{abstract}
OEIS A269097 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A269097 is ``T(n,k) = Number of n X k 0..3 arrays with some element plus some horizontally or vertically adjacent neighbor totalling three exactly once.''. It has offset $1$, is
read by antidiagonals, and begins
\[
0, 4, 4, 24, 96, 24, 108, 1152,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 23 2026 13:50:40, revision 6) these are still
...
The sequence is ``Number of n X 2 0..3 arrays with some element plus some horizontally or vertically adjacent neighbor totalling three exactly once.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 3 0..3 arrays with some element plus some horizontally or vertically adjacent neighbor totalling three exactly once.''. Its rows are the vertices of a digraph on
...
model reproduces $13$ published values, $65$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269097.
