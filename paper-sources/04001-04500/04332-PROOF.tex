...
\title{The empirical column recurrences for the table OEIS A298917}
...
\begin{abstract}
OEIS A298917 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A298917 is ``T(n,k) = Number of n X k 0..1 arrays with every element equal to 0, 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset $1$, is
...
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 04 2022 13:18:37, revision 6) these are still
...
column $k$$=1$ & $1$ & $4$ & $>3$ & $13$ \\
column $k$$=2$ & $2$ & $16$ & $>3$ & $12$ \\
column $k$$=3$ & $2$ & $64$ & $>3$ & $11$ \\
column $k$$=4$ & $3$ & $256$ & $>4$ & $10$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of n X 1 0..1 arrays with every element equal to 0, 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 2 0..1 arrays with every element equal to 0, 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 3 0..1 arrays with every element equal to 0, 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 4 0..1 arrays with every element equal to 0, 3, 5, 7 or 8 king-move adjacent elements, with upper left element zero.''. Its rows are the vertices of a digraph on
...
model reproduces $46$ published values, $58$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298917.
