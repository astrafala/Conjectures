...
\title{The empirical column recurrences for the table OEIS A231419}
...
\begin{abstract}
OEIS A231419 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231419 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with no element equal to a strict majority of its horizontal, diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. It has offset $1$, is
read by antidiagonals, and begins
\[
9, 71, 50, 514, 1032, 285, 3838, 20896,\ \dots
\]
...
k=4: [order 99]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:28:48, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with no element equal to a strict majority of its horizontal, diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=73$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;6\,a(n-1) - 11\,a(n-3) + 4\,a(n-4)
\]
holds from index $4+1$ on.

\subsection*{Row $n$$=1$}
The sequence is ``Number of (2)X(n+1) 0..2 arrays with no element equal to a strict majority of its horizontal, diagonal and antidiagonal neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
$S=111$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;8\,a(n-1) + 4\,a(n-2) - 58\,a(n-3) - 24\,a(n-4) + 40\,a(n-5) - 16\,a(n-6)
\]
...
$S=579$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=1158$ exact terms returns the minimal one, of order
$7$ --- the stated order --- with integer coefficients
\begin{center}\small\begin{tabular}{cccccccc}
$c_{1}$ & $17$ & $c_{3}$ & $-119$ & $c_{5}$ & $134$ & $c_{7}$ & $-200$ \\
$c_{2}$ & $-28$ & $c_{4}$ & $373$ & $c_{6}$ & $-268$ &  &  \\
\end{tabular}\end{center}
Repeating with the first $7+4$ terms removed gives order $7$ again.

\subsection*{Row $n=2$, recovered}
The entry gives only the order for this line: ``n=2: [order 28]''. Its model is a walk on
$S=829$ vertices, so it satisfies a monic recurrence of order at most $S$;
Berlekamp--Massey on $2S=1658$ exact terms returns the minimal one, of order
$28$ --- the stated order --- with integer coefficients
\[
(c_1,\dots,c_{28})=(20,\ 56,\ -821,\ -2559,\ 2274,\ 11333,\ 75045,\ 58970,\ -744820,\ -338743,\ 3350990,\ -566226,\ -8533262,\ 6626396,\ 10836980,\ -13238580,\ -6337152,\ 13746128,\ -1255112,\ -8079152,\ 2081168,\ 460736,\ -1034208,\ 221216,\ -114304,\ -122880,\ -16384,\ -512).
\]
...
model reproduces $15$ published values, $61$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231419.
