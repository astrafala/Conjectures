...
\title{The empirical column recurrences for the table OEIS A231302}
...
\begin{abstract}
OEIS A231302 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A231302 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with no element unequal to a strict majority of its horizontal, vertical and antidiagonal neighbors, with values 0..2 introduced in row major order.''. It has offset $1$, is
...
k=1: a(n) = 2*a(n-1) +3*a(n-2) -4*a(n-4)\\
k=2: [order 8] for n>9\\
k=3: [order 14] for n>16\\
k=4: [order 44] for n>48\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:23:38, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..2 arrays with no element unequal to a strict majority of its horizontal, vertical and antidiagonal neighbors, with values 0..2 introduced in row major order.''. Its rows are the vertices of a digraph on
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231302.
