...
\title{The empirical closed form for OEIS A269498, proved}
...
\begin{abstract}
OEIS A269498 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=30$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A269498 is ``Number of length-7 0..n arrays with no repeated value differing from the previous repeated value by one.''. It has offset $1$ and begins
\[
66, 1609, 13946, 71115, 263794, 791381, 2039274, 4686391,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = n\^{}7 + 7*n\^{}6 + 21*n\^{}5 + 15*n\^{}4 + 7*n\^{}3 + 17*n\^{}2 - n - 1.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 03 2016 03:50:52, revision 7) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;n^{7} + 7 n^{6} + 21 n^{5} + 15 n^{4} + 7 n^{3} + 17 n^{2} - n - 1.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=30$ vertices, and $u,w$ the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A269498.
