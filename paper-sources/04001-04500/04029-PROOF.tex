...
\title{The conjectured generating function for OEIS A285646, proved}
...
\begin{abstract}
OEIS A285646 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (1 + x - x\^{}2 - 2*x\^{}3 + 4*x\^{}5 + 32*x\^{}6 - 32*x\^{}8) / ((1 - x)*(1 + x)*(1 - 2*x)*(1 + 2*x)).
\end{quote}
