...
\title{The empirical recurrence for OEIS A270052, proved by a counting transfer matrix}
...
\begin{abstract}
OEIS A270052 counts arrays over $\{0,\dots,4\}$ subject to a condition on the array as a
...
which is what keeps the digraph sparse. That makes $a(n)$ a walk count on $S=50$ vertices,
...
OEIS A270052 is ``Number of nX2 0..4 arrays with some element plus some horizontally or vertically adjacent neighbor totalling four exactly once.''. It has offset $1$ and begins
\[
5, 240, 5400, 99840, 1683240, 26891280, 414705720, 6237722400,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 26*a(n-1) -169*a(n-2) for n>3.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 09 2016 12:21:03, revision 6) the statement is
...
\[
\mathcal V=\bigl\{(r,c):\ r\in\{0,\dots,4\}^{2},\ c\in\{0,1\}\bigr\},
\qquad |\mathcal V|=S=50,
\]
...
the digraph dense, with $(4+1)^{2\cdot2}$ edges instead of the sparse set used here.
\end{remark}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A270052.
