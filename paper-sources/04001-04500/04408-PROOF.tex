...
\title{The conjectured generating function for OEIS A279876, proved}
...
\begin{abstract}
OEIS A279876 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A279876 is ``Decimal representation of the x-axis, from the origin to the right edge, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 211", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 2, 5, 14, 1, 58,\ \dots
\]
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=14$ and $\deg D_2=6$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $35$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{35}$ then $R$ has a zero of order greater than its own
...
of them --- more than the $35+1$ the argument requires --- agree.
\end{proof}
...
\[
a(n)\;=\;\sum_{i\ge1}c_i\,a(n-i)\qquad\text{for every }n>14,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A279876.
