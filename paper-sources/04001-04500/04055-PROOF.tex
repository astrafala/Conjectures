...
\title{The conjectured generating function for OEIS A283651, proved}
...
\begin{abstract}
OEIS A283651 counts arrays of a fixed width under a local condition, and carries a conjectured
...
\begin{quote}
G.f.: (1 - x + 3*x\^{}2 + 8*x\^{}3 - 32*x\^{}4 + 24*x\^{}5) / ((1 - x)*(1 - 2*x)*(1 + 2*x)).
\end{quote}
