...
\title{The empirical column recurrences for the table OEIS A238523}
...
\begin{abstract}
OEIS A238523 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A238523 is ``T(n,k)=Number of (n+1)X(k+1) 0..3 arrays with no element equal to all horizontal neighbors or equal to all vertical neighbors.''. It has offset $1$, is
read by antidiagonals, and begins
\[
84, 588, 588, 7056, 9612, 7056, 74676, 345744,\ \dots
\]
The entry carries the following block:
\begin{quote}\small
Empirical for column k:\\
k=1: a(n) = 9*a(n-1) +21*a(n-2) -14*a(n-3)\\
k=2: [order 12]\\
k=3: [order 6]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:53:13, revision 6) these are still
...
The sequence is ``Number of (n+1)X(2) 0..3 arrays with no element equal to all horizontal neighbors or equal to all vertical neighbors.''. Its rows are the vertices of a digraph on
$S=156$ vertices, edges being the admissible consecutive pairs, and the count is
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A238523.
