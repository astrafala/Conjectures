...
\title{The conjectured generating function for OEIS A239987, proved}
...
\begin{abstract}
OEIS A239987 counts arrays of a fixed width under a local condition, and carries a conjectured
generating function --- and nothing else. It is true. The count is a walk count in a finite
digraph on $S=24$ vertices, so its generating function is a rational function whose numerator
...
coefficients of their expansions do. Comparing $39$ coefficients in exact arithmetic
...
OEIS A239987 is ``Number of 3 X n 0..3 arrays with no element equal to zero plus the sum of elements to its left or two plus the sum of the elements above it or one plus the sum of the elements diagonally to its northwest, modulo 4.''. It has offset $1$ and begins
\[
3, 6, 13, 22, 38, 65,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 27 2018 06:21:00, revision 8) this is still
...
\]
and there are $S=24$ states. The digraph is built by reading the entry's own English, and the
model reproduces all $41$ terms the entry publishes, exactly, in integer arithmetic --- a
...
The family this entry belongs to is read by one model, described by its author as: ```Number of n X W 0..m-1 arrays with no element equal to c1 plus the sum of elements to its left or c2 plus the sum of elements above it [or c3 plus the sum of the elements diagonally to its northwest] [or c4 plus the sum of the elements antidiagonally to its northeast], modulo m.'''
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=8$ and $\deg D_2=5$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $32$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{32}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $39$
of them --- more than the $32+1$ the argument requires --- agree.
\end{proof}
...
\[
a(n)\;=\;\sum_{i\ge1}c_i\,a(n-i)\qquad\text{for every }n>8,
\]
a constant-coefficient linear recurrence of order $5$, valid past the degree of the
numerator. Its coefficients are the integers $c_1,\dots,c_{5}$ read off $D_2$, the first few being $5,\ -10,\ 10,\ -5,\ 1$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
First, the model reproduces every one of the $41$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A239987.
