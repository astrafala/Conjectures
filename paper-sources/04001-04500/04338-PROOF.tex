...
\title{The conjectured generating function for OEIS A207107, proved}
...
\begin{abstract}
OEIS A207107 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $103$ coefficients in exact arithmetic
...
OEIS A207107 is ``Number of n X 4 0..1 arrays avoiding 0 0 0 and 0 1 1 horizontally and 0 0 1 and 1 1 0 vertically.''. It has offset $1$ and begins
\[
9, 81, 271, 643, 1271, 2239,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: x*(9 + 36*x - 44*x\^{}2 + 8*x\^{}3 + x\^{}4) / (1 - x)\^{}5.
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 20 2018 14:27:59, revision 9) this is still
...
\]
a polynomial of degree at most $96$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{96}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $103$
of them --- more than the $96+1$ the argument requires --- agree.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A207107.
