...
\title{The empirical product formula for OEIS A140916, proved}
...
\begin{abstract}
OEIS A140916 counts $7\times8$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $7\times8\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $56$. The entry's own expression, cleared of its denominator,
...
OEIS A140916 is ``Number of 7 X 8 matrices with elements in 0..n with each row and each column in nondecreasing order. 7,8,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 6435, 9202050, 4971151900, 1318349483880, 201299981193168,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2025 11:08:55, revision 8) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $7\times8$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$7\times8$ rectangle --- a plane partition in a $7\times8\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{7}\prod_{j=1}^{8}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $56$ with rational coefficients.
\end{theorem}
...
$7\cdot8=56$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,8,7)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $77$ and $21$ respectively.
\end{lemma}

\begin{proof}
With $s=n+8+7-1$ one has $s-(n+i)=8+7-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{8+7-1-i}
=\frac{(s-8-7+2+i)(s-8-7+3+i)\cdots s}{(8+7-1-i)!},
\]
a polynomial in $n$ of degree $8+7-1-i$. Multiplying over $i=0,\dots,7-1$ gives
$\deg N=\sum_{i=0}^{7-1}(8+7-1-i)=77$. The denominator is
...
of degree at most $56+21=77$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,77$, that is at $78$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $77$ has at most $77$ roots, so it is identically zero. Since $D(n)\neq0$
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,77$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140916.
