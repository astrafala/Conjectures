...
\title{The empirical column recurrences for the table OEIS A262319}
...
\begin{abstract}
OEIS A262319 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A262319 is ``T(n,k)=Number of (n+2)X(k+2) 0..1 arrays with each row and column divisible by 7, read as a binary number with top and left being the most significant bits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 3, 3, 5, 5, 5, 10, 9,\ \dots
\]
...
k=4: [order 29]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 17 2015 22:23:41, revision 4) these are still
...
The sequence is ``Number of (n+2)X3 0..1 arrays with each row and column divisible by 7, read as a binary number with top and left being the most significant bits.''. Its rows are the vertices of a digraph on
...
model reproduces $10$ published values, $20$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262319.
