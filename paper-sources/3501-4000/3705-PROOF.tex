...
\title{The empirical product formula for OEIS A140917, proved}
...
\begin{abstract}
OEIS A140917 counts $8\times8$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $8\times8\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $64$. The entry's own expression, cleared of its denominator,
...
OEIS A140917 is ``Number of 8 X 8 matrices with elements in 0..n with each row and each column in nondecreasing order. 8,8,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 12870, 34763300, 33803832920, 15484613937936, 3940599631842016,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2025 11:09:09, revision 8) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $8\times8$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$8\times8$ rectangle --- a plane partition in a $8\times8\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{8}\prod_{j=1}^{8}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $64$ with rational coefficients.
\end{theorem}
...
$8\cdot8=64$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,8,8)$ and claims
\[
a(n)\;=\;\prod_{i=0}^{8-1}\binom{s}{n+i}\,\frac{i!}{(s-i)^{8-1-i}}\;=\;
\frac{N(n)}{D(n)} .
\]

\begin{lemma}\label{lem:deg}
$N$ and $D$ are polynomials in $n$ of degrees $92$ and $28$ respectively.
\end{lemma}

\begin{proof}
With $s=n+8+8-1$ one has $s-(n+i)=8+8-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{8+8-1-i}
=\frac{(s-8-8+2+i)(s-8-8+3+i)\cdots s}{(8+8-1-i)!},
\]
a polynomial in $n$ of degree $8+8-1-i$. Multiplying over $i=0,\dots,8-1$ gives
$\deg N=\sum_{i=0}^{8-1}(8+8-1-i)=92$. The denominator is
$\prod_{i=0}^{8-1}(s-i)^{8-1-i}$, of degree
$\sum_{i=0}^{8-1}(8-1-i)=28$, and it does not vanish for $n\ge0$.
\end{proof}
...
of degree at most $64+28=92$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,92$, that is at $93$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $92$ has at most $92$ roots, so it is identically zero. Since $D(n)\neq0$
...
definition: the nondecreasing rows were listed and the chains of $8$ of them increasing
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,92$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140917.
