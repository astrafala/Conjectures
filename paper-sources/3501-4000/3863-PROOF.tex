...
\title{The empirical recurrence for OEIS A234328, proved by transfer matrix}
...
\begin{abstract}
OEIS A234328 counts $4$-column arrays over $\{0,\dots,3\}$ in which every
$2\times2$ subblock has its diagonal sum differing from its antidiagonal sum by
exactly $1$, and it
carries an empirical recurrence of order $48$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=256$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-48}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234328 is ``Number of (n+1) X (3+1) 0..3 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 1 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
2368, 25728, 287888, 3271560, 37234216, 425662432, 4858165384,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 02:22:46, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;507\,a(n-2) - 102295\,a(n-4) + 10646830\,a(n-6) - 635915377\,a(n-8) + 23832852746\,a(n-10) - 596280829182\,a(n-12) + 10371633339873\,a(n-14) - 128744093807520\,a(n-16) + 1159671655063068\,a(n-18) - 7660667577596808\,a(n-20) + 37368059037902308\,a(n-22) - 135219186772494408\,a(n-24) + 364034788822163472\,a(n-26) - 730004249849579520\,a(n-28) + 1089413122161376064\,a(n-30) - 1205481757762071168\,a(n-32) + 981742220067000064\,a(n-34) - 581000394777277440\,a(n-36) + 244895745327540224\,a(n-38) - 71316845470162944\,a(n-40) + 13706205520330752\,a(n-42) - 1618885433491456\,a(n-44) + 103796813856768\,a(n-46) - 2721872609280\,a(n-48)
\end{equation}
for all $n>48$, which is every $n$ at which a recurrence of order $48$ can be
stated.
...
Let $V$ consist of all of $\{0,\dots,3\}^{4}$, and put $S=|V|=256$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge48$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-48}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-48$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-48}\bigl(M^{48}-\sum_i c_i M^{48-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-48$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $48$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=48-48+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>48$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234328.
