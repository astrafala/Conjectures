...
\title{The empirical recurrence for OEIS A238041, proved by transfer matrix}
...
\begin{abstract}
OEIS A238041 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=1440$
...
OEIS A238041 is ``Number of (n+1)X(3+1) 0..2 arrays with the upper median minus the lower median of every 2X2 subblock differing from its horizontal and vertical neighbors by exactly one.''. It has offset $1$ and begins
\[
1440, 10812, 81524, 621780, 4837484, 36440340, 282305082, 2134998732,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 10:42:39, revision 6) this is still
...
differ by $1$. The entry's first term $1440$ counts those, and the model was made to
...
\]
over the $S=1440$ pairs whose own subblock row already satisfies the condition along itself.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A238041.
