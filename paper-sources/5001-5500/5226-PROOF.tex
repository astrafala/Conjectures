...
\title{The empirical product formula for OEIS A140907, proved}
...
\begin{abstract}
OEIS A140907 counts $3\times7$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $3\times7\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $21$. The entry's own expression, cleared of its denominator,
...
OEIS A140907 is ``Number of 3 X 7 matrices with elements in 0..n with each row and each column in nondecreasing order. 3,7,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 120, 4950, 108900, 1557270, 16195608,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 17 2025 09:28:50, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $3\times7$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$3\times7$ rectangle --- a plane partition in a $3\times7\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{3}\prod_{j=1}^{7}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $21$ with rational coefficients.
\end{theorem}
...
$3\cdot7=21$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,7,3)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $24$ and $3$ respectively.
\end{lemma}

\begin{proof}
With $s=n+7+3-1$ one has $s-(n+i)=7+3-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{7+3-1-i}
=\frac{(s-7-3+2+i)(s-7-3+3+i)\cdots s}{(7+3-1-i)!},
\]
a polynomial in $n$ of degree $7+3-1-i$. Multiplying over $i=0,\dots,3-1$ gives
$\deg N=\sum_{i=0}^{3-1}(7+3-1-i)=24$. The denominator is
...
of degree at most $21+3=24$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,24$, that is at $25$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $24$ has at most $24$ roots, so it is identically zero. Since $D(n)\neq0$
...
First, Theorem~\ref{thm:poly} reproduces all $20$ terms the entry publishes,
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,24$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140907.
