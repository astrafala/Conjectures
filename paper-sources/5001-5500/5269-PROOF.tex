...
\title{The empirical recurrence for OEIS A302377, proved by transfer matrix}
...
\begin{abstract}
OEIS A302377 counts $4$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the six horizontal, diagonal and antidiagonal neighbours, a number $equal to$ itself lying in $\{1,2,3,4\}$, and it carries an empirical
recurrence of order $26$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
OEIS A302377 is ``Number of nX4 0..1 arrays with every element equal to 1, 2, 3 or 4 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
2, 46, 430, 4640, 48980, 514655, 5421003, 57068484,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 06 2018 12:38:51, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;8\,a(n-1) + 33\,a(n-2) - 25\,a(n-3) - 427\,a(n-4) - 361\,a(n-5) + 1618\,a(n-6) + 3070\,a(n-7) - 2572\,a(n-8) - 6300\,a(n-9) + 3216\,a(n-10) + 3053\,a(n-11) - 8191\,a(n-12) + 955\,a(n-13) + 12197\,a(n-14) - 2105\,a(n-15) + 634\,a(n-16) - 1833\,a(n-17) - 1727\,a(n-18) - 2285\,a(n-19) + 2346\,a(n-20) - 1596\,a(n-21) + 780\,a(n-22) - 134\,a(n-23) - 6\,a(n-24) + 44\,a(n-25) - 16\,a(n-26)
\end{equation}
for all $n>26$.
...
has score in $\{1,2,3,4\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{1,2,3,4\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{1,2,3,4\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{1,2,3,4\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{1,2,3,4\}$. Read the sequence as the
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-26}$ whenever $n-26\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-26$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-26}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $26$ matrix--vector
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302377.
