...
\title{The empirical recurrence for OEIS A234782, proved by transfer matrix}
...
\begin{abstract}
OEIS A234782 counts arrays of width $5$ over $\{0,1,2,3\}$ subject to a condition read round the
perimeter of every $2\times2$ subblock: no two adjacent entries are equal and the two edge-increase counts agree in every subblock. The entry carries an empirical recurrence of
...
count is a walk count on a finite digraph with $S=325$ vertices, and whether a linear
...
OEIS A234782 is ``Number of (n+1)X(4+1) 0..3 arrays with no adjacent elements equal and with each 2X2 subblock having the number of clockwise edge increases equal to the number of counterclockwise edge increases.''. It has offset $1$ and begins
\[
19652, 1269036, 83120724, 5465582060, 359793857812, 23692759265012, 1560344155530604, 102763262525972764,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 08:26:36, revision 6) this is still
...
\]
and, in addition, no two horizontally or vertically adjacent entries of the array are equal.
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=325,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234782.
