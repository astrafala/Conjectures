...
\title{The empirical recurrence for OEIS A250703, proved by transfer matrix}
...
\begin{abstract}
OEIS A250703 counts arrays over $\{0,1,2,3\}$ in which a statistic taken over short runs
...
window of consecutive rows, so the count is a walk count on $S=1024$ states, and the
...
OEIS A250703 is ``Number of (n+1)X(4+1) 0..3 arrays with nondecreasing x(i,j)+x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
19876, 253578, 2492054, 19962956, 133888216, 770143684, 3888150856, 17539038598,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 12:43:19, revision 6) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

The array has $5$ cells across and entries $x(i,j)\in\{0,1,2,3\}$.
...
So a single row is a state. Let $M$ be the matrix on the $4^{5}=1024$ rows with
...
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=1024.
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250703.
