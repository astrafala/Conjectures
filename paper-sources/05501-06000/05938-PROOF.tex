...
\title{The empirical recurrence for OEIS A197094, proved by transfer matrix}
...
\begin{abstract}
OEIS A197094 counts $n\times4$ arrays over $\{0,\dots,4\}$ subject to a condition imposed on
...
walk, and $a(n)$ is a walk count on $S=1628$ vertices, which is $C$-finite. Whether the
...
OEIS A197094 is ``Number of nX4 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 4,4,0,0,1 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
3, 55, 384, 3477, 30071, 259856, 2253537, 19514155,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 06:58:36, revision 9) this is still
...
settled by the terminal weight, which evaluates it with $\varnothing$ below. Most of those pairs are indistinguishable, and exactly so. Every offset named above has $|dj|\le 1$, so whether the cell at column $j$ of the middle row is satisfied depends on the three rows only through their windows at $j-1,j,j+1$. Collect for each $j$ the indicator over the window of the row below, and call that tuple $P(r,s)$: the number of cells any $t$ violates is read off $P$, so is the number violated at the bottom edge, and the successor $(s,t)$ carries $P(s,t)$, which does not mention $r$. Hence two pairs $(r,s)$ and $(r',s)$ with the same $P$ have the same outgoing rows and the same successors, and the vertex is $(s,P)$. That leaves $S=1628$ vertices in place of $390625$. Hence,
with $M$ the adjacency matrix on $S=1628$ vertices, $\iota$ the starting vector and $\tau$ the
terminal weight,
\[
...
\begin{theorem}
\[
a(n)\;=\;4\,a(n-1) + 24\,a(n-2) + 113\,a(n-3) + 220\,a(n-4) + 289\,a(n-5) - 162\,a(n-6) - 133\,a(n-7) - 927\,a(n-8) + 1912\,a(n-9) - 1647\,a(n-10) + 1077\,a(n-11) + 1038\,a(n-12) + 260\,a(n-13) - 2794\,a(n-14) + 3652\,a(n-15) - 4151\,a(n-16) + 1142\,a(n-17) - 408\,a(n-18) - 687\,a(n-19) + 347\,a(n-20) - 96\,a(n-21) + 100\,a(n-22) + 31\,a(n-23) + 28\,a(n-24) - a(n-25) - 3\,a(n-26) - a(n-28)
\]
for every $n>28$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A197094.
