...
\title{The conjectured generating function for OEIS A289763, proved}
...
\begin{abstract}
OEIS A289763 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A289763 is ``Binary representation of the diagonal from the origin to the corner of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 597", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 1, 11, 1011, 10111, 10111,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: (1 - 10*x + 100*x\^{}2 - 1000*x\^{}4) / ((1 - x)*(1 - 10*x)*(1 - 10*x\^{}2)*(1 + 100*x\^{}2)) (conjectured). - \_Colin Barker\_, Jul 14 2017
\end{quote}
...
pre-period to raise the numerator's degree gives a monic annihilator of order $S=16$.
...
$A(x)=\sum_{n\ge0}a(n)x^n=N_1(x)/D_1(x)$ where $D_1(x)$ has degree at most $S=16$ and
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=4$ and $\deg D_2=6$; its expansion as a
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A289763.
