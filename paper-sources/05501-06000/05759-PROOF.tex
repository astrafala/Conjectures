...
\title{The empirical closed form for OEIS A187287, proved}
...
\begin{abstract}
OEIS A187287 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=6$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A187287 is ``Number of 2-step one or two space at a time rook's tours on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 8, 36, 80, 140, 216, 308, 416,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 8*n\^{}2 - 12*n for n>1.
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 24 2023 18:01:11, revision 21) this is still
...
walk, so with $M$ the adjacency matrix of that digraph on $S=6$ vertices, and $u,w$ the
...
$n=5,\dots,7$ and agree at each. A monic recurrence of order $3$
determines $a(m)$ from $a(m-1),\dots,a(m-3)$, and for every $m\ge8$ both
$m>4$ and $m-3\ge5$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge5$.
Below $n=5$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=5$ on. The remaining indices $n=2,\dots,4$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187287.
