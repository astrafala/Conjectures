...
\title{The empirical column recurrences for the table OEIS A263224}
...
\begin{abstract}
OEIS A263224 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A263224 is ``T(n,k)=Number of (n+1)X(k+1) 0..2 arrays with each row and column divisible by 5, read as a base-3 number with top and left being the most significant digits.''. It has offset $1$, is
read by antidiagonals, and begins
\[
1, 2, 2, 4, 14, 4, 7, 72,\ \dots
\]
...
k=4: [order 89]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 12 2015 11:50:08, revision 4) these are still
...
The sequence is ``Number of (n+1)X2 0..2 arrays with each row and column divisible by 5, read as a base-3 number with top and left being the most significant digits.''. Its rows are the vertices of a digraph on
...
model reproduces $9$ published values, $16$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263224.
