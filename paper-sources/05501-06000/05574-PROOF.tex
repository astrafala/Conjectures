...
\title{The empirical column recurrences for the table OEIS A259427}
...
\begin{abstract}
OEIS A259427 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A259427 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with each 2X2 subblock having clockwise pattern 0000 0001 0011 or 0111.''. It has offset $1$, is
read by antidiagonals, and begins
\[
13, 43, 43, 142, 243, 142, 469, 1366,\ \dots
\]
...
k=6: [order 62]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 02 2025 11:51:34, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with each 2X2 subblock having clockwise pattern 0000 0001 0011 or 0111.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1)X3 0..1 arrays with each 2X2 subblock having clockwise pattern 0000 0001 0011 or 0111.''. Its rows are the vertices of a digraph on
$S=9$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
a(n)\;=\;6\,a(n-1) + 2\,a(n-2) - 22\,a(n-3) - 6\,a(n-4) + 6\,a(n-5) + a(n-6)
\]
...
model reproduces $17$ published values, $74$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A259427.
