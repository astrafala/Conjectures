...
\title{The empirical product formula for OEIS A140913, proved}
...
\begin{abstract}
OEIS A140913 counts $4\times8$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $4\times8\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $32$. The entry's own expression, cleared of its denominator,
...
OEIS A140913 is ``Number of 4 X 8 matrices with elements in 0..n with each row and each column in nondecreasing order. 4,8,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 495, 70785, 4723719, 184225041, 4789851066,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2025 11:09:21, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $4\times8$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$4\times8$ rectangle --- a plane partition in a $4\times8\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{4}\prod_{j=1}^{8}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $32$ with rational coefficients.
\end{theorem}
...
$4\cdot8=32$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,8,4)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $38$ and $6$ respectively.
\end{lemma}

\begin{proof}
With $s=n+8+4-1$ one has $s-(n+i)=8+4-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{8+4-1-i}
=\frac{(s-8-4+2+i)(s-8-4+3+i)\cdots s}{(8+4-1-i)!},
\]
a polynomial in $n$ of degree $8+4-1-i$. Multiplying over $i=0,\dots,4-1$ gives
$\deg N=\sum_{i=0}^{4-1}(8+4-1-i)=38$. The denominator is
...
of degree at most $32+6=38$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,38$, that is at $39$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $38$ has at most $38$ roots, so it is identically zero. Since $D(n)\neq0$
...
First, Theorem~\ref{thm:poly} reproduces all $17$ terms the entry publishes,
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,38$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140913.
