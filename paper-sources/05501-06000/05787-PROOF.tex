...
\title{The empirical column recurrences for the table OEIS A264071}
...
\begin{abstract}
OEIS A264071 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A264071 is ``T(n,k) = number of (n+1) X (k+1) arrays of permutations of 0..(n+1)*(k+1)-1 with each element having index change +-(.,.) 0,0 1,1 or 1,2.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 5, 4, 13, 21, 8, 34, 121,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 25 2025 21:55:20, revision 14) these are still
...
column $k$$=1$ & $1$ & $16$ & $>0$ & $9$ \\
column $k$$=2$ & $2$ & $64$ & $>0$ & $8$ \\
column $k$$=3$ & $1$ & $256$ & $>2$ & $7$ \\
column $k$$=4$ & $2$ & $1024$ & $>3$ & $6$ \\
row $n$$=1$ & $2$ & $256$ & $>0$ & $9$ \\
\end{tabular}
\end{center}
...
The sequence is ``Number of (n+1) X (4) arrays of permutations of 0..(n+1)*(4)-1 with each element having index change +-(.,.) 0,0 1,1 or 1,2.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of (n+1) X (5) arrays of permutations of 0..(n+1)*(5)-1 with each element having index change +-(.,.) 0,0 1,1 or 1,2.''. Its rows are the vertices of a digraph on
...
\subsection*{Row $n$$=1$}
The sequence is ``Number of (2) X (n+1) arrays of permutations of 0..(2)*(n+1)-1 with each element having index change +-(.,.) 0,0 1,1 or 1,2.''. Its rows are the vertices of a digraph on
$S=256$ vertices, edges being the admissible consecutive pairs, and the count is
$\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ up to the constant factor in the entry's name.
The conjectured recurrence
\[
...
model reproduces $39$ published values, $133$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A264071.
