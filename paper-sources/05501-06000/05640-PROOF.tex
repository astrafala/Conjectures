...
\title{The empirical recurrence for OEIS A253018, proved by transfer matrix}
...
\begin{abstract}
OEIS A253018 counts arrays of width $3$ over an alphabet of $4$ letters in which every three
...
brings the state count down to $S=51$.
\end{abstract}
...
OEIS A253018 is ``Number of (n+2) X (1+2) 0..3 arrays with every consecutive three elements in every row and column not having exactly two distinct values, and in every diagonal and antidiagonal having exactly two distinct values, and new values 0 upwards introduced in row major order.''. It has offset $1$ and begins
\[
25, 37, 51, 63, 105, 153, 189, 315,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 3*a(n-3) for n>5.
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 08 2018 06:45:38, revision 8) this is still
...
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,
\]
with $S=51$ states in all. The walk index $j$ and the entry's own $n$ differ by a constant,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253018.
