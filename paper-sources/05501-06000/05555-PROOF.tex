...
\title{The empirical recurrence for OEIS A251946, proved by transfer matrix}
...
\begin{abstract}
OEIS A251946 counts $(n+2)\times6$ arrays over $\{0,\dots,3\}$ subject to a condition imposed on
every $3\times3$ subblock, and it carries an empirical recurrence of order $36$
...
$a(n)$ is a walk count on $S=465700$ vertices. Such a sequence is $C$-finite, so whether a given
...
OEIS A251946 is ``Number of (n+2)X(4+2) 0..3 arrays with every 3X3 subblock row and column sum prime and every diagonal and antidiagonal sum nonprime.''. It has offset $1$ and begins
\[
2735, 2053, 3572, 8937, 25231, 68456, 177132, 512051,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 13:34:15, revision 6) this is still
...
\begin{theorem}
\[
a(n)\;=\;42\,a(n-3) - 485\,a(n-6) + 1193\,a(n-9) - 6286\,a(n-12) + 16999\,a(n-15) - 43581\,a(n-18) + 70906\,a(n-21) - 74639\,a(n-24) + 37823\,a(n-27) - 54\,a(n-30) - 1971\,a(n-33) + 54\,a(n-36)
\]
for every $n>39$.
\end{theorem}

\begin{proof}
The vector $w=q(M)\mathbf{1}$ was formed by $36$ matrix--vector products in exact
integer arithmetic and $\mathbf{1}^{\!\top}M^{j}w$ evaluated for $j=0,1,\dots$, again
exactly. Those integers vanish from the index corresponding to $n=39+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>39$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A251946.
