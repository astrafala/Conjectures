...
\title{The empirical closed form for OEIS A187178, proved}
...
\begin{abstract}
OEIS A187178 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=18$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A187178 is ``Number of 8-step left-handed knight's tours (moves only out two, left one) on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 0, 0, 0, 0, 280, 1784,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2172*n\^{}2 - 30972*n + 103944 for n>13.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 22 2018 17:47:47, revision 10) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;2172 n^{2} - 30972 n + 103944.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=18$ vertices, and $u,w$ the
...
$n=17,\dots,19$ and agree at each. A monic recurrence of order $3$
determines $a(m)$ from $a(m-1),\dots,a(m-3)$, and for every $m\ge20$ both
$m>16$ and $m-3\ge17$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge17$.
Below $n=17$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=17$ on. The remaining indices $n=14,\dots,16$ were
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187178.
