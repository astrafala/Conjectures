...
\title{The empirical recurrence for OEIS A235235, proved by transfer matrix}
...
\begin{abstract}
OEIS A235235 counts $5$-column arrays over $\{0,\dots,6\}$ in which every
...
carries an empirical recurrence of order $41$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=9072$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-41}q(M)\mathbf{1}$, so the recurrence
...
OEIS A235235 is ``Number of (n+1) X (4+1) 0..6 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 6, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
4652, 11728, 28400, 77368, 201676, 580668, 1593940,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 00:04:44, revision 7) the statement is
...
\end{equation}
for all $n>41$, which is every $n$ at which a recurrence of order $41$ can be
stated.
...
Let $V$ consist of those $r\in\{0,\dots,6\}^{5}$ with $r_j\ne r_{j+1}$ for every
$j$ --- the rows in which no two horizontally adjacent entries are equal, and put $S=|V|=9072$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge41$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-41}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-41$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-41}\bigl(M^{41}-\sum_i c_i M^{41-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-41$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $41$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=41-41+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>41$.
\end{proof}
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=9072$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A235235.
