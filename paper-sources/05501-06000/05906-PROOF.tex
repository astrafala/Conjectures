...
\title{The empirical recurrence for OEIS A222941, proved by transfer matrix}
...
\begin{abstract}
OEIS A222941 counts arrays in which no entry may equal the entries at a short list of fixed offsets
from it, the arrays being counted up to renaming the letters. The entry carries an empirical
recurrence of order $1$ contributed by R.~H.~Hardin. It is true, and it is decidable
...
$2$ consecutive rows, $S=4387$ states in all.
\end{abstract}
...
OEIS A222941 is ``Number of nX3 0..4 arrays with no element equal to another at a city block distance of exactly two, and new values 0..4 introduced in row major order.''. It has offset $1$ and begins
\[
3, 33, 420, 5880, 82320, 1152480, 16134720, 225886080,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 14*a(n-1) for n>3
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 21:01:39, revision 8) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{What is being counted}

Entries lie in $\{0,1,\dots,4\}$ and the condition is that
\[
...
list above is that rule read out. The closing clause ``new values $0..4$ introduced in row
...
$N_1+\dots+N_{5}$, so $a=\sum_i w_iL_i$ with $w=\mathbf{1}^{\!\top}F^{-1}$, $F$ the
...
of $t$ rows. Assembling $i=1,\dots,5$ as a disjoint union with $\iota$ carrying the
weights cleared of denominators and $\tau=\mathbf{1}$,
\[
a(n)\;=\;\iota^{\!\top}M^{\,j}\tau ,\qquad S=4387.
\]
...
Let $q(t)=t^{1}-\sum_i c_it^{1-i}$ be the characteristic polynomial of the
...
\begin{proof}
Factor $M^{j}$ out of $M^{j+1}-\sum_ic_iM^{j+1-i}$. For the bound, $M^{S}$
...
\begin{proof}
The vector $w=q(M)\tau$ was formed by $1$ matrix--vector products in exact integer
...
enumerate every array over $\{0,\dots,4\}$, keep those whose values are introduced in the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222941.
