...
\title{The conjectured generating function for OEIS A250654, proved}
...
\begin{abstract}
OEIS A250654 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250654 is ``Number of (n+1) X (6+1) 0..1 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
64, 134, 274, 554, 1114, 2234,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: 2*x*(32 - 29*x) / ((1 - x)*(1 - 2*x)). - \_Colin Barker\_, Nov 15 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 15 2018 12:50:26, revision 7) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250654.
