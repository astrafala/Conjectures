...
\title{The empirical product formula for OEIS A140936, proved}
...
\begin{abstract}
OEIS A140936 counts $4\times11$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $4\times11\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $44$. The entry's own expression, cleared of its denominator,
...
OEIS A140936 is ``Number of 4 X 11 matrices with elements in 0..n with each row and each column in nondecreasing order. 4,11,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 1365, 496860, 78835120, 6892441920, 381644355456,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 25 2025 12:15:33, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $4\times11$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$4\times11$ rectangle --- a plane partition in a $4\times11\times n$ box. The
correspondence is a bijection, being an involution on arrays, so
\[
a(n)\;=\;\mathrm{PP}(4,11,n).
\]
...
\[
a(n)\;=\;\prod_{i=1}^{4}\prod_{j=1}^{11}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $44$ with rational coefficients.
\end{theorem}
...
$4\cdot11=44$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,11,4)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $50$ and $6$ respectively.
\end{lemma}

\begin{proof}
With $s=n+11+4-1$ one has $s-(n+i)=11+4-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{11+4-1-i}
=\frac{(s-11-4+2+i)(s-11-4+3+i)\cdots s}{(11+4-1-i)!},
\]
a polynomial in $n$ of degree $11+4-1-i$. Multiplying over $i=0,\dots,4-1$ gives
$\deg N=\sum_{i=0}^{4-1}(11+4-1-i)=50$. The denominator is
...
of degree at most $44+6=50$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,50$, that is at $51$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $50$ has at most $50$ roots, so it is identically zero. Since $D(n)\neq0$
...
First, Theorem~\ref{thm:poly} reproduces all $15$ terms the entry publishes,
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,50$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140936.
