...
\title{The empirical recurrence for OEIS A298961, proved by transfer matrix}
...
\begin{abstract}
OEIS A298961 counts $6$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{3,5,6,7,8\}$, and it carries an empirical
...
OEIS A298961 is ``Number of nX6 0..1 arrays with every element equal to 3, 5, 6, 7 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 5, 5, 34, 91, 360, 1144, 4062,\ \dots
\]
...
has score in $\{3,5,6,7,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{3,5,6,7,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{3,5,6,7,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{3,5,6,7,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{3,5,6,7,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298961.
