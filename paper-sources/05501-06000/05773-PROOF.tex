...
\title{The empirical closed form for OEIS A187587, proved}
...
\begin{abstract}
OEIS A187587 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=7$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A187587 is ``Number of 4-step E, S, NW and NE-moving king's tours on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 5, 84, 286, 604, 1038, 1588, 2254,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 58*n\^{}2 - 204*n + 174 for n>2.
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 23 2025 00:02:36, revision 17) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;58 n^{2} - 204 n + 174.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=7$ vertices, and $u,w$ the
...
$n=6,\dots,8$ and agree at each. A monic recurrence of order $3$
determines $a(m)$ from $a(m-1),\dots,a(m-3)$, and for every $m\ge9$ both
$m>5$ and $m-3\ge6$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge6$.
Below $n=6$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=6$ on. The remaining indices $n=3,\dots,5$ were
...
First, the walk model reproduces every one of the $40$ terms the entry publishes,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187587.
