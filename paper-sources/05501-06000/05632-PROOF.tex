...
\title{The empirical product formula for OEIS A140906, proved}
...
\begin{abstract}
OEIS A140906 counts $6\times6$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $6\times6\times n$ box, so
...
OEIS A140906 is ``Number of 6 X 6 matrices with elements in 0..n with each row and each column in nondecreasing order. 6,6,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 924, 226512, 24293412, 1447482465, 55197331332,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 17 2025 09:28:47, revision 7) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $6\times6$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$6\times6$ rectangle --- a plane partition in a $6\times6\times n$ box. The
...
\[
a(n)\;=\;\prod_{i=1}^{6}\prod_{j=1}^{6}\frac{n+i+j-1}{i+j-1},
\]
...
$6\cdot6=36$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,6,6)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $51$ and $15$ respectively.
\end{lemma}

\begin{proof}
With $s=n+6+6-1$ one has $s-(n+i)=6+6-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{6+6-1-i}
=\frac{(s-6-6+2+i)(s-6-6+3+i)\cdots s}{(6+6-1-i)!},
\]
a polynomial in $n$ of degree $6+6-1-i$. Multiplying over $i=0,\dots,6-1$ gives
$\deg N=\sum_{i=0}^{6-1}(6+6-1-i)=51$. The denominator is
...
of degree at most $36+15=51$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,51$, that is at $52$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $51$ has at most $51$ roots, so it is identically zero. Since $D(n)\neq0$
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,51$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140906.
