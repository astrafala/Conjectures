...
\title{The empirical closed form for OEIS A187177, proved}
...
\begin{abstract}
OEIS A187177 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=16$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
monic linear recurrence whose characteristic polynomial is
$\prod_j(x-\lambda_j)^{\deg p_j+1}$; here that polynomial is $q(x)=(x-1)^{3}$, of degree
$3$. Two things are then checked exactly: that the walk count satisfies the same
recurrence from some index on, which the Cayley--Hamilton theorem reduces to a finite
computation, and that the two sequences agree at $3$ consecutive indices past that
...
OEIS A187177 is ``Number of 7-step left-handed knight's tours (moves only out two, left one) on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 0, 0, 0, 16, 368, 1600,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 780*n\^{}2 - 9880*n + 29384 for n>11.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 22 2018 17:47:40, revision 13) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;780 n^{2} - 9880 n + 29384.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=16$ vertices, and $u,w$ the
...
Here $q(x)=(x-1)^{3}$, of degree $3$; the closed form is a polynomial in $n$, so this is $(x-1)^{3}$, and the recurrence it encodes is
...
\begin{theorem}
$a(n)=f(n)$ for every $n\ge12$.
\end{theorem}

\begin{proof}
The vector $w'=q(M)w$ was formed by $3$ matrix--vector products in exact integer
arithmetic, and $u^{\!\top}M^{j}w'$ evaluated for $j=0,1,\dots$, again exactly. Those integers
vanish from the point corresponding to $n=15$ onwards, and the run was continued until the
working vector was identically zero, which settles every larger $j$ at once. So $a$ satisfies
\eqref{eq:rec} for every $n>14$. By Lemma~\ref{lem:ann} $f$ satisfies \eqref{eq:rec}
identically. Both were then evaluated in exact arithmetic at the $3$ consecutive indices
$n=15,\dots,17$ and agree at each. A monic recurrence of order $3$
determines $a(m)$ from $a(m-1),\dots,a(m-3)$, and for every $m\ge18$ both
$m>14$ and $m-3\ge15$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge15$.
Below $n=15$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=15$ on. The remaining indices $n=12,\dots,14$ were
therefore checked one at a time, directly against the walk count in exact integer arithmetic,
and agree.
\end{proof}

The range is tight in the sense that was checked: at $n=11$ the closed form and the sequence differ, so no larger range is available.

\section{Verification}

Four checks, each independent of the algebra above.

First, the walk model reproduces every one of the $36$ terms the entry publishes,
...
at every published term from $n=12$ on, and agrees with the entry's own DATA at each.
...
$n=12$, far past where the proof already settles the matter.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187177.
