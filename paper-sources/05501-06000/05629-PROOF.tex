...
\title{The empirical recurrence for OEIS A230893, proved by transfer matrix}
...
\begin{abstract}
OEIS A230893 counts arrays of width $2$ over $\{0,1,2,3\}$ in which every cell must see, among its
...
OEIS A230893 is ``Number of n X 2 0..3 arrays x(i,j) with each element horizontally or vertically next to at least one element with value (x(i,j)+1) mod 4 and at least one element with value (x(i,j)-1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
0, 2, 2, 10, 22, 70, 186, 538,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + 3*a(n-2) - 2*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 17 2018 07:10:09, revision 9) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A230893.
