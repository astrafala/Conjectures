...
\title{The conjectured generating function for OEIS A263053, proved}
...
\begin{abstract}
OEIS A263053 counts arrays of a fixed width under a local condition, and carries a conjectured
...
coefficients of their expansions do. Comparing $12$ coefficients in exact arithmetic
...
OEIS A263053 is ``Number of (n+1) X 2 0..1 arrays with each row and column not divisible by 3, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
2, 2, 10, 10, 42, 42,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
G.f.: 2*x / ((1 - x)*(1 - 2*x)*(1 + 2*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 27 2026 09:09:36, revision 41) this is still
...
\]
and there are $S=6$ states. The digraph is built by reading the entry's own English, and the
...
The family this entry belongs to is read by one model, described by its author as: ```Number of (n+k) X (W+k) 0..m arrays with each row and column divisible by d, read as a base-b number with top and left being the most significant digits.'''
...
\begin{proof}
Write the conjecture as $N_2/D_2$ with $\deg N_2=1$ and $\deg D_2=3$; its expansion as a
power series exists because $D_2(0)\neq0$. Put
\[
R(x)\;=\;N_1(x)D_2(x)-N_2(x)D_1(x),
\]
a polynomial of degree at most $8$ by Lemma~\ref{lem:rat}. The difference
$A-N_2/D_2$ equals $R/(D_1D_2)$, and $D_1(0)D_2(0)\neq0$, so if the two power series agree in
every coefficient up to $x^{8}$ then $R$ has a zero of order greater than its own
...
exact integer arithmetic, those of $N_2/D_2$ by exact rational long division, and $12$
of them --- more than the $8+1$ the argument requires --- agree.
\end{proof}
...
\[
a(n)\;=\;\sum_{i\ge1}c_i\,a(n-i)\qquad\text{for every }n>1,
\]
a constant-coefficient linear recurrence of order $3$, valid past the degree of the
numerator. Its coefficients are the integers $c_1,\dots,c_{3}$ read off $D_2$, the first few being $1,\ 4,\ -4$. The entry records no recurrence, so this is not a restatement of anything
already there.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A263053.
