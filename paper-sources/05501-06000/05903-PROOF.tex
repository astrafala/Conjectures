...
\title{The empirical recurrence for OEIS A196919, proved by transfer matrix}
...
\begin{abstract}
OEIS A196919 counts $n\times5$ arrays over $\{0,\dots,4\}$ subject to a condition imposed on
...
walk, and $a(n)$ is a walk count on $S=4222$ vertices, which is $C$-finite. Whether the
...
OEIS A196919 is ``Number of n X 5 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 1,3,2,4,0 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
1, 6, 27, 114, 602, 2659, 11843, 54151,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 30 2023 22:46:19, revision 7) this is still
...
settled by the terminal weight, which evaluates it with $\varnothing$ below. Most of those pairs are indistinguishable, and exactly so. Every offset named above has $|dj|\le 1$, so whether the cell at column $j$ of the middle row is satisfied depends on the three rows only through their windows at $j-1,j,j+1$. Collect for each $j$ the indicator over the window of the row below, and call that tuple $P(r,s)$: the number of cells any $t$ violates is read off $P$, so is the number violated at the bottom edge, and the successor $(s,t)$ carries $P(s,t)$, which does not mention $r$. Hence two pairs $(r,s)$ and $(r',s)$ with the same $P$ have the same outgoing rows and the same successors, and the vertex is $(s,P)$. That leaves $S=4222$ vertices in place of $9765625$. Hence,
with $M$ the adjacency matrix on $S=4222$ vertices, $\iota$ the starting vector and $\tau$ the
terminal weight,
\[
...
\begin{theorem}
\[
a(n)\;=\;6\,a(n-1) - 5\,a(n-2) - 3\,a(n-3) - 14\,a(n-4) + 21\,a(n-5) - 55\,a(n-6) - 220\,a(n-7) - 191\,a(n-8) - 132\,a(n-9) + 188\,a(n-10) + 12\,a(n-11) + 165\,a(n-12) + 510\,a(n-13) + 814\,a(n-14) + 641\,a(n-15) + 80\,a(n-16) - 21\,a(n-17) - 118\,a(n-18) + 47\,a(n-19) - 4\,a(n-20) + 15\,a(n-21) - 7\,a(n-22)
\]
for every $n>23$.
\end{theorem}
...
integers vanish from the index corresponding to $n=23+1$ onwards, and the run was
continued until the working vector was identically zero, which settles all larger $j$ at once.
By Lemma~\ref{lem:crit} the recurrence holds for every $n>23$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196919.
