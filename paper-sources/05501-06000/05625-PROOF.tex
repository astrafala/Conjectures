...
\title{The conjectured generating function for OEIS A250750, proved}
...
\begin{abstract}
OEIS A250750 counts arrays of a fixed width under a local condition, and carries a conjectured
...
OEIS A250750 is ``Number of (n+1) X (3+1) 0..2 arrays with nondecreasing x(i,j)-x(i,j-1) in the i direction and nondecreasing x(i,j)+x(i-1,j) in the j direction.''. It has offset $1$ and begins
\[
129, 423, 1353, 4239, 13089, 40023,\ \dots
\]
The entry states, as a conjecture contributed by a reader and never marked settled:
\begin{quote}
Empirical g.f.: 3*x*(43 - 117*x + 78*x\^{}2) / ((1 - x)*(1 - 2*x)*(1 - 3*x)). - \_Colin Barker\_, Nov 17 2018
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 17 2018 13:12:25, revision 8) this is still
...
model reproduces all $23$ terms the entry publishes, exactly, in integer arithmetic --- a
...
First, the model reproduces every one of the $23$ published terms, with the index shift
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250750.
