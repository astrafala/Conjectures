...
\title{The empirical product formula for OEIS A140930, proved}
...
\begin{abstract}
OEIS A140930 counts $7\times10$ matrices over $\{0,\dots,n\}$ whose rows and columns are
nondecreasing, and carries a product formula for $a(n)$ marked empirical. Reversing rows and
columns turns such a matrix into a plane partition inside a $7\times10\times n$ box, so
MacMahon's box theorem applies; the triple product telescopes in its third index and leaves a
polynomial in $n$ of degree $70$. The entry's own expression, cleared of its denominator,
...
OEIS A140930 is ``Number of 7 X 10 matrices with elements in 0..n with each row and each column in nondecreasing order. 7,10,n can be permuted, see formula.''. It has offset $0$ and begins
\[
1, 19448, 77364144, 108284013552, 69951472754592, 24648355308799872,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 20 2025 10:52:10, revision 8) these are still
recorded as empirical, and nothing on the entry records them as proved.

\section{What is being counted}

A matrix $x$ of shape $7\times10$ with entries in $\{0,\dots,n\}$, nondecreasing along
...
$7\times10$ rectangle --- a plane partition in a $7\times10\times n$ box. The
correspondence is a bijection, being an involution on arrays, so
\[
a(n)\;=\;\mathrm{PP}(7,10,n).
\]
...
\[
a(n)\;=\;\prod_{i=1}^{7}\prod_{j=1}^{10}\frac{n+i+j-1}{i+j-1},
\]
a polynomial in $n$ of degree $70$ with rational coefficients.
\end{theorem}
...
$7\cdot10=70$ factors, each linear in $n$, and the denominators are constants.
\end{proof}
...
The entry writes $s=p+q+r-1$ with $(p,q,r)=(n,10,7)$ and claims
\[
...
$N$ and $D$ are polynomials in $n$ of degrees $91$ and $21$ respectively.
\end{lemma}

\begin{proof}
With $s=n+10+7-1$ one has $s-(n+i)=10+7-1-i$, a constant, so
\[
\binom{s}{n+i}=\binom{s}{10+7-1-i}
=\frac{(s-10-7+2+i)(s-10-7+3+i)\cdots s}{(10+7-1-i)!},
\]
a polynomial in $n$ of degree $10+7-1-i$. Multiplying over $i=0,\dots,7-1$ gives
$\deg N=\sum_{i=0}^{7-1}(10+7-1-i)=91$. The denominator is
...
of degree at most $70+21=91$. It was evaluated in exact rational arithmetic at
$n=0,1,\dots,91$, that is at $92$ points, and vanishes at all of them; a nonzero polynomial
of degree at most $91$ has at most $91$ roots, so it is identically zero. Since $D(n)\neq0$
...
Third, the polynomial identity was evaluated over the whole range $n=0,\dots,91$ rather than
sampled, in exact rational arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A140930.
