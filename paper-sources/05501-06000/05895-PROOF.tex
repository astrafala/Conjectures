...
\title{The empirical column recurrences for the table OEIS A295918}
...
\begin{abstract}
OEIS A295918 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A295918 is ``T(n,k)=Number of n X k 0..1 arrays with each 1 adjacent to 0 or 3 king-move neighboring 1s.''. It has offset $1$, is
read by antidiagonals, and begins
\[
2, 3, 3, 5, 6, 5, 8, 13,\ \dots
\]
...
k=6: [order 92]\\
\end{quote}
As of the ``Last modified'' line on the live entry (Sep 16 2025 22:24:18, revision 21) these are still
...
The sequence is ``Number of n X 1 0..1 arrays with each 1 adjacent to 0 or 3 king-move neighboring 1s.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 2 0..1 arrays with each 1 adjacent to 0 or 3 king-move neighboring 1s.''. Its rows are the vertices of a digraph on
...
The sequence is ``Number of n X 3 0..1 arrays with each 1 adjacent to 0 or 3 king-move neighboring 1s.''. Its rows are the vertices of a digraph on
...
model reproduces $28$ published values, $63$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A295918.
