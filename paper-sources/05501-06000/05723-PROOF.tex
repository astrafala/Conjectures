...
\title{The empirical closed form for OEIS A187048, proved}
...
\begin{abstract}
OEIS A187048 carries an empirical closed form for $a(n)$ --- not a recurrence relating
...
$S=10$ vertices. Any function of the form $\sum_j p_j(n)\lambda_j^{\,n}$ satisfies the
...
OEIS A187048 is ``Number of 4-step one or two space at a time bishop's tours on an n X n board summed over all starting positions.''. It has offset $1$ and begins
\[
0, 0, 24, 272, 1084, 2660, 5032, 8164,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 380*n\^{}2 - 2568*n + 4388 for n>5
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 03:51:53, revision 10) this is still
recorded as empirical, and nothing on the entry records it as proved. Write
\[
f(n)\;=\;380 n^{2} - 2568 n + 4388.
\]
...
walk, so with $M$ the adjacency matrix of that digraph on $S=10$ vertices, and $u,w$ the
...
vanish from the point corresponding to $n=9$ onwards, and the run was continued until the
working vector was identically zero, which settles every larger $j$ at once. So $a$ satisfies
\eqref{eq:rec} for every $n>8$. By Lemma~\ref{lem:ann} $f$ satisfies \eqref{eq:rec}
identically. Both were then evaluated in exact arithmetic at the $3$ consecutive indices
$n=9,\dots,11$ and agree at each. A monic recurrence of order $3$
determines $a(m)$ from $a(m-1),\dots,a(m-3)$, and for every $m\ge12$ both
$m>8$ and $m-3\ge9$ hold, so induction on $m$ gives $a(m)=f(m)$ for every
$m\ge9$.
Below $n=9$ the recurrence argument gives nothing, since \eqref{eq:rec} is only
established for $a$ from $n=9$ on. The remaining indices $n=6,\dots,8$ were
...
First, the walk model reproduces every one of the $35$ terms the entry publishes,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A187048.
