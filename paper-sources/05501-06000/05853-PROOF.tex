...
\title{The empirical column recurrences for the table OEIS A253698}
...
\begin{abstract}
OEIS A253698 is a two-dimensional table $T(n,k)$, and it carries a block of empirical
...
OEIS A253698 is ``T(n,k)=Number of (n+1)X(k+1) 0..1 arrays with every 2X2 subblock ne-sw antidiagonal difference unequal to its neighbors horizontally and nw+se diagonal sum unequal to its neighbors vertically.''. It has offset $1$, is
read by antidiagonals, and begins
\[
16, 40, 40, 104, 76, 104, 264, 150,\ \dots
\]
...
k=5: [order 26] for n>31\\
k=6: [order 28] for n>33\\
k=7: [order 32] for n>38\\
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 14:10:09, revision 6) these are still
...
The sequence is ``Number of (n+1)X2 0..1 arrays with every 2X2 subblock ne-sw antidiagonal difference unequal to its neighbors horizontally and nw+se diagonal sum unequal to its neighbors vertically.''. Its rows are the vertices of a digraph on
...
model reproduces $10$ published values, $36$ decimal digits in all, every one of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253698.
