...
\title{The empirical recurrence for OEIS A283508, proved from an exact model}
...
\begin{abstract}
OEIS A283508 carries an empirical recurrence of order $2$ contributed by Colin Barker. It is true, and it is
...
OEIS A283508 is ``Binary representation of the x-axis, from the origin to the right edge, of the n-th stage of growth of the two-dimensional cellular automaton defined by "Rule 643", based on the 5-celled von Neumann neighborhood.''. It has offset $0$ and begins
\[
1, 10, 101, 1011, 10111, 101111, 1011111, 10111111,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Conjectures from \_Colin Barker\_, Mar 10 2017: (Start) a(n) = 11*a(n-1) - 10*a(n-2) for n>2.
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 05 2025 15:22:37, revision 16)
...
\[
C(n+1)\;=\;C(n)\qquad\text{as configurations of the whole plane, for every } n\ge 2.
\]
...
neighbourhood to a live cell flips the ENTIRE plane, and rule 643 is read
...
vanish from $n=2+1$ onwards, and the run exceeds $S+2$, which by
Lemma~\ref{lem:crit} settles every larger index at once. The bound is exact: at $n=2$ the recurrence fails on the entry's own published terms, so no larger range holds.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A283508.
