...
\title{The empirical recurrence for OEIS A197883, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A197883 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A197883 is ``Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 0,2,1,1,1 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
0, 4, 3, 14, 18, 43, 78, 154,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-2) +2*a(n-3) +a(n-4) -3*a(n-5) -4*a(n-6) +7*a(n-7) -5*a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (May 14 2018 06:34:30, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-2) + 2\,a(n-3) + a(n-4) - 3\,a(n-5) - 4\,a(n-6) + 7\,a(n-7) - 5\,a(n-9)
\end{equation}
for all $n>10$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(0,2,1,1,1)
\end{equation}
for every cell $(t,u)$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A197883.
