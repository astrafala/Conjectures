...
\title{The empirical recurrence for OEIS A250426, proved by transfer matrix}
...
\begin{abstract}
OEIS A250426 counts arrays over $\{0,1\}$ in which a statistic taken over short runs
...
window of consecutive rows, so the count is a walk count on $S=36$ states, and the
...
OEIS A250426 is ``Number of (n+1)X(2+1) 0..1 arrays with nondecreasing sum of every two consecutive values in every row and column.''. It has offset $1$ and begins
\[
36, 108, 324, 720, 1600, 3000, 5625, 9450,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 14 2018 09:33:13, revision 9) this is still
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250426.
