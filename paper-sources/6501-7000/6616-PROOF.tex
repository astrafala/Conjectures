...
\title{The empirical recurrence for OEIS A203050, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A203050 counts arrays over $\{0,\dots,5\}$ in which every cell is constrained by the
...
with $S=36$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A203050 is ``Number of nX1 0..5 arrays with every nonzero element less than or equal to some horizontal or vertical neighbor.''. It has offset $1$ and begins
\[
1, 6, 36, 161, 636, 2507, 10213, 42182,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) -15*a(n-2) +35*a(n-3) -35*a(n-4) +56*a(n-5) -28*a(n-6) +36*a(n-7) -9*a(n-8) +10*a(n-9) -a(n-10) +a(n-11)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 10:04:39, revision 9) the statement is
...
Let $V=\{0,\dots,5\}^{1}$ be the possible rows and take
$\mathcal V=V\times V$, of size $S=36$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
publishes, $n=23$, the arrays have $1\cdot23$ cells over
$6$ values, so there are about $10^{17}$ of them to sift. The
walk count replaces that by $36$ states and one matrix--vector product per row.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=36$ pairs and each
of the $(5+1)^{1}$ possible next rows, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A203050.
