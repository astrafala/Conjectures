...
\title{The empirical recurrence for OEIS A298439, proved by transfer matrix}
...
\begin{abstract}
OEIS A298439 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{0,3,4,5,6\}$, and it carries an empirical
...
OEIS A298439 is ``Number of nX3 0..1 arrays with every element equal to 0, 3, 4, 5 or 6 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
1, 1, 11, 2, 27, 36, 86, 162,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 19 2018 08:00:28, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 3\,a(n-2) - 4\,a(n-3) - 4\,a(n-4) - 2\,a(n-5) + 2\,a(n-6) + 5\,a(n-7) - 2\,a(n-8) + 6\,a(n-9) - a(n-10) - a(n-11) - a(n-12)
\end{equation}
for all $n>13$.
...
has score in $\{0,3,4,5,6\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,3,4,5,6\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,3,4,5,6\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,3,4,5,6\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,3,4,5,6\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A298439.
