...
\title{The empirical recurrence for OEIS A184370, proved by transfer matrix}
...
\begin{abstract}
OEIS A184370 counts $(n+1)\times4$ arrays over $\{0,\dots,2\}$ subject to a condition imposed on
every $2\times2$ subblock, and carries an empirical recurrence of order $9$ contributed
...
number of walks: $a(n)=\tfrac1{3}\mathbf{1}^{\!\top}M^{n}\mathbf{1}$. Writing $q$
...
OEIS A184370 is ``1/3 the number of (n+1)X4 0..2 arrays with every 2X2 subblock having at least two equal elements connected horizontally or vertically.''. It has offset $1$ and begins
\[
1107, 49235, 2167173, 95528423, 4210059673, 185547841629, 8177524177277, 360402692568245,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n)=38*a(n-1)+302*a(n-2)-1368*a(n-3)-6770*a(n-4)+12666*a(n-5)-575*a(n-6)-5790*a(n-7)+828*a(n-8)+648*a(n-9)
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 03 2025 02:34:01, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;38\,a(n-1) + 302\,a(n-2) - 1368\,a(n-3) - 6770\,a(n-4) + 12666\,a(n-5) - 575\,a(n-6) - 5790\,a(n-7) + 828\,a(n-8) + 648\,a(n-9)
\end{equation}
for all $n>8$.
...
\alpha=\beta\ \text{ or }\ \gamma=\delta\ \text{ or }\ \alpha=\gamma\ \text{ or }\ \beta=\delta,
\end{equation}
...
\[
a(n)\;=\;\tfrac1{3}\mathbf{1}^{\!\top}M^{n}\mathbf{1} .
\]
\end{lemma}
...
length $k$ from $r$ to $s$. The entry's index $n$ corresponds to $L=n+1$ rows. The entry counts $\tfrac1{3}$ of those arrays, a constant factor that passes through every step below unchanged.
\end{proof}
...
For every $n$ with $n+1-1\ge9$,
\[
a(n)-\sum_i c_i\,a(n-i)\;=\;\tfrac1{3}\mathbf{1}^{\!\top}M^{\,n+1-1-9}\,q(M)\,\mathbf{1} .
\]
...
\begin{proof}
By Lemma~\ref{lem:walk}, $a(n-i)=\tfrac1{3}\mathbf{1}^{\!\top}M^{\,n+1-1-i}\mathbf{1}$, so
\[
a(n)-\sum_i c_i a(n-i)
=\tfrac1{3}\mathbf{1}^{\!\top}\Bigl(M^{m}-\sum_i c_i M^{m-i}\Bigr)\mathbf{1}
=\tfrac1{3}\mathbf{1}^{\!\top}M^{\,m-9}\,q(M)\,\mathbf{1}
\]
with $m=n+1-1$, which is the stated identity; the scalar $\tfrac1{3}$ never vanishes, so it does not affect whether the left side is zero, and the equivalence follows by letting
$m-9=j$ run over $\ge0$. For the last clause put $w=q(M)\mathbf{1}$. By the
...
$S^2$ pairs. The vector $w=q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was then formed by $9$
...
index corresponding to $n=8+1$ onwards, and the run continued until $w$ itself was the
...
$\tfrac1{3}\mathbf{1}^{\!\top}M^{L-1}\mathbf{1}$ compared against the entry's DATA at
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184370.
