...
\title{The empirical recurrence for OEIS A235169, proved by transfer matrix}
...
\begin{abstract}
OEIS A235169 counts $2$-column arrays over $\{0,\dots,7\}$ in which every
...
carries an empirical recurrence of order $9$ contributed by R.~H.~Hardin. It is true,
...
the residual is $\mathbf{1}^{\!\top}M^{\,n-9}q(M)\mathbf{1}$, so the recurrence
...
OEIS A235169 is ``Number of (n+1) X (1+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
268, 1528, 8688, 49464, 281580, 1603344, 9129276,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 6*a(n-1) +37*a(n-2) -234*a(n-3) -141*a(n-4) +1290*a(n-5) +108*a(n-6) -2340*a(n-7) +1296*a(n-9).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 02:12:44, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;6\,a(n-1) + 37\,a(n-2) - 234\,a(n-3) - 141\,a(n-4) + 1290\,a(n-5) + 108\,a(n-6) - 2340\,a(n-7) + 1296\,a(n-9)
\end{equation}
...
For $n\ge9$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-9}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-9$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-9}\bigl(M^{9}-\sum_i c_i M^{9-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-9$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $9$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=9-9+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>9$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235169.
