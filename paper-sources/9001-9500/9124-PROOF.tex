...
\title{The empirical recurrence for OEIS A233166, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233166 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 09 2018 15:13:04, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{1425408 x^{3} - 41216 x^{2} + 236 x}{8192 x^{2} - 192 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;8192 x^{2} - 192 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;1425408 x^{3} - 41216 x^{2} + 236 x,
\]
