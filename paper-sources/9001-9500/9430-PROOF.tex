...
\title{The empirical recurrence for OEIS A268052, derived from the entry's generating function}
...
\begin{abstract}
OEIS A268052 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 11 2019 09:06:35, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 26 x^{3} + 13 x^{2} - x}{140 x^{3} - 108 x^{2} + 21 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 140 x^{3} + 108 x^{2} - 21 x + 1
\]
