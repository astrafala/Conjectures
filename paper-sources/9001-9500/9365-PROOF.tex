...
\title{A proof of the conjectured recurrence for OEIS A166588}
...
\begin{abstract}
OEIS A166588 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A166588 is ``Partial sums of A097331; binomial transform of A166587''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 2, 3, 3, 5, 5, 10,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x + \sqrt{1 - 4 x^{2}} - 1}{2 x \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n-1)*a(n) + (n+1)*a(n-1) + 4*(n-2)*a(n-2) + 4*(-n+2)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 15 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2016-05-19, revision 14) the
...
\[
p_{0}(n)=- n - 1,\qquad p_{1}(n)=n + 1,\qquad p_{2}(n)=4 \left(n - 2\right),\qquad p_{3}(n)=- 4 \left(n - 2\right)
\]
...
\section{The computation for A166588}
...
\[
(- n - 1)\,a(n) + (n + 1)\,a(n-1) + 4 \left(n - 2\right)\,a(n-2) -  4 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A166588, agreeing on all 13 terms compared.
...
vanish, for all 35 values of $n$ from $n=3$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A166588.
