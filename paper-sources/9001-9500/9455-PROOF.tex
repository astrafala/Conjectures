...
\title{A proof of the conjectured recurrence for OEIS A159769}
...
\begin{abstract}
OEIS A159769 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{4} + 2 x^{3} - 9 x^{2} + 4 x$, a polynomial of degree $4$, which proves
...
OEIS A159769 is ``Number of n-leaf binary trees that do not contain (((()())())(()(()()))) as a subtree''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 2, 5, 14, 41, 124, 384,\ \dots
\]
The entry posts no generating function of its own. Its NAME states the
relation \texttt{the entry states that the generating function satisfies G.f. f(x) satisfies (x-2) x f(x)\textasciicircum{}2 + (2 x\textasciicircum{}2 - 2 x + 1) f(x) + (x-1) x = 0} to another entry, and that entry's exponential generating
...
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{- x^{2} + x + \frac{\sqrt{4 x^{3} - 4 x + 1}}{2} - \frac{1}{2}}{x \left(x - 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*(n+1)*a(n) +3*(-3*n+1)*a(n-1) +2*(2*n-1)*a(n-2) +4*(2*n-7)*a(n-3) +2*(-2*n+7)*a(n-4)=0. - \emph{R. J. Mathar}, May 30 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 22) the
...
\[
p_{0}(n)=2 \left(n + 1\right),\qquad p_{1}(n)=- 3 \left(3 n - 1\right),\qquad p_{2}(n)=2 \left(2 n - 1\right),\qquad p_{3}(n)=4 \left(2 n - 7\right),\qquad p_{4}(n)=- 2 \left(2 n - 7\right)
\]
...
\section{The computation for A159769}
...
\[
2 \left(n + 1\right)\,a(n) -  3 \left(3 n - 1\right)\,a(n-1) + 2 \left(2 n - 1\right)\,a(n-2) + 4 \left(2 n - 7\right)\,a(n-3) -  2 \left(2 n - 7\right)\,a(n-4) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A159769, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{4} + 2 x^{3} - 9 x^{2} + 4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A159769.
