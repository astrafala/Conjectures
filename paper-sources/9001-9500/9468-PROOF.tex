...
\title{The empirical recurrence for OEIS A223228, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223228 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A223228 is ``Rolling icosahedron footprints: number of n X 3 0..11 arrays starting with 0 where 0..11 label vertices of an icosahedron and every array movement to a horizontal, diagonal or antidiagonal neighbor moves along an icosahedral edge.''. It has offset $1$ and begins
\[
25, 785, 25225, 812225, 26157625, 842416625, 27130395625, 873746350625,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 17 2018 09:22:28, revision 10) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 90 x^{2} + 25 x}{90 x^{2} - 35 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;90 x^{2} - 35 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223228.
