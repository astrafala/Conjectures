...
\title{The empirical recurrence for OEIS A268775, derived from the entry's generating function}
...
\begin{abstract}
OEIS A268775 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 16 2025 07:35:42, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{4} - 8 x^{3} + 3 x^{2} + 4 x}{4 x^{4} + 4 x^{3} - 3 x^{2} - 2 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{4} + 4 x^{3} - 3 x^{2} - 2 x + 1
\]
