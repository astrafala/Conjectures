...
\title{The empirical recurrence for OEIS A208641, derived from the entry's generating function}
...
\begin{abstract}
OEIS A208641 carries an empirical recurrence of order $6$. The entry separately records a
...
OEIS A208641 is ``Number of 6 X n 0..1 arrays with new values 0..1 introduced in row major order and no element equal to more than one of its immediate leftward or upward or right-upward antidiagonal neighbors.''. It has offset $1$ and begins
\[
32, 233, 851, 2331, 5572, 12404, 26508, 55260,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 22 2025 21:27:21, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{8 x^{9} + 4 x^{8} - 34 x^{7} - 37 x^{6} + 85 x^{5} + 74 x^{4} - 140 x^{3} + 9 x^{2} + 32 x}{2 x^{6} - 11 x^{5} + 25 x^{4} - 30 x^{3} + 20 x^{2} - 7 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{6} - 11 x^{5} + 25 x^{4} - 30 x^{3} + 20 x^{2} - 7 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;7\,a(n-1) - 20\,a(n-2) + 30\,a(n-3) - 25\,a(n-4) + 11\,a(n-5) - 2\,a(n-6)
\]
for every $n>9$.
\end{theorem}
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;8 x^{9} + 4 x^{8} - 34 x^{7} - 37 x^{6} + 85 x^{5} + 74 x^{4} - 140 x^{3} + 9 x^{2} + 32 x,
\]
...
\[
2 x^{6} - 11 x^{5} + 25 x^{4} - 30 x^{3} + 20 x^{2} - 7 x + 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A208641.
