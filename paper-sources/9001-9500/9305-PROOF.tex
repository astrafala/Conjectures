...
\title{A proof of the conjectured recurrence for OEIS A248434}
...
\begin{abstract}
OEIS A248434 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A248434 is ``Number of length three 0..n arrays with the sum of two elements equal to twice the third''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;2, 9, 16, 29, 42, 61, 80, 105,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = A319127(n+1) + n + 1
\end{quote}
Substituting the formula that A319127 states for itself, and simplifying, this reads
\begin{equation}\label{eq:cf}
a(n)\;=\;- \frac{3 \left(-1\right)^{n + 1}}{4} + n + \frac{3 \left(n - 1\right) \left(n + 1\right)}{2} + \frac{7}{4} .
\end{equation}
...
\end{quote}
As of the ``Last modified'' line on the live entry (2020-08-29, revision 42) the
...
\section{The computation for A248434}

The closed form \eqref{eq:cf} splits into two similarity classes, with
representativees
\[
\frac{1}{4},\qquad \frac{3 \left(-1\right)^{n}}{4}
\]
...
exact integer arithmetic at every index where it applies: $45$ instances beginning at
...
entry \href{https://oeis.org/A248434}{A248434}, 2020-08-29.
