...
\title{A proof of the conjectured recurrence for OEIS A228333}
...
\begin{abstract}
OEIS A228333 carries a conjectured linear recurrence with polynomial coefficients, of
order 2. The entry separately states a recurrence of order 1 that is not
...
OEIS A228333 is ``Let h(m) denote the sequence whose n-th term is Sum\_\{k=0..n\} (k+1)\^{}m*T(n,k)\^{}2, where T(n,k) is the Catalan triangle A039598. This is h(7)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 132, 4260, 120400, 3017700, 69776784, 1524611088, 31951782720,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} n\^{}2*(304*n-411)*a(n) + 4*(-1814*n\^{}3+2554*n\^{}2-4776*n+7567)*a(n-1) + 32*(2*n-5)*(2*n-1)*(299*n-176)*a(n-2) = 0. - \emph{R. J. Mathar}, Dec 04 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-16, revision 39) it is still
...
\begin{quote}\small
Recurrence: n\^{}2*(6*n\^{}3 - 12*n\^{}2 + 6*n - 1)*a(n) = 4*(2*n-3)*(2*n+1)*(6*n\^{}3 + 6*n\^{}2 - 1)*a(n-1).
\end{quote}
...
\section{The computation for A228333}

Reading both relations as operators and dividing the conjectured one by the stated one
leaves remainder $0$, with quotient
\[
Q \;=\; \left(\frac{- 2392 n - 3376}{6 n^{3} + 24 n^{2} + 30 n + 11}\right) + \left(\frac{304 n + 197}{6 n^{3} + 24 n^{2} + 30 n + 11}\right)\,N .
\]
...
\[
n^{2} \left(304 n - 411\right)\,a(n) -  4 \left(1814 n^{3} - 2554 n^{2} + 4776 n - 7567\right)\,a(n-1) + 32 \left(2 n - 5\right) \left(2 n - 1\right) \left(299 n - 176\right)\,a(n-2) \;=\;0
\]
...
entry's own values and found to vanish, for all 17 values of $n$ from
$n=2$ upward.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A228333.
