...
\title{A proof of the conjectured recurrence for OEIS A166078}
...
\begin{abstract}
OEIS A166078 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 6 x$, a polynomial of degree $1$, which proves
...
OEIS A166078 is ``Expansion of (3(1-x)-sqrt(1+6x-7x\^{}2))/(2(1-x)(1-2x))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 2, -6, 30, -150, 806, -4494,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{4}{4 x + \left(x - 1\right) \left(\sqrt{- \frac{7 x + 1}{x - 1}} - 1\right) - 4} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + 2*(2*n-5)*a(n-1) + (34-19*n)*a(n-2) + 14*(n-2)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 14 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2016-04-24, revision 13) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=2 \left(2 n - 5\right),\qquad p_{2}(n)=34 - 19 n,\qquad p_{3}(n)=14 \left(n - 2\right)
\]
...
\section{The computation for A166078}
...
\[
n\,a(n) + 2 \left(2 n - 5\right)\,a(n-1) + (34 - 19 n)\,a(n-2) + 14 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A166078, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 6 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A166078.
