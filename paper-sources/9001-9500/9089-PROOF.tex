...
\title{The empirical recurrence for OEIS A221731, derived from the entry's generating function}
...
\begin{abstract}
OEIS A221731 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A221731 is ``Number of n X 2 arrays of occupancy after each element stays put or moves to some horizontal or antidiagonal neighbor, without move-in move-out left turns.''. It has offset $1$ and begins
\[
3, 17, 91, 489, 2627, 14113, 75819, 407321,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 14 2018 11:05:20, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{2} - 3 x}{2 x^{2} + 5 x - 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A221731.
