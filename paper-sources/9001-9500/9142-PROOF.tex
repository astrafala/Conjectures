...
\title{The empirical recurrence for OEIS A222939, derived from the entry's generating function}
...
\begin{abstract}
OEIS A222939 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A222939 is ``Number of n X 1 0..4 arrays with no element equal to another at a city block distance of exactly two, and new values 0..4 introduced in row major order.''. It has offset $1$ and begins
\[
1, 2, 3, 7, 20, 66, 238, 902,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 16 2018 13:23:13, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{5} - 6 x^{4} - 3 x^{3} + 5 x^{2} - x}{8 x^{3} - 14 x^{2} + 7 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 3 x^{5} + 6 x^{4} + 3 x^{3} - 5 x^{2} + x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A222939.
