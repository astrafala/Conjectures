...
\title{The empirical recurrence for OEIS A262482, derived from the entry's generating function}
...
\begin{abstract}
OEIS A262482 carries an empirical recurrence of order $8$. The entry separately records a
...
OEIS A262482 is ``Number of (n+3)X(1+3) 0..1 arrays with each row and column divisible by 13, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
2, 3, 5, 10, 20, 40, 79, 158,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 16 2024 06:30:43, revision 13) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2 x^{8} + x^{7} + x^{4} - 3 x^{2} + 2 x}{2 x^{8} - 3 x^{7} + x^{6} + 2 x^{2} - 3 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{8} - 3 x^{7} + x^{6} + 2 x^{2} - 3 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 2 x^{8} + x^{7} + x^{4} - 3 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262482.
