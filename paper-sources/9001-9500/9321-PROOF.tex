...
\title{The empirical recurrence for OEIS A185761, derived from the entry's generating function}
...
\begin{abstract}
OEIS A185761 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A185761 is ``Number of (n+1) X 2 binary arrays with no 2 X 2 subblock trace equal to any horizontal or vertical neighbor 2 X 2 subblock trace.''. It has offset $1$ and begins
\[
16, 40, 104, 264, 680, 1736, 4456, 11400,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 22 2018 09:13:55, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 24 x^{2} - 16 x}{4 x^{2} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 4 x^{2} - x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A185761.
