...
to the polynomial $20 x^{3}$, of degree $3$, which proves the recurrence for every
$n>3$.
\end{abstract}
...
\begin{quote}\small
\textbf{Conjecture:} (n-2)*(n-4)*a(n) -(n-1)*(3*n\textasciicircum{}2-21*n+35)*a(n-1) +(n-1)*(n-2)*(3*n\textasciicircum{}2-24*n+47)*a(n- 2) -(n-5)*(n-1)*(n-2)*(n-3)*(n-4)*a(n-3)=0.
\end{quote}
As of the ``Last modified'' line on the live entry (2026-07-11, revision 42) the
...
\[
p_{0}(n)=\left(n - 4\right) \left(n - 2\right),\qquad p_{1}(n)=- \left(n - 1\right) \left(3 n^{2} - 21 n + 35\right),\qquad p_{2}(n)=\left(n - 2\right) \left(n - 1\right) \left(3 n^{2} - 24 n + 47\right),\qquad p_{3}(n)=- \left(n - 5\right) \left(n - 4\right) \left(n - 3\right) \left(n - 2\right) \left(n - 1\right)
\]
...
\[
\left(n - 4\right) \left(n - 2\right)\,a(n) -  \left(n - 1\right) \left(3 n^{2} - 21 n + 35\right)\,a(n-1) + \left(n - 2\right) \left(n - 1\right) \left(3 n^{2} - 24 n + 47\right)\,a(n-2) -  \left(n - 5\right) \left(n - 4\right) \left(n - 3\right) \left(n - 2\right) \left(n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>6$.
\end{theorem}
...
vanish, for all 13 values of $n$ from $n=13$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=20 x^{3}$ is an
