...
\title{The empirical recurrence for OEIS A231104, derived from the entry's generating function}
...
\begin{abstract}
OEIS A231104 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A231104 is ``Number of n X 4 0..3 arrays x(i,j) with each element horizontally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, no adjacent elements equal, and upper left element zero.''. It has offset $1$ and begins
\[
0, 6, 48, 438, 3936, 35430, 318864, 2869782,\ \dots
\]
...
G.f.: 6*x\^{}2 / ((1 + x)*(1 - 9*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (May 25 2021 05:18:42, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;- \frac{6 x^{2}}{9 x^{2} + 8 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 9 x^{2} - 8 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231104.
