...
\title{A proof of the conjectured recurrence for OEIS A162475}
...
\begin{abstract}
OEIS A162475 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $3 x^{5} - 11 x^{4} + 14 x^{3} - 6 x^{2} - x + 1$, a polynomial of degree $5$, which proves
...
OEIS A162475 is ``Expansion of c(x/(1-x)\textasciicircum{}4), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 6, 31, 166, 931, 5412, 32334,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\left(1 - \sqrt{\frac{- 4 x + \left(1 - x\right)^{4}}{\left(x - 1\right)^{4}}}\right) \left(x - 1\right)^{4}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +3*(2-3n)*a(n-1) +2*(7n-20)*a(n-2) +2*(22-5n)*a(n-3) +(5n-31)*a(n-4) +(8-n)*a(n-5)=0. - \emph{R. J. Mathar}, Nov 17 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-07-07, revision 9) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 3 \left(3 n - 2\right),\qquad p_{2}(n)=2 \left(7 n - 20\right),\qquad p_{3}(n)=- 2 \left(5 n - 22\right),\qquad p_{4}(n)=5 n - 31,\qquad p_{5}(n)=8 - n
\]
...
\section{The computation for A162475}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \frac{x^{4} - 4 x^{3} + 6 x^{2} - 8 x + 1}{\left(x - 1\right)^{4}} .
\]
...
\[
B(x)\;=\;3 x^{5} - 11 x^{4} + 14 x^{3} - 6 x^{2} - x + 1 ,
\]
...
\[
(n + 1)\,a(n) -  3 \left(3 n - 2\right)\,a(n-1) + 2 \left(7 n - 20\right)\,a(n-2) -  2 \left(5 n - 22\right)\,a(n-3) + (5 n - 31)\,a(n-4) + (8 - n)\,a(n-5) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A162475, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=3 x^{5} - 11 x^{4} + 14 x^{3} - 6 x^{2} - x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A162475.
