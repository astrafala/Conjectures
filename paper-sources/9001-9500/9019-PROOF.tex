...
\title{A proof of the conjectured recurrence for OEIS A174107}
...
\begin{abstract}
OEIS A174107 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A174107 is ``Expansion of (1/(1-x+x\^{}2))c(x/(1-x+x\^{}2)), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 4, 11, 37, 134, 505, 1964,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{x^{2} - 5 x + 1}{x^{2} - x + 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +2*(1-3n)*a(n-1) +7*(n-1)*a(n-2) +2*(5-3*n)*a(n-3) +(n-3)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2014-02-05, revision 16) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 2 \left(3 n - 1\right),\qquad p_{2}(n)=7 \left(n - 1\right),\qquad p_{3}(n)=- 2 \left(3 n - 5\right),\qquad p_{4}(n)=n - 3
\]
...
\section{The computation for A174107}
...
\[
(n + 1)\,a(n) -  2 \left(3 n - 1\right)\,a(n-1) + 7 \left(n - 1\right)\,a(n-2) -  2 \left(3 n - 5\right)\,a(n-3) + (n - 3)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A174107, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A174107.
