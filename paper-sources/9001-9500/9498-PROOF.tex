...
\title{The empirical recurrence for OEIS A222834, derived from the entry's generating function}
...
\begin{abstract}
OEIS A222834 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 16 2018 08:59:10, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 17 x^{4} + 10 x^{3} + 82 x^{2} + 7 x}{x^{3} - 2 x^{2} - 2 x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 17 x^{4} + 10 x^{3} + 82 x^{2} + 7 x,
\]
