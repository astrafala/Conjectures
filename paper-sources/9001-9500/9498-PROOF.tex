...
\title{The empirical recurrence for OEIS A223322, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223322 carries an empirical recurrence of order $1$. The entry separately records a
...
OEIS A223322 is ``Rolling icosahedron footprints: number of 2 X n 0..11 arrays starting with 0 where 0..11 label vertices of an icosahedron and every array movement to a horizontal or antidiagonal neighbor moves along an icosahedral edge.''. It has offset $1$ and begins
\[
12, 125, 1625, 21125, 274625, 3570125, 46411625, 603351125,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 29 2023 11:16:29, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{31 x^{2} - 12 x}{13 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - 13 x
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223322.
