...
\title{A proof of the conjectured recurrence for OEIS A220250}
...
\begin{abstract}
OEIS A220250 carries a conjectured linear recurrence with polynomial coefficients, of
order 5. The entry posts no generating function, so the usual route through a
...
OEIS A220250 is ``Sum of neighbor maps: number of nX2 binary arrays indicating the locations of corresponding elements equal to the sum mod 3 of their king-move neighbors in a random 0..2 nX2 array''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;2, 16, 48, 256, 928, 4096, 15872, 65536,\ \dots
\]
The entry states, not as a conjecture,
\begin{quote}\small
a(n) = 4\textasciicircum{}n - 2\textasciicircum{}(n-2)*(n+1)*(1-(-1)\textasciicircum{}n).
\end{quote}
that is,
\begin{equation}\label{eq:cf}
a(n)\;=\;- 2^{n - 2} \left(1 - \left(-1\right)^{n}\right) \left(n + 1\right) + 4^{n} .
\end{equation}
...
\end{quote}
As of the ``Last modified'' line on the live entry (2025-10-03, revision 12) the
...
1\,a(n) - 4\,a(n-1) - 8\,a(n-2) + 32\,a(n-3) + 16\,a(n-4) - 64\,a(n-5)
\end{equation}
...
\section{The computation for A220250}

The closed form \eqref{eq:cf} splits into 3 similarity classes, with
representativees
\[
4^{n},\qquad - \frac{2^{n}}{4},\qquad \frac{\left(-1\right)^{n} 2^{n}}{4}
\]
...
exact integer arithmetic at every index where it applies: $18$ instances beginning at
...
entry \href{https://oeis.org/A220250}{A220250}, 2025-10-03.
