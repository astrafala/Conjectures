...
\title{The empirical recurrence for OEIS A235881, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235881 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A235881 is ``Number of (n+1) X (5+1) 0..2 arrays with the minimum plus the upper median equal to the lower median plus the maximum in every 2 X 2 subblock.''. It has offset $1$ and begins
\[
921, 1311, 2109, 3759, 7221, 14631, 30909, 67839,\ \dots
\]
...
\begin{quote}
G.f.: 3*x*(307 - 1405*x + 1458*x\^{}2) / ((1 - x)*(1 - 2*x)*(1 - 3*x)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 19 2018 09:12:19, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4374 x^{3} + 4215 x^{2} - 921 x}{6 x^{3} - 11 x^{2} + 6 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;4374 x^{3} - 4215 x^{2} + 921 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235881.
