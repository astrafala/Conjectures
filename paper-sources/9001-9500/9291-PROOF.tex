...
\title{A proof of the conjectured recurrence for OEIS A095981}
...
\begin{abstract}
OEIS A095981 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $x^{6} - x^{5} + 6 x^{4} - 9 x^{3} + 4 x^{2}$, a polynomial of degree $6$, which proves
...
OEIS A095981 is ``Number of plateau-free Motzkin paths of length n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 2, 5, 11, 26, 61,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{3} + x^{2} + 2 x + \sqrt{x^{6} + 2 x^{5} - 7 x^{4} + 6 x^{3} + 2 x^{2} - 4 x + 1} - 1}{2 x^{2} \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+2)*a(n) +(-5*n-4)*a(n-1) +6*n*a(n-2) +(4*n-13)*a(n-3) +(-13*n+43)*a(n-4) +3*(3*n-13)*a(n-5) +(-n+4)*a(n-6) +(-n+7)*a(n-7)=0. - \emph{R. J. Mathar}, Nov 23 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2014-11-23, revision 5) the
...
\[
p_{0}(n)=n + 2,\qquad p_{1}(n)=- 5 n - 4,\qquad p_{2}(n)=6 n,\qquad p_{3}(n)=4 n - 13,\qquad p_{4}(n)=43 - 13 n,\qquad p_{5}(n)=3 \left(3 n - 13\right),\qquad p_{6}(n)=4 - n,\qquad p_{7}(n)=7 - n
\]
...
\section{The computation for A095981}
...
\[
(n + 2)\,a(n) + (- 5 n - 4)\,a(n-1) + 6 n\,a(n-2) + (4 n - 13)\,a(n-3) + (43 - 13 n)\,a(n-4) + 3 \left(3 n - 13\right)\,a(n-5) + (4 - n)\,a(n-6) + (7 - n)\,a(n-7) \;=\;0
\]
holds for every $n>6$.
\end{theorem}
...
section of A095981, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=x^{6} - x^{5} + 6 x^{4} - 9 x^{3} + 4 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A095981.
