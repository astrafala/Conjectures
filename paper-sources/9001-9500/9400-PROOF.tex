...
\title{The empirical recurrence for OEIS A223204, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223204 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 17 2018 09:22:07, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{2} + 9 x}{2 x^{2} - 9 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{2} - 9 x + 1
\]
