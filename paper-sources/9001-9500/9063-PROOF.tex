...
\title{A proof of the conjectured recurrence for OEIS A097192}
...
\begin{abstract}
OEIS A097192 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A097192 is ``Main diagonal of triangle A097190, in which the n-th row polynomial R\_n(y) is formed from the initial (n+1) terms of g.f. A097191(y)\textasciicircum{}(n+1), where R\_n(1/3) = 9\textasciicircum{}n for all n>=0''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 24, 612, 15912, 417690, 11027016, 292215924, 7764594552,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1}{\left(1 - 27 x\right)^{\frac{8}{9}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +3*(1-9*n)*a(n-1) = 0. - \emph{R. J. Mathar}, Nov 16 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 10) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 3 \left(9 n - 1\right)
\]
...
\section{The computation for A097192}

The generating function \eqref{eq:gf} is algebraic over $\mathbb{Q}(x)$;
its minimal polynomial is
\[
P(x,y) \;=\; 282429536481 x^{8} y^{9} - 83682825624 x^{7} y^{9} + 10847773692 x^{6} y^{9} - 803538792 x^{5} y^{9} + 37200870 x^{4} y^{9} - 1102248 x^{3} y^{9} + 20412 x^{2} y^{9} - 216 x y^{9} + y^{9} - 1 ,
\]
...
\[
n\,a(n) -  3 \left(9 n - 1\right)\,a(n-1) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A097192, agreeing on all 13 terms compared.
...
vanish, for all 15 values of $n$ from $n=1$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A097192.
