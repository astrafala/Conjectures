...
\title{The empirical recurrence for OEIS A223711, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223711 carries an empirical recurrence of order $5$. The entry separately records a
...
OEIS A223711 is ``Number of n X 2 0..1 arrays with row sums and column sums unimodal.''. It has offset $1$ and begins
\[
4, 16, 53, 155, 418, 1066, 2611, 6205,\ \dots
\]
...
\begin{quote}
G.f.: x*(2 - 3*x + 2*x\^{}2)\^{}2 / ((1 - x)\^{}3*(1 - 2*x)\^{}2).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 16 2018 10:26:42, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 4 x^{5} + 12 x^{4} - 17 x^{3} + 12 x^{2} - 4 x}{4 x^{5} - 16 x^{4} + 25 x^{3} - 19 x^{2} + 7 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 4 x^{5} + 16 x^{4} - 25 x^{3} + 19 x^{2} - 7 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;4 x^{5} - 12 x^{4} + 17 x^{3} - 12 x^{2} + 4 x,
\]
...
\[
4 x^{5} - 16 x^{4} + 25 x^{3} - 19 x^{2} + 7 x - 1,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223711.
