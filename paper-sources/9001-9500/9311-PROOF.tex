...
\title{A proof of the conjectured recurrence for OEIS A103973}
...
\begin{abstract}
OEIS A103973 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $272 x^{2} + 64 x$, a polynomial of degree $2$, which proves
...
OEIS A103973 is ``Expansion of (sqrt(1-8*x\^{}2)+8*x\^{}2+2*x-1)/(2*x*sqrt(1-8*x\^{}2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 4, 4, 24, 16, 160, 80,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{8 x^{2} + 2 x + \sqrt{1 - 8 x^{2}} - 1}{2 x \sqrt{1 - 8 x^{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: 11*n*(n+1)*a(n)+4*n*(4*n+1)*a(n-1) +8*(27-11*n\^{}2)*a(n-2) -32*(4*n+9)*(n-3)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 09 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2021-11-19, revision 16) the
...
\[
p_{0}(n)=11 n \left(n + 1\right),\qquad p_{1}(n)=4 n \left(4 n + 1\right),\qquad p_{2}(n)=- 8 \left(11 n^{2} - 27\right),\qquad p_{3}(n)=- 32 \left(n - 3\right) \left(4 n + 9\right)
\]
...
\section{The computation for A103973}
...
\[
11 n \left(n + 1\right)\,a(n) + 4 n \left(4 n + 1\right)\,a(n-1) -  8 \left(11 n^{2} - 27\right)\,a(n-2) -  32 \left(n - 3\right) \left(4 n + 9\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A103973, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=272 x^{2} + 64 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A103973.
