...
\title{The empirical recurrence for OEIS A189450, derived from the entry's generating function}
...
\begin{abstract}
OEIS A189450 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 02 2018 09:23:14, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{4} + 4 x^{3} - x^{2} - x}{x^{4} - 4 x^{3} + 4 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- x^{4} + 4 x^{3} - 4 x + 1
\]
