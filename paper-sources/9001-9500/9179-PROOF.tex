...
\title{A proof of the conjectured recurrence for OEIS A116881}
...
\begin{abstract}
OEIS A116881 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $8 x$, a polynomial of degree $1$, which proves
...
OEIS A116881 is ``Row sums of triangle A116880 (generalized Catalan C(1,2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 23, 150, 1037, 7408, 54035, 399850,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{4 x \left(8 x + 3 \sqrt{1 - 8 x} - 1\right)}{32 x^{3} + 36 x^{2} + 3 x - \sqrt{1 - 8 x} \left(8 x^{2} + 7 x - 1\right) - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(3*n-4)*a(n) +(-21*n\^{}2+43*n-10)*a(n-1) -4*(3*n-1)*(2*n-1)*a(n-2)=0. - \emph{R. J. Mathar}, Jun 22 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-08-05, revision 26) the
...
\[
p_{0}(n)=n \left(3 n - 4\right),\qquad p_{1}(n)=- 21 n^{2} + 43 n - 10,\qquad p_{2}(n)=- 4 \left(2 n - 1\right) \left(3 n - 1\right)
\]
...
\section{The computation for A116881}
...
\[
n \left(3 n - 4\right)\,a(n) + (- 21 n^{2} + 43 n - 10)\,a(n-1) -  4 \left(2 n - 1\right) \left(3 n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A116881, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=8 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A116881.
