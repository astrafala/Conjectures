...
\title{A proof of the conjectured recurrence for OEIS A185369}
...
\begin{abstract}
OEIS A185369 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A185369 is ``Number of simple labeled graphs on n nodes of degree 1 or 2 without cycles''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 1, 3, 15, 90, 645, 5355,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;e^{- \frac{x^{2}}{2 \left(x - 1\right)}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +2*a(n) +4*(-n+1)*a(n-1) +2*(n-1)*(n-3)*a(n-2) +(n-1)*(n-2)*a(n-3)=0.
\end{quote}
As of the ``Last modified'' line on the live entry (2024-06-17, revision 32) the
...
\[
p_{0}(n)=2,\qquad p_{1}(n)=- 4 \left(n - 1\right),\qquad p_{2}(n)=2 \left(n - 3\right) \left(n - 1\right),\qquad p_{3}(n)=\left(n - 2\right) \left(n - 1\right)
\]
...
\section{The computation for A185369}
...
\[
2\,a(n) -  4 \left(n - 1\right)\,a(n-1) + 2 \left(n - 3\right) \left(n - 1\right)\,a(n-2) + \left(n - 2\right) \left(n - 1\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A185369, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A185369.
