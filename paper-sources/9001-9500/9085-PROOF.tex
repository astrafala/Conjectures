...
\title{The empirical recurrence for OEIS A211715, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211715 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A211715 is ``Number of (n+1) X (n+1) -11..11 symmetric matrices with every 2 X 2 subblock having sum zero and two or four distinct values.''. It has offset $1$ and begins
\[
34, 58, 106, 202, 394, 778, 1546, 3082,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 19 2018 15:28:39, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 44 x^{2} + 34 x}{2 x^{2} - 3 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211715.
