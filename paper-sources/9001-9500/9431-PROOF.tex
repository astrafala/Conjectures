...
\title{The empirical recurrence for OEIS A253230, derived from the entry's generating function}
...
\begin{abstract}
OEIS A253230 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A253230 is ``Number of (n+1) X (7+1) 0..1 arrays with every 2 X 2 subblock diagonal minimum minus antidiagonal minimum nondecreasing horizontally and diagonal maximum minus antidiagonal maximum nondecreasing vertically.''. It has offset $1$ and begins
\[
6740, 9551, 6231, 8216, 11022, 15363, 22656, 35736,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 11 2018 08:59:38, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{x^{8} - 11 x^{7} + 9682 x^{6} - 27356 x^{5} + 15840 x^{4} + 19136 x^{3} - 24149 x^{2} + 6740 x}{2 x^{4} - 7 x^{3} + 9 x^{2} - 5 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;2 x^{4} - 7 x^{3} + 9 x^{2} - 5 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;x^{8} - 11 x^{7} + 9682 x^{6} - 27356 x^{5} + 15840 x^{4} + 19136 x^{3} - 24149 x^{2} + 6740 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A253230.
