...
\title{A proof of the conjectured recurrence for OEIS A257838}
...
\begin{abstract}
OEIS A257838 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A257838 is ``Main diagonal of iterated partial sums array of Fibonacci numbers (starting with the first partial sums)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 4, 16, 63, 247, 967, 3785,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 4 x - \sqrt{1 - 4 x} + 1}{2 \left(4 x^{2} - x - \left(1 - 4 x\right)^{\frac{3}{2}}\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-3)*a(n) +2*(-4*n\^{}2+13*n-6)*a(n-1) +(15*n\^{}2-53*n+48)*a(n-2) +2*(2*n-3)*(n-2)*a(n-3)=0. - \emph{R. J. Mathar}, Dec 10 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2017-04-09, revision 37) the
...
\[
p_{0}(n)=n \left(n - 3\right),\qquad p_{1}(n)=- 2 \left(4 n^{2} - 13 n + 6\right),\qquad p_{2}(n)=15 n^{2} - 53 n + 48,\qquad p_{3}(n)=2 \left(n - 2\right) \left(2 n - 3\right)
\]
...
\section{The computation for A257838}
...
\[
n \left(n - 3\right)\,a(n) -  2 \left(4 n^{2} - 13 n + 6\right)\,a(n-1) + (15 n^{2} - 53 n + 48)\,a(n-2) + 2 \left(n - 2\right) \left(2 n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A257838, agreeing on all 13 terms compared.
...
vanish, for all 26 values of $n$ from $n=3$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A257838.
