...
\title{A proof of the conjectured recurrence for OEIS A072100}
...
\begin{abstract}
OEIS A072100 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A072100 is ``Column 2 of the array m(i,1)=m(1,j)=1 m(i,j)=m(i-1,j-1)+m(i-1,j+1) (a(n)=m(n,2))''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 2, 3, 5, 8, 14, 24, 44,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;- \frac{x \left(\sqrt{1 - 2 x} \sqrt{2 x + 1} + 1\right)}{\sqrt{1 - 2 x} \left(x - 1\right) \left(\sqrt{1 - 2 x} + \sqrt{2 x + 1}\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-n+1)*a(n) +(n+1)*a(n-1) +2*(2*n-7)*a(n-2) +4*(-n+3)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 26 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-09, revision 17) the
...
\[
p_{0}(n)=1 - n,\qquad p_{1}(n)=n + 1,\qquad p_{2}(n)=2 \left(2 n - 7\right),\qquad p_{3}(n)=- 4 \left(n - 3\right)
\]
...
\[
(1 - n)\,a(n) + (n + 1)\,a(n-1) + 2 \left(2 n - 7\right)\,a(n-2) -  4 \left(n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A072100, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A072100.
