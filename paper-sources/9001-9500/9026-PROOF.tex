...
\title{A proof of the conjectured recurrence for OEIS A176854}
...
\begin{abstract}
OEIS A176854 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- x^{3} + 3 x^{2} - 5 x + 1$, a polynomial of degree $3$, which proves
...
OEIS A176854 is ``Sequence defined by the recurrence formula a(n+1)=sum(a(p)*a(n-p)+k,p=0..n)+l for n>=1, with here a(0)=1, a(1)=0, k=-1 and l=0''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, -2, -7, -18, -37, -52, 10,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt{\frac{- 4 x^{3} + 13 x^{2} - 6 x + 1}{x^{2} - 2 x + 1}}}{2 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +(-7*n+2)*a(n-1) +(19*n-29)*a(n-2) +(-17*n+40)*a(n-3) +2*(2*n-7)*a(n-4)=0. - \emph{R. J. Mathar}, Mar 01 2016
\end{quote}
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=2 - 7 n,\qquad p_{2}(n)=19 n - 29,\qquad p_{3}(n)=40 - 17 n,\qquad p_{4}(n)=2 \left(2 n - 7\right)
\]
...
\section{The computation for A176854}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; - \frac{4 x^{3} - 13 x^{2} + 6 x - 1}{\left(x - 1\right)^{2}} .
\]
...
\[
(n + 1)\,a(n) + (2 - 7 n)\,a(n-1) + (19 n - 29)\,a(n-2) + (40 - 17 n)\,a(n-3) + 2 \left(2 n - 7\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A176854, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- x^{3} + 3 x^{2} - 5 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A176854.
