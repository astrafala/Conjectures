...
\title{A proof of the conjectured recurrence for OEIS A128723}
...
\begin{abstract}
OEIS A128723 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 5 x^{3} + 9 x^{2} - 7 x + 3$, a polynomial of degree $3$, which proves
...
OEIS A128723 is ``Number of skew Dyck paths of semilength n having no peaks at level 1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 2, 6, 22, 84, 334, 1368,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 3 x - \sqrt{5 x^{2} - 6 x + 1} + 3}{3 x + \sqrt{5 x^{2} - 6 x + 1} + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +3*(n+1)*a(n) +(-17*n+10)*a(n-1) +9*(n-3)*a(n-2) +5*(n-2)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 17 2016
\end{quote}
...
\[
p_{0}(n)=3 \left(n + 1\right),\qquad p_{1}(n)=10 - 17 n,\qquad p_{2}(n)=9 \left(n - 3\right),\qquad p_{3}(n)=5 \left(n - 2\right)
\]
...
\section{The computation for A128723}
...
\[
3 \left(n + 1\right)\,a(n) + (10 - 17 n)\,a(n-1) + 9 \left(n - 3\right)\,a(n-2) + 5 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A128723, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 5 x^{3} + 9 x^{2} - 7 x + 3$ is an
...
\url{https://oeis.org}, 2026. Sequence A128723.
