...
\title{A proof of the conjectured recurrence for OEIS A200312}
...
\begin{abstract}
OEIS A200312 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A200312 is ``a(n) = A000108(n)*A006130(n), where A000108 is the Catalan numbers and A006130(n) = A006130(n-1) + 3*A006130(n-2)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 8, 35, 266, 1680, 12804, 93093,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{- 52 x - 26 \sqrt{- 48 x^{2} - 4 x + 1} + 26}}{26 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n+1)*a(n) -2*n*(2*n-1)*a(n-1) -12*(2*n-1)*(2*n-3)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 17 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2023-04-12, revision 33) the
...
\[
p_{0}(n)=n \left(n + 1\right),\qquad p_{1}(n)=- 2 n \left(2 n - 1\right),\qquad p_{2}(n)=- 12 \left(2 n - 3\right) \left(2 n - 1\right)
\]
...
\[
n \left(n + 1\right)\,a(n) -  2 n \left(2 n - 1\right)\,a(n-1) -  12 \left(2 n - 3\right) \left(2 n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A200312, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A200312.
