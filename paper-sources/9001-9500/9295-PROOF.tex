...
\title{A proof of the conjectured recurrence for OEIS A285195}
...
\begin{abstract}
OEIS A285195 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $- 360 x^{5} - 7200 x^{4} + 2880 x^{3} - 270 x^{2}$, a polynomial of degree $5$, which proves
...
OEIS A285195 is ``G.f.: 2C(x)/(3-sqrt(1-4xC(x))) where C = g.f. for A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 0, -1, -6, -30, -142, -660,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{\sqrt{1 - 4 x} - 1}{x \left(\sqrt{2 \sqrt{1 - 4 x} - 1} - 3\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: +126*n*(n-1)*(n+1)*a(n) -3*n*(n-1)*(691*n-1037)*a(n-1) +(n-1)*(12023*n\^{}2-48049*n+47736)*a(n-2) +2*(-12889*n\^{}3+93970*n\^{}2-221427*n+169506)*a(n-3) +4*(-248*n\^{}3+12718*n\^{}2-88433*n+160113)*a(n-4) +16*(2918*n\^{}3-41574*n\^{}2+198532*n-317121)*a(n-5) +240*(4*n-19)*(2*n-9)*(4*n-21)*a(n-6)=0. - \emph{R. J. Mathar}, Jan 25 2020
\end{quote}
As of the ``Last modified'' line on the live entry (2020-01-30, revision 19) the
...
\[
p_{0}(n)=126 n \left(n - 1\right) \left(n + 1\right),\qquad p_{1}(n)=- 3 n \left(n - 1\right) \left(691 n - 1037\right),\qquad p_{2}(n)=\left(n - 1\right) \left(12023 n^{2} - 48049 n + 47736\right),\qquad p_{3}(n)=- 2 \left(12889 n^{3} - 93970 n^{2} + 221427 n - 169506\right),\qquad p_{4}(n)=- 4 \left(248 n^{3} - 12718 n^{2} + 88433 n - 160113\right),\qquad p_{5}(n)=16 \left(2918 n^{3} - 41574 n^{2} + 198532 n - 317121\right),\qquad p_{6}(n)=240 \left(2 n - 9\right) \left(4 n - 21\right) \left(4 n - 19\right)
\]
...
\section{The computation for A285195}

The generating function \eqref{eq:gf} is algebraic over $\mathbb{Q}(x)$;
its minimal polynomial is
\[
P(x,y) \;=\; x^{3} y^{4} + 6 x^{2} y^{4} - 6 x^{2} y^{3} - 6 x y^{3} + 6 x y^{2} + 2 y^{2} - 3 y + 1 ,
\]
...
\[
B(x)\;=\;- 360 x^{5} - 7200 x^{4} + 2880 x^{3} - 270 x^{2} ,
\]
...
\[
126 n \left(n - 1\right) \left(n + 1\right)\,a(n) -  3 n \left(n - 1\right) \left(691 n - 1037\right)\,a(n-1) + \left(n - 1\right) \left(12023 n^{2} - 48049 n + 47736\right)\,a(n-2) -  2 \left(12889 n^{3} - 93970 n^{2} + 221427 n - 169506\right)\,a(n-3) -  4 \left(248 n^{3} - 12718 n^{2} + 88433 n - 160113\right)\,a(n-4) + 16 \left(2918 n^{3} - 41574 n^{2} + 198532 n - 317121\right)\,a(n-5) + 240 \left(2 n - 9\right) \left(4 n - 21\right) \left(4 n - 19\right)\,a(n-6) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A285195, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 360 x^{5} - 7200 x^{4} + 2880 x^{3} - 270 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A285195.
