...
\title{The empirical recurrence for OEIS A302323, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A302323 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
with $S=16$ vertices, and the conjectured recurrence is then decided by a finite exact
computation.
\end{abstract}
...
OEIS A302323 is ``Number of 2Xn 0..1 arrays with every element equal to 0, 1, 2 or 4 horizontally, diagonally or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
2, 8, 20, 52, 136, 360, 960, 2576,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 4*a(n-1) -2*a(n-2) -4*a(n-3) for n>4
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 05 2018 09:53:34, revision 4) the statement is
...
Let $V=\{0,\dots,1\}^{2}$ be the possible columns and take
$\mathcal V=V\times V$, of size $S=16$, as the vertex set. Declare $(p,c)\to(c,x)$ an edge
...
publishes, $n=27$, the arrays have $2\cdot27$ cells over
$2$ values, so there are about $10^{16}$ of them to sift. The
walk count replaces that by $16$ states and one matrix--vector product per column.
...
\begin{proof}
The digraph was built directly from \eqref{eq:loc}: for each of the $S=16$ pairs and each
of the $(1+1)^{2}$ possible next columns, the condition was tested on every cell of
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A302323.
