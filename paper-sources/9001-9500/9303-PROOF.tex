...
\title{The empirical recurrence for OEIS A223181, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223181 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A223181 is ``Rolling icosahedron footprints: number of n X 3 0..11 arrays starting with 0 where 0..11 label vertices of an icosahedron and every array movement to a horizontal, vertical or antidiagonal neighbor moves along an icosahedral edge.''. It has offset $1$ and begins
\[
25, 80, 400, 2080, 10880, 56960, 298240, 1561600,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 17 2018 09:21:46, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{20 x^{3} - 70 x^{2} + 25 x}{4 x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{2} - 6 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223181.
