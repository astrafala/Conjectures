...
\title{A proof of the conjectured recurrence for OEIS A116406}
...
\begin{abstract}
OEIS A116406 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A116406 is ``Expansion of ((1 + x - 2x\^{}2) + (1+x)*sqrt(1-4x\^{}2))/(2(1-4x\^{}2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 3, 7, 11, 26, 42,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x^{2} - x - \sqrt{1 - 4 x^{2}} \left(x + 1\right) - 1}{2 \left(4 x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-3)*a(n) +2*(-n\^{}2+4*n-2)*a(n-1) -4*(n-2)\^{}2*a(n-2) +8*(n-2)*(n-3)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 28 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2022-01-18, revision 32) the
...
\[
p_{0}(n)=n \left(n - 3\right),\qquad p_{1}(n)=- 2 \left(n^{2} - 4 n + 2\right),\qquad p_{2}(n)=- 4 \left(n - 2\right)^{2},\qquad p_{3}(n)=8 \left(n - 3\right) \left(n - 2\right)
\]
...
\section{The computation for A116406}
...
\[
n \left(n - 3\right)\,a(n) -  2 \left(n^{2} - 4 n + 2\right)\,a(n-1) -  4 \left(n - 2\right)^{2}\,a(n-2) + 8 \left(n - 3\right) \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A116406, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A116406.
