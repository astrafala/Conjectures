...
\title{The empirical recurrence for OEIS A264185, derived from the entry's generating function}
...
\begin{abstract}
OEIS A264185 carries an empirical recurrence of order $1$. The entry separately records a
...
\begin{quote}
G.f.: x*(8 + x - 90*x\^{}2 - 144*x\^{}3) / (1 - 10*x).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 29 2023 11:24:39, revision 12) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{144 x^{4} + 90 x^{3} - x^{2} - 8 x}{10 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;1 - 10 x
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 144 x^{4} - 90 x^{3} + x^{2} + 8 x,
\]
...
10 x - 1,
\]
of degree $1$, and that degree is the order of the SHORTEST linear recurrence with
