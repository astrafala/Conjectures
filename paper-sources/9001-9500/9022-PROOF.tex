...
\title{A proof of the conjectured recurrence for OEIS A127154}
...
\begin{abstract}
OEIS A127154 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 2 x^{4} - 6 x^{3} + 5 x + 1$, a polynomial of degree $4$, which proves
...
OEIS A127154 is ``Number of Dyck paths of semilength n and having no UDUD's starting at level 0; here U=(1,1), D=(1,-1)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 1, 4, 11, 33, 105, 343,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 \left(x + 1\right)}{2 x^{2} + x + \sqrt{1 - 4 x} \left(x + 1\right) + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +3*a(n-1) +(-11*n+16)*a(n-2) +(-17*n+43)*a(n-3) +(-11*n+28)*a(n-4) +2*(-2*n+7)*a(n-5)=0. - \emph{R. J. Mathar}, Jun 17 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 18) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=3,\qquad p_{2}(n)=16 - 11 n,\qquad p_{3}(n)=43 - 17 n,\qquad p_{4}(n)=28 - 11 n,\qquad p_{5}(n)=- 2 \left(2 n - 7\right)
\]
...
\section{The computation for A127154}
...
\[
(n + 1)\,a(n) + 3\,a(n-1) + (16 - 11 n)\,a(n-2) + (43 - 17 n)\,a(n-3) + (28 - 11 n)\,a(n-4) -  2 \left(2 n - 7\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A127154, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 2 x^{4} - 6 x^{3} + 5 x + 1$ is an
...
\url{https://oeis.org}, 2026. Sequence A127154.
