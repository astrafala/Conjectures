...
\title{A proof of the conjectured recurrence for OEIS A176479}
...
\begin{abstract}
OEIS A176479 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A176479 is ``a(n) = (n+1)*A001003(n)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 9, 44, 225, 1182, 6321, 34232,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x + \sqrt{x^{2} - 6 x + 1} + 3}{4 \sqrt{x^{2} - 6 x + 1}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-1)*a(n) -3*(2*n-1)*(n-1)*a(n-1) +n*(n-2)*a(n-2) = 0. - \emph{R. J. Mathar}, Dec 03 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 73) the
...
\[
p_{0}(n)=n \left(n - 1\right),\qquad p_{1}(n)=- 3 \left(n - 1\right) \left(2 n - 1\right),\qquad p_{2}(n)=n \left(n - 2\right)
\]
...
\section{The computation for A176479}
...
\[
n \left(n - 1\right)\,a(n) -  3 \left(n - 1\right) \left(2 n - 1\right)\,a(n-1) + n \left(n - 2\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A176479, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A176479.
