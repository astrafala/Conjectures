...
\title{A condition on every subblock: the empirical recurrence for OEIS A181246}
...
\begin{abstract}
OEIS A181246 carries an empirical recurrence of order $3$ contributed by R.~H.~Hardin. It is
...
OEIS A181246 is ``Number of n X 3 binary matrices with no 2 X 2 block having four 1's.''. It has offset $1$ and begins
\[
8, 57, 417, 3032, 22077, 160697, 1169792, 8515337,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 6*a(n-1) + 10*a(n-2) - 5*a(n-3).
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 26 2018 14:25:17, revision 6) this is still
recorded as empirical, and nothing on the entry records it as proved.

\section{The count is a walk count}

The entry counts arrays with $3$ columns over the alphabet $\{0,1\}$, growing one row at a time, and the condition it imposes is that no $2\times2$ subblock may contain $4$ ones.

That condition is decided by a bounded window. A $2\times2$ subblock occupies $2$ consecutive rows, so a window of $2$ consecutive rows decides everything that the arrival of a new row settles; nothing outside the window is consulted. Take those windows as the vertices of a digraph, with an edge from $u$ to $v$ when $v$'s new row may follow $u$, and merge vertices with identical futures; that leaves $S=3$. An admissible array is exactly a walk, so with $M$ the adjacency matrix and $\iota,\tau$ the vectors recording which windows may begin and end an array,
\[
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A181246.
