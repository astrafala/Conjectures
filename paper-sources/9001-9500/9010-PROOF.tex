...
\title{A proof of the conjectured recurrence for OEIS A174783}
...
\begin{abstract}
OEIS A174783 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A174783 is ``Expansion of (1+2x-sqrt(1-4x\textasciicircum{}2))/(2(1-x\textasciicircum{}2)*sqrt(1-4x\textasciicircum{}2))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 1, 3, 4, 9, 14, 29,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 2 x + \sqrt{1 - 4 x^{2}} - 1}{2 \sqrt{1 - 4 x^{2}} \left(x^{2} - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) -2*a(n-1) +(8-5*n)*a(n-2) +2*a(n-3) +4*(n-2)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 13 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2022-07-18, revision 16) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=-2,\qquad p_{2}(n)=8 - 5 n,\qquad p_{3}(n)=2,\qquad p_{4}(n)=4 \left(n - 2\right)
\]
...
\section{The computation for A174783}
...
\[
n\,a(n) - 2\,a(n-1) + (8 - 5 n)\,a(n-2) + 2\,a(n-3) + 4 \left(n - 2\right)\,a(n-4) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A174783, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A174783.
