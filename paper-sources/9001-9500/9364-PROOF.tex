...
\title{The empirical recurrence for OEIS A165378, derived from the entry's generating function}
...
\begin{abstract}
OEIS A165378 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 26 2018 09:54:00, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{3 x^{6} - 18 x^{5} + 25 x^{4} + 5 x^{3} - 33 x^{2}}{x^{3} - 3 x^{2} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 3 x^{6} + 18 x^{5} - 25 x^{4} - 5 x^{3} + 33 x^{2},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A165378.
