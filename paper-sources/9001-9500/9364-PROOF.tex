...
\title{A proof of the conjectured recurrence for OEIS A034863}
...
\begin{abstract}
OEIS A034863 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $1960088 x^{3} - \frac{1633397 x^{2}}{2}$, a polynomial of degree $3$, which proves
...
OEIS A034863 is ``a(n) = n!*(4*n\^{}3 - 30*n\^{}2 + 40*n + 3)/24''. It has offset $4$ and begins
\[
a(4),\dots,a(11)\;=\;-61, -235, 810, 38850, 757680, 12836880, 212133600, 3554258400,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 4} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{x^{4} \left(39 x^{3} - 151 x^{2} + 197 x - 61\right)}{24 \left(x - 1\right)^{4}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (-96*n + 27161)*a(n) + (96*n\^{}2 - 99580*n + 199917)*a(n-1) +(72707*n - 61983)*(n-1)*a(n-2) = 0. - \emph{R. J. Mathar}, Apr 03 2017
\end{quote}
...
\[
p_{0}(n)=27161 - 96 n,\qquad p_{1}(n)=96 n^{2} - 99580 n + 199917,\qquad p_{2}(n)=\left(n - 1\right) \left(72707 n - 61983\right)
\]
...
\section{The computation for A034863}
...
\[
B(x)\;=\;1960088 x^{3} - \frac{1633397 x^{2}}{2} ,
\]
...
\[
(27161 - 96 n)\,a(n) + (96 n^{2} - 99580 n + 199917)\,a(n-1) + \left(n - 1\right) \left(72707 n - 61983\right)\,a(n-2) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A034863, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=1960088 x^{3} - \frac{1633397 x^{2}}{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A034863.
