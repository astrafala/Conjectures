...
\title{The empirical recurrence for OEIS A262415, derived from the entry's generating function}
...
\begin{abstract}
OEIS A262415 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A262415 is ``Number of (n+1) X (3+1) 0..1 arrays with each row divisible by 3 and column not divisible by 3, read as a binary number with top and left being the most significant bits.''. It has offset $1$ and begins
\[
6, 45, 270, 1701, 10206, 61965, 371790, 2237301,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 31 2018 11:26:33, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 54 x^{3} + 9 x^{2} + 6 x}{54 x^{3} - 9 x^{2} - 6 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;54 x^{3} - 9 x^{2} - 6 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A262415.
