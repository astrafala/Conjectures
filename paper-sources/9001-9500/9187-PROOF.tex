...
\title{A proof of the conjectured recurrence for OEIS A243022}
...
\begin{abstract}
OEIS A243022 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $92 x^{3} + 4 x^{2}$, a polynomial of degree $3$, which proves
...
OEIS A243022 is ``Expansion of -(2*x)/(1-sqrt(1-(2*(1-sqrt(1-4*x\^{}2)))/x))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;-1, 1, 2, 2, 7, 18, 61, 198,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x}{\sqrt{\frac{x + 2 \sqrt{1 - 4 x^{2}} - 2}{x}} - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence: +2*n*(n-1)*(2*n-3)*(5*n-12)*a(n) -(n-1)*(85*n\^{}3-459*n\^{}2+776*n-400)*a(n-1) +4*(-40*n\^{}4+396*n\^{}3-1455*n\^{}2+2324*n-1360)*a(n-2) +4*(n-4)*(170*n\^{}3-1258*n\^{}2+3003*n-2185)*a(n-3) +16*(n-4)*(n-5)*(20*n\^{}2-118*n+143)*a(n-4) -272*(n-4)*(n-5)*(n-6)*(5*n-7)*a(n-5)=0. - \emph{R. J. Mathar}, Jul 15 2017
\end{quote}
As of the ``Last modified'' line on the live entry (2020-01-30, revision 29) the
...
\[
p_{0}(n)=2 n \left(n - 1\right) \left(2 n - 3\right) \left(5 n - 12\right),\qquad p_{1}(n)=- \left(n - 1\right) \left(85 n^{3} - 459 n^{2} + 776 n - 400\right),\qquad p_{2}(n)=- 4 \left(40 n^{4} - 396 n^{3} + 1455 n^{2} - 2324 n + 1360\right),\qquad p_{3}(n)=4 \left(n - 4\right) \left(170 n^{3} - 1258 n^{2} + 3003 n - 2185\right),\qquad p_{4}(n)=16 \left(n - 5\right) \left(n - 4\right) \left(20 n^{2} - 118 n + 143\right),\qquad p_{5}(n)=- 272 \left(n - 6\right) \left(n - 5\right) \left(n - 4\right) \left(5 n - 7\right)
\]
...
\section{The computation for A243022}

The generating function \eqref{eq:gf} is algebraic over $\mathbb{Q}(x)$;
its minimal polynomial is
\[
P(x,y) \;=\; 16 x \left(x^{4} + 2 x^{3} y + x^{2} y^{2} + x y^{2} + y^{4} + y^{3}\right) ,
\]
...
\[
2 n \left(n - 1\right) \left(2 n - 3\right) \left(5 n - 12\right)\,a(n) -  \left(n - 1\right) \left(85 n^{3} - 459 n^{2} + 776 n - 400\right)\,a(n-1) -  4 \left(40 n^{4} - 396 n^{3} + 1455 n^{2} - 2324 n + 1360\right)\,a(n-2) + 4 \left(n - 4\right) \left(170 n^{3} - 1258 n^{2} + 3003 n - 2185\right)\,a(n-3) + 16 \left(n - 5\right) \left(n - 4\right) \left(20 n^{2} - 118 n + 143\right)\,a(n-4) -  272 \left(n - 6\right) \left(n - 5\right) \left(n - 4\right) \left(5 n - 7\right)\,a(n-5) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A243022, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=92 x^{3} + 4 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A243022.
