...
\title{A proof of the conjectured recurrence for OEIS A132364}
...
\begin{abstract}
OEIS A132364 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A132364 is ``Expansion of 1/(1-x\^{}2*c(x)), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 1, 1, 3, 7, 20, 59,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x \sqrt{1 - 4 x} - x + 2}{2 \left(x^{3} - x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(-n+1)*a(n) +(5*n-11)*a(n-1) +2*(-2*n+5)*a(n-2) +(-n+1)*a(n-3) +2*(2*n-5)*a(n-4)=0. - \emph{R. J. Mathar}, Aug 28 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2024-09-23, revision 27) the
...
\[
p_{0}(n)=1 - n,\qquad p_{1}(n)=5 n - 11,\qquad p_{2}(n)=- 2 \left(2 n - 5\right),\qquad p_{3}(n)=1 - n,\qquad p_{4}(n)=2 \left(2 n - 5\right)
\]
...
\section{The computation for A132364}
...
\[
(1 - n)\,a(n) + (5 n - 11)\,a(n-1) -  2 \left(2 n - 5\right)\,a(n-2) + (1 - n)\,a(n-3) + 2 \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A132364, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A132364.
