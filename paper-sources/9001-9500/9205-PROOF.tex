...
\title{A proof of the conjectured recurrence for OEIS A167481}
...
\begin{abstract}
OEIS A167481 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A167481 is ``Convolution of the central binomial coefficients A000984(n) and (-2)\^{}n''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 6, 8, 54, 144, 636, 2160,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1}{\sqrt{1 - 4 x} \left(2 x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) + 2*(1-n)*a(n-1) + 4*(1-2n)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 16 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2016-06-14, revision 16) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=- 2 \left(n - 1\right),\qquad p_{2}(n)=- 4 \left(2 n - 1\right)
\]
...
\section{The computation for A167481}
...
\[
n\,a(n) -  2 \left(n - 1\right)\,a(n-1) -  4 \left(2 n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A167481, agreeing on all 13 terms compared.
...
vanish, for all 23 values of $n$ from $n=2$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A167481.
