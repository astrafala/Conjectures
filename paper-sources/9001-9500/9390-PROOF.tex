...
\title{A proof of the conjectured recurrence for OEIS A192480}
...
\begin{abstract}
OEIS A192480 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A192480 is ``a(n) = n + A000108(n-1) for n > 1; a(0)=0, a(1)=1''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 3, 5, 9, 19, 48, 139,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x^{2} \left(2 - x\right) + \left(1 - \sqrt{1 - 4 x}\right) \left(x - 1\right)^{2}}{2 \left(x - 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(3*n\^{}2-16*n+19)*a(n) +(-15*n\^{}3+95*n\^{}2-188*n+120)*a(n-1) +2*(2*n-5)*(3*n\^{}2-10*n+6)*a(n-2)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
...
\[
p_{0}(n)=n \left(3 n^{2} - 16 n + 19\right),\qquad p_{1}(n)=- 15 n^{3} + 95 n^{2} - 188 n + 120,\qquad p_{2}(n)=2 \left(2 n - 5\right) \left(3 n^{2} - 10 n + 6\right)
\]
...
\section{The computation for A192480}
...
\[
n \left(3 n^{2} - 16 n + 19\right)\,a(n) + (- 15 n^{3} + 95 n^{2} - 188 n + 120)\,a(n-1) + 2 \left(2 n - 5\right) \left(3 n^{2} - 10 n + 6\right)\,a(n-2) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A192480, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A192480.
