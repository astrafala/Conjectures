...
\title{A proof of the conjectured recurrence for OEIS A098557}
...
\begin{abstract}
OEIS A098557 carries a conjectured linear recurrence with polynomial coefficients, of
order 2. We prove it. The entry's exponential generating function is
...
to the polynomial $-1$, of degree $0$, which proves the recurrence for every
$n>0$.
\end{abstract}
...
OEIS A098557 is ``Expansion of e.g.f. (1/2)*(1+x)*log((1+x)/(1-x))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 2, 2, 8, 24, 144, 720,\ \dots
\]
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;\frac{\left(x + 1\right) \log{\left(\frac{- x - 1}{x - 1} \right)}}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
conjecture: -a(n) +a(n-1) +(n-1)*(n-3)*a(n-2)=0. - \emph{R. J. Mathar}, Nov 14 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2026-06-30, revision 27) the
...
\[
p_{0}(n)=-1,\qquad p_{1}(n)=1,\qquad p_{2}(n)=\left(n - 3\right) \left(n - 1\right)
\]
...
\section{The computation for A098557}
...
\[
-1\,a(n) + 1\,a(n-1) + \left(n - 3\right) \left(n - 1\right)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A098557, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A098557.
