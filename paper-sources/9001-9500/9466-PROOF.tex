...
\title{A proof of the conjectured recurrence for OEIS A026031}
...
\begin{abstract}
OEIS A026031 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 16 x^{3} + 8 x^{2}$, a polynomial of degree $3$, which proves
...
OEIS A026031 is ``a(n) = number of (s(0), s(1), ..., s(2n)) such that s(i) is a nonnegative integer and |s(i) - s(i-1)| = 1 for i = 1,2,...,n, s(0) = 3, s(2n) = 7. Also a(n) = T(2n,n-2), where T is defined in A026022''. It has offset $2$ and begins
\[
a(2),\dots,a(9)\;=\;1, 6, 28, 120, 494, 1988, 7888, 31008,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 2} a(n)\,x^{n}\;=\;\frac{\left(16 x^{2} + \left(\sqrt{1 - 4 x} - 1\right)^{4}\right) \left(\sqrt{1 - 4 x} - 1\right)^{6}}{1024 x^{6}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+6)*a(n) +10*(-n-4)*a(n-1) +2*(17*n+32)*a(n-2) +4*(-11*n+4)*a(n-3) +8*(2*n-5)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 22 2013
\end{quote}
As of the ``Last modified'' line on the live entry (2026-01-21, revision 17) the
...
\[
p_{0}(n)=n + 6,\qquad p_{1}(n)=- 10 \left(n + 4\right),\qquad p_{2}(n)=2 \left(17 n + 32\right),\qquad p_{3}(n)=- 4 \left(11 n - 4\right),\qquad p_{4}(n)=8 \left(2 n - 5\right)
\]
...
\section{The computation for A026031}
...
\[
(n + 6)\,a(n) -  10 \left(n + 4\right)\,a(n-1) + 2 \left(17 n + 32\right)\,a(n-2) -  4 \left(11 n - 4\right)\,a(n-3) + 8 \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A026031, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=- 16 x^{3} + 8 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A026031.
