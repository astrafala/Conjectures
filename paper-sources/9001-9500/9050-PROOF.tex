...
\title{A proof of the conjectured recurrence for OEIS A191309}
...
\begin{abstract}
OEIS A191309 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A191309 is ``Number of peaks at height >= 2 in all dispersed Dyck paths of length n (i.e., Motzkin paths of length n with no (1,0) steps at positive heights)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 0, 0, 1, 2, 8, 16,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x^{2} \left(1 - \sqrt{1 - 4 x^{2}}\right)}{\sqrt{1 - 4 x^{2}} \left(- 2 x + \sqrt{1 - 4 x^{2}} + 1\right)^{2}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-4)*a(n) +(n\^{}2-10*n+15)*a(n-1) +2*(-5*n\^{}2+28*n-27)*a(n-2) -4*(n-3)*(n-8) *a(n-3) +24*(n-3)*(n-4)*a(n-4)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-03-27, revision 20) the
...
\[
p_{0}(n)=n \left(n - 4\right),\qquad p_{1}(n)=n^{2} - 10 n + 15,\qquad p_{2}(n)=- 2 \left(5 n^{2} - 28 n + 27\right),\qquad p_{3}(n)=- 4 \left(n - 8\right) \left(n - 3\right),\qquad p_{4}(n)=24 \left(n - 4\right) \left(n - 3\right)
\]
...
\section{The computation for A191309}
...
\[
n \left(n - 4\right)\,a(n) + (n^{2} - 10 n + 15)\,a(n-1) -  2 \left(5 n^{2} - 28 n + 27\right)\,a(n-2) -  4 \left(n - 8\right) \left(n - 3\right)\,a(n-3) + 24 \left(n - 4\right) \left(n - 3\right)\,a(n-4) \;=\;0
\]
holds for every $n>-1$.
\end{theorem}
...
section of A191309, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A191309.
