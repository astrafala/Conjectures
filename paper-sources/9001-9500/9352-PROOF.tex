...
\title{The empirical recurrence for OEIS A253029, derived from the entry's generating function}
...
\begin{abstract}
OEIS A253029 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 08 2018 11:22:29, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 162 x^{2} - 297 x}{3 x^{2} + 5 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 3 x^{2} - 5 x + 1
\]
