...
\title{A proof of the conjectured recurrence for OEIS A165201}
...
\begin{abstract}
OEIS A165201 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $2 x^{3} + 2 x^{2}$, a polynomial of degree $3$, which proves
...
OEIS A165201 is ``Expansion of 1/(1-x*c(x)\^{}3), c(x) the g.f. of A000108''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 16, 65, 267, 1105, 4597,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x^{2} + 3 x + \sqrt{1 - 4 x} \left(x - 1\right) - 1}{2 \left(x^{2} + 4 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n-3)*a(n) +2*(-4*n\^{}2+15*n-10)*a(n-1) +(15*n\^{}2-69*n+80)*a(n-2) +2*(n-2)*(2*n-5)*a(n-3) =0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
...
\[
p_{0}(n)=n \left(n - 3\right),\qquad p_{1}(n)=- 2 \left(4 n^{2} - 15 n + 10\right),\qquad p_{2}(n)=15 n^{2} - 69 n + 80,\qquad p_{3}(n)=2 \left(n - 2\right) \left(2 n - 5\right)
\]
...
\section{The computation for A165201}
...
\[
n \left(n - 3\right)\,a(n) -  2 \left(4 n^{2} - 15 n + 10\right)\,a(n-1) + (15 n^{2} - 69 n + 80)\,a(n-2) + 2 \left(n - 2\right) \left(2 n - 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A165201, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=2 x^{3} + 2 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A165201.
