...
\title{A proof of the conjectured recurrence for OEIS A228771}
...
\begin{abstract}
OEIS A228771 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 1148064 x^{6} + 2145732 x^{5} - 4015548 x^{4} + 4465368 x^{3} - 1642287 x^{2} + 200001 x$, a polynomial of degree $6$, which proves
...
OEIS A228771 is ``The number of skew sum indecomposable permutations which avoid the patterns 3124 and 4312''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 1, 3, 12, 53, 234, 1013, 4306,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{x \left(- 8 x^{5} + 28 x^{4} - 50 x^{3} + 35 x^{2} - 10 x + \sqrt{1 - 4 x} \left(6 x^{4} - 18 x^{3} + 21 x^{2} - 8 x + 1\right) + 1\right)}{- 8 x^{5} + 46 x^{4} - 71 x^{3} + 43 x^{2} - 11 x + \sqrt{1 - 4 x} \left(12 x^{4} - 31 x^{3} + 27 x^{2} - 9 x + 1\right) + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -163*(n+2)*(4*n-413) *a(n) +(-652*n\^{}2-725425*n-452889) *a(n-1) +5*(14473*n\^{}2+512276*n-443094) *a(n-2) +(-410045*n\^{}2-2408964*n+8429009) *a(n-3) +2*(404156*n\^{}2-1297075*n-1518084)*a(n-4) -8*(29333*n-32490)*(2*n-11)*a(n-5)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 13) the
...
\[
p_{0}(n)=- 163 \left(n + 2\right) \left(4 n - 413\right),\qquad p_{1}(n)=- 652 n^{2} - 725425 n - 452889,\qquad p_{2}(n)=5 \left(14473 n^{2} + 512276 n - 443094\right),\qquad p_{3}(n)=- 410045 n^{2} - 2408964 n + 8429009,\qquad p_{4}(n)=2 \left(404156 n^{2} - 1297075 n - 1518084\right),\qquad p_{5}(n)=- 8 \left(2 n - 11\right) \left(29333 n - 32490\right)
\]
...
\section{The computation for A228771}
...
\[
B(x)\;=\;- 1148064 x^{6} + 2145732 x^{5} - 4015548 x^{4} + 4465368 x^{3} - 1642287 x^{2} + 200001 x ,
\]
...
\[
- 163 \left(n + 2\right) \left(4 n - 413\right)\,a(n) + (- 652 n^{2} - 725425 n - 452889)\,a(n-1) + 5 \left(14473 n^{2} + 512276 n - 443094\right)\,a(n-2) + (- 410045 n^{2} - 2408964 n + 8429009)\,a(n-3) + 2 \left(404156 n^{2} - 1297075 n - 1518084\right)\,a(n-4) -  8 \left(2 n - 11\right) \left(29333 n - 32490\right)\,a(n-5) \;=\;0
\]
holds for every $n>6$.
\end{theorem}
...
section of A228771, agreeing on all 13 terms compared.
...
vanish, for all 23 values of $n$ from $n=8$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 1148064 x^{6} + 2145732 x^{5} - 4015548 x^{4} + 4465368 x^{3} - 1642287 x^{2} + 200001 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A228771.
