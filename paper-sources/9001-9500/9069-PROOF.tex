...
\title{A proof of the conjectured recurrence for OEIS A001465}
...
\begin{abstract}
OEIS A001465 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A001465 is ``Number of degree-n odd permutations of order 2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 3, 6, 10, 30, 126,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;e^{x} \sinh{\left(\frac{x^{2}}{2} \right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) -3*a(n-1) +3*a(n-2) -a(n-3) -(n-1)*(n-3)*a(n-4) +(n-3)*(n-4)*a(n-5)=0.
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 56) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=-3,\qquad p_{2}(n)=3,\qquad p_{3}(n)=-1,\qquad p_{4}(n)=- \left(n - 3\right) \left(n - 1\right),\qquad p_{5}(n)=\left(n - 4\right) \left(n - 3\right)
\]
...
\section{The computation for A001465}
...
\[
1\,a(n) - 3\,a(n-1) + 3\,a(n-2) - 1\,a(n-3) -  \left(n - 3\right) \left(n - 1\right)\,a(n-4) + \left(n - 4\right) \left(n - 3\right)\,a(n-5) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A001465, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A001465.
