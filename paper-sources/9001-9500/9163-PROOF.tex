...
\title{The empirical recurrence for OEIS A233106, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233106 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A233106 is ``Number of n X 1 0..5 arrays with no element x(i,j) adjacent to value 5-x(i,j) horizontally or vertically, top left element zero, and 1 appearing before 2 3 and 4, and 2 appearing before 3 in row major order.''. It has offset $1$ and begins
\[
1, 2, 6, 23, 99, 452, 2136, 10313,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 19 2018 11:01:46, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 11 x^{3} + 7 x^{2} - x}{15 x^{3} - 23 x^{2} + 9 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 15 x^{3} + 23 x^{2} - 9 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233106.
