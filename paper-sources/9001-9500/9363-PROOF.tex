...
\title{The empirical recurrence for OEIS A265989, derived from the entry's generating function}
...
\begin{abstract}
OEIS A265989 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A265989 is ``Number of n X 6 integer arrays with each element equal to the number of horizontal and antidiagonal neighbors not equal to itself.''. It has offset $1$ and begins
\[
2, 6, 9, 4, 26, 5, 101, 17,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 09 2019 09:12:23, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{8 x^{7} - x^{6} + 10 x^{5} - 21 x^{4} - 5 x^{3} + 4 x^{2} + 2 x}{4 x^{3} - 4 x^{2} - x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{3} - 4 x^{2} - x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;8 x^{7} - x^{6} + 10 x^{5} - 21 x^{4} - 5 x^{3} + 4 x^{2} + 2 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A265989.
