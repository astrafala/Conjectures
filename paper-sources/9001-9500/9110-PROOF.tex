...
\title{A proof of the conjectured recurrence for OEIS A171556}
...
\begin{abstract}
OEIS A171556 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A171556 is ``a(n) = 3*C(n)-2, where C(n) = A000108(n)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 4, 13, 40, 124, 394, 1285,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{4 x - 3 \left(x - 1\right) \left(\sqrt{1 - 4 x} - 1\right)}{2 x \left(x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*(n-2)*a(n) +(-5*n\^{}2+11*n-4)*a(n-1) +2*(n-1)*(2*n-3)*a(n-2)=0. - \emph{R. J. Mathar}, Sep 30 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-05-26, revision 10) the
...
\[
p_{0}(n)=\left(n - 2\right) \left(n + 1\right),\qquad p_{1}(n)=- 5 n^{2} + 11 n - 4,\qquad p_{2}(n)=2 \left(n - 1\right) \left(2 n - 3\right)
\]
...
\section{The computation for A171556}
...
\[
\left(n - 2\right) \left(n + 1\right)\,a(n) + (- 5 n^{2} + 11 n - 4)\,a(n-1) + 2 \left(n - 1\right) \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A171556, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A171556.
