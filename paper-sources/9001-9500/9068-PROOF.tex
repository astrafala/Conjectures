...
\title{The empirical recurrence for OEIS A304304, proved by transfer matrix}
...
\begin{abstract}
OEIS A304304 counts $2$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{0,1,2,3,5\}$, and it carries an empirical
recurrence of order $4$ contributed by R.~H.~Hardin. It is true. The neighbourhood
...
on $S=16$ vertices. Unlike a plain walk count the boundary rows matter here --- the first
...
OEIS A304304 is ``Number of nX2 0..1 arrays with every element unequal to 0, 1, 2, 3 or 5 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
2, 8, 23, 65, 192, 569, 1709, 5162,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) +3*a(n-2) -6*a(n-3) -8*a(n-4) for n>5
\end{quote}
As of the ``Last modified'' line on the live entry (May 10 2018 09:09:46, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 3\,a(n-2) - 6\,a(n-3) - 8\,a(n-4)
\end{equation}
for all $n>5$.
...
Take as state an ordered pair $(r,s)$ of rows, so $S=16$, and set
\begin{itemize}
...
$a(n)-\sum_i c_i a(n-i)=u_{\,n-2-4}$ whenever $n-4\ge2$. So \eqref{eq:conj}
holds for every $n>t$ exactly when $u_j=0$ for all $j>t-2-4$. Moreover $(u_j)$
...
\begin{proof}
Substituting Lemma~\ref{lem:walk} and factoring $P^{\,n-2-4}$ out of
...
\begin{proof}
The vector $q(P)\mathbf{e}\in\mathbb{Z}^{S}$ was formed by $4$ matrix--vector
products in exact integer arithmetic and the $u_j$ evaluated against $\mathbf{s}$ in turn.
They vanish from the index corresponding to $n=5+1$ onwards, and the run was continued
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=16$ consecutive
zeros rather than to a sampled prefix.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A304304.
