...
\title{The empirical recurrence for OEIS A184037, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184037 carries an empirical recurrence of order $4$. The entry separately records a
...
OEIS A184037 is ``1/16 the number of (n+1) X 8 0..3 arrays with all 2 X 2 subblocks having the same four values.''. It has offset $1$ and begins
\[
625, 637, 658, 700, 778, 934, 1234, 1834,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 05:55:49, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 2476 x^{4} + 1253 x^{3} + 1238 x^{2} - 625 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;2476 x^{4} - 1253 x^{3} - 1238 x^{2} + 625 x,
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A184037.
