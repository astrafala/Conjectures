...
\title{A proof of the conjectured closed form for OEIS A128153}
...
\begin{abstract}
OEIS A128153 carries a conjectured closed form for its terms. We prove it. The entry
...
explicit rational function; here it collapses to the polynomial $1$, of degree
...
OEIS A128153 is ``The number of regular pentagons found by constructing n equally-spaced points on each side of the pentagon and drawing lines parallel to the pentagon sides, as well as lines connecting vertices''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 9, 20, 37, 58, 85, 116, 153,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x^{4} + x^{3} - 2 x^{2} - 7 x - 1}{\left(x - 1\right)^{3} \left(x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = (10*n\^{}2 + 16*n + 9 -(-1)\^{}n)/4 for n > 0.
\end{quote}
As of the ``Last modified'' line on the live entry (2017-10-21, revision 24) the
...
\[
f(n)\;=\;- \frac{\left(-1\right)^{n}}{4} + \frac{5 n^{2}}{2} + 4 n + \frac{9}{4} .
\]
...
\section{The computation for A128153}

By Lemma~\ref{lem:gf} the conjectured formula has generating function
\[
F(x)\;=\;\frac{- x^{3} - 2 x^{2} - 5 x - 2}{x^{4} - 2 x^{3} + 2 x - 1} ,
\]
...
For every $n>0$,
\[
a(n)\;=\;- \frac{\left(-1\right)^{n}}{4} + \frac{5 n^{2}}{2} + 4 n + \frac{9}{4} .
\]
\end{theorem}
...
section of A128153, agreeing on all 13 terms compared.
...
with the entry's own value for every $n>0$ in range, agreeing in all 12 cases
...
cancellation in $K$, not by series truncation, so the identity $B=1$ is an
...
\url{https://oeis.org}, 2026. Sequence A128153.
