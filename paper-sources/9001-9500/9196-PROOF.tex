...
\title{A proof of the conjectured recurrence for OEIS A127361}
...
\begin{abstract}
OEIS A127361 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A127361 is ``a(n) = Sum\_\{k=0..n\} binomial(n, floor(k/2))*(-2)\^{}(n-k)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -1, 4, -7, 22, -46, 130, -295,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2 x - \sqrt{1 - 4 x^{2}} + 1}{2 \sqrt{1 - 4 x^{2}} \left(x - \sqrt{1 - 4 x^{2}} + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*n*a(n) + (5*n-4)*a(n-1) - 2*(4*n-3)*a(n-2) - 20*(n-2)*a(n-3) = 0. - \emph{R. J. Mathar}, Nov 30 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-22, revision 29) the
...
\[
p_{0}(n)=2 n,\qquad p_{1}(n)=5 n - 4,\qquad p_{2}(n)=- 2 \left(4 n - 3\right),\qquad p_{3}(n)=- 20 \left(n - 2\right)
\]
...
\section{The computation for A127361}
...
\[
2 n\,a(n) + (5 n - 4)\,a(n-1) -  2 \left(4 n - 3\right)\,a(n-2) -  20 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A127361, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A127361.
