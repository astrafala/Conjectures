...
\title{A proof of the conjectured recurrence for OEIS A105872}
...
\begin{abstract}
OEIS A105872 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $12 x + 6$, a polynomial of degree $1$, which proves
...
OEIS A105872 is ``a(n) = Sum\_\{k=0..floor(n/3)\} binomial(2*n-3*k, n)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 6, 21, 75, 273, 1009, 3770,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{2}{4 x^{2} - 5 x + \sqrt{1 - 4 x} \left(3 x + 1\right) + 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -3*(n+1)*(7*n-2)*a(n) +6*(7*n+5)*(2*n-1)*a(n-1) -(n+1)*(7*n-2)*a(n-2) +2*(7*n+5)*(2*n-1)*a(n-3)=0. - \emph{R. J. Mathar}, Nov 28 2014
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-13, revision 34) the
...
\[
p_{0}(n)=- 3 \left(n + 1\right) \left(7 n - 2\right),\qquad p_{1}(n)=6 \left(2 n - 1\right) \left(7 n + 5\right),\qquad p_{2}(n)=- \left(n + 1\right) \left(7 n - 2\right),\qquad p_{3}(n)=2 \left(2 n - 1\right) \left(7 n + 5\right)
\]
...
\section{The computation for A105872}
...
\[
- 3 \left(n + 1\right) \left(7 n - 2\right)\,a(n) + 6 \left(2 n - 1\right) \left(7 n + 5\right)\,a(n-1) -  \left(n + 1\right) \left(7 n - 2\right)\,a(n-2) + 2 \left(2 n - 1\right) \left(7 n + 5\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A105872, agreeing on all 13 terms compared.
...
vanish, for all 22 values of $n$ from $n=3$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=12 x + 6$ is an
...
\url{https://oeis.org}, 2026. Sequence A105872.
