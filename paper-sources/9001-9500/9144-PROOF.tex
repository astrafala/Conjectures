...
\title{A proof of the conjectured recurrence for OEIS A002104}
...
\begin{abstract}
OEIS A002104 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A002104 is ``Logarithmic numbers''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 1, 3, 8, 24, 89, 415, 2372,\ \dots
\]
The entry gives the exponential generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,\frac{x^{n}}{n!}\;=\;- e^{x} \log{\left(1 - x \right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) +(-n-1)*a(n-1) +2*(n-1)*a(n-2) +(-n+2)*a(n-3)=0.
\end{quote}
As of the ``Last modified'' line on the live entry (2026-05-30, revision 122) the
...
\[
p_{0}(n)=1,\qquad p_{1}(n)=- n - 1,\qquad p_{2}(n)=2 \left(n - 1\right),\qquad p_{3}(n)=2 - n
\]
...
\section{The computation for A002104}
...
\[
1\,a(n) + (- n - 1)\,a(n-1) + 2 \left(n - 1\right)\,a(n-2) + (2 - n)\,a(n-3) \;=\;0
\]
holds for every $n>3$.
\end{theorem}
...
section of A002104, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A002104.
