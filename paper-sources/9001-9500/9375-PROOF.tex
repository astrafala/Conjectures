...
\title{A proof of the conjectured recurrence for OEIS A182894}
...
\begin{abstract}
OEIS A182894 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A182894 is ``Number of weighted lattice paths in L\_n having no (1,0)-steps at level 0. The members of L\_n  are paths of weight n that start at (0,0) , end on the horizontal axis and whose steps are of the following four kinds: an (1,0)-step with weight 1, an (1,0)-step with weight 2, a (1,1)-step with weight 2, and a (1,-1)-step with weight 1. The weight of a path is the sum of the weights of its steps''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 0, 0, 2, 2, 4, 12, 24,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1}{x^{2} + x + \sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*a(n) +(-4*n+3)*a(n-1) +(n-3)*a(n-2) -3*a(n-3) +3*(5*n-14)*a(n-4) +6*(n-3)*a(n-5) +6*(n-4)*a(n-6) +4*(-n+6)*a(n-7)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2017-09-23, revision 14) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=3 - 4 n,\qquad p_{2}(n)=n - 3,\qquad p_{3}(n)=-3,\qquad p_{4}(n)=3 \left(5 n - 14\right),\qquad p_{5}(n)=6 \left(n - 3\right),\qquad p_{6}(n)=6 \left(n - 4\right),\qquad p_{7}(n)=- 4 \left(n - 6\right)
\]
...
\section{The computation for A182894}
...
\[
n\,a(n) + (3 - 4 n)\,a(n-1) + (n - 3)\,a(n-2) - 3\,a(n-3) + 3 \left(5 n - 14\right)\,a(n-4) + 6 \left(n - 3\right)\,a(n-5) + 6 \left(n - 4\right)\,a(n-6) -  4 \left(n - 6\right)\,a(n-7) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A182894, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A182894.
