...
\title{A proof of the conjectured recurrence for OEIS A025251}
...
\begin{abstract}
OEIS A025251 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 102 x^{6} - 138 x^{5} + 120 x^{4} + 114 x^{3} + 14 x^{2}$, a polynomial of degree $6$, which proves
...
OEIS A025251 is ``a(n) = a(1)*a(n-1) + a(2)*a(n-2) + ... + a(n-3)*a(3) for n >= 4, with initial terms 0, 1, 2, 0''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;0, 1, 2, 0, 2, 4, 2, 12,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{x^{2}}{2} - \frac{\sqrt{x^{4} - 8 x^{3} - 2 x^{2} + 1}}{2} + \frac{1}{2} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} n*(n\^{}2+n+1)*a(n) +(n-1)*(n\^{}2+2*n+3)*a(n-1) -2*(n-3)*(n\^{}2+n+1)*a(n-2) +2*(-5*n\^{}3+16*n\^{}2+19*n+30)*a(n-3) +(-7*n\^{}3+23*n\^{}2+59*n+126)*a(n-4) +(n-7)*(n\^{}2+2*n+3)*a(n-5)=0. - \emph{R. J. Mathar}, Feb 25 2015
\end{quote}
As of the ``Last modified'' line on the live entry (2025-01-16, revision 14) the
...
\[
p_{0}(n)=n \left(n^{2} + n + 1\right),\qquad p_{1}(n)=\left(n - 1\right) \left(n^{2} + 2 n + 3\right),\qquad p_{2}(n)=- 2 \left(n - 3\right) \left(n^{2} + n + 1\right),\qquad p_{3}(n)=- 2 \left(5 n^{3} - 16 n^{2} - 19 n - 30\right),\qquad p_{4}(n)=- 7 n^{3} + 23 n^{2} + 59 n + 126,\qquad p_{5}(n)=\left(n - 7\right) \left(n^{2} + 2 n + 3\right)
\]
...
\section{The computation for A025251}
...
\[
B(x)\;=\;- 102 x^{6} - 138 x^{5} + 120 x^{4} + 114 x^{3} + 14 x^{2} ,
\]
...
\[
n \left(n^{2} + n + 1\right)\,a(n) + \left(n - 1\right) \left(n^{2} + 2 n + 3\right)\,a(n-1) -  2 \left(n - 3\right) \left(n^{2} + n + 1\right)\,a(n-2) -  2 \left(5 n^{3} - 16 n^{2} - 19 n - 30\right)\,a(n-3) + (- 7 n^{3} + 23 n^{2} + 59 n + 126)\,a(n-4) + \left(n - 7\right) \left(n^{2} + 2 n + 3\right)\,a(n-5) \;=\;0
\]
holds for every $n>6$.
\end{theorem}
...
section of A025251, agreeing on all 13 terms compared.
...
vanish, for all 29 values of $n$ from $n=8$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 102 x^{6} - 138 x^{5} + 120 x^{4} + 114 x^{3} + 14 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A025251.
