...
\title{The empirical recurrence for OEIS A211327, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211327 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 17 2018 08:21:05, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 28 x^{4} - 24 x^{3} + 18 x^{2} + 15 x}{4 x^{4} + 2 x^{3} - 4 x^{2} - x + 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;- 28 x^{4} - 24 x^{3} + 18 x^{2} + 15 x,
\]
