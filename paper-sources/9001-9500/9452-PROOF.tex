...
\title{The empirical recurrence for OEIS A184688, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184688 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 14 2018 13:11:37, revision 11) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 6 x^{2} + 9 x}{16 x^{2} - 22 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;16 x^{2} - 22 x + 1
\]
