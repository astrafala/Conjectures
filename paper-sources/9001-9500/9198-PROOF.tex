...
\title{A proof of the conjectured recurrence for OEIS A007901}
...
\begin{abstract}
OEIS A007901 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $12 x^{7} + 36 x^{6} - 24 x^{5}$, a polynomial of degree $7$, which proves
...
OEIS A007901 is ``Number of minimal unavoidable n-celled pebbling configurations''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;0, 0, 0, 0, 4, 22, 98, 412,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 1} a(n)\,x^{n}\;=\;\frac{x^{3} \left(6 x^{3} + x^{2} - 5 x + \sqrt{1 - 4 x} \left(- x^{2} + 3 x - 1\right) + 1\right)}{9 x^{3} - 14 x^{2} + 7 x - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n-3)*(n-8)*a(n) +(-11*n\^{}2+127*n-324)*a(n-1) +42*(n\^{}2-12*n+34)*a(n-2) +(-65*n\^{}2+799*n-2400)*a(n-3) +18*(n-6)*(2*n-13)*a(n-4)=0. - \emph{R. J. Mathar}, Aug 14 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-12-12, revision 36) the
...
\[
p_{0}(n)=\left(n - 8\right) \left(n - 3\right),\qquad p_{1}(n)=- 11 n^{2} + 127 n - 324,\qquad p_{2}(n)=42 \left(n^{2} - 12 n + 34\right),\qquad p_{3}(n)=- 65 n^{2} + 799 n - 2400,\qquad p_{4}(n)=18 \left(n - 6\right) \left(2 n - 13\right)
\]
...
\section{The computation for A007901}
...
\[
\left(n - 8\right) \left(n - 3\right)\,a(n) + (- 11 n^{2} + 127 n - 324)\,a(n-1) + 42 \left(n^{2} - 12 n + 34\right)\,a(n-2) + (- 65 n^{2} + 799 n - 2400)\,a(n-3) + 18 \left(n - 6\right) \left(2 n - 13\right)\,a(n-4) \;=\;0
\]
holds for every $n>7$.
\end{theorem}
...
section of A007901, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=12 x^{7} + 36 x^{6} - 24 x^{5}$ is an
...
\url{https://oeis.org}, 2026. Sequence A007901.
