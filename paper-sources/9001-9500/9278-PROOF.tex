...
\title{The empirical recurrence for OEIS A206687, derived from the entry's generating function}
...
\begin{abstract}
OEIS A206687 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 23 2018 08:35:39, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{2 x^{4} + x^{3} - 2 x^{2} - x}{4 x^{2} + 2 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 4 x^{2} - 2 x + 1
\]
