...
\title{A proof of the conjectured recurrence for OEIS A098664}
...
\begin{abstract}
OEIS A098664 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A098664 is ``Expansion of e.g.f. BesselI(0,4*x)+BesselI(1,4*x)/2''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 8, 12, 96, 160, 1280, 2240,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{8 x - \sqrt{1 - 16 x^{2}} + 1}{8 x \sqrt{1 - 16 x^{2}}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 8*(n-1)*a(n-1) - 16*n*a(n-2) + 128*(2-n)*a(n-3) = 0. - \emph{R. J. Mathar}, Dec 08 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-10, revision 18) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=8 \left(n - 1\right),\qquad p_{2}(n)=- 16 n,\qquad p_{3}(n)=- 128 \left(n - 2\right)
\]
...
\section{The computation for A098664}
...
\[
(n + 1)\,a(n) + 8 \left(n - 1\right)\,a(n-1) -  16 n\,a(n-2) -  128 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A098664, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A098664.
