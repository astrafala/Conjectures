...
\title{The empirical recurrence for OEIS A233251, derived from the entry's generating function}
...
\begin{abstract}
OEIS A233251 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 10 2018 15:54:06, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 64 x^{2} + 3 x}{384 x^{2} - 56 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;384 x^{2} - 56 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A233251.
