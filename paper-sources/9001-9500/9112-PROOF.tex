...
\title{A proof of the conjectured recurrence for OEIS A191585}
...
\begin{abstract}
OEIS A191585 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $18 x^{2} + 4 x$, a polynomial of degree $2$, which proves
...
OEIS A191585 is ``Central coefficients of the Riordan matrix (1/(1-3*x\^{}2),x/(1-x)) (A191582)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 6, 19, 74, 276, 1056, 4047,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{12 x^{2} - 11 x + \sqrt{1 - 4 x} \left(2 - 9 x\right) + 2}{2 \left(4 x - 1\right) \left(9 x^{2} + 6 x - 2\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 2*n*(n+3)*a(n) +2*(-7*n\^{}2-19*n+24)*a(n-1) +3*(5*n\^{}2+11*n-48)*a(n-2) +18*(n+4)*(2*n-3)*a(n-3)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2016-06-14, revision 14) the
...
\[
p_{0}(n)=2 n \left(n + 3\right),\qquad p_{1}(n)=- 2 \left(7 n^{2} + 19 n - 24\right),\qquad p_{2}(n)=3 \left(5 n^{2} + 11 n - 48\right),\qquad p_{3}(n)=18 \left(n + 4\right) \left(2 n - 3\right)
\]
...
\section{The computation for A191585}
...
\[
2 n \left(n + 3\right)\,a(n) -  2 \left(7 n^{2} + 19 n - 24\right)\,a(n-1) + 3 \left(5 n^{2} + 11 n - 48\right)\,a(n-2) + 18 \left(n + 4\right) \left(2 n - 3\right)\,a(n-3) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A191585, agreeing on all 13 terms compared.
...
vanish, for all 23 values of $n$ from $n=3$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=18 x^{2} + 4 x$ is an
...
\url{https://oeis.org}, 2026. Sequence A191585.
