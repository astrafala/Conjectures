...
\title{A proof of the conjectured recurrence for OEIS A189176}
...
\begin{abstract}
OEIS A189176 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A189176 is ``Row sums of the Riordan matrix (1+x/sqrt(1-4*x),(1-sqrt(1-4*x))/2) (A189175)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 5, 15, 49, 168, 594, 2145,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 4 x^{2} + 5 x + \left(1 - 5 x\right) \sqrt{1 - 4 x} - 1}{2 x \left(4 x - 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) + 3*(1-3*n)*a(n-1) + 10*(2*n-3)*a(n-2) = 0. - \emph{R. J. Mathar}, Nov 15 2011
\end{quote}
As of the ``Last modified'' line on the live entry (2025-09-26, revision 29) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 3 \left(3 n - 1\right),\qquad p_{2}(n)=10 \left(2 n - 3\right)
\]
...
\section{The computation for A189176}
...
\[
(n + 1)\,a(n) -  3 \left(3 n - 1\right)\,a(n-1) + 10 \left(2 n - 3\right)\,a(n-2) \;=\;0
\]
holds for every $n>2$.
\end{theorem}
...
section of A189176, agreeing on all 13 terms compared.
...
\url{https://oeis.org}, 2026. Sequence A189176.
