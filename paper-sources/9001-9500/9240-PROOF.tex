...
\title{A proof of the conjectured recurrence for OEIS A157713}
...
\begin{abstract}
OEIS A157713 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A157713 is ``a(n) = (2*n+1)!*(2*n-2)!/((n-1)!*(n!)\textasciicircum{}2*6)''. It has offset $1$ and begins
\[
a(1),\dots,a(8)\;=\;1, 10, 280, 12600, 776160, 60540480, 5708102400, 630745315200,\ \dots
\]
The name is itself the definition of the sequence:
\begin{quote}\small
a(n) = (2*n+1)!*(2*n-2)!/((n-1)!*(n!)\textasciicircum{}2*6).
\end{quote}
that is,
\begin{equation}\label{eq:cf}
a(n)\;=\;\frac{\left(2 n - 2\right)! \left(2 n + 1\right)!}{6 n!^{2} \left(n - 1\right)!} .
\end{equation}
...
\begin{quote}\small
\textbf{Conjecture:} n*a(n) -4*(2*n+1)*(2*n-3)*a(n-1)=0. - \emph{R. J. Mathar}, Jun 08 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2025-05-20, revision 19) the
...
n\,a(n) -  4 \left(2 n - 3\right) \left(2 n + 1\right)\,a(n-1)
\end{equation}
...
\section{The computation for A157713}

The closed form \eqref{eq:cf} splits into one similarity class, with
representative
\[
\frac{\left(2 n - 2\right)! \left(2 n + 1\right)!}{6 n!^{2} \left(n - 1\right)!}
\]
...
exact integer arithmetic at every index where it applies: $16$ instances beginning at
...
entry \href{https://oeis.org/A157713}{A157713}, 2025-05-20.
