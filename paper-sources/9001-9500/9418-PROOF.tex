...
\title{The empirical recurrence for OEIS A259243, derived from the entry's generating function}
...
\begin{abstract}
OEIS A259243 carries an empirical recurrence of order $2$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 24 2018 16:21:26, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 12 x^{2} - 9 x}{3 x^{2} + x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 3 x^{2} - x + 1
\]
