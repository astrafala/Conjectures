...
\title{The empirical recurrence for OEIS A235510, derived from the entry's generating function}
...
\begin{abstract}
OEIS A235510 carries an empirical recurrence of order $5$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 19 2018 10:09:39, revision 9) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 8 x^{5} - 6 x^{4} + 23 x^{3} + 6 x^{2} - 16 x}{2 x^{5} + x^{4} - 6 x^{3} + 4 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 2 x^{5} - x^{4} + 6 x^{3} - 4 x + 1
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;8 x^{5} + 6 x^{4} - 23 x^{3} - 6 x^{2} + 16 x,
\]
