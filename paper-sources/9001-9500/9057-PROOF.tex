...
\title{A proof of the conjectured recurrence for OEIS A227081}
...
\begin{abstract}
OEIS A227081 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A227081 is ``Row sums of A124576''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 2, 8, 40, 212, 1152, 6360, 35520,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1}{6 x + 2 \sqrt{\left(2 x - 1\right) \left(6 x - 1\right)} - 1} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} 3*n*a(n) +2*(-13*n+9)*a(n-1) +4*(13*n-21)*a(n-2) +24*(-n+2)*a(n-3)=0.
\end{quote}
...
\[
p_{0}(n)=3 n,\qquad p_{1}(n)=- 2 \left(13 n - 9\right),\qquad p_{2}(n)=4 \left(13 n - 21\right),\qquad p_{3}(n)=- 24 \left(n - 2\right)
\]
...
\section{The computation for A227081}

The generating function \eqref{eq:gf} lies in $K=\mathbb{Q}(x)[\sqrt{D}]$ with
\[
D \;=\; \left(2 x - 1\right) \left(6 x - 1\right) .
\]
...
\[
3 n\,a(n) -  2 \left(13 n - 9\right)\,a(n-1) + 4 \left(13 n - 21\right)\,a(n-2) -  24 \left(n - 2\right)\,a(n-3) \;=\;0
\]
holds for every $n>1$.
\end{theorem}
...
section of A227081, agreeing on all 13 terms compared.
...
vanish, for all 20 values of $n$ from $n=3$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A227081.
