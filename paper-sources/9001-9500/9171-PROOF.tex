...
\title{The empirical recurrence for OEIS A275229, derived from the entry's generating function}
...
\begin{abstract}
OEIS A275229 carries an empirical recurrence of order $2$. The entry separately records a
...
OEIS A275229 is ``Number of 3 X n 0..2 arrays with no element equal to any value at offset (-2,-1) (-2,1) or (-1,0) and new values introduced in order 0..2.''. It has offset $1$ and begins
\[
2, 11, 61, 339, 1885, 10483, 58301, 324243,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 02 2019 10:46:56, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 3 x^{2} + 2 x}{8 x^{2} - 7 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;8 x^{2} - 7 x + 1
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A275229.
