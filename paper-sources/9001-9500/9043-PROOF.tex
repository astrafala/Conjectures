...
\title{The empirical recurrence for OEIS A268093, derived from the entry's generating function}
...
\begin{abstract}
OEIS A268093 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jan 11 2019 15:23:07, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- x^{3} - x^{2} + x}{4 x^{3} + 2 x^{2} - 4 x + 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;4 x^{3} + 2 x^{2} - 4 x + 1
\]
