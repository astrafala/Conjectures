...
\title{A proof of the conjectured recurrence for OEIS A165540}
...
\begin{abstract}
OEIS A165540 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $456300 x^{4} - 14661846 x^{3} + 19419476 x^{2} - 6768780 x + 563862$, a polynomial of degree $4$, which proves
...
OEIS A165540 is ``Number of permutations of length n which avoid the patterns 1234 and 2341''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 1, 2, 6, 22, 89, 376, 1611,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 12 x^{4} + 39 x^{3} - 41 x^{2} + 16 x + \sqrt{1 - 4 x} \left(- 4 x^{4} + 7 x^{3} + 9 x^{2} - 10 x + 2\right) - \left(4 x - 1\right) \left(- 3 x + \sqrt{1 - 4 x} \left(1 - x\right) + 1\right) \left(x^{2} - 3 x + 1\right) - 2}{\left(4 x - 1\right) \left(3 x + \sqrt{1 - 4 x} \left(x - 1\right) - 1\right) \left(x^{2} - 3 x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} +(14681*n+187954)*(n+3) *a(n) +(14681*n\^{}2-2696783*n-4897218) *a(n-1) +(-888761*n\^{}2+12771539*n+5490342) *a(n-2) +2*(1635223*n\^{}2-14850835*n+13281012) *a(n-3) +2*(-1908503*n\^{}2+15402653*n-25889820) *a(n-4) +156*(2*n-7)*(3301*n-14374) *a(n-5)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2026-06-28, revision 33) the
...
\[
p_{0}(n)=\left(n + 3\right) \left(14681 n + 187954\right),\qquad p_{1}(n)=14681 n^{2} - 2696783 n - 4897218,\qquad p_{2}(n)=- 888761 n^{2} + 12771539 n + 5490342,\qquad p_{3}(n)=2 \left(1635223 n^{2} - 14850835 n + 13281012\right),\qquad p_{4}(n)=- 2 \left(1908503 n^{2} - 15402653 n + 25889820\right),\qquad p_{5}(n)=156 \left(2 n - 7\right) \left(3301 n - 14374\right)
\]
...
\section{The computation for A165540}
...
\[
B(x)\;=\;456300 x^{4} - 14661846 x^{3} + 19419476 x^{2} - 6768780 x + 563862 ,
\]
...
\[
\left(n + 3\right) \left(14681 n + 187954\right)\,a(n) + (14681 n^{2} - 2696783 n - 4897218)\,a(n-1) + (- 888761 n^{2} + 12771539 n + 5490342)\,a(n-2) + 2 \left(1635223 n^{2} - 14850835 n + 13281012\right)\,a(n-3) -  2 \left(1908503 n^{2} - 15402653 n + 25889820\right)\,a(n-4) + 156 \left(2 n - 7\right) \left(3301 n - 14374\right)\,a(n-5) \;=\;0
\]
holds for every $n>4$.
\end{theorem}
...
section of A165540, agreeing on all 13 terms compared.
...
vanish, for all 27 values of $n$ from $n=5$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=456300 x^{4} - 14661846 x^{3} + 19419476 x^{2} - 6768780 x + 563862$ is an
...
\url{https://oeis.org}, 2026. Sequence A165540.
