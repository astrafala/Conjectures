...
\title{The empirical recurrence for OEIS A211322, derived from the entry's generating function}
...
\begin{abstract}
OEIS A211322 carries an empirical recurrence of order $3$. The entry separately records a
...
OEIS A211322 is ``Number of (n+1) X (n+1) -3..3 symmetric matrices with every 2 X 2 subblock having sum zero and one or two distinct values.''. It has offset $1$ and begins
\[
11, 15, 21, 31, 47, 73, 115, 183,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 16 2018 05:22:30, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 9 x^{3} - 7 x^{2} + 11 x}{x^{3} - 2 x + 1},
\]
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A211322.
