...
\title{A proof of the conjectured recurrence for OEIS A185966}
...
\begin{abstract}
OEIS A185966 carries a conjectured linear recurrence with polynomial coefficients, of
...
OEIS A185966 is ``Series reversion of A028310''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, -1, 0, 2, -2, -5, 14, 5,\ \dots
\]
The entry carries the following comment, still recorded as a conjecture:
\begin{quote}\small
\textbf{Conjecture:} 6*n*(n+1)*a(n) -n*(n-14)*a(n-1) +2*n*(14*n-19)*a(n-2) -4*(n-2)*(17*n-48)*a(n-3) +6*(2*n-5)*(n-4)*a(n-4)=0. - \emph{R. J. Mathar}, Nov 15 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2017-07-24, revision 16) it is still
...
\begin{quote}\small
Recurrence (of order 3): 3*n*(n+1)*(19*n-27)*a(n) = -2*n*(38*n\^{}2 - 73*n + 9)*a(n-1) - 20*(19*n\^{}3 - 65*n\^{}2 + 66*n - 18)*a(n-2) + 2*(n-3)*(2*n-3)*(19*n-8)*a(n-3). - \emph{Vaclav Kotesovec}, Jan 22 2014
\end{quote}
...
\[
6 n \left(n + 1\right)\,a(n) -  n \left(n - 14\right)\,a(n-1) + 2 n \left(14 n - 19\right)\,a(n-2) -  4 \left(n - 2\right) \left(17 n - 48\right)\,a(n-3) + 6 \left(n - 4\right) \left(2 n - 5\right)\,a(n-4) \;=\;0
\]
...
entry's own values and found to vanish, for all 34 values of $n$ from
$n=4$ upward.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A185966.
