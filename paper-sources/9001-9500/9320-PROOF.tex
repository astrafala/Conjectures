...
\title{The empirical recurrence for OEIS A223499, derived from the entry's generating function}
...
\begin{abstract}
OEIS A223499 carries an empirical recurrence of order $3$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 21 2018 05:54:40, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 10 x^{3} + 20 x^{2} - 9 x}{10 x^{3} - 24 x^{2} + 15 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 10 x^{3} + 24 x^{2} - 15 x + 1
\]
...
\begin{theorem}
\[
a(n)\;=\;15\,a(n-1) - 24\,a(n-2) + 10\,a(n-3)
\]
for every $n>3$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A223499.
