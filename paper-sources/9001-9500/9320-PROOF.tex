...
\title{The empirical recurrence for OEIS A184043, derived from the entry's generating function}
...
\begin{abstract}
OEIS A184043 carries an empirical recurrence of order $4$. The entry separately records a
...
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 10 2018 11:52:09, revision 8) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 168 x^{4} + 92 x^{3} + 84 x^{2} - 45 x}{4 x^{4} - 6 x^{3} + 3 x - 1},
\]
...
In exact arithmetic,
\[
D(x)\,G(x)\;=\;168 x^{4} - 92 x^{3} - 84 x^{2} + 45 x,
\]
