...
\title{A proof of the conjectured recurrence for OEIS A097188}
...
\begin{abstract}
OEIS A097188 carries a conjectured linear recurrence with polynomial coefficients, of
...
computed in closed form. Here it equals $1$, a polynomial of degree $0$, which proves
...
OEIS A097188 is ``G.f. A(x) satisfies A057083(x*A(x)) = A(x) and so equals the ratio of the g.f.s of any two adjacent diagonals of triangle A097186''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 3, 15, 90, 594, 4158, 30294, 227205,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{1 - \sqrt[3]{1 - 9 x}}{3 x} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} (n+1)*a(n) +3*(-3*n+1)*a(n-1)=0. - \emph{R. J. Mathar}, Nov 16 2012
\end{quote}
As of the ``Last modified'' line on the live entry (2025-11-05, revision 54) the
...
\[
p_{0}(n)=n + 1,\qquad p_{1}(n)=- 3 \left(3 n - 1\right)
\]
...
\[
(n + 1)\,a(n) -  3 \left(3 n - 1\right)\,a(n-1) \;=\;0
\]
holds for every $n>0$.
\end{theorem}
...
section of A097188, agreeing on all 13 terms compared.
...
vanish, for all 20 values of $n$ from $n=1$ upward. This is what
...
\url{https://oeis.org}, 2026. Sequence A097188.
