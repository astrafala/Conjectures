...
\title{A proof of the conjectured recurrence for OEIS A385252}
...
\begin{abstract}
OEIS A385252 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $- 6912 x^{5} + 5760 x^{4} - 1104 x^{3} + 64 x^{2}$, a polynomial of degree $5$, which proves
...
OEIS A385252 is ``Number of ternary strings of length 2*n that have at least one 0 but less 0's than the combined number of 1's and 2's''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 32, 432, 4608, 45440, 432896, 4051712,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- x \left(288 x^{2} - 104 x + 5 \sqrt{1 - 8 x} \left(- 12 x - \sqrt{1 - 8 x} + 1\right) + 8\right) + \sqrt{1 - 8 x} \left(12 x + \sqrt{1 - 8 x} - 1\right) \left(36 x^{2} - 13 x + 1\right)}{\sqrt{1 - 8 x} \left(12 x + \sqrt{1 - 8 x} - 1\right) \left(36 x^{2} - 13 x + 1\right)} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
Conjecture D-finite with recurrence n*a(n) +(-37*n+36)*a(n-1) +4*(131*n-245)*a(n-2) +16*(-221*n+605)*a(n-3) +192*(59*n-213)*a(n-4) +6912*(-2*n+9)*a(n-5)=0. - \emph{R. J. Mathar}, Jul 31 2025
\end{quote}
As of the ``Last modified'' line on the live entry (2025-08-12, revision 15) the
...
\[
p_{0}(n)=n,\qquad p_{1}(n)=36 - 37 n,\qquad p_{2}(n)=4 \left(131 n - 245\right),\qquad p_{3}(n)=- 16 \left(221 n - 605\right),\qquad p_{4}(n)=192 \left(59 n - 213\right),\qquad p_{5}(n)=- 6912 \left(2 n - 9\right)
\]
...
\section{The computation for A385252}
...
\[
B(x)\;=\;- 6912 x^{5} + 5760 x^{4} - 1104 x^{3} + 64 x^{2} ,
\]
...
\[
n\,a(n) + (36 - 37 n)\,a(n-1) + 4 \left(131 n - 245\right)\,a(n-2) -  16 \left(221 n - 605\right)\,a(n-3) + 192 \left(59 n - 213\right)\,a(n-4) -  6912 \left(2 n - 9\right)\,a(n-5) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A385252, agreeing on all 13 terms compared.
...
vanish, for all 16 values of $n$ from $n=6$ upward. This is what
...
cancellation in $K$, not by series truncation, so the identity $B=- 6912 x^{5} + 5760 x^{4} - 1104 x^{3} + 64 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A385252.
