...
\title{A proof of the conjectured recurrence for OEIS A166290}
...
\begin{abstract}
OEIS A166290 carries a conjectured linear recurrence with polynomial coefficients, of
...
closed form. Here it equals $29643 x^{5} + 291942 x^{4} + 1728135 x^{3} + 793956 x^{2}$, a polynomial of degree $5$, which proves
...
OEIS A166290 is ``Number of UDUD's in all Dyck paths of semilength n with no UUU's and no DDD's (U=(1,1), D=(1,-1))''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;0, 0, 1, 3, 9, 24, 63, 164,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{4} - 2 x^{3} - 2 x^{2} - 3 x + \sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)} \left(x - 1\right) \left(x + 2\right) + 2}{2 x^{2} \sqrt{\left(x^{2} - 3 x + 1\right) \left(x^{2} + x + 1\right)}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} -(n+2)*(60283*n-319055)*a(n) +(-60283*n\^{}2-13090*n+236862)*a(n-1) +(588710*n\^{}2-2858167*n+437486)*a(n-2) +(-32043*n\^{}2+2024290*n-2826488)*a(n-3) +(134686*n\^{}2+585099*n-3417942)*a(n-4) +(-514307*n\^{}2+4366464*n-8945002)*a(n-5) +(166729*n-863288)*(n-6)*a(n-6)=0. - \emph{R. J. Mathar}, Jun 14 2016
\end{quote}
As of the ``Last modified'' line on the live entry (2021-12-08, revision 12) the
...
\[
p_{0}(n)=- \left(n + 2\right) \left(60283 n - 319055\right),\qquad p_{1}(n)=- 60283 n^{2} - 13090 n + 236862,\qquad p_{2}(n)=588710 n^{2} - 2858167 n + 437486,\qquad p_{3}(n)=- 32043 n^{2} + 2024290 n - 2826488,\qquad p_{4}(n)=134686 n^{2} + 585099 n - 3417942,\qquad p_{5}(n)=- 514307 n^{2} + 4366464 n - 8945002,\qquad p_{6}(n)=\left(n - 6\right) \left(166729 n - 863288\right)
\]
...
\section{The computation for A166290}
...
\[
B(x)\;=\;29643 x^{5} + 291942 x^{4} + 1728135 x^{3} + 793956 x^{2} ,
\]
...
\[
- \left(n + 2\right) \left(60283 n - 319055\right)\,a(n) + (- 60283 n^{2} - 13090 n + 236862)\,a(n-1) + (588710 n^{2} - 2858167 n + 437486)\,a(n-2) + (- 32043 n^{2} + 2024290 n - 2826488)\,a(n-3) + (134686 n^{2} + 585099 n - 3417942)\,a(n-4) + (- 514307 n^{2} + 4366464 n - 8945002)\,a(n-5) + \left(n - 6\right) \left(166729 n - 863288\right)\,a(n-6) \;=\;0
\]
holds for every $n>5$.
\end{theorem}
...
section of A166290, agreeing on all 13 terms compared.
...
cancellation in $K$, not by series truncation, so the identity $B=29643 x^{5} + 291942 x^{4} + 1728135 x^{3} + 793956 x^{2}$ is an
...
\url{https://oeis.org}, 2026. Sequence A166290.
