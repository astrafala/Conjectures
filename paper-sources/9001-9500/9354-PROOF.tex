...
\title{A proof of the conjectured identity for OEIS A315520}
...
\begin{abstract}
OEIS A315520 carries a conjectured identity expressing its terms through the terms of
A188510. We prove it. Each entry involved posts a generating function, so the claim is
a single identity between generating functions: writing $F_{a}$ for the generating
function of A315520 and $F_{j}$ for those of the entries on the right, the conjecture
...
closed form and equals $1$, of degree $0$, which proves the identity for
every $n>0$.
\end{abstract}
...
OEIS A315520 is ``Coordination sequence Gal.4.140.3 where Gal.u.t.v denotes the coordination sequence for a vertex of type v in tiling number t in the Galebach list of u-uniform tilings''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 6, 11, 17, 22, 27, 33, 38,\ \dots
\]
Its generating function, as posted on the entry, is
\begin{equation}\label{eq:gf}
F_{a}(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{x^{6} + 4 x^{5} + x^{3} + 4 x + 1}{\left(x - 1\right)^{2} \left(x^{4} + 1\right)} ,
\end{equation}
and the entry carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = (22*n + 2*A188510(n))/4 for n > 0. - \emph{Stefano Spezia}, Jul 30 2022
\end{quote}
As of the ``Last modified'' line on the live entry (2023-10-21, revision 13) the
...
\section{The computation for A315520}

Assembling the right-hand side by Lemmas~\ref{lem:shift} and~\ref{lem:theta} gives
\[
R(x)\;=\;\frac{x \left(11 x^{4} + \left(x^{2} + 1\right) \left(x^{2} - 2 x + 1\right) + 11\right)}{2 \left(x^{4} + 1\right) \left(x^{2} - 2 x + 1\right)} ,
\]
...
\begin{theorem}
The conjectured identity holds for every $n>0$.
\end{theorem}
...
both sides were formed from the entries' own values and found to agree, for all 49
...
functions and the theorem holds for all $n>0$ simultaneously.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer Sequences},
\url{https://oeis.org}, 2026. Sequence A315520.
