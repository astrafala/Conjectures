...
\title{The empirical recurrence for OEIS A232951, derived from the entry's generating function}
...
\begin{abstract}
OEIS A232951 carries an empirical recurrence of order $3$. The entry separately records a
...
\begin{quote}
G.f.: 3*x*(9 - 22*x + 5*x\^{}2) / ((1 - x)*(1 - 30*x + 5*x\^{}2)).
\end{quote}
As of the ``Last modified'' line on the live entry (Oct 06 2018 09:39:15, revision 7) the first is still
...
Write
\[
G(x)\;=\;\sum_{n}a(n)\,x^{n}\;=\;\frac{- 15 x^{3} + 66 x^{2} - 27 x}{5 x^{3} - 35 x^{2} + 31 x - 1},
\]
the entry's own generating function, and let
\[
D(x)\;=\;1-\sum_i c_i x^{i}\;=\;- 5 x^{3} + 35 x^{2} - 31 x + 1
\]
