...
\title{The empirical recurrence for OEIS A237162, proved by transfer matrix}
...
\begin{abstract}
OEIS A237162 counts arrays over $\{0,\dots,2\}$ subject to a condition on the order
...
The condition is local to two consecutive rows, so the count is a walk count on $S=209$
...
OEIS A237162 is ``Number of (n+1)X(3+1) 0..2 arrays with the maximum plus the minimum minus the upper median minus the lower median of every 2X2 subblock equal.''. It has offset $1$ and begins
\[
777, 4365, 25705, 165317, 1096689, 7519165, 52597313, 374547449,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 09:54:54, revision 6) this is still
...
array, which is not among the objects counted. There are $S=209$ states left.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A237162.
