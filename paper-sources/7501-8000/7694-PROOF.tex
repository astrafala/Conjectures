...
\title{The empirical recurrence for OEIS A250578, proved by transfer matrix}
...
\begin{abstract}
OEIS A250578 counts arrays over $\{0,1\}$ in which a statistic taken over short runs
...
OEIS A250578 is ``Number of (n+1) X (3+1) 0..1 arrays with nondecreasing max(x(i,j),x(i,j-1)) in the i direction and nondecreasing min(x(i,j),x(i-1,j)) in the j direction.''. It has offset $1$ and begins
\[
55, 142, 336, 778, 1779, 4077, 9346, 21510,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 5*a(n-1) - 6*a(n-2) - 7*a(n-3) + 21*a(n-4) - 14*a(n-5) - a(n-6) + 4*a(n-7) - a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 15 2018 08:38:01, revision 7) this is still
...
So a single row is a state. Let $M$ be the matrix on the $2^{4}=16$ rows with
...
\begin{theorem}
\[
a(n)\;=\;5\,a(n-1) - 6\,a(n-2) - 7\,a(n-3) + 21\,a(n-4) - 14\,a(n-5) - a(n-6) + 4\,a(n-7) - a(n-8)
\]
for every $n>8$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A250578.
