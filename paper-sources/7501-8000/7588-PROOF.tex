...
and $5$ cells in a row, so $|V|=6^{5}=7776$, and the vertex set is
$V\times V$, of size $7776^2=7776$. That is the whole of the state: two consecutive
...
publishes, $n=10$, the arrays have $5\cdot10$ cells over
$6$ values, so there are about $10^{38}$ of them to sift. The
walk count replaces that by $7776$ states and one matrix--vector product per row.
