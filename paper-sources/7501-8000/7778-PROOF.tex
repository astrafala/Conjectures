...
\title{The empirical recurrence for OEIS A234263, proved by transfer matrix}
...
\begin{abstract}
OEIS A234263 counts $6$-column arrays over $\{0,\dots,2\}$ in which every
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=729$ vertices, which
...
OEIS A234263 is ``Number of (n+1) X (5+1) 0..2 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
560, 754, 1100, 1816, 3188, 6076, 11876,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) + 6*a(n-2) - 24*a(n-3) + 4*a(n-4) + 36*a(n-5) - 24*a(n-6).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 21:33:28, revision 12) the statement is
...
Let $V$ consist of all of $\{0,\dots,2\}^{6}$, and put $S=|V|=729$. For $r,s\in V$ declare $r\to s$ an edge
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=729$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A234263.
