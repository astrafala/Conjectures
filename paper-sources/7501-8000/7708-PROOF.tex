...
\title{The empirical recurrence for OEIS A256803, proved by transfer matrix}
...
\begin{abstract}
OEIS A256803 counts $(n+2)\times3$ arrays over $\{0,\dots,1\}$ subject to a condition imposed on
...
OEIS A256803 is ``Number of (n+2) X (1+2) 0..1 arrays with no 3 x 3 subblock diagonal sum 0 and no antidiagonal sum 0 and no row sum 0 or 1 and no column sum 0 or 1.''. It has offset $1$ and begins
\[
32, 86, 237, 641, 1731, 4690, 12707, 34408,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = a(n-1) + a(n-2) + 7*a(n-3) + 7*a(n-4) + 3*a(n-5) - a(n-6) - 6*a(n-7) - 2*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Dec 20 2018 23:07:15, revision 8) this is still
...
\begin{theorem}
\[
a(n)\;=\;a(n-1) + a(n-2) + 7\,a(n-3) + 7\,a(n-4) + 3\,a(n-5) - a(n-6) - 6\,a(n-7) - 2\,a(n-8)
\]
for every $n>8$.
\end{theorem}
...
exactly. Those integers vanish from the index corresponding to $n=8+1$ onwards, and the
run was continued until the working vector was identically zero, which settles all larger $j$
at once. By Lemma~\ref{lem:crit} the recurrence holds for every $n>8$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A256803.
