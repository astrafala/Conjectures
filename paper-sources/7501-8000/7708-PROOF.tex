...
\title{The empirical recurrence for OEIS A196961, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A196961 counts arrays over $\{0,\dots,4\}$ in which every cell is constrained by the
...
OEIS A196961 is ``Number of n X 2 0..4 arrays with each element x equal to the number its horizontal and vertical neighbors equal to 4,1,2,0,3 for x=0,1,2,3,4.''. It has offset $1$ and begins
\[
2, 6, 15, 48, 145, 449, 1397, 4342,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) +3*a(n-2) +2*a(n-3) -5*a(n-5) -3*a(n-6) +2*a(n-7).
\end{quote}
As of the ``Last modified'' line on the live entry (May 10 2018 11:26:46, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 3\,a(n-2) + 2\,a(n-3) - 5\,a(n-5) - 3\,a(n-6) + 2\,a(n-7)
\end{equation}
for all $n>8$.
...
\#\{\text{neighbours with value }f(x_{t,u})\}=x_{t,u},\quad f=(4,1,2,0,3)
\end{equation}
for every cell $(t,u)$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A196961.
