...
\title{The empirical recurrence for OEIS A318338, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A318338 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A318338 is ``Number of nX3 0..1 arrays with every element unequal to 0, 1, 2 or 6 horizontally, vertically or antidiagonally adjacent elements, with upper left element zero.''. It has offset $1$ and begins
\[
4, 10, 20, 36, 76, 160, 344, 748,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 2*a(n-1) +2*a(n-2) -3*a(n-3) -2*a(n-4) +2*a(n-5) for n>6
\end{quote}
As of the ``Last modified'' line on the live entry (Aug 24 2018 11:01:39, revision 4) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;2\,a(n-1) + 2\,a(n-2) - 3\,a(n-3) - 2\,a(n-4) + 2\,a(n-5)
\end{equation}
for all $n>6$.
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A318338.
