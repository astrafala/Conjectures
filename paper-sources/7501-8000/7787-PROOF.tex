...
\title{The empirical recurrence for OEIS A185461, proved by transfer matrix}
...
\begin{abstract}
OEIS A185461 counts arrays over $\{0,\dots,1\}$ under a condition on the determinant of every
$2\times2$ subblock, and carries an empirical recurrence of order $6$ contributed by
R.~H.~Hardin. It is true, and it is decidable rather than empirical. The condition is local to
a bounded window of consecutive rows, so the count is a walk count on $S=92$ states and is
...
OEIS A185461 is ``Number of (n+1) X 4 binary arrays with no 2 X 2 subblock determinant equal to any horizontal or vertical neighbor 2 X 2 subblock determinant.''. It has offset $1$ and begins
\[
92, 240, 866, 2766, 8762, 27102, 83688, 258056,\ \dots
\]
The entry states, as a conjecture contributed by its author and never marked settled:
\begin{quote}
Empirical: a(n) = 2*a(n-1) + 2*a(n-2) + 3*a(n-3) + 2*a(n-4) + 4*a(n-5) + 2*a(n-6) for n>12.
\end{quote}
As of the ``Last modified'' line on the live entry (Apr 15 2018 09:16:39, revision 8) this is still
...
entry's first term is $92$, and the model reproduces it along with every later term; that
...
\begin{theorem}
\[
a(n)\;=\;2\,a(n-1) + 2\,a(n-2) + 3\,a(n-3) + 2\,a(n-4) + 4\,a(n-5) + 2\,a(n-6)
\]
for every $n>12$.
\end{theorem}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A185461.
