...
\title{The empirical recurrence for OEIS A234241, proved by transfer matrix}
...
\begin{abstract}
OEIS A234241 counts $2$-column arrays over $\{0,\dots,7\}$ in which every
$2\times2$ subblock has its diagonal sum differing from its antidiagonal sum by
exactly $2$, and it
carries an empirical recurrence of order $8$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=64$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-8}q(M)\mathbf{1}$, so the recurrence
...
OEIS A234241 is ``Number of (n+1) X (1+1) 0..7 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 2 (constant-stress 1 X 1 tilings).''. It has offset
$1$ and begins
\[
630, 6706, 72616, 790718, 8612920, 93934380, 1023822944,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 7*a(n-1) +181*a(n-2) -1022*a(n-3) -7657*a(n-4) +21504*a(n-5) +54144*a(n-6) -96768*a(n-7) -80640*a(n-8).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 20 2022 19:08:06, revision 7) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;7\,a(n-1) + 181\,a(n-2) - 1022\,a(n-3) - 7657\,a(n-4) + 21504\,a(n-5) + 54144\,a(n-6) - 96768\,a(n-7) - 80640\,a(n-8)
\end{equation}
for all $n>7$, which is every $n$ at which a recurrence of order $8$ can be
stated.
...
Let $V$ consist of all of $\{0,\dots,7\}^{2}$, and put $S=|V|=64$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge8$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-8}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-8$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-8}\bigl(M^{8}-\sum_i c_i M^{8-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-8$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $8$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=7-8+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>7$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A234241.
