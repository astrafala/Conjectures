...
\title{The empirical recurrence for OEIS A251148, proved by transfer matrix}
...
\begin{abstract}
OEIS A251148 counts $8$-column arrays over $\{0,\dots,2\}$ in which every
...
\mathbf{1}$ for the adjacency matrix $M$ of that digraph, on $S=6561$ vertices. Writing
...
OEIS A251148 is ``Number of (n+1) X (7+1) 0..2 arrays with every 2 X 2 subblock summing to 4 and no 2 X 2 subblock having exactly two nonzero entries.''. It has offset $1$
and begins
\[
853, 1075, 1413, 1987, 2925, 4491, 7101, 11491,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) - 5*a(n-3) + a(n-4) + 2*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Nov 26 2018 16:16:51, revision 16) the statement is
...
Let $V=\{0,1,\dots,2\}^{8}$, the set of possible rows, so $|V|=S=6561$. For
$r,s\in V$ declare $r\to s$ an edge when, for every $j$ with $1\le j\le 8-1$,
\begin{itemize}
...
\begin{proof}
An array with $n+1$ rows and $8$ columns is precisely a sequence
...
bound $j<S=6561$ rather than to a sampled prefix, in exact integer arithmetic, and returned
zero throughout.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A251148.
