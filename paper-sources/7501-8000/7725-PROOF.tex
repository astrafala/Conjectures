...
\title{The empirical recurrence for OEIS A299735, proved by transfer matrix}
...
\begin{abstract}
OEIS A299735 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $equal to$ itself lying in $\{0,1,2,3,4,5,8\}$, and it carries an empirical
...
OEIS A299735 is ``Number of nX3 0..1 arrays with every element equal to 0, 1, 2, 3, 4, 5 or 8 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
4, 32, 220, 1578, 11303, 81105, 582032, 4177161,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 8*a(n-1) -4*a(n-2) -13*a(n-3) -6*a(n-4) +5*a(n-5) +23*a(n-6) -20*a(n-7) -12*a(n-8)
\end{quote}
As of the ``Last modified'' line on the live entry (Feb 18 2018 10:12:15, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;8\,a(n-1) - 4\,a(n-2) - 13\,a(n-3) - 6\,a(n-4) + 5\,a(n-5) + 23\,a(n-6) - 20\,a(n-7) - 12\,a(n-8)
\end{equation}
for all $n>8$.
...
has score in $\{0,1,2,3,4,5,8\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{0,1,2,3,4,5,8\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{0,1,2,3,4,5,8\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{0,1,2,3,4,5,8\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{0,1,2,3,4,5,8\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A299735.
