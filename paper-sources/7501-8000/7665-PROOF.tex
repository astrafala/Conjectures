...
\title{The empirical recurrence for OEIS A231376, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A231376 counts arrays over $\{0,\dots,1\}$ in which every cell is constrained by the
...
OEIS A231376 is ``Number of n X 2 0..1 arrays with no element less than a strict majority of its horizontal and vertical neighbors.''. It has offset $1$ and begins
\[
2, 10, 21, 48, 113, 261, 601, 1390,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) - 2*a(n-2) + 2*a(n-3) - 2*a(n-4) - a(n-5) for n>6.
\end{quote}
As of the ``Last modified'' line on the live entry (Mar 18 2018 06:59:19, revision 8) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) - 2\,a(n-2) + 2\,a(n-3) - 2\,a(n-4) - a(n-5)
\end{equation}
for all $n>6$.
...
$2$ values, so there are about $10^{17}$ of them to sift. The
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231376.
