...
\title{The empirical recurrence for OEIS A235091, proved by transfer matrix}
...
\begin{abstract}
OEIS A235091 counts $2$-column arrays over $\{0,\dots,5\}$ in which every
...
carries an empirical recurrence of order $5$ contributed by R.~H.~Hardin. It is true,
...
length $n$. Hence $a(n)=\mathbf{1}^{\!\top}M^{n}\mathbf{1}$ on $S=30$ vertices, which
is $C$-finite. Writing $q$ for the characteristic polynomial of the conjectured recurrence,
the residual is $\mathbf{1}^{\!\top}M^{\,n-5}q(M)\mathbf{1}$, so the recurrence
...
OEIS A235091 is ``Number of (n+1) X (1+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
80, 256, 808, 2580, 8184, 26164, 83224,\ \dots
\]
The entry carries the following comment:
\begin{quote}\small
Empirical: a(n) = 3*a(n-1) + 13*a(n-2) - 39*a(n-3) - 26*a(n-4) + 78*a(n-5).
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 01:32:25, revision 9) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 13\,a(n-2) - 39\,a(n-3) - 26\,a(n-4) + 78\,a(n-5)
\end{equation}
...
Let $V$ consist of those $r\in\{0,\dots,5\}^{2}$ with $r_j\ne r_{j+1}$ for every
$j$ --- the rows in which no two horizontally adjacent entries are equal, and put $S=|V|=30$. For $r,s\in V$ declare $r\to s$ an edge
...
For $n\ge5$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-5}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-5$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-5}\bigl(M^{5}-\sum_i c_i M^{5-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-5$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $5$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=5-5+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>5$.
\end{proof}
...
Third, the vanishing test of Lemma~\ref{lem:crit} was carried past $S=30$ consecutive
zeros rather than to a sampled prefix, in exact integer arithmetic.

\begin{thebibliography}{9}
\bibitem{oeis} The OEIS Foundation, \emph{The On-Line Encyclopedia of Integer
Sequences}, \url{https://oeis.org}, 2026. Sequence A235091.
