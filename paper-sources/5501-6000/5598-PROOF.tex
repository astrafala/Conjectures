...
\title{The empirical recurrence for OEIS A231286, proved by a two-line transfer matrix}
...
\begin{abstract}
OEIS A231286 counts arrays over $\{0,\dots,3\}$ in which every cell is constrained by the
values of its neighbours, and carries an empirical recurrence of order $19$ contributed
...
OEIS A231286 is ``Number of 3Xn 0..3 arrays x(i,j) with each element horizontally or antidiagonally next to at least one element with value (x(i,j)+1) mod 4, and upper left element zero.''. It has offset $1$ and begins
\[
0, 0, 8, 76, 834, 9488, 105962, 1179364,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jul 23 2025 06:22:30, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;26\,a(n-1) - 267\,a(n-2) + 1560\,a(n-3) - 6087\,a(n-4) + 17438\,a(n-5) - 38845\,a(n-6) + 70000\,a(n-7) - 108952\,a(n-8) + 161144\,a(n-9) - 214028\,a(n-10) + 150344\,a(n-11) + 187284\,a(n-12) - 587360\,a(n-13) + 482160\,a(n-14) + 11936\,a(n-15) - 126224\,a(n-16) - 26496\,a(n-17) - 29888\,a(n-18) + 19968\,a(n-19)
\end{equation}
for all $n>21$.
...
column $t$ and row $u$, each in $\{0,\dots,3\}$. With the neighbour offsets
\[
...
\exists\ \text{a neighbour with value }(x_{t,u}+1)\bmod 4
\end{equation}
...
Let $V=\{0,\dots,3\}^{3}$ be the possible columns and take
...
and $3$ cells in a column, so $|V|=4^{3}=64$, and the vertex set is
...
publishes, $n=19$, the arrays have $3\cdot19$ cells over
$4$ values, so there are about $10^{34}$ of them to sift. The
...
$o=1n_0+0-2$. Then for $j\ge19$,
\[
a(n_0+j)-\sum_i c_i\,a(n_0+j-i)\;=\;\,\mathbf{v}^{\!\top}G^{\,j-19}w,
...
\begin{proof}
Substitute Lemma~\ref{lem:walk} and factor $G^{j-19}$ out of
...
of the $(3+1)^{3}$ possible next columns, the condition was tested on every cell of
the middle column. The vector $w$ was formed by $19$ applications of $G$ in exact integer
arithmetic and $\mathbf{v}^{\!\top}G^{m}w$ evaluated for $m=0,1,\dots$, again exactly.
Every one of those integers vanishes from the index corresponding to $n=21+1$ onwards,
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A231286.
