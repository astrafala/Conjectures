...
\title{The empirical recurrence for OEIS A235093, proved by transfer matrix}
...
\begin{abstract}
OEIS A235093 counts $4$-column arrays over $\{0,\dots,5\}$ in which every
...
carries an empirical recurrence of order $22$ contributed by R.~H.~Hardin. It is true,
...
the residual is $\mathbf{1}^{\!\top}M^{\,n-22}q(M)\mathbf{1}$, so the recurrence
...
OEIS A235093 is ``Number of (n+1) X (3+1) 0..5 arrays with every 2 X 2 subblock having its diagonal sum differing from its antidiagonal sum by 4, with no adjacent elements equal (constant-stress tilted 1 X 1 tilings).''. It has offset
$1$ and begins
\[
808, 1728, 3832, 9460, 23792, 63884, 173440,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (Jun 19 2022 02:35:50, revision 6) the statement is
still recorded as empirical, and nothing on the entry records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;7\,a(n-1) + 20\,a(n-2) - 239\,a(n-3) + 36\,a(n-4) + 3285\,a(n-5) - 4086\,a(n-6) - 23439\,a(n-7) + 45882\,a(n-8) + 91959\,a(n-9) - 251226\,a(n-10) - 184209\,a(n-11) + 783240\,a(n-12) + 99087\,a(n-13) - 1431666\,a(n-14) + 277611\,a(n-15) + 1499767\,a(n-16) - 513754\,a(n-17) - 855386\,a(n-18) + 293540\,a(n-19) + 256932\,a(n-20) - 54600\,a(n-21) - 32760\,a(n-22)
\end{equation}
for all $n>22$, which is every $n$ at which a recurrence of order $22$ can be
stated.
...
For $n\ge22$,
$a(n)-\sum_i c_i a(n-i)=\mathbf{1}^{\!\top}M^{\,n-22}q(M)\mathbf{1}$. Writing
$u_j=\mathbf{1}^{\!\top}M^{j}q(M)\mathbf{1}$, the recurrence therefore holds for every
$n>t$ exactly when $u_j=0$ for every $j>t-22$. Moreover $(u_j)_{j\ge0}$ satisfies the
...
$a(n)-\sum_i c_i a(n-i)
=\mathbf{1}^{\!\top}M^{\,n-22}\bigl(M^{22}-\sum_i c_i M^{22-i}\bigr)
\mathbf{1}$, which is the identity. Putting $j=n-22$ gives the equivalence. For the
...
\begin{proof}
The vector $q(M)\mathbf{1}\in\mathbb{Z}^{S}$ was formed by $22$ matrix--vector
products in exact integer arithmetic and the values $u_j$ evaluated for
$j=0,1,\dots$. They vanish from $j=22-22+1$ onwards, and the run was continued past
$S$ consecutive zeros, which by Lemma~\ref{lem:crit} settles every larger $j$. Hence the
recurrence holds for every $n>22$.
\end{proof}
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A235093.
