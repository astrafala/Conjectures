...
\title{The empirical recurrence for OEIS A305170, proved by transfer matrix}
...
\begin{abstract}
OEIS A305170 counts $3$-column arrays over $\{0,\dots,1\}$ in which every entry has,
among the eight king-move neighbours, a number $different from$ itself lying in $\{2,3,4,6,7\}$, and it carries an empirical
...
OEIS A305170 is ``Number of nX3 0..1 arrays with every element unequal to 2, 3, 4, 6 or 7 king-move adjacent elements, with upper left element zero.''. It has offset
$1$ and begins
\[
0, 5, 4, 46, 151, 543, 2120, 8155,\ \dots
\]
...
\end{quote}
As of the ``Last modified'' line on the live entry (May 26 2018 18:13:59, revision
4) the statement is still recorded as empirical, and nothing on the entry
records it as settled either way.

Written out, the claim is
\begin{equation}\label{eq:conj}
a(n)\;=\;3\,a(n-1) + 4\,a(n-2) + 3\,a(n-3) - 19\,a(n-4) - 42\,a(n-5) + 50\,a(n-6) + 44\,a(n-7) - 33\,a(n-8) - 9\,a(n-9) - 2\,a(n-10) - 107\,a(n-11) - 33\,a(n-12) + 116\,a(n-13) + 112\,a(n-14) + 80\,a(n-15) + 34\,a(n-16) + 16\,a(n-17) + 8\,a(n-18)
\end{equation}
for all $n>18$.
...
has score in $\{2,3,4,6,7\}$. The entry also fixes the top-left entry to be $0$; that is a condition on the first row alone, so it restricts the start vector and nothing else.
...
\begin{itemize}
\item $\mathbf{s}(r,s)=1$ when every cell of $r$ has score in $\{2,3,4,6,7\}$ \emph{with no row
above $r$} and $r_1=0$, and $0$ otherwise;
\item an edge $(r,s)\to(s,t)$ when every cell of $s$ has score in $\{2,3,4,6,7\}$ computed from
$r$, $s$ and $t$;
\item $\mathbf{e}(r,s)=1$ when every cell of $s$ has score in $\{2,3,4,6,7\}$ \emph{with no row
...
exactly when every one of its $n$ rows has all scores in $\{2,3,4,6,7\}$. Read the sequence as the
...
Sequences}, \url{https://oeis.org}, 2026. Sequence A305170.
