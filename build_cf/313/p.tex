...
\title{A proof of the conjectured closed form for OEIS A367015}
...
\begin{abstract}
OEIS A367015 carries a conjectured closed form for its terms. We prove it. The entry
...
OEIS A367015 is ``Number of regions formed after n points have been placed in general position on each edge of a triangle (as in A365929)''. It has offset $0$ and begins
\[
a(0),\dots,a(7)\;=\;1, 4, 28, 136, 445, 1126, 2404, 4558,\ \dots
\]
The entry gives the generating function
\begin{equation}\label{eq:gf}
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;\frac{- 10 x^{4} - 26 x^{3} - 18 x^{2} + x - 1}{\left(x - 1\right)^{5}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = (9*n\^{}4 - 12*n\^{}3 + 15*n\^{}2 + 4)/4. [This is now a theorem - \emph{N. J. A. Sloane}, Dec 31 2025]
\end{quote}
As of the ``Last modified'' line on the live entry (2026-04-17, revision 38) the
...
\[
f(n)\;=\;\frac{9 n^{4}}{4} - 3 n^{3} + \frac{15 n^{2}}{4} + 1 .
\]
...
\section{The computation for A367015}

By Lemma~\ref{lem:gf} the conjectured formula has generating function
\[
F(x)\;=\;\frac{- 10 x^{4} - 26 x^{3} - 18 x^{2} + x - 1}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1} ,
\]
...
For every $n>0$,
\[
a(n)\;=\;\frac{9 n^{4}}{4} - 3 n^{3} + \frac{15 n^{2}}{4} + 1 .
\]
\end{theorem}
...
section of A367015, agreeing on all 13 terms compared.
...
with the entry's own value for every $n>0$ in range, agreeing in all 38 cases
...
\url{https://oeis.org}, 2026. Sequence A367015.
