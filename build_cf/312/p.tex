...
\title{A proof of the conjectured closed form for OEIS A366932}
...
\begin{abstract}
OEIS A366932 carries a conjectured closed form for its terms. We prove it. The entry
...
A(x)\;=\;\sum_{n\ge 0} a(n)\,x^{n}\;=\;- \frac{3 \left(x + 1\right) \left(5 x^{3} + 15 x^{2} - 3 x + 1\right)}{\left(x - 1\right)^{5}} ,
\end{equation}
and it carries the following comment:
\begin{quote}\small
\textbf{Conjecture:} a(n) = 3*(3*n\^{}4/2 - 2*n\^{}3 + 3*n\^{}2/2 + n + 1). [This is now a theorem - \emph{N. J. A. Sloane}, Dec 31 2025]
\end{quote}
As of the ``Last modified'' line on the live entry (2026-04-17, revision 111) the
...
\[
f(n)\;=\;\frac{9 n^{4}}{2} - 6 n^{3} + \frac{9 n^{2}}{2} + 3 n + 3 .
\]
...
\section{The computation for A366932}

By Lemma~\ref{lem:gf} the conjectured formula has generating function
\[
F(x)\;=\;\frac{3 \left(- 5 x^{4} - 20 x^{3} - 12 x^{2} + 2 x - 1\right)}{x^{5} - 5 x^{4} + 10 x^{3} - 10 x^{2} + 5 x - 1} ,
\]
...
For every $n>0$,
\[
a(n)\;=\;\frac{9 n^{4}}{2} - 6 n^{3} + \frac{9 n^{2}}{2} + 3 n + 3 .
\]
\end{theorem}
...
section of A366932, agreeing on all 13 terms compared.
...
with the entry's own value for every $n>0$ in range, agreeing in all 43 cases
...
\url{https://oeis.org}, 2026. Sequence A366932.
